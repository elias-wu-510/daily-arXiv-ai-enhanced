...
TL;DR: Monocular depth estimation for robotic surgery using synthetic priors from Depth Anything V2, adapted with DV-LORA for medical domain, achieving SOTA with 98.1% accuracy and 17% error reduction.


<details>
  <summary>Details</summary>
Motivation: Current self-supervised MDE methods for robotic surgery are fragile in specular, fluid-filled endoscopic environments, suffering from boundary collapse on thin surgical tools and transparent surfaces.

Method: Leverages high-fidelity synthetic priors from Depth Anything V2 architecture, adapts to medical domain using Dynamic Vector Low-Rank Adaptation (DV-LORA) to minimize parameters while bridging synthetic-to-real gap, with physically-stratified evaluation protocol on SCARED dataset.
...
Conclusion: The approach effectively addresses boundary collapse issues in surgical depth estimation by leveraging synthetic priors and efficient domain adaptation, establishing superior performance in challenging endoscopic environments.
...
Method: Computer vision models extract 2D skeletons, gaze vectors, and movement trajectories from training videos, then task-specific metrics measure psychomotor fluency, situational awareness, and team coordination, feeding into an extended Cognitive Task Analysis hierarchy with weighted combination for overall scores.

Result: Demonstrated approach with real-world Enter and Clear the Room drills, providing actionable domain-specific metrics capturing individual and team performance, supporting After Action Reviews with interactive dashboards in Gamemaster and GIFT.

Conclusion: Video-based assessment enables scalable evaluation in STEs, though limitations include tracking difficulties and ground-truth validation; future work includes 3D video analysis expansion.
...
TL;DR: PFP is a neural network that compresses long videos into short contexts while preserving high-frequency details of individual frames at any temporal position, enabling efficient long-term memory for video models.


<details>
  <summary>Details</summary>
Motivation: To enable efficient long-term memory in video models by compressing lengthy video sequences into compact representations without losing important visual details, addressing the challenge of high context costs in autoregressive video modeling.

Method: PFP uses a neural network structure with explicit pretraining objective to preserve high-frequency details of single frames at arbitrary temporal positions. It compresses 20-second videos into ~5k length contexts while maintaining perceptual appearance of random frames.

Result: The framework successfully compresses long videos into short contexts, allows random frame retrieval with perceptually preserved appearances, and enables fine-tuning as memory encoders for autoregressive video models with low context cost and minimal fidelity loss.

Conclusion: PFP provides an effective approach for video compression that preserves visual details while enabling efficient long-term memory in video models, with trade-offs in neural architecture design that require careful consideration.
...
<details>
  <summary>Details</summary>
Motivation: 3D HPE struggles with generalization to diverse real-world scenarios due to reliance on controlled environment data. Existing domain adaptation methods overlook non-stationary target pose datasets, creating a need for lifelong adaptation that handles sequential domain shifts while preserving previous knowledge.

Method: Introduces lifelong domain adaptation task for 3D HPE with a novel GAN framework containing: 3D pose generators, 2D pose discriminator, and 3D pose estimator. Creates a 3D pose generator paradigm integrating pose-aware, temporal-aware, and domain-aware knowledge to enhance current domain adaptation and reduce catastrophic forgetting of previous domains.
...
Conclusion: First to introduce lifelong domain adaptation to 3D HPE, successfully addressing challenges of adapting to current domains while preserving knowledge from previous domains through innovative GAN framework and multi-aware pose generator paradigm.
...
TL;DR: Deep learning pipeline transforms pediatric T1-weighted MRI into synthetic CTs for cranial bone segmentation and suture analysis without radiation exposure.


<details>
  <summary>Details</summary>
Motivation: CT scans provide crucial cranial and sutural information but use ionizing radiation contraindicated in children without significant abnormalities. MRI is radiation-free but cannot visualize cranial sutures or assess bone density and vault growth.

Method: Proposed deep learning pipeline transforms T1-weighted MRIs (children aged 0.2-2 years) into synthetic CTs, predicts cranial bone segmentation, generates suture probability heatmaps, and derives direct suture segmentation using domain-specific variational autoencoders.

Result: Synthetic CTs achieved 99% structural similarity and Frechet inception distance of 1.01 relative to real CTs. Skull segmentation attained 85% average Dice coefficient across seven cranial bones, and sutures achieved 80% Dice. Equivalence confirmed using TOST (p < 0.05).

Conclusion: First pediatric cranial CT synthesis framework enabling suture segmentation from MRI, bridging critical gaps in non-invasive cranial evaluation by generating perceptually indistinguishable synthetic CTs from routine pediatric MRIs.
...
TL;DR: This paper explores scaling laws for training foundation models on massive-scale electro-optical satellite data, finding that performance remains data-limited even at petascale training.


<details>
  <summary>Details</summary>
Motivation: There's a lack of understanding about scaling laws in high-value domains like remote sensing, unlike natural-image domains where internet-scale data enables well-established scaling relationships. This gap hinders development of robust, domain-specialized encoders for remote sensing applications like image captioning, search, and reasoning.

Method: The researchers used over a quadrillion pixels of commercial satellite EO data and the MITRE Federal AI Sandbox to train progressively larger vision transformer (ViT) backbones. They systematically scaled model capacity, training compute, and dataset size while reporting success and failure modes observed at petascale.

Result: Even at petascale training with quadrillion pixels, performance remained consistent with a data-limited regime rather than a model parameter-limited one. The study provides practical insights into scaling behaviors and identifies domain gaps across additional remote sensing modalities.

Conclusion: The findings inform data-collection strategies, compute budgets, and optimization schedules for developing frontier-scale remote sensing foundation models, highlighting that data availability remains the primary constraint even at unprecedented scales.
...
TL;DR: Proposes bi-level optimization framework for automated fetal ultrasound skill assessment without manual ratings, using task performance as skill indicator.


<details>
  <summary>Details</summary>
Motivation: Traditional ultrasound skill assessment relies on subjective expert supervision. Supervised learning methods are limited by predetermined factors. Need for objective, automated skill assessment without manual ratings.

Method: Bi-level optimization framework with clinical task predictor and skill predictor networks optimized jointly. Predicts skills based on how well tasks are performed on acquired ultrasound images without predefined skill ratings.

Result: Validated on real-world clinical ultrasound videos of fetal head scanning. Demonstrates feasibility of predicting ultrasound skills using optimized task performance as skill indicator.

Conclusion: Proposed framework successfully quantifies ultrasound skills through task performance, offering objective assessment without manual ratings or predetermined factors.
...
TL;DR: MGML framework for brain tumor segmentation with incomplete multimodal MRI using meta-guided adaptive fusion and consistency regularization.


<details>
  <summary>Details</summary>
Motivation: Clinical MRI data often has missing modalities, making it challenging to fully utilize available information for lesion segmentation. Need to maximize use of incomplete multimodal data.

Method: Meta-guided multi-modal learning framework with two components: 1) Meta-parameterized adaptive modality fusion (Meta-AMF) that generates adaptive soft-label supervision based on available modalities, and 2) Consistency regularization module to enhance segmentation and robustness.

Result: Superior performance on BraTS2020 and BraTS2023 datasets. On BraTS2020, achieved average Dice scores of 87.55 (WT), 79.36 (TC), and 62.67 (ET) across 15 missing modality combinations.

Conclusion: The MGML framework effectively handles incomplete multimodal MRI data without altering original model architecture, can be integrated into training pipelines, and outperforms state-of-the-art methods.
...
TL;DR: A novel cross-view geo-localization system using DINOv2 backbone with convolution adapter, multi-scale channel reallocation, and MoE-enhanced aggregation for better feature alignment across viewpoint discrepancies.


<details>
  <summary>Details</summary>
Motivation: Cross-view geo-localization faces significant challenges due to viewpoint discrepancies between query and database images, which hinder effective feature aggregation and alignment. Existing methods struggle with these variations.

Method: Three key improvements: 1) DINOv2 backbone with convolution adapter fine-tuning for better adaptation to cross-view variations; 2) Multi-scale channel reallocation module to enhance spatial representation diversity and stability; 3) Improved aggregation module integrating Mixture-of-Experts routing in cross-attention framework for adaptive processing of heterogeneous domains.

Result: Extensive experiments on University-1652 and SUES-200 datasets show competitive performance with fewer trained parameters compared to existing methods.

Conclusion: The proposed CVGL system effectively addresses viewpoint discrepancy challenges through adaptive feature processing and achieves state-of-the-art performance with parameter efficiency.
...
TL;DR: AI framework for automated assessment of microanastomosis surgical skills using object tracking, action segmentation, and proficiency classification.


<details>
  <summary>Details</summary>
Motivation: Microanastomosis requires precise surgical skills but current assessment methods are subjective, time-consuming, and inconsistent. There's a need for automated, objective evaluation systems to improve surgical training.

Method: Three-component AI system: 1) YOLO+DeepSORT for instrument tip tracking, 2) self-similarity matrix for unsupervised action segmentation, 3) supervised classification for skill proficiency assessment.

Result: Achieved 92.4% frame-level action segmentation accuracy and 85.5% skill classification accuracy on 58 expert-rated microanastomosis videos.

Conclusion: The AI framework provides objective, real-time feedback for microsurgical training, enabling standardized, data-driven assessment in high-stakes surgical environments.
...
<details>
  <summary>Details</summary>
Motivation: Traditional dehazing methods rely on atmospheric scattering models, while deep learning approaches using CNNs struggle with long-range dependencies and Transformers require heavy computational resources. There's a need for efficient dehazing methods that can handle both local and long-range dependencies without excessive computational burden.

Method: Proposes DehazeSNN, a U-Net-like architecture integrating Spiking Neural Networks (SNNs) with an Orthogonal Leaky-Integrate-and-Fire Block (OLIFBlock) to enhance cross-channel communication and capture multi-scale image features efficiently.

Result: DehazeSNN achieves competitive performance to state-of-the-art methods on benchmark datasets, producing high-quality haze-free images with smaller model size and fewer multiply-accumulate operations (reduced computational burden).

Conclusion: DehazeSNN successfully addresses the limitations of CNNs and Transformers by combining SNNs with U-Net architecture, offering an efficient and effective solution for image dehazing with superior performance and computational efficiency.
...
TL;DR: T2VAttack: First comprehensive study of adversarial attacks on Text-to-Video diffusion models, revealing vulnerabilities through semantic and temporal attacks using word substitution/insertion.


<details>
  <summary>Details</summary>
Motivation: Despite rapid advancements in Text-to-Video (T2V) diffusion models generating high-quality videos, their vulnerability to adversarial attacks remains largely unexplored, creating a security gap in these increasingly important systems.

Method: Proposes T2VAttack with two attack objectives (semantic for video-text alignment, temporal for temporal dynamics) and two attack methods: T2VAttack-S (identifies critical words and replaces with synonyms via greedy search) and T2VAttack-I (iteratively inserts optimized words with minimal perturbation).
...
<details>
  <summary>Details</summary>
Motivation: Existing view extrapolation methods rely on expensive sensors (LiDAR) or labor-intensive annotations (3D bounding boxes, lane markings), limiting real-world deployment. The authors aim to develop a solution that works with only images and optional camera poses.

Method: 1) Estimate global static and per-frame dynamic point clouds from images/poses, 2) Use deformable 4D Gaussian framework for scene reconstruction, 3) Train video diffusion model on degraded renders from initial 4D Gaussian model, 4) Iteratively refine progressively shifted Gaussian renderings using diffusion model, 5) Incorporate enhanced results back into 4DGS training until reaching target viewpoints.
...
Conclusion: The proposed approach enables practical view extrapolation for autonomous driving using only images and optional camera poses, overcoming limitations of sensor/annotation-dependent methods while achieving superior image quality.
...
TL;DR: Deep learning inpainting model uses satellite time series to predict expected surface appearance, enabling sensitive anomaly detection for surface change monitoring.
...
Result: The method detects earthquake-triggered surface ruptures from the 2023 Turkey-Syria earthquake sequence with higher sensitivity and specificity than traditional methods (temporal median or Reed-Xiaoli anomaly detectors), achieving detection thresholds approximately three times lower than baseline approaches.

Conclusion: Deep learning-based temporal prediction enables unprecedented sensitivity for surface change detection, providing a path toward automated, global-scale monitoring using freely available multi-spectral satellite data.
...
TL;DR: GCA-ResUNet: A lightweight CNN with Grouped Coordinate Attention module for efficient medical image segmentation, outperforming both CNN and Transformer methods on Synapse and ACDC benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current U-Net based CNNs struggle with long-range dependencies in multi-organ segmentation and low-contrast regions, while Transformers require heavy computational resources impractical for clinical deployment. Need an efficient solution that balances accuracy and computational efficiency.

Method: Propose GCA-ResUNet with Grouped Coordinate Attention module that: 1) decouples channel-wise context modeling into multiple groups to handle semantic heterogeneity, 2) integrates direction-aware coordinate encoding to capture structured spatial dependencies along horizontal/vertical axes, 3) maintains CNN backbone efficiency while enhancing global representation.

Result: Achieved Dice scores of 86.11% on Synapse and 92.64% on ACDC benchmarks, outperforming representative CNN and Transformer methods including Swin-UNet and TransUNet. Particularly effective for small anatomical structures with complex boundaries.

Conclusion: GCA-ResUNet provides favorable trade-off between segmentation accuracy and computational efficiency, offering practical and scalable solution for clinical deployment in resource-constrained environments.
...
TL;DR: GASeg improves self-supervised semantic segmentation by using topological information to bridge appearance and geometry, addressing appearance ambiguities through a Differentiable Box-Counting module and Topological Augmentation.


<details>
  <summary>Details</summary>
Motivation: Self-supervised semantic segmentation methods often fail with appearance ambiguities (shadows, glare, local textures) due to over-reliance on unstable appearance-based features. There's a need for more stable representations that combine appearance with geometric information.

Method: Proposes GASeg framework with: 1) Differentiable Box-Counting (DBC) module that quantifies multi-scale topological statistics from geometric and appearance feature streams; 2) Topological Augmentation (TopoAug) - adversarial strategy using morphological operators to simulate real-world ambiguities; 3) GALoss - multi-objective loss enforcing cross-modal alignment between geometric and appearance features.

Result: Achieves state-of-the-art performance on four benchmarks: COCO-Stuff, Cityscapes, and PASCAL, demonstrating effectiveness of bridging geometry and appearance via topological information.
...
TL;DR: Novel pipeline for 3D spacecraft reconstruction using 3D Gaussian Splatting with sun position priors to handle dynamic space lighting for improved photometric accuracy.
...
Method: Uses 3D Gaussian Splatting to represent spacecraft geometry/appearance, but incorporates prior knowledge of Sun's position (estimated by servicer spacecraft) into training pipeline to handle changing illumination and improve photometric quality.

Result: Experimental studies show 3DGS models learn to adapt to rapidly changing space illumination conditions and accurately reflect global shadowing and self-occlusion effects.

Conclusion: Incorporating sun position priors enables effective 3D spacecraft reconstruction from RPO imagery despite dynamic lighting, improving both geometric accuracy and photometric quality for downstream pose estimation.
...
TL;DR: Hilbert-VLM: A two-stage fusion framework that uses Hilbert space-filling curves with SAM2 architecture for 3D medical image segmentation, then generates enhanced prompts for VLM-based disease classification.


<details>
  <summary>Details</summary>
Motivation: VLMs show promise for medical diagnosis but struggle with 3D multimodal medical images due to ineffective integration of complementary information and oversight of subtle pathological features.

Method: Two-stage framework: 1) HilbertMed-SAM module with redesigned SAM2 architecture using Hilbert space-filling curves in Mamba SSM scanning, HMCA mechanism, and scale-aware decoder for lesion segmentation; 2) Prompt enhancement module that unifies segmentation masks with textual attributes to guide VLM classification.

Result: Achieves Dice score of 82.35% on BraTS2021 segmentation benchmark and diagnostic classification accuracy of 78.85%.

Conclusion: Hilbert-VLM demonstrates substantial potential to improve accuracy and reliability of medical VLM-based analysis by effectively handling 3D multimodal medical images.
...
TL;DR: CVC is a flow-based diffusion editing framework that corrects velocity errors in latent trajectories to improve semantic consistency and structural fidelity through dual-perspective velocity conversion and posterior-consistent updates.
...
Method: Conditioned Velocity Correction (CVC) reformulates flow-based editing as distribution transformation with dual-perspective velocity conversion: structure-preserving branch maintains source trajectory consistency, and semantically-guided branch drives controlled deviation toward target distribution. Uses posterior-consistent updates derived from Empirical Bayes Inference and Tweedie correction to compensate for velocity errors.
...
Conclusion: CVC provides a principled framework for flow-based diffusion editing that addresses velocity error accumulation through mathematically grounded correction mechanisms, resulting in improved editing quality and consistency.
...
TL;DR: FitControler is a learnable plug-in for virtual try-on models that enables customized garment fit control by generating fit-aware layouts and injecting them into existing VTON pipelines.


<details>
  <summary>Details</summary>
Motivation: Current VTON methods focus on garment detail rendering but neglect garment fit, which is crucial for holistic style coordination. Garment fit determines how clothing aligns with the body and is fundamental in fashion design.

Method: FitControler features: 1) fit-aware layout generator that redraws body-garment layouts using garment-agnostic representations, and 2) multi-scale fit injector to deliver layout cues for layout-driven VTON. Built Fit4Men dataset with 13,000 body-garment pairs of different fits.

Result: Extensive experiments show FitControler works with various VTON models and achieves accurate fit control. Introduced two fit consistency metrics to assess generation fitness.

Conclusion: FitControler enables fit-aware virtual try-on by addressing layout delineation and rendering challenges, providing a plug-in solution for customized garment fit control in existing VTON systems.
...
TL;DR: Structure-guided 2D Gaussian Splatting improves rate-distortion efficiency by coupling image structure with representation capacity and quantization precision while maintaining fast decoding.


<details>
  <summary>Details</summary>
Motivation: Existing 2DGS-based pipelines allocate representation capacity and parameter precision without considering image structure, limiting their rate-distortion efficiency at low bitrates.

Method: Three key components: 1) Structure-guided initialization assigning Gaussians based on spatial structural priors, 2) Adaptive bitwidth quantization of covariance parameters (higher precision for complex regions), and 3) Geometry-consistent regularization aligning Gaussian orientations with local gradient directions.
...
Conclusion: The proposed structure-guided allocation principle effectively couples image structure with representation capacity and quantization precision, achieving superior rate-distortion efficiency for 2D Gaussian Splatting while preserving fast decoding capabilities.
...
TL;DR: MF-RSVLM is a multi-feature fusion vision-language model for remote sensing that addresses visual forgetting and fine-grained feature extraction challenges through multi-scale representations and recurrent visual injection.


<details>
  <summary>Details</summary>
Motivation: Large VLMs struggle with remote sensing due to domain differences between RS and natural images, failing to extract fine-grained features and suffering from visual forgetting during language processing.

Method: MF-RSVLM learns multi-scale visual representations, combines global context with local details, and uses recurrent visual feature injection to keep language model grounded in visual evidence.
...
Conclusion: The proposed multi-feature fusion approach effectively addresses RS-specific challenges and demonstrates superior performance across multiple remote sensing vision-language tasks.
...
Conclusion: RSAgent's agentic approach with multi-turn reasoning and tool interactions enables more accurate and robust text-guided object segmentation through iterative refinement, outperforming existing one-shot methods.
...
<details>
  <summary>Details</summary>
Motivation: Long-form video editing faces computational challenges due to high costs of joint editing and DDIM inversion across extended sequences, which grow exponentially with video length.
...
Conclusion: PipeFlow enables scalable editing of long videos by dividing them into segments for parallel processing, allowing theoretically infinite video length editing without computational overhead growth.
...
Conclusion: The reinforcement learning approach enables adaptive anisotropic diffusion that effectively handles complex image structures, bridging the gap between traditional diffusion methods and modern learning-based approaches for image denoising.
...
TL;DR: NIRNL is a robust cross-modal retrieval framework that handles noisy labels through neighbor-aware instance refining and cross-modal margin preserving to maximize data utilization while mitigating error propagation.
...
Method: Proposes Neighbor-aware Instance Refining with Noisy Labels (NIRNL) framework with two key components: 1) Cross-modal Margin Preserving (CMP) to adjust relative distances between positive/negative pairs for better discrimination, and 2) Neighbor-aware Instance Refining (NIR) that identifies pure, hard, and noisy subsets through cross-modal neighborhood consensus, then applies tailored optimization strategies for each subset.
...
Conclusion: NIRNL effectively handles noisy labels in cross-modal retrieval by combining margin preserving with neighbor-aware instance refining, achieving superior performance while maximizing data utilization and mitigating error propagation.
...
TL;DR: PCRNet integrates pathology context and expert experience priors into DNNs for improved ocular disease recognition with better interpretability.
...
Method: Developed Pathology Recalibration Module (PRM) to leverage pathology context prior via pixel-wise context compression and pathology distribution concentration operators, and Expert Prior Guidance Adapter (EPGA) to highlight significant pixel-wise regions using expert experience. Combined with Integrated Loss (IL) considering sample-wise loss distributions and training label frequencies.
...
TL;DR: Proposes balanced hierarchical contrastive loss with decoupled learning in DETR to address hierarchical label imbalance and task interference in fine-grained remote sensing detection.


<details>
  <summary>Details</summary>
Motivation: Existing hierarchical contrastive learning methods for fine-grained remote sensing detection suffer from two issues: (1) imbalanced data distribution across label hierarchy causes high-frequency classes to dominate learning, and (2) learning semantic relationships interferes with class-agnostic localization.

Method: Combines balanced hierarchical contrastive loss with decoupled learning strategy in DETR framework. The loss introduces learnable class prototypes and equilibrates gradients from different classes at each hierarchical level. The decoupled strategy separates DETR's object queries into classification and localization sets for task-specific feature extraction.

Result: Outperforms state-of-the-art approaches on three fine-grained datasets with hierarchical annotations.

Conclusion: The proposed balanced hierarchical contrastive loss with decoupled learning effectively addresses hierarchical label imbalance and task interference, improving fine-grained detection performance in remote sensing.
...
TL;DR: RainFusion2.0 proposes an online adaptive, hardware-efficient sparse attention mechanism for Diffusion Transformers that achieves 80% sparsity and 1.5-1.8x speedup without quality loss, working across diverse hardware platforms.


<details>
  <summary>Details</summary>
Motivation: DiT models have extremely high computational costs due to attention mechanisms, limiting practical applications. Existing sparse attention methods have overhead issues and lack hardware generality, being mostly GPU-specific despite increasing adoption of diverse hardware like ASICs.

Method: Uses block-wise mean values as representative tokens for sparse mask prediction, implements spatiotemporal-aware token permutation, and introduces a first-frame sink mechanism specifically for video generation scenarios.
...
Conclusion: RainFusion2.0 successfully addresses computational cost and hardware generality limitations of existing sparse attention methods, providing an efficient solution for accelerating both video and image generative models across multiple hardware platforms.
...
TL;DR: D²VLM is a video-language model framework that factorizes temporal grounding and textual response learning using evidence tokens and factorized preference optimization to improve event-level video understanding.
...
Method: Proposes D²VLM framework that decouples learning of temporal grounding and textual response while maintaining their dependency. Uses "grounding then answering with evidence referencing" paradigm with evidence tokens for event-level visual semantic capture. Introduces factorized preference optimization (FPO) algorithm that explicitly incorporates probabilistic temporal grounding modeling into optimization objective for both tasks. Also constructs synthetic dataset for factorized preference learning with explicit temporal grounding.

Result: Experiments on various tasks demonstrate clear advantages of the approach. The framework shows improved performance in temporal grounding and textual response generation compared to existing methods.

Conclusion: Factorized learning approach with explicit temporal grounding modeling significantly improves video-language models' event-level understanding capabilities. The D²VLM framework with FPO algorithm and evidence tokens provides an effective solution for hierarchical video understanding tasks.
...
TL;DR: The paper proposes Latent Motion Reasoning (LMR), a two-stage "Think-then-Act" approach for Text-to-Motion generation that addresses the Semantic-Kinematic Impedance Mismatch by separating motion planning from execution.


<details>
  <summary>Details</summary>
Motivation: Current T2M approaches treat generation as direct translation from language to motion, which faces a fundamental bottleneck called Semantic-Kinematic Impedance Mismatch - the difficulty of grounding semantically dense language into kinematically dense motion data in a single step.

Method: LMR reformulates generation as a two-stage process: 1) Reasoning stage using a compressed, semantically rich latent space for planning global trajectory, and 2) Execution stage using a high-frequency latent space for physical fidelity. Uses a Dual-Granularity Tokenizer to disentangle motion into two distinct manifolds.

Result: LMR shows non-trivial improvements in both semantic alignment and physical plausibility when implemented on two representative baselines (T2M-GPT and MotionStreamer), demonstrating that motion-aligned concept spaces are better than natural language for motion planning.

Conclusion: The optimal substrate for motion planning is not natural language but a learned, motion-aligned concept space. The two-stage Think-then-Act approach effectively bridges the gap between language and physics in motion generation.
...
Conclusion: The framework demonstrates strong practical implications for autonomous driving safety assessment by creating more realistic and effective adversarial attacks for testing and improving depth estimation robustness.
...
TL;DR: LLM-based architecture generation for computer vision with systematic prompt engineering (FSAP) and fast deduplication (Whitespace-Normalized Hash Validation), achieving 100x speedup over AST parsing.
...
Method: 1) Few-Shot Architecture Prompting (FSAP): systematic study of supporting examples (n=1-6) for LLM-based architecture generation. 2) Whitespace-Normalized Hash Validation: lightweight deduplication method (<1ms) that provides 100x speedup over AST parsing. 3) Dataset-balanced evaluation methodology for comparing architectures across heterogeneous vision tasks.

Result: Found n=3 examples best balances architectural diversity and context focus for vision tasks. Generated 1,900 unique architectures across seven computer vision benchmarks (MNIST, CIFAR-10, CIFAR-100, CelebA, ImageNette, SVHN, Places365). Deduplication method prevents redundant training of duplicate architectures.
...
<details>
  <summary>Details</summary>
Motivation: Existing RLHF methods for text-to-image diffusion models suffer from Preference Mode Collapse - a form of reward hacking where models converge on narrow, high-scoring outputs (monolithic styles, overexposure), severely degrading generative diversity despite high automated reward scores.

Method: Proposes Directional Decoupling Alignment (D²-Align): 1) Learns a directional correction within the reward model's embedding space while keeping the model frozen, 2) Applies this correction to the reward signal during optimization to prevent mode collapse and maintain diversity.
...
TL;DR: IMDD-1M is a large-scale industrial multimodal defect dataset with 1M image-text pairs covering 60+ materials and 400+ defect types, enabling various vision-language tasks. A diffusion-based foundation model trained on this dataset achieves comparable performance to specialized models with only 5% of task-specific data.


<details>
  <summary>Details</summary>
Motivation: There is a need for large-scale multimodal datasets specifically designed for industrial defect detection and manufacturing quality inspection to advance multimodal learning in industrial scenarios.

Method: Created IMDD-1M dataset with 1M aligned image-text pairs covering diverse materials and defect types. Trained a diffusion-based vision-language foundation model from scratch on this dataset, designed for efficient adaptation to specialized domains through lightweight fine-tuning.

Result: The foundation model achieves comparable performance to dedicated expert models while requiring less than 5% of task-specific data, demonstrating data-efficient adaptation capabilities for industrial inspection and generation tasks.

Conclusion: IMDD-1M enables scalable, domain-adaptive, and knowledge-grounded manufacturing intelligence through data-efficient foundation model adaptation, paving the way for advanced industrial inspection systems.
...
Result: BSD consistently improves test accuracy (+1.4% for ResNet-50 on CIFAR-100) and reduces Expected Calibration Error (-40% ResNet-50, CIFAR-100) across various architectures and datasets. Also improves robustness against data corruptions, perturbations, and label noise.

Conclusion: BSD provides a principled approach to self-distillation that eliminates hard target dependency, achieving superior accuracy, calibration, and robustness compared to existing methods, with state-of-the-art robustness under label noise when combined with contrastive loss.
...
TL;DR: DiffThinker introduces a diffusion-based generative multimodal reasoning framework that treats reasoning as native image-to-image generation, outperforming text-centric MLLMs on vision-centric tasks.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Large Language Models (MLLMs) have text-centric reasoning processes that perform poorly on complex long-horizon, vision-centric tasks, creating a need for more native visual reasoning approaches.

Method: DiffThinker establishes a Generative Multimodal Reasoning paradigm using diffusion-based framework that reformulates multimodal reasoning as native generative image-to-image tasks, focusing on logical consistency and spatial precision.
...
Conclusion: Generative multimodal reasoning via diffusion-based frameworks like DiffThinker represents a promising approach for vision-centric reasoning, offering efficiency, controllability, native parallelism, and collaboration advantages over text-centric MLLMs.
...
TL;DR: DGC is a memory-efficient HSI segmentation framework that learns global clustering from local patches without pre-training, achieving fast training on consumer hardware but suffering from optimization instability due to loss balancing issues.
...
Method: Deep Global Clustering (DGC) framework operates on small patches with overlapping regions to enforce consistency, learning global clustering structure from local patch observations without pre-training, enabling training in under 30 minutes on consumer hardware with constant memory usage.

Result: On a leaf disease dataset, DGC achieves background-tissue separation with mean IoU 0.925 and demonstrates unsupervised disease detection through navigable semantic granularity, but suffers from optimization instability due to multi-objective loss balancing causing cluster over-merging in feature space.

Conclusion: The design philosophy has merit as intellectual scaffolding for memory-efficient HSI segmentation, but stable implementation requires principled approaches to dynamic loss balancing to address optimization instability issues.
...
TL;DR: Internal Guidance (IG) improves diffusion models by adding auxiliary supervision during training and extrapolating intermediate outputs during sampling, achieving state-of-the-art FID scores on ImageNet.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion models struggle with low-probability areas due to insufficient training data coverage, while existing guidance methods like CFG cause over-simplified/distorted samples or require complex degradation strategies and extra training.
...
Result: Significant improvements in training efficiency and generation quality: SiT-XL/2+IG achieves FID=5.31 (80 epochs) and 1.75 (800 epochs) on ImageNet 256x256; LightningDiT-XL/1+IG achieves FID=1.34, and combined with CFG reaches state-of-the-art FID=1.19.

Conclusion: Internal Guidance is a simple yet effective strategy that enhances diffusion models by leveraging internal layer supervision and extrapolation, outperforming existing guidance methods while being more efficient and achieving state-of-the-art results.
...
<details>
  <summary>Details</summary>
Motivation: Potato yield estimation using RGB-D cameras on harvesters faces challenges due to incomplete point clouds from self-occlusion, leading to systematic underestimation of tuber weight. Current methods that attempt full 3D reconstruction are inefficient for high-throughput agricultural applications.

Method: PointRAFT directly predicts continuous 3D shape properties (like tuber weight) from partial point clouds without reconstructing full geometry. Key innovation is an object height embedding that incorporates tuber height as an additional geometric cue to improve weight prediction under practical harvesting conditions.

Result: On a test set of 5,254 point clouds from 172 tubers, PointRAFT achieved MAE of 12.0g and RMSE of 17.2g, significantly outperforming linear regression and PointNet++ baselines. With 6.3ms inference time per point cloud, it supports processing rates up to 150 tubers/second, meeting commercial harvester requirements.

Conclusion: PointRAFT provides an efficient, high-throughput solution for potato weight estimation from partial point clouds, with potential applications in 3D phenotyping and robotic perception. The method is publicly available with code, weights, and dataset.
...
TL;DR: CorGi is a training-free framework that accelerates DiT inference by caching and reusing low-contribution transformer blocks across denoising steps, achieving up to 2.0x speedup while maintaining generation quality.
...
Method: CorGi selectively reuses outputs of low-contribution transformer blocks across denoising steps using interval caching. CorGi+ extends this for text-to-image tasks by using cross-attention maps to identify salient tokens and applying partial attention updates to protect object details.

Result: The framework achieves up to 2.0x speedup on average on state-of-the-art DiT models while preserving high generation quality.

Conclusion: CorGi and CorGi+ effectively reduce redundant computation in DiT inference through contribution-guided caching and partial attention updates, enabling significant acceleration without compromising visual quality.
...
TL;DR: Proposes a progressive GAN with weighted synthetic data combination and meta-heuristic optimization for COVID-19 classification from imbalanced chest X-ray images.


<details>
  <summary>Details</summary>
Motivation: Addresses the challenge of imbalanced medical image data, especially during pandemics like COVID-19, where dataset imbalance hinders accurate AI/ML classification performance.
...
Result: Achieves 95.5% accuracy for 4-class and 98.5% accuracy for 2-class imbalanced COVID-19 chest X-ray classification, outperforming existing methods.

Conclusion: The proposed model effectively handles imbalanced medical image classification during pandemics through synthetic data generation and optimization techniques.
...
TL;DR: ARM is a lightweight learnable module that refines CLIP's internal features for open-vocabulary semantic segmentation, enabling "train once, use anywhere" capability across training-free frameworks.


<details>
  <summary>Details</summary>
Motivation: Existing training-free OVSS methods either rely on expensive external models like SAM/DINO or use suboptimal static heuristics on CLIP features, which lack pixel-level details needed for segmentation.

Method: ARM uses a semantically-guided cross-attention block that fuses hierarchical CLIP features: deep features (K,V) select and refine detail-rich shallow features (Q), followed by self-attention. It's trained once on COCO-Stuff then used as a plug-and-play post-processor.

Result: ARM consistently boosts baseline performance across multiple benchmarks with negligible inference overhead, establishing an efficient paradigm for training-free OVSS.

Conclusion: ARM effectively unlocks CLIP's internal potential for segmentation through adaptive feature fusion, offering a computationally efficient solution that outperforms static-fusion methods and avoids costly external model dependencies.
...
Method: Mirage builds on text-to-video diffusion prior for temporal consistency. It injects temporally agnostic latents from a pretrained 2D encoder into the 3D decoder to restore detail while preserving causal structures. A two-stage data alignment strategy combines coarse 3D alignment and fine 2D refinement to address distribution mismatch between scene objects and inserted assets.

Result: Extensive experiments show Mirage achieves high realism and temporal consistency across diverse editing scenarios. It also generalizes to other video-to-video translation tasks, serving as a reliable baseline for future research.

Conclusion: Mirage provides an effective solution for photorealistic and coherent asset editing in driving scenes, addressing key challenges in visual fidelity, temporal coherence, and distribution alignment for autonomous driving data augmentation.
...
TL;DR: MotivNet is a generalizable facial emotion recognition model using Meta-Sapiens backbone that achieves competitive cross-dataset performance without cross-domain training, making FER more viable for real-world applications.


<details>
  <summary>Details</summary>
Motivation: Current FER models have weak generalization on diverse real-world data, requiring cross-domain training which is impractical for real-world applications. There's a need for models that generalize well without such training constraints.

Method: Uses Meta-Sapiens (human vision foundational model with Masked Autoencoder pretraining) as backbone. MotivNet is added as a downstream task to Sapiens. Defines three evaluation criteria: benchmark performance, model similarity, and data similarity.

Result: MotivNet achieves competitive performance across datasets without cross-domain training. It meets all three evaluation criteria and can be benchmarked against existing SOTA models, validating it as a viable Sapiens downstream task.

Conclusion: MotivNet demonstrates strong generalization across domains without cross-domain training, making facial emotion recognition more practical for real-world applications and validating it as a suitable downstream task for the Sapiens foundation model.
...
TL;DR: MambaSeg: A dual-branch semantic segmentation framework using parallel Mamba encoders for RGB and event data fusion with spatial-temporal interaction modules for improved cross-modal alignment and efficiency.


<details>
  <summary>Details</summary>
Motivation: RGB-based segmentation degrades under challenging conditions (fast motion, low-light, high dynamic range), while event cameras alone lack color/texture. Existing multimodal fusion approaches are computationally expensive and neglect temporal dynamics of event streams.

Method: Proposes MambaSeg with parallel Mamba encoders for RGB and event streams. Introduces Dual-Dimensional Interaction Module (DDIM) with Cross-Spatial Interaction Module (CSIM) and Cross-Temporal Interaction Module (CTIM) for fine-grained spatial-temporal fusion to reduce cross-modal ambiguity.
...
TL;DR: PGMP framework for dental CBCT metal artifact reduction uses physics-based synthetic training, deterministic manifold projection, and medical foundation model priors to achieve efficient, reliable artifact removal in single forward pass.


<details>
  <summary>Details</summary>
Motivation: Current deep learning MAR methods have limitations: supervised methods suffer from spectral blurring due to "regression-to-the-mean", unsupervised methods risk structural hallucinations, and DDPMs are too slow for clinical use due to iterative sampling.

Method: Three components: 1) AAPS pipeline synthesizes high-fidelity training pairs via Monte Carlo spectral modeling and patient-specific digital twins; 2) DMP-Former adapts Direct x-Prediction paradigm for deterministic manifold projection in single forward pass; 3) SSA module uses medical foundation model priors (MedDINOv3) for clinical plausibility.

Result: PGMP outperforms state-of-the-art methods on synthetic and multi-center clinical datasets, setting new benchmarks in efficiency and diagnostic reliability for unseen anatomy.

Conclusion: PGMP framework successfully addresses limitations of existing MAR methods by combining physics-based synthesis, deterministic restoration, and medical priors, enabling efficient and clinically reliable metal artifact reduction in dental CBCT.
...
Method: DualityForge uses controllable diffusion-based video editing to transform real videos into counterfactual scenarios, generating QA pairs with structured contextual information. Duality-Normalized Advantage Training (DNA-Train) applies two-stage SFT-RL with pair-wise ℓ₁ advantage normalization for stable optimization.

Result: 24.0% relative improvement over Qwen2.5-VL-7B baseline on DualityVidQA-Test, with significant gains on both hallucination and general-purpose benchmarks, showing strong generalization.

Conclusion: The framework effectively reduces MLLM hallucinations on counterfactual videos through synthetic data generation and contrastive training, with open-source release planned.
...
Method: 1) Lift each input image to dense 3D point representation in unified coordinate system with global cross-view fusion and confidence metrics; 2) Establish unified 3D projection center and use equidistant cylindrical projection to map fused data to panoramic manifold; 3) Perform hole filling in canvas domain to address unknown regions from viewpoint transitions.
...
Conclusion: The framework successfully re-conceptualizes stitching from 2D warping to 3D consistency paradigm, offering flexibility to incorporate various 3D lifting and completion modules for improved panoramic quality in complex real-world scenes.
...
TL;DR: EndoRare is a one-shot generative framework that synthesizes diverse, realistic rare gastrointestinal lesion images from a single reference, enhancing both AI model performance and novice clinician training.
...
Result: Validated across four rare pathologies; synthetic images were clinically plausible, enhanced AI classifiers (improved true positive rate at low false-positive rates), and improved novice endoscopists' recall by 0.400 and precision by 0.267 in blinded studies.
...
TL;DR: Virtual-Eyes CT quality-control pipeline improves generalist foundation models for lung cancer screening but can degrade specialist models adapted to raw clinical data.


<details>
  <summary>Details</summary>
Motivation: Robust preprocessing is rarely quantified in deep-learning pipelines for low-dose CT lung cancer screening, and there's a need to measure how quality control affects different types of models.
...
TL;DR: UniAct is a two-stage framework that enables humanoid robots to follow diverse multimodal instructions (language, music, trajectories) with sub-500ms latency by integrating a fine-tuned MLLM with a causal streaming pipeline.


<details>
  <summary>Details</summary>
Motivation: Bridging high-level multimodal perception with whole-body execution remains a bottleneck in humanoid robotics. Existing methods struggle to translate heterogeneous instructions into stable, real-time actions, limiting the development of versatile agents with human-level flexibility.

Method: Two-stage framework: 1) Fine-tuned Multimodal Large Language Model (MLLM) processes diverse instructions, 2) Causal streaming pipeline generates motions. Uses FSQ (Finite Scalar Quantization) to unify inputs through a shared discrete codebook, ensuring cross-modal alignment while constraining motions to physically grounded manifolds.

Result: Achieves sub-500ms latency for humanoid motion execution. Shows 19% improvement in zero-shot tracking of imperfect reference motions. Validated on UniMoCap, a 20-hour humanoid motion benchmark, demonstrating robust generalization across diverse real-world scenarios.

Conclusion: UniAct represents a critical step toward responsive, general-purpose humanoid assistants capable of seamless interaction through unified perception and control, addressing the long-standing challenge of bridging multimodal instructions with real-time whole-body execution.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for Ego-RVOS struggle due to ambiguities in egocentric videos and biases in training data, learning spurious correlations from skewed object-action pairings and visual confounding factors like rapid motion and occlusions.
...
Result: Extensive experiments show CERES achieves state-of-the-art performance on Ego-RVOS benchmarks, demonstrating the effectiveness of causal reasoning for egocentric video understanding.

Conclusion: CERES highlights the potential of applying causal reasoning to build more reliable models for broader egocentric video understanding by addressing fundamental biases and confounding factors in the domain.
...
TL;DR: SenseNova-MARS is a multimodal agentic reasoning framework that enables VLMs to interleave visual reasoning with tool use via reinforcement learning, achieving SOTA on search-oriented benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current VLMs are limited to text-oriented reasoning or isolated tool use, lacking the ability to seamlessly interleave dynamic tool manipulation with continuous reasoning in knowledge-intensive, visually complex scenarios that require coordinated external tools like search and image cropping.

Method: SenseNova-MARS framework integrates image search, text search, and image crop tools dynamically. Uses reinforcement learning with Batch-Normalized Group Sequence Policy Optimization (BN-GSPO) algorithm to improve training stability and tool invocation capabilities. Introduces HR-MMSearch benchmark for evaluation.

Result: Achieves state-of-the-art performance: SenseNova-MARS-8B scores 67.84 on MMSearch and 41.64 on HR-MMSearch, surpassing proprietary models like Gemini-3-Flash and GPT-5.

Conclusion: SenseNova-MARS represents a promising step toward agentic VLMs by providing effective and robust tool-use capabilities. The authors will release all code, models, and datasets to facilitate further research.
...
Method: Proposes LVLDrive framework that adds LiDAR point cloud as an extra input modality. Uses Gradual Fusion Q-Former to incrementally inject LiDAR features without disrupting pre-trained VLM knowledge. Develops spatial-aware question-answering (SA-QA) dataset to teach advanced 3D perception and reasoning.
...
TL;DR: The paper proposes elementary information mechanics as a new model for understanding convolutional filtering with rectification, drawing analogies to special relativity and quantum mechanics by analyzing kernels through even/odd decomposition and DCT spectral analysis.
...
Method: Decomposes convolutional kernels into orthogonal even and odd components, analyzes their effects on image content (even: isotropic diffusion, odd: directional displacement), and examines these properties in the spectral domain using discrete cosine transform (DCT), focusing on low-frequency bases like DC and gradient components.

Result: Shows that even kernels preserve center of mass (analogous to rest/potential energy), odd kernels displace center of mass (analogous to kinetic energy), and information displacement speed is linearly related to odd vs total kernel energy ratio. Small filters are dominated by low-frequency DCT bases.

Conclusion: This work establishes the first demonstrated link between generic CNN information processing and the energy-momentum relation from modern physics, providing a novel "elementary information mechanics" framework for understanding convolutional filtering mechanics.
...
TL;DR: DermaVQA-DAS extends DermaVQA dataset with two tasks: closed QA and lesion segmentation, using expert-developed Dermatology Assessment Schema (DAS) with 63 structured questions in English/Chinese.


<details>
  <summary>Details</summary>
Motivation: Existing dermatology datasets focus on dermatoscopic images without patient queries and clinical context, limiting applicability to patient-centered care. Need for datasets that incorporate patient perspectives and clinical assessment frameworks.

Method: Introduces Dermatology Assessment Schema (DAS) with 36 high-level and 27 fine-grained assessment questions. Extends DermaVQA dataset to support closed QA and segmentation tasks. Benchmarks state-of-the-art multimodal models with various prompting strategies.

Result: For segmentation: Prompt design impacts performance - default prompt best under Mean-of-Max/Mean-of-Mean, augmented prompt (patient query title+content) best under majority-vote (Jaccard: 0.395, Dice: 0.566). For QA: Models perform well (0.729-0.798 accuracy), o3 leads (0.798), GPT-4.1 close (0.796), Gemini-1.5-Pro competitive (0.783).

Conclusion: DermaVQA-DAS addresses limitations of existing dermatology datasets by incorporating patient queries and clinical context through DAS framework. Public release of dataset, schema, and protocols aims to accelerate patient-centered dermatological vision-language research.
...
TL;DR: This paper presents a comprehensive framework and taxonomy for multi-modal pre-training to achieve Spatial Intelligence in autonomous systems, analyzing sensor integration, learning strategies, and proposing a roadmap for general-purpose foundation models.
...
Result: The paper identifies the core set of techniques driving progress toward multi-modal Spatial Intelligence, evaluates the role of platform-specific datasets in enabling advancements, and provides a taxonomy for pre-training paradigms applicable to advanced tasks like 3D object detection and semantic occupancy prediction.
...
TL;DR: RedunCut is a dynamic model size selection system for live video analytics that reduces compute costs by 14-62% at fixed accuracy through better sampling decisions and accuracy prediction.


<details>
  <summary>Details</summary>
Motivation: Live video analytics (LVA) runs continuously across massive camera fleets but faces high inference costs with modern vision models. Existing dynamic model size selection (DMSS) methods fail to generalize to diverse workloads, particularly mobile videos and lower accuracy targets, due to inefficient sampling and inaccurate accuracy prediction.
...
Conclusion: RedunCut successfully addresses the limitations of prior DMSS systems by optimizing sampling decisions and improving accuracy prediction, making it a robust solution for cost-effective live video analytics across diverse workloads.
...
TL;DR: DyStream: A real-time flow matching-based autoregressive model for generating dyadic talking head video with ultra-low latency (<100ms) using causal encoding with short lookahead.


<details>
  <summary>Details</summary>
Motivation: Existing chunk-based methods for dyadic talking head video generation require full non-causal context windows, introducing significant delays that prevent the immediate, non-verbal feedback required for realistic listener responses.

Method: 1) Stream-friendly autoregressive framework with flow-matching heads for probabilistic modeling. 2) Causal encoder enhanced by a lookahead module to incorporate short future context (e.g., 60ms) to improve quality while maintaining low latency.
...
Conclusion: DyStream enables real-time dyadic talking head video generation with ultra-low latency, outperforming alternative causal strategies and providing the immediate feedback needed for realistic listener responses.
...
TL;DR: AI-driven framework using video transformers and motion tracking for automated assessment of microanastomosis surgical skills, achieving 93.62% action segmentation accuracy and 76% classification accuracy matching expert evaluations.


<details>
  <summary>Details</summary>
Motivation: Current surgical skill assessment methods rely on expert supervision which is subjective, time-consuming, and impractical for scaling in low-resource settings, creating a need for automated, objective evaluation systems.
...
Result: Achieved 87.7% frame-level action segmentation accuracy (increased to 93.62% with post-processing) and 76% average classification accuracy in replicating expert assessments across five skill aspects using a dataset of 58 expert-annotated videos.

Conclusion: The AI framework provides objective, consistent, and interpretable feedback for surgical skill assessment, enabling standardized, data-driven training and evaluation that can scale effectively, especially in resource-limited settings.
...
TL;DR: ViT encoder representations show compositionality when using DWT primitives, with composed representations approximating original image representations.


<details>
  <summary>Details</summary>
Motivation: To understand how Vision Transformers structure information by investigating compositionality in their learned representations, analogous to language transformer analysis.

Method: Introduced framework using Discrete Wavelet Transform (DWT) to obtain input-dependent primitives, then tested compositionality by examining if composed representations can reproduce original image representations.
...
Conclusion: ViT encoders exhibit compositional representation structure, offering new insights into how they organize visual information, similar to language transformers.
...
TL;DR: A learned compression framework for multispectral solar imagery using graph-based modules to model inter-band relationships and spatial redundancy, achieving better spectral fidelity and reconstruction quality than baselines.
...
Conclusion: The proposed learned compression framework effectively addresses the challenges of multispectral solar imagery compression by leveraging graph-based modeling of spectral and spatial relationships, achieving superior performance in preserving both spectral fidelity and spatial details.
...
TL;DR: The paper introduces F2IDiff, a Feature-to-Image Diffusion Foundation Model for smartphone SISR that uses DINOv2 features for stricter, hallucination-free conditioning instead of text features.


<details>
  <summary>Details</summary>
Motivation: Current T2IDiff models for SISR cause undesirable hallucinations in smartphone photography where LR images have high fidelity and need minimal, hallucination-free generation. Text features are too high-level for subtle textures and can't accurately describe small patches needed for high-resolution smartphone images.

Method: Proposes F2IDiff (Feature-to-Image Diffusion Foundation Model) using DINOv2 features for conditioning instead of text. Lower-level features provide stricter conditioning while being rich descriptors of small patches, enabling hallucination-free super-resolution for high-fidelity smartphone images.

Result: The method addresses limitations of T2IDiff models by providing more precise conditioning that prevents hallucinations while maintaining generation quality for smartphone SISR applications.

Conclusion: F2IDiff with DINOv2 feature conditioning offers a better solution for smartphone SISR by providing stricter, hallucination-free generation that matches the high-fidelity requirements of consumer photography.
...
TL;DR: AI pipeline combining YOLO object detection with GPT-4 generates radiology reports from chest X-rays, achieving strong clinical accuracy but with stylistic differences from human radiologists.


<details>
  <summary>Details</summary>
Motivation: Current AI systems in medical imaging only output structured predictions, requiring radiologists to translate these into narrative reports. LLMs like GPT-4 offer potential to automate this narrative generation, bridging the gap between AI detection and clinical reporting.

Method: Integrated pipeline using YOLOv5 and YOLOv8 for anomaly detection in chest X-rays, followed by GPT-4 to generate natural-language reports from the structured findings (bounding boxes and class labels). Comparison of YOLO models on detection accuracy, inference latency, and generated text quality measured by cosine similarity to ground-truth reports.

Result: Strong semantic similarity between AI and human reports. YOLOv8 showed better detection performance. GPT-4 excelled in clarity (4.88/5) but scored lower for natural writing flow (2.81/5), indicating clinical accuracy but stylistic distinguishability from radiologist-authored text.

Conclusion: Current AI systems can generate clinically accurate radiology reports but remain stylistically different from human radiologists. The pipeline demonstrates feasibility but highlights the need for improved natural language generation to match radiologist writing style.
...
<details>
  <summary>Details</summary>
Motivation: Traditional video codecs lack native support for machine learning latent representations, limiting integration into deep learning pipelines. There's a need for efficient compression suitable for bandwidth-sensitive scenarios like edge devices and CDNs.

Method: Extends VQ-VAE-2 to spatiotemporal setting with two-level hierarchical latent structure using 3D residual convolutions. Incorporates perceptual loss from pre-trained VGG16. Lightweight (18.5M parameters) optimized for 64x64 video clips.

Result: Achieves 25.96 dB PSNR and 0.8375 SSIM on UCF101 test set. Outperforms single-scale baseline by 1.41 dB PSNR and 0.0248 SSIM on validation.

Conclusion: The framework is well-suited for scalable video compression in bandwidth-sensitive scenarios including real-time streaming, mobile video analytics, and CDN storage optimization.
...
TL;DR: PhyGDPO introduces a physics-aware video generation framework with PhyAugPipe for data collection and PhyGDPO for optimization, achieving state-of-the-art physical consistency in text-to-video generation.


<details>
  <summary>Details</summary>
Motivation: Current text-to-video generation methods produce visually appealing results but often fail to follow physical laws, struggling with physical reasoning and generalization beyond simple simulated environments due to limited training data with rich physics interactions.

Method: 1) PhyAugPipe: Uses vision-language models with chain-of-thought reasoning to collect PhyVidGen-135K dataset. 2) PhyGDPO: Physics-aware Groupwise Direct Preference Optimization framework with Physics-Guided Rewarding (PGR) scheme using VLM-based physics rewards, and LoRA-Switch Reference (LoRA-SR) for efficient training without memory-heavy reference duplication.

Result: The method significantly outperforms state-of-the-art open-source methods on PhyGenBench and VideoPhy2 benchmarks, demonstrating superior physical consistency in generated videos.

Conclusion: PhyGDPO provides an effective solution for physics-aware video generation through data augmentation and principled optimization, addressing the challenge of physical consistency in text-to-video synthesis.
...
TL;DR: Novel second-order optimization algorithm for deep learning that combines OCP method with Hessian diagonal approximation, achieving faster convergence and better robustness on visual localization tasks.


<details>
  <summary>Details</summary>
Motivation: Address large-scale optimization problems in deep learning by developing efficient second-order methods that overcome computational challenges of traditional Hessian-based approaches.

Method: Proposes a novel second-order optimization algorithm that incorporates the OCP method and appropriately approximates the diagonal elements of the Hessian matrix for computational efficiency.
...
Conclusion: The proposed second-order optimization framework effectively addresses large-scale deep learning optimization challenges, offering a practical solution that balances computational efficiency with optimization performance for visual localization applications.
...
TL;DR: RGBT-Ground: First large-scale RGB-Thermal visual grounding benchmark for complex real-world scenarios, with RGBT-VGNet baseline outperforming adapted methods.


<details>
  <summary>Details</summary>
Motivation: Existing VG benchmarks from clean environments (like COCO) lack scene diversity and fail to reflect real-world complexities (illumination, weather changes), limiting evaluation of model robustness for safety-critical applications.

Method: 1) Create RGBT-Ground benchmark with aligned RGB-Thermal image pairs, referring expressions, bounding boxes, and fine-grained scene/environment/object annotations. 2) Propose RGBT-VGNet - a unified framework supporting uni-modal (RGB/TIR) and multi-modal (RGB-TIR) inputs with effective fusion of complementary visual modalities.

Result: RGBT-VGNet significantly outperforms adapted existing methods, especially in challenging nighttime and long-distance scenarios, demonstrating robust grounding in complex real-world conditions.

Conclusion: RGBT-Ground enables comprehensive evaluation of visual grounding robustness, and RGBT-VGNet provides an effective baseline for multi-modal fusion, promoting future research on robust VG in complex real-world environments.
...
TL;DR: DARFT framework improves CDVQA by targeting decision-ambiguous samples through reinforcement fine-tuning, reducing ambiguity between correct answers and strong distractors.


<details>
  <summary>Details</summary>
Motivation: Current CDVQA models often fail due to decision ambiguity where models assign similar confidence to correct answers and strong distractors, rather than clearly incorrect predictions.

Method: DARFT: Decision-Ambiguity-guided Reinforcement Fine-Tuning framework that mines Decision-Ambiguous Samples (DAS) using SFT-trained reference policy, then applies group-relative policy optimization on the mined subset using multi-sample decoding and intra-group relative advantages.
...
Conclusion: Explicitly optimizing decision-ambiguous samples is crucial for improving CDVQA model performance, and DARFT effectively addresses this challenge without additional supervision.
...
<details>
  <summary>Details</summary>
Motivation: Existing slice discovery methods are limited to image classification and fail at multi-instance tasks (detection, segmentation, pose estimation). Current approaches lack fine-grained reasoning for complex visual relationships in corner cases, and benchmarks are biased toward classification with artificial ground truth.

Method: SliceLens leverages LLMs and VLMs to generate and verify diverse failure hypotheses through grounded visual reasoning, enabling reliable identification of fine-grained and interpretable error slices.

Result: SliceLens achieves state-of-the-art performance, improving Precision@10 by 0.42 (0.73 vs. 0.31) on the new FeSD benchmark, and identifies interpretable slices that facilitate actionable model improvements validated through model repair experiments.

Conclusion: The proposed framework effectively addresses limitations of existing slice discovery methods by enabling fine-grained error analysis for instance-level vision tasks through LLM/VLM-powered hypothesis generation and verification.
...
<details>
  <summary>Details</summary>
Motivation: Natural disasters cause severe threats to lives and economic losses, but existing 3D semantic segmentation models lack datasets specifically designed for post-disaster environments, creating a gap in post-disaster assessment capabilities.
...
Result: The evaluation exposed significant limitations in existing 3D semantic segmentation methods for disaster-stricken regions, showing that current SOTA models perform poorly on post-disaster environments.
...
TL;DR: This paper introduces MoniRefer, the first large-scale dataset for 3D visual grounding in outdoor monitoring scenarios, and proposes Moni3DVG, an end-to-end method that fuses image appearance with point cloud geometry for accurate object localization.


<details>
  <summary>Details</summary>
Motivation: Existing 3D visual grounding research focuses on indoor and outdoor driving scenes, but roadside infrastructure monitoring scenarios remain unexplored due to lack of paired point cloud-text data from roadside sensors. Infrastructure-level understanding of traffic scenes is critical for intelligent transportation systems.

Method: The authors propose Moni3DVG, an end-to-end method that leverages both appearance information from images and geometry/optical information from point clouds for multi-modal feature learning and 3D object localization. They also create MoniRefer dataset with 136,018 objects and 411,128 natural language expressions from real-world traffic intersections.

Result: Extensive experiments and ablation studies demonstrate the superiority and effectiveness of Moni3DVG on the proposed benchmarks. The dataset contains high-quality manually verified linguistic descriptions and 3D labels for objects in complex traffic environments.

Conclusion: This work introduces a novel 3D visual grounding task for outdoor monitoring scenarios, provides the first large-scale dataset (MoniRefer), and proposes an effective method (Moni3DVG) that outperforms existing approaches. The dataset and code will be publicly released to advance research in infrastructure-level scene understanding.
...
TL;DR: A Layer-by-Layer Hierarchical Attention Network improves feature point matching by addressing outlier issues through stage fusion, hierarchical extraction, and attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Feature point matching is fundamental but suffers from outlier interference, especially with high outlier proportions. The challenge is extracting high-quality information while reducing errors from negative samples.

Method: Proposes Layer-by-Layer Hierarchical Attention Network with: 1) Layer-by-layer channel fusion module preserving semantic information from each stage, 2) Hierarchical attention module capturing global perception and structural semantics, 3) Two architectures for feature extraction and integration.

Result: Experiments on YFCC100M and SUN3D datasets show superior performance in outlier removal and camera pose estimation compared to state-of-the-art methods.

Conclusion: The proposed network effectively handles outlier issues in feature point matching through hierarchical attention and fusion mechanisms, demonstrating improved accuracy and robustness for computer vision tasks.
...
TL;DR: The paper introduces FRS-YOLO, an improved object detection model for fire rescue scenarios, along with a new FireRescue dataset covering urban, mountainous, forest, and water rescue scenes with 8 key categories.


<details>
  <summary>Details</summary>
Motivation: Current fire detection research has two main limitations: 1) insufficient focus on complex urban rescue scenes (more frequent than forest/mountain scenarios), and 2) limited detection categories (mainly flames/smoke) without comprehensive coverage of command-critical targets like fire trucks and firefighters.

Method: 1) Constructed FireRescue dataset with 15,980 images and 32,000 bounding boxes covering multiple rescue scenarios and 8 key categories. 2) Proposed FRS-YOLO model with two improvements: a plug-and-play multidimensional collaborative enhancement attention module to reduce inter-class confusion, and a dynamic feature sampler to strengthen foreground features against smoke occlusion and background interference.

Result: Experimental results show that object detection in fire rescue scenarios is highly challenging, and the proposed FRS-YOLO method effectively improves the detection performance of YOLO series models in this specific context.

Conclusion: The paper addresses critical gaps in fire rescue object detection by creating a comprehensive dataset and developing an improved YOLO-based model that handles inter-class confusion and small target detection in chaotic rescue environments.
...
TL;DR: RadAR is a parallelizable framework that accelerates autoregressive visual generation using radial topology and nested attention to maintain spatial coherence while improving inference efficiency.


<details>
  <summary>Details</summary>
Motivation: Traditional autoregressive models for visual generation suffer from low inference efficiency due to sequential token-by-token decoding, despite their success in language modeling. Visual tokens exhibit strong local dependencies and spatial correlations that aren't fully exploited in standard raster-scan decoding orders.
...
Result: Significantly improves generation efficiency while preserving representational capacity. Maintains structural locality and spatial coherence of visual scenes while substantially increasing parallelization capabilities.

Conclusion: RadAR successfully addresses the inference efficiency bottleneck of autoregressive visual generation by leveraging spatial correlations through radial topology and preventing model collapse with nested attention, enabling efficient parallel generation without sacrificing quality.
...
TL;DR: RGTN is a physics-inspired tensor network search framework that uses renormalization group flows for multi-scale optimization, achieving state-of-the-art compression and 4-600× speedup over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing tensor network structure search methods have limitations: they use single-scale optimization missing multi-scale structures, discrete search spaces hindering smooth evolution, and separated structure-parameter optimization causing inefficiency. These challenges limit computational tractability, structure adaptivity, and optimization robustness across diverse tensor characteristics.

Method: RGTN uses dynamic scale-transformation for continuous structure evolution across resolutions, with learnable edge gates for topology modification during optimization. It employs intelligent proposals based on physical quantities like node tension (measuring local stress) and edge information flow (quantifying connectivity importance). The framework starts from low-complexity coarse scales and refines to finer ones, enabling escape from local minima via scale-induced perturbations.
...
Conclusion: RGTN successfully addresses key challenges in tensor network structure search through a physics-inspired renormalization group approach, demonstrating superior performance in both compression efficiency and computational speed compared to existing methods.
...
TL;DR: EVOL-SAM3 is a zero-shot reasoning segmentation framework that uses evolutionary search at inference time instead of fixed prompts, achieving state-of-the-art performance without training.


<details>
  <summary>Details</summary>
Motivation: Current reasoning segmentation methods have limitations: SFT suffers from catastrophic forgetting and domain dependency, RL has training instability and rigid reward functions, while training-free methods are limited by static "generate-then-segment" chains that lack reasoning depth and self-correction capabilities.

Method: EVOL-SAM3 reformulates reasoning segmentation as an inference-time evolutionary search process with a "Generate-Evaluate-Evolve" loop. It maintains a population of prompt hypotheses, uses a Visual Arena for reference-free pairwise tournament evaluation, Semantic Mutation operators for diversity and error correction, and a Heterogeneous Arena integrating geometric priors with semantic reasoning.

Result: EVOL-SAM3 substantially outperforms static baselines and significantly surpasses fully supervised state-of-the-art methods on the challenging ReasonSeg benchmark in a zero-shot setting.

Conclusion: The evolutionary search paradigm enables robust reasoning segmentation without training burdens, overcoming limitations of current SFT, RL, and static inference approaches through iterative refinement and self-correction capabilities.
...
TL;DR: FlowBlending: A stage-aware multi-model sampling strategy that uses large models for capacity-sensitive early/late diffusion stages and small models for intermediate stages, achieving faster inference with fewer FLOPs while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: The impact of model capacity varies across diffusion timesteps - crucial for early and late stages but negligible during intermediate stages. This observation motivates a stage-aware approach to optimize computational efficiency.

Method: FlowBlending uses a large model for capacity-sensitive stages (early/late) and a small model for intermediate stages. It includes criteria for choosing stage boundaries and uses velocity-divergence analysis to identify capacity-sensitive regions.

Result: Achieves up to 1.65x faster inference with 57.35% fewer FLOPs while maintaining visual fidelity, temporal coherence, and semantic alignment of large models. Compatible with existing acceleration techniques for up to 2x additional speedup.

Conclusion: FlowBlending effectively leverages varying model capacity requirements across diffusion stages to significantly improve inference efficiency without sacrificing quality, and can be combined with other acceleration methods.
...
TL;DR: EchoFoley introduces a new video-grounded sound generation task with fine-grained control, addressing limitations in current VT2A models through symbolic event representation and a large-scale curated dataset.
...
Method: Proposes EchoFoley task with symbolic representation specifying when, what, and how sounds are produced; constructs EchoFoley-6k dataset with 6,000 video-instruction-annotation triplets; develops EchoVidia framework with slow-fast thinking strategy for agentic generation.

Result: EchoVidia surpasses recent VT2A models by 40.7% in controllability and 12.5% in perceptual quality, demonstrating significant improvements in both control precision and audio quality.

Conclusion: The EchoFoley framework successfully addresses key limitations in video-grounded sound generation by enabling fine-grained control through symbolic event representation and hierarchical semantic control, with experimental results showing substantial improvements over existing methods.
...
Result: The paper introduces Splatwizard as a solution to the evaluation gap in 3DGS compression research, providing a standardized toolkit that enables comprehensive benchmarking of different methods across multiple dimensions.

Conclusion: Splatwizard addresses the critical need for standardized evaluation in 3DGS compression research by providing a unified benchmark toolkit that enables comprehensive assessment of compression methods across multiple performance dimensions, facilitating fair comparisons and advancing the field.
...
TL;DR: Unified framework for 3D instance segmentation using Gaussian Splatting that combines feature learning and label decoding in one stage, with boundary-aware hard mining for improved performance.


<details>
  <summary>Details</summary>
Motivation: Existing 3D segmentation methods suffer from inconsistent 2D instance labels across views, requiring two-stage approaches with hyperparameter-sensitive clustering or preprocessing. This leads to poor 3D predictions and increased training time.

Method: Proposes a unified framework with learnable feature embeddings in Gaussian primitives, decoded into instance labels via "Embedding-to-Label" process. Addresses boundary artifacts with hard-mining samples stabilized by applying a linear layer to rasterized embeddings before triplet loss calculation.
...
Conclusion: The unified framework effectively integrates optimization steps, addresses boundary issues through stabilized hard mining, and advances 3D instance segmentation for better scene understanding.
...
TL;DR: Proposes a physics-based adversarial attack on monocular depth estimation models using projected light perturbations, validated through real-world experiments.


<details>
  <summary>Details</summary>
Motivation: DNN-based monocular depth estimation models are vulnerable to adversarial attacks, threatening their reliability in practical applications. There's a need to validate this vulnerability and develop robust defense mechanisms.

Method: Projection-based adversarial attack using perturbation light projected onto target objects. Uses physics-in-the-loop optimization with real-world evaluation to account for device specifications and disturbances. Employs distributed covariance matrix adaptation evolution strategy for optimization.

Result: Successfully created adversarial examples that cause depth misestimations, making parts of objects disappear from the target scene. Confirmed vulnerability of DNN-based MDE models to physical adversarial attacks.

Conclusion: The proposed projection-based attack method effectively demonstrates the vulnerability of monocular depth estimation models to physical adversarial perturbations, highlighting the need for robustness enhancement in practical applications.
...
TL;DR: The paper proposes a method to apply certain nonlinear functions to noisy targets in Noise2Noise training without significant bias, enabling better denoising of HDR images from Monte Carlo rendering.


<details>
  <summary>Details</summary>
Motivation: Noise2Noise training requires noisy target images but cannot handle nonlinear preprocessing of these targets, which limits its applicability. This is particularly problematic for HDR image denoising where nonlinear tone mapping is needed to handle outliers.

Method: Developed a theoretical framework to analyze nonlinear functions applied to noisy targets, identifying a class of nonlinear functions that introduce minimal bias. Applied this to HDR denoising by combining specific loss functions and tone mapping functions.

Result: The method successfully denoises HDR images from Monte Carlo rendering using only noisy training data, achieving results comparable to the original implementation that used high-sample count reference images.

Conclusion: Certain nonlinear functions can be safely applied to noisy targets in Noise2Noise training, expanding its applicability to challenging domains like HDR image denoising where nonlinear preprocessing is essential.
...
TL;DR: A method using regret minimization and derivative-free optimization improves cross-modal systems' adaptation to 3D scenes by controlling an in-scene camera without pretraining.
...
Method: Uses regret minimization with derivative-free optimization to improve multivariate mutual information estimates, enabling off-the-shelf cross-modal systems to adapt online to object occlusions and differentiate features.
...
Conclusion: Expressive measures combined with value-based optimization allow effective control of in-scene cameras to learn directly from noisy vision-language model outputs, enabling 2D-trained systems to handle 3D scenes.
...
TL;DR: CropTrack is a novel MOT framework for agriculture that combines appearance and motion information to handle repetitive patterns, similar appearances, and frequent occlusions, outperforming traditional motion-based methods.


<details>
  <summary>Details</summary>
Motivation: Agricultural MOT faces challenges with repetitive patterns, similar object appearances, sudden illumination changes, and frequent occlusions. Current trackers rely mainly on motion and struggle with identity preservation during strong occlusions, while appearance-based association is difficult due to high similarity of objects.

Method: CropTrack integrates: 1) reranking-enhanced appearance association, 2) one-to-many association with appearance-based conflict resolution, and 3) exponential moving average prototype feature bank to improve appearance-based association.

Result: CropTrack demonstrates consistent identity preservation on agricultural MOT datasets, outperforming traditional motion-based methods and achieving significant gains in identification F1 and association accuracy scores with fewer identity switches compared to state-of-the-art.

Conclusion: The proposed CropTrack framework effectively addresses agricultural MOT challenges by combining appearance and motion information, providing better identity preservation and association accuracy than existing methods.
...
TL;DR: VLN-MME: A unified evaluation framework for testing MLLMs as zero-shot agents in Vision-and-Language Navigation, revealing poor 3D spatial reasoning despite instruction-following capabilities.


<details>
  <summary>Details</summary>
Motivation: While MLLMs excel at vision-language tasks, their performance as embodied agents requiring multi-round dialogue, spatial reasoning, and sequential action prediction remains unexplored, particularly in navigation contexts.

Method: Created VLN-MME, a standardized benchmark by bridging traditional navigation datasets into a unified framework with modular design. Evaluated MLLMs as zero-shot agents, testing various architectures, agent designs, and navigation tasks with structured comparisons and component-level ablations.

Result: Counterintuitively, enhancing baseline agents with Chain-of-Thought reasoning and self-reflection decreased performance. MLLMs show poor context awareness in embodied navigation - while they can follow instructions and structure output, their 3D spatial reasoning fidelity is low.

Conclusion: VLN-MME provides systematic evaluation groundwork for MLLMs in embodied navigation, revealing limitations in sequential decision-making. Findings offer crucial guidance for MLLM post-training as embodied agents, highlighting the gap between instruction-following and spatial reasoning capabilities.
...
<details>
  <summary>Details</summary>
Motivation: SAM2 shows promise for medical image segmentation but requires extensive annotated data and manual prompts, which are labor-intensive and require medical expertise. There's a need for label-efficient adaptation methods.

Method: OFL-SAM2 uses a lightweight mapping network that captures medical knowledge from limited annotated samples. It includes: (1) an online few-shot learner to train the mapping network to generate target features, and (2) an adaptive fusion module that dynamically integrates target features with SAM2's memory-attention features.
...
Conclusion: OFL-SAM2 provides an effective prompt-free framework for label-efficient medical image segmentation by leveraging limited annotated data to train a mapping network that eliminates the need for manual prompts while maintaining strong performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack the complexity needed to evaluate MLLMs on real-world financial reasoning tasks that involve implicit scenarios, complex documents, and multi-step computations.

Method: Created a benchmark with 1,200 expert-annotated problems incorporating 12 types of implicit financial scenarios, 837 bilingual documents averaging 50.8 pages with rich visual elements, and problems requiring 11-step reasoning on average with cross-page evidence.

Result: The best-performing MLLM achieves only 58.0% accuracy, and different RAG methods show significant performance variations, indicating the benchmark's challenging nature.
...
TL;DR: Novel lidar domain adaptation method using neuron activation patterns to achieve SOTA performance with minimal target domain annotations, preventing weight drift through continual learning techniques.


<details>
  <summary>Details</summary>
Motivation: 3D object detectors trained on standard benchmarks struggle to generalize across different geographic domains (e.g., US-trained models performing poorly in Asia/Europe), creating a need for efficient domain adaptation methods that require minimal annotation effort.
...
Result: Empirical evaluation shows the proposed approach outperforms both linear probing and state-of-the-art domain adaptation techniques, achieving superior performance with very small annotation budgets.

Conclusion: The method demonstrates that effective domain adaptation for 3D object detectors can be achieved with minimal target domain annotations through intelligent sample selection based on neuron activation patterns and weight drift prevention techniques.
...
<details>
  <summary>Details</summary>
Motivation: Existing open-source film restoration methods have limited performance due to training with low-quality synthetic data, noisy optical flows, and inability to handle high-resolution films. There's a need for better restoration of indistinguishable film defects.

Method: Proposes HaineiFRDM framework with: 1) patch-wise training/testing for high-resolution films on 24GB VRAM GPU, 2) position-aware Global Prompt and Frame Fusion Modules, 3) global-local frequency module for texture consistency, 4) low-resolution restoration as global residual to reduce artifacts, and 5) new dataset with real-degraded films and realistic synthetic data.

Result: Comprehensive experiments demonstrate superior defect restoration ability over existing open-source methods. The model effectively handles high-resolution films and produces consistent textures across patches.

Conclusion: HaineiFRDM successfully leverages diffusion models' content understanding to help experts restore film defects, addresses high-resolution limitations, and outperforms current open-source methods. Code and dataset will be released.
...
TL;DR: CPJ is a training-free few-shot framework that improves agricultural pest VQA using structured image captions and LLM-as-Judge refinement, achieving significant performance gains without fine-tuning.
...
Method: CPJ uses large vision-language models to generate multi-angle captions, refines them via LLM-as-Judge iterative refinement, then employs a dual-answer VQA process for both disease recognition and management responses.
...
<details>
  <summary>Details</summary>
Motivation: CAC scoring from chest CT is important for cardiovascular risk assessment but is limited by motion artifacts in non-gated CTs, which are more widely available than ECG-gated cardiac CTs but suffer from more severe motion issues.

Method: ProDM uses a generative diffusion framework with three key components: 1) CAC motion simulation data engine to synthesize non-gated acquisitions from cardiac-gated CTs, 2) property-aware learning with calcium-specific priors through differentiable calcium consistency loss, and 3) progressive correction scheme across diffusion steps.

Result: ProDM significantly improves CAC scoring accuracy, spatial lesion fidelity, and risk stratification performance compared to baselines. Reader studies confirm it suppresses motion artifacts and improves clinical usability.

Conclusion: ProDM demonstrates the potential of progressive, property-aware frameworks for reliable CAC quantification from routine chest CT imaging, offering an accessible alternative to ECG-gated CTs.
...
<details>
  <summary>Details</summary>
Motivation: Existing evaluation frameworks for video generation models rely on single-frame assessments, leading to outcome-hacking where models reach correct conclusions through erroneous processes. There's a need for process-aware evaluation to properly assess Chain-of-Frames reasoning capabilities.
...
Result: State-of-the-art video models achieve only about 20% POC@1.0, showing significant outcome-hacking. Test-time scaling and sampling robustness experiments reveal substantial gap between current video generation and true generalized visual reasoning.

Conclusion: Current video generation models lack genuine reasoning capabilities despite correct final answers. The VIPER benchmark provides comprehensive process-aware evaluation, highlighting the need for improved reasoning in video models. The benchmark will be publicly released.
...
TL;DR: ShowUI-π is a flow-based generative model for GUI agents that enables both discrete clicks and continuous drag actions, addressing limitations of existing agents that only support discrete interactions.


<details>
  <summary>Details</summary>
Motivation: Existing GUI agents only support discrete click predictions (x,y coordinates), which prevents them from performing continuous, closed-loop trajectories like dragging progress bars that require real-time perception and adjustment. This limits their ability to achieve human-like dexterous manipulation in digital environments.

Method: 1) Unified Discrete-Continuous Actions model integrating both click and drag actions; 2) Flow-based Action Generation for drag modeling using a lightweight action expert to predict incremental cursor adjustments; 3) Collection of 20K drag trajectories across five domains and creation of ScreenDrag benchmark with online/offline evaluation protocols.

Result: ShowUI-π achieves 26.98 score on ScreenDrag benchmark with only 450M parameters, outperforming proprietary GUI agents (Operator: 13.27, Gemini-2.5-CUA: 22.18). The benchmark reveals existing agents struggle with drag tasks.

Conclusion: ShowUI-π advances GUI agents toward human-like dexterous control by enabling continuous drag actions through flow-based generative modeling, with demonstrated superiority over existing approaches on the new ScreenDrag benchmark.
...
TL;DR: Compression techniques (quantization, pruning, clustering) can preserve or even improve model robustness against natural corruption, with customized combinations offering best multi-objective results for efficient deployment.


<details>
  <summary>Details</summary>
Motivation: Compressed deep learning models are needed for resource-constrained devices, but compression may affect robustness under natural corruption, requiring comprehensive evaluation of robustness alongside compression.

Method: Comprehensive evaluation of compression techniques (quantization, pruning, weight clustering) applied individually and in combination to CNNs (ResNet-50, VGG-19, MobileNetV2) using CIFAR-10-C and CIFAR-100-C datasets with multiobjective assessment.

Result: Certain compression strategies not only preserve but can improve robustness, especially on networks with more complex architectures. Customized technique combinations produce beneficial multi-objective results for robustness, accuracy, and compression ratio trade-offs.
...
<details>
  <summary>Details</summary>
Motivation: Current VLM benchmarks focus on ideal lighting conditions, but real-world embodied agents need to operate 24/7 in various lighting conditions including low-light environments. There's a lack of evaluation for VLMs' robustness in dark conditions, which is crucial for practical deployment.

Method: Created DarkEQA benchmark with physics-based low-light degradations modeled in linear RAW space, simulating illumination drop and sensor noise followed by ISP-inspired rendering. Evaluates question answering from egocentric observations under controlled degradations to isolate perception bottlenecks.
...
Conclusion: DarkEQA addresses a critical gap in VLM evaluation for embodied agents, providing a physically realistic benchmark that exposes robustness issues in low-light conditions, with code and dataset to be released for community use.
...
TL;DR: Proposes Bi-C2R framework for Re-index Free Lifelong person Re-ID (RFL-ReID) that enables continuous learning without re-indexing historical gallery images, addressing privacy and cost concerns while maintaining feature compatibility.


<details>
  <summary>Details</summary>
Motivation: Existing lifelong ReID methods require re-indexing historical gallery images after each model update, which raises privacy concerns and incurs high computational costs for large-scale galleries. This creates incompatible features between updated query models and old gallery features, degrading performance.

Method: Proposes Bidirectional Continuous Compatible Representation (Bi-C2R) framework that continuously updates gallery features extracted by old models to maintain compatibility with new models, enabling lifelong learning without re-indexing historical data.

Result: The Bi-C2R method achieves leading performance on both the new RFL-ReID task and traditional L-ReID task across multiple benchmarks, demonstrating effective feature compatibility and continuous learning capabilities.

Conclusion: RFL-ReID is a more challenging but practical lifelong ReID task that addresses privacy and cost concerns. The proposed Bi-C2R framework successfully enables continuous learning without re-indexing by maintaining feature compatibility between old and new models.
...
<details>
  <summary>Details</summary>
Motivation: Previous flow-based SLAM approaches lack geometric consistency, leading to inaccurate tracking and mapping. The authors aim to bridge flow estimation with geometric reasoning to improve accuracy and robustness.
...
Result: Superior trajectory accuracy and dense reconstruction quality across multiple challenging datasets, running in real-time at 18 FPS with strong generalization to various scenarios.

Conclusion: FoundationSLAM successfully addresses geometric consistency issues in flow-based SLAM by integrating foundation depth models, demonstrating practical applicability and robust performance in diverse real-world scenarios.
...
TL;DR: A novel self-bootstrapping framework reframes visual dubbing from ill-posed inpainting to well-conditioned video editing using Diffusion Transformers to generate ideal training data and achieve precise lip synchronization.


<details>
  <summary>Details</summary>
Motivation: Audio-driven visual dubbing lacks ideal training data where only lip movements differ while all other visual conditions remain identical. Existing mask-based inpainting methods force models to hallucinate missing content while syncing lips, leading to visual artifacts, identity drift, and poor synchronization.

Method: Proposes a self-bootstrapping framework using Diffusion Transformers: 1) as a data generator to synthesize ideal training data (lip-altered companion videos for each real sample), and 2) as an audio-driven editor trained end-to-end on these aligned video pairs. Also introduces timestep-adaptive multi-phase learning strategy to disentangle conflicting editing objectives across diffusion timesteps.

Result: Achieves highly accurate lip sync, faithful identity preservation, and exceptional robustness in challenging in-the-wild scenarios. The method provides complete visual context for the editor, enabling precise audio-driven lip modifications while maintaining scene interactions and spatiotemporal dynamics.

Conclusion: The framework successfully transforms visual dubbing from an ill-posed inpainting task into a well-conditioned video-to-video editing problem, overcoming fundamental data limitations and achieving superior performance through self-generated training data and complete visual conditioning.
...
<details>
  <summary>Details</summary>
Motivation: Real-world skeleton sequences often have substantial missing data from online pose estimation, making fine-grained action recognition challenging. Existing methods struggle to recover temporal dynamics and fine-grained spatial structures, losing subtle motion cues needed to distinguish similar actions.

Method: 1) Context-aware completion with diverse temporal masking to restore base skeleton sequence; 2) Skeleton-based spatial decomposition into five semantic regions, further divided into dynamic/static subgroups; 3) Targeted perturbation to generate augmented sequences; 4) Physics-driven estimation using Lagrangian dynamics to compute joint accelerations; 5) GCN-based action recognition head processing both fused skeleton positions and accelerations.

Result: Significantly outperforms previous methods on both coarse-grained (NTU-60, NTU-120) and fine-grained (Gym99, Gym288) benchmarks under various temporal corruption levels. Achieves 89.1% and 78.1% top-1 accuracy on Gym99-severe and Gym288-severe settings respectively.

Conclusion: FineTec effectively addresses fine-grained action recognition under temporal corruption by combining context-aware completion, spatial decomposition, physics-driven acceleration estimation, and GCN-based recognition, demonstrating robustness and generalizability across datasets.
...
<details>
  <summary>Details</summary>
Motivation: Prior methods require per-scene optimization for 3D editing, which is slow and computationally expensive. There's a need for fast, photorealistic 3D scene editing that can handle unposed, view-inconsistent images without optimization overhead.

Method: Uses a feed-forward framework with two key innovations: (1) SAM2-based recoloring strategy for generating cross-view-consistent supervision, and (2) asymmetric input strategy pairing recolored reference views with raw auxiliary views to fuse disparate observations. The model is trained to predict instruction-aligned 3D edits directly.

Result: Edit3r achieves superior semantic alignment and enhanced 3D consistency compared to recent baselines, while operating at significantly higher inference speed. It effectively handles images edited by 2D methods like InstructPix2Pix despite not being trained on such edits. The paper also introduces DL3DV-Edit-Bench benchmark for evaluation.

Conclusion: Edit3r enables fast, photorealistic 3D scene editing without optimization or pose estimation, making it promising for real-time 3D editing applications. The method addresses the challenge of lacking multi-view consistent edited images through innovative training strategies.
...
TL;DR: GaMO is a geometry-aware multi-view outpainting framework that addresses sparse-view 3D reconstruction by expanding field of view from existing camera poses instead of generating new viewpoints, achieving better geometric consistency and faster processing.
...
Result: State-of-the-art reconstruction quality on Replica and ScanNet++ datasets across 3, 6, and 9 input views, outperforming prior methods in PSNR and LPIPS metrics, with 25× speedup over SOTA diffusion-based methods (processing under 10 minutes).

Conclusion: GaMO demonstrates that multi-view outpainting is an effective alternative to novel view generation for sparse-view 3D reconstruction, providing better geometric consistency, broader coverage, and significantly faster processing without requiring training.
...
<details>
  <summary>Details</summary>
Motivation: To enable controllable generative rendering where users can independently manipulate spatial (camera viewpoint) and temporal (motion sequence) aspects of videos for continuous exploration of dynamic scenes.

Method: Introduces animation time-embedding for explicit motion control, temporal-warping training scheme using multi-view datasets to simulate temporal differences, improved camera-conditioning from first frame, and CamxTime synthetic dataset for full space-time coverage.

Result: Demonstrates clear space-time disentanglement and strong performance on both real-world and synthetic data compared to prior work, enabling independent control over camera viewpoint and motion sequence.

Conclusion: SpaceTimePilot successfully achieves robust space-time disentanglement for controllable video generation, allowing continuous exploration of dynamic scenes through independent manipulation of spatial and temporal dimensions.
