...
TL;DR: AtmoFuseNet is a framework that fuses sky camera imagery, millimeter-wave cloud radar, and ceilometer data to produce 4D estimates of cloud state and wind, achieving 0.026 g/m³ LWC MAE and 1.18 m/s wind speed MAE.


<details>
  <summary>Details</summary>
Motivation: Dense volumetric reconstruction of cloud microphysical fields from sparse ground-based instruments remains challenging due to heterogeneous modalities and spatial coverage.

Method: Three-stage framework: (1) cross-modal hierarchical aggregation with layer-wise cross-attention; (2) conditional variational refinement with differentiable radar/image forward models; (3) correlation-based motion estimator for per-voxel 3D wind vectors.

Result: 0.026 g/m³ liquid water content MAE and 1.18 m/s wind speed MAE, outperforming existing baselines on collocated semi-arid site observations.

Conclusion: AtmoFuseNet effectively fuses multi-modal ground-based observations for 4D cloud reconstruction, with ablation studies confirming the contribution of each module.
...
TL;DR: Proposes a streaming Gaussian encoder for camera-based 4D panoptic occupancy tracking that maintains persistent volumetric scene representations via propagated latent Gaussian queries with confidence-guided updates, achieving state-of-the-art results on nuScenes and Waymo.


<details>
  <summary>Details</summary>
Motivation: Existing mask-based pipelines recompute volumetric representations at each timestep, limiting geometric temporal consistency under occlusion and for static scene elements.

Method: Models scenes as fixed-size sets of latent Gaussian queries propagated via ego-motion compensation and refreshed under confidence-guided budget constraints. Uses depth-supervised Gaussian opacities as visibility proxies with warmup-based multi-frame training.

Result: Establishes new state-of-the-art on Occ3D-extended nuScenes and Waymo, improving tracking consistency with negligible computational overhead.

Conclusion: The streaming Gaussian encoder provides representation-level temporal coherence beyond decoder-only tracking and is fully compatible with existing mask-based pipelines.
...
TL;DR: A framework that learns latent representations of motion prediction datasets and quantifies their similarity to predict cross-dataset transferability, validated across 24 major datasets.


<details>
  <summary>Details</summary>
Motivation: Motion prediction models trained on one dataset often fail to generalize to other datasets due to differences in scene layouts, behaviors, and sensing conditions, creating a need for understanding dataset similarity and transferability.

Method: The authors propose a framework that learns latent representations of datasets and quantifies their similarity using distributional metrics, then calculates transferability scores correlated with cross-dataset model performance.

Result: The transferability scores strongly correlate with actual cross-dataset model performance across 24 major motion prediction datasets.

Conclusion: The framework provides practical guidance for dataset selection, pretraining strategies, and developing large-scale foundation models for more generalizable motion prediction.
...
TL;DR: RTSM, a simple method that mixes labeled target images into SFDA-OD, consistently outperforms pure SFDA by 1.7-18.3 AP50, and more complex sparse-label plugins offer limited gains, revealing that simple supervised mixing is hard to beat.


<details>
  <summary>Details</summary>
Motivation: To revisit SFDA-OD when a small labeled subset of target images is available, and to explore whether simple supervised mixing or more sophisticated steering of unlabeled self-training yields better gains.

Method: RTSM (Random-Target Supervised Mixing): adds a supervised detection loss for a small uniformly sampled labeled subset of target images while keeping the original unlabeled adaptation branch unchanged. Also evaluates ten sparse-label feedback plugins for pseudo-label selection, object completion, and optimization control.

Result: RTSM consistently improves SFDA by 1.7-18.3 AP50 across four methods, two detectors, multiple tasks, and 1%-10% label budgets. Sparse-label plugins yield limited and method-dependent gains over RTSM.

Conclusion: Simple supervision (RTSM) is hard to beat in sparse-label SFDA-OD; more complex feedback mechanisms offer marginal improvements, establishing RTSM as a strong anchor baseline.
...
TL;DR: GaussLite introduces a task-driven 3D Gaussian Splatting system that allocates representation capacity based on natural-language task specifications, achieving higher ROI quality at matched budgets through relevance-guided seeding, gradient flow, and scaling, with real-time map fusion capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS systems waste compute by uniformly distributing representation capacity across entire scenes, even though robotic tasks only care about relevant fractions. This limits online performance or under-optimizes critical regions.

Method: Uses a one-shot LLM parser to extract target/anchor objects from natural-language task specs, grounds them via open-vocabulary detector per frame, produces per-pixel relevance masks, and allocates Gaussian density/seeding/gradient flow by task relevance. Also supports real-time map fusion via per-voxel voting.

Result: +2.72 dB ROI PSNR on Replica Dataset, +2.23 dB on real hardware (indoor/outdoor). Map fusion outperforms concatenation by +3.42 dB while sharing only 7.08% of the map. Runs at 4 Hz on resource-constrained hardware.

Conclusion: Task-driven allocation of 3DGS capacity significantly improves reconstruction quality for task-relevant regions at matched budgets, enabling efficient online mapping and multi-agent map fusion for robotic applications.
...
TL;DR: AVTok is a unified tokenizer for audio-video generation that uses a dual-stream transformer with shared encoder-decoder and modal-specific queries to encode audio-video pairs into compact 1D latent representations with a unified codebook, enhanced by a hierarchical training strategy to handle heterogeneous information imbalance.


<details>
  <summary>Details</summary>
Motivation: Previous dual-branch audio-video generation methods suffer from a representation gap between modalities and require intensive computational resources. The authors aim to create a unified tokenization approach that bridges this gap efficiently.

Method: AVTok uses a dual-stream transformer architecture with shared encoder-decoder and modal-specific learnable queries to encode audio-video pairs into compact 1D latent representations with a unified codebook. A hierarchical training strategy progressively enables reconstruction capabilities for each modality to address heterogeneous information imbalance.

Result: AVTok achieves excellent performance in audio-video reconstruction and downstream tasks including audio-to-video, video-to-audio, and class-conditional joint audio-video generation.

Conclusion: AVTok advances joint audio-video tokenization and provides a potential direction for building unified large multimodal models for audio-video generation.
...
TL;DR: SyncCache is a training-free caching acceleration method for diffusion transformer-based audio-driven portrait animation that achieves up to 4.12x speedup by exploiting spatial and modality asymmetries through spatially-asymmetric probing and modality-decoupled caching.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) have high computational cost leading to substantial inference latency in audio-driven portrait animation, and existing training-free caching methods are designed for text-conditioned generation, failing to address spatial and modality imbalances specific to portrait animation.

Method: (1) Spatially-Asymmetric Probing to prioritize error sensitivity in dynamic human regions over static backgrounds; (2) Modality-Decoupled Caching that reuses stable inter-block residuals while recomputing lightweight audio blocks for precise lip sync; (3) Cache ratio control with offline dynamic programming for memory-adaptive cache selection.

Result: Up to 4.12x acceleration on HunyuanVideo-Avatar and 3.75x on Wan-S2V with near-lossless visual fidelity and precise audio alignment.

Conclusion: SyncCache demonstrates superior speed-quality trade-offs for DiT-based audio-driven portrait animation by explicitly exploiting spatial and modality asymmetries through specialized caching strategies.
...
TL;DR: A Turing-inspired Label Imitation Game (LIG) framework uses a Turing Test Network (TTN) to prune hallucinated pseudo-labels via adversarial interrogation, outperforming standard threshold methods and enabling zero-shot task transfer from classification to object detection, achieving up to 44% F1-score gains and Category Revival.


<details>
  <summary>Details</summary>
Motivation: Foundation model pseudo-labeling via zero-shot inference scales massively but suffers from hallucinations that evade standard confidence thresholds, creating a need for more robust error elimination methods.

Method: The Label Imitation Game (LIG) formalizes pseudo-label pruning as adversarial interrogation, training a task-agnostic Turing Test Network (TTN) as a "judge" that evaluates candidate pseudo-labels within dataset-wide context rather than using isolated thresholds.

Result: 28% F1-score gains for worst-performing baseline categories, 44% with task-specific fine-tuning; demonstrated robustness across four datasets and three vision-language models; TTN trained on classification data effectively prunes object detection pseudo-labels (zero-shot task transfer).

Conclusion: Learned semantic-contextual logic is a robust alternative to spatial-geometric verification for pseudo-label pruning, and the approach enables Category Revival where downstream models recover from zero recall on previously vulnerable classes.
...
TL;DR: This paper proposes a method to estimate the scale of real-world objects (like traffic signs) from monocular images by decomposing them into structural elements and integrating external design knowledge. The estimated components are used to generate 3D assets for digital twin environments, improving the accuracy of in-vehicle camera verification for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: In-vehicle camera verification using simulation often suffers from scale discrepancies between virtual and real environments. Traffic signs at high altitudes are hard to measure with LiDAR or stereo cameras, so a monocular-based scale estimation method is needed.

Method: Decomposes the target object into multiple structural elements, detects each component from a monocular image, and estimates its size by leveraging external knowledge (design rules, geometric relationships, and conventional dimensions) along with dimensional consistency with surrounding elements.

Result: The method enables generation of 3D assets with scales approximating the real environment, suitable for placement in digital twin spaces.

Conclusion: This approach is expected to enhance the verification accuracy of in-vehicle cameras for autonomous driving in virtual environments by reducing scale mismatches.
...
TL;DR: GRAPE is a unified prototype-based medical image classifier architecture that addresses three clinical limitations: modeling anatomical concept co-occurrence via Graph Attention (boosting macro-F1 by +13.8pp), detecting unsafe physician feedback via Concept-Mismatch Safety Check (85% vs 51% catch rate), and enabling open-vocabulary prototype anchoring for adding new findings from one labeled image without retraining.


<details>
  <summary>Details</summary>
Motivation: Prototype-based medical image classifiers suffer from three clinical limitations: treating findings as independent, silently amplifying unsafe physician feedback, and requiring full retraining when adding a new finding.

Method: GRAPE employs three components: (1) Graph Attention Task Head for modeling anatomical concept co-occurrence, (2) Concept-Mismatch Safety Check that warns when the dominant finding conflicts with the claimed label, and (3) Open-Vocabulary Prototype Anchoring that aligns visual prototypes to clinical text for few-shot addition of new findings.

Result: +13.8pp macro-F1 gain on TBX11K; 85% erroneous annotation catch rate (vs 51% for MC-Dropout); one-shot Effusion example recovers full-supervision localization on ChestX-ray14; 2.6x better lesion localization on TBX11K; only +1ms latency added.

Conclusion: GRAPE successfully addresses all three clinical limitations of prototype-based classifiers with minimal computational overhead, offering a safer and more flexible architecture for medical image analysis.
...
TL;DR: A geometry-constrained test-time prompt tuning framework for LiDAR semantic segmentation that uses per-location sensing reliability to reweight supervision and confines adaptation to lightweight prompt adapters with spatial gating, achieving stable online adaptation without annotations.


<details>
  <summary>Details</summary>
Motivation: LiDAR semantic segmentation degrades under real-world deployment due to evolving sensing conditions, but collecting new annotations for retraining is impractical. Standard TTA methods fail because pseudo-label reliability varies spatially due to range-dependent sparsity and occlusion.

Method: (1) Per-location sensing reliability estimated from depth-consistent beam terminations and neighborhood support; (2) Lightweight prompt adapters inserted into a frozen backbone; (3) Spatial gating to prevent unreliable regions from perturbing globally shared representations; (4) Temporally smoothed prototype alignment for stable online updates.

Result: Improved adaptation stability and segmentation performance under deployment variations on standard LiDAR benchmarks without additional annotations.

Conclusion: The proposed geometry-constrained test-time prompt tuning effectively handles spatially heteroscedastic reliability in LiDAR data, enabling stable test-time adaptation for semantic segmentation.
...
TL;DR: Prost-RL uses reinforcement learning policies with foundation models to improve micro-ultrasound prostate cancer detection, achieving 79.0 AUROC across 6,607 biopsy cores from 693 patients, with transparent spatial attention maps for interpretable diagnosis.


<details>
  <summary>Details</summary>
Motivation: Micro-ultrasound (μUS) for prostate cancer detection suffers from high inter-observer variability due to clinical experience dependence. Machine learning could help, but training is hindered by sparse, noisy supervision (only core-level labels, no pixel annotations) and severe class imbalance.

Method: Prost-RL frames PCa detection as a spatially aware, policy-driven inference problem, integrating a lightweight RL policy into a foundation-model encoder-decoder to generate interpretable spatial attention maps. It also introduces Adaptive Policy Optimization (APO) for stable training and a noise-robust objective combining symmetric cross-entropy with negative-entropy regularization.

Result: On 6,607 biopsy cores from 693 patients across five sites, Prost-RL achieves 79.0±3.5 AUROC with 64.6±6.3% sensitivity at 80% specificity for core-level detection, outperforming baselines by +2.1 AUROC and +4.5 sensitivity points, and 79.3±5.8 AUROC for clinically significant cancer classification.

Conclusion: Prost-RL provides transparent, spatially grounded evidence alongside quantitative risk predictions by highlighting biopsy-aligned regions, offering interpretable and high-performance micro-ultrasound prostate cancer detection.
...
TL;DR: PhotoQuilt is a training-free framework that generates high-resolution photomosaics by first creating a low-resolution global layout, then upscaling and denoising in fixed tiles to maintain both local detail and global coherence without expensive computational costs.


<details>
  <summary>Details</summary>
Motivation: High-resolution photomosaic generation is computationally expensive because the canvas must hold many detailed tiles simultaneously. Diffusion models struggle to satisfy both global structure and local detail at once.

Method: A bootstrapped tiled denoising procedure: (1) generate a low-resolution global composition for layout, (2) upscale in latent space and re-inject noise, (3) denoise within fixed tiles to preserve local quality while sharing global structure, enabling scaling without quadratic attention cost.

Result: Outperforms current baselines on both global structure and local realism.

Conclusion: PhotoQuilt provides an efficient, training-free solution for generating high-resolution photomosaics, resolving the tension between global coherence and local detail.
...
TL;DR: The paper reveals that current AI-generated video detection benchmarks suffer from uncontrolled confounds inflating reported generalization. It introduces VidAudit—an audited evaluation protocol with six standard controls—and shows that trivial classifiers can achieve near-perfect scores under unaudited settings but collapse under proper auditing. The authors evaluate six detectors, find that the 2025 forensic detector WaveRep achieves genuine OOD performance (AUC 0.996), and recommend reporting a four-metric tuple instead of a single AUC. A white-box positive control (TemporalSpec + XSFF) demonstrates genuine complementarity.


<details>
  <summary>Details</summary>
Motivation: Current AI-generated video detection benchmarks have uncontrolled confounds that inflate reported generalization scores, making leaderboard rankings misleading and failing to measure what they claim.

Method: The authors (1) conduct a 20-paper survey to identify six missing standard controls, (2) design an audited evaluation protocol incorporating all six controls, (3) evaluate six feature sources (three published detectors + three repurposed signals) on AIGVDBench, and (4) introduce TemporalSpec (codec motion vectors) with cross-substrate feature fusion (XSFF) as a white-box positive control.

Result: A trivial three-feature clip-length classifier achieves 0.998 LOGO AUC on GenVidBench unaudited but collapses to 0.529 under audit. WaveRep achieves 0.996 OOD LOGO AUC with chance-level real-vs-real coherence. At 0.1% FPR, many high-AUC methods drop to single-digit recall, changing leaderboard order. TemporalSpec + XSFF shows genuine complementarity surviving audit.

Conclusion: The paper recommends replacing single-number AUC evaluation with an audited tuple (AUC, above-floor margin, operating-point recall, calibration). It releases VidAudit—14 detectors behind one plugin API, a leaderboard, and Croissant metadata—to move evaluation from leaderboard rank toward measuring what actually claims.
...
TL;DR: This paper proposes a lightweight framework that uses SAM 3 as a structural prior for sparse action-aware landmark localization in surgical videos. Without manual pixel-level mask annotations, the method achieves 72.4% F1 for tip and 58.0% for anchor localization across diverse surgical datasets.


<details>
  <summary>Details</summary>
Motivation: Vision foundation models like SAM 3 provide transferable object-level structure but lack action-conditioned semantics needed for functional surgical landmarks (e.g., clipping, grasping). Manual pixel-level mask annotations are expensive and impractical.

Method: A lightweight refinement framework uses SAM 3 as a structural prior. A coarse multi-frame network predicts tip and anchor prompts, generating non-oracle masks fused with visual and heatmap features. Three strategies are compared: direct mask-augmented supervision, prediction-derived mask-prior refinement, and auxiliary mask supervision.

Result: Evaluated on 7,867 clips from 60 surgical videos (5 datasets). Achieves 72.4% F1 for tip and 58.0% for anchor localization without manual mask annotations. Direct mask imposition biases toward broad regions; prediction-derived priors and auxiliary supervision work better.

Conclusion: Vision foundation model-derived structure can effectively guide precision-oriented landmark localization without pixel-level annotations, but the manner of incorporating structural priors matters critically—intermediate structural guidance outperforms direct mask supervision.
...
TL;DR: The paper proposes DSAFormer, a dual sparse aggregation transformer for multispectral object detection that selectively computes cross-modal attention at both spatial and channel levels to avoid redundant information interaction, combined with a learnable fusion block, achieving state-of-the-art results on four public datasets.


<details>
  <summary>Details</summary>
Motivation: Previous transformer-based multispectral detection methods use all available tokens for similarity calculation, leading to redundant information interaction from irrelevant areas and degraded detection performance.

Method: DSAFormer consists of (1) DSFormer with three components: Spatial Sparse Multi-Head Cross-Attention (SSMHCA) for selective spatial-level cross-modal attention, Channel Sparse Multi-Head Cross-Attention (CSMHCA) for channel-level sparse computation, and Multi-Scale Feature Refinement Layer (MSFRL) for hierarchical feature aggregation; (2) Learnable Addition Fusion Block (LAFB) for multimodal feature fusion via reweighting.

Result: DSAFormer achieves better detection performance than state-of-the-art methods on four public datasets: MFAD, FLIR, M^3FD, and LLVIP.

Conclusion: The proposed dual sparse attention mechanism effectively eliminates irrelevant cross-modal interactions, and the learnable fusion block enhances multimodal feature integration, leading to superior multispectral object detection.
...
TL;DR: This paper proposes a saliency-guided warp-unwarp framework for image-to-image translation using latent diffusion models, which improves structural detail preservation by reallocating latent space to salient regions, without modifying the original architecture.


<details>
  <summary>Details</summary>
Motivation: Compact latent diffusion models struggle to preserve fine-grained structures because the encoder compresses high-resolution inputs into a spatially downsampled latent space.

Method: A saliency-guided warp-unwarp framework that warps salient regions to better utilize latent space, processes with original diffusion model, then inverse warps; plus an outpainting-based synthetic data pipeline for relighting.

Result: Improved structural preservation, lighting faithfulness, and image quality on human relighting, driving scene relighting, and translation tasks; extends to video with temporal stability.

Conclusion: The warp-unwarp framework is model-agnostic, requires no architectural modifications, adds negligible computational overhead, and effectively enhances detail preservation in I2I translation.
...
TL;DR: TerraDiT-Ω introduces a unified spatial control framework for satellite image generation that works with any native geospatial primitive (polygons, polylines, bounding boxes, points), uses Geometry-Aware Local Attention for conditioning, and outperforms existing dense/sparse control methods across multiple downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Satellite imagery generation is challenging due to its structured geometry. Prior methods either use dense rasters (high annotation cost) or sparse prompts (low fidelity), breaking compatibility with standard vector-based geospatial primitives.

Method: Proposes TerraDiT-Ω, a unified spatial control framework that accepts any native geospatial primitive (polygons, polylines, bounding boxes, points). Introduces Geometry-Aware Local Attention, a conditioning mechanism that injects explicit geometric cues into the attention space during generation.

Result: Consistently outperforms both dense-control and sparse-control baselines across all conditioning formats. Enables controllable synthetic data augmentation that improves downstream performance on land-cover segmentation, object detection, road graph extraction, and scene classification.

Conclusion: The proposed framework unifies geospatial primitive control for satellite image generation, broadening applicability to design tasks like urban planning while maintaining compatibility with end-to-end GeoAI workflows and boosting downstream task performance via data augmentation.
...
TL;DR: PDR introduces PredGS, a plug-and-play 2D Gaussian adapter for video prediction that bridges discrete and continuous representations, achieving superior detail preservation and visual fidelity with a fast CUDA-accelerated renderer.


<details>
  <summary>Details</summary>
Motivation: Existing deterministic video prediction models operate in discrete pixel space with MSE loss, producing over-smoothed predictions lacking fine-grained visual details.

Method: Proposes PDR with PredGS (2D Gaussian adapter) + predgsplat (CUDA-accelerated differentiable renderer) + combined L1 & SSIM loss for high-fidelity continuous video prediction.

Result: Consistently outperforms existing methods on TaxiBJ, WeatherBench, KTH, and Human3.6M benchmarks, with up to 10x faster rendering and superior detail preservation.

Conclusion: PDR effectively bridges discrete and continuous representations, overcoming MSE-induced blurring and setting a new standard for video prediction quality.
...
TL;DR: ADAPT is an attention-based framework that reduces hallucination in MLLMs by detecting and correcting text-to-image cross-attention drift during generation, achieving 40-60% hallucination reduction across multiple backbones.


<details>
  <summary>Details</summary>
Motivation: MLLMs suffer from hallucination (generating content inconsistent with images). Existing mitigation methods are outcome-driven and do not explicitly address the internal failure mode of progressive degradation of text-to-image cross-attention during generation.

Method: Three components: (1) cross-attention visual anchor refined from early decoding for stable spatial grounding, (2) attention-supervised inference mechanism that detects and corrects attention drift online, (3) Visual Attention Guidance DPO that aligns preferences toward visually grounded responses.

Result: Each component contributes to hallucination reduction; full framework achieves 40%-60% reduction in hallucination rates across mainstream backbones while preserving general multimodal capabilities.

Conclusion: Attention-based perspective effectively mitigates hallucinations by directly intervening on the internal text-to-image cross-attention behaviors of MLLMs.
...
TL;DR: This paper bridges physics-based inverse rendering and diffusion models by using diffusion predictions as a similarity kernel (regularization loss) rather than direct targets, enabling joint reconstruction of geometry, materials, and illumination that is both physically accurate and visually plausible for relighting.


<details>
  <summary>Details</summary>
Motivation: Physics-based inverse rendering is accurate but severely underconstrained (illumination gets baked into materials), while data-driven diffusion models predict visually plausible materials but don't satisfy the rendering equation. The authors aim to bridge these two paradigms to get the best of both worlds.

Method: The key idea is to treat diffusion model predictions as a similarity kernel (regularization loss) that penalizes material deviations where diffusion predictions are near-constant, while allowing optimization to match input images elsewhere. This regularizer is built into an end-to-end pipeline jointly reconstructing geometry, materials, and illumination.

Result: On Synthetic4Relight, Stanford-ORB, and DTC-Synthetic datasets, the method significantly outperforms state-of-the-art baselines in both reconstruction accuracy and relighting quality.

Conclusion: The proposed regularizer effectively bridges physics-based and data-driven approaches, producing high-quality 3D assets that work in standard rendering pipelines and support faithful relighting.
...
TL;DR: Proposes a hybrid UNet-Transformer architecture as an efficient surrogate for FEM to predict stress and strain fields in hierarchical composite microstructures, achieving strong accuracy (R² up to 0.9991) across diverse geometries, with physically meaningful attention patterns, though performance degrades on irregular geometries with sharp stress discontinuities.


<details>
  <summary>Details</summary>
Motivation: Accurate stress/strain field prediction in composite microstructures is critical for material design, but conventional FEM simulations are computationally prohibitive (minutes to days per evaluation), motivating the need for an efficient surrogate model.

Method: A hybrid UNet-Transformer architecture that predicts mechanical field distributions directly from microstructure geometry images, trained across ten stress/strain field types, diverse tessellations (square, hexagonal, triangular), multiple boundary conditions, and high-resolution geometries.

Result: Peak accuracy: R²=0.9991, SSIM=0.9936, MAE=0.0050 on periodic tessellations; MAE < 0.05 on 6/8 subdatasets. Attention analysis (Grad-CAM/Grad-CAM++) shows physically meaningful localization at phase boundaries, ligament junctions, and contact zones. Degradation on irregular geometries (S11: R²=0.7735, SSIM=0.7126) due to sharp stress discontinuities.

Conclusion: The hybrid UNet-Transformer serves as an effective FEM surrogate for composite microstructure analysis, learning physically meaningful representations, though limitations remain in handling sharp discontinuities in irregular geometries—consistent with known limitations of smooth-loss image translation models.
...
TL;DR: A zero-shot semantic navigation framework using online Hierarchical 3D Scene Graphs (HSG) and hierarchical belief-based planning to enable globally consistent long-distance navigation decisions.


<details>
  <summary>Details</summary>
Motivation: Current semantic navigation methods lack structured global representations, causing inefficient exploration and myopic behaviors in long-distance navigation due to reliance on local observations and greedy strategies.

Method: Proposes an online Hierarchical 3D Scene Graph (HSG) for multi-granular semantic topology, combined with a hierarchical belief-based planning framework that fuses semantic priors with exploration evidence and performs finite-horizon rollouts on an HSG-based simulator to estimate long-term returns of macro-actions.

Result: Outperforms state-of-the-art methods in long-distance scenarios, improving SR and SPL by 9.4% and 5.0% respectively across multiple tasks and datasets.

Conclusion: Structured hierarchical semantic representations with belief-based planning enable globally consistent zero-shot navigation, significantly reducing redundant backtracking in long-distance tasks.
...
TL;DR: AnyMatch generates large-scale synthetic multi-modal training data from single-view images using monocular depth, 3D reprojection, diffusion inpainting, and cross-modal translation, outperforming existing datasets on multi-modal matching benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-modal datasets are limited by high cost, lack of diversity, and 3D geometric errors from SfM-MVS pipelines; synthetic data suffers from poor geometric consistency or photorealism.

Method: AnyMatch uses monocular depth estimation → 3D reprojection → diffusion-based inpainting → cross-modal image translation to create multi-view, multi-modal image pairs with guaranteed 3D geometric consistency.

Result: Networks (LoFTR, EDM, RoMa) fine-tuned on the Any-syn dataset show significant gains on multi-modal benchmarks with better generalization and robustness.

Conclusion: AnyMatch provides a scalable, low-cost solution for generating geometrically consistent multi-modal training data, advancing multi-modal image matching research.
...
TL;DR: This paper proposes shifting AIGI detection from static generalization to dynamic adaptation. The Fleet framework enables few-shot evolution via constrained routing correction, restoring performance from 20.4% to 73.1% on modern generators.


<details>
  <summary>Details</summary>
Motivation: Current AIGI detection methods rely on static feature spaces that fail catastrophically against rapidly evolving generators (e.g., SD3, Nano Banana Pro). There is a need to move beyond static generalization to dynamic adaptation.

Method: Fleet replaces unconstrained feature updates with constrained routing correction. It uses avoidance routing to redirect novel AI samples away from Non-AI-dominated routes within decoupled subspaces, enabling continuous few-shot evolution.

Result: Static SOTA methods fail catastrophically on modern generators. Fleet restores performance from 20.4% to 73.1% with only 10-shot adaptation on "Doubao Seedream 4.0". The Treasure benchmark covers 64 models and 360k images.

Conclusion: The field should expand from "static generalization" to "dynamic adaptation" for AIGI detection. Fleet's constrained routing correction paradigm offers a promising direction for open-world adversarial defense.
...
TL;DR: CasaMaestro is a feedforward model that reconstructs metric 3D point clouds of entire houses from only 20-50 sparse multi-view indoor panoramas, addressing the limitations of pinhole-camera pipelines for large indoor spaces.


<details>
  <summary>Details</summary>
Motivation: Home-deployed embodied AI systems need fast, metric 3D reconstruction of residential spaces, but existing pinhole-camera pipelines are inefficient for large indoor residences due to limited field of view, requiring thousands of images and suffering from drift.

Method: A feedforward model that takes 20-50 sparse multi-view indoor panoramas as input and directly predicts metric depth along with camera poses for fast point-cloud reconstruction.

Result: CasaMaestro provides high-quality results in both real-world and synthetic scenes, achieving robust house-scale reconstruction with full coverage.

Conclusion: CasaMaestro is the first model to support house-scale reconstruction with multi-view panoramas, serving as a strong foundation for acquiring house-scale 3D indoor assets for close-loop simulation applications.
...
TL;DR: InterTalk is a real-time, motion-based framework for conversational talking face generation that supports flexible multi-round dialogues, natural non-verbal feedback, and efficient 30 FPS performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to balance flexibility (multi-round, arbitrary participant count), naturalness (coherent motion and non-verbal feedback), and efficiency (real-time, low computation) in conversational talking face generation.

Method: InterTalk uses a motion-based architecture with explicit modeling of multi-round conversational dynamics, motion feedback from multiple participants, iterative generation strategy, and disentanglement of motion into facial components (e.g., lip sync, eye blinking). A new multi-person conversational dataset with 3D face-based data augmentation is also introduced.

Result: Achieves superior interaction quality and real-time generation at 30 FPS, outperforming existing methods in balancing flexibility, naturalness, and efficiency.

Conclusion: InterTalk effectively bridges the gap in interactive conversational face generation by delivering a framework that is flexible, natural, and efficient for real-time multi-participant dialogues.
...
TL;DR: ART proposes a two-stage makeup transfer framework that shifts supervision from synthetic pseudo-targets to real references via a reality-anchored refinement cycle, achieving superior fidelity and identity preservation. It also introduces the MF2K dataset of 8,573 high-resolution makeup portraits.


<details>
  <summary>Details</summary>
Motivation: Makeup transfer suffers from lack of real paired training data. Existing methods using weak priors or synthetic pseudo-targets produce degraded details, artifacts, and identity drift.

Method: Two-stage framework: Stage I initializes with pseudo-targets for basic alignment; Stage II shifts supervision to real reference via differentiable cycle reconstructing from bare-skin counterpart to penalize omissions and override artifacts.

Result: Superior makeup fidelity, strong background stability, robust identity preservation, especially for complex makeup styles. Also introduces MF2K dataset (8,573 2K-resolution images).

Conclusion: Anchoring supervision on real (not synthetic) references significantly improves makeup transfer quality, and high-resolution datasets like MF2K are valuable for advancing this task.
...
TL;DR: SPMF framework preserves vascular topology during cross-modal image translation via orthogonality-constrained flow and prototype-guided refinement, achieving state-of-the-art results on NIRII-to-2PF and fundus datasets.


<details>
  <summary>Details</summary>
Motivation: Existing cross-modal translation methods preserve structure only as a final output property rather than an invariant of the generative process, leading to structural discontinuities and artifacts that compromise clinical reliability.

Method: 1) Formulates vascular image translation as topology-invariant transport with an orthogonality constraint on the flow velocity field. 2) Implements the constraint as a time-weighted surrogate objective within a Brownian bridge diffusion model. 3) Proposes a Prototype-Guided Structural Refinement (PGSR) module to align degraded inference structures with reliable training structures.

Result: Achieves peak PSNR of 24.96 dB on NIRII-to-2PF and 24.83 dB on fundus datasets, outperforming state-of-the-art methods consistently.

Conclusion: The SPMF framework effectively separates appearance transport from topological distortion, generating anatomically faithful vascular structures with improved clinical reliability.
...
TL;DR: Horizon3D is a sparse radar-camera fusion framework for long-range 3D object detection that uses Gaussian primitives and sparse BEV features, achieving state-of-the-art results on TruckScenes (+3.0 NDS, +1.6 mAP).


<details>
  <summary>Details</summary>
Motivation: Long-range 3D detection is critical for highway-speed autonomous driving, but existing radar-camera fusion methods struggle at extended ranges: BEV methods lose fine-grained detail due to high computation, and query-based methods lack sufficient scene-level context.

Method: Horizon3D uses Keypoint-Guided Gaussian Initialization, Object-Centric Sparse Fusion, and Dual-Path Temporal Fusion (BEV path for accumulation, Gaussian path for motion propagation) to combine Gaussian primitives with sparse BEV features.

Result: On TruckScenes validation set, Horizon3D achieves state-of-the-art performance, outperforming the previous best method by +3.0 NDS and +1.6 mAP while maintaining competitive inference speed.

Conclusion: Horizon3D effectively addresses the long-range detection challenge by fusing object-level Gaussian detail with scene-level BEV context and dual-path temporal fusion, setting a new SOTA for radar-camera 3D detection.
...
TL;DR: PiLoT v2 replaces heavy 3D mesh-based geo-localization with lightweight 2.5D ortho-map registration, achieving comparable accuracy with drastically reduced storage and computation.


<details>
  <summary>Details</summary>
Motivation: PiLoT v2 aims to overcome the storage, acquisition, and rendering bottlenecks of PiLoT's 3D mesh-based approach, enabling real-time UAV geo-localization on embedded platforms in GNSS-denied environments.

Method: Shifts from pixel-to-3D rendering to direct pixel-to-orthogonal map registration using TDOMs/DSMs; replaces GPU rendering with CPU-friendly map cropping; trains a cross-view registration network on a large-scale geometrically annotated dataset; integrates gravity direction and single-point laser range priors into pose optimization.

Result: PiLoT v2 achieves performance comparable to or exceeding PiLoT, with drastically lower storage and computational costs.

Conclusion: Lightweight 2.5D map-based geo-localization can effectively replace heavy 3D mesh approaches without sacrificing accuracy, enabling practical deployment on resource-constrained platforms.
...
TL;DR: View-PNDF is a parameter-efficient framework for radiology report generation that detects and fine-tunes view-specific neurons in models to produce consistent diagnoses across multi-view X-ray images, then uses LLMs to consolidate them into complete reports, achieving better performance with reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: Existing RRG methods use direct feature fusion for multi-view X-ray images, which overlooks clinical inconsistencies and inaccuracies when a single model processes different views, adversely impacting performance and clinical reliability.

Method: View-PNDF consists of (i) view-specific neuron detection module, (ii) verification module quantifying neuron existence, and (iii) selective fine-tuning strategy that strengthens detected neurons while preserving view-agnostic representations. LLMs consolidate view-specific reports into a complete report.

Result: Extensive experiments on two medical RRG benchmarks show View-PNDF substantially improves view-specific chest X-ray report generation quality while maintaining robust general-view performance.

Conclusion: View-PNDF effectively addresses multi-view inconsistencies in RRG by targeting view-specific neurons, achieving consistent diagnoses with reduced computational costs, and enables both NLG-based and LLM-based clinical assessment.
...
TL;DR: TaxoMIL reformulates WSI diagnosis as a taxonomy-constrained multi-granularity text generation task using a dual-head Transformer decoder, outperforming existing MIL and VLM methods on three datasets.


<details>
  <summary>Details</summary>
Motivation: Current MIL methods treat WSI diagnosis as flat classification over discrete labels, ignoring the hierarchical nature of clinical reasoning. Recent hierarchical/VLM approaches fail to capture semantic continuity or produce taxonomic hallucinations with parent-child label violations.

Method: TaxoMIL uses a dual-head Transformer decoder to generate coarse- and fine-level diagnostic text, with taxonomy-guided objectives that structure the label embedding space and ground visual representations within the clinical taxonomy.

Result: Consistently outperforms state-of-the-art MIL classifiers and VLM-based generative methods across three diverse WSI datasets, yielding accurate and hierarchy-aware diagnostic predictions.

Conclusion: Taxonomy-constrained multi-granularity text generation is an effective paradigm for WSI diagnosis that better aligns with clinical hierarchical reasoning while avoiding hallucinations and structural violations.
...
TL;DR: InfiniVerse is a unified pipeline for generating long-range, controllable, and temporally coherent dynamic urban scenes from a single multi-view frame, achieving state-of-the-art performance on Waymo and nuScenes datasets (FID 6.4, FVD 67.97).


<details>
  <summary>Details</summary>
Motivation: Generating realistic, controllable, and temporally coherent urban environments is critical yet unresolved in autonomous driving.

Method: (1) Reconstructs 3D occupancy from input multi-view frame; (2) Autoregressive scene extension along arbitrary trajectories; (3) Video diffusion model for realistic spatiotemporal video; (4) Hierarchical sketch-and-refine paradigm with re-projected video feedback for cross-modal alignment.

Result: State-of-the-art on Waymo Open Dataset and nuScenes; FID 6.4 and FVD 67.97, outperforming existing benchmarks in duration and stability.

Conclusion: InfiniVerse enables long-range, 2D-3D-aligned, controllable dynamic urban scene synthesis from a single frame, with mutual enhancement between visual and spatial domains.
...
TL;DR: This paper uses neural Jacobian fields with self-supervised learning to generate realistic human avatars with clothing dynamics from monocular video, introducing constrained Poisson solver, signed distance-based Jacobian regularization, and deformation-guided residual flow loss to handle occlusions and improve temporal consistency.


<details>
  <summary>Details</summary>
Motivation: Generating realistic human avatars in complex motions (e.g., clothing dynamics) requires modeling global and local deformations, which remains challenging in monocular settings due to self-occlusions and invisible surfaces.

Method: The method uses neural Jacobian fields (NJFs) for semi-rigid deformation representation, trained via self-supervised learning by solving a Poisson equation. Three key components: (1) constrained Poisson solver, (2) signed distance-based Jacobian regularization, (3) deformation-guided residual flow loss.

Result: Outperforms state-of-the-art methods on benchmark and in-the-wild videos, producing temporally stable and geometrically coherent avatars.

Conclusion: The proposed approach effectively addresses monocular challenges (occlusions, boundary artifacts) for realistic avatar animation with clothing dynamics.
...
TL;DR: WildProp is a training-free framework that estimates wildlife body proportion distributions from unconstrained image repositories using retrieval-driven correspondence, achieving 10-20% median relative errors across diverse taxa without per-species training.


<details>
  <summary>Details</summary>
Motivation: Traditional morphometric measurements require controlled imaging or physical handling, which limits scalability. The authors aim to enable population-level morphometric analysis directly from large-scale, unconstrained web image repositories.

Method: Given a single user-annotated canonical image, WildProp performs pose-aware retrieval using foundation model features, transfers part endpoints via dense patch-level matching, filters predictions with geometric consistency, and aggregates measurements across retrieved images to estimate population-level ratio distributions.

Result: Evaluations on three datasets (birds and amphibians) show median relative errors of 10-20%. Case studies demonstrate applicability across birds, frogs, insects, and flowers. Ablations confirm pose-aware retrieval is critical and robust aggregation mitigates noise.

Conclusion: Carefully curated 2D correspondences over web-scale imagery can serve as scalable morphometric proxies for comparative and subgroup analyses across taxa, geography, and seasonality.
...
TL;DR: This paper introduces SkillSpotter, a pose-aware multi-view architecture for jointly detecting and grading skilled actions in ego-exo video settings, achieving significant improvements (e.g., +76% mAP) over state-of-the-art baselines on Ego-Exo4D and HoloAssist benchmarks.


<details>
  <summary>Details</summary>
Motivation: To enable personalized, real-time coaching using AR glasses or fixed cameras in domains like sports, cooking, or music, systems must understand not just what a person does but how well they execute an activity - requiring simultaneous detection and grading of skilled actions in ego-exo video settings.

Method: SkillSpotter introduces a pose-aware multi-view architecture with three modules: (1) adaptive temporal suppression for handling varying action densities, (2) gated 3D body pose fusion to incorporate body kinematics, and (3) bidirectional cross-view attention for combining ego and exo views effectively.

Result: SkillSpotter improves class-specific mAP from 12.40 to 21.82 (+76%) and balanced accuracy from 55.99% to 60.40% over the best baseline. Modules transfer to other models with consistent gains, and the method generalizes to HoloAssist.

Conclusion: Existing temporal action detection methods grade near-randomly on the proficiency benchmark, while SkillSpotter's joint detection-grading approach with pose-aware multi-view design significantly outperforms baselines and generalizes across datasets.
...
TL;DR: LINet introduces a multi-stream neural network with continuous cross-modal fusion via a novel LIConv2d operator, addressing initialization pathologies and modality dropout for RGB-D scene classification, achieving State-Of-The-Art results on SUN RGB-D.


<details>
  <summary>Details</summary>
Motivation: Current multi-modal fusion methods (early, late, hybrid/attention) treat fusion as a discrete, ad-hoc event, requiring architectural guesswork and suffering from premature entanglement or excessive isolation of modalities.

Method: Three parallel streams (RGB, depth, integration) with LIConv2d enabling continuous cross-modal learning at every layer, 1/N constant initialization to fix gradient pathology, and progressive modality dropout to prevent pathway collapse.

Result: 45.2% mean class accuracy on SUN RGB-D 19-class (ResNet18, from scratch), 49.6% with ScanNet pretraining, outperforming prior from-scratch results.

Conclusion: Continuous integration fusion is more effective than discrete fusion strategies, but requires careful initialization and curriculum-based dropout to avoid gradient corruption and pathway collapse.
...
TL;DR: FROST is a training-free few-shot segmentation method for remote sensing that treats reference features as point clouds on a unit sphere and uses nonparametric density ratio estimation with DINOv3 features, outperforming both training-free and learning-based methods across 17 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Few-shot segmentation in remote sensing is challenged by scarce labels and the domain gap between natural images (where backbones are pretrained) and overhead imagery. Existing methods either overfit to the training distribution or use lossy class summaries (single prototypes, clusters) that fail to preserve the internal multimodal structure of a class.

Method: FROST uses frozen DINOv3 features to represent reference foreground/background as two point clouds on the unit sphere. Query tokens are labeled via a nonparametric density ratio, with a Bayes-optimal threshold at zero under equal priors. Kernel bandwidth and spatial gating are read from the support set. No training is required.

Result: Across 17 remote-sensing benchmarks, FROST surpasses both training-free and learning-based methods. It leads by 5.6 mIoU from a single annotated example and widens its lead as the support set grows, while remaining among the smallest models.

Conclusion: A class is better described by a distribution than a point/point summary. Frozen self-supervised features carry enough structure to estimate that distribution directly, enabling effective training-free few-shot segmentation for remote sensing.
...
TL;DR: WaterGen generates realistic, diverse underwater images by decoupling scene content generation from water medium effects using a latent diffusion framework, significantly improving downstream underwater vision tasks.


<details>
  <summary>Details</summary>
Motivation: Underwater computer vision tasks suffer from a scarcity of large-scale, diverse training data, and existing methods fail to simultaneously achieve realism, diversity, and independent control of scene content and water medium effects.

Method: Two-stage approach: (1) Fine-tune latent diffusion U-Net on degradation-free underwater images to generate diverse scene embeddings; (2) Apply physically accurate medium degradation synthesis as a conditional decoding process on those embeddings, enabling decoupled control of scene and water effects.

Result: WaterGen produces large-scale synthetic underwater datasets with diverse scene structures and accurate water effects, consistently improving downstream performance in underwater restoration and semantic segmentation tasks.

Conclusion: Decoupling scene generation from medium modeling within a latent diffusion framework achieves both diversity and controllability, enabling effective synthetic data generation that boosts real-world underwater vision performance.
...
TL;DR: PruneGround is a plug-and-play framework for 3D Visual Grounding that prunes irrelevant spatial regions using a VLM, decomposes complex expressions via multi-view reasoning, and fine-tunes a spatial LLM for accurate object localization, achieving SOTA results on ScanRefer and Nr3D/Sr3D benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing 3DVG methods reason over entire 3D scenes, causing ambiguous predictions and high computational cost in cluttered environments, despite many referential expressions only requiring local spatial context.

Method: Three components: (1) Language-Guided Spatial Pruning (LGSP) uses a frozen VLM to identify language-relevant regions; (2) MultiView-Conditioned Description Reformulation (MCDR) decomposes complex expressions and augments spatial cues; (3) LLM-Grounder adapts a detection-pretrained spatial LLM for language-conditioned grounding within pruned regions.

Result: Achieves state-of-the-art on all three ScanRefer settings and 9 out of 10 Nr3D/Sr3D settings.

Conclusion: PruneGround effectively reduces search space and improves grounding accuracy by exploiting local spatial context, outperforming prior methods on multiple benchmarks. Code and models are publicly available.
...
TL;DR: FAT is a framework where specialized models generate structured predictions and foundation models refine them through proxy tasks (selection/verification), outperforming both pure specialized models and direct foundation-model regression across detection, prediction, and segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: Foundation models excel at reasoning but lack precise geometric/numerical estimation, while specialized models are strong at structured prediction but lack contextual reasoning. The key question is whether these should compete or collaborate.

Method: FAT decomposes structured prediction into three components: (1) specialist prediction (geometrically valid hypotheses), (2) information-space reconstruction (multimodal candidates), (3) foundation-model proxy reasoning (selection or verification over candidates). Implemented as ProxySelect with a VLM.

Result: ProxySelect consistently improves specialized baselines across 2D/3D object detection, trajectory prediction, and semantic segmentation, while substantially outperforming direct foundation-model regression at lower computational cost.

Conclusion: Specialized models and foundation models should collaborate through task decomposition: specialists preserve task-specific structure, foundation models refine hypotheses via contextual proxy reasoning.
...
TL;DR: This paper proposes a two-reasoner system (routine + deliberative) using speculative decoding and techniques like FlatRoPE and Action-aware RL to reduce reasoning latency in VLA planners for autonomous driving by ~4x.


<details>
  <summary>Details</summary>
Motivation: Autoregressive CoC reasoning in VLA planners dominates inference latency, which is a critical constraint for autonomous driving. The goal is to accelerate the reasoning step without sacrificing quality.

Method: Splits CoC reasoning into a routine reasoner (predictable tokens attending to trajectory history, using FlatRoPE for positional encoding) and a deliberative reasoner (novel cases attending to visual evidence, based on speculative decoding). Additionally introduces Action-aware RL (AARL) for post-training with action-quality reward and static-reference KL anchor.

Result: The two-reasoner system reduces reasoning-step running time by approximately 4x compared to the original Alpamayo planner.

Conclusion: Specializing reasoning paths (routine vs. deliberative) and using FlatRoPE and AARL can significantly accelerate VLA planning without compromising on novel case handling.
...
TL;DR: This paper demonstrates that Scene Coordinate Regression (SCR) models, despite being claimed as privacy-preserving due to implicit scene encoding, are vulnerable to query-based attacks that can reconstruct 3D geometry and appearance information of training environments.


<details>
  <summary>Details</summary>
Motivation: SCR methods are increasingly used for visual localization and are often assumed to be privacy-preserving because the scene is encoded implicitly in network parameters rather than stored explicitly. The authors aim to disprove this assumption and reveal the real privacy risks.

Method: The authors introduce a query-based attack that repeatedly queries an SCR model with proxy images unrelated to the target scene to obtain dense pixel-wise 3D coordinates. They identify reliable points by checking stability under small input perturbations, refine them in white-box settings, and accumulate stable points across independent query batches to reconstruct scene geometry. They also invert network features to synthesize images from arbitrary viewpoints.

Result: Experiments on indoor and outdoor datasets show that substantial portions of training environments can be reconstructed with high geometric fidelity. Beyond geometry, approximate color appearance is also recovered, revealing recognizable layout and potentially sensitive scene elements.

Conclusion: The paper directly contradicts claims that SCR representations are privacy-preserving by design, revealing a real risk when such systems are deployed in private or security-critical spaces.
...
TL;DR: This paper introduces S-OBI, a benchmark for evaluating multimodal large language models (MLLMs) on sentence-level oracle bone inscription understanding. It synthesizes clean, standardized visual inputs from expert-verified rubbings, designs three tasks (semantic matching, slot extraction, contextual reasoning) with 695 QA pairs, and finds that current MLLMs perform poorly—primarily due to character-level recognition errors propagating to sentence-level reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing OBI research focuses on isolated character recognition, ignoring the long-form textual coherence and contextual dependencies in complete divination charges. There is a need to evaluate whether MLLMs can move beyond character-level to structured inscription-level understanding.

Method: S-OBI synthesizes sentence-level OBI instances by replacing characters in expert-verified rubbings with clean glyph samples from existing datasets, preserving structure and semantics. Three tasks are designed: semantic matching, semantic slot extraction, and contextual reasoning, yielding 695 QA pairs for evaluation.

Result: Current MLLMs show inferior performance on sentence-level OBI understanding. Visual perception errors in unmasked regions propagate through reasoning chains, causing erroneous predictions for masked characters, indicating strong dependency on character-level recognition.

Conclusion: S-OBI serves as a diagnostic benchmark revealing that MLLMs still struggle with structured inscription-level understanding and remain heavily reliant on character-level recognition, highlighting a critical gap for future research.
...
TL;DR: HSDF-Lane proposes a Height-aligned Signed Distance Field for implicit 3D road surface modeling and Lane-aware Semantic Positional Encoding to inject lane priors into transformers, achieving SOTA on OpenLane for 3D lane detection and height map estimation.


<details>
  <summary>Details</summary>
Motivation: Prior methods either assume flat ground (causing geometric distortion) or rely on sparse anchor-based regression that fails to leverage recovered geometry for semantic understanding. The authors aim to overcome these limitations by jointly modeling geometry and semantics.

Method: 1) HSDF: Implicitly models the road surface as a Height-aligned Signed Distance Field over a densely sampled 3D feature volume. Differentiable rendering produces both accurate height maps and surface-aligned features. 2) LSPE: Lane-aware Semantic Positional Encoding injects lane-existence priors derived from surface-aligned features into transformer queries, coupling geometry with semantics.

Result: State-of-the-art performance on the OpenLane benchmark for both 3D lane detection and height map estimation tasks.

Conclusion: Implicit 3D surface modeling (HSDF) combined with geometry-aware semantic encoding (LSPE) effectively addresses depth ambiguity and geometric distortion in monocular 3D lane detection.
...
TL;DR: GaussianMap proposes an adaptive Gaussian representation for online HD map construction, replacing fixed-resolution BEV grids with learnable Gaussian primitives that flexibly allocate capacity to map-relevant regions, achieving SOTA results on nuScenes and Argoverse 2.


<details>
  <summary>Details</summary>
Motivation: Existing methods use fixed-resolution dense BEV grids to represent surrounding scenes, but map elements are spatially sparse and require fine-grained geometric localization. Uniformly allocated BEV representations are thus redundant and less effective for vectorized map prediction.

Method: GaussianMap learns a set of Gaussian primitives on the BEV plane, each encoding a flexible local region with geometric properties and a feature vector. A feed-forward Gaussian encoder progressively refines these primitives through Gaussian interaction modeling and multi-sensor feature aggregation. The refined representation is then splatted into a BEV feature map and decoded into vectorized map predictions.

Result: Achieves state-of-the-art performance on nuScenes and Argoverse 2 datasets in both camera-only and camera-LiDAR fusion settings.

Conclusion: The adaptive Gaussian representation effectively allocates representational capacity to map-relevant sparse regions, outperforming fixed-resolution BEV-based methods for online HD map construction.
...
TL;DR: The paper introduces UCF101-AD, a benchmark testing whether MLLMs can recognize when an action is NOT happening despite strong contextual cues. Current models perform well on positive action recognition (>85%) but collapse on denial tasks (<50%), revealing a critical blind spot in causal reasoning about motion. The authors propose CausalAct, a causal graph formulation using natural-language prompts, which significantly reduces false positives.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs achieve impressive video understanding but have an unexplored blind spot: they cannot reliably deny an action when contextual cues strongly suggest it should be happening. The authors aim to diagnose and address this lack of causal reasoning about whether motion actually occurs versus being contextually plausible.

Method: The authors create UCF101-AD, a benchmark with paired Action-Presence (positive) and Action-Denial (negative) clips where negative clips share identical context (persons, objects, locations) but the defining action is absent. They evaluate 20 state-of-the-art MLLMs and propose CausalAct, a causal graph formulation that uses natural-language prompts to explicitly link context, interaction, and motion in the scene structure.

Result: All 20 evaluated MLLMs exceed 85% accuracy on positive action classes but collapse below 50% on action-denial counterparts, showing a strong bias to affirm plausible actions. Incorporating CausalAct causal cues into prompts substantially reduces false positive rates.

Conclusion: Action denial is a distinct and learnable reasoning skill that current MLLMs lack. Causal reasoning about motion can be improved through structured natural-language prompting. UCF101-AD offers a diagnostic lens for advancing causal understanding in multimodal models.
...
TL;DR: A temporally consistent learning framework for real-time prostate segmentation in TRUS video that distills temporal coherence into a 2D network, using confidence-weighted temporal consistency and dual-scale prototype alignment, achieving real-time speed with state-of-the-art accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing 2D segmentation methods ignore temporal context causing inter-frame inconsistencies, while 3D methods are too slow for real-time use. The clinical observation that the prostate is geometrically stable but acoustic surroundings fluctuate motivates a need for temporally coherent yet efficient 2D segmentation.

Method: (1) Confidence-Weighted Temporal Consistency objective using optical flow warping residuals to selectively focus on stable prostate regions; (2) Dual-scale Prototype Alignment Module enforcing semantic coherence via contrastive learning on local boundary and global features; (3) Geometric equivariance-based pseudo-labeling with knowledge distillation to avoid dense video annotations.

Result: State-of-the-art accuracy and temporal consistency on SUN-SEG and a new TRUS-V benchmark (2,679 frames) at real-time inference speed.

Conclusion: The proposed framework effectively resolves the dilemma between temporal consistency and real-time efficiency for prostate TRUS video segmentation, validated by extensive experiments.
...
TL;DR: ExPLoRe uses Soft MoE dispatch weights as learned per-patch loss coefficients for multi-objective masked image modeling, enabling content-dependent specialization through loss-coupling, achieving competitive results on ImageNet-1K and ADE20K.


<details>
  <summary>Details</summary>
Motivation: Existing multi-objective MIM methods use global scalar weights for combining complementary learning signals (token distillation, CLS alignment, pixel reconstruction), ignoring spatial heterogeneity across different patches in an image.

Method: ExPLoRe repurposes Soft MoE dispatch weights as per-patch loss coefficients, with loss-coupling allowing gradients to flow through dispatch weights to the router, enabling content-dependent specialization of loss weights across patches.

Result: On ImageNet-1K with ViT-Base: Token+CLS improves +0.5% k-NN, +4.4% linear probe; Token+Pixel improves +2.2% k-NN. Achieves 80.6% linear probe and 85.3% finetuning accuracy. Downstream recipes (Freeze Routing, Expert Dropout, Freeze Attention) improve MoE finetuning by +1.5% and close 2.5-2.9 mIoU segmentation gap on ADE20K.

Conclusion: ExPLoRe effectively introduces patch-level adaptive weighting for multi-objective MIM via MoE dispatch weights and loss-coupling, outperforming non-MoE baselines and enabling competitive downstream transfer.
...
TL;DR: AC3S is a diffusion-based framework that uses adaptive conditioning and multi-agent VLMs to generate 3D-consistent, photorealistic synthetic images without over-conditioning artifacts.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based synthetic data generation methods struggle with enforcing precise 3D structure and pose consistency, and often suffer from over-conditioning artifacts that degrade realism and limit dataset quality.

Method: A self-supervised visual prompt modulator dynamically adjusts ControlNet conditioning strength to prevent over-conditioning, paired with a multi-agent VLM framework that generates detailed, 3D-aware prompts aligned with geometric structure.

Result: The method significantly improves image quality and downstream utility over existing approaches, enabling scalable generation of high-quality synthetic datasets with accurate 2D and 3D annotations.

Conclusion: AC3S effectively balances 3D structural alignment and photorealism via adaptive conditioning, offering a scalable solution for high-quality synthetic data generation in computer vision.
...
TL;DR: AA is a new multi-view multimodal gaze estimation dataset with 10 cameras (8 screen-mounted + 2 side-view), offering synchronized facial observations and screen-space gaze targets for reproducible research.


<details>
  <summary>Details</summary>
Motivation: Existing gaze estimation datasets are limited to single-view, which fails to handle viewpoint variation and occlusion. AA aims to provide multi-view coverage for more robust gaze estimation.

Method: The dataset collects data from 8 fixed screen-mounted cameras and 2 side-view cameras, capturing synchronized facial observations with structured facial region crops, under controlled fixation conditions.

Result: AA provides subject-independent evaluation splits and a standardized data processing pipeline, enabling multimodal learning from both global and local visual cues for robust gaze estimation.

Conclusion: AA fills the gap of multi-view gaze estimation datasets, supporting reproducible research and more robust modeling under viewpoint variation and occlusion compared to single-view datasets.
...
TL;DR: CooperScene is a high-fidelity cooperative autonomy dataset with real-world C-V2X communication characterization, featuring 3 CAVs, 1 RSU, multi-modal sensors, and 59K frames with 344K objects across diverse scenes.


<details>
  <summary>Details</summary>
Motivation: Existing cooperative perception datasets overlook real-world deployment complexities like limited/dynamic communication bandwidth, heterogeneous sensing modalities, and scalability beyond a single partner.

Method: The dataset collects data from 3 CAVs and 1 RSU with multi-modal sensors and commercial C-V2X radios across diverse scenes (intersections, highway ramps, parking lots), with tight synchronization, centimeter-level localization, precise calibration, and 3GPP-compliant communication.

Result: 344K objects across 59K frames at 10 Hz with globally consistent 3D labels.

Conclusion: CooperScene provides a rigorous benchmark for evaluating multi-agent scaling and real-world deployable cooperative autonomy performance.
...
TL;DR: ForgeDrive is a unified autoregressive diffusion framework for autonomous driving that flips the traditional "imagine-then-act" paradigm into "act-then-imagine", using cross-conditioned visual-action generation to prevent error cascading from visual prediction to action planning, achieving strong results on NAVSIM across multiple tasks.


<details>
  <summary>Details</summary>
Motivation: The prevailing "imagine-then-act" paradigm in world-model-based autonomous driving allows errors from the more challenging visual generation stage to cascade into action planning, degrading overall performance.

Method: ForgeDrive uses a unified autoregressive diffusion framework with visual-action cross-conditioning. It factorizes future as per-timestep frame-action pairs, decouples diffusion timesteps of two modalities (UniDiffuser-style), and proposes an "act-then-imagine" inference paradigm with future ego-status prediction.

Result: Extensive experiments on NAVSIM show ForgeDrive unifies driving simulation, planning, and visual odometry into a single model, outperforming existing strong planners without post-training strategies.

Conclusion: The act-then-imagine paradigm effectively prevents error cascading, where generated actions improve future frame generation accuracy, which in turn enhances subsequent action quality, creating a virtuous cycle.
...
TL;DR: This paper introduces UHD-MFF, the first large-scale ultra-high-resolution multi-focus fusion dataset, along with UMF-LUT, a scale-specialized lookup-table framework that enables real-time 4K multi-focus image fusion on smartphones with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Existing multi-focus image fusion is limited to low-resolution images and faces three major barriers in ultra-high-definition (UHD) scenarios: data availability, model adaptability, and deployment feasibility.

Method: Proposes UMF-LUT, a scale-specialized lookup-table framework with two components: Coarse-Region Lookup Table (C-LUT) for low-resolution region-level decision-making using gradient and semantic cues, and Detail-Edge Lookup Table (D-LUT) for high-resolution edge-level decision-making using Laplacian cues.

Result: Outperforms state-of-the-art methods in both visual fidelity and quantitative metrics; enables real-time 4K multi-focus fusion with strong deployability on smartphones.

Conclusion: The work effectively advances multi-focus image fusion toward ultra-high-resolution imaging scenarios by shattering barriers of data, model adaptability, and deployment.
...
TL;DR: This paper proposes a hyperbolic surgical video-language pre-training framework that preserves the hierarchical structure of surgical knowledge (actions, steps, phases) by mitigating structural false negatives and enforcing cross-level semantic consistency, achieving gains in zero- and few-shot phase recognition.


<details>
  <summary>Details</summary>
Motivation: Prior surgical vision-language models collapse multi-dimensional, hierarchical surgical knowledge (fine-grained actions, mid-level steps, global procedural context) into a single flat embedding space, ignoring cross-level semantic containment and weakening long-range dependency modeling.

Method: A hyperbolic surgical video-language pre-training framework that explicitly preserves hierarchical structure by mitigating structural false negatives from procedural context and enforcing semantic consistency between parent phases and their constituent child steps.

Result: Consistent gains in zero- and few-shot phase recognition across multiple surgical benchmarks and different procedures/institutions.

Conclusion: Preserving the inherent hierarchical structure of surgical knowledge (via hyperbolic embedding and cross-level consistency) is more effective than flat embedding approaches for surgical phase recognition, especially in low-data scenarios.
...
TL;DR: XInsight Lab won first place in Track 3 of the 4th EI-MIGA-IJCAI Challenge (0.76923 test accuracy) by extending their previous winning solution with a compact multi-modal temporal modeling framework for weak/sparse implicit emotion evidence in long videos, using cross-attention (static pose as Query, dynamic micro-motion as Key/Value), MIL-based adaptive pooling, and analyzing representation collapse in vision foundation models.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of weak and sparse implicit emotion evidence in long videos, particularly the difficulty of detecting subtle emotional cues that are easily overwhelmed by background noise and static body shape biases.

Method: A compact multi-modal temporal modeling framework integrating 2D/3D skeletons, facial Blendshapes, DINOv2/v3, X-CLIP, and Gemini semantic priors; cross-attention mechanism using static pose (Base) as Query and dynamic micro-motion differential (Offset) as Key/Value to eliminate static biases; adaptive MIL-based pooling for extracting instantaneous emotions while suppressing background noise.

Result: Achieved first place in Track 3 with 0.76923 test accuracy; revealed representation collapse in general vision foundation models for micro-dynamic tasks.

Conclusion: The proposed cross-attention and MIL-based pooling effectively handle weak implicit emotions in long videos; however, general vision foundation models suffer from representation collapse and public-leaderboard-driven pseudo-generalization due to shortcut learning and rote memorization.
...
TL;DR: Linear probing and steering methods can overstate what VLMs actually perceive in images. A simple blank-image control reveals three grounding regimes: grounded (true vision-dependent), prior (vision-independent default), and inverted (decodable but deployed with wrong sign). Across 14 VLMs, horizontal is grounded, vertical is a prior, depth is inverted—an error that emerges at scale and requires different correction strategies.


<details>
  <summary>Details</summary>
Motivation: Standard methods (linear probing + steering recovery) systematically overstate what VLMs actually ground in images, especially on spatial reasoning. The error is invisible to both probing and steering, motivating a simple causal control (blank image) to expose it.

Method: Use a blank-image arbiter (replacing image with gray blank) as a causal control to separate vision-dependent from vision-independent decoding. Validate across 14 VLMs (2B–27B, six LM families) on spatial reasoning axes (horizontal, vertical, depth). Apply training-free projection and trained low-rank edits to test controllability.

Result: Three grounding regimes identified: grounded (vision-dependent, correct), prior (vision-independent, directional default), inverted (decodable, controllable, but wrong sign). Horizontal is grounded, vertical is a prior, depth is inverted across all models. Inversion emerges at scale. Different models require different correction complexity (rotation vs. low-rank edit).

Conclusion: The blank-image arbiter should be a default control for latent-knowledge and steering claims in VLMs, as it cleanly separates grounded perception, inverted perception, and prior substitution that standard methods conflate.
...
TL;DR: WarpHammer is a training-free framework that improves projection-conditioned novel view synthesis under large viewpoint changes by augmenting warped scenes with explicit 3D object reconstructions from generative priors, and further enables fusion of auxiliary object views from external sources without requiring camera poses.


<details>
  <summary>Details</summary>
Motivation: Existing projection-conditioned NVS methods work well for small viewpoint changes but fail under large orbital motion due to sparse warping, hidden surfaces, and mirror-like artifacts that degrade both pixel content and camera cues.

Method: WarpHammer augments the warped scene with explicit 3D reconstruction from a generative prior (e.g., SAM3D), adds missing foreground surfaces, occludes invisible background points, and uses a multi-view geometry foundation model to fuse external auxiliary views into the 3D reconstruction without requiring camera poses.

Result: On five benchmarks, WarpHammer produces stable novel views at large viewpoint deviations where strong baselines collapse, and is the first scene-level NVS method to naturally fuse auxiliary pose-unknown object views.

Conclusion: Explicit 3D object reconstruction can effectively resolve the large-motion failure mode in NVS and unlock cross-source view fusion, all without fine-tuning or user-provided camera poses.
...
TL;DR: This paper proposes a failure-driven self-improvement loop for computer-use agents, where LLMs diagnose failures from unsuccessful trajectories and generate code patches (lightly verified by humans) to upgrade the agent. Applied to OpenCUA-72B on OSWorld, it improves success rate from 42.3% to 48.9% (+6.6%) without additional training cost.


<details>
  <summary>Details</summary>
Motivation: Current self-improving loops for computer-use agents only exploit successful trajectories and discard failed ones, even though failures contain rich information about a model's weaknesses. The authors aim to turn failed trajectories into agent improvements.

Method: A failure-driven self-improvement loop where: (1) an LLM diagnoses failure modes from failed trajectories, (2) proposes inference-time solutions, and (3) generates code patches that are lightly verified by humans to upgrade the agent.

Result: Applied to OpenCUA-72B on OSWorld benchmark, the method improves success rate from 42.3% to 48.9% (+6.6 percentage points) without additional training cost and with only modest inference overhead.

Conclusion: Failure-driven self-improvement is a viable and complementary approach to success-based pipelines, enabling more efficient agent improvement by leveraging otherwise discarded failure information.
...
TL;DR: CLIMB combines a hierarchical centroid-based replay buffer with knowledge distillation to achieve state-of-the-art performance in online continual self-supervised learning.


<details>
  <summary>Details</summary>
Motivation: Existing OCSSL methods rely either on replay buffers or regularization alone; the paper aims to combine both simultaneously for better representation learning under memory constraints and without task boundaries.

Method: CLIMB uses a hierarchical centroid-based memory that groups similar images into centroids (bounded in total stored images) and applies knowledge distillation on replayed examples to limit representation drift, providing hard-to-discriminate examples for contrastive learning.

Result: CLIMB outperforms state-of-the-art OCSSL methods on Split CIFAR-100 and Split ImageNet-100 under standard benchmarks and a new protocol with irregular task distributions.

Conclusion: Combining replay buffers with regularization via hierarchical memory and knowledge distillation is an effective strategy for online continual self-supervised learning.
...
TL;DR: MICE is a training-free method for multi-instance image editing that modifies joint attention biases in Multimodal Diffusion Transformers to prevent semantic interference, enabling concurrent edits on multiple image regions in a single forward pass.


<details>
  <summary>Details</summary>
Motivation: Existing multi-instance image editing suffers from semantic interference (attribute leakage) when multiple instructions target different regions, worsening with more edits. A single-pass solution is needed for efficiency and better harmonization.

Method: MICE modifies the additive bias of joint attention in Multimodal Diffusion Transformers to: (1) allow intra-instance attention, (2) penalize interactions between neighboring region tokens, and (3) suppress unrelated cross-instance attention, guided by user-provided segmentation masks.

Result: MICE outperforms strong baselines and recent competitors on LoMOE-Bench and the newly introduced MICE-Bench (avg. 8.5 concurrent edits per image) in visual quality preservation and faithfulness to editing instructions.

Conclusion: A training-free attention regulation strategy can effectively disentangle multiple concurrent edits in diffusion transformers, enforcing attribute binding while maintaining global visual consistency.
...
TL;DR: ToothForge is a deep spectral generative framework for dental crown modeling that uses synchronized spectral embeddings to learn compact, intrinsic representations from limited datasets, achieving competitive performance against spatial methods while maintaining geometric interpretability.


<details>
  <summary>Details</summary>
Motivation: Dental crown morphology modeling is crucial for diagnosis and restoration, but available 3D shape datasets are typically small, making it difficult to train effective generative models. Existing approaches often require consistent mesh connectivity across samples, which is unrealistic in clinical practice.

Method: ToothForge operates in the spectral domain using synchronized spectral embeddings of 3D tooth shapes. It mitigates Laplace-Beltrami eigenbasis instability via spectral synchronization, enabling consistent learning across samples with varying connectivity in a low-dimensional latent space.

Result: Synchronized spectral modeling achieves reconstruction and generative performance comparable to or exceeding spatial approaches (PCA-based and point-based models), while offering compactness and geometric interpretability. It performs well on limited datasets.

Conclusion: The framework's compactness and low-dimensional learning space make it well-suited for small dental/medical datasets and real-world scenarios where consistent mesh connectivity across clinics cannot be guaranteed.
...
TL;DR: A wavelet-optimized pseudo-3D diffusion model (CL-DM) for computed laminography that extends reconstruction beyond truncated field-of-view, using 2D diffusion for slice aggregation, 3D MBIR for data consistency, and wavelet regularization to eliminate truncation artifacts and restore continuous 3D structures.


<details>
  <summary>Details</summary>
Motivation: Field-of-view limitations in Computed Laminography cause projection data truncation, leading to severe reconstruction artifacts. Existing deep learning methods lack rigorous data consistency constraints and confine artifact removal to the FOV, discarding recoverable information outside it.

Method: 1) Categorizes space into data-complete, data-incomplete, and data-free regions. 2) Extends reconstruction target to the data-incomplete region. 3) Proposes a wavelet-optimized pseudo-3D accelerated diffusion model (CL-DM): combines 2D diffusion for slice aggregation with 3D MBIR for data consistency, uses wavelet regularization along z-direction with translation-invariant mechanism and low-frequency preservation, plus a 3D fast sampling architecture for acceleration.

Result: Extensive simulations and real-world experiments demonstrate CL-DM effectively eliminates truncation artifacts and restores high-fidelity, continuous 3D structures, outperforming existing methods.

Conclusion: The proposed CL-DM significantly expands the effective imaging range and enhances scanning efficiency by recovering information in data-incomplete regions, representing a superior approach for CL truncation reconstruction.
...
TL;DR: ALM is a training-free sampling method for diffusion models that directly optimizes unobserved variables to generate globally coherent content, outperforming existing approaches across diverse tasks.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based generation methods face a trade-off: training-based approaches are costly and poorly generalizable, while training-free methods lack explicit optimization over unobserved variables, causing global inconsistency.

Method: ALM (Accelerated Likelihood Maximization) integrates a training-free sampling strategy that directly optimizes unobserved regions during the reverse diffusion process, coupled with an acceleration mechanism for efficiency.

Result: ALM consistently outperforms state-of-the-art methods across multiple data domains and tasks in terms of diversity, coherence, and plausibility.

Conclusion: ALM establishes a powerful paradigm for versatile content generation by overcoming the global inconsistency problem of training-free methods while avoiding the computational costs and poor generalization of training-based approaches.
...
TL;DR: Vega is a unified framework that bridges video generation and understanding by combining autoregressive prediction for semantic keyframes with diffusion-based rendering for high-resolution video output, achieving strong results on both generation and understanding benchmarks.


<details>
  <summary>Details</summary>
Motivation: While unified image generation/understanding has been explored, extending this to video remains underexplored. Video is a more suitable modality than static images for unified modeling because it captures both spatial semantics and temporal dynamics. The key challenge is balancing compact semantic representations (needed for understanding) with dense visual details (needed for generation).

Method: Vega uses a shared vocabulary to jointly model text and visual representations, along with a hybrid architecture: (1) an autoregressive model predicts semantically meaningful visual tokens for keyframes, (2) a diffusion module renders dense, high-resolution video frames guided by the structured AR representations.

Result: Achieves strong performance on video generation benchmarks (VBench) and video understanding benchmarks (VideoMME).

Conclusion: The hybrid AR+diffusion approach effectively bridges video understanding and generation within a single unified framework, validating that video is a promising modality for unified multimodal modeling.
...
TL;DR: This paper evaluates learned preprocessing pipelines (including DnCNN) for elbow abnormality detection using the MURA dataset. Under patient-level, leakage-aware evaluation, preprocessing yields modest, configuration-dependent improvements, with raw-input DenseNet121 remaining competitive across all metrics.


<details>
  <summary>Details</summary>
Motivation: To investigate whether learned preprocessing pipelines (e.g., DnCNN) improve discrimination and calibration for triage-oriented orthopedic abnormality detection compared to raw-input baselines, under a proper leakage-aware patient-level evaluation protocol.

Method: Used MURA elbow radiographs, compared multiple preprocessing pipelines (with/without lightweight DnCNN module), evaluated under patient-level, leakage-aware protocol; assessed via AUROC, PR-AUC, ECE, Brier score, and high-specificity operating point analysis.

Result: Preprocessing gains are modest and configuration-dependent; raw and diverse inputs + DnCNN reduced ECE and Brier score, but CLAHE + DnCNN did not improve calibration. No preprocessing strategy consistently outperformed raw-input DenseNet121 baseline across all metrics.

Conclusion: Learned preprocessing offers no consistent discrimination advantage over the raw-input DenseNet121 baseline under rigorous patient-level evaluation; benefits are modest and highly configuration-dependent.
...
TL;DR: RCL-Mamba is a dual-domain state space model that corrects rotational blur in projections and sparse artifacts in reconstructions for sparse-view RCL, achieving 8× speedup in PCB inspection without quality loss.


<details>
  <summary>Details</summary>
Motivation: To address cross-domain degradations (rotational blur from continuous scanning in projection domain + sparse artifacts from undersampling in image domain) in rapid sparse-view RCL for NDT of large planar components like PCBs.

Method: A cascaded dual-domain SSM-based restoration network (RCL-Mamba) with: (1) projection-domain rotational blur correction; (2) image-domain sparse artifact suppression; (3) Mamba-CNN dual-branch module balancing global blur correction and local detail recovery.

Result: Outperforms baselines in blur removal, artifact suppression, and structural preservation on simulated and real PCB data; reduces required scanning views from 512 to 64 (8× efficiency gain) without quality degradation; better preserves via/pad boundaries and trace profiles.

Conclusion: RCL-Mamba provides a robust measurement-oriented restoration solution enabling high-throughput RCL inspection with improved structural measurement fidelity at 8× fewer scanning views.
...
TL;DR: LA-SR proposes an unpaired super-resolution framework that extracts real LR patches from high-quality images based on depth, then uses vision-language models to bridge the LR-HR gap via language-guided losses for content and quality.


<details>
  <summary>Details</summary>
Motivation: Existing SR models rely on synthetic HR-LR pairs that fail to capture real-world degradation complexity, leading to poor generalization. Real paired data is difficult to obtain.

Method: Extracts real LR patches from single HQ images using depth-variance; redefines unpaired SR in language space via vision-language models; applies linguistic content and quality losses.

Result: LA-SR effectively super-resolves real LR inputs, producing realistic outputs that overcome limitations of synthetic-data-trained methods.

Conclusion: Using language space alignment for unpaired SR is a viable alternative to synthetic degradation approaches, enabling better generalization to real-world images.
...
TL;DR: This paper extends Cycle Self-Training (CST) from image classification to object detection by addressing three challenges: unreliable pseudo-labels (classification/localization errors), training instability from multiple objects, and exploding regression losses. The proposed Dual-Stream Bilevel-Cycle Optimization framework integrates CST with Mean Teacher, extra normalization, and theoretical regression bounds, achieving strong results across four cross-domain scenarios.


<details>
  <summary>Details</summary>
Motivation: To extend Cycle Self-Training (CST), which is effective for unsupervised domain adaptation in image classification, to object detection by overcoming three key challenges: unreliable pseudo-labels (classification error, localization error, and both combined), training instability from detecting multiple objects, and exploding losses from wide numerical ranges in regression coordinates.

Method: Proposes Dual-Stream Bilevel-Cycle Optimization framework: (1) Applies CST to both classification and regression branches; (2) Constructs CST upon Mean Teacher to stabilize training; (3) Uses extra normalization to map regression bounding boxes into a standardized space to prevent exploding losses; (4) Provides theoretical derivation of the regression bound.

Result: Extensive experiments across four cross-domain standard scenarios demonstrate that the proposed framework achieves considerable results in unsupervised domain adaptation for object detection.

Conclusion: The Dual-Stream Bilevel-Cycle Optimization successfully adapts Cycle Self-Training to object detection by addressing unreliable pseudo-labels, training instability, and exploding losses, validated across multiple cross-domain benchmarks.
...
TL;DR: MAPE is a deep learning defense against transferable adversarial attacks that uses a channel-attention U-Net and a probabilistic scheduling algorithm to eliminate multi-source adversarial perturbations, achieving over 95.1% defense rate on CIFAR-10 and 71.5% on Mini-ImageNet.


<details>
  <summary>Details</summary>
Motivation: Transferable adversarial attacks are highly stealthy and difficult to detect since they don't rely on target model information, making effective defense against them a critical challenge.

Method: Two components: (1) SAPE uses a channel-attention U-Net trained on adversarial examples from a single pre-trained model to eliminate known perturbations; (2) PPSA quantifies model differences and uses negative momentum to schedule multiple pre-trained models, maximizing diversity of adversarial examples during training to improve robustness.

Result: In black-box attacks against ResNet-34, MAPE achieves average defense rates of >95.1% on CIFAR-10 and >71.5% on Mini-ImageNet, achieving state-of-the-art performance.

Conclusion: MAPE effectively eliminates adversarial perturbations from diverse sources and provides robust defense against attacks from different substitute models.
...
TL;DR: This paper proposes MS-Resampler, a multi-scope visual resampling framework for MLLMs that uses multiple scope-specific resamplers with explicit spatial priors to capture both local details and global context, outperforming single-scope resamplers on multimodal benchmarks with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Existing resamplers in MLLMs use a single, fixed aggregation scope via global cross-attention, which blurs fine-grained local evidence and limits the ability to capture both local details and global context within a fixed token budget.

Method: MS-Resampler instantiates multiple scope-specific resamplers by injecting explicit spatial scope priors into the resampling attention, enabling each branch to aggregate visual information at a particular granularity from local to global. The outputs are then adaptively fused for language modeling.

Result: Extensive experiments on ten public multimodal benchmarks show consistent improvements in visual understanding and multimodal reasoning over conventional single-scope resamplers, with minimal computational overhead.

Conclusion: Multi-scope visual resampling effectively captures both local and global visual information, improving MLLM performance without significant computational cost.
...
TL;DR: OVOW is a training-free system that reconstructs instance-level, simulation-ready 4D mesh scenes from a single monocular video, using a four-stage pipeline involving vision-language discovery, category-aware reconstruction, render-match-optimize pose recovery, and physics-grounded assembly, achieving superior geometry, layout, and physical stability while being significantly faster than baselines.


<details>
  <summary>Details</summary>
Motivation: Existing 4D reconstruction methods produce implicit fields, Gaussian primitives, or point clouds that lack watertight topology, instance separation, and standardized physical interfaces needed by physics simulators and embodied AI. OVOW aims to close this gap by producing structured, simulation-ready 4D meshes.

Method: A four-stage pipeline: (1) vision-language model discovers, labels, and motion-classifies instances; (2) category-aware reconstruction yields per-instance meshes (rigid) and topology-consistent mesh sequences (deformable); (3) iterative render-match-optimize recovers metric scale and 6-DoF pose trajectories; (4) physics-grounded assembly enforces ground contact and inter-object support. All motion is modeled via direct vertex deformation without category-specific priors or skeleton rigging.

Result: OVOW achieves the best layout and geometry accuracy, lowest photometric and semantic error on synthetic benchmarks, runs 1-2 orders of magnitude faster than baselines on monocular video, and demonstrates physical stability in downstream physics simulation.

Conclusion: OVOW establishes the first benchmark for structured Video-to-4D evaluation with metrics beyond visual fidelity, and serves as a scalable engine for synthesizing paired video-to-4D simulation data for future 4D world models and embodied AI.
...
TL;DR: The paper identifies that existing entropy-based uncertainty methods for VLMs fail under visual ambiguity due to overconfident embeddings and textual dominance. The authors propose Visual Semantic Entropy (VSE), which perturbs only images while keeping text fixed and clusters generated answers into semantic prototypes, achieving SOTA uncertainty estimation.


<details>
  <summary>Details</summary>
Motivation: Existing uncertainty estimation methods (e.g., Semantic Entropy) fail for vision-language models on visually ambiguous inputs because overconfident visual embeddings suppress output diversity, and perturbation-based methods are dominated by textual rather than visual changes.

Method: VSE perturbs only the image while keeping the text query fixed, generates multiple answers, clusters them into semantic prototypes, and computes mass-weighted dispersion among prototypes to measure uncertainty.

Result: VSE achieves state-of-the-art uncertainty estimation across five modern VLMs and five diverse VQA benchmarks.

Conclusion: Probing visual variations via image-only perturbations while clustering semantic prototypes effectively captures visual ambiguity for VLM uncertainty estimation.
...
TL;DR: The paper proposes TemporalLens, a diagnostic framework to test whether video object detectors truly use temporal context, and introduces YOLO-3D, a real-time spatiotemporal detector. Findings show stacked 2D models collapse without the target frame, while YOLO-3D recovers predictions from earlier frames; preserving temporal depth through the backbone is the key performance driver (+3.7 pp mAP@50).


<details>
  <summary>Details</summary>
Motivation: Standard metrics like mAP reward correct predictions regardless of how they are reached, making it unclear whether single-stage video detectors genuinely reason over temporal context or simply exploit a single informative frame.

Method: Proposes TemporalLens, a model-agnostic diagnostic framework using controlled perturbations (structured occlusions, temporal shuffling, redundancy injection, resolution degradation). Also introduces YOLO-3D, a modular real-time spatiotemporal detector built on YOLOv8.

Result: Stacked 2D models collapse when target frame is removed; spatiotemporal models (YOLO-3D) recover predictions from earlier frames. Preserving temporal depth through the backbone yields +3.7 pp mAP@50 at 32 frames averaged across scales.

Conclusion: The diagnostics and architecture turn "does this detector reason over time?" into a measurable, actionable question.
...
TL;DR: A prompt-free diffusion image steganography method using style semantic priors and a Cascaded Affine Coupling Module (CACM) for robust stego image generation, with a predictor-corrector mechanism to refine generation trajectories.


<details>
  <summary>Details</summary>
Motivation: Existing LDM-based steganography methods are limited by the inflexibility of text prompts, struggling to generate high-quality stego images and accurately recover secret images.

Method: (1) Cascaded Affine Coupling Module (CACM) for bijective mapping between secret image and latent representation; (2) Integration of style semantic priors into diffusion process to control latent representation; (3) Predictor-corrector mechanism to iteratively refine the generation trajectory.

Result: Competitive performance compared to state-of-the-art methods in security, secret image reconstruction accuracy, and controllability.

Conclusion: The proposed prompt-free framework with style semantic priors effectively addresses text prompt inflexibility, improving stego image quality and secret recovery accuracy.
...
TL;DR: This study examines how temporal training-set design affects nnUNet-based segmentation of the left atrium and left atrial appendage in dynamic 4DCT, finding that training with all frames yields best performance in low-contrast phases, while a physiologically selected subset achieves comparable results from the filling phase onward, with foreground-based normalization partially helping reduced datasets.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of accurate segmentation across all temporal frames in dynamic contrast-enhanced cardiac CT for assessing blood stasis in atrial fibrillation, given the trade-off between training for robustness and limiting label noise.

Method: Compared nnUNet-based segmentation performance using three temporal training-set designs: minimal two-frame dataset (standard clinical practice), physiologically selected subset, and full 27-frame sequence; also evaluated foreground-based normalization.

Result: Full-frame training performed best in early low-contrast phases; physiologically selected subset matched performance from filling phase onward; normalization from full dataset improved reduced datasets in low-contrast frames but didn't fully close the gap.

Conclusion: Temporal diversity in training data is crucial for robust dynamic CT segmentation, but carefully selected frame subsets offer an effective trade-off between performance and efficiency for downstream applications.
...
TL;DR: This paper proposes a common encoding based on S-GABC for Gregorian notation, converts four existing datasets to it, and trains a shared end-to-end model that achieves state-of-the-art results across all datasets.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for optical recognition of Gregorian notation are encoded differently, preventing cross-dataset learning and limiting model generalization.

Method: Design a common encoding based on S-GABC, convert all four datasets to this encoding, and train a shared end-to-end foundational model.

Result: Establishes a new state of the art across all four datasets.

Conclusion: A unified encoding and shared model approach can improve recognition performance across diverse Gregorian notation datasets.
...
TL;DR: SatAgent is a UAV-Satellite collaborative spatial reasoning model that uses dual-pathway visual processing and geometric-aware 3D reconstruction to overcome occlusion, perspective distortion, and viewpoint inconsistency in UAV-based spatial reasoning, outperforming existing models by 11-26%.


<details>
  <summary>Details</summary>
Motivation: Existing UAV spatial reasoning is limited by single-view perception (occlusions, perspective distortions) and VLMs lacking explicit geometric modeling, yielding inconsistent reasoning under viewpoint and scale variations.

Method: SatAgent introduces (1) a Geometric-Aware 3D Reconstruction Encoder to lift 2D UAV features to 3D, (2) a multi-view topology-semantic alignment module unifying cross-view features in BEV coordinates, (3) a multi-view consistency loss for viewpoint-invariant representations, and (4) the SatAgent-SR130K dataset for training.

Result: Outperforms general-purpose foundation models by 25.91% and specialized spatial reasoning models by 11.69% across diverse tasks, with particularly high accuracy in complex geometric relationship reasoning.

Conclusion: UAV-Satellite collaboration with explicit geometric modeling and viewpoint-invariant learning significantly improves spatial reasoning in complex urban environments.
...
TL;DR: Proposes an asynchronous architecture for open-vocabulary 3D scene graphs that runs lightweight online mapping concurrently with heavyweight VLM-based semantic refinement, enabling queryable graphs during exploration and achieving state-of-the-art results on visual grounding benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing open-vocabulary 3D scene graph methods use a sequential two-stage pipeline (reconstruct first, then enrich with VLMs), which leaves the graph unqueryable during exploration. The authors argue this sequential coupling is unnecessary.

Method: An asynchronous architecture with (1) a probabilistic voxel-based backbone for incremental mapping and stable object identities, (2) background VLM agents for progressive semantic refinement, (3) semantic loop closure to resolve duplicate object tracks, (4) fine-grained visual attributes and spatial relations, and (5) a multi-target frame scheduler that selects informative frames to amortize VLM cost.

Result: Matches or outperforms existing methods on semantic segmentation (ScanNet, Replica); surpasses prior SOTA on Sr3D+, Nr3D, and ScanRefer visual grounding benchmarks by 15.3 to 18.8 A@0.25.

Conclusion: The asynchronous architecture enables queryable scene graphs during exploration with growing semantic richness over time, while outperforming prior methods on multiple benchmarks.
...
TL;DR: DrivingDepth proposes using sparse LiDAR as geometric prompts to locally calibrate a frozen depth foundation model via per-pixel scale correction, achieving SOTA metric accuracy and geometric consistency for autonomous driving depth estimation.


<details>
  <summary>Details</summary>
Motivation: Dense depth estimation faces a geometry-scale conflict: depth foundation models provide pixel-aligned relative geometry without metric scale, while LiDAR offers metric anchors that are sparse, noisy, and misaligned with image structures. Existing methods override foundation geometry, causing structural artifacts.

Method: DrivingDepth treats sparse LiDAR as geometric prompts to locally calibrate a frozen depth foundation prior through per-pixel residual scale correction, preserving dense visual geometry without retraining the foundation model.

Result: On nuScenes with 4-frame surround-view input, DrivingDepth achieves AbsRel of 11.19 and EdgeCR of 5.741, outperforming MapAnything (11.99/1.914) in both metric accuracy and geometric consistency.

Conclusion: Foundation models already capture coherent relative depth; only per-pixel scale mapping is needed, not full regeneration. DrivingDepth proves this approach simultaneously delivers SOTA metric accuracy and geometric consistency.
...
TL;DR: HVPNet is a bio-inspired architecture for multimodal salient and camouflaged object detection that simplifies complex cross-modal fusion by drawing from human visual processing. It uses a Retinal Integration Module (RIM) and a Cortical Decoder (CD) to achieve strong accuracy-efficiency trade-offs across 7 tasks, 4 modalities, and 22 datasets.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal SOD and COD methods rely on complex cross-modal fusion and decoding, leading to large model parameters and structural redundancy, yet often fail to deliver satisfactory performance. The authors question whether simpler, bio-inspired modeling can achieve both accurate and efficient detection.

Method: Proposes HVPNet with two key components: (1) Retinal Integration Module (RIM) — a level-specific multi-stage integration strategy conceptually inspired by the retina's multi-layered information integration; (2) Cortical Decoder (CD) — breaks decoding into low- and high-level visual stages, abstracting hierarchical processing in the human visual cortex.

Result: Extends to 7 tasks across 4 modalities, achieves an excellent accuracy-efficiency trade-off on 22 datasets spanning these tasks. Code is publicly available.

Conclusion: Bio-inspired simple architectures can rival or outperform complex cross-modal fusion designs, demonstrating that drawing conceptual inspiration from the human visual process is a promising direction for efficient and accurate object detection.
...
TL;DR: A deep learning framework automatically segments RPE loss, EZ loss, and EZ thinning in OCT images for GA assessment, achieving high accuracy (Dice >0.87, ICC >0.98) across diverse AMD phenotypes.


<details>
  <summary>Details</summary>
Motivation: Geographic atrophy (GA) in AMD requires precise monitoring of structural biomarkers (RPE loss, EZ loss, EZ thinning) for disease staging and treatment response, but manual assessment is labor-intensive and variable.

Method: Three specialized semantic segmentation models were developed on 298 SD-OCT volumes (full AMD spectrum) to delineate RPE loss, EZ boundaries, and Bruch's membrane, validated on an external dataset (n=43) with additional repeatability/reliability analyses.

Result: Dice scores: RPE loss 0.88, EZ loss 0.87; Pearson's r > 0.99; sub-pixel EZ thickness deviation of 2.15 μm; ICC > 0.98 for reproducibility.

Conclusion: The fully automated framework provides a highly reliable tool for GA assessment in clinical trials and routine ophthalmic care by accurately quantifying photoreceptor degeneration and RPE loss.
...
TL;DR: SimpleSearch-VL improves multimodal agentic search through factorized adaptive rollout for efficiency, evidence-verified reasoning for reliability, and lightweight webpage self-summary for practicality, achieving 15.8-16.0 point gains over Qwen3-VL baselines with minimal data.


<details>
  <summary>Details</summary>
Motivation: To build an efficient, reliable, and practical multimodal agentic search framework without relying on scaling data, tools, or auxiliary model components.

Method: Factorized Adaptive Rollout (FAR) for efficient sampling; evidence-verified reasoning via chain-of-thought for reliability; lightweight tool interface with webpage self-summary for practicality.

Result: Improves Qwen3-VL baselines by 15.8 (8B) and 16.0 (30B-A3B) average points using only 5K supervised trajectories and 2K RL data; 30B-A3B variant matches Gemini-3-Pro.

Conclusion: SimpleSearch-VL demonstrates that improving the agent's search-and-verification process itself is more effective than scaling data, tools, or model components for multimodal agentic search.
...
TL;DR: PRISM reformulates single-image reflection removal as a linear separation problem in pretrained VAE latent space (FLUX backbone), using flow matching, Latent Composition Consistency (LCC), and Layer Contrastive Separation (LCS) losses, achieving SOTA on six benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing SIRR methods operate in pixel space where the nonlinear sRGB formation model entangles layers and limits generalization. The authors observe that pretrained VAE latent spaces exhibit lower coherence between image layers, motivating a shift to latent-space decomposition.

Method: PRISM (1) reformulates SIRR as latent linear separation using a pretrained FLUX backbone; (2) learns a flow matching velocity field for single-pass recovery of both layers; (3) uses Latent Composition Consistency (LCC) with synthetic reflection-swapped mixtures and cycle loss; (4) applies Layer Contrastive Separation (LCS) loss for patch-level semantic disentanglement without explicit reflection targets.

Result: PRISM consistently outperforms state-of-the-art methods by significant margins on six benchmarks, with strong generalization to in-the-wild images.

Conclusion: Pretrained VAE latent spaces provide a more favorable working space for reflection removal, and the PRISM framework effectively leverages this insight for robust, generalizable SIRR.
...
TL;DR: MV-GEL is a framework for localizing fine-grained geometric entities (faces, edges, solids) on 3D polygon meshes using natural language queries. It selects optimal viewpoints via a prompt-conditioned ranking module (GELviews), uses VLM-based segmentation, and lifts predictions to meshes via ray casting, achieving up to 1.7X improvement in face IoU and 4.5X in edge F1 over baselines.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs excel at referring segmentation in 2D images but struggle with 3D geometric entities due to viewpoint sensitivity, occlusion, and foreshortening. Robust localization of precise CAD entities (edges, faces, curved surfaces) is critical for CAD, robotics, and simulation.

Method: (1) GELviews: a prompt-conditioned ranking module that selects viewpoints maximizing observability of the queried entity. (2) VLM-based reasoning segmentation on selected views. (3) Geometry-aware ray casting to lift 2D segmentation masks to 3D polygon meshes. The framework is CAD-agnostic, relying only on meshes.

Result: Up to 1.7X improvement in face-level IoU and over 4.5X gains in edge-level F1 compared to vanilla baselines, substantially outperforming CLIP-based and random view sampling, especially for thin and view-sensitive structures.

Conclusion: Viewpoint selection is critical for 3D geometric entity grounding; MV-GEL's prompt-conditioned view ranking significantly boosts localization accuracy. The framework's CAD-agnostic nature makes it broadly applicable.
...
TL;DR: DataEvolver is a self-evolving multi-agent framework that uses feedback from rejected samples (rather than discarding them) to iteratively improve text-rich image data construction, achieving significant OCR-F1 gains on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing text-rich image data pipelines follow a "crawl-filter-freeze" paradigm that discards rejected samples, losing valuable failure signals (e.g., OCR errors, semantic mismatches) and causing repeated failure modes across construction rounds.

Method: DataEvolver is a multi-agent framework with four agents: Retriever (collects candidates), Verifier (assigns quality scores & rejection causes), Critic (summarizes round-level feedback), and Generator (synthesizes under-covered regions). Feedback memory evolves across rounds to guide better data construction.

Result: At 0.75M scale on PixArt-alpha, DataEvolver improves OCR-F1 by 85.3% on TextScenesHQ and 35.3% on LongTextBench over the strongest baseline. Improvements also transfer to Show-o2, showing generality beyond a single generator.

Conclusion: Rejected samples provide actionable feedback that can be leveraged to systematically improve text-rich image data construction, outperforming static datasets under matched data budgets.
...
TL;DR: AugSplat improves Gaussian Splatting in sparse-view settings by using a radiance field to generate synthetic views as auxiliary supervision, achieving better reconstruction quality while maintaining real-time rendering.


<details>
  <summary>Details</summary>
Motivation: Gaussian Splatting methods achieve real-time rendering but are sensitive to initial geometry quality, especially in sparse-view settings where limited observations cause incomplete or noisy point-cloud reconstructions.

Method: First train a radiance field on sparse input views to synthesize additional images from novel viewpoints. Then use these synthetic views as auxiliary supervision during Gaussian Splatting optimization via two variants: Staged AugSplat (synthetic then real images) and Dual AugSplat (joint training with decaying synthetic loss weight).

Result: On sparse-view mip-NeRF 360 scenes, AugSplat improves reconstruction quality over standard Gaussian Splatting. Staged AugSplat achieves the strongest average performance, while Dual AugSplat offers competitive results with continual real-image supervision.

Conclusion: AugSplat effectively bridges radiance fields and Gaussian Splatting for sparse-view scenarios, with both variants preserving real-time rendering at inference.
...
TL;DR: MPL-MAE introduces recalibrated positional embedding and gated positional interface modules to reduce positional over-reliance in 3D masked autoencoding, improving semantic feature learning for point clouds.


<details>
  <summary>Details</summary>
Motivation: Existing 3D MAE frameworks suffer from positional leakage where decoders over-rely on spatial coordinates, weakening semantic representation learning and leading to suboptimal feature quality.

Method: Two key components: (1) A recalibrated positional embedding module that suppresses metric-dominant coordinate signals while preserving geometric topology, and (2) a gated positional interface module that dynamically regulates positional injection during reconstruction.

Result: Competitive performance across various downstream tasks, validating that the proposed approach yields robust and informative representations compared to existing 3D MAE methods.

Conclusion: MPL-MAE effectively mitigates positional over-reliance in 3D masked autoencoding, promoting balanced interaction between spatial priors and semantic features for improved point cloud representation learning.
...
TL;DR: The paper proposes PNOT, a Physics-aware Neural Operator Transformer for real-time reconstruction of divertor temperature fields in fusion devices, using graph attention and physics-constrained learning to overcome the computational limitations of traditional FEM methods.


<details>
  <summary>Details</summary>
Motivation: Conventional numerical methods like FEM are computationally expensive and unsuitable for real-time applications in fusion devices, creating a need for fast, generalizable methods for real-time divertor temperature field reconstruction and control.

Method: PNOT models boundary heat-flux relations as structured graphs with graph attention, develops a physics-aware neural operator module via slicing, and uses gradient-constrained Sobolev regularization loss for physical consistency.

Result: Physical constraints improve prediction accuracy while preserving physical consistency in the divertor temperature field modeling.

Conclusion: The proposed physics-aware approach enables fast, real-time temperature field reconstruction for fusion devices with maintained physical consistency.
...
TL;DR: This paper benchmarks localized conformal prediction for vision-language models on image classification, finding that raw cosine similarity fails to improve over baselines, and proposes a non-linear transformation that reduces prediction set sizes while maintaining marginal coverage.


<details>
  <summary>Details</summary>
Motivation: To address the gap in applying localized conformal prediction (which adapts prediction sets based on test-calibration similarity) to image classification tasks with vision-language models, as previous successes were mainly in regression.

Method: Extensive benchmarking of a localized conformal prediction algorithm on natural image classification with VLMs; analysis of cosine similarity between visual features; proposal of a simple non-linear transformation of cosine similarities to improve performance.

Result: Direct cosine similarity is insufficient to outperform non-local baselines; the proposed non-linear transformation achieves statistically significant mean set size reductions while preserving marginal coverage guarantees.

Conclusion: Non-linear transformation of similarities is an effective and simple way to enhance localized conformal prediction for VLM-based image classification, with open-source code provided.
...
TL;DR: The paper identifies that mixing rotation and translation in same value vector dimensions causes performance stagnation in scaled-up novel view synthesis training, and proposes Decoupled Pose Positional Encoding (DPPE) to separate them, enabling stable long-term training and better generalization.


<details>
  <summary>Details</summary>
Motivation: When scaling up training of novel view synthesis models with camera-based positional encoding, model performance stagnates in late training stages. The paper investigates this bottleneck.

Method: Proposes Decoupled Pose Positional Encoding (DPPE), a camera-based positional encoding that explicitly decouples rotation and translation into separate dimensions of the value vector, avoiding indeterminacy in their identification.

Result: DPPE enables stable long-term training under scaled-up setups and shows superior generalization in extrapolation settings including increased viewpoints and zoom-in scenarios.

Conclusion: Decoupling rotation and translation in positional encoding is crucial for scaling Transformers in 3D vision tasks like novel view synthesis.
...
TL;DR: ViToS is a dual-stream RL framework that jointly performs visual token pruning and medical multimodal reasoning, reducing visual tokens to 77% while achieving significant performance gains across seven medical benchmarks.


<details>
  <summary>Details</summary>
Motivation: Medical images have extremely sparse visual evidence, and existing VLMs with RL struggle to handle them effectively. A unified RL framework for adaptive visual token pruning and medical reasoning is missing.

Method: ViToS trains a single policy model with two task branches: one for grounding and another for token-sparse reasoning after VTP. It uses cross-feedback sequential optimization to resolve coupled policy learning and avoid gradient conflicts.

Result: Reduces visual tokens to 77% of original length; achieves 108.27% relative performance on Lingshu-7B and 104.16% on HuatuoGPT-Vision-7B; superior performance and inference speedup across seven medical benchmarks.

Conclusion: ViToS establishes an efficient paradigm for medical multimodal reasoning by jointly optimizing token pruning and question answering through a dual-stream RL framework.
...
TL;DR: The paper proposes an uncertainty-guided synthetic context augmentation method for semantic segmentation that preserves label validity by inpainting only uncertain visual contexts based on a baseline segmenter's predictive entropy, and computes loss only on original pixels during fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Semantic segmentation models struggle with data sparsity and rare/visually diverse regions (e.g., dense regions, small objects). Existing synthetic augmentation methods either rely on external models or use coarse heuristics that waste capacity on uninformative pixels.

Method: (1) Use a baseline segmenter's predictive entropy to identify uncertain semantic regions; (2) Inpaint only the complementary visual context (preserving label validity); (3) During fine-tuning, compute loss only over original pixels (excluding inpainted regions) to focus learning on unmodified uncertain regions in novel contexts.

Result: Substantial mIoU gains on Cityscapes, UAVID, and BDD100K, with largest improvements on rare/difficult classes (e.g., buses, trains, cars in aerial view).

Conclusion: Uncertainty-guided context augmentation is a highly effective approach for improving segmentation performance on complex datasets without requiring external guardrails.
...
TL;DR: A higher-order structural alignment framework using learnable hypergraphs and Unbalanced Optimal Transport for multi-view radar semantic segmentation, achieving state-of-the-art results on CARRADA and RADIal benchmarks.


<details>
  <summary>Details</summary>
Motivation: Radar sensors offer reliable perception in adverse conditions, but their sparse, noisy, and weakly semantic data makes dense semantic segmentation difficult. Existing grid-based and pairwise interaction methods fail to capture higher-order relational structures formed by multiple radar returns from the same physical object.

Method: Proposes a unified framework with (1) learnable hypergraphs for higher-order dependency modeling, (2) Unbalanced Optimal Transport (UOT) for correspondence-free cross-view alignment under varying densities and partial observations, and (3) adaptive attention for fusing complementary radar views (RA, RD, AD) under sparsity and noise.

Result: Achieves 63.8% mIoU on CARRADA (+1.7 over prior best) and 83.4% mIoU on RADIal (+2.3 over prior best), with consistent improvements over strong radar-specific baselines.

Conclusion: Higher-order relational modeling is crucial for robust radar perception; the proposed structural alignment framework effectively addresses sparsity, noise, and cross-view inconsistency in multi-view radar segmentation.
...
TL;DR: ATMem introduces an active memory system for mobile GUI agents that maintains dynamic execution states rather than passive storage, combined with STR-GRPO reinforcement learning to optimize memory usage, achieving better long-horizon task completion on a challenging benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing memory methods treat memory as passive storage where agents must infer from accumulated records whether values should be used, have been used, or must wait—causing repeated or missed operations in long trajectories with similar fields, repeated values, distractors, and outdated states.

Method: Proposes ATMem (Active Task Driving Memory) that maintains task-relevant information as a continually updated execution state linking each value to its role and current status. Also introduces STR-GRPO, an online RL method with memory-on/memory-off rollouts and memory-cost-aware reward to learn selective memory usage.

Result: Built a challenging mobile benchmark where agents must act on every entry satisfying instructions from near-identical entries while rejecting constraint-violating entries.

Conclusion: ATMem shifts GUI-agent memory paradigm from passive storage to active execution state management, with STR-GRPO enabling selective memory usage optimization.
...
TL;DR: This paper proposes a decoupled multi-vehicle landing framework using two independent DRL agents (SAC for deck stabilization and multimodal RL for UAV approach) to enable safe UAV landing on a USV in stochastic sea conditions, achieving 100% success in simulation trials.


<details>
  <summary>Details</summary>
Motivation: Autonomous aerial inspection of marine infrastructure is compromised by stochastic sea states causing high-kinetic impacts, toppling risks, and sensory occlusion during UAV landing on USVs.

Method: A decoupled multi-vehicle framework with a USV-mounted 3-RPU stabilized platform and a UAV, using two independent DRL agents: SAC for wave-motion compensation and multimodal RL for UAV final approach.

Result: 100% landing success across 15 trials (calm to rough seas); 87.8% mean stabilization efficacy; landing surface within 1° of horizontal for 96% of mission duration in rough conditions.

Conclusion: The decoupled DRL-based framework effectively mitigates sea-state disturbances, contributing to safer autonomous maritime UAV landings.
...
TL;DR: PrISM-IQA reformulates smartphone IQA from predicting a single quality score to a multi-issue ordinal diagnosis problem, predicting severity levels (absent, minor, severe, critical) for 53 ISP-relevant issues using cumulative ordinal encoding and structured inference.


<details>
  <summary>Details</summary>
Motivation: Existing IQA methods reduce quality to a single score, which is poorly aligned with practical ISP tuning where engineers need to identify specific quality issues, estimate severities, and decide whether intervention is needed.

Method: PrISM-IQA combines cumulative ordinal encoding with structured inference that captures within-issue monotonicity and cross-issue subsumption/exclusion relations, predicting ordered severity levels for each ISP-relevant issue.

Result: Evaluated on reconstructed SPAQ benchmark (53 ISP-relevant issues) and expert-annotated real-world dataset; demonstrates effectiveness for issue-level diagnosis, shows transferable perceptual representations, and supports actionable ISP tuning.

Conclusion: Multi-issue ordinal diagnosis is more practical and actionable for ISP tuning than single-score regression, offering fine-grained, logically consistent quality assessments.
...
TL;DR: LiteMatch is a lightweight stereo matching framework that achieves strong zero-shot generalization without expensive 3D convolutions, using two complementary encoders (CVCE and HFE) and a novel Cost Volume Consistency Loss (CVC-Loss) to stabilize cost volumes, delivering competitive accuracy across multiple datasets with only 3.36M-9.58M parameters.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based stereo matching methods achieve high accuracy but rely on heavy backbones and computationally intensive 3D cost volume processing, leading to substantial memory and runtime overhead, and more critically, they struggle to generalize across domains, limiting practical deployment.

Method: LiteMatch uses: (1) Cross-View Correspondence Encoder (CVCE) for global cross-view interactions, (2) High-Frequency Encoder (HFE) for fine structural details via FFT-based frequency cues, (3) Cost Volume Consistency Loss (CVC-Loss) - a voxel-wise binary cross-entropy objective on softmax-normalized cost distributions to encourage sharp and unimodal disparity probabilities, and (4) a lightweight refinement module for full-resolution disparities with low-iteration updates.

Result: With a flexible design ranging from 3.36M to 9.58M parameters, LiteMatch achieves exceptional zero-shot generalization, delivering competitive EPE and D1 performance across Scene Flow, KITTI, Middlebury, ETH3D, and DrivingStereo datasets.

Conclusion: Lightweight architectures can indeed generalize across domains without sacrificing accuracy, establishing that strong zero-shot stereo matching is achievable without expensive 3D convolutions or heavy backbones.
...
TL;DR: RoboSpatialBrain, built on RoboBrain2.5-8B-NV, uses training-free inference-time strategies (forced <think> prefix, task-specific post-prompt, reference-frame redirection) to improve spatial reasoning in vision-language models, achieving 80.9% success rate and first place in the RoboSpatial Challenge at CVPR 2026.


<details>
  <summary>Details</summary>
Motivation: Vision-language models have strong general perception capabilities but fail at spatial reasoning needed for embodied tasks, motivating the need for targeted improvements without expensive retraining.

Method: Two training-free inference-time mechanisms: (1) forced <think> prefix activation + task-specific post-prompt for deliberate reasoning, and (2) reference-frame redirection pipeline to resolve camera-centric vs. object-centric ambiguity. Also explores fine-tuning on compatibility data.

Result: First place in the RoboSpatial Challenge with an overall success rate of 80.9% on RoboSpatial-Home.

Conclusion: Lightweight, training-free prompting strategies can significantly improve spatial reasoning in VLMs for embodied tasks, and the approach generalizes well enough to win a competitive challenge.
...
TL;DR: The paper proposes Q-Margin, a novel α-divergence loss function that incorporates a principled probabilistic margin for face and speaker verification, achieving competitive performance and improved low-FAR accuracy while enabling sparse, memory-efficient training.


<details>
  <summary>Details</summary>
Motivation: Standard margin-penalty losses (CosFace, ArcFace) use geometric margins designed for softmax, which don't naturally extend to α-divergence probabilistic frameworks. The authors aim to introduce a principled probabilistic margin that preserves α-divergence's sparsity benefits.

Method: Q-Margin encodes the margin penalty directly into the reference measure (prior probabilities) rather than applying geometric penalties to logits, combining α-divergence loss with a probabilistic margin formulation.

Result: Achieves competitive or superior performance on IJB-B, IJB-C (face verification), and VoxCeleb (speaker verification). Consistently improves at low False Acceptance Rates (FARs) compared to ArcFace/CosFace baselines under identical training recipes.

Conclusion: Q-Margin offers a scalable, memory-efficient solution with extreme posterior sparsity, making it suitable for high-security applications and datasets with millions of identities.
...
TL;DR: SAMBA is a self-supervised pre-training foundation model for SAR target interpretation that uses a linear-complexity Mamba encoder, scattering-guided masking, and a SpatialMix module to achieve SOTA performance with fewer parameters than CNN/Transformer baselines.


<details>
  <summary>Details</summary>
Motivation: SAR ATR deployment is constrained by scarce annotated data. Existing self-supervised methods use Transformer architectures with quadratic complexity and conventional masking that ignores SAR's unique electromagnetic scattering properties.

Method: (i) Linear-complexity Mamba encoder with mid-sequence class token; (ii) Three-level hierarchical Scattering-Guided Masked Autoencoder (SG-MAE) masking based on SAR physical priors; (iii) SpatialMix feature interaction module for cross-region fusion; (iv) Two-stage cross-domain pre-training pipeline.

Result: SAMBA achieves SOTA on most metrics across 7 downstream datasets (classification & detection) with substantially fewer parameters than CNN/Transformer baselines. SG-MAE masking further boosts few-shot transfer over standard MAE.

Conclusion: SAMBA validates robust generalizability across diverse SAR interpretation tasks, offering an efficient and physically-grounded alternative to Transformer-based self-supervised pre-training for SAR ATR.
...
TL;DR: WorldRoamBench is a new benchmark for interactive world models that evaluates action, vision, physics, and memory across 600+ test cases, revealing that no existing model performs well on all dimensions.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for interactive world models only evaluate action following at the trajectory level and ignore memory and interaction physics, leaving critical failure modes undetected.

Method: The benchmark introduces four evaluation dimensions: (i) per-frame action metric, (ii) segment-based drift metric, (iii) controllability-gated physics evaluation, and (iv) action-decoupled memory protocol using 3D reconstruction and VLM reasoning. It covers 600+ cases across Nature, Urban, and Indoor scenes.

Result: Evaluating 10+ open/closed-source models shows none reliably satisfies all dimensions; even the best achieves only moderate scores.

Conclusion: Advances on WorldRoamBench represent steps toward IWMs that are stable, physically grounded, memory-faithful, and deployable in real-world applications.
...
TL;DR: REDI combines supervised TF-IDF (from DINOv3 patch vocabularies) with attention maps to rank patch importance, enabling 46.8% sequence reduction while improving ImageNet Top-1 accuracy from 83.514% to 84.706%.


<details>
  <summary>Details</summary>
Motivation: To understand how to optimally allocate a fixed token budget across patches for image classification, using a controlled supervised reference rather than solely relying on unsupervised attention signals.

Method: REDI quantizes DINOv3 patch representations into visual words, builds a class-conditioned TF-IDF corpus, computes per-patch relevance scores as the elementwise product of normalized TF-IDF maps and attention maps, then uses a keep/merge/compress operator guided by score rank and magnitude.

Result: 46.8% sequence reduction (201→107 tokens); REDI achieves 84.706% Top-1 accuracy vs. 83.514% (dense baseline), 82.634% (attention-only), and 81.796% (TF-IDF-only) on ImageNet-1K with ViT-B/16.

Conclusion: Class-specific corpus statistics (TF-IDF) and image-specific attention provide complementary signals for patch ranking; combining them yields accuracy gains even with significant token reduction.
...
TL;DR: ShellMaker is a language-guided framework that generates complete, stylistically consistent building exteriors (facades and roofs) while respecting fixed structural constraints from existing indoor floor plans and wall layouts.


<details>
  <summary>Details</summary>
Motivation: Current indoor scene generation methods produce floor plans and walls but stop at a structural shell, lacking coherent exteriors (facades and roofs). Completing these exteriors is challenging because the footprint, wall geometry, and openings must remain fixed—constraints that existing unconstrained generative models violate.

Method: ShellMaker combines parametric roof generation, LLM-based part-aware prompt refinement, joint wall-roof material retrieval, and geometry-aware assembly. It operates on a format-agnostic scaffold representation, enabling generalization across indoor generators, CityGML, and CAD inputs.

Result: ShellMaker outperforms retrieval and unconstrained generative baselines in maintaining structural consistency and improving architectural coherence across diverse input formats.

Conclusion: ShellMaker effectively addresses the underexplored problem of generating structurally consistent building exteriors that align with pre-generated interiors, offering a generalizable solution across multiple input types.
...
TL;DR: HIG introduces distributional control (e.g., color histograms) as a middle-ground mechanism for guiding diffusion models, formulated as an optimal transport problem, enabling precise and interpretable control compatible with existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion model control methods either operate at a coarse global level (text prompts) or a dense local level (ControlNet). There is a gap for intermediate-granularity control that balances global coherence with local precision. The authors aim to fill this gap with distributional constraints.

Method: The method models histogram constraints as an optimal transport (OT) problem and applies explicit guidance transformations during the sampling process to enforce user-specified distributional constraints (e.g., color histograms or latent token distributions) with exact precision.

Result: HIG demonstrates versatility across applications: constrained generation via color/latent histograms and high-capacity information embedding through histogram-level encoding. It is fully compatible with existing control mechanisms.

Conclusion: Distributional control is a flexible, interpretable control scheme that occupies the middle ground of control granularity, offering a promising hybrid strategy for controllable image generation when combined with existing mechanisms.
...
TL;DR: DRVR is a single-sweep LiDAR-based 3D semantic occupancy prediction method that uses dual range-view and voxel-view representations with a fusion module, achieving 5.4% mIoU improvement and 2.1x speedup over multi-sweep methods.


<details>
  <summary>Details</summary>
Motivation: Multi-sweep point cloud stacking for dense occupancy perception causes efficiency (computational burden) and robustness (pose noise) issues. Single-sweep methods suffer from sparsity and incompleteness, leading to insufficient semantic learning.

Method: Proposes a Dual Range-Voxel Representation (DRVR) with: (1) a range-view encoder for compact scene context, (2) a geometry-aware voxel-view encoder for multi-scale spatial features, and (3) a range-voxel fusion module with bidirectionally fused features (voxel-to-range and range-to-voxel).

Result: On nuScenes-Occupancy, DRVR achieves 5.4% mIoU improvement and 2.1x speedup over multi-sweep methods. Also validated on SemanticKITTI and SemanticPOSS.

Conclusion: Single-sweep DRVR eliminates multi-sweep concerns while achieving superior performance, making it more practical for real autonomous driving deployment.
...
TL;DR: IS-SNN removes batch normalization from deep spiking neural networks by using topology-aware weight standardization and modified residual connections to prevent catastrophic firing-rate decay, achieving competitive accuracy (68.05% on ImageNet) with 96.4% reduction in FPGA resource consumption.


<details>
  <summary>Details</summary>
Motivation: Dynamic batch normalization variants in SNNs introduce runtime multiplications that weaken the hardware-efficiency motivation of SNNs. The paper identifies catastrophic firing-rate decay as the cause of performance degradation in normalization-free SNNs.

Method: Proposes Intrinsically Stable SNN (IS-SNN) architecture that removes activation-normalization layers by enforcing signal homeostasis through topology-aware weight standardization and modified residual connections, folding standardization into static weights offline.

Result: Achieves 68.05% accuracy on ImageNet, matches or surpasses dynamic BN techniques across VGG, ResNet, and Transformer models, with 96.4% reduction in FPGA lookup table resource consumption for neuron implementations.

Conclusion: IS-SNN provides a practical framework for building accurate and hardware-friendly deep neuromorphic systems by restoring accumulation-oriented inference and removing runtime overhead.
...
TL;DR: SAEs are effective for detecting semantic concepts in diffusion models, but direct latent intervention causes artifacts; using SAE activations purely as detectors for patch replacement yields cleaner results, revealing a gap between concept detection and intervention.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate whether sparse autoencoders (SAEs) can serve as controllable intervention points for concept-level manipulation in diffusion models.

Method: Compare direct latent steering via SAE features vs. using SAE activations solely as semantic detectors to identify object regions, then replace patch embeddings with those not containing the target object.

Result: Direct SAE intervention induces out-of-distribution activations and severe visual artifacts, while detection-based patch replacement preserves activation statistics and produces cleaner erasure results.

Conclusion: Monosemantic/sparse features are suitable for concept detection but not inherently suitable as control knobs for direct manipulation; there is a fundamental gap between detection and intervention in diffusion models.
...
TL;DR: Phantom is a unified face-swap deepfake protection framework that jointly constrains perturbations in latent and spatial domains, achieving significant improvements in protection success rates (up to 27.8%) against state-of-the-art face-swapping deepfakes while enhancing visual quality.


<details>
  <summary>Details</summary>
Motivation: Face-swapping deepfakes threaten personal privacy through unauthorized identity manipulation, and existing adversarial approaches struggle due to: (1) reliance on fixed/random targets causing ambiguous latent guidance, (2) lack of spatial constraints causing perturbations to spill into irrelevant regions, and (3) identity-style disentanglement suppressing adversarial signals during deepfake generation.

Method: Phantom jointly constrains perturbations in latent and spatial domains by: (1) adaptively synthesizing identity-shifted yet attribute-preserving targets for identity-aware latent optimization, and (2) applying masked perturbations confined to semantically relevant facial regions.

Result: Phantom improves protection success rates in dodging scenarios by 27.8%, 25.6%, and 16.6% on UniFace, INSwapper, and SimSwap respectively, while also enhancing visual quality. In impersonation scenarios, it yields up to 10.2% higher protection while improving perceptual fidelity.

Conclusion: Jointly leveraging latent and spatial constraints provides robust and coherent facial privacy protection against face-swapping deepfakes.
...
TL;DR: This paper introduces WIDER-FAIR, a new dataset built on WIDER-FACE with manual annotations of perceived ethnicity (Asian, Black, Indian, White) and sex for 16,256 images. Experiments with YOLOv5 reveal lower detection performance for Black individuals and that excluding this group increases fairness disparity most significantly.


<details>
  <summary>Details</summary>
Motivation: Face detection models show performance disparities across demographic groups, but studying these biases is hindered by the lack of datasets with sensitive feature annotations (e.g., ethnicity, sex).

Method: The authors manually annotated the WIDER-FACE dataset with ethnicity (4 groups) and sex labels, validated annotations using face embeddings, KNN classifier, and t-SNE visualization, then trained YOLOv5 models and performed ablation studies by excluding each demographic group.

Result: Detection performance is notably lower for Black individuals; excluding Black individuals from training increases fairness disparity more than excluding any other ethnic group.

Conclusion: Demographically annotated datasets like WIDER-FAIR are valuable for understanding and evaluating bias in face detection models.
...
TL;DR: UniCoder uses Symbolic Attribute Alignment and Reference-Guided Code Optimization within a unified RL framework to overcome reward coarseness and exploration stagnation in visual-to-code generation, achieving SOTA with 8B parameters.


<details>
  <summary>Details</summary>
Motivation: Standard MLLMs with SFT fail to achieve pixel-precise alignment for visual-to-code generation. RL is theoretically promising but faces two obstacles: Reward Coarseness (semantic metrics miss fine-grained errors) and Exploration Stagnation (sparse code space prevents bootstrapping).

Method: UniCoder integrates (1) Symbolic Attribute Alignment: a lightweight LLM parses code into discrete visual attributes for dense, element-wise rewards, and (2) Reference-Guided Code Optimization: injects ground-truth trajectories into low-performing rollouts to guide policy improvement.

Result: UniCoder (8B) surpasses all open-source baselines and achieves SOTA comparable to proprietary models on ChartMimic, UniSVG, Design2Code, and ScreenBench benchmarks.

Conclusion: Establishes a new paradigm for generalized visual-to-code synthesis by effectively addressing RL limitations in this domain.
...
TL;DR: MemLearner is a learning-based adaptive context query method for video world models that improves long-term scene consistency by using query tokens to bridge context and predicted tokens, outperforming previous rule-based methods especially in occlusion and dynamic object scenarios.


<details>
  <summary>Details</summary>
Motivation: Video world models lack memory, causing inconsistent generated scenes over extended durations. Previous rule-based context frame retrieval methods fail to generalize in scenarios with scene occlusions and dynamic objects.

Method: Proposes MemLearner, a learning-based adaptive context query method using query tokens to bridge context and predicted tokens. It leverages the video generation model itself for context querying (exploiting pre-trained visual priors), incorporates efficient training/inference strategies, collects a long video dataset with occlusions and dynamic objects, and uses a multi-dataset training strategy with annotated rendered and unannotated real-world videos.

Result: MemLearner significantly outperforms prior video world models in scene consistency and memory, particularly under challenging occlusion and dynamic scenarios.

Conclusion: Learning-based adaptive context query is an effective approach for improving memory in video world models, outperforming rule-based retrieval methods.
...
TL;DR: RhythmJEPA proposes a self-supervised rPPG framework using joint-embedding predictive learning, a Cyclic Rhythm-State Planner for cyclic pulse modeling, a Dual Order Mamba Encoder combining chronological and state-ordered scanning, and a Spatial Pulse Mixer for efficient pulse-sensitive feature extraction, achieving competitive results on PURE, UBFC-rPPG, and MMPD.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised rPPG methods reconstruct masked pixels or low-level visual features, biasing models toward facial appearance rather than physiological dynamics. Also, Mamba-based approaches scan tokens only in chronological order, failing to exploit the cyclic structure of pulse signals.

Method: (1) RhythmJEPA predicts latent teacher representations from masked facial videos instead of reconstructing RGB frames. (2) Cyclic Rhythm-State Planner (CRSP) estimates latent physiological states and decodes cyclic state paths via dynamic programming with constrained grammar. (3) Dual Order Mamba Encoder (DOM) combines chronological and state-ordered scanning. (4) Spatial Pulse Mixer (SPM) extracts compact pulse-sensitive facial tokens.

Result: Competitive performance over representative rPPG methods on PURE, UBFC-rPPG, and MMPD datasets.

Conclusion: The proposed RhythmJEPA effectively addresses limitations of prior self-supervised rPPG methods by learning physiology-aware representations in embedding space and explicitly modeling cyclic pulse structure, outperforming existing approaches.
...
TL;DR: JL1-CC&QA is a multi-task remote sensing benchmark extending JL1-CD with change captioning (17,021 captions) and change QA (20,060 QA pairs) using 5,000 bi-temporal image pairs from Jilin-1 satellite, created through a three-stage pipeline (MLLM generation, vision-grounded LLM judging, human verification).


<details>
  <summary>Details</summary>
Motivation: Traditional remote sensing change detection only identifies where changes occur (binary segmentation) but cannot describe what changed or why. The authors aim to bridge this semantic gap by providing richer annotations for change understanding.

Method: The benchmark extends JL1-CD with two annotation layers: (1) JL1-CC with 17,021 quality-verified change captions, and (2) JL1-QA with 20,060 question-answer pairs across 8 question types. Annotations are produced via a three-stage pipeline: multi-modal LLM generation, vision-grounded LLM judging, and human expert verification.

Result: JL1-CC&QA provides 17,021 change captions and 20,060 QA pairs across 5,000 bi-temporal image pairs at 0.5-0.75m resolution, supporting three tasks: binary change masks, change captions, and change-oriented QA.

Conclusion: This unified benchmark with change masks, captions, and QA over the same image set will serve as a valuable resource for advancing multi-task change understanding in remote sensing.
...
TL;DR: This paper uses rolling-shutter distortions—usually treated as artifacts—to estimate 3D translational and angular velocities of fast-moving spherical objects from a single frame, via a correspondence-free, geometry-driven optimization method.


<details>
  <summary>Details</summary>
Motivation: Rolling-shutter distortions in fast motion are typically seen as artifacts to correct; this work instead treats them as useful temporal cues for motion estimation.

Method: A robust spherical pattern is designed, and a correspondence-free formulation enforces geometric consistency in a back-projection framework; translational and rotational motions are decoupled via two-stage optimization leveraging sphere geometry.

Result: Extensive synthetic and real experiments show accurate and robust estimation of motion parameters under high-speed conditions.

Conclusion: Rolling-shutter distortions can be effectively repurposed for velocity estimation, even for textureless spherical objects.
...
TL;DR: This paper proposes Barycentric Dominance Field (BDF), a continuous representation for triangular meshes that enables diffusion models to generate high-quality 3D meshes, overcoming limitations of autoregressive methods like low face counts and limited vertex resolution.


<details>
  <summary>Details</summary>
Motivation: Autoregressive (AR) models for 3D mesh generation suffer from inherent constraints: limited face counts, bounded vertex resolutions, and difficulties supporting textures. The paper aims to overcome these bottlenecks by bridging the gap between discrete mesh topology and continuous diffusion-based generative modeling.

Method: The authors propose Barycentric Dominance Field (BDF), a continuous representation defined on triangular mesh surfaces that encodes vertex topological connectivity. BDF transforms discrete connectivity into a continuous surface signal, similar to texture maps, allowing seamless integration into existing 3D diffusion pipelines without architectural modifications.

Result: BDF empowers diffusion models to generate native meshes with significantly higher quality, greater scalability, and stronger robustness compared to state-of-the-art autoregressive methods.

Conclusion: BDF effectively bridges discrete mesh topology and continuous diffusion models, enabling high-quality mesh generation with better scalability and robustness than AR approaches.
...
TL;DR: SpikeLogBERT is a spiking neural network framework for log parsing that achieves high accuracy (0.99997) while reducing energy consumption by up to 62.6% compared to ANN-based models.


<details>
  <summary>Details</summary>
Motivation: Neural log parsing approaches rely on dense matrix multiplications, leading to high computational cost and energy consumption. This paper aims to develop an energy-efficient alternative.

Method: SpikeLogBERT integrates a spiking transformer architecture with knowledge distillation from a BERT teacher model, using sparse spike activations and event-driven processing to reduce active operations during inference.

Result: On the HDFS dataset, SpikeLogBERT achieves a parsing accuracy of 0.99997, outperforming ANN-based neural log parsing models, while reducing estimated theoretical energy consumption by up to 62.6% under standard 45nm CMOS assumptions.

Conclusion: SpikeLogBERT demonstrates that spiking neural networks can be effectively applied to log parsing, combining high accuracy with significant energy efficiency gains, establishing a benchmark for future research in this direction.
...
TL;DR: This paper introduces a self-supervised temporal regularization method as post-training refinement for graph-based cardiac segmentation, enforcing temporal consistency across image sequences without per-frame annotations. It also enables automatic mapping to AHA 17-segment clinical standard for standardized regional assessment.


<details>
  <summary>Details</summary>
Motivation: Models trained on independent frames of cardiac ultrasound sequences produce temporal discontinuities that affect reliable clinical measurements. The authors aim to enforce consistent segmentation and motion estimation over time without requiring additional annotations.

Method: Self-supervised temporal regularization is applied as a post-training refinement stage, penalizing velocity and acceleration discontinuities across consecutive frames. Anatomical correspondences are leveraged to automatically map landmarks to the AHA 17-segment clinical standard.

Result: Validation on CAMUS dataset demonstrates clinical utility of combining temporal consistency with automatic regional mapping for detecting pathological myocardial motion patterns.

Conclusion: The proposed approach achieves temporally consistent segmentations while maintaining learned anatomical correspondences, enabling standardized regional assessment without per-frame annotations.
...
TL;DR: MuSViT is the first foundation vision model for sheet music, pre-trained via Masked Autoencoders on 9.7 million IMSLP pages, using a two-stage curriculum. It outperforms general-purpose encoders on sheet music tasks and encodes musical structure directly in its representation space.


<details>
  <summary>Details</summary>
Motivation: Foundation models exist for vision and language but sheet music lacks a strong domain-specific backbone. The authors aim to fill this gap with a dedicated sheet music representation model.

Method: A ViT encoder pre-trained via Masked Autoencoders on 9.7 million IMSLP pages using a two-stage curriculum: synthetic warm-up on typeset scores, then large-scale training on real-world IMSLP corpus.

Result: Under linear probing, MuSViT consistently outperforms modern vision encoders. Under fine-tuning, it generally improves upon task-specific SOTA. Embedding-transcription analysis shows MuSViT encodes symbolic musical structure, unlike general-purpose encoders.

Conclusion: MuSViT establishes itself as a foundation backbone for sheet music understanding, proving that domain-specific pre-training is crucial for structured symbolic properties of musical notation.
...
TL;DR: TopoGPT is a generative framework that uses autoregressive sequence modeling to learn geometry priors from lane graph structures, achieving geometrically consistent and complete lane topology reasoning, outperforming existing methods on OpenLane-V2 by +6.4 (lane-level) and +11.6 (point-level).


<details>
  <summary>Details</summary>
Motivation: Existing lane topology reasoning methods follow a detection-and-association paradigm treating each lane instance independently, leading to geometric inconsistency at connected endpoints and incomplete graphs due to visual occlusions.

Method: (1) Construct a large-scale map dataset (3.3M scenes). (2) Use a lane tokenizer to serialize lane graphs into discrete tokens and a scene context encoder to extract global features as scene tokens. (3) Pre-train an autoregressive lane sequence transformer via scene-conditioned next-token prediction. (4) Use a perception adapter to align BEV features with the pre-trained scene condition for sensor-based prediction.

Result: Outperforms existing methods by an average of +6.4 on lane-level and +11.6 on point-level metrics on OpenLane-V2 benchmark, producing geometrically consistent and structurally complete lane graphs.

Conclusion: Learning geometry priors from typical lane graph structures via autoregressive modeling effectively addresses geometric inconsistency and occlusion issues in lane topology reasoning.
...
TL;DR: ADER is a fast, absorption-guided framework for LWIR hyperspectral passive ranging that decouples distance from emissivity/temperature, classifies pixels by emission/reflection dominance, uses greedy band selection, and achieves LiDAR-consistent results with ~100x speedup over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing LWIR hyperspectral ranging methods are computationally expensive, rely on full-band measurements, and do not explicitly exploit sharp atmospheric absorption structures, while distance inversion is ill-posed due to nonlinear coupling with temperature, emissivity, and path radiance.

Method: ADER uses B-spline emissivity representation with smoothness prior to avoid overfitting to absorption structures; classifies pixels via ozone-absorption cues into emission-dominant (1D absorption-residual minimization) and reflection-dominant (downwelling-radiance compensation) categories; employs greedy band selection based on multi-scene Fisher information.

Result: ADER recovers LiDAR-consistent spatial distance structures under both full-band and 20-band settings, improves ranging accuracy, and achieves ~2 orders of magnitude speedup over a public full-band method.

Conclusion: ADER effectively addresses the ill-posed distance inversion problem in LWIR hyperspectral passive ranging with significant computational efficiency gains, while maintaining high accuracy comparable to LiDAR.
...
TL;DR: MRPO introduces step-wise process rewards to penalize early reasoning errors in medical VQA, reducing cascading failures and improving accuracy across MLLMs.


<details>
  <summary>Details</summary>
Motivation: Existing post-training pipelines are outcome-centric (final answer correctness), suffering from sparse credit assignment. Cascading errors from early-stage reasoning failures are a leading cause of incorrect predictions in medical VQA.

Method: MRPO (Medical Reasoning-aware Policy Optimization) - an RL algorithm incorporating step-wise process rewards that assigns exponentially larger penalties to tokens in earlier invalid reasoning steps when the final answer is incorrect.

Result: MRPO consistently outperforms GRPO and recent RL baselines across three MLLM backbones; Qwen3-VL-8B-Instruct surpasses HuatuoGPT-Vision-34B by 2.79 points; reduces early-stage reasoning failures from 64.0% to 13.0%.

Conclusion: Targeted mitigation of cascading failures via step-wise process rewards improves both reasoning quality and final answer accuracy in medical VQA.
...
TL;DR: This paper introduces geospatial visual priors (street-level visual context anchored to driving routes) and a dual-memory augmentation module that improves end-to-end autonomous driving by providing anticipatory foresight, boosting performance on NAVSIM-v2, and enhancing robustness against sensor corruption.


<details>
  <summary>Details</summary>
Motivation: End-to-end autonomous driving methods rely on instantaneous sensor data, limiting them to reactive behavior without the anticipatory foresight that human drivers gain from prior experience.

Method: Introduces geospatial visual priors (street-level visual context linked to the driving route) and a memory augmentation module with a dual-memory architecture (contextual memory + persistent fallback memory) and an adaptive memory gate that regulates memory influence based on current state compatibility.

Result: Consistently improves performance across diverse end-to-end baselines on the NAVSIM-v2 benchmark; improves robustness against sensor corruption; ensures safe fallback when retrieved priors are unreliable.

Conclusion: Geospatial visual priors and the dual-memory design effectively provide visual-spatial foresight independent of real-time sensors, enhancing both performance and robustness in autonomous driving.
...
TL;DR: RT-SFOD achieves state-of-the-art source-free object detection under domain shift using YOLOv10's dual-head architecture, introducing DHF for pseudo-label fusion and MARD for multi-scale feature diversification, yielding better accuracy, 1.3× higher throughput, and ~2× fewer parameters than prior methods.


<details>
  <summary>Details</summary>
Motivation: Real-world detectors must handle domain-shifts under strict latency/memory constraints, but existing SFOD methods rely on heavyweight architectures that prioritize accuracy alone, ignoring the speed-accuracy-model size trade-off.

Method: Proposes RT-SFOD building on YOLOv10 (NMS-free dual-head detector) with two key modules: (1) DHF (Dual-Head Pseudo-Label Fusion) that selectively fuses O2O and O2M head predictions for better supervision, and (2) MARD (Multi-scale Adaptive Representation Diversification) loss enforcing detection-aware variance/covariance constraints on multi-scale feature maps. Both are training-time only.

Result: 1.4-3.5% mAP gains, 1.3× higher throughput, ~2× fewer parameters than prior SFOD methods across domain-shift benchmarks. Generalizable to YOLO- and DETR-based dual-head detectors.

Conclusion: The speed-accuracy-model size trade-off in SFOD is unnecessary; RT-SFOD advances the Pareto frontier, showing efficient dual-head detectors can achieve superior adaptation performance without heavyweight architectures.
...
TL;DR: Consispace is a semantic-aware resampling framework for volumetric medical images that enforces consistent voxel spacing using ODE-based anatomical constraints and semantic feature reweighting, improving both reconstruction quality and downstream segmentation performance.


<details>
  <summary>Details</summary>
Motivation: Physical voxel spacing in volumetric medical images is often ignored in segmentation research; standard resampling is treated as a preprocessing step without systematic exploitation of its interaction with downstream segmentation.

Method: Consispace uses an ODE-based anatomical constraint for continuous inter-slice modeling, and leverages pretrained vision model features to build intra-slice semantic correlation maps with class-wise feature reweighting, integrated into an implicit neural network for arbitrary-scale resampling.

Result: Superior reconstruction quality and perceptual fidelity, smoother inter-slice anatomy, and improved downstream segmentation performance across multiple datasets.

Conclusion: Semantic-aware, consistent resampling that couples anatomy and segmentation objectives significantly benefits volumetric medical image analysis.
...
TL;DR: SENSE-VAD is a synthetic benchmark for socially complex anomaly detection in autonomous driving, featuring scenarios where danger arises from inter-agent relationships rather than motion alone. Current anomaly detectors fail on these challenges.


<details>
  <summary>Details</summary>
Motivation: AVs need to handle socially complex situations (e.g., child running from guardian, person being carried, pursuit) where anomalies are defined by social context, not motion statistics. Existing anomaly detectors only flag motion/appearance-based anomalies.

Method: Using CARLA + Unreal Engine, the authors generate synthetic videos with six anomaly categories (individual behaviors, group behaviors, person-object interactions, cyclist interactions, vehicle & agent), annotated per-frame. They also include real-world videos for sim-to-real evaluation.

Result: State-of-the-art video anomaly detection baselines perform poorly on SENSE-VAD, confirming that socially complex anomalies are a distinct and unsolved challenge.

Conclusion: Socially complex anomalies require new detection approaches beyond motion/appearance-based methods. The benchmark and code are publicly available to facilitate research.
...
TL;DR: RESOLVE is a large-scale real-world roadside benchmark dataset with multi-resolution LiDAR and synchronized cameras for evaluating unimodal and fusion-based 3D detection and tracking, revealing how multimodal fusion compensates for LiDAR point sparsity.


<details>
  <summary>Details</summary>
Motivation: To systematically investigate how unimodal and camera-LiDAR fusion architectures perform under varying LiDAR point sparsity caused by sensor configurations and scene-dependent conditions, which remains underexplored in roadside cooperative perception.

Method: Built a large-scale real-world dataset (RESOLVE) with over 100k images, 26k point cloud frames, and 220k labeled bounding boxes across 10 classes, captured at an urban intersection under diverse lighting and weather with three controlled LiDAR resolution levels for fair cross-architecture comparison.

Result: Benchmark experiments reveal how multimodal fusion compensates for LiDAR point sparsity, providing insights into cost-efficient roadside multimodal perception design under resolution variations, sensing distance, and training-inference resolution mismatches.

Conclusion: Multimodal fusion can effectively mitigate LiDAR point sparsity issues; the dataset enables systematic evaluation and offers clues for designing cost-efficient roadside perception systems.
...
TL;DR: MLLMs waste compute on redundant visual-token updates (answer-silent redundancy). The authors propose an operator-level skipping framework that selectively bypasses redundant attention/FFN operators per layer, preserving full visual sequences while cutting 33.7% TFLOPs with minimal accuracy loss (99.5% performance retention).


<details>
  <summary>Details</summary>
Motivation: Existing MLLM acceleration methods use coarse strategies (removing visual tokens or skipping entire layers), which discard fine-grained evidence or suppress useful operators alongside redundant ones. The authors aim for a finer-grained, operator-level skipping approach.

Method: From an answer-observable perspective, the authors find late visual-token updates have little effect on answer-token representations (answer-silent redundancy). They decompose each Transformer layer into attention and FFN operators, and propose an operator-level visual-token skipping framework that selectively bypasses redundant attention, FFN, or both, depending on layer-wise dominance.

Result: Across 3 MLLM architectures and 10 VQA benchmarks, the method reduces 33.7% TFLOPs on Qwen3-VL while retaining 99.5% of vanilla model performance, achieving strong efficiency-accuracy trade-offs.

Conclusion: Operator-level visual-token skipping is more effective than coarse token/layer removal, preserving full visual sequences while significantly reducing computation with negligible accuracy degradation.
...
TL;DR: DriveWeaver is a controllable vehicle insertion framework for autonomous driving simulation that uses video inpainting conditioned on vehicle point clouds to seamlessly blend foreground vehicles into background scenes, outperforming existing 3D asset-based methods in visual realism and geometric consistency.


<details>
  <summary>Details</summary>
Motivation: Existing vehicle insertion methods rely on pre-reconstructed 3D assets, causing lighting inconsistencies between foreground and background, and their dependence on limited manually-curated 3D assets hinders large-scale deployment.

Method: DriveWeaver uses video inpainting conditioned on vehicle point clouds for seamless blending, employs a global-to-local hierarchical inpainting strategy for long-term identity/appearance consistency, and extracts explicit 3D Gaussian representations via urban reconstruction for real-time rendering.

Result: Extensive experiments across diverse datasets show DriveWeaver outperforms existing baselines in visual realism and geometric consistency.

Conclusion: DriveWeaver provides a robust tool for scalable autonomous driving scene augmentation, enabling better testing and improvement of autonomous driving models through diverse corner case generation.
...
TL;DR: InstanceControl is a multi-instance controllable image generation method that eliminates manual instance labeling by using Vision-Language Models (VLM) to automatically establish instance-level correspondences between text prompts and visual conditions, with an adaptive mask refinement strategy to handle noisy predictions.


<details>
  <summary>Details</summary>
Motivation: Existing controllable generation methods (e.g., ControlNet) struggle with complex multi-instance scenes, causing attribute confusion among instances. Recent approaches require labor-intensive manual instance labeling, which is impractical.

Method: InstanceControl leverages VLM to automatically parse instance descriptions from text prompts and predict instance masks from visual conditions. An adaptive mask refinement strategy dynamically refines noisy instance masks during generation.

Result: Outperforms state-of-the-art methods with superior fidelity and precise instance-level control.

Conclusion: InstanceControl effectively addresses multi-instance attribute confusion without manual labeling, achieving high-quality controllable generation results.
...
TL;DR: VidPair-Halluc is a new benchmark of 1K adversarial video pairs and 11K QA pairs that isolates video hallucination in LVMs by keeping backgrounds consistent while varying foreground semantics, revealing persistent model weaknesses in fine-grained video understanding.


<details>
  <summary>Details</summary>
Motivation: Existing video hallucination benchmarks rely on text-based perturbations or adversarial questions but fail to control for visual background consistency, making it difficult to attribute errors to genuine hallucination vs. background variation.

Method: The benchmark is built via PairFlow, a pipeline using text-to-image and video generation to compose stories, generate coherent clips, and assemble adversarial video pairs with similar backgrounds but different foreground semantics. It covers 10 semantic aspects of spatial/temporal reasoning with 1K adversarial video pairs and 11K spatio-temporal QA pairs.

Result: Evaluations on mainstream LVMs show persistent difficulty with robust fine-grained video understanding under adversarial settings.

Conclusion: The benchmark effectively isolates genuine video hallucination and reveals that current LVMs still struggle with fine-grained video understanding when backgrounds are controlled.
...
TL;DR: WNM decouples physical world representation (4D structured narrative) from pixel generation, replacing black-box video sampling with deterministic pre-visualization, enabling controllable, professional-grade video production.


<details>
  <summary>Details</summary>
Motivation: Current video generation models lack controllability — they treat video as pixel distribution sampling, bypassing explicit 4D physical world (geometry, motion, camera, lighting). This causes the "gacha" loop, making professional content creation inefficient and expensive.

Method: Decouples "what to render" (structured physical narrative) from "how to render" (pixel generation). Collaborative agents translate sparse multimodal inputs into a fully editable 4D world representation (geometry, skeleton motion, trajectories, camera, lighting). This acts as a deterministic blueprint that drives existing video foundation models (frozen/lighty adapted) as neural shaders.

Result: WNM greatly reduces probabilistic "gacha" calls and produces videos whose layout, motion, and cinematography closely follow creator intent.

Conclusion: The framework is open and modular, enabling independent improvement of each component (world representation, control agents, adapters).
...
TL;DR: AnyBokeh is a physics-guided framework for any-to-any bokeh editing that estimates source blur state via signed circle-of-confusion and disparity maps, then performs relative blur synthesis conditioned on source and target settings, avoiding the need for all-in-focus reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing bokeh editing methods require all-in-focus input or reconstruction, discarding useful blur cues and propagating artifacts. A practical editor should handle arbitrary source focus/aperture settings.

Method: Estimates signed circle-of-confusion (CoC) map and disparity map; models linear relation between signed CoC and disparity difference to compute source-specific optical fingerprint; uses a generative editor conditioned on source and target CoC maps for relative blur synthesis.

Result: Achieves faithful and controllable any-to-any bokeh editing, all-in-focus-to-bokeh rendering, and defocus deblurring on real-world benchmarks, outperforming methods that require all-in-focus reconstruction or test-time calibration.

Conclusion: AnyBokeh demonstrates that leveraging source blur cues via physics-guided modeling enables more flexible and robust bokeh editing without intermediate all-in-focus reconstruction.
...
TL;DR: The paper proposes a unified SfM framework that uses planar surfaces (via homography decomposition) rather than standard epipolar geometry to robustly estimate camera poses, using a graph-based approach with spectral embedding to filter unreliable edges, achieving superior results on planar scenes and competitive results on general 3D scenes.


<details>
  <summary>Details</summary>
Motivation: Traditional SfM systems fail in planar scenes due to degeneracy in epipolar geometry; the authors aim to turn this limitation into an advantage by leveraging planes as geometric constraints.

Method: A homography-based pose estimation framework: (1) decompose homographies from planar surfaces across multiple views for pose estimation, (2) construct a pose-graph from these estimates, (3) use spectral embedding to filter unreliable edges based on geometric/visual consistency, (4) extract a maximally consistent spanning tree for final pose recovery.

Result: Superior performance on basketball court imagery (highly planar scenes) where traditional methods struggle; matches or exceeds state-of-the-art on unconstrained outdoor scenes from the IMC Phototourism benchmark.

Conclusion: Planar surfaces, often considered problematic for SfM, can be effectively used as a source of robust geometric constraints, enabling a unified approach that works for both planar and general 3D environments.
...
TL;DR: LUNA is an LBS-free neural animation model that directly maps 2D controls (images, keypoints, sketches) to 3D Gaussian deformations for photorealistic, animatable human avatars, using a transformer-based motion regressor and hybrid supervision to achieve competitive visual fidelity and zero-shot cross-identity generalization.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on Linear Blend Skinning (LBS) and parametric body models, which limit expressivity and introduce artifacts from imperfect fitting.

Method: A transformer-based motion regressor disentangles global rigid motion from local dynamics; hybrid supervision distills soft priors from an LBS teacher and supports training on both fitted data and unlabeled videos.

Result: Competitive visual fidelity vs. LBS methods, realistic motion, and zero-shot cross-identity generalization across diverse driving modalities.

Conclusion: First end-to-end 3D animatable model supporting implicit 2D driving, bypassing explicit body fitting entirely.
...
TL;DR: ERA is an entropy-guided visual token pruning framework with rectified attention that reduces inference costs in MLLMs by addressing Attention Logit Collapse through Dual-view Entropy Pruning, Bias-aware Token Recycling, and Logit-preserving Attention Rectification.


<details>
  <summary>Details</summary>
Motivation: MLLMs have prohibitive inference costs due to long visual token sequences, and existing training-free visual token reduction methods distort attention distributions, causing a phenomenon called Attention Logit Collapse.

Method: ERA has three components: 1) Dual-view Entropy Pruning (DEP) identifies anchor tokens via visual diversity and head-wise saliency; 2) Bias-aware Token Recycling (BTR) recycles pruned tokens into anchors with cluster-level logit bias estimation; 3) Logit-preserving Attention Rectification (LAR) injects estimated bias to rectify attention logit collapse.

Result: Preserves visual evidence under aggressive compression, enabling robust performance across single-image, multi-image, and video settings on a wide range of MLLMs.

Conclusion: ERA establishes logit-preserving visual token pruning as a principled framework for efficient MLLMs, unifying theoretical foundation, algorithmic design, and practical deployment.
...
TL;DR: CoLT (Chain of Latent Thoughts) is a framework enabling multi-modal LLMs to reason through compact latent state representations instead of verbose text tokens, using step-level supervision from a lightweight decoder (forward + backward modes) plus internal supervision. It reduces inference time by 10.1× and text decoding by 22.6× while outperforming existing latent reasoning methods on eight benchmarks.


<details>
  <summary>Details</summary>
Motivation: Text-based chain-of-thought reasoning is slow (thousands of tokens) and limited by natural language expressiveness. The authors aim to replace verbose text reasoning with compact, efficient latent thought chains.

Method: CoLT trains MLLMs to reason through a chain of latent thought representations (as few as 3 steps). It uses a lightweight external decoder for step-level supervision: (1) forward mode decodes latent thoughts into next-step textual reasoning, (2) backward mode aligns decoder hidden states with latent thoughts given prior text. Internal supervision ensures coherent latent transitions. The decoder is removed at inference for efficiency.

Result: Outperforms CODI, SIM-CoT, and latent visual reasoning methods with costly annotations. Reduces inference time by 10.1× and text decoding by 22.6× compared to text CoT. Validated on eight benchmarks.

Conclusion: Latent chain-of-thought reasoning is a viable, efficient alternative to text-based CoT, achieving faster inference and better or comparable accuracy without reliance on expensive annotations like auxiliary images.
...
TL;DR: AutoBackSwap reduces classifiers' reliance on spurious background correlations by automatically swapping backgrounds via a secondary network that disentangles foreground/background, requiring only a few hundred patch-wise labeled samples, and outperforms prior methods across multiple tasks.


<details>
  <summary>Details</summary>
Motivation: Deep neural network classifiers often fail due to relying on spurious correlations (e.g., background features) that don't generalize. The authors aim to reduce this reliance, particularly when no training samples break the spurious correlation.

Method: AutoBackSwap uses a secondary network to disentangle foreground and background, infills to synthesize complete backgrounds, then combines different foregrounds with inpainted backgrounds to augment training data. Only a few hundred patch-wise labeled samples are needed.

Result: AutoBackSwap consistently outperforms prior methods on image classification tasks with spurious backgrounds, and proves effective even when no training samples break the spurious correlation.

Conclusion: Automated background swapping is an effective data augmentation strategy to mitigate spurious background correlations, requiring minimal labeling effort and working in challenging scenarios where no counter-examples exist.
...
TL;DR: The paper introduces Cross-Space Distillation, a method for distilling knowledge from modern high-capacity diffusion models (like SD 3.5, Flux) into compact student models (like SD 1.5) that operate in different latent spaces. They propose the Bridge, a lightweight latent interface that aligns student and teacher latent representations, enabling effective one-step distillation despite resolution and VAE mismatches.


<details>
  <summary>Details</summary>
Motivation: Existing timestep distillation assumes Teacher and Student share the same latent space (Shared-Space constraint), which prevents distilling modern high-capacity Teachers into compact, deployment-friendly Students that use different latent resolutions and VAE parameterizations.

Method: Introduces Bridge — a lightweight latent interface combining a frozen Student VAE decoder (spatial prior) with a compact learnable projector, trained with latent reconstruction and attention fidelity objectives to map Student latents into Teacher space without modifying the Student backbone.

Result: Bridge enables substantial gains for compact one-step Students across diverse modern Teachers; e.g., SD 1.5 improves from 5.4 to 9.4 HPSv3 while preserving one-step inference, low latency, and ecosystem compatibility.

Conclusion: Heterogeneous large Teachers can be effectively distilled into efficient, deployable backbones through a lightweight latent-space interface (Bridge), overcoming the Shared-Space constraint.
...
TL;DR: FLORA is a deep learning framework using octree-based point cloud processing with auxiliary data fusion to predict six forest attributes from heterogeneous LiDAR data across France, achieving robust results (e.g., R²=0.88 for dominant height) from a single cross-season model.


<details>
  <summary>Details</summary>
Motivation: National LiDAR programs across Europe face heterogeneous acquisition conditions (varying sensors, flight parameters, seasons, scan angles), making wall-to-wall forest attribute prediction challenging since existing models are typically calibrated for local conditions only.

Method: FLORA combines an octree-based backbone for processing heterogeneous LiDAR point clouds with ecological and spatiotemporal auxiliary variables via a late-fusion gating mechanism, trained on 32,052 NFI plots from the French LiDAR HD program.

Result: A single model trained on both leaf-on and leaf-off data outperforms season-specific models; auxiliary variables help species-specific volume prediction; FLORA achieves rRMSE of 12.3% (R²=0.88) for dominant height and 39% (R²=0.74) for total volume.

Conclusion: FLORA provides a robust baseline for large-scale forest attribute estimation from heterogeneous national LiDAR programs, demonstrating cross-season robustness without needing separate models per acquisition condition.
...
TL;DR: A zero-shot, training-free framework for generating 360° panoramic images/videos by injecting spherical priors into pre-trained diffusion transformers via Spherical RoPE and Semantic Distortion CFG.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 360° panoramic generation either require costly fine-tuning on scarce panoramic data (limiting generalization) or use multi-step optimization (high inference latency). The authors aim to achieve high-quality panoramic generation without retraining or optimization.

Method: Spherical RoPE replaces standard rotary position embeddings: low-frequency channels re-parameterized as 3D Cartesian coordinates to encode spherical manifold, high-frequency channels harmonically quantized for exact periodicity. Coupled with Semantic Distortion classifier-free guidance (CFG) to steer geometry.

Result: Achieves competitive performance against baselines across text-to-panorama tasks using Flux.1, Flux.2, and LTX-Video backbones, while remaining training-free and optimization-free.

Conclusion: The proposed zero-shot approach generalizes across diverse backbones and 360° generation modalities, inheriting full creative breadth of state-of-the-art models without retraining.
...
TL;DR: PointSplat directly predicts compact 3D Gaussian primitives from input point sets via a coarse geometry proxy and Point-Image Transformer, achieving high-quality novel-view synthesis with significantly fewer Gaussians and robust performance across varied view counts and resolutions.


<details>
  <summary>Details</summary>
Motivation: Existing feed-forward reconstruction methods encode the same subject repeatedly across multiple views, causing high inter-view redundancy and inefficiency for live streaming where compactness and bandwidth are critical.

Method: PointSplat first estimates a coarse geometric proxy and performs ray casting to prune redundant points and establish 2D-3D correspondences. Then, a Point-Image Transformer fuses appearance and geometry features to predict Gaussian attributes in a single forward pass, restricting predictions to foreground regions only.

Result: PointSplat achieves higher efficiency and quality than existing methods, while showing strong robustness to varying view counts and image resolutions across multiple datasets.

Conclusion: Direct 3D-space prediction of Gaussian primitives from point sets is an effective strategy for compact and high-fidelity human representations, suitable for immersive live streaming systems.
...
TL;DR: GEAR jointly trains VQ tokenizer and autoregressive generator end-to-end via representation alignment, using a dual read-out (hard branch for AR training, soft branch for guiding tokenizer), achieving up to 10x faster ImageNet gFID convergence over LlamaGen-REPA.


<details>
  <summary>Details</summary>
Motivation: Current visual generative models train tokenizer and generator separately in two stages, where the tokenizer is unaware of what the generator finds easy to model, leading to suboptimal alignment between the two components.

Method: GEAR uses a dual read-out of the codebook assignment: a hard one-hot branch trains the AR with next-token prediction, while a differentiable soft branch carries a representation-alignment loss to guide only the tokenizer, enabling end-to-end joint training despite the non-differentiable VQ index.

Result: GEAR achieves up to 10x faster ImageNet gFID convergence compared to LlamaGen-REPA baseline, learns better patch-level and spatially-coherent features, and generalizes across different quantizers (VQVAE, LFQ, IBQ) and to text-to-image generation.

Conclusion: End-to-end joint training of tokenizer and generator with representation alignment shifts the alignment burden from tokenizer to AR model, producing more coherent features and significantly faster convergence, demonstrating a promising direction for visual generative modeling.
...
TL;DR: FaceMoE introduces a Mixture of Experts Transformer architecture for low-resolution face recognition, using specialized FFN experts and a top-k router to dynamically assign tokens, enabling resolution-aware feature extraction and mitigating catastrophic forgetting during fine-tuning. It outperforms SOTA across 11 datasets.


<details>
  <summary>Details</summary>
Motivation: LR-FR is challenging due to poor feature extraction from degraded probe images (blur, occlusion, low contrast) and the domain gap between HR gallery and LR probe images. Single feature encoders struggle to generalize across both domains, and catastrophic forgetting exacerbates this issue.

Method: FaceMoE adapts the MoE Transformer architecture by introducing multiple specialized FFN experts and a top-k router that dynamically assigns tokens to appropriate experts. This promotes specialization across semantic face regions for resolution-aware feature extraction. Sparse expert activation preserves pretrained knowledge and increases capacity without proportional compute overhead. Training combines face recognition loss, router z-loss, and load balancing loss.

Result: Extensive experiments across 11 datasets (HR, mixed-quality, LR benchmarks) show FaceMoE significantly outperforms state-of-the-art methods. This is the first work leveraging MoE for LR-FR.

Conclusion: FaceMoE effectively addresses LR-FR challenges through MoE-based dynamic token routing and expert specialization, achieving superior performance while maintaining efficient computation and mitigating catastrophic forgetting.
