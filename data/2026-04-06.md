...
TL;DR: A synthetic data pipeline for reasoning in long-document visual understanding that generates thinking traces, trains models via SFT with control tokens, and internalizes reasoning via model merging, achieving state-of-the-art performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Visual long-document understanding is critical for enterprise, legal, and scientific applications, but existing open solutions haven't explored reasoning capabilities, which have driven significant improvements in math and code performance.

Method: 1) Synthetic data pipeline generates thinking traces by scoring page relevance, extracting textual evidence, and ordering it by relevance. 2) Apply supervised fine-tuning (SFT) to these traces within <think> tags, gated by <cot> control token. 3) Internalize reasoning capability via low-strength model merging.

Result: Qwen3 VL 32B achieves 58.3 on MMLongBenchDoc, surpassing 7x larger Qwen3 VL 235B A22B (57.0). With Mistral Small 3.1 24B, synthetic reasoning outperforms distillation from Thinking version's traces by 3.8 points on MMLBD-C, and internalized reasoning shows 12.4x fewer mean output tokens compared to explicit reasoning.

Conclusion: The proposed synthetic reasoning pipeline effectively enhances visual long-document understanding, achieving better performance than much larger models while reducing output length, with the pipeline released for reproducibility and further exploration.
...
TL;DR: A quantitative flow matching framework for adaptive image denoising that estimates input noise levels and adapts inference trajectories accordingly, improving both accuracy and efficiency across varying noise conditions.


<details>
  <summary>Details</summary>
Motivation: Diffusion/flow-based models struggle with unknown and varying noise conditions due to inconsistent vector fields across noise levels, leading to degraded restoration when training and inference noise levels mismatch.

Method: Estimates input noise level from local pixel statistics, then adapts inference trajectory (starting point, number of integration steps, step-size schedule) based on the quantitative estimate to align with actual corruption level.

Result: Improves restoration accuracy and inference efficiency by reducing unnecessary computation for lightly corrupted images while providing sufficient refinement for heavily degraded ones.

Conclusion: The quantitative flow matching framework demonstrates robustness and strong generalization across diverse noise levels and imaging conditions (natural, medical, microscopy images).
...
<details>
  <summary>Details</summary>
Motivation: 6G requires environment-aware channel prediction for vehicular communications with reliability, latency, and adaptability demands. Traditional models lack balance between accuracy, generalization, and deployability, while sensing devices offer environmental priors.

Method: Multimodal visual feature fusion framework using GPS data and vehicle-side panoramic RGB images. Extracts semantic, depth, and position features via three-branch architecture with adaptive fusion via squeeze-excitation attention gating. Includes dedicated regression head and composite multi-constraint loss for 360° angular power spectrum prediction.

Result: Best RMSE of 3.26 dB for path loss, 37.66 ns for delay spread, 5.05° for azimuth spread of arrival, 5.08° for azimuth spread of departure, and mean/median APS cosine similarities of 0.9342/0.9571 on urban V2I measurement dataset.

Conclusion: The framework demonstrates strong accuracy, generalization, and practical potential for intelligent channel prediction in 6G vehicular communications by effectively leveraging multimodal environmental information.
...
TL;DR: VE-MD is a privacy-aware group emotion recognition framework that avoids individual monitoring by learning shared latent representations optimized for group-level affect prediction through structural supervision.
...
Method: Proposes VE-MD (Variational Encoder-Multi-Decoder) framework that learns shared latent representations jointly optimized for emotion classification and internal prediction of body/facial structural representations using transformer-based PersonQuery decoder and dense Heatmap decoder.

Result: Achieves SOTA on GAF-3.0 (90.06%) and VGAF (82.25% with audio fusion); structural supervision improves representation learning; reveals key distinction between GER and IER - structural outputs help GER while acting as denoising bottleneck for IER.

Conclusion: Preserving interaction-related structural information is crucial for group-level affect modeling without individual feature extraction; privacy-aware design can achieve competitive performance while avoiding explicit individual monitoring.
...
TL;DR: LumiVideo is an AI system that mimics professional colorists' workflow to automatically grade raw log video into cinematic visuals using LLM reasoning and generates industry-standard color configurations.


<details>
  <summary>Details</summary>
Motivation: Existing automated video color grading methods lack interpretability and iterative control, acting as black-box executors that don't match professional workflows. Professionals need systems that understand the cognitive process of color grading and provide editable, industry-standard outputs.

Method: LumiVideo uses a four-stage agentic system: 1) Perception analyzes raw log video's physical lighting and semantic content, 2) Reasoning combines LLM cinematic knowledge with RAG and Tree of Thoughts search to navigate color parameters, 3) Execution compiles parameters into ASC-CDL configurations and 3D LUTs with guaranteed temporal consistency, 4) Reflection allows natural language feedback for refinement.

Result: LumiVideo approaches human expert quality in automatic mode while enabling precise iterative control. The system also introduces LumiGrade, the first log-encoded video benchmark for evaluating automated grading.

Conclusion: LumiVideo successfully mimics professional colorist workflows, providing both automated high-quality results and the interpretability/control needed by professionals, representing a significant advancement over existing black-box methods.
...
TL;DR: Comparison of SVM and Decision Trees for detecting violin width reduction using 3D photogrammetric meshes, with geometry-based elevation maps vs. feature-engineered contour line fitting approaches.


<details>
  <summary>Details</summary>
Motivation: To automatically detect violin width reduction using 3D photogrammetric data, comparing different machine learning approaches and feature representations for this specific musical instrument analysis task.

Method: Two approaches compared: 1) SVM and Decision Trees applied to geometry-based raw representation from elevation maps, and 2) more targeted feature-engineered approach using parametric contour lines fitting on 3D photogrammetric meshes.

Result: Elevation maps occasionally achieve strong results but their performance does not surpass that of the contour-based inputs. The feature-engineered contour line approach outperforms the raw geometry-based elevation map representation.

Conclusion: For violin width reduction detection using 3D photogrammetric data, targeted feature engineering with parametric contour lines fitting is more effective than raw geometry-based elevation maps, even when using the same machine learning classifiers (SVM and Decision Trees).
...
TL;DR: PlayGen-MoG is a novel framework for generating realistic multi-agent trajectories in team sports from static formations, using Mixture-of-Gaussians with shared weights and relative spatial attention to avoid mode collapse and enable play design without observed history.


<details>
  <summary>Details</summary>
Motivation: Existing generative models (CVAE, diffusion) struggle with multi-agent trajectory generation in sports, suffering from posterior collapse or convergence to dataset mean. Most methods require observed trajectory history, limiting their use for play design where only initial formations are available.

Method: Three key design choices: 1) Mixture-of-Gaussians output head with shared mixture weights across all agents, 2) relative spatial attention encoding pairwise player positions and distances as learned attention biases, 3) non-autoregressive prediction of absolute displacements from initial formation to eliminate cumulative error drift.

Result: On American football tracking data, achieves 1.68 yard ADE and 3.98 yard FDE with full utilization of all 8 mixture components (entropy 2.06/2.08), demonstrating diverse generation without mode collapse.

Conclusion: PlayGen-MoG provides an extensible framework for formation-conditioned play generation that overcomes limitations of standard generative approaches, enabling realistic multi-agent trajectory generation from static formations alone for sports play design applications.
...
TL;DR: SPAR is a street-legal physical adversarial attack on ALPR systems that reduces accuracy by 60% and achieves 18% targeted impersonation, costing under $100 and implemented by AI coding assistants.


<details>
  <summary>Details</summary>
Motivation: Prior work focused on ALPR vulnerabilities but neglected legality and physical-world practicality. The authors investigate whether low-resourced threat actors can engineer successful adversarial attacks against modern open-source ALPR systems.

Method: Introduces Street-legal Physical Adversarial Rim (SPAR), a physically realizable white-box attack against fast-alpr ALPR system. SPAR requires no access to ALPR infrastructure during deployment and doesn't alter/obscure the license plate. Based on Texas legislation and case law, it's designed to be street-legal.

Result: Under optimal conditions: reduces ALPR accuracy by 60%, achieves 18% targeted impersonation rate, costs under $100 to produce, and was implemented entirely by commercial agentic coding assistants.

Conclusion: SPAR highlights practical vulnerabilities in modern ALPR systems under realistic physical-world conditions and suggests new directions for both attack and defense research.
...
TL;DR: VERTIGO is a framework for visual preference optimization of camera trajectory generators that uses real-time rendering and vision-language models to score camera shots, enabling "director in the loop" supervision through Direct Preference Optimization.


<details>
  <summary>Details</summary>
Motivation: Current generative camera systems produce diverse trajectories but lack the "director in the loop" feedback and explicit supervision of visual desirability, resulting in poor framing, off-screen characters, and undesirable aesthetics despite in-distribution camera motion.

Method: Uses Unity real-time graphics engine to render 2D visual previews from generated camera motion, then employs a cinematically fine-tuned vision-language model with cyclic semantic similarity mechanism to score renders against text prompts, providing visual preference signals for Direct Preference Optimization (DPO) post-training.

Result: Quantitative evaluations and user studies show consistent gains in condition adherence, framing quality, and perceptual realism. VERTIGO reduces character off-screen rate from 38% to nearly 0% while preserving geometric fidelity of camera motion. User studies confirm preference across composition, consistency, prompt adherence, and aesthetic quality.

Conclusion: VERTIGO successfully introduces visual preference optimization for camera trajectory generators, enabling "director in the loop" supervision through DPO post-training, significantly improving framing quality and perceptual realism while maintaining motion fidelity.
...
TL;DR: HIL-CBM extends Concept Bottleneck Models with hierarchical structure to better mimic human cognition, enabling multi-level classification and explanations without concept annotations.


<details>
  <summary>Details</summary>
Motivation: Existing CBMs operate at a single semantic level, unlike humans who use both general and specific features at different abstraction levels. This limits their interpretability and alignment with human cognitive processes.

Method: Proposes HIL-CBM with: (1) gradient-based visual consistency loss to align abstraction layers on similar spatial regions, (2) dual classification heads operating on feature concepts at different abstraction levels, enabling hierarchical classification without relational concept annotations.
...
Conclusion: HIL-CBM successfully extends CBMs into hierarchical framework that better mirrors human cognition, improving both classification performance and interpretability without requiring concept annotations.
...
TL;DR: A decomposition-first pipeline that converts full clinical practice guidelines into executable decision graphs through topology-aware chunking and interface-constrained aggregation, achieving significant improvements in precision/recall over existing methods.


<details>
  <summary>Details</summary>
Motivation: Clinical practice guidelines are long, multimodal documents with branching recommendations that are difficult to convert into executable clinical decision support (CDS). Existing LLM/VLM extractors are mostly local or text-centric, failing to preserve cross-page continuity and properly consolidate control flow across full documents into coherent decision graphs.

Method: A decomposition-first pipeline with three key components: 1) topology-aware chunking, 2) interface-constrained chunk graph generation, and 3) provenance-preserving global aggregation. The approach uses explicit entry/terminal interfaces and semantic deduplication rather than single-pass generation to preserve cross-page continuity while maintaining auditability and structural consistency.

Result: On an adjudicated prostate-guideline benchmark, the approach improved edge and triplet precision/recall from 19.6%/16.1% in existing models to 69.0%/87.5%, while node recall increased from 78.1% to 93.8%. The complete merged graph showed substantial improvements over existing methods.

Conclusion: The decomposition-first approach supports auditable guideline-to-CDS conversion, though current evidence is limited to one adjudicated prostate guideline, motivating broader multi-guideline validation for more comprehensive assessment.
...
TL;DR: EarthSynth diffusion model generates realistic post-wildfire Sentinel-2 imagery from burn masks without task-specific training, with inpainting pipelines outperforming full generation.


<details>
  <summary>Details</summary>
Motivation: Addresses the scarcity of labeled satellite imagery for deep learning-based wildfire monitoring systems by exploring synthetic data generation using foundation models.

Method: Uses EarthSynth diffusion model conditioned on CalFireSeg-50 burn masks. Evaluates six configurations varying: pipeline architecture (mask-only vs. inpainting), prompt engineering (hand-crafted vs. VLM-generated), and color-matching post-processing.

Result: Inpainting pipelines consistently outperform full-tile generation. Structured inpainting prompt achieved best spatial alignment (Burn IoU = 0.456) and burn saliency (Darkness Contrast = 20.44). Color matching reduced color distance (ΔC_burn = 63.22) but decreased burn saliency. VLM-assisted inpainting competitive with hand-crafted prompts.

Conclusion: Demonstrates feasibility of using diffusion foundation models for realistic wildfire imagery synthesis, providing foundation for generative data augmentation in wildfire detection pipelines.
...
TL;DR: VLMs fail on fine-grained visual tasks because their training focuses on mapping visual info to known language concepts, leaving novel/unnameable visual entities poorly supported.


<details>
  <summary>Details</summary>
Motivation: VLMs perform well on many multimodal tasks but struggle with fine-grained visual perception tasks even when the necessary information exists in their internal representations. The researchers investigate why this gap exists.

Method: Tested VLMs on visual correspondence tasks (semantic, shape, face correspondence) comparing nameable vs. unnameable entities. Used Logit Lens analyses to examine internal representations. Also experimented with teaching arbitrary names for unknown entities and task-specific finetuning.

Result: VLMs perform much better when entities are nameable in language than when unnameable. Logit Lens confirms VLMs assign semantic labels to nameable entities and surface more unique corresponding tokens. Teaching arbitrary names improves performance, but task-specific finetuning yields even stronger generalization without language priors.

Conclusion: Current VLM failures on visual tasks reflect learned shortcuts from their training pipeline (mapping vision to known language concepts) rather than fundamental limitations of multimodal architectures. Task-specific finetuning can overcome these limitations.
...
TL;DR: Image Prompt Packaging (IPPg) embeds structured text in images to reduce token costs in multimodal models, achieving 35.8-91.0% cost reductions with competitive accuracy, though performance varies by model and task.


<details>
  <summary>Details</summary>
Motivation: Large multimodal language models face high token-based inference costs, but the cost-performance trade-offs of different visual prompting strategies are not well understood. There's a need to systematically characterize how visual prompting affects both cost and accuracy.

Method: Introduced Image Prompt Packaging (IPPg), a prompting paradigm that embeds structured text directly into images to reduce text token overhead. Benchmarked across five datasets, three frontier models (GPT-4.1, GPT-4o, Claude 3.5 Sonnet), and two task families (VQA and code generation). Developed cost formulation decomposing savings by token type and conducted systematic error analysis with failure-mode taxonomy. Performed 125-configuration rendering ablation study.

Result: IPPg achieves 35.8-91.0% inference cost reductions with token compression up to 96%. Accuracy remains competitive in many settings but is highly model- and task-dependent: GPT-4.1 shows simultaneous accuracy and cost gains on CoSQL, while Claude 3.5 incurs cost increases on several VQA benchmarks. Error analysis reveals spatial reasoning, non-English inputs, and character-sensitive operations are most vulnerable, while schema-structured tasks benefit most. Rendering ablation shows 10-30 percentage point accuracy shifts.

Conclusion: Visual encoding choices are a first-class variable in multimodal system design. IPPg demonstrates significant cost savings potential but requires careful consideration of model-task interactions. The approach is particularly effective for schema-structured tasks but vulnerable in spatial reasoning and character-sensitive operations.
...
TL;DR: Delaunay Canopy: A novel method for accurate building wireframe reconstruction from noisy/sparse LiDAR point clouds using Delaunay graph as geometric prior to define adaptive search space.


<details>
  <summary>Details</summary>
Motivation: Conventional methods fail to achieve accurate wireframe reconstruction in regions with significant noise, sparsity, or internal corners due to inability to establish adaptive search space for leveraging 3D geometry of large, sparse building point clouds.

Method: Uses Delaunay graph as geometric prior to define geometrically adaptive search space. Core components: Delaunay Graph Scoring (reconstructs geometric manifold + yields region-wise curvature signatures), corner selection module, and wire selection module that leverage Delaunay-induced prior to focus on highly probable elements.

Result: State-of-the-art wireframe reconstruction on Building3D Tallinn city and entry-level datasets, delivering accurate predictions across diverse and complex building geometries, including previously intractable regions.

Conclusion: Delaunay Canopy successfully addresses limitations of conventional methods by establishing adaptive search space through Delaunay graph prior, enabling robust wireframe reconstruction even in challenging conditions of noise and sparsity.
...
TL;DR: An explainable vision-language model framework for Lumbar Spinal Stenosis diagnosis that combines precise text-guided segmentation with automated report generation, addressing class imbalance and spatial accuracy issues in medical imaging.


<details>
  <summary>Details</summary>
Motivation: Current LSS diagnosis relies on labor-intensive manual MRI interpretation with high inter-observer variability and diagnostic delays. Existing vision-language models fail to handle extreme class imbalance in clinical datasets while preserving spatial accuracy due to global pooling mechanisms that discard anatomical hierarchies.

Method: Proposes an end-to-end explainable vision-language model with two key components: 1) Spatial Patch Cross-Attention module for precise text-directed localization of spinal anomalies, and 2) Adaptive PID-Tversky Loss function that integrates control theory to dynamically adjust training penalties for difficult, under-segmented minority instances. Includes foundational VLMs and Automated Radiology Report Generation module.

Result: Achieves 90.69% diagnostic classification accuracy, macro-averaged Dice score of 0.9512 for segmentation, and CIDEr score of 92.80% for report generation. The framework demonstrates explainability by converting segmentation predictions into radiologist-style clinical reports.
...
TL;DR: DistillGaze: A framework that distills visual foundation models using synthetic data and unlabeled real data for rapid, high-accuracy on-device gaze estimation in AR/VR applications.


<details>
  <summary>Details</summary>
Motivation: Rapid deployment of high-accuracy gaze estimation for new AR/VR devices is challenging due to changing hardware configurations across device generations. Off-the-shelf visual foundation models struggle with specialized near-eye infrared imagery.
...
Result: Reduces median gaze error by 58.62% relative to synthetic-only baselines while maintaining a lightweight 256K-parameter model suitable for real-time on-device deployment, evaluated on a dataset spanning over 2,000 participants.
...
TL;DR: FASS is a new benchmark for evaluating feature attribution stability under realistic perturbations with prediction-invariance filtering, revealing that geometric changes cause more instability than photometric ones, and Grad-CAM performs most consistently.


<details>
  <summary>Details</summary>
Motivation: Current feature attribution methods lack proper stability evaluation under realistic input perturbations, with existing metrics focusing on additive noise, collapsing stability to single scalars, and failing to condition on prediction preservation.

Method: Introduced Feature Attribution Stability Suite (FASS) with prediction-invariance filtering, three complementary metrics (structural similarity, rank correlation, top-k Jaccard overlap), and evaluation across geometric, photometric, and compression perturbations on four attribution methods across multiple architectures and datasets.

Result: Stability estimates depend critically on perturbation family and prediction-invariance filtering; geometric perturbations cause substantially greater attribution instability than photometric changes; without prediction preservation conditioning, up to 99% of evaluated pairs involve changed predictions; Grad-CAM achieves highest stability across datasets.

Conclusion: Proper evaluation of attribution stability requires prediction-invariance filtering and consideration of perturbation types, with geometric changes being particularly challenging; FASS provides a comprehensive benchmark for assessing feature attribution robustness in safety-critical applications.
...
TL;DR: VLMs in medical VQA are systematically overconfident; post-hoc calibration (Platt scaling) helps but hallucination-aware calibration improves both calibration and discriminative performance.


<details>
  <summary>Details</summary>
Motivation: As VLMs are deployed in clinical decision support, trustworthiness through proper confidence calibration is critical, but systematic investigation of overconfidence in medical VLMs is lacking.

Method: Comprehensive empirical study across 3 model families (Qwen3-VL, InternVL3, LLaVA-NeXT), 3 scales (2B-38B), multiple confidence prompting strategies, and 3 medical VQA benchmarks. Investigated post-hoc calibration (Platt scaling) and hallucination-aware calibration using vision-grounded hallucination detection signals.

Result: 1) Overconfidence persists across models and isn't resolved by scaling or prompting. 2) Simple post-hoc calibration reduces calibration error but doesn't improve AUROC due to monotonicity limitations. 3) Hallucination-aware calibration improves both calibration and AUROC, especially on open-ended questions.

Conclusion: Post-hoc calibration should be standard practice for medical VLM deployment over raw confidence estimates, and hallucination signals enable more reliable use of VLMs in medical VQA.
...
TL;DR: UniScene3D is a transformer-based 3D encoder that learns unified scene representations from multi-view colored pointmaps by jointly modeling appearance and geometry, achieving SOTA performance on various 3D scene understanding tasks.


<details>
  <summary>Details</summary>
Motivation: Pretraining 3D encoders by aligning with CLIP shows promise for learning generalizable 3D representations, but there's a need for unified approaches that can jointly model both image appearance and geometry for comprehensive 3D scene understanding.

Method: Proposes UniScene3D, a transformer-based encoder that learns from multi-view colored pointmaps. Introduces two novel alignment techniques: cross-view geometric alignment (enforces cross-view geometry consistency) and grounded view alignment (enforces semantic consistency).

Result: Achieves state-of-the-art performance in extensive evaluations including low-shot and task-specific fine-tuning on viewpoint grounding, scene retrieval, scene type classification, and 3D VQA tasks.

Conclusion: The approach effectively enables unified 3D scene understanding by learning robust representations that capture both appearance and geometry through novel alignment techniques, demonstrating strong generalization across multiple 3D understanding tasks.
...
TL;DR: WSVD introduces a weighted SVD approach with finer granularity and adaptive importance allocation to reduce VLM latency while preserving accuracy, achieving 1.8× decoding speedup.


<details>
  <summary>Details</summary>
Motivation: Existing SVD techniques for Vision Language Models fail to achieve substantial latency reduction in practice despite theoretical efficiency gains. There's a need for a more effective approach that can deliver real execution speed improvements while maintaining model accuracy.

Method: Proposes Weighted SVD (WSVD) with two key innovations: 1) applying SVD at finer granularity with a new computational pattern for measurable latency reduction, and 2) adaptively allocating relative importance to weight elements during SVD to better preserve accuracy. Extends this framework with quantization of both weights and activations.

Result: WSVD outperforms other approaches by achieving over 1.8× decoding speedup while preserving accuracy. The method enables real and measurable improvements in execution latency for Vision Language Models.

Conclusion: Weighted SVD provides an effective solution for reducing VLM computational burden through finer-grained SVD application and adaptive importance allocation, resulting in significant speed improvements without sacrificing accuracy. The approach is open-sourced for community use.
...
TL;DR: FusionBERT is a multi-view visual fusion framework for image-3D multimodal retrieval that uses cross-attention to fuse features from multiple object viewpoints and normal-aware 3D encoding for better geometric representation.


<details>
  <summary>Details</summary>
Motivation: Existing image-3D representation learning methods focus only on single-view alignment, which is unrealistic since objects are typically observed from multiple viewpoints in real scenarios. Multi-view observations provide complementary geometric and appearance cues that current multimodal large models fail to effectively fuse for cross-modal retrieval.

Method: 1) Cross-attention-based multi-view visual aggregator that adaptively integrates features from multi-view images, fusing inter-view complementary relationships and selectively emphasizing informative visual cues. 2) Normal-aware 3D model encoder that jointly encodes point normals and 3D positions to enhance geometric features, especially for textureless or color-degraded 3D models.

Result: Extensive experiments show FusionBERT achieves significantly higher retrieval accuracy than state-of-the-art multimodal large models under both single-view and multi-view settings, establishing a strong baseline for multi-view multimodal retrieval.

Conclusion: FusionBERT successfully addresses the limitation of single-view alignment in image-3D retrieval by effectively fusing multi-view visual information and enhancing 3D geometric representation, demonstrating superior performance and providing a foundation for future multi-view multimodal retrieval research.
...
<details>
  <summary>Details</summary>
Motivation: Current Gaussian-based methods for dynamic scene reconstruction struggle with large inter-frame displacements, causing artifacts and temporal inconsistencies under fast object motions.

Method: Uses off-the-shelf point tracking models to extract pixel trajectories, triangulates them onto 3D Gaussians to guide Gaussian relocation, rotation, and scaling before training.

Result: Effectively handles large displacements, reduces fading and recoloring artifacts, enables parallel processing across multiple devices while preserving rendering quality, and achieves superior throughput.

Conclusion: TrackerSplat enhances robustness and scalability of 3DGS for dynamic scenes, maintaining visual quality while boosting reconstruction throughput in challenging scenarios with significant displacements.
...
TL;DR: Moondream Segmentation extends Moondream 3 for referring image segmentation, using autoregressive vector path decoding with iterative refinement and RL optimization for mask quality, achieving state-of-the-art results on RefCOCO and LVIS benchmarks.


<details>
  <summary>Details</summary>
Motivation: To extend vision-language models to referring image segmentation with high-quality mask generation, addressing ambiguity in supervised signals and evaluation noise from polygon annotations.

Method: Autoregressive decoding of vector paths with iterative rasterized mask refinement, plus reinforcement learning stage to optimize mask quality directly, using coarse-to-ground-truth targets from RL rollouts.

Result: Achieves 80.2% cIoU on RefCOCO (val) and 62.6% mIoU on LVIS (val), with release of RefCOCO-M cleaned validation split for more accurate boundary evaluation.

Conclusion: Moondream Segmentation demonstrates effective referring image segmentation through vector-based autoregressive decoding with RL optimization, setting new benchmarks while addressing annotation quality issues in evaluation.
...
TL;DR: Rascene uses mmWave OFDM communication signals for 3D scene imaging, overcoming sparse data through multi-frame fusion with confidence-weighted projection.


<details>
  <summary>Details</summary>
Motivation: Optical sensors (cameras, LiDAR) fail in adverse conditions like smoke/fog, while specialized radar systems are expensive and not scalable due to bespoke hardware and licensed spectrum requirements.

Method: Integrated sensing and communication (ISAC) framework using ubiquitous mmWave OFDM signals; performs multi-frame, spatially adaptive fusion with confidence-weighted forward projection to recover geometric consensus across arbitrary poses.

Result: Experimental results demonstrate high-precision 3D scene reconstruction, offering a new pathway toward low-cost, scalable, and robust 3D perception.

Conclusion: Rascene enables robust 3D environmental perception using existing communication infrastructure, overcoming limitations of optical sensors and specialized radar systems.
...
TL;DR: This paper proposes a federated learning framework for privacy-preserving autism behavior recognition using pose data, addressing data scarcity and privacy constraints in clinical settings.


<details>
  <summary>Details</summary>
Motivation: The paper addresses challenges in automated autism behavior recognition: strict privacy regulations (HIPAA) prevent centralized data aggregation, and individual clinical sites suffer from data scarcity, making it difficult to develop robust models or tailor them to site-specific patient distributions.

Method: The framework uses a two-layer privacy protection approach: 1) human skeletal abstraction to remove identifiable visual information from RGB videos, and 2) federated learning to keep sensitive pose data within each clinic while enabling multi-site collaboration. This allows learning generalized representations while supporting site-specific personalization.

Result: Experimental results on the MMASD benchmark show the framework achieves high recognition accuracy, outperforming traditional federated baselines and providing a robust, privacy-first solution for multi-site clinical analysis.

Conclusion: The study presents the first exploration of federated learning for pose-based child autism behavior recognition, offering a practical solution that balances privacy protection with effective multi-site collaboration for clinical applications.
...
TL;DR: Smart Transfer is a GeoAI framework using vision foundation models for rapid building damage mapping after earthquakes, featuring novel transfer learning strategies for cross-region generalization.
...
Method: A GeoAI framework with two novel transfer strategies: 1) Pixel-wise Clustering (PC) for robust prototype-level global feature alignment, and 2) Distance-Penalized Triplet (DPT) that integrates patch-level spatial autocorrelation patterns by penalizing semantically inconsistent but spatially adjacent patches.
...
<details>
  <summary>Details</summary>
Motivation: Current self-supervised depth estimation methods are designed for passenger vehicles and don't handle articulated vehicles well. Articulated structures introduce complex cross-segment geometry and motion coupling that make consistent depth reasoning across views challenging.

Method: Proposes ArticuSurDepth with multi-view spatial context enrichment, cross-view surface normal constraints, camera height regularization with ground plane-awareness for metric depth, and cross-vehicle pose consistency to bridge motion estimation between articulated segments.
...
Conclusion: The framework successfully addresses the challenges of articulated vehicle depth estimation through geometric consistency and structural priors, demonstrating superior performance across multiple benchmarks.
...
TL;DR: DTPTrack is a lightweight module that suppresses model drift in visual trackers by calibrating temporal reliability and synthesizing dynamic temporal priors, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing multi-frame trackers are vulnerable to model drift caused by naively aggregating noisy historical predictions, which compromises tracking accuracy over time.

Method: Two core components: (1) Temporal Reliability Calibrator (TRC) that learns per-frame reliability scores to filter noise while anchoring on ground-truth template; (2) Temporal Guidance Synthesizer (TGS) that synthesizes calibrated history into dynamic temporal priors for predictive guidance.

Result: Integrated into three tracking architectures (OSTrack, ODTrack, LoRAT) with consistent significant performance gains. Best model on extended LoRATv2 achieves 77.5% Success on LaSOT and 80.3% AO on GOT-10k, setting new state-of-the-art.

Conclusion: DTPTrack is a versatile, lightweight module that effectively suppresses drift in visual trackers by intelligently handling temporal information, demonstrating strong generalization across different architectures and benchmarks.
...
TL;DR: Efficient3D is a framework for visual token pruning that accelerates 3D Multimodal Large Language Models while maintaining accuracy through debiased importance estimation and adaptive rebalancing.


<details>
  <summary>Details</summary>
Motivation: 3D MLLMs face practical deployment challenges due to their large size and high-dimensional input features, which create substantial inference overhead on resource-constrained platforms.
...
Result: Achieves superior performance compared to unpruned baselines across five 3D vision-language benchmarks (ScanRefer, Multi3DRefer, Scan2Cap, ScanQA, SQA3D), with +2.57% CIDEr improvement on Scan2Cap dataset.

Conclusion: Efficient3D provides a scalable and effective solution for efficient inference in 3D MLLMs by enabling context-aware token reduction that maintains essential semantics with lower computation.
...
TL;DR: A lightweight structural refinement module is introduced between DETR-style detectors and parsers to stabilize parser interfaces by resolving layout inconsistencies and ordering issues in document parsing pipelines.


<details>
  <summary>Details</summary>
Motivation: In document parsing pipelines, unstable layout hypotheses from detectors can create inconsistencies between retained instance sets and parser input order, especially on dense pages with overlapping regions and ambiguous boundaries, leading to severe downstream parsing errors.

Method: A lightweight structural refinement stage is inserted between DETR-style detectors and parsers. The module treats raw detector outputs as a hypothesis pool and performs set-level reasoning using query features, semantic cues, box geometry, and visual evidence. It jointly determines instance retention, refines box localization, and predicts parser input order. The approach includes retention-oriented supervision and difficulty-aware ordering objectives to align with parser requirements.

Result: Extensive experiments on public benchmarks show consistent improvements in page-level layout quality. When integrated into standard end-to-end parsing pipelines, the method substantially reduces sequence mismatch, achieving a Reading Order Edit of 0.024 on OmniDocBench.

Conclusion: The proposed structural refinement module effectively stabilizes parser interfaces in document parsing pipelines by resolving layout inconsistencies and ordering issues, particularly benefiting structurally complex pages with dense layouts and overlapping regions.
...
TL;DR: DocShield is a unified framework for text-centric image forgery analysis that uses visual-logical co-reasoning to detect, localize, and explain document manipulations through a Cross-Cues-aware Chain of Thought mechanism.


<details>
  <summary>Details</summary>
Motivation: The rapid progress of generative AI enables increasingly realistic text-centric image forgeries that threaten document safety. Existing forensic methods rely mainly on visual cues without evidence-based reasoning, and treat detection, localization, and explanation as isolated tasks, limiting reliability and interpretability.

Method: Proposes DocShield framework with Cross-Cues-aware Chain of Thought (CCT) mechanism for implicit agentic reasoning that iteratively cross-validates visual anomalies with textual semantics. Uses Weighted Multi-Task Reward for GRPO-based optimization to align reasoning structure, spatial evidence, and authenticity prediction. Also introduces RealText-V1 dataset with multilingual document-like text images, pixel-level manipulation masks, and expert-level textual explanations.
...
Conclusion: DocShield provides a unified, evidence-based approach to text-centric forgery analysis through visual-logical co-reasoning, addressing limitations of existing methods. The framework, dataset, and code will be publicly released to advance document safety research.
...
TL;DR: XrayClaw: A multi-agent framework combining cooperative and competitive agents for trustworthy chest X-ray diagnosis, using Competitive Preference Optimization to reduce hallucinations and improve reasoning.


<details>
  <summary>Details</summary>
Motivation: Traditional monolithic AI models for CXR interpretation suffer from logical inconsistencies and diagnostic hallucinations. Multi-agent systems can help but existing frameworks are prone to consensus-based errors when using a single underlying model.

Method: XrayClaw framework with four specialized cooperative agents simulating clinical workflow plus a competitive agent as independent auditor. Uses Competitive Preference Optimization to penalize illogical reasoning by enforcing mutual verification between analytical and holistic interpretations.
...
TL;DR: VBGS-SLAM is a probabilistic 3D Gaussian Splatting SLAM framework that uses variational Bayesian inference to maintain uncertainty over poses and scene parameters, improving robustness against drift and catastrophic forgetting while preserving rendering quality.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS SLAM methods rely on deterministic pose optimization, making them sensitive to initialization and prone to catastrophic forgetting as the map evolves. There's a need for more robust uncertainty-aware approaches.

Method: Proposes Variational Bayesian Gaussian Splatting SLAM (VBGS-SLAM) that couples splat map refinement and camera pose tracking in a generative probabilistic framework. Uses conjugate properties of multivariate Gaussians and variational inference for efficient closed-form updates, maintaining posterior uncertainty over both poses and scene parameters.

Result: Demonstrates superior tracking performance and robustness in long sequence prediction, with efficient, high-quality novel view synthesis across diverse synthetic and real-world scenes. Mitigates drift and enhances robustness in challenging conditions while preserving 3DGS efficiency and rendering quality.

Conclusion: VBGS-SLAM provides a probabilistic uncertainty-aware approach to 3DGS SLAM that addresses limitations of deterministic methods, offering improved robustness against drift and catastrophic forgetting while maintaining the rendering advantages of 3D Gaussian Splatting.
...
<details>
  <summary>Details</summary>
Motivation: Current Vision-Language-Action (VLA) models for autonomous driving are limited by imitation learning's inability to explore beyond expert demonstrations, making them brittle in novel scenarios. Reinforcement learning could help but requires world modeling due to lack of observable state transitions in offline datasets.

Method: Proposes a unified understanding-and-generation framework with world modeling that: 1) augments trajectory prediction with future RGB and depth image generation as dense supervision, 2) uses world model's prediction uncertainty as intrinsic reward for exploration, 3) incorporates safety-gated rewards optimized via Group Relative Policy Optimization (GRPO).
...
Conclusion: World modeling enables both meaningful exploration and dense supervision for VLA-based autonomous driving, overcoming imitation learning limitations while maintaining safety through uncertainty-based exploration signals.
...
TL;DR: MOMO is the first multi-sensor foundation model for Mars remote sensing that integrates three Martian sensors (HiRISE, CTX, THEMIS) using model merging with a novel Equal Validation Loss strategy for checkpoint alignment.


<details>
  <summary>Details</summary>
Motivation: There's a need for foundation models that can handle multi-resolution Martian remote sensing data from different sensors, which current approaches (ImageNet pre-training, earth observation models, or sensor-specific training) don't adequately address.

Method: Uses model merging to integrate representations from three Martian sensors. Introduces Equal Validation Loss (EVL) strategy to align checkpoints across sensors based on validation loss similarity before fusion via task arithmetic, ensuring models are merged at compatible convergence stages.

Result: Trained on ~12 million Mars orbital samples and evaluated on 9 downstream tasks from Mars-Bench. Achieves better overall performance than ImageNet pre-trained, earth observation foundation models, sensor-specific pre-training, and fully-supervised baselines. Shows consistent significant improvement on segmentation tasks.
...
TL;DR: THOM is a training-free framework for generating photorealistic, physically plausible 3D hand-object interaction meshes from text without needing template object meshes.


<details>
  <summary>Details</summary>
Motivation: Generating 3D hand-object interactions from text is crucial for robotic grasping and VR/AR content, but existing methods struggle with mesh extraction from text-generated Gaussians and physics-based optimization on erroneous meshes.
...
Result: THOM consistently surpasses state-of-the-art methods in text alignment, visual realism, and interaction plausibility in comprehensive experiments.
...
TL;DR: LLaBIT extends LLMs to clinically relevant brain MRI tasks beyond simple text-to-image generation, achieving superior performance across report generation, VQA, segmentation, and image translation tasks while outperforming specialized models.


<details>
  <summary>Details</summary>
Motivation: While LLMs excel at vision-language tasks, simple text-to-image generation has limited clinical utility. Medical imaging requires more clinically important tasks like segmentation for pathology localization and image translation for reconstructing missing sequences, but integrating these diverse tasks within a single versatile language model remains unexplored.

Method: LLaBIT (Large Language Model for Brain Image Translation) extends LLM visual reasoning to brain MRI tasks. It incorporates a mechanism to reuse feature maps from the image encoder to mitigate spatial information loss during image tokenization. The method also generates text data using LLMs with strict predefined instructions to augment limited image-text paired data in brain MRI.

Result: Comprehensive evaluation on five brain MRI datasets across four distinct tasks (report generation, visual question answering, image segmentation, and image translation) showed superior performance across all tasks. The model outperformed specialized, task-specific models in direct comparisons.

Conclusion: LLaBIT demonstrates efficacy and versatility in handling diverse clinically relevant brain MRI tasks within a single language model framework, advancing beyond simple text-to-image generation to address more meaningful medical imaging applications.
...
TL;DR: A dual representation combining discrete polylines with continuous Bézier control points enables collaborative optimization for stroke-based rendering, reducing strokes by 30-50% and cutting optimization time by 30-40%.
...
Conclusion: The dual representation successfully bridges the gap between discrete and continuous optimization approaches in stroke-based rendering, enabling more efficient and structurally coherent vector graphics generation.
...
TL;DR: DeCo-DETR is a vision-centric open-vocabulary object detection framework that decouples semantic reasoning from localization using hierarchical semantic prototypes and parallel optimization streams, achieving competitive performance with improved inference efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing OVOD approaches have two main limitations: 1) multimodal designs require text encoders at inference time, causing substantial computational overhead, and 2) tightly coupled training objectives create a trade-off between closed-set detection accuracy and open-world generalization.

Method: Proposes Decoupled Cognition DETR (DeCo-DETR) with a unified decoupling paradigm: 1) constructs hierarchical semantic prototype space from region-level descriptions generated by pre-trained LVLMs and aligned via CLIP (no online text encoding), 2) disentangles semantic reasoning from localization through decoupled training strategy separating alignment and detection into parallel optimization streams.
...
<details>
  <summary>Details</summary>
Motivation: Recovering sewing patterns from draped 3D garments is fundamentally ill-posed for existing methods, creating a gap in human digitization research.

Method: Two-stage framework: Stage I uses geometry-driven autoregressive model to infer BoxMesh from 3D garment; Stage II uses semantics-aware autoregressive model to parse BoxMesh into parametric sewing patterns.
...
Conclusion: BoxMesh representation bridges 3D geometry and parametric patterns, enabling accurate and robust sewing pattern recovery through structured decomposition.
...
TL;DR: The paper introduces TinySet-9M, the first large-scale multi-domain dataset for small object detection, and proposes P2SOD, a new point-prompt paradigm that enables semantic augmentation at inference time, leading to DEAL framework with significant performance improvements.


<details>
  <summary>Details</summary>
Motivation: Small object detection faces three main challenges: 1) extremely limited pixels and ambiguous boundaries make annotation difficult, 2) lack of large-scale high-quality datasets, and 3) inherently weak semantic representations for small objects.
...
Result: DEAL achieves 31.4% relative improvement over fully supervised baselines on AP75 metric on TinySet-9M, while effectively generalizing to unseen categories and datasets with just a single click at inference time.

Conclusion: The work addresses fundamental challenges in small object detection through both data (TinySet-9M) and methodology (P2SOD paradigm), demonstrating that point-prompted detection can significantly enhance performance and generalization for small objects.
...
TL;DR: Adversarial perturbations can fool membership inference attacks (MIAs) into classifying non-member images as members, exposing a new vulnerability in vision model privacy assessment.


<details>
  <summary>Details</summary>
Motivation: Existing MIAs assume honest query inputs and their adversarial robustness is unexplored. The paper aims to investigate how imperceptible perturbations can manipulate MIAs to falsely identify non-members as members, revealing a new attack surface.

Method: 1) Demonstrate adversarial membership fabrication effectiveness across architectures/datasets; 2) Analyze geometric signature (gradient-norm collapse trajectory) distinguishing fabricated from true members; 3) Develop detection strategy using gradient-geometry signals; 4) Create robust inference framework to mitigate adversarial manipulation.

Result: Adversarial fabrication is broadly effective against state-of-the-art MIAs. The gradient-norm collapse trajectory reliably separates fabricated from true members. The proposed detection and robust inference strategies significantly enhance resilience against adversarial manipulation.

Conclusion: This work establishes the first comprehensive framework for adversarial membership manipulation in vision models, revealing a previously overlooked vulnerability in MIAs and providing principled defenses based on gradient-geometry analysis.
...
TL;DR: EnsemHalDet: An ensemble-based framework that uses multiple internal VLM representations (attention outputs, hidden states) with independent detectors combined through ensemble learning to improve multimodal hallucination detection.
...
Method: Proposes EnsemHalDet framework that leverages multiple internal VLM representations (attention outputs and hidden states), trains independent detectors for each representation, and combines them through ensemble learning.
...
TL;DR: CANDLE uses DINOv3 features as illumination-robust priors for color ambient lighting normalization, achieving state-of-the-art performance on challenging multi-colored illumination scenarios.
...
Method: CANDLE introduces DINO Omni-layer Guidance (D.O.G.) to adaptively inject multi-layer DINOv3 features into encoder stages, and a color-frequency refinement design (BFACG + SFFB) to suppress decoder-side chromatic collapse and detail contamination.
...
Conclusion: DINOv3's self-supervised features provide effective illumination-robust semantic priors for color ambient normalization, enabling strong generalization across both chromatic and luminance-dominant illumination conditions.
...
TL;DR: LumaFlux is a diffusion transformer for converting 8-bit SDR to 10-bit HDR using physical and perceptual guidance, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: The rapid adoption of HDR devices creates a need to convert existing 8-bit SDR content to 10-bit HDR. Existing inverse tone-mapping methods struggle with real-world degradations, stylistic variations, and camera pipelines, often producing clipped highlights, desaturated colors, or unstable tone reproduction.

Method: LumaFlux adapts a large pretrained diffusion transformer (DiT) with three key components: (1) Physically-Guided Adaptation module injecting luminance, spatial descriptors, and frequency cues via low-rank residuals; (2) Perceptual Cross-Modulation layer stabilizing chroma and texture via FiLM conditioning; (3) HDR Residual Coupler fusing physical and perceptual signals with adaptive modulation. A lightweight Rational-Quadratic Spline decoder reconstructs smooth tone fields.
...
Conclusion: LumaFlux represents a physically and perceptually guided diffusion transformer approach that effectively addresses SDR-to-HDR conversion challenges, producing high-quality HDR content with accurate luminance and color reproduction.
...
TL;DR: UNICA is a skeleton-free generative model that unifies avatar control into a single neural framework, generating 3D avatar geometry from keyboard inputs using action-conditioned diffusion on 2D position maps, then rendering with 3D Gaussian Splatting.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D avatar creation requires complex pipelines with appearance modeling, motion planning, rigging, and physical simulation. This paper aims to simplify and unify these components into a single neural framework for more efficient and natural avatar control.

Method: UNICA uses an action-conditioned diffusion model on 2D position maps to generate next-frame geometry from keyboard inputs. A point transformer then maps the geometry to 3D Gaussian Splatting for high-fidelity free-view rendering. The approach is skeleton-free and supports autoregressive generation.

Result: UNICA naturally captures hair and loose clothing dynamics without manual physical simulation, supports extra-long autoregressive generation, and unifies motion planning, rigging, physical simulation, and rendering in a single workflow.

Conclusion: UNICA presents the first unified neural framework for controllable 3D avatars, eliminating the need for complex traditional pipelines while enabling natural dynamics and high-fidelity rendering from simple keyboard inputs.
...
TL;DR: PaveBench is a new large-scale benchmark for pavement distress analysis that combines visual perception tasks with vision-language interaction, addressing limitations of existing unimodal datasets.


<details>
  <summary>Details</summary>
Motivation: Current pavement inspection research focuses only on visual recognition tasks (classification, detection, segmentation), but real-world applications require quantitative analysis, explanation, and interactive decision support. Existing datasets are unimodal and lack support for multi-turn interaction, fact-grounded reasoning, and vision-language analysis.

Method: Introduces PaveBench with: 1) Four core tasks (classification, object detection, semantic segmentation, vision-language QA), 2) Large-scale real-world pavement images with annotations, 3) Hard-distractor subset for robustness evaluation, 4) PaveVQA dataset supporting single-turn, multi-turn, and expert-corrected interactions covering recognition, localization, quantitative estimation, and maintenance reasoning, 5) Agent-augmented VQA framework integrating domain-specific models as tools with vision-language models.

Result: Created a comprehensive benchmark with unified task definitions and evaluation protocols. Evaluated several state-of-the-art methods and provided detailed analysis. The dataset is publicly available on Hugging Face.

Conclusion: PaveBench bridges the gap between visual perception and interactive vision-language analysis for pavement inspection, enabling more comprehensive and practical pavement distress assessment that better supports real-world maintenance decision-making.
...
TL;DR: The paper introduces CMCC-ReID, a new task combining Visible-Infrared and Clothing-Change ReID, creates the SYSU-CMCC benchmark dataset, and proposes PIA with DBDL and BPL modules to address both modality and clothing variations.


<details>
  <summary>Details</summary>
Motivation: Real-world surveillance systems face simultaneous challenges of modality discrepancy (visible vs infrared) and clothing variations, but existing research addresses these problems separately. There's a need for a unified approach to handle both challenges in long-term surveillance scenarios.

Method: Proposes Progressive Identity Alignment Network (PIA) with two key modules: 1) Dual-Branch Disentangling Learning (DBDL) separates identity cues from clothing factors for clothing-agnostic representations, and 2) Bi-Directional Prototype Learning (BPL) performs intra-modality and inter-modality contrast to bridge modality gaps while suppressing clothing interference.
...
Conclusion: The paper defines the novel CMCC-ReID task, provides the SYSU-CMCC benchmark dataset, and proposes an effective PIA framework that progressively addresses both clothing variation and modality discrepancy, advancing research in realistic long-term surveillance scenarios.
...
TL;DR: A quantization-aware vision token pruning framework that co-optimizes token pruning and post-training quantization for efficient multimodal LLMs, improving accuracy in low-bit regimes by preserving quantization-critical tokens.


<details>
  <summary>Details</summary>
Motivation: MLLMs have high computational/memory costs that hinder deployment in resource-constrained settings. While PTQ and token pruning are standard compression techniques, they're usually treated independently. The paper identifies that semantic-based token pruning can discard activation outliers important for quantization stability, worsening errors in low-bit regimes.
...
Result: Experiments on LLaVA architectures show consistent outperformance over naive integration baselines. At aggressive pruning (12.5% tokens retained), the framework improves accuracy by 2.24% over baseline and even surpasses dense quantization without pruning.

Conclusion: This is the first method that explicitly co-optimizes vision token pruning and PTQ for accurate low-bit MLLM inference, addressing the coupling between these compression techniques that was previously overlooked.
...
TL;DR: MMPhysVideo is a framework that improves physical plausibility in video generation by incorporating multimodal perceptual cues (semantics, geometry, trajectories) into video diffusion models through a unified pseudo-RGB format and bidirectional control architecture.


<details>
  <summary>Details</summary>
Motivation: Current video diffusion models produce visually appealing but physically inconsistent results due to relying solely on pixel-level reconstruction, lacking understanding of underlying physical dynamics.

Method: 1) Convert perceptual cues (semantics, geometry, spatio-temporal trajectories) into unified pseudo-RGB format for VDM processing. 2) Bidirectionally Controlled Teacher architecture with parallel branches to decouple RGB and perception processing, using zero-initialized control links for pixel-wise consistency. 3) Distill teacher's physical prior into single-stream student model via representation alignment. 4) MMPhysPipe pipeline for scalable physics-rich multimodal dataset construction using VLM-guided subject identification.

Result: MMPhysVideo consistently improves physical plausibility and visual quality over advanced models across various benchmarks without additional inference costs, achieving state-of-the-art performance compared to existing methods.

Conclusion: The proposed framework successfully scales physical plausibility in video generation through joint multimodal modeling, demonstrating that incorporating perceptual cues into video diffusion models can significantly enhance physical consistency while maintaining visual quality.
...
TL;DR: NavCrafter: A framework that creates flexible 3D scenes from single images by synthesizing novel-view videos with camera control and temporal-spatial consistency, improving 3D reconstruction.


<details>
  <summary>Details</summary>
Motivation: Direct 3D data acquisition is often costly or impractical, creating a need for methods that can generate flexible 3D scenes from single images.

Method: Leverages video diffusion models for 3D priors, uses geometry-aware expansion to extend scenes, introduces multi-stage camera control with dual-branch injection and attention modulation, and implements collision-aware trajectory planning with enhanced 3D Gaussian Splatting pipeline.
...
Conclusion: NavCrafter provides an effective solution for creating flexible 3D scenes from single images with controllable camera navigation and improved reconstruction quality.
...
TL;DR: STRNet: A unified spatio-temporal representation framework for visual navigation that preserves fine-grained spatial and temporal structure through graph reasoning and hybrid temporal modeling.
...
Method: Extracts features from both image sequences and goal observations, fuses them using a spatio-temporal fusion module that performs spatial graph reasoning within frames and models temporal dynamics using hybrid temporal shift module with multi-resolution difference-aware convolution.
...
Conclusion: The proposed unified spatio-temporal representation framework enhances visual encoding for robotic navigation by preserving fine-grained spatial and temporal structure, leading to better navigation performance.
...
<details>
  <summary>Details</summary>
Motivation: NeRF applications are limited by computational resources, and on-device training could enable applications in communication-limited, privacy-sensitive, and dynamic scenes, but current methods face memory and resource constraints.

Method: Fact-Hash projects 3D coordinates into multiple lower-dimensional forms (2D or 1D) before applying hash functions, then aggregates them into a single feature, combining benefits of rich high-resolution features and few-shot robustness.

Result: Fact-Hash demonstrates superior memory efficiency, preserving quality and rendering speed while reducing memory usage by over one-third compared to previous encoding methods, with better computational efficiency and energy consumption.

Conclusion: Fact-Hash is a promising solution for improving feature grid representation, addressing memory constraints, and enhancing quality for on-device NeRF applications across various domains.
...
TL;DR: DeformPIC is a deformation-based framework for point cloud in-context learning that outperforms masked modeling approaches by enabling explicit geometric reasoning and consistent training-inference objectives.


<details>
  <summary>Details</summary>
Motivation: Existing masked point modeling (MPM) approaches for point cloud ICL have two key limitations: 1) They predict target point clouds without leveraging geometric priors, forcing models to infer spatial structure solely from token correlations, and 2) They suffer from training-inference objective mismatch since they use target-side information during training that's unavailable at inference.

Method: DeformPIC learns to deform the query point cloud under task-specific guidance from prompts, rather than using masked reconstruction. This deformation-based approach enables explicit geometric reasoning and maintains consistent objectives between training and inference.
...
Conclusion: The deformation-based framework provides a more effective approach for point cloud in-context learning by addressing the limitations of masked modeling methods, enabling better geometric reasoning and eliminating training-inference objective mismatch.
...
TL;DR: Proposes adaptive local frequency filtering for Fourier-encoded implicit neural representations (INRs) to handle signals with spatially varying local spectra, improving reconstruction quality and optimization speed.
...
Method: Introduces a spatially varying parameter α(x) to modulate encoded Fourier components, enabling smooth transition among low-pass, band-pass, and high-pass behaviors at different spatial locations. Analyzes the effect from neural tangent kernel (NTK) perspective.

Result: Experiments on 2D image fitting, 3D shape representation, and sparse data reconstruction show consistent improvement in reconstruction quality and faster optimization compared with fixed-frequency baselines. Learned α(x) provides intuitive visualization of spatially varying frequency preferences.

Conclusion: Adaptive local frequency modulation is a practical enhancement for Fourier-encoded INRs that better handles non-stationary signals and provides interpretable frequency behavior visualization.
...
TL;DR: HiDiGen is a hierarchical framework for generating valid B-rep CAD models by decoupling geometry into two stages with explicit topological constraints.
...
TL;DR: End-to-end temporal sentence grounding with joint optimization of video backbone and localization head, using a Sentence Conditioned Adapter (SCADA) to integrate linguistic features into visual representations.


<details>
  <summary>Details</summary>
Motivation: Current TSGV methods use pre-trained frozen video backbones trained for classification, creating task discrepancy when used for temporal grounding. There's a need to bridge this gap by optimizing video backbones specifically for TSGV tasks.

Method: Proposes fully end-to-end paradigm with joint optimization of video backbone and localization head. Introduces Sentence Conditioned Adapter (SCADA) that uses sentence features to train a small portion of video backbone parameters adaptively, enabling deeper networks with less memory while enhancing visual representation through linguistic integration.
...
Conclusion: End-to-end optimization with SCADA effectively bridges the task discrepancy gap, improves visual representation through linguistic modulation, and achieves superior performance on temporal sentence grounding benchmarks.
...
TL;DR: Single-view 3D hair strand reconstruction using video generation priors and neural orientation extractor for full-view consistency.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to reconstruct realistic 3D hair from single images, especially in unseen regions, due to limited frontal cues and synthetic training data restrictions.

Method: 1) Leverage video generation model priors to transform single-view into calibrated multi-view reconstruction; 2) Neural orientation extractor trained on sparse real annotations for full-view orientation; 3) Two-stage strand-growing algorithm with hybrid implicit field for fast, detailed strand synthesis.

Result: Achieves state-of-the-art performance on diverse hair portraits, producing consistent results in both visible and invisible regions.

Conclusion: The framework successfully addresses single-view 3D hair reconstruction challenges by combining video generation priors, neural orientation estimation, and efficient strand synthesis.
...
TL;DR: Token-level warping outperforms pixel-level warping for MLLM viewpoint robustness by using backward warping on ViT tokens to preserve semantic coherence.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with viewpoint changes due to pixel-wise warping's sensitivity to depth errors and geometric distortions, while human mental imagery uses part-level structural representations for perspective transformation.

Method: Proposes token-level warping using backward warping approach: defines dense grid on target view and retrieves corresponding source-view token for each grid point, leveraging ViT-based MLLM image tokens as substrate.

Result: Token-level warping achieves greater stability and better semantic coherence than baselines including pixel-wise warping, spatially fine-tuned MLLMs, and generative warping methods on ViewBench benchmark.

Conclusion: Token-level warping enables reliable reasoning from nearby viewpoints, demonstrating that warping tokens rather than pixels helps MLLMs handle viewpoint changes more effectively.
...
<details>
  <summary>Details</summary>
Motivation: Existing prompt-based anomaly detection methods use handcrafted or learned prompt embeddings as reference vectors, which may be suboptimal. The authors aim to develop a prompt-free framework that can work in zero-shot settings without target-domain adaptation while providing interpretability.

Method: SPG learns sparse guide coefficients in Sparse Autoencoder (SAE) latent space to generate normal/anomaly guide vectors via the SAE dictionary. It uses a two-stage learning strategy: (1) train SAE on patch-token features, (2) optimize only guide coefficients using auxiliary pixel-level masks while freezing backbone and SAE.

Result: On MVTec AD and VisA datasets under cross-dataset zero-shot settings, SPG achieves competitive image-level detection and strong pixel-level segmentation. With DINOv3, SPG attains the highest pixel-level AUROC among compared methods. The learned coefficients reveal category-general and category-specific factors.

Conclusion: SPG provides an effective prompt-free framework for zero-shot anomaly detection that eliminates the need for prompt engineering, achieves strong performance, and offers interpretability through sparse dictionary atoms.
...
TL;DR: A self-reflection hierarchical prompt framework for class incremental surgical instrument segmentation that enables positive forward and backward knowledge transfer to learn new instruments while avoiding catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: Prior works in surgical video scene parsing overlook the potential of positive forward knowledge transfer (how past knowledge helps learn new classes) and positive backward knowledge transfer (how learning new classes helps refine past knowledge). Current incremental learning approaches don't effectively leverage these transfer mechanisms.

Method: Proposes a hierarchical prompt parsing tree with instrument-shared prompts as root nodes, n-part-shared prompts as intermediate nodes, and instrument-distinct prompts as leaf nodes. Uses self-reflection refining through directed-weighted graph propagation to examine knowledge associations and improve representativeness without catastrophic forgetting. Built on frozen pre-trained models with adaptive instrument-aware prompts.

Result: Achieves more than 5% and 11% improvements over competing methods on two public benchmarks. Framework is applicable to both CNN-based models and transformer-based foundation models.

Conclusion: The proposed framework successfully unlocks positive forward and backward knowledge transfer in class incremental segmentation, enabling proficient learning of new instruments, improvement of existing skills, and avoidance of catastrophic forgetting of old instruments.
...
TL;DR: InstructTable: An instruction-guided multi-stage training framework for table structure recognition that combines visual structural modeling with semantic understanding to handle complex layouts with merged/empty cells.


<details>
  <summary>Details</summary>
Motivation: Traditional visual-only models lack semantic support for complex tables, while vision-language models underemphasize visual structural information. There's a need for a framework that effectively combines both visual structure modeling and semantic understanding for accurate table parsing in complex scenarios.

Method: 1) Instruction-guided multi-stage training framework with table instruction pre-training to focus on fine-grained structural patterns; 2) Complementary TSR fine-tuning to preserve robust visual information modeling; 3) Table Mix Expand (TME) - a template-free method for synthesizing large-scale authentic tabular data; 4) Constructed BCDSTab benchmark with 900 complex table images.
...
Conclusion: InstructTable effectively addresses limitations of existing approaches by combining instruction-guided learning with robust visual modeling, enabling accurate table structure recognition in complex scenarios through multi-stage training and synthetic data augmentation.
...
TL;DR: A method for stable editing of animatable human avatars using constrained inversion in a structured latent space to prevent identity leakage and flicker.


<details>
  <summary>Details</summary>
Motivation: Current avatar editing methods using sparse supervision (few edited keyframes) suffer from identity leakage and temporal flicker due to ill-conditioned inversion where edited constraints don't sufficiently determine latent directions for intended edits.
...
Conclusion: The proposed constrained inversion approach in structured latent space effectively addresses identity leakage and temporal flicker problems in avatar editing with sparse supervision.
...
TL;DR: ProVCA: Progressive video condensation agent that efficiently identifies key frames for MLLM-based video understanding using multi-granularity localization.


<details>
  <summary>Details</summary>
Motivation: Existing methods either lose fine-grained visual details (text-then-LLM pipelines) or are computationally expensive (video-based MLLMs). Need efficient video understanding that preserves visual details while being computationally feasible.

Method: Progressive three-stage approach: 1) Segment localization to identify query-relevant video segments, 2) Snippet selection based on similarity, 3) Keyframe refinement to pinpoint specific frames. Progressively narrows scope from coarse segments to fine frames.
...
Conclusion: ProVCA enables efficient video understanding by progressively condensing video content to key frames for MLLM reasoning, balancing computational efficiency with preservation of visual details.
...
TL;DR: The paper addresses visual explanation in geometry education as a Referring Image Segmentation problem, creates synthetic training data for geometric diagrams, and fine-tunes vision-language models to achieve significant performance improvements over zero-shot approaches.


<details>
  <summary>Details</summary>
Motivation: Existing RIS models trained on natural images fail on geometric diagrams due to domain shift between photographic scenes and abstract schematics, creating a need for specialized models for geometry education.

Method: Developed a fully automated procedural data engine to generate synthetic geometry diagrams with pixel-perfect masks and diverse referring expressions, then fine-tuned vision-language models (specifically Florence-2) on this synthetic data.

Result: Fine-tuned Florence-2 achieved 49% IoU and 85% Buffered IoU, compared to <1% IoU in zero-shot settings. Introduced Buffered IoU as a geometry-aware metric that better reflects segmentation quality for thin structures.

Conclusion: The work establishes a foundation for building Artificial General Teachers capable of providing visually grounded, step-by-step explanations of geometry problems through effective visual explanation systems.
...
TL;DR: Proposes EvaNet, a unified evaluation framework for image fusion using a lightweight network to approximate metrics, with contrastive learning and LLM guidance, achieving 1000x speedup and better human consistency.
...
Method: Lightweight network using divide-and-conquer strategy: decomposes fusion results into infrared/visible components, uses contrastive learning with LLM perceptual guidance, and introduces consistency evaluation framework.

Result: Superior efficiency (up to 1000x faster) and greater consistency with human visual perception across standard image fusion benchmarks.

Conclusion: Proposes a novel learning-based evaluation paradigm that effectively addresses limitations of traditional metrics while being efficient and human-consistent.
...
<details>
  <summary>Details</summary>
Motivation: Long-range 3D object detection is challenging due to sparse and fragmented LiDAR observations in far field, making context modeling difficult. Existing SSM-based methods are limited by generic serialization strategies that fail to preserve meaningful contextual neighborhoods in sparse scenes.

Method: RayMamba is a geometry-aware plug-and-play enhancement for voxel-based 3D detectors. It organizes sparse voxels into sector-wise ordered sequences through a ray-aligned serialization strategy, preserving directional continuity and occlusion-related context for subsequent Mamba-based modeling. Compatible with both LiDAR-only and multimodal detectors.

Result: Extensive experiments on nuScenes and Argoverse 2 show consistent improvements across strong baselines. Achieves up to 2.49 mAP and 1.59 NDS gain in challenging 40-50 m range on nuScenes, and improves VoxelNeXt on Argoverse 2 from 30.3 to 31.2 mAP.

Conclusion: RayMamba effectively addresses long-range 3D detection challenges by preserving geometric context through ray-aligned serialization, demonstrating significant performance gains with modest computational overhead.
...
TL;DR: UniSpector: A visual prompting method for open-set defect detection in industrial inspection that prevents prompt embedding collapse through semantically structured prompt topology.


<details>
  <summary>Details</summary>
Motivation: Existing industrial inspection systems fail at detecting novel defects due to closed-set assumptions, and current visual prompting methods suffer from prompt embedding collapse caused by high intra-class variance and subtle inter-class differences.

Method: UniSpector shifts from naive prompt-to-region matching to designing semantically structured prompt topology using: 1) Spatial-Spectral Prompt Encoder for orientation-invariant fine-grained representations, 2) Contrastive Prompt Encoder to regularize prompt space into semantically organized angular manifold, and 3) Prompt-guided Query Selection for adaptive object queries aligned with prompts.
...
Conclusion: Enables scalable, retraining-free inspection for evolving industrial environments and provides critical insights for generic visual prompting design.
...
TL;DR: SentiAvatar is a framework for creating expressive 3D digital humans that can speak, gesture, and emote in real-time, addressing multimodal data scarcity, semantic-to-motion mapping, and motion-prosody synchronization challenges.


<details>
  <summary>Details</summary>
Motivation: Building expressive interactive 3D digital humans is challenging due to three key problems: lack of large-scale high-quality multimodal data, robust semantic-to-motion mapping, and fine-grained frame-level motion-prosody synchronization.
...
Result: Achieved state-of-the-art on SuSuInterActs (R@1 43.64%, nearly 2x best baseline) and BEATv2 (FGD 4.941, BC 8.078), producing 6s output in 0.3s with unlimited multi-turn streaming.

Conclusion: SentiAvatar successfully addresses key challenges in creating expressive 3D digital humans through a comprehensive dataset, pre-trained motion foundation model, and novel architecture, enabling real-time interactive virtual characters with semantic and rhythmic alignment.
...
TL;DR: GP-4DGS integrates Gaussian Processes into 4D Gaussian Splatting for probabilistic modeling of dynamic scenes, enabling uncertainty quantification, motion estimation for unobserved regions, and temporal extrapolation.
...
Method: The framework combines Gaussian Processes with 4D Gaussian Splatting using spatio-temporal kernels to model deformation field correlations, and employs variational Gaussian Processes with inducing points for scalable inference.
...
Conclusion: This work bridges probabilistic modeling and neural graphics by addressing uncertainty in dynamic scene reconstruction, representing a meaningful advancement in the field.
...
<details>
  <summary>Details</summary>
Motivation: Traditional modular perception pipelines suffer from cumulative errors and latency. Instance prediction models provide unified solutions but struggle with processing dense spatial-temporal information in dynamic driving environments while maintaining real-time performance.
...
Result: Extensive ablation studies validate architectural components. On nuScenes dataset, BEVPredFormer achieves state-of-the-art or comparable performance to existing methods.

Conclusion: BEVPredFormer demonstrates potential for robust and efficient autonomous driving perception through effective temporal-spatial comprehension without compromising real-time performance.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs excel in general domains but struggle with complex real-world science. Polymer science, with its diverse multimodal data and interdisciplinary nature, serves as an ideal testbed, but existing benchmarks overlook real-world workflows and fail to evaluate the full experimental lifecycle.

Method: Introduces PolyReal, a multimodal benchmark grounded in real-world scientific practices that evaluates MLLMs across five critical capabilities: (1) foundational knowledge application, (2) lab safety analysis, (3) experiment mechanism reasoning, (4) raw data extraction, and (5) performance & application exploration.

Result: Evaluation of leading MLLMs on PolyReal reveals a capability imbalance - models perform well on knowledge-intensive reasoning (e.g., experiment mechanism reasoning) but drop sharply on practice-based tasks (e.g., lab safety analysis and raw data extraction), exposing a severe gap between abstract knowledge and practical application.

Conclusion: PolyReal addresses the evaluation gap in real-world scientific workflows and provides a practical benchmark for assessing AI systems in complex, interdisciplinary scientific domains like polymer science, highlighting that real-world practice-based tasks remain challenging for current MLLMs.
...
TL;DR: MHENet is an RGB-D camouflaged object detection framework that enhances modality-specific features (texture and geometry) before fusion, outperforming 16 state-of-the-art methods on four benchmarks.
...
Method: Proposes MHENet with: 1) Texture Hierarchical Enhancement Module (THEM) to amplify subtle texture variations via high-frequency extraction, 2) Geometry Hierarchical Enhancement Module (GHEM) to enhance geometric structures via learnable gradient extraction, and 3) Adaptive Dynamic Fusion Module (ADFM) for spatially varying fusion of enhanced features.

Result: MHENet surpasses 16 state-of-the-art methods on four benchmarks both qualitatively and quantitatively, demonstrating superior camouflaged object detection performance.

Conclusion: Modality-specific hierarchical enhancement before fusion significantly improves RGB-D camouflaged object detection, and the proposed MHENet framework effectively addresses the limitations of direct feature fusion in existing methods.
...
<details>
  <summary>Details</summary>
Motivation: Current speech-driven 3D facial animation methods struggle with maintaining accurate lip synchronization and producing realistic facial expressions due to the ill-posed nature of cross-modal mapping from 1D speech to 3D facial motion.

Method: 1) Continuous 3D face representation via mesh parameterization and non-uniform differentiable sampling for detail acquisition; 2) Residual graph convolutional network with dual cross-attention for multi-modal feature fusion; 3) Lightweight regression network predicting vertex-wise geometric displacements in UV space.

Result: Significant improvements over state-of-the-art methods, particularly in synchronization accuracy of lip and eye movements, with accurate reconstruction of rich 3D facial motion details.

Conclusion: MMTalker effectively addresses the challenges of speech-driven 3D facial animation through innovative multi-resolution representation and multi-modal fusion, achieving superior lip-sync accuracy and realistic facial expressions.
...
TL;DR: Proposes a learning framework that uses provenance information from synthetic data generation to guide models to focus on target regions, suppressing reliance on non-target areas and artifacts.


<details>
  <summary>Details</summary>
Motivation: Existing synthetic data methods only indirectly improve robustness through data diversification and don't explicitly teach models which input regions truly contribute to discrimination, leading to learning of spurious correlations from synthesis biases and artifacts.

Method: Uses provenance information from synthetic data generation (indicating target vs. non-target regions) as auxiliary supervision. Decomposes input gradients based on this information and introduces gradient guidance to suppress gradients over non-target regions, promoting focus on target areas.

Result: Demonstrates effectiveness and generality across multiple tasks and modalities including weakly supervised object localization, spatio-temporal action localization, and image classification.

Conclusion: The proposed framework successfully suppresses model reliance on non-target regions and directly promotes learning of discriminative representations for target regions using synthetic data provenance information.
...
TL;DR: CrossWeaver: A flexible multimodal fusion framework for semantic segmentation that enables selective cross-modal interactions and achieves SOTA performance with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal segmentation methods rely on rigid fusion strategies with modality-specific adaptations or loosely coupled interactions, limiting flexibility and cross-modal coordination. They struggle to balance efficient information exchange while preserving unique modality characteristics across different modality combinations.

Method: Proposes CrossWeaver with two key components: 1) Modality Interaction Block (MIB) for selective, reliability-aware cross-modal interaction within the encoder, and 2) lightweight Seam-Aligned Fusion (SAF) module for aggregating enhanced features. Designed for arbitrary-modality semantic segmentation.
...
Conclusion: CrossWeaver provides an effective solution for flexible multimodal fusion in semantic segmentation, addressing limitations of existing approaches while maintaining efficiency and generalization capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing pruning methods for VLMs focus on either parameters OR tokens, missing the opportunity to explore redundancy in both modes simultaneously, which causes significant performance drops at high pruning ratios.

Method: 1) Collaborative Importance Metric (CIM) that accounts for mutual interference between parameters and tokens, 2) Multi-Mode Pruning Strategy (MPS) that decomposes pruning into stages with adaptive mode switching based on cost, and integrates historical cost with random exploration.
...
Conclusion: CoMP successfully addresses the limitations of single-mode pruning by jointly optimizing parameter and token pruning, achieving better compression-performance trade-offs for resource-constrained deployment of VLMs.
...
TL;DR: UAVGen is a novel layout-to-image generation framework for UAV-based object detection that improves synthesis quality through visual prototype conditioning and focal region enhancement.


<details>
  <summary>Details</summary>
Motivation: UAV-based object detection faces challenges in dynamic scenarios with limited annotated data. Existing layout-to-image generation methods using diffusion models often produce artifacts near layout boundaries of tiny objects, limiting detection performance.

Method: UAVGen introduces: 1) Visual Prototype Conditioned Diffusion Model (VPC-DM) that constructs representative instances for each class and integrates them into latent embeddings for high-fidelity object generation; 2) Focal Region Enhanced Data Pipeline (FRE-DP) to emphasize object-concentrated foreground regions in synthesis; 3) Label refinement to correct missing, extra, and misaligned generations.
...
Conclusion: The proposed UAVGen framework effectively addresses artifact issues in layout-to-image generation for UAV-based object detection, providing high-quality synthetic data that enhances detection performance in data-limited scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Text-driven human motion generation struggles with accurately aligning motion dynamics with textual semantics, and existing methods suffer from attention sink phenomenon where attention disproportionately focuses on start tokens, limiting semantic grounding.
...
Conclusion: The proposed approach effectively addresses motion-language alignment challenges and attention sink problems, leading to better text-to-motion generation performance.
...
TL;DR: Downsampling fundus images for retinal vessel segmentation harms thin vessel detection, especially in high-resolution datasets, but standard Dice metrics fail to capture this loss.


<details>
  <summary>Details</summary>
Motivation: The common practice of resizing fundus images for GPU memory constraints may cause irreversible information loss for thin vessels, but this impact is not captured by standard evaluation metrics like Dice score.

Method: Trained UNet at multiple downsampling ratios across five fundus datasets (DRIVE, STARE, CHASE_DB1, HRF, FIVES). Introduced width-stratified sensitivity metric to evaluate thin, medium, and thick vessel detection separately using native resolution width estimates from Euclidean distance transform.
...
Conclusion: Downsampling significantly impacts thin vessel detection, and Dice score alone is insufficient for evaluating microvascular segmentation quality. The optimal processing width depends on dataset resolution.
...
TL;DR: SCOPE: A training-free framework that accelerates autoregressive video diffusion models through tri-modal scheduling and selective computation, achieving up to 4.73x speedup while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Current autoregressive video diffusion models are computationally expensive due to repeated multi-step denoising. Existing training-free acceleration methods use binary cache-or-recompute decisions that overlook intermediate cases, and they process entire intervals uniformly despite asynchronous AR schedules assigning different noise levels to co-generated frames.

Method: SCOPE introduces a tri-modal scheduler with cache, predict, and recompute modes. Prediction uses noise-level Taylor extrapolation to fill the gap between reuse and recomputation, with explicit stability controls backed by error propagation analysis. It also implements selective computation that restricts execution to the active frame interval only.
...
Conclusion: SCOPE effectively addresses AR-specific inefficiencies in video diffusion models through its novel tri-modal scheduling and selective computation approach, providing significant acceleration without requiring retraining while preserving output quality.
...
TL;DR: MM-GS: A hierarchical 3D Gaussian Splatting framework for reconstructing dynamic scenes with multiple humans and objects from sparse views, addressing occlusion and interaction challenges.


<details>
  <summary>Details</summary>
Motivation: Reconstructing dynamic scenes with multiple interacting humans and objects (MHMO rendering) is critical for digital twins in robotics and VR/AR, but faces challenges with view consistency under severe occlusion and modeling complex interaction dependencies.

Method: Proposes MM-GS built on 3D Gaussian Splatting with two modules: 1) Per-Instance Multi-View Fusion to establish robust representations by aggregating visual information across views, and 2) Scene-Level Instance Interaction module that operates on a global scene graph to reason about relationships and refine attributes.

Result: Extensive experiments on challenging datasets show MM-GS significantly outperforms strong baselines, achieving state-of-the-art results with high-fidelity details and plausible inter-instance contacts.

Conclusion: MM-GS effectively addresses the challenges of MHMO rendering through hierarchical representation and explicit interaction modeling, enabling high-quality reconstruction of complex dynamic scenes from sparse views.
...
TL;DR: A plug-and-play Wavelet Feature Stream that enhances skeleton-based gait recognition by adding time-frequency analysis of joint velocities, improving performance especially under appearance changes like carrying bags or wearing coats.


<details>
  <summary>Details</summary>
Motivation: Skeleton-based gait recognizers focus on spatial configurations but underutilize explicit motion dynamics, which are crucial for handling appearance changes like clothing variations and carrying objects.

Method: Proposes a Wavelet Feature Stream that transforms per-joint velocity sequences using continuous wavelet transform (CWT) into multi-scale scalograms, then uses a lightweight multi-scale CNN to learn discriminative dynamic cues, which are fused with backbone representations without modifying the backbone architecture.

Result: Consistent performance gains across strong skeleton backbones (GaitMixer, GaitFormer, GaitGraph) on CASIA-B dataset, establishing new state-of-the-art when attached to GaitMixer, with particularly significant improvements under covariate shifts (carrying bags and wearing coats).

Conclusion: Explicit time-frequency modeling of motion dynamics complements standard spatio-temporal encoders, providing robust gait recognition under appearance changes through a plug-and-play module that requires no architectural changes or additional supervision.
...
TL;DR: Multi-stage pipeline for smoke-degraded 3D scene restoration combining image restoration, dehazing, MLLM enhancement, and 3DGS-MCMC optimization to improve visibility while maintaining cross-view consistency.
...
Method: Multi-stage pipeline with image restoration, dehazing, MLLM-based enhancement, 3DGS-MCMC optimization, and averaging over repeated runs to improve visibility while limiting scene-content changes across views.

Result: Achieved 1st place out of 14 participants in Track 2 of NTIRE 2026 3DRR Challenge, showing improved quantitative performance and better visual quality than baselines.
...
TL;DR: QVAD is a question-centric agentic framework for video anomaly detection that uses dynamic LLM-VLM dialogue instead of static prompts, achieving SOTA performance with lightweight models.


<details>
  <summary>Details</summary>
Motivation: Current VAD methods rely on massive foundation models to compensate for static prompt ambiguity. The bottleneck is not model capacity but the static nature of inquiry.

Method: QVAD treats VLM-LLM interaction as dynamic dialogue where an LLM agent iteratively refines queries based on visual context to guide smaller VLMs, producing high-fidelity captions without parameter updates.

Result: Achieves state-of-the-art performance on UCF-Crime, XD-Violence, and UBNormal using fraction of parameters, demonstrates exceptional generalizability on ComplexVAD, with high inference speeds and minimal memory footprint.

Conclusion: QVAD's prompt-updating mechanism unlocks latent capabilities of lightweight models, making advanced VAD deployable on resource-constrained edge devices through efficient dynamic dialogue.
...
TL;DR: STEAR is a layer-aware framework that mitigates spatiotemporal hallucinations in Video-LLMs by identifying high-risk decoding steps, selecting token-conditioned visual evidence from middle layers, and using this evidence to restore missing grounding and construct counterfactuals for inconsistent reasoning.
...
Result: Experiments across representative Video-LLM backbones and challenging benchmarks show STEAR consistently reduces hallucinations while improving faithfulness, temporal consistency, and robustness. The approach confirms that reliable video decoding requires intervening on precise evidence at the right layer rather than enforcing global penalties.

Conclusion: STEAR demonstrates that effective hallucination mitigation in Video-LLMs requires layer-aware intervention strategies that address the distinct roles of different decoder layers in visual grounding and linguistic composition, rather than applying uniform correction rules across all layers.
...
TL;DR: Nano Banana 2 shows strong potential as a unified image restoration solution, achieving superior full-reference metrics and competitive perceptual quality with proper prompt design, though it requires careful prompt formulation.
...
Result: Nano Banana 2 achieves superior performance in full-reference metrics while remaining competitive in perceptual quality compared to specialized restoration models. It shows strong generalization in challenging scenarios like small faces, dense crowds, and severe degradations, but is sensitive to prompt formulation.

Conclusion: General-purpose generative models hold strong potential as unified image restoration solvers, but require improved controllability and robustness in prompt formulation for optimal results.
...
TL;DR: GMMD is a new image realism metric that uses Gram matrices from neural network activations to capture fine-grained textural information, complementing existing semantic-level metrics like FID and CMMD.


<details>
  <summary>Details</summary>
Motivation: Existing distributional metrics (FID, CMMD) focus on semantic-level feature distributions but overlook fine-grained textural information that's important for distinguishing real from generated images.

Method: GMMD extracts Gram matrices from intermediate activations of pretrained networks, takes the upper-triangular part, and computes Maximum Mean Discrepancy (MMD) between real and generated image distributions. Hyperparameters are selected using a meta-metric protocol with controlled degradations and rank correlation measures.

Result: GMMD captures complementary textural information to semantic metrics, correctly ranks real vs synthetic images in cross-domain scenarios where CMMD fails, and performs well across various backbone architectures on KADID-10k and RAISE datasets.

Conclusion: GMMD provides a valuable complement to existing semantic-level realism metrics by capturing finer textural details, addressing limitations of current approaches in distinguishing real from generated images.
...
TL;DR: SparseSplat is a feed-forward 3D Gaussian Splatting model that adaptively adjusts Gaussian density based on scene structure, achieving state-of-the-art rendering quality with significantly fewer Gaussians (22% of previous methods).


<details>
  <summary>Details</summary>
Motivation: Previous feed-forward 3DGS methods generate spatially uniform and highly redundant 3DGS maps, which limits their integration into downstream reconstruction tasks due to inefficiency and lack of structural adaptation.
...
Result: Achieves state-of-the-art rendering quality with only 22% of the Gaussians used by previous methods, and maintains reasonable rendering quality with only 1.5% of Gaussians.

Conclusion: SparseSplat successfully creates highly compact 3DGS maps through adaptive density adjustment, enabling efficient high-quality rendering while being more suitable for downstream reconstruction tasks.
...
TL;DR: MI-Pruner uses mutual information between visual and textual features for surgical visual token pruning in MLLMs, outperforming attention-based methods without architectural changes.


<details>
  <summary>Details</summary>
Motivation: Visual information is sparse compared to text in MLLMs, requiring efficient inference through visual pruning. Current attention-based methods have limitations, so the authors pursue a more surgical approach using direct feature-level crossmodal dependency measurement.

Method: MI-Pruner computes Mutual Information (MI) between visual and textual features themselves before their interaction, explicitly measuring crossmodal dependency at feature levels. It's simple, efficient, non-intrusive, requiring no access to internal attention maps or architectural modifications.

Result: Experimental results show MI-Pruner outperforms previous attention-based pruning methods with minimal latency.

Conclusion: Direct mutual information computation between visual and textual features provides a superior approach for visual token pruning in MLLMs compared to attention-based methods, offering better performance with minimal computational overhead.
...
TL;DR: Vision Transformer with focal loss achieves best performance for sea ice classification using SAR data, establishing a trustworthy baseline for future multimodal fusion.


<details>
  <summary>Details</summary>
Motivation: Accurate and automated sea ice classification is crucial for Arctic climate monitoring and maritime safety, but current SAR-based methods struggle with distinguishing morphologically similar ice classes under severe class imbalance.

Method: Used AI4Arctic/ASIP Sea Ice Dataset with Sentinel-1 SAR data, combined full-resolution inputs, leakage-aware stratified patch splitting, SIGRID-3 labels, and training-set normalization. Evaluated Vision Transformers (ViT-Base with cross-entropy/weighted cross-entropy vs ViT-Large with focal loss).

Result: ViT-Large with focal loss achieved 69.6% held-out accuracy, 68.8% weighted F1, and 83.9% precision on minority Multi-Year Ice class, showing better precision-recall trade-off for rare ice classes.

Conclusion: Focal-loss training provides superior performance for rare ice classes compared to weighted cross-entropy, establishing a clean baseline for future multimodal fusion with optical, thermal, or meteorological data.
...
TL;DR: VLM-UnBench is the first benchmark for training-free visual concept unlearning in VLMs, revealing that current prompt-based approaches fail to achieve true concept erasure and only work under unrealistic oracle conditions.
...
Result: Realistic unlearning prompts leave forget accuracy near baseline; meaningful reductions only occur under oracle conditions. Object/scene concepts are most resistant, and stronger instruction-tuned models remain capable despite explicit forget instructions.
...
TL;DR: UCGP is a universal physical adversarial patch framework that disrupts infrared vision-language models by attacking their visual representation space with curved-grid patches, maintaining effectiveness across models, datasets, and real-world conditions.


<details>
  <summary>Details</summary>
Motivation: Infrared VLMs lack robustness analysis against adversarial attacks, and existing RGB-based patch methods don't address the open-ended semantic understanding and physical deployment needs of infrared multimodal systems.

Method: UCGP integrates Curved-Grid Mesh parameterization for continuous patch generation with a unified representation-driven objective, plus Meta Differential Evolution and EOT-augmented TPS deformation for real-world robustness.
...
Conclusion: The work reveals previously overlooked robustness vulnerabilities in current infrared multimodal systems through a novel adversarial attack framework.
...
TL;DR: SC-DMD improves video generation quality at extremely low inference budgets (2-4 NFEs) by combining distribution matching with explicit regularization of endpoint-consistent denoising updates, plus cache-aware training for real-time autoregressive generation.


<details>
  <summary>Details</summary>
Motivation: Current distillation methods for low-NFE video generation have limitations: trajectory-style consistency distillation produces over-smoothed results with weak motion, while distribution matching distillation (DMD) suffers from drift in composed rollouts due to lack of regularization across timesteps.

Method: Proposes Self-Consistent Distribution Matching Distillation (SC-DMD) that explicitly regularizes endpoint-consistent composition of consecutive denoising updates. For real-time autoregressive generation, introduces Cache-Distribution-Aware training with SC-DMD over multi-step rollouts and cache-conditioned feature alignment to steer low-quality outputs toward high-quality references.

Result: The method (dubbed Salt) consistently improves low-NFE video generation quality across experiments with non-autoregressive backbones (Wan 2.1) and autoregressive real-time paradigms (Self Forcing), while remaining compatible with diverse KV-cache memory mechanisms.

Conclusion: SC-DMD effectively addresses the limitations of existing distillation approaches for low-NFE video generation, enabling high-quality real-time video synthesis at extremely low inference budgets through explicit regularization of denoising composition and cache-aware training.
...
TL;DR: SCC-Loc is a semantic-cascade-consensus framework for cross-modal thermal geo-localization that addresses modality gaps between thermal and visible imagery, achieving state-of-the-art accuracy with 9.37m mean error.


<details>
  <summary>Details</summary>
Motivation: Thermal geo-localization is crucial for UAVs in GNSS-denied environments, but significant modality gaps between thermal and visible satellite imagery cause feature ambiguity that corrupts conventional registration methods.

Method: A unified framework with three components: 1) Semantic-Guided Viewport Alignment for optimizing satellite crop regions, 2) Cascaded Spatial-Adaptive Texture-Structure Filtering for geometric consistency, and 3) Consensus-Driven Reliability-Aware Position Selection for optimal pose estimation. Uses shared DINOv2 backbone for efficiency.

Result: Achieves state-of-the-art performance with 9.37m mean localization error and 7.6x accuracy improvement within 5m threshold. Also introduces Thermal-UAV dataset with 11,890 thermal queries.

Conclusion: SCC-Loc effectively addresses thermal-visible modality gaps through semantic guidance, cascade filtering, and consensus optimization, providing robust geo-localization for UAVs in challenging environments.
...
TL;DR: SD-FSMIS adapts Stable Diffusion for few-shot medical image segmentation using support-query interaction and visual-to-textual condition translation, achieving competitive results with strong cross-domain generalization.
...
Method: Adapt pre-trained Stable Diffusion with two key components: Support-Query Interaction (SQI) for adapting to FSMIS paradigm, and Visual-to-Textual Condition Translator (VTCT) that converts visual cues from support set into implicit textual embeddings to guide diffusion.
...
TL;DR: CAMEO is a multi-agent framework that reformulates conditional image editing as a quality-aware, feedback-driven process instead of single-step generation, achieving better structural control and consistency.


<details>
  <summary>Details</summary>
Motivation: Existing conditional image editing methods rely on single-step generation which lacks explicit quality control, often introduces excessive deviation from original images, produces structural artifacts, and requires manual prompt tuning for acceptable results.
...
Conclusion: The structured multi-agent framework with embedded quality control and iterative refinement significantly improves conditional image editing performance over existing single-step generation approaches.
...
TL;DR: EffiMiniVLM: A compact dual-encoder vision-language regression framework for product quality prediction in cold-start scenarios, achieving competitive performance with 27.7M parameters while being 4-8x more resource-efficient than larger models.
...
Method: Proposes EffiMiniVLM with EfficientNet-B0 image encoder and MiniLM-based text encoder, plus lightweight regression head. Uses weighted Huber loss that leverages rating counts to emphasize reliable samples for better training efficiency.

Result: Achieves CES score of 0.40 with only 27.7M parameters and 6.8 GFLOPs, trained on just 20% of Amazon Reviews 2023 dataset. Outperforms larger models in resource efficiency (4-8x better) and scales well - with 40% data, overtakes other methods using larger models and datasets.

Conclusion: EffiMiniVLM demonstrates that compact vision-language models can achieve competitive performance for product quality prediction while being significantly more resource-efficient, especially valuable for cold-start scenarios without requiring external datasets.
...
TL;DR: SFFNet is a synergistic feature fusion network with dual-domain edge enhancement for UAV object detection, addressing background noise and scale imbalance through frequency/spatial domain edge extraction and adaptive feature pyramid fusion.
...
Method: 1) MDDC module: dual-driven edge extraction in frequency and spatial domains to decouple multi-scale object edges from background noise. 2) SFPN: synergistic feature pyramid network using linear deformable convolutions for irregular shapes and wide-area perception module for contextual associations. 3) Six detector scales (N/S/M/B/L/X) for different application scenarios.
...
Conclusion: The proposed SFFNet effectively addresses UAV object detection challenges through dual-domain edge enhancement and synergistic feature fusion, demonstrating superior performance on challenging aerial datasets while offering scalable solutions for different resource constraints.
...
TL;DR: NTIRE 2026 challenge review on efficient single-image super-resolution, focusing on solutions that reduce computational costs while maintaining image quality metrics.


<details>
  <summary>Details</summary>
Motivation: To advance efficient single-image super-resolution by developing networks that reduce computational aspects (runtime, parameters, FLOPs) while maintaining high PSNR scores on benchmark datasets.

Method: Challenge-based approach where 95 registered participants and 15 teams submitted solutions. The challenge required maintaining PSNR around 26.90 dB on DIV2K_LSDIR_valid and 26.99 dB on DIV2K_LSDIR_test datasets while optimizing efficiency metrics.

Result: The challenge successfully gauged state-of-the-art results for efficient single-image super-resolution, with multiple teams developing solutions that balance computational efficiency with image quality preservation.

Conclusion: The NTIRE 2026 challenge effectively benchmarked current progress in efficient super-resolution, highlighting innovative approaches to reduce computational costs while maintaining performance standards.
...
TL;DR: PR3DICTR is an open-source platform for 3D medical image classification research, built on PyTorch and MONAI, offering modular design with minimal coding requirements.
...
Method: Built using community-standard distributions (PyTorch and MONAI) with modular design principles. Provides pre-established functionality for model architectures, hyperparameters, and training methods while allowing users to "plug in" their own modules.

Result: A platform that can be applied to any binary or event-based 3D classification task with as little as two lines of code, offering both convenience and adjustability.

Conclusion: PR3DICTR successfully creates an accessible, standardized platform for 3D medical image classification research that balances ease of use with customization capabilities.
...
TL;DR: ProtoFlow: A continual learning framework for remote sensing segmentation that models class prototypes as temporal trajectories using vector fields to reduce forgetting and representation drift.


<details>
  <summary>Details</summary>
Motivation: Remote sensing segmentation faces continual learning challenges: new semantic categories emerge over time, and acquisition conditions shift across seasons, cities, and sensors. Existing incremental approaches treat training steps as isolated updates, leading to insufficient control over representation drift and forgetting.

Method: ProtoFlow introduces a time-aware prototype dynamics framework that models class prototypes as trajectories and learns their evolution with an explicit temporal vector field. The approach jointly enforces low-curvature motion and inter-class separation to stabilize prototype geometry throughout incremental learning.
...
Conclusion: Explicitly modeling temporal prototype evolution is a practical and interpretable strategy for robust continual remote sensing segmentation, addressing the inherent continual nature of remote sensing deployment.
...
TL;DR: VOSR is a vision-only generative super-resolution framework that rivals T2I-based methods without multimodal pretraining, achieving competitive quality with 10x less training cost.


<details>
  <summary>Details</summary>
Motivation: Current generative SR methods rely on adapting large text-to-image diffusion models, which is inefficient for a fundamentally LR input-conditioned restoration task. The authors investigate whether SR models trained purely on visual data can match T2I-based approaches.

Method: 1) Extract semantically rich features from LR input using pretrained vision encoder as visual semantic guidance. 2) Replace standard unconditional branch in classifier-free guidance with restoration-oriented guidance that preserves weak LR anchors. 3) Train multi-step model from scratch then distill into one-step model for efficient inference.

Result: VOSR requires <10% training cost of T2I-based methods, achieves competitive/better perceptual quality and efficiency in both multi-step and one-step settings, produces more faithful structures with fewer hallucinations on synthetic and real-world benchmarks.

Conclusion: High-quality generative SR can be achieved without multimodal pretraining, challenging the current paradigm of adapting T2I models for SR tasks.
...
TL;DR: CoME-VL integrates contrastive and self-supervised vision encoders via entropy-guided multi-layer aggregation and RoPE-enhanced cross-attention, achieving SOTA on visual understanding and grounding tasks.


<details>
  <summary>Details</summary>
Motivation: Contrastive vision encoders (like CLIP) are good for cross-modal alignment but self-supervised encoders (like DINO) capture richer dense semantics and robustness. The paper aims to fuse these complementary representations for better vision-language modeling.

Method: Proposes CoME-VL: Complementary Multi-Encoder Vision-Language framework with representation-level fusion using (1) entropy-guided multi-layer aggregation with orthogonality-constrained projections to reduce redundancy, and (2) RoPE-enhanced cross-attention to align heterogeneous token grids and produce compact fused visual tokens that can be injected into decoder-only LLMs.

Result: Consistently outperforms single-encoder baselines with average improvements of 4.9% on visual understanding tasks and 5.4% on grounding tasks. Achieves state-of-the-art performance on RefCOCO for detection and shows large margin improvements over baselines.

Conclusion: Fusing complementary contrastive and self-supervised visual representations through modular fusion framework significantly improves vision-language model performance across diverse benchmarks, demonstrating the value of multi-encoder approaches.
