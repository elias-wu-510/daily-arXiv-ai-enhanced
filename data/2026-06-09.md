...
TL;DR: This paper develops an automated image classification system for historical document pages (text, tables, graphics) using deep learning, achieving ~99% accuracy with RegNetY-16GF and ViT-large models, outperforming traditional feature-based methods and deployed on 649,508 unlabeled archival pages.


<details>
  <summary>Details</summary>
Motivation: Digitization projects in the humanities produce vast, heterogeneous archives of historical documents, making manual sorting impractical at scale. An automated system is needed to classify scanned page images by visual content type (text, tables, graphics) to enable content-specific downstream processing like OCR or structured data extraction.

Method: An image classification dataset of 48,000+ annotated historical page images from Czech archaeological archives was created through four iterative annotation stages with domain-expert review. A Random Forest baseline using hand-crafted features was compared against fine-tuned deep learning models: CNNs (EfficientNetV2, RegNetY), Vision/Document Image Transformers (ViT, DiT), and multimodal CLIP. An 11-category label scheme was evaluated via five-fold cross-validation.

Result: Random Forest baseline achieved ~75% accuracy. RegNetY-16GF achieved 99.16%, CLIP ViT-B/16 99.14%, and ViT-large 99.12% Top-1 accuracy. On 649,508 unlabeled pages, image-only models showed >90% inter-model agreement, while CLIP had <65% agreement with image-only models despite competitive test accuracy.

Conclusion: Image-only models, particularly RegNetY-16GF, deliver near-perfect classification accuracy and consistent labeling at scale. CLIP is less suitable for deployment due to poor agreement on unlabeled data. The models, dataset, and software are publicly available as open-source resources.
...
TL;DR: This thesis tackles group emotion recognition (GER) in real-world settings while preserving privacy. It proposes two frameworks: a cross-attention multimodal audio-video model with Frames Attention Pooling (FAP), and a Variational Encoder Multi-Decoder (VE-MD) for shared latent space learning. Results show competitive GER performance without needing individual-level features.


<details>
  <summary>Details</summary>
Motivation: Traditional emotion recognition relies on individual-level cues (face, gaze, voice), raising privacy and surveillance risks. The authors aim to infer group-level emotions from collective signals to reduce individual monitoring while maintaining effectiveness.

Method: Two frameworks: (1) Cross-attention multimodal architecture for audio-video fusion with Frames Attention Pooling (FAP) for temporal aggregation, plus synthetic data augmentation; (2) Variational Encoder Multi-Decoder (VE-MD) that learns a shared latent space for emotion classification and structural representation prediction, with DETR-based and heatmap-based decoding strategies.

Result: The proposed architectures demonstrate robustness in real-world GER conditions through ablation studies, achieving competitive performance without using individual features as input.

Conclusion: The thesis clarifies the role of multimodality and structural cues in group-level affective computing, and shows that privacy-preserving GER is feasible without sacrificing performance, reducing risks of individual surveillance.
...
TL;DR: SlideCheck is a lightweight tool that uses frozen pathology foundation model features to score patches for abnormality and malignancy, enabling guided filtering and auditing of pretraining data for pathology foundation models.


<details>
  <summary>Details</summary>
Motivation: Pathology foundation models are pretrained on large WSI-derived patch collections with slide-level supervision that is sparse or heterogeneous, creating a mismatch that makes it difficult to understand which biological patterns enter pretraining data. There is a need for controllable and auditable pretraining data construction.

Method: SlideCheck uses a dual-head MLP to separately model broad abnormal morphology and malignant evidence. It employs a regularized feature-space scorer for supervised patch-level evidence estimation, score-attention agreement to combine patch scores with MIL attention for high-confidence pseudo labels, and constructs broad-positive ViT pretraining subsets by thresholding abnormality or malignancy scores.

Result: SlideCheck-defined data distributions influence downstream behavior of self-supervised ViT pretraining. Curated subsets can approach full-data performance, demonstrating that biological composition is a controllable factor in pathology foundation model development.

Conclusion: SlideCheck serves as a data guidance and auditing layer that transforms large, undifferentiated patch pools into controllable and reusable pretraining datasets for pathology foundation models.
...
TL;DR: This paper analyzes how adversarial fine-tuning affects Vision Transformers (ViTs) by examining attention mechanisms, internal representations, and knowledge evolution when models are trained on low-frequency and high-frequency image corruptions. Results show improvements on seen corruptions but no transfer to unseen ones, and no fundamental changes in sparse representations.


<details>
  <summary>Details</summary>
Motivation: Image classification models deployed in high-risk, real-world situations need robustness to perturbations like blurring or sharpening. Vision Transformers (ViTs), despite being integral to modern multi-modal models (VLMs, VLA models), have received insufficient attention regarding robustness to such perturbations.

Method: Adversarial fine-tuning of a ViT on low-frequency and high-frequency image corruptions, followed by mechanistic analysis of attention mechanisms, internal representations, and knowledge evolution across layers.

Result: Fine-tuning on common corruptions improves performance and certainty on new instances of corrupted data, but these improvements do not transfer to unseen corruption classes. Adversarial training did not lead to fundamental changes in the sparse representations learned by ViTs.

Conclusion: Adversarial fine-tuning on specific corruptions yields only narrow, non-transferable robustness in ViTs, and does not fundamentally alter the underlying sparse representations.
...
TL;DR: This paper introduces VisualLeakBench, a 500-image benchmark to study how vision-language agents propagate sensitive text (PII and unsafe content) from images into tool arguments. Experiments show 78.8% PII and 85.5% unsafe-text propagation at baseline, with defensive prompts reducing PII propagation to 2.0% but often by suppressing tool use. The work focuses on visual-to-tool propagation rather than downstream execution.


<details>
  <summary>Details</summary>
Motivation: To study the concrete failure mode of action-boundary propagation, where sensitive visible text from images gets copied into downstream tool arguments in vision-language agent systems.

Method: Constructs VisualLeakBench (500 images across 5 scene types: UI, chat, document, form, dashboard), evaluates a stratified 100-image subset with 4 production VLM systems under two workflows (note capture and external handoff), with and without defensive prompts, and provides a labeled-target oracle diagnostic.

Result: Baseline propagation rates: 78.8% PII, 85.5% unsafe-text. Defensive prompts reduce PII to 2.0% but unsafe-text remains high at 52.6%. PII reduction often results from suppressing tool use, not safe handling. Tool surface type matters: search-like tools suppress PII but not unsafe text.

Conclusion: Action-boundary propagation is a significant vulnerability in vision-language agents, especially for unsafe rendered text. Current defensive measures trade utility for safety unevenly. The tool boundary is the primary failure locus, with response-side leakage as residual risk.
...
TL;DR: This study finds that both humans (64.8% accuracy) and frontier MLLMs (100% specificity but poor sensitivity) struggle to distinguish real from AI-generated evidence photos in legal contexts, and recommends a multi-layered verification approach combining human review, MLLM screening, and provenance infrastructure.


<details>
  <summary>Details</summary>
Motivation: AI-generated images are becoming indistinguishable from real evidence photos, threatening the long-held assumption that visual evidence is reliable legal proof, yet no systematic study has assessed human and AI detection capabilities in legal scenarios.

Method: Created SLED-1400 dataset (200 authentic + 1,200 synthetic images from 6 generators across 10 evidence categories); conducted controlled web experiment with 136 lay participants and standardized evaluation of 4 MLLMs (GPT-5.1, Gemini-3-Pro, Gemini-3-Flash, Qwen3-VL-235B) using identical stimuli.

Result: Humans achieved 64.8% overall accuracy (48.5-51% on strongest generators, near chance); MLLMs had 100% specificity but only 5.9% detection on hardest generators; human and MLLM errors uncorrelated, but MLLMs strongly correlated with each other.

Conclusion: Neither humans nor MLLMs are reliable standalone authenticators; visual evidence should be treated as inherently contestable, requiring combined trained human review, MLLM screening, and provenance infrastructure (e.g., C2PA).
...
TL;DR: This paper introduces a statistical fault injection framework using finite-population sampling theory to evaluate Vision Transformer reliability against soft errors, achieving up to 10,700x cost reduction while providing formal guarantees (1% margin at 99% confidence). It reveals that only 3% of FP32 bit-flips cause failure, primarily in normalization layers and critical exponent bits, leading to catastrophic accuracy collapse.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers are increasingly deployed in safety-critical domains (autonomous systems, medical imaging), but their massive parameter counts make exhaustive fault injection infeasible. There is a need for efficient, statistically rigorous methods to assess reliability against soft errors.

Method: A statistical fault injection framework based on finite-population sampling theory is proposed, providing formal reliability guarantees with bounded failure rates (1% margin at 99% confidence) using only a few thousand samples, regardless of model scale.

Result: Achieves up to 10,700x reduction in experimental cost versus exhaustive approaches. Only 3% of FP32 bit-flips cause failure, but the vast majority of those lead to catastrophic accuracy collapse. Vulnerabilities are localized to normalization layers and critical exponent bits in the IEEE-754 format.

Conclusion: The framework provides a mathematical foundation and actionable insights for designing hardened, edge-deployed ViT architectures, highlighting a highly non-uniform reliability landscape across architectural components.
...
TL;DR: This paper studies a 360-degree LiDAR perception pipeline for autonomous driving, combining sector-wise panoramic processing with rotation equivariant sparse convolutions for object detection in dense urban traffic. It achieves strong detection for cars (92.02/90.51), buses (80.53/76.34), and trucks (78.59/74.16), with lower performance for pedestrians (67.45/61.02) and cyclists (73.21/69.54).


<details>
  <summary>Details</summary>
Motivation: Autonomous driving perception in dense, unstructured urban traffic remains challenging due to diverse road users, occlusions, irregular motion, and lack of standardized layouts. Existing LiDAR-based 3D detectors are mostly evaluated under limited field-of-view settings, and their behavior under full 360-degree surround sensing is not well understood.

Method: The paper proposes a 360-degree perception framework combining sector-wise panoramic processing with rotation equivariant sparse convolutions. A custom Ouster OS0 LiDAR dataset is collected across diverse Indian urban traffic conditions for evaluation.

Result: Cars achieve the highest detection scores (92.02/90.51), followed by buses (80.53/76.34) and trucks (78.59/74.16). Pedestrians (67.45/61.02), cyclists (73.21/69.54), and motorcyclists (71.20/68.13) show lower performance due to their smaller size and higher variability.

Conclusion: The proposed 360-degree perception framework demonstrates generally stable detection across object classes, with the most robust results for larger vehicles. Smaller, more variable road users remain challenging, highlighting areas for future improvement in dense urban perception.
...
TL;DR: AMN is a dual-encoder (Swin Transformer + ResNet-50) nuclei segmentation framework with adaptive gated fusion and multi-objective loss, achieving state-of-the-art results on CoNIC and MoNuSeg benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing nuclei classification methods use either CNNs or transformers alone, failing to capture both fine-grained local texture (CNNs) and long-range spatial context (transformers) simultaneously.

Method: Dual-encoder (Swin Transformer + ResNet-50 FPN) with per-channel gating mechanism for adaptive fusion, trained with multi-objective loss: class-weighted focal loss + boundary-aware loss + uncertainty-modulated classification term to suppress overconfident errors.

Result: Mean Dice 0.82, mean F1 0.68 on CoNIC (7 classes); F1 0.67 on lymphocyte class; outperforms 8 baselines (U-Net, ResU-Net, DeepLabV3+, SegNet, ViT-Small, HmsU-Net, ConvFormer-UNet, BEFUnet); strong cross-dataset generalization on MoNuSeg.

Conclusion: The hybrid dual-encoder design with adaptive gating effectively balances local and global features, improving nuclei classification accuracy and domain generalization over pure-CNN, pure-transformer, and existing hybrid architectures.
...
TL;DR: NeuroAlign is a hierarchical multimodal fusion framework that aligns fMRI and DTI data for cognitive impairment detection, achieving competitive performance on MCI/SCD classification with interpretable brain pattern attribution.


<details>
  <summary>Details</summary>
Motivation: Multimodal neuroimaging fusion (fMRI + DTI) offers complementary insights for cognitive impairment analysis, but is hindered by heterogeneous feature spaces and misaligned representations across modalities.

Method: Proposes NeuroAlign with three components: (1) Dual-Modal Hierarchical Alignment (DMHA) for multi-scale dynamic connectivity alignment; (2) Dual-Domain Hierarchical Interaction (DDHI) for fine-grained modulation and global interaction; (3) Synergistic Activation Mapping (SAM) for gradient-free attribution analysis.

Result: Evaluated on GUTCM, ADNI, and OASIS with five-fold validation, achieving competitive MCI/SCD detection and preliminary cross-dataset transferability. Attribution reveals modality-specific and partially consistent brain patterns.

Conclusion: NeuroAlign effectively addresses multimodal fusion challenges and provides model-derived interpretable evidence for multimodal representation analysis in cognitive impairment.
...
TL;DR: Crayotter is an open-source multimodal multi-agent system for prompt-driven long-form video editing. It structures production into three phases (material preparation, editing research, timeline execution) with externalized artifacts for traceability and selective revision. It outperforms baselines on human evaluation (3.40/5 vs. 2.44 and 1.70) and prepares workflows for future policy optimization.


<details>
  <summary>Details</summary>
Motivation: Editing long-form video from heterogeneous footage requires maintaining narrative intent across multiple production stages while enabling diagnosis and selective revision of failures, rather than requiring full restarts.

Method: A multimodal multi-agent system with three phases: (1) coverage-aware material preparation, (2) artifact-based editing research, (3) tool-grounded timeline execution. Externalizes inspectable artifacts (coverage reports, analyses, blueprints, tool calls, renders) for traceability and selective revision.

Result: On 23 editing themes, Crayotter achieves 3.40/5 average human evaluation score vs. 2.44 (CapCut-Mate) and 1.70 (CutClaw), with consistent gains in theme alignment, narrative coherence, and editing smoothness.

Conclusion: Crayotter demonstrates effective long-form video editing with traceable, selectively revisable workflows, and provides a trajectory schema and reward design for future policy optimization. Code is publicly available.
...
TL;DR: This paper proposes an anchor-conditioned finetuning framework for landscape image generation that injects 4D compositional anchor vectors into diffusion models via decoupled cross-attention with Fourier encoding and three-way classifier-free guidance dropout, achieving 0.850 horizon detection rate and 0.817 rule-of-thirds alignment, with category-specific training reducing horizon deviation by up to 40%.


<details>
  <summary>Details</summary>
Motivation: Image generative models lack compositional control for photographers/artists; current tools don't support precise scene composition like horizon placement or rule-of-thirds alignment.

Method: Anchor-conditioned finetuning framework extracting 4D compositional anchor vectors from training images, injected via decoupled cross-attention with Fourier encoding and three-way classifier-free guidance dropout.

Result: Horizon detection rate of 0.850, rule-of-thirds alignment of 0.817; category-specific training reduces horizon deviation by 40% vs mixed training.

Conclusion: Compositional control precision is category-dependent; training on homogeneous scene subsets yields better compositional fidelity.
...
TL;DR: MOSS-Video-Preview proposes a two-channel architecture for real-time video understanding where perception (cross-attention) and generation run on separate non-blocking pathways, enabling continuous perception, answer revision, and timely silence with 5x faster time-to-first-token and 2.7x higher decoding throughput compared to Qwen2.5-VL-7B.


<details>
  <summary>Details</summary>
Motivation: Current video understanding follows an offline paradigm that produces a single answer after viewing the entire video, but real-time interaction requires models to perceive new frames while replying, revise answers as new evidence appears, and remain silent when appropriate. Perception must not be blocked by generation.

Method: A two-channel architecture using cross-attention backbone where visual features enter through a side channel rather than joining the autoregressive sequence, keeping perception and generation on separate non-blocking pathways. A data synthesis pipeline converts dense captions into real-time understanding QA with revised answers, and an offline model is specialized on these data to elicit real-time behavior.

Result: Trails Qwen2.5-VL-7B overall but achieves competitive offline video/multimodal understanding, robust spatial and fine-grained temporal reasoning, and acquires real-time behaviors (continuous perception, answer revision, timely silence). On single H200 with 256 frames: ~5x speedup in time-to-first-token, 2.7x higher decoding throughput, negligible offline degradation.

Conclusion: The study of paradigm, architecture, and data outlines a viable path toward real-time video understanding, with the performance gap attributed primarily to data and scale rather than architecture.
...
TL;DR: This paper evaluates VAE, GAN, and DDPM models on fidelity, privacy, and utility trade-offs in synthetic data generation with differential privacy, finding GAN and DDPM more robust than VAE under privacy constraints.


<details>
  <summary>Details</summary>
Motivation: To investigate the trade-offs between fidelity, privacy, and utility in synthetic data generation under conditions of data scarcity and privacy sensitivity.

Method: Proposes an evaluation framework jointly assessing fidelity, privacy, and utility, applied to VAE, GAN, and DDPM across MNIST, OCTMNIST, and OrganAMNIST datasets.

Result: GAN and DDPM show greater robustness with higher fidelity and downstream utility under differential privacy, while VAE degrades more rapidly as privacy constraints increase.

Conclusion: Highlights the importance of multidimensional evaluation of deep generative models, noting significant behavioral differences when privacy techniques are applied.
...
TL;DR: Vision-language models struggle to predict what a 180° rotated image would reveal, even though they can recognize content when directly shown either the original or rotated version.


<details>
  <summary>Details</summary>
Motivation: To investigate whether VLMs can mentally simulate spatial transformations (specifically 180° rotation) and predict the resulting visual content without seeing the transformed image.

Method: Introduces RotOutBench, a paired diagnostic benchmark with open visual cases and controlled text-image rotations, along with a model-level case study comparing prediction vs. direct recognition accuracy.

Result: VLMs show high accuracy on direct recognition of both original and rotated images, but predicted-rotation accuracy collapses to near zero, especially on controlled text-image rotations.

Conclusion: Current VLMs can recognize a transformed visual state when presented, but fail to predict that state from the original view, revealing a gap in mental transformation abilities.
...
TL;DR: This paper proposes an expert-guided retrieval-augmented framework using vision-language models (VLMs) to assess wheelchair accessibility from Google Street View imagery, showing partial alignment with real-world wheelchair dwell behavior as a mobility proxy.


<details>
  <summary>Details</summary>
Motivation: Assessing built-environment interaction like wheelchair accessibility is difficult because real-world mobility is shaped by distributed, context-dependent, and temporary barriers that are hard to capture at scale.

Method: An expert-guided retrieval-augmented framework that combines GSV images, ADA-informed guidance, and expert-derived rubrics to evaluate accessibility dimensions. A campus-scale dataset at the University of Florida links 407 GSV locations with GPS-derived wheelchair dwell behavior.

Result: VLM ratings are negatively correlated and distributionally similar with dwell time. Curb ramps and crosswalks are associated with higher VLM scores, while alignment is limited for subtle surface conditions, transient obstructions, and viewpoint-dependent barriers.

Conclusion: Expert-guided VLMs show potential for scalable accessibility assessment, aligning with sensor-derived indicators of real-world wheelchair navigation.
...
TL;DR: AVI-Bench is a cognitively inspired benchmark evaluating Omni-Multimodal Large Language Models across perception, understanding, and reasoning stages using cross-modal audio-visual tasks, plus an extension (AVI-Bench-PriSe) testing primitive sensation with unfamiliar stimuli. Experiments reveal significant limitations in current models and propose a four-level AVI taxonomy.


<details>
  <summary>Details</summary>
Motivation: Current Omni-MLLMs lack systematic evaluation of audio-visual intelligence (AVI), especially regarding joint cross-modal interpretation and robustness beyond common training distributions.

Method: The benchmark is structured across three cognitive stages (perception, understanding, reasoning) with cross-modal tasks. An extension (AVI-Bench-PriSe) uses low-semantic, unfamiliar stimuli to probe primitive audio-visual sensation and generalization.

Result: Experiments on both open-source and closed-source models show substantial limitations in current Omni-MLLMs' audio-visual intelligence, leading to the formulation of a four-level AVI taxonomy.

Conclusion: AVI-Bench provides a principled evaluation framework that can guide the development of more robust and generalizable audio-visual intelligence in future Omni-MLLMs.
...
TL;DR: FineGen is a VLM-based Multi-Agent framework that automatically constructs high-quality hard negative datasets (FineGen-100K) with a 96.7% validity rate, yielding +14.4% accuracy improvement on fine-grained open-vocabulary detection benchmarks.


<details>
  <summary>Details</summary>
Motivation: The scarcity of hard negative samples in current vision-language datasets significantly hinders fine-grained perception in models.

Method: A VLM-based Multi-Agent framework using a collaborative Generation-Verification-Correction pipeline with closed-loop feedback to synthesize hard negatives that are semantically valid but visually contradictory.

Result: Constructed FineGen-100K with 147K+ attribute-specific hard negatives (1:10 positive-to-negative ratio), achieving 96.7% attribute validity rate, and +14.4% accuracy improvement on FG-OVD hard samples.

Conclusion: FineGen effectively addresses the hard negative scarcity problem and significantly outperforms state-of-the-art methods on fine-grained perception tasks.
...
TL;DR: DOME proposes a zero-shot domain encoder using vision-language pretraining to explicitly model sample-specific domain representations, achieving SOTA TTA performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing TTA methods infer a single global domain distribution, ignoring multidimensional and sample-specific nature of real-world domain shifts, leading to fragile adaptation.

Method: DOME uses vision-language pretraining to extract dense continuous representations, parameterizes domains as distributional variables, and employs a momentum-updated sparse domain bank for disentangled supervision.

Result: Achieves SOTA performance on ImageNet-C, ImageNet-R, and ImageNet-Sketch, outperforming complex TTA approaches even with basic entropy-minimization.

Conclusion: Robust adaptation stems not from intricate adaptation algorithms, but from explicit, structured domain representation.
...
TL;DR: TLVS is a token-level, visual-sensitivity-adaptive activation steering method that selectively suppresses hallucinations in LVLMs by applying fine-grained steering only to critical decoding steps.


<details>
  <summary>Details</summary>
Motivation: Existing activation steering methods for hallucination mitigation suffer from: (1) averaging image-versus-no-image differences over the entire sequence, which dilutes critical signals and yields low signal-to-noise ratio; (2) applying fixed steering strength, which over-perturbs non-critical tokens and causes instability.

Method: TLVS extracts token-level steering vectors, refines them, and applies visual-sensitivity-adaptive steering only to hallucination-prone tokens during decoding. It uses a lightweight calibration mechanism.

Result: Evaluated on POPE, AMBER, CHAIR (COCO), MMHal, and HallusionBench, TLVS shows consistent improvements over previous steering methods.

Conclusion: Token-level, adaptive steering effectively mitigates hallucinations in LVLMs without heavy training overhead, improving controllability and stability compared to prior steering approaches.
...
TL;DR: AQIFormer is a transformer-based ensemble model that estimates air quality from front and rear traffic images combined with weather data, achieving 89.96% accuracy (14.96% improvement over SOTA) and strong cross-city generalization with only 8.29% performance drop via few-shot adaptation.


<details>
  <summary>Details</summary>
Motivation: Traditional sensor-based air pollution monitoring faces scalability and economic constraints, while existing image-based methods lack cross-city generalization and effective multi-view exploitation.

Method: Proposes AQIFormer, a transformer-based ensemble architecture with dual-view integration (front+rear images), weather-aware attention mechanisms, and multi-task learning.

Result: Achieves 89.96% accuracy on 26,678 image pairs (14.96% improvement over SOTA), and 81.67% accuracy on cross-city dataset (Nagpur) with only 8.29% performance degradation using few-shot adaptation.

Conclusion: The dual-view transformer ensemble with weather-aware attention significantly improves air quality estimation accuracy and cross-city generalization, demonstrating practical viability for scalable monitoring.
...
TL;DR: ViMax is a multi-agent framework for long-form video generation that uses coordinated specialized agents, a hierarchical narrative engine with retrieval-augmented generation, and dependency-aware visual consistency to maintain narrative coherence and visual fidelity across multi-scene timelines.


<details>
  <summary>Details</summary>
Motivation: Current short-clip video generation methods fail at long-form content because they produce isolated sequences without narrative structure and lack mechanisms for maintaining character and environmental consistency across scenes.

Method: ViMax employs a multi-agent collaboration framework including (1) a hierarchical narrative engine with retrieval-augmented generation for story coherence, (2) a dependency-aware visual consistency mechanism tracking character/environment states across temporal boundaries, and (3) VLM-guided agents that continuously monitor and refine narrative and visual quality.

Result: The framework enables coordinated agent collaboration to generate extended narrative content while maintaining both storytelling integrity and visual coherence across multi-scene timelines.

Conclusion: Multi-agent coordination with hierarchical narrative planning and visual state tracking is a promising approach for addressing the challenges of long-form video generation.
...
TL;DR: Introduces a benchmark to evaluate how well Vision Language Models (VLMs) adapt to dynamic, in-context human preferences, moving beyond static capability evaluations.


<details>
  <summary>Details</summary>
Motivation: Current VLMs are increasingly used in human-interactive settings, but existing benchmarks only test static capabilities and generally-held preferences, not real-time personalization to individual users.

Method: Proposes an automated pipeline to generate a benchmark with variations on image dependence, creating a dynamic multi-modal human-preference dataset for evaluating VLMs on in-context preference understanding.

Result: Includes evaluations of state-of-the-art models on the newly introduced benchmark dataset.

Conclusion: This benchmark addresses a gap in evaluating VLMs' ability to handle dynamic, user-specific preferences presented at inference time, which is crucial for interactive applications.
...
TL;DR: MM-Matryoshka proposes a 2D training framework that makes multi-vector visual document retrieval elastic along both vector dimension and layer depth, enabling a single model to operate at various efficiency-accuracy trade-offs without separate retraining.


<details>
  <summary>Details</summary>
Motivation: Multi-vector visual document retrievers (e.g., ColPali-style) are expensive in storage and computation due to representing each page with multiple vectors from deep VLMs. Existing efficiency methods only optimize part of this budget and lack a unified way to trade accuracy for both vector width and encoder depth.

Method: Proposes MM-Matryoshka, a 2D Matryoshka training framework that enables budget-elastic Visual Document Retrieval (VDR) by making retrieval elastic along both dimension (vector width) and layer (encoder depth). A single retriever can select a 2D budget at inference without training separate models.

Result: Retains significantly higher quality than direct truncation baselines while substantially reducing storage and computational overhead across multiple representative backbones.

Conclusion: MM-Matryoshka offers robust budget elasticity for efficient Visual Document Retrieval, allowing flexible efficiency-accuracy trade-offs with a single trained model.
...
TL;DR: This paper presents an end-to-end pipeline that integrates preoperative MRI, intraoperative ultrasound (ioUS), and synthetic MRI generated from ioUS using a ResViT-2.5D backbone, combined with a two-stage deformable registration, to produce an updated whole-brain MRI volume reflecting the intraoperative post-resection state for improved glioma surgery navigation.


<details>
  <summary>Details</summary>
Motivation: Brain shift after dural opening degrades neuronavigation accuracy during glioma surgery. While intraoperative MRI can compensate, it is expensive and rarely available. ioUS is inexpensive and repeatable but suffers from limited field of view, speckle contrast, and inability to visualize structures like the resection cavity. The goal is to generate an updated whole-brain MRI reflecting the current surgical state without dedicated intraoperative MRI infrastructure.

Method: An end-to-end pipeline combining: (1) a 2.5D residual-transformer synthesis backbone (ResViT-2.5D) that generates synthetic MRI from ioUS, (2) a two-stage deformable registration coupling NiftyReg with a synthesis-anchored SynthMorph stage, merging the preoperative MRI, synthetic MRI, and deformable registration to produce a whole-brain volume in preoperative imaging space that reflects the intraoperative state.

Result: On the post-resection ReMIND cohort, ResViT-2.5D produced synthetic images closely matching intraoperative T2 across structural, intensity, and perceptual metrics. In 14 subjects with 215 expert landmarks, synthesis-anchored registration reduced mean target registration error from 6.27 to 5.86 mm, matching a strong NiftyReg baseline (5.85 mm) while producing diffeomorphic deformation fields in all subjects.

Conclusion: The key contribution is not improved registration accuracy but the integrated volume itself, which reflects the intraoperative post-resection state within the ultrasound field of view. This provides surgeons with an MRI-like update of the operative field, potentially integrable into surgical-navigation workflows to compensate for brain shift without requiring dedicated intraoperative MRI.
...
TL;DR: Industrial-YOLO is an edge-optimized defect detection framework based on YOLOv8, achieving 120+ FPS and 98.5% mAP on resource-constrained hardware via TensorRT/OpenVINO acceleration, validated on steel, MVTec AD, and automotive assembly line datasets.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between deep learning model accuracy and real-time deployment constraints on edge hardware for industrial surface defect detection in high-speed manufacturing.

Method: Fine-tuned YOLOv8 with target-specific optimizations using TensorRT and OpenVINO acceleration engines, benchmarked on NEU surface defect database, MVTec AD dataset, and custom automotive manufacturing extensions.

Result: 120+ FPS inference on NVIDIA Jetson Orin, 98.5% mAP, zero-latency performance on active automotive assembly line.

Conclusion: Industrial-YOLO provides a scalable, robust blueprint for next-generation automated optical inspection systems on edge hardware.
...
TL;DR: Gradient-based fine-tuning of foundation models for forgery detection causes "anchor drift" — harmful feature space perturbations. The authors propose a gradient-free OOD anomaly detection method using frozen foundation models as stable coordinate systems, achieving superior zero-shot performance across domains (face forgeries to Text-to-Image) with edge-deployable efficiency.


<details>
  <summary>Details</summary>
Motivation: Gradient-based fine-tuning of foundation models for forgery detection damages their intrinsic representations due to overfitting on limited samples and erroneous inductive biases, causing "anchor drift" — measurable L2 norm perturbations in high-dimensional feature space that impair zero-shot generalization across unseen domains.

Method: A gradient-free OOD anomaly measurement approach: treats a frozen foundation model as a stable coordinate system, establishes an absolute natural anchor on the real visual manifold by decoupling statistical and semantic deviations via attention-weighted spatial moments and orthogonal projection of perceptual inconsistencies. Uses Sherman-Morrison formula for instantaneous online learning and covariance delta transmission for federated learning.

Result: In extreme zero-shot setting (trained on face forgeries, tested on universal Text-to-Image generations), significantly outperforms gradient-optimized paradigms. Backpropagation-free forward passes and linear solvers enable hardware-agnostic, edge-deployable calibration with minimal latency.

Conclusion: Gradient-free OOD anomaly detection on frozen foundation models is superior to gradient-based fine-tuning for cross-domain forgery detection, offering better generalization, edge-deployability, and privacy-preserving federated collaboration capabilities.
...
TL;DR: PereStruct is an automated pipeline combining fine-tuned YOLO for layout analysis with a semantic assembly module (TF-IDF, visual embeddings, geometric constraints) that achieves state-of-the-art F1=0.904 on block-to-article mapping for historical newspapers, vastly outperforming VLMs (BLEU 0.96 vs 0.34).


<details>
  <summary>Details</summary>
Motivation: Historical documents with complex, non-standard layouts and physical degradation present severe out-of-distribution challenges that confound even state-of-the-art vision-language models, creating a bottleneck in large-scale archival digitization.

Method: A fine-tuned YOLO architecture for layout analysis trained on 1,426 human-annotated pages, plus a novel semantic assembly module that reconstructs articles by jointly modeling TF-IDF lexical-semantic similarity, YOLO visual embeddings, and geometric layout constraints.

Result: F1 score of 0.904 on block-to-article mapping. End-to-end BLEU score of ~0.96 vs VLMs' ~0.34. Released training corpus (599 annotated pages) and PereStruct benchmark (93 pages).

Conclusion: Modular architectures excel where generic VLMs fail on complex historical layouts, providing a robust foundation for high-fidelity digitization and semantic reconstruction of archival materials.
...
TL;DR: MemoVAD is an edge-cloud collaborative framework for video anomaly detection that uses lightweight edge inference with a gating mechanism to query cloud-based VLMs only for uncertain cases, while caching verified semantics for efficient retrieval, achieving SOTA performance with reduced communication overhead.


<details>
  <summary>Details</summary>
Motivation: There is a fundamental tension between the need for high-level semantic understanding (provided by VLMs) for effective video anomaly detection and the limited computational resources of edge devices in real-world surveillance systems.

Method: A lightweight edge detector with a causal Temporal Context Encoder (TCE), an Uncertainty-Aware Gating (UAG) policy based on Subjective Logic to selectively query cloud VLMs, and a Dynamic Semantic Memory (DSM) to cache and retrieve VLM-verified prototypes via a semantic adapter.

Result: Outperforms state-of-the-art methods on UCF-Crime and XD-Violence datasets while substantially reducing communication overhead when deployed on a real edge device.

Conclusion: Edge-cloud collaboration with uncertainty-aware selective VLM querying and dynamic semantic memory can effectively bridge the gap between semantic richness and edge resource constraints for practical VAD deployment.
...
TL;DR: D-3DGS is improved by replacing its MLP deformation field with Liquid Neural Network (CfC) cells, which bake smooth temporal continuity into the architecture itself, achieving competitive or better results on dynamic scene reconstruction, especially for high-frequency motion.


<details>
  <summary>Details</summary>
Motivation: The original D-3DGS uses an MLP that treats each time step independently, producing discrete per-frame offsets without architectural temporal coupling, so smoothness is only an optimization byproduct rather than an inherent property.

Method: Replace the MLP deformation field with a stack of Closed-form Continuous-time (CfC) cells — a liquid neural network offering closed-form solution of the Liquid Time-constant ODE — using a sigmoidal time gate to interpolate between hidden states, while keeping the rest of D-3DGS unchanged.

Result: On 8 D-NeRF and 7 NeRF-DS scenes, the liquid field matches or exceeds MLP baseline performance, with largest gains on scenes containing high-frequency articulated motion.

Conclusion: A simple architectural swap (MLP → CfC cells) turns discrete deformation into explicit continuous-time function of t with near-zero friction, improving temporal coherence.
...
TL;DR: A video-based framework using pose estimation, kinematics, and a foundation model successfully detected multiple hyperkinetic movement disorder phenomenologies across adult and pediatric populations, with external validation showing improved performance after lightweight calibration on clinician-selected subsets.


<details>
  <summary>Details</summary>
Motivation: To develop a framework that can simultaneously detect multiple hyperkinetic movement disorder phenomenologies (dystonia, tremor, myoclonus, chorea, athetosis, ballismus, stereotypies, tics) from routine clinical videos, and explicitly test cross-cohort transfer from adults to children.

Method: Combined markerless pose estimation, kinematic descriptors, and a pretrained foundation model. Trained on 21 adults + 4 controls (standardized protocol), then externally validated on 12 pediatric patients (monogenic combined MDs) without retraining; only final decision layer was calibrated using a small clinician-selected subset.

Result: After calibration on the clinician-selected subset, Hamming accuracy improved from 0.804 to 0.839 and Jaccard index from 0.548 to 0.633 on held-out pediatric patients. For phenotypes with stronger clinician agreement, performance reached Hamming accuracy 0.9 and Jaccard index 0.786.

Conclusion: The framework demonstrates feasible cross-cohort (adult-to-pediatric) transfer for multi-phenomenology MD detection, with lightweight calibration effectively improving performance without full retraining, and gains are robust (not reliant on unreliable labels).
...
TL;DR: This paper investigates what pretraining signals lead to action-relevant latent structures in video world models, finding that temporal video pretraining—not pixel reconstruction quality—is the key driver. Video-pretrained self-supervised encoders achieve the best balance between visual fidelity and action prediction, and inverse-dynamics supervision improves robustness to visual corruption.


<details>
  <summary>Details</summary>
Motivation: Video world models are widely used for predictive visual representations, but it remains unclear which pretraining signals (e.g., image vs. video, reconstruction vs. latent prediction) actually induce action-relevant structure in their latent spaces. The authors aim to isolate the key ingredients.

Method: The authors conduct a unified probe-based evaluation across diverse encoder families: image-only self-supervision, video pretraining with/without latent prediction, reconstruction-based autoencoders, diffusion models, and shortcut-forcing dynamics models. They use a common inverse-dynamics probing objective to measure action-relevant structure.

Result: Action-relevant structure is driven by temporal video pretraining, not pixel reconstruction fidelity. Models with strong pixel decoding can have near-zero action recoverability. V-JEPA vs. VideoMAE comparison shows most gains come from natural-video temporal context, with latent prediction adding smaller benefit. Trends transfer to robotic benchmarks, though static environments (CALVIN) can mask temporal importance. Inverse-dynamics supervision improves robustness to visual corruption.

Conclusion: Temporal predictive structure—not reconstruction fidelity—is the primary ingredient underlying action-relevant video representations. Action-aware objectives help regularize latent geometry beyond clean-setting performance.
...
TL;DR: Struct-Searcher is a plug-and-play, model-agnostic deep research agent that uses a structural multimodal graph grounded in belief revision theory to handle contradictory information, achieving significant accuracy improvements over existing methods across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing agentic workflows rely on evidence accumulation models that linearly aggregate evidence and lack principled mechanisms for handling contradictory information across heterogeneous modalities.

Method: Struct-Searcher proposes a structural agentic workflow grounded in belief revision theory, which explicitly maintains an evolving multimodal structural graph throughout the reasoning process for conflict-aware multimodal deep information seeking.

Result: Achieves 17.2% average relative accuracy improvement on BrowseComp-VL across five backbones; outperforms state-of-the-art VLMs and deep research agents with 3.7% on MM-BrowseComp, 1.5% on HLE-VL, and 0.7% on BrowseComp-VL over the second-best approach.

Conclusion: Struct-Searcher is plug-and-play, model-agnostic, and top-performing for multimodal deep information seeking, effectively handling contradictory multimodal information.
...
TL;DR: This paper introduces a cross-view urban traffic benchmark with synchronized bicycle and drone videos at intersections, supporting identity matching and BEV prediction tasks with standardized evaluation.


<details>
  <summary>Details</summary>
Motivation: Intersection-centric traffic analysis requires joint reasoning about identity preservation, local interactions, and global spatial structure across radically different viewpoints (street-view vs. drone aerial view), which existing datasets do not adequately support.

Method: The authors build a dataset of synchronized ego-centric bicycle and aerial drone videos at real urban intersections. They define two tasks: (1) cross-view identity matching between street-view and drone-view tracks, (2) ego-to-BEV prediction using aerial supervision. They provide standardized evaluation metrics, annotation tooling, and baseline implementations including wedge-based matching and three BEV prediction baselines (IPM, MonoLayout-style, regression).

Result: Cross-view matching achieves strong recall but suffers from over-assignment and temporal inconsistency. Ego-to-BEV prediction benefits from aerial supervision but remains far from saturated under lightweight monocular sensing.

Conclusion: The benchmark is feasible yet challenging, and the authors hope it will support future research on cross-view perception, urban scene alignment, and ego-to-global traffic understanding.
...
TL;DR: This paper studies monocular drone distance estimation using image crops and bounding-box geometry, introducing DroneDAR (a CNN with gating mechanism) and analyzing how backbone capacity, crop resolution, and loss functions affect performance, especially for long-range, small-target scenarios.


<details>
  <summary>Details</summary>
Motivation: Accurate distance estimation for small drones in long-range imagery is important for tracking and situational awareness, yet remains challenging due to extreme target scale variation, background clutter, and noisy visual cues.

Method: A new DroneDAR model that combines a convolutional backbone with explicit bounding-box cues through a lightweight gating mechanism, evaluated against a Droneranger-style baseline.

Result: Experiments analyze how backbone capacity, crop resolution, and regression loss functions affect performance across distance regimes, and examine common failure modes at long distances (bounding-box noise, reduced texture detail).

Conclusion: Provides guidance for designing and training range estimators robust under real-world long-range conditions, highlighting directions for improving reliability when drones occupy only a few pixels.
...
TL;DR: A quantum-classical hybrid pipeline for polarimetric material classification uses SWAP-test circuits to compare 32D embeddings of polarized light reflections, achieving competitive accuracy on 23 materials with open-set discrimination potential for NISQ devices.


<details>
  <summary>Details</summary>
Motivation: To leverage quantum computing for material recognition by casting polarimetric classification as a point-matching problem, aiming to exploit quantum parallelism for similarity estimation in high-dimensional embedding spaces.

Method: Train an encoder on voxel cubes of polarized light reflections to produce 32D embeddings; at inference, encode embeddings as quantum state amplitudes, then use a SWAP-test circuit to estimate fidelity between query and anchor embeddings; aggregated fidelity determines material class via highest similarity.

Result: Evaluated on 23 materials (~800 samples each) from Mueller matrices; compared against Optimal Transport classifier; demonstrated competitive classification accuracy and open-set discrimination capability.

Conclusion: The proposed quantum SWAP-test approach is viable for NISQ-based material recognition, offering competitive accuracy with the added benefit of open-set discrimination.
...
TL;DR: DALE-CT is a 2D slice-based vision encoder for CT trained from scratch using LeJEPA with 3D depth-aware pre-training (dual supervision from anatomical masks and abnormality annotations). It achieves near-parity with SOTA 3D models in multi-abnormality detection (Macro AUROC 0.833) without text supervision.


<details>
  <summary>Details</summary>
Motivation: Demand for adaptable, high-capacity vision encoders in CT; desire for flexible 2D slice-based alternatives to native 3D models; need for strong self-supervised learning in medical imaging without reliance on textual supervision.

Method: Trained DALE-CT using LeJEPA on CT-RATE dataset from scratch; developed a 3D depth-aware pre-training strategy with dense auxiliary supervision from automated anatomical masks and human-annotated abnormalities; evaluated via linear probing with Multiple Instance Learning (MIL).

Result: DALE-CT-2S achieves Macro AUROC of 0.833 under linear probe evaluation; near-parity with state-of-the-art 3D vision-language models using significantly less data and no text supervision.

Conclusion: 2D slice-based architectures with depth-aware SSL can rival 3D models for CT analysis; dual-supervision strategy improves representation quality; all code and weights are publicly released for reproducibility.
...
TL;DR: VLMs have poor fine-grained visual perception—perception saturates at ~12px, reasoning remains flawed even at larger scales.


<details>
  <summary>Details</summary>
Motivation: To systematically probe how small visual patterns VLMs can reliably perceive, extending the "Strawberry" question to vision.

Method: Introduces FineSightBench, a benchmark separating perception (pixel-level recognition) from reasoning (spatial reasoning, counting, ordering) across controlled scales of 4–48px.

Result: Perception saturates around 12px; reasoning remains limited even at larger scales with persistent numeracy and sequence errors.

Conclusion: VLMs have fundamental deficiencies in fine-scale visual reasoning, requiring more rigorous evaluation.
...
TL;DR: VisualFLIP is a paired benchmark (1,374 images) that tests whether multimodal LLMs truly use visual evidence for reasoning by creating perturbation pairs where the correct answer flips. Evaluations of 24 models reveal that correct answers often coexist with flawed visual grounding, measured via pair accuracy and Collapse Rate.


<details>
  <summary>Details</summary>
Motivation: Accuracy alone is insufficient for testing visual grounding in multimodal LLMs, as correct answers can arise from flawed reasoning. The authors aim to detect whether models actually depend on task-critical visual evidence.

Method: The authors create VisualFLIP, a benchmark of 1,374 images organized into same-question perturbation pairs (cardinality, attribute, spatial, logic). Each pair minimally changes visual evidence so the gold answer flips. They evaluate 24 MLLMs using pair accuracy (solving both sides) and Collapse Rate (repeating the same answer despite visual changes).

Result: Paired correctness and evidence dependence are related but distinct; capable models can fail to update after task-critical visual changes, and collapse worsens in sequential settings when the edited image follows an earlier answer.

Conclusion: VisualFLIP effectively reveals visual grounding failures that accuracy alone misses, highlighting the need for evidence-dependent evaluation of multimodal reasoning.
...
TL;DR: Thirteen diverse vision encoders share the same 16-dimensional geometric substrate of top principal directions, which emerges early in training, transfers across visual domains, and enables four practical applications while being modality-specific and not predictive of transfer accuracy.


<details>
  <summary>Details</summary>
Motivation: To investigate whether different vision neural networks (trained with fundamentally different objectives like classification, contrastive learning, reconstruction, or image-text matching) develop distinct internal representations as one would expect.

Method: Applied PCA, centred kernel alignment (CKA), and Pang 2026 calibration to analyze the top 16 principal directions across 13 modern vision encoders; tested across up to 8 visual domains and validated against baselines (pixel statistics, Gabor features, random projections).

Result: All 13 encoders converge to the same 16-dimensional subspace (cross-architecture substrate); it transfers well across domains (median Procrustes-CKA 0.679 for 4 domains, 0.604 for 8); emerges in first 10% of training; enables label-free transferability filter (3x faster, +0.15 Kendall-tau vs LogME), 99.6% domain detection, superior few-shot probe, and teacher-free distillation (7.56pp peak gain).

Conclusion: A universal low-dimensional geometric structure exists across diverse vision encoders despite different training objectives; it is useful for several applications but does not cross modalities, help cross-paradigm distillation, or predict transfer quality.
...
TL;DR: C3VD-DEFCOL is a new dataset and framework for evaluating deformable 3D reconstruction in colonoscopy, featuring simulated peristaltic deformations with realistic textures, enabling controlled stress testing of algorithms.


<details>
  <summary>Details</summary>
Motivation: Current datasets lack both realistic in vivo appearance and dense, time-resolved 3D ground truth under non-rigid deformation, limiting algorithm development for 3D reconstruction in colonoscopy to improve mucosal coverage during screening.

Method: Starting from C3VD colon meshes, the authors generate controlled deformations (peristaltic waves, centerline motion), render per-frame geometry (depth, normals, optical flow, poses, 3D meshes), and use an LTX-2.3-based sim-to-real translation model conditioned on depth to produce RGB clips with in vivo-like appearance while preserving 3D structure.

Result: The dataset includes 110 videos from 11 colon mesh geometries with varying trajectories, appearances, and three peristaltic severity levels. Evaluations confirm appearance realism, geometric and temporal consistency. Pose estimation benchmarks show error increases with deformation severity, demonstrating controlled stress testing impossible with in vivo datasets.

Conclusion: C3VD-DEFCOL provides a reproducible, quantitative evaluation platform for deformable 3D reconstruction in colonoscopy, helping bridge the domain gap between synthetic data and real in vivo colonoscopy.
...
TL;DR: TBD-VLA introduces block diffusion for discrete token-based Vision-Language-Action models, partitioning action sequences into temporal blocks with masked diffusion within blocks and autoregressive generation across blocks, achieving faster inference and better temporal coherence than prior VLA approaches.


<details>
  <summary>Details</summary>
Motivation: Existing discrete VLA models suffer from high inference latency due to autoregressive token prediction and ignore temporal structure in action trajectories. Prior parallel decoding methods improve speed but lack explicit token dependency modeling.

Method: TBD-VLA partitions action sequences into temporal blocks, applies masked discrete diffusion within each block for parallel decoding, and maintains autoregressive generation across blocks, unifying temporal coherence with efficient inference.

Result: TBD-VLA significantly outperforms prior VLA approaches in both simulation and real-world manipulation tasks, with improved inference speed and temporal coherence.

Conclusion: The proposed block diffusion framework offers a scalable path toward fast, temporally aware, discrete VLA models, and enables asynchronous execution like Real-Time Chunking via temporal in-painting.
...
TL;DR: Previous 3D intraoral reconstruction model (77.49% accuracy) had vertex clustering issues. This paper improves with Hungarian matching filtering and Repulsion Loss, achieving 68.02% accuracy but with much more uniform vertex distribution.


<details>
  <summary>Details</summary>
Motivation: To address the vertex clustering problem in previous 3D intraoral reconstruction, where predicted vertices concentrated in high-density regions leaving other areas uncovered.

Method: Improved loss function using Hungarian matching with filtering and Repulsion Loss to enforce uniform vertex distribution across the reconstructed model.

Result: Accuracy dropped from 77.49% to 68.02%, but vertex distribution became substantially more uniform across the reconstructed surface.

Conclusion: The proposed method sacrifices numerical accuracy for significantly better spatial coverage, effectively solving the vertex clustering problem from prior work.
...
TL;DR: A training-free, two-stage cascaded Video RAG pipeline for cross-lingual long-video comprehension with strict persona adherence and zero-hallucination temporal grounding, using dense retrieval on clean visual/text summaries followed by LLM-based cognitive reranking and Prompt Sculpting for JSON output.


<details>
  <summary>Details</summary>
Motivation: To address critical challenges in cross-lingual long-video comprehension: strict persona adherence and zero-hallucination temporal grounding, while maintaining resource efficiency by avoiding training.

Method: Two-stage cascaded Video RAG pipeline: (1) High-recall semantic pre-fetching via dense retrieval on high-fidelity visual summaries and global text descriptions; (2) Adaptive, Iterative, and Reasoning-based (A.I.R.) filtering agent using a commercial LLM for cognitive reranking with full multimodal context; plus Prompt Sculpting for structured JSON output.

Result: Evaluated on the MAGMaR RAG track, the approach shows exceptional precision in both information retrieval and persona-conditioned generation.

Conclusion: The resource-aware, training-free architecture effectively decouples semantic retrieval from logical reasoning, achieving strong performance on cross-lingual video QA with hallucination-free grounding.
...
TL;DR: LEGS improves 3D Gaussian Splatting by replacing first-order gradient guidance with second-order Laplacian structural guidance and nonlinear weighting, achieving up to 1.68 dB PSNR improvement over 3DGS and 0.52 dB over EGGS on standard benchmarks.


<details>
  <summary>Details</summary>
Motivation: Standard 3DGS uses photometric loss that treats flat and structure-rich regions similarly, limiting recovery of sharp contours and fine details. EGGS improves this but relies on first-order gradient responses and linear weighting, which may not capture structural information optimally.

Method: LEGS replaces first-order gradient guidance with second-order Laplacian structural guidance and maps the normalized Laplacian response into pixel-wise weights through nonlinear response-to-weight functions, while keeping the original 3DGS rendering pipeline unchanged.

Result: On Tanks&Temples and Mip-NeRF360 datasets, LEGS improves PSNR by up to 1.68 dB over 3DGS and 0.52 dB over EGGS. When applied to FastGS and FasterGS, it further improves PSNR by up to 1.69 dB.

Conclusion: LEGS provides an effective general loss-level extension for Gaussian Splatting pipelines with potential applications in AR/VR, immersive visualization, and real-time 3D content generation.
...
TL;DR: The REACT 2026 challenge focuses on generating multiple appropriate, personalized, diverse, realistic, and synchronized facial reactions from a listener in dyadic interactions, expanding on previous challenges by incorporating Big-Five personality labels and EEG data alongside the MARS dataset.


<details>
  <summary>Details</summary>
Motivation: To advance the under-explored problem of one-to-many personalised facial reaction generation in dyadic interactions by introducing new multimodal signals (personality & EEG) and benchmarking ML models.

Method: The challenge proposes four sub-challenges: Offline/Online generic and Offline/Online personalised MAFRG, using the MARS dataset augmented with Big-Five personality labels and EEG recordings. Baselines and guidelines are publicly released.

Result: New baselines are established for the four sub-challenges, providing benchmarks for future model development.

Conclusion: REACT 2026 pushes forward dyadic interaction modelling by integrating behavioural, affective, and neurophysiological signals for personalised facial reaction generation, offering a new challenge setting and baselines.
...
TL;DR: DAL-PCQA is a distortion-aware, language-annotated dataset for Point Cloud Quality Assessment that augments benchmark point clouds with multi-level distortion severity labels, quality categories, and natural language descriptions, enabling interpretable and explainable quality assessment beyond scalar MOS scores.


<details>
  <summary>Details</summary>
Motivation: Existing PCQA methods only predict scalar MOS scores, failing to reveal the causes of perceptual degradation, whereas humans naturally reason in terms of specific distortions (blur, color shifts, missing regions, etc.).

Method: The paper introduces DAL-PCQA, which augments benchmark point clouds with multi-level distortion severity labels, discrete quality categories, and structured natural language descriptions. It defines a point-cloud-specific distortion taxonomy covering both photometric and geometric artifacts, and compares zero-shot vs. fine-tuned multimodal models for generating perceptual quality descriptions.

Result: Statistical analysis reveals characteristic degradation patterns across distortion types and quality levels. Distortion-aware supervision substantially improves lexical and semantic alignment with ground-truth descriptions in multimodal models.

Conclusion: DAL-PCQA enables interpretable, distortion-level reasoning and facilitates language-driven, explainable point cloud quality assessment.
...
TL;DR: The paper introduces ChronoPhyBench, a multimodal benchmark that tests whether MLLMs truly perform physically grounded reasoning or just exploit language biases. It uses video+text inputs with next-state prediction tasks (single image selection and multi-frame sorting). Evaluations show open-source models struggle with genuine multimodal physical reasoning.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs show impressive reasoning abilities, but it's unclear if they genuinely integrate cross-modal information or rely on language priors to fake multimodal understanding. This paper aims to rigorously detect and mitigate language modality bias.

Method: Proposes ChronoPhyBench, a benchmark combining next-state prediction with VQA using video history and textual captions. Includes two tasks: single image selection and multi-frame chronological sorting. Also constructs a large dataset of 10,000+ long-form videos with 5M tokens of annotated captions.

Result: Open-source MLLMs perform poorly on physically grounded multimodal reasoning, contrary to prior benchmarks' conclusions. Their capacity for genuine multimodal physical reasoning is still in its infancy.

Conclusion: This work provides a robust evaluation framework to stress-test MLLM reasoning, quantify hallucination rates, and advance Physical AI toward AGI.
...
TL;DR: DisCo introduces discrete action primitives for controllable video generation, solving action entanglement issues in continuous camera trajectory-based models, with a new benchmark (DisCoBench) for evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing controllable video world models use continuous camera trajectories as action conditions, leading to unreliable action following due to action representation entanglement (high feature similarity across distinct motion patterns).

Method: Proposes DisCo, a controllable video world model that conditions generation on a compact set of discrete action primitives instead of continuous trajectories, improving action separability. Also introduces DisCoBench for comprehensive evaluation.

Result: DisCo achieves significantly more reliable action following while preserving visual quality, validated on short-term, long-horizon, and highly dynamic exploration scenarios.

Conclusion: Discrete action representations effectively address the action entanglement bottleneck, enabling better controllability in video world models for interactive world exploration.
...
TL;DR: FMRFusion is a frequency-aware multi-view representation learning network for infrared and visible image fusion that uses multi-scale structural perception, bilinear frequency decomposition, cross-view complementary interaction, and flow matching to achieve superior fusion performance, especially in nighttime scenarios.


<details>
  <summary>Details</summary>
Motivation: Previous single-module stacking approaches incompletely learn distinct characteristics of two modalities, limiting fusion effectiveness and robustness in real-world heterogeneous data scenarios.

Method: 1) Multi-Scale Structural Perception Module for capturing discriminative structures; 2) Bilinear frequency decomposition to separate high/low-frequency features; 3) Cross-View Complementary Interaction for modeling complementary characteristics between reflected light and radiative intensity; 4) Flow matching for progressively refining fused features.

Result: Superior and consistent performance across multiple benchmark datasets, especially in nighttime scenarios.

Conclusion: FMRFusion effectively addresses heterogeneous data challenges and achieves state-of-the-art fusion performance.
...
TL;DR: SCN enhances open-vocabulary semantic segmentation by explicitly modeling inter-class semantic correlations via Class Disambiguation and Logits Fusion modules, achieving better discriminative power while preserving CLIP's generalization.


<details>
  <summary>Details</summary>
Motivation: Prior OVS methods focus on feature aggregation or simple fine-tuning of pre-trained models, which may not adequately handle semantic ambiguities between classes or maintain the generalization ability of pre-trained CLIP models.

Method: SCN consists of two components: (1) Class Disambiguation (CD) - uses cross-attention to create visually-aware pseudo-text embeddings and a residual architecture to capture inter-class dependencies; (2) Logits Fusion (LF) - dynamically integrates multiple semantic evidence sources to achieve robust consensus while preserving CLIP's generalization.

Result: Comprehensive experiments on mainstream benchmarks show significant performance improvements over state-of-the-art OVS algorithms.

Conclusion: Explicitly modeling semantic correlations between classes with calibration mechanisms effectively enhances discriminative power and preserves generalization for open-vocabulary segmentation.
...
TL;DR: LUSIS-DETR is a lightweight detection-transformer for underwater instance segmentation that uses AquaBSAM (boundary-saliency attention) to embed underwater-specific cues into DINOv2 features, achieving competitive performance on four datasets with real-time inference (4.31-6.34ms on T4 GPU).


<details>
  <summary>Details</summary>
Motivation: Existing prompt-based and auxiliary-modality methods improve mask quality but rely on large foundation models, prompt generation, or extra modality estimation, complicating efficient deployment for underwater tasks like marine resource exploration.

Method: Proposes LUSIS-DETR with AquaBSAM that embeds underwater boundary, contrast, attenuation, chroma, dark-channel, and center-prior cues into DINOv2-initialized multi-scale features via bounded residual modulation, plus training-only auxiliary mask supervision and small-object copy-paste.

Result: Competitively leading performance on UIIS, UIIS10K, USIS10K, and USIS16K datasets compared to prior state-of-the-art, with 4.31-6.34ms latency (FP16) on NVIDIA T4 GPU for real-time inference.

Conclusion: LUSIS-DETR achieves efficient, real-time underwater instance segmentation without reliance on large foundation models or extra modalities, making it suitable for accessible deployment.
...
TL;DR: GVC-Seg is a training-free 3D instance segmentation method that mitigates confidence bias across foundation models by leveraging geometric-visual correspondence, achieving SOTA performance on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D instance segmentation methods that aggregate multiple pre-trained models suffer from confidence bias favoring higher-confidence models, leading to sub-optimal results. This bias is model-dependent and affected by preprocessing and training strategies.

Method: Proposes GVC-Seg using Geometric Visual Correspondence to align 3D geometric cues with 2D visual cues, reducing confidence bias. Includes a 3D proposal generation module and a mask-aware CLIP feature extraction module for mask generation and semantic reasoning.

Result: Achieves state-of-the-art performance on several challenging 3D instance segmentation benchmarks, and shows strong potential in open-vocabulary semantic segmentation.

Conclusion: GVC-Seg effectively addresses confidence bias in multi-model ensemble 3D segmentation through geometric-visual correspondence, delivering superior results without requiring additional training.
...
TL;DR: IEA is a conversational Image Editing Agent that uses 16 interpretable parameterized tools for transparent, step-by-step editing. It's trained via a three-stage pipeline (SFT + GRPO + synthetic fine-tuning) and outperforms baselines in pixel distance, ROUGE-L, instruction following, and perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Current image editing software requires expert tuning or fixed filters, and generative models produce artifacts, implausible details, or stylistic drift without explainability. There's a gap between amateur users' intent and outcomes.

Method: Three-stage multitask pipeline: (1) SFT on distilled expert edits, (2) GRPO with rewards for likeness, tool usefulness, and intent summarization, (3) large-scale synthetic fine-tuning. Uses 16 interpretable parameterized tools for step-by-step editing with transparent edit traces.

Result: Lower pixel distance on edit task, higher ROUGE-L on summary task vs. baselines. Best instruction-following among tool-calling methods and best perceptual quality among generative methods in user studies.

Conclusion: Interpretable, tool-centric VLMs are a reliable path to human instruction-guided image retouching.
...
TL;DR: This paper uses bistable illusions (like Wittgenstein's duck-rabbit) to probe when and where vision-language models commit to one interpretation of an ambiguous image. Using behavioral experiments and sparse autoencoders on CLIP representations, the authors find that both interpretations are often simultaneously represented in the vision tower, but the final caption commitment happens downstream. Causal steering can flip default-dominant stimuli but fails on force-balanced ones, revealing a "dominance bottleneck" that separates visual representation (seeing) from linguistic commitment (seeing-as).


<details>
  <summary>Details</summary>
Motivation: To understand where in a vision-language model the commitment to one interpretation of an ambiguous image occurs, and to operationalize the philosophical distinction between "seeing" (visual representation) and "seeing-as" (linguistic commitment) through empirical methods.

Method: 1) Behavioral baseline with 3,320 generations over 83 bistable stimuli under neutral vs forced-choice prompting. 2) Training a TopK sparse autoencoder on CLIP layer representations consumed by LLaVA-1.6-7B (validation EV 0.93). 3) Analyzing feature pool activation across 69 bistable stimuli. 4) Causal steering at CLIP layer 22 to flip caption outputs under fluency constraints.

Result: Three regimes identified: default-dominant, force-dominant, force-balanced. 72% of bistable stimuli show simultaneous activation of both aspect feature pools at the vision tower. Causal steering flips captions on default-dominant stimuli (33% rabbit-flip rate) but fails on force-balanced young/old. Rank-based statistics on TopK SAE outputs require tie-corrected ranking to avoid silent row-order bias.

Conclusion: The dominance bottleneck lives downstream of the vision tower. The gap between vision-side superposition and language-side commitment provides an empirical handle on the seeing/seeing-as distinction. Methodological note: rank-based statistics on TopK SAE outputs require tie-corrected ranking.
...
TL;DR: This paper explores using synthetic crack data to complement real data for training CNNs to detect cracks in masonry buildings, showing that 20% real + 80% synthetic data achieves comparable or better results than real-only training.


<details>
  <summary>Details</summary>
Motivation: Crack detection is critical for building health monitoring, but collecting large, diverse real-world datasets of masonry cracks is time-consuming and challenging. Public datasets are often inadequate, motivating the need for synthetic data generation to reduce collection efforts.

Method: The authors generated synthetic crack data using a crack overlay tool with controlled orientation and placement. They trained multiple DL architectures on real masonry crack images from Bologna, selected InceptionV4 as the best model, then tested six training scenarios with varying ratios of real and synthetic data, evaluating using F1-score and mIoU.

Result: Training on synthetic data with only 20% real data achieved results comparable to real-only training. The 20/80 (synthetic/real) scenario achieved 76% F1-score and 80% mean IoU, outperforming the real-only case.

Conclusion: Synthetic data can effectively reduce real data collection efforts while maintaining or even improving crack detection accuracy in masonry structures.
...
TL;DR: Sci-Rho is a dynamic, multilingual, visually-grounded STEM benchmark with 42,420 instances across 5 subjects and 7 languages, revealing that state-of-the-art VLMs show significant gaps between average and worst-case accuracy, with smaller models exhibiting cross-lingual performance degradation.


<details>
  <summary>Details</summary>
Motivation: Existing symbolic benchmarks are limited to mathematical reasoning, lack visual grounding, and are predominantly English-only. The authors aimed to create a more comprehensive benchmark to assess model robustness under minor modifications across STEM disciplines and languages.

Method: Domain experts (including Olympiad medalists) crafted 4,242 problem templates (606 per language) across 5 STEM subjects and 7 languages. Templates are executable Python code generating diverse instances by varying numerical values, visual patterns, geometric shapes, color schemes, and function types. 17 state-of-the-art VLMs were evaluated.

Result: Found a noticeable gap between worst-case accuracy (proportion of templates answered correctly across all variations) and average accuracy. Smaller models show performance degradation across languages while proprietary/larger models remain robust. Step-level evaluation shows similar gap between average F1 and worst-case F1. Attention head analysis reveals cross-lingual variation in relative attention to image vs. text tokens.

Conclusion: The work highlights the importance of evaluation beyond static benchmarks as a metric to measure the quality of VLMs.
...
TL;DR: DyCo-RL improves visual reasoning in MLLMs by integrating dynamic cross-modal coordination into RLVR optimization, using Fisher-Rao geodesic distance to measure attention shifts and alignment-guided advantage reweighting, consistently boosting four RLVR algorithms across seven benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing RLVR methods only optimize for reasoning outcomes, ignoring the fine-grained cross-modal coordination (alternating between visual evidence extraction and textual context synthesis) during the CoT generation process, which is causally linked to reasoning failures.

Method: DyCo-RL uses Fisher-Rao geodesic distance to measure within-modality attention shifts, assigns tokens to visually-oriented or text-oriented roles, evaluates token-level attention-role alignment, and uses this score for alignment-guided advantage reweighting during policy optimization.

Result: Applied to Qwen2.5-VL-3B/7B, DyCo-RL consistently improves four representative RLVR algorithms across seven benchmarks spanning visual-centric and mathematical reasoning tasks.

Conclusion: Addressing cross-modal coordination at the token level during RLVR optimization significantly enhances visual reasoning in MLLMs, and the approach is algorithm-agnostic, generalizing across different RLVR methods.
...
TL;DR: Robust-U1 is a framework that equips Multimodal Large Language Models with explicit visual self-recovery capability to handle real-world and adversarial visual corruptions, using supervised fine-tuning, reinforcement learning with dual rewards, and multimodal reasoning.


<details>
  <summary>Details</summary>
Motivation: MLLMs degrade significantly under real-world visual corruptions, and existing robustness methods are limited—black-box alignment lacks interpretability while white-box reasoning cannot restore lost pixel-level details.

Method: Three-stage framework: (1) supervised fine-tuning for initial reconstruction, (2) reinforcement learning with dual rewards (pixel-level SSIM + semantic-level CLIP similarity) for high-quality visual alignment, (3) multimodal reasoning jointly using corrupted input and recovered image.

Result: Achieves state-of-the-art robustness on real-world corruption benchmarks and maintains superior performance under adversarial corruptions on general VQA benchmarks.

Conclusion: High-quality visual recovery directly enhances reasoning performance, establishing self-recovery as a critical mechanism for robust visual understanding in MLLMs.
...
TL;DR: VideoWeaver is a benchmark and agent harness for long video generation that lets agents compose foundation skills into custom workflows, features 285 test cases across 16 categories, uses an agent-as-judge for evaluating both process and output, and includes a skill evolution algorithm. Results show explicit composition skills outperform foundation skills alone, and the judge aligns well with human evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing agent frameworks excel at tool use but are underexplored for long-horizon multimodal tasks like long video generation. Previous video agents rely on handcrafted pipelines, lacking the ability to build and refine their own workflows adaptively.

Method: VideoWeaver provides a harness and benchmark with 16 task categories and 285 cases. It introduces an agent-as-judge that inspects both execution trace and final video using metadata and intermediate files. A skill evolution algorithm refines and merges agent skills based on this feedback.

Result: Explicit composition skills improve generation over foundation skills alone; skill evolution further enhances output quality; performance varies across harness and model choices; the agent-as-judge aligns well with human judgments, especially on process metrics.

Conclusion: Agent frameworks with self-composed workflows outperform fixed pipelines for long video generation. The agent-as-judge and skill evolution are effective. Open-source code and dataset are released for further research.
...
TL;DR: This paper introduces a predicate-level reliability framework for manipulation understanding under visual degradation (blur, occlusion, illumination change, low resolution, frame dropping, detection noise). The framework defines a structured predicate vocabulary with confidence-aware estimation and multiple reliability metrics. Experiments show that predicate failures are structured: static spatial predicates are robust, while contact-sensitive and dynamic predicates (grasp, release) are fragile. Degradation reduces manipulation accuracy from 0.89 to 0.58.


<details>
  <summary>Details</summary>
Motivation: Although visual predicates (contact, support, containment, grasp, release, etc.) are widely used in event-chain, graph-based, and neuro-symbolic models for manipulation understanding, their reliability under visual degradation has rarely been analyzed directly.

Method: The paper proposes a predicate-level reliability framework with: (1) structured predicate vocabulary, (2) confidence-aware predicate estimation, and (3) reliability metrics including predicate preservation, degradation sensitivity, temporal consistency, confidence-weighted stability, and downstream impact.

Result: Static spatial predicates remain robust, while contact-sensitive, dynamic, and derived predicates (grasp, release) are fragile. Detection noise, occlusion, and frame dropping cause strongest reliability losses. Degraded predicates reduce manipulation accuracy from 0.89 to 0.58; removing confidence weighting under moderate degradation reduces accuracy from 0.74 to 0.64.

Conclusion: Predicate reliability provides a diagnostic layer between visual perception and structured manipulation reasoning, showing that failures are structured rather than uniform.
...
TL;DR: The paper proposes a Human-Centered Benchmarking Framework (HCBF) evaluating models across accuracy, explainability, efficiency, and robustness. Applied to four lightweight architectures on eye-state classification, ShuffleNetV2 ranks first overall but fails under sensor noise, while the transformer remains robust, showing aggregate rankings can mask critical vulnerabilities.


<details>
  <summary>Details</summary>
Motivation: Current driver monitoring systems are evaluated primarily on classification accuracy, which is insufficient for determining real-world deployment fitness. The paper argues for a multi-dimensional human-centered evaluation approach.

Method: Proposes HCBF evaluating models across four dimensions: accuracy, explainability, efficiency, and robustness. Applied to MobileNetV3, ShuffleNetV2, EfficientNet-B0, and DeiT-Tiny on the MRL Eye Dataset. A Human-Centered Score is computed under three deployment-oriented weighting scenarios.

Result: Models are nearly indistinguishable on clean accuracy but each leads in exactly one dimension, all on Pareto frontier. ShuffleNetV2 ranks first under all weighting scenarios but retains less than half its performance under sensor noise, misclassifying closed eyes as open, while the transformer remains robust.

Conclusion: Aggregate ranking can mask dimension-specific vulnerabilities that are operationally decisive, highlighting the need for multi-dimensional, human-centered evaluation in safety-critical applications.
...
TL;DR: OSTB is a training-free framework that uses self-adaptive optimal transport to jointly solve model selection, target adaptation, and prediction ensembling for multiple VLMs in cross-domain deployment without target labels.


<details>
  <summary>Details</summary>
Motivation: Single-backbone VLM deployment assumes the chosen model is compatible with the target domain, which is unrealistic when multiple general-purpose and domain-specialized VLMs are plausible but no target labels exist to identify reliable ones.

Method: A training-free framework (OSTB) that estimates a consensus sample-to-class transport plan from multiple frozen VLMs via self-adaptive optimal transport, then reuses this structure for model selection (ranking reliability), target adaptation (transport-conditioned classifiers), and ensembling (reliability-aware probabilistic integration).

Result: Extensive experiments on natural-image, remote-sensing, and medical-pathology benchmarks show improvements in model ranking, adaptation stability, and ensemble robustness under heterogeneous candidate pools.

Conclusion: The three deployment decisions (model selection, adaptation, ensembling) share a common latent structure (sample-class relationships), and solving for it jointly via optimal transport is more effective than treating them independently.
...
TL;DR: Vision Transformers suffer from phase-dependent instability due to fixed patch grids. The authors propose Phase Marginalization, a post-hoc method that aggregates predictions across multiple patch-grid phases (K=4) to improve dense prediction tasks like segmentation, depth, and matching, outperforming standard test-time augmentation.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers use fixed patch grids, causing instability in dense predictions when the patch partition changes (phase-dependent boundary effects). The authors formalize patch-grid phase as a nuisance variable that should be marginalized.

Method: Phase Marginalization evaluates multiple structured patch-grid phases (K=4 as the central variant), inverse-aligns dense outputs, and aggregates them in the original image coordinate system. This is a training-free, post-hoc method.

Result: Uniform Phase Marginalization (K=4) outperforms the canonical K=1 baseline across segmentation, depth, and local matching. On Cityscapes, it achieves +0.31 mIoU over the strongest generic shift-based four-forward TTA. K=8 shows negligible change, K=16 adds little accuracy at much higher latency.

Conclusion: Patch-grid phase is a measurable nuisance variable in dense ViT prediction. Phase Marginalization serves as a simple, effective diagnostic and post-hoc marginalization baseline.
...
TL;DR: GraCE uses center of gravity and body mass distribution to estimate ground contact forces and pressure distribution from motion capture data, outperforming prior methods on two benchmarks.


<details>
  <summary>Details</summary>
Motivation: Ground contact forces are essential for biomechanics and sports analysis, but existing methods require force plates/pressure mats (limited to lab settings) or crude distance-based estimates that fail to capture complex pressure distributions. A scalable, accurate estimation method from motion data alone is needed.

Method: GraCE leverages the human center of gravity (CoG) and its relative distance to body segments for contact estimation. Contact forces are computed as the product of predicted contact probabilities and total exterior force derived from center of mass trajectory, guided by gravity and mass distribution.

Result: Outperforms related work on the GroundLink dataset (ground reaction force estimation) and the MOYO dataset (detailed contact pressure prediction).

Conclusion: Gravity-guided estimation with center-of-gravity reasoning provides a more accurate and scalable approach to contact dynamics estimation from motion capture, eliminating the need for specialized hardware.
...
TL;DR: IMAGINE is a novel network for Composed Video Retrieval that handles implicit semantics (e.g., "cake" implying "birthday party") by using dynamic multimodal prototypes to capture latent concepts and modulate visual features, achieving state-of-the-art results on CVR and CIR benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing CVR methods rely on explicit feature alignment but fail when modification texts describe concepts not directly shown in videos (implicit semantics like "cake" → "birthday party"). The paper aims to bridge this gap between explicit visual content and implicit retrieval intentions.

Method: Proposes IMAGINE (adaptIve scheMa-ImAGery enhanced composItional NEtwork) that materializes implicit semantics (schema imagery) via dynamic multimodal prototypes. These prototypes capture shared latent concepts and adaptively modulate visual features, injecting implicit guidance into retrieval.

Result: Achieves state-of-the-art performance on both Composed Video Retrieval (CVR) and Composed Image Retrieval (CIR) tasks across three widely used benchmarks.

Conclusion: Bridging explicit visual contents and implicit retrieval intentions via dynamic schema imagery prototypes is effective for compositional retrieval tasks, outperforming existing explicit matching approaches.
...
TL;DR: A property-informed diffusion network generates 3D metamaterial microstructures from text descriptions, using dual alignment (contrastive + reward-guided) to ensure semantic and physical consistency.


<details>
  <summary>Details</summary>
Motivation: Traditional inverse design for microstructures requires domain expertise, iterative simulations, and manual tuning; existing methods suffer from limited diversity and poor physical feasibility.

Method: A property-informed diffusion network conditioned on text prompts, with a dual alignment strategy combining contrastive text-structure alignment and test-time reward-guided alignment to enforce consistency.

Result: The model generates semantically meaningful and physically plausible 3D microstructures across diverse material categories.

Conclusion: This approach enables interactive microstructure design and opens new directions for combining language-based interfaces with inverse material discovery.
...
TL;DR: RAPID is a training-free, depth-aware token reduction framework for Vision Transformers that uses pruning in shallow layers and merging in deep layers, achieving superior accuracy-compression trade-offs compared to ToMe and ToFu on ImageNet-1K.


<details>
  <summary>Details</summary>
Motivation: ViTs suffer from quadratic self-attention complexity. Existing token reduction methods (pruning/merging) ignore how token representations evolve across network depth, leading to suboptimal accuracy-compression trade-offs.

Method: RAPID employs a bifurcated depth-aware strategy: (1) shallow-to-middle layers use redundancy-similarity aware pruning to eliminate over-represented local patterns; (2) deeper layers shift to importance-similarity aware merging, leveraging CLS token attention weights to protect critical tokens while fusing less important similar neighbors.

Result: On ImageNet-1K with ViT/DeiT, RAPID achieves superior accuracy-compression Pareto frontier vs. ToMe and ToFu; up to 4.29% higher accuracy than ToMe at extreme reduction rates.

Conclusion: RAPID provides a training-free template for optimizing vision models by aligning reduction strategies with hierarchical feature evolution.
...
TL;DR: This paper investigates the impact of MRI preprocessing levels on self-supervised 3D brain MRI pretraining, finding that moderate preprocessing (P2) is cost-effective and stronger preprocessing yields marginal or uneven gains across downstream tasks.


<details>
  <summary>Details</summary>
Motivation: MRI preprocessing is typically treated as routine data cleaning rather than a modeling choice, and its impact on self-supervised pretraining for brain MRI foundation models has not been systematically evaluated.

Method: The authors compare a graded P0-P7 preprocessing spectrum for MAE and JEPA on 20,000 heterogeneous brain MRI volumes, keeping corpus, 3D ViT backbone, masking protocol, and downstream evaluations fixed, then transfer encoders to four tasks: IDH prediction, MCI classification, brain age regression, and tumor segmentation.

Result: No simple "more is better" pattern; P0/P1 are numerically unstable; P2 is the lowest feasible level; beyond P2, aggregate utility gain is only 3.4 pp (MAE) and 1.8 pp (JEPA); stronger preprocessing benefits only specific tasks (MCI shows the clearest gain at P7); P7 advantages can be largely recovered by applying stronger preprocessing downstream.

Conclusion: MRI preprocessing should be treated as a downstream-aware cost-utility decision rather than a default escalation pipeline.
...
TL;DR: A satellite-guided framework that resolves scale ambiguity in feed-forward 3D reconstruction by using satellite imagery as a global metric reference, enabling metric-scale geometry and pose estimation without costly annotations or calibration.


<details>
  <summary>Details</summary>
Motivation: Feed-forward 3D reconstruction models suffer from unknown global scale ambiguity, and existing metric methods rely on expensive metric annotations or accurate camera calibration, which are impractical in many real-world settings.

Method: Retrieves a local satellite patch using a coarse camera pose, integrates it with a feed-forward reconstruction backbone via bidirectional cross-view interaction, and enforces consistency between the reconstructed scene and satellite reference to infer absolute scale, refine geometry, and estimate metric camera pose.

Result: Consistent improvements on KITTI, nuScenes, and Oxford RobotCar for metric depth estimation, multi-view point-cloud reconstruction, and cross-view camera localization, while maintaining strong generalization across datasets and regions.

Conclusion: Satellite imagery is a practical and scalable global reference for resolving scale ambiguity, enabling metric 3D reconstruction without costly annotations or calibration.
...
TL;DR: SegmentAnyTreeV2 is a sensor-agnostic framework for semantic and instance segmentation of forest point clouds, using a Point Transformer v3 backbone with specialized mask decoding and a new FOR-instance v3 benchmark (427 scenes, 26,496 trees). It achieves 85.0% F1 and 87.6% semantic mIoU, outperforming prior methods with strong cross-domain generalization.


<details>
  <summary>Details</summary>
Motivation: To enable accurate and generalizable tree segmentation from forest point clouds across diverse sensors, platforms, and forest structures—addressing limitations of prior methods in dense and complex stands.

Method: Serialization-based Point Transformer v3 backbone + lightweight semantic head + tree-focused cross-attention mask decoder; instance-aware query initialization, one-to-many seed supervision, asymmetric mask scoring; coupled with a large-scale benchmark (FOR-instance v3: 427 scenes, 26,496 trees).

Result: 90.5% precision, 80.2% recall, 85.0% F1, 90.7% coverage, 87.6% semantic mIoU on FOR-instanceV2 test split; outperforms prior learning-based methods; strong zero-shot cross-domain generalization.

Conclusion: SegmentAnyTreeV2 provides a robust, platform-agnostic solution for tree instance segmentation with state-of-the-art performance and strong generalization to unseen forest environments.
...
TL;DR: This paper provides the first comprehensive survey of Test-time Scaling (TTS) for Multimodal Foundation Models (MFMs), proposing a unified taxonomy of three strategies (sampling-based, feedback-based, search-based) and reviewing benchmarks, applications, and future directions.


<details>
  <summary>Details</summary>
Motivation: Despite rapid progress in adapting Test-time Scaling (TTS) to Multimodal Foundation Models (MFMs), the field lacks a systematic survey and unified theoretical framework to delineate the developmental landscape of multimodal TTS.

Method: The authors conduct a comprehensive review and propose a unified taxonomic framework categorizing TTS methods into three strategies: sampling-based, feedback-based, and search-based approaches.

Result: A unified taxonomy of multimodal TTS methods, summarization of representative applications and benchmarks for generation and reasoning tasks.

Conclusion: The survey provides a systematic roadmap for subsequent studies and outlines open challenges and future research directions in multimodal TTS.
...
TL;DR: Light-WAM is a lightweight World Action Model that uses a compact video backbone, latent-space future prediction, and an efficient StateFusionActionExpert to achieve strong robot manipulation performance with only 0.44B parameters, 72ms inference latency, and 4.1GiB GPU memory.


<details>
  <summary>Details</summary>
Motivation: Current World Action Models rely on large-scale generative architectures with high training costs and inference latency, making them impractical as efficient closed-loop policies. The authors aim to reduce these costs while retaining the benefits of future-prediction for representation learning.

Method: Light-WAM uses a compact video backbone with future-video supervision in a downsampled latent space. It introduces StateFusionActionExpert, which reads adapted states from multiple backbone layers, fuses them via learned-query pooling, and predicts action chunks in a single forward pass, avoiding heavy generative action experts.

Result: Light-WAM maintains strong performance on LIBERO and achieves usable multi-task performance on RoboTwin 2.0 with only 0.44B trainable parameters. It achieves 72.03ms inference latency, 4.1GiB peak GPU memory, and improved training throughput.

Conclusion: Light-WAM demonstrates that efficient, lightweight WAMs can compete with larger models while offering significant advantages in inference speed and memory usage, making them more practical for real-world robot deployment.
...
TL;DR: TIDE is a unified diffusion transformer framework that handles instruction-based editing, reference-guided editing, and multi-reference generation by introducing per-token task embeddings, a dual-path conditioning scheme (vision-language model + VAE latent path), and a multi-task progressive training strategy, achieving SOTA on multiple video editing/generation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current video generation and editing tasks are handled by separate, task-specific models. Building a unified framework is challenging because existing attempts need dedicated auxiliary encoders or lack mechanisms to distinguish heterogeneous conditioning tokens, struggling when visual conditions vary across tasks.

Method: (1) Per-token task embeddings assign each input token a task-specific identifier to disambiguate target, source, and reference tokens. (2) Dual-path conditioning scheme couples a vision-language model with a VAE latent path for complementary signals (semantic understanding + structural fidelity). (3) Multi-task progressive training strategy incrementally introduces tasks of increasing complexity to harmonize diverse objectives.

Result: Extensive experiments on multiple video editing and generation benchmarks demonstrate state-of-the-art performance across all evaluated tasks (instruction-based editing, reference-guided editing, and multi-reference generation).

Conclusion: TIDE successfully unifies multiple video tasks in a single framework, showing that explicit per-token task disambiguation and dual-path conditioning can effectively handle heterogeneous task distributions without requiring dedicated auxiliary encoders.
...
TL;DR: This paper introduces UQ-DAAAM, a spatial-semantic memory system that uses object-level uncertainty scores to detect unreliable VLM captions and actively refine them by selecting high-quality views, improving long-horizon robot operation and question answering.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based memory systems treat captions as reliable oracles with no mechanism to detect noisy, viewpoint-inconsistent descriptions, making them unreliable for long-horizon robot operation.

Method: The authors propose object-level semantic uncertainty scores to measure cross-view semantic scatter, then integrate these scores into UQ-DAAAM, which actively refines uncertain objects under a fixed query budget by selecting high-quality views and fusing multi-view captions.

Result: UQ-DAAAM achieves substantially larger uncertainty reduction and better spatio-temporal question answering performance than baselines on the OC-NaVQA benchmark.

Conclusion: Uncertainty quantification can make embodied 4D memory systems more reliable and effective for long-horizon robot tasks.
...
TL;DR: G2G introduces a method to recover 6-DoF pose between two image groups by freezing a multi-view foundation model and adding three lightweight trainable modules (perceiver resampler, cross-group bridge with merged self-attention, multi-frame pose head), totaling ~32M parameters. It achieves SOTA on cross-sequence relocalization and multi-camera rig odometry across four datasets.


<details>
  <summary>Details</summary>
Motivation: Current multi-view models treat all views as an unstructured set and lack explicit cross-group reasoning, which is needed for tasks like cross-sequence relocalization and multi-camera rig odometry where intra-group geometry is known but inter-group pose must be inferred.

Method: Freezes a pretrained multi-view foundation model entirely, adds three lightweight trainable modules: (1) a perceiver resampler to compress per-group features, (2) a cross-group bridge with merged self-attention to enable inter-group communication, and (3) a multi-frame pose head to regress relative 6-DoF pose. Trained with only relative pose supervision (~32M trainable params, <6% of full model).

Result: Attains state-of-the-art accuracy on both cross-sequence relocalization and multi-camera rig odometry across four datasets (indoor/outdoor simulation, real-world cross-season, zero-shot sim-to-real), outperforming baselines retrained with full supervision.

Conclusion: Lightweight trainable modules added to a frozen multi-view foundation model can effectively bridge two image groups for relative pose estimation, achieving superior accuracy with minimal trainable parameters and without requiring full model retraining.
...
TL;DR: HACK++ is a training-free KV cache compression framework for VAR models that classifies attention heads into Contextual (semantic) and Structural (spatial) types, then applies adaptive, head-specific compression strategies to drastically reduce memory and computation overhead while maintaining near-lossless generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing VAR models suffer from high attention complexity and severe memory overhead due to KV cache accumulation across scales, and existing one-size-fits-all compression methods perform poorly because VAR attention heads divide into two functionally distinct types (Contextual vs. Structural) with different compression needs.

Method: HACK++ uses a one-time offline calibration to classify head types and derive head-specific priors. At inference, it decouples attention from cache compression under independent budgets, employing pattern-specific strategies and reliance-aware budget allocation to compress the accumulated cache aggressively while bounding current-scale attention cost.

Result: On Infinity-2B/8B, HACK++ achieves near-lossless generation with only 30% attention budget and 10% cache budget, and remains robust even under extreme 1% cache budget. Validated across text-to-image, class-conditional, and unified understanding-and-generation tasks.

Conclusion: HACK++ effectively addresses the KV cache bottleneck in VAR models through head-aware, adaptive compression, demonstrating strong generalizability and robustness across diverse tasks and model scales.
...
TL;DR: The paper presents a lightweight deep learning framework for atmospheric compensation in passive long-wave infrared hyperspectral imaging, using multiple radiance measurements at different standoff ranges to jointly estimate transmittance, path radiance, and downwelling spectrum, with experiments showing low spectral distortion.


<details>
  <summary>Details</summary>
Motivation: Atmospheric compensation in passive LWIR hyperspectral imaging is essential but has been largely overlooked due to practical and modeling difficulties, despite being critical for understanding targets of interest under standoff geometry.

Method: A lightweight set-based deep learning framework that takes multiple radiance measurements at different standoff ranges as input, jointly estimating transmittance, atmospheric path radiance, and a shared downwelling spectrum. The learned representation is analyzed using a sparse autoencoder.

Result: Low spectral distortion across all estimated products on a MODTRAN-generated standoff LWIR dataset. Latent features activate on geographically coherent subsets of test data despite no location supervision.

Conclusion: The proposed method effectively addresses atmospheric compensation in LWIR hyperspectral imaging, with promising results and publicly available dataset/code for reproducibility.
...
TL;DR: SMI proposes a lightweight self-supervised learning objective that uses nonlinear dependency optimization on sample-level correlation matrices, achieving competitive performance to state-of-the-art SSL methods with lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing SSL methods require large batch sizes, memory banks, momentum encoders, or global synchronization, which increase computational cost and training complexity.

Method: Semantic Mutual Information (SMI) — a mutual-information-inspired objective that performs optimization on a sample-level dependency matrix via nonlinear transformation of pairwise correlations under Gaussian assumptions.

Result: SMI achieves competitive linear evaluation on ImageNet (ResNet-50) with lower complexity, improves transfer on low-resource and fine-grained datasets over Barlow Twins, and shows better alignment-redundancy balance, feature diversity, and spatially localized representations.

Conclusion: Nonlinear dependency optimization is an effective and computationally efficient alternative to conventional correlation-based SSL objectives.
...
TL;DR: The paper proposes Direct Latent Augmentation (DLA) to avoid the inefficient decode-encode loop in multimodal learning, and Multilayer Explicit Simulated Synesthesia (MESSy) to transfer privileged generative latent knowledge to visual-only students via predictive objectives, achieving strong performance without paired data.


<details>
  <summary>Details</summary>
Motivation: Multimodal integration improves vision models but suffers from high inference costs and scarce paired datasets. Existing generative methods waste computation by decoding generative latents to noise and re-encoding them (Decode-Encode Loop).

Method: DLA uses undecoded generative latents as privileged information. MESSy transfers this knowledge to a visual student via predictive objectives (rather than rigid representation matching), enabling "simulated synesthesia."

Result: Outperforms raw data augmentation and traditional distillation. Produces accurate unimodal students with latent structures aligned to unobserved physical properties.

Conclusion: Bypassing the decode-encode loop and using predictive distillation yields more efficient and effective multimodal-to-unimodal knowledge transfer.
...
TL;DR: This paper studies DINO-family self-supervised vision transformers for TMJ osteoarthritis detection on CBCT images, finding that partially unfreezing the final two transformer blocks dramatically improves AUC from 0.671 to 0.902, outperforming other backbones and a supervised baseline.


<details>
  <summary>Details</summary>
Motivation: TMJ OA osseous changes are often subtle on CBCT, making automated detection challenging. The authors want to understand how well DINO-family models transfer to this low-data medical imaging domain and what adaptation strategies are needed.

Method: A slice-based pipeline using ViT backbones (DINOv1, DINOv2, DINOv2+reg, RAD-DINO) with frozen or partially adapted encoders, aggregating per-slice features via attention-based multiple instance learning (MIL) for patient-level binary classification. Systematic ablation of unfreezing strategies and aggregation designs.

Result: Partial unfreezing of final two transformer blocks was decisive, boosting AUC from 0.671 to 0.902, outperforming DINOv1 (0.867), DINOv2+reg (0.774), and supervised ImageNet ViT-B/16 (0.843).

Conclusion: Adaptation strategy matters more than backbone choice for DINO-family models in low-data medical imaging; partial fine-tuning of final blocks is highly effective.
...
TL;DR: A multi-agent framework decomposes single-image 3D scene generation into three stages—scene initialization, environment construction, and multi-agent refinement—achieving superior geometric accuracy and realism while reducing reliance on scene-level supervision.


<details>
  <summary>Details</summary>
Motivation: Existing methods for single-image 3D scene generation rely on holistic pipelines that entangle multiple factors and require extensive scene-level supervision, limiting generalization to complex real-world environments.

Method: A multi-agent orchestration framework with three stages: (1) initialization (object masks, 3D representations, layout prediction), (2) environment construction (surfaces, boundaries, materials, illumination using point-map geometry), (3) multi-agent refinement (planner agent fixes simple issues, specialist agents handle complex localized revisions). Also introduces a geometry-aware layout predictor trained with sparse geometric priors from point maps.

Result: Outperforms prior methods on benchmark datasets in geometric accuracy, spatial consistency, and perceptual realism.

Conclusion: The proposed multi-agent decomposition and geometry-aware layout prediction effectively address generalization challenges, reducing the need for full scene-level supervision while improving 3D scene quality.
...
TL;DR: The paper introduces a Poisson-equation-based framework that models cortical folding as a geometry-driven flow to detect subtle, spatially distributed cortical abnormalities in juvenile myoclonic epilepsy (JME).


<details>
  <summary>Details</summary>
Motivation: Cortical folding abnormalities in JME are often subtle and spatially distributed, making them difficult to detect with conventional morphometric measures that rely on indirect scalar summaries rather than explicitly modeling folding geometry.

Method: A Poisson-equation-based framework that models cortical folding as a geometry-driven flow derived from mean curvature on the cortical manifold, treating folding patterns as a stationary source-sink structure to produce a smooth potential field with an interpretable surface gradient flux.

Result: The framework yields a smooth, globally balanced potential field whose surface gradient defines a physically interpretable flux, enabling spatially coherent analysis of sulcal-gyral folding organization.

Conclusion: This approach provides a principled, geometry-driven representation of cortical structure in JME, potentially improving detection of subtle folding abnormalities beyond conventional morphometric measures.
...
TL;DR: CoVEBench is a new benchmark for compositional video editing, containing 416 videos, 626 multi-point instructions, and 9,990 checklist items. It reveals current models struggle with multiple simultaneous edits, often omitting changes, violating preservation constraints, or introducing artifacts.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks are limited to isolated edits and coarse global metrics, failing to diagnose how models handle real-world compositional user requests that involve multiple coupled edits (e.g., modifying subjects, actions, and camera views simultaneously) while preserving unrelated content.

Method: The authors construct CoVEBench with 416 curated source videos, 626 multi-point editing instructions, and 9,990 fine-grained checklist items. They use MLLM-judged instruction compliance and video fidelity, along with automated quality metrics, to evaluate models across diverse editing dimensions.

Result: Compositional editing remains a profound challenge for current models, which frequently omit edits, violate preservation constraints, or introduce artifacts when handling multiple simultaneous operations.

Conclusion: CoVEBench provides a challenging, diagnostic testbed to advance video editing toward realistic user workflows by revealing critical shortcomings in existing models.
...
TL;DR: CheXanatomy integrates anatomical knowledge into VLMs via autoregressive token-space supervision for segmentation, showing competitive performance with U-Net and improved robustness under domain shift.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs excel at global image-level understanding but lack fine-grained anatomical structure encoding, limiting their effectiveness for spatially precise tasks like segmentation.

Method: The framework trains a VLM to generate anatomical segmentation masks via next-token prediction using synthetic chest radiographs generated from CT volumes with forward-projected labels.

Result: Achieves comparable performance to U-Net in-distribution, improved geometric robustness under domain shift to real CXR data, and better sample efficiency for novel localization tasks.

Conclusion: Embedding anatomical structure into the generative objective promotes spatially grounded representations and supports anatomy-aware medical vision-language modeling.
...
TL;DR: A synthesis-centric, segmentation-assisted closed-loop framework with retrieval augmentation (SSCF) that synthesizes missing brain MRI contrasts using a GAN architecture, segmentation feedback, and dual-bank retrieval for tumor-aware synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing multi-contrast brain MRI synthesis methods struggle to synthesize critical tumor regions and effectively exploit contextual information, especially when dealing with incomplete contrast images.

Method: (1) A single GAN model that synthesizes missing contrasts from any available combination; (2) An auxiliary segmentation branch that predicts tumor masks and feeds them back as semantic conditioning; (3) A dual-bank retrieval augmentation strategy querying tumor masks memory bank and cross-image contrast feature memory bank.

Result: Outperforms previous methods on BraTs2020 and UCSF-BMSR datasets in medical brain image synthesis tasks.

Conclusion: The closed-loop segmentation-assisted design with retrieval augmentation significantly improves tumor-aware synthesis fidelity for multi-contrast brain MRI.
...
TL;DR: This paper proposes CACR, a Candidate-Aware Causal Reasoning framework for temporal answer grounding in instructional videos, which uses a VBCS algorithm to select candidate segments and a temporal logic reasoning module with rejection reward and GRPO optimization, achieving SOTA performance on six benchmarks.


<details>
  <summary>Details</summary>
Motivation: The task of temporal answer grounding in instructional videos is challenging due to semantically complex questions and significant length mismatch between untrimmed videos and short target moments. Existing methods suffer from sensitivity to irrelevant content and insufficient visual reasoning capabilities.

Method: The CACR framework has two main components: (1) a Visual-Language Pre-training based Candidate Selection (VBCS) algorithm to efficiently generate K candidate segments, and (2) a temporal logic reasoning module enhanced by a rejection reward mechanism and optimized via Group Relative Policy Optimization (GRPO) for robust inference.

Result: Extensive experiments on six benchmarks demonstrate state-of-the-art performance in terms of mean Intersection-over-Union (mIoU).

Conclusion: The proposed CACR framework provides a new perspective for reasoning-based retrieval in long videos, effectively addressing limitations of existing methods.
...
TL;DR: TVI-CoT introduces learnable control tokens (<THINK>, <LOOK>, <ANSWER>) enabling multimodal LLMs to interleave textual reasoning with active visual feature access, overcoming the "vision-blind" limitation of standard chain-of-thought reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing CoT methods for multimodal LLMs perform reasoning entirely in text without accessing visual features after initial encoding, creating a "vision-blind reasoning" paradigm that limits fine-grained visual extraction, error verification, and adaptive attention.

Method: Proposes Text-Visual Interleaved Chain-of-Thought (TVI-CoT) with three learnable control tokens: <THINK> for textual reasoning, <LOOK> for dynamic visual feature access conditioned on reasoning state, and <ANSWER> for final output. These tokens enable switching between reasoning and visual grounding.

Result: State-of-the-art results among MLLM-based CoT methods on 8 benchmarks; performance boosts over baseline: +6.1% on MMMU, +3.8% on MathVerse, +3.4% on MathVista, +3.4% on ScienceQA.

Conclusion: Explicitly interleaving visual access with textual reasoning via control tokens significantly improves multimodal reasoning performance, addressing a fundamental limitation of current CoT approaches.
...
TL;DR: FaithRewriter is a prompt-enhancement framework that uses MLLM to generate visual cues, LLM for visually grounded augmentations, and distills into a small LLM for efficient T2I prompt rewriting to narrow the intent-generation gap.


<details>
  <summary>Details</summary>
Motivation: Existing prompt rewriting methods lack visual grounding and may over-infer missing details, causing an intent-generation gap between user prompts and T2I model outputs.

Method: (1) MLLM generates an image from the original prompt as an intermediate visual cue. (2) Visual cue + prompt are fed into a large-scale LLM to produce visually grounded augmentations. (3) Augmentations are distilled into a small-scale LLM for efficient deployment.

Result: FaithRewriter produces prompts more faithful to user intent and more visually plausible than strong baselines.

Conclusion: Visually grounded prompt enhancement helps narrow the intent-generation gap in text-to-image generation.
...
TL;DR: V-Skip is a training-free inference method that selectively bypasses saturated visual self-attention modules in deeper layers of MLLMs, reducing computational cost while maintaining 94-100% performance across diverse models.


<details>
  <summary>Details</summary>
Motivation: Multimodal Large Language Models (MLLMs) suffer from quadratic inference costs due to self-attention over long visual token sequences. The authors identify that visual-to-visual self-attention becomes computationally redundant in deeper layers after visual tokens establish spatial structure early on.

Method: V-Skip introduces block-wise structured sparsity by bypassing saturated visual self-attention modules in deeper layers while keeping FFNs active. It uses lightweight few-shot calibration to dynamically select the optimal sparsity path for different downstream tasks.

Result: V-Skip achieves 94.16% to 100.31% performance retention across various MLLMs while reducing redundant computation by skipping saturated visual attention blocks.

Conclusion: Models do not need to discard visual tokens to reason more efficiently; they simply need to "look less" at the right depth by bypassing attention that has already saturated.
...
TL;DR: This paper proposes the Try-On Anything task for simultaneous multi-object video virtual try-on without external priors, introduces TryAny-Bench as a benchmark, and presents OmniTryOn, a generative framework using First Frame Wearable Cache, STC-RoPE, and Gradual Try-On training, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing VVT methods are limited to single-garment transfer and rely heavily on external priors (e.g., garment masks), which destroy physical dynamics and degrade visual quality. The authors aim to overcome these limitations by enabling simultaneous multi-object try-on in videos without external priors.

Method: OmniTryOn: (1) First Frame Wearable Cache — provides diverse wearable objects from the initial video frame; (2) Spatiotemporally Consistent RoPE (STC-RoPE) — establishes robust spatiotemporal anchors to preserve motion and background dynamics; (3) Gradual Try-On (GTO) training strategy — progressively trains the model for robust multi-object synthesis. Introduces TryAny-Bench benchmark with paired video dataset and evaluation protocol.

Result: Extensive experiments on TryAny-Bench show OmniTryOn significantly outperforms existing specialized VVT models and general video editing baselines.

Conclusion: OmniTryOn establishes a powerful new standard for the Try-On Anything task. Dataset, code, and models are publicly available.
...
TL;DR: This paper introduces DriveReward, a specialized Vision-Language Reward Model for autonomous driving, trained on a counterfactual-augmented reasoning trajectory dataset, which outperforms larger VLMs on reward alignment tasks and achieves performance comparable to rule-based methods in RL finetuning and trajectory scoring.


<details>
  <summary>Details</summary>
Motivation: Current reward models for autonomous driving rely on hand-crafted rule-based objectives or perception ground truth, limiting generalization for data-scaling. While VLMs have been used as reward models in other domains, their effectiveness in driving tasks is underexplored. This work aims to bridge that gap.

Method: (1) Constructed DriveReward, a reasoning trajectory evaluation dataset with temporally-grounded visual guidance and counterfactual driving behaviors. (2) Proposed a counterfactual data annotation scheme to address scarcity of failure cases. (3) Tailored a specialized 1B Vision-Language Reward Model trained on this dataset.

Result: Leading open-source and proprietary VLMs fail to excel across all driving reward tasks. The specialized 1B reward model outperforms larger VLMs on task-specific reward alignment. When integrated into RL finetuning and multi-modal trajectory scoring, it achieves performance comparable to rule-based reward calculations in both open-loop and closed-loop evaluations.

Conclusion: The proposed DriveReward and specialized VLM-based reward model effectively bridge the gap between VLM capabilities and driving reward modeling, demonstrating that a smaller, task-specific model can outperform larger general-purpose VLMs for autonomous driving reward alignment.
...
TL;DR: NGram-MoSE is a lightweight Transformer for single-image super-resolution in remote sensing that uses N-Gram Context Injection for texture continuity and Mixture-of-Experts for efficient capacity scaling, achieving 31.68 dB PSNR with 14× fewer FLOPs than heavyweight models and improving landslide segmentation mAP@50 by 4.47%.


<details>
  <summary>Details</summary>
Motivation: Remote sensing faces a spatial-temporal trade-off: fine spatial detail imagery is acquired less frequently than coarser imagery. Existing Transformer-based SR models are computationally expensive and sensitive to biased training data, degrading robustness under out-of-distribution conditions.

Method: NGram-MoSE introduces two key components: (1) N-Gram Context Injection to strengthen cross-window local consistency and reduce window-boundary artifacts, and (2) a Mixture-of-Experts (MoE) feed-forward design for sparse activation that scales capacity without proportional inference cost growth.

Result: Achieves 31.68 dB PSNR on a geographically disjoint OOD test set with 14× FLOPs reduction vs. heavyweight Transformer. In landslide segmentation, yields 4.47% absolute gain in mAP@50 over bicubic upsampling, with stronger cross-scale consistency.

Conclusion: NGram-MoSE provides an effective, efficient SR module for resource-constrained remote sensing pipelines requiring robust generalization under out-of-distribution conditions.
...
### [94] [FiberTune: Preserving Action-Fiber Visual Residuals in Vision-Language-Action Fine-Tuning](https://arxiv.org/abs/2606.08653)
*Haihao Lin,Xiangsheng Huang,Xiao Yang,Weibang Zhou,Yiqi Zhang,Bo Yang,Simin Zeng,Jiawei Yang,Zhengyang Wang,Jiahui Du*

Main category: cs.CV

TL;DR: FiberTune prevents visual feature collapse in VLA policy fine-tuning by preserving teacher-structured residuals after removing action-predictive directions, improving success rates across benchmarks and real-world tasks without inference overhead.


<details>
  <summary>Details</summary>
Motivation: Standard action-supervised fine-tuning of VLA policies causes visual representations to collapse along directions not affecting action prediction (residual visual collapse along "action fibers"), limiting generalization.

Method: FiberTune removes action-predictive features (via an online action probe) from intermediate visual tokens, then aligns the remaining residuals to a frozen visual teacher while regularizing effective rank.

Result: +10.7 pp SR(5) on CALVIN ABC-to-D, real-world SO-101 success from 72.7% to 78.1%, consistent improvements across 2 benchmarks & 2 architectures (pi_0.5, OpenVLA-OFT).

Conclusion: FiberTune is a simple, inference-free training objective that systematically alleviates visual collapse in VLA fine-tuning, validated by residual diagnostics showing increased teacher alignment and effective rank.
...
### [95] [WaveDiT: Distribution-Aware Wavelet Flow Matching for Efficient 3D Brain MRI Synthesis](https://arxiv.org/abs/2606.08670)
*Danilo Danese,Angela Lombardi,Giuseppe Fasano,Matteo Attimonelli,Tommaso Di Noia*

Main category: cs.CV

TL;DR: WaveDiT is a conditional flow matching model that operates in 3D Haar wavelet coefficient space, enabling full-resolution brain MRI synthesis on a single GPU with improved anatomical fidelity and downstream task performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Large, balanced 3D brain MRI datasets are needed for reliable neuroimaging biomarkers, but existing generative augmentation methods are either computationally prohibitive at full resolution or lose anatomical detail through lossy compression.

Method: WaveDiT uses conditional flow matching in 3D Haar Discrete Wavelet Transform coefficient space, combining factorized spatio-depth attention with band-wise heteroscedastic uncertainty modeling from higher-order wavelet statistics; predicted log-variance is integrated into both the flow objective and conditioning pathway.

Result: Outperforms diffusion, latent, and wavelet-based baselines on a multi-site cohort in distribution alignment, brain age prediction, and region-level anatomical agreement, all under practical memory/time constraints on a single modern GPU.

Conclusion: Wavelet-domain flow matching with adaptive uncertainty modeling enables practical full-resolution 3D MRI synthesis for data augmentation without requiring specialized compute infrastructure.
...
### [96] [Learning to Solve Generative ODEs Beyond the Linear Span](https://arxiv.org/abs/2606.08672)
*Sihyeon Kim,Seunghun Lee,Vikas Singh,Hyunwoo J. Kim*

Main category: cs.CV

TL;DR: This paper proposes SpanLift, a lightweight neural solver that improves few-step sampling in diffusion and flow generative models by augmenting scalar-coefficient updates with a spatial residual operator, achieving state-of-the-art results with only 3 NFE.


<details>
  <summary>Details</summary>
Motivation: Existing scalar-coefficient solver learning methods are bottlenecked because each step is span-limited—the update can only fit components within the span of buffered velocity evaluations, leaving out-of-span residuals unreachable by scalar recombination alone.

Method: SpanLift augments a fixed base solver (in-span prior) with a lightweight spatial residual operator learned via endpoint teacher matching, operating over the state and velocity buffer without adding model NFEs.

Result: Achieves SOTA few-step sampling: CIFAR-10 FID from 8.16→5.69, ImageNet FID from 17.37→11.83 at 3 NFE. The learned correction transfers across base solvers and is predominantly out-of-span.

Conclusion: Adding a spatial residual operator to scalar-coefficient updates effectively overcomes the span-limitation bottleneck, enabling high-quality few-step generation while preserving pretrained backbones.
...
### [97] [BioVid: Autoregressive Video Generation with Biological Behavior Semantic Comprehension](https://arxiv.org/abs/2606.08674)
*Tsung-Wei Pan,Jung-Hua Wang*

Main category: cs.CV

TL;DR: BioVid is a data-driven autoregressive video generation framework that learns temporal structure and natural length distributions of biological behaviors from training data, outperforming fixed-length baselines in matching duration distributions on human drinking behavior data.


<details>
  <summary>Details</summary>
Motivation: Existing video generation frameworks treat sequence duration as an externally prescribed parameter (fixed frame counts or text prompts), which is misaligned with biological behavior where action duration varies naturally across individuals and is encoded in the data itself.

Method: Two-stage framework: (1) FSQ-R3GAN tokenizer encodes video frames into compact discrete representations with high-fidelity spatial reconstruction; (2) A causal Transformer models token sequences autoregressively and learns to emit an End-of-Sequence (EOS) token when the behavioral event reaches semantic closure, with termination distribution emerging naturally from training data.

Result: On NTU RGB+D (A001, n=94) human drinking dataset, BioVid achieves Wasserstein-1 distance of 1.24 vs ground truth, compared to 6.05 for fixed-length baseline and 15.48 for VideoGPT, while maintaining competitive spatial fidelity.

Conclusion: Learning temporal duration distributions directly from data enables more realistic video generation of biological behaviors compared to externally prescribed duration constraints.
...
### [98] [Distortion-Aware PETR for BEV Object Detection with Mixed Pinhole-Fisheye Cameras](https://arxiv.org/abs/2606.08680)
*Xiangzhong Liu*

Main category: cs.CV

TL;DR: DAPETR is a projection-free 3D object detector for mixed pinhole-fisheye camera setups, using learned-adaptive modules (distortion-aware positional embedding and feature-geometry co-modulation) to handle fisheye distortion, outperforming baselines on KITTI-360 while revealing that learned adaptation and explicit geometric reparameterization can conflict.


<details>
  <summary>Details</summary>
Motivation: Fisheye cameras are widely used in autonomous driving for their low cost and wide FOV, but their severe radial distortion violates the uniform sampling assumption of most BEV detectors, leaving their potential underleveraged in 3D object detection.

Method: Proposes DAPETR with two key modules: (1) unified distortion-aware positional embedding that harmonizes positional encodings with fisheye geometry, and (2) bidirectional feature-geometry co-modulation that mutually adapts image features and 3D positional embeddings, without relying on image rectification.

Result: On a converted KITTI-360 benchmark, DAPETR outperforms both the baseline PETR and PolarPETR (polar coordinate variant). The study also reveals a negative interaction when combining learned adaptation with explicit geometric reparameterization.

Conclusion: DAPETR advances fisheye BEV detection research and provides insights into effective distortion-aware 3D perception design beyond traditional image rectification approaches.
...
### [99] [BLUE: Toward Better Language Use in Efficient Vision-Language-Action Models for Autonomous Driving](https://arxiv.org/abs/2606.08684)
*George Ling,Lijin Yang,Hao Yang,Zhongzhan Huang*

Main category: cs.CV

TL;DR: BLUE is a lightweight gate (0.11M parameters) that selectively activates language generation in VLA models for autonomous driving, achieving SOTA results (76.2% on Bench2Drive, 36 on Longest6 v2) with 2.54x speedup and 8.9% success rate improvement, by using frozen VLA hidden states to decide per-frame whether language helps.


<details>
  <summary>Details</summary>
Motivation: Language benefits only a small fraction of driving routes but can greatly impact performance. Generating language at every frame is computationally wasteful, and existing methods lack selective language activation.

Method: BLUE trains a lightweight gate (0.11M parameters) on frozen VLA hidden states to decide per frame whether to activate language generation or directly predict actions, without modifying the backbone or needing additional annotations.

Result: Achieves 76.2% success rate on Bench2Drive and 36 driving score on Longest6 v2, with 2.54x inference speedup and 8.9% success rate improvement over the backbone.

Conclusion: BLUE provides a practical, efficient path toward language-augmented autonomous driving, showing VLA models can retain language benefits at a fraction of the computational cost.
...
### [100] [Shift-Dependent Asymmetry: Orthogonal Inverse Low-Rank Adaptation for Federated Medical Segmentation](https://arxiv.org/abs/2606.08687)
*Xingyue Zhao,Wenke Huang,Linghao Zhuang,Haoran Wu,Anwen Jiang,Zhifeng Wang,Wenwen He,Ming Feng,Mang Ye,Bo Xu*

Main category: cs.CV

TL;DR: IAT proposes personalized LoRA modules for encoder (appearance) and decoder (supervision) with a subspace orthogonality regularizer to prevent information leakage, improving federated medical image segmentation.


<details>
  <summary>Details</summary>
Motivation: Federated LoRA methods use uniform aggregation that fails under encoder-decoder asymmetry in medical segmentation: encoders handle appearance shifts, decoders handle supervision variations, causing entanglement of anatomy with site-specific biases.

Method: IAT personalizes module-specific LoRA components in encoder (appearance) and decoder (supervision) with a shared pathway, plus a Subspace Orthogonality Regularizer to penalize shared-local collinearity and prevent site-specific leakage.

Result: Consistent improvements over strong federated LoRA and parameter-efficient FL baselines in medical image segmentation tasks.

Conclusion: Structural separation with subspace orthogonality regularization effectively handles heterogeneous federated medical segmentation under LoRA's bilinear parameterization.
...
### [101] [PRPO: Perception-Reinforced Policy Optimization via Token-Level Dynamic Advantage Reshaping](https://arxiv.org/abs/2606.08708)
*Qiming Li,Tianlun Li,Xiaolong Cheng,Hangyu Li,Ruiyan Gong,Kangning Niu,Kaitao Jiang,Mu Xu*

Main category: cs.CV

TL;DR: PRPO is a token-level RL framework that identifies and reinforces perceptually important tokens in multimodal reasoning, outperforming baselines by 23.3% (3B) and 21.1% (7B) on average across seven benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing RLVR methods use coarse-grained trajectory-level rewards that assign identical signals to all tokens, failing to properly supervise the sparse subset of causally grounded perceptual tokens in multimodal reasoning.

Method: PRPO introduces Robust Visual Dependency (RVD) to identify visually grounded, perturbation-stable tokens, and Perceptual Advantage Reshaping (PAR) for token-level credit assignment that amplifies perceptual tokens while maintaining stable gradients for others.

Result: PRPO achieves state-of-the-art performance on seven multimodal reasoning benchmarks, with average gains of 23.3% (3B) and 21.1% (7B) over strong LVLM baselines, with improved training efficiency and cross-task generalization.

Conclusion: Fine-grained credit assignment is crucial for scalable multimodal reinforcement learning, and identifying perceptually pivotal tokens significantly improves reasoning capability.
...
### [102] [Thinking Without Images: Internalizing Visual Manipulation with On-Policy Self-Distillation](https://arxiv.org/abs/2606.08719)
...
TL;DR: The paper proposes Imagine-OPD, an on-policy self-distillation framework where a teacher model uses actual zoomed-in visual evidence to supervise a student model's internal "imagined" visual reasoning process, achieving competitive accuracy with lower inference cost compared to explicit "Thinking with Images" methods.


<details>
  <summary>Details</summary>
Motivation: Current "Thinking with Images" methods improve fine-grained visual reasoning by zooming into image regions, but incur high inference costs (redundant tool invocations, longer traces) and often produce noisy intermediate crops. The authors ask whether these benefits can be internalized through imagination rather than actual tool use.

Method: Imagine-OPD: An on-policy self-distillation framework. During training, a teacher model receives privileged access to annotated zoomed-in region evidence (acting as "Thinking with Images"). It supervises the student model to learn its own "imagination" reasoning trajectories — internal decisions about where to look and what visual cues would be found — without requiring actual tool invocation at inference time.

Result: Imagine-OPD achieves the best average performance among compared models on vision-centric benchmarks, while significantly reducing inference overhead compared to explicit "Thinking with Images" methods. No external teacher or high-quality imagination demonstrations are required.

Conclusion: The reasoning benefits of explicit visual zooming ("Thinking with Images") can be effectively internalized via imagination-based training. The self-distillation setup (teacher with privileged annotations supervising student imagination) is effective without external supervision, offering a practical trade-off between accuracy and efficiency.
...
### [103] [AUCp: Pseudo-AUC for Inference Model Selection with Unlabeled Validation Data in Abnormality Detection](https://arxiv.org/abs/2606.08742)
*Md Mahfuzur Rahman Siddiquee,Fazle Rafsani,Jay Shah,Teresa Wu,Catherine D Chong,Todd J Schwedt,Baoxin Li*

Main category: cs.CV

TL;DR: The paper proposes AUCp, a novel metric for unsupervised/self-supervised abnormality detection that selects the best model without requiring labeled validation data, by assuming all test samples as pseudo-positive and using traditional AUC calculation.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised abnormality detection methods often rely on labeled validation sets for model selection, but labeled data is frequently unavailable and time-consuming to obtain. This creates a practical gap between unsupervised training and supervised model selection.

Method: AUCp treats all unannotated test samples as pseudo-positive (abnormal) and uses standard AUC calculation for model selection, assuming a large and representative normal training set. Mathematical and empirical validation is provided.

Result: AUCp outperforms conventional metrics (e.g., reconstruction realism) for model selection in both unsupervised and self-supervised abnormality detection across neurologic disease detection and diverse datasets.

Conclusion: AUCp is an effective, label-free metric for selecting the optimal model for inference in abnormality detection tasks, eliminating the need for annotated validation sets.
...
### [104] [MB-Loc: Multi-planar Bird's-eye-view Localization in outdoor LiDAR scenes](https://arxiv.org/abs/2606.08744)
*Ayaan Choudhury,Preet Savalia,Anirudh Pydah,Avinash Sharma*

Main category: cs.CV

TL;DR: MB-Loc is a lightweight LiDAR localization framework that projects 3D point clouds into 2.5D multi-planar BEV representations, uses KL-regularized latent bottlenecks for spatial uncertainty, and applies 3D augmentations for viewpoint robustness, achieving real-time state-of-the-art performance on the NCLT dataset.


<details>
  <summary>Details</summary>
Motivation: Existing SCR-based LiDAR localization methods suffer from computational inefficiency due to heavy 3D convolutions on raw point clouds, and performance degrades significantly under varying sensor viewpoints.

Method: (1) Projects LiDAR scans into 2.5D multi-planar BEV representation by slicing along Z-axis; (2) Uses standard 2D CNNs instead of heavy 3D convolutions; (3) Introduces KL-regularized latent bottleneck for spatial uncertainty modeling; (4) Applies 3D spatial augmentations before projection for rotation robustness.

Result: Outperforms current state-of-the-art on NCLT dataset, operates at real-time inference speeds, and significantly surpasses traditional 3D-SCR architectures in computational efficiency.

Conclusion: The 2.5D BEV projection combined with KL-regularized uncertainty modeling and viewpoint augmentation effectively addresses the efficiency and robustness bottlenecks of 3D-SCR methods.
...
### [105] [Stain-Aware Wavelet Regularization for Instant Adversarial Purification in Histopathology](https://arxiv.org/abs/2606.08745)
*Zhe Li,Bernhard Kainz*

Main category: cs.CV

TL;DR: SAWR uses Haar wavelet transform and stain-aware regularization to purify adversarial perturbations from histopathology images, improving robustness by up to 10.69% while preserving diagnostic tissue structures.


<details>
  <summary>Details</summary>
Motivation: Neural networks in computational pathology are vulnerable to adversarial attacks, and histopathological images are especially challenging because high-frequency adversarial noise can be easily confused with subtle, diagnostically relevant tissue structures.

Method: SAWR applies multi-level wavelet-domain regularization via Haar transform to hierarchically separate adversarial noise from diagnostic information, then extends this spectral constraint to individual H&E stain channels for stain-specific frequency regulation.

Result: SAWR improves adversarial robustness by up to 10.69% over the baseline while maintaining texture and spectral fidelity under adversarial perturbations.

Conclusion: Stain-aware wavelet regularization is an effective adversarial purification approach for histopathology, balancing robustness with preservation of diagnostically relevant image features.
...
### [106] [Less Is More: Training-Free Acceleration Framework of 3D Diffusion Models for Low-Count PET Denoising via Global-Local Trajectory Reduction](https://arxiv.org/abs/2606.08751)
*Yuhan Liu,Scott M. Leonard,Marlee Crews,Muhannad Fadhel,Jinkui Hao,Tianqi Chen,Ryan J. Avery,Bo Zhou*

Main category: cs.CV

TL;DR: This paper proposes a training-free Global-Local Skipping Strategy to accelerate diffusion model-based 3D PET denoising for low-count PET reconstruction. It uses global denoising step skipping (starting from an intermediate step) and local feature reuse shortcuts across neighboring steps, achieving >10x speedup with improved or comparable quality across multiple PET tracers.


<details>
  <summary>Details</summary>
Motivation: Low-count PET protocols reduce radiation dose and acquisition time, but restoration to high-count quality is challenging. Diffusion models show promise but are computationally prohibitive for 3D volumes due to iterative sampling, limiting clinical deployment.

Method: A training-free plug-and-play Global-Local Skipping Strategy with two components: (i) global denoising step skipping — initializing from an intermediate step via noise-consistent transformation of low-count input; (ii) local feature reuse shortcut — reusing slowly-varying high-level U-Net features across neighboring steps to reduce per-step computation.

Result: Evaluated on 18F-FDG, 68Ga-DOTATATE, and 18F-PSMA PET datasets, achieving over an order of magnitude acceleration with improved or comparable reconstruction quality versus full-step baseline. Blinded reader studies confirmed enhanced clinical confidence and diagnostic quality.

Conclusion: The proposed acceleration strategy enables practical clinical deployment of diffusion-based low-count PET denoising without retraining, reducing inference latency while maintaining or improving image quality across multiple tracers.
...
### [107] [Beyond Consistency: Preserving Temporal Structure in Zero-Shot Video Editing](https://arxiv.org/abs/2606.08780)
*Deyin Liu,Yisheng Ding,Zhe Jin,Xiatian Zhu,Anjan Dutta,Lin Wu*

Main category: cs.CV

TL;DR: This paper introduces a zero-shot video editing method that preserves the source video's temporal structure (narrative/rhythm/semantic flow) by adaptively partitioning videos into semantically distinct clips, using anchor frames and clip-adaptive token merging, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot video editing methods achieve spatial control and basic temporal consistency but fail to preserve the video's original temporal structure (high-level narrative, rhythm, semantic flow), leading to narratively incoherent and semantically ambiguous outputs, especially for long videos with complex semantic variations.

Method: (1) Adaptive partitioning of videos into semantically distinct clips based on feature similarity. (2) Selection of a representative anchor frame for each clip. (3) Clip-adaptive token merging strategy leveraging anchor's semantic dominance for intra-clip fidelity and efficiency. (4) Alternating combination strategy for seamless inter-clip transitions while maintaining semantic distinction.

Result: State-of-the-art results, successfully balancing preservation of original temporal structure with computational efficiency, setting a new benchmark for zero-shot video editing fidelity.

Conclusion: The proposed method effectively addresses the critical limitation of preserving temporal structure in zero-shot video editing, achieving superior fidelity and coherence compared to existing approaches.
...
### [108] [DeepMine-Mamba: Mitigating Information Dilution in Mamba-Based State Space Models for Document Image Binarization](https://arxiv.org/abs/2606.08781)
*Sheng-Wei Chan,Yung-Che Wang,Hsin-Jui Pan,Chia-Min Lin,Jen-Shiun Chiang*

Main category: cs.CV

TL;DR: DeepMine-Mamba is a Mamba-based document image binarization framework with an Anti-Dilution Gate that prevents weak foreground cues from being diluted during long-range feature propagation, achieving competitive performance on DIBCO benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for document binarization use CNNs, transformers, or GANs, but Mamba-based state space models remain unexplored. Direct Mamba propagation can dilute weak foreground cues like faint ink, fragmented characters, and stroke details.

Method: Proposes DeepMine-Mamba, a Mamba-based binarization framework with an Anti-Dilution Gate that estimates feature changes from propagation and selectively restores stroke-sensitive local responses while suppressing unnecessary background enhancement.

Result: Achieves competitive overall performance on DIBCO/H-DIBCO benchmarks under a strict leave-one-year-out protocol, with strong average FM and Fps scores. Ablations show the Anti-Dilution Gate improves stroke preservation and reduces binarization errors.

Conclusion: Mamba-based architectures with anti-dilution mechanisms are effective for document binarization, particularly for preserving weak foreground cues.
...
### [109] [MaskAlign: Token-Subset Representation Alignment for Efficient Diffusion Training](https://arxiv.org/abs/2606.08788)
*Lianyu Pang,Tianlin Pan,Cheng Da,Changqian Yu,Huan Yang,Kun Gai,Song Guo,Wenhan Luo*

Main category: cs.CV

TL;DR: MaskAlign improves diffusion transformer training by randomly aligning only subsets of tokens with clean-image features, reducing dependence on complete token sets while using a pre-mask mixing block to compensate for information loss.


<details>
  <summary>Details</summary>
Motivation: Existing representation alignment methods between noisy diffusion features and clean-image features suffer from a mismatch due to varying noise levels across timesteps. The authors discover that full-token alignment creates uneven gradient norms with stable spatial preferences, indicating the model becomes overly reliant on complete clean-image token sets.

Method: MaskAlign applies representation alignment only on randomly sampled token subsets during training, breaking the dependence on complete token sets. A lightweight pre-mask token mixing block is added before masking to share information across tokens and mitigate information loss from dropped tokens.

Result: The token-level analysis reveals that full-token alignment produces uneven gradient norms across tokens with consistent spatial patterns. MaskAlign reduces reliance on complete token sets and achieves more stable alignment behavior under token-subset perturbations.

Conclusion: Token-subset alignment (MaskAlign) with pre-mask mixing offers an effective alternative to full-token alignment for better diffusion transformer training.
...
### [110] [PairWise Image Finder: An Open-source Tool for Finding Visually Aligned Street-Level Image Pairs for Urban Perception Studies](https://arxiv.org/abs/2606.08795)
*Jussi Torkko*

Main category: cs.CV

TL;DR: This paper presents PairWise image finder, a tool that integrates feature detection, matching, and semantic segmentation to find visually aligned image pairs from Street View Imagery across different time periods, enabling accurate longitudinal change analysis.


<details>
  <summary>Details</summary>
Motivation: Metadata alone is insufficient to reliably find visually aligned image pairs from Street View Imagery for change detection and scene recognition across years.

Method: The tool integrates feature detection and matching with semantic segmentation masks to quantify visual alignment, outputting share of matched key features, matched feature distance/coverage, and semantic mask alignment.

Result: The tool enables users to filter image pairs based on alignment quality and use case, demonstrated through comparison of longitudinal changes highlighting the importance of perspective.

Conclusion: The proposed method provides a scalable and open tool for researchers to find high-quality image pairs for urban analysis, perception, and related applications, reducing manual effort.
...
### [111] [Classifying galaxies in the Galaxy10 DECals dataset using Inception and Residual CNNs](https://arxiv.org/abs/2606.08826)
*Lanz Anthonee A. Lagman,Prospero C. Naval,Reinabelle C. Reyes*

Main category: cs.CV

TL;DR: This paper compares ResNet101 and InceptionV4 on the Galaxy10 DECals dataset, finding both achieve ~90% accuracy with ResNet101 slightly outperforming InceptionV4, suggesting both are cost-effective foundations for galaxy classification pipelines.


<details>
  <summary>Details</summary>
Motivation: With the expected increase in quantity and quality of galactic morphology image data, it is important to explore which deep learning architectures are cost-effective for classification tasks.

Method: The authors analyze ResNet101 and InceptionV4 on a spatially-augmented Galaxy10 DECals dataset, retaining ten-class classification while modifying image counts per class.

Result: Both models achieved ~90% accuracy; ResNet101 outperformed InceptionV4 across performance metrics.

Conclusion: Either CNN architecture could serve as a robust foundation for specialized pipelines for galaxy image classification from upcoming surveys.
...
### [112] [CSFlow: Aligning Flow Matching with Human Contrast Sensitivity](https://arxiv.org/abs/2606.08833)
*Malgorzata Galinska,Bart Pogodzinski,Jan Eric Lenssen*

Main category: cs.CV

TL;DR: CSFlow introduces a weighting scheme that aligns the iterative denoising steps of flow matching with the human Contrast Sensitivity Function (CSF), improving image generation quality by prioritizing frequencies that human vision is most sensitive to at appropriate timesteps.


<details>
  <summary>Details</summary>
Motivation: Real-world images have uneven frequency distribution (low frequencies dominate early in diffusion), and the human visual system has unequal sensitivity to different spatial frequencies. The paper aims to merge these two observations to improve generative model performance.

Method: (1) A metric estimating which frequencies are generated at each reverse flow interval; (2) Timestep weights derived by aligning generated frequencies with human contrast sensitivity (CSF). Applied via inference-only timestep modification or short fine-tuning.

Result: FID reduced by 4.7%, Inception Score increased by 2.2%, GenEval scores improved by 2.5%. Qualitatively, images appear more realistic and less cartoonish.

Conclusion: Aligning diffusion timesteps with human contrast sensitivity is a simple yet effective way to improve generative quality, suggesting that perceptual priors can guide generative modeling.
...
### [113] [Geometry-Aware Fisheye-LiDAR Fusion for Robust 3D Object Detection in Low-Overlap Setups](https://arxiv.org/abs/2606.08844)
*Xiangzhong Liu,Xihao Wang,Hao Shen*

Main category: cs.CV

TL;DR: GA-HF is a novel fusion framework for dual-fisheye + LiDAR setups that uses distortion-aware polar BEV grids and dual-attention warping to significantly improve 3D object detection over Cartesian baselines across three benchmarks.


<details>
  <summary>Details</summary>
Motivation: Autonomous systems are shifting to cost-effective sparse-view sensor setups (dual-fisheye + LiDAR), but existing BEV fusion methods force fisheye images into Cartesian grids early on, causing severe feature distortion and information loss due to extreme radial distortion, minimal overlap, and spherical-to-rectilinear misalignment.

Method: GA-HF employs a Distortion-Aware Lift-Splat-Shoot (LSS) module that lifts fisheye features into a polar BEV grid to preserve native angular density, while LiDAR features remain in Cartesian space. A Dual-Attention Warping Correction module applies spatial and channel attention to suppress artifacts in low-quality peripheral regions before fusion.

Result: On KITTI-360, GA-HF improves NDS by 4.2% over Cartesian baselines; on Dur360BEV, it surpasses LiDAR-only and BEVFusion with reduced orientation error; on Fisheye3DOD, it achieves the highest detection score among all fusion methods.

Conclusion: GA-HF is the first approach to explore LiDAR-fisheye camera fusion, effectively addressing geometric challenges in sparse-view setups and demonstrating consistent improvements across multiple benchmarks.
...
### [114] [BLM-SGAN: Bidirectional Language Modeling for Semantic-Spatial Text-to-Image Generation](https://arxiv.org/abs/2606.08847)
*Ahmed Abdelmoneim Mazrou,Haidy Maher El-Amir,Ali Hamdi*

Main category: cs.CV

TL;DR: BLM-SGAN is a novel GAN-based text-to-image model that uses BERT's bidirectional attention mechanisms to overcome challenges like vanishing gradients and poor long-range dependency capture, achieving state-of-the-art results on bird image generation (IS 5.45 ± 0.08).


<details>
  <summary>Details</summary>
Motivation: Existing GAN-based T2I models struggle with capturing long-range dependencies, vanishing gradients, and limitations of sequential processing, motivating the need for a model with better contextual understanding.

Method: BLM-SGAN incorporates Bidirectional Language Modeling via BERT's attention mechanisms to capture rich contextual information and efficiently manage extended sequences for semantic-spatial text-to-image generation.

Result: Achieves Inception Score of 5.45 ± 0.08, outperforming SSA-GAN, DF-GAN, SD-GAN, and AttnGAN on bird image generation.

Conclusion: BLM-SGAN effectively generates highly realistic images from detailed text descriptions, demonstrating the value of bidirectional language modeling in T2I synthesis.
...
### [115] [Vision-Language Work Zone Intelligence for Safety-Critical Speed Regulation of Mixed-Autonomy Vehicles in Dynamic Environments](https://arxiv.org/abs/2606.08860)
...
TL;DR: This paper presents a real-time, onboard perception system that detects active work zones and recognizes temporary speed limits using low-cost embedded hardware, achieving high recall for work-zone event detection (96.5%) and 95.45% precision for speed-limit recognition, offering a scalable alternative to map-based or infrastructure-dependent approaches.


<details>
  <summary>Details</summary>
Motivation: Temporary work-zone speed limits are communicated through visually inconsistent signage and are often missing from digital maps, creating safety risks for both human drivers and automated vehicle systems.

Method: The system fuses object detections with semantic verification and temporally smoothed, hysteresis-based state transitions to reduce false activations and flicker in dynamic scenes, running fully on low-cost embedded hardware.

Result: On the ROADWork dataset (490 sequences): inside-work-zone event-level recall of 96.5% and precision of 68.7%. On 35 minutes of in-house driving data: speed-limit recognition precision of 95.45% and recall of 53.85%, with no incorrect speed classifications and a single false positive.

Conclusion: The system demonstrates a practical, scalable approach for grounding work-zone speed awareness directly in onboard perception rather than relying on maps or infrastructure.
...
### [116] [CHROMA: Detecting AI-Generated Images through Inter-Channel Color-Space Correlations](https://arxiv.org/abs/2606.08864)
*Juan Pablo Sotelo,Marina Gardella,Pablo Musé*

Main category: cs.CV

TL;DR: This paper introduces Chroma, a detector of AI-generated images that uses inter-channel color correlation features to distinguish synthetic from real photographs. It shows that cross-channel statistics are inconsistently constrained by perceptual training objectives, and that RGB and Lab color spaces effectively separate real and generated images. Chroma augments RGB inputs with correlation maps using a fixed CNN backbone, achieving competitive performance with simple architecture and training.


<details>
  <summary>Details</summary>
Motivation: Existing automated detectors of AI-generated images struggle to generalize to unseen generators. The authors aim to address this limitation by exploring inter-channel color correlations as a lightweight, underexploited forensic cue that can improve robustness and discrimination between real and synthetic images.

Method: The authors first analyze LPIPS perceptual metric inconsistencies under perturbations affecting cross-channel statistics. Then they study pairwise inter-channel correlation distributions across multiple color spaces. Based on these findings, they propose Chroma, which augments standard RGB inputs with inter-channel correlation maps and uses a fixed CNN backbone trained under a modest computational budget.

Result: Correlation-augmented inputs improve real-vs-generated discrimination and robustness. Chroma achieves performance competitive with recent detectors under both single-generator training and limited multi-generator supervision, while maintaining a simple architecture and straightforward training procedure.

Conclusion: Inter-channel color correlations serve as a simple yet effective forensic cue for detecting AI-generated images. The proposed Chroma detector demonstrates that leveraging this cue can yield competitive detection performance with minimal architectural complexity and computational requirements.
...
### [117] [Generalizing Geometry-Guided Mamba as a Plug-and-Play Context Module for CNN-based Semantic Segmentation](https://arxiv.org/abs/2606.08866)
*Sheng-Wei Chan,Hsin-Jui Pan,Chun-Po Shen,Chia-Min Lin,Yung-Che Wang,Jen-Shiun Chiang*

Main category: cs.CV

TL;DR: This paper proposes using Directional Geometric Mamba (G-Mamba) as a plug-and-play context aggregation module to replace conventional context heads (ASPP, PPM, attention) in CNN-based semantic segmentation, achieving consistent mIoU gains on Cityscapes with moderate computational cost.


<details>
  <summary>Details</summary>
Motivation: CNN-based semantic segmentation networks rely on context heads (e.g., ASPP, PPM, attention) to enlarge receptive fields, but these heads introduce heavy computation, memory cost, or boundary leakage issues.

Method: Inject geometric guidance into the selective scan process, enabling long-range feature propagation modulated by boundary and centripetal-flow cues. Replace original context heads of six CNN models (DeepLabV3+, DANet, CCNet, PSPNet, PSANet, OCRNet) with G-Mamba while keeping ResNet-101 backbone unchanged.

Result: Consistent mIoU gains across all six models on Cityscapes with only moderate extra GFLOPs at 1024×1024 resolution.

Conclusion: Geometry-guided SSM modules can serve as practical alternatives or enhancements to conventional CNN context heads for semantic segmentation.
...
### [118] [Are Reasoning Vision-Language Models Robust to Semantic Visual Distractions?](https://arxiv.org/abs/2606.08894)
...
TL;DR: Distract-Bench is a benchmark evaluating VLM robustness to semantic visual distractions (meaningful but task-irrelevant cues) revealing that reasoning VLMs are less robust to distractions than to conventional vision corruptions, often incorporating distractions into their reasoning and producing incorrect answers.


<details>
  <summary>Details</summary>
Motivation: Existing VLM reliability evaluations focus on input corruptions (noise, blur, weather effects) that degrade perception, but overlook a critical failure mode: models may perceive correctly yet reason from plausible but irrelevant/distracting visual evidence, propagating mistakes to final answers.

Method: The authors propose Distract-Bench, a benchmark of semantic visual distractions (meaningful but task-irrelevant cues) that preserve ground-truth answers. They evaluate eight open-source and two closed-source VLMs across both conventional vision corruptions and Distract-Bench, comparing reasoning VLMs against their non-reasoning base models.

Result: Distract-Bench exposes a distinct robustness failure: reasoning VLMs largely track base model performance under perceptual degradation but show consistently lower robustness to semantic distractions. Distractions often enter the reasoning process, are treated as evidence, and lead to incorrect answers.

Conclusion: The findings reframe robustness evaluation for reasoning VLMs, shifting focus from degraded perception to distractions as the key challenge for reliable real-world visual reasoning.
...
### [119] [A multi-agent system for spine MRI report generation from multi-sequence imaging](https://arxiv.org/abs/2606.08897)
...
TL;DR: SpineAgent is a multi-agent framework for spine MRI report generation, using a multi-sequence foundation model trained on 32K+ patients and 13M+ slices. It features separate DINOv3-based encoders for T1/T2 sequences, continual training for other sequences, and achieves state-of-the-art performance in report generation, pathology localization, and multimodal retrieval, validated by five radiologists.


<details>
  <summary>Details</summary>
Motivation: Spine pathology is a leading cause of pain and disability; spine MRI interpretation is complex and time-consuming, and effectively combining multi-sequence data while preserving sequence-specific diagnostic information remains an open challenge.

Method: Pre-trains two DINOv3-based encoders on T1/T2 sequences; introduces continual training to learn a synthesizer embedding other sequences; produces patient-level embeddings integrating multi-sequence signals; integrates capabilities into 37 specialized agents; outputs structured tokens fed into a Medical Report Agent trained end-to-end for report generation.

Result: Achieves state-of-the-art performance in spine MRI report generation; demonstrates strong generalizability under cross-manufacturer and cross-cohort evaluation; enables pathology localization and multimodal image-report retrieval; leading performance validated by both automated metrics and five radiologists.

Conclusion: SpineAgent provides a scalable, explainable, and generalizable foundation for automated spine MRI report generation using multi-agent architecture and multi-sequence embedding.
...
### [120] [DifferSeg: Towards Diverse Multimodal Binary Segmentation via Differential Perception and Frequency Guidance](https://arxiv.org/abs/2606.08906)
*Qiangqiang Zhou,Jiawei Xu,Yong Chen,Dandan Zhu,Yugen Yi,Xiaoqi Zhao*

Main category: cs.CV

TL;DR: DifferSeg is a simple yet general multimodal binary segmentation framework that uses differential perception fusion (DPF) for adaptive cross-modal alignment and a frequency-guided decoder (FGD) to balance high/low-frequency representations, surpassing 67 SOTA methods across 29 datasets and 18 tasks.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal binary segmentation methods rely on fixed feature concatenation and straightforward decoders dominated by low-frequency semantics, failing to handle modality discrepancies/complementarity and lacking efficient decoding strategies that balance high- and low-frequency representations.

Method: DifferSeg introduces two key modules: (1) Differential Perception Fusion (DPF) using learnable differential operators to adaptively align multimodal features and enhance complementarity via residual fusion; (2) Frequency-Guided Decoder (FGD) that builds cross-frequency interactions and multi-path upsampling to maintain consistency between detailed high-frequency structures and semantic low-frequency representations.

Result: Consistently surpasses 67 state-of-the-art methods across 29 public datasets involving 18 downstream tasks, spanning both natural and medical modalities.

Conclusion: DifferSeg is a general, plug-and-play framework that achieves superior generalization and segmentation accuracy across diverse binary segmentation tasks without bells and whistles.
...
### [121] [Failure-Aware Refinement of Vision-Language Model for Lithography Defect Detection](https://arxiv.org/abs/2606.08908)
*Pangyun Jeong,Jiyeong Kong,Yuehua Hu,Dohee Jeong,Kyung-Tae Kang*

Main category: cs.CV

TL;DR: A two-stage vision-language framework for semiconductor lithography defect detection, where Qwen3-VL is first fine-tuned with LoRA, then a refinement module corrects its errors using failure cases.


<details>
  <summary>Details</summary>
Motivation: Semiconductor lithography inspection requires reliable detection of small pattern defects (bridge, burr, pinch, contamination), which is challenging and prone to errors in single-stage fine-tuning.

Method: Two-stage vision-language framework: Stage 1 fine-tunes Qwen3-VL with LoRA to predict defect counts, categories, and bounding boxes; Stage 2 trains a refinement module using first-stage prediction failures and corrected labels to review and revise initial outputs.

Result: The refinement module, trained on first-stage failure cases, improves defect inference by correcting false positives, missed defects, and incorrect defect types beyond single-stage fine-tuning.

Conclusion: Learning from initial adapter failures through a refinement stage enhances defect detection reliability, outperforming single-stage fine-tuning alone.
...
### [122] [When Vision Misleads, Let Location Speak: A Worldwide Image Geo-Localization Method via Location Attention Mechanism and Large Multimodal Models](https://arxiv.org/abs/2606.08918)
*Junchao Cui,Wenqi Shi,Xuanzi Ma,Nan Wu,Shaoyong Du,Xiangyang Luo*

Main category: cs.CV

TL;DR: TransGeoCLIP proposes a retrieval-based framework combining location attention mechanisms and large multimodal models to improve worldwide image geo-localization, achieving state-of-the-art results by better distinguishing geographically distinct but visually similar scenes.


<details>
  <summary>Details</summary>
Motivation: Existing image geo-localization methods often mislocalize images by matching them to visually similar scenes from different geographic regions, limiting their reliability in practical applications.

Method: A two-stage retrieval framework: 1) Database construction using Transformer encoder with location attention to encode GPS coordinates, enabling joint image-text-GPS embedding via CLIP; 2) Retrieval-augmented inference leveraging LMMs to predict final image location from retrieved results.

Result: Street-level accuracy (within 1km error) improved by 1.5%, 1.07%, 7.18%, and 9.75% on IM2GPS, IM2GPS3k, YFCC4k, and YFCC26k benchmarks respectively over state-of-the-art methods.

Conclusion: TransGeoCLIP significantly enhances localization performance for visually similar images, effectively distinguishing geographic features that prior methods confuse.
...
### [123] [PolyBuild: An End-to-End Method for Polygonal Building Contour Extraction from High-Resolution Remote Sensing Images](https://arxiv.org/abs/2606.08920)
*Yaoteng Zhang,Julin Zhang,Guangshuai Wang,Jiwei Deng,Hui Sheng,Yasir Muhammad,Shiqing Wei*

Main category: cs.CV

TL;DR: PolyBuild is an end-to-end method that directly extracts building vector polygons from high-resolution remote sensing images using an Initial Contour Generation Module (ICGM) and a Contour Optimization Module (COM), outperforming state-of-the-art methods on three datasets.


<details>
  <summary>Details</summary>
Motivation: Existing building extraction methods rely on pixel-level segmentation followed by multiple post-processing steps to produce building contours, which is computationally intensive and error-prone. This paper aims to address these limitations.

Method: PolyBuild uses an Initial Contour Generation Module (ICGM) that generates initial building contours via sub-region center features and bounding boxes, and a Contour Optimization Module (COM) that refines contours using a hybrid CNN-Transformer decoder integrating CNN features and contour positional information.

Result: Extensive experiments on three building datasets show PolyBuild significantly outperforms state-of-the-art mask-based and contour-based approaches.

Conclusion: The proposed end-to-end PolyBuild method effectively extracts building vector polygons without post-processing, achieving superior performance through its hybrid CNN-Transformer contour optimization architecture.
...
### [124] [NutriMLLM: Multimodal Large Language Models for Dietary Micronutrient Analysis](https://arxiv.org/abs/2606.08948)
*Runze Yan,Minxiao Wang,Jiaying Lu,Darren Liu,Xiao Hu,Hanqi Luo*

Main category: cs.CV

TL;DR: The paper presents NutriMLLM, the first vision-language model family specialized in estimating 65 dietary micronutrients from food images. By repurposing a decade of 24-hour dietary recalls as structured prompts for text-to-image generation, the authors created a synthetic corpus of ~1.1 million image-description-nutrient triplets. Fine-tuning Qwen3-VL and GLM models on this data produced models that achieve near-complete nutrient coverage on real food images, matching or surpassing proprietary models like GPT-5 and Gemini 3.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs (including GPT-5, Gemini 3, Claude Sonnet 4.5) are unreliable for micronutrient estimation—they frequently abstain or return statistically implausible values. Manually annotating large multimodal datasets with complete 65-nutrient profiles is prohibitively expensive.

Method: 1) Synthesized ~1.1M image-description-nutrient triplets using recall-driven text-to-image generation from population-scale 24-hour dietary recalls. 2) Fine-tuned Qwen3-VL (2B/4B/8B/30B) and GLM-4.6V-Flash on this corpus to produce NutriMLLM. 3) Evaluated with a four-component framework measuring abstention, hallucination, usability, and per-nutrient accuracy across four benchmarks (ASA24, SNAPMe, FNDDS, NutriBench).

Result: Every NutriMLLM variant achieved near-complete coverage across all 65 nutrients on real food images. The largest variant matched or exceeded proprietary baselines (GPT-5, Gemini 3, Claude Sonnet 4.5) in accuracy on most nutrients.

Conclusion: Recall-driven synthetic supervision makes image-based comprehensive micronutrient estimation a tractable engineering problem, supporting dietary assessment, personalized nutrition guidance, and population-scale micronutrient surveillance.
...
### [125] [Rethinking 3D Shape Generation: Diffusion over Superquadrics](https://arxiv.org/abs/2606.08957)
*Zhiyang Liu,Wanze Li,Yuwei Wu,Chengran Yuan,Jiawei Sun,Rui Zheng,Marcelo H Ang*

Main category: cs.CV

TL;DR: This paper proposes replacing dense 3D geometry representations (voxels, meshes, point clouds) with compact superquadric primitives for diffusion-based 3D shape generation, reducing state dimensionality from millions to ~7KB parameters, enabling faster generation (0.6s/shape), resolution-free decoding, part-level editing, and constraint-based design.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models for 3D shapes operate on high-cardinality representations (voxel/SDF grids, meshes, point clouds), which are computationally and memory intensive, limiting scalability in resolution and controllability.

Method: The authors replace dense 3D geometry with compact superquadric primitives (pose, size, shape parameters ≈7KB), performing diffusion directly on these low-dimensional parameters rather than on millions of geometric values.

Result: The method achieves competitive surface fidelity and distributional performance on standard benchmarks, supports resolution-free decoding, part-level editing, and constraint-based design, and generates shapes in ~0.6s per shape under most conditions.

Conclusion: Moving diffusion from dense geometry to compact superquadric primitives improves scalability, efficiency, and controllability for 3D shape generation while maintaining quality.
...
### [126] [ChinaHeritaQA: A Culturally-Grounded Visual Question Answering Dataset for World Heritage Sites in China](https://arxiv.org/abs/2606.08959)
*Yi Zhang,Bolei Ma,Yong Cao,Chengyan Wu,Daniel Hershcovich,Anna-Carolina Haensch*

Main category: cs.CV

TL;DR: ChinaHeritaQA is a multimodal benchmark of 2,279 images and 14,133 bilingual QA pairs about Chinese UNESCO World Heritage sites. It tests VLMs across seven cognitive dimensions. Current VLMs beat humans on average but struggle with culturally grounded reasoning, revealing a gap between visual recognition and cultural understanding.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks lack focus on culturally grounded reasoning for non-Western heritage. The authors aim to evaluate whether VLMs truly understand cultural context beyond visual pattern matching.

Method: The dataset includes 2,279 images of Chinese UNESCO sites with 14,133 bilingual (Chinese/English) multiple-choice questions across seven cognitive dimensions. It uses a UNESCO-aligned heritage ontology with rigorous human annotation for quality control.

Result: Top VLMs exceed human average performance but show task-level variation—strong on visual recognition, weak on cultural reasoning. Performance varies by dynasty and region.

Conclusion: Visual retrieval capabilities do not translate to cultural and historical understanding. The dataset is released to promote culturally aware multimodal learning.
...
### [127] [EPS3D: End-to-End Feed-Forward 3D Panoptic Segmentation](https://arxiv.org/abs/2606.08980)
...
TL;DR: EPS3D is an end-to-end feed-forward framework for open-vocabulary 3D panoptic segmentation that uses multi-view images, distillation-based training, and a mutual enhancement module to achieve state-of-the-art results with high efficiency (1s per scene).


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on additional preprocessing steps, which leads to error accumulation and poor 3D consistency. The authors aim to design an end-to-end solution that avoids these issues.

Method: 1) End-to-end architecture with distillation-based training on diverse 3D scenes. 2) Mutual enhancement module with Ins2Sem (aligning semantics within instances) and Sem2Ins (refining instance features with semantic guidance) for inherent semantic-instance consistency.

Result: Outperforms SOTA baselines: +13% mIoU for semantics on Replica benchmark, processes 1 scene per second.

Conclusion: EPS3D achieves efficient and coherent 3D panoptic segmentation, supporting downstream applications like robotic manipulation and 3D scene editing.
...
### [128] [Scaling by Diversified Experience for Vision-Language-Action Models](https://arxiv.org/abs/2606.09009)
*Leiyu Wang,Zhaofengnian Wang,Xueqi Li,Luoyi Fan,Cewu Lu,Nanyang Ye*

Main category: cs.CV

TL;DR: SyVLA is a robust Vision-Language-Action model that uses Intention Decoupling to separate control features from reasoning and a similar-sample guided RL pipeline to stabilize training, achieving superior performance in real-world robotic tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models suffer from entanglement of high-level reasoning with low-level control and instability in policy optimization during real-world deployment.

Method: (1) Intention Decoupling algorithm to isolate control-relevant features from reasoning contexts; (2) Similar-sample guided RL pipeline to stabilize policy updates and mitigate distribution shift.

Result: Achieves superior task success rates and stronger out-of-distribution generalization, while preserving core vision-language capabilities.

Conclusion: SyVLA effectively addresses key VLA challenges and outperforms existing methods in real-world robotic tasks and multi-modal benchmarks.
...
### [129] [ATM: Action-Consistency Transfer Matrix for Diagnosing and Improving Latent World Models](https://arxiv.org/abs/2606.09028)
*Jiaheng Chen*

Main category: cs.CV

TL;DR: ATM is a lightweight diagnostic method using Action-Consistency Transfer Matrix to evaluate latent world models for planning, achieving 100x speedup over traditional CEM-based evaluation.


<details>
  <summary>Details</summary>
Motivation: Traditional evaluation of latent world models for planning requires slow simulator rollouts with planners like CEM, which can take minutes to hours per checkpoint and is black-box, making it impractical for rapid diagnosis.

Method: ATM compares action information between real encoded transitions and model-predicted transitions using lightweight post-hoc probes, producing an interpretable matrix. It can be collapsed into a screening score for ranking. AITS uses action-identifiability as a training signal.

Result: ATM reduces minutes-to-hours CEM evaluation to seconds-level transition analysis (100x speedup) while achieving highly reliable pairwise ranking when the true success gap is non-trivial.

Conclusion: Action-consistency diagnostic provides efficient evaluation of latent world models and action-identifiability can serve as a useful training signal for improving downstream planning.
...
### [130] [Frequency Decoupled Framework for Screen Content Image Super-Resolution](https://arxiv.org/abs/2606.09029)
*Xufei Wang,Qicheng Zhang,Qi Wu,Ziyang Gu,Shizhuang Weng*

Main category: cs.CV

TL;DR: The paper proposes a frequency decoupled framework (FDF) for Screen Content Image Super-Resolution that separately processes amplitude (periodic patterns) and phase (coherent context) information using specialized networks, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing implicit neural representation methods for SCISR overlook inherent frequency characteristics of screen content images, leading to suboptimal performance.

Method: Two-stage framework: (1) Amplitude-Phase Factorization Network (APFN) separates images into amplitude/phase streams with Amplitude Clustering Module (ACM) for periodic patterns and Phase Consistency Self-Attention (PCSA) for configuration; (2) Oscillation-Anharmonic Implicit Fitting Network (OAIF-Net) integrates both representations.

Result: Achieves state-of-the-art SCISR performance at multiple scales across four public SCI datasets; ablation studies confirm each component's effectiveness.

Conclusion: The frequency decoupled phasor perspective is effective for recovering regular textures and global configuration in screen content images.
...
### [131] [CRANE: Knowledge Editing for Reasoning MLLMs](https://arxiv.org/abs/2606.09033)
*Han Huang,Hao Wang,Mengqi Zhang,Shu Wu,Qiang Liu,Liang Wang*

Main category: cs.CV

TL;DR: The paper identifies three failure modes of knowledge editing in reasoning MLLMs (Structural Collapse, Cognitive Dissonance, Shallow Internalization), proposes a new CoT-aware evaluation benchmark (ReasonEdit-Bench), and introduces CRANE—a retrieval-augmented framework achieving high Grounded Success without parameter modification.


<details>
  <summary>Details</summary>
Motivation: Existing knowledge editing methods appear successful under traditional metrics (teacher-forcing accuracy up to 100%) but fail severely when the model's explicit CoT reasoning is examined (Grounded Success as low as 0%), revealing critical gaps in evaluation and methodology.

Method: (1) Proposed ReasonEdit-Bench with conflict stratification, multi-level probes, and multi-hop portability tests. (2) Introduced CRANE, a retrieval-augmented framework with modality-aware dual-library retrieval and two-phase training: SFT for structural initialization + GRPO with Cognitive Routing Reward for arbitrating between visual priors and edit facts.

Result: CRANE achieves 96.9% Grounded Success on conflict scenarios, 96.9% intermediate entity usage in multi-hop chains, 97.6% text-locality and 68.1% image-locality Edit Independence on ReasonEdit-Bench, and 87.0% on the out-of-distribution MMEVOKE benchmark under gold retrieval.

Conclusion: Current knowledge editing methods for reasoning MLLMs are fundamentally flawed under CoT scrutiny. CRANE demonstrates that retrieval-augmented approaches without per-edit parameter modification can effectively resolve the identified failure modes, setting a new standard for evaluation and method design in this domain.
...
### [132] [Leveraging NeRF-Rendered Images for 3D Gaussian Splatting](https://arxiv.org/abs/2606.09034)
*Mizuki Morikawa,Yuta Shimizu,Chunyu Li,Yusuke Monno,Masatoshi Okutomi*

Main category: cs.CV

TL;DR: The paper proposes a hybrid method that combines NeRF and 3DGS for street scene novel view synthesis, using NeRF-rendered images to train 3DGS, removing transient objects, adding bird's-eye views, and applying diffusion-based enhancement to achieve NeRF's quality with 3DGS's speed.


<details>
  <summary>Details</summary>
Motivation: NeRF and 3DGS show complementary performance (NeRF: higher quality; 3DGS: faster rendering). The authors aim to combine the strengths of both for street scene rendering.

Method: (1) Use pre-trained street-specific NeRF to produce training images for 3DGS. (2) Leverage NeRF-rendered images to remove transient objects in street-level views. (3) Generate bird's-eye views as additional training views. (4) Incorporate diffusion-based image enhancement to improve additional view quality.

Result: Experiments on one synthetic and two real datasets show improved street-scene rendering, preserving both 3DGS's speed and NeRF's quality.

Conclusion: The proposed hybrid approach successfully combines NeRF and 3DGS for street scenes, achieving the quality of NeRF and the speed of 3DGS simultaneously.
...
### [133] [MilliVid: Hierarchical Latents for Long-Range Consistency in Video Generation](https://arxiv.org/abs/2606.09056)
...
TL;DR: A multi-scale token approach where video generation uses coarse-to-fine rollout to maintain long-range consistency efficiently, demonstrated on Minecraft videos.


<details>
  <summary>Details</summary>
Motivation: Long-range consistency in video generation is challenging due to impractical sequence lengths required by transformers for even short video clips.

Method: 1) Pre-train a hierarchical autoencoder compressing frames into multi-scale tokens (coarse: layout/semantics; fine: texture). 2) Train a video diffusion model with coarse-to-fine rollout, generating and using context at controlled detail levels per step.

Result: The approach produces substantially more consistent rollouts than existing baselines on a long Minecraft video dataset.

Conclusion: Coarse-to-fine generation in multi-scale token space effectively preserves long-range consistency in geometry and object permanence while reducing unnecessary compute on perceptual details.
...
### [134] [See More, Think Deeper: Query-Expanded Visual Evidence and Answer-Clue Guided Reflection for Long Video Understanding](https://arxiv.org/abs/2606.09064)
*Shuning Wang,Zhiheng Wu,YiNuo Lu,Naiming Liu,Chen Jia,Bowen Liu,Shuo Nie,Weijie Zhu,Yumeng Zhang*

Main category: cs.CV

TL;DR: CoVER introduces a framework that enhances Video-LLMs for long-video understanding by dynamically gathering query-expanded visual evidence (See More) and using visual feedback to verify draft answers (Think Deeper), achieving state-of-the-art performance even with a 7B parameter model.


<details>
  <summary>Details</summary>
Motivation: Existing Video-LLMs for long-video understanding suffer from two key limitations: (1) evidence acquisition relies on a single search intent, and (2) answer generation lacks effective visual feedback mechanisms.

Method: CoVER (Comprehensive Visual Evidence and Reflection) framework with two core mechanisms: (1) See More - dynamically gathers query-expanded visual evidence, and (2) Think Deeper - verifies draft answers using answer-specific visual feedback, shifting from answer-centric to evidence-centric and visually verifiable reasoning.

Result: CoVER-7B substantially outperforms same-scale models and even surpasses state-of-the-art closed-source models on certain metrics.

Conclusion: The evidence-centric and visually verifiable reasoning approach effectively addresses limitations in long-video understanding, enabling better performance with relatively small models.
...
### [135] [REFINE: Super-efficient 3D Gaussian Splatting Pruning via Rendering-Free Primitive Importance](https://arxiv.org/abs/2606.09074)
*Zhang Chen,Shuai Wan,Mengting Yu,Fuzheng Yang,Junhui Hou*

Main category: cs.CV

TL;DR: REFINE is a fast 3DGS pruning method using a rendering-free Hessian-based importance metric, achieving 3000× speedup with competitive quality.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS pruning methods suffer from either severe quality degradation or prohibitive computational overhead, motivating the need for an efficient yet high-fidelity pruning framework.

Method: Proposes a rendering-free primitive importance metric via an analytically approximated Hessian field that models visibility, projection geometry, and content-adaptive hyperparameters, bypassing costly forward rendering.

Result: Achieves 3000× reduction in pruning-related computational complexity compared to SOTA, while maintaining highly competitive rendering quality across benchmarks.

Conclusion: REFINE enables highly accelerated, high-quality 3DGS pruning through a novel rendering-free Hessian-based importance metric.
...
### [136] [Beyond Scalar Rewards by Internalizing Reasoning into Score Distributions](https://arxiv.org/abs/2606.09076)
...
TL;DR: Z-Reward is a teacher-student framework for text-to-image reward modeling that uses reasoning-based distributional scoring (teacher) and distills it into a compact VLM (student) for efficient deployment, achieving ~89% human preference accuracy and 41.3% improvement over SFT baseline.


<details>
  <summary>Details</summary>
Motivation: Visual preference is subjective and better modeled as a distribution over rubric scores rather than deterministic scalars. Existing reward models over-compress uncertainty, while reasoning-based generative rewards are costly and hard to use as optimization signals.

Method: Proposes Z-Reward with two components: (1) Teacher: a large VLM trained with Group-wise Direct Score Optimization (GDSO) that uses reasoning to infer rubric-aligned score distributions. (2) Student: trained with Reasoning-Internalized Score Distillation (RISD) to transfer reasoning-conditioned distributions into a compact VLM without requiring explicit reasoning at inference.

Result: 27B GDSO teacher achieves 89.6% human preference accuracy; 9B RISD student achieves 88.6% accuracy, outperforming RewardDance, GRPO, and OPD baselines. Z-Reward yields 41.3% net human-preference improvement over SFT baseline in text-to-image optimization.

Conclusion: Z-Reward effectively decouples reasoning-heavy judgment from efficient deployment, enabling both strong preference modeling and differentiable reward signals for text-to-image post-training.
...
### [137] [Edge-Constrained UAV Small-Object Detection with P2 Enhancement and Quantum-Inspired Lightweight Structure Search](https://arxiv.org/abs/2606.09081)
*Wuming Lei,Yanbin Gao,Mingyan Sun,Xiaobin Li,Xuechen Liang*

Main category: cs.CV

TL;DR: This paper enhances YOLOX-Nano for UAV object detection by adding a P2 high-resolution detection branch and using a quantum-inspired evolutionary algorithm (QIEA) to search for lightweight structures, achieving significant small-object accuracy improvements on VisDrone dataset.


<details>
  <summary>Details</summary>
Motivation: UAV object detection needs compact detectors that preserve small-object details under edge-device constraints, but lightweight networks lose spatial information through repeated downsampling, and manually adding attention/fusion modules is costly with unstable gains.

Method: Combines a P2 high-resolution detection branch with a quantum-inspired evolutionary algorithm (QIEA) for lightweight structure screening, using a search space defined by lightweight priority and task specificity, with evaluations balancing accuracy, FLOPs, latency, memory, and recall.

Result: On VisDrone, P2 branch improves AP_small by 31.10% over YOLOX-Nano baseline; YOLOX-Nano+P2 outperforms NanoDet-Plus with 17.5% higher AP_s0.50 and 44.9% higher AP_small. QIEA-selected candidate achieves highest Recall_50, but +P2 remains the best AP-oriented variant.

Conclusion: P2 branch is recommended as the primary small-object enhancement method, and QIEA serves as a lightweight tool for candidate screening and accuracy-cost analysis, though proxy rankings may not fully transfer to final AP scores.
...
### [138] [Driving Video Retrieval for Complex Queries with Structured Grounding](https://arxiv.org/abs/2606.09109)
*Manyi Yao,Sparsh Garg,Christian Shelton,Amit Roy-Chowdhury,Abhishek Aich*

Main category: cs.CV

TL;DR: STRIVE-D is a data-calibrated retrieval framework for driving videos that combines calibrated rule-based retrieval with vision-language and keyword-based methods, using weakly labeled in-domain videos to estimate rule reliability and adapt mismatched rules, achieving up to 84% improvement in top-1 accuracy over SOTA.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language and keyword-based retrieval methods miss dynamic driving events (e.g., cut-ins, hard braking) because motion is often not explicitly described in text, while rule-based retrieval is brittle and fails when assumptions don't match real driving data.

Method: STRIVE-D uses weakly labeled in-domain videos to estimate rule reliability, adapts rules that mismatch observed data, and fuses calibrated rule scores with vision-language and keyword-based retrieval signals.

Result: Across three driving benchmarks (including newly released human-annotated event data on DrivingDojo), STRIVE-D delivers up to 84% relative improvement in top-1 accuracy over state-of-the-art methods.

Conclusion: Data-calibrated rule fusion effectively bridges the gap between rigid rule-based retrieval and flexible vision-language methods for dynamic event retrieval in autonomous driving videos.
...
### [139] [HDRAgent: An Agentic Framework for Multi-Exposure HDR Imaging](https://arxiv.org/abs/2606.09110)
*Weiyu Zhou,Tao Hu,Yijian Wang,Xiaogang Xu,Ruixing Wang,Qingsen Yan*

Main category: cs.CV

TL;DR: HDRAgent is an agent-driven HDR imaging framework that adaptively selects reconstruction strategies using MLLM-based scene perception, fine-grained contextual knowledge matching, and a perception-distortion feedback mechanism to reduce ghosting artifacts in dynamic scenes.


<details>
  <summary>Details</summary>
Motivation: Existing multi-exposure HDR methods use fixed feed-forward reconstruction, causing ghosting artifacts in complex dynamic scenes.

Method: Proposes HDRAgent with: (1) Fine-grained Contextual Knowledge Matching (FCM) module using MLLM for scene perception and retrieval; (2) Perception-distortion feedback mechanism for quality assessment and artifact diagnosis; (3) Agent-guided generative alignment strategy for extreme motion handling via MLLM-based dynamic-region parsing.

Result: Effectively reduces ghosting and local artifacts with competitive or superior objective performance and visual quality.

Conclusion: Agent-driven adaptive strategy selection is more effective than fixed feed-forward paradigms for HDR imaging in dynamic scenes.
...
### [140] [Illumination-Invariant Anomaly Detection for Sub-Canopy UAV Multispectral Point Clouds](https://arxiv.org/abs/2606.09111)
*Likun Chen,Yanfeng Gu,Xian Li*

Main category: cs.CV

TL;DR: A prior-free anomaly detection framework for UAV multispectral point clouds that handles illumination heterogeneity by formulating solar angle estimation as inverse optimization and using illumination-consistent sparse representation, outperforming baselines in sub-canopy target detection.


<details>
  <summary>Details</summary>
Motivation: UAV multispectral point clouds suffer from severe illumination heterogeneity caused by vegetation shadows, which compromises sub-canopy target detection. Existing methods rely on flight metadata or struggle to distinguish dark objects from true shadows.

Method: (1) Prior-Free Shadow Extraction via inverse optimization coupling spectral indices with ray-tracing model for solar angle estimation. (2) Illumination-Consistent Sparse Representation that constructs background dictionaries from neighbors sharing the same illumination state to disentangle reflectance from lighting variations.

Result: The method significantly improves separability between anomalies and background in complex forest environments, achieving superior performance over state-of-the-art baselines.

Conclusion: The framework is suitable for camouflaged military target identification, fallen tree trunk mapping, and archaeological ruin detection under dense foliage, without requiring prior knowledge or flight metadata.
...
### [141] [An Enhanced Geometric-Spectral Feature Learning Framework for Airborne Multispectral Point Cloud Classification](https://arxiv.org/abs/2606.09123)
*Xian Li,Yanfeng Gu,Aleksandra Pižurica*

Main category: cs.CV

TL;DR: This paper proposes an attention-based two-stream geometric-spectral feature learning framework with a joint loss function to improve classification accuracy on airborne multispectral point clouds, addressing challenges like high-dimensional heterogeneity, unbalanced samples, and inter-class spectral similarity.


<details>
  <summary>Details</summary>
Motivation: Classification models for airborne multispectral point clouds suffer from limited representation power due to high-dimensional heterogeneous spatial-spectral information, unbalanced sample distribution, and inter-class spectral similarity.

Method: A two-stream feature fusion method with attention mechanisms: (1) position-encoded global spectral features with fusion self-attention, (2) multikernel point convolution and feature aggregation attention for spectral-guided geometric features, plus a residual attention fusion block and a joint loss function.

Result: Experimental results on two airborne MPC datasets demonstrate effectiveness compared to state-of-the-art methods. Codes and datasets will be publicly available.

Conclusion: The proposed geometric-spectral feature learning framework with attention mechanisms and joint loss function effectively improves MPC classification, addressing key challenges in this domain.
...
### [142] [A Geometric Framework for Absolute Pose and Velocity Estimation with Event Cameras](https://arxiv.org/abs/2606.09139)
*Zibin Liu,Shunkun Liang,Banglei Guan,Yang Shang,Qifeng Yu,Ji Zhao*

Main category: cs.CV

TL;DR: This paper proposes a geometric framework for simultaneous absolute pose and velocity estimation from event cameras using 3D line correspondences, achieving state-of-the-art performance with efficient linear and globally optimal polynomial solvers.


<details>
  <summary>Details</summary>
Motivation: Current event-based geometric methods focus on velocity estimation, but absolute pose estimation remains underexplored. Simultaneously recovering both from event streams is an open and challenging problem.

Method: The framework leverages two geometric constraints: (1) orthogonality between a 3D line and its event plane's normal vector, and (2) collinearity of events with their 2D line projections. It includes linear solvers (efficient) and polynomial solvers (globally optimal for rotation), requiring a minimum of three event-line correspondences.

Result: Extensive simulation and real-world experiments show state-of-the-art performance with significant improvements in accuracy and computational efficiency over existing methods.

Conclusion: The proposed framework successfully addresses the gap in simultaneous absolute pose and velocity estimation from event data, and demo code is publicly available.
...
### [143] [DiffSight-Former: Modeling Structural Differences and Temporal Dynamics for Glaucoma Progression Prediction](https://arxiv.org/abs/2606.09140)
*Yi Huang,Lei Bi,Jinman Kim*

Main category: cs.CV

TL;DR: DiffSight-Former is a Transformer-based framework that predicts glaucoma progression from sequential fundus images by modeling temporal changes in optic nerve structures and vasculature, achieving strong results (91.54% AUC on SIGF, 87.48% accuracy on GRAPE).


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on single time-point images or fixed-length sequences, struggling to detect subtle early progression signals. There is a need for a framework that captures longitudinal structural and vascular changes for early glaucoma prediction.

Method: Uses a fundus-specific foundation model for time-variant feature extraction, a multi-structure difference modeling module to quantify changes in optic disc/cup and vasculature, and a time-aware Transformer with temporal interval embeddings to model progression and predict future glaucoma onset.

Result: On SIGF: 91.54% AUC, 92.16% sensitivity. On GRAPE: 87.48% average accuracy across three progression criteria. Outperforms existing approaches and shows robustness across different temporal settings.

Conclusion: DiffSight-Former effectively leverages sequential fundus images for longitudinal glaucoma monitoring and early risk prediction, demonstrating strong clinical utility.
...
### [144] [Decoding Pedestrian Crossing Intention from Egocentric Vision via Vision Language Models](https://arxiv.org/abs/2606.09142)
*Danya Li,Xiang Su,Yan Feng,Rico Krueger*

Main category: cs.CV

TL;DR: This paper uses vision language models (VLMs) fine-tuned with parameter-efficient methods to predict pedestrian crossing intentions from egocentric video, achieving a 14.5% accuracy improvement over a transformer baseline by incorporating contextual cues like eye gaze and ego motion.


<details>
  <summary>Details</summary>
Motivation: Egocentric vision's potential for traffic-safety prediction is underexplored; the authors aim to decode pedestrian crossing intentions from short egocentric video clips to improve safety prediction.

Method: The task is formulated as a closed-ended VQA problem using VLMs. They benchmark three VLM families in zero-shot settings, then apply parameter-efficient fine-tuning, and finally incorporate additional contextual cues (ego motion, vehicle motion, eye gaze).

Result: Fine-tuned models outperform zero-shot counterparts by 9% over a transformer baseline. The best model (Qwen3-VL-2B with eye gaze and ego motion cues) achieves a 14.5% accuracy improvement, setting a new state of the art.

Conclusion: Fine-tuning VLMs with contextual cues significantly enhances pedestrian intent prediction from egocentric video, demonstrating the value of first-person vision for traffic safety.
...
### [145] [CAMF-Det: Closure-Aware Multimodal Fusion for LiDAR-Camera 3D Object Detection on UAV Platforms](https://arxiv.org/abs/2606.09143)
*Yanze Jiang,Yanfeng Gu,Xian Li*

Main category: cs.CV

TL;DR: CAMF-Det introduces a closure-aware multimodal fusion framework for LiDAR-camera 3D object detection on UAV platforms, explicitly modeling occlusion intensity from both modalities using a Beer-Lambert-inspired approach, and embedding these priors throughout the detection pipeline to handle spatially varying ground-object occlusion, achieving state-of-the-art performance on self-built UAV datasets.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal 3D detection methods are designed for ground-vehicle scenarios and fail to address frequent ground-object occlusion (e.g., tree canopies) in UAV top-down scenes, which causes spatially varying and modality-dependent information degradation.

Method: 1) A dual-modal closure modeling module constructs occlusion intensity ground truth via a Beer-Lambert-inspired formulation with building-mask correction. 2) A dual-modal prediction network predicts occlusion intensity online from single-frame inference. 3) Both ground-truth and predicted occlusion intensities are injected into data augmentation, feature encoding, fusion, and detection head for adaptive detection.

Result: On two self-built UAV datasets (SI3D-DI and SI3D-DII), CAMF-Det achieves best performance across all difficulty levels, with hard-level mAP_BEV improvements of 9.43% and 4.88% over the best competing methods.

Conclusion: Explicit occlusion prior modeling and exploitation significantly improve robust multimodal 3D detection in UAV scenes with spatially varying and modality-dependent occlusion.
...
### [146] [Ultra Flash: Scaling Real-Time Streaming Video Generation to High Resolutions](https://arxiv.org/abs/2606.09150)
...
TL;DR: Ultra Flash is a cascaded streaming framework for real-time high-resolution video generation, achieving ~30 FPS at 1K and ~18 FPS at 2K on a single GPU through architecture-preserving super-resolution training, causal streaming latent upsampling, and cascade optimization with preference learning.


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive video diffusion models are limited to low resolutions (e.g., 480P), and efficient, scalable real-time high-resolution video generation remains an open challenge.

Method: Three key contributions: (1) architecture-preserving T2V-to-TV2V super-resolution training with AIGC-oriented data degradation; (2) causal streaming latent upsampler with high-resolution decoder; (3) cascade optimization including hybrid-reward causalization, single-step distillation, and streaming self-forcing preference optimization with dynamic cache management.

Result: Achieves ~30 FPS at 1K and ~18 FPS at 2K on a single GPU with state-of-the-art visual quality and superior efficiency.

Conclusion: Ultra Flash reliably produces ultra-high-resolution streaming video while maintaining SOTA quality and efficiency, bridging the gap between low-resolution streaming models and real-time high-resolution generation.
...
### [147] [OmniGen-AR: AutoRegressive Any-to-Image Generation](https://arxiv.org/abs/2606.09156)
*Junke Wang,Xun Wang,Qiushan Guo,Peize Sun,Weilin Huang,Zuxuan Wu,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: OmniGen-AR is a unified autoregressive framework for Any-to-Image generation that handles diverse conditional inputs (text, spatial signals, visual context) within a single model, using Disentangled Causal Attention to prevent information leakage, achieving state-of-the-art results on GenEval (0.63) and VBench (80.02).


<details>
  <summary>Details</summary>
Motivation: Existing autoregressive visual generation models are limited to single-modality conditions (e.g., text), which restricts their applicability in real-world scenarios that require image synthesis from diverse control signals.

Method: OmniGen-AR discretizes various visual conditions via a shared visual tokenizer and text prompts via a text tokenizer, supporting multiple conditional inputs. It introduces Disentangled Causal Attention (DCA) that separates full-sequence causal masking into condition causal attention and content causal attention to mitigate information leakage, serving as a training-time regularizer without affecting inference.

Result: Achieves new state-of-the-art or competitive results across benchmarks: 0.63 on GenEval and 80.02 on VBench.

Conclusion: OmniGen-AR demonstrates effectiveness in flexible and high-fidelity visual generation by unifying diverse conditional inputs within a single autoregressive framework.
...
### [148] [Zero-Parameter Geometric Gating for Temporally Stable Low-Altitude UAV Video Semantic Segmentation](https://arxiv.org/abs/2606.09162)
*Jingpu Yang,Fengxian Ji,Zhengzhao Lai,Juanfan Wu,Mingxuan Cui,Yufeng Wang*

Main category: cs.CV

TL;DR: A zero-parameter geometric gate that uses RANSAC homography statistics to route planar regions to homography warp and non-planar regions to optical flow, improving UAV video segmentation by +4.24-4.91% mIoU with minimal added parameters.


<details>
  <summary>Details</summary>
Motivation: Temporal consistency in UAV video segmentation is challenged by spatially structured noise from dense optical flow, especially in planar regions that dominate aerial imagery.

Method: A zero-parameter geometric gate applies RANSAC homography inlier ratios on a 16×16 grid to decide per-region whether to use homography or optical flow warping, then fuses via Semantic Similarity Propagation (SSP) with only 211K trainable parameters.

Result: +4.24-4.91% mIoU improvement over SegFormer-b2 and Hiera-S+UPerNet on synthetic UAVid. Flow residuals in planar regions show spatial autocorrelation (Moran's I=0.32), predicting boundary instability (ρ=0.66), and rigidification recovers temporal consistency from 62% to 92% in homography-valid regions.

Conclusion: A simple, parameter-efficient geometric routing mechanism effectively mitigates optical flow noise in planar regions, significantly boosting temporal consistency and segmentation accuracy for low-altitude UAV video.
...
### [149] [Vision-Language Guided Hyperspectral Object Tracking via Semantics Fusion and Contextual Template Updating](https://arxiv.org/abs/2606.09167)
*Rui Yao,Yuhong Zhang,Kunyang Sun,Hancheng Zhu,Jiaqi Zhao,Zhiwen Shao,Abdulmotaleb El Saddik*

Main category: cs.CV

TL;DR: VLHTrack proposes a vision-language joint tracking framework that uses LLM-guided band selection to handle spectral redundancy and a Mamba-based dynamic template update to address target deformation, achieving SOTA on HOT2023/2024.


<details>
  <summary>Details</summary>
Motivation: Existing HOT methods struggle with (1) efficiently extracting useful information from redundant spectral bands, limiting generalization, and (2) handling target appearance variations (occlusion, illumination) that cause large deformations between frames, which challenges current temporal modeling approaches.

Method: 1) Language-Guided Band Selection Module (LBSM) leverages LLM descriptions to create a semantic-to-spectral mapping for selecting discriminative bands. 2) Multi-Modal Vision-Language Fusion Module integrates visual and linguistic embeddings. 3) Dynamic Template Update with Mamba (DTUM) uses selective state space modeling to learn inter-frame dependencies for updating template features.

Result: VLHTrack achieves state-of-the-art tracking performance on the HOT2023 and HOT2024 benchmarks, outperforming existing methods.

Conclusion: Incorporating language priors and dynamic temporal modeling via Mamba effectively addresses both spectral redundancy and target deformation challenges in hyperspectral object tracking.
...
### [150] [Claude Code-Driving Scenario Mining for the Argoverse 2 Challenge](https://arxiv.org/abs/2606.09180)
*Wei Deng,Caoshengzhe Xue,Shuaikun Liu,Zhaohong Liu,Mengshi Qi,Huadong Ma*

Main category: cs.CV

TL;DR: A four-stage pipeline for the CVPR 2026 Argoverse 2 Scenario Mining Challenge using Claude Code for code generation/review, iterative screening with Timestamp Balanced Accuracy, and Qwen3-VL for verification.


<details>
  <summary>Details</summary>
Motivation: To address the scenario mining challenge by developing an automated, multi-stage pipeline that reduces false positives and improves scenario detection accuracy.

Method: Four-stage pipeline: (1) autonomous code generation via Claude Code agent (GLM~5.1), (2) iterative training set screening with Timestamp Balanced Accuracy threshold 0.8, (3) semantic code review via separate Claude Code session, (4) Qwen3-VL scene-level verification.

Result: Results reported on the Argoverse 2 test set (specific metrics not detailed in abstract).

Conclusion: The proposed pipeline demonstrates a systematic approach to automated scenario mining, combining LLM-based code generation, screening, review, and vision-language verification.
...
### [151] [Counterfactual Reasoning for Fine-Grained Evidence Disentanglement in VideoQA](https://arxiv.org/abs/2606.09181)
*Zhou Du,Hamid Krim,Xiao Wu,Zhaoquan Yuan,Liangwei Li,Keisuke Fujii*

Main category: cs.CV

TL;DR: The paper proposes CREDiT, a counterfactual reasoning framework that disentangles causal and non-causal visual evidence in video for more faithful and reliable VideoQA, outperforming existing methods on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: VideoQA models often rely on spurious statistical correlations instead of causal evidence, leading to brittle and unfaithful reasoning. Existing methods fail to explicitly disentangle causal cues from confounders and provide limited fine-grained evidence localization.

Method: A Counterfactual Reasoning framework for fine-grained Evidence Disentanglement (CREDiT) that uses a structural causal model, decomposes cross-modality representations into causal and non-causal components under independence and minimality constraints, and introduces feature-level causal interventions with counterfactual inputs to approximate causal effects while suppressing non-causal correlations.

Result: Experiments on NExT-GQA, SportsQA, and SPORTU-video show consistent improvement in answer accuracy and reasoning reliability across generic and complex sports scenarios.

Conclusion: CREDiT leads to more trustworthy VideoQA systems by effectively disentangling causal evidence from confounders through counterfactual reasoning.
...
### [152] [CP4D: Compositional Physics-aware 4D Scene Generation](https://arxiv.org/abs/2606.09187)
*Hanxin Zhu,Cong Wang,Tianyu He,Long Chen,Xin Jin,Chen Gao,Zhibo Chen*

Main category: cs.CV

TL;DR: CP4D introduces a novel paradigm for physically consistent 4D scene generation by decomposing scenes into static 3D environments and dynamic foreground objects, then composing them through a three-stage pipeline involving expert models, hybrid motion synthesis, and automated composition.


<details>
  <summary>Details</summary>
Motivation: Existing 4D generation methods fail to capture underlying physical principles, producing physically inconsistent and visually implausible results. The authors aim to generate 4D scenes that adhere to real-world physical dynamics.

Method: Three-stage pipeline: (1) Use pre-trained expert models to generate high-fidelity 3D representations of static environment and foreground objects separately; (2) Hybrid motion synthesis integrating physical simulator priors with video diffusion model common sense for plausible trajectories and interactions; (3) Automated composition mechanism fusing static environment and dynamic objects into coherent 4D scenes.

Result: CP4D generates explorable and interactive 4D scenes with high visual fidelity, strong physical plausibility, and fine-grained controllability, significantly outperforming existing methods.

Conclusion: Compositional decomposition (static + dynamic) with physically grounded motion synthesis effectively addresses physical inconsistency in 4D generation, achieving superior performance.
...
### [153] [Event-driven dynamic trajectories reconstruction and measurement of mechanical parameters for fragments](https://arxiv.org/abs/2606.09208)
*Haoyang Li,Banglei Guan,Muxi Zha,Yifei Bian,Minzu Liang,Yang Shang,Qifeng Yu*

Main category: cs.CV

TL;DR: This paper presents an event-driven method using multiple event cameras to reconstruct 3D trajectories and measure mechanical parameters (position, velocity, kinetic energy) of high-speed warhead fragments under strong flash and smoke interference, employing time-correlated epipolar, trifocal tensor line, and local homography constraints with entropy-weighted probability filtering.


<details>
  <summary>Details</summary>
Motivation: High-speed, occluded fragments generated during warhead detonation are difficult to measure accurately due to high-intensity flash and smoke, which hinder acquisition of their mechanical parameters essential for lethality assessment.

Method: A multi-event-camera system is built with three geometric constraints (time-correlated epipolar, trifocal tensor line, local homography) for matching event points, combined with an entropy-weighted probability model for mismatch filtering, followed by line-line intersection and nonlinear optimization for 3D trajectory reconstruction.

Result: The method successfully reconstructs 3D fragment trajectories and calculates velocity and kinetic energy, overcoming flash and smoke interference using event cameras' microsecond temporal resolution and high dynamic range.

Conclusion: This event-driven approach provides reliable technical support for mechanical damage evaluation of warhead fragment fields and tactical protection design.
...
### [154] [Minimal Solvers for Full-DoF Motion Estimation from Asynchronous Differential SfM](https://arxiv.org/abs/2606.09218)
*Shuo Pan,Banglei Guan,Bin Li,Zhenbao Yu,Zibin Liu,Zi Wang,Yang Shang,Qifeng Yu*

Main category: cs.CV

TL;DR: This paper presents a novel framework for full 6-DoF egomotion estimation directly from asynchronous optical flow using event cameras, including a minimal 5-point algebraic solver and an accelerated version for real-time high-speed applications, demonstrating superior accuracy and robustness over traditional synchronous methods.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer high temporal resolution and low latency but their asynchronous data streams are incompatible with traditional frame-based algorithms, motivating the need for new methods that can directly handle asynchronous optical flow for egomotion estimation.

Method: The authors decouple the differential epipolar constraint into angular and linear velocity components, derive an asynchronous formulation, develop an optimization algorithm requiring at least five points, transform constraint equations into polynomial form for a minimal 5-point solver, and propose an accelerated solver via truncating high-order angular velocity terms.

Result: Extensive evaluations on synthetic and real-world datasets show the asynchronous approach outperforms traditional synchronous methods in accuracy and robustness to spatiotemporal noise.

Conclusion: This work establishes a critical foundation for efficient and accurate continuous-time motion estimation in high-speed robotics applications.
...
### [155] [Semi-supervised Source Detection in Astronomical Images: New Benchmark and Strong Baseline](https://arxiv.org/abs/2606.09219)
*Longhan Feng,Zihuang Cao,Ali Luo,Yuanhao Guo,Shuilian Yao,Yixin Guo,Qi Jia,Yu Liu*

Main category: cs.CV

TL;DR: The paper introduces LAMOST-DET, a large benchmark of 18,400 astronomical images with 728,898 source instances, and proposes Nova Teacher, a semi-supervised learning framework that effectively detects dense, small, and faint astronomical sources using sparse annotations, achieving 4.04%-5.22% mAP improvement over previous methods.


<details>
  <summary>Details</summary>
Motivation: Astronomical source detection faces challenges due to high density, PSF effects, and low SNR, while fully-supervised methods are impractical because of the difficulty in annotating dense, small, and faint sources.

Method: Nova Teacher: a semi-supervised learning framework with a dual-teacher paradigm, incorporating a source light enhancement module, confidence-guided pseudo-supervision, and cross-view complementary mining.

Result: Nova Teacher achieves 4.04% and 5.22% mAP improvements over previous methods under two semi-supervised settings on LAMOST-DET, and shows competitive generalization on natural images.

Conclusion: The proposed benchmark and semi-supervised method effectively address the scarcity of annotated astronomical data and the challenges of detecting dense, small sources, with code publicly available.
...
### [156] [EgoTactile: Learning Grasp Pressure for Everyday Objects from Egocentric Video](https://arxiv.org/abs/2606.09243)
*Yuan Zeng,Yujia Shi,Tiao Tan,Xingting Li,Yaqi Qin,Zongqing Lu,Wenming Yang,Jing-Hao Xue,Qingmin Liao*

Main category: cs.CV

TL;DR: EgoTactile benchmark pairs egocentric video with full-hand pressure data; EgoPressureDiff uses conditional diffusion with video priors and physical constraints to infer grasp pressure maps, outperforming baselines.


<details>
  <summary>Details</summary>
Motivation: Estimating full-hand grasp pressure from egocentric video is needed for VR/robotics, but dense tactile sensing requires intrusive hardware and existing vision methods fail on complex 3D objects.

Method: EgoTactile benchmark (egocentric video + full-hand pressure + bare-hand subset); EgoPressureFormer (discriminative baseline); EgoPressureDiff (conditional diffusion with pre-trained video diffusion backbone + Physically-Informed Feature Rectification).

Result: EgoPressureDiff achieves superior performance on the benchmark and robust transferability to in-the-wild scenarios.

Conclusion: Combining large-scale video priors with physical constraints effectively resolves visual-physical ambiguities for grasp pressure estimation from egocentric video.
...
### [157] [Proposal Refinement for Few-Shot Object Detection](https://arxiv.org/abs/2606.09245)
*Yuan Zeng,Bin Song,Jie Guo,Yuwen Chen*

Main category: cs.CV

TL;DR: This paper proposes a proposal refinement approach to address the unbalanced distribution of region proposals between novel and base classes in few-shot object detection, achieving 1-6% improvement without extra inference cost.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot object detection algorithms rely heavily on few-shot classification performance and suffer from unbalanced distribution of region proposals between novel classes and base classes, which limits detection accuracy.

Method: Introduces a refinement loss during base training to enhance model sensitivity to novel classes, and a refinement branch as an auxiliary component for RPN during fine-tuning to generate more novel class proposals.

Result: Outperforms baseline methods by roughly 1% to 6% on current benchmarks without any increase in inference time, establishing a new state-of-the-art.

Conclusion: Rebalancing the proposal distribution effectively improves few-shot object detection performance, and the method achieves SOTA results with no additional inference overhead.
...
### [158] [SOMA: From Surface Observations to Muscle Anatomy](https://arxiv.org/abs/2606.09246)
*Eduardo Alvarado,Emily Kim,Gerrit Nolte,Friedemann Runte,Mario Botsch,Marc Habermann,Christian Theobalt*

Main category: cs.CV

TL;DR: SOMA and SKIM: A method to infer muscle deformations from multi-view RGB video data, bypassing expensive biomechanical simulations by learning muscle behavior directly from skin surface observations.


<details>
  <summary>Details</summary>
Motivation: Parametric body models only capture skin surface, not underlying biomechanical structures. Traditional FEM simulations are accurate but computationally expensive, while existing biomechanical tools model muscle forces but don't capture external shape changes. There's a need for scalable methods that recover muscle deformations directly from observable surface data.

Method: SOMA (Surface Observations to Muscle Anatomy): a person-specific model that infers spatio-temporal muscle behavior from surface signals obtained via RGB cameras. Also introduces SKIM, a subject-specific soft-tissue deformation dataset.

Result: First method to recover muscle deformations from multi-view RGB data; provides anatomically grounded animations without the complexity of traditional simulations, offering a scalable and cost-effective solution.

Conclusion: SOMA enables scalable, cost-effective muscle deformation inference from RGB video, bridging the gap between surface observations and internal biomechanics for applications in medicine, sports, and entertainment.
...
### [159] [Temporal-Aware Reasoning Optimization for Video Temporal Grounding](https://arxiv.org/abs/2606.09248)
*Minghang Zheng,Zihao Yin,Yi Yang,Yuxin Peng,Yang Liu*

Main category: cs.CV

TL;DR: TaRO enhances video temporal grounding in MLLMs by introducing constructive reasoning exploration with dense captions and a temporal-sensitivity reward that evaluates reasoning quality, achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs for video temporal grounding produce superficial reasoning due to inefficient random exploration and reward functions that only consider answer correctness while ignoring reasoning quality.

Method: (1) Constructive Reasoning Exploration: uses pre-generated dense captions to build reasoning paths with explicit visual cues and timestamps. (2) Temporal-Sensitivity Reward: evaluates reasoning quality by measuring logit drop when event boundaries are disrupted. (3) Progressive curriculum: starts with reward-guided path selection, then free exploration.

Result: Achieves state-of-the-art performance on video temporal grounding benchmarks.

Conclusion: TaRO effectively enhances temporal-aware reasoning in MLLMs by combining structured exploration with reasoning quality rewards, outperforming prior methods.
...
### [160] [MAGIS: Evidence-Based Multi-Agent Reasoning for Interpretable Strabismus Clinical Decision-Making](https://arxiv.org/abs/2606.09249)
...
TL;DR: MAGIS is a multi-agent framework for interpretable strabismus diagnosis that achieves 91.3% weighted F1 score by using dual-evidence constraints and corrective verification to reduce hallucination in large vision-language models.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods lack transparent reasoning for strabismus diagnosis, and large vision-language models (LVLMs) are prone to hallucination in evidence-sensitive medical tasks requiring fine-grained subtype diagnosis.

Method: MAGIS transforms black-box generation into a structured multi-agent process: (1) candidate hypothesis generation, (2) Dual-Evidence Constrained Context (DECC) organizing visual evidence from nine cardinal gaze positions plus clinical diagnostic rules, (3) Evidence-Based Corrective Verification (EBCV) checking consistency between hypothesis, visual evidence, heatmaps, and clinical rules, with refinement upon inconsistency detection, and (4) report generation.

Result: MAGIS improves weighted F1 score from 72.0% to 91.3% on a fine-grained strabismus benchmark, and substantially improves clinical reliability (consistency, alignment, completeness) of generated diagnostic reports.

Conclusion: MAGIS provides an effective solution for building accurate, evidence-based, and clinically interpretable strabismus diagnosis systems.
...
### [161] [LiteVSR: Lightweight Adaptation of Frozen Diffusion Transformers for Video Super-Resolution](https://arxiv.org/abs/2606.09250)
*Yu Cao,Ziquan Liu,Zhensong Zhang,Jiankang Deng,Shaogang Gong,Jifei Song*

Main category: cs.CV

TL;DR: LiteVSR achieves efficient video super-resolution by using a frozen Diffusion Transformer with a lightweight State-Aware Adapter (only 11.25% trainable params), leveraging flow matching's constant velocity field to reduce the adaptation to learning a fixed injection pattern, trained in just 12 GPU-hours on a single A100.


<details>
  <summary>Details</summary>
Motivation: Adapting large-scale pre-trained video generators for VSR in novel domains is computationally prohibitive due to extensive fine-tuning requirements, and existing methods (ControlNet-style adapters) lose efficiency under modern Diffusion Transformers due to the lack of encoder-decoder hierarchy.

Method: LiteVSR uses a frozen Diffusion Transformer with a lightweight State-Aware Adapter that employs a dual-stream architecture: one stream extracts static structural cues from LQ input, another extracts dynamic cues from intermediate denoising states, aligned via time-dependent cross-attention for adaptive transition from structural alignment to texture refinement. It leverages flow matching's constant velocity field to reduce adaptation to learning a fixed injection pattern.

Result: Competitive restoration quality with only 11.25% trainable parameters, 12 GPU-hours training on a single A100, and compatibility with fast sampling down to a single step.

Conclusion: Flow matching provides a principled alternative for cross-domain VSR adaptation, enabling a minimalist and efficient framework that significantly reduces computational costs while maintaining quality.
...
### [162] [A practical probabilistic framework for deformable image registration uncertainty in radiotherapy dose propagation](https://arxiv.org/abs/2606.09253)
*Stefan Heldmann,Sven Kuckertz,Nasim Givehchi,Thomas Coradi,Mikel Byrne,Ben Archibald-Heeren,Nils Papenberg*

Main category: cs.CV

TL;DR: A lightweight probabilistic framework for propagating DIR uncertainty to dose statistics and DVHs in radiotherapy, demonstrated on a prostate case, showing certainty-map design impacts uncertainty bounds more than kernel choice.


<details>
  <summary>Details</summary>
Motivation: DIR uncertainty can significantly affect clinically relevant dose estimates in radiotherapy, yet existing uncertainty models are often complex, biomechanical, or ensemble-based. The authors aim to provide a practical, interpretable, and computationally feasible alternative.

Method: Models voxel correspondence as a random variable using a local certainty map (safety margins, boundary mismatch, or conservative values). Introduces a structure-guided in/out strategy as an optional refinement. Computes dose probabilities, expected dose, confidence bounds, and DVH envelopes.

Result: Prostate radiotherapy case study demonstrates that certainty-map design has a stronger effect on dose and DVH uncertainty bounds than the specific probability kernel choice. The in/out strategy's benefit is case-dependent and modest in this example.

Conclusion: The framework provides a transparent, lightweight way to incorporate DIR uncertainty into radiotherapy dose assessment and to study how modeling choices affect propagated dose metrics.
...
### [163] [Self-supervised Learning Matters: A Simple Ensemble Solution for Micro-Gesture Recognition](https://arxiv.org/abs/2606.09261)
*Tingyi Liu,Kun Li,Fei Wang,Junjie Chen,Zhiliang Wu,Jihao Gu,Haixu Liu,Dan Guo*

Main category: cs.CV

TL;DR: XInsight Lab won 1st place in the MiGA Challenge micro-gesture classification track (IJCAI 2026) by proposing a multimodal ensemble framework combining a self-supervised RGB model (pretrained on 120K unlabeled clips via masked video modeling, fine-tuned on iMiGUE) with supervised multi-stream models, achieving 74.419% top-1 accuracy (+1.206% over prior SOTA).


<details>
  <summary>Details</summary>
Motivation: To improve micro-gesture recognition by leveraging unlabeled in-domain video data through self-supervised representation learning, overcoming limitations of fully supervised approaches.

Method: A multimodal ensemble framework integrating: (1) a self-supervised RGB model pretrained on 120K unlabeled clips via masked video modeling, fine-tuned on iMiGUE; (2) supervised multi-stream models from prior solutions. The RGB model serves as a complementary branch in the final ensemble.

Result: Self-supervised RGB baseline: 69.224% top-1 accuracy on iMiGUE test set. Final ensemble: 74.419% top-1 accuracy, surpassing previous SOTA by 1.206 percentage points. Achieved 1st place in the challenge.

Conclusion: Self-supervised RGB representation learning is effective for micro-gesture recognition; combining it with supervised multi-stream models via ensemble yields new state-of-the-art results.
...
### [164] [See More, Match Better: Multi-Source Feature Fusion for Two-View Correspondence Learning](https://arxiv.org/abs/2606.09262)
*Xiaojie Li,Xin Jiang,Luanyuan Dai,Jinnan Yang,Yongdong Zhang,Zechao Li*

Main category: cs.CV

TL;DR: TriMatch is a multi-source feature fusion framework for two-view correspondence learning that jointly extracts geometric, texture semantic, and structural semantic features, aligned via dedicated modules, to better distinguish true correspondences from false ones, especially in challenging repetitive or textureless scenes.


<details>
  <summary>Details</summary>
Motivation: Existing two-view correspondence learning methods rely on coordinate-based geometric consistency and struggle with pseudo-consistent outliers in repetitive structures, textureless regions, or locally similar geometric patterns.

Method: TriMatch consists of: (1) Feature extraction with joint geometric, texture semantic, and structural semantic features; (2) Texture-Geometric Alignment and Structural-Geometric Alignment modules; (3) Semantic-Guided Correspondence Modulation to suppress semantically inconsistent correspondences; (4) Hierarchical Semantic-Enhanced Correspondence Refinement for progressive modeling and multi-context recalibration.

Result: Extensive experiments demonstrate effectiveness, robustness, and generalization capability of TriMatch.

Conclusion: TriMatch addresses the limitation of existing geometric-only methods by fusing multi-source semantic features with geometric features, achieving more reliable inlier-outlier discrimination in challenging scenes.
...
### [165] [EditSSC: Toward Editable Semantic Occupancy Scenes with Unconditional Diffusion Models](https://arxiv.org/abs/2606.09273)
*Fatima Balde,Raoul de Charette,Alexandre Boulch*

Main category: cs.CV

TL;DR: EditSSC repurposes 2D BEV representations and Stable Diffusion for 3D semantic scene generation and editing, outperforming 3D-specific methods on SemanticKITTI.


<details>
  <summary>Details</summary>
Motivation: Existing 3D semantic scene generation methods rely on complex 3D-specific architectures (triplane encoders, adapted diffusion networks), limiting simplicity and editing capabilities.

Method: Reshapes 3D semantic occupancy grids into multi-channel BEV images, uses quantized autoencoder and UNet from Stable Diffusion with minimal modifications, performs diffusion on latents after quantization, and exploits class-to-code correspondences for editing.

Result: Outperforms existing 3D-specific baselines on unconditional generation on SemanticKITTI, and supports training-free sketch-guided generation, inpainting, and outpainting.

Conclusion: Well-established 2D architectures can be effectively repurposed for 3D scene generation and editing, offering both simplicity and editing flexibility.
...
### [166] [Zero-Shot Semantic Re-Identification for Autonomous Driving: A VLM Baseline Study](https://arxiv.org/abs/2606.09362)
*Eduardo Borges,Manuel Abreu,Luís Garrote,Urbano J. Nunes*

Main category: cs.CV

TL;DR: This paper explores using Vision-Language Models (VLMs) to generate textual descriptions of traffic participants (vehicles, pedestrians, cyclists) for re-identification in autonomous driving, instead of traditional visual matching. The zero-shot semantic approach achieves comparable performance to supervised CNN baselines while offering better interpretability, but faces challenges with attribute inconsistency and fine-grained discrimination.


<details>
  <summary>Details</summary>
Motivation: Purely visual ReID representations are sensitive to viewpoint, occlusion, illumination, and sensor-domain variations, limiting interpretability and robustness in complex driving scenes. The authors propose leveraging VLMs for language-based re-identification as an alternative.

Method: A zero-shot pipeline using VLMs to generate structured textual descriptions of detected traffic participants, including attributes like category, color, shape, pose, visible parts, spatial context, and distinctive visual cues. These descriptions are then used for identity matching across observations.

Result: Zero-shot semantic descriptions achieve retrieval performance comparable to a supervised CNN baseline, with greater interpretability through explicit identity cues.

Conclusion: Language-based re-identification is feasible for autonomous driving scenarios, but challenges remain including attribute inconsistency across viewpoints and limited fine-grained discrimination between visually similar instances.
...
### [167] [RT-SDGOD: Real-Time Single-Domain Generalized Object Detection](https://arxiv.org/abs/2606.09367)
*Yupeng Zhang,Fangzhuo Gao,Ruize Han,Wei Feng,Liang Wan*

Main category: cs.CV

TL;DR: The paper introduces RT-SDGOD, a framework for real-time single-domain generalized object detection under domain shifts, and proposes RT-SDGDet, a multi-evidence collaborative modeling method that improves DETR-based detectors' cross-domain generalization with zero extra inference overhead by using one-to-many supervision, discriminative evidence diversity learning, and dual-view evidence consistency learning to address increased missed detections caused by limited and unstable object-level discriminative evidence.


<details>
  <summary>Details</summary>
Motivation: Real-time detectors under strict inference budgets suffer severe performance degradation due to domain shifts from weather and imaging variations, yet existing single-domain generalized object detection methods rarely investigate generalization capability under such constrained real-time conditions.

Method: Proposes RT-SDGDet framework with three components: (1) one-to-many (O2M) supervision to construct stable object-specific query groups, (2) Discriminative Evidence Diversity Learning (DEDL) to expand object-level evidence coverage, and (3) Dual-view Evidence Consistency Learning (DvECL) to improve evidence stability under appearance perturbations. All components are training-only, incurring zero extra inference overhead.

Result: Extensive experiments show the proposed method achieves better generalization performance than existing approaches across multiple unseen target domains.

Conclusion: RT-SDGDet effectively addresses the core degradation pattern (increased missed detections due to limited and unstable object-level discriminative evidence) in DETR-based real-time detectors under domain shift, enabling cross-domain generalization without additional inference cost.
...
### [168] [PhysScene: A Scene Graph Dataset for Scientific Visual Reasoning in Physics Experiments](https://arxiv.org/abs/2606.09368)
*Minghao Zou,Qingtian Zeng,Shangkun Liu,Yanda Meng,Guanghui Yue,Baoquan Zhao,Abdulmotaleb El Saddik,Wei Zhou*

Main category: cs.CV

TL;DR: PhysScene is the first Scene Graph dataset for physics experiments, featuring specialized instruments and functional relations to advance scientific visual reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing SG datasets focus on generic natural contexts, lacking domain-specific scenes like physics experiments, which hinders intelligent monitoring and analysis in scientific environments.

Method: The authors construct PhysScene, a scene graph dataset tailored to physics experiments, emphasizing strong semantic constraints, high relation density, and functional (not just spatial) relationships.

Result: PhysScene poses new challenges for existing scene parsing algorithms and complements current benchmarks, demonstrating its value as a testbed for scientific visual reasoning.

Conclusion: PhysScene fills a gap in domain-specific scene graph datasets and provides a valuable resource for advancing relational reasoning in scientific experimental scenes.
...
### [169] [Echo-DM: Ultrasound Marker Removal via Conditional Latent Diffusion and Region-Aware Fusion](https://arxiv.org/abs/2606.09378)
*Zhiwei Wang,Tao Huang,Wentao Jiang,Muyi Li,Jianxin Liu,Jian Chen,Jie Zou,Yong Luo,Bo Du,Jing Zhang*

Main category: cs.CV

TL;DR: Echo-DM is a diffusion-based framework for removing artificial markers from clinical ultrasound images without requiring masks, using conditional latent diffusion and region-aware fusion to produce anatomically faithful results.


<details>
  <summary>Details</summary>
Motivation: Clinical ultrasound images contain artificial markers (calipers, text) that cause shortcut bias in deep learning models. Existing removal methods are either mask-dependent (vulnerable to error propagation) or mask-free but over-smooth texture and perturb backgrounds.

Method: Encoder-diffusion-decoder pipeline with DiT-based conditional latent diffusion for global restoration, plus region-aware fusion module for preservation-aware image-space refinement under end-to-end mask-free inference. Two instantiations: Echo-DM-V (VAE-based) and Echo-DM-R (RAE-based).

Result: Superior marker removal and strong anatomical fidelity compared to two-stage baselines on the Echo-PAIR dataset, with favorable quality-efficiency trade-offs.

Conclusion: Echo-DM effectively removes ultrasound markers without masks, preserves anatomy, and is compatible with diverse latent modules, offering a practical solution for downstream automated analysis.
...
### [170] [An Opticalmechanics Framework for Dynamic Estimation of Multibody Systems](https://arxiv.org/abs/2606.09383)
*Banglei Guan,Xuanyu Bai,Qingquan Chen,Zibin Liu,Dongcai Tan,Zhenbao Yu,Yang Shang,Qifeng Yu*

Main category: cs.CV

TL;DR: This paper proposes an optical-mechanics framework that combines image measurement with multibody dynamics modeling to estimate joint torques without physical contact sensors, achieving 0.46 Nm error on wrist torque estimation.


<details>
  <summary>Details</summary>
Motivation: Traditional human body dynamics analysis requires contact force/torque sensors and controlled lab environments, which limits applicability in real-world or hard-to-measure scenarios.

Method: A constrained multibody model describes system dynamics; image-measured kinematic quantities serve as non-contact inputs; unknown joint torque is identified via genetic-algorithm optimization minimizing discrepancy between model-predicted and image-measured kinematics.

Result: Wrist joint torque estimated from image data achieved MAE of 0.46 Nm vs sensor measurements; forward prediction of angular velocity achieved MAE of 0.006 rad/s vs image-measured results.

Conclusion: Combining image measurement with mechanical modeling enables non-contact dynamic estimation where direct force/torque measurement is difficult.
...
### [171] [Real-time body pose non-verbal communication with a consistency-based reliability measure](https://arxiv.org/abs/2606.09390)
*Alina Marcu,Dragos Costea,Cristina Lazar,Marius Leordeanu*

Main category: cs.CV

TL;DR: This paper studies recognizing communicative intent from 2D body pose alone, releasing a dataset of ten intents, benchmarking models on embedded GPUs, and using autoregressive self-consistency as an unsupervised reliability signal.


<details>
  <summary>Details</summary>
Motivation: Body movement communicates intent at distances where face/speech are unavailable, but existing resources combine multiple modalities or label actions rather than conveyed messages, lacking isolated body-pose communicative intent data.

Method: Release a real-frame full-body pose dataset of ten communicative intents; compare against synthetic datasets (IPC, MotionLCM, VEO3.1, Kimodo); benchmark skeleton graph classifiers and joint motion-forecasting networks on embedded GPU (NVIDIA Orin Nano); propose autoregressive self-consistency as an unsupervised reliability signal with a theoretical bound.

Result: Benchmarked models show performance metrics and frame rates on embedded GPU; self-consistency probability grows with consistent steps; conditions identified where confident predictions can still be false.

Conclusion: Body pose alone is a viable signal for communicative intent recognition on limited hardware, and autoregressive self-consistency serves as an effective unsupervised reliability indicator.
...
### [172] [CapRL++: Unified Reinforcement Learning with Verifiable Rewards for Dense Image and Video Captioning](https://arxiv.org/abs/2606.09393)
...
TL;DR: CapRL++ introduces a reinforcement learning framework with verifiable rewards (RLVR) for image/video captioning, using a vision-free LLM's question-answering accuracy as reward to train LVLMs, outperforming traditional supervised fine-tuning and matching much larger models like Qwen2.5-VL-72B.


<details>
  <summary>Details</summary>
Motivation: Traditional SFT-based captioning relies on expensive annotations and causes models to memorize specific answers, limiting generality, diversity, and scalability of descriptions.

Method: A reference-free RLVR framework: an LVLM generates a caption, then a separate vision-free LLM answers Multiple-Choice Questions based solely on that caption; reward is derived from the accuracy of those answers.

Result: Improves dense caption quality across 20+ image/video benchmarks; compact models achieve performance comparable to Qwen2.5-VL-72B and Qwen3-VL-235B-A22B.

Conclusion: CapRL++ validates that RLVR can produce generalizable, high-fidelity descriptions beyond the limitations of SFT, establishing a scalable and robust foundation for captioning.
...
### [173] [vesselFM-CT: Segmenting All Blood Vessels in CT Images for System-Level Cardiovascular Analysis](https://arxiv.org/abs/2606.09400)
*Bastian Wittmann,Chinmay Prabhakar,Suprosanna Shit,Bjoern Menze*

Main category: cs.CV

TL;DR: This paper introduces vesselFM-CT, the first model capable of robustly segmenting all blood vessels (from largest to smallest) in 3D CT images, using a novel TubeLoss function and iterative multi-step training to handle the structural heterogeneity of the cardiovascular system.


<details>
  <summary>Details</summary>
Motivation: Existing research focuses on narrow, isolated vascular segments, limiting the ability to assess systemic vascular health. The heterogeneity of vessel structures and anatomical background variations makes large-scale analysis of the entire cardiovascular system challenging.

Method: The authors propose vesselFM-CT, trained via an iterative multi-step process with a novel TubeLoss function designed to address the inherent heterogeneity of the cardiovascular system for segmenting all vessels in 3D CT images.

Result: vesselFM-CT outperforms all baselines in segmenting all blood vessels, from the largest cardiovascular components to minuscule mesenteric vessels, enabling automated and precise extraction of the entire cardiovascular system from CT images.

Conclusion: This work bridges the gap in systemic vascular analysis, unlocking clinical and technical perspectives such as automated disease classification and synthetic CT image generation.
...
### [174] [Leveraging Morphology for Historical Script Metrological Analysis](https://arxiv.org/abs/2606.09446)
*Malamatenia Vlachou Efstathiou,Raphaël Baena,Dominique Stutzmann,Mathieu Aubry*

Main category: cs.CV

TL;DR: This paper presents a deep learning architecture that learns character prototypes from line-level transcriptions to enable scalable, meaningful paleographic measurements. The method uses a transformer-based detection with prototype-based line reconstruction, achieving accurate character bounding box prediction and enabling analysis of characters, bi-grams, and spacing across a 14th-century manuscript (160 pages).


<details>
  <summary>Details</summary>
Motivation: Current handwritten text recognition enables large-scale transcription but provides limited access to interpretable visual measurements for paleography. There is a need for scalable, meaningful, and stable paleographic measurements from historical documents.

Method: The authors leverage a transformer-based detection architecture with a prototype-based line reconstruction module to learn prototypical characters, their occurrence, deformation, and positioning. The model is trained with only line-level transcription supervision (no character-level annotations needed).

Result: The method significantly improves over the Learnable Typewriter baseline, enables accurate character bounding box prediction, and produces paleographically relevant automatic measurements for characters, bi-grams, and spaces. A case study on 160 pages of a 14th-century manuscript demonstrates the ability to differentiate graphical profiles and discover subtle variations.

Conclusion: The approach is scalable and frugal in terms of training data (a single column of text suffices per page), enabling practical paleographic analysis of historical scripts. Data and code are publicly available.
...
### [175] [GD-MIL: Grade-Disentangled Multiple Instance Learning for Multimodal Biochemical Recurrence Prediction in Prostate Cancer](https://arxiv.org/abs/2606.09453)
*Dasari Naga Raju*

Main category: cs.CV

TL;DR: This paper evaluates whether H&E whole slide images (WSIs) provide prognostic signal for biochemical recurrence after prostatectomy beyond Gleason grade, using a rigorous benchmark on TCGA-PRAD. The authors find that imaging-only models underperform a clinical baseline, but their proposed Grade-Disentangled MIL (GD-MIL), which removes grade influence before late fusion with clinical variables, significantly outperforms both imaging-only and clinical models (C-index 0.704 vs. 0.687).


<details>
  <summary>Details</summary>
Motivation: To determine whether H&E whole slide images contain prognostic signal for biochemical recurrence beyond what is captured by Gleason grade, and whether multiple instance learning can reliably recover it, given that prior work may have inflated results due to methodological flaws like selecting checkpoints on evaluation folds.

Method: Five-fold cross-validation repeated over five seeds on TCGA-PRAD (487 patients, 101 BCR events) with strict out-of-fold scoring. Compared multiple MIL aggregators (ABMIL, CLAM, TransMIL, PatchGCN) and feature extractors (ResNet50 vs. pathology foundation models). Proposed GD-MIL: a gated-attention MIL encoder with gradient-reversal grade adversary to make slide representations invariant to Gleason grade, followed by late fusion with clinical variables.

Result: MIL aggregator choice has little effect (C-index 0.61-0.64 with UNI2-h), feature extractor dominates (ResNet50 0.566 vs. foundation models 0.639). Clinical Cox model achieves 0.687; no imaging-only model significantly outperforms it. GD-MIL achieves 0.704, significantly outperforming clinical baseline (p=0.0005) and best imaging-only model (p=0.039). Median risk split yields log-rank p<0.0001 (~20% vs ~70% BCR-free at 5 years).

Conclusion: H&E morphology contains prognostic information complementary to Gleason grade that can be extracted when explicitly disentangled from grade. GD-MIL demonstrates that combining grade-invariant histology features with clinical variables improves risk stratification for biochemical recurrence.
...
### [176] [Training-Free Generalized Few-Shot Segmentation through Open-Vocabulary Semantic Arbitration](https://arxiv.org/abs/2606.09474)
*Silas Kwabla Gah,Ebenezer Owusu*

Main category: cs.CV

TL;DR: Open-V is a training-free GFSS framework that coordinates frozen foundation models (SAM + CLIP) at inference time, achieving strong few-shot segmentation without any parameter adaptation.


<details>
  <summary>Details</summary>
Motivation: To explore whether GFSS can be solved through inference-time coordination of frozen semantic priors from foundation models, rather than requiring task-specific parameter adaptation.

Method: Combines SAM's Promptable Concept Segmentation with a K-shot CLIP support centroid, using calibrated per-pixel semantic arbitration, with no trainable components.

Result: On PASCAL-5i (1-shot), achieves base/novel/harmonic mIoU of 78.4/77.5/77.9, surpassing the strongest trained baseline by +17.7 HM. Validated across PASCAL-5i, COCO-20i, and ADE-OW.

Conclusion: Training-free coordination of foundation-model priors generalizes across GFSS and open-vocabulary settings, and support information can be incorporated via inference-time semantic grounding.
...
### [177] [Efficient Minimal Solvers for Visual-Inertial Relative Pose Estimation in Multi-Camera Systems](https://arxiv.org/abs/2606.09477)
*Tao Li,Zhenbao Yu,Banglei Guan,Jianli Han,Weimin Lv*

Main category: cs.CV

TL;DR: This paper proposes two efficient minimal solvers for multi-camera relative pose estimation that require only four point correspondences (instead of six) and reduce the problem to solving a univariate 6th-degree polynomial, leveraging IMU priors for improved efficiency in RANSAC frameworks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-camera relative pose estimation methods suffer from high computational complexity or require an excessive number of point correspondences, limiting their real-world applicability in autonomous vehicles, mobile devices, and UAVs.

Method: Two minimal solvers are proposed: (1) using vertical direction prior from IMUs, and (2) using rotation axis direction prior from IMUs. Both require only four point correspondences and reduce the problem to solving a univariate 6th-degree polynomial.

Result: Achieved superior computational efficiency and competitive accuracy compared to state-of-the-art algorithms on synthetic data and the KITTI benchmark.

Conclusion: The proposed solvers significantly reduce computational complexity and correspondence requirements (from 8th-degree to 6th-degree polynomials), making them particularly effective for RANSAC-based visual odometry applications.
...
### [178] [Optical Music Recognition for Real-World Manuscripts with Synthetic Data](https://arxiv.org/abs/2606.09479)
...
TL;DR: This paper addresses OMR for real-world manuscripts by showing that synthetic domain adaptation with MuNG annotations and Smashcima synthesis can significantly improve recognition, even without expensive fine-grained in-domain symbol annotations.


<details>
  <summary>Details</summary>
Motivation: Existing OMR systems fail on real-world manuscripts due to domain gaps between born-digital training data and diverse manuscript visuals. Heritage institutions lack resources to create large in-domain datasets, motivating the need for domain adaptation.

Method: The authors provide a baseline on real-world manuscripts with complex piano notation. They use MuNG (fine-grained music notation graph) annotations and Smashcima synthesis tool to generate synthetic data, then perform domain adaptation by combining synthetic images with limited real in-domain data.

Result: Direct transcriptions of in-domain data remain essential, but synthetic musical manuscript images bring significant improvement. Importantly, the symbols used in synthetic data do not need to be in-domain, avoiding expensive fine-grained annotation.

Conclusion: The work brings OMR closer to real-world application in preserving and promoting musical cultural heritage by enabling effective recognition on manuscripts with limited resources.
...
### [179] [ContextShift: A Controlled Benchmark for Context Dependence in Object Detection](https://arxiv.org/abs/2606.09495)
*Dan Zlotnikov,Alex Lazarovich,Ohad Ben-Shahar*

Main category: cs.CV

TL;DR: ContextShift is a controlled benchmark that systematically manipulates object-context relationships to reveal that object detectors suffer from prediction suppression (up to 227% more false negatives, 44% fewer predictions) under context change—failures masked by aggregate metrics like AP—and that context-aware augmentation improves robustness.


<details>
  <summary>Details</summary>
Motivation: Existing robustness evaluations rely on aggregate metrics (e.g., AP) over uncontrolled distribution shifts, which fail to capture how performance degrades specifically due to context changes while object appearance is preserved.

Method: ContextShift manipulates object-context relationships on COCO 2017 via geometric transformations and synthetic/natural background substitutions, using NPMI to define a continuous compatibility axis, and evaluates diverse detector architectures.

Result: False negatives increase up to 227%, prediction volume drops up to 44%, while false positives remain stable or decline; AP masks this suppression. Degradation stems from reduced valid detection candidates, not just lower confidence. Performance is non-monotonic along NPMI, peaking at intermediate values.

Conclusion: Context-aware augmentation improves robustness—every variant outperforms the baseline—by exposing models to object-context decoupling during training, partially recovering from prediction-suppression failures.
...
### [180] [Prisma-World: Camera-Controllable Multi-Agent Video World Model](https://arxiv.org/abs/2606.09507)
...
TL;DR: Prisma-World is a camera-controllable multi-agent world model that generates consistent multi-view videos by using joint geometry-aware denoising, multi-agent RoPE, relative camera geometry attention, and an overlap-decaying curriculum with minimap guidance, supported by a new large-scale UE5 dataset PrismaDataset.


<details>
  <summary>Details</summary>
Motivation: Existing video world models simulate the world from a single observer. Extending to multiple agents is challenging because independent generation leads to inconsistent objects, layouts, and appearances across overlapping views.

Method: Prisma-World processes all agent videos in a full-attention sequence, uses multi-agent RoPE to distinguish identities, injects relative camera geometry into attention for cross-view consistency, and employs overlap-decaying curriculum training with minimap-conditioned structural guidance.

Result: A single Prisma-World model generates high-fidelity multi-agent videos with flexible agent counts, camera controllability, improved cross-view consistency, and spatial grounding under minimap guidance.

Conclusion: Prisma-World effectively addresses multi-view consistency in multi-agent world modeling, enabling joint generation of spatially coherent videos across multiple agents with varying viewpoints.
...
### [181] [Securing Self-supervised Data Curation for Foundation Models Robustness](https://arxiv.org/abs/2606.09511)
*Sandeep Gupta,Roberto Passerone*

Main category: cs.CV

TL;DR: The paper proposes Poisoned Data Detectors (PDD) using ImageBind and classical ML classifiers (RF, KNN, NB, SVM) to detect poisoned data in SSL-curated datasets before foundation model training, with SVM-PDD achieving the best results across in-distribution and out-of-distribution scenarios.


<details>
  <summary>Details</summary>
Motivation: SSL-curated datasets often rely on unvetted external sources, increasing the risk of data poisoning. There is a need for active defense mechanisms to ensure dataset integrity before training foundation models, without requiring extensive human oversight.

Method: The authors combine the pretrained ImageBind model with traditional classifiers (Random Forest, k-Nearest Neighbors, Naive Bayes, Support Vector Machines) to build Poisoned Data Detectors (PDDs). The system is evaluated on 176,200 images from three datasets under three adversarial attack types, covering both in-distribution and out-of-distribution scenarios.

Result: SVM-PDD achieves superior performance across both in-distribution (Set3-Set5) and out-of-distribution (TrueFace and 140K RealFace) settings. The design demonstrates strong scalability and supports rapid integration of new attack detectors via ensemble methods.

Conclusion: The proposed PDD framework, particularly SVM-PDD, is an effective and scalable solution for detecting poisoned data in SSL-curated datasets, reducing risks before foundation model training while maintaining low human oversight requirements.
...
### [182] [SwiftVR: Real-Time One-Step Generative Video Restoration](https://arxiv.org/abs/2606.09516)
*Jiaqi Yan,Xiangyu Chen,Xinlin Zhong,Haibin Huang,Chi Zhang,Jie Liu,Jiantao Zhou,Xuelong Li*

Main category: cs.CV

TL;DR: SwiftVR is a streaming one-step generative video restoration framework that uses mask-free shifted-window self-attention and a lightweight Restoration-aware Autoencoder to achieve real-time 1080p video restoration on consumer-grade GPUs, reaching 31 FPS at 2560x1440 on an H100 and 26 FPS at 1920x1080 on an RTX 5090.


<details>
  <summary>Details</summary>
Motivation: Existing one-step diffusion-based VR models are difficult to deploy on consumer-grade GPUs due to two bottlenecks: quadratic spatial attention at high resolutions and the latency-memory overhead of large video autoencoders. Real-time video restoration for live streams requires high-resolution outputs under strict per-frame latency constraints.

Method: SwiftVR introduces (1) mask-free shifted-window self-attention that gathers spatial windows into dense tensors via deterministic indexing, using only standard dense SDPA calls without masks, cyclic shifts, padding, or sparse kernels, enabling transfer to consumer GPUs; and (2) a lightweight Restoration-aware Autoencoder for fast chunk-wise decoding with preserved reconstruction quality, under a causal chunk-wise protocol.

Result: On a single H100: 31 FPS at 2560x1440 and 14 FPS at 3840x2160. On an RTX 5090: 26 FPS at 1920x1080. All compared diffusion-based VR baselines exceed memory limits at 4K. First generative VR model to achieve real-time 1080p streaming on consumer GPUs with strong no-reference perceptual quality and lower inference cost.

Conclusion: SwiftVR is the first generative VR model to achieve real-time 1080p streaming on a consumer-grade GPU, successfully reducing both spatial attention and autoencoder bottlenecks while maintaining strong perceptual quality.
...
### [183] [Adversarial Attack and Disturbance Detection by Hadamard-Coded Output Representations for Object Detection and Semantic Segmentation](https://arxiv.org/abs/2606.09536)
*Lucas Görnhardt,Timo Bartels,Niklas Schwarz,Tim Fingscheidt*

Main category: cs.CV

TL;DR: This paper introduces HadamardNet, a framework using Hadamard-coded output representations for semantic segmentation and object detection. It proposes a novel decoding procedure that delivers optimal class-wise probabilities, measures prediction inconsistencies, and exploits them for adversarial attack detection—achieving state-of-the-art perturbation detection in a single pass with minimal performance loss on clean data.


<details>
  <summary>Details</summary>
Motivation: One-hot encodings produce poorly calibrated, overconfident models vulnerable to attacks, causing entropy-based detection to fail. Hadamard codes have helped image classification but remain unexplored in object detection and underperform in segmentation. Additionally, no prior work addressed intrinsic codeword inconsistencies or exploited codeword redundancy.

Method: (1) A novel decoding procedure for Hadamard codewords that solves an optimization problem using projection onto the probability simplex for optimal class-wise probabilities. (2) A measure of prediction inconsistency derived from the optimization. (3) Exploiting these inconsistencies for adversarial attack/disturbance detection. (4) HadamardNet: a framework integrating Hadamard codes into segmentation and detection models.

Result: State-of-the-art perturbation detection performance for both semantic segmentation and object detection in a single detection pass, while delivering equivalent or close-by reference performance on clean data.

Conclusion: Hadamard output encodings, when properly decoded with inconsistency exploitation, can effectively defend against adversarial attacks in both segmentation and detection tasks without sacrificing clean-data performance.
...
### [184] [A VideoMAE-v2 Approach to Zero-Shot Traffic Accident Anticipation](https://arxiv.org/abs/2606.09542)
*Siyuan Li,Xiaoyang Bi,Mengshi Qi*

Main category: cs.CV

TL;DR: The paper proposes a zero-shot traffic accident anticipation framework using VideoMAE-v2 with per-frame prediction head, achieving 2nd place in CVPR@AUTOPILOT 2026 competition without any target-domain training data.


<details>
  <summary>Details</summary>
Motivation: Traffic accident anticipation is safety-critical but difficult to scale due to the high cost of collecting annotated accident footage for every deployment scenario.

Method: Couples a VideoMAE-v2 backbone with a per-frame prediction head under a sliding-window protocol, bridging frame-level temporal risk estimation with coarsely labeled binary accident datasets.

Result: Achieved 2nd place in the 2026 CVPR@AUTOPILOT Zero-Shot Accident Anticipation competition.

Conclusion: The proposed zero-shot framework can effectively generalize to unseen dashcam footage without requiring target-domain training data, demonstrating practical scalability for real-world deployment.
...
### [185] [Streaming Interventions: Can Video Large Language Models Correct Mistakes as They Occur?](https://arxiv.org/abs/2606.09547)
...
TL;DR: The paper introduces Ego-MC-Bench, a benchmark for evaluating proactive mistake-correction capabilities in video LLMs for cooking task guidance. To address training data scarcity, they also create Ego-CoMist, a synthetic counterfactual dataset, and show fine-tuning on it improves smaller video LLMs.


<details>
  <summary>Details</summary>
Motivation: Existing video LLMs lack the ability to proactively intervene when users make mistakes during real-world task guidance. No benchmark or training data exists to evaluate or teach this crucial capability.

Method: (1) Created Ego-MC-Bench benchmark for evaluating proactive mistake corrections in cooking scenarios. (2) Developed Ego-CoMist, a counterfactual synthetic dataset created by transforming non-interactive cooking videos into supervised training examples with interventions. (3) Evaluated SOTA video LLMs and fine-tuned smaller models on the new dataset.

Result: State-of-the-art video LLMs perform poorly on Ego-MC-Bench, revealing it as a highly challenging benchmark. Fine-tuning on Ego-CoMist yields significant performance gains, especially for smaller, more efficient video LLMs suitable for edge devices.

Conclusion: Proactive intervention capability is critical yet lacking in current video LLMs. The Ego-CoMist synthetic data approach effectively mitigates training data limitations, enabling smaller models to become viable task guidance assistants on edge devices.
...
### [186] [TUDSR: Twice Upsampling-Diffusion for Higher Super-Resolution](https://arxiv.org/abs/2606.09608)
*Zhiqiang Wu,Yitong Dong,Xian Wei*

Main category: cs.CV

TL;DR: TUDSR proposes a two-stage upsampling-diffusion framework (TUDSR-S) that achieves high-quality super-resolution at 1024² and 2048² by tackling issues with tiled diffusion when upsampling ratios and resolutions exceed model limits, outperforming existing methods on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based SR models produce poor quality at high resolutions (e.g., 2048²) due to the upsampling ratio exceeding the model's native-supported ratio (e.g., ×8 vs ×4) and limitations of native-supported resolution. Training native high-res models is computationally prohibitive.

Method: Two-stage framework: Stage 1 trains at R-resolution using a one-step GAN (generator+discriminator); Stage 2 introduces a looped chunk-based training strategy at N×R-resolution, also adapted to one-step GAN architecture. Built on SD2.1-base (TUDSR-S).

Result: TUDSR-S achieves state-of-the-art performance across multiple benchmarks, generating high-quality images at 1024² and 2048² resolutions, significantly outperforming existing approaches.

Conclusion: TUDSR effectively addresses resolution and upsampling ratio limitations in diffusion-based SR without requiring larger architectures or massive computational resources, enabling high-quality super-resolution at very high resolutions.
...
### [187] [ATN3D: Density-Aware LiDAR-Radar Early 3D Object Detection Under Extreme Sparsity](https://arxiv.org/abs/2606.09634)
*Debojyoti Biswas,Xianbiao Hu*

Main category: cs.CV

TL;DR: ATN3D is a LiDAR-Radar fusion framework for long-range 3D object detection in automated vehicles. It addresses sparsity challenges via density-aware fusion, occupancy-gated aggregation, evidence-conditioned channel attention, and a range-aware loss, achieving significant mAP gains (up to +8.41% in heavy fog) on the VoD benchmark.


<details>
  <summary>Details</summary>
Motivation: Long-range 3D detection (>30m) is critical for AV safety but suffers from extreme sensor sparsity. Early multimodal fusion discards sparsity cues and injects noise, while uniform supervision under-optimizes far/small objects, delaying earliest detection.

Method: (i) Density-aware early fusion with cross-modal gating conditioned on per-voxel sparsity and Radar evidence, (ii) Occupancy-gated neighborhood aggregation using circular kernels to filter credible cells, (iii) Evidence-conditioned channel self-attention for adaptive weighting by weather/range, (iv) Range-aware loss that re-balances classification and localization by distance.

Result: On VoD benchmark: +3.55% mAP (clear), +8.41% mAP (heavy fog); for >30m objects: +3.33% (clear), +2.09% (heavy fog), surpassing strong baselines.

Conclusion: ATN3D enables earlier and more reliable long-range detections under sparse sensing conditions, demonstrating the effectiveness of sparsity-aware design for on-road traffic perception.
...
### [188] [CineDance: Towards Next-Generation Multi-Shot Long-Form Cinematic Audio-Video Generation](https://arxiv.org/abs/2606.09639)
*Yuheng Chen,Teng Hu,Yuji Wang,Qingdong He,Zhucun Xue,Qianyu Zhou,Xiangtai Li,Lizhuang Ma,Jiangning Zhang,Dacheng Tao*

Main category: cs.CV

TL;DR: CineDance-1M is a large-scale, open-source dataset for long-form, multi-shot text-to-audio-video generation, featuring 92.8-second average length, 24.2 continuous shots per video, and a rigorous three-stage curation pipeline. The authors also propose CineBench for evaluation and adapt LTX-2.3 to validate their approach.


<details>
  <summary>Details</summary>
Motivation: Open-source video generation models lag behind commercial systems due to scarcity of high-quality, multi-shot, long-form training data with synchronized audio and video.

Method: Three-stage curation pipeline: (i) diverse sourcing and comprehensive cleansing, (ii) film-theory-inspired narrative parsing, and (iii) hierarchical dual-modal captioning. They also propose CineBench with a six-dimensional human-aligned metric system.

Result: The adapted LTX-2.3 (CineDance) demonstrates exceptional single-modality quality, precise audio-video alignment, and robust subject/environment consistency, validating the curation strategy.

Conclusion: CineDance-1M provides a solid foundation for accelerating future research in multi-shot, long-form joint audio-video generation.
...
### [189] [MAVIS: Multi-Agent Video Retrieval via Structured Video Understanding](https://arxiv.org/abs/2606.09641)
*Jie Zhang,Qilang Ye,Hao Zhou,Haochen Liang,Fei Luo*

Main category: cs.CV

TL;DR: MAVIS is a multi-agent framework that rethinks video retrieval as cooperative reasoning instead of brute-force search, using a Structured Semantic Library, planner-dispatched agents, and a Logic-aware Debate mechanism with veto protocol to prune mismatches and identify controversial candidates for verification, achieving competitive performance without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current embedding-based full-corpus video retrieval suffers from computational inefficiency and semantic asymmetry between dense videos and sparse textual queries.

Method: MAVIS uses a Structured Semantic Library for attribute-level indexing, a planner to decompose intents into sub-tasks, specialized agents to nominate candidates, and a Logic-aware Debate mechanism with veto protocol to prune logical mismatches.

Result: Competitive performance on MSR-VTT, MSVD, and ActivityNet without task-specific fine-tuning.

Conclusion: MAVIS offers a scalable, interpretable alternative to traditional dual-encoder approaches by rethinking retrieval as cooperative reasoning.
...
### [190] [Do Video Foundation Models Understand Intuitive Physics? A Layerwise Probing Analysis](https://arxiv.org/abs/2606.09646)
*Samuele Punzo,Niccolò Caselli,Ippokratis Pantelidis,Francesco Massafra,Salvatore Lo Sardo,Mohammadreza Salehi*

Main category: cs.CV

TL;DR: This paper investigates whether pretrained video foundation models encode intuitive physics in their frozen representations, comparing V-JEPA (strongest), VideoMAE (competitive), and LTX-Video (weaker but non-trivial) across benchmarks, finding that physics information is weakest in early layers and most accessible at intermediate-to-late depth, with temporal order being critical for performance.


<details>
  <summary>Details</summary>
Motivation: To determine whether video foundation models implicitly learn intuitive physics (e.g., object permanence, gravity) without explicit supervision, and how this knowledge varies across different model architectures, layers, and probing methods.

Method: Frozen-feature probing on IntPhys2 and Minimal Video Pairs (MVP) benchmarks, comparing V-JEPA (predictive joint-embedding), VideoMAE (masked reconstruction), and LTX-Video (diffusion-based) models, with layerwise and temporal-order analysis.

Result: V-JEPA achieves strongest results, especially with temporal-dynamics probes; VideoMAE is competitive; LTX-Video shows weaker signal. Physics information is weakest in early layers and peaks at intermediate-to-late depth. Disrupting frame order significantly reduces performance.

Conclusion: Intuitive physics knowledge emerges reliably in pretrained video representations, but accessibility strongly depends on pretraining paradigm, representational depth, and readout mechanism.
...
### [191] [Visual Prompting Meets Feature Reconstruction-Based Anomaly Detection with Dual-Teacher Supervision](https://arxiv.org/abs/2606.09670)
...
TL;DR: This paper improves anomaly detection by introducing visual prompting (foreground-background masking), unfreezing the teacher in student-teacher models, and using diffusion-generated synthetic data augmentation, achieving 3.5% improvement on AeBAD dataset.


<details>
  <summary>Details</summary>
Motivation: Current anomaly detection methods fail in real-world scenarios when foundational assumptions (consistent scale, viewpoint, background, illumination, centered placement) are violated, limiting their practical applicability.

Method: (1) Visual prompting pipeline with foreground-background masking; (2) Unfreezing teacher in student-teacher models for domain adaptability; (3) Data augmentation using diffusion-generated synthetic images; with Masked Multiscale Reconstruction (MMR) as backbone.

Result: Achieved a 3.5 percentage point improvement over previous state-of-the-art on the AeBAD dataset.

Conclusion: The proposed three contributions collectively address real-world anomaly detection challenges, significantly outperforming prior methods on a more realistic benchmark.
...
### [192] [SoccerNet 2026 Player-Centric Ball-Action Spotting:Retraining and Post-Processing Extensions to the FOOTPASS Baselines](https://arxiv.org/abs/2606.09679)
*Parthsarthi Rawat*

Main category: cs.CV

TL;DR: A system for SoccerNet 2026 player-centric ball-action spotting, extending FOOTPASS baselines with gradient checkpointing, GNN-DST fusion, square-root class weighting, and post-processing pipelines, achieving 0.548 Macro F1 on test set.


<details>
  <summary>Details</summary>
Motivation: To improve player-centric action spotting in broadcast soccer video by addressing class imbalance and combining graph-based tactical context with per-player visual features, building on existing FOOTPASS baselines.

Method: Four extensions: (1) gradient checkpointing for full fine-tuning on single GPU; (2) GNN logits fused into DST encoder; (3) square-root frequency class weighting for handling 213:1 pass-to-tackle imbalance; (4) post-processing pipeline with per-class logit gating, temporal refinement, jersey re-assignment, and two-model ensemble.

Result: 0.548 Macro F1 on test set; 0.446 on challenge set.

Conclusion: The proposed extensions effectively improve ball-action spotting performance, particularly addressing class imbalance and combining tactical context with visual features.
...
### [193] [GenEyePose: Patient-Free, Knowledge-Based Saccadic Eye Movement Modeling for Digital Neurophysiologic Biomarker Development](https://arxiv.org/abs/2606.09681)
...
TL;DR: This paper proposes the first fully synthetic, patient-free pipeline for generating eye movement data to train a deep learning classifier that distinguishes normal from abnormal saccades (hypometria/hypermetria), achieving an AUROC of 0.76 and sensitivity of 0.71 on real clinical data.


<details>
  <summary>Details</summary>
Motivation: Eye movements are sensitive biomarkers for neurologic conditions, but current AI-enabled video-oculographic solutions face privacy issues and scarce datasets. There is a need for robust, privacy-preserving, and generalizable tools for screening and localizing brain abnormalities without relying on real patient data.

Method: The authors developed a fully synthetic, patient-free multimodal eye movement generation pipeline to create training data. A deep learning classifier was then trained on this synthetic dataset to classify saccadic accuracy as normal, hypometria, or hypermetria. The model was evaluated on real-world clinical data.

Result: The classifier achieved an AUROC of 0.76 and a sensitivity of 0.71 when tested on real clinical data, demonstrating generalization capability from synthetic to real-world settings.

Conclusion: Synthetic data shows strong potential for generalizing to clinical applications, including as a screening tool in at-home and emergency room settings, or for precise neuroanatomic localization of brain abnormalities.
...
### [194] [Cranio-Diff: Diffusion-based Cross-domain Craniofacial Reconstruction with 2D X-ray Skull Guidance and Structural Identity Constraints](https://arxiv.org/abs/2606.09699)
*Ravi Shankar Prasad,Naresh Gurjar,Shashank Baghel,Chirag,Dinesh Singh*

Main category: cs.CV

TL;DR: Cranio-Diff is a diffusion-based framework that uses ControlNet with biometric text conditioning to reconstruct faces from 2D X-ray skull images, outperforming existing methods on a new 4320-paired skull-face dataset across age and BMI variations.


<details>
  <summary>Details</summary>
Motivation: Existing generative models like CycleGAN and diffusion models fail in craniofacial reconstruction due to cross-modality misalignment of structural identity between skull (X-ray) and face (optical) domains.

Method: Cranio-Diff integrates skull-conditioned structural guidance via ControlNet with biometric text conditioning to generate semantically and structurally aligned faces from 2D X-ray skull images.

Result: Evaluated on 4320 paired samples across three age groups (25, 45, 65) and three BMI variations (-10%, baseline, +10%) from 120 subjects. Outperforms recent approaches in image quality (FID, IS, SSIM, LPIPS, PSNR, ArcFace) and retrieval tasks (recall@k, mAP@k, MRR@k).

Conclusion: The proposed method can serve as an alternative tool for aiding forensic investigations.
...
### [195] [HDSL: A Hierarchical Domain-Specific Language for Structured 3D Indoor Scene Generation and Localized Editing with LLM Agents](https://arxiv.org/abs/2606.09738)
*Letian Li,Chao Shen,Shuzhao Xie,Chenghao Gu,ZhengXiao He,Yu Meng,Xin Yang,Wenyuan Jiang,Zhi Wang*

Main category: cs.CV

TL;DR: The paper proposes HDSL, an XML/CSS-style DSL for 3D indoor scenes that represents scenes as hierarchical trees with local coordinates, enabling LLMs to generate and edit scenes through structured program generation and local program repair. The approach improves generation quality, editing efficiency (5.22× token reduction, 6.19× faster runtime), and object/scene alignment compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based systems for 3D indoor scene generation rely on scene graphs or global constraint lists that are compact but underspecify local geometry and make it difficult to localize instruction-based edits.

Method: HDSL (Hierarchical Descriptive Scene Language) - an XML/CSS-style DSL representing rooms, regions, objects, and support surfaces as a tree with local coordinates. Uses LLM agents to generate HDSL subtrees with bounded verification, multimodal asset retrieval, force-directed layout optimization, and Hierarchical Retrieval-Augmented Generation (HRAG) for editing with three-way merge.

Result: HDSL improves average object coverage, text-scene alignment, and generation time over full text-to-scene baselines. For editing, HRAG reduces token use by 5.22× and runtime by 6.19×, produces valid DSL for all eight paired edits, and better preserves unrelated scene objects.

Conclusion: The hierarchical structured approach (HDSL) effectively addresses the underspecification and edit localization problems in LLM-based 3D scene generation, outperforming baselines in both generation quality and editing efficiency.
...
### [196] [Hybrid Robustness Verification for Spatio-Temporal Neural Networks](https://arxiv.org/abs/2606.09746)
*Sherwin Varghese,Matthew Wicker,Alessio Lomuscio*

Main category: cs.CV

TL;DR: STBP is a verification framework for 3D CNNs that exploits realistic spatio-temporal adversarial constraints (e.g., attacking only subsets of frames or patches) to compute exact closed-form bounds for the first convolutional layer and propagate certified bounds through deeper layers, achieving 1.7x higher certified robust accuracy with better scalability than existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing robustness verification methods are either too conservative (loose approximations) or computationally prohibitive. They also rely on lp-norm perturbations that assume noise in every frame, which is unrealistic for video—real adversarial perturbations are structured in space and time, confined to lower-dimensional subspaces.

Method: STBP (Spatio-Temporal Bound Propagation): computes an exact closed-form characterization of the first convolutional layer under spatio-temporal constraints (subset of frames or patches within consecutive frames), then propagates certified bounds through deeper layers using scalable approximation methods.

Result: Achieves 1.7x higher certified robust accuracy compared to existing verification approaches under identical perturbation budgets, with significantly improved scalability.

Conclusion: Modelling realistic spatio-temporal adversarial constraints enables tighter and more scalable robustness verification for 3D CNNs in safety-critical video applications like action recognition, autonomous driving, and medical imaging.
...
### [197] [SemDINO: A DINOv3-Driven Network for Cross-Temporal Semantic Alignment in Change Detection](https://arxiv.org/abs/2606.09772)
*Xinyu Tong,Meihua Zhou,Jinxiao Sun,Yingjie Tang,Lei Wang*

Main category: cs.CV

TL;DR: SemDINO is a novel end-to-end semantic change detection network that integrates dual-branch encoder (CNN + frozen DINOv3), multi-scale temporal interaction, semantic purification, and change enhancement modules to achieve superior performance in complex remote sensing scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing SCD methods suffer from insufficient cross-temporal alignment, weak multi-scale representation, and poor robustness to pseudo-changes caused by illumination, season, and registration noise.

Method: SemDINO integrates: (1) dual-branch encoder with CNN backbone and frozen DINOv3 features via gated pyramid fusion; (2) multi-scale temporal bidirectional transformer interaction (M-TBTT) for cross-temporal alignment; (3) semantic purification (SCP), bidirectional change enhancement (BiChangeEnhance), and multi-scale change enhancement (MCE) modules; (4) multi-branch CD prediction head for binary change mask, bi-temporal semantic maps, and edge constraint.

Result: Extensive experiments on public remote sensing CD datasets show SemDINO achieves superior performance and generalization ability against state-of-the-art methods, especially in complex scenarios with interference factors.

Conclusion: The proposed unified framework effectively addresses key SCD challenges and outperforms existing methods in complex remote sensing scenarios.
...
### [198] [POTATR: A Lightweight Image-to-Graph Model for Page-Level Table Extraction](https://arxiv.org/abs/2606.09788)
*Brandon Smock,Libin Liang,Max Sokolov,Amrit Ramesh,Valerie Faucon-Morin,Tayyibah Khanam,Maury Courtland*

Main category: cs.CV

TL;DR: POTATR is a lightweight 29M parameter model for page-level table extraction that outperforms larger models while being 130x faster and 300x cheaper.


<details>
  <summary>Details</summary>
Motivation: Current table extraction methods require billions of parameters, hundreds of autoregressive steps, or costly API inference, lacking efficiency for large-scale document processing.

Method: POTATR extends the Table Transformer (TATR) into a lightweight 29M parameter image-to-graph model for contextualized page-level table extraction.

Result: Achieves GriTS_Con of 0.964 on PubTables-v2 Single Pages, outperforming all tested models including frontier MLLMs, with 130x faster speed and 300x lower cost.

Conclusion: POTATR enables efficient, spatially grounded page-level TE that composes with other models for scanned documents and full-document extraction via cross-page merging.
...
### [199] [End-to-End Optimization of Incoherent Imaging for Classification Under Detector-Limited Readout](https://arxiv.org/abs/2606.09792)
*Archer Wang,Joshua Chen,Sachin Vaidya,Marin Soljačić*

Main category: cs.CV

TL;DR: This paper analyzes when end-to-end co-optimization of phase masks and neural networks improves classification over conventional lenses, finding gains primarily under constrained detector readout, especially with low noise and when discriminative features are at lower spatial frequencies.


<details>
  <summary>Details</summary>
Motivation: To characterize when and why end-to-end co-optimization of optical front-ends and neural network back-ends outperforms conventional lens-based imaging, as a formal understanding is currently lacking despite widespread application.

Method: The authors prove theoretical limits (ideal-channel mutual information ceiling), analyze scenarios with unconstrained vs. constrained detector readout, examine the effects of noise and spatial frequency structure, and validate predictions on synthetic data and benchmarks (MNIST, FashionMNIST, SVHN).

Result: Under full detector readout, a conventional focusing lens approaches the theoretical ceiling and co-optimization yields no gain. Gains appear under constrained readout (coarse sampling or limited measurements), are largest with low detector noise, and depend on the spectral structure of the task.

Conclusion: End-to-end optical-neural co-design improves classification mainly under constrained detector readout and specific noise/spectral conditions, providing a theoretical framework to predict when such co-optimization is beneficial.
...
### [200] [Beyond Spherical Harmonics: Rethinking Appearance Models for Radiance Reconstruction](https://arxiv.org/abs/2606.09794)
*Ewa Miazga,Jorge Condor,Piotr Didyk*

Main category: cs.CV

TL;DR: This paper introduces a novel Normalized Anisotropic Spherical Gabor (NASG) function for efficient, high-quality view-dependent appearance modeling in scene reconstruction. Compared to traditional spherical harmonics (SH), NASG captures high-frequency effects (e.g., specular reflections, glints) with up to 5x better memory efficiency and faster evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing view-dependent appearance models (e.g., based on low-order spherical harmonics) struggle to capture high-frequency angular effects like specular reflections without excessive memory and computational cost. The goal is to find a more compact and efficient spherical representation that preserves high-frequency detail.

Method: The authors systematically evaluate a wide range of spherical functions for scene reconstruction, some introduced to graphics/vision for the first time. Based on empirical insights, they propose the Normalized Anisotropic Spherical Gabor (NASG) function, designed to efficiently model high-frequency appearance while keeping a compact representation.

Result: The NASG function achieves higher-quality reconstruction of view-dependent phenomena (e.g., glints) compared to existing approaches, while being up to 5x more memory-efficient and more efficient to evaluate. Validation was performed on radiance-field reconstruction tasks.

Conclusion: The proposed NASG function effectively addresses the trade-off between representation quality and memory/computation cost in view-dependent appearance modeling, offering a superior alternative to low-order SH for high-frequency effects.
...
### [201] [Echo-Memory: A Controlled Study of Memory in Action World Models](https://arxiv.org/abs/2606.09803)
...
TL;DR: Echo-Memory presents a controlled study isolating memory mechanisms in action-conditioned world models, finding that raw context and state-space recurrence outperform compressed memory, and that replay quality alone is insufficient for evaluating world memory.


<details>
  <summary>Details</summary>
Motivation: Existing memory designs in action-conditioned world models are hard to compare because gains are entangled with backbone, training, retrieval, and evaluation differences. The central failure is often memory rather than local image synthesis.

Method: Echo-Memory fixes the action-to-video interface and varies only how history is stored and read. Under a shared video diffusion backbone, it compares raw context, compression-based memory, spatial summaries, and state-space recurrence across capacity, compression, read-out, and recurrence axes.

Result: Raw context is a strong baseline and improves open-domain return more than replay metrics. Compactness is not a substitute for capacity. Block-wise state-space recurrence is the strongest open-domain return mechanism.

Conclusion: Replay fidelity is not a sufficient proxy for remembering a world. The structure of implicit memory matters as much as the decision to use it. The paper provides a compact protocol for studying memory beyond isolated replay metrics.
...
### [202] [PTL-Diffusion: Manifold-Aware Diffusion with Periodic Terminal Laws](https://arxiv.org/abs/2606.09816)
*Danqi Zhuang,Jisui Huang,Xiaoyue Xi,Andrew Kiggins,Xiaojie Wang,Ke Chen,Yue Wu*

Main category: cs.CV

TL;DR: PTL-Diffusion replaces the standard single Gaussian terminal distribution in diffusion models with a periodic family of Gaussian terminal laws, embedding phase structure directly into the forward noising process to better capture low-dimensional manifold structure in data.


<details>
  <summary>Details</summary>
Motivation: Standard diffusion models use a single unstructured Gaussian terminal distribution, which provides little explicit structure for data concentrated near low-dimensional manifolds, forcing the reverse model to recover manifold-level structure almost entirely from an unstructured reference.

Method: PTL-Diffusion uses a periodically forced Ornstein-Uhlenbeck forward process that converges to a periodic family of Gaussian terminal laws instead of a single invariant law. It derives closed-form forward marginals, limiting periodic distributions, and explicit Gaussian reverse posteriors, plus an invariant-average regularization term coupling phase-conditioned reverse dynamics.

Result: Experiments on torus/cylinder point clouds and Olivetti faces show PTL-Diffusion improves manifold-level distributional matching over DDPM baselines, reducing phase-conditioned errors, feature-space covariance errors, and nearest-neighbour manifold distances.

Conclusion: Structured terminal reference laws are a promising direction for diffusion models, motivating more expressive phase constructions and larger-scale evaluations.
...
### [203] [OmniGameArena: A Unified UE5 Benchmark for VLM Game Agents with Improvement Dynamics](https://arxiv.org/abs/2606.09826)
...
TL;DR: Introduces OmniGameArena (12 Unreal Engine 5 games with solo/PvP/coop) and IDC (agentic-reflection for skill refinement) to evaluate VLM agents beyond single-attempt scores.


<details>
  <summary>Details</summary>
Motivation: Current game benchmarks for VLM agents lack multi-attempt evaluations, unified protocols for heterogeneous agents, and support for PvP/coop modes beyond solo play.

Method: Builds 12 new Unreal Engine 5 games with unified action interfaces; proposes IDC where a reflector LLM iteratively refines skill prompts via tool use across multiple rounds.

Result: Reports cold-start leaderboard for 12 VLM agents and IDC observables (score evolution, held-out generalization) for 4 top agents.

Conclusion: OmniGameArena and IDC provide richer evaluation of VLM agents by exposing learning dynamics and generalization, not just static scores.
...
### [204] [Latent Spatial Memory for Video World Models](https://arxiv.org/abs/2606.09828)
*Weijie Wang,Haoyu Zhao,Yifan Yang,Feng Chen,Zeyu Zhang,Yefei He,Zicheng Duan,Donny Y. Chen,Yuqing Yang,Bohan Zhuang*

Main category: cs.CV

TL;DR: Mirage introduces latent spatial memory for video world models, storing 3D scene information directly in diffusion latent space instead of pixel space, achieving 10.57× faster generation and 55× memory reduction while maintaining state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing video world models use explicit point cloud memory in RGB space, which is computationally expensive (repeated rendering/VAE encoding) and lossy (round trip through pixel space discards rich latent features).

Method: Mirage constructs a persistent 3D cache in diffusion latent space by lifting latent tokens into 3D via depth-guided back-projection, and queries it via direct latent-space warping for novel view synthesis, eliminating pixel-space reconstruction.

Result: Latent spatial memory achieves up to 10.57× faster end-to-end video generation and 55× reduction in memory footprint vs. explicit 3D baselines. State-of-the-art on WorldScore and strong on RealEstate10K.

Conclusion: Operating spatial memory directly in latent space is both more efficient and more effective than pixel-space approaches, leveraging the geometric prior of diffusion models for superior video world modeling.
