...
TL;DR: CraterBench-R benchmark transforms crater analysis from detection to retrieval, showing self-supervised ViTs with in-domain pretraining excel at instance-level matching, with proposed token aggregation method achieving near-full accuracy with much less storage.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches treat craters as detection problems, but scientific workflows like catalog deduplication, cross-observation matching, and morphological analog discovery are inherently retrieval tasks that require instance-level matching capabilities.

Method: Formulate crater analysis as instance-level image retrieval; create CraterBench-R with 25,000 crater identities; evaluate various architectures; propose instance-token aggregation method that selects K seed tokens, assigns remaining tokens via cosine similarity, and aggregates clusters into single representative tokens; implement two-stage pipeline with single-vector shortlisting followed by instance-token reranking.

Result: Self-supervised Vision Transformers with in-domain pretraining outperform generic models; retaining multiple ViT patch tokens improves accuracy but is storage-inefficient; instance-token aggregation at K=16 improves mAP by 17.9 points over raw token selection; at K=64 matches accuracy of using all 196 tokens with less storage; two-stage pipeline recovers 89-94% of full late-interaction accuracy while searching small candidate sets.

Conclusion: Crater analysis should be approached as retrieval rather than detection; self-supervised ViTs excel at this task; proposed instance-token aggregation method provides scalable solution for planetary-scale applications; benchmark enables future research in crater retrieval.
...
Method: Proposes a fully automatic parameter optimization framework using modified crow search algorithm (CSA) with superior local search, search-space-aware global search, objective-driven balance, and chaotic diagonal linear uniform initialization for better convergence.

Result: Outperforms manual settings and original CSA with 4.19% average fitness improvement, 4.89% improvement on CHILL@UK metric, 3.82% on RPI_AXIS metric. Maintains fine details better in qualitative results.

Conclusion: The framework is effective and robust across different imaging machines, datasets, and reconstruction methods, demonstrating superiority in automatic parameter optimization for CBCT iterative reconstruction.
...
TL;DR: Researchers identify a "perception-integration gap" in Vision-Language Models where models can perceive visual content but fail to integrate it during reasoning. They introduce DISSECT benchmark to diagnose this gap across 12K chemistry/biology questions.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks conflate perception and integration into single accuracy scores, hiding failures where models correctly extract visual information but fail to use it during reasoning. The paper aims to systematically expose this perception-integration gap.

Method: Develop DISSECT benchmark with 12,000 questions across Chemistry (7K) and Biology (5K). Evaluate each question under 5 input modes: Vision+Text, Text-Only, Vision-Only, Human Oracle, and novel Model Oracle where VLM first verbalizes image then reasons from its description.

Result: Chemistry has lower language-prior exploitability than Biology, making it harder test of visual reasoning. Open-source models score higher with Model Oracle than raw images, showing integration bottleneck. Closed-source models show no gap, indicating they've bridged perception-integration.

Conclusion: The perception-integration gap is a critical failure mode in VLMs. The Model Oracle protocol provides model-agnostic diagnostic tool to decompose performance and identify integration failures, revealing this gap as frontier separating open-source from closed-source multimodal capabilities.
...
TL;DR: Telescope is a two-stage detection model for ultra-long range autonomous driving that achieves 76% relative improvement in mAP for detecting objects beyond 250 meters compared to state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Autonomous highway driving for heavy trucks requires detecting objects at ranges beyond 500 meters due to braking distance requirements at high speeds. Current object detectors fail at these distances because vehicles occupy only a few pixels, and LiDAR sensors have limited effective range due to quadratic resolution loss with distance, making image-based detection the most scalable solution.

Method: Telescope is a two-stage detection model with a novel re-sampling layer and image transformation specifically designed to address the challenges of detecting small, distant objects. It uses a powerful detection backbone and requires minimal computational overhead.

Result: Telescope achieves 76% relative improvement in mAP for ultra-long range detection (improving from absolute mAP of 0.185 to 0.326 at distances beyond 250 meters) compared to state-of-the-art methods while maintaining strong performance across all detection ranges.

Conclusion: Telescope provides an effective image-based solution for ultra-long range object detection in autonomous driving, addressing the limitations of current detectors and LiDAR sensors for long-haul heavy truck applications.
...
TL;DR: A comprehensive survey on video generation technology evolution from GANs to diffusion models, AR-based approaches, and multimodal integration, with applications in VR/AR, education, and world models.


<details>
  <summary>Details</summary>
Motivation: Existing reviews on video generation are too narrow, focusing only on specific techniques like GANs or diffusion models, or particular tasks like video editing. There's a lack of comprehensive perspective on the field's evolution, especially regarding Auto-Regressive models and multimodal information integration.

Method: The survey provides a systematic review tracing video generation evolution from early GANs to dominant diffusion models, then to emerging AR-based and multimodal techniques. It analyzes foundational principles, key advancements, and comparative strengths/limitations, and explores multimodal integration trends.

Result: The survey bridges historical developments with contemporary innovations, offering insights to guide future research in video generation applications including virtual/augmented reality, personalized education, autonomous driving simulations, digital entertainment, and advanced world models.

Conclusion: This comprehensive survey addresses gaps in existing literature by providing a holistic view of video generation evolution, emphasizing the importance of AR models and multimodal integration, and offering guidance for future research directions in this rapidly advancing field.
...
TL;DR: EchoTrust is an evidence-driven Actor-Verifier framework for trustworthy reasoning in echocardiography visual language models, addressing template shortcuts and spurious explanations in automated cardiac analysis.


<details>
  <summary>Details</summary>
Motivation: Automated intelligent analysis of echocardiographic data is challenging due to complex cardiac dynamics and strong view heterogeneity. Existing VLM methods are vulnerable to template shortcuts and spurious explanations when directly mapping video and question to answer.
...
Conclusion: EchoTrust addresses critical limitations in existing echocardiography VLM methods by introducing structured intermediate analysis and role-based verification for improved reliability and interpretability in clinical decision support.
...
TL;DR: A vision-language framework for food-item-level nutritional analysis using paired before-and-after eating images, leveraging natural language prompts instead of segmentation masks.


<details>
  <summary>Details</summary>
Motivation: Current image-based dietary assessment methods are limited - they use single pre-consumption images, provide only coarse meal-level estimates, cannot determine actual consumption, and often require restrictive inputs like depth sensing or multi-view imagery.

Method: A vision-language framework that uses natural language prompts to localize specific food items and estimate their weight directly from single RGB images. For consumption estimation, it predicts weight differences between paired before-and-after images using a two-stage training strategy.

Result: The method was evaluated on three publicly available datasets and demonstrated consistent improvements over existing approaches, establishing a strong baseline for before-and-after dietary image analysis.

Conclusion: The proposed framework provides a simpler, more accurate approach to food-item-level nutritional analysis using paired before-and-after images, overcoming limitations of existing methods that rely on restrictive inputs and provide only coarse estimates.
...
TL;DR: MTA-Agent creates high-quality multi-hop vision-language training data for multimodal deep-search agents, achieving SOTA performance on challenging benchmarks.


<details>
  <summary>Details</summary>
Motivation: MLLMs have strong visual understanding but lack complex multi-step reasoning requiring deep searching and integrating visual evidence with external knowledge.

Method: Proposed MTA-Agent automatically selects tools and parameters to retrieve/validate evidence from visual/textual sources, generating structured multi-hop QA trajectories from VQA seed datasets. Uses multi-stage verification for factual consistency.

Result: Created MTA-Vision-DeepSearch dataset (21K examples). 32B open-source multimodal search agent achieves 54.63% average across 6 benchmarks, outperforming GPT-5, Gemini-2.5-Pro, Gemini-3-Pro. Improves reasoning depth (steps from 2.27 to 4.28) and tool-use behavior.

Conclusion: Training on high-quality multi-hop data improves multimodal deep search capabilities. Approach enables training without real-time tool calls via cached interactions. Fully open recipe released for reproducibility and future research.
...
TL;DR: MorphDistill is a two-stage framework that distills knowledge from multiple pathology foundation models into a compact CRC-specific encoder for improved survival prediction in colorectal cancer.


<details>
  <summary>Details</summary>
Motivation: Colorectal cancer remains a leading cause of cancer mortality, but existing pathology foundation models often overlook organ-specific features critical for accurate survival prediction and treatment stratification.

Method: Two-stage framework: Stage I trains a student encoder using dimension-agnostic multi-teacher relational distillation with supervised contrastive regularization on large-scale colorectal datasets. Stage II extracts patch-level features from whole-slide images and aggregates them via attention-based multiple instance learning to predict five-year survival.

Result: On Alliance/CALGB 89803 cohort (n=424), achieved AUC of 0.68 (8% improvement over best baseline), C-index of 0.661, and hazard ratio of 2.52. On external TCGA cohort (n=562), achieved C-index of 0.628, demonstrating strong generalization and robustness across clinical subgroups.
...
TL;DR: First comprehensive benchmark for Visual Anomaly Detection on edge devices with continual learning constraints, proposing Tiny-Dinomaly with 13x smaller memory and 20x lower computation while improving performance.


<details>
  <summary>Details</summary>
Motivation: Visual Anomaly Detection needs to address two unaddressed challenges simultaneously: edge deployment with constrained resources and continual learning requiring adaptation to evolving data without forgetting. Current methods fail when both constraints are imposed together.

Method: Created first comprehensive benchmark for VAD on edge with continual learning, evaluated 7 VAD models across 3 lightweight backbones. Proposed Tiny-Dinomaly (lightweight adaptation of Dinomaly on DINO foundation model) and modified PatchCore and PaDiM for efficiency in continual learning.

Result: Tiny-Dinomaly achieves 13x smaller memory footprint, 20x lower computational cost, and improves Pixel F1 by 5 percentage points. Benchmark provides guidance for selecting optimal backbone and VAD method under efficiency and adaptability constraints.

Conclusion: The work addresses critical gap in VAD research by jointly considering edge deployment and continual learning constraints, providing practical solutions and benchmark for real-world applications where both efficiency and adaptability are essential.
...
TL;DR: Activation Prompting (AP) extends visual prompting by applying universal perturbations to intermediate activation maps, outperforming input-level visual prompting and matching parameter-efficient fine-tuning in accuracy and efficiency.


<details>
  <summary>Details</summary>
Motivation: Visual prompting (VP) has emerged as a popular alternative to model fine-tuning, but suffers from a performance gap compared to conventional methods. The authors aim to understand and advance input-level prompting by exploring a more generalized approach.

Method: Introduces Activation Prompting (AP), which extends visual prompting by applying universal perturbations to activation maps in intermediate layers rather than just input data. Uses AP as an analytical tool to study VP limitations, analyzes layer preferences theoretically through global feature analysis, and conducts extensive experiments across 29 datasets and various architectures.

Result: AP demonstrates superiority over VP in both accuracy and efficiency (time, parameters, memory, throughput). Shows AP is closely related to normalization tuning in CNNs and ViTs, with each model type having distinct layer preferences for prompting. Theoretically explains these preferences through global feature analysis across layers.

Conclusion: Activation Prompting provides a more effective approach than input-level visual prompting, bridging the performance gap with conventional fine-tuning methods while maintaining efficiency advantages. The work offers both theoretical insights and practical improvements for model adaptation.
...
TL;DR: PhysHead: A hybrid representation for animatable head avatars with realistic hair dynamics using 3D Gaussians and physics-based hair simulation.


<details>
  <summary>Details</summary>
Motivation: Existing head avatar methods assume rigid hair movement, treating hair as a simple outer shell rather than capturing its natural volumetric behavior and dynamics. This limits realism in digital avatars.

Method: Combines 3D parametric mesh for head with strand-based hair that can be physically simulated. Uses Gaussian primitives attached to both head mesh and hair segments for appearance. Employs VLM-based models to generate appearance for occluded regions in training sequences.

Result: Creates photorealistic head avatars with dynamic hair behavior (e.g., wind-blown motion). Quantitative and qualitative studies show physically plausible hair motion alongside expression and camera control, outperforming existing baselines.

Conclusion: PhysHead overcomes rigid hair limitations in existing methods by introducing a hybrid representation that enables realistic hair dynamics while maintaining photorealistic appearance, advancing the field of animatable head avatars.
...
TL;DR: A graph neural network with RNN component predicts progression of Alzheimer's disease stages using fMRI connectivity data, achieving 82.9% accuracy even with irregular visit patterns.


<details>
  <summary>Details</summary>
Motivation: Alzheimer's disease affects millions with no cure, but early detection could slow progression. Current methods struggle with predicting transitions between cognitive impairment stages, especially from normal to mild impairment.

Method: GNN-based model with RNN block processes functional connectivity graphs from rs-fMRI scans of 303 subjects with varying visit histories. Handles irregular time gaps by incorporating visit distance information and works with missing visits.

Result: Model achieves 82.9% overall accuracy, with impressive 68.8% accuracy on CN to MCI conversions - a particularly challenging task. Demonstrates robustness even with missing visits in subject histories.

Conclusion: rs-fMRI is effective for predicting MCI/AD onset. Combined with other modalities, this approach could enable timely interventions to slow cognitive impairment progression.
...
TL;DR: Hybrid deep learning model (ResNet-1D + BiGRU + Multi-Head Attention) for IIoT intrusion detection achieves high accuracy (98.71-99.99%) with low latency, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Need for effective real-time intrusion detection in Industrial IoT systems with spatial-temporal feature extraction and handling class imbalance issues.
...
Result: Achieved 98.71% accuracy, 0.0417% loss, 0.0001 sec/instance inference on EdgeHoTset; 99.99% accuracy, 0.0028 loss, 0% FPR, 0.00014 sec/instance on CICIoV2024. Outperformed existing methods across all metrics.

Conclusion: The hybrid model demonstrates robustness, effectiveness, and strong real-time capability for IoT intrusion detection, with excellent generalization across different datasets.
...
TL;DR: DesigNet: A hierarchical Transformer-VAE for SVG generation that incorporates designer tools like continuity control and axis alignment to produce editable vector graphics compatible with professional workflows.


<details>
  <summary>Details</summary>
Motivation: Neural networks and human designers operate fundamentally differently, creating challenges for collaboration in AI-driven content generation, especially for Scalable Vector Graphics (SVG) where professional designers need editable outputs with proper continuity and alignment.

Method: DesigNet uses a hierarchical Transformer-VAE operating directly on SVG sequences with continuous command parameterization. It includes two differentiable modules: 1) continuity self-refinement module predicting C⁰, G¹, and C¹ continuity for curve points and enforcing it via Bézier control point modification, and 2) alignment self-refinement module with snapping capabilities for horizontal/vertical lines.

Result: DesigNet produces editable outlines and achieves competitive results against state-of-the-art methods, with notably higher accuracy in continuity and alignment, making outputs easier to refine and integrate into professional design workflows.

Conclusion: By equipping neural networks with designer tools like continuity control and alignment snapping, DesigNet bridges the gap between AI-generated content and professional design workflows, producing SVG outputs that are both high-quality and editable for human designers.
...
<details>
  <summary>Details</summary>
Motivation: Monocular depth estimation is a highly ill-posed problem in 3D vision. Existing methods struggle to effectively bridge the gap between image color features and geometric depth values, and often fail to accurately predict depth around edges where sharp changes occur.

Method: Proposes LiftFormer based on lifting theory topology: 1) Constructs Depth-oriented Geometric Representation (DGR) subspace using frame theory with linearly dependent vectors corresponding to depth bins, transforming image features into this subspace. 2) Builds Edge-aware Representation (ER) subspace to enhance depth prediction around edges by transforming depth features for local edge enhancement.

Result: LiftFormer achieves state-of-the-art performance on widely used datasets. Ablation studies validate the effectiveness of both proposed lifting modules (DGR and ER subspaces).

Conclusion: The lifting theory approach successfully bridges the learning from color values to geometric depth values through intermediate subspaces, with edge-aware enhancement improving accuracy around depth discontinuities.
...
TL;DR: VAMAE: Vessel-aware masked autoencoder for self-supervised pretraining on OCTA images using anatomically informed masking and multi-target reconstruction to better capture vascular geometry.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised learning methods (like masked autoencoders) designed for dense natural images don't work well for OCTA images due to sparse vessel structures and strong topological constraints. Uniform masking and pixel-level reconstruction fail to adequately capture vascular geometry.

Method: Proposed VAMAE framework with two key innovations: 1) Anatomically informed masking that emphasizes vessel-rich regions using vesselness and skeleton-based cues to focus on vascular connectivity and branching patterns, 2) Multi-target reconstruction objective that captures appearance, structural, and topological information simultaneously.

Result: Evaluated on OCTA-500 benchmark for vessel segmentation tasks under varying supervision levels. Vessel-aware masking and multi-target reconstruction consistently outperformed standard masked autoencoding baselines, especially in limited-label settings.

Conclusion: Geometry-aware self-supervised learning (via vessel-aware masking and multi-target reconstruction) shows strong potential for OCTA analysis, particularly when labeled data is scarce.
...
TL;DR: HopS improves prompt learning under partial supervision by combining local density-based filtering and global optimal transport for robust label selection.


<details>
  <summary>Details</summary>
Motivation: Prompt learning struggles with limited performance when only partial labels are available due to label ambiguity and insufficient supervisory information.

Method: HopS uses two complementary strategies: 1) local density-based filter that selects top frequent labels from nearest neighbors, and 2) global selection objective based on optimal transport that maps uniform sampling to candidate label distributions.
...
Conclusion: HopS offers a practical solution for prompt learning in weakly supervised settings by providing holistic label selection from both local and global perspectives.
...
TL;DR: MARMamba is a novel CT metal artifact reduction method using a streamlined UNet with multi-scale Mamba blocks that eliminates metal artifacts while preserving anatomical structures, requiring only CT images without additional input data.


<details>
  <summary>Details</summary>
Motivation: Metal implants in CT imaging create severe artifacts that degrade image quality and diagnostic accuracy. Existing methods have three main problems: they deteriorate organ/tissue structures, depend on sinogram data, and have poor resource-restoration efficiency balance.
...
Result: The model effectively eliminates artifacts from metals of different sizes while maintaining original anatomical structure integrity. It outperforms other models in artifact reduction and achieves optimal balance between computational demands, memory usage, and parameter count.

Conclusion: MARMamba provides an efficient, practical solution for CT metal artifact reduction that doesn't require additional input data, preserves anatomical structures, and offers excellent computational efficiency for real-world clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing weather removal methods are often specialized for single weather conditions, and multi-weather approaches tend to be computationally expensive with large parameter sizes, long inference times, and high memory costs, limiting practical deployment.

Method: UNet-like architecture with gating mechanisms and multi-scale pyramid vision Transformer; uses channel-wise attention from CNNs for feature extraction optimization; linear spatial reduction reduces attention computational cost; strategic gating in feed-forward and downsampling phases selectively addresses redundancy.
...
Conclusion: WeatherRemover provides an effective lightweight solution for multi-weather image restoration that meets practical application demands by balancing performance and efficiency, with source code publicly available.
...
TL;DR: A feature extraction framework that suppresses cross-model transfer while preserving accuracy for designated classifiers, using variational latent bottleneck with dynamic binary masking based on KL divergence and gradient saliency.
...
Conclusion: Proposed framework effectively suppresses cross-model transfer while preserving accuracy for designated classifiers, though further evaluation needed to assess robustness against adaptive adversaries.
...
TL;DR: CGVC introduces controllable generative video compression that uses keyframes and dense control priors to balance perceptual quality with signal fidelity, outperforming previous methods on both metrics.


<details>
  <summary>Details</summary>
Motivation: Existing perceptual video compression methods improve perceptual realism but sacrifice signal fidelity, creating a dilemma between perception and fidelity that contradicts the core goal of video compression to faithfully reproduce visual signals.

Method: Proposes Controllable Generative Video Compression (CGVC) paradigm using: 1) coded keyframes as structural priors for non-keyframe generation, 2) dense per-frame control priors to preserve finer structure and semantics, 3) controllable video generation model with temporal/content consistency, and 4) color-distance-guided keyframe selection algorithm for accurate color recovery.
...
Conclusion: The CGVC paradigm successfully alleviates the perception-fidelity dilemma in video compression by using controllable generative modeling with multiple visual conditions, achieving better balance between perceptual realism and signal fidelity.
...
<details>
  <summary>Details</summary>
Motivation: 3D medical image segmentation faces challenges due to modality diversity, high-dimensional data, and anatomical heterogeneity. Existing methods struggle to balance segmentation accuracy with computational efficiency, especially for multi-organ segmentation in clinical settings.

Method: GPAFormer incorporates two core modules: MASA (multi-scale attention-guided stacked aggregation) with three parallel paths of different receptive fields for handling varying structure sizes, and MPGA (mutual-aware patch graph aggregator) that dynamically aggregates similar feature regions using graph-guided approach based on feature similarity and spatial adjacency.

Result: Achieved state-of-the-art DSC scores on multiple public datasets: BTCV (75.70%), Synapse (81.20%), ACDC (89.32%), and BraTS (82.74%) with only 1.81M parameters. Inference time was less than 1 second per case on consumer-level GPU.
...
TL;DR: ISTS watermarking for diffusion models resists removal/forgery attacks via prompt-aware dynamic injection and two-sided detection.


<details>
  <summary>Details</summary>
Motivation: Current diffusion model watermarks are vulnerable to adversarial attacks (removal/forgery), creating copyright and provenance concerns for AI-generated content.

Method: ISTS uses prompt semantics to dynamically control watermark injection timing and patterns, plus a two-sided detection approach for enhanced robustness.

Result: Experiments show ISTS watermarking is superior against both removal and forgery attacks compared to existing methods.

Conclusion: ISTS provides a robust watermarking solution for diffusion models that addresses key security vulnerabilities in AI-generated content protection.
...
TL;DR: VDPP is a video depth post-processing framework that achieves real-time performance (>43.5 FPS) on edge devices by focusing on geometric refinement in low-resolution space rather than full scene reconstruction, enabling modular integration with any evolving image depth model.


<details>
  <summary>Details</summary>
Motivation: Current end-to-end video depth models suffer from adaptation lag when new single-image depth estimators are released, while existing post-processing methods lack the speed, accuracy, and practicality needed for real-time deployment. There's a need for a modular solution that can quickly integrate evolving image depth models without retraining.
...
Result: VDPP achieves exceptional speed (>43.5 FPS on NVIDIA Jetson Orin Nano) while matching the temporal coherence of end-to-end systems. It provides superior balance of speed, accuracy, and memory efficiency, making it practical for real-time edge deployment.

Conclusion: VDPP revitalizes post-processing for video depth estimation by offering a modular, scalable framework that can immediately integrate any evolving image depth model, achieving the best balance of speed, accuracy, and efficiency for real-world applications.
...
TL;DR: RASR framework improves multimodal fake news video detection by retrieving associative evidence and incorporating domain knowledge, achieving state-of-the-art performance with 0.93% accuracy improvement.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack cross-instance global semantic correlations and struggle with domain transfer due to semantic discrepancies, limiting their ability to use historical evidence and domain-specific knowledge for verification.

Method: Three-module framework: 1) CSPR parses videos into semantic primitives and retrieves relevant evidence from memory bank; 2) DGMP uses domain priors to guide multimodal LLM for domain-aware analysis; 3) MVDFF integrates multi-dimensional features via adaptive gating for robust detection.
...
Conclusion: The RASR framework effectively addresses limitations in existing fake news detection by leveraging associative evidence retrieval and domain-guided reasoning, demonstrating strong performance and generalization capabilities.
...
TL;DR: Proposes an agent-driven VLM framework for deciphering Oracle Bone Script by analyzing recurring pictographic components, with new OB-Radix dataset and improved accuracy over baselines.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat OBS decipherment as closed-set image recognition, failing to bridge the "interpretation gap" - they miss that rare characters are composed of recurring components with transferable semantic meanings.

Method: Agent-driven VLM framework integrating a Vision-Language Model for visual grounding with an LLM-based agent that performs component identification, graph-based knowledge retrieval, and relationship inference for linguistically accurate interpretation.

Result: Introduced OB-Radix dataset (1,022 character images, 934 unique characters, 1,853 component images across 478 distinct components with verified explanations). Framework demonstrated more detailed and precise decipherments across three benchmarks compared to baselines.

Conclusion: The proposed agent-driven VLM framework successfully bridges the interpretation gap in OBS decipherment by leveraging structural logic of recurring components, enabling more accurate and detailed interpretations than traditional image recognition approaches.
...
<details>
  <summary>Details</summary>
Motivation: Two long-standing issues impair semi-dense image matching performance: 1) Over-exclusion in mutual nearest neighbor matching struggles with scale differences between images, and 2) Existing methods neglect local consistency of final matches, undermining robustness.

Method: 1) Proposes a scale-aware matching module that exploits hints in the score matrix to indicate scale ratio, addressing scale differences at coarse stage. 2) Reformulates fine stage as cascaded flow refinement problem with novel gradient loss to encourage local consistency of flow field.

Result: Extensive experiments demonstrate the proposed matching pipeline achieves robust and accurate matching performance on downstream tasks with negligible overhead.

Conclusion: The paper presents a novel semi-dense image matching approach that effectively addresses scale handling and local consistency issues, resulting in improved performance for image matching tasks.
...
TL;DR: HQF-Net is a hybrid quantum-classical network for remote sensing segmentation that combines DINOv3 ViT-L/16 features with U-Net using deformable attention fusion and quantum-enhanced components.


<details>
  <summary>Details</summary>
Motivation: Classical encoder-decoder architectures like U-Net struggle to fully exploit global semantics and structured feature interactions in remote sensing segmentation, requiring better integration of spatial details and semantic context.

Method: HQF-Net integrates multi-scale semantic guidance from a frozen DINOv3 ViT-L/16 backbone with a customized U-Net via Deformable Multiscale Cross-Attention Fusion (DMCAF). It adds quantum-enhanced skip connections (QSkip) and a Quantum bottleneck with Mixture-of-Experts (QMoE) using complementary quantum circuits with adaptive routing.

Result: Achieves 0.8568 mIoU and 96.87% OA on LandCover.ai, 71.82% mIoU on OpenEarthMap, and 55.28% mIoU with 99.37% OA on SeasoNet. Ablation studies confirm contributions of each component.
...
TL;DR: DeSOPE is a large-scale dataset for 6DoF deformed objects featuring 26 object categories with canonical and deformed configurations, plus 133K RGB-D frames with 665K pose annotations.


<details>
  <summary>Details</summary>
Motivation: Most 6D object pose methods assume rigid or articulated objects, but this fails in practice as objects deviate from canonical shapes due to wear, impact, or deformation.

Method: Created DeSOPE dataset with high-fidelity 3D scans of 26 object categories (canonical + 3 deformed states). Annotation pipeline: 2D mask annotation → initial pose estimation via object pose method → refinement via object-level SLAM → manual verification.

Result: Dataset includes 133K RGB-D frames across diverse scenarios with 665K pose annotations. Evaluation shows object pose methods' performance drops sharply with increasing deformation.

Conclusion: Robust handling of object deformations is critical for practical applications, and DeSOPE provides a valuable benchmark for developing deformation-aware 6D pose estimation methods.
...
TL;DR: Training-free framework improves 3D spatial reasoning in MLLMs using visual chain-of-thought with explicit 3D reconstruction from single images.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with complex 3D spatial reasoning due to reliance on 2D visual priors. Existing solutions require expensive post-training on limited 3D datasets or use rigid tool-calling mechanisms lacking geometric understanding and viewpoint flexibility.
...
Result: Significantly enhances spatial comprehension. Outperforms specialized spatial models and general-purpose MLLMs (including GPT-5.2 and Gemini-2.5-Flash) on major benchmarks such as 3DSRBench and Rel3D.

Conclusion: The training-free framework with explicit 3D reconstruction and visual chain-of-thought effectively addresses MLLMs' limitations in 3D spatial reasoning without requiring expensive retraining, demonstrating superior performance over existing approaches.
...
TL;DR: URMF introduces uncertainty-aware multimodal fusion for sarcasm detection, modeling modality reliability to handle noisy text/image inputs and dynamically regulating contributions during fusion.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal sarcasm detection methods assume all modalities are equally reliable, but real-world social media often has ambiguous text and weakly relevant/irrelevant images, causing deterministic fusion to introduce noise and weaken robust reasoning.

Method: URMF uses multi-head cross-attention to inject visual evidence into text, then multi-head self-attention for incongruity reasoning. It models unimodal aleatoric uncertainty via learnable Gaussian posteriors, using estimated uncertainty to dynamically regulate modality contributions during fusion. Includes joint training with task supervision, modality prior regularization, cross-modal alignment, and uncertainty-driven contrastive learning.
...
Conclusion: Uncertainty-aware fusion effectively addresses modality reliability issues in real-world multimodal sarcasm detection, improving performance by suppressing unreliable modalities and enhancing robust reasoning.
...
<details>
  <summary>Details</summary>
Motivation: Sparse-view 3DGS reconstruction suffers from severe overfitting, structural distortions, and translucent haze-like artifacts due to insufficient geometric supervision and unobservability of Gaussian primitive reliability.

Method: Proposes Dual-domain Observation and Calibration (DOC-GS) framework with: 1) Optimization-domain: Continuous Depth-Guided Dropout (CDGD) strategy using dropout probability as reliability proxy; 2) Observation-domain: Dark Channel Prior (DCP) to identify floater artifacts via atmospheric scattering analogy, plus reliability-driven geometric pruning.

Result: The framework suppresses weakly constrained Gaussians, improves optimization stability, identifies and removes low-confidence Gaussians through cross-view aggregated evidence.

Conclusion: DOC-GS provides a unified approach to address sparse-view 3DGS artifacts by synergistically modeling Gaussian reliability through both optimization and observation domains.
...
<details>
  <summary>Details</summary>
Motivation: Existing dynamic scene representation methods require ground-truth camera parameters and involve lengthy optimizations (≈2.67s), making them unsuitable for live streaming applications. There's a need for real-time novel view synthesis from unposed multi-view inputs.

Method: Proposes LiveStre4m with three key components: 1) Multi-view vision transformer for keyframe 3D scene reconstruction, 2) Diffusion-transformer interpolation module for temporal consistency, and 3) Camera Pose Predictor module to estimate poses and intrinsics directly from RGB images without calibration. Works with as few as two synchronized unposed input streams.

Result: Achieves average reconstruction time of 0.07s per-frame at 1024×768 resolution, outperforming optimization-based methods by orders of magnitude in runtime. Enables temporally consistent novel-view video streaming in real-time.

Conclusion: LiveStre4m makes real-time novel view synthesis streaming feasible in practical settings, marking a substantial step toward deployable live novel-view synthesis systems for applications requiring live streaming from unposed multi-view video.
...
TL;DR: Transforms static visual in-context learning models into interactive systems that can incorporate user guidance (scribbles, clicks, boxes) without fine-tuning, enabling user-controlled predictions for tasks like segmentation, super-resolution, and object removal.


<details>
  <summary>Details</summary>
Motivation: Current visual in-context learning models are static and lack mechanisms to incorporate user guidance signals like scribbles, clicks, or bounding boxes, which limits their practical utility in real-world applications where users want to actively steer model predictions.

Method: Proposes Interactive DeLVM by encoding user interactions directly into the example input-output pairs, keeping the visual in-context learning philosophy intact while enabling users to prompt models with unseen interactions without fine-tuning.

Result: SOTA visual in-context learning models fail to leverage interaction cues, while Interactive DeLVM achieves +7.95% IoU for interactive segmentation, +2.46 PSNR for directed super-resolution, and -3.14% LPIPS for interactive object removal.

Conclusion: Bridges the gap between rigid static task adaptation and fluid interactivity for user-centric visual in-context learning, transforming static models into highly controllable, user-driven systems.
...
TL;DR: VENUSS framework evaluates Vision-Language Models on sequential driving scenes, revealing VLMs achieve only 57% accuracy (vs human 65%), struggle with vehicle dynamics and temporal relations, and analyzes how input configurations affect performance.
...
Method: VENUSS framework extracts temporal sequences from driving videos, generates structured evaluations across custom categories, and compares 25+ existing VLMs across 2,600+ scenarios with systematic sensitivity analysis of input configurations (resolution, frame count, temporal intervals, spatial layouts, presentation modes).

Result: Top VLMs achieve only 57% accuracy, not matching human performance (65%), exposing significant capability gaps. VLMs excel with static object detection but struggle with understanding vehicle dynamics and temporal relations.

Conclusion: VENUSS provides the first systematic sensitivity analysis of VLMs for sequential driving scenes, establishing baselines for future research and revealing critical limitations in current models' understanding of dynamic driving scenarios.
...
TL;DR: FlowInOne unifies multimodal generation into a purely visual flow, converting all inputs (text, layouts, instructions) into visual prompts for an image-in, image-out pipeline using a single flow matching model, eliminating cross-modal alignment bottlenecks.
...
Method: FlowInOne reformulates multimodal generation as a purely visual flow, converting all inputs into visual prompts and enabling a clean image-in, image-out pipeline governed by a single flow matching model. They introduce VisPrompt-5M (5M visual prompt pairs) and VP-Bench benchmark for evaluation.
...
Conclusion: FlowInOne establishes a new foundation for fully vision-centric generative modeling where perception and creation coexist within a single continuous visual space, eliminating cross-modal alignment bottlenecks and unifying diverse generation tasks under one coherent paradigm.
...
TL;DR: FlowExtract is a pipeline that extracts directed graphs from ISO 5807-standardized flowcharts in manufacturing documents, outperforming vision-language models by separating element detection from connectivity reconstruction.


<details>
  <summary>Details</summary>
Motivation: Manufacturing maintenance procedures are documented as static flowcharts in PDFs/images, encoding essential procedural knowledge that is inaccessible to modern operator support systems. Current vision-language models struggle to reconstruct connection topology from such diagrams.

Method: A pipeline that separates element detection from connectivity reconstruction: uses YOLOv8 and EasyOCR for standard domain-aligned node detection and text extraction, combined with a novel edge detection method that analyzes arrowhead orientations and traces connecting lines backward to source nodes.

Result: Evaluated on industrial troubleshooting guides, FlowExtract achieves very high node detection and substantially outperforms vision-language model baselines on edge extraction.

Conclusion: FlowExtract offers organizations a practical path toward queryable procedural knowledge representations from static flowchart documents, with implementation available as open source.
...
TL;DR: MAPO bridges reasoning-action gap in multimodal LLMs by requiring explicit textual descriptions of visual content and using semantic alignment in advantage estimation to reduce gradient variance and improve performance.


<details>
  <summary>Details</summary>
Motivation: Current RL methods for MLLMs rely on outcome-based rewards, ignoring that plausible textual reasoning can mask poor visual actions. This reasoning-action discrepancy creates noise that accumulates during multi-turn reasoning, degrading multimodal capabilities and potentially causing training collapse.

Method: MAPO requires models to generate explicit textual descriptions of visual content obtained via tool usage, then uses novel advantage estimation that couples semantic alignment between descriptions and actual observations with task reward.
...
Conclusion: MAPO effectively bridges the gap between textual reasoning and visual actions in multimodal chain-of-thought, addressing the reasoning-action discrepancy problem and improving multimodal reasoning capabilities.
...
TL;DR: EventFace: A framework for event-based face recognition that models structure-driven spatiotemporal identity representations using LoRA for spatial priors and MPE/STM for temporal dynamics, achieving 94.19% Rank-1 accuracy.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer illumination robustness and privacy benefits for face recognition, but lack stable photometric appearance of RGB systems. Existing methods don't adequately model structure-driven spatiotemporal identity representations shaped by facial motion and geometry, and dedicated event-based face datasets are lacking.

Method: 1) Construct EFace dataset with rigid facial motion; 2) Use Low-Rank Adaptation (LoRA) to transfer structural facial priors from pretrained RGB models to event domain; 3) Introduce Motion Prompt Encoder (MPE) to encode temporal features; 4) Use Spatiotemporal Modulator (STM) to fuse spatial and temporal features for enhanced identity representation.

Result: Achieves 94.19% Rank-1 identification rate and 5.35% equal error rate (EER), outperforming baselines. Shows stronger robustness under degraded illumination and reduced template reconstructability for improved privacy.

Conclusion: EventFace successfully models structure-driven spatiotemporal identity representations for event-based face recognition, demonstrating superior performance, illumination robustness, and privacy benefits through effective integration of spatial priors and temporal dynamics.
...
TL;DR: OG-ReG Transformer: A dual-path video transformer that mimics human visual attention with Glance path for coarse spatiotemporal info and Gaze path for local details, achieving SOTA on multiple video datasets.


<details>
  <summary>Details</summary>
Motivation: Current video transformers use factorized/window-based attention that splits spatiotemporal correlations, limiting motion capture and long-range dependencies. Inspired by human vision where temporal/spatial importance varies across time scales and attention is sparse through glance/gaze behavior.

Method: Dual-path network: 1) Glance path extracts coarse-grained overall spatiotemporal information, 2) Gaze path supplements with local details. Mimics human visual attention mechanism.

Result: Achieves state-of-the-art results on Kinetics-400, Something-Something v2, and Diving-48 datasets.

Conclusion: Proposed OG-ReG Transformer effectively balances computation and efficiency while capturing motion and long-range dependencies through human-inspired dual-path attention mechanism.
...
TL;DR: Proposes a globally video-guided multimodal translation framework using semantic encoding and vector database retrieval to capture narrative context across long videos, outperforming baselines on documentary translation.
...
Method: Uses pretrained semantic encoder and vector database-based subtitle retrieval to construct context set of video segments related to target subtitle semantics. Employs attention mechanism to focus on relevant visual content while preserving broader context. Designs region-aware cross-modal attention for enhanced semantic alignment during translation.

Result: Experiments on large-scale documentary translation dataset demonstrate significant performance improvement over baseline models, highlighting effectiveness in long-video scenarios.

Conclusion: The proposed globally video-guided multimodal translation framework effectively addresses limitations of local alignment methods and successfully captures global narrative context in long videos, showing superior performance on documentary translation tasks.
...
TL;DR: FedDAP addresses domain shift in federated learning by creating domain-specific global prototypes and using dual alignment to improve cross-domain generalization.


<details>
  <summary>Details</summary>
Motivation: Real-world FL faces domain shift issues when clients have data from different domains, causing degraded global model performance. Existing prototype-based methods fail to preserve domain information and use domain-agnostic alignment.

Method: FedDAP constructs domain-specific global prototypes by aggregating local prototypes within the same domain using similarity-weighted fusion. It then guides local training by aligning local features with same-domain prototypes while separating from different-domain prototypes.
...
Conclusion: FedDAP successfully addresses domain shift in FL through domain-aware prototype construction and dual alignment, improving cross-domain generalization while maintaining privacy.
...
TL;DR: RePL is a semi-supervised LiDAR semantic segmentation framework that improves pseudo-label quality through error identification and correction via masked reconstruction, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Semi-supervised learning for LiDAR semantic segmentation suffers from error propagation and confirmation bias caused by noisy pseudo-labels, which is a chronic issue that needs to be addressed.

Method: RePL enhances pseudo-label quality by identifying and correcting potential errors in pseudo-labels through masked reconstruction, along with a dedicated training strategy. The framework includes theoretical analysis demonstrating conditions for beneficial pseudo-label refinement.

Result: Extensive evaluations on nuScenes-lidarseg and SemanticKITTI datasets show that RePL significantly improves pseudo-label quality and achieves state-of-the-art performance in LiDAR semantic segmentation.

Conclusion: RePL successfully addresses the chronic issue of noisy pseudo-labels in semi-supervised LiDAR segmentation through error correction via masked reconstruction, with both theoretical and empirical validation of its effectiveness.
...
TL;DR: VGGT-SLAM++ is a visual SLAM system that uses VGGT geometry outputs with DEM-based mapping and local bundle adjustment to reduce drift and improve accuracy while maintaining bounded memory usage.


<details>
  <summary>Details</summary>
Motivation: Previous transformer-based SLAM systems like VGGT-SLAM suffer from short-horizon pose drift due to reliance on sparse loop closures or global constraints, lacking frequent local optimization.

Method: System combines VGGT feed-forward transformer with Sim(3) solution for visual odometry, constructs dense planar-canonical DEM submaps partitioned into patches with DINOv2 embeddings, builds covisibility graph, and uses VPR module for spatial neighbor retrieval to trigger frequent local bundle adjustment.
...
Conclusion: VGGT-SLAM++ demonstrates that restoring high-cadence local bundle adjustment through spatially corrective back-end with DEM-based graph construction enables accurate large-scale mapping with bounded memory, overcoming limitations of prior transformer-based SLAM approaches.
...
TL;DR: CloudMamba: A two-stage CNN-Mamba hybrid framework for cloud detection that uses uncertainty estimation to refine thin-cloud regions and dual-scale processing for fragmented clouds and boundaries.
...
Conclusion: CloudMamba effectively addresses thin-cloud ambiguity and fragmented cloud challenges through its two-stage uncertainty-guided approach and efficient dual-scale Mamba architecture, achieving superior cloud detection performance.
...
TL;DR: PASS is an intelligent MRI framework that uses Vision-Language Models to guide personalized, task-oriented accelerated imaging with superior clinical outcomes.


<details>
  <summary>Details</summary>
Motivation: Traditional accelerated MRI methods optimize for generic image quality rather than specific clinical tasks, lacking adaptability for personalized patient needs and clinical applications.

Method: PASS integrates three components: 1) deep unrolled reconstruction network from physics-based MRI model, 2) patient-specific k-space trajectory sampling module, and 3) anomaly-aware prior from pretrained VLM that guides both sampling and reconstruction toward clinically relevant regions.

Result: PASS achieves superior image quality across diverse anatomies, contrasts, anomalies, and acceleration factors, with improvements in downstream diagnostic tasks including fine-grained anomaly detection, localization, and diagnosis.

Conclusion: By integrating high-level clinical reasoning from VLMs with interpretable physics-aware networks, PASS enables task-oriented, personalized MRI acceleration that directly enhances clinical diagnostic capabilities.
...
TL;DR: This paper reviews physical adversarial attacks from a surveillance-oriented perspective, arguing that traditional isolated image benchmarks are insufficient for evaluating real-world surveillance systems where temporal persistence, multi-modal sensing, carrier realism, and system-level objectives matter.


<details>
  <summary>Details</summary>
Motivation: Physical adversarial attacks are increasingly studied in real-world surveillance contexts rather than isolated image benchmarks. Traditional evaluation methods fail to capture critical surveillance factors like temporal persistence across frames, multi-modal sensing (visible+infrared), realistic attack carriers, and system-level tracking objectives.

Method: The paper reviews physical attacks through a surveillance-oriented lens, organizing prior work using a four-part taxonomy focusing on: 1) temporal persistence, 2) sensing modality, 3) carrier realism, and 4) system-level objectives. It analyzes how recent work on multi-object tracking, dual-modal evasion, and controllable clothing reflects broader field changes.

Result: The review reveals that surveillance robustness cannot be reliably judged from isolated per-frame benchmarks alone. Key evaluation gaps include distance robustness, camera-pipeline variation, identity-level metrics, and activation-aware testing. Recent work shows progress in addressing temporal, multi-modal, and practical carrier challenges.

Conclusion: Surveillance robustness must be examined as a system problem unfolding over time, across multiple sensors, and under realistic physical deployment constraints. The field is shifting from isolated image attacks to more comprehensive evaluations that consider the practical realities of deployed surveillance systems.
...
<details>
  <summary>Details</summary>
Motivation: Existing image editing models often fail at local detail restoration (text, logos, thin structures) and may inadvertently change background pixels, especially when editing small regions. There's a need for precise local refinement that strictly preserves unedited areas.

Method: Proposes RefineAnything with Focus-and-Refine strategy: crop-and-resize target region to allocate resolution budget, refine using multimodal diffusion (supports reference-based and reference-free), then paste back with blended-mask to preserve background. Uses Boundary Consistency Loss to reduce seam artifacts.

Result: Outperforms baselines on RefineEval benchmark with strong edited-region fidelity and near-perfect background preservation. Introduces Refine-30K dataset (20K reference-based, 10K reference-free samples) for evaluation.

Conclusion: RefineAnything establishes a practical solution for high-precision local refinement, effectively addressing local detail collapse while maintaining strict background preservation through innovative region-focused strategy.
...
TL;DR: SCT-MOT: A swarm UAV tracking framework with swarm-coupled motion modeling and trajectory-guided feature fusion to address challenges in air-to-air tracking of small objects with complex group motion.


<details>
  <summary>Details</summary>
Motivation: Air-to-air tracking of swarm UAVs faces challenges due to complex nonlinear group motion and weak visual cues for small objects, causing detection failures, trajectory fragmentation, and identity switches. Existing methods model objects independently and have limited integration between motion prediction and appearance representation.

Method: Proposes SCT-MOT with two key modules: 1) Swarm Motion-Aware Trajectory Prediction (SMTP) that jointly models historical trajectories and posture-aware appearance features from a swarm-level perspective, and 2) Trajectory-Guided Spatio-Temporal Feature Fusion (TG-STFF) that aligns predicted positions with historical visual cues and integrates them with current frame features.

Result: SMTP achieves more accurate trajectory forecasts and yields 1.21% IDF1 improvement over state-of-the-art trajectory prediction module EqMotion when integrated into the same MOT framework. SCT-MOT consistently achieves superior accuracy and robustness across multiple metrics on three public air-to-air swarm UAV tracking datasets (AIRMOT, MOT-FLY, UAVSwarm).

Conclusion: The proposed SCT-MOT framework effectively addresses challenges in swarm UAV tracking by integrating swarm-coupled motion modeling and trajectory-guided feature fusion, demonstrating improved performance in complex swarm scenarios compared to existing state-of-the-art trackers.
...
TL;DR: Deep learning framework combining FDG-PET/CT images with temporal data predicts overall survival in NSCLC patients, outperforming image-only baseline by 4.3% AUC.
...
Method: ResNet-50 backbone processes tissue-wise FDG-PET/CT projections to generate image embeddings, which are combined with temporal data (time horizon in days) to produce OS probabilities as a function of time. Developed on U-CAN cohort (n=556) and evaluated against baseline ResNet-50 image-only model.

Result: Temporal data integration improved OS prediction by 4.3% AUC over baseline. Ensemble of imaging and clinical+IDP models achieved best performance (AUC=0.788). Model enabled risk stratification (high vs low risk) and saliency analysis highlighted tumor regions as key predictive structures.

Conclusion: The method provides an automated framework for time-dependent OS prediction and demonstrates the complementary value of combining imaging and tabular data for improved survival prediction in NSCLC.
...
TL;DR: ERSM is a framework that uses energy minimization to autonomously select important visual features, producing sparse, robust, and interpretable models without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Current vision models process dense feature maps inefficiently, creating computational redundancy and relying on spurious background correlations, making them brittle and hard to interpret.

Method: Introduces Energy-Regularized Spatial Masking (ERSM) with an Energy-Mask Layer that assigns scalar energy to each visual token using unary importance cost and pairwise spatial coherence penalty, formulated as differentiable energy minimization.

Result: ERSM produces emergent sparsity, improved robustness to occlusion, highly interpretable spatial masks while preserving classification accuracy, and outperforms magnitude-based pruning in deletion-based robustness tests.

Conclusion: ERSM serves as an intrinsic denoising mechanism that autonomously discovers optimal information density and isolates semantic object regions without pixel-level supervision.
...
TL;DR: Q-Zoom: A query-aware adaptive high-resolution perception framework for MLLMs that uses dynamic gating and self-distilled region proposals to efficiently process only relevant visual regions, achieving significant speedups while maintaining or improving accuracy.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs use global resolution scaling which floods attention mechanisms with redundant visual tokens, causing severe inference bottlenecks while ignoring spatial sparsity and query intent.

Method: Two-stage coarse-to-fine approach: 1) Dynamic Gating Network bypasses high-res processing when coarse features suffice; 2) Self-Distilled Region Proposal Network localizes task-relevant RoIs from intermediate features. Uses consistency-aware generation for gating labels and self-supervised distillation for RPN.

Result: 2.52× speedup on Document & OCR benchmarks, 4.39× speedup in High-Resolution scenarios while matching baseline accuracy. When optimized for fidelity, surpasses baseline by 1.1% and 8.1% on respective benchmarks. Improvements transfer to multiple MLLMs.

Conclusion: Q-Zoom establishes a dominant Pareto frontier for MLLM visual processing, enabling efficient high-resolution perception through query-aware adaptive mechanisms that maintain or improve accuracy while dramatically accelerating inference.
...
TL;DR: Multi-modal YOLOv5 extension integrates GPT-generated textual descriptions of UI images via cross-attention, improving UI control detection by combining visual and semantic information.


<details>
  <summary>Details</summary>
Motivation: UI control detection from screenshots is challenging due to visual ambiguities, design variability, and lack of contextual cues in pixel-only approaches. Current methods struggle with edge cases where visual information alone is insufficient.

Method: Novel multi-modal extension of YOLOv5 that integrates GPT-generated textual descriptions of UI images through cross-attention modules. Three fusion strategies were compared: element-wise addition, weighted sum, and convolutional fusion.

Result: Evaluated on 16,000+ annotated UI screenshots across 23 control classes. Convolutional fusion achieved strongest performance with significant gains for semantically complex or visually ambiguous classes. All fusion strategies outperformed baseline YOLOv5.

Conclusion: Combining visual and textual modalities substantially enhances UI element detection, especially in edge cases. Opens opportunities for more reliable tools in software testing, accessibility, and UI analytics, setting stage for future multi-modal detection research.
...
TL;DR: POS-ISP is a sequence-level reinforcement learning framework that optimizes image signal processing pipelines by predicting entire module sequences and parameters in one forward pass, improving task performance while reducing computational costs.
...
Method: POS-ISP formulates modular ISP optimization as a global sequence prediction problem using sequence-level RL. It predicts the entire module sequence and its parameters in a single forward pass and optimizes the pipeline using terminal task reward, eliminating intermediate supervision and redundant executions.

Result: Experiments across multiple downstream tasks show that POS-ISP improves task performance while reducing computational cost compared to existing methods.

Conclusion: Sequence-level optimization provides a stable and efficient paradigm for task-aware ISP, addressing limitations of previous NAS and step-wise RL approaches.
...
TL;DR: Grounded Forcing is a novel framework for autoregressive video synthesis that addresses semantic forgetting, visual drift, and controllability loss through three interlocking mechanisms to enable long-term coherent video generation.


<details>
  <summary>Details</summary>
Motivation: Autoregressive video synthesis faces three fundamental challenges: semantic forgetting from context limitations, visual drift due to positional extrapolation, and controllability loss during interactive instruction switching. Current methods address these issues in isolation, limiting long-term coherence in infinite-horizon video generation.

Method: The framework introduces three interlocking mechanisms: 1) Dual Memory KV Cache to decouple local temporal dynamics from global semantic anchors, addressing semantic forgetting; 2) Dual-Reference RoPE Injection to confine positional embeddings within training manifold while making global semantics time-invariant, suppressing visual drift; 3) Asymmetric Proximity Recache for smooth semantic inheritance during prompt transitions via proximity-weighted cache updates, resolving controllability issues.
...
Conclusion: Grounded Forcing provides a novel framework that bridges time-independent semantics and proximal dynamics through synergistic components, effectively addressing the intertwined challenges of semantic forgetting, visual drift, and controllability loss in autoregressive video synthesis for infinite-horizon generation.
...
<details>
  <summary>Details</summary>
Motivation: To advance research on recovering visually coherent videos from corrupted bitstreams, which often produce severe spatial-temporal artifacts and content distortion during decoding. The challenge addresses an emerging practical problem in video restoration.

Method: The paper organizes a benchmark challenge with a common dataset and evaluation protocol for comparing different restoration methods. It describes the dataset, evaluation setup, and analyzes participating methods and their technical approaches.

Result: The challenge highlights the difficulty of bitstream-corrupted video restoration as an emerging task. It provides a comprehensive benchmark and summarizes final results, technical trends, and insights from participating methods.

Conclusion: The BSCVR challenge establishes a valuable benchmark for this practical problem, reveals the task's difficulty, and provides useful insights for future research on robust video restoration under realistic bitstream corruption scenarios.
...
TL;DR: Adversarial smuggling attacks exploit the Human-AI capability gap by encoding harmful content into human-readable but AI-unreadable visual formats, evading MLLM content moderation with over 90% success rates.


<details>
  <summary>Details</summary>
Motivation: To uncover and address a critical security threat in MLLM-based content moderation where harmful content can be smuggled past automated detection by exploiting the gap between human and AI perceptual/reasoning capabilities.

Method: Developed SmuggleBench benchmark with 1,700 adversarial smuggling attack instances, classified attacks into Perceptual Blindness (disrupting text recognition) and Reasoning Blockade (inhibiting semantic understanding), and evaluated both proprietary and open-source MLLMs.

Result: State-of-the-art MLLMs (including GPT-5 and Qwen3-VL) are highly vulnerable with Attack Success Rates exceeding 90%. Identified root causes: limited vision encoder capabilities, OCR robustness gaps, and scarcity of domain-specific adversarial examples.

Conclusion: Adversarial smuggling represents a serious threat to MLLM content moderation. Preliminary mitigation via test-time scaling (CoT) and adversarial training (SFT) shows promise, but significant vulnerabilities remain that require further research and defense development.
...
TL;DR: Compression amplifies adversarial attacks on deep image classifiers by reducing decision space and increasing sensitivity to perturbations.


<details>
  <summary>Details</summary>
Motivation: Image compression is widely used in modern visual pipelines, but its impact on adversarial robustness is poorly understood, creating potential vulnerabilities in compression-in-the-loop deployment settings.

Method: Study attacks applied directly in compressed representations, analyze compression as an adversarial amplifier through decision space reduction, and conduct extensive experiments across standard benchmarks and architectures.

Result: Compression-aware attacks are substantially more effective than pixel-space attacks under identical perturbation budgets, revealing a critical vulnerability in systems using compression.

Conclusion: Compression acts as an adversarial amplifier by inducing non-invertible transformations that contract classification margins and increase sensitivity to perturbations, highlighting security risks in compression-in-the-loop deployments.
...
TL;DR: The paper finds that demographic biases in facial landmark detection are largely due to confounding factors like head pose and image resolution, not demographic attributes themselves. After controlling for these confounders, gender and race disparities vanish, but age-related bias persists, particularly affecting older individuals.


<details>
  <summary>Details</summary>
Motivation: Fairness in human-robot interaction depends on reliable perceptual models. While demographic biases have been studied in high-level facial analysis, they remain unexplored in facial landmark detection, a low-level vision component that could propagate biases through the HRI pipeline and disproportionately affect vulnerable populations.

Method: The authors conduct a systematic audit of demographic bias (age, gender, race) in facial landmark detection. They introduce a controlled statistical methodology to disentangle demographic effects from confounding visual factors, particularly head pose and image resolution. They evaluate a standard representative model using this approach.

Result: Confounding visual factors (head pose and image resolution) heavily outweigh the impact of demographic attributes. After accounting for these confounders, performance disparities across gender and race vanish. However, a statistically significant age-related effect persists, with higher biases observed for older individuals.

Conclusion: Fairness issues can emerge even in low-level vision components like facial landmark detection and can propagate through the HRI pipeline, disproportionately affecting vulnerable populations (especially older individuals). Auditing and correcting such biases is necessary for trustworthy and equitable robot perception systems.
...
TL;DR: The paper proposes a stabilized RL framework for masked autoregressive models (MAR) that addresses gradient noise and instability issues in hybrid AR-diffusion frameworks through multi-trajectory expectation and uncertainty-aware token selection.
...
Method: Proposes a stabilized RL framework for MAR with two key components: 1) Multi-trajectory expectation (MTE) that estimates optimization direction by averaging over multiple diffusion trajectories to reduce gradient noise, and 2) Uncertainty-aware token selection that applies MTE only to top-k% uncertain tokens to avoid over-smoothing, plus a consistency-aware token selection strategy that filters out AR tokens less aligned with final generated content.

Result: Extensive experiments across multiple benchmarks show the method consistently improves visual quality, training stability, and spatial structure understanding over baseline GRPO and pre-RL models.

Conclusion: The proposed stabilized RL framework effectively addresses gradient noise and instability issues in hybrid AR-diffusion models, enabling more stable and effective RL training for masked autoregressive models with diffusion heads.
...
<details>
  <summary>Details</summary>
Motivation: Palmprint recognition is widely used in security-critical applications, but the robustness of deep palmprint systems against physically realizable attacks is poorly understood. Existing studies focus on digital settings and don't account for texture-dominant nature of palmprints or physical acquisition distortions.

Method: CAAP uses a cross-shaped patch topology to disrupt long-range texture continuity. It integrates three modules: ASIT for input-conditioned patch rendering, RaS for stochastic capture-aware simulation, and MS-DIFE for feature-level identity-disruptive guidance. The framework learns universal patches reusable across inputs while remaining effective under realistic acquisition variations.

Result: CAAP achieves strong untargeted and targeted attack performance on Tongji, IITD, and AISEC datasets against both generic CNN backbones and palmprint-specific models. It shows favorable cross-model and cross-dataset transferability. Adversarial training only partially reduces attack success rates, leaving substantial residual vulnerability.
...
TL;DR: Quantile regression enables uncertainty quantification in satellite-based tree height estimation models, providing calibrated uncertainty estimates that correlate with remote sensing challenges.


<details>
  <summary>Details</summary>
Motivation: Current tree height estimation models provide only point predictions without uncertainty quantification, limiting their use in risk-sensitive ecological monitoring and biomass assessment scenarios where understanding prediction confidence is crucial.

Method: Apply quantile regression to existing tree height estimation models based on satellite data. With minor modifications to the prediction head, existing models can be adapted to provide statistically calibrated uncertainty estimates through quantile regression.

Result: The approach successfully provides uncertainty estimates that correlate with known remote sensing challenges - the model shows less confidence in more challenging conditions such as terrain complexity and vegetation heterogeneity.

Conclusion: Quantile regression offers a practical way to add uncertainty quantification to existing tree height estimation models, enhancing their utility for ecological monitoring and biomass assessment by providing confidence measures that align with real-world remote sensing difficulties.
...
<details>
  <summary>Details</summary>
Motivation: Traditional photomosaic methods rely on large image collections and color-based matching, which limits diversity and structural consistency. There's a need for more expressive and coherent photomosaic generation.
...
Result: Enables photomosaic composition that is both semantically expressive and structurally coherent, effectively overcoming fundamental limitations of matching-based approaches.

Conclusion: The generative approach represents a significant advancement over traditional photomosaic methods, offering improved diversity, structural consistency, and personalization capabilities without requiring extensive image collections.
...
TL;DR: IQ-LUT reduces LUT size by 50x vs ECNN while improving super-resolution quality through interpolation-quantization integration, residual learning, and knowledge-distilled non-uniform quantization.


<details>
  <summary>Details</summary>
Motivation: LUT methods for image super-resolution face storage bottlenecks due to exponential growth in index space when pursuing higher quality through larger receptive fields and bit-depth, limiting deployment on resource-constrained devices.

Method: 1) Integrate interpolation and quantization into single-input, multiple-output ECNN to reduce index space; 2) Use residual learning to mitigate LUT bit-depth dependence and prioritize fine-grained details; 3) Apply knowledge distillation to guide non-uniform quantization optimization.

Result: Substantial storage reduction (up to 50x compared to ECNN) while achieving superior super-resolution quality across extensive benchmarking.
...
TL;DR: V-Scan: A virtual scanning framework in Unity for generating realistic synthetic 3D scan datasets with complete ground truth, addressing the high cost of acquiring real-world scan datasets.
...
Method: A Unity-based virtual scanning framework that simulates real-world scanners with configurable parameters (resolution, range, noise). Uses ray-based scanning from virtual viewpoints for realistic occlusion modeling, and integrates with procedural indoor scene generation for diverse dataset creation.

Result: Created the V-Scan dataset containing synthetic indoor scans with object-level partial point clouds, voxel-based occlusion grids, and complete ground-truth geometry for training and evaluating learning-based methods.

Conclusion: The virtual scanning framework provides a scalable solution for generating realistic synthetic 3D scan datasets with complete supervision, enabling better training and evaluation of scene reconstruction and object completion methods.
...
TL;DR: ModuSeg is a training-free WSSS framework that decouples object discovery and semantic assignment using mask proposals and feature retrieval, achieving competitive performance without parameter tuning.
...
Method: Decouples object discovery and semantic assignment using: 1) general mask proposer for geometric proposals with reliable boundaries, 2) semantic foundation models to build offline feature bank for non-parametric feature retrieval, 3) semantic boundary purification and soft-masked feature aggregation to mitigate boundary ambiguity and quantization errors.
...
Conclusion: ModuSeg presents an effective training-free framework for WSSS that addresses key challenges through explicit decoupling of object discovery and semantic assignment, leveraging foundation models while avoiding complex optimization procedures.
...
TL;DR: PRISM proposes PSAR framework for real-world image dehazing using physical scattering model reconstruction with self-distillation adaptation to bridge synthetic-to-real gap.


<details>
  <summary>Details</summary>
Motivation: Real-world image dehazing is challenging due to non-uniform haze distribution, varying illumination from multiple light sources, and scarcity of paired real hazy-clean data. Existing methods struggle with complex regions and mixed-light conditions.

Method: Proposes Proximal Scattered Atmosphere Reconstruction (PSAR) - a physically structured framework that jointly reconstructs clear scenes and scattering variables under atmospheric scattering model. Uses online non-uniform haze synthesis pipeline and Selective Self-distillation Adaptation scheme for unpaired real-world scenarios, enabling selective learning from high-quality perceptual targets while leveraging scattering understanding to audit residual haze.
...
Conclusion: PRISM effectively addresses real-world dehazing challenges through physical modeling and adaptive learning strategies, outperforming existing methods in complex real-world scenarios.
...
TL;DR: AnchorSplat: A feed-forward 3DGS framework using anchor-aligned Gaussian representation guided by 3D geometric priors for efficient scene-level reconstruction with fewer Gaussians and better view consistency.


<details>
  <summary>Details</summary>
Motivation: Current feed-forward Gaussian reconstruction models use pixel-aligned formulations that tightly entangle Gaussian representations with input images, making them dependent on image resolution and number of views. This approach lacks geometry awareness and computational efficiency.

Method: AnchorSplat introduces an anchor-aligned Gaussian representation guided by 3D geometric priors (sparse point clouds, voxels, or RGB-D point clouds). This creates geometry-aware renderable 3D Gaussians independent of image resolution and view count. The method also includes a Gaussian Refiner to adjust intermediate Gaussians with few forward passes.

Result: Experiments on ScanNet++ v2 NVS benchmark demonstrate state-of-the-art performance, outperforming previous methods with more view-consistent results and substantially fewer Gaussian primitives.

Conclusion: AnchorSplat provides a more efficient and geometry-aware approach to 3D Gaussian reconstruction that reduces computational requirements while improving reconstruction fidelity and view consistency.
...
TL;DR: LIANet is a coordinate-based neural representation that models multi-temporal Earth observation data as a continuous spatiotemporal neural field, enabling reconstruction of satellite imagery from just spatial-temporal coordinates and serving as a user-friendly alternative to Geospatial Foundation Models.


<details>
  <summary>Details</summary>
Motivation: To create a more accessible alternative to Geospatial Foundation Models (GFMs) that eliminates the overhead of data access and preprocessing for end-users, enabling fine-tuning solely based on labels without requiring original satellite data.

Method: Develops LIANet, a coordinate-based neural representation that models multi-temporal Earth observation data as a continuous spatiotemporal neural field. The network reconstructs satellite imagery given only spatial and temporal coordinates, then can be fine-tuned for downstream tasks.

Result: LIANet demonstrates competitive performance when fine-tuned for downstream tasks (semantic segmentation, pixel-wise regression) compared to training from scratch or using established GFMs, across target areas of varying sizes.

Conclusion: LIANet serves as a practical, user-friendly alternative to GFMs by eliminating data access/preprocessing burdens and enabling label-based fine-tuning, with publicly available source code and datasets.
...
TL;DR: Proposes RePaste method and new evaluation scenarios for anomaly detection with ambiguous normal samples in industrial settings, achieving SOTA on MVTec AD benchmark.


<details>
  <summary>Details</summary>
Motivation: Real-world anomaly detection faces ambiguity in normal samples - small scratches/stains may be acceptable or unacceptable depending on specifications. Current methods don't address specification changes in industrial applications.

Method: RePaste method enhances learning by re-pasting regions with high anomaly scores from previous step into input for next step. Also proposes novel scenarios and evaluation metric for specification changes.

Result: RePaste achieved state-of-the-art performance on proposed scenarios using MVTec AD benchmark while maintaining high AUROC and PRO scores.

Conclusion: The paper addresses real-world ambiguity in normal samples through RePaste method and new evaluation scenarios, demonstrating effectiveness in industrial anomaly detection with changing specifications.
...
<details>
  <summary>Details</summary>
Motivation: Recent advances in open-access image generation models raise concerns about falsifying visual evidence in surveillance contexts. Existing forgery detection models struggle with surveillance scenarios because tampering in such imagery is typically localized, subtle, and occurs in scenes with varied viewpoints, small/occluded subjects, and lower visual quality.
...
Conclusion: SurFITR addresses a critical gap in surveillance-style image forgery detection and is publicly available on GitHub to advance research in this important area.
...
TL;DR: Onboard processing in Earth Observation constellations enables faster, higher-resolution burnt-area mapping by moving data processing from ground to space, complementing existing Copernicus services.


<details>
  <summary>Details</summary>
Motivation: Current EO services are limited by ground-based processing constraints including latency, bandwidth, and lack of autonomous observation prioritization, creating need for onboard intelligence to improve responsiveness and detail.

Method: The IRIDE programme implements a "constellation of constellations" with heterogeneous sensing technologies. Hawk for Earth Observation (HEO) enables onboard generation of data products, moving information extraction earlier in processing chain. The burnt-area mapping service demonstrates this approach.

Result: Onboard intelligence enables higher spatial detail (sub-3m ground sampling distance), detection of smaller events (minimum mapping unit of 3 hectares), and improved system responsiveness compared to ground-only architectures.

Conclusion: IRIDE HEO capability serves as complementary layer to existing Copernicus services, providing image-driven pre-classification to support emergency and land-management workflows, highlighting operational value of onboard intelligence for low-latency EO architectures.
...
TL;DR: Proposes two improvements to semi-supervised semantic segmentation: 1) pasting labeled image regions onto unlabeled data instead of using pseudo-labels, and 2) aligning predictions between labeled and unlabeled images to reduce feature gap.


<details>
  <summary>Details</summary>
Motivation: Semantic segmentation requires expensive pixel-level labeling. Semi-supervised methods like ClassMix use pseudo-labels from unlabeled images, which can be inaccurate and create feature gaps between labeled and unlabeled data.

Method: 1) LabeledMix: Paste class labels and corresponding image regions from labeled images onto unlabeled images and their pseudo-labeled versions. 2) Feature alignment: Train model to make predictions on unlabeled images more similar to those on labeled images.
...
Conclusion: The proposed approach addresses pseudo-label inaccuracy and feature gap issues in semi-supervised segmentation, achieving better performance by using labeled data more effectively and aligning feature representations.
...
TL;DR: UPDP is a utility-preserving de-identification pipeline for cross-hospital radiology data sharing that removes privacy-sensitive information while maintaining diagnostic utility for AI model training.


<details>
  <summary>Details</summary>
Motivation: Large-scale radiology data is crucial for robust medical AI, but privacy concerns constrain cross-hospital sharing. Existing de-identification focuses on compliance but doesn't address whether de-identified data preserves sufficient utility for vision-language model training and cross-hospital transfer.

Method: UPDP uses: 1) blacklist of privacy-sensitive terms and whitelist of pathology-related terms for text filtering; 2) generative filtering mechanism that synthesizes privacy-filtered, pathology-preserved counterparts of original images; 3) secure sharing of synthetic images and ID-filtered reports.

Result: Experiments on chest X-ray benchmarks show effective privacy removal while preserving diagnostic cues. Models trained on de-identified data maintain competitive diagnostic accuracy vs. original data, with marked decline in identity-related accuracy. Cross-hospital combination with local data yields better performance.

Conclusion: The UPDP pipeline enables privacy-preserving radiology data sharing while maintaining utility for AI model development, facilitating cross-hospital collaboration without compromising diagnostic value.
...
TL;DR: Researchers created CSA-Graphs, a privacy-preserving dataset for Child Sexual Abuse Imagery classification using structural representations instead of actual images, enabling safer research while respecting legal/ethical constraints.


<details>
  <summary>Details</summary>
Motivation: Current CSAI research faces major limitations due to strict legal/ethical restrictions preventing public sharing of datasets, which hinders reproducibility and slows progress in developing automated detection methods.

Method: Created CSA-Graphs dataset with two complementary graph-based modalities: scene graphs describing object relationships and skeleton graphs encoding human pose. These structural representations remove explicit visual content while preserving contextual information.

Result: Both graph representations retain useful information for classifying CSAI, and combining them further improves performance. The dataset enables broader computer vision research for child safety while respecting constraints.

Conclusion: CSA-Graphs provides a privacy-preserving solution that allows researchers to work on CSAI classification without accessing explicit content, addressing legal/ethical barriers while advancing child safety technology.
...
Result: Experiments on an in-house dataset show USCNet achieves outstanding performance across all metrics, with classification efficacy significantly surpassing existing mainstream methods.

Conclusion: USCNet provides a promising solution for precise preoperative kidney stone classification with substantial clinical benefits, and the source code is publicly available.
...
Method: Reformulate KB-VQA as search-agent problem with multi-step decision-making. Agent selects from four actions (Answer, Image Retrieval, Text Retrieval, Caption) based on current state. Automated pipeline collects multi-step trajectories for supervision fine-tuning.

Result: Achieves state-of-the-art performance on InfoSeek and E-VQA datasets, consistently outperforming prior baselines.

Conclusion: The search-agent framework effectively addresses limitations of fixed RAG pipelines by integrating retrieval with reasoning through multi-step decision-making.
...
TL;DR: Proposes subspace decomposition framework for multimodal imaging that separates shared vs modality-specific information using orthogonal subspaces, applied to PSMA PET and MRI in prostate cancer.


<details>
  <summary>Details</summary>
Motivation: Current multimodal fusion approaches lack clear distinction between shared and modality-specific information, which is clinically important for understanding irreducible contributions of each modality and informing acquisition strategies.

Method: Decomposes PSMA PET uptake into MRI-explainable physiological envelope and orthogonal residual using implicit neural representation (INR) with projection-based regularization via singular value decomposition to enforce orthogonality between MRI features and PSMA expression.

Result: Tested on 13 prostate cancer patients, the model shows residual components within MRI feature span are absorbed into learned envelope, while orthogonal residual is largest in tumor regions, indicating PSMA PET contains signal not recoverable from MRI.

Conclusion: The framework provides structured characterization of modality complementarity grounded in representation geometry rather than image translation, clarifying what information is shared vs modality-specific in multimodal imaging.
...
TL;DR: DINO-QPM is a lightweight interpretability adapter that converts complex DINOv2 features into human-interpretable, contrastive representations while maintaining high classification accuracy.


<details>
  <summary>Details</summary>
Motivation: Visual foundation models like DINOv2 provide excellent feature extraction but their complex, high-dimensional representations are difficult for humans to interpret, creating barriers for understanding model decisions.
...
Result: DINO-QPM exceeds DINOv2 linear probe accuracy while providing globally interpretable features that can be spatially localized within input images. It outperforms other methods for frozen visual foundation models in both classification accuracy and explanation quality.

Conclusion: DINO-QPM successfully bridges the gap between powerful visual foundation models and human interpretability, offering a practical solution for making state-of-the-art feature extractors understandable while maintaining or improving their performance.
...
TL;DR: Proposes domain generalization method for medical image segmentation that separates domain-invariant category information from domain-specific information, using quantum vectors in SQ-VAE to bridge domain gaps.


<details>
  <summary>Details</summary>
Motivation: Models trained on specific datasets (source domains) often fail on different datasets (target domains) due to domain differences caused by varying imaging equipment and staining methods. Need for segmentation methods that don't depend on domain differences.

Method: 1) Separates category information independent of domain differences from source-domain-specific information. 2) Uses quantum vectors in Stochastically Quantized Variational AutoEncoder (SQ-VAE) to absorb domain gaps between training and test data.

Result: Improved accuracy compared to conventional methods on vascular segmentation and cell nucleus segmentation datasets.

Conclusion: The proposed approach effectively addresses domain generalization in medical image segmentation by extracting domain-invariant features and using quantum vectors to handle remaining domain gaps.
...
<details>
  <summary>Details</summary>
Motivation: To develop a segmentation-free approach for longitudinal multi-parametric MRI analysis that can track tissue evolution relative to baseline geometry without requiring explicit anatomical segmentations or supervised classification.

Method: Represent each voxel as multi-sequence intensity vector (T1, T1c, T2, FLAIR, ADC), train implicit neural representation via denoising score matching to learn energy function E_θ(u) from single baseline scan. Use learned energy landscape as fixed geometric reference for evaluating subsequent scans.

Result: In pediatric case with later recurrence, follow-up scans showed progressive deviation in energy and directional displacement toward baseline tumor-associated regime before clear radiological reappearance. In stable disease case, voxel distributions remained confined to low-energy basins without systematic drift.

Conclusion: Patient-specific energy manifolds can function as geometric reference systems for longitudinal mpMRI analysis without explicit segmentation or supervised classification, providing foundation for manifold-based tissue-at-risk tracking in neuro-oncology.
...
Result: DenseNet201 achieved the highest test accuracy of 99% among the evaluated models. The interpretability techniques enhanced model reliability and noise resistance.

Conclusion: Deep learning models demonstrate strong capabilities for real-life tea leaf disease detection and management, with practical applications in agriculture through the developed prototype.
...
TL;DR: INSPATIO-WORLD is a real-time framework that generates high-fidelity, dynamic interactive scenes from single reference videos using a Spatiotemporal Autoregressive architecture and Joint Distribution Matching Distillation to overcome spatial consistency and realism challenges.
...
Method: Proposes INSPATIO-WORLD with two key components: 1) Spatiotemporal Autoregressive (STAR) architecture with Implicit Spatiotemporal Cache for global consistency and Explicit Spatial Constraint Module for geometric structure and camera trajectories; 2) Joint Distribution Matching Distillation (JDMD) to prevent fidelity degradation from synthetic data over-reliance.
...
Conclusion: INSPATIO-WORLD successfully addresses spatial consistency and realism challenges in interactive scene generation, enabling high-fidelity, real-time navigation of dynamic environments reconstructed from single videos.
...
TL;DR: VersaVogue is a unified diffusion framework for multi-condition fashion synthesis that supports both garment generation and virtual dressing using trait-routing attention and automated preference optimization.


<details>
  <summary>Details</summary>
Motivation: Current fashion image generation methods treat garment generation and virtual dressing as separate problems, limiting flexibility in real-world workflows. Existing multi-condition synthesis methods suffer from attribute entanglement and semantic interference due to simple feature concatenation or static injection approaches.

Method: 1) Trait-routing attention (TA) module using mixture-of-experts to dynamically route condition features to compatible experts and generative layers for disentangled injection of texture, shape, and color attributes. 2) Automated multi-perspective preference optimization (MPO) pipeline that constructs preference data without human annotation by combining evaluators for content fidelity, textual alignment, and perceptual quality, then optimizes via direct preference optimization (DPO).
...
Conclusion: VersaVogue provides a unified framework for multi-condition controllable fashion synthesis that addresses limitations of prior works by enabling disentangled attribute injection and automated preference optimization, supporting both design and showcase stages of the fashion lifecycle.
...
TL;DR: PhyEdit is a 3D-aware image editing framework that uses explicit geometric simulation to improve physical accuracy in object manipulation, outperforming existing methods on a new benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing visual generative models often fail at precise spatial manipulation (incorrect scaling and positioning) due to lack of explicit 3D geometry and perspective projection mechanisms, limiting their potential for interactive world models.

Method: PhyEdit combines plug-and-play 3D prior (explicit geometric simulation as contextual 3D-aware visual guidance) with joint 2D-3D supervision. Also introduces RealManip-10K dataset and ManipEval benchmark for evaluation.

Result: Outperforms existing methods including strong closed-source models in both 3D geometric accuracy and manipulation consistency, as shown through extensive experiments.

Conclusion: Explicit geometric simulation combined with joint 2D-3D supervision effectively improves physical accuracy in image editing, enabling more realistic object manipulation for interactive applications.
...
TL;DR: Geo-EVS improves extrapolative novel view synthesis for autonomous driving by using geometry-conditioned framework with sparse supervision, addressing degradation outside recorded trajectories through explicit exposure to out-of-trajectory condition defects.
...
Method: Two-component framework: 1) Geometry-Aware Reprojection (GAR) uses fine-tuned VGGT to reconstruct colored point clouds and reproject them to observed/virtual poses for geometric condition maps; 2) Artifact-Guided Latent Diffusion (AGLD) injects reprojection-derived artifact masks during training to learn structure recovery under missing support.
...
Conclusion: Geo-EVS effectively addresses extrapolative novel view synthesis challenges in autonomous driving by explicitly exposing models to out-of-trajectory condition defects during training, enabling better performance with sparse supervision and improving downstream tasks.
...
TL;DR: Deep networks predict human authenticity judgments well but produce inconsistent explanations across architectures, suggesting post-hoc explanations are weak evidence for cognitive mechanisms.


<details>
  <summary>Details</summary>
Motivation: To test whether deep neural networks that predict human judgments provide robust, human-like explanations, or if their explanations are architecture-dependent and unreliable despite good predictive performance.

Method: Fitted lightweight regression heads to multiple frozen pretrained vision models, generated attribution maps using Grad-CAM, LIME, and multiscale pixel masking, evaluated explanation consistency within and across architectures, and used ensembles to improve predictions.

Result: Models predicted authenticity ratings well (~80% noise ceiling), but VGG tracked image quality rather than authenticity. Attribution maps were stable within architectures (especially EfficientNetB3 and Barlow Twins) but showed weak cross-architecture agreement. Ensembles improved predictions and enabled image-level attribution.

Conclusion: Deep networks can predict human authenticity judgments well but do not produce identifiable explanations for those judgments. Post-hoc explanations from successful behavioral models should be treated as weak evidence for cognitive mechanisms.
...
TL;DR: GenLCA is a diffusion-based generative model that creates photorealistic full-body avatars from text/image inputs, enabling high-fidelity animations by training on millions of real-world videos using a novel visibility-aware 3D diffusion approach.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with generating photorealistic full-body avatars that are faithful to inputs while supporting high-quality animations. The challenge is training 3D diffusion models with limited complete 3D data, as real-world videos typically provide only partial body observations.

Method: 1) Repurpose a pretrained avatar reconstruction model as an animatable 3D tokenizer to encode video frames into structured 3D tokens. 2) Introduce visibility-aware diffusion training that replaces invalid regions with learnable tokens and computes losses only over valid regions. 3) Train a flow-based diffusion model on the token dataset to maintain photorealism and animatability.

Result: GenLCA achieves superior photorealism and generalizability, producing diverse and high-fidelity generation and editing results that outperform existing solutions by a large margin. The method effectively leverages large-scale real-world video data for native 3D diffusion training.

Conclusion: The proposed approach successfully enables training full-body 3D diffusion models from partially observable 2D data at scale, overcoming the limitations of incomplete real-world video observations and achieving state-of-the-art photorealistic avatar generation and animation.
...
TL;DR: Mem3R is a streaming 3D reconstruction model with hybrid memory design that decouples camera tracking from geometric mapping to improve temporal consistency in long sequences.


<details>
  <summary>Details</summary>
Motivation: Existing recurrent models for streaming 3D perception suffer from drift accumulation and temporal forgetting over long sequences due to limited capacity of compressed latent memories, making them unsuitable for robotics and AR applications requiring long visual stream processing.

Method: Proposes Mem3R with hybrid memory: 1) implicit fast-weight memory (lightweight MLP updated via Test-Time Training) for camera tracking, and 2) explicit token-based fixed-size state for geometric mapping. This decouples tracking from mapping to improve temporal consistency.

Result: Reduces model size from 793M to 644M parameters compared to CUT3R, decreases Absolute Trajectory Error by up to 39% on 500-1000 frame sequences when integrated with TTT3R, improves downstream tasks (video depth estimation, 3D reconstruction), while maintaining constant GPU memory usage and comparable inference throughput.

Conclusion: Mem3R's hybrid memory design effectively addresses drift and forgetting in long-sequence 3D perception, offering improved performance with reduced parameters while maintaining efficiency for robotics and AR applications.
...
TL;DR: Different DNN models (domain-specific and foundation models) encode facial identity in surprisingly compatible ways, with simple affine transformations able to align their embedding spaces for improved cross-model face recognition.


<details>
  <summary>Details</summary>
Motivation: To understand whether different DNN models encode facial identity similarly despite being trained on different datasets, loss functions, and architectures, and to explore the geometric structure of their embedding spaces.

Method: Analyze geometric structure of embedding spaces by treating face image embeddings as point clouds and studying whether simple affine transformations can align face representations between different models.
...
TL;DR: ROAM is a spatially-aware Mixture-of-Experts MIL aggregator for whole-slide image classification that uses capacity-constrained optimal transport routing to ensure balanced expert utilization and spatial coherence.


<details>
  <summary>Details</summary>
Motivation: Current MIL aggregators route all instances through shared pathways, limiting specialization across pathological heterogeneity. Standard MoE methods suffer from imbalanced expert utilization where a few experts dominate, collapsing back to near-single-pathway solutions.

Method: ROAM compresses dense patch bags into spatial region tokens, then uses capacity-constrained entropic optimal transport (Sinkhorn) for region-to-expert assignment with explicit capacity marginals. It incorporates graph-regularized Sinkhorn iterations that diffuse routing assignments over spatial region graphs to encourage neighboring regions to route to the same experts.

Result: ROAM achieves competitive performance against strong MIL and MoE baselines on four WSI benchmarks. On NSCLC generalization (TCGA-CPTAC), it reaches external AUC of 0.845 ± 0.019.

Conclusion: ROAM effectively addresses imbalanced expert utilization in MoE-MIL frameworks through spatially-aware optimal transport routing, enabling better specialization across pathological heterogeneity while maintaining balanced expert utilization and spatial coherence.
...
TL;DR: AlignPrune is a noise-robust module that improves dynamic data pruning under noisy-label settings by using a loss-trajectory-based Dynamic Alignment Score to better identify noisy samples.


<details>
  <summary>Details</summary>
Motivation: Existing dynamic data pruning methods fail under noisy-label settings because they rely on per-sample loss as ranking criterion, which mistakenly preserves noisy samples (due to their high loss values) and causes significant performance drops.

Method: Proposes AlignPrune with Dynamic Alignment Score (DAS), a loss-trajectory-based criterion that enables more accurate identification of noisy samples. It's a plug-and-play module that can be integrated into existing dynamic pruning frameworks without modifying model architecture or training pipeline.
...
Conclusion: AlignPrune offers a generalizable solution for pruning under noisy data, encouraging further exploration of learning in real-world scenarios. Code is publicly available.
...
<details>
  <summary>Details</summary>
Motivation: Photon-counting CT provides superior image quality but has limited clinical availability. The goal is to bridge this gap by enhancing routine EICT scans to PCCT-like quality using limited high-quality PCCT scans as reference, enabling PCCT benefits without requiring widespread PCCT deployment.

Method: SUMI uses a simulated degradation-to-enhancement approach that explicitly models realistic acquisition degradations to transform PCCT into clinically plausible lower-quality counterparts, then learns to invert this process. The method involves: (1) training a latent diffusion model on PCCTs using an autoencoder pre-trained on both PCCTs and EICTs, (2) creating a large-scale dataset of enhanced EICTs with radiologist-validated annotations, and (3) validating simulated degradations for clinical realism by board-certified radiologists.

Result: SUMI outperforms state-of-the-art image translation methods by 15% in SSIM and 20% in PSNR on external data. It improves radiologist-rated clinical utility in reader studies and enhances downstream lesion detection performance, increasing sensitivity by up to 15% and F1 score by up to 10%. The method also produces a large dataset of 17,316 enhanced EICTs with voxel-wise annotations.

Conclusion: Emerging imaging advances like PCCT can be systematically distilled into routine EICT using limited high-quality scans as reference. SUMI provides a practical solution to bridge the gap between advanced imaging technologies and clinical availability, enabling PCCT benefits without requiring widespread deployment of expensive new hardware.
...
TL;DR: Gaussian Wrapping introduces a principled occupancy field for 3D Gaussian Splatting to extract accurate watertight meshes, addressing 3DGS's fundamental limitation in surface extraction.


<details>
  <summary>Details</summary>
Motivation: 3D Gaussian Splatting (3DGS) revolutionized fast novel view synthesis but lacks a global geometric field, making surface extraction difficult. Unlike implicit methods with SDFs or occupancy, 3DGS forces existing approaches to use heuristics like TSDF fusion of blended depth maps.

Method: 1) Introduce learnable oriented normals at each Gaussian element with adapted attenuation formulation for closed-form normal and occupancy fields. 2) Novel consistency loss and densification strategy to enforce Gaussians to wrap surfaces by closing geometric holes. 3) Modify differentiable rasterizer to output depth as isosurface of continuous model. 4) Introduce Primal Adaptive Meshing for region-of-interest meshing at arbitrary resolution.
...
Conclusion: Gaussian Wrapping provides principled geometric foundation for 3DGS, enabling accurate surface extraction while exposing biases in standard evaluation protocols and proposing more rigorous alternatives.
...
Method: Introduced a multi-category, cross-cultural benchmark for structured cultural metadata inference and evaluated VLMs using an LLM-as-Judge framework that measures semantic alignment with reference annotations.
...
TL;DR: TC-AE is a ViT-based autoencoder that improves deep compression by addressing token space issues through two-stage token-to-latent compression and self-supervised token enhancement, avoiding latent collapse while maintaining reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Existing deep compression methods increase latent channel numbers to maintain quality, but this causes latent representation collapse that degrades generative performance. The paper aims to address this fundamental limitation without complex architectures or multi-stage training.

Method: Two complementary innovations: 1) Decompose token-to-latent compression into two stages to reduce structural information loss and enable effective token number scaling; 2) Enhance semantic structure of image tokens via joint self-supervised training to create more generative-friendly latents.
...
Conclusion: The research advances ViT-based tokenizers for visual generation by addressing fundamental token space challenges in deep compression autoencoders, offering a more effective approach than simply increasing latent channel dimensions.
...
TL;DR: MoRight: A framework for generating motion-controlled videos with disentangled camera/object control and motion causality modeling.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to provide disentangled motion control (camera vs object) and don't model causal relationships between object motions, treating motion as mere kinematic displacement.

Method: Uses disentangled motion modeling: object motion specified in canonical static-view, transferred via temporal cross-view attention. Decomposes motion into active (user-driven) and passive (consequence) components, learning causality from data.

Result: State-of-the-art performance on three benchmarks for generation quality, motion controllability, and interaction awareness. Supports both forward reasoning (predict consequences) and inverse reasoning (recover driving actions).

Conclusion: MoRight enables physically plausible scene dynamics with disentangled camera/object control and motion causality, allowing flexible user interaction through forward/inverse reasoning while adjusting viewpoints.
...
TL;DR: Elastic Test-Time Training (ETTT) stabilizes Large Chunk Test-Time Training with elastic weight consolidation to handle longer sequences in smaller chunks, enabling Fast Spatial Memory for efficient 4D reconstruction.


<details>
  <summary>Details</summary>
Motivation: LaCT suffers from catastrophic forgetting and overfitting during inference-time updates, limiting it to single large chunks and preventing handling of arbitrarily long sequences in one pass.

Method: Propose Elastic Test-Time Training with Fisher-weighted elastic prior around anchor state, plus Fast Spatial Memory model for 4D reconstruction with spatiotemporal representations from long sequences.

Result: FSM enables fast adaptation over long sequences with smaller chunks, delivers high-quality 3D/4D reconstruction, and mitigates camera-interpolation shortcut while reducing activation-memory bottleneck.

Conclusion: Advances LaCT beyond single-chunk setting toward robust multi-chunk adaptation for longer sequences, enabling efficient 4D reconstruction with improved stability and scalability.
