...
TL;DR: ReactiveGWM decouples player controls from NPC behaviors in game world models, enabling zero-shot transfer of NPC interaction logic across different games without retraining, demonstrated on Street Fighter games.


<details>
  <summary>Details</summary>
Motivation: Existing game world models treat NPCs as background pixels, failing to capture player-NPC interactions. They act as passive video renderers lacking physical understanding for action-induced NPC reactivities.

Method: ReactiveGWM explicitly decouples player controls from NPC behaviors: player actions are injected via lightweight additive bias into diffusion backbone; high-level NPC responses are grounded through cross-attention modules that learn game-agnostic interactive logic.

Result: Enables zero-shot strategy transfer across different Street Fighter games; maintains fine-grain player controllability while achieving robust, prompt-aligned NPC strategy adherence without domain-specific retraining.

Conclusion: ReactiveGWM paves the way for scalable, strategy-rich interaction with NPCs by enabling plug-and-play NPC interaction modules across unannotated game world models.
...
TL;DR: DPA replaces the standard ViT encoder with a small VLM as perceiver to deeply align visual features with text space, achieving significant performance gains (up to 3.0 points at 32B scale) and 32.9% reduction in language capability forgetting across multiple LLM families.


<details>
  <summary>Details</summary>
Motivation: Standard VLM architectures suffer from an alignment challenge where visual features remain distant from the text space in initial LLM layers, forcing the model to waste critical depth on superficial modality alignment rather than deep understanding and reasoning.

Method: Proposes Deep Pre-Alignment (DPA), which replaces the standard ViT encoder with a small VLM as a perceiver, ensuring visual features are deeply aligned with the text space of the target LLM before entering the main language model.

Result: At 4B scale, DPA outperforms baselines by 1.9 points across 8 multimodal benchmarks; at 32B scale, gains widen to 3.0 points. Achieves 32.9% reduction in language capability forgetting over 3 text benchmarks. Gains are consistent across Qwen3 and LLaMA 3.2 families.

Conclusion: DPA offers an effective and general approach to addressing the modality alignment challenge in VLMs, providing a seamless upgrade path with marginal computation overhead by modularly replacing the visual encoder.
...
TL;DR: RTM improves generative models by replacing single-pass latent mapping with iterative refinement, achieving better precision and recall alongside competitive FID across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: FID is saturated and conflates fidelity with coverage; models can achieve low FID while suffering mode collapse. Precision and recall are needed as complementary metrics, and improving diversity/coverage is the more meaningful goal.

Method: RTM replaces the single-pass latent mapping in style-based generators with an iterative refinement process, integrated with IMLE which optimizes mode coverage by design.

Result: RTM achieves the highest precision and recall among SOTA methods while maintaining competitive FID on CIFAR-10, CelebA-HQ (256x256), nine few-shot benchmarks, and improves StyleGAN2/StyleGAN2-ADA on CIFAR-10 and AFHQ-v1 (512x512).

Conclusion: Recursive refinement simultaneously improves both quality and diversity, unlike flow-matching baselines that trade coverage for FID.
...
TL;DR: COPRA is a conditional parameter adaptation framework for VLM-based video anomaly detection that generates input-specific parameter updates to dynamically adapt a frozen VLM per video segment, addressing distribution mismatch and training-inference inconsistency issues.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based VAD methods suffer from (1) static post-training adaptation limiting generalization under distribution shifts, and (2) inconsistency between training on sparse frames and inference on densely sampled short segments.

Method: COPRA generates input-specific (conditional) parameter updates to dynamically adapt a frozen VLM for each video segment during both training and inference, avoiding fixed prompts or shared parameter updates.

Result: Outperforms static baselines on standard VAD benchmarks in both in-domain and cross-domain settings; generalizes beyond VAD to unseen tasks like multiple-choice Video QA and Dense Captioning.

Conclusion: COPRA is an effective weight-space generation framework for scalable, adaptive, and context-aware video understanding.
...
TL;DR: This paper proposes a multimodal pipeline combining LiDAR evaluation, visible-thermal fusion, and Airborne Optical Sectioning (AOS) for detecting humans under forest canopy, with a fine-tuned YOLOv5 achieving ~0.83 mAP on FLIR thermal classes.


<details>
  <summary>Details</summary>
Motivation: Reliable detection of humans beneath forest canopy is difficult due to sparse, structured, and viewpoint-dependent occlusion from vegetation.

Method: Three complementary approaches: (i) LiDAR returns evaluation for active sensing feasibility, (ii) visible-thermal image fusion via multi-scale transform and sparse-representation, (iii) synthetic-aperture imaging via AOS to suppress canopy clutter. Detection is performed using fine-tuned YOLOv5 on FLIR thermal dataset.

Result: Terrestrial LiDAR showed limited penetration for object-level detection; visible-thermal fusion improved target visibility in low-contrast scenes; AOS enhanced ground-plane detection in synthetic forest imagery. Fine-tuned YOLOv5 achieved ~0.83 mAP on top three FLIR classes.

Conclusion: Establishes an initial baseline for UAV-deployable search-and-rescue and surveillance systems in forests, motivating future work on dedicated forest datasets and real-time multimodal integration.
...
TL;DR: Minerva-Ego is a new benchmark for evaluating complex egocentric visual reasoning, featuring multi-step multimodal questions, human-annotated reasoning traces, and spatiotemporal mask annotations. Experiments show a large gap between state-of-the-art models and humans, and performance improves when models are prompted with "where" and "when" hints.


<details>
  <summary>Details</summary>
Motivation: Standard benchmarks only evaluate output answers (often text-only) without assessing intermediate reasoning steps. The authors aim to fill this gap by providing a benchmark that evaluates complex egocentric visual reasoning with fine-grained reasoning traces.

Method: The authors extend existing egocentric video data sources with multi-step multimodal questions and spatiotemporally-dense human-annotated reasoning traces. Each reasoning trace is annotated with object-of-interest masks to investigate model failures.

Result: State-of-the-art models still have a large gap compared to human performance. Prompting frontier models with hints about "where" and "when" to look leads to substantial performance improvements.

Conclusion: Minerva-Ego provides a more comprehensive evaluation of egocentric video reasoning, revealing that current models struggle with spatiotemporal reasoning, and explicit location/timing cues help bridge this gap.
...
TL;DR: GCNNs are computationally expensive due to dense geometric sampling of transformation groups. This paper proposes feature-space sampling to replace dense geometric sampling with representative samples selected by feature similarity, decoupling geometric resolution from computational cost and enabling substantial acceleration of equivariant 3D classifiers.


<details>
  <summary>Details</summary>
Motivation: GCNNs incur exponential computational costs with increasing degrees of freedom (e.g., translations and rotations in 3D), hindering practical applications.
...
Result: Coarse feature-space sampling preserves classification accuracy remarkably well, enabling precomputation based on geometric similarity and substantially accelerating equivariant 3D classifier training.

Conclusion: Feature-space sampling provides a novel way to trade off computational effort and accuracy in GCNNs, making them more practical for applications with high degrees of freedom.
...
TL;DR: ChangeFlow is a generative framework for remote sensing change detection that reformulates the task as latent space synthesis via rectified flow, achieving state-of-the-art F1 scores (80.4% average, +1.3 points improvement) while maintaining fast inference speed.


<details>
  <summary>Details</summary>
Motivation: Existing RSCD methods suffer from two key limitations: (1) discriminative per-pixel classification fails to model change regions as coherent wholes and cannot handle ambiguity, while (2) generative approaches lag behind discriminative baselines due to high computational cost and complex conditioning mechanisms.

Method: ChangeFlow reformulates change detection as the synthesis of a change mask in latent space via rectified flow, guided by a structured yet lightweight conditioning signal, with stochastic design enabling sampling-based prediction ensembling.

Result: Across four benchmarks, ChangeFlow achieves an average F1 of 80.4%, improving by 1.3 points over the previous best method, with inference speed comparable to recent strong baselines.

Conclusion: The generative formulation with latent space rectified flow bridges the gap between discriminative and generative methods in RSCD, offering both superior accuracy and practical features like ambiguity estimation through sample agreement.
...
TL;DR: MorphoHELM is a comprehensive open benchmark for evaluating feature extraction methods on Cell Painting microscopy images, assessing methods across diverse tasks and batch effect levels to reveal trade-offs, finding that no deep learning model consistently outperforms classic computer vision approaches.


<details>
  <summary>Details</summary>
Motivation: Microscopy image analysis is crucial for drug screening, but evaluation of representation extraction methods is fragmented—each model is tested on different tasks, datasets, and metrics, making fair comparison difficult. The authors aim to standardize and unify evaluation.

Method: The authors consolidate and extend evaluation standards for Cell Painting, evaluating methods across the widest range to date. A key innovation is evaluating each task at varying degrees of batch effects (technical noise) to quantify how biological signal detection degrades with noise.

Result: MorphoHELM reveals trade-offs between methods—models excelling at certain biological signals underperform at others. No existing deep learning model outperforms classic computer vision analytic strategies across all settings; classical methods remain the strongest for general-purpose use.

Conclusion: Standardized benchmarks like MorphoHELM are essential for fair method comparison, and classic computer vision approaches still hold value. The benchmark is publicly available for community use.
...
TL;DR: PanoWorld is a panoramic video world model that generates geometry-consistent 360° video from a single image and caption by introducing depth consistency and trajectory consistency regularizers, along with a unified PanoGeo dataset, achieving improved geometric consistency while maintaining visual realism.


<details>
  <summary>Details</summary>
Motivation: Existing panoramic video methods optimize for visual realism but do not explicitly constrain the underlying 3D scene state, resulting in inconsistent depth, broken correspondences, and implausible motion on the spherical surface.

Method: PanoWorld builds on a pre-trained perspective video world model and adds two lightweight regularizers: a depth consistency loss against pseudo ground-truth panoramic depth, and a trajectory consistency loss supervising 3D world-frame positions of tracked points. It also applies spherical-geometry-aware adaptation to conditioning and positional encoding, and introduces the PanoGeo dataset for training/evaluation.
...
Conclusion: Panoramic video generation must be treated as a geometric modeling problem, not just visual synthesis, to support holistic spatial understanding for embodied AI applications.
...
TL;DR: ELDOR is a large-scale UAV benchmark with over 2,500 hectares of annotated orthomosaic imagery for monitoring illegal gold mining in the Amazon, spanning four benchmark tasks that reveal current methods struggle with small-scale mining structures and fine-grained recovery classes.


<details>
  <summary>Details</summary>
Motivation: Illegal gold mining in the Amazon causes severe environmental damage but is hard to monitor at fine scales; satellite imagery misses small structures and subtle land-cover changes, especially under cloud cover.

Method: The authors introduce ELDOR, a UAV benchmark with pixel-level semantic labels covering 2,500+ hectares, and establish four tasks: semantic segmentation, segmentation-derived recognition, multi-label classification, and vision-language model recognition under closed-set protocol.

Result: Current methods struggle with rare small-scale mining structures and fine-grained recovery classes; generic and remote-sensing-specific models were compared across all four tasks.

Conclusion: Context-aware and multimodal modeling are needed to improve monitoring of illegal gold mining; an interactive explorer for domain experts was also built to support practical use.
...
TL;DR: This paper conducts a large-scale study on under-explored topics in uncertainty estimation for segmentation, examining how factors like task difficulty (panoptic vs. semantic), datasets, backbones, temporal information, sample diversity, and ensemble vs. deterministic approaches affect uncertainty estimation quality.


<details>
  <summary>Details</summary>
Motivation: Many factors affecting uncertainty estimation quality in segmentation remain unclear, yet uncertainty estimates are critical for identifying prediction errors in real-world applications. The authors aim to systematically identify which variables degrade or improve performance.

Method: The authors create a framework for large-scale experimentation across multiple variables: datasets, backbones, downstream tasks, and segmentation types (semantic and panoptic), specifically investigating four key questions about domain difficulty, temporal frames, combined estimation approaches, and ensemble vs. deterministic methods.

Result: a) Panoptic segmentation yields worse uncertainty performance, with high variance across datasets/backbones. b) Time series (prior frames) help only in specific configurations. c) Sample diversity aids calibration but is otherwise outperformed by simpler alternatives. d) Deterministic methods suffice for some tasks, but ensembles provide significant gains under the right deployment conditions.

Conclusion: Uncertainty estimation generalization is not guaranteed across tasks and architectures; simpler methods often compete well, but ensembles remain valuable under favorable conditions.
...
TL;DR: MR2-ByteTrack enables real-time Video Object Detection on resource-constrained microcontrollers by alternating full/low-resolution inference, linking detections across frames via ByteTrack, and correcting misclassifications with a Rescore algorithm, achieving up to 55% energy savings while maintaining competitive accuracy.


<details>
  <summary>Details</summary>
Motivation: Smart vision sensors need on-device intelligence for video processing due to cloud computing limitations (bandwidth, latency, privacy), but conventional VOD methods require too much memory/compute for ultra-low-power MCUs.

Method: MR2-ByteTrack reduces compute by alternating between full and low-resolution inference; links detections across frames via ByteTrack; corrects misclassifications via Rescore algorithm (probability union rules to aggregate confidence scores). Works with both CNN and Transformer detectors.

Result: Achieves mAP 49.0 (CNN) and 48.7 (Transformer) on ImageNetVID; reduces MACs by 53% (CNN) and 32% (Transformer); yields up to 55% energy savings on GAP9 RISC-V MCU; enables first real-time Transformer-based VOD on an MCU-class node.

Conclusion: MR2-ByteTrack effectively bridges the gap between accurate VOD and ultra-low-power MCU constraints, demonstrating generality across CNN and Transformer architectures while enabling practical on-device video intelligence.
...
TL;DR: Social-Mamba introduces a linear-time alternative to quadratic attention mechanisms for human trajectory forecasting by reformulating social interactions as structured sequential processes using Selective State-Space Models and a Cycle Mamba block, achieving state-of-the-art accuracy with superior parameter efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory forecasting methods rely on attention mechanisms that incur quadratic computational costs scaling poorly with dense crowds, while linear-time Selective State-Space Models (SSMs) are inherently sequential and misaligned with the unstructured, dynamic nature of social interactions.

Method: Social-Mamba organizes agents on an egocentric grid, introduces social triplet factorization (decomposing interactions into temporal, egocentric, and goal-centric scans), and uses a Cycle Mamba block with learnable social gate and global scan for continuous bidirectional information flow.

Result: Achieves state-of-the-art accuracy on five trajectory forecasting benchmarks with superior parameter efficiency and computational scalability. Embedding into a flow-matching framework further enhances both accuracy and efficiency.

Conclusion: Social-Mamba establishes a flexible, robust, and efficient foundation for future trajectory forecasting research, offering a linear-time alternative to attention-based methods that scales well to dense crowds.
...
TL;DR: The paper introduces RIDE, a Retinex-based homogeneous image decomposition method for Concealed Object Segmentation (COS). It proves a Discriminability Gap Theorem showing that Retinex decomposition preserves or improves foreground-background discriminability due to systematic anti-correlation between illumination and reflectance. RIDE includes a Task-Driven Retinex Decomposition module, Discriminability Gap Attention, and a Camouflage-Breaking Contrastive loss.


<details>
  <summary>Details</summary>
Motivation: Existing COS methods either use raw RGB images or heterogeneous decompositions (Fourier, wavelet) that spread spatial evidence across frequency/scale coefficients, making pixel-aligned cues less direct. The authors aim to exploit homogeneous decomposition via Retinex theory, which keeps illumination and reflectance in the same spatial domain, to better handle visual entanglement in concealed objects.

Method: The method RIDE has three components: (1) Task-Driven Retinex Decomposition module that learns segmentation-optimal factorizations end-to-end; (2) Discriminability Gap Attention mechanism that adaptively leverages decomposition benefits; (3) Camouflage-Breaking Contrastive loss operating in reflectance feature space. The theoretical foundation is the Discriminability Gap Theorem, which proves that Retinex decomposition preserves or improves discriminability due to anti-correlated illumination and reflectance differences.

Result: The paper provides theoretical guarantees that across diverse COS sub-tasks, Retinex decomposition preserves or strictly improves total foreground-background discriminability, with anti-correlation between illumination and reflectance maximizing the gain.

Conclusion: Retinex-based homogeneous decomposition offers a principled and effective alternative to heterogeneous decompositions for COS, with theoretical guarantees on discriminability improvement.
...
TL;DR: VideoRLVR optimizes video diffusion models using reinforcement learning with verifiable rewards, enabling rule-consistent visual reasoning across tasks like Maze, FlowFree, and Sokoban.


<details>
  <summary>Details</summary>
Motivation: Video diffusion models excel at perceptual realism but struggle with verifiable reasoning tasks requiring explicit spatial, temporal, or logical constraints.

Method: VideoRLVR combines SDE-GRPO optimization, dense decomposed rewards, and an Early-Step Focus strategy (which reduces training latency by ~40%) to train video diffusion models with rule-based feedback.

Result: VideoRLVR consistently outperforms supervised fine-tuning baselines and other video generation models on verifiable reasoning benchmarks and out-of-domain benchmarks.
...
TL;DR: IA-JEPA uses a motion-centric masking strategy to prioritize physical interactions over visual texture, achieving significant gains in causal reasoning (14.26% vs 3.22% on CLEVRER) by breaking static bias and learning more discriminative latent representations.


<details>
  <summary>Details</summary>
Motivation: Standard JEPA models are physics-blind due to patch-based masking that prioritizes visual texture over rare kinematic events, failing to capture causal dynamics needed for reasoning.

Method: IA-JEPA employs a self-supervised motion-centric masking strategy that specifically targets entities engaged in collisions/momentum transfers, forcing the architecture to reconstruct latent trajectories instead of static background features.

Result: 14.26% accuracy on CLEVRER causal reasoning (vs 3.22% baseline), +10% entropy gain in latent space, physical energy linearization with R²=0.43, and generalization to Something-Something V2 and PHYRE-Lite.

Conclusion: Interaction-aware masking provides a scalable, fully self-supervised path toward foundational world models that internalize causal structure of the physical world by breaking static bias.
...
TL;DR: This paper introduces an empowered transposed Fully Connected Weighted (t-FCW) graph representation for point cloud embedding, revealing its robustness, interpretability, and efficiency properties while demonstrating its use as both a standalone network and a plug-in module for classification, part segmentation, and semantic segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: To understand why the original t-FCW is effective for point cloud classification and extend its applicability to broader tasks like part segmentation and semantic segmentation, while also improving its interpretability and efficiency.

Method: The authors analyze properties of empowered t-FCW, design a network using it exclusively as feature extractors, and build memory banks for multiple tasks. The method leverages surface descriptor robustness and dimension-wise relations for interpretability.

Result: The empowered t-FCW network processes ModelNet40 in ~7 seconds on an NVIDIA RTX A5000 GPU, demonstrating high efficiency. It inherits robustness from surface descriptors and provides interpretability through dimension-wise relations.

Conclusion: Empowered t-FCW works effectively as both a lightweight standalone baseline and a complementary plug-in to existing deep models for point cloud tasks.
...
TL;DR: This paper bridges the gap between abundant exocentric video data and egocentric world models by extracting structured body poses from exocentric video and converting them to egocentric video, enabling large-scale training of egocentric world models that significantly improves prediction and planning performance.


<details>
  <summary>Details</summary>
Motivation: Egocentric world models are limited by scarce egocentric training data and partial observability of human actions, while exocentric video is abundant and reveals body poses well but is not aligned with egocentric action spaces.

Method: Extract structured body pose from exocentric video, transform exocentric to egocentric video using a human kinematics prior, then train whole-body action-conditioned egocentric world models with the converted data.

Result: Training with converted exocentric data significantly improves prediction quality and downstream planning performance for inferring body pose sequences to achieve visual goals.

Conclusion: This approach enables leveraging arbitrary in-the-wild videos for building powerful egocentric world models, benefiting robot planning and augmented-reality guidance.
...
TL;DR: This paper examines sparse top-k routing with hard capacity constraints in Mixture-of-Experts (MoE) networks for vision tasks. It finds that positive accuracy gains require a substantial fraction of total FLOPs to be routed (compute-leverage pattern), and at ImageNet scale, multi-expert routing is also necessary. Controlled experiments reveal that backbone family is not the active variable, top-k routing alone can reverse accuracy gaps, and batch-axis dispatch is identified as the dominant failure mode in per-sample CNN settings.


<details>
  <summary>Details</summary>
Motivation: MoE networks promise favorable accuracy-compute trade-offs but suffer from expert collapse and limited end-to-end efficiency gains in practical vision deployments.

Method: The paper studies sparse top-k routing with hard capacity constraints under multi-seed protocols on CIFAR-10/100, Tiny-ImageNet, and ImageNet-1K. Controlled experiments include hidden-size sweeps and top-k ablations, plus a per-sample variant of Soft MoE.

Result: A compute-leverage pattern is found: positive accuracy gaps require a substantial fraction ρ of total FLOPs to be routed. At ImageNet scale, multi-expert routing (k≥2) is necessary but not sufficient. Hidden-size sweep rules out backbone family as the active variable. Top-k ablation holding ρ fixed reverses the gap from positive to negative. Per-sample Soft MoE rescues CIFAR-100 above dense baseline, identifying batch-axis dispatch as the dominant failure mode.

Conclusion: Sparse MoE routing in vision requires careful balancing of compute allocation (ρ) and routing topology (k), with batch-axis dispatch identified as a key failure mechanism in per-sample CNN settings.
...
TL;DR: AnyAct is a method that converts monocular videos of non-human characters into plausible human reenactments by using sparse local 2D articulated motion cues as a bridge, trained with human-motion-only supervision and progressive 3D-to-2D projection.


<details>
  <summary>Details</summary>
Motivation: To enable direct human reenactment from non-human character videos for downstream animation authoring, overcoming the limitation that existing motion capture methods are human-centric and retargeting requires structured 3D source motions.

Method: AnyAct formulates the task as conditional human motion generation from transferable sparse local 2D articulated motion, with three key designs: (1) human-motion-only supervision via augmented 3D-to-2D projection, (2) progressive 3D-to-2D training to reduce conditioning ambiguity, and (3) global-local motion decoupling for reliable local motion control.

Result: AnyAct produces high-fidelity initial human reenactments that preserve essential dynamics of characters in reference videos, validated on a benchmark covering diverse non-human character videos.

Conclusion: Sparse local articulated motion cues effectively bridge structural differences between non-human characters and humans, enabling direct video-to-human reenactment without requiring 3D source motion or known topologies.
...
TL;DR: DiffVAS uses diffusion models and reinforcement learning to enable UAV-based visual active search for multiple target objects in partially observable environments, outperforming prior methods that assume full observability and single-target search.


<details>
  <summary>Details</summary>
Motivation: Prior VAS methods assume the entire search space is known upfront (unrealistic due to limited field of view and high costs) and learn policies for specific single target objects, limiting multi-category search capability.

Method: DiffVAS combines a diffusion model to reconstruct the full geospatial area from partial observations, with a target-conditioned reinforcement learning module to plan and guide subsequent search steps.

Result: DiffVAS excels in searching diverse objects in partially observable environments, significantly surpassing state-of-the-art methods on several datasets.

Conclusion: DiffVAS advances real-world deployment of VAS by handling partial observability and multi-target search simultaneously, demonstrating superior performance.
...
TL;DR: This paper proposes a self-prompting scene text editing method that constructs style and glyph prompts directly from the original image using a diffusion transformer with two-stage training, achieving open-vocabulary and style-consistent text editing without external encoders.


<details>
  <summary>Details</summary>
Motivation: Existing scene text editing methods neglect visual details of target regions (discarding original text's stylistic features) and rely on pre-trained glyph encoders that limit editable text scope, effectively degrading the task to trivial text rendering.

Method: Proposes a self-prompting method using Multi-Modal Diffusion Transformer (MM-DiT) with two-stage training: (1) large-scale self-supervised pre-training, (2) fine-tuning on small paired datasets. Style and glyph prompts are constructed directly from the original image without additional encoders.

Result: Achieves state-of-the-art performance in both text accuracy and style consistency across various languages.

Conclusion: Leveraging in-context learning capability of MM-DiT enables open-vocabulary and style-consistent text editing without external encoders or limited glyph conditions.
...
TL;DR: A tuning-free, instruction-based video editing framework that improves quality by strategically initializing noise (SNIS) and using a Noise Guidance Mechanism (NGM) to leverage noisy latent information for better content preservation and coherence.


<details>
  <summary>Details</summary>
Motivation: Existing tuning-free video editing methods underutilize rich information in noisy latent, leading to unsatisfactory editing results.

Method: Proposes a Structural Noise Initialization Strategy (SNIS) that assigns higher noise to edited regions and lower noise to unedited regions, plus a Noise Guidance Mechanism (NGM) that leverages video priors to guide denoising.

Result: Achieves better visual quality and state-of-the-art performance compared to existing methods.

Conclusion: The proposed noise-based approach effectively improves tuning-free video editing by better utilizing noisy latent information.
...
TL;DR: DSS-USOD is a new RGB-based underwater salient object detection method that uses dynamic structural specialization—decomposing shared base representations into boundary-sensitive and region-coherent branches, coordinated by a spatial coordination module—to better handle underwater image degradations, achieving superior results on benchmarks and real-world robot deployment.


<details>
  <summary>Details</summary>
Motivation: Existing USOD methods struggle with underwater image degradations (e.g., color distortion, low contrast) leading to inaccurate object localization, fragmented salient regions, and coarse boundary prediction.

Method: DSS-USOD extracts a shared base representation from a single RGB underwater image, decomposes it into boundary-sensitive (for fine-grained details) and region-coherent (for structural consistency) branches, uses a spatial coordination module to adaptively regulate their contributions based on local context, and applies cooperative structural supervision to promote branch specialization and stabilize coordination.

Result: Achieves superior performance on benchmark datasets, and real-world deployment on an underwater robot validates its practical effectiveness for underwater object inspection.

Conclusion: Dynamic structural specialization effectively balances boundary precision and region coherence under degraded underwater conditions, demonstrating both state-of-the-art performance and real-world applicability.
...
TL;DR: Proposes 3DTMDet, a point cloud object detector combining Mamba (state space models) with Transformers to handle the conflict between point sparsity and long-range context, achieving SOTA on KITTI and ONCE datasets.


<details>
  <summary>Details</summary>
Motivation: Existing 1D serialization methods for expanding receptive field discard local geometric details, hurting detection of distant and small objects. The authors aim to resolve the tension between receptive field enlargement and geometric detail preservation.

Method: Proposes 3D Hybrid Mamba Transformer (3DHMT) block using an SSM-Attention-SSM pipeline, plus a LiDAR-physics-inspired voxel generation block that diffuses features along sensor observation direction to reconstruct occluded/ distant object structures.

Result: Outperforms state-of-the-art detectors on KITTI and ONCE datasets.

Conclusion: The hybrid SSM-Transformer design effectively balances global context understanding and local detail preservation for point cloud detection, especially benefiting distant and small objects.
...
TL;DR: RoiMAM is an efficient VLM for medical VQA that uses a training-free ROI Generation Module with Semantic Selective Suppression to focus on lesion regions and a Text Prompt Enhancer for modality-specific context, achieving better accuracy than MedVInT-TD at under 20% of its model size.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs for MedVQA rely on large architectures and closed-set answers, limiting efficiency and clinical applicability.

Method: Integrates a training-free ROI Generation Module with Semantic Selective Suppression to focus on lesion-relevant regions, plus a Text Prompt Enhancer for modality-specific context without extra parameters.

Result: Achieves ~2% higher accuracy on SLAKE and ~4.6% higher on PMC-VQA than MedVInT-TD, at less than 20% of the model size.

Conclusion: RoiMAM demonstrates that efficient, smaller VLMs can outperform larger counterparts in MedVQA by focusing on relevant image regions and using effective text prompts.
...
TL;DR: MI-CXR is a new benchmark for evaluating multi-visit longitudinal reasoning in chest X-ray interpretation across five-visit timelines, using three task families. Testing 14 VLMs revealed only 29.3% average accuracy, with models failing to enforce temporal constraints or compose global reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing medical VQA benchmarks focus on single images or short-horizon pairs, lacking evaluation of longitudinal reasoning over multiple patient visits, which is critical for real-world clinical CXR interpretation.

Method: MI-CXR provides five-way multiple-choice questions over five-visit patient timelines with three task families: Temporal Event Localization, Interval-wise Change Reasoning, and Global Trajectory Summarization. 14 VLMs were evaluated with stage-wise diagnostic probing.

Result: Average accuracy of 29.3% across 14 state-of-the-art VLMs, only modestly above random guessing (20%). Models produced locally plausible interval descriptions but failed to enforce temporal constraints or compose evidence globally.

Conclusion: Current VLMs show fundamental limitations in longitudinal medical reasoning. MI-CXR serves as a principled benchmark highlighting the need for temporally grounded and globally consistent visual reasoning in medical AI.
...
TL;DR: Proposes LDGuid, a framework that explicitly learns and injects task-relevant semantic differences into change detection models using adversarial autoencoding and information bottleneck, improving performance across multiple benchmarks especially under noisy conditions.


<details>
  <summary>Details</summary>
Motivation: Modern CD models fail to explicitly represent task-relevant semantic differences between pre- and post-event images, limiting their robustness.

Method: LDGuid uses adversarial autoencoding with information bottleneck pretraining to learn a difference embedding (DE) module, which explicitly guides CD models (U-Net, BIT, AERNet) by injecting latent semantic differences.

Result: LDGuid improves segmentation across LEVIR-CD, WHU-CD, SVCD, and CaBuAr datasets, with significant gains under spectral noise, and demonstrates ability to incorporate domain knowledge like spectral indices.

Conclusion: Explicit semantic difference learning enhances CD model robustness in remote sensing, suggesting a promising direction for handling challenging environmental noise.
...
TL;DR: A method (cMAS) estimates 3D human pose from single-view 2D poses without 3D supervision by leveraging 2D diffusion priors from pre-trained motion diffusion models, using multi-view ancestral sampling conditioned on 2D poses and anatomical constraints. Outperforms SOTA on Yoga dataset, especially for extreme poses.


<details>
  <summary>Details</summary>
Motivation: Existing 3D pose estimation methods require 3D supervision, which is scarce for extreme/unusual human poses. The authors aim to lift 2D poses to 3D without any 3D-labeled data by exploiting powerful 2D diffusion priors.

Method: Extends multi-view ancestral sampling (diffusion models) to propose conditional multi-view ancestral sampling (cMAS). Optimizes 3D pose so its multi-view 2D projections lie on the 2D MDM noise manifold, conditioned on input 2D poses and human anatomical constraints.

Result: On the Yoga dataset, cMAS achieves better cross-domain performance than supervised and unsupervised SOTA methods, particularly for extreme human poses where 3D supervision is unavailable.

Conclusion: Leveraging 2D diffusion priors via cMAS enables robust 3D pose estimation without 3D supervision, with strong generalization to out-of-distribution poses. Code is open-sourced.
...
TL;DR: This paper proposes Adaptive Geodesic Correction (AGC), a training-free defense mechanism for CLIP models that leverages specific data augmentations as geometric anchors to correct adversarial inputs without parameter updates, achieving 44.4% higher robust accuracy and 10× faster inference than existing methods.


<details>
  <summary>Details</summary>
Motivation: CLIP models are vulnerable to imperceptible adversarial perturbations, and existing test-time defenses rely on costly gradient-based optimization during inference, making them impractical for deployment.

Method: AGC identifies a reliable augmentation as a geometric anchor in CLIP's hyperspherical feature space and corrects the input feature towards it using an adaptive step size, requiring no training or parameter updates.

Result: AGC achieves 44.4% higher average robust accuracy across eight fine-grained datasets and three CLIP backbones compared to state-of-the-art baselines, while reducing inference latency by 10×.

Conclusion: The findings reveal a fundamental geometric property of CLIP features—specific augmentations provide robust geometric cues—offering an efficient paradigm for robust multimodal deployment without computational overhead.
...
TL;DR: A paper that improves few-step image generation by decoupling Sphere Encoder into a fixed pretrained encoder and separate latent denoising model, achieving better quality and speed.


<details>
  <summary>Details</summary>
Motivation: Sphere Encoder has computational inefficiency and objective conflicts due to repeated pixel-latent space transitions and jointly optimizing reconstruction and generation.
...
Result: Significantly outperforms Sphere Encoder in generation quality and inference speed on Animal-Faces, Oxford-Flowers, and ImageNet-1K; competitive with strong few-step and multi-step baselines.

Conclusion: Decoupling reconstruction and generation in spherical latent space improves efficiency and performance for few-step image generation.
...
TL;DR: The paper proposes COVER, a training-free method to convert 3D assets into sparse panoramic RGB-D-pose datasets with complete scene coverage and low redundancy, and builds CM-EVS, a large-scale panoramic dataset with 36,373 curated frames from 1,275 indoor scenes and outdoor panoramas.


<details>
  <summary>Details</summary>
Motivation: Modern 3D visual learning lacks a sparse, comparable, and geometry-consistent panoramic training interface because existing scans, meshes, and point clouds produce dense trajectories, heterogeneous annotations, or depth-inconsistent observations.

Method: COVER (Coverage-Oriented Viewpoint curation with ERP Range-depth warping) greedily selects viewpoints by projecting geometry into candidate ERP probes, scoring incremental coverage, and penalizing depth conflicts, with theoretical guarantees on coverage approximation.

Result: CM-EVS dataset contains 36,373 curated ERP frames from 1,275 indoor scenes (median 25 frames/scene) across three indoor datasets and two outdoor datasets, covering all 13 unified room types. COVER improves the coverage-conflict trade-off.

Conclusion: COVER produces sparse, compact, and auditable RGB-D-pose resources for geometry-consistent panoramic 3D learning, and CM-EVS serves as a benchmark dataset for this purpose.
...
TL;DR: This paper compares four pretrained ViT-B/16 encoders (supervised, contrastive, masked reconstruction, self-distillation) for extreme low-data fine-grained emerald inclusion grading, finding supervised and contrastive encoders perform best with linear probes while MAE improves with nonlinear probes, and DINOv3 underperforms overall.


<details>
  <summary>Details</summary>
Motivation: Extreme low-data fine-grained classification is common in expert domains where labeling is expensive. Practitioners need principled guidance for selecting pretrained encoders under such data-scarce conditions.

Method: Four frozen ViT-B/16 encoders (supervised classification, SigLIP2 contrastive learning, MAE masked reconstruction, DINOv3 self-distillation) were compared on a custom emerald inclusion grading dataset. Evaluation used leave-one-out cross-validation with linear (logistic regression, SVM) and nonlinear (XGBoost) probes, with permutation testing (N=1000) on macro one-vs-rest AUC to control statistical noise.

Result: Supervised (logistic AUC: 0.768, SVM AUC: 0.739) and contrastive (0.735, 0.697) encoders showed strongest linear separability. MAE improved under nonlinear probes (XGBoost AUC: 0.713). DINOv3 underperformed across all probe families.

Conclusion: For extreme low-data FGVC, prioritize margin-enforcing pretraining objectives when data scarcity restricts probing to linear classifiers; consider reconstruction-style encoders when nonlinear classifiers are feasible.
...
TL;DR: NeRP is a plug-and-play prompting correction strategy that addresses the Base-New Trade-off in vision-language model transfer learning by correcting asymmetric inter-class confusion on unseen classes without modifying model parameters.


<details>
  <summary>Details</summary>
Motivation: The paper observes that VLMs exhibit asymmetric confusion on downstream data (class A systematically mispredicted as B but not vice versa). While known-class bias can be corrected by cross-entropy tuning, unseen-class bias persists and harms generalization. The goal is to boost unseen class recognition without sacrificing known class performance.

Method: NeRP uses neutral text prompts and reference images to measure class-wise prior preferences along the pre-trained inter-class geometry. It combines these priors with sample likelihood to obtain surrogate scores. When prior strongly favors current prediction but observed evidence is insufficient, NeRP performs a local flip between easily confusable class pairs to correct prior-dominated mispredictions.

Result: Extensive experiments across multiple backbones and 15 few-shot and cross-domain benchmarks show NeRP substantially improves accuracy on unseen classes while preserving known-class prediction performance.

Conclusion: The paper concludes that addressing asymmetric confusion is key to resolving the Base-New Trade-off, and NeRP effectively mitigates this issue in a parameter-free, plug-and-play manner.
...
TL;DR: This paper systematically evaluates four video foundation models across five robustness axes, showing that latent-prediction models (V-JEPA) are more robust than reconstruction-based models for world modeling.


<details>
  <summary>Details</summary>
Motivation: Current evaluation of video models as world models is limited to single top-1 accuracy on clean benchmarks, leaving a major gap in understanding their true potential as world models.

Method: Analyzed four matched-capacity video foundation models (V-JEPA 2.1, V-JEPA 2, VideoPrism, VideoMAEv2) across five robustness axes: feature discriminability, corruption robustness, fine-grained discrimination, occlusion robustness, and sensitivity to temporal direction.

Result: Latent-prediction models degrade more gracefully under pixel corruption, preserve class structure under occlusion, capture fine-grained physical contact cues, and uniquely encode temporal direction. A frozen V-JEPA 2 with lightweight probe outperforms fully fine-tuned VideoMAE and supervised TimeSformer on corruption and occlusion robustness.

Conclusion: Latent prediction offers concrete advantages for robust world modeling over reconstruction-based approaches.
...
TL;DR: LRCP is a training-free visual token compression method that leverages PCA to identify and retain only the tokens with significant projection residuals relative to the dominant low-rank subspace, achieving ~89% token reduction while preserving ~95% of performance in image and video understanding tasks.


<details>
  <summary>Details</summary>
Motivation: LVLMs suffer from high inference cost due to the growing number of visual tokens. Existing attention-based and representation-based compression methods have limitations: attention methods suffer from positional bias, while representation methods overlook global structure of the visual token set.

Method: First, observe that visual token representations exhibit a low-rank structure with a stable dominant subspace. Then propose LRCP: (1) estimate the dominant low-rank subspace of visual tokens via PCA, (2) score each token by its projection residual onto this subspace, (3) retain tokens poorly explained by the low-rank background. No training required.

Result: Image understanding: 94.7% performance preserved with 88.9% token reduction. Video understanding: 97.8% accuracy preserved with 87.5% token reduction.

Conclusion: Visual tokens are highly compressible due to low-rank structure. LRCP effectively exploits this property for efficient, training-free token reduction across both image and video tasks.
...
TL;DR: GMAE proposes a generalized multi-view auto-encoder with disentangled representation learning, cross-view adversarial discriminators, and mutual information modulation to address view-distribution entanglement in multi-view clustering, outperforming SOTA on 13 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing deep MVC methods suffer from view-distribution entanglement during cross-view fusion, which degrades the shared latent space quality and leads to suboptimal clustering results.

Method: (1) Dual-path autoencoders to decouple source features into view-specific and view-common embeddings. (2) Cross-view adversarial discriminators to guide view-specific encoders for more discriminative features. (3) Mutual information modulation to align distributions and prevent representation collapse.
...
Conclusion: Disentangled representation learning with adversarial training and mutual information modulation effectively resolves view-distribution entanglement and produces robust, non-trivial embeddings for multi-view clustering.
...
TL;DR: MaTe is a zero-shot, training-free diffusion framework for material transfer that eliminates text guidance, reference networks, and fine-tuning by integrating input images at the token level via multi-modal attention in a shared latent space, outperforming existing methods in quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based material transfer methods suffer from text dependency, extra computational costs from fine-tuning or assistive networks, and feature misalignment. The authors aim to address these limitations with a streamlined approach.

Method: MaTe integrates input images at the token level and processes them via multi-modal attention in a shared latent space, removing the need for additional adapters, ControlNet, inversion sampling, or model fine-tuning.

Result: MaTe achieves high-quality material generation under a zero-shot, training-free paradigm and outperforms state-of-the-art methods in both visual quality and efficiency while preserving precise detail alignment.

Conclusion: MaTe significantly simplifies inference prerequisites and demonstrates effectiveness in material transfer without textual guidance or complex architectural components.
...
TL;DR: VAGS proposes a training-free, velocity-adaptive guidance scale that adjusts CFG strength per timestep based on temporal signal level and cosine similarity between velocity fields, improving both editing and generation quality.


<details>
  <summary>Details</summary>
Motivation: Standard CFG uses a fixed guidance scale across the entire ODE trajectory, which is suboptimal because early steps are noise-dominated (weak signal) while late steps demand stronger commitment; also, guidance strength should depend on whether the guided velocity aligns with the model's current dynamics.

Method: VAGS multiplies the nominal CFG scale by a bounded factor combining (1) temporal signal-level term and (2) cosine similarity between task-relevant velocity fields. For editing, it measures alignment between source- and target-guided velocities; for generation, alignment between unconditional and conditional velocities. No fine-tuning, auxiliary networks, or extra forward passes needed.

Result: Evaluated on PIE-Bench, DIV2K (editing), COCO17, CUB-200, Flickr30K (generation). VAGS consistently improves structural fidelity and generation quality over fixed CFG and recent training-free guidance variants.

Conclusion: VAGS is a simple, training-free replacement for fixed CFG that adapts guidance strength per timestep based on velocity alignment, achieving better results across editing and generation tasks while recovering fixed CFG as a special case.
...
TL;DR: ChronoEarth-492K is the first large-scale, temporally calibrated hyperspectral SSL dataset built from NASA's EO-1 Hyperion mission (2001-2017), containing 492K patches across 185K global locations. It includes a benchmark suite for static, short-horizon, and long-horizon temporal tasks, enabling systematic spatiotemporal hyperspectral representation learning.


<details>
  <summary>Details</summary>
Motivation: Existing hyperspectral SSL datasets are temporally shallow, lacking sufficient temporal depth to support long-horizon spatiotemporal modeling for land cover and ecosystem dynamics analysis.

Method: The authors built ChronoEarth-492K from NASA's EO-1 Hyperion archive (2001-2017), comprising 492,354 radiometrically harmonized patches across 185,398 global locations, with 28,786 sites having multi-temporal sequences (≥3 observations). They also created the ChronoEarth-Benchmark with six open-source geospatial products covering various tasks and established a standardized evaluation protocol.

Result: ChronoEarth-492K provides the largest temporally calibrated hyperspectral dataset to date, spanning 17 years. The benchmark enables systematic evaluation across static, short-horizon, and long-horizon tasks, with extensive baseline results reported for state-of-the-art hyperspectral foundation models.

Conclusion: ChronoEarth and its benchmark provide the first large-scale, temporally grounded platform for systematic spatiotemporal hyperspectral representation learning, filling a critical gap in temporally deep hyperspectral SSL datasets.
...
TL;DR: VLMs struggle with line tracing, frequently losing the target path to nearby similar alternatives due to local competition, and standard remedies like scaling, reasoning, or explicit instructions fail to fix this bottleneck.


<details>
  <summary>Details</summary>
Motivation: Despite strong benchmark performance, VLMs may lack robust control over basic visual operations like line tracing. The paper aims to isolate and understand this fundamental failure mode.

Method: The authors design controlled tracing tasks that reduce semantic and topological ambiguity (e.g., crossings, overlaps) but introduce nearby competitors. They conduct behavioral interventions and internal analyses to diagnose failures, and test on real-world tangled-cable scenes and metro maps.

Result: State-of-the-art VLMs frequently lose the target path and switch to nearby alternatives when those look locally similar. Failures arise from local competition. Model scaling offers limited gains, reasoning is costly and only partially compensates, and explicit tracing instructions do not help.

Conclusion: Line tracing remains a persistent bottleneck for current VLMs due to local competition, which standard remedies cannot reliably overcome. This failure extends to real-world visually complex scenes.
...
TL;DR: DreamSR is a diffusion-based super-resolution model that addresses over-generation and poor local texture synthesis in patch-wise inference by using a dual-branch MM-ControlNet (local patch-level + global prompts) and a tailored training strategy with Receptive-Field Enhancement.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based SR methods suffer from over-generation (misalignment between global LR prompt and local patch semantics) and fail to generate detailed local textures due to overemphasis on global generation capabilities.

Method: DreamSR proposes (1) a dual-branch MM-ControlNet where ControlNet generates local textual features with patch-level prompts and DiT provides global textual features, (2) stage-specific data processing pipelines, and (3) a Receptive-Field Enhancement training strategy.

Result: DreamSR outperforms state-of-the-art methods, providing high-quality SR results with suppressed over-generation and improved fine-detail synthesis.

Conclusion: The proposed dual-branch architecture and specialized training strategy effectively solve over-generation and local texture degradation issues in diffusion-based super-resolution, achieving visually faithful high-resolution results.
...
TL;DR: ElasticDiT is a single-model framework for mobile deployment of Diffusion Transformers that dynamically balances fidelity and latency by adjusting spatial compression ratios and block depths, using SSBA attention, T-DVAE, and Flow-GRPO to outperform task-specific baselines.


<details>
  <summary>Details</summary>
Motivation: Deploying state-of-the-art Diffusion Transformer models (e.g., Stable Diffusion-3, FLUX.1) on resource-constrained mobile devices is infeasible due to prohibitive computational and memory costs. Existing efficiency methods like Linear-DiT and static pruning cause quality degradation, and unlike cloud environments, mobile constraints demand a single-model paradigm that can dynamically trade off fidelity and latency without multiple specialized models.

Method: ElasticDiT dynamically adjusts spatial compression ratios and DiT block depths on-the-fly within a single model. It integrates: (1) Shift Sparse Block Attention (SSBA) for high sparsity (84.16% average) with maintained quality, (2) Tiny DWT-Distilled VAE (T-DVAE) for SD3-level reconstruction at 1/8x computation, and (3) Flow-GRPO for improved semantic alignment (GenEval from 66.93 to 73.62).

Result: ElasticDiT's flex lite variant achieves an HPS of 32.87, surpassing the Flux model, with 84.16% average sparsity via SSBA. T-DVAE provides competitive reconstruction at 1/8x standard VAE cost. Flow-GRPO boosts GenEval from 66.93 to 73.62. A single ElasticDiT model covers a wide fidelity-latency trade-off range, outperforming task-specific baselines.

Conclusion: ElasticDiT provides a versatile, hardware-adaptive solution for deploying high-resolution image generation on mobile devices, eliminating the need for multiple specialized models and offering a promising path for future mobile deployment of Diffusion Transformers.
...
TL;DR: This paper introduces Ratio 1-2, a teacher selection metric for knowledge distillation in fine-grained image recognition, which improves teacher selection by 18% and enables up to 17% accuracy gains for small student models.


<details>
  <summary>Details</summary>
Motivation: Existing knowledge distillation methods lack effective metrics for selecting the optimal teacher model, which significantly impacts student performance on fine-grained image recognition tasks.

Method: The authors propose a teacher selection metric called "Ratio 1-2" based on teacher prediction ratios, validated through extensive experiments (1000+ experiments) across 3 students, 8 teachers, 8 datasets, and 4 training strategies.

Result: The proposed metric improves teacher selection by 18% over previous methods, and small student models achieve up to 17% accuracy gains.

Conclusion: The Ratio 1-2 metric is an effective and practical tool for selecting teachers in knowledge distillation for fine-grained image recognition, enabling better deployment on resource-constrained devices.
...
TL;DR: This paper introduces a large viewpoint-aware 3D referring segmentation dataset (220k samples, scalable to millions) that requires observer-centric spatial reasoning, and shows that explicit viewpoint conditioning improves segmentation accuracy (mIoU from 0.30 to 0.47) over models without it.


<details>
  <summary>Details</summary>
Motivation: Existing 3D referring segmentation methods lack explicit observer viewpoint representation, making spatial relations like "left/right/front/behind" ambiguous and difficult to evaluate.

Method: The authors create a viewpoint-aware 3D referring segmentation dataset by leveraging camera poses to automatically annotate observer-centric relations. They evaluate existing 3D LMMs in zero-shot setting and propose a viewpoint representation that encodes camera poses to condition models on observation viewpoint.

Result: Current models struggle with viewpoint-dependent spatial instructions. Introducing explicit viewpoint conditioning improves segmentation accuracy on viewpoint-dependent relations, increasing mIoU from 0.30 to 0.47.

Conclusion: Explicit viewpoint information is crucial for accurate 3D referring segmentation involving observer-centric spatial relations, and the proposed dataset enables further research in this direction.
...
TL;DR: EntropyScan is a lightweight, trigger-agnostic method for detecting backdoor attacks in LVLMs by measuring structural anomalies in visual attention distributions using Tsallis entropy and Z-score normalization, achieving 98.5% F1 and 96.6% AUC.


<details>
  <summary>Details</summary>
Motivation: Existing backdoor defense methods focus on sample-level detection (requiring knowledge of training data or triggers), but identifying whether a given LVLM model itself is backdoored remains unexplored.

Method: Extracts visual attention distributions from the initial LLM layers, applies Tsallis entropy to capture structural distortions caused by backdoor injection, and uses reference-anchored Z-score normalization on a small set of benign samples to detect backdoored models.

Result: EntropyScan achieves an average F1 score of 98.5% and AUC of 96.6% across two LVLM architectures and three advanced attack scenarios.

Conclusion: EntropyScan is an effective, lightweight, and trigger-agnostic solution for model-level backdoor detection in LVLMs, addressing a critical gap in current defense research.
...
TL;DR: Semi-MedRef is a teacher-student SSL framework for medical referring image segmentation that preserves cross-modal alignment via T-PatchMix (synchronized patch mixing), PosAug (position-aware text augmentation), and ITCL (position-guided contrastive learning), outperforming baselines on QaTa-COV19 and MosMedData+.


<details>
  <summary>Details</summary>
Motivation: Medical referring image segmentation (MRIS) requires costly pixel-level masks aligned with textual descriptions. Semi-supervised learning can reduce annotation burden, but existing methods fail to maintain reliable image-text alignment under strong augmentations, and CutMix—effective in single-modal SSL—remains underexplored in multi-modal settings due to disrupting image-text coherence.

Method: A teacher-student SSL framework with three alignment-preserving components: (1) T-PatchMix—a cross-modal CutMix augmentation synchronizing patch mixing with referring expressions via position-constrained and probability-driven rules; (2) PosAug—position-aware text augmentation masking or fuzzing anatomical phrases; (3) ITCL—position-guided image-text contrastive learning using positional pseudo-labels to construct soft anatomical positives for stronger cross-modal alignment.

Result: Experiments on QaTa-COV19 and MosMedData+ show Semi-MedRef consistently outperforms both fully supervised and semi-supervised baselines across all label regimes.

Conclusion: The proposed alignment-preserving components (T-PatchMix, PosAug, ITCL) effectively maintain cross-modal consistency under strong augmentations, making semi-supervised MRIS feasible and superior to existing methods in low-label settings.
...
TL;DR: DiLA introduces a disentangled latent action world model that resolves the trade-off between action abstraction and generation fidelity by separating content and structure pathways, achieving superior video generation, action transfer, visual planning, and interpretability.


<details>
  <summary>Details</summary>
Motivation: Latent Action Models (LAMs) face a fundamental trade-off between action abstraction and generation fidelity, which existing methods only circumvent via two-stage training or limiting predictions to optical flow.

Method: DiLA performs content-structure disentanglement, where the predictive bottleneck from latent action learning drives the model to distill spatial layouts into the structure pathway and visual details into a separate content pathway.

Result: DiLA achieves superior results in video generation quality, action transfer, visual planning, and manifold interpretability, simultaneously enabling high-level action abstraction and high-fidelity generation.

Conclusion: DiLA establishes a unified self-supervised framework that resolves the action abstraction vs. generation fidelity trade-off, advancing the frontier of world model learning.
...
TL;DR: DecomPose is a difficulty-aware decomposition framework for category-level 6D object pose estimation that mitigates cross-category optimization conflicts by using gradient diagnostics, difficulty-aware gradient decoupling, and stability-driven asymmetric branching, achieving state-of-the-art results on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Category-level 6D pose estimation suffers from gradient conflicts and negative transfer due to geometric heterogeneity across object categories when using fully shared model parameters, which entangles incompatible optimization signals.

Method: (1) Gradient-based diagnostics to quantify module-level cross-category contention; (2) Difficulty-aware gradient decoupling that groups categories via data-driven difficulty proxy and routes instances to group-specific branches; (3) Stability-driven asymmetric branching that assigns higher-capacity branches to simple categories as anchors and lightweight branches to complex categories to suppress noisy updates.

Result: Extensive experiments on REAL275, CAMERA25, and HouseCat6D demonstrate that DecomPose effectively reduces cross-category optimization contention and delivers superior pose estimation performance across multiple benchmarks.

Conclusion: DecomPose successfully addresses the negative transfer problem in multi-category 6D pose estimation by decomposing the learning based on category difficulty, leading to improved performance.
...
TL;DR: Standard VLA fine-tuning erodes VLM multimodal competence ("embodiment tax"). Inspired by biological vision's two pathways, the authors propose UAM with a parallel Dorsal Expert initialized from a pretrained generative model and trained with a mid-level visual dynamics objective, achieving >95% VLM capability retention and top success rates on out-of-distribution manipulation tasks without freezing, gradient stopping, or auxiliary co-training.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning VLMs on action data causes systematic degradation of multimodal competence (embodiment tax). The authors trace this to a structural bottleneck: a single encoder must serve both semantics and control, unlike biological vision which separates these into distinct pathways.

Method: UAM adds a parallel Dorsal Expert (analogous to the brain's dorsal pathway) initialized from a pretrained generative model and trained with a mid-level reasoning objective predicting visual dynamics. The entire VLA is trained end-to-end on action data alone without parameter freezing, gradient stopping, or auxiliary VL co-training.

Result: UAM retains over 95% of the underlying VLM's multimodal capability while achieving the highest average success rate among baselines on manipulation tasks probing out-of-distribution generalization (unseen objects, novel compositions, instruction variation).

Conclusion: Semantic preservation in VLAs can emerge from architectural separation alone, not requiring frozen weights or auxiliary data replay. Preserved semantic capability naturally transfers to generalization in actions.
...
TL;DR: BiomedAP is a vision-informed dual-anchor framework with gated cross-modal fusion that significantly improves few-shot medical diagnosis robustness under noisy prompt variations, outperforming existing methods across 11 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing biomedical VLMs are fragile to prompt variations because they optimize visual and textual prompts independently, leading to unstable cross-modal alignment when clinical descriptions are noisy and heterogeneous.

Method: BiomedAP introduces (1) Gated Cross-Modal Fusion for layer-wise interaction as a dynamic noise regulator, and (2) a Dual-Anchor Constraint that regularizes prompts toward stable semantic centroids from expert templates (High Anchors) and few-shot visual prototypes (Low Anchors).

Result: Across 11 benchmarks, BiomedAP consistently surpasses baselines with competitive few-shot accuracy and markedly enhanced robustness under prompt perturbations.

Conclusion: BiomedAP effectively addresses prompt fragility in biomedical VLMs by synergistically aligning modalities, offering a robust solution for real-world clinical few-shot diagnosis.
...
TL;DR: BARRIER is a new machine unlearning framework that shifts intervention from model weights to hidden-layer activation geometry using Interval Arithmetic on SVD-based projections, enabling aggressive targeted erasure with formal mathematical guarantees for preserving non-targeted knowledge.


<details>
  <summary>Details</summary>
Motivation: Existing weight-space unlearning methods cause collateral damage by suppressing unintended representations, lack formal guarantees for preserving neutral concepts, and are forced into conservative updates to avoid degradation.

Method: BARRIER shifts intervention from static weights to hidden-layer activation geometry, employing Interval Arithmetic on SVD-based projections to encapsulate target regions within a bounding hypercube, driving unlearning updates exclusively within the forget interval while mathematically bounding model response on the complement.

Result: BARRIER matches state-of-the-art trade-offs across classifiers and diffusion models, maximizing targeted concept erasure while safeguarding all other representations.

Conclusion: The geometric construction transforms knowledge preservation from empirical heuristic into a formal optimization target with probabilistic tail bound on functional drift, permitting highly aggressive unlearning updates within the forget region.
...
TL;DR: HyperDiT proposes Hyper-Connected Cross-Scale Interactions to resolve the pixel-space diffusion dilemma between global semantics and fine-grained details, achieving SOTA FID of 1.56 on ImageNet 256×256.


<details>
  <summary>Details</summary>
Motivation: Pixel-space diffusion models face a "granularity dilemma": large patches capture global semantics but lose details, while fine-grained inputs preserve details but miss high-level semantics. Existing methods (e.g., AdaLN) are insufficient for bridging this gap.

Method: (1) Cross-Attention mechanism enables fine-grained tokens to query multi-level semantic anchors globally (instead of AdaLN). (2) Scale-Aware Rotary Position Embedding (SA-RoPE) ensures precise geometric alignment across different patch scales. (3) Registers learn dense semantics from a pretrained Visual Foundation Model (VFM) to reduce hallucination and artifacts.

Result: Achieves state-of-the-art FID of 1.56 on ImageNet 256×256 in pixel space, demonstrating superior high-fidelity generation.

Conclusion: HyperDiT effectively unifies global semantic understanding and fine-grained detail generation in pixel space, offering a superior paradigm over VAE-based approaches.
...
TL;DR: This paper addresses attribute flooding in MLLMs for artwork emotion understanding by introducing AGSR (Attribute-Grounded Selective Reasoning) and FAB-G, a supervised multi-agent framework that predicts which formal attributes are emotionally salient, achieving better emotion prediction and more compact explanations.


<details>
  <summary>Details</summary>
Motivation: MLLMs produce fluent artwork emotion explanations but suffer from attribute flooding—enumerating many visible attributes without identifying which cues actually support the affective judgment. The authors aim to make emotion reasoning more selective and grounded.

Method: The authors propose FAB-G (Formal-Attribute Bottleneck-Guided reasoning), a supervised multi-agent framework that first predicts attribute-level salience, then constrains downstream emotional analysis to only emotionally operative cues. They extend the EmoArt dataset with a 1,400-artwork human salience extension annotated by 15 art-trained annotators.

Result: FAB-G achieves consistent gains in emotion, arousal, and valence prediction, stronger agreement with human-marked salient attributes (Dice, Tversky metrics), and more compact explanations than prompting-based baselines. Cross-dataset evaluation shows transferable attribute-grounded salience selection.

Conclusion: Attribute-grounded selective reasoning improves both the accuracy and interpretability of artwork emotion understanding, and the human salience extension provides valuable supervision for distinguishing present vs. emotionally salient attributes.
...
TL;DR: This paper introduces a learned optimizer for 3D Gaussian Splatting that avoids degradation over long optimization horizons without manual learning rate scheduling, using a meta-learning scheme with checkpoint buffer and optimizer rollout strategy.


<details>
  <summary>Details</summary>
Motivation: Standard optimizers (Adam, SGD) for 3DGS produce independent parameter updates that fail to capture structural and spatial relationships within a scene, leading to inefficient optimization and slow convergence.

Method: A learned optimizer that encodes gradient scale information in latent states, combined with a meta-learning scheme featuring a checkpoint buffer and optimizer rollout strategy to extend the optimization horizon without degradation.

Result: Improved early novel view synthesis quality, stable performance over long horizons, and zero-shot generalization to unseen reconstruction settings.

Conclusion: The proposed learned optimizer outperforms conventional optimizers for 3DGS, and the authors introduce the first unified framework for training and evaluating optimizers across sparse and dense view settings.
...
TL;DR: GRASP is a large-scale social reasoning dataset (290K QA pairs over 46K videos, 749 hours) connecting high-level social QA with fine-grained gaze and gesture events. The paper also proposes Social Grounding Reward (SGR), a learning signal that improves MLLM performance on social reasoning tasks.
...
Method: (1) GRASP dataset: 16-category taxonomy spanning gaze, gesture, and joint gaze-gesture reasoning with identity-consistent annotations. (2) Social Grounding Reward (SGR): a learning signal leveraging these social events to train models.
...
Conclusion: Annotating fine-grained social cues (gaze, gesture) as training signals can improve MLLMs' social reasoning in multi-person video understanding.
...
TL;DR: This paper proposes Generation-to-Understanding (G2U) synergy, where visual generation serves as an explicit intermediate reasoning step to enhance multimodal understanding, demonstrating that self-generated visual feedback improves perception across 12 benchmarks without retraining.


<details>
  <summary>Details</summary>
Motivation: Current multimodal AI unification is one-directional (understanding guides generation), but how generation supports understanding is rarely investigated. The authors aim to reverse this asymmetry.

Method: G2U framework: models perform controlled generative acts (detail enhancement, context expansion, structural visualization) to produce self-generated visual thoughts, which are fed back to refine perception without retraining or external tools.

Result: Evaluation on 12 benchmarks shows consistent improvement in multimodal understanding. Generative fidelity bounds perceptual gain, and distinct edit prompt families govern transfer efficiency.

Conclusion: Self-generated visual thoughts lack stable task alignment, revealing current LMMs fall short of true self-reflection. Imagination is not the end of understanding but its beginning.
...
TL;DR: A unified framework for preprocessing and registering 3D fingerprints with both contactless and contact-based 2D fingerprints, achieving ridge-level accuracy and improved cross-modal compatibility.


<details>
  <summary>Details</summary>
Motivation: 3D fingerprints preserve global geometry and local ridge structure without contact deformation, but are difficult to integrate with legacy 2D fingerprint systems.

Method: Four-component framework: (1) nonparametric visualization/unwrapping of 3D point cloud to rolled-equivalent 2D; (2) point-cloud fusion for multi-view registration and mosaicing; (3) ellipse-based pose normalization; (4) pose-aware cross-modal registration for 2D compatibility.

Result: 3D fusion error ~0.09mm; ridge-scale projection accuracy for contactless 2D-3D registration; pose-aware unwrapping improves genuine matching scores.

Conclusion: 3D fingerprints can serve as an effective geometric bridge across heterogeneous fingerprint modalities.
...
TL;DR: E²PO introduces embedding-level perturbations to prevent variance decay in group-based RL optimization, improving alignment with human preference.


<details>
  <summary>Details</summary>
Motivation: Group-based RL optimization (e.g., GRPO) suffers from rapid intra-group variance decay, leading to training instability, premature stagnation, and reward hacking. Existing solutions (varying initial noise, increasing group size) are insufficient.

Method: E²PO (Embedding-perturbed Exploration Preference Optimization) applies structured, embedding-level perturbations within sample groups to maintain robust variance and preserve discriminative learning signals throughout training.

Result: E²PO significantly outperforms state-of-the-art baselines, achieving more faithful alignment with human preference.

Conclusion: Embedding-level perturbation is an effective and principled solution to the variance decay problem in group-based RL alignment, enabling stable and superior optimization.
...
TL;DR: FashionChameleon is a real-time interactive framework for multi-garment video customization that achieves 23.8 FPS on a single GPU (30-180× faster than baselines) using three key techniques: In-Context Learning with single garment data, Streaming Distillation for consistency/efficiency, and Training-Free KV Cache Rescheduling for interactive garment switching.


<details>
  <summary>Details</summary>
Motivation: Existing approaches cannot support low-latency and interactive garment control for human-centric video customization, which is crucial for e-commerce and content creation applications. The goal is to achieve interactive multi-garment video customization while preserving motion coherence using only single-garment video data.

Method: Three key techniques: (i) Teacher Model with In-Context Learning trained on single reference-garment pairs using mismatch enforcement; (ii) Streaming Distillation with In-Context Learning using teacher forcing and gradient-reweighted distribution matching distillation; (iii) Training-Free KV Cache Rescheduling including garment KV refresh, historical KV withdraw, and reference KV disentangle.

Result: Achieves real-time generation at 23.8 FPS on a single GPU, 30-180× faster than existing baselines, while supporting interactive customization and consistent long-video extrapolation.

Conclusion: FashionChameleon uniquely enables interactive multi-garment video switching in real-time with motion coherence, demonstrating significant commercial potential for e-commerce and content creation.
...
TL;DR: This paper proposes Fisher-Guided Quantization (FGQ), a PTQ method that uses the diagonal Fisher information matrix to quantify task/block/channel-level sensitivities in feed-forward 3D reconstruction models (like VGGT), enabling mixed-precision quantization that preserves critical channels for sensitive tasks, achieving up to 39% relative improvement under 4-bit quantization.


<details>
  <summary>Details</summary>
Motivation: Feed-forward 3D reconstruction models (e.g., VGGT) have billion-scale parameters causing memory/computation overhead, making on-device deployment challenging. Existing PTQ methods treat all tasks equally, overlooking that different transformer blocks and channels have substantially different sensitivities to quantization errors across multiple tasks (depth, camera pose, point cloud), causing significant accuracy loss on sensitive tasks.

Method: FGQ uses the diagonal Fisher information matrix to quantify sensitivity differences across tasks, blocks, and channels. These sensitivity measures are incorporated into a Learnable Affine Transformation during calibration to selectively preserve the most critical channels and blocks for each task.

Result: Consistently outperforms state-of-the-art quantization baselines on VGGT across camera pose estimation, point map reconstruction, and depth estimation tasks, achieving up to 39% relative improvement under 4-bit quantization.

Conclusion: Treating multi-task feed-forward 3D models with uniform quantization is suboptimal; explicitly modeling sensitivity differences via Fisher information leads to significantly better quantization performance, especially under low-bit settings.
...
TL;DR: This paper shows that sample-level metrics (AUROC, FPR) are insufficient for evaluating open-set plankton recognition in ecological monitoring, proposing a new community-level metric (OSCD) and demonstrating that community-aware threshold calibration outperforms fixed-recall approaches across three plankton datasets.


<details>
  <summary>Details</summary>
Motivation: Deployed plankton classifiers inevitably encounter unseen taxa, but standard open-set recognition metrics (sample-level AUROC, AUPR, FPR) do not align with ecological monitoring needs for accurate community-level abundance and diversity estimates.

Method: 1) Defines Open-Set Community Distortion (OSCD), a Bray-Curtis-style error with directional components. 2) Uses controlled pseudo-communities across three datasets (ZooScan, IFCB, in-situ camera). 3) Compares sample-level OOD metrics vs. community-aware threshold calibration.

Result: Closed-set classifiers absorbed unknowns with high confidence; sample-level OOD metrics were insufficient. Community-aware threshold calibration reduced OSCD on two datasets but not ZooLake; prototype-distance variant performed best for community-level.

Conclusion: Open-set plankton recognition should be evaluated as an ecological measurement problem with community-level metrics, not solely as a sample-level detection task.
...
TL;DR: WorldAct converts static generated 3D worlds into editable, interactive scenes using a multimodal agent for scene decomposition, object identification, mesh reconstruction, and background inpainting.


<details>
  <summary>Details</summary>
Motivation: Existing 3D world generation systems produce static monolithic assets with limited editability and physical interaction, restricting their use in immersive content creation and embodied simulation where worlds must be actively modified.

Method: WorldAct uses a multimodal agent to (1) guide scene decomposition, (2) identify actionable objects, (3) reconstruct geometrically aligned object-level meshes for interaction, and (4) restore residual background via 3D inpainting.
...
Conclusion: WorldAct enables richer interaction scenarios than original generated scenes, offering a practical path toward editable and interactive 3D world models.
...
TL;DR: GHOST is a training-free KV cache management framework for streaming 3D reconstruction that leverages the model's own 3D geometry outputs to evict redundant tokens, cutting cache by nearly half with 1.75x faster inference while preserving quality.


<details>
  <summary>Details</summary>
Motivation: Existing KV cache approaches for long monocular video 3D reconstruction either truncate to fixed anchor frames (degrading quality) or use attention-score heuristics that ignore 3D scene structure, failing to preserve geometrically valuable tokens.

Method: GHOST introduces three innovations: (1) hierarchical dual-level importance scoring, (2) a privilege mechanism protecting special tokens from eviction, and (3) cosine-similarity-guided layer-wise budget allocation.

Result: GHOST cuts KV cache by nearly half, achieves 1.75x faster inference than state-of-the-art methods, and preserves excellent reconstruction quality across various benchmarks.

Conclusion: Exploiting the model's own 3D geometry outputs for online token eviction is an effective, training-free approach to addressing the memory bottleneck in streaming 3D reconstruction from long videos.
...
TL;DR: AdaScope is an RL-enhanced plug-in that adaptively identifies optimal intervention timing during diffusion model denoising, achieving 66% better performance while cutting computational cost by 59% through avoiding early-stage action-reward mismatch and late-stage reward hacking.


<details>
  <summary>Details</summary>
Motivation: Existing RL fine-tuning of diffusion models applies RL across the full denoising trajectory, which is computationally costly and weakens preference alignment. Early denoising stages have unstable structures causing delayed rewards and high variance, while later stages suffer from reward saturation and overfitting.

Method: AdaScope adaptively identifies optimal RL intervention timing by perceiving structural evolution and semantic consistency during denoising, and dynamically terminates training when denoising converges and reward gains saturate.

Result: Improves performance by 66% while cutting computational cost by 59% compared to SOTA methods.

Conclusion: Selective, adaptive RL intervention at the right denoising stage achieves a rare "dual benefit" of both better performance and lower computational cost, challenging the full-trajectory RL paradigm.
...
TL;DR: A stereo calibration framework for RGB (2028x1520 px) and ultra-low-resolution TIR (80x62 px) cameras using an active OLED screen with modality-specific patterns, achieving 0.382 px reprojection error and 32.7 mm baseline.


<details>
  <summary>Details</summary>
Motivation: Accurate geometric calibration of RGB-TIR stereo systems is essential for multimodal building envelope analysis but remains challenging when using low-cost thermal sensors with very low spatial resolution (80x62 px vs 2028x1520 px, ~1:625 ratio).

Method: Active OLED screen dynamically switches patterns (checkerboard for TIR, ChArUco for RGB) on a single surface; dedicated corner detection combining perspective rectification, Hessian saddle-point analysis, and Mean Shift localization; baseline-constrained bundle adjustment enforcing physically consistent rig geometry.

Result: Stereo baseline of 32.7 mm (nominal 30 mm), overall reprojection error of 0.382 px; validated on a thermally active building mock-up with consistent TIR-to-RGB projection.

Conclusion: The proposed framework enables reliable RGB-TIR calibration for building energy performance assessment even with extreme resolution disparity between sensors.
...
TL;DR: VLMs frequently state they will re-examine images during reasoning, but VisualSwap testing shows they rarely actually do—they "say" rather than "see." Thinking models are 3x more vulnerable, scaling doesn't help, and only multi-turn user instructions (not self-reflection) restore visual grounding.
...
Method: VisualSwap: an image-swap probing framework where images are replaced with visually similar but semantically different ones after initial reasoning, testing whether models notice the change. VS-Bench contains 800 curated image pairs. Attention analysis is performed to explain observed behaviors.

Result: Models overwhelmingly miss the swap (accuracy drops up to 60%). Thinking models are ~3x more vulnerable than instructed models. Scaling offers no mitigation. Self-reflection doesn't restore visual grounding; only multi-turn user instructions do. Attention analysis shows user instructions elevate visual token attention while self-reflection does not.

Conclusion: Current VLMs tend to "say" rather than actually "see" when claiming to perform visual re-examination. Self-reflective statements are learned textual patterns, not indications of genuine visual engagement.
...
TL;DR: SOLAR is a self-supervised framework for symmetric multimodal-to-multimodal retrieval that uses unlabeled image-text pairs, learning intersection masks to align modalities while preserving semantic differences, and outperforms supervised VLMs by 7.08 points with 50x fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Symmetric MM2MM retrieval (queries and contexts interchangeable) is underexplored; existing universal multimodal retrieval methods fail because they rely on labeled asymmetric datasets.

Method: Two-stage self-supervised framework: Stage 1 learns intersection masks of image-text pairs to align shared semantics while preserving unique semantics; Stage 2 uses learned masks to construct positive and hard-negative samples for self-supervised multimodal embedding learning.

Result: Outperforms the strongest supervised VLM by 7.08 points on a new benchmark, with over 50x fewer model parameters and 5x smaller embedding dimension.

Conclusion: Self-supervised learning from unlabeled web-scale data can effectively address symmetric MM2MM retrieval, surpassing supervised methods with much smaller models.
...
TL;DR: DepthVLM transforms a VLM into a native dense geometry predictor by attaching a lightweight depth head to the LLM backbone, enabling full-resolution depth map generation alongside language outputs in a single forward pass, outperforming existing VLMs and pure vision models on 3D understanding tasks.


<details>
  <summary>Details</summary>
Motivation: VLMs excel at 2D tasks but are limited in 3D understanding due to text-only supervision that under-constrains fine-grained visual perception and prevents dense geometry recovery. Prior methods suffer from error accumulation (distilling from external vision models) or inefficiency (per-pixel query or coarse token-level outputs).

Method: DepthVLM attaches a lightweight depth head to the LLM backbone and trains under a unified vision-text supervision paradigm with a two-stage schedule, generating full-resolution depth maps alongside language outputs in a single forward pass.

Result: DepthVLM significantly outperforms existing VLMs with higher inference efficiency, surpasses leading pure vision models, and improves complex 3D spatial reasoning on a newly introduced unified indoor-outdoor metric depth benchmark.

Conclusion: DepthVLM moves toward a truly unified foundation model by enabling native dense geometry prediction within a VLM, eliminating the need for external vision models or inefficient per-pixel processing.
...
TL;DR: FSCM is a spectral-information-guided GAN for infrared image colorization that uses frequency-enhanced spatial-spectral state-space modeling to produce visually and semantically consistent color images, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Infrared images lack natural color and fine texture, hindering target recognition and human interpretation. Existing colorization methods rely on single-band images with insufficient spectral cues, causing structural distortion and semantic confusion.

Method: Proposes FSCM, a GAN framework with: (1) a frequency-enhanced spatial-spectral state-space generator with cascaded FSB units (state-space modeling for global dependencies, FEM for multi-level wavelet/Fourier frequency enhancement, DGM for deformation-aware sparse attention); (2) an online semantic segmentation-guided loss for semantic consistency.

Result: FSCM outperforms existing infrared colorization methods in visual quality and semantic fidelity on complex road scenes.

Conclusion: Leveraging infrared hyperspectral information and frequency-enhanced spatial-spectral modeling significantly improves infrared colorization quality and semantic consistency.
...
TL;DR: A probabilistic deep learning framework for wildfire smoke severity classification (Light, Moderate, Heavy) that provides decomposed epistemic and aleatoric uncertainty in a single forward pass using an EfficientNet-B3 backbone with evidential deep learning head, achieving 93.8% accuracy with well-calibrated uncertainty estimates.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods only perform binary smoke detection and produce point estimates without confidence measures, which is insufficient for critical applications like emergency response and air quality modeling where knowing prediction reliability is essential.

Method: Uses pre-trained EfficientNet-B3 backbone with CBAM attention module, followed by an evidential deep learning head that predicts Dirichlet concentration parameters to directly estimate vacuity (epistemic uncertainty) and dissonance (aleatoric uncertainty) in a single forward pass without Monte Carlo sampling.

Result: 93.8% weighted test accuracy (91.1% unweighted) with ECE=0.0274 on 16,298 satellite patches. Selective prediction on top 50% most certain patches achieves 96.7% accuracy. Uncertainty increases monotonically with image degradation. The Moderate class shows highest epistemic uncertainty (mean vacuity=0.187).

Conclusion: The proposed framework successfully provides accurate smoke severity classification with well-calibrated uncertainty estimates, enabling trustworthy deployment in high-stakes applications. Vacuity serves as a practical scan quality measure, and the model correctly identifies ambiguous boundary regions.
...
TL;DR: The paper proposes a causally grounded framework to unify how image degradations are categorized across different research communities (robustness benchmarks, quality assessment, and physical imaging analysis) by introducing a dual-axis taxonomy (causal source × perceptual effect) and a lightweight severity measurement layer using PSNR, SSIM, and LPIPS, demonstrated via the COCO Degradation benchmark.


<details>
  <summary>Details</summary>
Motivation: Image degradations are studied across different communities (synthetic corruptions, perceptual quality assessment, physical imaging failures) with incompatible grouping schemes and severity definitions, making cross-dataset and cross-task comparisons difficult.

Method: Proposes a dual-axis representation: (1) causal source axis (environment, sensor/optics, ISP/renderer/codec, transfer/system), and (2) perceptual effect axis. Also introduces a severity measurement layer using PSNR, SSIM, and LPIPS to quantify degradation strength across different backends without altering existing implementations.

Result: Demonstrated the framework through COCO Degradation, a taxonomy-aligned benchmark for evaluating object detector robustness under diverse imaging conditions.

Conclusion: The framework provides an interpretive layer that makes implicit assumptions explicit, enabling comparable analysis across datasets, degradation sources, and tasks without introducing new degradations or redefining existing benchmarks.
...
TL;DR: RaPD performs diffusion in a continuous Neural Image Field latent space, enabling resolution-agnostic image generation where a single denoised latent can be rendered at arbitrary resolutions without extra diffusion cost.


<details>
  <summary>Details</summary>
Motivation: Natural images are continuous but most generative models synthesize them on discrete grids, limiting resolution-flexible generation. Prior continuous neural field methods only add continuity at the decoding stage, leaving the latent space discretized and reconstruction-oriented.

Method: RaPD performs diffusion in a continuous Neural Image Field (NIF) latent space, using Semantic Representation Guidance for generation-aware latent learning and a Coordinate-Queried Attention Renderer for coordinate-conditioned, scale-aware rendering.

Result: A single denoised latent can be rendered at arbitrary resolutions by changing only the query coordinates, keeping diffusion cost fixed. Experiments demonstrate superior generation quality and resolution scalability.

Conclusion: The proposed RaPD bridges the reconstruction-generation gap and achieves continuous, resolution-agnostic image generation with fixed diffusion cost.
...
TL;DR: AdaEraser is an adaptive training-free framework for object removal that dynamically modulates self-attention suppression based on estimated object presence, outperforming both training-free and training-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing training-free object removal methods blindly suppress self-attention in object regions, which degrades generation quality because the model must simultaneously reconstruct background content in those areas.

Method: AdaEraser analyzes self-attention map evolution across denoising timesteps before and during removal, then applies token-wise adaptive attention suppression that adjusts suppression strength progressively throughout the denoising process.

Result: Superior performance in object removal, outperforming even training-based methods.

Conclusion: Dynamic, adaptive attention modulation based on estimated object presence resolves the conflict between suppression and background reconstruction, leading to better removal quality.
...
TL;DR: Invaria is a point cloud encoder that achieves scale and density invariance through next-resolution prediction and receptive field calibration, enabling robust performance under resolution shifts with smaller model size and fewer tokens.


<details>
  <summary>Details</summary>
Motivation: Existing 3D point cloud encoders fail to generalize across different sampling resolutions and scales, overfitting to specific quantization densities and object scales, which limits real-world deployment in robotics.

Method: Proposes Invaria, a point cloud encoder that uses next-resolution prediction and receptive field calibration to learn robust structural invariants without explicit high-resolution point cloud generation.

Result: On ScanNet, Invaria achieves 56.0% higher mIoU at 3× lower resolution and 20% improvement when object scale is reduced by 3×, while using 45% smaller model size and 40% fewer input tokens on average.

Conclusion: Next-resolution prediction training objective encourages learning scale- and density-invariant features, achieving high efficiency and robust generalization under resolution shifts without explicit super-resolution generation.
...
TL;DR: The paper proposes a Decomposed Vision-Language Alignment framework that separates textual prompts into concept and attribute tokens, using Feature-Gated Cross-Attention and log-space similarity aggregation to improve compositional generalization in open-vocabulary segmentation.


<details>
  <summary>Details</summary>
Motivation: Open-vocabulary segmentation models fail to generalize to unseen combinations of object categories and attributes because fine-grained descriptions are encoded as holistic sentences that entangle multiple semantic units.

Method: (1) Factorizes textual prompts into a concept token and multiple attribute tokens for separate cross-modal interactions. (2) Introduces Feature-Gated Cross-Attention that generates attribute-specific gating maps for multiplicative feature fusion. (3) Aggregates per-token similarities in log-space for stable compositional matching.

Result: The method significantly improves generalization to unseen attribute-category compositions in fine-grained open-vocabulary segmentation benchmarks, and can be seamlessly integrated into existing transformer-based segmentation architectures.

Conclusion: Decomposing vision-language alignment at both feature and scoring levels enables better compositional generalization without architectural overhauls.
...
TL;DR: The paper proposes GroupRevision, a group-revision optimization paradigm that uses reward shaping to convert sparse, response-level rewards into denser learning signals for finetuning Large Vision-Language Models, achieving consistent gains across object-level grounding benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing GRPO-based methods assign sparse, criterion-induced rewards at the response level, providing minimal learning signals when all candidate responses fail in challenging object-level grounding scenarios.

Method: The method samples an initial response, generates a set of revised candidates, then applies a consolidation process that quantifies each candidate's improvement over the initial attempt using reward shaping. These shaping signals refine the reward and modulate the advantage to amplify high-quality revisions.

Result: Consistent gains across referring segmentation, reasoning segmentation, REC, and counting benchmarks compared to prior GRPO-based models.

Conclusion: Group-revision optimization with reward shaping effectively enhances learning on hard cases for Vision-Language Model grounding tasks.
...
TL;DR: SAE-FT is a robust fine-tuning method that uses Sparse Autoencoders to regularize changes in CLIP's visual representations, preventing catastrophic forgetting while being computationally efficient and interpretable.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning CLIP improves downstream performance but degrades robustness against distribution shifts. Existing solutions rely on computationally expensive text-guidance.

Method: SAE-FT fine-tunes only visual representations, regularizing changes by penalizing addition/removal of semantically meaningful features identified by a Sparse Autoencoder trained on the pre-trained model.

Result: Matches or exceeds state-of-the-art performance on ImageNet and associated distribution shift benchmarks, while being computationally efficient and mechanistically transparent.

Conclusion: SAE-FT effectively balances downstream performance and robustness without text-guidance, offering an interpretable and efficient alternative for robust fine-tuning of vision-language models.
...
TL;DR: Flash-GRPO is a single-step training framework for aligning video diffusion models that outperforms full trajectory training in alignment quality while being computationally efficient, using iso-temporal grouping and temporal gradient rectification to overcome instability issues in existing methods.


<details>
  <summary>Details</summary>
Motivation: Group Relative Policy Optimization (GRPO) for video diffusion models suffers from massive computational costs (hundreds of GPU days for 14B models). Existing efficiency methods like sliding window subsampling cause severe instability and fail to achieve full trajectory performance.

Method: Two key components: (1) iso-temporal grouping - enforces prompt-wise temporal consistency to eliminate timestep-confounded variance, and (2) temporal gradient rectification - neutralizes time-dependent scaling factors that cause inconsistent gradient magnitudes across timesteps.

Result: Validated on 1.3B to 14B parameter models, showing substantial training acceleration with consistent stability and state-of-the-art alignment quality.

Conclusion: Flash-GRPO enables efficient single-step training that outperforms full trajectory training in alignment quality under low computational budgets, solving the computational bottleneck of aligning large video diffusion models.
...
TL;DR: This paper proposes a unified autoregressive framework integrating language-guided visual reasoning for CT interpretation, using task-routing tokens and a "closer-look" coarse-to-fine mechanism to jointly perform detection, segmentation, and textual reasoning, achieving SoTA results on BTCV and MosMed+ benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning advances in CT image analysis are limited to image-level pattern recognition without explicit anatomical/contextual reasoning. Large vision-language models focus on single tasks, insufficient for clinical workflows requiring multiple fine-grained analyses like anatomy detection and segmentation simultaneously.

Method: A unified autoregressive framework where (1) task-routing tokens trigger detection and segmentation heads conditioned on LVLM hidden states, enabling coherent visual and textual outputs; (2) a "closer-look" mechanism performs progressive coarse-to-fine region-of-interest refinement; (3) a new multimodal CT dataset with pixel-wise masks, bounding boxes, spatial prompts, and structured descriptions is curated via AI-assisted annotation with human verification.

Result: Consistent improvements over state-of-the-art: up to 1.0% Dice on BTCV and 1.7% Dice on MosMed+, while additionally providing appearance reasoning outputs.

Conclusion: The proposed language-guided autoregressive framework effectively unifies multiple CT analysis tasks (detection, segmentation, reasoning) in a single model, outperforming prior task-specific methods and adding interpretable textual reasoning capabilities.
...
TL;DR: Echo-Forcing is a training-free memory framework for interactive long video generation that decouples historical KV states into stable anchors, compressed history, and recent windows to handle prompt switching, scene forgetting, and long-range recall under bounded cache budgets.


<details>
  <summary>Details</summary>
Motivation: Existing training-free long-video optimization methods fail in interactive scenarios involving prompt switching, old scene forgetting, and historical scene recall because historical KV states functionally entangle stable anchors with recent dynamics, causing background contamination, delayed new prompt response, and long-range memory loss.

Method: Echo-Forcing proposes three core mechanisms: (1) Hierarchical Temporal Memory - decouples stable anchors, compressed history, and recent windows under relative RoPE; (2) Scene Recall Frames - compresses historical scenes into spatially structured KV representations; (3) Difference-aware Memory Decay - adaptively forgets conflicting tokens based on old-new scene discrepancies.

Result: Extensive evaluations on VBench-Long show Echo-Forcing achieves the best overall performance in both long-video generation and interactive video generation settings.

Conclusion: The proposed memory framework uniformly supports smooth transitions, hard cuts, and long-range scene recall under a bounded cache budget, addressing the core bottleneck of entangled KV states in autoregressive video diffusion models.
...
TL;DR: We present an end-to-end SAM-based workflow for automatic plaque assay analysis that segments wells and plaques, computes PFU/mL, achieves strong correlation with manual annotations (Pearson 0.88-0.92), and integrates into a web platform.


<details>
  <summary>Details</summary>
Motivation: Plaque assays are the gold standard for virus infectivity measurement, but manual plaque counting is labor-intensive, time-consuming, and suffers from inter-operator variability.

Method: Combines two models derived from SAM: a SAM2-based well-segmentation module for localizing wells and a SAM-based plaque-segmentation model for detecting/enumerating plaques; integrated into a web-based platform with automated PFU/mL computation.

Result: Evaluated on mixed datasets (Mayaro virus, Coxsackievirus B3, Vaccinia virus). Pearson correlation: 0.92 (MAYV/CVB3) and 0.88 (VACV) against manual annotations. High concordance with four independent expert annotators.

Conclusion: The proposed system enables reproducible, scalable, and audit-ready plaque assay analysis while substantially reducing manual annotation effort; will be open-sourced upon acceptance.
...
TL;DR: A unified framework using MLLM-guided 4D Gaussian Splatting achieves physics-aware reconstruction and simulation of endoscopic scenes for robot-assisted surgery.


<details>
  <summary>Details</summary>
Motivation: Existing surgical scene reconstruction methods focus only on visual reconstruction and lack physics-based descriptions needed for realistic simulation.

Method: Combines 4D Gaussian Splatting with segmentation/depth estimation, introduces an object-wise material field initialized via MLLM and refined through differentiable Material Point Method (MPM) under joint supervision from rendered images and optical flow.
...
Conclusion: The framework shows strong potential to advance robot-assisted surgical applications by enabling physics-aware reconstruction and simulation.
...
TL;DR: The paper proposes a framework that uses SAM-HQ to generate better 2D masks and a prior-guided label reassignment method for consistent 3D segmentation, enabling real-time, interactive object editing in 3D-GS scenes with state-of-the-art accuracy.


<details>
  <summary>Details</summary>
Motivation: 3D-GS enables real-time 3D reconstruction but lacks robust segmentation for editing tasks; existing 2D-to-3D lifting approaches suffer from view inconsistencies and coarse masks.

Method: Leverages SAM-HQ for accurate 2D masks, then applies a prior-guided label reassignment method to assign labels to 3D Gaussians with multiview consistency enforcement.

Result: Achieves state-of-the-art segmentation accuracy, superior boundary preservation, and enables interactive real-time object editing with high visual fidelity. Demonstrated utility in VR and robotics.

Conclusion: The proposed approach advances 3D scene editing by providing robust, consistent segmentation and interactive editing capabilities for 3D-GS reconstructed scenes.
...
TL;DR: AgriMind ensemble (ResNet50, EfficientNet-B0, DenseNet121) achieves 99.23% accuracy on plant disease classification using PlantVillage dataset (20,638 images, 15 classes), outperforming individual models (96-97%) on an NVIDIA T4 GPU at 53 FPS.


<details>
  <summary>Details</summary>
Motivation: Plant disease detection in Bangladesh remains manual and inefficient across millions of smallholder farms, motivating the need for an automated, lightweight deep learning solution.

Method: Ensemble of ResNet50, EfficientNet-B0, and DenseNet121 with transfer learning (frozen ImageNet backbones, 10 epochs head-only training) using softmax output averaging; trained on 20,638 PlantVillage images covering 15 disease classes (pepper, potato, tomato).

Result: Individual models achieve 96-97% accuracy; ensemble reaches 99.23% (two-thirds error reduction). Pepper and potato achieve perfect classification; tomato (10 visually similar classes) reaches 99.01%. Full ensemble runs at 53 FPS on NVIDIA T4 GPU.

Conclusion: Softmax averaging effectively boosts accuracy over individual models; dropping any member or biasing weights reduces performance. Real-time mobile deployment requires pending TensorFlow Lite optimization.
...
TL;DR: VideoSeeker is a new paradigm for instance-level video understanding that uses visual prompts (instead of text-only) and agentic reasoning to proactively perceive and retrieve relevant video segments, achieving +13.7% average improvement over baselines and outperforming GPT-4o and Gemini-2.5-Pro.


<details>
  <summary>Details</summary>
Motivation: Existing LVLMs struggle with precise spatiotemporal localization at the instance level because text prompts cannot provide precise spatial/temporal references, and current approaches decouple visual perception from language reasoning, limiting proactive fine-grained visual evidence perception.

Method: VideoSeeker integrates agentic reasoning with instance-level video understanding via visual prompts, using a four-stage fully automated data synthesis pipeline for large-scale high-quality data, and internalizes tool-calling and proactive perception capabilities through cold-start supervision and RL training.

Result: Achieves +13.7% average improvement over baselines on instance-level video understanding tasks, surpassing GPT-4o and Gemini-2.5-Pro, with effective transferability on general video understanding benchmarks.

Conclusion: Visual prompts combined with agentic reasoning significantly advance instance-level video understanding, and the proposed data synthesis pipeline enables scalable training; datasets and code will be released.
...
TL;DR: ReAlign is a lightweight AIGI detection framework that distills GRPO-optimized LLM-generated reasoning texts into a small detector via contrastive learning, achieving superior accuracy and generalization against modern AI-generated image forgeries.


<details>
  <summary>Details</summary>
Motivation: Existing non-LLM detectors lack semantic understanding, while LLM-based methods are computationally heavy and have unclear value from explanatory reasoning texts. The paper investigates the intrinsic value of LLM-generated reasoning texts for improving forgery detection.

Method: ReAlign uses GRPO-optimized LLM to generate high-quality reasoning texts, then distills them into a lightweight AIGI detector via contrastive learning, with a joint optimization strategy combining contrastive loss (image-text alignment) and classification loss (forgery discrimination).

Result: ReAlign consistently outperforms existing state-of-the-art detectors on AIGCDetectBenchmark, AIGI-Holmes, and UltraSynth-10k in both accuracy and generalization, especially against complex high-fidelity forgeries.

Conclusion: LLM-generated reasoning texts have intrinsic value for improving generalization and semantic-error sensitivity. ReAlign effectively inherits these capabilities while remaining lightweight and efficient for deployment.
...
TL;DR: GenShield is a unified autoregressive framework for both detecting and correcting artifacts in AI-generated images, using a closed-loop "diagnose-then-repair" process with Visual Chain-of-Thought curriculum learning.


<details>
  <summary>Details</summary>
Motivation: AI-generated images are becoming highly photorealistic, creating urgent needs for authenticity verification. While AIGI detection has advanced, correcting detected artifacts remains underexplored, and no prior work has linked detection with artifact correction.

Method: A unified autoregressive framework that jointly performs explainable detection and controllable artifact correction. It uses Visual Chain-of-Thought based curriculum learning for a self-explained, multi-step "diagnose-then-repair" process with an explicit stopping criterion. A large-scale "artifact-restored" paired dataset is also constructed.

Result: State-of-the-art performance on both correction benchmarks and mainstream AIGI detection benchmarks, with strong generalization capability.

Conclusion: The proposed framework successfully establishes a mutually reinforcing relationship between AIGI detection and artifact correction, achieving superior results on both tasks.
...
TL;DR: A VLM-assisted framework using CLIP to dynamically adjust multi-modal fusion (camera/LiDAR) under adverse conditions, achieving superior occupancy prediction on nuScenes.


<details>
  <summary>Details</summary>
Motivation: Existing static fusion strategies fail to handle modality trust issues when camera degrades in low-light or LiDAR suffers backscatter noise in heavy rain, limiting robustness in variable environments.

Method: Leverages pre-trained CLIP with a parameter-efficient adapter to align weather-specific text embeddings with sensor features, plus a gating strategy decomposing environmental uncertainty into visibility and illumination factors for dynamic fusion ratio modulation.

Result: On nuScenes, OccMamba + framework achieves 26.3 mIoU; M-CONet + framework achieves 21.1 mIoU, significantly surpassing traditional baselines.

Conclusion: VLM-guided dynamic fusion effectively resolves modality trust issues under adverse weather, substantially improving multi-modal 3D occupancy prediction.
...
TL;DR: STABLE is a semantics-physics dual-system for generating simulation-ready tabletop scenes from task instructions, using a fine-tuned LLM (Semantic Reasoner) for coarse layout generation and a physics-aware flow-based denoising model (Physics Corrector) for physically plausible refinement, achieving superior scene validity over prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing task-to-scene generation methods rely solely on LLMs for layout prediction, which suffer from object collisions or floating due to LLMs' inherent limitations in 3D spatial reasoning.

Method: STABLE employs a dual-system with a Semantic Reasoner (fine-tuned LLM for coarse layout generation) and a Physics Corrector (physics-aware flow-based denoising model for pose updates), using a progressive generation paradigm that alternates between the two modules to expand scenes from task-critical to background objects.
...
Conclusion: The semantics-physics dual-system approach effectively addresses the 3D spatial reasoning limitations of LLMs for tabletop scene generation, producing physically valid scenes that maintain semantic alignment with task instructions.
...
TL;DR: This paper investigates whether register tokens, known to mitigate patch-token outliers in Vision Transformers (ViTs), are also useful for Diffusion Transformers (DiTs). The authors find that DiTs do not exhibit patch-token outliers like ViTs, yet register tokens still improve convergence and generation quality in pixel-space DiTs. They analyze intermediate representations to explain this effectiveness and propose a parameter-efficient dual-stream architecture that improves pixel-space generation without significant runtime overhead.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers suffer from high-norm patch-token outliers that degrade feature map quality, which register tokens effectively address. As diffusion models adopt transformer architectures and move toward pixel-space training, the authors question whether register tokens are beneficial for Diffusion Transformers (DiTs).

Method: The authors empirically show DiTs differ from ViTs by not exhibiting patch-token outliers. They investigate register tokens' impact on convergence and generation quality in pixel-space DiTs, analyze intermediate representations at different noise levels, and design a parameter-efficient dual-stream architecture that specializes register token processing.

Result: Register tokens significantly improve convergence and generation quality of pixel-space DiTs despite the absence of patch-token outliers. They produce cleaner feature maps at high noise levels, and recent pixel-space DiT architectures implicitly incorporate register-like mechanisms. The proposed dual-stream architecture improves generation quality with negligible runtime overhead.

Conclusion: Register tokens are beneficial for pixel-space DiTs through a mechanism different from ViTs (mitigating outliers), likely by providing cleaner feature maps at high noise levels. The findings also motivate a practical dual-stream architecture that enhances generation efficiency.
...
TL;DR: Proposes ML-FOP-SOAP, a second-order optimizer with multi-level variance correction, to resolve modality competition in autoregressive multimodal training, achieving 1.4× sample efficiency and 1.5× wall-clock speedup over AdamW at batch size 8192.


<details>
  <summary>Details</summary>
Motivation: Autoregressive next-token training for multimodal models suffers from modality competition due to cross-modality gradient heterogeneity, which destabilizes optimization and limits large-batch scaling with first-order optimizers like AdamW.

Method: Proposes ML-FOP-SOAP: a second-order optimization framework with Fisher-Orthogonal Projection to suppress variance-induced modality conflicts, plus a hierarchical folding strategy for efficient variance estimation under large gradient accumulation.

Result: Experiments on Janus and Emu3 show consistent multimodal improvements, stable training at batch size 8192, 1.4× sample efficiency gain, and 1.5× wall-clock speedup over AdamW.

Conclusion: Second-order preconditioning (specifically SOAP-based) with variance correction effectively mitigates modality competition, offering a practical and robust optimizer for scaling multimodal foundation models.
...
TL;DR: Res²CLIP introduces a residual-to-residual alignment framework for few-shot generalist anomaly detection, shifting multimodal alignment into a unified residual space to solve cross-granularity mismatch and cross-category generalization degradation.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based anomaly detection methods suffer from coarse-grained text prompts causing cross-granularity mismatch, and fine-tuning on auxiliary data disrupting CLIP's open-world generalization via domain shift.

Method: Proposes a residual-to-residual alignment framework with three branches (text prompt, visual prompt, and novel residual alignment branch), constraining all learnable optimizations within the residual domain and designing objectives to focus on relative anomaly deviations.

Result: Experiments on multiple datasets demonstrate the effectiveness of the proposed architecture.

Conclusion: Shifting multimodal alignment into a unified residual space simultaneously resolves cross-granularity mismatch and cross-category generalization degradation for few-shot generalist anomaly detection.
...
TL;DR: MAgSeg is a decoder-free MLLM approach that segments complex smallholder agricultural landscapes from high-resolution satellite imagery, outperforming state-of-the-art MLLM baselines across three Global South countries.


<details>
  <summary>Details</summary>
Motivation: Agricultural landscape segmentation in the Global South is difficult due to fragmented plots, high intra-class variance, and limited labeled data. Existing MLLM approaches face context length bottlenecks and domain alignment gaps when understanding satellite features.

Method: A novel, decoder-free MLLM segmentation approach that uses an instruction tuning data format enabling scalable fine-tuning on high-resolution satellite imagery. It learns from the global image context while generating text tokens for only a patch within the image.

Result: Extensive evaluations on datasets from three Global South countries show MAgSeg significantly outperforms state-of-the-art MLLM baselines.

Conclusion: MAgSeg offers a scalable solution to map smallholder agricultural environments, addressing key limitations of context length and domain alignment in existing MLLM approaches.
...
TL;DR: VLA-AD is a distillation framework that uses a Vision-Language Model as an offline semantic supervisor to compress large VLA teachers (e.g., OpenVLA-7B) into lightweight student policies (158M params), achieving 44× size reduction, 3.28× speedup, and matching teacher performance within 0.27% gap.


<details>
  <summary>Details</summary>
Motivation: Billion-parameter VLA policies are too large and slow for real-time closed-loop robotic control, motivating the need for efficient distillation that preserves performance.

Method: VLA-AD augments low-level action imitation with high-level semantic guidance from a VLM (task phase anchors and multi-frame operating-direction descriptions) during training only. At test time, only the lightweight student policy runs without the teacher or VLM.

Result: On LIBERO benchmarks, VLA-AD reduces model size from 7B to 158M (44× reduction), achieves 12.5 Hz inference (3.28× speedup) on RTX 4090, and matches teacher with 0.27% average gap. The method also generalizes to a π₀.₅-4B teacher and improves robustness to noisy teacher actions.

Conclusion: Offline semantic guidance from VLMs substantially improves the efficiency, robustness, and deployability of VLA policy distillation, enabling practical real-time robot control.
...
TL;DR: IVGT is an Implicit Visual Geometry Transformer that reconstructs continuous 3D geometry and appearance from unposed multi-view images using a neural scene representation with signed distance functions, enabling various downstream tasks like mesh reconstruction, novel view synthesis, and pose estimation.


<details>
  <summary>Details</summary>
Motivation: Existing visual geometry foundation models predict explicit geometry (e.g., pixel-aligned pointmaps) from multi-view images, which suffers from redundancy and limited geometric continuity. The goal is to achieve continuous and coherent 3D reconstruction without requiring camera poses.

Method: IVGT learns a continuous neural scene representation in a canonical coordinate system. It supports spatial queries at any 3D position, retrieves local features, and uses lightweight decoders to predict signed distance (SDF) values and colors. The model is trained via multi-dataset joint optimization with 2D supervision and 3D geometric regularization.

Result: IVGT demonstrates strong generalization across scenes and achieves competitive performance on mesh/point cloud reconstruction, novel view synthesis, depth/surface normal estimation, and camera pose estimation.

Conclusion: The implicit formulation effectively addresses limitations of explicit geometry prediction, enabling continuous and coherent surface reconstruction from pose-free multi-view images with versatile downstream applicability.
