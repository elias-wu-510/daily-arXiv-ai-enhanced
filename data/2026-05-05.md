...
TL;DR: SDRS applies Design of Experiments to synthetic data pipelines, using fractional factorial designs to audit model failures (coverage vs. spurious dependencies) and prescribe targeted synthetic data, improving accuracy from 49.9% to 79.0% on dSprites and mIoU from 0.948 to 0.998 on segmentation.


<details>
  <summary>Details</summary>
Motivation: Current synthetic data pipelines randomly sample generator outputs without diagnosing what the downstream model actually needs, treating synthetic data as cheap real data and missing the opportunity to leverage controllable, independent variation of scene factors.

Method: SDRS uses fractional factorial designs from Design of Experiments to audit a model's factor-sensitivity via ANOVA decomposition, classifying failures into Type I (coverage gaps) and Type II (spurious nuisance dependencies), then prescribes targeted synthetic data accordingly.

Result: Three experiments: (1) dSprites accuracy 49.9%→79.0%; (2) segmentation mIoU 0.948→0.998; (3) ANOVA detects cross-factor contamination in imperfect generators. Per-factor invariance penalties can transfer sensitivity between factors.

Conclusion: SDRS provides a principled framework for diagnosing and correcting model failures using synthetic data's unique controllability, while revealing an open problem for representation-level correction via invariance penalties.
...
TL;DR: The paper proposes a latent space probing framework for real-time adult content detection in AI-generated videos by intercepting internal representations of the CogVideoX diffusion model, achieving 97.29% F1 with 4-6ms overhead.


<details>
  <summary>Details</summary>
Motivation: Existing detection methods only analyze prompts or decoded pixel outputs, missing the rich internal representations formed during video generation, which limits detection performance and efficiency.

Method: The authors intercept denoised latent representations from the CogVideoX video diffusion model during inference and attach lightweight binary classifiers. They built a dataset of 11,039 ten-second clips (5,086 violating, 5,953 non-violating) and introduced two probing classifier architectures.

Result: The approach achieves 97.29% F1 on a held-out test set with only 4-6ms of added latency, demonstrating that latent-space signals encode strong discriminative features for harmful content detection.

Conclusion: Latent space probing provides improvements in both detection performance and cost compared to existing methods that rely on prompts or pixel-space outputs.
...
TL;DR: This paper systematically compares visual representations and neural architectures for cryptocurrency regime prediction from candlestick charts, finding that a simple 4-layer CNN on raw price-only charts at 128x128 resolution (0.892 AUC-ROC) outperforms complex alternatives, with transfer learning boosting performance by 4-16%.


<details>
  <summary>Details</summary>
Motivation: While deep learning excels in image classification and technical traders rely on candlestick chart patterns, the application of neural networks to financial chart images for regime prediction remains largely unexplored.

Method: Eight controlled experiments comparing three image encoding methods (raw charts, GAF, multi-channel GAF), five chart configurations, four architectures (CNN, ResNet18, EfficientNet-B0, ViT), and transfer learning effects on Bitcoin, Ethereum, and S&P 500 data (2018-2024).

Result: Simple 4-layer CNN on raw candlestick charts achieves 0.892 AUC-ROC, simpler representations outperform complex ones, and transfer learning yields 4-16% improvement despite domain gap.

Conclusion: Simplicity is key for visual regime classification in finance; complex representations and larger architectures do not guarantee better performance.
...
TL;DR: This paper proposes a hybrid image defogging model combining a fourth-order nonlinear PDE with a physical haze formation model, using Dark Channel Prior for parameter estimation and demonstrating competitive performance against existing methods.


<details>
  <summary>Details</summary>
Motivation: Image defogging is challenging in real-world scenarios due to unknown scene depth, atmospheric scattering, and the common lack of ground truth data.

Method: The method integrates a fourth-order nonlinear PDE (telegraph type) with a physical haze formation model, uses Dark Channel Prior to estimate atmospheric parameters and generate guidance images, and incorporates edge-adaptive diffusion coefficients with a transmission map-weighted fidelity term. Convergence is controlled via relative error norm criteria.

Result: The proposed method achieves comparable visual quality and better structural detail preservation compared to Dark Channel Prior, modified Dark Channel Prior, and variational-based techniques. Quantitative evaluation uses MSE/SSIM (when ground truth available) and no-reference metrics like FADE, Contrast Restoration Index, Average Gradient, and Entropy.

Conclusion: The hybrid PDE-based defogging model effectively suppresses haze while preserving structural details, with improved numerical stability and convergence behavior due to the hyperbolic fourth-order formulation.
...
TL;DR: This paper proposes Adversarial Flow Matching (AFM), a gray-box attack framework that exploits Transformer structural vulnerabilities in end-to-end autonomous driving models to generate effective, imperceptible, and transferable adversarial examples in a single step.


<details>
  <summary>Details</summary>
Motivation: Both monolithic VLA and modular end-to-end AD models rely on Transformer backbones, creating a shared vulnerability to visually imperceptible perturbations that can cause hazardous maneuvers. Existing white-box attacks require full model transparency, while black-box approaches suffer from high query latency or limited transferability.

Method: AFM is a gray-box attack framework that uses a neural average velocity field to generate adversarial examples in a single step. It perturbs both the generative latent space and the neural average velocity field synergistically to achieve visually imperceptible attacks.

Result: AFM achieves superior trade-off between attack effectiveness and imperceptibility, degrading performance of both VLA and modular AD agents across various scenarios while maintaining state-of-the-art visual imperceptibility. It also exhibits robust cross-model transferability, approximating a black-box setting.

Conclusion: AFM effectively exploits Transformer vulnerabilities in E2E AD models, offering a practical gray-box attack that requires only the prior knowledge of a Transformer-based module in the target model.
...
TL;DR: The paper proposes Physiological Causal Probing (PCP), a new SSL paradigm for rPPG that uses active intervention on video chrominance to verify rPPG signal hypotheses, avoiding the correlation trap that plagues existing methods. The Interv-rPPG framework implements this with PhysMambaFormer (extractor) and a Controllable Physiological Signal Editor, achieving strong performance particularly in cross-domain settings.


<details>
  <summary>Details</summary>
Motivation: Existing SSL methods for rPPG fall into a "correlation trap" where they learn dominant periodic signals (motion/illumination noise) instead of the faint true rPPG signal, resulting in poor model generalization.

Method: PCP paradigm shifts from passive correlation learning to active intervention. It proposes an rPPG hypothesis, then intervenes on the video via a Controllable Physiological Signal Editor that edits low-frequency chrominance components. The framework validates the hypothesis through 'Falsifiability via Nulling' and 'Axiomatic Equivariance'. Uses PhysMambaFormer as the rPPG extractor.

Result: Improves both in-domain and cross-domain performance on VIPL-HR and MMPD. Surpasses supervised baseline in complex cross-dataset settings. Competitive on clean datasets despite possible residual chrominance noise from intervention.

Conclusion: The PCP paradigm effectively resists motion and illumination artifacts, demonstrating that causal intervention outperforms passive correlation learning for robust rPPG signal extraction.
...
TL;DR: This paper presents a comprehensive survey and benchmark for face swapping, organizing existing methods into five paradigms, introducing the CASIA FaceSwapping benchmark for standardized evaluation, and offering new insights through extensive experiments.


<details>
  <summary>Details</summary>
Motivation: Existing face swapping methods are fragmented across different paradigms (GANs, diffusion models, etc.) with highly inconsistent evaluation due to lack of standardized datasets and protocols. Prior surveys focus on broader deepfake generation/detection, leaving face swapping insufficiently studied as a standalone problem.

Method: The authors (1) provide a structured review organizing methods into five major paradigms with analysis of design principles, strengths, and limitations; (2) introduce CASIA FaceSwapping, a high-quality benchmark with balanced demographics and attribute variations; (3) establish standardized evaluation protocols; (4) conduct extensive experiments on representative approaches.

Result: A unified perspective and principled evaluation framework for face swapping, along with new insights into performance characteristics and limitations of current techniques through standardized benchmarking.

Conclusion: This work provides a unified perspective and evaluation framework to facilitate development of more robust and controllable face swapping methods, addressing the fragmentation and inconsistency issues in the field.
...
TL;DR: LiteVLA-H is a compact 256M-parameter VLA system for drones that achieves dual-rate operation on Jetson AGX Orin: fast reactive actions at 19.74 Hz and slower semantic outputs at 6-7 Hz, with latency dominated by multimodal pre-fill rather than token decoding.


<details>
  <summary>Details</summary>
Motivation: VLA models struggle in aerial deployment due to drones' need for low-latency closed-loop guidance under strict onboard compute and communication constraints.

Method: A 256M-parameter VLA system with dual-rate scheduler (fast action mode, slower semantic mode), knowledge-preserving fine-tuning on mixed reactive flight, aerial semantic, and generic caption/VQA data.

Result: Action tokens at 50.65ms (19.74Hz), semantic outputs at 149.90-164.57ms (6.08-6.67Hz), outperforming AnywhereVLA, FutureVLA, and ReMem-VLA in edge inference rate.

Conclusion: Compact VLAs can achieve practical dual-rate aerial deployment on embedded hardware by addressing multimodal pre-fill bottleneck and preserving descriptive competence via knowledge-preserving fine-tuning.
...
TL;DR: CPIFNet decomposes non-homogeneous dehazing into homogeneous sub-problems using a two-stage multi-branch network with concentration partitioning and image fusion.


<details>
  <summary>Details</summary>
Motivation: Existing dehazing methods work well on homogeneous thin-haze but fail on non-homogeneous hazy images with spatially varying haze and abrupt density transitions.

Method: A two-stage multi-branch framework: (1) IENet stage with multiple branches trained on different homogeneous haze levels, (2) IFNet stage that fuses advantageous regions via deep feature stacking and merging, supervised by a comprehensive loss function.

Result: The approach decomposes a difficult non-homogeneous problem into tractable homogeneous sub-problems, with intelligent fusion producing a unified high-quality result.

Conclusion: CPIFNet effectively addresses non-homogeneous dehazing by partitioning the problem into homogeneous sub-problems and fusing region-wise optimal enhancements.
...
TL;DR: SANet is a U-Net-based infrared small target detection network that uses a Dual-path Semantic-aware Module (DSM) for better feature extraction and a Selective Attention Fusion Module (SAFM) for adaptive cross-scale skip connections, addressing information bottleneck and static fusion limitations in existing methods.


<details>
  <summary>Details</summary>
Motivation: Infrared small target detection suffers from tiny target sizes (few pixels), low signal-to-clutter ratios, and background confusion. Existing encoder-decoder architectures face two key limitations: an information bottleneck in early convolutional layers that loses fine-grained target details, and static skip connections that cannot dynamically adapt to distinguish real targets from false alarms.

Method: SANet builds upon U-Net with two novel modules: (1) Dual-path Semantic-aware Module (DSM) combines standard convolutions (local detail preservation) with pinwheel-shaped convolutions (direction-sensitive receptive fields), followed by CBAM for spatial-channel recalibration; (2) Selective Attention Fusion Module (SAFM) replaces static skip connections with a learnable, spatially adaptive weighting mechanism for context-aware cross-scale feature fusion.

Result: The paper proposes SANet but does not report specific experimental results in the abstract.

Conclusion: SANet addresses fundamental IRSTD challenges by enhancing fine-grained target perception through dual-path semantic extraction and enabling dynamic, context-aware cross-scale feature fusion via selective attention, potentially improving detection of dim small targets amid cluttered backgrounds.
...
TL;DR: SparseContrast merges dynamic sparse attention with contrastive learning for efficient chest X-ray disease detection, achieving up to 40% speedup with comparable or better accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional contrastive learning uses dense attention that is computationally expensive and processes redundant regions in medical images, hindering deployment in low-resource settings.

Method: Introduces a sparse attention mechanism guided by a compact saliency predictor that adaptively trims attention maps, optimizing sparsity and feature quality jointly during training.

Result: Achieves up to 40% faster training/inference relative to dense attention benchmarks, with comparable or better diagnostic accuracy, and is backbone-agnostic (CNNs & transformers).

Conclusion: SparseContrast offers a practical, efficient contrastive learning framework for medical imaging in resource-limited settings without sacrificing diagnostic performance.
...
TL;DR: The paper proposes Selective Correlation Based Knowledge Distillation (SCKD) to estimate ground reaction force (GRF) from noisy wearable insole sensor data, achieving accurate and resource-efficient human gait analysis by transferring knowledge from a teacher model to a compact student model using temporally-aware feature correlations.


<details>
  <summary>Details</summary>
Motivation: Traditional GRF measurement requires expensive lab equipment (force plates), while portable insole sensors suffer from noise and deep learning models demand high computing resources. The goal is to enable accurate, real-time GRF estimation on portable devices.

Method: SCKD uses selective features considering temporal characteristics to extract correlation maps for knowledge transfer from a teacher to a compact student model, enhancing interpretability and handling high-dimensional data issues.

Result: SCKD outperforms existing methods in estimating GRF from wearable insole sensor data across different walking speeds and window sizes, based on multiple evaluation criteria.

Conclusion: The proposed approach provides a reliable and resource-efficient solution for human gait analysis, suitable for real-time portable applications.
...
TL;DR: The paper investigates linear-min-max neural networks, which at initialization behave like k-medoids with infinity norm distance, are trained via subgradient descent, are universal approximators, and use a single most activated neuron for output decisions. A pixel fragility measure is introduced to explain classification changes from single-pixel modifications, outperforming SHAP and Integrated Gradient on PneumoniaMnist.


<details>
  <summary>Details</summary>
Motivation: To enhance explainability in neural networks by leveraging the unique architecture of linear-min-max networks, where a single activated neuron determines the output, enabling traceable decision processes.

Method: Linear-min-max neural networks initialized as k-medoids with infinity norm, trained via subgradient descent. A pixel fragility measure is designed to detect single-pixel changes that could alter classification.

Result: The proposed pixel fragility explanation method outperforms SHAP and Integrated Gradient on the PneumoniaMnist dataset.

Conclusion: Linear-min-max networks offer a promising trade-off between universal approximation and interpretability, with the pixel fragility measure providing competitive explanations for model decisions.
...
TL;DR: This study evaluates ML models (Geometric, XGBoost, SVM, deep learning) for classifying walker usage, sitting/standing, and posture detection in smart walkers. XGBoost achieved near-perfect binary classification (99.84% walker choice, 99.69% standing/sitting), Geometric approach reached 89.9% for 8-posture classification, and deep learning models exceeded 98% accuracy in binary tasks.


<details>
  <summary>Details</summary>
Motivation: Falls among older adults are a major public health issue causing severe injuries, loss of independence, and high healthcare costs. The study aims to enhance fall prevention through improved human-robot interaction in smart walkers.

Method: Evaluated multiple models: Geometric approach, XGBoost, SVM, 4-layer CNN, and Encoder-Decoder CNN for three classification tasks: walker usage, standing vs. sitting, and posture classification.

Result: XGBoost achieved 99.84% (walker choice) and 99.69% (standing/sitting) in binary classification. Geometric approach got 89.9% for 8-posture classification. XGBoost obtained 99.24% during training for 17-posture classification. Deep learning models achieved >98% accuracy in binary tasks.

Conclusion: Machine learning, particularly XGBoost and Geometric approaches, shows strong potential for enhancing human-robot interaction in smart walkers and contributing to fall prevention in older adults.
...
TL;DR: X2SAM is a unified multimodal large language model that extends segmentation capabilities from images to videos, supporting both textual and visual prompts, with a Mask Memory module for temporally consistent mask generation across multiple segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs lack pixel-level perception across images and videos, SAM series cannot interpret complex conversational instructions, and existing segmentation MLLMs are usually specialized for either images or videos and rarely support both textual and visual prompts in one interface.

Method: X2SAM couples an LLM with a Mask Memory module that stores guided vision features for temporally consistent video mask generation, using a unified joint training strategy over heterogeneous image and video datasets.

Result: X2SAM delivers strong video segmentation performance, remains competitive on image segmentation benchmarks, and preserves general image and video chat ability.

Conclusion: The unified formulation supports generic, open-vocabulary, referring, reasoning, grounded conversation generation, interactive, and visual grounded segmentation across both image and video inputs.
...
TL;DR: This paper systematically compares model-side personalization and data-side harmonization strategies in federated learning across six medical imaging tasks, finding that harmonization works better for appearance-based domain shifts while personalization excels for structural differences, with similar performance when heterogeneity is limited.


<details>
  <summary>Details</summary>
Motivation: Federated learning in medical imaging suffers from domain heterogeneity across clients, but existing solutions (personalization vs. harmonization) have never been systematically compared to guide practitioners in choosing the right strategy.

Method: Comprehensive evaluation of state-of-the-art harmonization and personalization methods under a unified framework across six diverse medical imaging settings (segmentation and classification) covering different types of domain shift.

Result: Harmonization outperforms for appearance-based variation (e.g., CXR classification); personalization outperforms for structural differences (e.g., colon polyp segmentation); both perform similarly when inter-client variation is limited.

Conclusion: The effectiveness of federated medical imaging adaptation depends on the type and magnitude of domain shift, not just the strategy; practical guidelines for selection and future hybrid approaches are proposed.
...
TL;DR: Retrieval-guided generation (RGG) for medical image captioning, which summarizes expert text from similar cases, outperforms generative models like MedGemma in semantic alignment (cosine similarity ~0.60 vs ~0.47) and reduces hallucinations, though it has failure modes like concept mixing.


<details>
  <summary>Details</summary>
Motivation: Generative vision-language models for medical image captioning suffer from hallucination, over-specific diagnoses, and factual inconsistency, which are dangerous in pathology. The authors explore retrieval-guided generation as a safer, more transparent alternative.

Method: Instead of generating captions de novo, RGG retrieves visually similar cases from a database and summarizes their expert-written text to form the caption. Evaluated on the ARCH histopathology dataset with cosine similarity and pathologist-led qualitative review.

Result: RGG achieves cosine similarity ≈0.60 vs MedGemma's ≈0.47 with non-overlapping confidence intervals. Pathologist review confirms better preservation of morphology terminology and fewer unsupported diagnoses. Failure modes include concept mixing and inherited over-specific labeling.

Conclusion: Retrieval-guided captioning provides a more transparent and reliable approach than fully generative methods, with clearer opportunities for auditing, making it a promising direction for safe medical image captioning.
...
TL;DR: Dino-NestedUNet couples a DINOv3 encoder with a Nested Dense Decoder to improve boundary fidelity for infiltrative tumor segmentation in computational pathology, outperforming existing methods across multiple histopathology cohorts.


<details>
  <summary>Details</summary>
Motivation: Current adaptations of Vision Foundation Models (VFMs) for pathology segmentation pair frozen encoders with lightweight decoders, creating a capacity mismatch that limits boundary fidelity for infiltrative tumor bulk segmentation.

Method: The framework uses a pre-trained DINOv3 encoder coupled with a Nested Dense Decoder that forms a dense grid of intermediate pathways with continuous feature reuse and multi-scale recalibration, replacing sparse skip connections and linear upsampling.

Result: Consistent improvements over UNet++ and standard Dino-UNet variants on three histopathology cohorts (CHTN, OSU, CAMELYON16), with superior cross-domain generalization demonstrated via zero-shot evaluation on unseen TIGER WSIBULK and OSU CRC cohorts.

Conclusion: Dense decoding is a key ingredient for unlocking foundation encoders in boundary-sensitive pathology segmentation.
...
TL;DR: This paper challenges the trend of using complex attention-based architectures for InSAR phase unwrapping, showing through a large-scale ablation study that a simple vanilla U-Net outperforms attention-based models by 34% in R² and 51% in RMSE, while being 2.5x faster, because attention injects unphysical high-frequency artifacts violating smoothness constraints of geophysical fields.


<details>
  <summary>Details</summary>
Motivation: Operational phase unwrapping is the primary computational bottleneck in InSAR-based volcanic and seismic monitoring. The authors challenge the industry trend of adopting high-complexity computer vision architectures without validating their suitability for physics-constrained geophysical regression, aiming to bridge the "publication-to-practice" gap.

Method: First large-scale architectural ablation study on a global LiCSAR benchmark (20 frames, 39,724 patches, 651M pixels). Compared vanilla U-Net (7.76M parameters) against attention-based models (11.37M parameters). Used Power Spectral Density (PSD) analysis for physical justification.

Result: Vanilla U-Net achieves R²=0.834 and RMSE=1.01 cm, outperforming attention-based models by 34% in R² and 51% in RMSE. Vanilla U-Net has 2.92ms inference latency (2.5x speedup). Attention models inject unphysical high-frequency artifacts (>0.3 cycles/pixel) into geophysical fields.

Conclusion: Convolutional locality outperforms modern complexity for smooth-field regression. Advocates for physics-informed simplicity in ML4RS. Vanilla U-Net is the only candidate to comfortably meet sub-100ms requirement for operational early-warning systems.
...
TL;DR: LatentDiff is a scalable framework that compares datasets semantically in latent space using sparse autoencoders and density ratio estimation, achieving superior accuracy and robustness even when <1% of images differ, while being computationally cheaper than caption-based methods. Noisy-Diff benchmark is also introduced to test realistic sparse distribution shifts.


<details>
  <summary>Details</summary>
Motivation: Existing dataset comparison methods (e.g., caption-based) are computationally expensive and struggle with realistic sparse distribution shifts where only a tiny fraction of images differ semantically.

Method: Combines sparse autoencoder-based divergence testing with density ratio estimation in the latent space of pretrained vision encoders to identify interpretable semantic differences between datasets.

Result: LatentDiff achieves superior accuracy compared to caption-based alternatives and remains robust even when extremely small fractions (5% to <1%) of images differ semantically, while requiring significantly less computation.

Conclusion: LatentDiff provides a scalable, accurate, and robust solution for semantic dataset comparison, outperforming existing methods particularly in challenging sparse distribution shift scenarios.
...
TL;DR: This paper proposes a conditional MeanFlow pipeline with reinforcement learning-based spatial enhancement control for reconstructing CT images that vary due to different imaging protocols and scanner models. The approach combines flow-based enhancement with adaptive refinement, achieving high accuracy (radiomic feature CCC of 0.96, PSNR up to 34.23, SSIM up to 0.95).


<details>
  <summary>Details</summary>
Motivation: CT images acquired with different protocols/scanner models show large differences in noise, contrast, and texture, which negatively impacts lung cancer screening, diagnosis, and therapy planning. A method is needed to standardize/enhance image quality across heterogeneous acquisitions.

Method: (1) A conditional MeanFlow network that models enhancement trajectory by predicting image-conditioned flow fields given intermediate states. (2) A MeanFlow consistency loss combined with image reconstruction loss for training. (3) A reinforcement learning-driven policy network that provides tile-wise refinement budgets, stopping criteria, and budget allocation based on MeanFlow rollouts. The policy is trained via policy gradient to maximize enhancement improvement while minimizing computation and avoiding instabilities.

Result: Tumor ROI: average radiomic feature CCC of 0.96, average PSNR of 31.30 ± 4.16, average SSIM of 0.94 ± 0.07. Overall image quality: average PSNR of 34.23 ± 1.71, average SSIM of 0.95 ± 0.01.

Conclusion: The proposed approach effectively combines conditional flow-based enhancement with reinforcement learning-based spatial control, enabling focused attention on difficult areas while stabilizing already sufficient-quality regions, leading to both accurate and efficient CT image reconstruction.
...
TL;DR: This paper benchmarks ten whole-slide image retrieval pipelines on 9,387 TCGA slides across 17 organs and 60 diagnoses, finding that performance varies more by organ/diagnosis than architecture, patch-level features drive results more than aggregation methods, and even the best model achieves only ~68% accuracy with some subtypes at 0%, questioning the clinical readiness of current histopathology foundation models.


<details>
  <summary>Details</summary>
Motivation: To determine whether slide foundation models offer clear value for whole-slide image retrieval compared to simpler patch-based and supervised aggregation baselines, given the unclear advantage of these models in this specific task.

Method: Benchmarked ten pipelines (four slide foundation models, supervised ABMIL aggregator, patch-level retrieval across five sampling densities) on 9,387 diagnostic slides from TCGA (17 organs, 60 diagnoses) using patient-level leave-one-patient-out evaluation.

Result: Performance varied more by organ/diagnosis than architecture; TITAN was modestly best overall but not dominant; patch-level features drove results more than aggregation methods; best model achieved ~68% ± 21% accuracy; some subtypes showed 0% accuracy across all methods.

Conclusion: No universally optimal architecture exists; recommends organ-resolved benchmarking, ensemble/multimodal strategies, stronger feature representations; current morphology-only retrieval has fundamental limitations that prevent reliable clinical deployment.
...
TL;DR: The paper proposes a lightweight Multi-View Convolutional Neural Network (MV-CNN) for plant disease detection that adds extra features beyond standard RGB images. It achieves 2.9% higher accuracy than baseline CNN models while being computationally efficient enough for deployment in resource-constrained rural areas.


<details>
  <summary>Details</summary>
Motivation: Traditional plant disease detection is labor-intensive and time-consuming, while existing deep CNN methods are too computationally heavy to deploy in rural, resource-constrained settings of developing countries where agriculture is crucial.

Method: A Multi-View Convolutional Neural Network (MV-CNN) that incorporates additional features beyond standard RGB images to improve plant disease identification while reducing model complexity and parameter count.

Result: The proposed model achieves 2.9% improvement in classification accuracy over the baseline CNN model on the PlantVillage dataset, and offers comparable accuracy to state-of-the-art deep CNNs with significantly lower computational cost.

Conclusion: The lightweight MV-CNN provides an efficient and accurate solution for plant disease identification suitable for deployment in resource-constrained rural areas of developing countries.
...
TL;DR: This paper evaluates the robustness of a two-stage transformer pipeline for fluence map prediction in IMRT, finding that hierarchical attention models (e.g., SwinUNETR) are more robust to distribution shifts and that physics-informed evaluation is necessary beyond SSIM.


<details>
  <summary>Details</summary>
Motivation: To assess the robustness of learning-based fluence map prediction under realistic distribution shifts, as prior work lacks systematic evaluation of failure modes in clinical deployment.

Method: A two-stage transformer pipeline (anatomy→dose→fluence) comparing hierarchical, global, and hybrid attention backbones trained with a physics-informed energy consistency loss, tested under geometric perturbations, radiometric noise, reduced data, and domain shifts on prostate IMRT and public datasets.

Result: Smooth degradation under moderate perturbations but sharp failures under severe rotations/noise; hierarchical transformers (SwinUNETR) show slower upper-quartile energy error growth; SSIM alone misses clinically relevant errors.

Conclusion: Physics-informed evaluation is critical for assessing clinical reliability, and hierarchical attention architectures offer superior robustness for fluence map prediction.
...
TL;DR: This paper studies Generalized Category Discovery under domain shifts and proposes three frameworks (HiLo, HLPrompt, VLPrompt) adapting foundation models to handle both domain and semantic shifts in unlabeled data.


<details>
  <summary>Details</summary>
Motivation: Existing GCD methods assume single-domain data, but real-world unlabeled data often exhibits domain shifts alongside semantic shifts, which is underexplored.

Method: Three frameworks: (i) HiLo - disentangles domain/semantic features via multi-level extraction, mutual info minimization, PatchMix augmentation, and curriculum sampling. (ii) HLPrompt - extends HiLo with semantic-aware spatial prompt tuning. (iii) VLPrompt - leverages vision-language models with factorized textual prompts and cross-modal consistency.

Result: Consistent improvements over strong baselines on synthetic corruptions and real-world multi-domain shifts.

Conclusion: The three methods share core design principles while operating on different foundation backbones, suitable for different deployment scenarios.
...
TL;DR: TRIP-Evaluate is an open multimodal benchmark (837 items) for evaluating LLMs/MLLMs on transportation tasks, covering text, image, and point-cloud modalities with a role-task-knowledge taxonomy, revealing strengths in text but weaknesses in multi-step calculations, rule-constrained reasoning, and multimodal/point-cloud understanding.


<details>
  <summary>Details</summary>
Motivation: Existing general benchmarks fail to assess whether models can correctly apply regulations, perform verifiable engineering calculations, or reliably interpret traffic scenes; public transportation benchmarks are narrow and lack fine-grained multimodal diagnosis.

Method: The benchmark organizes 837 items via a role-task-knowledge taxonomy across four transportation functions (vehicle, traffic-management, traveler, planning-and-design), annotated with capability, modality, and difficulty labels, covering 596 text, 198 image, and 43 point-cloud items, with standardized construction, quality control, prompting, decoding, and scoring.

Result: Text-based model performance is improving, but significant weaknesses exist in multi-step engineering calculation, rule-constrained reasoning, multimodal scene understanding, and point-cloud understanding.

Conclusion: TRIP-Evaluate provides a reproducible, diagnosable, and engineering-aligned evaluation baseline for model selection, regression testing, and safer deployment in transportation applications.
...
TL;DR: This paper compares CNN-based (YOLOv26-nano) and transformer-based (RTDETR, RF-DETR) object detectors for early weed detection, finding CNNs offer better efficiency while transformers provide superior global context capture at higher computational cost.


<details>
  <summary>Details</summary>
Motivation: To evaluate and compare different object detection paradigms (CNN vs. transformer) specifically for early weed detection in precision agriculture, providing practical selection criteria.

Method: Comparative evaluation of YOLOv26-nano (CNN-based), RTDETR, and RF-DETR (transformer-based) on the GROUNDBASED_WEED dataset, measuring precision, recall, average precision, and inference speed.

Result: CNN-based detectors (YOLOv26-nano) achieve high performance with lower computational cost; transformer-based approaches offer better global context capture but require higher resource demands.

Conclusion: A clear efficiency vs. contextual modeling trade-off exists, providing practical criteria for model selection in precision agriculture.
...
TL;DR: This paper introduces InduOCRBench, a new OCR benchmark designed for industrial RAG systems, covering 11 challenging document types. It reveals that high OCR accuracy (low WER/CER) doesn't guarantee strong downstream RAG performance due to structural and semantic errors, and demonstrates this mismatch is category-dependent and stable across pipeline choices.


<details>
  <summary>Details</summary>
Motivation: Existing OCR benchmarks use character-level metrics that fail to measure downstream RAG effectiveness in real-world industrial settings, motivating the need for a more practical benchmark.

Method: The authors build a benchmark with 11 challenging document types, evaluate SOTA OCR models under a controlled OCR-first RAG pipeline, and analyze the mismatch between OCR accuracy and RAG performance.

Result: SOTA OCR models show clear performance degradation on industrial documents despite strong conventional scores; high OCR accuracy does not translate to strong RAG performance due to structural/semantic errors; the mismatch is category-dependent and stable across pipeline choices.

Conclusion: Character-level OCR metrics are insufficient for evaluating RAG readiness; the community should adopt task-aware benchmarks like InduOCRBench for industrial document understanding.
...
TL;DR: This paper proposes an object-centric analysis pipeline to interpret geolocation models by decomposing attribution maps (like Grad-CAM) into coherent object-level segments, showing that attribution-guided crops retain more predictive information than random crops in a three-country geolocation benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing attribution methods (e.g., Grad-CAM) for geolocation models highlight diffuse, incoherent regions rather than specific objects, making it difficult to interpret model predictions at the level of concrete visual cues (road markings, vegetation, architecture) that humans use in games like GeoGuessr.

Method: 1) Extract attribution maps from a geolocation model; 2) Segment salient regions into object-like elements; 3) Evaluate predictive relevance via deletion/insertion tests comparing attribution-guided crops with random crops of similar coverage.

Result: On a three-country benchmark, attribution-guided crops consistently retain more information for the model's predictions than random crops of similar coverage.

Conclusion: Attribution maps can be decomposed into interpretable, perceptible object-level elements, offering a step toward object-level analysis and interpretability of geolocation models.
...
TL;DR: MCSTN proposes a manifold-consistent spatio-temporal network that uses dual-level corruption modeling and dual-stream architecture to achieve robust human activity recognition under imperfect IoMT sensing conditions.


<details>
  <summary>Details</summary>
Motivation: Real-world wearable sensing signals are often corrupted by missing measurements, sensor failures, and noise, which degrade conventional deep learning models that assume clean inputs.

Method: MCSTN introduces (1) dual-level corruption modeling (physical-level + diffusion-driven continuous corruption), (2) representation consistency learning across corrupted views, and (3) a dual-stream spatio-temporal architecture decoupling temporal dynamics and spatial correlations.

Result: Achieves competitive performance on PAMAP2, Opportunity, and WISDM datasets, particularly under imperfect sensing conditions, outperforming existing state-of-the-art methods.

Conclusion: The framework demonstrates effectiveness and robustness for real-world wearable IoMT sensing applications.
...
TL;DR: SSMProbe uses State Space Models as permutation-sensitive probes to reveal that token order critically impacts performance in frozen visual representations, with learned soft permutations outperforming fixed scans and exposing systematic differences across pre-training objectives (MAE, BEiT, DINOv2, ViT).


<details>
  <summary>Details</summary>
Motivation: Standard probing methods (GAP, CLS) treat patch tokens as an unordered bag-of-words, ignoring token order. This paper challenges that assumption by asking whether sequence order holds exploitable information in frozen visual representations.

Method: Proposes SSMProbe, a probing framework using State Space Models (discrete LTI systems) as permutation-sensitive probes. Compares fixed scan heuristics vs. a learned soft permutation via Sinkhorn operator, formulated as an information scheduling problem.

Result: Fixed scans fail on localized patch features. Learned soft permutation achieves competitive performance. Reveals systematic heterogeneity across models: DINOv2 concentrates semantics in CLS tokens, MAE preserves distributed representations, ViT is CLS-dominated, BEiT is intermediate. Order matters critically and is not reducible to spatial topology.

Conclusion: SSMProbe serves as a powerful diagnostic tool for visual representation analysis by exploiting token order heterogeneity, offering a new lens to understand how pre-training objectives shape token structure in vision models.
...
TL;DR: SAMamba3D adapts SAM (2D foundation model) to 3D pore-scale rock image segmentation using Mamba-based volumetric context, achieving generalizable multiphase segmentation without dataset-specific retraining.


<details>
  <summary>Details</summary>
Motivation: Current 3D segmentation methods for pore-scale X-ray images are dataset-specific, requiring retraining when rock type, fluid pattern, scanner, or acquisition conditions change. Foundation models like SAM offer strong 2D priors but cannot directly handle 3D data.

Method: A parameter-efficient framework coupling a largely frozen SAM encoder with Mamba-based volumetric context modeling and progressive cross-scale feature interaction for 3D pore-scale segmentation.

Result: Matches or outperforms current 3D baselines on sandstone and carbonate datasets with different fluids, wettability, and scanning conditions, while reducing need for case-specific retraining.

Conclusion: SAMamba3D enables reliable, rapid analysis of large 3D multiphase images while preserving physically meaningful descriptors (fluid saturation, connectivity, interface morphology).
...
TL;DR: Introduces energy-based constraint networks — a modality-agnostic architecture that learns structural coherence via energy scores, achieves strong text corruption detection (93.4% trained, 87.2% unseen) and deepfake detection (0.959 AUC FF++, 0.870 Celeb-DF zero-shot) with few parameters, and transfers across modalities by simply respecifying corruptions.


<details>
  <summary>Details</summary>
Motivation: To create a unified, modality-agnostic framework that learns structural coherence (i.e., what makes data "consistent" within a modality) without needing modality-specific architectural changes, and to allow detection of various violations (corruptions, deepfakes) via composable energy-based branches.

Method: Energy-based constraint networks: frozen encoder embeddings → state-space model + dual-head attention → scalar energy (global coherence) + per-position energy scores (localize violations). Multiple independently trained branches detect different violation types and compose at inference. Encoder-agnostic (BERT, DINOv2); domain-agnostic; supports designer-specified corruptions, real-world paired data, or both.

Result: Text: 93.4% accuracy on trained corruption types, 87.2% on 9 unseen types (frozen BERT, 7.4M params). Vision: 0.959 AUC on FF++ Deepfakes, 0.870 AUC on Celeb-DF zero-shot (frozen DINOv2, 3.6M params/branch). Five incompatible composition approaches failed before a compatible one succeeded.

Conclusion: First architecture to learn within-modality structural coherence as an explicit energy landscape with per-position decomposition. Demonstrates cross-modal transfer via corruption respecification alone, with composable branches requiring representation compatibility. Encoder- and domain-agnostic design enables broad applicability.
...
TL;DR: This paper presents an AI pipeline for transcribing medieval English legal manuscripts written in abbreviated Latin, achieving up to 88% word accuracy using neural networks, n-gram language models, and Gemini Pro 3 correction.


<details>
  <summary>Details</summary>
Motivation: Millions of pages of early Anglo-American legal records exist in handwritten, abbreviated medieval Latin that only a few dozen scholars worldwide can read, creating an enormous accessibility barrier for legal scholars, medievalists, and students.

Method: The authors built a dataset of 4029 lines from 193 medieval cases, trained an R-Blla model for line segmentation and a CNN+LSTM with CTC decoding for handwriting recognition, then applied n-gram language model post-processing and Gemini Pro 3 correction. They also compared against TrOCR (transformer-based).

Result: Baseline CNN+LSTM achieved 79% word accuracy; adding n-gram LM boosted to 82%; Gemini Pro 3 correction raised accuracy to 88%. TrOCR showed comparable word accuracy but worse character accuracy due to over-guessing.

Conclusion: The pipeline effectively opens up medieval legal manuscripts to broader scholarship, and the system is deployed at glyphmachina.com.
...
TL;DR: This paper introduces WildTableBench, a benchmark for evaluating multimodal foundation models on real-world table images with varied layouts and diverse domains. It includes 402 images and 928 QA pairs across 17 subtypes. Evaluating 21 frontier models reveals that only one exceeds 50% accuracy, with most models performing poorly, highlighting persistent weaknesses in structural perception and numerical reasoning.


<details>
  <summary>Details</summary>
Motivation: Current evaluations of multimodal foundation models on table understanding rely on structured-text tables or clean rendered images, failing to capture the complexity of in-the-wild table images with varied layouts and diverse domains. Existing benchmarks overlook the visual complexity of naturally occurring tables, leaving a critical gap in assessing structural perception and numerical reasoning capabilities.

Method: The authors collected 402 high-information-density table images from online forums and websites across diverse domains, and created 928 manually annotated and verified question-answer pairs spanning 17 subtypes across five categories. They evaluated 21 frontier proprietary and open-source multimodal foundation models on this benchmark, measuring accuracy and conducting diagnostic analyses to characterize failure modes.

Result: Only one model (likely GPT-4V or similar frontier model) exceeded 50% accuracy. All remaining models ranged from 4.1% to 49.9%, indicating significant struggles with real-world table images. Diagnostic analyses revealed persistent weaknesses in structural perception and reasoning across all models.

Conclusion: WildTableBench serves as a valuable diagnostic benchmark for table image understanding, revealing that current multimodal foundation models have substantial limitations in handling naturally occurring table images. The findings provide useful insights into model capabilities and highlight the need for improvements in structural perception and numerical reasoning for real-world table understanding tasks.
...
TL;DR: This paper investigates how Multimodal Large Language Models (MLLMs) behave under modality conflict and missingness in emotion recognition, discovers a Video Contribution Collapse (VCC) phenomenon where video evidence is marginalized, and proposes CHASE—a lightweight attention-steering mechanism that mitigates decision bias without retraining.


<details>
  <summary>Details</summary>
Motivation: Multimodal Emotion Recognition (MER) is critical for real-world interactions, yet the internal decision-making mechanisms of MLLMs under modality conflict and missingness remain underexplored.

Method: The authors introduce EmoMM, a benchmark with modality-aligned, conflict, and missing subsets. They uncover the Video Contribution Collapse (VCC) phenomenon and propose CHASE (Conflict-aware Head-level Attention Steering), a lightweight inference-time mechanism that detects modality conflicts and steers attention without retraining.

Result: CHASE consistently improves performance across various settings, significantly enhancing MLLM reliability in complex affective scenarios.

Conclusion: The proposed CHASE mechanism effectively mitigates decision bias caused by VCC, improving multimodal emotion recognition robustness under challenging modality conditions.
...
TL;DR: A physics-aware framework for generating realistic human motion in dynamic scenes by modeling full-body forces (human-object, human-scene, internal body dynamics) with soft physical constraints and a continuous distance-based force model for arbitrary surface interactions.


<details>
  <summary>Details</summary>
Motivation: Existing physics-aware human motion synthesis methods produce physically unrealistic motions due to limited contact modeling, typically restricted to hands only.

Method: (1) Explicit modeling of full human-related forces: human-object, human-scene, and internal body dynamics. (2) Soft physical constraints enforcing force and torque balance. (3) A continuous distance-based force model for contact modeling on arbitrary surfaces, including dynamic moving objects.

Result: Significantly improved physical plausibility; strong generalization to complex scenes; sets a new benchmark for physically consistent human motion generation.

Conclusion: The proposed approach outperforms prior works in physical realism and generalizes well to diverse dynamic environments, establishing a new standard for physically consistent human motion synthesis.
...
TL;DR: Neighbor2Inverse is a self-supervised denoising framework for low-dose phase-contrast X-ray CT that uses subsampling of noisy projections to train a denoising network without needing paired low/high-dose data, achieving superior noise suppression and detail preservation.


<details>
  <summary>Details</summary>
Motivation: Propagation-based X-ray phase-contrast imaging (PBI) offers high-contrast lung visualization but requires substantial radiation dose reduction for clinical translation, which increases noise. Supervised denoising needs paired low/high-dose data that is rarely available, and existing self-supervised methods are ill-adapted to PBI-CT.

Method: Neighbor2Inverse builds on the Neighbor2Neighbor principle: each noisy projection is subsampled into two variants preserving structure but with independent noise. These are reconstructed separately, and the resulting image pairs train a denoising network in the image domain in a self-supervised manner.

Result: In region-of-interest PBI-CT experiments, Neighbor2Inverse achieves superior noise suppression and preserves fine structural details, shown by improved contrast-to-noise ratio, spatial resolution, and composite image quality metrics. Competitive performance is also observed on clinical CT data under simulated low-dose conditions.

Conclusion: Neighbor2Inverse generalizes to clinical CT and enables safe dose reduction for PBI-CT without requiring paired datasets, advancing clinical translation of phase-contrast imaging.
...
TL;DR: Two approaches (WILD and WILD SAM) are proposed to improve object detector performance under adverse weather by denoising pseudo-labels and combining with simulation-based training, achieving up to 13% AP improvement.


<details>
  <summary>Details</summary>
Motivation: State-of-the-art object detectors degrade significantly under adverse weather, causing a domain shift problem for autonomous vehicles. Existing solutions rely on synthetic data (limited real-world applicability) or pseudo-labeling (noisy under harsh weather).

Method: (1) WILD: a pseudo-label denoising framework that filters noisy labels from real data under adverse weather. (2) WILD SAM: a hybrid training methodology combining pseudo-label denoising with simulation-based training using real target-domain data.

Result: Validated on the Four Seasons dataset across rainy and snowy scenarios; improves Average Precision (AP) by up to 13% and significantly reduces the weather-induced performance gap.

Conclusion: The proposed frameworks effectively mitigate weather-induced domain shift for object detection, outperforming baseline methods. Code is publicly available.
...
TL;DR: OsteoOpt++ is an image-to-decision AI planning tool that optimizes cut-plane and donor placement for mandibular reconstruction, improving bone apposition (up to 29 percentage points / 329% relative) over conventional surgical planning, validated on generic and patient-specific cases.


<details>
  <summary>Details</summary>
Motivation: Mandibular reconstruction using vascularized bone grafts suffers from donor-host nonunion. Current virtual surgical planning produces geometric plans rather than configurations that explicitly promote bone union.

Method: A pre-operative CT is converted into a personalized digital twin via registration and CT-derived muscle/TMJ parameter updates. Bayesian optimization with expected-improvement-plus acquisition searches six clinically controllable variables (cut-plane and donor positioning) under an apposition-driven objective with safety-factor regularization.

Result: Against common surgical approaches, apposition improved by up to 29 percentage points (329% relative) in generic cases and up to 26 percentage points in patient-specific cases vs. surgeon-implemented post-op configurations. Sensitivity analysis showed ≤4% objective change. Longitudinal case showed Dice overlap of 0.70-0.76 between predicted apposition and year-1 bone formation.

Conclusion: OsteoOpt++ provides surgeons with a pre-operative, image-driven recommendation for cut-plane orientation and donor placement predicted to improve union conditions over current operating room configurations.
...
TL;DR: DDiffusion is a robust text-to-image diffusion model that counters NSFW generation by using semantic retrieval to detect malicious intent in prompt embeddings and selectively editing only harmful regions in generated images, rather than applying binary allow/block filtering.


<details>
  <summary>Details</summary>
Motivation: Existing safety filters for T2I models use binary allow/block mechanisms that are vulnerable to adversarial keyword attacks and cause high false-alarm rates, degrading user experience and leaving models exploitable.

Method: DDiffusion uses (1) a semantic retrieval mechanism that evaluates prompts against concept distributions to uncover implicit malicious semantics, and (2) a localization method that selectively edits only harmful image regions during diffusion, returning sanitized images instead of blocking entirely.

Result: DDiffusion suppresses malicious NSFW content while preserving generation fidelity for benign prompts, avoiding the binary allow-deny signal that existing probing attacks exploit.

Conclusion: Moving beyond binary safety filters toward localized, semantics-aware sanitization improves both robustness against adversarial attacks and user experience for benign inputs.
...
TL;DR: ScribbleEdit is a large-scale synthetic dataset that combines freehand scribbles with natural language instructions to improve multimodal image editing, enabling models to achieve both spatial precision and semantic accuracy through a synthetic inpainting pipeline and finetuning.


<details>
  <summary>Details</summary>
Motivation: Existing image editing models struggle to jointly interpret abstract scribbles and natural language because no specialized training data exists for this combined modality, making precise spatial+semantic control difficult.

Method: The authors construct the ScribbleEdit dataset via a synthetic pipeline: (1) generate source-target image pairs through inpainting, (2) pair them with human-drawn scribbles and VLM-generated text instructions. They then finetune both diffusion-based and autoregressive unified multimodal models on this dataset.

Result: Off-the-shelf models perform poorly on abstract scribble inputs, but finetuning on ScribbleEdit significantly improves their ability to produce spatially aligned and semantically consistent edits.

Conclusion: ScribbleEdit effectively bridges the modality gap between scribbles and text, enabling more controllable and accurate image editing with combined spatial and semantic user input.
...
TL;DR: SCOUT is a context-aware multimodal framework for pathology report generation that integrates local histological patterns, whole-slide context, and expert semantic descriptors through progressive conditioning, achieving state-of-the-art BLEU/METEOR scores across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing pathology foundation models can generate fluent reports but lack clinical grounding—they fail to accurately represent key diagnostic concepts and relationships observed by pathologists due to difficulty integrating heterogeneous visual evidence (cellular patterns, tissue architecture, diagnostic concepts) while maintaining interpretability.

Method: SCOUT (Semantic Context-aware mOdality fUsion Transformer) is a context-aware concept-grounded multimodal framework that progressively conditions image representations using global slide information and explicit diagnostic concepts. It integrates local histological patterns, whole-slide context, and expert-curated semantic descriptors, using depth-aware contextual modulation and adaptive multimodal fusion during text generation.

Result: Using CONCH1.5 features, SCOUT achieves best BLEU-1 to BLEU-4 and METEOR scores on all three datasets (TCGA-BRCA, MICCAI REG, HistAI), plus best ROUGE-L on two datasets. On TCGA-BRCA: BLEU-1/2/3/4 = 0.436/0.303/0.202/0.156, METEOR = 0.204. On REG 2025: BLEU-1/2/3/4 = 0.865/0.834/0.805/0.780, METEOR = 0.568.

Conclusion: The results support that progressive contextual conditioning is effective for grounded pathology report generation, and SCOUT outperforms existing methods (WSI-Caption, HistGen, BiGen) across multiple metrics and datasets.
...
TL;DR: A contrastive learning-based zero-shot action recognition method that uses automatic negative sampling and joint embedding spaces to address semantic gap and domain shift, achieving SOTA on UCF-101 and Kinetics-400.


<details>
  <summary>Details</summary>
Motivation: To solve two key problems in Zero-Shot Action Recognition: the semantic gap (mismatch between textual label representations and visual features) and domain shift (difference between training and test distributions).

Method: Proposes a joint embedding space that encodes videos and sentences via contrastive learning, aligning videos with natural-language descriptions. An automatic negative sampling procedure is designed to augment training with unpaired data (visual appearance + unrelated descriptions).

Result: Achieves state-of-the-art results on UCF-101 and Kinetics-400 datasets across multiple split configurations.

Conclusion: The joint embedding space with contrastive learning and automatic negative sampling effectively bridges the semantic gap and mitigates domain shift in zero-shot action recognition.
...
TL;DR: CADFit is a hybrid optimization-based framework that reconstructs parametric CAD construction sequences from meshes by incrementally fitting operations (extrusions, revolutions, fillets, chamfers) using IoU-driven optimization, outperforming existing methods in accuracy and reducing invalid reconstructions.


<details>
  <summary>Details</summary>
Motivation: Existing CAD reconstruction methods are limited to difficult-to-edit formats (meshes, Breps) or simple sketch-and-extrude pipelines with low-complexity datasets, hindering practical design and manufacturing workflows that require editable parametric CAD sequences.

Method: CADFit formulates reconstruction as an IoU-driven optimization over structured CAD programs, incrementally fitting and validating parametric operations (extrusions, revolutions, fillets, chamfers) using geometric feedback from the input mesh.

Result: Outperforms state-of-the-art mesh-to-CAD methods in volumetric IoU and Chamfer Distance, with substantially reduced Invalid Ratio of reconstructed CAD programs, especially for complex designs.

Conclusion: CADFit enables accurate reconstruction of higher-complexity CAD models, providing a practical foundation for generating richer datasets and advancing learning-based CAD reverse engineering, with potential for multimodal end-to-end reconstruction from images.
...
TL;DR: The paper proposes using conditional score-based diffusion models (SBDMs) to synthesize realistic phase maps from magnitude-only MR images, enabling generation of synthetic k-space data for training deep learning-based MRI reconstruction models when raw k-space data is unavailable.


<details>
  <summary>Details</summary>
Motivation: Raw k-space data is rarely stored in clinical MRI due to storage and privacy concerns; only magnitude-only images are saved. This limits the availability of large, diverse training datasets for DL-based MRI reconstruction models.

Method: A conditional score-based diffusion model (SBDM) is used to synthesize realistic phase maps from magnitude-only MR images. The phase maps are combined with magnitude images to generate synthetic k-space data, which is then used to train DL reconstruction models.

Result: DL models trained on SBDM-synthesized k-space data outperform models trained on smooth phase, GAN-synthesized phase, and approach ground-truth-based training in both quantitative metrics and reconstruction fidelity (fewer hallucinations/errors).

Conclusion: Conditional SBDMs offer a viable approach to leverage large magnitude-only image databases for training high-quality DL-based MRI reconstruction models, addressing the k-space data scarcity problem.
...
TL;DR: ARFP is a privacy protection framework for face images that uses key-conditioned adversarial perturbations to resist restoration attacks while allowing authorized recovery with tamper detection.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial face privacy methods use input-space perturbations, but adversaries can learn restoration mappings to invert the protection. Current defense paradigms don't control reversibility, leaving them vulnerable to asymmetric attacks.

Method: ARFP has three components: (1) Key-Conditioned Manifold Binding (ties protection to a user key), (2) Adversarial Restoration-Aware Training (uses a surrogate restoration adversary during training), and (3) Authorized Reversible Restoration (enables recovery with correct key + nonce-based tamper indication).

Result: ARFP improves resistance against evaluated restoration attacks while preserving authorized recovery utility, demonstrating key-sensitive recovery behavior and tamper awareness.

Conclusion: The proposed restoration-aware approach effectively addresses the asymmetric adversarial attack problem by controlling reversibility via key-conditioned transformations and adversarial training, providing a more robust face privacy protection framework.
...
TL;DR: VIAR introduces an implicit equilibrium layer between shallow pre/post blocks for visual autoregressive generation, using next-scale prediction with Jacobian-Free Backpropagation. It achieves competitive FID scores (2.16 on ImageNet 256x256) while drastically reducing parameters (38.4% of VAR), lowering peak memory (8.53 GB vs 19.24 GB), and doubling throughput, with per-scale compute control at inference without retraining.


<details>
  <summary>Details</summary>
Motivation: VAR's explicit deep stacks fix computation per scale and inflate memory at high resolutions, limiting efficiency and flexibility. The authors aim to address these inefficiencies by introducing an implicit modeling approach that enables adaptive compute control and reduced memory footprint.

Method: VIAR uses an implicit equilibrium layer sandwiched between shallow pre/post blocks, trained with Jacobian-Free Backpropagation (constant memory). A per-scale iteration knob allows compute control during inference by adjusting fixed-point iteration steps.

Result: FID 2.16, sFID 8.07 on ImageNet 256x256 with only 38.4% parameters of VAR. Peak memory reduced from 19.24 GB to 8.53 GB, throughput doubled from 15.16 to 32.08 images/s on RTX 4090. Competitive with diffusion models and strong AR baselines. Superior zero-shot inpainting and editing quality.

Conclusion: Implicit equilibria and per-scale compute control offer practical benefits for visual generation, enabling better efficiency-quality trade-offs and deployability without retraining. VIAR consistently dominates VAR across operating points.
...
TL;DR: TT4D is a large-scale, high-fidelity table tennis dataset with 140+ hours of reconstructed gameplay from monocular videos, featuring multimodal annotations. It uses a novel "lift-first" pipeline that first reconstructs 3D ball trajectories before time segmentation, overcoming limitations of prior 2D-based approaches.


<details>
  <summary>Details</summary>
Motivation: Existing table tennis reconstruction methods fail under occlusion and varied camera viewpoints because they rely on 2D ball tracks for time segmentation before reconstruction, causing reliability issues.

Method: The "lift-first" paradigm: first lift the entire unsegmented 2D ball track to 3D using a learned lifting network, then perform reliable time segmentation. The network also infers ball spin, handles unreliable detections, and reconstructs trajectories under occlusion.

Result: 140+ hours of reconstructed singles and doubles gameplay with multimodal annotations including camera calibrations, 3D ball positions, ball spin, time segmentation, and 3D human meshes.

Conclusion: The dataset enables downstream tasks like racket pose/velocity estimation at impact and generative modeling of competitive rallies, and the lift-first pipeline is uniquely capable of reconstructing table tennis from broadcast monocular videos.
...
TL;DR: DACG-IR is a degradation-aware adaptive context gating framework for unified image restoration that uses lightweight multi-scale modules to extract degradation information and generate prompts, guiding attention temperature, output gating, and spatial-channel dual-gated fusion to suppress degradation noise while preserving structures, achieving SOTA performance across single-task, all-in-one, adverse weather, and composite degradation settings.


<details>
  <summary>Details</summary>
Motivation: Unified image restoration using a single model faces task interference due to diverse degradations, motivating the need for explicit degradation perception to dynamically modulate feature representations.

Method: Proposes DACG-IR with: (1) a lightweight multi-scale degradation-aware module extracting coarse degradation info and generating layer-wise prompts; (2) prompts guiding attention temperature and output gating in encoder/decoder blocks for adaptive feature extraction; (3) a spatial-channel dual-gated adaptive fusion mechanism that refines encoder features and suppresses noise propagation from shallow to deep layers.

Result: Outperforms state-of-the-art methods in single-task, all-in-one, adverse weather removal, and composite degradation settings.

Conclusion: Explicit degradation perception via adaptive context gating effectively addresses task interference in unified image restoration, achieving superior results across diverse degradation scenarios.
...
TL;DR: This paper investigates what prompt dimensions drive spatial attention in zero-shot VLMs for NSCLC tumor delineation, finding anatomical location as the dominant factor, and shows VoxTell achieves competitive performance comparable to fine-tuned models.


<details>
  <summary>Details</summary>
Motivation: To understand which prompt dimensions (e.g., diagnosis, anatomy, staging) govern spatial behavior in zero-shot VLMs for medical image segmentation, as this remains poorly understood despite their potential as promptable alternatives to task-specific models.

Method: Probing alignment directions in VoxTell via sub-prompt decomposition, attribute-wise perturbation robustness, specificity ladders, and cross-case prompt swaps, with statistical benchmarking against fine-tuned and zero-shot baselines using DSC, Wilcoxon signed-rank tests, and Benjamini-Hochberg correction.

Result: Anatomical location dominates spatial attention (63.4% perturbations cause catastrophic drops); histology/stage have minimal effect. VoxTell achieves mean DSC 0.613, statistically indistinguishable from fine-tuned nnUNet (0.690) and Ahmed et al. (0.675), significantly outperforming other zero-shot models.

Conclusion: Segmentation VLMs should be evaluated not only by Dice score but also by the prompt dimensions they align to, since the model prioritizes "where to look" over "what to look for."
...
TL;DR: This paper introduces GameScope, the largest gaming video quality dataset with 4,048 samples across H.264, H.265, and AV1 codecs, including both UGC and PGC content with quality attribute annotations, and demonstrates that a vision language model outperforms existing quality assessment benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing gaming video quality datasets are limited in size, diversity, and codec coverage, creating a gap for developing consistent quality assessment models that work across different streaming platforms and codecs.

Method: The authors collected 4,048 gaming video samples covering H.264, H.265, and AV1 codecs with both UGC and PGC content, annotated each with an average of 37 MOS ratings and coarse-grained quality attributes, then evaluated leading video quality assessment methods including a vision language model.

Result: The dataset is the largest of its kind, and the vision language model outperformed all benchmark methods on this dataset.

Conclusion: This is the first comprehensive gaming video quality assessment dataset spanning multiple codecs and content types with quality attributes, publicly available for research community use.
...
TL;DR: MIMO-ESP is a CNN-based Transformer architecture for spatiotemporal prediction that efficiently handles global information and achieves competitive performance on video, traffic, and precipitation prediction tasks.


<details>
  <summary>Details</summary>
Motivation: Existing RNN-based models suffer from parallelization inefficiency and stacked errors; CNN models struggle with global information due to local kernel properties and mix time with channel axes; Transformer models have high complexity and long training time.

Method: Proposes MIMO-ESP, a CNN-based Multi-In-Multi-Out model that configures a Transformer architecture using CNN to consider global information with significantly improved complexity. Treats time axis as independent and applies dilation for effective spatiotemporal information processing.

Result: Achieves competitive efficiency while outperforming existing models on three benchmark datasets (video, traffic, precipitation prediction). Ablation study confirms usefulness of each component.

Conclusion: MIMO-ESP demonstrates that combining CNN efficiency with Transformer's global awareness yields high-performance spatiotemporal prediction with reduced complexity.
...
TL;DR: This thesis identifies open datasets and benchmarks models for plant leaf disease classification, constructs a new dataset based on findings and augmentation studies, and trains a DenseNet201-based Base Model that outperforms baselines through faster, more robust, and more stable Transfer Learning with less data.


<details>
  <summary>Details</summary>
Motivation: Plant diseases cause large annual losses, but manual detection is laborious and time-consuming. Although ML/CNN methods show promise, there is a gap between publicly available datasets and what is needed to train fully capable models.

Method: 1) Identified open datasets and benchmarked models for plant leaf disease classification. 2) Conducted an augmentation applicability study. 3) Constructed a new dataset based on findings. 4) Trained a new Base Model using DenseNet201 architecture. 5) Applied Transfer-Learning experiments on another new dataset.

Result: The DenseNet201-based Base Model outperformed the baseline on the new dataset and also outperformed it in domain-specific Transfer-Learning experiments on another new dataset. It trains models faster, more robustly, more stably, and with less data.

Conclusion: The new Base Model overcomes major issues in the field (data scarcity, training inefficiency, instability) by enabling faster, more robust, and more stable Transfer Learning with reduced data requirements.
...
TL;DR: Chain of Evidence (CoE) is a visual attribution framework for Iterative RAG that uses Vision-Language Models to reason directly over document screenshots, outputting precise bounding boxes instead of vague text citations, eliminating format-specific parsing while improving interpretability and layout-aware reasoning.


<details>
  <summary>Details</summary>
Motivation: Current iRAG systems rely on parsed text, causing two issues: (1) coarse-grained attribution forcing users to manually locate evidence in lengthy documents, and (2) visual semantic loss where converting visually rich documents (slides, PDFs with charts) to text discards spatial logic and layout cues.

Method: CoE is a retriever-agnostic visual attribution framework that leverages Vision-Language Models to reason directly over screenshots of retrieved document candidates, outputting precise bounding boxes and visualizing the complete reasoning chain within the retrieved candidate set.

Result: Fine-tuned Qwen3-VL-8B-Instruct achieves robust performance, significantly outperforming text-based baselines in scenarios requiring visual layout understanding on two benchmarks: Wiki-CoE (structured web pages) and SlideVQA (presentation slides with complex diagrams).

Conclusion: CoE establishes a retriever-agnostic solution for pixel-level interpretable iRAG, eliminating format-specific parsing while providing precise visual attribution through bounding boxes, enabling effective reasoning over visually rich documents.
...
TL;DR: SIFT-VTON enhances diffusion-based virtual try-on by using SIFT keypoint matching to explicitly supervise cross-attention layers, improving spatial alignment and fine-detail preservation like text and patterns.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based virtual try-on methods struggle to preserve fine details (text, illustrations) due to reliance on implicit learning of spatial correspondences, lacking explicit geometric guidance.

Method: Applies domain-specific filtering to SIFT keypoint matches between garment and person images, converts them into spatial probability distributions, and uses these to supervise cross-attention layers during training for precise spatial alignment.

Result: Significant improvements on unpaired metrics on VITON-HD, competitive paired reconstruction, superior preservation of text clarity and pattern alignment, and sharply focused attention on garment details.

Conclusion: Classical geometric correspondence methods (SIFT) can effectively enhance modern diffusion models for conditional synthesis tasks like virtual try-on.
...
TL;DR: CUE addresses long-tailed learning by introducing multi-label concept signals via zero-shot CLIP visual cues and LLM semantic cues, using weighted Binary Logit-Adjustment losses to mitigate both distribution bias and inter-class concept confusion.


<details>
  <summary>Details</summary>
Motivation: Existing long-tailed learning methods focus on mitigating distribution bias but overlook concept confusion caused by single-label supervision under long-tailed distributions, which suppresses feature sharing among related classes and amplifies head class dominance.

Method: CUE introduces Concept-aware Multi-label Expansion: (1) extracts instance-level visual cues from zero-shot CLIP, (2) generates class-level semantic cues via LLM, (3) combines them using separately weighted Binary Logit-Adjustment (BLA) auxiliary losses, (4) jointly optimized with baseline Logit-Adjustment (LA) loss.

Result: Achieves balanced and strong performance on several long-tailed benchmarks, surpassing recent state-of-the-art methods.

Conclusion: Multi-label concept signals effectively preserve disrupted inter-class relationships in long-tailed learning, outperforming existing approaches.
...
TL;DR: PACE proposes a new LiDAR point cloud compression framework that decouples context aggregation from probability prediction using a non-causal backbone and lightweight scalable predictor, achieving state-of-the-art compression with over 90% decoding latency reduction.


<details>
  <summary>Details</summary>
Motivation: Existing octree-based learned entropy models suffer from prohibitive decoding latency due to causal multi-stage context modeling and a rigid performance-latency trade-off that prevents a single model from adapting to varying constraints.

Method: PACE reformulates ancestral context aggregation as a non-causal backbone and confines causality to a lightweight, stage-scalable predictor, eliminating repetitive backbone executions and supporting arbitrary prediction stages for seamless adaptation.

Result: PACE achieves new state-of-the-art compression efficiency with notable BD-BR savings and reduces decoding latency by over 90% in autoregressive mode.

Conclusion: Decoupling context aggregation from probability prediction effectively resolves bottlenecks in latency and flexibility, making PACE highly attractive for practical autonomous systems.
...
TL;DR: VideoThinker is a causal-inspired framework that addresses perceptual bias in lightweight multimodal models during RL fine-tuning, using a two-stage debiasing process (Bias Aware Training + CDPO) to achieve state-of-the-art video reasoning efficiency without SFT, outperforming larger models like VideoRFT-3B and Video-UTR-7B.


<details>
  <summary>Details</summary>
Motivation: RL-based fine-tuning struggles with lightweight MLLMs needed for edge deployment, as it compels them to exploit perceptual shortcuts from data biases rather than develop genuine reasoning abilities.

Method: Two-stage debiasing framework: (1) Bias Aware Training creates a dedicated "bias model" that embodies shortcut behaviors; (2) Causal Debiasing Policy Optimization (CDPO) fine-tunes the primary model with a repulsive objective that pushes it away from biased logic while pulling toward correct solutions.

Result: VideoThinker-R1 achieves SOTA in video reasoning efficiency: surpasses VideoRFT-3B by 3.2% average gain (7% on VideoMME) using only 1% training data and no SFT; outperforms larger Video-UTR-7B by 2.1% on MVBench and 3.8% on TempCompass.

Conclusion: Causal debiasing effectively enables lightweight MLLMs to develop genuine reasoning capabilities for video tasks, demonstrating that addressing perceptual bias is key to RL fine-tuning for edge-deployable models.
...
TL;DR: This paper systematically investigates what makes vision encoders suitable for Vision-Language Models, finding that common selection metrics like model size or zero-shot accuracy correlate weakly with VLM performance, and instead proposes using Gromov-Wasserstein distance to measure cross-modal structural similarity as a more effective predictor.


<details>
  <summary>Details</summary>
Motivation: Current practices for selecting vision encoders in VLMs lack principled understanding; commonly used heuristics (largest model size, highest zero-shot accuracy) often fail to identify optimal encoders, motivating the need for a systematic investigation of what factors truly matter.

Method: Conducted comprehensive experiments on 19 pre-trained vision encoders; analyzed correlation between common encoder metrics and VLM performance; introduced Gromov-Wasserstein distance to measure cross-modal structural similarity as a selection criterion; provided theoretical analysis linking Gromov-Wasserstein distance to cross-modality mapping learnability; validated on 60+ full VLM training runs.

Result: Common metrics (model size, zero-shot accuracy) show only weak to moderate correlation with VLM performance; the proposed Gromov-Wasserstein distance metric correlates significantly better with final VLM performance and enables effective prediction before full training.

Conclusion: Structural similarity across modalities, measured via Gromov-Wasserstein distance, is a crucial but overlooked factor for vision encoder selection in VLMs, offering an efficient inference-only metric for predicting VLM performance.
...
TL;DR: Colinearity-Decay (CD) is a structural regularizer for vision Transformers that penalizes harmful cross-matrix alignment to control activation outliers, improving low-bit quantized accuracy without sacrificing full-precision performance.


<details>
  <summary>Details</summary>
Motivation: Activation outliers complicate the deployment of quantized vision Transformers. Existing methods either suppress outliers post-training or during training, but aggressive restriction leads to poor trade-off between full-precision and quantized accuracy.

Method: Introduces Colinearity-Decay (CD), a structural regularizer for ordered matrix pairs within Transformer blocks that penalizes detrimental cross-matrix alignment. Applied as a decoupled update, it is non-invasive with minimal training overhead.

Result: Consistently boosts quantized accuracy across ImageNet-1K pre-training, COCO detection, and downstream fine-tuning, while preserving or even improving full-precision performance.

Conclusion: Structural regularization effectively prepares vision Transformers for low-bit deployment with zero inference-time overhead.
...
TL;DR: This paper proposes a novel interpretable image steganalysis framework for invertible image hiding that works under zero-shot settings, integrates hiding/revealing/steganalysis, and uses residual augmentation to enhance cross-dataset and cross-architecture generalization, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Prior steganalysis methods are limited to binary classification (stego vs. cover) and require training/testing data to follow similar distributions, which hinders real-world applicability. The need to detect emerging invertible image hiding methods under challenging zero-shot settings motivates this work.

Method: The authors propose a unified framework integrating image hiding, revealing, and steganalysis, where the steganalysis component can also recover hidden secret information. A residual augmentation strategy is used to generate stego images, improving cross-dataset and cross-architecture generalization.

Result: Extensive experiments on benchmark datasets show the proposed approach significantly outperforms existing steganalysis techniques for invertible image hiding schemes.

Conclusion: The proposed interpretable zero-shot steganalysis framework overcomes limitations of prior binary classification approaches and demonstrates superior generalization and detection capability for invertible image hiding.
...
TL;DR: This paper addresses the perceptual bandwidth bottleneck in Vision-Language Models by framing visual perception as a sequential Bayesian optimal experimental design problem, proposing a training-free inference strategy that uses multiple vision tools to dynamically balance spatial coverage and resolution, achieving state-of-the-art results on gigapixel-level benchmarks.


<details>
  <summary>Details</summary>
Motivation: VLMs suffer from a fundamental trade-off between broad field of view and fine-grained detail, limiting complex reasoning in high-resolution visual tasks.

Method: Formulates visual perception as a sequential Bayesian optimal experimental design (S-BOED) problem with tractable approximations, and proposes a training-free inference strategy using multiple vision tools with flexible optimization (greedy sampling to look-ahead planning).

Result: Outperforms state-of-the-art models on gigapixel-level benchmarks, significantly narrowing the gap to human-annotated oracles.

Conclusion: Active, sequential decision-making through S-BOED effectively overcomes the perceptual bandwidth bottleneck in VLMs without requiring additional training.
...
TL;DR: CHASE is a selective prediction framework that compares competing temporal hypotheses to decide when to abstain from decisions under ambiguity, outperforming standard uncertainty methods on hidden connectivity inference tasks (e.g., GUV dynamics).


<details>
  <summary>Details</summary>
Motivation: Standard selective prediction methods relying on single-branch uncertainty estimation fail under partial observability where temporal evidence is contradictory and confidence scores become misleading.

Method: CHASE explicitly compares structured temporal explanations (competing hypotheses) and uses a ranking-aware selector over hypothesis margins to separate safe commitments from fundamentally uncertain ones.

Result: Up to 11.0% relative mean improvement in overall ambiguity-aligned abstention, 8.8% relative boost in three-way accuracy in high-ambiguity regimes, and 9.9% overall risk reduction at 90% coverage.

Conclusion: Reasoning over competing hypotheses provides a more reliable approach to selective prediction under structured ambiguity compared to canonical uncertainty baselines.
...
TL;DR: AgriKD introduces a cross-architecture knowledge distillation framework that transfers knowledge from a Vision Transformer teacher to a compact CNN student, achieving 172x fewer parameters, 47.57x lower computational cost, and 18-22x faster inference while maintaining performance on leaf disease classification tasks.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers (ViTs) offer strong representation for leaf disease classification but are computationally too expensive for edge device deployment, and existing methods fail to effectively transfer these rich transformer representations to lightweight models.

Method: A cross-architecture knowledge distillation framework (AgriKD) that transfers knowledge from a ViT teacher to a compact CNN student using multiple distillation objectives at output, feature, and relational levels to bridge the representational gap between Transformer and CNN architectures.

Result: The distilled student achieves comparable performance to the teacher with ~172x fewer parameters, 47.57x lower computational cost, and 18-22x faster inference. Consistent performance across ONNX, TFLite, and TensorRT formats with negligible accuracy degradation on edge devices.

Conclusion: AgriKD demonstrates practical real-time inference on NVIDIA Jetson and mobile applications, proving its viability for AI-powered agricultural applications in resource-constrained environments.
...
TL;DR: VoxAfford enhances open-vocabulary 3D affordance detection by injecting multi-scale geometric features from a frozen 3D VQVAE encoder into MLLM output tokens, achieving ~8% mIoU improvement and zero-shot robot transfer.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM-based methods for 3D affordance detection use autoregressive output tokens that capture sequential dependencies but lack spatial neighborhood information, resulting in semantically rich but spatially impoverished 3D localization.

Method: VoxAfford uses a frozen pre-trained 3D VQVAE encoder to extract multi-scale geometric features. Output tokens query these features via cross-attention with a learned compatibility gate controlling injection strength. Enhanced tokens are aggregated into a spatially-aware affordance prompt via semantic-conditioned attention, then fused with per-point features for mask prediction.

Result: State-of-the-art performance on open-vocabulary affordance detection with ~8% mIoU improvement; real robot experiments demonstrate zero-shot transfer to novel objects.

Conclusion: Injecting multi-scale geometric priors into MLLM output tokens significantly improves spatial grounding for 3D affordance detection without sacrificing semantic richness.
...
TL;DR: VAEsselSparse is an efficient encoder-decoder model that leverages sparse convolutions and attention mechanisms to achieve 8x8x8 compression of organ-level 3D vascular networks while preserving clinically relevant features for classification and generative modeling.


<details>
  <summary>Details</summary>
Motivation: Current methods rely on small sub-regions or simplified tree-like structures, making analysis of entire organ-level vascular networks at clinical resolution computationally challenging.

Method: VAEsselSparse uses an encoder-decoder architecture with sparse convolutions and attention mechanisms to exploit the inherent sparsity of 3D vascular structures, achieving 8x8x8 spatial compression.

Result: Superior reconstruction performance compared to dense counterparts and previous methods; latent space retains clinically discriminative features for tasks like aneurysm/stenosis classification and circle of Willis subvariant identification.

Conclusion: The compact latent space serves as an effective representation for learning vessel-specific priors via generative models, enabling realistic vasculature synthesis.
...
TL;DR: VISTA is a new benchmark for evaluating Vision-Language Models on complex, multi-entity, multi-action spatio-temporal understanding in videos, with ~12K curated video-query pairs and taxonomy-driven diagnostics revealing biases obscured by traditional metrics.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs benchmarks fail to capture freeform, multi-action interactions between diverse entities in real-world videos, and lack a systematic framework for analyzing model failures across spatio-temporal axes.

Method: VISTA decomposes videos into interpretable entities, actions, and relational dynamics; integrates multiple datasets into a unified interaction-aware taxonomy; uses ~12K curated video-query pairs for evaluation.

Result: Evaluated 11 state-of-the-art VLMs on VISTA, revealing pronounced spatio-temporal biases and shortcomings in relational, spatial, and temporal understanding that are obscured by traditional metrics.

Conclusion: VISTA provides the first large-scale, interaction-aware diagnostic benchmark for spatio-temporal understanding in VLMs, offering a nuanced framework to guide model design, pretraining strategies, and evaluation protocols.
...
TL;DR: A differentiable motion forecasting framework that retrieves interpretable motion priors from a contrastively-learned "motion bank" via anchor retrieval and cross-attention, then refines them with a DETR-style decoder, achieving competitive accuracy on Argoverse 2 and Waymo Open Motion datasets.


<details>
  <summary>Details</summary>
Motivation: To improve interpretability in motion forecasting without sacrificing accuracy, addressing the issues of latent collapse in anchor-based architectures and limited multi-modal diversity in naive trajectory sampling.

Method: End-to-end differentiable framework with: (1) a motion bank embedding space via contrastive learning, (2) Anchor Retrieval Layer with orthogonal initialization and Dual-Level Gated Cross-Attention, (3) Straight-Through Gumbel-Softmax for discrete selection with gradient flow, (4) DETR-style decoder with WTA kinematic GMM, diversity penalty, and soft-min weighted endpoint loss.

Result: Competitive multi-modal accuracy on Argoverse 2 and Waymo Open Motion datasets, with improved interpretability over standard latent query approaches.

Conclusion: Grounding predictions on interpretable motion primitives eliminates the black-box nature of latent queries while maintaining state-of-the-art performance.
...
TL;DR: MOCHI is a multi-view 3D face prediction framework that eliminates the need for registered training data by using topological consistency enforcement via a pseudo-linear inverse kinematic solver and synthetic-data-trained 2D landmarks, achieving superior reconstruction accuracy and visual quality without manual registration pipelines.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitation of existing learning-based 3D face correspondence methods (ToFu, TEMPEH) that rely on slow, manual registration pipelines for training supervision, by eliminating the need for registered training data entirely.

Method: MOCHI uses (1) a pseudo-linear inverse kinematic solver for topological consistency without registered data, (2) dense keypoints from a 2D landmark predictor trained on synthetic data for semantic alignment, (3) pointmap- and normal-based losses instead of standard point-to-surface distances to avoid training instabilities, and (4) test-time optimization that refines network weights over a few dozen iterations.

Result: MOCHI outperforms traditional labor-intensive pipelines in both reconstruction accuracy and visual quality, bridging the gap between feed-forward efficiency and iterative optimization precision.

Conclusion: MOCHI successfully eliminates the registration data dependency for multi-view 3D face prediction while achieving superior results, demonstrating that learning-based frameworks can replace manual registration pipelines without requiring them for supervision.
...
TL;DR: This paper proposes SRGAN--CKAN, a hybrid super-resolution framework that replaces standard linear convolutions with nonlinear spline-based Kolmogorov-Arnold Networks within a GAN setup, achieving better perceptual quality and efficiency under constrained computational resources.


<details>
  <summary>Details</summary>
Motivation: Existing SISR methods (transformers, diffusion models) improve quality but suffer from high computational complexity. This work aims to enhance the expressivity of local operators to achieve efficient super-resolution under minimal hardware resources.

Method: The authors integrate Convolutional Kolmogorov-Arnold Networks (CKAN) into an adversarial learning framework (SRGAN). They reformulate convolution as a nonlinear patch-based transformation using spline-based functional representations, replacing linear local mappings.

Result: The proposed approach improves perceptual quality while preserving reconstruction fidelity, achieving a favorable balance between distortion-based and perceptual metrics under constrained computational settings.

Conclusion: This work offers a complementary direction to globally intensive architectures by improving local transformation representational power, providing an efficient and scalable alternative for SISR.
...
TL;DR: SplAttN addresses Cross-Modal Entropy Collapse in point cloud completion by replacing hard projection with Differentiable Gaussian Splatting, achieving SOTA on PCN/ShapeNet and robust cross-modal dependency on KITTI.


<details>
  <summary>Details</summary>
Motivation: Identifies that standard hard projection in multi-modal point cloud completion causes Cross-Modal Entropy Collapse (extremely sparse support on image plane), hindering visual prior propagation and cross-modal connection learnability.

Method: Proposes SplAttN, which replaces hard projection with Differentiable Gaussian Splatting to produce dense, continuous image-plane representations, reformulating projection as continuous density estimation.

Result: State-of-the-art on PCN and ShapeNet-55/34; on KITTI, counter-factual evaluation shows SplAttN maintains robust visual cue dependency while baselines degenerate into unimodal template retrievers.

Conclusion: Differentiable Gaussian Splatting effectively establishes cross-modal connections, prevents entropy collapse, and improves gradient flow and learnability in multi-modal point cloud completion.
...
TL;DR: This paper proposes Decision Boundary-aware Generation (DBG) to address boundary ambiguity caused by head-to-tail transfer in diffusion-based generative augmentation for long-tailed learning, improving tail class accuracy and decision space separability.


<details>
  <summary>Details</summary>
Motivation: Long-tailed data bias causes decision boundaries toward head classes and degrades tail class accuracy. While head-to-tail transfer helps, it induces latent non-local feature mixing that entangles inter-class features, causing decision boundary overlap and tail class distribution shift. The authors identify this boundary ambiguity problem.

Method: The paper proposes Decision Boundary-aware Generation (DBG) framework that promotes near-boundary representation learning by generating informative near-boundary samples, rebalancing the long-tailed dataset while yielding more separable decision space.

Result: Across standard long-tailed benchmarks, DBG consistently improves tail class and overall accuracy with less inter-class overlap.

Conclusion: DBG effectively addresses boundary ambiguity in head-to-tail transfer for long-tailed learning, producing more separable decision spaces and improving both tail class and overall accuracy.
...
TL;DR: LIE is a LiDAR-only HD map construction method that uses Knowledge Distillation from a teacher branch (fusing LiDAR features with 2D intensity maps) to overcome LiDAR's lack of dense semantic cues, achieving 8.2% higher mIoU than SOTA camera-based models on nuScenes and robust performance in challenging conditions.


<details>
  <summary>Details</summary>
Motivation: Camera-based HD map methods lack depth information for accurate geometry, while LiDAR provides precise 3D measurements but lacks dense semantic and texture cues. The goal is to leverage LiDAR's strengths while compensating for its weaknesses using knowledge distillation.

Method: The teacher branch fuses student LiDAR features with corresponding 2D intensity map tiles to provide dense supervision for segmenting map elements via an online distillation scheme.

Result: Outperforms all single-modality approaches, achieving 8.2% higher mIoU than SOTA camera-based models on nuScenes. Robust over long ranges and under challenging weather/lighting. Efficiently adapts to Argoverse2 with only 10% fine-tuning, surpassing full-dataset camera-based models.

Conclusion: LiDAR-only HD map construction via knowledge distillation is effective, surpassing camera-based methods and showing strong generalization with minimal fine-tuning.
...
TL;DR: CSGuard is a forgery-resistant watermarking schema using compressed sensing to bind watermark generation/verification to a secret matrix, reducing attack success from 100% to 28.12% while maintaining 100% benign detection.


<details>
  <summary>Details</summary>
Motivation: Latent-based diffusion model watermarking is vulnerable to forgery attacks where attackers extract watermarks via inversion and re-generate with arbitrary prompts, enabling false attribution.

Method: CSGuard uses compressed sensing with a secret matrix to bind watermark embedding and verification, ensuring only authorized users with the secret matrix can correctly embed or verify watermarks.

Result: Attack success rate reduced from 100% to 28.12%, with 100% detection rate on benign watermarked images without compromising generation quality or watermark integrity.

Conclusion: CSGuard is the first forgery-resistant watermarking schema that effectively prevents forgery attacks while maintaining watermarking effectiveness and generation quality.
...
TL;DR: This paper introduces KVInject and AttnRouter for training-free image editing on Qwen-Image-Edit-2511 (MMDiT), showing KVInject outperforms MasaCtrl on MMDiT, AttnRouter improves structure preservation via per-category routing, and editing-effective attention sub-circuits are localized to early denoising steps with moderate alpha values.


<details>
  <summary>Details</summary>
Motivation: Existing training-free editing methods like MasaCtrl rely on a two-pass attention mechanism that fails on multi-modal diffusion transformers (MMDiT) due to prompt mismatch (31% drop in composite score). The authors aim to develop a simpler, single-forward editing approach compatible with MMDiT architectures.

Method: (i) KVInject: single-forward attention manipulation that alpha-blends source-half key/value projections into noise-half within a localized layer/step band. (ii) AttnRouter: per-category routing table dispatching edits to the best operation for each edit type. (iii) Systematic ablations to localize editing-effective attention sub-circuits.

Result: KVInject avoids MasaCtrl's prompt-mismatch failure on MMDiT. AttnRouter improves CLIP-T+DINO-I composite by 6.4% with ground-truth categories; automatic CLIP zero-shot classifier closes 98% of this gap. Early denoising steps (S0-7) recover most of the gain. Alpha in [0.3, 0.5] is optimal. Simple K/V rescaling never beats baseline.

Conclusion: Training-free editing on MMDiT is feasible with KVInject and AttnRouter. The editing-effective sub-circuit is highly localized, and UNet-derived heuristics (e.g., K/V rescaling) do not transfer. Code, routing tables, and benchmark subsets are released.
...
TL;DR: A hierarchical cross-modal fusion model for vision-language QA in industrial robotics improves semantic alignment, accuracy, and robustness by integrating object detection, multi-scale visual encoding, syntactic parsing, and attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: To address semantic ambiguity, complex environmental layouts, and domain-specific terminology challenges in vision-language question answering for modern manufacturing settings.

Method: Proposes a hierarchical cross-modal fusion framework combining region-based deep networks for visual features, weighted embedding aggregation, recurrent neural parsing for sentence structures, and adaptive fusion with cross-attention mechanisms for fine-grained semantic alignment.

Result: Outperforms existing VLQA benchmarks (IVQA, RIF) in semantic alignment, Top-1 accuracy, and robustness; ablation studies confirm the necessity of multi-level feature integration and context-driven gating.

Conclusion: The proposed methodology enhances interpretability and operational effectiveness of industrial robots in diverse human-robot interaction tasks.
...
TL;DR: CGFformer is a cluster-guidance frequency Transformer for pansharpening that uses adaptive frequency separation via K-means clustering, dual-stream denoising with cross-attention, and frequency-spatial fusion to outperform existing methods.


<details>
  <summary>Details</summary>
Motivation: Current frequency-based pansharpening methods use fixed frequency filters that cannot adapt to complex spatial-frequency distributions, and existing denoising strategies insufficiently leverage frequency components to suppress various noise types.

Method: 1) Adaptive separation module using K-means clustering for precise high/low-frequency separation; 2) Dual-stream refinement module with Transformer cross-attention for noise removal; 3) Frequency-spatial fusion module for detail enhancement and spatial-frequency interaction.

Result: Notable improvements over existing pansharpening approaches on multiple benchmark datasets.

Conclusion: CGFformer effectively addresses the limitations of fixed filters and insufficient denoising in frequency-based pansharpening through adaptive clustering, dual-stream denoising, and frequency-spatial fusion.
...
TL;DR: This paper reports results from the first SF20K Competition (ICCV 2025) on open-ended video QA for short films, finding that narrative-aware shot-level processing, multi-stage pipelines with small models, and subtitle quality matter more than raw model capacity. Best accuracy was 65.7% vs. 91.7% human performance.


<details>
  <summary>Details</summary>
Motivation: To advance story-level video understanding beyond short-clip action recognition by introducing an open-ended QA task on amateur short films, preventing reliance on memorization of popular movies.

Method: Competition with two tracks (Main Track: unrestricted model size; Special Track: <8B parameters), evaluated on SF20K-Test benchmark (95 movies, 979 QA pairs) scored by LLM-QA-Eval (GPT-4.1-nano). 22 teams, 286 submissions.

Result: Winning team: 65.7% accuracy (Main Track), 48.7% (Special Track); human ceiling 91.7%. Narrative-aware shot-level processing beats uniform sampling; multi-stage small-model pipelines match 30x larger end-to-end models; subtitle quality dominates performance.

Conclusion: The primary bottleneck in long-form video QA is information selection and reasoning structure, not raw model capacity. A significant gap remains between current methods and human-level narrative comprehension.
...
TL;DR: This paper introduces 3DVQL, the first benchmark for visual query localization in 3D space, containing 2,002 sequences with multimodal data (point clouds, RGB, depth). It proposes a lift-and-attention fusion (LaF) algorithm that outperforms existing baselines.


<details>
  <summary>Details</summary>
Motivation: Existing VQL research focuses on 2D videos, while 3D visual query localization remains unexplored. The authors aim to fill this gap by introducing a dedicated 3D benchmark.

Method: The authors create 3DVQL (2,002 sequences, 170K frames, 6.4K track segments, 38 categories) with multimodal annotations (point clouds, RGB, depth). They implement several 3D multimodal baselines and propose a lift-and-attention fusion (LaF) algorithm.

Result: Existing methods show significant performance variations across different fusion modules. The proposed LaF algorithm substantially outperforms all baseline models.

Conclusion: 3DVQL serves as the first benchmark for 3D multimodal VQL, and LaF provides a strong baseline. The benchmark and model will be publicly released to facilitate future research.
...
TL;DR: RADMI is a single-pass method that estimates epistemic uncertainty in segmentation by measuring mutual information between consecutive decoder layers, achieving strong correlation with deep ensemble uncertainty without heavy computation.


<details>
  <summary>Details</summary>
Motivation: Existing epistemic uncertainty estimation methods (ensembles, multiple stochastic forward passes) are computationally expensive and not scalable to dense prediction tasks like segmentation.

Method: Proposes Resolution-Aggregated Decoder Mutual Information (RADMI), which computes mutual information between consecutive decoder layers in a single forward pass to estimate prediction uncertainty, based on the observation that elevated inter-layer MI correlates with ambiguous regions.

Result: On seismic facies segmentation, RADMI outperforms single-pass baselines by 5.5% (Pearson) and 10.7% (Spearman) in correlation with deep ensemble uncertainty, producing sharp boundary-localized maps.

Conclusion: Linear aggregation of normalized information flow is a principled and efficient proxy for prediction uncertainty in encoder-decoder architectures, enabling scalable uncertainty estimation.
...
TL;DR: Omni-Encoder is a unified Transformer backbone that co-embeds video and audio at symmetrical 25 fps, using Omni-Encoder Token Template, Omni-RoPE, and Temporal Window Shifting to achieve better cross-modal interaction and fine-grained motion understanding compared to modality-specific baselines.


<details>
  <summary>Details</summary>
Motivation: Current omni-modal models use modality-specific encoders with asymmetric frame rates (1-2 fps video, 25 fps audio), leading to impoverished cross-modal interaction and inability to capture fine-grained visual motion, unlike holistic human perception.

Method: Omni-Encoder uses a unified Transformer backbone with symmetrical 25 fps co-embedding of visual and audio signals, incorporating Omni-Encoder Token Template, Omni-RoPE, and Temporal Window Shifting to balance modality disentanglement and computational efficiency.

Result: Outperforms Qwen2.5-Omni on visual continuous understanding tasks (sign language recognition, fine-grained sports analysis) while maintaining competitive performance on AVQA and Speaker Identification and Localization benchmarks.

Conclusion: Unified omnivorous encoding is a promising direction for building omni-modal models that more closely reflect integrated human perception.
...
TL;DR: SwiftPie is the first one-step diffusion model for image personalization, using a dual-branch identity injection mechanism and mask-guided rescaling to achieve lightning-fast generation without iterative optimization or multi-step denoising.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based image personalization methods rely on computationally intensive fine-tuning, iterative optimization, or multi-step denoising, hindering real-time deployment and interactive capabilities.

Method: SwiftPie introduces a dual-branch identity injection mechanism to integrate subject identity into a one-step diffusion model, plus a mask-guided rescaling strategy for enhanced subject contextualization within a single diffusion step.

Result: SwiftPie achieves superior personalization speed (one-step) while maintaining comparable performance to multi-step approaches in identity fidelity and prompt alignment.

Conclusion: This work enables real-time, high-quality personalized image generation, opening opportunities for interactive visual synthesis applications.
...
TL;DR: A frozen VLM pipeline (no fine-tuning) for traffic accident grounding in CCTV footage achieves ACC^S=0.539 on the ACCIDENT@CVPR 2026 benchmark, outperforming prior best by +0.127, using coarse-to-fine two-pass decomposition and specialist role assignment across two VLMs.


<details>
  <summary>Details</summary>
Motivation: Traffic accident grounding in real CCTV footage is a rare-event problem where labeled accident videos are often unavailable for training, yet accurate joint localization in time, space, and collision type is needed.

Method: A no-fine-tuning pipeline combining (1) coarse-to-fine two-pass decomposition: first pass at 1 fps for coarse (t,x,y,c) tuple, second pass at 5 fps within ±3s window for refinement with deterministic confidence gates; (2) specialist role assignment: Qwen3-VL-Plus for grounding, Gemini 3.1 Flash-Lite for collision typing on a centered clip.

Result: ACC^S = 0.539 (95% CI [0.525, 0.553]) on ACCIDENT@CVPR 2026 benchmark (2,027 real CCTV videos): +0.127 over best-of-baselines oracle (0.412), +0.143 over strongest single-VLM baseline (Molmo-7B, 0.396), +0.250 over naive baseline (0.289). Uses ≤3 API calls per video; 17% fallback to physics on API failures; total cost ~$20.

Conclusion: Frozen, no-fine-tuning VLMs can effectively ground traffic accidents in time, space, and type, outperforming fine-tuned baselines, at very low cost (~$20) with minimal API calls.
...
TL;DR: VAnim is the first LLM-based framework for text-to-SVG animation that uses Sparse State Updates (SSU) on a persistent SVG DOM tree, compressing sequence length by 9.8x while preserving structure. It employs Identification-First Motion Planning and Rendering-Aware Reinforcement Learning (GRPO) to overcome non-differentiable rendering challenges. The paper introduces SVGAnim-134k benchmark and shows significant improvements over baselines.


<details>
  <summary>Details</summary>
Motivation: SVG animation generation is challenging because it must bridge discrete code representations with continuous visual dynamics. Existing optimization-based methods destroy topological consistency, and general-purpose LLMs rely on rigid CSS/SMIL transformations that fail to model geometry-level non-rigid deformations.

Method: 1) Sparse State Updates (SSU) on a persistent SVG DOM tree compresses sequence length by 9.8x while preserving structure. 2) Identification-First Motion Planning grounds textual instructions in explicit visual entities. 3) Rendering-Aware Reinforcement Learning via GRPO with hybrid reward from a video perception encoder aligns discrete code updates with visual feedback.

Result: VAnim significantly outperforms state-of-the-art baselines in semantic alignment and structural validity, with additional metrics validating motion quality and identity preservation. The SSU paradigm achieves 9.8x sequence compression.

Conclusion: The proposed VAnim framework effectively addresses limitations of existing SVG animation generation methods, offering a practical LLM-based solution for open-domain text-to-SVG animation with strong structural validity and semantic alignment.
...
TL;DR: HyCAS is a novel adversarial defense that combines deterministic 1-Lipschitz convolutions with stochastic components (random projection filters and attention-noise mechanism) to create a ≤2-Lipschitz network, achieving state-of-the-art certified L2 and empirical L∞ robustness across multiple benchmarks including CIFAR, ImageNet, and medical imaging datasets.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between provable robustness under L2 certificates and empirical robustness against strong L∞ attacks, while maintaining clean accuracy on diverse imaging benchmarks.

Method: HyCAS unifies deterministic spectrally normalized 1-Lipschitz convolutions with two stochastic components: (1) spectral normalized random projection filters, and (2) a randomized attention-noise mechanism, resulting in a ≤2-Lipschitz network that provides formal certificates.

Result: Outperforms prior leading defenses: up to 7.3% certified accuracy improvement (NIH Chest X-ray), up to 3.1% empirical robustness improvement (HAM10000), without sacrificing clean accuracy.

Conclusion: A randomized Lipschitz-constrained architecture can simultaneously improve both certified L2 and empirical L∞ adversarial robustness, enabling safer deployment in high-stakes applications.
...
TL;DR: MIRL is a decoupled RLVR framework that uses mutual information between descriptions and visual inputs as a cheap pre-screening signal to improve budget allocation and provide separate rewards for visual perception, reducing hallucinations in VLMs and achieving 70.22% average accuracy with 25% fewer complete trajectories.


<details>
  <summary>Details</summary>
Motivation: Current RLVR methods waste sampling budget on doomed trajectories due to early visual errors, and sparse rewards cannot distinguish whether failures come from visual perception or reasoning stages.

Method: MIRL uses mutual information (MI) between generated descriptions and visual inputs as a pre-screening signal for intelligent budget allocation via forking, and provides decoupled MI-based rewards for visual perception optimization separately from reasoning rewards.

Result: 70.22% average accuracy on six vision-language reasoning benchmarks, outperforming sampling 16 complete trajectories using only 10 pre-samples with top-6 selection (25% fewer complete trajectories).

Conclusion: MIRL effectively addresses both budget waste and reward blindness in RLVR, enabling more efficient and accurate VLM reasoning with fewer complete sampling trajectories.
...
TL;DR: This paper introduces a method to extract a fundamental matrix from a single motion-blurred image by establishing correspondences between time instances within the camera exposure window, handling time-direction ambiguities, and incorporating uncertainty in smear pattern prediction.


<details>
  <summary>Details</summary>
Motivation: To infer 3D camera motion during exposure from a single motion-blurred image, which is challenging because classic methods (e.g., 8-point algorithm) fail due to per-smear time-direction ambiguity.

Method: The method establishes correspondences between two time instances within the exposure window, modifies fundamental matrix estimation to handle time-direction ambiguous correspondences, and incorporates uncertainty in smear pattern prediction into the estimator's sampling process.

Result: Experiments on synthetic and real-world motion-blur datasets show the approach can estimate the fundamental matrix encoding 3D camera motion from single frames.

Conclusion: The proposed method enables fundamental matrix extraction from single motion-blurred images, with practical applicability demonstrated on motion segmentation.
...
TL;DR: A unified cross-domain transfer learning framework using teacher-student knowledge distillation improves segmentation, classification, and detection across diverse medical imaging datasets.


<details>
  <summary>Details</summary>
Motivation: To leverage knowledge from multiple heterogeneous medical imaging datasets to improve performance across various tasks (segmentation, classification, detection) and enhance robustness to distributional shifts.

Method: Teacher-student paradigm: a joint teacher model aggregates domain-invariant representations from diverse source datasets, while task-specific student models are trained via multi-level knowledge distillation; extended from segmentation to classification and detection tasks.

Result: Consistent improvements over strong baselines across six segmentation benchmarks (MRI & CT), multiple classification datasets (pulmonary disease, dementia), and detection datasets, with enhanced robustness and generalization.

Conclusion: Multi-dataset knowledge distillation is a scalable, task-agnostic approach that effectively improves performance across heterogeneous medical imaging domains.
...
TL;DR: This paper unifies three families of stochastic image enhancement methods (unconditional diffusion models, Ornstein-Uhlenbeck processes, and diffusion bridges) under a common SDE framework, shows no single method dominates via controlled experiments, and releases ItoVision, a modular PyTorch library.


<details>
  <summary>Details</summary>
Motivation: To clarify the relationship between conditional stochastic processes and standard diffusion models for image enhancement, and to unify disparate methods under a common mathematical framework.

Method: Introduces a unified SDE formulation that categorizes stochastic image enhancement methods into three continuous-time process families, and conducts controlled empirical studies using identical architectures and training protocols.

Result: No consistently dominant method was found; specific design choices (drift, diffusion terms, terminal distributions, boundary conditions) were identified as key performance drivers, while schedulers/samplers are orthogonal choices.

Conclusion: The unified framework enables fair comparison and rapid prototyping; the authors release ItoVision library to facilitate this.
...
TL;DR: Omni-Fake is a large-scale multimodal deepfake detection benchmark with 1M+ samples spanning image, audio, video, and talking-head modalities, plus a reinforcement-learning-driven detector (Omni-Fake-R1) that jointly performs detection, localization, and explanation.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake benchmarks are limited by single-modality scope, simplified manipulations, or unrealistic distributions, failing to assess real-world robustness. Omni-Fake addresses this gap with a unified, multi-modal, and realistic benchmark.

Method: The paper constructs Omni-Fake-Set (1M+ samples) and Omni-Fake-OOD (200k+ out-of-distribution samples) across four modalities. On top, Omni-Fake-R1 uses reinforcement learning to adaptively integrate visual and auditory cues, outputting structured decisions, localization, and natural-language explanations.

Result: Extensive experiments demonstrate significant gains in detection accuracy, cross-modal generalization, and explainability over state-of-the-art baselines.

Conclusion: Omni-Fake provides a more realistic and comprehensive benchmark for multimodal deepfake detection, and Omni-Fake-R1 shows that reinforcement-learning-driven fusion improves both performance and explainability.
...
TL;DR: SteeringDiffusion introduces a lightweight, bottlenecked control interface for diffusion models that allows smooth, real-time adjustment of the content-style trade-off at inference without retraining, outperforming LoRA and ControlNet in controllability while keeping the base model frozen.


<details>
  <summary>Details</summary>
Motivation: Existing control methods like LoRA, ControlNet, and rank-1 adapters either lack fine-grained, monotonic control over the content-style trade-off or require retraining; the goal is to provide a simple, interpretable, runtime-adjustable control surface for frozen diffusion models.

Method: A small, prompt-conditioned latent code is learned and projected to FiLM/AdaGN modulation parameters inserted into a frozen U-Net backbone. Zero-initialization ensures identity at zero scale; timestep-aware gating limits modulation to later denoising stages. A single scalar at inference controls the strength continuously.

Result: On Stable Diffusion 1.5 and SDXL, SteeringDiffusion achieves smooth, monotonic content-style trade-offs, outperforming LoRA in controllability and stability under matched parameter budgets, while ControlNet and rank-1 adapters fail to expose a comparable control surface. Inversion-stability diagnostics confirm strong correlations with intervention magnitude.

Conclusion: Steering Bottlenecked Explicit Control (S-BEC) is positioned as a practical, general-purpose control interface for frozen diffusion models, enabling precise runtime steering without retraining.
...
TL;DR: Act2See enables VLMs to actively interleave video frames within text Chain-of-Thought reasoning, allowing dynamic retrieval or generation of visual evidence as needed.


<details>
  <summary>Details</summary>
Motivation: Current VLMs rely on static initial frames for video reasoning, limiting their ability to incorporate dynamic information during reasoning, and existing CoT augmentation methods produce suboptimal quality and cannot synthesize visual evidence for hypothetical/counterfactual scenarios.

Method: Act2See uses Supervised Fine-Tuning (SFT) on a high-quality dataset of reasoning traces with active calls to retrieve existing frames or generate new ones, verified against human-annotated CoTs, to cultivate emergent active perception capabilities at inference time.

Result: Achieves SOTA on VideoEspresso and ViTIB, and outperforms comparable/larger models on Video-MME, EgoNormia, and VCR-Bench.

Conclusion: Act2See advances VLMs by enabling active visual perception for video reasoning, demonstrating that interleaving frame retrieval/generation within CoT reasoning chains significantly improves performance.
...
TL;DR: TRIMMER is a self-supervised reinforcement learning framework for video summarization that uses entropy-based reward functions to capture temporal dynamics, achieving state-of-the-art performance among unsupervised methods and competing with supervised approaches.


<details>
  <summary>Details</summary>
Motivation: Existing video summarization methods rely on expensive manual annotations, struggle to generalize across domains, incur high computational costs, and unsupervised/weakly supervised methods underperform in capturing long-range temporal dependencies and semantic structure.

Method: TRIMMER operates in two stages: (1) self-supervised learning for robust representation learning, followed by (2) spatio-temporal decision making via reinforcement learning guided by information-theoretic (entropy-based) reward functions, computing rewards directly over selected frame indices for efficiency.

Result: Achieves state-of-the-art performance among unsupervised and self-supervised methods, and remains competitive with leading supervised approaches on standard benchmarks.

Conclusion: TRIMMER demonstrates effectiveness for scalable and generalizable video summarization, highlighting that entropy-based objectives can outperform similarity-based ones in capturing temporal dynamics and semantic diversity.
...
TL;DR: VAP is a training-free method for long-form video QA that treats keyframe selection as active perception, using a lightweight text-conditioned video generation model to select frames, achieving SOTA zero-shot results with up to 5.6x better frame efficiency.


<details>
  <summary>Details</summary>
Motivation: Standard uniform frame sampling is expensive and performance plateaus; VLMs need a more effective and efficient method for selecting keyframes in long-form video QA tasks.

Method: VAP leverages active perception theory: keyframe selection is framed as data acquisition where frames differing from model expectations are prioritized. A lightweight text-conditioned video generation model represents prior world knowledge, and the approach is training-free, directly applicable to existing VLMs.

Result: SOTA zero-shot results on EgoSchema, NExT-QA, ActivityNet-QA, IntentQA, CLEVRER; up to 5.6x frame efficiency improvement over GPT-4o, Gemini 1.5 Pro, and LLaVA-OV; stronger reasoning abilities and better question-relevant keyframe selection.

Conclusion: Active perception principles can significantly improve both effectiveness and efficiency of frame selection in long-form video QA, highlighting a promising direction for future VLM research.
...
TL;DR: IMPACT-HOI is a mixed-initiative framework for annotating egocentric procedural videos with structured event graphs for Human-Object Interactions, using trust-calibrated controller and risk-bounded execution to reduce manual effort while maintaining annotation quality.


<details>
  <summary>Details</summary>
Motivation: The need for high-quality structured supervision data for learning robot manipulation from human demonstration, which requires detailed annotations of human-object interactions in procedural videos.

Method: Frames annotation as incremental resolution of partially specified, onset-anchored event states; uses a trust-calibrated controller that selects among direct queries, human-confirmed suggestions, and conservative completions based on annotator behavior and evidence quality; employs a risk-bounded execution protocol with atomic rollback to preserve human-confirmed decisions.

Result: User study with 9 participants shows 13.5% reduction in manual annotation actions, 46.67% event match rate, and zero confirmed-field violations under the studied protocol.

Conclusion: IMPACT-HOI effectively reduces manual annotation effort while maintaining high annotation quality and consistency, supporting the goal of learning robot manipulation from human demonstrations.
...
TL;DR: The paper proposes integrating Fisher Vectors (an orderless encoding method) into hybrid CNN + Vision Transformer architectures for image classification, addressing computational cost issues of Gaussian Mixture Model estimation on large datasets. The method achieves state-of-the-art results on MedMNIST (v2) and competitive results on other medical image datasets.


<details>
  <summary>Details</summary>
Motivation: Orderless encoding methods (like Fisher Vectors) improve CNNs in low-data settings, and hybrid CNN+ViT models address CNN locality bias. However, combining these hybrid models with richer feature representations like Fisher Vectors remains unexplored. Additionally, the computational cost of GMM estimation for Fisher Vectors on large datasets is a limiting factor that needs to be addressed.

Method: The authors introduce Fisher Vectors orderless encoding into hybrid CNN + Vision Transformer architectures. They estimate a Gaussian Mixture Model (GMM) on image features for encoding. To handle large datasets, they propose a method to limit the growth of GMM estimation costs as dataset size increases.

Result: The proposed method outperforms benchmark results on all MedMNIST (v2) datasets and achieves literature-competitive results on Clean-CC-CCII and ISIC2018 medical image datasets.

Conclusion: Integrating Fisher Vectors encoding with hybrid CNN+ViT models yields a versatile architecture suitable for both small and large datasets, particularly effective in medical image classification tasks across varying data scales and modalities.
...
TL;DR: IMPACT-Scribe is a correction-driven framework for dense temporal annotation of procedural videos that iteratively improves human-machine collaboration by leveraging each correction to reduce future effort and enhance labeling quality.


<details>
  <summary>Details</summary>
Motivation: Dense temporal annotation of procedural videos is labor-intensive due to reactive tools where each correction is treated as an isolated edit, wasting information about annotator uncertainty and model reliability.

Method: Combines uncertainty-aware boundary scribble supervision, local proposal modeling, cost-aware query planning, structured propagation, and correction-driven adaptation in a closed-loop design.

Result: Experiments and human study show improved labeling quality per effort, enhanced boundary accuracy, and better human-machine interaction over time.

Conclusion: The closed-loop correction-driven framework effectively reduces annotation burden while improving quality, demonstrating benefits for human-machine collaborative dense labeling.
...
TL;DR: TrajRAG is a retrieval-augmented generation framework that accumulates geometric-semantic experiences from past navigation episodes using a topo-polar trajectory representation and hierarchical chunking, improving zero-shot Object Goal Navigation across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot ObjectNav methods rely on internet-scale text knowledge from large models (LLMs/VLMs) that lacks embodied 3D experience, and they discard episodic observations collected during navigation, preventing lifelong experience accumulation.

Method: TrajRAG incrementally accumulates episodic observations using a topological-polar (topo-polar) trajectory representation to compactly encode spatial layouts and semantics, employs hierarchical chunking for coarse-to-fine retrieval, and generates trajectory hypotheses from candidate frontiers to query similar past experiences for guiding large-model reasoning.

Result: Experiments on MP3D, HM3D-v1, and HM3D-v2 demonstrate that TrajRAG effectively retrieves relevant geometric-semantic experiences and improves zero-shot ObjectNav performance.

Conclusion: TrajRAG enables lifelong navigation experience accumulation through retrieval-augmented generation, bridging the gap between internet-scale knowledge and embodied 3D experience for zero-shot ObjectNav.
...
TL;DR: This paper introduces a fairness-aware active learning framework for medical image segmentation that uses a Weighted Entropy selection strategy to reduce performance disparities across demographic groups, achieving 75-86% reduction in disparity compared to standard entropy methods.


<details>
  <summary>Details</summary>
Motivation: Standard uncertainty-based active learning methods for medical image segmentation focus on maximizing performance metrics while ignoring fairness across groups with sensitive attributes. Fair active learning has been explored in classification but not in medical image segmentation.

Method: A Weighted Entropy selection strategy that modulates uncertainty based on current group-specific performance estimates on the labeled set, plus a masked, scaled entropy restricted to the region of interest to decouple epistemic uncertainty from anatomical volume variances.

Result: The method reduced disparity by 75% (strong bias) and 86% (weak bias) relative to standard entropy at the final budget, and consistently achieved the highest equity-scaled performance across both balanced and imbalanced initial labeled sets.

Conclusion: This is among the first studies on fair active learning for medical image segmentation, offering an efficient strategy to train more equitable models in resource-constrained environments.
...
TL;DR: This paper reframes attention as an MLP with dynamically predicted parameters, and shows this can achieve Transformer-level global modeling with linear complexity instead of quadratic.


<details>
  <summary>Details</summary>
Motivation: To address the quadratic computational cost of attention in Transformers by offering a new theoretical perspective and designing a linear-complexity alternative.

Method: Mathematically reframes attention as an MLP with dynamically predicted parameters; designs various dynamic parameter prediction strategies integrated into standard network layers.

Result: Dynamic parameterization serves as a highly effective, linear-complexity alternative to explicit attention, validated through extensive empirical studies on vision models.

Conclusion: Attention's global modeling power can be achieved via implicit dynamic parameterization with linear complexity, opening new pathways for efficient sequence modeling.
...
TL;DR: DUNE is a dual-branch unlearnable ensemble method that optimizes perturbations in both spatial and color domains to improve robustness against defenses, outperforming 12 SOTA schemes across 7 defense methods on CIFAR-10 and ImageNet.


<details>
  <summary>Details</summary>
Motivation: Existing unlearnable examples suffer from limited robustness against advanced defenses due to heuristic design or narrowly scoped domain perturbations.

Method: DUNE uses a dual-branch framework to separately optimize perturbations in spatial and color domains, establishing a mapping between perturbations and shift-induced labels. It also employs an unlearnability-enhancing ensemble strategy aggregating diverse pre-trained models during optimization.

Result: DUNE outperforms 12 SOTA UE schemes under 7 mainstream defenses, achieving a lower average test accuracy of 14.95% to 50.82% on CIFAR-10 and ImageNet.

Conclusion: DUNE effectively improves robustness of unlearnable examples against advanced defenses by extending perturbation domains to both spatial and color spaces with ensemble optimization.
...
TL;DR: SignVerse-2M is a large-scale multilingual sign language dataset that converts raw videos into unified 2D pose sequences (via DWPose) across 25+ sign languages, enabling pose-native modeling for open-world recognition, translation, and generation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing sign language datasets provide only raw video-text alignment (lab settings), and lack a unified pose-native interface for robust open-world recognition (RGB models are background/clothing-dependent) and compatibility with modern pose-driven generation models like DWPose.

Method: SignVerse-2M applies DWPose in a unified preprocessing pipeline to convert multilingual sign language videos into 2D pose sequences (~2M clips, 25+ languages), preserving real-world recording diversity while eliminating appearance variation. A SignDW Transformer baseline is also proposed.

Result: A consolidated corpus of ~2 million pose sequences covering 25+ sign languages, with demonstrated feasibility for multilingual pose-space modeling via the SignDW Transformer baseline.

Conclusion: SignVerse-2M fills the gap in pose-native sign language resources, enabling robust open-world modeling and integration with modern pose-driven pipelines, while limitations are acknowledged.
...
TL;DR: MotionCache is a motion-aware cache framework for autoregressive video generation that uses inter-frame differences to dynamically adjust denoising step frequencies per pixel, achieving 6.28x and 1.64x speedups on SkyReels-V2 and MAGI-1 with negligible quality loss.


<details>
  <summary>Details</summary>
Motivation: Existing cache reuse strategies for autoregressive video generation use coarse-grained chunk-level skipping that fails to capture fine-grained pixel dynamics, causing error accumulation in high-motion regions or unnecessary computation in static regions.

Method: MotionCache uses inter-frame differences as a lightweight proxy for pixel-level motion characteristics, employing a coarse-to-fine strategy: an initial warm-up phase for semantic coherence, followed by motion-weighted cache reuse that dynamically adjusts update frequencies per token.

Result: Achieves 6.28x speedup on SkyReels-V2 and 1.64x on MAGI-1 with only 1% and 0.01% VBench quality degradation respectively.

Conclusion: Motion-aware cache skipping is more effective than uniform chunk-level skipping; pixel-level motion guidance enables significant acceleration while preserving generation quality.
...
TL;DR: GEASS is a training-free module that selectively gates self-generated captions by query-level confidence and entropy, reducing object hallucination in VLMs without requiring additional training.


<details>
  <summary>Details</summary>
Motivation: VLMs suffer from object hallucination; prior work treats self-generated captions as always helpful, but the authors find naive caption embedding can actually degrade performance (e.g., 10-point drop on HallusionBench). Caption usefulness is per-query, not per-corpus.

Method: GEASS (Gated Evidence-Aware Selective Steering): a training-free module that (1) gates caption input based on the clean path's confidence, (2) weights it by entropy reduction, and (3) raises evidence bar when pathways disagree. Requires only two extra forward passes per query.

Result: Consistent improvement over vanilla inference and contrastive decoding across POPE and HallusionBench benchmarks, tested on four different VLMs.

Conclusion: Caption usefulness is query-dependent, not uniformly positive. Selective gating of self-generated captions effectively mitigates object hallucination without training.
...
TL;DR: GLMap introduces a multi-scale Gaussian-Language Map that combines explicit 3D geometry with multi-scale semantic descriptions (instance and region) via a dual-modality interface of natural language and 3D Gaussians, enabling efficient incremental construction without gradient optimization and achieving strong results on ObjectNav, InstNav, and SQA tasks.


<details>
  <summary>Details</summary>
Motivation: Existing semantic mapping methods trade off between explicit geometry and multi-scale semantics, and lack a native interface for large models, requiring additional feature projection training for semantic alignment.

Method: Three key designs: (1) explicit geometry, (2) multi-scale semantics (instance and region), (3) dual-modality interface storing both natural language descriptions and 3D Gaussian representations. A Gaussian Estimator analytically derives Gaussian parameters from dense point clouds without gradient-based optimization.

Result: Experiments on ObjectNav, InstNav, and SQA tasks show GLMap effectively enhances target navigation and contextual reasoning while remaining compatible with large-model-based methods in a zero-shot manner.

Conclusion: GLMap successfully bridges 3D geometric mapping with language semantics, providing a native interface for large models without additional training, enabling zero-shot compatibility and efficient incremental construction.
...
TL;DR: ATMask introduces an adaptive masking strategy for self-supervised learning on dental CBCT volumes, selectively masking texturally complex regions to improve representation learning, validated on a large-scale dataset of 6,314 scans across three downstream tasks.


<details>
  <summary>Details</summary>
Motivation: Standard MIM uses random masking which fails to prioritize diagnostically critical regions (e.g., pathological changes, anatomical boundaries) in dental CBCT volumes, limiting AI model robustness given the scarcity of large annotated datasets.

Method: ATMask computes inter-slice texture variation maps to identify high-complexity regions, then selectively masks those areas during SSL pre-training instead of random masking, without requiring computationally expensive attention modules.

Result: ATMask outperforms random masking and other SSL baselines on three downstream dental CBCT tasks, achieving more data-efficient representation learning. The first large-scale CBCT dataset (6,314 scans) is also contributed.

Conclusion: Adaptive masking based on texture complexity is an effective and efficient strategy for dental CBCT self-supervised learning, and the released dataset can facilitate future research in dental AI.
...
TL;DR: This paper proposes a holistic multi-axis optimization framework for Vision Transformers that jointly optimizes architecture (via NAS), token count (via token merging), and bit-width (via fp16 mixed-precision), achieving >10x improvements in throughput, parameters, FLOPs, and energy on semiconductor defect classification while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: ViTs are powerful but impractical for resource-constrained industrial deployment due to high computational cost, memory, and energy consumption. Existing optimization methods target only a single axis (architecture, token, or bit-width), limiting overall deployment gains.

Method: A holistic framework jointly optimizing three complementary axes: (1) architecture via Neural Architecture Search (AutoFormer), (2) token efficiency via token merging (ToMe), and (3) bit-width via fp16 mixed-precision inference. Applied on DeiT-B/16 baseline and evaluated on ImageNet-1K and a real-world 3D X-ray semiconductor defect dataset.

Result: Over 10x improvement in throughput, parameter count, FLOPs, and energy consumption while maintaining required accuracy on the industrial semiconductor defect classification task.

Conclusion: This is among the earliest works jointly optimizing architecture, token, and bit-width in ViTs, and the first resource-efficient deployment-focused study tailored to semiconductor manufacturing.
...
TL;DR: MOC-3D improves text-to-3D generation by addressing macro-topological inconsistency (Janus problem) and micro-geometric discontinuity using a Semantic View-Order Constraint Module (CLIP-based monotonicity rank) and a Manifold-based Feature Continuity Module (SPD Riemannian metric).


<details>
  <summary>Details</summary>
Motivation: Existing SDS-based text-to-3D methods suffer from view bias of 2D priors and mode-seeking ambiguity from high CFG, leading to macro-topological inconsistency (e.g., Janus problem) and micro-geometric discontinuity.

Method: Built on ScaleDreamer, MOC-3D introduces two modules: (1) Semantic View-Order Constraint Module using CLIP prior knowledge to impose Monotonicity Rank Constraint on semantic scores across views; (2) Manifold-based Feature Continuity Module using Riemannian Metric on SPD manifold to measure feature distribution distances for smooth micro-texture evolution.

Result: The macro-micro synergistic optimization simultaneously improves macro-structural consistency and micro-detail continuity in 3D generation.

Conclusion: MOC-3D effectively mitigates both the Janus problem and geometric discontinuity by enforcing view-order semantic consistency and manifold-based feature smoothness.
...
TL;DR: This paper introduces ProfileSynth, a synthetic dataset of extreme-profile face images, and a profile-specific FLAME regression baseline for single-image 3D face reconstruction from lateral views, aiming to enable non-invasive cephalometric analysis for orthodontics.


<details>
  <summary>Details</summary>
Motivation: Traditional cephalometric analysis relies on lateral X-rays, but frequent radiation exposure is impractical. Existing 3D face reconstruction methods fail on extreme profile views (yaw ~90°) due to occlusion and lack of training data, while current landmark detection approaches use only 2D features, missing crucial 3D facial geometry for accurate diagnosis.

Method: The authors create ProfileSynth by sampling FLAME shape/pose parameters at extreme yaw angles, then generating photorealistic profile images via a diffusion model conditioned on depth and normal maps. They also propose a profile-specific FLAME regression baseline with visibility-aware jawline regularization.

Result: A synthetic dataset (ProfileSynth) for extreme-profile face reconstruction and a profile-specific regression baseline that enables "profile × 3DMM" reconstruction from single lateral images.

Conclusion: This work provides a practical baseline for 3D face reconstruction from profile views and a promising foundation for non-invasive cephalometric analysis from lateral RGB images, potentially replacing X-ray-based orthodontic diagnosis.
...
TL;DR: Proposes a cross-sample semantic propagation (CSP) framework for point cloud self-supervised learning that uses a state-space model to serialize batch samples, enabling cross-sample semantic consistency, plus asymmetric semantic preservation distillation (SPD) for stable single-scene finetuning.


<details>
  <summary>Details</summary>
Motivation: Existing PC-SSL methods suffer from sample-independent modeling, which limits consistent semantic representations across scenes and hinders building a unified, transferable semantic space.

Method: (1) CSP: serializes batch samples into continuous input, processed by a state-space model for semantic state propagation. (2) SPD: asymmetric semantic preservation distillation during finetuning with heterogeneous input and semantic feature alignment to eliminate batch dependency issues.

Result: Outperforms state-of-the-art methods on multiple benchmark datasets in both performance and semantic consistency.

Conclusion: The proposed CSP+SPD framework effectively resolves cross-sample semantic inconsistency in PC-SSL, achieving superior generalization and robust single-scene performance.
...
TL;DR: TrajShield is a training-free, inference-time defense framework for Text-to-Video models that handles explicit unsafe prompts, jailbreak attacks, and temporally emergent risks by simulating prompt trajectories, localizing causal risk origins, and applying minimally invasive rewrites, achieving 52.44% average ASR reduction.


<details>
  <summary>Details</summary>
Motivation: Existing prompt-level defenses for T2V models are inherited from text-to-image safety, focusing only on lexical surface patterns, making them vulnerable to jailbreak attacks. Additionally, T2V introduces a unique challenge—temporally emergent risk—where benign-seeming prompts produce unsafe content through temporal extrapolation, which prior work has overlooked.

Method: TrajShield reformulates T2V safety as causal intervention in a temporally structured semantic space. It (1) simulates the implied trajectory of a prompt, (2) localizes the causal origin of potential risk, and (3) applies a minimally invasive rewrite that neutralizes risk while preserving safety-irrelevant semantics—all without additional training.

Result: Evaluated on T2VSafetyBench across 14 safety categories and multiple T2V backbones, TrajShield achieves state-of-the-art defensive performance with an average Attack Success Rate (ASR) reduction of 52.44%, while maintaining high semantic fidelity.

Conclusion: TrajShield provides a unified, training-free defense that addresses three types of T2V safety risks (explicit prompts, jailbreak attacks, temporally emergent risks) and substantially outperforms existing defenses.
...
TL;DR: MedScribe is a hypothesis-driven framework for radiology report generation that reformulates the task as iterative evidence acquisition via pathology-specific diagnostic tools, improving clinical accuracy and reducing hallucinations in 3D CT imaging without task-specific fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs use global embedding compression of volumetric data, leading to hallucinated findings and limited anatomical grounding in 3D CT imaging.

Method: Reformulates report generation as a sequential decision process where an LLM invokes pathology-specific diagnostic tools to extract localized volumetric features, then queries a multidimensional retrieval space aligned with pathology-specific textual evidence.

Result: Outperforms state-of-the-art 2D and 3D VLMs on CT-RATE and RadChestCT datasets in clinical accuracy, factual consistency, and interpretability without task-specific fine-tuning.

Conclusion: Hypothesis-driven reasoning (iterative evidence acquisition) is valuable for reliable medical image reporting, reducing unsupported claims and improving grounding.
...
TL;DR: Embody4D is a video-to-video world model that generates multi-view 3D-consistent videos from monocular input for embodied robotic scenarios, addressing data scarcity, geometric consistency, and manipulation fidelity challenges.


<details>
  <summary>Details</summary>
Motivation: Existing embodied world models are limited to 2D representations and lack multi-view information essential for spatial reasoning in robotics, while data scarcity, spatiotemporal inconsistency in 3D geometry, and hallucination of manipulation details prevent progress.

Method: (1) 3D-aware compositional synthesis pipeline to create heterogeneous paired multi-view datasets, (2) adaptive noise injection strategy using confidence disparities to enforce geometric stability, (3) interaction-aware attention mechanism to ensure manipulation fidelity.

Result: State-of-the-art performance in synthesizing high-fidelity, view-consistent videos for downstream robotic planning and learning.

Conclusion: Embody4D serves as a robust world model that overcomes key challenges in 3D embodied video synthesis, enabling downstream robotic applications.
...
TL;DR: This paper explores using DCGAN-based generative augmentation to improve classifier performance in low-resource vision tasks (Bangla handwritten characters and chest X-rays), showing that adding synthetic data improves diversity and accuracy, while also discussing stability techniques, evaluation metrics, and privacy concerns.


<details>
  <summary>Details</summary>
Motivation: To address data scarcity in vision tasks by leveraging GAN-based generative augmentation, particularly in low-resource domains like Bangla handwritten character recognition and medical chest X-ray imaging.

Method: DCGAN-style models trained at 64x64 resolution; evaluation via IS, FID, t-SNE/UMAP; classifier training on real vs. real+synthetic data; ablations on synthetic-to-real ratios and sample filtering; stability enhancements like gradient-penalized objectives and spectral normalization.

Result: Generative augmentation improves sample diversity and yields consistent gains in classifier performance under limited-data regimes.

Conclusion: The proposed protocol is simple, reproducible, and provides a strong baseline for applying generative augmentation to resource-constrained imaging tasks, while also noting evaluation caveats for medical images and discussing licensing/privacy risks.
...
TL;DR: This paper introduces a hybrid two-channel visual telemetry system for bandwidth-constrained robotics, combining a continuous low-bitrate video stream (x265/HEVC) with selectively transmitted high-detail JPEG still ROIs, and establishes a methodological foundation for future JPEG AI integration.


<details>
  <summary>Details</summary>
Motivation: Bandwidth-constrained robotic and surveillance systems using a single compressed video stream face a trade-off: low-bitrate video preserves motion and coarse context but loses fine detail needed for reliable object recognition and decision-making.

Method: The paper formalizes a two-channel visual telemetry scheme: a continuous low-bitrate x265/HEVC video stream for scene awareness, augmented by selectively triggered high-detail JPEG still ROIs for close-up identification. It defines a bitrate-constrained information selection problem and proposes an experimental protocol comparing video-only vs. hybrid schemes under matched total communication budgets using UAV-oriented datasets.

Result: This paper does not present superiority results for a new codec; rather, it establishes the hybrid transmission paradigm using a practical codec stack and lays the methodological groundwork for a second-stage investigation of JPEG AI as the semantic still-image channel.

Conclusion: The hybrid two-channel approach is feasible and reproducible, offering a practical foundation for future work where JPEG AI could serve as the semantic still-image channel to improve object-level classification refinement in bandwidth-constrained robotic vision.
...
TL;DR: CSteer is a training-free method that guides Large Multimodal Models to perform multi-region visual referring by steering latent representations with pre-computed contextual vectors, achieving SOTA without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing LMMs struggle with region-level visual perception guided by visual prompts, especially when handling multiple regions simultaneously or requiring global context for precise referring. Fine-tuning approaches are expensive and inflexible.

Method: CSteer pre-computes contextual vectors that encode visual referring behaviors (region differentiation, global attention), then edits model representations during inference via steering—requiring no training or architectural changes.

Result: General LMMs with CSteer outperform existing tailored referring LMMs on multiple datasets, achieving new state-of-the-art results in most cases.

Conclusion: Training-free latent steering is a promising and effective paradigm for enhancing LMMs' multi-region visual referring capabilities, eliminating the need for costly fine-tuning.
...
TL;DR: GeoSAE uses geometry-guided sparse autoencoders to extract interpretable, age-deconfounded clinical features from brain MRI foundation models, achieving high MCI-to-AD conversion prediction with only 2% of embedding dimensions and strong cross-cohort replication.


<details>
  <summary>Details</summary>
Motivation: Brain MRI foundation models encode rich anatomical information, but interpreting what clinical information they capture remains difficult. Standard SAEs suffer from feature collapse in deep layers, and aging confounds clinical variables in Alzheimer's research, making naive annotation unreliable.

Method: GeoSAE leverages the foundation model's learned manifold structure to prevent feature collapse via geometry guidance, and annotates surviving features using age-deconfounded partial correlations. Applied to ~14k T1-weighted MRI scans from ADNI and AIBL datasets.

Result: GeoSAE identifies a compact interpretable feature set that predicts MCI-to-AD conversion with AUC 0.746 using only 2% of embedding dimensions. Comorbidity-annotated features achieve chance-level performance. Features replicate across cohorts (r=0.97) and localize to regions consistent with Braak staging.

Conclusion: Geometry-guided SAEs can extract interpretable, reproducible biomarkers from frozen brain MRI foundation models, overcoming feature collapse and age confounding in Alzheimer's disease research.
...
TL;DR: DADD is a diffusion-based framework that disentangles patient anatomy from disease severity to synthesize longitudinal endoscopic images of ulcerative colitis, enabling controllable disease stage transitions while preserving individual anatomical structures.


<details>
  <summary>Details</summary>
Motivation: Synthesizing medical images at controllable disease stages is challenging because pathological textures and anatomical features are entangled, making it difficult to generate realistic longitudinal images that preserve patient-specific anatomy while varying disease severity.

Method: DADD conditions a latent diffusion model on two embeddings: a pretrained image encoder (patient anatomy) and an ordinal embedder (disease severity). A Feature Purifier uses cross-attention to remove disease-correlated features from anatomical representations. A Triple-Pathway Cross-Attention mechanism with resolution-dependent routing gates injects purified anatomy and disease tokens into the U-Net. Delta Steering provides training-free directional control for disease transitions at inference.

Result: Validated on the LIMUC dataset, DADD produces high-fidelity images across all severity levels and effectively rebalances skewed class distributions, improving downstream classification performance.

Conclusion: The proposed disentanglement approach successfully enables controllable disease progression synthesis in medical imaging, with practical benefits for data augmentation and class imbalance mitigation.
...
TL;DR: This paper shows that dual-pixel (DP) sensors can resolve the unknown scale ambiguity in multi-view 3D reconstruction without needing a reference object, by using defocus blur information from DP images to estimate absolute scale.


<details>
  <summary>Details</summary>
Motivation: Multi-view 3D reconstruction suffers from unknown scale ambiguity, typically requiring a reference object of known size. The authors aim to eliminate this need using dual-pixel sensors.

Method: A linear method estimates absolute scale from defocus blur in DP images paired with scale-ambiguous depth maps, followed by intensity-based optimization that aligns left and right DP images using cross-view blur kernels.

Result: The approach is effective across diverse scenes captured with different cameras and lenses, as demonstrated experimentally.

Conclusion: DP sensors can automatically resolve scale ambiguity in multi-view 3D reconstruction without a reference object or prior calibration.
...
TL;DR: Proposes a method for high-fidelity human avatar reconstruction from multi-view video using local linear blendshapes, optimized for mobile devices with 120 FPS at 2K resolution via WebGPU.


<details>
  <summary>Details</summary>
Motivation: Existing Gaussian-based full-body avatars achieve high quality but require heavy computation for pose-dependent appearance, making mobile deployment difficult. Recent distillation methods run on mobile but lose detail due to global linear modeling.

Method: Uses local linear blendshapes in small body parts (instead of global blendshapes) to capture pose-dependent Gaussian attributes, leverages local correlations among nearby Gaussians. Proposes minimal blendshape representation by removing blendshapes for low-variance Gaussians. End-to-end training without pretrained models. Implemented with WebGPU for cross-device deployment.

Result: Achieves high-quality human avatars with better detail than prior mobile methods. Runs at 120 FPS at 2K resolution on mobile devices.

Conclusion: Local linear blendshapes effectively balance quality and efficiency for mobile avatar rendering, and the minimal blendshape approach reduces computation/model size without significant quality loss.
...
TL;DR: DSCache is a training-free cache construction mechanism that adapts pretrained offline VideoVLLMs to streaming video understanding by maintaining a cumulative past KV cache alongside a decoupled instant cache, with position-agnostic encoding to handle unbounded streams.


<details>
  <summary>Details</summary>
Motivation: Existing streaming VideoVLLMs fail to scale to unbounded video streams, and prior work focuses on cache reuse strategies while leaving the impact of cache construction underexplored.

Method: DSCache maintains a cumulative past KV cache while constructing a separate decoupled instant cache for recent inputs, and incorporates position-agnostic encoding for position extrapolation beyond training length.

Result: State-of-the-art performance on Streaming Video QA benchmarks with average 2.5% accuracy gains over prior methods.

Conclusion: DSCache effectively enables pretrained offline models to handle unbounded video streams without additional training.
...
TL;DR: BadmintonGRF is a multimodal dataset combining synchronized multi-view video, force plates, and motion capture for badminton, with two-tier access enabling pose-to-GRF estimation benchmarks.


<details>
  <summary>Details</summary>
Motivation: Multimodal resources for non-periodic court sports with laboratory-grade sensing are scarce, limiting markerless load estimation in realistic training settings.

Method: Records eight synchronized RGB views (~120 FPS), four Kistler force plates, and Vicon motion capture; alignment uses human-verified events, automated QA, per-camera time offsets with uncertainty metadata. Two-tier distribution: Tier 1 (pose, GRF, metadata) and Tier 2 (raw RGB, C3D).

Result: 17,425 impact-segment archives from 156 instrumented trials across 10 subjects; 12,867 view-specific instances and 1,732 unique impacts after deduplication. Raw multi-view RGB exceeds 1 TB.

Conclusion: First public badminton corpus combining this sensing layout with audited video–GRF alignment for impact-centric GRF estimation, with preprocessing code, splits, baselines, and fusion provided.
...
TL;DR: Chart-FR1 is a new focus-driven chart reasoning model that uses Focus-CoT (visual focusing chain-of-thought), Focus-GRPO (reinforcement learning with information-efficiency reward and adaptive KL penalty) to tackle challenges in high information density (HID) charts, outperforming SOTA MLLMs on multiple benchmarks including a new HID-Chart benchmark.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with high information density (HID) charts due to three challenges: limited fine-grained perception (missing critical visual cues), redundant/noisy visual information harming reasoning, and lack of adaptive deep reasoning relative to visual information amount.

Method: Proposes Chart-FR1 with two key components: (1) Focus-CoT, a visual focusing chain-of-thought that explicitly links reasoning steps to key visual cues (local image regions, OCR signals); (2) Focus-GRPO, a focus-driven reinforcement learning algorithm with an information-efficiency reward (to compress redundant visual info) and an adaptive KL penalty mechanism (to control reasoning depth). Also introduces HID-Chart benchmark with information-density metric.

Result: Chart-FR1 outperforms state-of-the-art MLLMs on multiple chart understanding and reasoning benchmarks.

Conclusion: The focus-driven approach effectively addresses the three key challenges of HID chart reasoning through better fine-grained perception, efficient focusing, and adaptive reasoning depth.
...
TL;DR: AFFormer is a Transformer-based framework that enhances the robustness of V2X cooperative perception under channel impairments (noise, fading, interference) using multi-agent/temporal aggregation, dual spatial attention, uncertainty-guided fusion, and teacher-student knowledge distillation, outperforming existing methods on V2XSet and DAIR-V2X datasets.


<details>
  <summary>Details</summary>
Motivation: Cooperative perception enhances 3D detection for autonomous vehicles via V2X communication, but is vulnerable to channel impairments (noise, fading, interference). The paper aims to improve the robustness of cooperative perception under realistic communication conditions.

Method: Proposes AFFormer (Adaptive Feature Fusion Transformer) with three key modules: (1) Multi-Agent and Temporal Aggregation for context-aware fusion across agents and time, (2) Dual Spatial Attention for efficient spatial dependency modeling, (3) Uncertainty-Guided Fusion for entropy-driven feature refinement. Additionally uses teacher-student knowledge distillation to align with reliable early-collaboration supervision.

Result: Validated on V2XSet and DAIR-V2X datasets; consistently outperforms existing methods under both ideal and impaired communication conditions, demonstrating improved robustness to feature degradation while maintaining competitive efficiency-accuracy trade-off.

Conclusion: AFFormer effectively mitigates communication-induced feature corruption in V2X cooperative perception, offering a robust solution for reliable intelligent transportation systems.
...
TL;DR: M²-REPA is a representation alignment method for multi-modal video generation that aligns diffusion model features with expert foundation models across modalities (RGB, depth, mask), using decoupling regularization and alignment loss to improve visual quality and long-term consistency.


<details>
  <summary>Details</summary>
Motivation: Existing multi-modal world models fail to fully exploit rich priors from existing foundation models that are trained on different modality spaces, each acting as complementary "experts" with distinct domain-specific knowledge.

Method: Decouples modality-specific features from the diffusion model's intermediate representations and aligns each with its corresponding expert foundation model using two objectives: (1) multi-modal representation alignment loss for feature-to-expert matching, and (2) modality-specific decoupling regularization to encourage complementarity across modalities.

Result: The method significantly outperforms baselines in visual quality and long-term consistency.

Conclusion: Jointly optimizing with alignment loss and decoupling regularization enables full exploitation of priors from multiple foundation models, leading to better multi-modal video generation.
...
TL;DR: GeoLaneRep is a behavior-grounded lane representation learning framework for traffic digital twins that jointly encodes static lane geometry, observed vehicle trajectories, and operational descriptors into a shared semantic embedding, achieving superior performance in cross-camera matching, anomaly detection, and lane geometry synthesis.


<details>
  <summary>Details</summary>
Motivation: Existing traffic digital twins rely on static geometric representations that fail to capture dynamic functional semantics (e.g., how a lane operates under complex traffic conditions), limiting behavior-aware reasoning capabilities.

Method: GeoLaneRep uses a joint encoder trained with a multi-objective loss combining (1) contrastive cross-camera alignment, (2) auxiliary role supervision, and (3) temporal anomaly detection, to embed lane geometry, trajectories, and operational descriptors into a shared semantic space.

Result: Across 16 cameras and 132 lanes: 0.004 lateral-rank error, 1.000 edge-role F1 (zero-shot cross-camera matching), 0.991 AUROC (anomaly detection), and 87.9% specification accuracy for diffusion-based lane geometry synthesis across 38 lane groups.

Conclusion: GeoLaneRep provides a semantic interface between roadside observations and downstream digital twin tasks, enabling cross-camera transfer, behavior-aware monitoring, and goal-directed lane synthesis.
...
TL;DR: The paper introduces SurgCheck, a diagnostic benchmark to quantify how much surgical VQA models rely on linguistic shortcuts rather than true visual understanding, revealing that strong benchmark performance often masks reliance on language biases.


<details>
  <summary>Details</summary>
Motivation: Existing surgical VQA datasets contain linguistic shortcuts where question phrasing implicitly constrains answer spaces, making it unclear whether reported performance reflects genuine visual understanding or shortcut reliance.

Method: SurgCheck uses a paired-question design: each surgical frame has an original question (with entity names) and a less-biased counterpart (without entity names) but with identical visual content and answers. Four grounding cues (bounding box, arrow, spatial position, periphrasis) keep less-biased questions well-defined. An LLM-as-a-judge protocol evaluates open-ended zero-shot responses.

Result: All five VLMs showed consistent performance degradation on less-biased questions despite identical visual inputs. Text-only ablation showed minimal performance drops for action and target prediction, indicating these tasks are driven by linguistic shortcuts rather than visual reasoning.

Conclusion: SurgCheck provides a controlled diagnostic framework exposing failure modes masked by linguistic bias; strong benchmark performance does not imply faithful visual understanding, highlighting the need for bias-aware evaluation in surgical VQA.
...
TL;DR: EAPFusion is a novel infrared-visible image fusion method that uses self-evolving intrinsic priors to dynamically generate convolutional kernels, replacing static pre-trained filters, and employs a channel-level fusion module for cross-modal complementarity, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing fusion methods rely on static trained weights that cannot adapt to scene-specific content at inference time, and suffer from granularity mismatch when coarse auxiliary semantics are injected, making it difficult to simultaneously highlight targets and preserve details.

Method: EAPFusion maintains a compact set of intrinsic priors that are progressively updated across scales. These evolved priors dynamically generate convolutional kernels (prior-conditioned dynamic convolution). A channel-level fusion module shuffles and interleaves infrared and visible channels with local channel mixing to boost cross-modal complementarity.

Result: Achieves state-of-the-art quantitative and qualitative fusion results on different datasets, including cross-dataset evaluation, and consistently boosts downstream semantic segmentation performance.

Conclusion: Using self-evolving intrinsic priors instead of external auxiliary models enables instance-adaptive fusion that simultaneously highlights thermal targets and preserves visible textures, demonstrating strong generalization and downstream task benefits.
...
TL;DR: SimPB++ proposes an end-to-end hybrid decoder model that simultaneously detects 2D objects in perspective view and 3D objects in Bird's Eye View from multiple cameras, achieving state-of-the-art performance on nuScenes and long-range detection on Argoverse2.


<details>
  <summary>Details</summary>
Motivation: Existing two-stage pipelines only use 2D results as a one-time cue for 3D detection, lacking deep interaction between the two tasks. Additionally, 3D annotations are expensive, motivating a method that can leverage mixed supervision (2D-only + fully annotated data).

Method: SimPB++ uses a hybrid decoder architecture with interactive multi-view 2D and 3D decoders. Key modules include: Dynamic Query Allocation (adaptively assigns 2D queries to 3D candidates), Adaptive Query Aggregation (refines 3D representations using multi-view 2D features), Query-group Attention (intra-group communication for multi-view 2D), Crop-and-Scale (long-range perception), and Propagating Denoising with an auxiliary RoI detector.

Result: State-of-the-art performance on nuScenes for both 2D and 3D detection tasks, and strong long-range detection up to 150m on Argoverse2.

Conclusion: SimPB++ effectively unifies 2D and 3D detection in an end-to-end framework with deep cyclic interaction, reduces reliance on expensive 3D labels via mixed supervision, and achieves strong performance especially for long-range scenarios.
...
TL;DR: CADFS is a data-centric framework that uses FeatureScript representation and a dataset of 450k real-world CAD models (15 operations) to fine-tune VLMs, achieving state-of-the-art text-conditioned CAD generation and image-based reconstruction with more complex and diverse designs.


<details>
  <summary>Details</summary>
Motivation: Existing generative CAD systems are limited to simple sketch-extrude operations due to simplified representations and insufficient datasets, motivating the need for a framework that can handle more complex, real-world CAD design histories.

Method: The authors introduce a FeatureScript-based representation, construct a 450k CAD model dataset via a reconstruction pipeline with multimodal annotations, and fine-tune a VLM on this data for text-conditioned generation and image-based reconstruction.

Result: State-of-the-art results in both text-conditioned CAD generation and image-based reconstruction, producing more accurate, diverse, and feature-rich designs compared to prior frameworks. All components (FeatureScript, extended operations, aligned textual descriptions) significantly improve performance.

Conclusion: The CADFS framework substantially broadens the complexity and realism achievable in generative CAD, offering a new benchmark and dataset for the field.
...
TL;DR: CASA resolves LoRA incompatibility with distilled video diffusion models via spectral arbitration in weight space.


<details>
  <summary>Details</summary>
Motivation: Existing LoRAs fail when applied to step-distilled or causally distilled video diffusion models due to weight space mismatches, causing style degradation and structural collapse.

Method: CASA identifies spectral interference in shared functional clusters and uses a data-free spectral arbitration mechanism to reconcile routing conflicts between the target model's manifold and LoRA alignment.

Result: CASA effectively mitigates artifacts and revives LoRA functionality across various distilled video diffusion model variants.

Conclusion: Spectral analysis of weight spaces enables practical, data-free adaptation of LoRAs to distilled video diffusion models.
...
TL;DR: ProtoFair is a fairness-aware contrastive loss that improves representation fairness by pulling together cross-group samples with similar content, without modifying existing SSL objectives.


<details>
  <summary>Details</summary>
Motivation: Self-supervised learning captures demographic biases from training data, and existing fairness methods redesign the SSL objective, limiting portability across different SSL frameworks.

Method: ProtoFair uses unsupervised prototype clustering to identify pseudo-counterfactual pairs (same cluster, different sensitive groups) and pulls them together in embedding space to learn invariant representations.

Result: Experiments on CelebA and UTKFace show consistent fairness improvements while maintaining competitive accuracy.

Conclusion: ProtoFair integrates seamlessly with SSL objectives like SimCLR and SupCon, requiring only sensitive attribute annotations, and offers a portable fairness solution for SSL.
...
TL;DR: This paper proposes a framework to systematically evaluate 3D Gaussian Splatting (3DGS) for reconstructing safety-critical scenes involving vehicles and pedestrians, addressing fidelity gaps in industrial ADS pipelines.


<details>
  <summary>Details</summary>
Motivation: Field data collection lacks completeness for rare safety-related scenarios needed for ADS development. While 3DGS shows promise for scene reconstruction, its industrial fidelity evaluation remains underexplored, particularly in dynamic, uncontrolled environments with limited viewpoints and occlusions.

Method: The authors propose and implement a framework to systematically analyze capabilities and limitations of 3DGS for reconstructing safety-related scenes, focusing on reconstruction quality of vehicles and pedestrians from multiple novel viewpoints (lateral and longitudinal).

Result: The analysis provides industry insights into fidelity degradation of 3DGS reconstructions across different viewpoints; specific metrics for vehicles and pedestrians are evaluated.

Conclusion: The findings enable integration of 3DGS-based reconstruction methods into real-world industrial AD software development and testing pipelines, addressing the gap in systematic fidelity evaluation for safety-related systems.
...
TL;DR: This paper shows that cross-script joint training across Arabic, Urdu, and Persian improves Handwritten Text Recognition under low-resource conditions, achieving new SotA on Persian (CER 9.99) and significant gains on Urdu (CER from 17.20 to 14.45). The improvement is driven primarily by shared characters across scripts, while language-specific characters show limited or negative transfer.


<details>
  <summary>Details</summary>
Motivation: Handwritten Text Recognition for Arabic-script languages suffers from severe performance degradation under limited labeled data. Arabic-script languages share substantial character overlap, which motivates cross-script training as a strategy to mitigate data scarcity.

Method: Controlled line-level cross-script joint training experiments using a CRNN model trained on the union of Arabic (KHATT), Urdu (NUST-UHWR), and Persian (PHTD) datasets under low-resource regimes (100, 500, 1000 labeled lines), with statistical character-level analysis to examine transfer dynamics.

Result: New SotA on Persian PHTD with CER 9.99; Urdu UNHD CER reduced from 17.20 to 14.45. Transfer gains are structurally concentrated on characters shared across scripts, while language-specific characters show limited or negative transfer.

Conclusion: Cross-script training is effective for low-resource Arabic-script HTR, but the benefit is driven by script-level character overlap rather than uniform improvements. This provides insight into transfer dynamics in low-resource script families.
...
TL;DR: Proposes a self-supervised pretraining method for sign language recognition using segmentation-based masking that focuses on key body parts (especially hands), achieving state-of-the-art performance on WLASL, NMFs-CSL, and Slovo with fewer input frames and modalities.


<details>
  <summary>Details</summary>
Motivation: Existing sign language recognition methods use encoders pretrained on generic action datasets that fail to capture fine-grained hand cues, which are crucial for distinguishing subtle sign differences.

Method: Self-supervised pretraining with segmentation-based masking that adapts to the presence and motion of key body parts, using a mask-and-reconstruct objective for fine-grained sign representation learning.

Result: Achieves state-of-the-art per-instance and per-class Top-1 accuracy on WLASL, NMFs-CSL, and Slovo, while using fewer input frames and modalities than comparable encoders.

Conclusion: Segmentation-based masking for self-supervised pretraining effectively improves sign language recognition by better capturing subtle hand differences.
...
TL;DR: Proposes alternative cropping methods (exponential, Gaussian, linear) for large-scale 3D point clouds that preserve more geometric context than standard spherical cropping, improving model performance especially in outdoor scenes.


<details>
  <summary>Details</summary>
Motivation: Standard spherical cropping of large point clouds loses surrounding geometric context, degrading performance of 3D neural networks on large-scale scenes.

Method: Compares exponential, Gaussian, and linear cropping methods against spherical cropping; evaluates two 3D deep learning architectures on multiple indoor/outdoor datasets.

Result: Alternative cropping strategies enhance model performance, particularly for large-scale outdoor scenes, achieving new state-of-the-art results.

Conclusion: Altering the cropping strategy is an effective way to improve 3D scene understanding without changing the underlying model architecture.
...
TL;DR: EchoCare-CLIP is a vision-language contrastive framework for ultrasound that aligns images with clinical text using 16K+ image-text pairs. It outperforms baselines in cross-modal alignment, but zero-shot/few-shot downstream transfer depends on balancing domain adaptation, encoder capacity, and caption quality, with template-based captions often matching or outperforming LLM-generated ones.


<details>
  <summary>Details</summary>
Motivation: Existing ultrasound foundation models are vision-only, limiting zero-shot and few-shot transfer to novel tasks where task-specific annotations are scarce. The authors aim to bridge this gap by enabling multimodal (vision-language) alignment for ultrasound.

Method: EchoCare-CLIP uses a CLIP-style dual-encoder contrastive framework aligning ultrasound images with clinical text. A multi-organ corpus of 16K+ image-text pairs was curated (78% from expert reports, rest via template-based and LLM-generated captions). Evaluated configurations include CLIP vs. BioClinicalBERT text encoders and template vs. LLM captions, compared against OpenAI CLIP and BiomedCLIP.

Result: Best paired alignment score of 0.682. CLIP-based variants with partial fine-tuning achieved best zero-shot classification (0.709 on BUSI; 0.626 on AULI). Full end-to-end fine-tuning degraded transfer due to overfitting. Template-based captions matched or outperformed LLM-generated captions. Model rankings for linear probing and few-shot were dataset-dependent.

Conclusion: Ultrasound vision-language alignment is achievable from public data alone. Robust clinical transfer requires careful balancing of domain adaptation, encoder capacity, and caption supervision quality; lexical diversity alone is not a proxy for caption quality.
...
TL;DR: SFI-Bench is a video-based benchmark with 1,500+ expert-annotated questions that evaluates MLLMs on structured spatial reasoning and functional reasoning, revealing current models struggle to integrate spatial memory with functional understanding for grounded intelligence.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks only evaluate geometric perception (where things are), but fail to probe higher-order cognitive abilities (what things are for). The authors aim to address this gap by introducing a benchmark for grounded intelligence assessment.

Method: SFI-Bench is built from egocentric indoor video scans with over 1,500 expert-annotated questions covering two dimensions: Structured Spatial Reasoning (complex layouts, spatial representations) and Functional Reasoning (object affordances, context-dependent utility). Tasks include conditional counting, multi-hop relational reasoning, functional pairing, and knowledge-grounded troubleshooting.

Result: Current MLLMs consistently struggle to combine spatial memory with functional reasoning and external knowledge, highlighting a critical bottleneck in achieving grounded intelligence.

Conclusion: SFI-Bench serves as a diagnostic tool for measuring progress toward more cognitively capable and truly grounded multimodal agents, revealing that integrating perception, memory, and inference remains a major challenge.
...
TL;DR: PV-VAE introduces a predictive reconstruction objective that unifies predictive learning with video reconstruction, achieving faster convergence and better generation quality.


<details>
  <summary>Details</summary>
Motivation: Existing video VAEs focus on optimizing reconstruction quality, but this does not necessarily improve generative performance. Enhancing the "diffusability" of video latents for better generative modeling remains an unresolved challenge.

Method: Randomly discard future frames, encode only partial past observations, and train decoder to reconstruct observed frames and predict future ones simultaneously, encouraging temporally predictive latent structures.

Result: 52% faster convergence and 34.42 FVD improvement over Wan2.2 VAE on UCF101; favorable scalability; consistent gains in downstream video understanding tasks.

Conclusion: Predictive learning unified with reconstruction improves video generative performance, temporal coherence, and motion priors in the latent space.
...
TL;DR: FLoRA is a cross-modal multi-task framework that jointly reconstructs optical imagery and segments flood regions from Sentinel-1 SAR data by fusing complementary optical and SAR strengths in a teacher-guided latent space, outperforming fusion baselines on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current flood mapping methods fail to fully exploit spaceborne imagery: optical data offers high interpretability but is limited by environmental conditions, while SAR provides all-weather coverage with reduced visual interpretability. There is a need to fuse both modalities to achieve reliable and interpretable flood water intelligence.

Method: FLoRA uses a lightweight optical teacher (driven by RGB and NDVI priors) to guide SAR representations into a fusion latent space via multiscale windowed cross-attention and FiLM conditioning, with gated residuals preventing overcorrection. It performs multi-task learning for SAR-to-optical translation (RGB reconstruction) and flood water segmentation, optimized by Charbonnier SSIM, edge FFT magnitude losses, Dice BCE hydrology-aware edge alignment, and feature distillation constraints.

Result: Evaluations on SEN1FLOODS11, DEEPFLOOD, and SEN12MS show FLoRA surpasses fusion baselines in PSNR, SSIM, and LPIPS metrics.

Conclusion: Multi-modal fusion within a teacher-guided latent space yields semantically faithful and physically consistent flood-water intelligence from spaceborne observations.
...
TL;DR: SpecEdit is a training-free dynamic-resolution framework for diffusion-based image editing that uses a draft-and-verify scheme to identify and process only edit-relevant tokens at high resolution, achieving up to 10x-13x acceleration while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Existing dynamic-resolution sampling methods upscale using low-level heuristics (e.g., edge detection) that are weakly aligned with editing semantics, and they often process all spatial regions at high resolution even when no semantic change is needed, causing redundancy and accumulated errors.

Method: SpecEdit follows a draft-and-verify scheme: first generates a low-resolution draft to estimate the semantic outcome, then uses token-level discrepancies to identify which tokens are edit-relevant. Only those tokens undergo high-resolution denoising, while the rest stay at coarse resolution.

Result: Up to 10x and 7x acceleration on Qwen-Image-Edit and FLUX.1-Kontext-dev respectively, maintaining strong quality; up to 13x speedup when combined with step distillation and other acceleration techniques.

Conclusion: SpecEdit provides a training-free, efficient dynamic-resolution approach for diffusion-based editing, complementary to existing acceleration methods, and will be open-sourced.
...
TL;DR: This paper shows that fusing VV and VH SAR polarizations using deep learning improves flood mapping accuracy, especially in vegetated and complex environments, outperforming single-polarization approaches.


<details>
  <summary>Details</summary>
Motivation: Single-polarization SAR data struggles with flood mapping in complex environments where surface and volume scattering coexist, motivating the need for cross-polarization fusion to improve reliability.

Method: A deep learning-based segmentation framework is used to fuse VV and VH polarization data. Three configurations (VV only, VH only, fused VV-VH) are compared under identical training conditions using IoU and F1-score metrics.

Result: VV-VH fusion consistently outperforms single-polarization models, particularly in vegetated and heterogeneous flood regions, leading to more accurate flood boundary delineation.

Conclusion: Cross-polarization SAR fusion is important for enhancing the reliability of SAR-based flood mapping in disaster monitoring applications.
...
TL;DR: HeroCrystal is a privacy-preserving framework for multi-camera domain-adaptive object detection that uses a one-shot diffusion-based generation module, federated learning with probabilistic Faster R-CNN and dynamic model contrastive strategy, and a distillation stage with inconsistent categories integration. It achieves SOTA 33.4% mAP, improving by +2.1% over prior privacy-preserving methods.


<details>
  <summary>Details</summary>
Motivation: Address challenges in multi-camera domain-adaptive object detection including data privacy, class imbalance (long-tailed categories), and heterogeneous architectures across clients.

Method: Three stages: (1) Generated Stage - one-shot target-aware diffusion-based generation for privacy-preserving augmentation and controllable rare object synthesis; (2) Federated Stage - probabilistic Faster R-CNN on clients, dynamic model contrastive strategy, server-side model fusion; (3) Distilled Stage - inconsistent categories integration algorithm for label/architecture heterogeneity.

Result: Outperforms existing multi-source domain adaptation and federated learning baselines; achieves +2.1% mAP improvement over prior privacy-preserving approaches; new SOTA 33.4% mAP.

Conclusion: HeroCrystal is effective for enabling practical multi-camera AI surveillance systems with privacy guarantees.
...
TL;DR: RAFNet is a region-aware fusion network for pansharpening that uses DWT-based frequency separation, K-means clustering for adaptive convolution kernels, and cluster-guided sparse attention to achieve state-of-the-art performance while reducing computational complexity.


<details>
  <summary>Details</summary>
Motivation: Existing pansharpening methods suffer from quadratic complexity in frequency-based attention, fail to exploit regional sparsity in remote sensing images, and use static convolutions that cannot adapt to complex frequency/regional variations.

Method: RAFNet introduces two key modules: (1) Spatial Adaptive Refinement (SAR) using DWT for directional frequency separation and K-means clustering to build region-specific adaptive convolution kernels; (2) Clustered Frequency Aggregation (CFA) using cluster-guided sparse attention to reduce redundancy. These are integrated into a progressive multi-level spatial-frequency network.

Result: Extensive experiments on multiple benchmark datasets show RAFNet significantly outperforms state-of-the-art pansharpening methods in both reduced- and full-resolution assessments.

Conclusion: The proposed region-aware synergistic modeling of spatial and frequency information, combined with adaptive convolutions and sparse attention, effectively addresses key bottlenecks in pansharpening, achieving superior fusion quality with reduced computational cost.
...
TL;DR: SlimDiffSR is a lightweight diffusion-based framework for remote sensing image super-resolution that uses uncertainty-guided timestep assignment, structured pruning with specialized lightweight modules (frequency-separable convolution, direction-separable convolution, query-driven global aggregation), and MMD-based distillation, achieving up to 200× inference acceleration and 20× parameter reduction while maintaining competitive perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models achieve remarkable SR performance but suffer from high computational cost, limiting practical deployment in remote sensing applications. The goal is to design a lightweight and efficient diffusion framework specifically for real-world remote sensing image SR.

Method: (1) Uncertainty-guided timestep assignment for stronger single-step teacher model; (2) Structured pruning removing redundant semantic modules; (3) Lightweight designs: frequency-separable convolution, direction-separable convolution, query-driven global aggregation; (4) MMD-based distillation for feature distribution alignment between teacher and student.

Result: Up to 200× inference acceleration and 20× parameter reduction vs. multi-step diffusion models; competitive perceptual quality; clearly outperforms existing lightweight diffusion baselines in efficiency.

Conclusion: SlimDiffSR achieves a favorable balance between efficiency and reconstruction quality for remote sensing SR, enabling practical deployment of diffusion-based methods in resource-constrained settings.
...
TL;DR: The paper proposes 3DFSR, a voxel-based U-Net CNN model that enhances sparse, noisy ALS point clouds for forest inventory. It achieves superior point cloud quality, improves stem detection F1 from 0.71 to 0.97, DBH estimation RMSE from 13.45 to 6.43 cm, and enables TLS/MLS algorithms to work on ALS data.


<details>
  <summary>Details</summary>
Motivation: ALS point clouds are sparse and noisy, making individual-tree-level forest inventory (stem localization, tree size estimation) inaccurate. Existing methods struggle to extract reliable tree-level information from such low-quality data.

Method: A voxel-based CNN with U-Net architecture (3DFSR) that simultaneously increases point density and reduces noise. It processes ALS forest point clouds to produce enhanced, high-resolution outputs.

Result: 3DFSR achieves 0.249m Chamfer Distance and 2.711m Hausdorff Distance, outperforming other super-resolution algorithms. Stem detection F1 improves from 0.71 to 0.97; DBH RMSE reduces from 13.45cm to 6.43cm. Works across 10-1700 points/m² densities and generalizes across LiDAR platforms without transfer learning.

Conclusion: 3DFSR effectively enhances ALS point clouds to enable accurate individual-tree-level forest inventory, allowing algorithms originally designed for TLS/MLS to be applied to ALS data. The model is robust across different point densities and LiDAR platforms.
...
TL;DR: This paper systematically evaluates metric reliability for machine unlearning in Vision-Language Models, identifies inconsistencies among five standard metrics, proposes a Unified Quality Score (UQS), and releases benchmarks/checkpoints.


<details>
  <summary>Details</summary>
Motivation: Machine unlearning in VLMs is required for GDPR compliance, but current evaluation practices use inconsistent metrics that yield conflicting rankings, necessitating a systematic study of metric reliability.

Method: Conducted Kendall tau analysis over 36 unlearned LLaVA-1.5-7B models using five metrics (FA, RA, MIA, AD, JS) on three VQA benchmarks. Proposed UQS composite metric with weights derived from Spearman correlation with oracle distance.

Result: Two opposing metric clusters identified: {FA, RA, MIA} vs {AD, JS}. Multimodal VQA shows lower agreement (avg tau=0.086) than unimodal (avg tau=0.158). RA is most reliable (rho=0.484), FA negatively correlated (rho=-0.418). UQS yields stable rankings (tau=0.647±0.262).

Conclusion: Current unlearning metrics in VLMs are unreliable and inconsistent; the proposed UQS provides more stable evaluation. Released benchmark, 36 checkpoints, and interactive leaderboard for community use.
...
TL;DR: MultiSense-Pneumo is a multimodal AI framework for pneumonia screening that integrates symptoms, cough audio, speech, and chest X-rays, designed for offline deployment on low-resource hardware.


<details>
  <summary>Details</summary>
Motivation: Pneumonia causes high morbidity/mortality globally, especially in low-resource settings. Existing computational approaches are mostly unimodal and rely only on radiographs, missing the inherently multimodal nature of clinical assessment.

Method: The framework integrates five components: (1) deterministic symptom triage, (2) LightGBM-based acoustic classification, (3) domain-adversarial ResNet-18 for radiograph analysis, (4) transformer-based speech recognition, and (5) an interpretable multimodal fusion operator that normalizes each modality into risk signals and aggregates them into a unified screening estimate.

Result: Radiograph pathway shows robustness under domain shifts; acoustic signals exhibit limitations in minority class recall. The system runs fully offline on laptop-class hardware.

Conclusion: MultiSense-Pneumo is a promising research prototype for screening and triage support in low-resource settings, though not yet a clinically validated diagnostic system.
...
TL;DR: Review of the NITRE 2026 E-LLIE Challenge on efficient low-light image enhancement for mobile devices, with 207 registrations, 27 valid submissions, and 17 valid factsheets, showing significant improvements in performance and efficiency.


<details>
  <summary>Details</summary>
Motivation: To address the need for lightweight networks that enhance low-light images on mobile devices under limited computational resources.

Method: A challenge-based review (NITRE 2026 E-LLIE) with 207 participants, 27 valid submissions, and 17 valid factsheets; systematic evaluation of submitted methods.

Result: 27 teams submitted valid entries, 17 provided valid factsheets; significant improvements in both performance and efficiency over prior state-of-the-art.

Conclusion: The challenge demonstrates notable progress in efficient low-light image enhancement, with practical deployability on mobile devices.
...
TL;DR: InfiltrNet is a dual-branch CNN-Swin Transformer architecture that predicts three-zone infiltration risk maps for gliomas from multimodal MRI, outperforming five baselines on BraTS 2020/2025 with clinically relevant attention to peritumoral regions.


<details>
  <summary>Details</summary>
Motivation: Gliomas infiltrate beyond visible MRI tumor margins, and existing deep learning methods only segment the visible tumor rather than estimating infiltration risk in surrounding tissue, which is critical for surgical planning and radiation therapy.

Method: A dual-branch architecture combining CNN encoder with Swin Transformer encoder via cross-attention fusion modules, using a distance-transform-based label generation strategy to derive three infiltration risk zones, trained with combined Dice-CrossEntropy and boundary-aware loss with auxiliary supervision heads.

Result: InfiltrNet outperforms five established baselines on BraTS 2020 and BraTS 2025 datasets, with GradCAM++ and Occlusion sensitivity confirming attention to clinically relevant peritumoral regions.

Conclusion: The proposed approach effectively predicts glioma infiltration beyond visible tumor margins and offers clinically meaningful risk maps for treatment planning.
...
TL;DR: FedPuReL addresses long-tailed personalized federated learning by purifying gradients with zero-shot predictions and modeling personalization as residual correction, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Real-world federated learning faces both non-IID and long-tailed class distributions simultaneously. Existing PFL methods struggle because fine-tuning erodes the inherent class balance of foundation models, and personalization techniques transfer this bias to local models.

Method: FedPuReL (Federated Learning via Gradient Purification and Residual Learning): (1) Purifies local gradients using zero-shot predictions to maintain a class-balanced global model; (2) Models personalization as residual correction on top of the frozen global model.

Result: Consistently outperforms state-of-the-art methods on both global and personalized models across diverse long-tailed scenarios.

Conclusion: Preserving balanced knowledge in the global model while enabling unbiased personalization is key to long-tailed PFL. Code is available at https://github.com/shihaohou/FedPuReL.
...
TL;DR: SpectraDINO extends DINOv2 to multispectral imaging (NIR, SWIR, LWIR) using lightweight adapters and a multi-stage teacher-student distillation, achieving SOTA on object detection and semantic segmentation benchmarks.


<details>
  <summary>Details</summary>
Motivation: VFMs are pretrained on RGB data but unexplored for multispectral modalities (NIR/SWIR/LWIR), which offer critical sensing under adverse conditions but suffer a domain gap from RGB models.

Method: Extends DINOv2 ViT with per-modality bottleneck adapters; multi-stage teacher-student training with cosine distillation, symmetric contrastive loss, patch-level alignment, and neighborhood-structure-preservation loss, keeping the RGB backbone frozen.

Result: SOTA performance on multispectral object detection and semantic segmentation across NIR, SWIR, and LWIR benchmarks using standard fusion strategies.

Conclusion: SpectraDINO is a general-purpose backbone for spectral generalization, effectively bridging the RGB-multispectral domain gap while preserving RGB priors.
...
TL;DR: WindowQuant optimizes KV cache in video language models using window-adaptive mixed-precision quantization, reducing inference latency and GPU memory while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: VLMs suffer from long visual token sequences causing high inference latency and GPU memory usage. Existing mixed-precision quantization methods are time-consuming and hardware-inefficient.

Method: WindowQuant has two modules: (1) window-level quantization search that determines optimal bit-width per window based on similarity scores with text prompt, and (2) window-level KV cache computation that reorders windows before quantization for hardware efficiency.

Result: Outperforms state-of-the-art VLMs and KV cache quantization methods on various datasets.

Conclusion: Window-adaptive mixed-precision quantization is an effective and hardware-efficient solution for KV cache optimization in VLMs.
...
TL;DR: This paper benchmarks lightweight image-based networks for LiDAR place recognition using BEV representations, evaluating FP32, FP16, and INT8 quantization for EdgeAI deployment. FP16 offers efficient accuracy while INT8 causes architecture-dependent degradation.


<details>
  <summary>Details</summary>
Motivation: Enabling efficient LiDAR-based place recognition on resource-constrained EdgeAI platforms for long-term autonomous navigation, addressing the deployment challenge of deep learning models.

Method: Uses Bird's Eye View (BEV) representations to transform LiDAR data for lightweight image-based networks. Benchmarks architectures without aggregation heads, using a unified descriptor scheme based on global pooling and linear projection, evaluated under FP32, FP16, and INT8 quantization.

Result: FP16 matches FP32 accuracy with lower computational cost; INT8 quantization leads to architecture-dependent performance degradation. Clear trade-offs exist between accuracy, robustness, and efficiency.

Conclusion: The findings provide a strong foundation for future research into 'use-case'-aware quantization of neural networks for Edge deployment.
...
TL;DR: This study compares EO-specific and generalist vision foundation models for remote sensing image retrieval, finding that generalist models are competitive with or better than EO-specific models, especially in cross-scene generalization, suggesting current EO pretraining strategies need improvement.


<details>
  <summary>Details</summary>
Motivation: To determine whether EO-specific vision foundation models are truly more effective than strong generalist models for remote sensing image retrieval, given the high cost of data acquisition and annotation in remote sensing.

Method: Controlled comparison between EO-specific and generalist vision foundation models using identical datasets, retrieval protocol, and evaluation metric, assessing both in-domain performance and cross-scene generalization.

Result: Generalist models are competitive with and sometimes outperform EO-specific models; EO-specific models show substantial degradation in cross-scene evaluation, while generalist models exhibit more stable transfer.

Conclusion: EO pretraining alone does not guarantee stronger retrieval-oriented representations; future EO models should better exploit physical, spatial, spectral, and geographic characteristics of remote sensing imagery.
...
TL;DR: NAN-SPOT is a training-light framework for Open-Set Object Detection that uses a negative-aware norm metric from off-the-shelf detectors to estimate objectness, requiring only minutes of training on hundreds of images, and introduces COCO-Open, an expanded dataset for OSOD evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing OSOD methods require extensive retraining of detectors to learn objectness. The authors hypothesize that standard off-the-shelf detectors already contain helpful cues for objectness due to training on diverse known categories, motivating a lightweight alternative.

Method: Proposes NAN-SPOT, which leverages a hidden layer metric called Negative-Aware Norm (NAN) from pre-trained detectors to estimate objectness without retraining the base detector, requiring only minutes of training on hundreds of images.

Result: NAN-SPOT achieves better performance on unknown object detection than methods requiring heavy training, without compromising performance on known objects. Also introduces COCO-Open dataset with 1853 unknown object annotations.

Conclusion: NAN-SPOT is an efficient and robust training-light framework for OSOD, making it a promising step towards open-world perception in autonomous driving.
...
TL;DR: LabBuilder is an end-to-end system that generates and verifies 3D laboratory layouts from text specifications, outperforming existing methods in creating realistic, functionally valid, and safe lab environments for scientific experiments.


<details>
  <summary>Details</summary>
Motivation: Automated laboratories are bottlenecked by the difficulty of designing safe and executable environments; existing 3D scene generation methods focus on household settings and neglect functional semantics and safety constraints essential for scientific experimentation.

Method: Three-component system: (1) LabForge curates annotated assets and chemical knowledge, translating text into structured protocols; (2) LabGen synthesizes layouts via iterative constraint-aware optimization; (3) LabTouchstone evaluates layouts as a unified benchmark.

Result: LabBuilder significantly outperforms existing state-of-the-art methods, producing laboratory environments that are realistic, functionally valid, and safe for complex experimental workflows.

Conclusion: The proposed system successfully addresses the gap in automated laboratory design by integrating functional semantics and safety constraints, enabling scalable layout generation for scientific discovery.
...
TL;DR: This paper proposes a hybrid approach combining diffusion-based (FLUX.2-4B Klein) and image-to-image translation (REGEN) models to enhance photorealism of synthetic game engine datasets, showing the hybrid method outperforms each individual model.


<details>
  <summary>Details</summary>
Motivation: Despite advances in game engine visual fidelity (e.g., ray-tracing), a significant sim2real appearance gap between synthetic and real-world images persists, limiting the practical utility of synthetic datasets for real-world computer vision.

Method: The authors investigate FLUX.2-4B Klein (diffusion model) and REGEN (traditional image-to-image translation) individually, then propose a hybrid approach combining diffusion-based geometry/material transformations with distribution-matching capabilities of I2I translation.

Result: REGEN outperforms FLUX.2-4B Klein individually, and the hybrid combination of both models achieves better visual realism than either model alone while maintaining semantic consistency.

Conclusion: The hybrid diffusion + I2I translation approach is effective for bridging the sim2real gap, and the code is publicly available.
...
TL;DR: Proposes a Momentum-Anchored Multi-Scale Fusion Network using EMA to stabilize chest X-ray classification under class imbalance, achieving 0.8682 AUC on ChestX-ray14 with notable gains on rare pathologies.


<details>
  <summary>Details</summary>
Motivation: Chest X-ray classification suffers from severe class imbalance where gradient updates bias toward majority classes, causing feature drift and poor performance on rare but critical pathologies.

Method: Uses exponential moving averages (EMA) as a temporal anchoring mechanism on the final expansion block of EfficientNet, combined with multi-scale spatial fusion (1x1, 3x3, 5x5 convolutions) to resist gradient-induced drift.

Result: Achieves 0.8682 average AUC on ChestX-ray14, with strong results on rare pathologies: Hernia (0.9470) and Pneumonia (0.8165).

Conclusion: Momentum anchoring effectively counters feature instability in long-tailed medical image classification.
...
TL;DR: An open-access deep learning model for detecting dispersed solid waste from UAV imagery was trained across 29 regions in 10 African countries, revealing heterogeneous waste patterns linked to population density and local infrastructure gaps, providing a ready-to-use monitoring tool for municipalities.


<details>
  <summary>Details</summary>
Motivation: Managing municipal solid waste in rapidly urbanizing Sub-Saharan Africa is challenging due to dispersed informal dumping and lack of high-resolution spatial monitoring datasets.

Method: A deep learning model was trained on manually annotated UAV image tiles across 29 regions in 10 countries, encompassing diverse environmental contexts.

Result: The model achieved excellent detection performance. Waste accumulation is most strongly associated with population density and lack of local infrastructure access, with hotspots along waterways and heterogeneous urban distribution.

Conclusion: The released model enables municipalities and local communities to monitor dispersed solid waste via UAV imagery without extensive technical expertise, supporting targeted waste management interventions across Sub-Saharan Africa.
...
TL;DR: This paper integrates Convolutional Block Attention Module (CBAM) into CNN blocks to improve multi-label classification of thoracic diseases from chest X-rays, achieving 0.8695 mean AUC on ChestXray14 dataset.


<details>
  <summary>Details</summary>
Motivation: To address challenges in chest radiography interpretation: time-consuming manual diagnosis, class imbalance, and difficulty localizing multiple co-existing pathologies.

Method: Integrating Convolutional Block Attention Module (CBAM) into traditional CNN blocks for enhanced feature refinement and extraction in multi-label classification.

Result: Achieved mean AUC of 0.8695 on ChestXray14 dataset, outperforming several state-of-the-art baselines.

Conclusion: CBAM-enhanced CNN blocks effectively improve multi-label thoracic disease classification performance.
...
TL;DR: PointCRA introduces a channel-level metric-based enhancement mechanism with temporal trend variation to improve feature discrimination in 3D point cloud understanding, achieving strong results on S3DIS, ScanObjectNN, and ShapeNetPart datasets.


<details>
  <summary>Details</summary>
Motivation: Existing methods for 3D point cloud understanding use coarse-grained metrics (point-level spatial distribution or channel responses) that cause significant information loss in deeper layers of multi-scale networks.

Method: Proposes PointCRA network with three key components: (1) temporal trend variation as a new evaluation dimension to avoid weight dimension collapse, (2) multi-level calibration framework guided by neighborhood homogeneity, and (3) dedicated loss function to enhance channel discriminability.

Result: 77.5% mIoU on S3DIS, 90.4% OA on ScanObjectNN, 87.4% instance mIoU on ShapeNetPart.

Conclusion: The proposed method effectively enhances feature aggregation with strong interpretability, low parameter overhead, and good transferability across diverse datasets and benchmark models.
...
TL;DR: GDMRG is a graph-augmented dual-stream framework for medical report generation that uses a GCN-based Topological Knowledge Internalization module to inject disease co-occurrence priors, a dual-stream classification system with asymmetric optimization for handling imbalanced samples, and a Diagnosis-Guided Spatial Attention mechanism to align clinical semantics with visual features, achieving competitive results on MIMIC-CXR and zero-shot generalization on IU X-Ray.


<details>
  <summary>Details</summary>
Motivation: Existing medical report generation methods treat chest abnormalities as isolated classification targets, ignoring inherent disease co-occurrences and failing to translate medical topological structures into explicit data correlations, which limits reasoning on complex or subtle lesions.

Method: (1) Topological Knowledge Internalization (TKI) module using GCN to generate parameterized weight matrix from global disease co-occurrence priors; (2) Dual-stream classification: main branch generates diagnostic prompts with topological constraints, auxiliary branch uses asymmetric optimization for imbalanced samples; (3) Diagnosis-Guided Spatial Attention (DGSA) that uses high-dimensional clinical semantics to recalibrate the visual encoder and reduce feature hallucinations.

Result: Competitive clinical efficacy (CE) with natural language fluency on MIMIC-CXR dataset; robust zero-shot generalization on IU X-Ray dataset.

Conclusion: This work presents an integrated and interpretable paradigm for medical report generation that effectively incorporates topological disease knowledge and establishes a logical closed loop between diagnosis and visual grounding.
...
TL;DR: This paper identifies an inductive gap in multimodal in-context learning (ICL) for vision-language models (VLMs), caused by redundant visual tokens and skewed attention distribution. It proposes a framework combining visual token compression, dynamic attention rebalancing, chain-of-thought reasoning, and reinforcement learning to improve inductive reasoning across eight benchmarks.


<details>
  <summary>Details</summary>
Motivation: VLMs fail to perform reliable in-context learning due to an inductive gap—models often produce correct answers from flawed reasoning and cannot extract consistent rules from demonstrations. This is worsened by redundant visual tokens and attention bias toward the first image.

Method: A framework restructuring ICL as an inductive-deductive process with: (1) similarity-based visual token compression to filter redundant patches, (2) dynamic attention rebalancing for equitable focus across images, (3) chain-of-thought for explicit rule derivation, and (4) auxiliary learning combining supervised fine-tuning with reinforcement learning using verifiable rewards.

Result: Evaluated on eight benchmarks (visual perception, logical reasoning, STEM, sarcasm detection), the framework achieves consistent and significant improvements over standard ICL baselines across multiple open-source VLMs.

Conclusion: The proposed framework effectively equips VLMs with genuine inductive capabilities in multimodal settings, addressing the fragility of ICL by bridging the inductive gap through structured reasoning and attention optimization.
...
TL;DR: UnGAP improves real-time crack segmentation by using aleatoric uncertainty as an active visual prompt to refine feature representations, rather than treating it as a passive post-hoc metric.


<details>
  <summary>Details</summary>
Motivation: Real-time crack segmentation suffers from aleatoric uncertainties (lighting, blur, texture ambiguity). Existing uncertainty-aware methods treat uncertainty as a passive endpoint, failing to use it to improve feature learning. Additionally, heteroscedastic modeling for cracks suffers from gradient attenuation in high-variance regions, causing under-fitting of complex boundaries.

Method: Proposes UnGAP framework with a closed-loop mechanism: (1) Uncertainty-Prompted Feature Modulator (UPFM) uses aleatoric uncertainty as an active visual prompt to calibrate features via pixel-wise affine transformations; (2) This mitigates heteroscedastic pathology by converting high variance into a constructive signal for stronger feature rectification; (3) A boundary-aware detection head further constrains prediction precision.

Result: Extensive experiments show UnGAP achieves superior segmentation accuracy with real-time inference speed, validating the benefit of transforming uncertainty from a passive metric into an active calibration tool.

Conclusion: Uncertainty can be effectively repurposed as an active feedback signal to improve feature learning in crack segmentation, closing the loop between uncertainty estimation and model optimization.
...
TL;DR: FEAT is a method for fashion editing and virtual try-on that supports diverse design sources (including non-garment images like artwork and photos) and handles complete outfits with accessories, using Disentangled Dual Injection for selective design cue injection and Orthogonal-Guided Noise Fusion for garment removal and try-on.


<details>
  <summary>Details</summary>
Motivation: Existing fashion design methods are limited to garment-related images and cannot handle creative design sources (e.g., artwork, abstract imagery, natural photos) nor support complete outfits including accessories.

Method: 1) Disentangled Dual Injection (DDI): selectively injects design cues from both apparel and non-apparel sources via content/style disentanglement. 2) Orthogonal-Guided Noise Fusion (OGNF): a training-free mechanism that removes residual garments via orthogonal projection and applies region-specific noise strategies for virtual try-on of garments and accessories.

Result: FEAT achieves state-of-the-art performance in design flexibility, prompt consistency, and visual realism.

Conclusion: FEAT expands fashion editing and virtual try-on to diverse design sources and complete outfits with accessories, outperforming existing methods.
...
TL;DR: DirectEdit is a training-free image editing method for flow transformers that eliminates accumulated reconstruction drift by directly aligning forward paths, achieving superior editing fidelity and editability without extra neural function evaluations.


<details>
  <summary>Details</summary>
Motivation: Existing training-free image editing methods suffer from accumulated drift caused by mismatched noisy latents between reconstruction and editing paths, which fundamentally limits reconstruction fidelity.

Method: DirectEdit directly aligns forward paths (rather than rectifying the inversion path) to eliminate reconstruction errors without additional NFEs, combined with attention feature injection and multi-branch mask-guided noise blending for balancing fidelity and editability.

Result: Extensive experiments demonstrate efficient and accurate image editing with superior performance over state-of-the-art methods.

Conclusion: DirectEdit provides a simple yet effective training-free editing approach for flow transformers that achieves precise reconstruction and reliable feature sharing.
...
TL;DR: This paper improves calibration of medical image segmentation models by treating multi-rater annotations as ordinal targets, using ordinal-aware scoring rules like Ranked Probability Score loss to better align model confidence with inter-rater variability, achieving better probabilistic calibration without sacrificing accuracy across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Deep segmentation networks are often poorly calibrated, especially when multiple expert annotations show substantial disagreement. Inter-rater variability, typically treated as noise, actually carries valuable information about annotation ambiguity that should be reflected in model confidence for safe clinical deployment.

Method: The authors reformulate multi-rater supervision as an ordinal learning problem, treating voxel-wise annotator agreement as an ordered target. They combine ordinal-aware scoring rules (Ranked Probability Score ordinal loss) with a standard binary objective to maintain discriminative performance while improving calibration.

Result: Evaluated on four public segmentation benchmarks (ophthalmology, histopathology, thoracic imaging), ordinal-aware training consistently improved calibration with respect to inter-rater agreement (measured via multi-rater expected calibration error) without degrading segmentation accuracy.

Conclusion: Treating multi-rater annotations as ordered information offers a principled, architecture-agnostic approach to building more reliable probabilistic segmentation models for clinical use.
...
TL;DR: MPFM is a flow-based framework for open-set supervised anomaly detection that uses Gaussian mixture prototype modeling and mutual information maximization to capture multi-modal normal data distributions and improve anomaly detection.


<details>
  <summary>Details</summary>
Motivation: Existing prototype-based OSAD methods use unimodal Gaussian priors that fail to capture the inherent multi-modality of normal data, leading to blurred decision boundaries.

Method: MPFM learns a continuous transformation from normal feature distributions to a structured Gaussian mixture prototype space via flow matching, where each component corresponds to a distinct normal class. A Mutual Information Maximization Regularizer (MIMR) is introduced to prevent prototype collapse and enhance separability.

Result: State-of-the-art performance across diverse benchmarks under both single- and multi-anomaly settings.

Conclusion: The proposed MPFM effectively addresses multi-modality in normal data and achieves superior open-set anomaly detection performance.
...
TL;DR: Anomaly Preference Optimization (APO) reformulates anomaly generation as a preference learning problem, using implicit alignment from denoising trajectory deviations and a Time-Aware Capacity Allocation module to dynamically balance diversity and fidelity, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing anomaly generation methods struggle to reconcile fidelity and diversity due to distribution misalignment and overfitting, especially when limited data is available.

Method: 1) Anomaly Preference Optimization (APO) reformulates anomaly generation as preference learning; 2) Implicit preference alignment uses real anomalies as positive references, deriving signals from denoising trajectory deviations without human annotation; 3) Time-Aware Capacity Allocation (TACA) dynamically distributes model capacity, prioritizing diversity in high-noise phases and fidelity in low-noise phases; 4) Hierarchical sampling strategy for coherence-alignment trade-off control.

Result: Significantly outperforms existing baselines, achieving state-of-the-art in both realism and diversity.

Conclusion: The proposed framework effectively solves the fidelity-diversity dilemma in limited-data anomaly generation through preference optimization and adaptive capacity allocation.
...
TL;DR: M⁴Fuse is a lightweight 3D brain tumor segmentation model that uses a balanced encoder-decoder design with a grouped state space mixer, dual-stage gating bridge, and sample-level mixture-of-experts to achieve superior performance with 62.63% fewer parameters at half the input resolution.


<details>
  <summary>Details</summary>
Motivation: Existing 3D brain tumor segmentation models suffer from encoder-decoder imbalance and heavy compute requirements due to reliance on large input volumes, making them brittle and resource-intensive.

Method: M⁴Fuse uses three key components: (1) a grouped state space mixer for long-range context with linear complexity, (2) a cross-scale dual-stage gating bridge to denoise and align skip features, and (3) a sample-level mixture-of-experts to absorb cross-site acquisition shifts.

Result: On BraTS2019 and BraTS2021, M⁴Fuse outperforms other lightweight methods. At 64×128×128 resolution, it reduces parameters by 62.63% while improving average performance by 0.09%.

Conclusion: M⁴Fuse achieves exceptional parameter-to-accuracy efficiency and robustness across diverse data centers, validated through ablation studies.
...
TL;DR: ExpoCM is a one-step generative framework for single-image HDR reconstruction that uses exposure-aware consistency trajectories and a specialized loss function to recover high dynamic range radiance from a single LDR image, achieving over 400× faster inference than DDPM while maintaining state-of-the-art quality.


<details>
  <summary>Details</summary>
Motivation: Single-image HDR reconstruction is highly ill-posed due to detail saturation in over-exposed regions and noise amplification in under-exposed areas. Existing diffusion-based methods overlook exposure-dependent degradation and suffer from high computational costs due to iterative sampling.

Method: ExpoCM reformulates HDR reconstruction as a Probability Flow ODE (PF-ODE) and constructs exposure-aware consistency trajectories. It uses a soft exposure mask to separate over-, under-, and well-exposed regions, then applies region-conditioned consistency trajectories for hallucinating details, suppressing noise, and preserving structures in a single distillation-free step. An Exposure-guided Luminance-Chromaticity Loss in CIE L*a*b* space is introduced to handle brightness bias and color drift.

Result: ExpoCM achieves state-of-the-art fidelity and perceptual accuracy on HDR-REAL, HDR-EYE, and AIM2025 benchmarks, with over 400× faster inference than DDPM (1000 steps) and 20× faster than DDIM (50 steps).

Conclusion: ExpoCM effectively addresses the ill-posed nature of single-image HDR reconstruction by incorporating exposure awareness into a one-step generative framework, dramatically reducing computational cost while improving quality.
...
TL;DR: The paper proposes ADA-Nets, a blind super-resolution framework with attention-guided domain adaptation, to enhance low-resolution multispectral aerial imagery for standing dead tree segmentation. It works on unpaired real images, handles multiple degradation types, and achieves 54-64% Dice scores without requiring high-resolution annotations.


<details>
  <summary>Details</summary>
Motivation: Standing dead tree mapping is critical for monitoring climate change effects on forests, but high-quality aerial imagery is limited by sensor availability and scarcity of annotated data. Existing methods often rely on synthetic downsampling which doesn't reflect real-world conditions.

Method: Proposes Attention-Guided Domain Adaptation Networks (ADA-Nets) for blind super-resolution that learns mapping from low-res to high-res multispectral image domains using unpaired samples. The framework handles real-world degradations (saturation, noise, low contrast) and serves as a general-purpose restorer.

Result: Achieves 54% and 64% Dice scores for dead tree segmentation. The first result (54%) is obtained without any high-resolution annotations—training on super-resolved low-res images, evaluating on high-res data. Dataset publicly released on Kaggle.

Conclusion: This is the first study to perform real-world, generic super-resolution for multispectral data in dead tree segmentation. The approach enables segmentation without high-resolution annotated data, addressing practical sensor limitations for forest monitoring.
...
TL;DR: MooD is the first framework using continuous Valence-Arousal (VA) values for fine-grained and efficient affective image editing, outperforming existing methods in controllability, visual fidelity, and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing AIE methods overlook inference efficiency and rely on discrete emotion representations, limiting practical applicability and the ability to capture complex, subtle human emotions.

Method: MooD introduces a VA-Aware retrieval strategy to link vague affective values with concrete visual semantics, integrates visual transfer and semantic guidance for controllable editing, and constructs an AffectSet dataset with VA annotations.

Result: Superior performance in affective controllability and visual fidelity while maintaining high efficiency, validated through extensive experiments and ablation studies.

Conclusion: Continuous VA-based editing is a promising direction for practical, fine-grained affective image editing; code and dataset will be open-sourced.
...
TL;DR: TemPose-TF-ASF enhances badminton stroke prediction by modeling bidirectional temporal dependencies between adjacent strokes, achieving consistent accuracy and Macro-F1 improvements across multiple models.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to model rich temporal context for accurate badminton stroke prediction, limiting fine-grained sports analysis and tactical decision support.

Method: Proposes Adjacent-Stroke Fusion (ASF) module that incorporates stroke-type info from both preceding and subsequent strokes, using two-stage training: baseline predictions as estimated temporal context, then joint optimization of ASF and classifier.

Result: Consistent improvements over baseline and variants on large-scale badminton dataset in Accuracy and Macro-F1; integrating ASF into other advanced models yields notable performance gains with strong transferability.

Conclusion: Explicitly modeling bidirectional temporal stroke dependencies improves stroke recognition and can be seamlessly integrated into existing state-of-the-art models.
...
TL;DR: This paper proposes a lightweight multi-task neural network for real-time driver monitoring that predicts attentiveness, fatigue, and distraction from camera input with minimal computational requirements.


<details>
  <summary>Details</summary>
Motivation: Road traffic accidents are a major global concern, with human factors like driver distraction and fatigue being the primary causes. There is a need for an efficient, real-time system to monitor driver state that can run on limited hardware.

Method: The authors develop a lightweight multi-task neural network that predicts multiple facial indicators in a single forward pass. This is integrated into a complete execution workflow for real-time estimation of attentiveness, fatigue, and distracting activities.

Result: The proposed pipeline satisfies stringent real-time requirements while minimizing computational demands, enabling deployment on systems with tight computational budgets.

Conclusion: A camera-based, computationally efficient approach can effectively assess driver attentiveness and alertness in real-time, addressing a key safety concern.
...
TL;DR: This paper proposes a data-centric continual adaptation framework for AI-generated image detection that uses both in-the-wild data (via automated fact-check article retrieval) and generator-driven data, achieving significant accuracy improvements (+9.14% and +8%) on two state-of-the-art detectors.


<details>
  <summary>Details</summary>
Motivation: Existing AI-generated image detectors suffer from performance degradation under distribution shifts and when encountering newly emerging generative models, necessitating a robust adaptation framework.

Method: The authors propose (1) an automated weakly supervised pipeline for constructing in-the-wild datasets via fact-check article retrieval, (2) use of generator-driven data to adapt to new models, and (3) a continual learning framework combining both data types to mitigate catastrophic forgetting.

Result: The framework achieves +9.14% and +8% average accuracy improvements on two state-of-the-art detectors.

Conclusion: Combining in-the-wild data with generator-driven data within a continual learning framework enables robust and effective adaptation of AI-generated image detectors to evolving generative models.
...
TL;DR: Proposes SA-INR, a self-supervised implicit neural representation, to reconstruct high-resolution diffusion MRI from a single rotating-view per direction, achieving massive acceleration with high-fidelity spatial and angular super-resolution.


<details>
  <summary>Details</summary>
Motivation: Rotating-view thick-slice acquisition is SNR-efficient for mesoscale dMRI but requires many views to satisfy Nyquist sampling, leading to long scan times. The goal is to accelerate acquisition while maintaining high resolution.

Method: An MLP-based implicit neural representation conditioned on a b=0 structural prior and b-direction via FiLM, trained end-to-end on anisotropic input in a self-supervised manner.

Result: Achieved 34.82 dB for trained directions and 33.08 dB for unseen directions on simulated data; synthesized angular data improved downstream DTI model fitting accuracy.

Conclusion: SA-INR breaks classical sampling limits, enabling fast, quantitative high-resolution dMRI by learning continuous q-space representation from a single view per direction.
...
TL;DR: Hyp2Former is an end-to-end framework for Open-Set Panoptic Segmentation that leverages hyperbolic space to encode hierarchical semantic relationships among known categories, enabling reliable detection of unknown objects without requiring explicit unknown modeling during training.


<details>
  <summary>Details</summary>
Motivation: Existing OPS methods treat known categories as a flat label set, ignoring the semantic hierarchy that provides valuable structural priors for distinguishing unknown objects from in-distribution classes.

Method: Hyp2Forme encodes hierarchical semantic similarities continuously in hyperbolic space, learning a structured embedding space that captures multiple levels of semantic abstraction without explicit modeling of unknowns during training.

Result: Outperforms existing methods on MS COCO, Cityscapes, and Lost&Found datasets, achieving the best balance between unknown object discovery and in-distribution robustness.

Conclusion: Encoding hierarchical relationships among known categories in hyperbolic space allows reliable detection of unknown objects even when their fine-grained category was not seen during training.
...
TL;DR: This paper proposes a method for fine-grained, parametric control of stylistic attributes in latent diffusion models by learning disentangled editing directions from synthetic datasets, using guidance composition to preserve semantics, and introducing regularization loss plus optimized DDIM inversion for consistent edits.


<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models lack precise control over stylistic attributes and often cause unintended content modifications during editing.

Method: (1) Learning disentangled editing directions from synthetic stylistically filtered datasets. (2) Guidance composition to bridge domain gap between finetuned and foundation models. (3) Training regularization loss and optimized DDIM inversion with null-conditional embeddings for real image editing.

Result: Compared to text-based editing techniques, the method offers well-integrated, more precise, and continuously adjustable stylistic modifications across attributes like outlines, local contrast, watercolorization, and geometric patterns.

Conclusion: The proposed parametric approach enables fine-grained, consistent, and disentangled stylistic control in latent diffusion models without unintended content changes.
...
TL;DR: StableMind is a regularized adaptation framework for cross-subject fMRI decoding that addresses brain-side instability and image-side supervision unreliability when adapting to new subjects with limited data. It uses ridge projection priors and Fourier-based augmentation for brain stability, plus difficulty-aware image blur for reliable supervision, achieving SOTA results on NSD with 84.02% image retrieval and 81.66% brain retrieval accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing cross-subject fMRI decoding methods degrade when adapting to new subjects due to: (1) brain-side instability from large subject differences in fMRI responses, and (2) image-side supervision unreliability because fine-grained visual details aren't reliably supported by limited fMRI signals. Additionally, raw data from previous subjects may be inaccessible in realistic scenarios.

Method: StableMind introduces two regularization branches: (1) Brain-side stabilization via reusing ridge projections from pretrained models as adaptation priors, plus Fourier-based feature-level brain augmentation for robustness. (2) Image-side reliability improvement via difficulty-aware image blur during brain-image alignment, reducing fine-grained detail influence while preserving stable visual structure.

Result: On the Natural Scenes Dataset under 1-hour adaptation protocol, StableMind achieves 84.02% image retrieval accuracy and 81.66% brain retrieval accuracy (averaged over 4 subjects), surpassing SOTA by 5.71% brain retrieval accuracy with fewer trainable parameters.

Conclusion: The proposed regularized adaptation framework effectively handles limited fMRI data from new subjects and inaccessible previous-subject data, achieving superior cross-subject fMRI decoding performance with high efficiency.
...
TL;DR: This survey comprehensively reviews representation learning methods for retinal OCT image analysis, covering from early deep learning to foundation models and vision-language systems, organized by a taxonomy of learning paradigms with analysis of methodological contributions, limitations, datasets, evaluation protocols, and open research directions.


<details>
  <summary>Details</summary>
Motivation: The clinical need to process large volumes of OCT images and reduce reliance on expert annotation while improving diagnostic consistency across devices and populations.

Method: Structured literature review organized along a principled taxonomy of learning paradigms: supervised learning (CNN/transformer), self-supervised/semi-supervised methods, generative approaches, 3D volumetric modeling, multimodal representation learning, and large-scale pretrained foundation models.

Result: Provides structured overview of public OCT datasets, evaluation protocol considerations, and a unified problem formulation situating each learning paradigm within a common mathematical framework.

Conclusion: Identifies pressing open research directions: volumetric foundation model pretraining, uncertainty-aware representation learning, federated/privacy-preserving training, fairness/bias mitigation, and concept-based interpretability.
...
TL;DR: This paper proposes VODA (ViL-Only Domain Adaptation), a stricter setting than SFDA that eliminates all source domain dependencies, using only a randomly initialized model, a Vision-Language (ViL) model, and unlabeled target data. They introduce TS-DRD (Two-Stage Denoised-Region Distillation) which achieves competitive or superior performance to existing SFDA methods on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing SFDA methods still initialize from source pre-trained models, thus are not truly source-free. The authors observe that different source models yield minimal variation in final results for the same target domain, suggesting the source model itself has limited impact. This motivates them to propose VODA, which eliminates all source dependencies.

Method: The authors propose VODA (ViL-Only Domain Adaptation) setting that uses only a randomly initialized model, a ViL model, and unlabeled target data. They introduce TS-DRD (Two-Stage Denoised-Region Distillation): Stage 1 warms up the model with ViL guidance; Stage 2 seeks a Denoised-Region inherent in both the ViL and adapting model, yielding cleaner supervision for distillation.

Result: Experiments on Office-Home, VisDA, and DomainNet-126 show that under VODA, TS-DRD achieves competitive or superior performance to existing SFDA methods that still use source models.

Conclusion: The VODA setting and TS-DRD framework demonstrate that source-free domain adaptation can be achieved without any source model dependency, challenging the necessity of source pre-trained initialization in domain adaptation.
...
TL;DR: A GleasonAI model demonstrated robust and consistent performance in prostate cancer grading across 10,366 biopsy cores from 14 Swedish regions collected over 17 years (1998-2015), achieving a quadratic-weighted kappa of 0.86 for ISUP grading with stable performance over time and significant prognostic value for cancer-specific mortality.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether AI-based prostate pathology grading generalizes across variations in sample preparation and preservation over prolonged time periods, addressing the gap in understanding temporal robustness of AI models in clinical pathology.

Method: Evaluated an end-to-end attention-based multiple instance learning model (GleasonAI) on an independent validation cohort of 10,366 biopsy cores from 1,028 patients across 14 Swedish regions, using archival specimens collected between 1998-2015.

Result: Achieved quadratic-weighted kappa of 0.86 for core-level ISUP grading, comparable to experienced pathologists, stable across geographic regions and the 17-year collection period, with significant prognostic gradient for prostate cancer-specific mortality.

Conclusion: The AI grading model demonstrates strong generalizability and temporal robustness, supporting the use of pathology archives as large-scale resources for AI development, validation, and retrospective prognostic research.
...
TL;DR: GLASSNet leverages a frozen SAMv2 encoder with a lightweight adapter (97% fewer learnable parameters) and a dual-decoder (global semantics + local details) to achieve SOTA on SOD and camouflaged object detection benchmarks.


<details>
  <summary>Details</summary>
Motivation: SOD is underexplored in the era of large vision models; foundation models like SAM have potential but full fine-tuning is computationally expensive and prone to overfitting with limited data.

Method: Uses SAMv2 as a frozen encoder + lightweight spatially aware convolutional adapter (97% parameter reduction); dual-decoder with one branch for global long-range semantics and another for fine local details (edges/textures), then fuses both cues.

Result: Surpasses state-of-the-art methods on standard SOD and camouflaged object detection benchmarks.

Conclusion: Frozen foundation models combined with targeted adaptation and global-local decoding can achieve superior performance efficiently, demonstrating a viable paradigm for leveraging large models without full fine-tuning.
...
TL;DR: This paper introduces Generalized Moment Retrieval (GMR) to handle real-world scenarios where queries may match multiple or no video moments, along with Soccer-GMR benchmark and tailored methods (GMR adapter and GRPO reward) for this unified setting.


<details>
  <summary>Details</summary>
Motivation: Traditional VMR assumes a single matching moment per query, which fails in real-world cases where queries correspond to multiple relevant moments or none at all. The authors aim to address this gap by formulating a more realistic, unified setting.

Method: The authors (1) formulate GMR as a unified retrieval setting; (2) build Soccer-GMR benchmark via a semi-automated pipeline with human verification; (3) design a unified evaluation protocol with complementary metrics; (4) propose a lightweight plug-and-play GMR adapter for discriminative models and a GMR-tailored GRPO reward for MLLMs.

Result: Extensive experiments show consistent gains across all metrics, revealing key limitations of current methods and positioning GMR as a more realistic and challenging benchmark.

Conclusion: GMR better reflects real-world video-language understanding needs, and the proposed baselines demonstrate effectiveness while exposing limitations of existing methods.
...
TL;DR: A method using explicit reconstruction constraints from decoupled representations improves low-light image enhancement, achieving 29.71 dB PSNR and 0.89 SSIM on LOLv2-Real, with code available on GitHub.


<details>
  <summary>Details</summary>
Motivation: To suppress abnormal channel amplification and chromatic noise in low-light image enhancement.

Method: Imposes explicit reconstruction constraints from a decoupled representation to regularize the enhancement process.

Result: Achieves 29.71 dB PSNR and 0.89 SSIM on LOLv2-Real, with competitive/superior performance on MIT-Adobe FiveK and LSRW; improved face detection on DarkFace.

Conclusion: The proposed method effectively enhances low-light images and benefits downstream tasks like face detection.
...
TL;DR: AutoFocus is a training-free, uncertainty-aware active visual search framework that uses token-level perplexity to model spatial uncertainty and adaptively zoom in on GUI elements, improving grounding performance in high-resolution interfaces.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs struggle with GUI grounding in high-resolution interfaces due to dense layouts and small interactive elements. Current zoom-in strategies rely on fixed anchors, heuristic grids, or reinforcement learning without a principled way to adaptively determine where refinement is needed.

Method: 1) Samples multiple coordinate hypotheses from VLMs, 2) Converts token-level perplexity into anisotropic Gaussian spatial probability fields to model directional uncertainty, 3) Generates global/local region proposals with Shape-Aware Zooming balancing localization and context, 4) Uses visual prompt-based aggregation to select the most consistent prediction.

Result: Extensive experiments on ScreenSpot-Pro and ScreenSpot-V2 demonstrate consistent improvements across both general-purpose and GUI-specialized VLMs.

Conclusion: AutoFocus provides a principled, training-free approach to adaptive zooming for GUI grounding by leveraging inherent uncertainty signals (perplexity) from VLMs, outperforming existing fixed/heuristic zoom strategies.
...
TL;DR: This paper proposes ViewSAM, a weakly supervised framework for Cross-view Referring Multi-Object Tracking (CRMOT) that leverages foundation models (SAM2, SAM3) as pseudo-label generators, requiring only category-level supervision instead of costly frame-level annotations.


<details>
  <summary>Details</summary>
Motivation: Existing CRMOT methods rely heavily on expensive frame-level spatial annotations and cross-view identity supervision. The authors aim to reduce this annotation burden by exploring weak supervision with only object category labels.

Method: A two-stage framework: (1) Affinity-guided Cross-view Re-prompting strategy to refine SAM3-generated tracklets into reliable cross-view pseudo labels; (2) ViewSAM model built on SAM2 with view-aware cross-modal semantics, using learnable view-induced conditions to bridge visual-textual gaps.

Result: ViewSAM achieves state-of-the-art performance under weak supervision and remains competitive with fully supervised methods, with only ~10% additional parameters.

Conclusion: Foundation models can be effectively repurposed as pseudo-label generators for CRMOT under weak supervision, and explicitly modeling view-aware semantics is key to bridging cross-view visual-textual discrepancies.
...
TL;DR: Mamoda2.5 is a unified AR-Diffusion framework integrating multimodal understanding and generation, using a 25B MoE model (128 experts, Top-8 routing, 3B active) that achieves top-tier video generation and editing performance, while a joint distillation and RL framework compresses inference to 4 steps, yielding 95.9× speedup.


<details>
  <summary>Details</summary>
Motivation: To seamlessly integrate multimodal understanding and generation within a single architecture while efficiently scaling up model capacity without prohibitive training costs.

Method: (1) Diffusion Transformer backbone with fine-grained MoE (128 experts, Top-8 routing) for a 25B-parameter model with only 3B activated parameters. (2) Joint few-step distillation and reinforcement learning framework compressing 30-step editing into 4-step inference.

Result: Top-tier VBench 2.0 performance; new record on OpenVE-Bench matching Kling O1; up to 95.9× faster video editing inference; 98% success rate in internal advertising video editing scenario.

Conclusion: Mamoda2.5 demonstrates that MoE-based scaling combined with distillation-RL optimization can match proprietary models while achieving massive inference speedup, with proven real-world deployment success.
...
TL;DR: This paper develops a real-time system using YOLO11/YOLO11-Pose and Random Forest to detect pushing violence in surveillance footage, achieving precision from 0.98 (controlled) to 0.72 (real-world challenging angles).


<details>
  <summary>Details</summary>
Motivation: Physical violence in public spaces is a public health concern; minor incidents like pushing often escalate into severe violence. Automated early detection can enable intervention before escalation.

Method: Uses YOLO11 and YOLO11-Pose for human detection and skeletal keypoint extraction, calculates body inclination and joint angles (shoulders/hips), then trains a Random Forest classifier to distinguish normal behavior from aggressive pushing.

Result: Precision of 0.98 in controlled frontal-view environments; precision of 0.72 in challenging real-world high-altitude, steep-angle surveillance footage with perspective distortion and noise.

Conclusion: Skeletal analysis is feasible for early violence intervention in urban security contexts, though performance degrades under real-world surveillance conditions.
...
TL;DR: SAIL framework integrates retinal anatomical priors with semantic features via fusion to produce sharper, anatomically aligned attribution maps for OCT-based retinal disease diagnosis, outperforming existing post-hoc XAI methods.


<details>
  <summary>Details</summary>
Motivation: Existing post-hoc XAI methods for OCT-based retinal disease detection struggle to delineate fine-grained lesion structures, respect anatomical boundaries, and suppress noise, limiting clinical trust and regulatory approval.

Method: SAIL integrates retinal anatomical priors at the representation level and couples them with semantic features via a fusion design, without modifying standard post-hoc explainability methods.

Result: SAIL consistently enhances interpretability across diverse OCT datasets, producing sharper, anatomically aligned attribution maps. Ablation studies show both structural priors and semantic features are needed, and proper fusion is critical.

Conclusion: Structure-aware representations are a key step toward reliable explainability in OCT imaging for retinal disease diagnosis.
...
TL;DR: OphMAE is a multimodal foundation model for ophthalmic AI that fuses 3D OCT with 2D en face OCT images, achieving state-of-the-art diagnostic performance (96.9% AUC for AMD, 97.2% AUC for DME) while maintaining high accuracy even with single-modality inputs or limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Current ophthalmic AI models are limited to single-modality inference, conflicting with clinical practice that relies on multi-modal diagnosis. Additionally, high-performance AI deployment is hindered in resource-limited settings lacking advanced 3D imaging hardware.

Method: OphMAE employs a cross-modal fusion architecture combining 3D OCT volumetric depth with 2D en face OCT planar context, using a novel adaptive inference mechanism. It was pre-trained on 183,875 paired OCT images from 32,765 patients.

Result: In a benchmark of 17 diagnostic tasks with 48,340 paired images from 8,191 patients, OphMAE achieved 96.9% AUC for AMD and 97.2% AUC for DME, surpassing existing models. It maintained 93.7% AUC for AMD with single-modality 2D inputs and 95.7% AUC with only 500 labeled samples.

Conclusion: OphMAE provides a scalable, adaptable framework for ophthalmic AI that ensures robust performance across diverse tasks and resource conditions.
...
TL;DR: PubMed-Ophtha provides a large-scale, high-quality ophthalmological image-text dataset with 102,023 image-caption pairs from 15,842 articles, featuring hierarchical decomposition into panels and subcaptions, achieving strong performance in caption alignment (BLEU 0.913) and panel detection (mAP 0.909).


<details>
  <summary>Details</summary>
Motivation: Vision-language models for ophthalmology require large-scale, high-quality image-text datasets, but such resources remain scarce in the medical domain.

Method: Figures are extracted from PubMed Central article PDFs at full resolution, decomposed into panels and individual images, annotated with imaging modality and mark status, and captions split into panel-level subcaptions using a two-step LLM approach.

Result: Mean average sentence BLEU score of 0.913 for subcaption generation; panel/image detection mAP@0.50 of 0.909 and 0.892; figure extraction median IoU of 0.997.

Conclusion: The dataset and its generation pipeline are publicly released to support reproducibility and advance vision-language research in ophthalmology.
...
TL;DR: PFlowNet is a new framework for Large-Vision Language Models that replaces rigid geometric expert supervision with a self-conditioned generation process combining variational reinforcement learning and multi-dimensional rewards, achieving state-of-the-art results on V* Bench (90.6%) and MME-RealWorld-lite (67.0%).


<details>
  <summary>Details</summary>
Motivation: Existing LVLMs suffer from language bias and hallucination because standard MLE objectives fail to constrain visual trajectories. Current mitigation methods introduce geometric priors from visual experts, but these are suboptimal—biased toward geometric precision and offering limited reasoning utility.

Method: PFlowNet decouples perception from reasoning into a self-conditioned generation process, then integrates multi-dimensional rewards with vicinal geometric shaping via variational reinforcement learning to promote reasoning-oriented perceptual behaviors while maintaining visual reliability.

Result: Achieves new SOTA on V* Bench (90.6%) and MME-RealWorld-lite (67.0%), with provable performance guarantees.

Conclusion: By eschewing rigid alignment with expert priors, PFlowNet enables more interpretable and effective visual reasoning, outperforming prior methods that rely on geometric supervision.
...
TL;DR: SIAM is a 3D whole-head segmentation model trained on only six high-quality manual templates using domain randomization across intensity and shape, achieving contrast-agnostic segmentation of 16 brain and extra-cerebral structures that matches or outperforms state-of-the-art methods across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: Existing synthetic training for brain MRI segmentation relies on hundreds of automatically labeled templates, which introduces systematic biases and limits flexibility to incorporate new anatomical structures.

Method: SIAM uses domain randomization extended to both intensity (synthetic image generation for contrast variability) and shape domains (high-resolution spatial transformations modeling cortical thickness and deep nuclei morphology), trained on only six manually annotated templates.

Result: Evaluated on eight heterogeneous datasets (N=301) across multiple contrasts (T1-weighted, T2-weighted, CT) and age ranges, SIAM matches or outperforms state-of-the-art methods for brain structures and extends segmentation to non-brain structures with superior consistency and sensitivity to gray matter atrophy.

Conclusion: SIAM demonstrates that high-quality manual templates combined with domain randomization can produce a robust, contrast-agnostic whole-head segmentation model without relying on large-scale automatically labeled data, enabling fully automated preprocessing-free analysis.
...
TL;DR: This paper proposes using a 3D Pix2Pix GAN to generate synthetic PET scans from CT images, which are then fused with CT data via the MINT multi-stage fusion architecture to improve NSCLC histological subtype classification (ADC vs. SCC). The method achieved AUC improvement from 0.489 to 0.591 and GMean from 0.305 to 0.524 on a 714-subject multi-center dataset.


<details>
  <summary>Details</summary>
Motivation: Histological differentiation between ADC and SCC in NSCLC is critical for personalized treatment. While [18F]FDG PET/CT is standard, its utility is limited by high costs and radiation exposure. The authors investigate whether synthetic PET data can serve as a feature-enhancement strategy to supplement anatomical CT scans.

Method: A 3D Pix2Pix GAN, pretrained on the FDG-PET/CT Lesions dataset, synthesizes pseudo-PET volumes from CT scans. These synthetic volumes are integrated with structural CT data using the MINT framework, a multi-stage intermediate fusion architecture, for histological subtype classification.

Result: On a multi-center dataset of 714 subjects, the multimodal approach improved AUC from 0.489 to 0.591 and GMean from 0.305 to 0.524 over the CT-only baseline, with statistical significance.

Conclusion: Synthetic PET scans provide discriminatory metabolic cues that enable models to exploit complementary cross-modal information, offering a potential feature-enhancement strategy when physical PET scans are unavailable.
...
TL;DR: This paper identifies that current text-guided class-agnostic counting (CAC) models struggle to correctly ground textual prompts in visual scenes, leading to unreliable counting. The authors propose a new evaluation framework including PrACo++ (two test protocols: negative-label test and distractor test) and the MUCCA dataset (multi-category images). Experiments on 10 SOTA methods reveal significant weaknesses in semantic grounding despite strong standard counting performance.


<details>
  <summary>Details</summary>
Motivation: Current CAC evaluation protocols focus only on standard counting errors within single-category images, overlooking whether models can correctly ground the textual prompt to the right object class. This misalignment undermines real-world reliability.

Method: (i) PrACo++: a test suite with negative-label test and distractor test protocols plus new metrics; (ii) MUCCA dataset: real-world images with multiple annotated object categories per scene.

Result: 10 SOTA CAC models show strong performance under standard metrics but exhibit significant weaknesses in grounding object class descriptions. Semantic similarity between prompts influences failure patterns.

Conclusion: Current CAC models need more semantically grounded architectures. The proposed framework provides a reliable assessment tool for future open-world text-guided CAC methods.
...
TL;DR: This paper proposes a video augmentation framework that converts simulated robotic videos into realistic training videos for VLA models, addressing the domain gap between simulation and reality through semantic segmentation, caption rewriting, and conditional video generation with diffusion feature-reuse for efficiency.


<details>
  <summary>Details</summary>
Motivation: Simulated data is cheap and parallelizable but suffers from a visual domain gap and limited diversity, resulting in poor real-world generalization for VLA models.

Method: Extracts structured conditions via video semantic segmentation and captioning, rewrites captions for diversity, uses a conditional video transfer model, with a diffusion feature-reuse mechanism and coreset sampling for efficiency.

Result: Improves RDT-1B by 8% on Robotwin 2.0, boosts π₀ by 5.1% on LIBERO-Plus, with consistent gains across multiple benchmarks and a real robotic platform.

Conclusion: The proposed simulation-to-reality video augmentation framework effectively bridges the domain gap for VLA models, improving real-world generalization with practical scalability.
...
TL;DR: UMPE is a unified encoder that fuses four types of map priors (HD/SD vector maps, rasterized SD maps, satellite imagery) with BEV features for autonomous mapping and planning, using alignment-aware gating and fusion mechanisms. It achieves +5.9 mAP on nuScenes mapping, +4.1 mAP on Argoverse2, and reduces trajectory error by 0.30m L2 and collision rate by 0.10% on E2E planning.


<details>
  <summary>Details</summary>
Motivation: Existing autonomous driving mapping and planning approaches are sensor-centric and underutilize heterogeneous map priors (HD/SD vector maps, rasterized maps, satellite imagery) due to issues like pose drift, modality differences, and inconsistent availability during test time.

Method: UMPE has two branches: (1) a vector encoder that pre-aligns HD/SD polylines with SE(2) correction, encodes points with sinusoidal features, and uses confidence-biased cross-attention with normalized channel-wise gating; (2) a raster encoder with FiLM-conditioned ResNet-18, SE(2) micro-alignment, and zero-initialized residual fusion. Priors are fused in vector-then-raster order (geometry first, appearance second).

Result: On nuScenes: MapTRv2 improved from 61.5 to 67.4 mAP (+5.9), MapQR from 66.4 to 71.7 mAP (+5.3). On Argoverse2: +4.1 mAP. For E2E planning with VAD: trajectory L2 error reduced from 0.72 to 0.42m (-0.30m), collision rate from 0.22% to 0.12% (-0.10%). Demonstrates powerset robustness when only subsets of priors are available.

Conclusion: A unified, alignment-aware encoder that handles heterogeneous map priors compositionally can significantly improve both mapping and planning performance, outperforming prior-injection methods and showing robustness to varying prior availability at test time.
...
TL;DR: FoR-Net is a lightweight semantic segmentation architecture that uses a learned importance map and Top-K activation to selectively focus on hard regions like thin structures and boundaries, achieving competitive performance on Cityscapes with limited computation.


<details>
  <summary>Details</summary>
Motivation: To design a lightweight semantic segmentation model that efficiently focuses on challenging regions (e.g., thin structures, object boundaries) without relying on heavy global modeling.

Method: FoR-Net uses a selector module to predict region-wise importance, adopts a Top-K activation mechanism to emphasize informative regions, and employs multi-scale convolutional branches with different receptive fields for diverse spatial context aggregation.

Result: On the Cityscapes benchmark with limited computational resources, FoR-Net achieves competitive performance and demonstrates improved consistency in challenging regions.

Conclusion: Region-focused reasoning provides a simple yet effective inductive bias for efficient semantic segmentation, suggesting that selectively emphasizing hard regions can match the performance of heavier models.
...
TL;DR: TOC-SR is a framework that creates efficient one-step super-resolution models by discovering compact diffusion backbones through generative distillation and Bayesian optimization, achieving 6.6x parameter reduction and 2.8x GMACs reduction while maintaining reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Diffusion models are powerful for image super-resolution but suffer from high computational costs due to large model size and iterative sampling, making them impractical for real-world deployment.

Method: The method involves three key steps: (1) constructing parameter-efficient surrogate blocks via feature-wise generative distillation from a 16-channel latent diffusion model, (2) performing architecture discovery using epsilon-constrained Bayesian Optimization to balance model complexity and generative fidelity, and (3) distilling the diffusion process into a single-step generator for super-resolution.

Result: The compact diffusion backbone achieves a 6.6x reduction in parameters and a 2.8x reduction in GMACs compared to the expanded model, while maintaining strong reconstruction quality in super-resolution tasks.

Conclusion: TOC-SR successfully enables efficient, one-step super-resolution by combining compact architecture discovery with diffusion process distillation, offering a practical solution for deploying diffusion-based super-resolution models.
...
TL;DR: The paper converts pretrained Softmax attention Transformers (specifically Stable Diffusion 3.5) into linear-complexity TTT models via architectural and representational alignment, achieving comparable quality with 1.32-1.47x inference speedup using only 1 hour of fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Training linear-complexity attention models from scratch is prohibitively expensive, and transferring weights from pretrained Softmax Transformers fails due to the representational gap between Softmax and linear attention mechanisms.

Method: (1) Architectural alignment: identifies TTT as a structurally compatible linear-complexity architecture with Softmax attention due to its two-layer dynamic formulation; (2) Representational alignment: introduces key instance normalization for shift-invariance and a lightweight locality enhancement module to bridge the representational gap.

Result: SD3.5-T⁵ achieves comparable text-to-image quality to fine-tuned Softmax model with only 1 hour of fine-tuning on 4×H20 GPUs, while speeding up inference by 1.32× at 1K and 1.47× at 2K resolutions.

Conclusion: Softmax-to-linear attention conversion is feasible through careful architectural and representational alignment, enabling efficient weight inheritance without expensive training from scratch.
...
TL;DR: HumanSplatHMR jointly optimizes 3D human pose and high-fidelity avatar reconstruction from video by closing the loop between geometric pose estimation and differentiable rendering, using only mesh estimates from a pose estimator without requiring motion capture systems.


<details>
  <summary>Details</summary>
Motivation: Existing methods (ViT-based, NeRF/Gaussian Splatting-based) fail to accurately recover 3D human geometry — ViT overfits to 2D views, while NeRF/Gaussian approaches treat pose and appearance separately, limiting generalization to new poses.

Method: A joint optimization framework that backpropagates photometric, segmentation, and depth losses through a differentiable renderer to refine pose parameters and global position, using only human mesh estimates from a SOTA pose estimator (no motion capture needed).

Result: Consistent improvements over pose recovery baselines that omit image-level refinement and avatar baselines that decouple pose estimation from avatar reconstruction.

Conclusion: Closing the loop between pose estimation and differentiable rendering improves both 3D pose accuracy and novel-view/novel-pose avatar rendering quality in real-world scenarios without reliance on impractical motion capture systems.
...
TL;DR: A framework combining transformer and SSM blocks for efficient image restoration on edge devices, using neural architecture search (ENS) to find optimal hybrid configurations that reduce inference latency 1.17x–3.4x on Snapdragon 8 Elite while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Transformers are powerful for image restoration but suffer from high latency on edge hardware due to costly attention kernels. The authors aim to balance accuracy and computational efficiency by replacing transformer blocks with lightweight SSM surrogates in hybrid architectures.

Method: (1) Train lightweight SSM blocks (Mamba) as feature-distilled surrogates of transformer blocks. (2) Construct hybrid U-Net architectures combining SSM and transformer blocks. (3) Use Efficient Network Search (ENS), a multi-objective search that optimizes restoration quality while penalizing transformer usage as a proxy for latency, enabling architecture discovery without hardware profiling.

Result: On Snapdragon 8 Elite CPU: ENS-Deblurring 2973 ms (3.4x faster), ENS-Deraining 5816 ms (1.74x faster), ENS-Denoising 8666 ms (1.17x faster) vs Restormer baseline 10119.52 ms, with competitive restoration quality.

Conclusion: Hybrid transformer-SSM architectures discovered via multi-objective search can significantly reduce inference latency on mobile hardware while maintaining restoration performance, offering a practical solution for edge deployment.
...
TL;DR: IConFace is a unified framework for blind face restoration that uses identity-structure asymmetric conditioning to exploit same-identity references when available and fall back to no-reference restoration when absent, improving identity consistency and detail recovery.


<details>
  <summary>Details</summary>
Motivation: Blind face restoration is ill-posed under severe degradation as identity-critical details are missing. Same-identity references reduce ambiguity but can cause overuse of reference appearance due to mismatched pose, expression, illumination, etc.

Method: References are distilled into a norm-weighted global AdaFace identity anchor for image-only modulation, while the degraded image is reinforced as the spatial structure anchor via low-rank residuals and block-wise degraded cross-attention with two-route memory.

Result: A single checkpoint exploiting references when available and falling back to no-reference restoration when absent, improving identity consistency, fine-detail recovery, and degraded-only restoration quality.

Conclusion: The proposed asymmetric conditioning approach enables unified reference-aware and no-reference restoration, effectively leveraging references without overusing them.
...
TL;DR: The paper introduces VideoNet, a domain-specific action recognition benchmark with 1,000 distinct actions across 37 domains, revealing that VLMs significantly underperform compared to humans, especially in few-shot settings. The authors also collect a large-scale training dataset (~500k QA pairs) and demonstrate that fine-tuning a 4B model on this data surpasses all 8B open-weight models on the benchmark.


<details>
  <summary>Details</summary>
Motivation: Modern VLMs lack evaluation on action recognition due to insufficient diverse data. The authors aim to revitalize action recognition by focusing on domain-specific actions, revealing gaps in VLM capabilities.

Method: The authors create VideoNet (1,000 actions, 37 domains), evaluate VLMs in multiple-choice and binary settings, test few-shot learning with k=1,2,3 examples, and collect ~500k video QA pairs to fine-tune a Molmo2-4B model.

Result: Gemini 3.1 Pro achieves 69.9% vs Qwen3-VL-8B's 45.0% in multiple-choice. In binary (50% chance baseline), Qwen gets 59.2%. Few-shot: Qwen improves +7.0%, Gemini declines -4.8%, both far below humans' +13.6%. Fine-tuned Molmo2-4B surpasses all 8B open-weight models.

Conclusion: Domain-specific action recognition remains a challenging gap for VLMs; training on targeted data effectively improves performance, suggesting the need for continued focus on this area in the VLM era.
...
TL;DR: ActDiff-VC is a diffusion-based video compression framework for ultra-low-bitrates that partitions videos into variable-length segments, transmits keyframes selectively, and uses tracked point trajectories as conditioning signals for a diffusion decoder to reconstruct frames, achieving significant bitrate reduction and perceptual quality improvements.


<details>
  <summary>Details</summary>
Motivation: To enable effective video compression at ultra-low bitrates by leveraging diffusion models as generative priors, while overcoming the challenge of controlling the generative process with highly compact conditioning signals.

Method: 1) Partitions videos into variable-length segments; 2) Transmits keyframes only when needed; 3) Summarizes temporal dynamics using tracked point trajectories; 4) Uses content-adaptive keyframe selection and budget-aware sparse trajectory selection; 5) Conditional diffusion decoder synthesizes remaining frames.

Result: Up to 64.6% bitrate reduction at matched NIQE; 64.6% improvement in KID; 37.7% improvement in FID at comparable bitrates against strong learned codecs on UVG and MCL-JCV benchmarks.

Conclusion: ActDiff-VC delivers favorable perceptual rate-distortion trade-offs relative to learned and diffusion-based baselines in the ultra-low-bitrate regime, demonstrating the effectiveness of sparse trajectory-based conditioning for generative video reconstruction.
...
TL;DR: A self-supervised IQA method that predicts distortion types, intensity, and direction maps plus relational quality scores without needing human-labeled MOS data.


<details>
  <summary>Details</summary>
Motivation: Traditional IQA relies on MOS, which is expensive to collect and lacks interpretable, localized feedback on specific distortions.

Method: Uses a self-supervised synthetic distortion engine for training data, a distortion prediction network with anti-symmetric objective for disentangled maps, and a scoring network via contrastive learning on ordinally ranked image sets.

Result: Produces spatially-aware maps identifying distortion type, intensity, and direction, plus a relational quality score.

Conclusion: Enables granular, interpretable IQA for targeted optimization of image processing algorithms without human-labeled quality scores.
...
TL;DR: LFINet is a Laplacian Frequency Interaction Network for rural road network construction from agricultural machinery trajectories, achieving state-of-the-art F1-score of 92.54% and IoU of 86.12% by decoupling low/high-frequency features and using cross-frequency interaction to handle noise and sparse structures.


<details>
  <summary>Details</summary>
Motivation: Existing downsampling methods blur sparse high-frequency road structures, and dense field operation noise causes fragmented or redundant topologies in extracted road networks from agricultural trajectory images.

Method: LFINet consists of: (1) Laplacian Multi-scale Separator (LMS) for decoupling low-frequency semantics and high-frequency details; (2) Cross-Frequency Interaction Block (CFIB) with dual pathways including High-Frequency Block (HFB) and Spatial Transformer (ST); (3) Frequency Gated Modulation (FGM) for integrating features; (4) Progressive Reconstruction Decoder for multi-scale feature fusion.

Result: LFINet achieves 92.54% F1-score and 86.12% IoU on a real-world agricultural trajectories dataset from Henan Province, China, surpassing the second-ranked method by 0.64% and 1.1% respectively.

Conclusion: LFINet effectively constructs topological road networks from noisy and sparse field data, establishing a new state-of-the-art for rural thematic road network construction.
...
TL;DR: AlbumFill is a training-free framework that retrieves identity-consistent reference images from personal albums to perform personalized image completion, using a vision-language model for semantic cue inference and reference-based inpainting.


<details>
  <summary>Details</summary>
Motivation: Existing personalized image completion methods either fail to maintain identity consistency (generic inpainting) or assume suitable reference images are explicitly provided, which is unrealistic in practice. The authors aim to solve the problem of automatically retrieving identity-consistent references from personal photo collections.

Method: AlbumFill is a training-free framework that: (1) uses a vision-language model to infer missing semantic cues from the occluded image, (2) guides composed image retrieval from personal albums using those cues, and (3) feeds retrieved identity-consistent references into reference-based completion models.

Result: The authors introduce a 54K human-centric sample dataset with album images. Experiments across multiple baselines demonstrate the difficulty of personalized completion and highlight the importance of identity-consistent reference retrieval.

Conclusion: Identity-consistent reference retrieval is crucial for personalized image completion, and AlbumFill's training-free approach effectively addresses the gap where suitable reference images are not explicitly provided.
