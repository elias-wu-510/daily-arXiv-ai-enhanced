...
TL;DR: TeleWorld is a real-time multimodal 4D world modeling framework that unifies video generation, dynamic scene reconstruction, and long-term memory in a closed-loop system, enabling practical interactive world models.
...
Method: Introduces a generation-reconstruction-guidance paradigm where generated videos are continuously reconstructed into dynamic 4D spatio-temporal representations that guide subsequent generation. Uses autoregressive diffusion-based video model with Macro-from-Micro Planning (hierarchical planning) and Distribution Matching Distillation for real-time synthesis.
...
TL;DR: Simple noise optimization approach reduces mode collapse in text-to-image models while maintaining generation quality.


<details>
  <summary>Details</summary>
Motivation: Text-to-image models suffer from mode collapse where multiple generations from the same prompt look similar, limiting diversity. Existing approaches use guidance mechanisms or candidate refinement, but this work explores noise optimization as a simpler alternative.

Method: Proposes noise optimization objective to mitigate mode collapse. Analyzes frequency characteristics of noise and explores alternative noise initializations with different frequency profiles to improve optimization and search.
...
Conclusion: Simple noise optimization effectively addresses mode collapse in text-to-image models while preserving base model fidelity, with frequency-aware noise initialization further improving performance.
...
TL;DR: Spatial4D-Bench is a large-scale benchmark with ~40K questions across 18 tasks in 6 categories to evaluate MLLMs' 4D spatial intelligence, revealing current models' substantial limitations.


<details>
  <summary>Details</summary>
Motivation: To assess whether Multimodal Large Language Models (MLLMs) can achieve human-level 4D spatial intelligence (perceiving object changes over time), which is lacking in existing small-scale or limited benchmarks.

Method: Created Spatial4D-Bench, a comprehensive benchmark with ~40,000 question-answer pairs covering 18 tasks organized into 6 cognitive categories: object understanding, scene understanding, spatial relationship understanding, spatiotemporal relationship understanding, spatial reasoning, and spatiotemporal reasoning.

Result: Benchmarking various state-of-the-art MLLMs revealed substantial limitations in 4D spatial reasoning abilities, including difficulties with route planning, action recognition, and physical plausibility reasoning.

Conclusion: Current MLLMs have significant gaps in 4D spatial intelligence compared to humans, and Spatial4D-Bench provides a structured evaluation framework to guide development toward human-level spatial cognition.
...
TL;DR: SMAGNet is a multimodal deep learning model that adaptively integrates SAR and MSI data for post-flood water extent mapping, maintaining performance even when MSI data is partially or completely missing.


<details>
  <summary>Details</summary>
Motivation: Current flood water mapping relies heavily on SAR data, but multimodal approaches combining SAR and MSI data show promise. However, adaptive integration of partially available MSI data into SAR-based mapping remains underexplored, limiting real-world applicability when timely post-flood observations are limited.
...
Result: SMAGNet consistently outperformed other multimodal models across varying MSI data availability levels on the C2S-MS Floods dataset. Even with completely missing MSI data, SMAGNet performed comparably to SAR-only U-Net, demonstrating robustness to missing data.

Conclusion: SMAGNet enhances model robustness to missing data and improves applicability of multimodal deep learning in real-world flood management scenarios by adaptively integrating available MSI data with SAR-based water extent mapping.
...
TL;DR: CMP learns spatial priors from historic traversals using compressed binarized hashmaps (32KB/km²) to improve 3D object detection in autonomous vehicles.


<details>
  <summary>Details</summary>
Motivation: Current autonomous vision systems treat every location as new, ignoring that most deployment areas have been visited before. This wastes valuable historical data that could improve perception.

Method: Compressed Map Priors (CMP) framework learns spatial priors from historic traversals using binarized hashmaps that require only 32KB/km² storage (20× reduction). These priors integrate into existing 3D perception systems with minimal computational overhead.

Result: Significant and consistent improvement in 3D object detection on nuScenes dataset across several architectures, with minimal storage requirements and computational costs.

Conclusion: Learning spatial priors from historic traversals is effective for improving autonomous vehicle perception, and can be done efficiently with compressed storage that integrates seamlessly into existing systems.
...
TL;DR: GLASS is a novel architecture for AI-generated image detection that combines global resized views with multiple original-resolution local crops using stratified sampling and attention-based aggregation, outperforming standard transfer learning methods.


<details>
  <summary>Details</summary>
Motivation: Most AI-generated image detection methods downsample images before processing, which risks losing fine-grained details crucial for distinguishing AI-generated content from real images. As AI-generated images become increasingly realistic and high-resolution, there's a need for architectures that can leverage both global context and local details without computational explosion.

Method: GLASS combines a globally resized view with multiple randomly sampled local crops at original resolution. It uses spatially stratified sampling to efficiently select local regions and aggregates them using attention-based scoring. The architecture can be integrated into various vision backbones (Vision Transformer, ResNet, ConvNeXt) to process images of any size.

Result: Experiments show that GLASS outperforms standard transfer learning approaches by achieving higher predictive performance while maintaining feasible computational constraints. The method effectively leverages both global and local information for improved AI-generated image detection.

Conclusion: GLASS provides an effective solution for AI-generated image detection by preserving fine-grained details through original-resolution local crops while maintaining computational efficiency through stratified sampling and attention-based aggregation, demonstrating superior performance across different backbone architectures.
...
TL;DR: FCMBench-V1.0 is a new multimodal benchmark for financial credit applications that addresses privacy compliance while maintaining practical utility, featuring 4,043 images and 8,446 QA samples across 18 certificate types.


<details>
  <summary>Details</summary>
Motivation: There's an urgent need for a domain-specific benchmark for financial credit applications that reflects real-world documents and workflows, includes credit-specific understanding and robustness testing, and preserves privacy compliance without sacrificing utility.

Method: Created FCMBench-V1.0 with a closed synthesis-capture pipeline: manually synthesized document templates with virtual content and captured scenario-aware images in-house to ensure privacy compliance and avoid pre-training data leakage. The benchmark includes 3 perception tasks, 4 credit-specific reasoning tasks, and 10 real-world acquisition artifact types for robustness testing.
...
Conclusion: FCMBench effectively discriminates performance disparities and robustness across modern VLMs, revealing that current models struggle with financial credit document understanding and robustness challenges, highlighting the need for domain-specific solutions.
...
TL;DR: A new vision-language model called Focal-RegionFace for generating and recognizing multi-attribute natural language descriptions of arbitrary face regions, including action units, emotions, and age estimation.


<details>
  <summary>Details</summary>
Motivation: To address the underexplored problem of fine-grained facial analysis where systems can focus on individual facial areas for better understanding and control, enabling multi-attribute descriptions for arbitrarily selected face regions.

Method: Created a new multi-attribute description dataset with region-level annotations and natural language descriptions. Proposed Focal-RegionFace, a fine-tuned vision-language model based on Qwen2.5-VL that incrementally refines focus on localized facial features through multiple progressively fine-tuning stages.

Result: Focal-RegionFace achieves best performance on the new benchmark using both traditional metrics and newly proposed metrics, demonstrating effectiveness and versatility in fine-grained multi-attribute face region-focal analysis.

Conclusion: The proposed approach successfully addresses the challenging problem of generating and recognizing multi-attribute natural language descriptions for arbitrary face regions, providing interpretable age estimation, facial action unit detection, and emotion analysis through localized focus.
...
TL;DR: DichroGAN is a cGAN that recovers seafloor colors from satellite imagery by modeling light attenuation through water using multiple generators for atmospheric radiance and underwater transmission estimation.


<details>
  <summary>Details</summary>
Motivation: Recovering in-air seafloor colors from satellite imagery is challenging due to exponential light attenuation in water, requiring specialized methods to address absorption and scattering effects.

Method: Two-step cGAN with four generators: two estimate diffuse/specular reflections for atmospheric radiance, one processes spectral band features, and another estimates underwater light transmission based on underwater image formation equation.

Result: DichroGAN achieves competitive performance compared to state-of-the-art underwater restoration techniques on both satellite and underwater datasets trained on PRISMA satellite imagery.

Conclusion: DichroGAN effectively removes water column effects to restore seafloor colors, demonstrating the potential of cGANs for satellite-based underwater image restoration.
...
Method: The framework leverages SLAT representations and introduces two key attention mechanisms: Morphing Cross-Attention (MCA) for structural coherence by fusing source/target features, and Temporal-Fused Self-Attention (TFSA) for temporal consistency using preceding frame features. Also includes orientation correction to mitigate pose ambiguity.

Result: Extensive experiments show state-of-the-art morphing sequences, even for challenging cross-category cases. The method supports advanced applications like decoupled morphing and 3D style transfer, and generalizes to other SLAT-based generative models.

Conclusion: MorphAny3D provides an effective training-free solution for high-quality 3D morphing by intelligently blending SLAT features within attention mechanisms, enabling semantically consistent and temporally smooth deformations across categories.
...
TL;DR: A novel 3D instance segmentation framework for accurate crop counting using multi-view images and NeRF, achieving robust performance across different crop types without crop-specific tuning.


<details>
  <summary>Details</summary>
Motivation: Traditional image-based segmentation methods struggle with partial occlusions and ambiguity in distinguishing clustered crops in outdoor field environments, making accurate crop counting challenging.

Method: Uses 2D images from multiple viewpoints, associates independent instance masks with neural radiance field (NeRF) view synthesis, incorporates crop visibility and mask consistency scores with 3D information from NeRF for 3D instance segmentation.

Result: Demonstrated consistent counting performance on three agricultural datasets (cotton bolls, apples, pears) despite major variations in crop color, shape, and size. Superior performance compared to state-of-the-art methods, and contributed a cotton plant dataset.

Conclusion: The proposed framework enables highly-accurate crop counting by addressing occlusion and clustering challenges through 3D instance segmentation, eliminates crop-specific parameter tuning, and shows robust performance across diverse crop types.
...
<details>
  <summary>Details</summary>
Motivation: Previous domain adaptation methods focus mainly on source-target domain shift while neglecting intra-domain variability. Existing style diversification approaches require pre-specified intra-domain variations, which is impractical. There's a need for methods that can capture diverse intra-domain styles without prior knowledge.

Method: IntraStyler uses exemplar images to guide style synthesis, matching output styles to exemplar styles. A style encoder learns style-only features discriminatively using contrastive learning to extract style information effectively.

Result: Evaluated on CrossMoDA 2023 dataset, the method demonstrates efficacy in controllable style synthesis and shows benefits of diverse synthetic data for downstream segmentation tasks.

Conclusion: IntraStyler successfully addresses intra-domain variability in domain adaptation through exemplar-based style synthesis without requiring prior knowledge of intra-domain variations, improving segmentation performance.
...
TL;DR: Using reinforcement learning with specialized reward functions to improve multimodal LLMs' visual reasoning by encouraging longer, structured reasoning chains that properly integrate visual information.


<details>
  <summary>Details</summary>
Motivation: Current multimodal large language models (MLLMs) generate reasoning chains that lack proper integration of visual information, limiting their ability to solve visual perception tasks like visual puzzles. The paper identifies visual perception as the key bottleneck, showing that converting images to text significantly improves performance.

Method: Proposes reward-driven reinforcement learning using group relative policy optimization (GRPO) with six specialized reward functions targeting different reasoning aspects: image understanding, thinking steps, and answer accuracy. This approach explicitly incentivizes longer, structured reasoning while mitigating bypassing of visual information.

Result: Achieves 5.56% improvement over the base Qwen-2.5-VL-7B model, with consistent gains across both in-domain and out-of-domain settings. The approach also shows significant performance gains when converting images to text (26.7% for Claude 3.5, 23.6% for Claude 3.7).

Conclusion: Reward-driven RL is an effective mechanism for unlocking long visual reasoning in open-source MLLMs without requiring costly supervision, addressing the visual perception bottleneck in multimodal reasoning tasks.
...
TL;DR: LooC introduces a low-dimensional compositional vector quantization method that uses compact codebooks more efficiently through compositional units and extrapolation-by-interpolation, achieving SOTA performance with smaller codebooks.


<details>
  <summary>Details</summary>
Motivation: As data and models become more diverse and complex, there's a need for high-capacity yet compact vector quantization methods that can handle this increased complexity without excessive memory usage.

Method: LooC uses low-dimensional codebooks where codevectors serve as compositional units within feature vectors. It introduces parameter-free extrapolation-by-interpolation to enhance feature smoothing during VQ, enabling full codebook usage without collapse.

Result: LooC outperforms existing VQ methods across different tasks, datasets, and architectures, achieving state-of-the-art performance with significantly smaller codebooks.

Conclusion: LooC successfully reconciles the conflict between high capacity and compactness in VQ through its compositional approach and extrapolation mechanism, serving as an effective plug-and-play module for various VQ-based applications.
...
TL;DR: SynDR-IQA improves BIQA generalization by reshaping synthetic data distribution using diversity upsampling and redundancy downsampling to address clustered feature patterns in synthetic datasets.


<details>
  <summary>Details</summary>
Motivation: BIQA faces challenges due to limited labeled data. Synthetic datasets offer a solution but models trained on them show poor generalization because learned representations form discrete clusters around reference images (high-quality) and distortion types (low-quality), which stems from synthetic data distribution rather than model architecture.

Method: SynDR-IQA framework reshapes synthetic data distribution based on theoretical analysis of sample diversity and redundancy's impact on generalization error. Uses two strategies: 1) Distribution-aware diverse content upsampling to enhance visual diversity while preserving content distribution, and 2) Density-aware redundant cluster downsampling to balance samples by reducing density in densely clustered areas.

Result: Extensive experiments across three cross-dataset settings (synthetic-to-authentic, synthetic-to-algorithmic, and synthetic-to-synthetic) demonstrate the effectiveness of the method in improving BIQA generalization performance.

Conclusion: The proposed SynDR-IQA framework successfully addresses the generalization limitations of BIQA models trained on synthetic data by reshaping the data distribution through diversity enhancement and redundancy reduction, offering a practical solution to the labeled data scarcity problem in BIQA.
...
TL;DR: Cross-modal data augmentation framework using CycleGAN and YOLOv8 to address IR data scarcity for PCB defect detection by generating pseudo-IR images from visible-light data.


<details>
  <summary>Details</summary>
Motivation: Infrared (IR) data scarcity is a critical bottleneck for PCB defect detection, limiting the effectiveness of deep learning models that require large datasets for training.

Method: Uses CycleGAN for unpaired image-to-image translation to map abundant visible-light PCB images into infrared domain, creating pseudo-IR samples. Then employs heterogeneous training strategy combining generated pseudo-IR data with limited real IR samples to train YOLOv8 detector.

Result: The method effectively enhances feature learning under low-data conditions, significantly outperforming models trained on limited real data alone and approaching performance benchmarks of fully supervised training.

Conclusion: Pseudo-IR synthesis via cross-modal data augmentation is an effective strategy for industrial inspection, addressing data scarcity while maintaining detection accuracy comparable to fully supervised approaches.
...
TL;DR: Lightweight framework for pest detection and pesticide recommendation using compact CNN with meta-learning for few-shot learning, designed for low-resource devices like smartphones and drones to help small farmers.


<details>
  <summary>Details</summary>
Motivation: Traditional pest management methods are costly, time-consuming, labor-intensive, and environmentally harmful. There's a need for accessible, efficient solutions for small farmers using low-resource devices.

Method: Two-component framework: 1) Pest Detection Module using lightweight CNN with prototypical meta-learning for few-shot pest identification, 2) Pesticide Recommendation Module incorporating environmental factors (crop type, growth stage) for eco-friendly recommendations. Trained on comprehensive pest image dataset combining multiple sources.
...
Conclusion: The framework demonstrates potential for real-time applications in precision agriculture, offering accessible pest management solutions for small farmers using low-resource devices while promoting environmental sustainability.
...
TL;DR: TotalFM is a radiological foundation model that efficiently learns 3D-CT to text correspondences using organ separation, achieving better zero-shot classification than existing models while maintaining computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Foundation models for radiology face computational cost challenges when training on 3D-CT volumetric data, making practical implementation difficult despite their potential for various clinical tasks.

Method: Uses organ separation concept with automated creation of organ volume and finding-sentence pairs via segmentation and LLM-based report processing. Combines self-supervised pre-training (VideoMAE) with contrastive learning using volume-text pairs to balance efficiency and representation capability.

Result: Outperformed CT-CLIP in 83% of organs and Merlin in 64% of organs for zero-shot organ-wise lesion classification. Achieved higher AUROC in 83% of finding categories compared to Merlin for zero-shot finding-wise classification. Showed comparable performance to existing VLMs in radiology report generation.
...
TL;DR: S1-MMAlign is a large-scale multimodal dataset of 15.5M scientific image-text pairs from 2.5M papers across physics, biology, engineering, etc., enhanced with AI recaptioning to bridge the semantic gap between complex scientific imagery and sparse text descriptions.


<details>
  <summary>Details</summary>
Motivation: Multimodal learning has transformed general tasks but struggles in scientific discovery due to the semantic gap between complex scientific imagery and sparse textual descriptions in papers. Existing scientific datasets lack proper image-text alignment.

Method: Created S1-MMAlign dataset from 2.5M open-access papers, spanning multiple disciplines. Used Qwen-VL multimodal large model series to recaption images by synthesizing context from paper abstracts and citation contexts, improving semantic alignment.

Result: Dataset contains 15.5M high-quality image-text pairs. AI enhancement pipeline significantly improves data quality: SciBERT pseudo-perplexity metrics show reduced semantic ambiguity, and CLIP scores indicate 18.21% improvement in image-text alignment.

Conclusion: S1-MMAlign provides a foundational resource for advancing scientific reasoning and cross-modal understanding in AI for Science, addressing the semantic gap in scientific multimodal learning. Dataset is publicly available on Hugging Face.
...
TL;DR: ActErase: A training-free method for concept erasure in diffusion models that identifies activation difference regions via prompt-pair analysis and dynamically replaces activations during inference.
...
Conclusion: Establishes a new plug-and-play paradigm for lightweight yet effective concept manipulation in diffusion models without requiring training.
...
TL;DR: FaithSCAN: A lightweight network that detects VQA hallucinations by exploiting rich internal signals from vision-language models, outperforming existing methods in both effectiveness and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing VQA hallucination detection methods have limitations: external verification approaches suffer from high computational overhead and dependency on external resources, while uncertainty-driven methods capture only limited facets of model uncertainty and fail to explore rich internal signals associated with diverse failure modes.

Method: FaithSCAN detects hallucinations by exploiting three types of internal VLM signals: token-level decoding uncertainty, intermediate visual representations, and cross-modal alignment features. These signals are fused via branch-wise evidence encoding and uncertainty-aware attention. The method also extends LLM-as-a-Judge paradigm to VQA hallucination detection, using a low-cost strategy to automatically generate model-dependent supervision signals for training without human labels.
...
TL;DR: DUAL is an uncertainty-aware framework for long-tailed remote sensing data that disentangles epistemic uncertainty (for hard tail samples) from aleatoric uncertainty (for noisy ambiguous samples) using evidential deep learning.


<details>
  <summary>Details</summary>
Motivation: Long-tailed distributions are common in remote sensing due to imbalanced object occurrences, but existing methods fail to distinguish between genuinely hard tail samples and noisy ambiguous ones, leading to overfitting on noise.
...
Conclusion: DUAL effectively addresses the critical challenge of disentangling hard tail samples from noisy ambiguous ones in long-tailed remote sensing data through uncertainty-aware modeling, offering a model-agnostic solution with superior performance over existing methods.
...
TL;DR: SV-GS is a framework for dynamic object reconstruction from sparse observations using skeleton-guided deformation fields and motion interpolation.


<details>
  <summary>Details</summary>
Motivation: Dynamic object reconstruction from real-world sparse observations (like security cameras) is challenging because standard methods require dense multi-view video coverage, which isn't available in practical scenarios.

Method: Uses a skeleton-driven deformation field with coarse joint pose estimation (time-dependent) and fine-grained deformation modules. Initializes with rough skeleton graph and static reconstruction, later relaxed to diffusion-based generative priors.

Result: Outperforms existing methods by up to 34% in PSNR on synthetic sparse datasets, achieves comparable performance to dense monocular video methods using significantly fewer frames on real-world data.

Conclusion: SV-GS enables practical dynamic reconstruction from sparse real-world observations by combining skeleton guidance with deformation fields, with flexibility in initialization requirements.
...
TL;DR: Deep learning model using Swin Transformer achieves 87.71% accuracy on ISIC2019 dataset for classifying 8 skin lesion types, demonstrating potential as diagnostic support tool.
...
Method: Developed deep learning model using Swin Transformer architecture with pretraining on public skin disease image datasets. Refined model architecture, optimized data preprocessing workflows, and applied targeted data augmentation techniques to improve performance.

Result: Achieved 87.71% prediction accuracy across eight skin lesion classes on the ISIC2019 dataset, demonstrating effective visual feature extraction and accurate classification of various dermatological cases.

Conclusion: The model shows potential as a diagnostic support tool for clinicians and a self-assessment aid for patients, addressing the gap between increasing skin disease prevalence and limited dermatologist availability.
...
<details>
  <summary>Details</summary>
Motivation: Existing colorization models only condition on a single reference (typically first frame), ignoring other valuable conditional data sources like character sheets, background images, or arbitrary colorized frames.
...
Result: Experiments on SAKUGA-42M show TimeColor improves color fidelity, identity consistency, and temporal stability over prior baselines under both single- and multi-reference protocols.
...
Method: VisNet combines multiple innovations: multi-scale feature fusion from ResNet50 stages 1-4 without parallel paths, semantic clustering with anatomical body partitioning using rule-based pseudo-labeling, dynamic weight averaging for classification regularization, and FIDI loss function for improved metric learning.

Result: Achieves 87.05% Rank-1 and 77.65% mAP on Market-1501 dataset with only 32.41M parameters and 4.601 GFLOPs, making it computationally efficient.

Conclusion: VisNet provides a practical, computationally efficient solution for real-time person re-identification in resource-constrained surveillance and mobile applications.
...
TL;DR: ReMA is a plug-and-play video augmentation strategy that uses controlled mixing with representation alignment and dynamic selection to improve behavior recognition robustness without extra supervision.


<details>
  <summary>Details</summary>
Motivation: Current video data augmentation methods are perturbation-driven and introduce uncontrolled variations that amplify non-discriminative factors, weakening intra-class structure and causing representation drift with inconsistent gains across temporal scales.
...
Conclusion: ReMA improves representation robustness for video behavior recognition through controlled mixing augmentation without requiring additional supervision or trainable parameters.
...
TL;DR: DMDNet uses depth-aware Mamba models and memory compensation to separate image reflections, especially effective for challenging nighttime scenarios with a new NightIRS dataset.


<details>
  <summary>Details</summary>
Motivation: Existing reflection separation methods struggle when transmission and reflection layers have similar contrasts, which is particularly severe at night due to limited information from single images.

Method: Proposes Depth-Memory Decoupling Network (DMDNet) with three key components: Depth-Aware Scanning (DAScan) to guide Mamba toward salient structures, Depth-Synergized State-Space Model (DS-SSM) to modulate state activations by depth, and Memory Expert Compensation Module (MECM) leveraging cross-image historical knowledge for layer-specific compensation.

Result: DMDNet outperforms state-of-the-art methods in both daytime and nighttime reflection separation. The paper also introduces the Nighttime Image Reflection Separation (NightIRS) dataset to address the lack of nighttime datasets.

Conclusion: DMDNet effectively addresses the challenge of separating image reflections when layers have similar contrasts, particularly in nighttime conditions, through depth-aware state-space modeling and memory compensation mechanisms.
...
TL;DR: HarmoniAD is a frequency-guided dual-branch anomaly detection framework that balances structure and semantics by decoupling features into high- and low-frequency paths for complementary modeling, achieving state-of-the-art performance on industrial defect detection benchmarks.
...
Method: Proposes HarmoniAD, a frequency-guided dual-branch framework where features from CLIP image encoder are transformed to frequency domain and decoupled into high- and low-frequency paths. High-frequency branch uses fine-grained structural attention module (FSAM) for textures/edges, while low-frequency branch uses global structural context module (GSCM) for long-range dependencies and semantic consistency. Also employs multi-class joint training strategy.

Result: Experiments on MVTec-AD, VisA, and BTAD benchmarks show state-of-the-art performance with both sensitivity and robustness in detecting industrial defects.

Conclusion: HarmoniAD effectively addresses the structure-semantics trade-off in anomaly detection through complementary frequency-domain modeling, achieving superior performance in industrial defect detection while balancing fine detail preservation and global semantic understanding.
...
Method: Unifies geometry and appearance modeling into joint latent representation using 3D Gaussian representations compressed via shared sparse VAE, then uses bridge diffusion to infer missing components from partial observations.

Result: Outperforms state-of-the-art approaches in both geometry fidelity and appearance quality, including challenging in-the-wild scenarios.

Conclusion: JGA-LBD provides a unified framework that successfully integrates geometry and appearance reconstruction from single images, achieving superior results through joint latent representation and bridge diffusion.
...
<details>
  <summary>Details</summary>
Motivation: Speeding is a major contributor to road fatalities in developing countries like Uganda where road safety infrastructure is limited. There's an urgent need for automated traffic enforcement systems in resource-constrained environments.

Method: Used computer vision techniques with YOLOv8 for license plate detection (97.9% mAP), CNN and transformer models for character recognition (3.85% and 1.79% CER respectively), and source/target ROI for speed estimation. Collected dataset using speed gun, Canon camera, and mobile phone. Integrated Africa's Talking API for automated SMS ticket issuance.

Result: High-performance results: 97.9% mAP for license plate detection, 1.79% CER for character recognition with transformer model, and 10 km/h margin of error for speed estimation. System successfully correlates user information with vehicle data for automated enforcement.

Conclusion: The proposed system effectively addresses critical traffic management needs in resource-constrained environments and shows strong potential to reduce road accidents through automated traffic enforcement in developing countries where such interventions are urgently needed.
...
Method: Proposes OmniVaT framework with: 1) Multimodal Fractional Fourier Adapter (MFFA) to map visual and tactile embeddings into unified embedding-frequency space, mitigating modality gap; 2) Discrete Tree Generation (DTG) module that obtains diverse multimodal fractional representations through hierarchical tree structure for better domain adaptation.

Result: Extensive experiments demonstrate superior cross-domain generalization performance on the SDG-VTL (single domain generalization for multimodal visual-tactile learning) task.
...
<details>
  <summary>Details</summary>
Motivation: Robots in domestic environments need comprehensive scene understanding to interact effectively with untrained humans, requiring more flexible approaches than traditional fixed-class semantic segmentation.

Method: Uses knowledge distillation from Alpha-CLIP teacher embeddings to train efficient student model DVEFormer, a Transformer-based RGB-D approach that learns fine-grained pixel-wise embeddings aligned with text.

Result: Achieves competitive performance on indoor datasets while meeting real-time requirements (26.3 FPS full model, 77.0 FPS smaller variant on NVIDIA Jetson AGX Orin), enabling text-based querying and 3D mapping applications.
...
<details>
  <summary>Details</summary>
Motivation: Digital restoration of cultural heritage artifacts requires joint reconstruction of both geometry and color from damaged 3D objects.
...
Result: Produces more complete geometry and coherent color reconstructions than symmetry-based baselines at fixed 32^3 resolution on synthetic damage datasets.

Conclusion: Explicit mask conditioning is a practical way to guide volumetric diffusion models for joint 3D geometry and color inpainting tasks.
...
TL;DR: A probabilistic dual-stream framework for skeleton-based human action recognition that unifies reliability modeling and multi-modal integration, focusing on both body and subtle hand articulations for fine-grained recognition.


<details>
  <summary>Details</summary>
Motivation: Existing skeleton-based HAR methods are body-centric and neglect subtle hand articulations crucial for fine-grained recognition. There's a need for a framework that can handle uncertainty and integrate multiple modalities while focusing on both large-scale motions and fine hand movements.

Method: Three key components: 1) Calibration-free preprocessing pipeline that learns directly from native coordinates without canonical-space transformations; 2) Probabilistic Noisy-OR fusion for reliability-aware dual-stream learning without explicit confidence supervision; 3) Intra- to cross-modal ensemble coupling four skeleton modalities (Joint, Bone, Joint Motion, Bone Motion) with RGB representations.

Result: Comprehensive evaluations across multiple benchmarks (NTU RGB+D 60/120, PKU-MMD, N-UCLA) and a newly defined hand-centric benchmark show consistent improvements and robustness under noisy and heterogeneous conditions.

Conclusion: The proposed probabilistic dual-stream framework successfully addresses the limitations of body-centric approaches by focusing on both body and hand articulations, demonstrating improved performance and robustness through unified reliability modeling and multi-modal integration.
...
TL;DR: NeoVerse is a versatile 4D world model that achieves scalable 4D reconstruction and novel-trajectory video generation from monocular videos, overcoming limitations of existing methods that require expensive multi-view data or cumbersome pre-processing.


<details>
  <summary>Details</summary>
Motivation: Current 4D world modeling methods suffer from scalability limitations due to either expensive specialized multi-view 4D data requirements or cumbersome training pre-processing, making them impractical for diverse real-world applications.

Method: NeoVerse features pose-free feed-forward 4D reconstruction, online monocular degradation pattern simulation, and other well-aligned techniques to create a scalable pipeline that works with diverse in-the-wild monocular videos.
...
Conclusion: NeoVerse provides a scalable and versatile 4D world modeling solution that enables 4D reconstruction, novel-trajectory video generation, and rich downstream applications from monocular videos, overcoming key limitations of existing approaches.
...
TL;DR: RoLID-11K is the first large-scale dataset for roadside litter detection from dashcams, featuring 11k annotated images with extreme small objects and long-tail distribution, benchmarked against modern detectors.
...
Result: CO-DETR and related transformers achieve best localization accuracy, while real-time models are constrained by coarse feature hierarchies. The dataset exhibits pronounced long-tail and small-object distributions.
...
TL;DR: ABFR-KAN is a transformer-based classification network that uses Kolmogorov-Arnold Networks (KANs) and novel brain function representation components to improve functional connectivity analysis for autism spectrum disorder diagnosis, addressing limitations of traditional atlas-based parcellation methods.


<details>
  <summary>Details</summary>
Motivation: Traditional functional connectivity analysis relies on atlas-based parcellation, which suffers from selection bias and lacks subject specificity. These limitations can affect the reliability of brain disorder diagnosis.

Method: Proposes ABFR-KAN, a transformer-based classification network incorporating novel advanced brain function representation components with Kolmogorov-Arnold Networks (KANs) to mitigate structural bias, improve anatomical conformity, and enhance FC estimation reliability.

Result: Extensive experiments on the ABIDE I dataset show ABFR-KAN consistently outperforms state-of-the-art baselines for ASD classification, validated through cross-site evaluation and ablation studies across varying model backbones and KAN configurations.

Conclusion: ABFR-KAN effectively addresses limitations of traditional atlas-based parcellation methods and demonstrates superior performance for autism spectrum disorder classification, offering a more reliable approach for computer-aided brain disorder diagnosis.
...
TL;DR: Proposes Anomaly Quadruplet-Net for robust assembly progress estimation using Quadruplet Loss and strategic data sampling, improving accuracy by 1.3% and reducing adjacent task misclassification by 1.9% on PC assembly dataset.


<details>
  <summary>Details</summary>
Motivation: Manual multi-day assembly tasks challenge smart factory implementation. Existing Anomaly Triplet-Net struggles with subtle visual changes between consecutive tasks, leading to misclassification. Need robust progress estimation despite occlusion or minimal visual changes using small datasets.

Method: Proposes Anomaly Quadruplet-Net with Quadruplet Loss-based learning for anomaly images. Introduces custom data loader that strategically selects training samples to enhance estimation accuracy. Evaluated on desktop PC assembly image dataset.
...
Conclusion: Proposed method effectively addresses challenges of subtle visual changes in assembly progress estimation, demonstrating robustness with small-scale datasets for smart factory applications.
...
<details>
  <summary>Details</summary>
Motivation: While RL has advanced language model reasoning, extending it to multimodal reasoning requires improving both perception and reasoning. Previous methods struggle with disentangling perception from reasoning tokens, requiring extra LLMs, ground-truth data, or applying rewards indiscriminately to all tokens.

Method: CPPO detects perception tokens via entropy shifts in model outputs under perturbed input images. It extends the RL objective with Contrastive Perception Loss (CPL) that enforces consistency under information-preserving perturbations and sensitivity under information-removing perturbations.

Result: Experiments show CPPO surpasses previous perception-rewarding methods while avoiding extra models, making training more efficient and scalable.

Conclusion: CPPO provides an effective approach for finetuning vision-language models by addressing the perception-reasoning disentanglement challenge through entropy-based token detection and contrastive learning, enabling more efficient and scalable multimodal reasoning.
...
TL;DR: MotionPhysics is a differentiable framework that infers physical parameters from natural language prompts to generate realistic 3D dynamic simulations without needing ground-truth trajectories or annotated videos.


<details>
  <summary>Details</summary>
Motivation: Simulating 3D objects with various materials requires expert knowledge and time-consuming parameter tuning. Current methods need ground-truth trajectories or annotated videos, which are expensive to obtain.

Method: Uses multimodal LLM to estimate material parameters within plausible ranges, plus a learnable motion distillation loss that extracts motion priors from pretrained video diffusion models while minimizing appearance/geometry biases.

Result: Evaluated across 30+ scenarios with real-world, human-designed, and AI-generated 3D objects, covering diverse materials. Produces visually realistic dynamic simulations guided by natural language, surpassing state-of-the-art.

Conclusion: MotionPhysics enables automatic determination of physically plausible parameters for realistic 3D simulations from natural language prompts, eliminating need for expert tuning or ground-truth data.
...
TL;DR: ORCANet is a video restoration model that handles smoothly evolving unknown degradations over time using recurrent conditional adaptive prompting and flow-based degradation feature extraction.


<details>
  <summary>Details</summary>
Motivation: Existing video restoration approaches focus on frame-wise degradation variation but ignore temporal continuity in real-world degradation processes where degradation types and intensities evolve smoothly over time, and multiple degradations may coexist or transition gradually.
...
Conclusion: ORCANet effectively addresses the Smoothly Evolving Unknown Degradations (SEUD) scenario in video restoration through a novel architecture that captures temporal degradation continuity and provides adaptive prompting for diverse evolving degradations.
...
TL;DR: FreeText is a training-free framework that improves text rendering in diffusion models by localizing writing regions via attention analysis and injecting glyph priors with frequency modulation.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image diffusion models struggle with precise text rendering, especially for complex layouts, dense typography, and scripts like Chinese. Existing solutions require costly retraining or rigid external constraints that degrade aesthetics and flexibility.

Method: FreeText decomposes text rendering into two parts: 1) "where to write" - localizes writing regions using token-wise spatial attribution from image-to-text attention with sink-like tokens as anchors and topology-aware refinement; 2) "what to write" - introduces Spectral-Modulated Glyph Injection (SGMI) that injects noise-aligned glyph priors with frequency-domain band-pass modulation to strengthen glyph structure while suppressing semantic leakage.
...
Conclusion: FreeText provides an effective training-free, plug-and-play solution for improving text rendering in diffusion models without requiring retraining or rigid constraints, offering better flexibility and maintaining aesthetic quality.
...
TL;DR: VNS-SAM enhances Segment Anything Model's performance in visually non-salient scenarios (low contrast foreground/background) while preserving zero-shot generalizability, using minimal parameter/computation overhead.
...
Method: Proposes VNS-SAM with two key designs: 1) Mask-Edge Token Interactive decoder, and 2) Non-Salient Feature Mining module, which exploit SAM's low-level features to better understand non-salient characteristics. Also introduces VNS-SEG dataset with 35K+ images for training and benchmarking.

Result: VNS-SAM achieves superior performance across various VNS segmentation tasks, especially in zero-shot settings, with minimal parameter increments (optimizable within 4 hours) while maintaining SAM's original generalizability.

Conclusion: VNS-SAM effectively addresses SAM's limitations in visually non-salient scenarios, demonstrating practical feasibility and potential for broad real-world applications through efficient architecture modifications and comprehensive dataset creation.
...
TL;DR: DynaDrag introduces a new "predict-and-move" framework for drag-style image editing that iteratively predicts handle point movements and drags them, addressing issues in previous methods.
...
Method: DynaDrag uses a predict-and-move framework with iterative Motion Prediction (predicting where handle points should move) and Motion Supervision (dragging them accordingly), plus dynamic adjustment of valid handle points.

Result: Experiments on face and human datasets demonstrate superiority over previous works in drag-style image editing.

Conclusion: DynaDrag effectively addresses limitations of previous drag-style editing methods through its novel predict-and-move framework with iterative motion prediction and supervision.
...
TL;DR: SlingBAG Pro is an advanced 3D photoacoustic imaging reconstruction algorithm that extends the original SlingBAG method to work with irregular transducer arrays, achieving faster reconstruction (2.2× speedup) while maintaining quality.
...
Result: SlingBAG Pro achieves up to 2.2-fold speed improvement in point cloud-based 3D PA reconstruction under irregular array geometries compared to the original SlingBAG algorithm, while maintaining high reconstruction quality. The method is validated through simulation and in vivo mouse experiments.

Conclusion: SlingBAG Pro successfully addresses the challenges of irregular array configurations in 3D photoacoustic imaging, offering faster reconstruction times while maintaining quality, making it suitable for clinical applications with limited space and cost constraints. The source code is publicly available for community use.
...
TL;DR: MS COCOAI is a new dataset for AI-generated image detection with 96K real/synthetic images from 5 generators, enabling classification of real vs. fake and model attribution tasks.


<details>
  <summary>Details</summary>
Motivation: Multimodal AI systems like Stable Diffusion and DALL-E enable image generation but also facilitate spread of misleading content and manipulated media. As synthetic images become indistinguishable from real photos, detecting them has become an urgent priority.

Method: Created MS COCOAI dataset using MS COCO as base, generating synthetic images with 5 generators: Stable Diffusion 3, Stable Diffusion 2.1, SDXL, DALL-E 3, and MidJourney v6. Dataset contains 96,000 real and synthetic datapoints.

Result: Released a novel dataset for AI-generated image detection available on HuggingFace. Proposed two benchmark tasks: (1) classifying images as real or generated, and (2) identifying which specific model produced a given synthetic image.

Conclusion: MS COCOAI provides a comprehensive resource for developing and evaluating AI-generated image detection methods, addressing the growing challenge of distinguishing synthetic content from real photographs in the era of advanced generative AI.
...
TL;DR: AEGIS benchmark evaluates UMMs' world knowledge across multiple tasks using deterministic checklist evaluation, revealing significant knowledge deficits and reasoning limitations.
...
Method: Proposed AEGIS benchmark with 1,050 manually-annotated questions spanning 21 topics and 6 reasoning types, covering visual understanding, generation, editing, and interleaved generation. Also introduced Deterministic Checklist-based Evaluation (DCE) protocol using atomic "Y/N" judgments instead of ambiguous prompt-based scoring.
...
Conclusion: World-knowledge-based reasoning represents a critical frontier for UMM development, and the AEGIS benchmark with DCE provides a more reliable evaluation framework for assessing UMM capabilities.
...
TL;DR: Proposes a cascaded CNN with Global Information Guidance Module for robust visual segmentation in complex scenarios, outperforming SOTA methods.
...
Conclusion: The proposed framework demonstrates effectiveness and promising potential for deployment in practical robotic applications requiring robust visual segmentation.
...
Result: Achieves new state-of-the-art across all three major benchmarks (QVHighlights, Charades-STA, ActivityNet-Captions), with 3.23% mAP@avg improvement on challenging QVHighlights dataset.

Conclusion: GranAlign effectively resolves semantic granularity mismatches in zero-shot video moment retrieval without requiring task-specific training, demonstrating superior performance through complementary query and video adaptation techniques.
...
TL;DR: SafeMo is a trustworthy motion generation framework that uses Minimal Motion Unlearning (MMU) to enable safe human motion generation in continuous space, avoiding artifacts from discrete codebook methods while maintaining good performance on benign prompts.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-motion generation methods have safety concerns, and current safety approaches using discrete VQ-VAE codebook replacement cause two main problems: 1) degradation of benign performance when codebook entries are reused, and 2) artifacts and jerky transitions from quantization loss. Additionally, existing datasets contain unsafe content unsuitable for safety-focused learning.

Method: Proposes SafeMo framework with Minimal Motion Unlearning (MMU), a two-stage machine unlearning strategy that operates in continuous space to avoid codebook quantization loss. Also introduces SafeMoVAE-29K dataset with rewritten safe text prompts and refined continuous motions for trustworthy unlearning. Built upon DiP for efficient safe motion generation with natural transitions.

Result: Achieves effective unlearning with strengthened forgetting on unsafe prompts - reaches 2.5x and 14.4x higher forget-set FID on HumanML3D and Motion-X respectively compared to previous SOTA method LCR. Maintains comparable or better performance on safe prompts, demonstrating strong safety-utility trade-offs.

Conclusion: SafeMo provides a trustworthy motion generation framework that addresses safety concerns in text-to-motion systems through continuous-space unlearning, avoiding the limitations of discrete codebook methods while maintaining good performance on everyday tasks.
...
TL;DR: MDACL framework addresses optimization bias in RGB-IR fusion by measuring modality dominance and balancing cross-modal learning.
...
Method: Proposes Modality Dominance Index (MDI) to quantify dominance via feature entropy and gradient contribution. Develops MDACL framework with Hierarchical Cross-modal Guidance (HCG) for feature alignment and Adversarial Equilibrium Regularization (AER) to balance optimization dynamics.
...
Conclusion: MDACL successfully addresses modality dominance issues in RGB-IR fusion through quantitative measurement and balanced optimization, advancing multimodal perception for embodied systems.
...
<details>
  <summary>Details</summary>
Motivation: Tiny object detection suffers from a persistent performance gap compared to normal-scale objects, and these tiny objects are highly sensitive to annotation noise. Current methods that optimize strict localization objectives risk overfitting to this noise.
...
Result: Extensive experiments across three datasets validate the approach's effectiveness. TOLF boosts the DINO baseline by 1.2% AP on the AI-TOD dataset, demonstrating improved performance for tiny object detection.

Conclusion: TOLF provides a robust solution for tiny object localization by addressing annotation noise sensitivity through flexible error modeling and uncertainty-guided optimization, bridging the performance gap between tiny and normal-scale object detection.
...
TL;DR: RePose: A real-time 3D human pose estimation and motion analysis system for rehabilitation training that monitors patients' movements, provides immediate feedback, and displays muscle stress conditions using Unity platform.


<details>
  <summary>Details</summary>
Motivation: To develop a real-time system for rehabilitation training that can monitor and evaluate patients' motions, provide immediate feedback and guidance, and help patients execute rehabilitation exercises correctly to regain muscle strength and motor functions.

Method: 1) Unified pipeline for end-to-end real-time human pose estimation and motion analysis using RGB video from multiple cameras; 2) Fast tracking method for medical rehabilitation scenarios with multiple-person interference (<1ms per frame); 3) Modified SmoothNet for real-time posture estimation to reduce errors and restore true motion state; 4) Unity platform for real-time monitoring, evaluation, and muscle stress visualization.

Result: The system achieves real-time 3D human pose estimation and motion analysis for rehabilitation, with fast tracking (<1ms per frame), reduced pose estimation errors, smoother visual motion representation, and real-time monitoring capabilities with muscle stress display.

Conclusion: RePose provides an effective real-time solution for rehabilitation training by combining pose estimation, motion analysis, and immediate feedback, enabling better patient monitoring, guidance, and recovery support through visual feedback and muscle stress analysis.
...
TL;DR: HyperPriv-EPN: A hypergraph-based LUPI framework that uses post-operative surgical reports as privileged information during training to improve preoperative ependymoma prognosis, without needing text at inference.


<details>
  <summary>Details</summary>
Motivation: Preoperative prognosis of ependymoma is challenging because MRI lacks semantic insights compared to post-operative surgical reports. Existing multimodal methods fail when privileged text data is unavailable during inference.

Method: Hypergraph-based Learning Using Privileged Information (LUPI) framework with Severed Graph Strategy. Uses shared encoder for both Teacher graph (with post-surgery info) and Student graph (pre-op only). Dual-stream distillation enables Student to hallucinate semantic community structures from visual features alone.
...
<details>
  <summary>Details</summary>
Motivation: To develop a non-invasive, scalable solution for monitoring potato quality during storage, addressing challenges like sprout detection, weight loss estimation, and shelf-life prediction to improve inventory management and reduce food waste.

Method: Collected images and weight data over 200 days under controlled conditions; used pre-trained architectures (ResNet, VGG, DenseNet, ViT) to design two specialized models: a binary classifier for sprout detection and a multi-class predictor for weight loss and shelf-life estimation.
...
Conclusion: Image-based models can be integrated into automated systems for early sprout detection and dynamic categorization, improving inventory management and reducing waste. Future research should focus on generalized models for diverse potato varieties and storage conditions.
...
Conclusion: Proposed learning-based framework effectively converts raw TomoSAR points into continuous height maps, addressing key challenges in TomoSAR-based building height estimation.
...
TL;DR: CRoPS is a training-free framework that mitigates hallucinations in Large Vision-Language Models by using multiple hallucinated models and generalized contrastive decoding, achieving 20% improvement in CHAIR scores across multiple benchmarks.
...
Conclusion: CRoPS effectively addresses hallucination mitigation in LVLMs through a novel training-free approach that better captures diverse hallucination sources and maintains effectiveness throughout generation.
...
TL;DR: Single-image video generation with explicit 3D Gaussian scene representation and object motion sampling in one forward pass for camera-controlled video synthesis.
...
Method: Proposes a novel framework that constructs a 3D Gaussian scene representation and samples plausible object motion from a single image in a single forward pass, enabling fast camera-guided video generation without iterative denoising.
...
TL;DR: Downsampling in isotropic networks improves efficiency and performance for mobile image demosaicing applications.
...
Method: Designed simple fully convolutional networks with and without downsampling using DeepMAD mathematical architecture design. Created JD3Net as the downsampled variant for empirical testing.

Result: Downsampling improves empirical performance. JD3Net shows strong performance on various image demosaicing and joint-demosaicing-and-denoising (JDD) tasks.

Conclusion: Contrary to previous isotropic network designs, spatial downsampling significantly improves efficiency and performance for mobile demosaicing applications, making deep learning approaches more practical for real-world mobile imaging.
...
TL;DR: RGS-SLAM replaces residual-driven densification in GS-SLAM with a training-free correspondence-to-Gaussian initialization using DINOv3 descriptors and confidence-aware inlier classification for faster, more stable mapping.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of progressive Gaussian densification in GS-SLAM, which can lead to unstable early mapping and slower convergence, by providing a better initialization strategy.

Method: Uses DINOv3 descriptors to extract dense multi-view correspondences, refines them with a confidence-aware inlier classifier, performs one-shot triangulation to generate structure-aware Gaussian seeds, and integrates this into existing GS-SLAM pipelines.

Result: Achieves ~20% faster convergence, higher rendering fidelity in texture-rich/cluttered scenes, competitive/superior localization and reconstruction accuracy on TUM RGB-D and Replica datasets, while maintaining real-time performance up to 925 FPS.

Conclusion: RGS-SLAM demonstrates that training-free correspondence-to-Gaussian initialization provides more stable early mapping and faster convergence than residual-driven densification, while maintaining compatibility with existing GS-SLAM systems and achieving state-of-the-art performance.
...
TL;DR: This paper proposes a framework for detecting performance degradation in Vision-Language Models (VLMs) under data shift in medical pathology, combining input-level data shift detection with output-level confidence-based monitoring.


<details>
  <summary>Details</summary>
Motivation: VLMs show promise in medical image analysis but can degrade after deployment when input data distribution shifts from development data. Detecting such degradation is crucial for clinical reliability but challenging for large pre-trained VLMs without labeled data.

Method: Developed DomainSAT toolbox for input data shift detection, and introduced a label-free, confidence-based degradation indicator for output monitoring. Combined both approaches for comprehensive performance degradation detection.

Result: Input data shift detection identifies distributional changes but doesn't always correlate with actual performance degradation. The confidence-based indicator closely relates to performance degradation and effectively complements input shift detection.

Conclusion: Combining input data shift detection with output confidence-based indicators provides a practical, complementary framework for reliable detection and interpretation of performance degradation in VLMs under data shift in digital pathology.
...
Method: Multi-level feature fusion (MLFF) approach that utilizes representations from different depths of a pretrained network. This method leverages features at various network levels rather than just end-to-end training.
...
Conclusion: The MLFF approach enables efficient continual learning for manufacturing quality inspection by balancing computational efficiency with performance retention, making deep learning more practical for volatile industrial settings like remanufacturing.
...
Conclusion: The workflow provides reliable automated grading for handwritten STEM exams while preserving standard exam processes. Structured prompting and reference grounding are critical components for accurate, unbiased grading.
...
TL;DR: UniCo is a single-pass structured shape completion method that predicts complete primitives with semantics and inlier membership using primitive proxies, outperforming baselines by up to 50% in Chamfer distance.


<details>
  <summary>Details</summary>
Motivation: Current structured shape completion methods follow a cascade approach where primitives and points interact in suboptimal ways. The authors rethink this interaction to find a more effective way to decode primitives from shared shape features.

Method: UniCo uses primitive proxies (learnable queries) that are contextualized to produce assembly-ready outputs in a single feed-forward pass. It features a dedicated pathway for primitive decoding that attends to shared shape features, coupled with an online target update training strategy that links primitives and points.
...
Conclusion: The approach establishes an attractive recipe for structured 3D understanding from incomplete data by rethinking primitive-point interaction and using primitive proxies for unified representation.
...
Method: Examined different combinations of training schemes for self-supervised auxiliary tasks and primary deepfake detection tasks. Used feature fusion from self-supervised representations to create powerful combined representations.

Result: Fusing self-supervised feature representations achieved better performance on the primary deepfake detection task and showed superior cross-dataset generalization on DF40, FaceForensics++, Celeb-DF, DFD, FaceShifter, and UADFV datasets compared to state-of-the-art detectors.

Conclusion: Self-supervised learning as an auxiliary task provides powerful feature representations that enhance generalized deepfake detection performance and improve cross-dataset generalization capabilities.
...
TL;DR: Two novel deep learning architectures (LNU-Net and IBU-Net) for left ventricle segmentation from cardiac MRI, using layer normalization and instance-batch normalization respectively, outperform U-Net and other state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Left ventricle segmentation is critical for clinical quantification and diagnosis of cardiac images, requiring accurate and reliable automated methods to assist medical professionals.

Method: Proposed two U-Net variants: LNU-Net applies layer normalization in each convolutional block, while IBU-Net incorporates instance and batch normalization together in the first convolutional block. Both use down-sampling for feature extraction and up-sampling for localization, with affine transformations and elastic deformations for data augmentation.

Result: Both LNU-Net and IBU-Net outperform the original U-Net and other state-of-the-art approaches on a dataset of 805 MRI images from 45 patients, achieving better dice coefficient and average perpendicular distance metrics.

Conclusion: The proposed normalization-based U-Net variants (LNU-Net and IBU-Net) provide effective solutions for left ventricle segmentation in cardiac MRI, demonstrating superior performance compared to existing methods.
...
TL;DR: AdaGaR is a unified framework for dynamic 3D scene reconstruction from monocular videos that addresses frequency adaptivity and temporal continuity using Adaptive Gabor Representation and Cubic Hermite Splines with Temporal Curvature Regularization.


<details>
  <summary>Details</summary>
Motivation: Existing methods using single Gaussian primitives have low-pass filtering limitations, standard Gabor functions suffer from energy instability, and lack of temporal continuity constraints leads to motion artifacts during interpolation in dynamic scene reconstruction.

Method: 1) Adaptive Gabor Representation extends Gaussians with learnable frequency weights and adaptive energy compensation; 2) Cubic Hermite Splines with Temporal Curvature Regularization for smooth motion; 3) Adaptive Initialization combining depth estimation, point tracking, and foreground masks for stable point cloud distributions.

Result: State-of-the-art performance on Tap-Vid DAVIS with PSNR 35.49, SSIM 0.9433, LPIPS 0.0723, and strong generalization across frame interpolation, depth consistency, video editing, and stereo view synthesis tasks.

Conclusion: AdaGaR successfully addresses both frequency adaptivity and temporal continuity challenges in dynamic scene reconstruction, achieving superior performance and generalization across multiple applications through its unified framework.
