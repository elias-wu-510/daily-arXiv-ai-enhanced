...
TL;DR: The paper analyzes how motion is encoded across timesteps in text-to-video diffusion models, identifying distinct motion-dominant and appearance-dominant regimes, and proposes a simplified motion customization method using only the motion-dominant regime.


<details>
  <summary>Details</summary>
Motivation: Current understanding of how motion is encoded across timesteps in video diffusion models is poor, despite practitioners using the heuristic that early timesteps shape motion/layout while later ones refine appearance. This behavior hasn't been systematically characterized.

Method: The authors proxy motion encoding by analyzing the trade-off between appearance editing and motion preservation when injecting new conditions over specific timestep ranges. They conduct large-scale quantitative studies across diverse architectures to map how motion and appearance compete along the denoising trajectory.

Result: Consistently identified an early motion-dominant regime and later appearance-dominant regime across different architectures, establishing an operational motion-appearance boundary in timestep space. This enables simplification of motion customization by restricting training/inference to the motion-dominant regime.

Conclusion: The analysis transforms a widely used heuristic into a spatiotemporal disentanglement principle. The timestep-constrained recipe can be integrated into existing motion transfer and editing methods, achieving strong motion transfer without needing auxiliary debiasing modules or specialized objectives.
...
TL;DR: Real-time ASL recognition system using 3D CNN-LSTM hybrid architecture achieves high accuracy (F1-scores 0.71-0.99) on word-level sign recognition from webcam video streams.
...
Method: Hybrid deep learning architecture combining 3D CNNs to capture spatial-temporal features from video frames with LSTM layers to model sequential dependencies in sign language gestures. Trained on WLASL dataset (2,000 words), ASL-LEX database (~2,700 signs), and 100 expert-annotated signs.
...
Conclusion: The hybrid 3D CNN-LSTM architecture effectively recognizes ASL signs in real-time, with practical deployment considerations for accessibility applications.
...
<details>
  <summary>Details</summary>
Motivation: The rapid evolution of AI in healthcare creates opportunities to enhance medical billing and transcription processes, aiming to reduce human error and streamline operations for faster, more accurate patient care documentation and financial transactions.

Method: Integration of AI with Locally Linear Embedding (LLE) to create a specialized model for handling high-dimensional medical data, with comprehensive mathematical modeling and real-world healthcare scenario experiments.
...
Conclusion: AI-enhanced LLE shows strong potential for medical data analysis and sets a foundation for broader healthcare applications, demonstrating value in automating complex medical data processes.
...
TL;DR: VISTA is a modular framework that separates perception from reasoning in vision-language models using an information bottleneck to reduce shortcut learning and improve robustness to spurious correlations.


<details>
  <summary>Details</summary>
Motivation: End-to-end VLMs often exploit spurious correlations instead of causal visual evidence, becoming even more shortcut-prone when fine-tuned. There's a need for more robust visual reasoning that doesn't rely on these shortcuts.

Method: VISTA uses a modular approach: a frozen VLM sensor handles short, objective perception queries, while a text-only LLM reasoner decomposes questions, plans queries, and aggregates visual facts in natural language. This creates an information bottleneck that separates perception from reasoning. The system is trained with reinforcement learning (GRPO) using only 641 curated multi-step questions.

Result: VISTA significantly improves robustness to real-world spurious correlations on SpuriVerse (+16.29% with Qwen-2.5-VL-7B and +6.77% with Llama-3.2-Vision-11B), remains competitive on MMVP and SeedBench, transfers robustly across unseen VLM sensors, and can recognize/recover from VLM perception failures. Human analysis shows more neutral reasoning traces with less reliance on spurious attributes.

Conclusion: The modular VISTA framework effectively addresses shortcut learning in VLMs by separating perception from reasoning through an information bottleneck, leading to more robust visual reasoning that's less susceptible to spurious correlations while maintaining competitive performance on standard benchmarks.
...
TL;DR: SAMM2D improves aneurysm detection by 32% over clinical baseline, but surprisingly finds that data augmentation hurts performance when using strong pretrained backbones, challenging conventional wisdom in medical imaging.


<details>
  <summary>Details</summary>
Motivation: Aneurysm detection is challenging due to subtle morphology, class imbalance, and scarce annotated data. Current approaches often rely on data augmentation to address data scarcity, but this may not be optimal with modern pretrained models.

Method: SAMM2D is a dual-encoder framework tested on the RSNA intracranial aneurysm dataset. The study conducted comprehensive ablation across six augmentation regimes to evaluate the impact of data augmentation when using strong pretrained backbones.

Result: Achieved 0.686 AUC (32% improvement over clinical baseline). Surprisingly, any form of data augmentation degraded performance with pretrained backbones - unaugmented baseline outperformed all augmented variants by 1.75-2.23 percentage points (p<0.01). With threshold calibration, reached 95% sensitivity surpassing radiologist performance, projected $13.9M savings per 1,000 patients. Grad-CAM showed 85% true positives attend to relevant vascular regions.

Conclusion: Strong pretrained features already capture robust invariances, making additional augmentations redundant and disruptive. Future medical imaging workflows may benefit more from strong pretraining than complex augmentation pipelines. The study overturns the assumption that "more augmentation is always better" in low-data medical settings.
...
TL;DR: HookMIL is a context-aware Multiple Instance Learning framework for computational pathology that uses learnable hook tokens for efficient contextual aggregation with linear complexity, achieving SOTA performance with improved efficiency and interpretability.


<details>
  <summary>Details</summary>
Motivation: Traditional MIL approaches lose crucial contextual information in whole-slide image analysis, while transformer-based variants suffer from quadratic complexity and redundant computations. There's a need for a more efficient and context-aware MIL framework for computational pathology.

Method: Proposes HookMIL with learnable hook tokens that can be initialized from: (1) key-patch visual features, (2) text embeddings from vision-language pathology models, and (3) spatially grounded features from spatial transcriptomics-vision models. Uses bidirectional attention with linear complexity, Hook Diversity Loss to encourage specialization on distinct patterns, and hook-to-hook communication to refine contextual interactions.

Result: Extensive experiments on four public pathology datasets demonstrate state-of-the-art performance with improved computational efficiency and interpretability compared to existing methods.

Conclusion: HookMIL provides an effective solution for context-aware MIL in computational pathology, balancing performance, efficiency, and interpretability through multimodal initialization and structured contextual aggregation mechanisms.
...
TL;DR: Tiny-YOLOSAM combines YOLOv12 detection with TinySAM segmentation to achieve faster, more efficient full-scene segmentation by using box prompts for salient objects and sparse point prompts for uncovered regions.
...
Method: Proposes Tiny-YOLOSAM: a hybrid pipeline using YOLOv12 detector to generate box prompts for salient foreground objects, supplemented with sparse point prompts only where YOLO-guided masks provide no coverage.

Result: On COCO val2017: class-agnostic coverage improves dramatically (AR from 16.4% to 77.1%, mIoU from 19.2% to 67.8%) while reducing runtime from 49.20s/image to 10.39s/image (4.7x speedup) on Apple M1 Pro CPU.
...
<details>
  <summary>Details</summary>
Motivation: Current AI models for diabetic retinopathy detection lack practical clinical explainability - they either require impractical manual lesion annotation or provide limited one-dimensional explanations. Physicians need models that explain classification reasoning rather than just highlighting lesions, and current approaches are limited to single imaging modalities.

Method: Multimodal explainability model using Vision-Language Model (VLM) with few-shot learning that mimics ophthalmologist reasoning by analyzing lesion distributions within retinal quadrants. The model generates paired Grad-CAM heatmaps showing individual neuron weights across both OCT and fundus images to visually highlight regions contributing to DR severity classification.

Result: Developed using a dataset of 3,000 fundus images and 1,000 OCT images, the model provides visual explanations of DR severity classification through heatmaps that show contributing regions in both imaging modalities, addressing key limitations in current DR diagnostics.

Conclusion: This innovative multimodal approach offers a practical and comprehensive tool for improving DR patient outcomes by providing interpretable explanations that mimic clinical reasoning, enabling diverse applications in screening, treatment, and research settings.
...
Method: Uses efficient vision transformer as feature extractor, Learnable Density-Weighted Averaging module for spatial adaptation without extra annotations, and density-level classification loss for regularization.
...
Conclusion: TCFormer provides an effective solution for edge device deployment by combining weak supervision, transformer efficiency, and adaptive density modeling.
...
TL;DR: Deep learning CNN achieves 96% accuracy for automated malaria diagnosis from blood cell images, outperforming traditional methods and enhanced with explainable AI techniques.


<details>
  <summary>Details</summary>
Motivation: Traditional malaria diagnostic methods (microscopic blood smear analysis) have low sensitivity, depend on expert judgment, and lack resources in remote settings, creating a need for automated, accurate diagnostic solutions.

Method: Proposes a custom Convolutional Neural Network (CNN) for classifying blood cell images as parasitized or uninfected, with comparison to established architectures (ResNet50, VGG16, MobileNetV2, DenseNet121) and application of Explainable AI techniques (SHAP, LIME, Saliency Maps) for interpretability.

Result: The custom CNN achieves 96% accuracy with precision and recall scores exceeding 0.95 for both classes, demonstrating superior performance for automated malaria diagnosis.

Conclusion: Deep learning provides quick, accurate, and understandable malaria diagnosis, especially valuable in resource-limited areas, with explainable AI enhancing model interpretability and clinical trust.
...
TL;DR: Signal-SGN++ is a topology-aware spiking graph framework that combines structural adaptivity with time-frequency spiking dynamics for energy-efficient action recognition, achieving competitive accuracy with substantially reduced energy consumption compared to GCNs.


<details>
  <summary>Details</summary>
Motivation: GCNs are effective for skeletal action recognition but have high energy costs due to dense floating-point computations. SNNs offer energy efficiency through event-driven sparse activation but struggle to capture coupled temporal-frequency and topological dependencies of human motion. The paper aims to bridge this gap.
...
Conclusion: Signal-SGN++ successfully bridges the gap between GCNs and SNNs by combining structural adaptivity with time-frequency spiking dynamics, offering an energy-efficient solution for skeletal action recognition without compromising accuracy.
...
TL;DR: VLM-PAR: A vision-language framework using frozen SigLIP 2 multilingual encoders with cross-attention fusion achieves SOTA on imbalanced PA100K and improves performance on PETA and Market-1501 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Pedestrian Attribute Recognition faces challenges of severe class imbalance, complex attribute dependencies, and domain shifts, requiring robust solutions for accurate fine-grained attribute prediction.

Method: VLM-PAR uses frozen SigLIP 2 multilingual vision-language encoders with a compact cross-attention fusion module to align image and prompt embeddings, refining visual features for better attribute recognition.

Result: Achieves state-of-the-art on the imbalanced PA100K benchmark and delivers significant mean accuracy improvements on PETA and Market-1501 benchmarks.
...
TL;DR: ViSignVQA: First large-scale Vietnamese signboard VQA dataset with 10,762 images and 25,573 QA pairs, showing OCR-enhanced models improve F1-score by up to 209%.


<details>
  <summary>Details</summary>
Motivation: Signboard text understanding in natural scenes is crucial for real-world VQA applications but remains underexplored, especially for low-resource languages like Vietnamese. Current VQA datasets lack focus on signboard-specific challenges including bilingual text, informal phrasing, and visual elements.

Method: Created ViSignVQA dataset capturing Vietnamese signboard characteristics. Adapted SOTA VQA models (BLIP-2, LaTr, PreSTU, SaL) by integrating Vietnamese OCR (SwinTextSpotter) and language model (ViT5). Proposed multi-agent VQA framework combining perception and reasoning agents with GPT-4 using majority voting.

Result: OCR-enhanced context significantly improves performance with F1-score improvements up to 209% when OCR text is appended to questions. Multi-agent framework achieves 75.98% accuracy via majority voting. First large-scale multimodal dataset for Vietnamese signboard understanding.
...
<details>
  <summary>Details</summary>
Motivation: To enable household robots to efficiently find lost objects that are hidden/occluded, saving time and effort compared to naive random search approaches.

Method: Uses 2D Vision-Language Models' relevancy activations as confidence maps for object presence, treating them as "abstract object" representations. Learns 3D localization and completion specifically for hidden objects (occluded objects that VLMs cannot directly identify), incorporating historical placement data to optimize unstructured search.

Result: The model can accurately identify complete 3D locations of hidden objects on the first try, significantly faster than naive random search methods.

Conclusion: Semantic Abstraction extensions provide household robots with improved skills for efficiently finding lost objects by leveraging VLM relevancy maps and historical data for hidden object localization.
...
TL;DR: VideoScaffold is a dynamic framework for streaming video understanding that adaptively adjusts event granularity and preserves visual semantics through elastic event segmentation and hierarchical consolidation.
...
Result: Extensive experiments across both offline and streaming video understanding benchmarks demonstrate that VideoScaffold achieves state-of-the-art performance, and the framework is modular and plug-and-play, seamlessly extending existing image-based MLLMs to continuous video comprehension.

Conclusion: VideoScaffold provides an effective dynamic representation framework for streaming video understanding that addresses the limitations of static methods by adaptively adjusting event granularity while preserving visual semantics, enabling better handling of temporal coherence and redundancy in continuous video streams.
...
TL;DR: The paper introduces KAN-FPN-Stem, a novel front-end architecture for Vision Transformers that replaces the standard linear smoothing convolution in FPN with a KAN-based convolutional layer to address feature fusion bottlenecks in dense prediction tasks like pose estimation.


<details>
  <summary>Details</summary>
Motivation: Current ViT models like ViTPose suffer from simplistic front-end designs with naive patchification mechanisms that struggle with multi-scale variations and cause irreversible information loss during initial feature extraction. The authors identified that the performance bottleneck lies in the post-fusion non-linear smoothing step within FPN rather than attention modules.
...
Result: Extensive experiments on COCO dataset show KAN-FPN-Stem achieves up to +2.0 AP improvement over the lightweight ViTPose-S baseline, demonstrating significant performance gains in pose estimation tasks.

Conclusion: The work provides a plug-and-play, high-performance module that reveals the performance bottleneck in ViT front-ends often lies in feature fusion quality rather than feature refinement via attention. It offers an effective solution through KAN operators to address this bottleneck.
...
TL;DR: MiG-DM is a meta-information guided cross-domain diffusion model that integrates patient-specific clinical data and projection-domain physics to generate high-quality low-dose PET images, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Low-dose PET imaging reduces patient radiation exposure but suffers from noise, reduced contrast, and loss of physiological details. Existing methods neglect both projection-domain physics knowledge and patient-specific meta-information needed for functional-semantic correlation mining.

Method: MiG-DM integrates comprehensive cross-modal priors through: 1) Meta-information encoding module that transforms clinical parameters (patient characteristics, dose info, semi-quantitative parameters) into semantic prompts for cross-modal alignment; 2) Cross-domain architecture combining projection-domain and image-domain processing, with a specialized sinogram adapter capturing global physical structures through convolution operations equivalent to global image-domain filtering.
...
Conclusion: The proposed meta-information guided cross-domain synergistic diffusion model effectively integrates patient-specific clinical data and projection-domain physics to generate superior low-dose PET images, addressing key limitations of existing methods.
...
Result: On rice seed variety classification, distilled student achieves 98.56% accuracy (vs teacher's 98.65%) with only 0.68 GFLOPs and ~1.07M parameters - 2.7x computational cost reduction and >10x model size reduction compared to ResNet18. Outperforms DenseNet121 (6x fewer parameters) and ViT (80x fewer parameters) while maintaining comparable accuracy.
...
TL;DR: Adaptive RGB-LWIR fusion framework for robotic vision improves object detection across varying light conditions by dynamically selecting optimal fusion ratios and models.


<details>
  <summary>Details</summary>
Motivation: Traditional RGB detection struggles in low-light, while thermal (LWIR) lacks color/texture information. Need robust vision for emergency services robots operating in diverse illumination conditions.

Method: Trained 33 YOLO models on 22k+ annotated images across three light levels. Fused aligned RGB and LWIR frames at 11 ratios (0-100% in 10% increments). Dynamic selection of optimal fusion model based on illumination.

Result: Best full-light model (80/20 RGB-LWIR): 92.8% mean confidence; dim-light model (90/10): 92.0%; both significantly outperformed YOLOv5n/YOLOv11n baselines. No-light model (40/60): 71.0%, exceeding baselines though not statistically significant.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image diffusion models generate detailed textures but often fail to follow strict geometric constraints, especially when those constraints conflict with text prompt styles. There's a semantic gap between human perception and current generative models regarding geometric understanding.

Method: Proposes Human Perception Embedding (HPE) teacher trained on THINGS triplet dataset to capture human sensitivity to object shape. Uses lightweight, off-the-shelf discriminators as external guidance signals, injecting their gradients into the latent diffusion process to separate geometry and style controllably.

Result: Evaluated across Stable Diffusion v1.5, SiT-XL/2 flow-matching model, and PixArt-Σ diffusion transformer. Shows flow models drift back without continuous guidance, demonstrates zero-shot transfer of complex 3D shapes onto conflicting materials. Guided generation improves semantic alignment by ~80% compared to unguided baselines.

Conclusion: Small teacher models can reliably guide large generative systems, enabling stronger geometric control and broadening the creative range of text-to-image synthesis. Lightweight discriminators can introduce geometric understanding without specialized training.
...
TL;DR: GeCo is a geometry-grounded metric for detecting geometric deformation and occlusion-inconsistency artifacts in video generation, used for benchmarking models and as training-free guidance.
...
Method: Fuses residual motion and depth priors to produce interpretable, dense consistency maps that reveal geometric artifacts.

Result: Systematically benchmarks recent video generation models, uncovering common failure modes, and reduces deformation artifacts when used as training-free guidance.

Conclusion: GeCo provides an effective geometry-grounded metric for detecting and mitigating geometric artifacts in video generation, serving both as an evaluation tool and guidance mechanism.
...
TL;DR: The B&J Benchmark reveals AI models excel at structured medical questions but struggle with multimodal clinical reasoning, showing a 90% vs 60% performance gap between multiple-choice and open-ended tasks.


<details>
  <summary>Details</summary>
Motivation: Current medical AI benchmarks based on licensing exams fail to capture real-world multimodal clinical reasoning needed for patient care, necessitating more comprehensive evaluation frameworks.

Method: Developed the Bones and Joints Benchmark with 1,245 questions from real orthopedic cases, assessing 7 clinical reasoning tasks across 11 VLMs and 6 LLMs compared to expert ground truth.

Result: Models achieved >90% accuracy on multiple-choice questions but dropped to ~60% on open-ended multimodal tasks; VLMs showed poor medical image interpretation with text-driven hallucinations; medical fine-tuning offered no consistent advantage.
...
TL;DR: Fetal-Gauge is the first large-scale visual question answering benchmark for evaluating Vision-Language Models on fetal ultrasound tasks, revealing current VLMs perform poorly (55% accuracy) and highlighting need for domain-specific improvements.


<details>
  <summary>Details</summary>
Motivation: Global shortage of trained sonographers creates barriers to prenatal care. Deep learning could enhance efficiency and training, but no standardized benchmark exists to evaluate VLMs for fetal ultrasound due to modality challenges and limited public datasets.

Method: Created Fetal-Gauge benchmark with over 42,000 images and 93,000 question-answer pairs covering anatomical plane identification, visual grounding, fetal orientation assessment, clinical view conformity, and clinical diagnosis. Systematically evaluated state-of-the-art VLMs including general-purpose and medical-specific models.

Result: Best-performing VLM achieved only 55% accuracy, far below clinical requirements. Analysis revealed substantial performance gap and critical limitations of current VLMs in fetal ultrasound interpretation.

Conclusion: Fetal-Gauge establishes rigorous foundation for advancing multimodal deep learning in prenatal care, highlights urgent need for domain-adapted architectures and specialized training approaches, and provides pathway to address global healthcare accessibility challenges.
...
TL;DR: Uni4D is a unified framework for large-scale open-vocabulary 3D retrieval and controlled 4D generation using three-level alignment across text, 3D models, and images.


<details>
  <summary>Details</summary>
Motivation: To advance dynamic multimodal understanding by creating a unified system that can both retrieve 3D models from text queries and generate temporally consistent 4D assets through improved cross-modal alignment.

Method: Built on Align3D 130 dataset, uses 3D text multi-head attention and search model for text-to-3D retrieval optimization. Features three alignment components: precise text-to-3D retrieval, multi-view 3D-to-image alignment, and image-to-text alignment for 4D generation.

Result: Experimental results show Uni4D achieves high-quality 3D retrieval and controllable 4D generation, advancing dynamic multimodal understanding and practical applications.

Conclusion: Uni4D successfully demonstrates a unified framework that bridges text, 3D, and image modalities for both retrieval and generation tasks, representing significant progress in multimodal 3D/4D understanding.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches struggle with motion modeling accuracy and spatiotemporal consistency in dynamic scenes with fast movement and multiple targets.

Method: Combines periodic activation properties of sinusoidal representation networks with geometric priors from keypoint structures, introduces physics-inspired constraint mechanisms for coherent keypoint predictions across spatial and temporal dimensions, and expands training dataset to 600,000 annotated instances.
...
Conclusion: SirenPose effectively addresses limitations in dynamic 3D scene reconstruction by enforcing geometric and temporal coherence through novel loss formulation and expanded training data.
...
TL;DR: A real-time driver behavior recognition system optimized for low-cost edge devices (Raspberry Pi 5 & Coral Edge TPU) that detects 17 distraction/drowsiness behaviors using a compact vision model with temporal decision logic.


<details>
  <summary>Details</summary>
Motivation: Driver Monitoring Systems need to operate with low latency under strict compute, power, and cost constraints for in-cabin deployment, requiring efficient solutions for real-time behavior recognition.

Method: Combines: (1) compact per-frame vision model, (2) confounder-aware label design to reduce false positives, (3) temporal decision head that triggers alerts only for confident and sustained predictions.

Result: Achieves 16 FPS on Raspberry Pi 5 (INT8 inference, <60ms latency) and 25 FPS on Coral Edge TPU, enabling real-time monitoring on inexpensive hardware. Covers 17 behavior classes validated on diverse datasets and real vehicle tests.

Conclusion: The system demonstrates reliable in-cabin human-state perception suitable for low-cost edge deployment, serving as upstream input for human-centered vehicle intelligence and emerging agentic vehicle concepts.
...
TL;DR: A robust deepfake detector with calibrated probabilities and tamper heatmaps using red-team training and test-time defenses for attack-aware detection under realistic conditions.


<details>
  <summary>Details</summary>
Motivation: To create a practical deepfake and image-forensics detector that remains robust under realistic deployment conditions, provides well-calibrated probabilities for reliable decision-making, and offers transparent evidence through tamper heatmaps while being attack-aware.

Method: Two-stream architecture: one stream encodes semantic content using a pretrained backbone, the other extracts forensic residuals, fused via lightweight residual adapter. Uses red-team training with worst-of-K counter-forensics (JPEG manipulations, resampling warps, denoise-to-regrain, etc.) and test-time defense with low-cost jitters. Heatmaps generated via shallow Feature Pyramid Network style head with weak supervision using face-box masks.

Result: Near-perfect ranking across attacks, low calibration error, minimal abstention risk, controlled degradation under regrain attacks, and effective weak-localization scores on standard deepfake datasets and surveillance-style splits with challenging conditions.

Conclusion: The approach establishes a modular, data-efficient, and practically deployable baseline for attack-aware detection with calibrated probabilities and actionable heatmaps, demonstrating robustness under realistic adversarial conditions.
...
TL;DR: PortionNet: Cross-modal knowledge distillation framework for food nutrition estimation from RGB images by learning geometric features from point clouds during training, achieving SOTA performance without depth sensors.


<details>
  <summary>Details</summary>
Motivation: Food nutrition estimation from single images is challenging due to loss of 3D information. Depth-based methods are reliable but inaccessible on most smartphones due to depth-sensor requirements.

Method: Proposes PortionNet, a cross-modal knowledge distillation framework that learns geometric features from point clouds during training but only requires RGB images at inference. Uses dual-mode training with lightweight adapter network to mimic point cloud representations for pseudo-3D reasoning.
...
Conclusion: PortionNet enables accurate food nutrition estimation from RGB images without specialized hardware by leveraging cross-modal knowledge distillation from point clouds, making it practical for smartphone applications.
...
TL;DR: MoFu is a unified framework for multi-subject video generation that solves scale inconsistency and permutation sensitivity problems using LLM-guided scale modulation and Fourier fusion.


<details>
  <summary>Details</summary>
Motivation: Current multi-subject video generation methods suffer from scale inconsistency (unnatural subject sizes) and permutation sensitivity (subject distortion based on reference input order), which degrade generation quality.

Method: Proposes MoFu with three key components: 1) Scale-Aware Modulation (SMO) using LLM guidance to extract scale cues from prompts, 2) Fourier Fusion strategy using Fast Fourier Transform for permutation-invariant feature representation, and 3) Scale-Permutation Stability Loss for joint optimization.
...
Conclusion: MoFu effectively addresses both scale inconsistency and permutation sensitivity in multi-subject video generation through unified scale-aware modulation and Fourier-based feature fusion, achieving superior performance over existing approaches.
...
TL;DR: VideoZoomer: An agentic framework enabling MLLMs to dynamically control visual focus during reasoning for long video understanding, using temporal zooming to progressively gather fine-grained evidence.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs are limited in long video understanding due to context window constraints, relying on uniform frame sampling or static pre-selection that may miss critical evidence and cannot correct initial selection errors during reasoning.

Method: Proposes VideoZoomer framework where MLLMs start from coarse low-frame-rate overview, then invoke temporal zoom tool to obtain high-frame-rate clips at autonomously chosen moments, gathering evidence in multi-turn interactive manner. Uses two-stage training: cold-start supervised fine-tuning on distilled exemplar and reflection trajectories, followed by reinforcement learning.

Result: 7B model delivers diverse and complex reasoning patterns, achieving strong performance across broad set of long video understanding benchmarks. Consistently surpasses existing open-source models and rivals proprietary systems on challenging tasks, with superior efficiency under reduced frame budgets.

Conclusion: VideoZoomer enables MLLMs to dynamically control visual focus during reasoning, overcoming limitations of static approaches for long video understanding through agentic temporal zooming capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion transformer models uniformly process and denoise all image tokens at every timestep during editing, causing redundant computation for unchanged regions and potentially degrading unmodified areas. Most edits only modify small regions, making full regeneration unnecessary.

Method: SpotEdit uses two components: SpotSelector identifies stable regions via perceptual similarity and skips computation by reusing conditional image features; SpotFusion adaptively blends these features with edited tokens through dynamic fusion to preserve contextual coherence.
...
Conclusion: Selective region updating is more efficient than uniform processing for diffusion-based image editing, and SpotEdit demonstrates this through its training-free approach that preserves editing quality while reducing computational overhead.
...
TL;DR: DeMoGen is a compositional training paradigm using energy-based diffusion models to decompose holistic human motions into semantically meaningful sub-components without ground-truth labels, enabling discovery and recombination of motion primitives.


<details>
  <summary>Details</summary>
Motivation: Existing motion modeling approaches focus on forward modeling (text-to-motion or composition), but lack the inverse capability to decompose complex motions into meaningful sub-components for better understanding and reuse of motion primitives.

Method: Proposes DeMoGen, an energy-based diffusion model framework with three training variants: DeMoGen-Exp (explicit decomposed text training), DeMoGen-OSS (orthogonal self-supervised decomposition), and DeMoGen-SC (semantic consistency enforcement). Also constructs a text-decomposed dataset for compositional training.

Result: The approach successfully disentangles reusable motion primitives from complex sequences, enables flexible recombination to generate diverse novel motions beyond training distribution, and provides a dataset resource for text-to-motion generation and composition.

Conclusion: DeMoGen addresses the inverse problem of motion decomposition, offering a novel compositional training paradigm that discovers motion concepts without ground-truth labels and enables creative recombination for motion generation and analysis.
...
<details>
  <summary>Details</summary>
Motivation: Current radiomics-based methods for non-invasive MGMT promoter methylation inference in glioblastoma are limited by feature redundancy and incomplete modeling of modality-specific information from different MRI sequences.

Method: Multi-view latent representation learning using variational autoencoders (VAE) with independent probabilistic encoders for each modality (T1Gd and FLAIR MRI), performing fusion in compact latent space to preserve modality-specific structure.

Result: The approach enables effective multimodal integration of complementary radiomic features for MGMT promoter methylation classification, though specific performance metrics are not provided in the abstract.

Conclusion: The proposed multi-view VAE framework addresses limitations of conventional radiomics methods by better preserving modality-specific information while enabling effective multimodal integration for MGMT methylation prediction in glioblastoma.
...
TL;DR: End-to-end vision transformer pipeline for pixel-level HER2 scoring from H&E and IHC whole slide images with 94% accuracy.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for HER2 scoring lack pixel-level localization and struggle with joint analysis of H&E and IHC stained images, which is crucial for accurate HER2 protein expression assessment in cancer treatment.

Method: Proposes an end-to-end vision transformer pipeline that: 1) processes H&E WSIs patch-wise for tumor localization, 2) uses a novel mapping function to correlate IHC regions with malignant H&E regions, 3) embeds clinically inspired HER2 scoring mechanism for automatic pixel-level annotation (0, 1+, 2+, 3+), and 4) distinguishes HER2-negative/positive cases.

Result: Achieved 0.94 classification accuracy and 0.933 specificity for 4-way HER2 scoring, with good tumor localization accuracy. The pipeline performed comparably to human pathologists on WSI patches.

Conclusion: The study demonstrates the effectiveness of jointly evaluating H&E and IHC images using end-to-end vision transformer models for accurate pixel-level HER2 scoring, showing practical applicability comparable to human pathologists.
...
Method: Use perception-informed pseudo-coloring to encode clinically salient temporal features (like QT-interval) into structured color representations. Apply prototypical networks and ResNet-18 architecture for one-shot/few-shot learning on ECG images from single cycles and full rhythms.
...
TL;DR: MLLMs extended for 3D scene manipulation via MCP-based API, visual tools, and multi-agent framework, outperforming baselines on complex object arrangement tasks.


<details>
  <summary>Details</summary>
Motivation: Multimodal Large Language Models (MLLMs) have shown strong performance in 2D vision-language tasks but remain underexplored for complex 3D scene manipulation, creating a critical gap that needs to be addressed.

Method: Three key innovations: 1) MCP-based API to shift from brittle raw code to robust function-level updates, 2) specialized visual tools for scene analysis, spatial information gathering, and action validation, 3) collaborative multi-agent framework with planning, execution, and verification roles for iterative error handling.

Result: The approach demonstrates effectiveness on 25 complex object arrangement tasks, significantly outperforming existing baselines.

Conclusion: The proposed framework successfully bridges the gap between MLLMs and 3D scene manipulation by addressing visual grounding, scene understanding, and iterative error management challenges.
...
TL;DR: Self-E is a novel text-to-image model trained from scratch that supports any-step inference through self-evaluation, combining local learning with global matching to achieve high-quality generation at both few-step and many-step settings.


<details>
  <summary>Details</summary>
Motivation: Traditional diffusion/flow models require many inference steps for quality generation, while distillation approaches need pretrained teachers. There's a need for a unified model that can generate high-quality images from scratch at any step count without external supervision.
...
Result: Self-E excels at few-step generation while being competitive with state-of-the-art Flow Matching models at 50 steps. Performance improves monotonically with inference steps, enabling both ultra-fast few-step and high-quality long-trajectory sampling in a single model.
...
TL;DR: iOSPointMapper is a mobile app that enables real-time sidewalk mapping using iPhones/iPads with on-device semantic segmentation, LiDAR depth estimation, and GPS/IMU fusion to detect sidewalk features, with user validation and data anonymization for TDEI integration.


<details>
  <summary>Details</summary>
Motivation: Current sidewalk data collection methods are costly, fragmented, and difficult to scale, creating barriers to building accessible and inclusive pedestrian infrastructure. There's a critical need for scalable, user-centered approaches to close data gaps in pedestrian mapping.

Method: iOSPointMapper uses recent-generation iPhones and iPads with on-device semantic segmentation, LiDAR-based depth estimation, and fused GPS/IMU data to detect and localize sidewalk-relevant features (traffic signs, traffic lights, poles). It includes a user-guided annotation interface for validation and anonymizes data for transmission to the Transportation Data Exchange Initiative (TDEI).

Result: Detailed evaluations show promising performance in feature detection and spatial mapping, demonstrating the application's potential for enhanced pedestrian mapping. The system offers a scalable solution that integrates seamlessly with broader multimodal transportation datasets.

Conclusion: iOSPointMapper provides a scalable, privacy-conscious, and user-centered approach to sidewalk mapping that addresses critical data gaps in pedestrian infrastructure, enabling more accessible and inclusive urban planning through real-time, on-the-ground data collection.
...
TL;DR: DeFloMat is a fast generative object detection framework using Conditional Flow Matching instead of diffusion models, achieving state-of-the-art accuracy in just 3 steps for clinical Crohn's Disease detection in MRE.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based detectors like DiffusionDet have high accuracy but require many sampling steps (T ≫ 60), making them too slow for time-sensitive clinical applications such as Crohn's Disease detection in Magnetic Resonance Enterography (MRE).

Method: Replaces slow stochastic diffusion paths with a deterministic flow field derived from Conditional Optimal Transport theory, specifically approximating Rectified Flow. Uses Conditional Flow Matching (CFM) and enables fast inference via a simple ODE solver.

Result: Achieves 43.32% AP₁₀:₅₀ in only 3 inference steps, representing 1.4× performance improvement over DiffusionDet's maximum converged performance (31.03% AP₁₀:₅₀ at 4 steps). Shows superior recall and stability in few-step regime with enhanced localization characteristics.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D brightfield microscopy offers fast, noninvasive cellular visualization but lacks robust volumetric segmentation without fluorescence or extensive post-processing.

Method: Combines Native Sparse Attention (local/global context), depth-width residual HyperConnections, soft Mixture-of-Experts, and anisotropic patch embedding respecting confocal geometry for 3D tokenization.

Result: Produces morphology-accurate segmentations of nuclei, mitochondria, and organelles from brightfield alone, outperforming CNN and Transformer baselines across multiple confocal datasets.

Conclusion: Bright-4B enables label-free 3D cell mapping with fine structural detail preservation; code, weights, and models will be released for downstream use.
...
TL;DR: FluenceFormer is a transformer-based framework for radiotherapy fluence map prediction that uses a two-stage approach with physics-informed loss to address limitations of convolutional methods in capturing long-range dependencies.


<details>
  <summary>Details</summary>
Motivation: Current convolutional methods for fluence map prediction struggle with long-range dependencies, leading to structurally inconsistent or physically unrealizable radiotherapy plans. There's a need for better geometry-aware approaches that can handle the complex relationship between anatomy and beam modulation.
...
Result: FluenceFormer with Swin UNETR achieves best performance among evaluated transformer backbones, reducing Energy Error to 4.5% and showing statistically significant gains in structural fidelity (p < 0.05) compared to CNN and single-stage methods.

Conclusion: The transformer-based FluenceFormer framework effectively addresses long-range dependency issues in fluence map prediction, demonstrating superior performance through its two-stage design and physics-informed loss function for automated radiotherapy planning.
...
TL;DR: EmoCtrl enables controllable emotional image generation that maintains content fidelity while expressing target emotions, outperforming existing methods through textual and visual emotion enhancement modules.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models lack emotional awareness, while emotion-driven models sacrifice content fidelity. There's a need for models that can generate images faithful to content descriptions while expressing specific emotions.

Method: Proposes EmoCtrl with textual and visual emotion enhancement modules, supported by a dataset annotated with content, emotion, and affective prompts. Uses learned emotion tokens that bridge abstract emotions to visual cues.

Result: Outperforms existing methods in faithful content and expressive emotion control. User studies confirm strong alignment with human preference. Generalizes well to creative applications, demonstrating robustness of learned emotion tokens.

Conclusion: EmoCtrl successfully addresses the gap between content-faithful and emotion-expressive image generation, achieving controllable emotional image content generation with complementary textual and visual emotion enhancement.
...
<details>
  <summary>Details</summary>
Motivation: LiDAR perception in autonomous systems faces limitations from fixed vertical beam resolution and beam dropout due to environmental occlusions, which degrade point cloud quality and perception accuracy.

Method: Models LiDAR scans as beam-aware graphs, uses graph attention with gated residual fusion and feed-forward refinement to reconstruct missing elevation information without increasing network depth. Evaluated by simulating structured beam dropout (removing every 4th vertical beam).

Result: SuperiorGAT consistently achieves lower reconstruction error and improved geometric consistency across diverse KITTI environments (Person, Road, Campus, City sequences) compared to PointNet-based models and deeper GAT baselines. X-Z projections show preserved structural integrity with minimal vertical distortion.

Conclusion: Architectural refinement offers a computationally efficient method for improving LiDAR resolution without requiring additional sensor hardware, addressing beam dropout limitations through intelligent reconstruction rather than hardware upgrades.
...
<details>
  <summary>Details</summary>
Motivation: Current event camera calibration methods are time-consuming and require manually placed calibration objects, which cannot meet the needs of rapidly changing scenarios. There's a need for more efficient, automatic calibration approaches.
...
Conclusion: The proposed line-based event camera calibration method provides an efficient alternative to traditional pattern-based approaches, enabling automatic calibration using naturally occurring geometric features in man-made environments.
...
Conclusion: QDFNet effectively addresses the challenges of optical-SAR fusion by providing robust object detection through quality-aware dynamic fusion that handles missing or degraded modalities, outperforming existing approaches in practical scenarios with modality corruption.
...
TL;DR: SonoVision is a smartphone app that helps visually impaired people locate everyday objects using directional sound cues through headphones, with offline object detection capability.


<details>
  <summary>Details</summary>
Motivation: Visually impaired individuals face significant challenges in locating objects, which hinders their independence and can lead to dangerous situations. The goal is to make them more self-sufficient by reducing reliance on others.

Method: Developed a Flutter-based smartphone app using Efficientdet-D2 model for object detection. The system provides directional sound cues through headphones: sinusoidal sounds in left/right ear for objects on respective sides, and simultaneous sounds in both ears for objects directly in front.

Result: Created a functional application that can work completely offline, providing visually impaired users with an accessible way to locate objects using sound cues. The app is available as an open-source project on GitHub.

Conclusion: SonoVision offers a safe and user-friendly solution to help visually impaired individuals become more independent by enabling them to locate everyday objects through smartphone-based directional audio cues, with the advantage of offline functionality.
...
TL;DR: SAM 3D foundation model outperforms specialized TRELLIS for monocular building reconstruction from remote sensing, producing better roof geometry and boundaries, with potential for urban scene modeling.


<details>
  <summary>Details</summary>
Motivation: Existing methods for monocular 3D building reconstruction require task-specific architectures and intensive supervision, limiting scalability for urban modeling. The paper aims to evaluate whether general-purpose foundation models like SAM 3D can provide better performance.

Method: Systematic evaluation of SAM 3D against TRELLIS on NYC Urban Dataset using Frechet Inception Distance (FID) and CLIP-based Maximum Mean Discrepancy (CMMD) metrics. Extended SAM 3D to urban scenes via segment-reconstruct-compose pipeline.

Result: SAM 3D produces more coherent roof geometry and sharper boundaries than TRELLIS. The model shows potential for urban scene reconstruction through the proposed pipeline.

Conclusion: Foundation models like SAM 3D offer practical advantages for urban 3D reconstruction, outperforming specialized methods. Future work should integrate scene-level structural priors to further improve performance.
...
TL;DR: This paper compares three computer vision models (YOLOv8, YOLOv11, RF-DETR) for autonomous mapping of electrical substation components to improve grid vulnerability assessment.
...
Method: The researchers trained and compared three object detection models (YOLOv8, YOLOv11, RF-DETR) on a manually labeled dataset of US substation images. They evaluated each model's detection accuracy, precision, and efficiency.

Result: The paper presents comparative performance analysis of the three models, identifying their key strengths and limitations for substation component mapping. The models were successfully used to map substation components across the United States.

Conclusion: The research demonstrates the practical application of machine learning for large-scale substation component mapping, providing an efficient alternative to manual methods for grid vulnerability assessment.
...
TL;DR: PGR²M introduces a hybrid motion representation combining interpretable pose codes with residual codes to capture both global structure and fine-grained temporal details, improving text-based 3D motion generation and editing quality.
...
Method: Proposes PGR²M (pose-guided residual refinement for motion) using a hybrid representation: pose codes for coarse global structure plus residual codes learned via residual vector quantization (RVQ) for fine-grained temporal variations. Includes pose-guided RVQ tokenizer, residual dropout to prevent over-reliance on residuals, and a two-stage Transformer architecture (base Transformer for pose codes, refine Transformer for residual codes).

Result: Experiments on HumanML3D and KIT-ML show improved Fréchet inception distance and reconstruction metrics for both generation and editing compared to CoMo and recent diffusion/tokenization baselines. User studies confirm intuitive, structure-preserving motion edits.

Conclusion: PGR²M's hybrid representation effectively addresses limitations of pure pose-code frameworks by capturing both interpretable global structure and fine-grained temporal details, enabling higher-quality text-based motion generation and editing with better preservation of motion structure.
...
TL;DR: A novel framework using multiple event cameras to measure shock wave motion parameters with high spatiotemporal resolution, achieving high-precision measurement through event-based processing and 3D reconstruction.
...
Method: Uses multiple event cameras to estimate shock wave asymmetry; establishes polar coordinate system for event encoding, adaptive ROI extraction, iterative slope analysis for wave front extraction, and derives geometric models for 3D reconstruction and motion parameter estimation.

Result: Achieves high-precision measurement with maximum error of 5.20% and minimum error of 0.06% compared to pressure sensors and empirical formulas, enabling multi-angle measurement, motion field reconstruction, and explosive equivalence inversion.

Conclusion: The method successfully achieves high-precision measurement of shock wave motion fields with both high spatial and temporal resolution, representing significant progress in shock wave measurement technology.
...
TL;DR: A semi-supervised paradigm for infrared small object detection using hierarchical MoE adapters to distill SAM into an expert teacher (Scalpel-SAM) with minimal annotations, enabling downstream models to match fully supervised performance.


<details>
  <summary>Details</summary>
Motivation: Infrared small object detection suffers from high annotation costs, and existing methods like SAM face domain gaps, inability to encode physical priors, and architectural complexity, creating a need for efficient semi-supervised approaches.
...
Result: With minimal annotations, downstream models achieve performance comparable to or surpassing their fully supervised counterparts, demonstrating effective knowledge transfer.

Conclusion: This is the first semi-supervised paradigm that systematically addresses data scarcity in IR-SOT using SAM as teacher model, providing an efficient solution for infrared small object detection with limited annotations.
...
<details>
  <summary>Details</summary>
Motivation: The motivation is to develop a self-supervised video representation learning approach that leverages the inherent 3D structure of dynamic scenes. The key insight is that 2D videos are often consistent projections of 3D scenes, and representing videos as moving Gaussian splats provides a natural inductive bias for learning meaningful video representations.

Method: Video-GMAE uses a masked autoencoder architecture that encodes video frames into a set of Gaussian splats that move over time. The method represents videos as dynamic 3D scenes using Gaussian representations, enforcing the inductive bias that 2D videos are projections of 3D scenes. The architecture learns to track objects through the movement of these Gaussians.

Result: The method demonstrates emergent tracking capabilities - mapping Gaussian trajectories onto the image plane achieves zero-shot tracking performance comparable to state-of-the-art. With finetuning, it achieves 34.6% improvement on Kinetics and 13.1% improvement on Kubric datasets, surpassing existing self-supervised video approaches.

Conclusion: Video-GMAE successfully leverages 3D scene representation as an inductive bias for self-supervised video learning, enabling emergent tracking and achieving superior performance on video understanding tasks compared to existing self-supervised methods.
...
TL;DR: SCAFusion is a multimodal 3D object detection model for lunar robotics that improves small, irregular object detection through cognitive adaptation, contrastive alignment, and section-aware coordinate attention.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal 3D perception methods for autonomous driving perform poorly in lunar environments due to feature misalignment, limited multimodal synergy, and weak small object detection capabilities needed for meteor fragments and rocks.
...
Result: Achieves 69.7% mAP and 72.1% NDS on nuScenes validation set (5.0% and 2.7% improvement over baseline). In simulated lunar environments on Isaac Sim, achieves 90.93% mAP (11.5% improvement), with significant gains in detecting small meteor-like obstacles.

Conclusion: SCAFusion effectively addresses lunar environment challenges with minimal parameter/computation overhead, demonstrating strong performance for small, irregular object detection critical for autonomous lunar surface exploration.
...
TL;DR: DreamOmni3 is a unified framework for scribble-based image editing and generation that combines text, images, and freehand sketches, addressing limitations of text-only interfaces for precise visual control.


<details>
  <summary>Details</summary>
Motivation: Text prompts alone often fail to capture users' intended edit locations and fine-grained visual details in image editing/generation tasks. Language limitations in specifying precise spatial information and visual details motivate the need for more flexible interfaces combining text, images, and freehand sketches.

Method: Two main components: 1) Data creation pipeline for scribble-based editing (4 tasks) and generation (3 tasks) using DreamOmni2 dataset with editable regions overlaid with hand-drawn elements; 2) Framework design using joint input scheme feeding both original and scribbled source images with color differentiation and shared index/position encodings for precise localization.

Result: DreamOmni3 achieves outstanding performance on comprehensive benchmarks established for scribble-based editing and generation tasks, demonstrating effective handling of complex edits involving multiple scribbles, images, and instructions.

Conclusion: The proposed scribble-based approach enables more flexible creation on GUI interfaces, overcoming text-only limitations. The framework, models, and code will be publicly released to promote further research in this direction.
...
TL;DR: CoAgent is a collaborative closed-loop framework for coherent long-form video generation that addresses identity drift and scene inconsistency through a plan-synthesize-verify pipeline with entity-level memory management.
...
Result: Extensive experiments show CoAgent significantly improves coherence, visual consistency, and narrative quality in long-form video generation compared to existing methods.

Conclusion: CoAgent's collaborative closed-loop framework effectively addresses narrative coherence and visual consistency challenges in open-domain video generation through structured planning and entity-level memory management.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal LLMs produce fluent but unreliable reasoning with weak step-to-step coherence and insufficient visual grounding because existing alignment approaches only supervise final answers while ignoring the reliability of intermediate reasoning processes.
...
Conclusion: SR-MCR effectively addresses multimodal LLM reasoning reliability issues by leveraging intrinsic process signals from model outputs, providing a lightweight, label-free solution that enhances both answer accuracy and reasoning coherence without external supervision.
...
<details>
  <summary>Details</summary>
Motivation: Gliomas are aggressive cancers with high mortality. Existing methods have issues with imaging variability, computational inefficiency, and poor segmentation/classification performance.

Method: Developed radiomics-enhanced fused residual multiparametric 3D network (ReFRM3D) based on 3D U-Net with multi-scale feature fusion, hybrid upsampling, and extended residual skip mechanism. Also created multi-feature tumor marker classifier using radiomic features from segmented regions.
...
Conclusion: The proposed ReFRM3D network with radiomics enhancement significantly improves glioma segmentation and classification performance, addressing key challenges in brain tumor diagnosis.
...
TL;DR: KV-Tracker enables real-time 6-DoF pose tracking and online reconstruction from monocular RGB videos by caching attention KV pairs from multi-view 3D geometry networks, achieving 15× speedup while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Multi-view 3D geometry networks provide strong priors but are too slow for real-time applications. There's a need to adapt these powerful models for online use in pose tracking and reconstruction tasks.
...
Result: Achieves up to 15× speedup during inference without drift or catastrophic forgetting. Maintains high frame rates (~27 FPS) while showing strong performance on TUM RGB-D, 7-Scenes, Arctic, and OnePose datasets for both scene-level and object tracking/reconstruction.

Conclusion: KV-Tracker successfully adapts powerful but slow multi-view 3D geometry networks for real-time applications through KV pair caching, enabling efficient online pose tracking and reconstruction without depth measurements or object priors.
...
TL;DR: PTalker: A novel framework for personalized 3D talking head generation that preserves individual speaking styles through style-content disentanglement and improves lip-sync accuracy via three-level audio-mesh alignment.


<details>
  <summary>Details</summary>
Motivation: Existing speech-driven 3D talking head methods focus on lip-synchronization but overlook individual speaking styles, limiting personalization and realism. There's a need to capture the nuanced speaking styles of different individuals while maintaining accurate lip-sync.

Method: PTalker uses style-content disentanglement with constraints to encode audio and motion into separate style/content spaces. It employs a three-level alignment mechanism: 1) spatial alignment using Graph Attention Networks for mesh vertex connectivity, 2) temporal alignment using cross-attention for temporal dependencies, and 3) feature alignment with top-k bidirectional contrastive losses and KL divergence constraints.
...
Conclusion: PTalker successfully addresses the limitation of existing methods by preserving individual speaking styles while improving lip-synchronization accuracy, advancing personalized 3D talking head animation through effective style-content disentanglement and multi-level modality alignment.
...
TL;DR: Proposed Sparse Differential Transformer (SDT) with prediction-driven Top-K Jaccard similarity for face clustering, achieving SOTA performance by improving neighbor purity and reducing noise in similarity measurements.


<details>
  <summary>Details</summary>
Motivation: Existing face clustering methods using Jaccard similarity coefficients introduce too many irrelevant nodes, producing limited discriminative power and adversely affecting clustering performance. Current approaches also struggle with accurately predicting optimal neighbor counts (Top-K) and vanilla Transformers introduce noise by overemphasizing irrelevant feature relationships.
...
Result: Extensive experiments on multiple datasets (including MS-Celeb-1M) demonstrate state-of-the-art performance, outperforming existing methods and providing a more robust solution for face clustering.

Conclusion: The proposed SDT with prediction-driven Top-K Jaccard similarity effectively addresses limitations of existing methods by improving similarity measurement reliability through enhanced neighbor purity and noise reduction, achieving superior face clustering performance.
...
TL;DR: Dream-VL is a diffusion-based vision-language model that outperforms autoregressive VLMs on visual planning tasks, and Dream-VLA extends this to vision-language-action tasks with superior robotic control performance.


<details>
  <summary>Details</summary>
Motivation: Autoregressive VLMs have limitations in complex visual planning and robotic control due to sequential generation. The authors investigate diffusion-based LLMs as a foundation to overcome these limitations with bidirectional, parallel generation capabilities.

Method: Develop Dream-VL as a diffusion-based VLM using diffusion-based LLMs as backbone. Then extend to Dream-VLA through continuous pre-training on open robotic datasets, leveraging the diffusion backbone's bidirectional nature for action chunking and parallel generation.

Result: Dream-VL achieves SOTA among dVLMs and comparable to top AR-based VLMs on benchmarks, with superior visual planning. Dream-VLA achieves 97.2% success on LIBERO, 71.4% on SimplerEnv-Bridge, and 60.5% on SimplerEnv-Fractal, surpassing leading models like π₀ and GR00T-N1.

Conclusion: Diffusion-based backbones provide superior foundations for VLMs and VLA tasks due to bidirectional nature, enabling better action chunking, parallel generation, and faster convergence. The approach shows clear advantages over autoregressive baselines for visual planning and robotic control.
...
TL;DR: Systematic study of memory mechanisms in SAM-based visual object tracking, analyzing SAM2 trackers, extending to SAM3, and proposing unified hybrid memory framework for improved robustness.
...
Conclusion: Memory design principles are crucial for SAM-based tracking, and the proposed unified hybrid memory framework provides a modular, principled approach that enhances tracking robustness across challenging scenarios and different backbone models.
...
TL;DR: Envision is a diffusion-based visual planning framework that generates goal-consistent video trajectories for embodied agents by first synthesizing goal images and then interpolating between start and goal states.


<details>
  <summary>Details</summary>
Motivation: Existing video diffusion models for embodied visual planning are forward predictive without explicit goal modeling, leading to spatial drift and goal misalignment. There's a need for goal-constrained generation to ensure physical plausibility and goal consistency.
...
Conclusion: Envision effectively addresses goal misalignment in visual planning by explicitly constraining generation with goal images, producing reliable trajectories for embodied agents that connect start and goal states with physical plausibility.
...
TL;DR: Proposes FinPercep-RM, a fine-grained perceptual reward model for RLHF in image super-resolution, with co-evolutionary curriculum learning to prevent reward hacking and stabilize training.
...
Method: 1) FinPercep-RM: Encoder-Decoder architecture that outputs both global quality score and Perceptual Degradation Map for spatial localization of defects. 2) FGR-30k dataset for training with diverse real-world distortions. 3) Co-evolutionary Curriculum Learning (CCL): Synchronized curricula where reward model complexity increases while ISR model starts with simple global reward, gradually transitioning to complex outputs.

Result: Experiments validate effectiveness across ISR models in both global quality and local realism on RLHF methods, showing improved perceptual quality while suppressing reward hacking.

Conclusion: The proposed FinPercep-RM with CCL mechanism addresses limitations of traditional IQA models in RLHF for ISR, enabling stable training and better alignment with human perceptual preferences through fine-grained reward modeling.
...
TL;DR: Monocular depth estimation using visual autoregressive (VAR) priors as alternative to diffusion models, achieving competitive results with minimal fine-tuning data.
...
Method: Adapts a large-scale text-to-image VAR model with scale-wise conditional upsampling and classifier-free guidance. Performs inference in ten fixed autoregressive stages and requires only 74K synthetic samples for fine-tuning.

Result: Achieves state-of-the-art performance in indoor benchmarks under constrained training conditions, and strong performance on outdoor datasets. Establishes autoregressive priors as complementary geometry-aware generative models for depth estimation.
...
TL;DR: Deep learning models for automated LVEF estimation from echocardiogram videos achieve RMSE of 6.79% with modified 3D Inception architectures performing best.


<details>
  <summary>Details</summary>
Motivation: Manual LVEF assessment from echocardiograms is time-consuming and has high inter-observer variability, creating need for automated deep learning solutions.

Method: Evaluated multiple deep learning architectures (3D Inception, two-stream, CNN-RNN) on EchoNet-Dynamic dataset (10,030 videos), testing architectural modifications and fusion strategies.

Result: Modified 3D Inception achieved best performance (RMSE 6.79%). Smaller/simpler models showed better generalization due to overfitting tendency. Performance highly sensitive to hyperparameters like kernel sizes and normalization.

Conclusion: Deep learning can effectively automate LVEF estimation from echocardiography, with architectural insights applicable to broader video analysis tasks beyond medical imaging.
...
TL;DR: CLAdapter is a novel Cluster Attention Adapter that adapts rich pre-trained vision model representations to data-limited downstream tasks through attention mechanisms and cluster centers, achieving SOTA performance across diverse scientific domains.


<details>
  <summary>Details</summary>
Motivation: While large-scale datasets and pre-trained models provide substantial knowledge, many downstream tasks in specialized scientific domains remain challenging due to data limitations. There's a need to effectively adapt rich pre-trained features to various data-limited scenarios.

Method: CLAdapter introduces attention mechanisms and cluster centers to personalize feature enhancement through distribution correlation and transformation matrices. It uses a unified interface compatible with CNNs and Transformers in both 2D and 3D contexts.

Result: Extensive experiments on 10 datasets across domains including generic, multimedia, biological, medical, industrial, agricultural, environmental, geographical, materials science, OOD, and 3D analysis show state-of-the-art performance in data-limited scientific domains.

Conclusion: CLAdapter effectively unleashes the potential of foundation vision models via adaptive transfer, enabling models to learn distinct representations tailored to different feature sets and adapt from rich pre-trained features to various downstream scenarios.
...
<details>
  <summary>Details</summary>
Motivation: Accurate grading of Conjunctival Melanocytic Intraepithelial Lesions (CMIL) is essential for treatment and melanoma prediction but remains difficult due to subtle morphological cues and interrelated diagnostic criteria, highlighting the need for standardized computational approaches.

Method: INTERACT-CMIL uses a multi-head deep learning framework with Shared Feature Learning, Combinatorial Partial Supervision, and an Inter-Dependence Loss to jointly predict five histopathological axes (WHO4, WHO5, horizontal spread, vertical spread, and cytologic atypia) while enforcing cross-task consistency.

Result: Trained on a multi-center dataset of 486 expert-annotated conjunctival biopsy patches, INTERACT-CMIL achieves consistent improvements over CNN and foundation-model baselines with relative macro F1 gains up to 55.1% (WHO4) and 25.0% (vertical spread), providing coherent, interpretable multi-criteria predictions aligned with expert grading.

Conclusion: INTERACT-CMIL offers a reproducible computational benchmark for CMIL diagnosis and represents a step toward standardized digital ocular pathology by providing coherent, interpretable predictions that align with expert grading practices.
...
TL;DR: CritiFusion improves text-to-image diffusion models by adding semantic critique and frequency-domain refinement at inference time, enhancing prompt alignment and visual quality without retraining.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image diffusion models achieve high visual fidelity but often fail to maintain semantic alignment with complex prompts, requiring better methods to ensure generated content matches the intended meaning.

Method: CritiFusion introduces two key components: CritiCore module uses vision-language and large language models to provide semantic feedback and enrich prompt context, while SpecFusion merges intermediate generation states in the spectral domain to preserve structural information and high-frequency details.

Result: Experiments show significant improvements in text-to-image correspondence metrics and visual quality, achieving state-of-the-art performance on human preference scores and aesthetic evaluations comparable to reward optimization approaches.

Conclusion: CritiFusion effectively enhances prompt fidelity, detail, and realism in text-to-image generation through semantic critique and spectral alignment, serving as a plug-in refinement stage compatible with existing diffusion models without requiring additional training.
...
TL;DR: ARFM (autoregressive flow matching) is a new probabilistic method for modeling sequential continuous data, trained on diverse video datasets to predict future point track locations over long horizons, improving downstream tasks like robot action and human motion prediction.


<details>
  <summary>Details</summary>
Motivation: Current motion prediction models are trained on narrow distributions and struggle with complex motions. While video prediction models have achieved visual realism through scaling, they still fail to accurately model complex motions. The authors aim to bridge this gap by developing a scalable method for probabilistic motion prediction.

Method: Developed autoregressive flow matching (ARFM), a new probabilistic modeling approach for sequential continuous data. Trained on diverse video datasets to generate future point track locations over long horizons. Created benchmarks for evaluating motion prediction models on human and robot motion.

Result: The model successfully predicts complex motions. Conditioning robot action prediction and human motion prediction on predicted future tracks significantly improves downstream task performance. Code and models are publicly available.

Conclusion: ARFM enables accurate modeling of complex motions through scaling on diverse video data, and using predicted future tracks as conditioning can substantially enhance performance in practical applications like robotics and human motion prediction.
...
TL;DR: Multimodal diffeomorphic registration using Neural ODEs with structural descriptors for modality-agnostic registration without requiring training data.
...
Method: Proposes an instance-specific framework using Neural ODEs with structural descriptors as modality-agnostic metric models. Three variants integrate image-based or feature-based structural descriptors and nonstructural image similarities computed by local mutual information.
...
Conclusion: The proposed Neural ODE-based multimodal diffeomorphic registration framework provides accurate, efficient, and robust registration without requiring training data, overcoming limitations of traditional methods and learning-based approaches in multimodal settings.
...
<details>
  <summary>Details</summary>
Motivation: Autonomous driving needs diverse training data covering long-tailed scenarios. Current methods treat 3D asset insertion and novel view synthesis separately, lacking realistic integration. A unified framework is needed to create more diverse and realistic driving scenarios for training.

Method: SCPainter integrates 3D Gaussian Splat car asset representations with 3D scene point clouds and uses diffusion-based generation. The 3D GS assets and scene point clouds are projected into novel views, and these projections condition a diffusion model to generate high-quality images.
...
Conclusion: SCPainter provides a unified solution that combines 3D asset insertion and novel view synthesis to enhance autonomous driving simulation by creating more diverse and realistic training scenarios.
...
TL;DR: Self-supervised pretraining (USF-MAE) improves automated detection of cystic hygroma in first-trimester ultrasound images compared to supervised DenseNet-169 baseline.


<details>
  <summary>Details</summary>
Motivation: Cystic hygroma is a high-risk prenatal finding with poor outcomes. Automated detection needs better accuracy and reproducibility, but supervised deep learning is limited by small labeled datasets. Self-supervised pretraining could overcome this limitation.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods for decomposed 4D scene reconstruction rely heavily on video segmentation quality, which is often unstable and leads to unreliable reconstruction results. The authors aim to overcome this limitation by eliminating the need for video segmentation altogether.

Method: The method represents decomposed 4D scenes using Freetime FeatureGS - Gaussian primitives with learnable features and linear motion ability. It uses a contrastive loss to align primitive features with 2D segmentation maps, and employs a streaming feature learning strategy with temporally ordered sampling to propagate features over time.

Result: Experimental results on several datasets show that the reconstruction quality of this method outperforms recent methods by a large margin, demonstrating superior performance in decomposed 4D scene reconstruction.

Conclusion: The proposed Freetime FeatureGS with streaming feature learning successfully achieves decomposed 4D scene reconstruction without relying on video segmentation, overcoming limitations of previous methods and achieving significantly better reconstruction quality.
...
TL;DR: This paper introduces an adaptive visual token pruning method for Large Multimodal Models that handles long context inputs with multiple images by dynamically allocating token budgets based on intra-image diversity and inter-image variation.
...
Method: Two-stage adaptive pruning: 1) Intra-image stage allocates content-aware token budgets per image and greedily selects most representative tokens; 2) Inter-image stage performs global diversity filtering and Pareto selection balancing diversity with text alignment.
...
Conclusion: The proposed adaptive pruning method effectively addresses visual token redundancy in multi-image, long-context scenarios by decomposing redundancy into intra-image and inter-image components and dynamically allocating token budgets.
...
<details>
  <summary>Details</summary>
Motivation: Existing token merging/pruning methods for Vision Transformers often overlook spatial continuity and neighbor relationships, causing loss of local context. There's a need for methods that preserve spatial structure while reducing computational redundancy.
...
Result: Achieves state-of-the-art accuracy-efficiency trade-offs compared to existing token reduction methods, demonstrating the effectiveness of preserving spatial continuity.

Conclusion: The work highlights the importance of spatial continuity and neighbor structure in ViT token reduction, offering new insights for architectural optimization of Vision Transformers.
...
<details>
  <summary>Details</summary>
Motivation: Semantics and dynamics understanding is crucial for embodied agents, but these tasks have high data redundancy compared to static scene understanding. Current approaches lack principled methods for selecting informative views from multi-camera setups.

Method: Formulates view selection as an active learning problem using Fisher Information to quantify informativeness of candidate views. The method jointly handles semantic Gaussian parameters and deformation networks, providing a principled alternative to heuristic strategies.

Result: Evaluated on large-scale static images and dynamic video datasets. The approach consistently improves rendering quality and semantic segmentation performance, outperforming baseline methods based on random selection and uncertainty-based heuristics.

Conclusion: The proposed active learning framework with Fisher Information provides an effective, principled approach for selecting informative views that enhances both semantic reasoning and dynamic scene modeling in multi-camera setups.
...
TL;DR: Proposes AGQG module using Arithmetic Graded Response Model to address semantic drift in AGIQA by modeling quality as ability meeting difficulty levels, improving existing frameworks.


<details>
  <summary>Details</summary>
Motivation: Existing AGIQA models suffer from "semantic drift" where image embeddings show inconsistent similarities across quality grades due to semantic inconsistencies between text embeddings and their descriptions, undermining reliability of text-image shared-space learning.

Method: Proposes Arithmetic GRM based Quality Grading (AGQG) module with two-branch design: one estimates image ability, the other constructs multiple difficulty levels in arithmetic manner to ensure monotonicity and unimodal quality distribution. Inspired by psychometric Graded Response Model.
...
Conclusion: The proposed AGQG module addresses semantic drift in AGIQA by modeling quality assessment through psychometric principles, offering a promising component for future IQA models with improved reliability and interpretability.
...
TL;DR: EPD-Solver is a novel ODE solver for diffusion models that uses parallel gradient evaluations to reduce truncation errors while maintaining low latency, with a two-stage optimization framework including distillation and RL fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Diffusion models have excellent generative performance but suffer from high sampling latency due to sequential denoising. Existing acceleration methods degrade image quality under low-latency budgets due to accumulated truncation errors from high-curvature trajectory segments.

Method: EPD-Solver incorporates multiple parallel gradient evaluations per step using Mean Value Theorem for vector-valued functions, leveraging that sampling trajectories are confined to low-dimensional manifolds. It uses a two-stage optimization: 1) distillation-based learning of parameters, 2) parameter-efficient RL fine-tuning that reformulates the solver as a stochastic Dirichlet policy operating only in solver space.

Result: The method reduces truncation errors while preserving low-latency sampling through parallelizable gradient computations. It can also serve as a plugin (EPD-Plugin) to improve existing ODE samplers.

Conclusion: EPD-Solver effectively mitigates truncation errors in diffusion model sampling acceleration while maintaining low latency, with RL fine-tuning that avoids reward hacking by operating only in the low-dimensional solver space rather than fine-tuning the entire backbone model.
...
TL;DR: VPTracker is the first global tracking framework using Multimodal LLMs for vision-language tracking, introducing location-aware visual prompting to handle distractions while maintaining global search capabilities.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language tracking methods are limited to local search, making them vulnerable to viewpoint changes, occlusions, and rapid target movements. There's a need for more robust tracking that can search globally across the entire image space.

Method: Proposes VPTracker, a global tracking framework using Multimodal Large Language Models with location-aware visual prompting. Creates region-level prompts based on target's previous location, enabling prioritized region-level recognition and fallback to global inference when needed.

Result: Extensive experiments show significant improvements in tracking stability and target disambiguation under challenging scenarios, effectively suppressing interference from distracting visual content.

Conclusion: The approach opens a new avenue for integrating MLLMs into visual tracking, balancing global search robustness with effective distraction suppression through location-aware prompting.
...
TL;DR: Proposes efficient 3D medical image reconstruction using 3D Gaussian and tri-plane representations to overcome computational cost and detail loss in sparse slices.
...
Method: Combines 3D Gaussian representation with tri-plane representations to maintain efficient rendering and geometric representation while enhancing structural continuity and semantic consistency under sparse slicing conditions.
...
Conclusion: Provides an efficient and reliable new approach for 3D visualization and clinical analysis of medical images, addressing key limitations of traditional methods.
...
Method: Created a new dataset simulating real-world low-quality images and evaluated multiple open-vocabulary object detection models (OWLv2, OWL-ViT, GroundingDINO, Detic) under different levels of image degradation.
...
Conclusion: Open-vocabulary object detection models are sensitive to high-level image degradation, with OWLv2 showing the most robustness. The new dataset will be released to support future research in this area.
...
TL;DR: EgoReAct: First autoregressive framework generating 3D-aligned human reaction motions from egocentric video in real-time, addressing spatial misalignment and causality challenges.


<details>
  <summary>Details</summary>
Motivation: Existing datasets for egocentric video-reaction modeling suffer from spatial inconsistency (e.g., dynamic motions paired with fixed-camera videos), making it challenging to model adaptive, context-sensitive human responses while maintaining strict causality and precise 3D spatial alignment.

Method: 1) Construct Human Reaction Dataset (HRD) to address data scarcity and misalignment. 2) Develop EgoReAct framework: compress reaction motion into compact latent space via Vector Quantised-VAE, then train Generative Pre-trained Transformer for autoregressive generation from visual input. 3) Incorporate 3D dynamic features (metric depth, head dynamics) to enhance spatial grounding.
...
Conclusion: EgoReAct successfully addresses the dual challenges of causal generation and 3D spatial alignment for human reaction modeling from egocentric video, with plans to release code, models, and data upon acceptance.
...
<details>
  <summary>Details</summary>
Motivation: Panoramic depth estimation has poor zero-shot generalization compared to perspective images due to limited training data, creating a need to transfer capabilities from the perspective domain.

Method: Adapts Depth Anything V2 for panoramic use by learning a shift parameter from ViT backbone to transform scale-and-shift-invariant output into scale-invariant estimates, plus circular padding in DPT decoder to eliminate seam artifacts.

Result: Achieves over 50% and 10% relative depth error reduction on indoor/outdoor benchmarks vs base model, and ~30% relative error improvement over PanDA across all test datasets, establishing new SOTA for zero-shot panoramic depth estimation.

Conclusion: DA360 successfully bridges the gap in zero-shot panoramic depth estimation by effectively transferring capabilities from perspective domain, with significant performance improvements on both indoor and outdoor benchmarks.
...
<details>
  <summary>Details</summary>
Motivation: Existing interpretable SR methods rely on black-box deep networks, leaving degradation processes unknown and uncontrollable. Need for transparent, structured representation of latent degradation fitting.

Method: Kolmogorov-Arnold Neural Operator (KANO) uses additive structure of finite B-spline functions to approximate spectral curves piecewise. Learns shape parameters of splines to capture spectral characteristics like local linear trends and peak-valley structures.

Result: KANO accurately captures key spectral characteristics and provides physical interpretability to SR results. Comparative study shows advantages of KANO over MLPs and KANs in handling complex sequence fitting tasks.

Conclusion: KANO offers transparent degradation modeling for interpretable SR, providing insights for developing interpretable SR techniques through systematic comparison of different network architectures.
...
Result: The method achieves both high accuracy and efficiency, outperforming existing methods. The authors also contribute CCS3D, a challenging synthetic dataset for controlled evaluation of 3D change detection.

Conclusion: SCaR-3D provides an effective solution for 3D scene change detection with explicit separation of pre- and post-change states, enabling better scene monitoring, exploration, and continual reconstruction applications.
...
<details>
  <summary>Details</summary>
Motivation: Cameras with IMUs are widely used in applications like self-driving cars and smartphones. IMUs provide vertical direction, reducing relative pose estimation from 5DOF to 3DOF. Existing methods need improvement for accurate focal length and pose estimation from minimal correspondences.
...
Conclusion: The paper presents an efficient solver for estimating 3DOF relative pose and focal length from two affine correspondences when vertical direction is known, achieving superior performance compared to existing methods for applications combining cameras with IMUs.
...
TL;DR: ByteLoom is a Diffusion Transformer framework for generating realistic human-object interaction videos with geometrically consistent objects, using simplified human conditioning and 3D object inputs.
...
Method: Introduces ByteLoom, a DiT-based framework with: (1) RCM-cache mechanism using Relative Coordinate Maps as universal representation to maintain object geometry consistency and control 6-DoF transformations; (2) Progressive training curriculum to compensate for HOI dataset scarcity and relax hand mesh requirements.

Result: Extensive experiments demonstrate faithful preservation of human identity and object's multi-view geometry, while maintaining smooth motion and object manipulation.

Conclusion: ByteLoom addresses key limitations in HOI video generation by providing geometrically consistent object illustration with simplified conditioning, reducing dependency on hand mesh annotations while improving cross-view consistency.
...
Conclusion: MUSON addresses critical gaps in social navigation datasets by providing structured reasoning supervision and balanced action distributions, serving as an effective benchmark for developing socially compliant navigation systems.
...
TL;DR: Lamps is a self-supervised learning framework that learns anatomical representations from chest radiographs by leveraging consistency, coherence, and hierarchy of human anatomy as supervision signals.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised learning methods in medical imaging often overlook the fundamental anatomical structure of human body that serves as the true foundation for medical images, limiting their ability to effectively learn anatomical features.

Method: Built Lamps framework that pre-trains on large-scale chest radiographs using consistency, coherence, and hierarchy of human anatomy as supervision signals in a self-supervised manner.
...
Conclusion: Lamps demonstrates that learning from multiple anatomical perspectives enables foundation models to develop meaningful, robust representations aligned with human anatomy structure, offering unique opportunities for medical imaging foundation models.
...
TL;DR: Proposes a data-oriented pipeline (NSF) to mitigate spurious correlations in deep learning by identifying dispersed feature distributions, neutralizing spurious features via grouping, aligning transformations to eliminate them, and updating classifiers.


<details>
  <summary>Details</summary>
Motivation: Existing methods for addressing spurious correlations rely on manual annotation of spurious attributes or empirical assumptions about bias simplicity, which often fail due to the intricate and elusive nature of real-world spurious correlations.

Method: Four-step pipeline: 1) Identify spurious features by observing dispersed distribution patterns in feature space, 2) Obtain bias-invariant representation by neutralizing spurious features through simple grouping, 3) Learn feature transformation to eliminate spurious features by aligning with bias-invariant representation, 4) Update classifier with learned transformation.
...
Conclusion: The proposed data-oriented approach effectively mitigates spurious correlations without requiring manual annotation of spurious attributes, providing a practical solution for real-world debiasing tasks across both image and NLP domains.
...
TL;DR: M-ErasureBench is a multimodal evaluation framework for concept erasure methods, showing existing approaches fail against learned embeddings and inverted latents, while IRECE improves robustness across modalities.


<details>
  <summary>Details</summary>
Motivation: Existing concept erasure methods focus only on text prompts, ignoring other input modalities (learned embeddings, inverted latents) that can be exploited as attack surfaces, allowing erased concepts to re-emerge despite defenses.

Method: Introduces M-ErasureBench framework with 5 evaluation scenarios across text prompts, learned embeddings, and inverted latents (with white-box/black-box access). Proposes IRECE module that localizes target concepts via cross-attention and perturbs associated latents during denoising.

Result: Existing methods achieve strong erasure against text prompts but fail under learned embeddings and inverted latents (CRR >90% in white-box). IRECE reduces CRR by up to 40% in challenging white-box latent inversion while preserving visual quality.

Conclusion: M-ErasureBench provides first comprehensive benchmark beyond text prompts, and IRECE offers practical safeguards for building more reliable protective generative models against multimodal attacks.
...
<details>
  <summary>Details</summary>
Motivation: Existing 3D segmentation frameworks rely exclusively on visual learning from large annotated datasets, restricting adaptability to new domains and clinical tasks. They lack semantic understanding, making them ineffective for flexible, user-defined segmentation objectives.
...
<details>
  <summary>Details</summary>
Motivation: Standard Classifier-Free Guidance (CFG) causes iterative refinement methods to fail by pushing sampling paths off the data manifold, leading to diverging approximation errors and undermining refinement quality.

Method: Guided Path Sampling (GPS) replaces CFG's extrapolation with principled, manifold-constrained interpolation to keep sampling paths on the data manifold. Includes optimal scheduling strategy that dynamically adjusts guidance strength to align with coarse-to-fine generation.
...
Conclusion: Path stability is essential for effective iterative refinement, and GPS provides a robust framework to achieve it by replacing unstable extrapolation with manifold-constrained interpolation.
...
TL;DR: Proposes hash grid feature pruning for Gaussian splatting compression by identifying and removing invalid features from sparse regions, reducing storage/transmission overhead without performance loss.


<details>
  <summary>Details</summary>
Motivation: Hash grids in Gaussian splatting have many invalid features due to irregular 3D distribution of splats, causing redundant storage and transmission overhead that needs to be addressed.
...
Conclusion: The proposed feature pruning effectively reduces hash grid storage requirements for Gaussian splatting compression without compromising quality, offering practical compression improvements.
...
TL;DR: JavisGPT is the first unified multimodal LLM for joint audio-video comprehension and generation, using a SyncFusion module and three-stage training pipeline, achieving state-of-the-art performance on JAV benchmarks.


<details>
  <summary>Details</summary>
Motivation: There's a need for unified models that can handle both audio and video modalities simultaneously for comprehensive understanding and generation tasks, particularly for temporally synchronized content.

Method: Uses encoder-LLM-decoder architecture with SyncFusion module for spatio-temporal audio-video fusion and synchrony-aware learnable queries. Employs three-stage training: multimodal pretraining, audio-video fine-tuning, and large-scale instruction-tuning using JavisInst-Omni dataset (200K+ GPT-4o-curated dialogues).
...
Conclusion: JavisGPT demonstrates effective joint audio-video understanding and generation capabilities through its unified architecture and comprehensive training approach, setting new standards for multimodal LLMs.
...
TL;DR: ColaVLA is a vision-language-action framework for autonomous driving that transfers VLM reasoning to a latent space and uses hierarchical parallel planning for efficient, safe trajectory generation.
...
TL;DR: DRMNet uses density maps as spatial priors to focus computational resources on dense object clusters in remote sensing imagery, improving detection of tiny, occluded objects through density-guided attention and frequency-based feature fusion.


<details>
  <summary>Details</summary>
Motivation: Current detection methods fail to adaptively focus computational resources on dense regions of tiny objects in remote sensing imagery, leading to poor performance with severe occlusion and limited pixel footprints.

Method: Three key components: 1) Density Generation Branch (DGB) models object distribution patterns; 2) Dense Area Focusing Module (DAFM) uses density maps to focus on dense areas for efficient local-global feature interaction; 3) Dual Filter Fusion Module (DFFM) disentangles multi-scale features into high/low-frequency components using DCT and performs density-guided cross-attention.

Result: Extensive experiments on AI-TOD and DTOD datasets show DRMNet surpasses state-of-the-art methods, especially in complex scenarios with high object density and severe occlusion.

Conclusion: DRMNet effectively addresses tiny object detection challenges in dense remote sensing imagery by using density maps as explicit spatial priors to guide adaptive feature learning and resource allocation.
...
<details>
  <summary>Details</summary>
Motivation: Conventional object detectors using cross-entropy classification are vulnerable to class imbalance and label noise. The paper aims to address these limitations by incorporating CLIP's contrastive vision-language supervision.

Method: A detector-agnostic framework with a lightweight parallel head projects region/grid features into CLIP embedding space and aligns them with learnable class-specific text embeddings using InfoNCE contrastive loss plus auxiliary cross-entropy. All standard detection losses are optimized simultaneously.

Result: Substantial improvements on Pascal VOC 2007+2012 with Faster R-CNN and on MS COCO 2017 with YOLOv11, while preserving real-time inference speed. Consistent enhancements across diverse architectures and datasets.

Conclusion: Joint optimization with learnable text embeddings significantly enhances closed-set detection performance across various detector architectures and datasets, providing a simple yet effective framework.
...
Method: WRCFormer fuses raw radar cubes with camera inputs via multi-view representations of decoupled radar cube. Uses Wavelet Attention Module in wavelet-based FPN to enhance sparse radar and image representations. Introduces Geometry-guided Progressive Fusion, a two-stage query-based modality-agnostic fusion mechanism.
...
Conclusion: WRCFormer effectively addresses challenges of 4D radar-camera fusion by directly processing raw radar data with efficient multi-view representations and attention mechanisms, providing superior performance especially in adverse weather conditions.
...
TL;DR: YOLO-IOD is a real-time incremental object detection framework built on YOLO-World that addresses catastrophic forgetting through conflict-aware pseudo-label refinement, importance-based kernel selection, and cross-stage asymmetric knowledge distillation.


<details>
  <summary>Details</summary>
Motivation: Current incremental object detection methods rely on Faster R-CNN or DETR detectors and don't support real-time YOLO frameworks. The paper aims to enable incremental learning for YOLO-based detectors while addressing catastrophic forgetting issues.
...
Result: The method achieves superior performance with minimal forgetting on both conventional benchmarks and the new LoCo COCO benchmark (which eliminates data leakage across stages).

Conclusion: YOLO-IOD successfully enables real-time incremental object detection using YOLO frameworks while effectively addressing three key types of knowledge conflicts that cause catastrophic forgetting, outperforming existing methods.
...
TL;DR: ReamCamo: A unified out-painting framework for realistic camouflaged image generation with layout controls and multi-modal conditions to improve semantic coherence and visual fidelity.
...
Method: Proposes ReamCamo, a unified out-painting based framework that introduces layout controls to regulate global image structure and improve semantic coherence. Uses multi-modal textual-visual conditions combining fine-grained textual task descriptions with texture-oriented background retrieval. Also introduces a background-foreground distribution divergence metric to quantitatively assess camouflage quality.
...
Conclusion: ReamCamo addresses key limitations in existing camouflaged image generation methods by providing better layout control and multi-modal guidance, resulting in more realistic training data for camouflaged object detection tasks.
...
<details>
  <summary>Details</summary>
Motivation: Standard RGB cameras struggle with motion blur in high-speed and low-light scenarios for 6DoF pose estimation. Event cameras offer high temporal resolution but current methods still have suboptimal performance for fast-moving objects.
...
Result: PoseStreamer achieves superior accuracy in high-speed moving scenarios and exhibits strong generalizability for unseen moving objects. Also introduces MoCapCube6D dataset for benchmarking rapid motion performance.

Conclusion: The proposed PoseStreamer framework effectively addresses 6DoF pose estimation challenges in high-speed scenarios using event cameras, demonstrating robust performance and generalizability through its multi-modal approach and novel dataset.
...
<details>
  <summary>Details</summary>
Motivation: Current text-guided medical image segmentation methods struggle with processing both diagnostic and descriptive texts simultaneously, and fail to capture positional constraints, leading to inaccurate segmentation (e.g., mislocating lesions like "in the left lower lung").

Method: Proposes Spatial-aware Symmetric Alignment (SSA) framework with: 1) symmetric optimal transport alignment mechanism for bi-directional fine-grained multimodal correspondences between image regions and multiple text expressions; 2) composite directional guidance strategy that explicitly introduces spatial constraints by constructing region-level guidance masks.
...
<details>
  <summary>Details</summary>
Motivation: Existing prompt-based methods for removing/modifying identity-specific features require either subjects to be well-represented in pre-trained models or need model fine-tuning for specific identities, lacking control over facial attributes during anonymization.

Method: Analyzes identity generation process and introduces reverse personalization framework using conditional diffusion inversion for direct image manipulation without text prompts. Incorporates identity-guided conditioning branch to generalize beyond subjects in training data, supporting attribute-controllable anonymization.
...
Conclusion: The proposed reverse personalization framework provides an effective solution for face anonymization with attribute control, addressing limitations of existing methods while maintaining high image quality.
...
TL;DR: A unified RGB-only UAV pipeline for apple orchards that integrates disease detection, freshness assessment, and yield estimation using deep learning models on low-cost hardware.


<details>
  <summary>Details</summary>
Motivation: Existing UAV systems for apple orchards address tasks in isolation, rely on expensive multispectral sensors, and often require cloud support, limiting accessibility and practicality for precision agriculture.

Method: Integrated pipeline using ResNet50 for leaf disease detection, VGG16 for apple freshness determination, and YOLOv8 for real-time apple detection/localization. Runs on ESP32-CAM and Raspberry Pi for fully offline on-site inference without cloud support.

Result: Achieved 98.9% accuracy for leaf disease classification, 97.4% accuracy for freshness classification, and 0.857 F1 score for apple detection, demonstrating high performance on affordable RGB-only hardware.

Conclusion: The framework provides an accessible, scalable alternative to multispectral UAV solutions, enabling practical precision agriculture on low-cost hardware with offline capabilities.
...
Conclusion: OpenGround effectively addresses the limitation of pre-defined OLTs in 3D visual grounding, enabling open-world applications through active cognition-based reasoning and demonstrating superior performance across multiple benchmarks.
...
TL;DR: A novel framework for object classification using Geo-Semantic Contextual Graphs (GSCG) that integrates depth estimation with panoptic/material segmentation to create structured scene representations, achieving 73.4% accuracy and outperforming context-agnostic and LLM-based approaches.


<details>
  <summary>Details</summary>
Motivation: Humans use rich contextual understanding (spatial relationships, material properties, object co-occurrence) for object recognition, while most computational systems operate on isolated image regions, ignoring vital contextual information.

Method: Constructs Geo-Semantic Contextual Graph (GSCG) from monocular images by integrating metric depth estimation with unified panoptic/material segmentation. Objects become nodes with geometric, chromatic, material attributes; spatial relationships become edges. Uses graph-based classifier that aggregates features from target object, immediate neighbors, and global scene context.

Result: Achieves 73.4% classification accuracy on COCO 2017 train/val splits, dramatically outperforming context-agnostic versions (38.4%), fine-tuned ResNet models (max 53.5%), and state-of-the-art multimodal LLM Llama 4 Scout (42.3%).

Conclusion: Explicitly structured and interpretable context representation (GSCG) is superior for object recognition tasks, providing both high performance and interpretable reasoning processes compared to context-agnostic or LLM-based approaches.
...
TL;DR: Analysis of two vision-language models (Qwen2.5-VL-7B-Instruct and Llama-4-Scout-17B-16E-Instruct) architecture and their application in a video-to-artifact pipeline for person detection with emotion attributes.


<details>
  <summary>Details</summary>
Motivation: To understand how architectural properties of modern VLMs map to practical video analysis systems, specifically for person detection with bounding boxes and emotion attributes, and to clarify system constraints for robust interface design and evaluation planning.

Method: Architecture-led analysis of two VLMs, connecting their multimodal foundation (visual tokenization, Transformer attention, instruction following) to a video-to-artifact pipeline that samples frames, prompts VLMs for person detection with bounding boxes and attributes, validates output structure using predefined schema, and optionally renders annotated videos.

Result: Identified key system constraints: structured outputs can be syntactically valid but semantically incorrect, schema validation is structural not geometric, person identifiers are frame-local, and interactive single-frame analysis returns free-form text rather than schema-enforced JSON.

Conclusion: Understanding the distinctions between model behavior and system constraints is critical for writing defensible claims, designing robust interfaces, and planning proper evaluation of vision-language model applications in video analysis pipelines.
...
Method: Proposes Co2S with: 1) Heterogeneous dual-student architecture using ViT-based models initialized with CLIP and DINOv3; 2) Explicit-implicit semantic co-guidance using text embeddings and learnable queries; 3) Global-local feature collaborative fusion to combine CLIP's global context with DINOv3's local details.

Result: Extensive experiments on six popular datasets show superior performance across various partition protocols and diverse scenarios, achieving leading results in semi-supervised remote sensing segmentation.

Conclusion: Co2S effectively mitigates pseudo-label drift by synergistically fusing priors from vision-language and self-supervised models, providing a stable solution for semi-supervised remote sensing image segmentation with reduced annotation burden.
...
TL;DR: LAM3C learns 3D representations from unlabeled videos without real 3D sensors, using video-generated point clouds and achieving state-of-the-art performance on indoor segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: Collecting large-scale 3D scene scans is expensive and labor-intensive, so the paper explores whether 3D representations can be learned from unlabeled videos recorded without any real 3D sensors.

Method: Proposes LAM3C framework with RoomTours dataset (49,219 video-generated point clouds from room-walkthrough videos), and a noise-regularized loss that enforces local geometric smoothness and feature stability under noisy point clouds.
...
TL;DR: Video-BrowseComp is a new benchmark for evaluating agentic video reasoning that requires actively navigating video timelines and cross-referencing visual evidence on the open web, unlike existing passive video perception benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current video benchmarks focus on passive perception of curated clips and fail to evaluate agentic video research, which requires active interrogation of video timelines, cross-referencing dispersed evidence, and verifying claims against the open web. There's a significant modality gap in processing dynamic video content.

Method: Created Video-BrowseComp benchmark with 210 questions that enforce mandatory dependency on temporal visual evidence. Answers cannot be derived through text search alone - models must navigate video timelines to verify external claims. Evaluated state-of-the-art models including GPT-5.1 (with Search).

Result: Even advanced search-augmented models like GPT-5.1 (w/ Search) achieve only 15.24% accuracy. Models rely heavily on textual proxies, performing well in metadata-rich domains (TV shows with plot summaries) but collapsing in metadata-sparse, dynamic environments (sports, gameplay) where visual grounding is essential.

Conclusion: Video-BrowseComp advances the field beyond passive perception toward proactive video reasoning, revealing a critical bottleneck in current models' ability to perform agentic video research that requires active visual evidence gathering from the open web.
...
TL;DR: Segmentation-guided CXR classification using MedSAM for lung extraction improves interpretability and shows task-dependent masking effects on abnormality classification.


<details>
  <summary>Details</summary>
Motivation: Automated CXR interpretation faces challenges from weak disease signals, dataset bias, and limited spatial supervision. Foundation models like MedSAM offer anatomically grounded priors to improve robustness and interpretability in CXR analysis.

Method: Proposed segmentation-guided CXR classification pipeline: fine-tuned MedSAM for lung region extraction, applied to NIH CXR dataset for multi-label classification of five abnormalities (Mass, Nodule, Pneumonia, Edema, Fibrosis) using deep CNNs, with normal case evaluated via derived score.

Result: MedSAM produces anatomically plausible lung masks across diverse conditions. Masking effects are task/architecture-dependent: ResNet50 on original images best for abnormality discrimination; loose masking yields comparable macro AUROC but improves No Finding discrimination; tight masking reduces abnormality performance but improves training efficiency; loose masking partially preserves perihilar/peripheral context.

Conclusion: Lung masking should be treated as a controllable spatial prior selected to match backbone architecture and clinical objective, rather than applied uniformly. There's a trade-off between abnormality-specific classification and normal case screening.
...
<details>
  <summary>Details</summary>
Motivation: Current generative models for medical imaging suffer from feature entanglement when operating in global pixel domains or using binary masks, leading to corrupted anatomical substrates and structural discontinuities. There's a need for a principled approach that preserves anatomical integrity while modeling pathological variations.
...
Result: PathoSyn significantly outperforms holistic diffusion and mask-conditioned baselines on tumor imaging benchmarks in both perceptual realism and anatomical fidelity. The framework generates high-fidelity patient-specific synthetic datasets with interpretable counterfactual disease progression modeling.

Conclusion: PathoSyn provides a mathematically principled pipeline for medical image synthesis that preserves anatomical integrity while modeling pathological variations, facilitating robust diagnostic algorithm development in low-data regimes and supporting precision intervention planning and clinical decision-support benchmarking.
...
TL;DR: Deep deformable registration models have inherent domain-shift immunity due to their reliance on local features rather than global appearance, enabling robust cross-domain performance without large diverse training datasets.


<details>
  <summary>Details</summary>
Motivation: To understand why learning-based deformable registration models can be robust to domain shift despite conventional wisdom suggesting they need large diverse datasets, and to identify the underlying mechanisms of this robustness.

Method: Introduce UniReg, a universal registration framework that decouples feature extraction from deformation estimation using fixed, pre-trained feature extractors and a UNet-based deformation network. Train on single dataset and test cross-domain.

Result: UniReg shows robust cross-domain and multi-modal performance comparable to optimization-based methods despite single-dataset training. Analysis reveals conventional CNN failures under modality shift come from dataset-induced biases in early convolutional layers.

Conclusion: Domain-shift immunity is inherent in deep deformable registration due to reliance on local features. Local feature consistency drives robustness, motivating backbone designs that preserve domain-invariant local features rather than requiring large diverse datasets.
...
<details>
  <summary>Details</summary>
Motivation: Previous semi-supervised 3D detection methods overlook that models with limited labeled data struggle to capture geometric information, which is crucial for object perception and localization. The paper addresses this gap by enhancing geometric relation understanding.

Method: 1) Keypoint-based geometric relation supervision module transfers teacher model's geometric knowledge to student; 2) Voxel-wise data augmentation strategy increases object geometry diversity with distance-decay mechanism to preserve distant object integrity; 3) Framework can be combined with existing SS3D methods.

Result: Extensive experiments on ONCE and Waymo datasets show effectiveness and generalization, achieving state-of-the-art results. The method can be integrated with different SS3D approaches for further performance improvements.

Conclusion: GeoTeacher successfully enhances student models' ability to capture geometric relations in semi-supervised 3D object detection, addressing a key limitation of previous methods and achieving superior performance on benchmark datasets.
...
<details>
  <summary>Details</summary>
Motivation: Existing T2I alignment evaluation methods rely on coarse-grained metrics or static QA pipelines that lack fine-grained interpretability and struggle to reflect human preferences.

Method: Adopts a structured "grounding-reasoning-conclusion" paradigm using MLLMs to localize semantic elements and derive interpretable alignment judgments, optimized via Group Relative Policy Optimization with composite reward function.

Result: Achieves state-of-the-art performance across four benchmarks (EvalMuse-40K, RichHF, MHaluBench, GenAI-Bench), outperforming both proprietary models and supervised baselines with superior inference efficiency.

Conclusion: REVEALER provides a unified, interpretable framework for fine-grained element-level alignment evaluation that better reflects human preferences while maintaining computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Image-based 3D object detection faces a critical trade-off: methods with high accuracy need dense 3D supervision, while unsupervised methods struggle to extract accurate geometry from images alone. The authors aim to overcome this limitation by leveraging complementary geometric representations.

Method: The framework uses a dual-representation architecture that: 1) adapts generalizable Gaussian Splatting to extract fine-grained surface details, 2) employs voxel representations for structured spatial context, and 3) develops a cross-representation enhancement mechanism that enriches voxel features with geometric details from Gaussian fields through learnable integration.
...
Conclusion: The synergistic combination of continuous Gaussian and discrete voxel representations enables accurate 3D object detection from RGB images alone, overcoming the limitations of previous approaches that either required dense supervision or struggled with geometry extraction.
...
TL;DR: A novel Driving World Model framework using 3D Gaussian scene representation that enables 3D scene understanding and multi-modal generation through early modality alignment and language-guided sampling.


<details>
  <summary>Details</summary>
Motivation: Existing Driving World Models lack 3D scene understanding capabilities and cannot interpret or reason about driving environments. Current approaches using point clouds or BEV features fail to accurately align textual information with 3D scenes, limiting their ability to understand and generate multi-modal content.

Method: Proposes a unified DWM framework based on 3D Gaussian scene representation that embeds linguistic features into each Gaussian primitive for early modality alignment. Includes a task-aware language-guided sampling strategy to remove redundant 3D Gaussians and inject compact 3D tokens into LLMs. Also designs a dual-condition multi-modal generation model combining high-level language conditions with low-level image conditions.

Result: Achieves state-of-the-art performance on nuScenes and NuInteract datasets, demonstrating effective 3D scene understanding and multi-modal generation capabilities.

Conclusion: The proposed 3D Gaussian-based framework successfully addresses limitations of existing DWMs by enabling both 3D scene understanding and multi-modal generation through early modality alignment and language-guided sampling, with code to be publicly released.
...
<details>
  <summary>Details</summary>
Motivation: To improve forest cover mapping accuracy by combining traditional OBIA techniques with modern deep learning approaches, leveraging freely available Sentinel-2 satellite imagery and accessible tools like QGIS for environmental monitoring.

Method: Proposes ForCM approach that integrates Object-Based Image Analysis with Deep Learning models (UNet, UNet++, ResUNet, AttentionUNet, ResNet50-Segnet) applied to multispectral Sentinel-2 Level 2A imagery of the Amazon Rainforest using three different band combinations.

Result: ForCM significantly improves mapping accuracy: ResUNet-OBIA achieves 94.54% overall accuracy, AttentionUNet-OBIA achieves 95.64% accuracy, compared to 92.91% using traditional OBIA alone.

Conclusion: The hybrid ForCM approach combining OBIA with deep learning outperforms traditional methods, demonstrating the effectiveness of integrating computer vision techniques with geospatial analysis for accurate forest cover mapping using freely available tools and data.
...
TL;DR: Proposes using Unreal Engine to generate synthetic RGB data for military target detection, addressing high costs of SAR data and lack of military datasets, with synthetic-to-real transfer experiments showing supervised methods outperform unsupervised ones.


<details>
  <summary>Details</summary>
Motivation: Military object detection faces challenges with mixed environments and limited datasets. SAR data is robust but expensive, while RGB cameras are affordable but lack military datasets. Need for cost-effective cross-domain adaptation solutions.

Method: Generate synthetic RGB data using Unreal Engine for military targets. Conduct synthetic-to-real transfer experiments: train on synthetic data, validate on web-collected real military datasets. Benchmark state-of-the-art domain adaptation methods with varying supervision levels.

Result: Supervised methods using minimal image hints (e.g., object class) achieve substantial improvement over unsupervised or semi-supervised DA methods. Identifies remaining challenges in cross-domain military target detection.

Conclusion: Synthetic data generation via Unreal Engine offers promising approach for military target detection, but current domain adaptation methods still face challenges in synthetic-to-real transfer, with supervised approaches showing better performance than unsupervised ones.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion model applications rely on heuristic selection of diffusion timestep features for single-task prediction, which often leads to sub-optimal performance biased toward certain tasks. The authors aim to address this limitation by investigating versatile diffusion timestep features for few-shot dense prediction.

Method: Proposes two modules: 1) Task-aware Timestep Selection (TTS) to select ideal diffusion timesteps based on timestep-wise losses and similarity scores, and 2) Timestep Feature Consolidation (TFC) to consolidate selected timestep features. Accompanied by parameter-efficient fine-tuning adapter for few-shot learning.

Result: Empirically validated on the large-scale Taskonomy dataset for dense prediction, achieving superiority in dense prediction performance given only a few support queries in practical universal and few-shot learning scenarios.

Conclusion: The proposed learnable timestep consolidation method effectively addresses the limitations of heuristic timestep selection in diffusion models, improving few-shot dense prediction performance through adaptive timestep selection and feature consolidation.
...
TL;DR: AVOID dataset for real-time obstacle detection in adverse conditions using simulated environment with comprehensive sensor data.


<details>
  <summary>Details</summary>
Motivation: Existing datasets lack diverse adverse conditions and consistent visual domains for small road hazards detection needed for reliable autonomous driving.

Method: Created AVOID dataset in simulated environment with various weather/time conditions, including semantic/depth maps, LiDAR data, and waypoints. Benchmarked real-time networks and proposed multi-task network for segmentation, depth, and waypoint prediction.

Result: Introduced comprehensive dataset supporting multiple perception tasks with benchmark results on real-time obstacle detection networks.

Conclusion: AVOID dataset addresses limitations of existing datasets and enables better evaluation of obstacle detection under adverse conditions for autonomous driving.
...
TL;DR: MM-UAVBench is a comprehensive benchmark for evaluating Multimodal Large Language Models in low-altitude UAV scenarios across perception, cognition, and planning capabilities.


<details>
  <summary>Details</summary>
Motivation: Current MLLM benchmarks don't cover low-altitude UAV challenges, and existing UAV evaluations focus on specific tasks rather than evaluating MLLMs' general intelligence in these scenarios.

Method: Created MM-UAVBench with 19 sub-tasks and over 5.7K manually annotated questions derived from real-world UAV data from public datasets. Evaluated 16 open-source and proprietary MLLMs across perception, cognition, and planning dimensions.
...
Conclusion: MM-UAVBench fills an important gap in evaluating MLLMs for UAV applications and should foster research on more robust and reliable MLLMs for real-world UAV intelligence.
...
TL;DR: Holi-DETR: A transformer-based fashion item detector that uses holistic contextual information (co-occurrence, spatial arrangements, and body keypoints) to reduce ambiguities in fashion item detection.
...
Method: Proposes Holi-DETR, a novel architecture that integrates three types of contextual information into DETR framework: (1) co-occurrence relationships between fashion items, (2) relative position/size based on inter-item spatial arrangements, and (3) spatial relationships between items and human body keypoints.
...
Conclusion: Holistic detection leveraging contextual information significantly improves fashion item detection performance by reducing ambiguities through co-occurrence, spatial, and body-aware relationships.
...
TL;DR: UniMAGE is a unified director model that combines script drafting and key-shot design in a single framework using Mixture-of-Transformers architecture, enabling non-experts to create long-context, multi-shot films.
...
Conclusion: UniMAGE successfully bridges user prompts with well-structured scripts, empowering non-experts to produce long-context, multi-shot films by unifying script drafting and key-shot design in a single director model.
...
TL;DR: Proposes a novel framework combining pre-trained text-guided image translation and image retrieval to efficiently generate realistic synthetic defect images for anomaly detection, with a two-stage training strategy to maximize performance while minimizing costs.


<details>
  <summary>Details</summary>
Motivation: Addresses the trade-off in synthetic defect image generation for anomaly detection: rule-based methods are cheap but unrealistic, while generative models are realistic but expensive. Aims to develop a cost-effective solution that produces high-quality synthetic defects.

Method: Uses pre-trained text-guided image-to-image translation model to generate defect images, combined with image retrieval model to filter out irrelevant outputs by assessing similarity to real normal images. Implements two-stage training: pre-training on large volume of rule-based synthetic images, then fine-tuning on smaller set of high-quality generated images.

Result: Experiments on MVTec AD dataset demonstrate the effectiveness of the approach in improving anomaly detection performance while significantly reducing data collection costs.

Conclusion: The proposed framework successfully balances cost-effectiveness and realism in synthetic defect generation, enabling better anomaly detection performance with reduced resource requirements compared to existing methods.
...
TL;DR: SGPS uses SURE gradient updates and PCA noise estimation to correct diffusion sampling errors, enabling high-quality inverse problem reconstruction in under 100 NFEs.
...
TL;DR: PEG-DRNet is a physics-edge hybrid network for infrared gas leak detection that combines gas transport modeling, edge enhancement, and adaptive routing to improve detection of faint, small, semitransparent plumes with weak boundaries.
...
Method: Three key components: 1) Gas Block with diffusion-convection modeling for local and long-range gas transport; 2) AGPEO edge operator with multi-scale edge perception for boundary reinforcement; 3) CASR-PAN with adaptive routing for selective feature propagation across scales.
...
<details>
  <summary>Details</summary>
Motivation: Existing remote sensing diffusion foundation models rely on large datasets with redundancy, noise, and class imbalance, which reduce training efficiency and prevent convergence. Current approaches either aggregate multiple datasets or use simplistic deduplication, ignoring the specific distributional needs of generative modeling and RS imagery heterogeneity.
...
TL;DR: Proposes a Vision-language Model framework with Dynamic Resolution Input Strategy and Multi-scale Vision-language Alignment Mechanism for multimodal remote sensing fusion, improving both accuracy and efficiency.
...
Result: Significant improvements in semantic understanding accuracy and computational efficiency on RS-GPT4V dataset. Superior performance in image captioning (BLEU-4, CIDEr metrics) and cross-modal retrieval (R@10 metric) compared to conventional methods.

Conclusion: Provides novel approach for efficient and robust multimodal remote sensing systems, laying theoretical foundation and offering technical guidance for intelligent remote sensing interpretation engineering applications.
...
TL;DR: ViLaCD-R1 is a two-stage VLM-based framework for remote sensing change detection that uses a Multi-Image Reasoner trained with SFT+RL and a Mask-Guided Decoder to achieve precise semantic change localization while suppressing non-semantic variations.


<details>
  <summary>Details</summary>
Motivation: Traditional pixel-based and encoder-decoder approaches inadequately capture high-level semantics and are vulnerable to non-semantic perturbations. Recent VLM-based methods still suffer from inaccurate spatial localization, imprecise boundary delineation, and limited interpretability.

Method: Two-stage framework: 1) Multi-Image Reasoner (MIR) - VLM trained via supervised fine-tuning and reinforcement learning on block-level dual-temporal inference tasks, taking image patches as input and outputting coarse change masks; 2) Mask-Guided Decoder (MGD) - integrates dual-temporal image features with coarse masks to predict precise binary change maps.
...
Conclusion: ViLaCD-R1 effectively addresses limitations of existing methods by combining VLM-based semantic reasoning with precise mask-guided decoding, achieving superior performance in remote sensing change detection tasks.
...
TL;DR: ASemconsist: A novel framework for generating image sequences with consistent character identity across diverse scenes using selective text embedding modification and semantic control strategies.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image diffusion models struggle to maintain consistent character identity across multiple images while preserving per-image prompt alignment, creating a trade-off between identity consistency and scene-specific text alignment.
...
Result: Achieves state-of-the-art performance, effectively overcoming prior trade-offs between identity consistency and prompt alignment.

Conclusion: ASemconsist provides a comprehensive solution for consistent character generation across diverse scenes, with the proposed Consistency Quality Score (CQS) offering a unified evaluation protocol for this challenging task.
...
TL;DR: Proposes Contour Information-Aware 2D Gaussian Splatting that incorporates object segmentation priors to preserve edge structures under high compression, preventing cross-boundary blending.


<details>
  <summary>Details</summary>
Motivation: Existing 2D Gaussian Splatting approaches produce blurry or indistinct boundaries when using few Gaussians due to lack of contour awareness, limiting their effectiveness in high compression scenarios.

Method: Incorporates object segmentation priors into Gaussian-based image representation, constraining each Gaussian to specific segmentation regions during rasterization to prevent cross-boundary blending. Also introduces a warm-up scheme to stabilize training and improve convergence.

Result: Achieves higher reconstruction quality around object edges compared to existing 2DGS methods, particularly evident with very few Gaussians. Maintains fast rendering and low memory usage.

Conclusion: The proposed contour-aware 2DGS framework effectively preserves edge structures under high compression while maintaining the efficiency advantages of Gaussian splatting representations.
...
TL;DR: CEM is a fidelity-optimization plugin that minimizes caching errors in Diffusion Transformers by using dynamic programming to optimize caching strategies based on error sensitivity across timesteps.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiT) have slow inference due to iterative denoising, and existing caching-based acceleration methods suffer from computational errors that limit their effectiveness. Current error correction strategies use fixed caching approaches that don't adapt to complex error variations during denoising.

Method: CEM predefines error metrics to characterize model sensitivity to acceleration based on timesteps and cache intervals. It uses a dynamic programming algorithm with cumulative error approximation to optimize caching strategies, minimizing caching errors while being adaptable to arbitrary acceleration budgets.

Result: CEM significantly improves generation fidelity of existing acceleration models across nine generation models and quantized methods in three tasks. It outperforms original generation performance on FLUX.1-dev, PixArt-α, StableDiffusion1.5 and Hunyuan models.

Conclusion: CEM is a model-agnostic, training-free plugin that can be seamlessly integrated into existing error correction frameworks and quantized models without additional computational overhead, offering substantial fidelity improvements for Diffusion Transformer acceleration.
...
Conclusion: YOLO-Master successfully addresses the limitations of static computation in RTOD through adaptive resource allocation, achieving superior accuracy-speed trade-off, especially for complex scenes, while maintaining real-time performance.
...
TL;DR: This paper presents multimodal frameworks for micro-gesture recognition and behavior-based emotion prediction using RGB video and skeletal pose data, achieving 2nd place in the MiGA 2025 Challenge emotion prediction task.


<details>
  <summary>Details</summary>
Motivation: Micro-gesture recognition and behavior-based emotion prediction are challenging tasks requiring modeling of subtle, fine-grained human behaviors from video and skeletal pose data. The authors aim to develop effective multimodal approaches to tackle both problems on the iMiGUE dataset.

Method: Two multimodal frameworks: 1) For micro-gesture classification, uses MViTv2-S for video embeddings and 2s-AGCN for skeletal embeddings, integrated via Cross-Modal Token Fusion. 2) For emotion recognition, uses SwinFace for facial embeddings and MViTv2-S for contextual embeddings, fused through an InterFusion module.

Result: Experiments on iMiGUE dataset within MiGA 2025 Challenge demonstrate robust performance, with the approach securing 2nd place in the behavior-based emotion prediction task.

Conclusion: The proposed multimodal frameworks effectively capture nuanced spatio-temporal patterns for micro-gesture recognition and integrate facial and contextual information for emotion prediction, showing strong performance on challenging fine-grained behavior analysis tasks.
...
TL;DR: MedGemma outperforms GPT-4 in medical imaging diagnosis, achieving 80.37% accuracy vs 69.58% after domain-specific fine-tuning with LoRA, showing superior sensitivity for critical conditions like cancer and pneumonia.


<details>
  <summary>Details</summary>
Motivation: To compare specialized open-source medical AI (MedGemma) against proprietary multimodal LLMs (GPT-4) for disease classification from medical scans, addressing the need for reliable clinical implementation with minimal hallucinations.

Method: Critical comparison between two AI architectures: fine-tuned MedGemma-4b-it using Low-Rank Adaptation (LoRA) versus untuned GPT-4, evaluated on diagnosing six different diseases from medical imaging data with quantitative analysis via confusion matrices and classification reports.

Result: MedGemma achieved significantly higher mean test accuracy (80.37%) compared to GPT-4 (69.58%), with notably superior sensitivity for high-stakes clinical tasks like cancer and pneumonia detection, demonstrating the effectiveness of domain-specific fine-tuning.

Conclusion: Domain-specific fine-tuning is essential for minimizing hallucinations in clinical AI applications, positioning specialized models like MedGemma as sophisticated tools for complex, evidence-based medical reasoning compared to general-purpose multimodal LLMs.
...
<details>
  <summary>Details</summary>
Motivation: Traditional CAD modeling is complex, and existing sketch-to-3D methods produce non-editable, approximate models that don't meet industrial precision requirements. Text/image-based methods require manual annotation, limiting scalability in industrial settings.

Method: Proposes Heterogeneous Collaborative Multi-Expert Reinforcement Learning (CME-CAD) with two-stage training: Multi-Expert Fine-Tuning (MEFT) and Multi-Expert Reinforcement Learning (MERL). Integrates complementary model strengths for collaborative learning.

Result: Introduces CADExpert benchmark with 17,299 instances including orthographic projections with precise dimensions, expert-generated Chain-of-Thought processes, executable CADQuery code, and rendered 3D models.

Conclusion: CME-CAD improves generation of accurate, constraint-compatible, fully editable CAD models by leveraging collaborative multi-expert learning, addressing industrial design requirements for precision and editability.
...
TL;DR: Visual representation learning requires a fiber bundle structure where semantic states form a quotient space that cannot be learned through smooth deformation alone, requiring discriminative targets and architectural mechanisms supporting topology change.


<details>
  <summary>Details</summary>
Motivation: To understand visual representation learning from a structural and topological perspective, starting from the hypothesis that visual understanding requires a semantic language where many perceptual observations correspond to few discrete semantic states.

Method: Theoretical analysis using fiber bundle structures: visual observation space organized as a fiber bundle where nuisance variation populates fibers and semantics correspond to a quotient base space. Derives theoretical consequences about the nature of semantic invariance and architectural requirements.

Result: Two key theoretical findings: 1) The semantic quotient X/G is not a submanifold of X and cannot be obtained through smooth deformation alone, requiring discriminative targets like supervision, cross-instance identification, or multimodal alignment. 2) Approximating the quotient requires architectural mechanisms supporting topology change through an "expand-and-snap" process.

Conclusion: The framework provides a topological lens that aligns with empirical regularities in large-scale discriminative and multimodal models, offering interpretive rather than prescriptive insights into why certain learning approaches work.
...
TL;DR: CountGD++ introduces enhanced prompt flexibility for object counting by allowing specification of what NOT to count, automating visual example annotation with pseudo-exemplars, and accepting both natural/synthetic external images, significantly improving counting accuracy and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing object counting methods have limitations in how objects can be specified: they require manual annotation of visual examples, don't allow specifying what not to count, and lack flexibility in prompt design, restricting practical applications.

Method: Extends counting prompts to include negative specifications (what not to count) via text/visual examples; introduces pseudo-exemplars for automated visual example annotation at inference; enables models to accept visual examples from both natural and synthetic external images; integrates CountGD++ as a vision expert agent for LLMs.

Result: Significant improvements in accuracy, efficiency, and generalization across multiple datasets; expands prompt flexibility for multi-modal open-world counting; code publicly available.

Conclusion: The novel capabilities introduced in CountGD++ address key limitations in object counting methods, providing more flexible and accurate counting through enhanced prompt design, automation of visual examples, and integration with LLMs as vision expert agents.
...
TL;DR: SpatialMosaic: A scalable pipeline for generating 2M QA pairs to train VLMs for 3D spatial reasoning from multi-view images, addressing real-world challenges like occlusion and partial visibility.


<details>
  <summary>Details</summary>
Motivation: Existing MLLMs for 3D scene understanding rely on pre-constructed 3D representations or reconstruction pipelines, limiting scalability and real-world applicability. Real-world challenges like partial visibility, occlusion, and low-overlap conditions requiring reasoning from fragmented visual cues remain under-explored.

Method: 1) Proposed scalable multi-view data generation and annotation pipeline to construct realistic spatial reasoning QAs, creating SpatialMosaic dataset (2M QA pairs). 2) Created SpatialMosaic-Bench benchmark (1M QA pairs across 6 tasks) for evaluating multi-view spatial reasoning. 3) Developed SpatialMosaicVLM, a hybrid framework integrating 3D reconstruction models as geometry encoders within VLMs.
...
Conclusion: The work addresses key limitations in 3D scene understanding by providing a scalable approach to train VLMs for spatial reasoning directly from multi-view images, handling real-world challenges like occlusion and partial visibility without requiring explicit 3D reconstructions.
...
<details>
  <summary>Details</summary>
Motivation: Existing two-view correspondence methods have limitations in local geometric modeling and cross-stage information optimization, reducing their robustness in capturing geometric constraints for matching tasks.

Method: Proposes Multi-Graph Contextual Attention Network (MGCA-Net) with two modules: Contextual Geometric Attention (CGA) for adaptive integration of spatial and feature information, and Cross-Stage Multi-Graph Consensus (CSMGC) for maintaining geometric consistency across stages via sparse graph networks.
...
Conclusion: The proposed MGCA-Net effectively addresses geometric modeling limitations in two-view correspondence learning through contextual attention and cross-stage consensus, achieving superior performance in practical vision applications.
...
TL;DR: NeXT-IMDL is a diagnostic benchmark that exposes the fragility of current image manipulation detection models by systematically testing their generalization across four fundamental axes of AI-generated content manipulation.


<details>
  <summary>Details</summary>
Motivation: Current IMDL research uses oversimplified cross-dataset evaluation that conceals model fragility with diverse AI-generated content, creating misleading impressions of progress despite urgent need for robust detection methods.

Method: NeXT-IMDL categorizes AIGC manipulations along four axes (editing models, manipulation types, content semantics, forgery granularity) and implements five rigorous cross-dimension evaluation protocols to systematically probe generalization boundaries.

Result: Experiments on 11 representative models reveal systemic failures and significant performance degradation under the designed protocols, showing models perform well in original settings but fail in real-world generalization scenarios.

Conclusion: The benchmark provides a diagnostic toolkit to advance development of truly robust, next-generation IMDL models by exposing current limitations and enabling more realistic evaluation of generalization capabilities.
...
Method: Self-correcting Bidirectional Distillation strategy retains bidirectional attention within video chunks, Multi-step Retrospective Self-Correction Mechanism for error recovery, and full-stack inference acceleration with hybrid sequence parallelism, Parallel VAE, and kernel-level optimizations.

Result: First 14B-scale system achieving sub-second start-up latency (0.87s) and real-time throughput of 32 FPS, setting new standard for high-fidelity interactive digital human synthesis.
...
TL;DR: SOFTooth: A 2D-3D fusion framework for 3D tooth instance segmentation that leverages frozen SAM embeddings without 2D mask supervision, addressing challenges like boundary leakage, center drift, and inconsistent tooth identities.


<details>
  <summary>Details</summary>
Motivation: 3D tooth segmentation faces challenges from crowded arches, ambiguous boundaries, missing teeth, and rare third molars. Native 3D methods suffer from boundary leakage and center drift, while 2D foundation models like SAM can't be directly applied in clinical 3D workflows.

Method: 1) Point-wise residual gating module injects occlusal-view SAM embeddings into 3D point features to refine boundaries. 2) Center-guided mask refinement regularizes consistency between masks and centroids. 3) Order-aware Hungarian matching integrates anatomical tooth order and center distance for coherent labeling.
...
Conclusion: Rich 2D semantics from foundation models can be effectively transferred to 3D tooth instance segmentation without 2D fine-tuning, addressing key challenges in dental segmentation through a semantics-enhanced, order-aware 2D-3D fusion approach.
...
TL;DR: ArtQuant: A novel aesthetic assessment framework using large-scale RAD dataset and LLM decoders to address data scarcity and model fragmentation in AIGC evaluation.


<details>
  <summary>Details</summary>
Motivation: Current aesthetic quality assessment faces two challenges: (1) data scarcity and imbalance in existing datasets that focus only on visual perception, and (2) model fragmentation where current methods isolate aesthetic attributes or struggle with long-form textual descriptions.

Method: Proposes two solutions: (1) RAD dataset - a large-scale (70k), multi-dimensional structured dataset generated via iterative pipeline without heavy annotation costs, and (2) ArtQuant framework - uses LLM decoders to couple isolated aesthetic dimensions through joint description generation and better model long-text semantics.

Result: Achieves state-of-the-art performance on several datasets while requiring only 33% of conventional training epochs, narrowing the cognitive gap between artistic images and aesthetic judgment.

Conclusion: The proposed approach effectively addresses fundamental challenges in aesthetic assessment through data (RAD) and model (ArtQuant) innovations, with theoretical analysis confirming the symbiosis minimizes prediction entropy. Code and dataset will be released.
...
<details>
  <summary>Details</summary>
Motivation: Current autonomous driving approaches treat world models and motion planning as separate processes, creating a gap between scenario forecasting and trajectory planning. The authors aim to bridge this gap by creating inherent consistency between high-fidelity future generation and reliable trajectory planning.

Method: DriveLaW consists of two core components: DriveLaW-Video (a powerful world model for high-fidelity forecasting with expressive latent representations) and DriveLaW-Act (a diffusion planner that generates consistent trajectories from DriveLaW-Video's latent representations). Both components are optimized using a three-stage progressive training strategy.
...
Conclusion: The unified paradigm of DriveLaW successfully bridges the gap between world modeling and motion planning in autonomous driving, demonstrating that direct injection of video generator latents into planners ensures inherent consistency and superior performance across both tasks.
...
TL;DR: DDSPO is a new preference optimization method for diffusion models that uses per-timestep supervision from winning/losing policies instead of relying on costly human-labeled datasets.


<details>
  <summary>Details</summary>
Motivation: Diffusion models struggle with aligning outputs to nuanced user intent and maintaining consistent aesthetic quality. Existing preference-based methods like DDPO rely on expensive and potentially noisy human-labeled datasets.

Method: DDSPO directly derives per-timestep supervision from winning and losing policies when available. It avoids labeled data by automatically generating preference signals using a pretrained reference model, contrasting outputs conditioned on original prompts vs semantically degraded variants.
...
Conclusion: DDSPO provides an effective score-space preference supervision approach without explicit reward modeling or manual annotations, offering dense transition-level signals across the denoising trajectory.
...
TL;DR: This paper introduces UncertSAM, a benchmark to evaluate uncertainty quantification methods for SAM segmentation models under challenging conditions like shadows and camouflage, finding that Laplace approximation provides meaningful uncertainty signals.


<details>
  <summary>Details</summary>
Motivation: Foundation segmentation models like SAM show strong zero-shot performance but struggle in shifted or limited-knowledge domains. The authors investigate whether uncertainty quantification can improve model generalizability in a domain-agnostic way.

Method: (1) Created UncertSAM benchmark with 8 datasets designed to stress-test SAM under challenging segmentation conditions (shadows, transparency, camouflage), (2) evaluated lightweight post-hoc uncertainty estimation methods, and (3) assessed preliminary uncertainty-guided prediction refinement.

Result: Among evaluated approaches, last-layer Laplace approximation yields uncertainty estimates that correlate well with segmentation errors, indicating meaningful signal. While refinement benefits are preliminary, uncertainty shows potential for improving robust, domain-agnostic performance.

Conclusion: Uncertainty quantification can enhance segmentation model robustness in challenging domains. The UncertSAM benchmark and code are publicly released to support further research in this area.
...
TL;DR: Real-time road surface monitoring system using mobile phone camera data and acceleration sensors with deep learning classification achieving over 95% accuracy for 5 road condition classes, enhanced with fuzzy logic for weather/time-of-day classification.
...
Method: Collected data using mobile phone camera on roads around Karlsruhe Institute of Technology campus. Tested multiple deep learning algorithms (AlexNet, LeNet, VGG, ResNet) for road classification. Used both road image data and acceleration data (converted to images) for training. Implemented fuzzy logic for weather and time-of-day classification using acceleration or camera images.

Result: Achieved over 95% accuracy for 5-class road condition classification: asphalt, damaged asphalt, gravel road, damaged gravel road, pavement road. Compared performance of acceleration-based vs. camera image-based approaches.

Conclusion: Proposed system provides real-time, systematic road monitoring using mobile sensors and deep learning, achieving high accuracy. The integration of fuzzy logic enables weather and time-of-day classification, making it practical for real-world vehicle control applications.
...
TL;DR: RealX3D is a real-capture benchmark for multi-view visual restoration and 3D reconstruction under diverse physical degradations, grouping corruptions into four families with multiple severity levels.


<details>
  <summary>Details</summary>
Motivation: Current multi-view reconstruction pipelines are fragile in real-world challenging environments with physical degradations, but existing benchmarks lack comprehensive real-capture data with diverse physical corruptions.

Method: Created a unified acquisition protocol capturing pixel-aligned LQ/GT views with four corruption families (illumination, scattering, occlusion, blurring) at multiple severity levels, including high-resolution capture, RAW images, and dense laser scans to derive world-scale meshes and metric depth.

Result: Benchmarking shows substantial degradation in reconstruction quality under physical corruptions for both optimization-based and feed-forward methods, highlighting the fragility of current multi-view pipelines in challenging real-world environments.

Conclusion: RealX3D provides a comprehensive real-capture benchmark that reveals significant weaknesses in current multi-view reconstruction methods when facing physical degradations, emphasizing the need for more robust approaches for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: LVLMs tend to produce hallucinated content inconsistent with visual inputs, limiting their reliability in real-world applications. There's a need for methods to mitigate hallucinations without requiring additional training.

Method: 1) Generates two intermediate textual responses conditioned on coarse- and fine-grained views of images (inspired by human visual processing). 2) Transforms responses into synthetic images using text-to-image model to create multi-level visual hypotheses. 3) Uses Wasserstein-based fusion mechanism to align predictive distributions into geometrically consistent decoding trajectory.

Result: Extensive experiments on six hallucination-focused benchmarks show CoFi-Dec substantially reduces both entity-level and semantic-level hallucinations, outperforming existing decoding strategies.

Conclusion: CoFi-Dec is an effective, model-agnostic framework that requires no additional training and can be seamlessly applied to various LVLMs, improving reliability by reducing hallucinated content through principled multi-level visual grounding.
...
TL;DR: Hybrid framework combining vision-based waterline detection, YOLOv8 scale extraction, and multimodal LLMs (GPT-4o, Gemini 2.0) for automated river gauge reading with high accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional hydrological monitoring methods suffer from manual measurement errors and environmental constraints, requiring automated, accurate, and continuous river water level monitoring for flood forecasting and water resource management.

Method: Sequential pipeline: image preprocessing → annotation → waterline detection → YOLOv8 pose scale extraction → scale gap estimation → numeric reading extraction using multimodal LLMs (GPT-4o, Gemini 2.0 Flash) with geometric metadata.

Result: Waterline detection: 94.24% precision, 83.64% F1 score. Scale gap detection provided accurate geometric calibration. Gemini Stage 2 achieved best performance: MAE 5.43 cm, RMSE 8.58 cm, R² 0.84 under optimal conditions. Performance degraded with poor image quality.

Conclusion: Combining geometric metadata with multimodal AI enables robust, scalable automated hydrological monitoring. The approach shows potential for real-time river gauge digitization and improved water resource management.
...
TL;DR: A novel denoising Brownian bridge model with dual approximators (Dual-approx Bridge) for deterministic image-to-image translation that guarantees consistent outputs with high fidelity to ground truth.


<details>
  <summary>Details</summary>
Motivation: To address the need for deterministic image-to-image translation that produces consistent, predictable outputs with high fidelity to ground truth, unlike stochastic methods that introduce variance.

Method: Proposes Dual-approx Bridge model using Brownian bridge dynamics with two neural network approximators - one for forward process and one for reverse process - to achieve faithful output with negligible variance.
...
Conclusion: The Dual-approx Bridge model effectively enables deterministic image-to-image translation with high fidelity and minimal variance, outperforming existing approaches in producing faithful outputs.
...
Conclusion: HY-Motion 1.0 represents a major advancement in motion generation, successfully scaling DiT-based models to billion parameters, and is released open-source to accelerate research and commercial development of 3D human motion generation.
...
<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for point cloud registration rely on Euclidean neighborhood strategies that struggle to capture implicit semantics and structural consistency, limiting feature discriminability and registration performance.

Method: 1) Graph neighborhood aggregation module constructs global graph to capture overall structural relationships; 2) Progressive context interaction module enhances feature discriminability through intra-domain feature decoupling and inter-domain context interaction; 3) Dynamic inlier selection method optimizes inlier weights using residual information from multiple pose estimation iterations.

Result: Achieves state-of-the-art performance with 96.4% registration recall on 3DMatch dataset, outperforming existing methods on both indoor RGB-D and outdoor LiDAR datasets.

Conclusion: MCI-Net effectively addresses limitations of Euclidean neighborhood-based feature extraction by integrating multi-domain contextual cues, leading to more robust and discriminative feature learning for high-quality point cloud registration.
...
TL;DR: SC-Net is a novel CNN-based network for two-view correspondence learning that integrates bilateral context from spatial and channel perspectives to address global context aggregation and motion field oversmoothing issues.


<details>
  <summary>Details</summary>
Motivation: Standard CNN backbones for two-view correspondence learning often fail to effectively aggregate global context and oversmooth dense motion fields in scenes with large disparity, limiting their performance in complex scenarios.

Method: SC-Net integrates bilateral context through three key modules: 1) Adaptive Focused Regularization (AFR) for position-awareness and robustness against spurious motion, 2) Bilateral Field Adjustment (BFA) to refine motion fields by modeling long-range relationships across spatial/channel dimensions, and 3) Position-Aware Recovery (PAR) for consistent and precise motion vector recovery.

Result: SC-Net outperforms state-of-the-art methods in relative pose estimation and outlier removal tasks on YFCC100M and SUN3D datasets, demonstrating superior performance in handling complex correspondence scenarios.

Conclusion: The proposed SC-Net effectively addresses limitations of standard CNN backbones in two-view correspondence learning by integrating bilateral context from spatial and channel perspectives, leading to improved performance in challenging computer vision tasks.
...
TL;DR: TV-RAG is a training-free architecture that improves long-video reasoning for Large Video Language Models by combining temporal alignment with entropy-guided semantics, addressing limitations in temporal windows and fine-grained semantic shifts.


<details>
  <summary>Details</summary>
Motivation: Current LVLMs struggle with long videos due to narrow temporal windows and inability to detect fine-grained semantic shifts over extended durations. Existing text-based retrieval pipelines ignore temporal interdependence among visual, audio, and subtitle channels.

Method: TV-RAG introduces two main mechanisms: 1) a time-decay retrieval module that injects explicit temporal offsets into similarity computation to rank text queries by true multimedia context, and 2) an entropy-weighted key-frame sampler that selects evenly spaced, information-dense frames to reduce redundancy while preserving representativeness.

Result: TV-RAG consistently surpasses most leading baselines across established long-video benchmarks including Video-MME, MLVU, and LongVideoBench, confirming the effectiveness of the approach.

Conclusion: TV-RAG provides a lightweight, budget-friendly upgrade path for any LVLM without requiring re-training or fine-tuning, offering a dual-level reasoning routine that weaves temporal and semantic signals together for improved long-video understanding.
...
TL;DR: PanCAN introduces a novel deep network that hierarchically integrates multi-order geometric contexts through cross-scale feature aggregation in Hilbert space, significantly improving multi-label image classification performance.
...
Method: PanCAN hierarchically integrates multi-order geometric contexts through cross-scale feature aggregation in high-dimensional Hilbert space. It learns multi-order neighborhood relationships at each scale using random walks with attention, cascades modules from different scales, selects salient anchors at finer scales, and dynamically fuses neighborhood features via attention.
...
Conclusion: PanCAN effectively enables cross-scale modeling that significantly enhances complex scene understanding by combining multi-order and cross-scale context-aware features, substantially improving multi-label classification performance.
...
TL;DR: IdentityStory is a framework for generating human-centric stories with consistent character identities across multiple images, addressing challenges in face consistency and multi-character coordination.


<details>
  <summary>Details</summary>
Motivation: Existing visual generative models can create stories with consistent characters from text, but human-centric story generation faces additional challenges: maintaining detailed and diverse human face consistency, and coordinating multiple characters across different images in a sequence.

Method: The framework features two key components: 1) Iterative Identity Discovery - extracts cohesive character identities, and 2) Re-denoising Identity Injection - re-denoises images to inject identities while preserving desired context. The approach tames identity-preserving generators.

Result: Experiments on the ConsiStory-Human benchmark show that IdentityStory outperforms existing methods, particularly in face consistency, and supports multi-character combinations. The framework demonstrates strong potential for applications like infinite-length story generation and dynamic character composition.

Conclusion: IdentityStory presents an effective framework for human-centric story generation that ensures consistent character identity across sequential images, addressing key challenges in face consistency and multi-character coordination while showing promising applications for extended storytelling.
...
Method: IAFS (Iterative Diffusion Inference-Time Scaling with Adaptive Frequency Steering) uses iterative refinement to correct structural deviations progressively, combined with frequency-aware particle fusion that adaptively integrates high-frequency perceptual cues with low-frequency structural information.
...
Conclusion: IAFS provides an effective training-free solution to balance perceptual quality and structural fidelity in diffusion-based super-resolution through iterative refinement and adaptive frequency steering.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-subject customization methods struggle to balance text alignment, subject identity preservation, and layout control simultaneously, while requiring additional training that limits scalability and efficiency.
...
Conclusion: AnyMS provides an effective training-free solution for layout-guided multi-subject customization that successfully balances all three critical objectives while maintaining scalability and efficiency.
...
TL;DR: PathFound is an agentic multimodal model for pathology that uses evidence-seeking inference to improve diagnostic accuracy by mimicking clinical workflows of repeated observation and targeted evidence acquisition.


<details>
  <summary>Details</summary>
Motivation: Current pathological foundation models use static inference where whole-slide images are processed once without reassessment, unlike clinical workflows that refine diagnoses through repeated observations and further examinations.
...
Result: The evidence-seeking strategy consistently improves diagnostic accuracy across several large multimodal models. PathFound achieves state-of-the-art performance across diverse clinical scenarios and demonstrates strong ability to discover subtle pathological details like nuclear features and local invasions.

Conclusion: Evidence-seeking workflows are effective in computational pathology, and PathFound represents a significant advancement in agentic multimodal models for pathological diagnosis that better mimics clinical diagnostic processes.
...
TL;DR: PurifyGen is a training-free, plug-and-play method for safe text-to-image generation that purifies prompts by identifying risky tokens and transforming their embeddings to remove harmful semantics while preserving original intent.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models risk generating unsafe content, and traditional safety methods (text blacklisting, harmful content classification) have drawbacks: they can be circumvented or require extensive datasets and retraining.

Method: Dual-stage prompt purification: 1) Evaluate token safety using complementary semantic distance to measure proximity to toxic/clean concept embeddings; 2) For risky prompts, apply dual-space transformation - project toxic-aligned embeddings into null space of toxic concepts and align them into range space of clean concepts, with token-wise selective replacement of only risky tokens.

Result: PurifyGen surpasses current methods in reducing unsafe content across five datasets and competes well with training-dependent approaches, offering strong generalization to unseen prompts and models.

Conclusion: PurifyGen provides an effective, training-free solution for safe T2I generation that preserves model weights while removing harmful semantics and reinforcing safe ones through theoretically grounded dual-space transformation.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs lack rigorous evaluation on their ability to understand the dense, graphical language of chemical reactions in authentic scientific literature, which is crucial for advancing AI-assisted chemistry research.

Method: Created RxnBench with two tasks: Single-Figure QA (1,525 questions from 305 reaction schemes) for fine-grained visual perception and mechanistic reasoning, and Full-Document QA (108 articles) requiring cross-modal integration of text, schemes, and tables.

Result: MLLMs show critical capability gaps - they excel at text extraction but struggle with deep chemical logic and precise structural recognition. Models with inference-time reasoning outperform standard architectures, but none achieve 50% accuracy on Full-Document QA.

Conclusion: There's an urgent need for domain-specific visual encoders and stronger reasoning engines to advance autonomous AI chemists, as current MLLMs cannot adequately comprehend chemical reactions from scientific literature.
...
Method: ThinkGen uses a decoupled architecture with a pretrained MLLM and Diffusion Transformer (DiT). The MLLM generates tailored instructions based on user intent, and DiT produces images guided by these instructions. A separable GRPO-based training paradigm (SepGRPO) alternates reinforcement learning between MLLM and DiT modules.

Result: Extensive experiments show ThinkGen achieves robust, state-of-the-art performance across multiple generation benchmarks, demonstrating effective CoT reasoning for a wide range of generative scenarios.

Conclusion: ThinkGen successfully extends CoT reasoning to generation tasks through a flexible framework that enables joint training across diverse datasets, overcoming limitations of scenario-specific mechanisms and achieving strong performance.
...
Conclusion: Haar-tSVD provides a computationally simple, parallelizable plug-and-play denoiser that balances denoising speed and performance without requiring learning of local bases.
...
<details>
  <summary>Details</summary>
Motivation: Existing defense methods are reactive and limited in handling emerging multimodal safety risks. Traditional approaches require model adjustments and struggle with out-of-distribution risks. There's a need for proactive safety moderation that works across text, image, and text-image inputs without model modifications.

Method: 1) Constructed a modality-balanced dataset of 87K samples with binary safety labels and hierarchical risk categories to mitigate modality bias. 2) Trained a vision-language base model purely through reinforcement learning for efficient reasoning. 3) Introduced OOD safety category inference task with synonym-bank-based similarity reward to encourage concise descriptions of unseen unsafe categories.

Result: ProGuard achieves performance comparable to closed-source large models on binary safety classification and substantially outperforms existing open-source guard models on unsafe content categorization. Most notably, it improves OOD risk detection by 52.6% and OOD risk description by 64.8%.

Conclusion: ProGuard provides an effective proactive safety moderation solution for multimodal content, addressing the limitations of reactive approaches and demonstrating strong performance in identifying and describing out-of-distribution safety risks without requiring model adjustments.
...
TL;DR: Real-time interactive video diffusion via improved distillation for multimodal conditioning (text, image, audio), achieving 20x faster inference with quality matching full-step baselines, enabling LiveTalk system for seamless human-AI interaction.
...
Method: Improved distillation recipe addressing challenges in Self Forcing approach for multimodal conditioning. Focuses on quality of condition inputs, initialization, and schedule for on-policy optimization. Integrated with audio language models and Anchor-Heavy Identity Sinks technique for long-form video inference.

Result: Distilled model matches visual quality of full-step bidirectional baselines with 20x less inference cost/latency on HDTF, AVSpeech, and CelebV-HQ benchmarks. LiveTalk system outperforms Sora2 and Veo3 in multi-turn video coherence and content quality, reducing response latency from 1-2 minutes to real-time.

Conclusion: The improved distillation enables real-time multimodal interactive video generation, bridging the gap for seamless human-AI interaction through the LiveTalk system, which demonstrates superior performance over state-of-the-art models while achieving real-time responsiveness.
...
<details>
  <summary>Details</summary>
Motivation: Current vision-language models (VLMs) are coarse-grained, exhibit visual biases, and miss subtle visual details because existing training corpora emphasize general recognition over fine-grained perception.

Method: Created TWIN dataset with 561,000 image-pair queries requiring models to determine if two visually similar images depict the same object, encouraging attention to nuanced visual cues. Also introduced FGVQA benchmark suite of 12,000 queries from multiple fine-grained domains to evaluate improvements.

Result: Fine-tuning VLMs on TWIN yields notable gains in fine-grained recognition across unseen domains (art, animals, plants, landmarks), improving by up to 19.3% on FGVQA benchmark without compromising general VQA performance. Dataset scales favorably with object annotations.

Conclusion: TWIN dataset effectively enhances perceptual precision of VLMs and can serve as a drop-in addition to open-source VLM training corpora, with scale being key to performance improvements.
...
TL;DR: This paper proposes a two-stage detection pipeline combining YOLOv11 and EfficientNetV2-B0 to improve night-time fire detection accuracy while reducing false positives from artificial lights, along with Patched-YOLO for better small/distant fire detection in RGB images and an augmented NIR dataset.


<details>
  <summary>Details</summary>
Motivation: Improving fire detection accuracy using infrared night vision cameras is challenging due to dataset limitations and frequent misclassification of bright artificial lights as fire, despite decent performance from existing models like YOLOv7, RT-DETR, and YOLOv9.

Method: Three main contributions: 1) Additional NIR dataset with various data augmentation strategies; 2) Two-stage pipeline combining YOLOv11 and EfficientNetV2-B0 for night-time fire detection; 3) Patched-YOLO for improved fire detection in RGB images, especially for small and distant objects through patch-based processing.

Result: The proposed two-stage approach achieves higher detection accuracy compared to previous methods, particularly for night-time fire detection, while reducing false positives caused by artificial lights. Patched-YOLO enhances detection capability for small and distant fire objects in RGB images.

Conclusion: The paper presents a comprehensive solution addressing data scarcity, false positive reduction, and small object detection challenges in fire detection systems, with promising results for both NIR and RGB-based fire detection applications.
...
TL;DR: SRFA framework achieves 96.23% accuracy for early pancreatic tumor detection using multimodal CT imaging with hybrid ViT-EfficientNet model and metaheuristic optimization.
...
Method: Proposed Scalable Residual Feature Aggregation (SRFA) framework with: 1) MAGRes-UNet for preprocessing and segmentation, 2) DenseNet-121 with residual feature storage for hierarchical feature extraction, 3) Hybrid HHO-BA metaheuristic feature selection, 4) Hybrid Vision Transformer-EfficientNet-B3 classification model, 5) Dual SSA-GWO optimization for hyperparameter tuning.
...
Conclusion: The SRFA framework demonstrates strong potential as a useful instrument for early pancreatic tumor detection, effectively addressing challenges of subtle visual cues and anatomical variations in CT imaging.
...
TL;DR: This paper develops a framework to quantify memorization in 3D generative models and analyzes how data and modeling choices affect memorization, finding that data modality, diversity, conditioning granularity, guidance scale, vector-set length, and rotation augmentation all influence memorization levels.


<details>
  <summary>Details</summary>
Motivation: The motivation is to understand whether 3D generative models memorize training data, which could lead to data leakage and limit output diversity. Quantifying memorization helps prevent these issues and improve generation quality.

Method: The authors design an evaluation framework to measure memorization in 3D generative models. They apply it to existing methods and conduct controlled experiments using a latent vector-set (Vecset) diffusion model to study how data factors (modality, diversity, conditioning) and modeling factors (guidance scale, Vecset length, augmentation) affect memorization.

Result: Results show that memorization depends on data modality and increases with data diversity and finer-grained conditioning. For modeling, memorization peaks at moderate guidance scales and can be reduced with longer Vecsets and simple rotation augmentation, without degrading generation quality.

Conclusion: The framework provides empirical understanding of memorization in 3D generative models and suggests effective strategies to reduce memorization while maintaining generation quality, offering practical guidance for model design and data handling.
...
<details>
  <summary>Details</summary>
Motivation: Existing methods rely on attention mechanisms with simplified motion models (like constant velocity) for cross-frame object alignment, but these approaches are suboptimal due to variations in motion states and object features across categories and frames.

Method: HAT uses multiple explicit motion models to generate spatial anchors and motion-aware feature proposals for historical instances, then performs multi-hypothesis decoding by incorporating semantic and motion cues from cached object queries to provide optimal alignment proposals for target frames.

Result: On nuScenes, HAT consistently improves 3D temporal detectors and trackers, achieving 46.0% AMOTA (state-of-the-art) with DETR3D detector. In E2E AD methods, it enhances perception (+1.3% mAP, +3.1% AMOTA) and reduces collision rate by 32%. It also enables robust perception under corrupted semantics (nuScenes-C).

Conclusion: HAT provides adaptive spatio-temporal alignment that outperforms existing methods by combining multiple motion hypotheses with semantic and motion cues, demonstrating significant improvements in autonomous driving perception and planning, especially under challenging conditions.
...
Method: OmniAgent uses dynamic planning to autonomously orchestrate tool invocation, employing a novel coarse-to-fine audio-guided perception paradigm that leverages audio cues to localize temporal events and guide reasoning.

Result: Extensive evaluations on three audio-video understanding benchmarks show OmniAgent achieves state-of-the-art performance, surpassing leading open-source and proprietary models by 10-20% accuracy margins.

Conclusion: The paper demonstrates a paradigm shift from passive response generation to active multimodal inquiry, enabling more sophisticated audio-visual reasoning through dynamic tool orchestration and audio-guided perception.
...
TL;DR: DKT: A video diffusion model adapted for depth/normal estimation of transparent/reflective objects using synthetic training data, achieving SOTA zero-shot performance on challenging transparency benchmarks.


<details>
  <summary>Details</summary>
Motivation: Transparent/reflective objects break assumptions of traditional depth estimation methods (stereo, ToF, monocular), causing holes and temporal instability. Video diffusion models already synthesize convincing transparent phenomena, suggesting they've internalized optical rules that can be leveraged.

Method: 1) Create TransPhy3D synthetic corpus (11k sequences) with Blender/Cycles rendering of transparent/reflective scenes. 2) Adapt large video diffusion model via lightweight LoRA adapters to learn video-to-video translation for depth/normals. 3) Concatenate RGB and noisy depth latents in DiT backbone during training. 4) Co-train on TransPhy3D and existing synthetic datasets for temporal consistency.

Result: Zero-shot SOTA on transparency benchmarks: ClearPose, DREDS (CatKnown/CatNovel), and TransPhy3D-Test. Improves accuracy and temporal consistency over strong baselines. Normal variant achieves best video normal estimation on ClearPose. 1.3B version runs at ~0.17 s/frame. Integrated into grasping stack, boosts success rates across translucent, reflective, and diffuse surfaces.

Conclusion: "Diffusion knows transparency" - generative video priors can be efficiently repurposed into robust, temporally coherent perception for challenging real-world manipulation without requiring labels. Demonstrates the potential of leveraging internalized knowledge in diffusion models for perception tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current diffusion-based VSR methods achieve good perceptual quality but are impractical for latency-sensitive applications due to reliance on future frames and expensive multi-step denoising.

Method: Proposes a causally conditioned diffusion framework with: 1) four-step distilled denoiser for fast inference, 2) Auto-regressive Temporal Guidance (ARTG) module for motion-aligned cues during latent denoising, and 3) lightweight temporal-aware decoder with Temporal Processor Module (TPM) for enhanced detail and temporal coherence.
...
Conclusion: Stream-DiffVSR achieves the lowest latency reported for diffusion-based VSR, making it the first diffusion VSR method suitable for low-latency online deployment, bridging the gap between perceptual quality and practical efficiency.
