...
TL;DR: AI framework analyzes multiple pathology slides simultaneously to predict biochemical recurrence in prostate cancer patients, outperforming clinical benchmarks and reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Precise prediction of biochemical recurrence after radical prostatectomy is challenging due to prostate cancer's multifocality and tumor distribution throughout the gland.

Method: Novel AI framework that simultaneously processes multi-section pathology slides to capture comprehensive tumor landscape across entire prostate gland, using patch and slide sub-sampling strategies to reduce computational costs.

Result: Strong predictive performance for 1- and 2-year BCR prediction, substantially outperforming clinical benchmarks; AI-derived risk score validated as most potent independent prognostic factor; computational costs reduced without compromising performance; generalizability confirmed through external validation.
...
TL;DR: Transformer VLMs have depth redundancy; domain-aware activation similarity identifies which decoder layers to prune for math vs. non-math tasks, revealing three pruning regimes with different sensitivity patterns.


<details>
  <summary>Details</summary>
Motivation: Transformer-based vision-language models contain substantial depth redundancy, but the effect of removing specific decoder layers remains poorly understood, especially for domains requiring tight coupling between perception and multi-step reasoning like mathematics.

Method: Study structured decoder layer pruning through domain-aware activation similarity, measuring how strongly each layer transforms representations for math versus non-math inputs. Create math-aware, non-math-aware, and mixed ranking criteria that identify layers whose input-output activations change least within target domains.

Result: Across two state-of-the-art VLMs and math/general multimodal benchmarks, uncover consistent three-regime structure: low pruning budgets are highly sensitive to which layers are removed; moderate budgets converge as structural damage accumulates; high budgets favor spacing-aware strategies. Domain-aware rankings achieve strongest stability in ranking-sensitive regime while matching/exceeding structure-aware baselines at larger budgets.
...
TL;DR: Mix-and-Match Pruning is a globally guided, layer-wise sparsification framework that uses sensitivity scores and architectural rules to generate diverse pruning configurations, eliminating repeated runs while providing deployment-ready accuracy-sparsity trade-offs.


<details>
  <summary>Details</summary>
Motivation: Deploying DNNs on edge devices requires strong compression with minimal accuracy loss. Different layers and architectures respond differently to pruning, making single-strategy approaches suboptimal.

Method: Uses sensitivity scores (magnitude, gradient, or combination) and architectural rules to derive architecture-aware sparsity ranges. Preserves normalization layers while pruning classifiers more aggressively. Systematically samples these ranges to produce ten strategies per sensitivity signal.

Result: Achieves Pareto-optimal results on CNNs and Vision Transformers. Reduces accuracy degradation on Swin-Tiny by 40% relative to standard single-criterion pruning.
...
TL;DR: STAC is a cache compression framework for streaming 3D reconstruction that reduces memory usage 10x and speeds inference 4x while maintaining reconstruction quality through spatio-temporal token compression.


<details>
  <summary>Details</summary>
Motivation: Current causal VGGT transformers for online 3D reconstruction suffer from linear memory growth with stream length due to key-value cache accumulation. Under limited memory budgets, early cache eviction degrades reconstruction quality and temporal consistency.

Method: STAC uses three components: 1) Working Temporal Token Caching with decayed cumulative attention scores to preserve long-term informative tokens, 2) Long-term Spatial Token Caching that compresses spatially redundant tokens into voxel-aligned representations, and 3) Chunk-based Multi-frame Optimization for joint processing of consecutive frames to improve temporal coherence and GPU efficiency.

Result: STAC achieves state-of-the-art reconstruction quality while reducing memory consumption by nearly 10x and accelerating inference by 4x compared to baseline methods.

Conclusion: STAC substantially improves the scalability of real-time 3D reconstruction in streaming settings by effectively compressing the KV cache while maintaining reconstruction quality and temporal consistency.
...
TL;DR: Two efficient visual anomaly detection methods (PatchCore-Lite and Padim-Lite) designed for edge deployment achieve significant memory and computational efficiency gains while maintaining performance on industrial inspection benchmarks.


<details>
  <summary>Details</summary>
Motivation: Real production lines require VAD systems that meet strict real-time and privacy requirements, necessitating edge deployment. However, edge devices have limited memory and computational resources, creating challenges for existing VAD methods.

Method: PatchCore-Lite uses product quantization for memory bank compression with coarse search followed by exact search on decoded subsets. Padim-Lite employs diagonal covariance to simplify Mahalanobis distance into element-wise computation.

Result: PatchCore-Lite achieves 79% reduction in total memory footprint. Padim-Lite achieves 77% reduction in total memory and 31% decrease in inference time, while maintaining performance on MVTec AD and VisA benchmarks.

Conclusion: VAD can be effectively deployed on edge devices, enabling real-time, private, and cost-efficient industrial inspection through efficient methods that overcome edge hardware limitations.
...
TL;DR: Comprehensive survey of remote sensing image dehazing covering methodological evolution, benchmark assessment, and physical consistency analysis with open-source resources.


<details>
  <summary>Details</summary>
Motivation: Remote sensing images are frequently degraded by haze, fog, and thin clouds, which obscure surface reflectance and hinder downstream applications, necessitating a systematic survey to guide future research.

Method: Categorizes approaches into three-stage progression: handcrafted physical priors → data-driven deep restoration → hybrid physical-intelligent generation. Analyzes 30+ methods across CNNs, GANs, Transformers, and diffusion models. Conducts large-scale experiments on 5 datasets using 12 metrics.

Result: Transformer- and diffusion-based models improve SSIM by 12%~18% and reduce perceptual errors by 20%~35% on average. Hybrid physics-guided designs achieve higher radiometric stability. Models with explicit transmission/airlight constraints reduce color bias by up to 27%.

Conclusion: Identifies open challenges: dynamic atmospheric modeling, multimodal fusion, lightweight deployment, data scarcity, and joint degradations. Outlines research directions for trustworthy, controllable, and efficient (TCE) dehazing systems with all resources publicly available.
...
TL;DR: Proposes a visual-tactile fusion framework for transparent fragment contour estimation, including dataset creation, grasping detection, material classification, and reassembly algorithms.


<details>
  <summary>Details</summary>
Motivation: Transparent fragment contour estimation is crucial for autonomous reassembly in precision optical instrument repair, cultural relic restoration, and precious device identification, but faces challenges due to strict optical properties and irregular shapes.

Method: 1) Creates TransFrag27K dataset with synthetic transparent fragments; 2) Develops TransFragNet for visual grasping position detection; 3) Uses Gelsight Mini sensors for tactile edge reconstruction; 4) Fuses visual-tactile data for material classification; 5) Implements contour estimation framework inspired by human vision-touch integration; 6) Proposes multi-dimensional similarity metrics for contour matching and reassembly.

Result: The framework demonstrates strong performance in real-world validation, with experimental results showing validity. Dataset and code are publicly available.

Conclusion: The proposed visual-tactile fusion approach effectively addresses transparent fragment contour estimation challenges and provides a reproducible benchmark for fragment reassembly applications.
...
TL;DR: Proposes teacher-based knowledge retention with mask-based partial category distillation for incremental hyperspectral image classification, reducing catastrophic forgetting without needing old category samples.


<details>
  <summary>Details</summary>
Motivation: Address catastrophic forgetting in incremental hyperspectral image classification where memory recall methods are limited by their dependence on old category samples, which may not always be available.

Method: Teacher-based knowledge retention method that uses incremental category samples instead of old ones, combined with mask-based partial category knowledge distillation that decouples and filters potentially misleading information to prevent student model misguidance.

Result: Comparative and ablation experiments demonstrate robust performance, showing the method effectively mitigates catastrophic forgetting and enhances overall accuracy in incremental classification tasks.

Conclusion: The proposed approach successfully addresses catastrophic forgetting in incremental hyperspectral image classification by leveraging teacher knowledge and selective distillation, achieving improved performance without reliance on old category samples.
...
TL;DR: InjectFlow is a training-free method that fixes bias in flow matching models by injecting orthogonal semantics during initial velocity computation, preventing trajectory lock-in toward majority modes while maintaining quality.


<details>
  <summary>Details</summary>
Motivation: Flow Matching models are highly sensitive to dataset biases, causing severe semantic degradation when generating out-of-distribution or minority-class samples due to conditional expectation smoothing that leads to trajectory lock-in during inference.

Method: Introduces InjectFlow, a novel training-free method that injects orthogonal semantics during the initial velocity field computation without changing random seeds, preventing latent drift toward majority modes while maintaining generative quality.

Result: Extensive experiments show InjectFlow successfully fixes 75% of prompts that standard flow matching models fail to generate correctly on the GenEval dataset, providing effective bias mitigation.

Conclusion: The theoretical analysis and InjectFlow algorithm provide a ready-to-use solution for building more fair and robust visual foundation models by addressing bias manifold issues in flow matching frameworks.
...
TL;DR: DiffMark is a plug-and-play watermarking method for diffusion models that enables single-pass multi-bit detection, per-image key flexibility, and cross-model transferability without model retraining.


<details>
  <summary>Details</summary>
Motivation: Existing watermarking methods for diffusion models have limitations: sampling-based approaches require costly DDIM inversion (N=50 steps) for detection, while fine-tuning-based methods couple watermarks to specific model checkpoints requiring retraining for each architecture.

Method: DiffMark injects a persistent learned perturbation δ at every denoising step of a frozen diffusion model, accumulating the watermark signal in the final denoised latent z₀. It uses Latent Consistency Models (LCM) as a differentiable training bridge to backpropagate gradients without traversing the full denoising chain, reducing gradient steps from 50 DDIM to 4 LCM.

Result: Single-pass detection at 16.4 ms (45x speedup over sampling-based methods), genuine per-image key flexibility, transferability to unseen diffusion architectures without fine-tuning, and competitive robustness against distortion, regeneration, and adversarial attacks.

Conclusion: DiffMark provides an efficient, flexible, and transferable watermarking solution for diffusion models that addresses key limitations of existing approaches while maintaining robustness.
...
TL;DR: The paper critiques current geospatial data fusion approaches for being one-sided ("master-slave") and proposes more symmetric, mutually beneficial integration schemes across scales and communities.


<details>
  <summary>Details</summary>
Motivation: Current geospatial data fusion operates under biased "master-slave" paradigms where one source dominates, lacking mutual benefits and failing to leverage the full potential of diverse data sources from satellites to citizen contributions across multiple scales.
...
Result: Identifies that numerous key data fusion configurations, particularly symmetric exploitation of multiple data sources, are insufficiently addressed despite being highly beneficial for generic or thematic applications.

Conclusion: There is a need to establish more symmetric data fusion schemes that create bridges and feedback loops between scales and communities to fully leverage the potential of diverse geospatial data sources.
...
TL;DR: EARTalking: An end-to-end GPT-style autoregressive model for interactive audio-driven talking head generation with frame-by-frame streaming control and identity consistency.


<details>
  <summary>Details</summary>
Motivation: Existing methods have limitations: AR-based methods rely on intermediate facial representations limiting expressiveness, while diffusion-based methods generate clip-by-clip lacking fine-grained control and causing latency due to overall denoising across windows.

Method: Proposes EARTalking with: 1) Frame-by-frame, in-context, audio-driven streaming generation paradigm; 2) Sink Frame Window Attention (SFA) mechanism for variable-length video generation with identity consistency; 3) Streaming Frame Condition In-Context (FCIC) scheme for efficient injection of diverse control signals in streaming manner.

Result: EARTalking outperforms existing autoregressive methods and achieves performance comparable to diffusion-based methods. Demonstrates feasibility of in-context streaming autoregressive control for flexible, efficient generation.

Conclusion: The work unlocks a scalable direction for flexible, efficient audio-driven talking head generation with interactive control at every frame and arbitrary moments. Code will be released for reproducibility.
...
TL;DR: GraphiContact: A pose-aware framework for joint monocular vertex-level human-scene contact prediction and 3D human mesh reconstruction using complementary Transformer priors and SIMU training for robustness.


<details>
  <summary>Details</summary>
Motivation: Existing approaches either focus on contact prediction without sufficiently exploiting 3D human priors, or emphasize pose/mesh reconstruction without directly optimizing robust vertex-level contact inference under occlusion and perceptual noise. There's a gap in jointly addressing both tasks effectively.

Method: Proposes GraphiContact framework that transfers complementary human priors from two pretrained Transformer encoders to predict per-vertex human-scene contact on reconstructed mesh. Introduces Single-Image Multi-Infer Uncertainty (SIMU) training strategy with token-level adaptive routing to simulate occlusion/noisy observations during training while preserving efficient single-branch inference.

Result: Experiments on five benchmark datasets show GraphiContact achieves consistent gains on both contact prediction and 3D human reconstruction tasks.

Conclusion: GraphiContact effectively bridges the gap between 3D human reconstruction and contact prediction, providing a robust solution for monocular vertex-level human-scene contact analysis with improved performance on both tasks.
...
TL;DR: VGS-Decoding is a training-free method that reduces hallucinations in medical VLMs by reweighting token probabilities based on visual dependency scores during inference.


<details>
  <summary>Details</summary>
Motivation: Medical VLMs often hallucinate by relying on language priors rather than visual evidence, creating risks for clinical applications where accuracy is critical.

Method: Introduces Visual Grounding Score (VGS) that measures token visual dependency by comparing distributions from original vs. distorted images. During decoding, amplifies visually grounded tokens while suppressing hallucinations with per-token adaptive control.

Result: Experiments on MIMIC-Diff-VQA and VQA-RAD with LLaVA-Med, CheXagent, and MedGemma show consistent improvements: up to +9.12% overall gain and +8.98% in open-ended recall, with only 2× inference overhead and no additional training.

Conclusion: VGS-Decoding effectively mitigates hallucinations in medical VLMs through visual grounding guidance during inference, offering practical clinical deployment with minimal overhead and no training requirements.
...
TL;DR: Space-based computing is viable with falling launch costs and growing AI workloads. This paper proposes a framework for deciding which tasks should be done in orbit vs. on Earth, with prototypes showing 99.5-99.99% data reduction through semantic processing.


<details>
  <summary>Details</summary>
Motivation: Space-based computing is becoming feasible due to decreasing launch costs and increasing data-intensive AI workloads. There's a need to determine which computational tasks are best suited for orbital execution versus terrestrial cloud processing.

Method: The paper proposes a workload-centric framework for orbital vs. terrestrial task allocation, along with a phased adoption model tied to orbital data center maturity. It validates the approach with in-orbit semantic-reduction prototypes: 1) Earth-observation pipeline on Sentinel-2 imagery from Seattle and Bengaluru, and 2) multi-pass stereo reconstruction prototype.

Result: The Earth-observation pipeline achieved 99.7-99.99% payload reduction by converting raw imagery to compact semantic artifacts. The stereo reconstruction prototype reduced ~306 MB to ~1.57 MB (99.49% reduction) of derived 3D representations.

Conclusion: Results support a "workload-first" view where semantic abstraction, not raw compute scale, drives early workload suitability for space-based computing. The framework helps determine optimal task placement between orbital and terrestrial systems.
...
TL;DR: A node-centric framework for video-based human pose estimation that integrates visual, temporal, and structural reasoning through specialized attention mechanisms and graph networks.


<details>
  <summary>Details</summary>
Motivation: Existing video-based human pose estimation methods struggle with motion blur, occlusion, and complex spatiotemporal dynamics. Current approaches relying on heatmaps or implicit feature aggregation limit joint topology expressiveness and weaken cross-frame consistency.

Method: 1) Visuo-temporal velocity-based joint embedding fusing sub-pixel joint cues and inter-frame motion; 2) Attention-driven pose-query encoder mapping joint representations to pose-aware node space; 3) Dual-branch decoupled spatio-temporal attention graph modeling temporal propagation and spatial constraints; 4) Node-space expert fusion module adaptively combining local and global cues.

Result: Extensive experiments on three widely used video pose benchmarks demonstrate that the method outperforms state-of-the-art methods.

Conclusion: The explicit node-centric reasoning approach offers a new perspective for advancing video-based human pose estimation, highlighting the value of integrating visual, temporal, and structural reasoning.
...
TL;DR: DCG-Net is an interpretable medical image analysis framework that combines multimodal alignment with structured concept reasoning using dual cross-attention and parametric concept graphs.


<details>
  <summary>Details</summary>
Motivation: Deep learning models in medical imaging lack interpretability, and existing Concept Bottleneck Models ignore contextual dependencies among clinical concepts, making explanations less clinically meaningful.
...
Result: DCG-Net achieves state-of-the-art classification performance on white blood cell morphology and skin lesion diagnosis tasks while producing clinically interpretable diagnostic explanations.

Conclusion: The framework successfully integrates multimodal alignment with structured concept reasoning, providing both high performance and clinically meaningful interpretability in medical image analysis.
...
TL;DR: A prompt-free, lightweight SAM adaptation for histopathology nuclei segmentation that uses multi-level encoder features and residual decoding with only 4.1M trainable parameters via LoRA fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing nuclei segmentation methods are computationally heavy and lack generalization across datasets, while SAM-based approaches rely on prompts or complex decoders that are unsuitable for dense histopathology images with heterogeneous appearances.

Method: Proposes a prompt-free lightweight SAM adaptation using multi-level encoder features and residual decoding. Fine-tunes only LoRA modules within the frozen SAM encoder, requiring just 4.1M trainable parameters.

Result: Achieves state-of-the-art performance on three benchmark datasets (TNBC, MoNuSeg, PanNuke) and demonstrates strong cross-dataset generalization.

Conclusion: The framework is effective and practical for histopathology applications, offering accurate and efficient nuclei segmentation with minimal computational requirements.
...
TL;DR: Proposes scale-encoded neural surface representation for multi-view normal integration that accounts for pixel coverage area variations due to camera distance, preventing blurring of high-frequency details.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view normal integration methods sample single rays per pixel without considering spatial area covered by each pixel, which varies with camera intrinsics and distance. This causes normal inconsistency across views when objects are captured at different distances, leading to blurring of high-frequency details in reconstructed surfaces.

Method: Scale-encoded neural surface representation incorporating pixel coverage area into neural representation. Each 3D point associated with spatial scale, with normals calculated from hybrid grid-based encoding. Mesh extraction module assigns optimal local scale to each vertex based on training observations.

Result: Approach consistently yields high-fidelity surface reconstruction from normals observed at varying distances, outperforming existing multi-view normal integration methods.

Conclusion: Scale-aware representation effectively addresses multi-view normal inconsistency caused by varying capture distances, enabling preservation of high-frequency surface details in reconstruction.
...
TL;DR: MV-BrainFM is a multi-view brain network foundation model that learns generalizable representations from brain networks with arbitrary atlases by incorporating anatomical priors and cross-view consistency learning.


<details>
  <summary>Details</summary>
Motivation: Existing brain network foundation models have limitations: atlas dependency, insufficient exploitation of multiple network views, and weak incorporation of anatomical priors. There's a need for more flexible, generalizable models that can handle diverse atlas configurations.

Method: Proposes MV-BrainFM with: 1) Explicit incorporation of anatomical distance information into Transformer-based modeling to guide inter-regional interactions; 2) Unsupervised cross-view consistency learning to align representations from multiple atlases of the same subject; 3) Unified multi-view pretraining paradigm for simultaneous learning from multiple datasets and atlases.

Result: Extensive experiments on >20K subjects from 17 fMRI datasets show MV-BrainFM consistently outperforms 14 existing brain network foundation models and task-specific baselines under both single-atlas and multi-atlas settings. Demonstrates strong scalability and stable performance across unseen atlas configurations.

Conclusion: MV-BrainFM provides a flexible, scalable foundation model for brain network analysis that overcomes atlas dependency, effectively captures complementary information across heterogeneous views, and shows superior performance across diverse datasets and configurations.
...
TL;DR: Proposes a novel 360° saliency graph representation for scenes that encodes visual, contextual, semantic, and geometric information to improve scene localization and vision-based indoor navigation.


<details>
  <summary>Details</summary>
Motivation: Existing scene representations (RGB-D, LiDAR, keypoints, etc.) are inefficient for applications like scene indexing and vision-based navigation because they don't explicitly encode relevant information and are sensitive to view changes, illumination variations, occlusions, and shadows.

Method: Develops a 360° saliency graph representation that encodes scene information as nodes, edges, edge weights, and angular positions. Uses this representation for two tasks: 1) localizing query scenes in topological maps, and 2) facilitating 2D navigation by estimating movement directions toward target destinations using embedded geometric information.

Result: Experimental results demonstrate the proposed representation enhances both scene localization and vision-based indoor navigation compared to existing methods that suffer from poor scene representation and lack scene-specific information.

Conclusion: The 360° saliency graph representation is an effective, robust solution for scene representation that addresses challenges in indoor environments and improves performance in vision-based navigation applications.
...
<details>
  <summary>Details</summary>
Motivation: Complex AI workflows often chain multiple generative models (e.g., 2D image → video/3D generation), but distributional mismatches between upstream outputs and downstream expected inputs degrade overall quality.

Method: Uni-C leverages video diffusion priors to guide the denoising process of preceding models, aligning their outputs with downstream requirements. It can be used as a plug-and-play module between models or independently to enhance individual model outputs.

Result: Extensive experiments across video and 3D generation tasks show Uni-C consistently improves generation quality in both workflow-based settings (chained models) and standalone applications.

Conclusion: Uni-C demonstrates versatility and strong generalization capability as an effective solution for distributional alignment in generative AI workflows, while also enhancing individual model performance.
...
TL;DR: Multi-stage fine-tuning approach for 13-class WBC classification using DINOBloom-base with specialized classifier heads and ensemble strategy to address class imbalance, domain shift, and morphological continuum confusion in leukemia diagnosis.


<details>
  <summary>Details</summary>
Motivation: Automated white blood cell classification faces challenges including class imbalance, domain shift, and morphological continuum confusion where adjacent maturation stages have subtle, overlapping features, making leukemia diagnosis difficult.

Method: Multi-stage fine-tuning of DINOBloom-base model with three classifier head families (linear, cosine, MLP). Constructed head-diverse ensemble where MLP head is primary predictor, with predictions replaced only when two other heads agree within predefined confusion pairs.

Result: Different heads specialized for different granulocyte classes: cosine head best for mature granulocytes (BNE F1=0.470), linear head for immature granulocytes (MMY F1=0.585), MLP head for most immature granulocytes (PMY F1=0.733). Ensemble improved performance, and consistent misclassifications revealed probable labeling errors or inherent ambiguity.

Conclusion: Class-specific head specialization enables effective ensemble construction for WBC classification, and consistent model errors highlight dataset quality issues that could inform future annotation improvements.
...
TL;DR: A graph-based multiple instance learning method for computational pathology that uses vision foundation model embeddings for local spatial structure and jigsaw regularization for across-patch spatial awareness, outperforming attention-based MIL approaches on cancer datasets.


<details>
  <summary>Details</summary>
Motivation: Most multiple instance learning (MIL) methods in computational pathology treat tissue patches as exchangeable, ignoring the rich spatial and topological structure inherent in whole-slide images, which limits their performance.

Method: Combines vision foundation model embeddings to capture local spatial structure within patches with graph neural networks and novel jigsaw regularization to achieve across-patch spatial awareness in a graph-based MIL framework.

Result: Markedly improves classification performance over state-of-the-art attention-based MIL approaches on benchmark datasets for breast, head-and-neck, and colon cancer.

Conclusion: Incorporating both local spatial structure (via foundation model embeddings) and across-patch spatial awareness (via graph neural networks with jigsaw regularization) significantly enhances computational pathology classification compared to existing MIL methods.
...
TL;DR: Training-free multi-view human mesh recovery framework using pretrained single-view models as priors, achieving SOTA without multi-view training data.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view HMR methods have limitations: geometry-based approaches need cumbersome camera calibration, while learning-based methods generalize poorly to unseen camera setups due to lack of multi-view training data. Need calibration-free reconstruction that generalizes to arbitrary camera setups.

Method: Proposes training-free framework leveraging pretrained single-view HMR models as strong priors. First constructs robust multi-view initialization from single-view predictions, then refines via test-time optimization guided by multi-view consistency and anatomical constraints.
...
Conclusion: The proposed calibration-free framework enables robust multi-view human mesh recovery that generalizes to arbitrary camera setups without requiring multi-view training data or camera calibration.
...
TL;DR: FAAR introduces frequency-aware automatic rank adaptation for efficient multi-task learning fine-tuning, using performance-driven rank shrinking and task-spectral pyramidal decoder to improve accuracy while reducing parameters 9x.


<details>
  <summary>Details</summary>
Motivation: Traditional full fine-tuning is inefficient for large models in multi-task learning, and existing parameter-efficient methods use fixed ranks that are suboptimal and fail to capture spatial inter-task relationships.

Method: FAAR uses Performance-Driven Rank Shrinking (PDRS) to allocate optimal ranks per adapter location and task, and Task-Spectral Pyramidal Decoder (TS-PD) that analyzes image frequency spectrum to inject input-specific context into spatial bias learning for better cross-task relationships.

Result: Experiments on dense visual task benchmarks show FAAR outperforms other PEFT methods in both accuracy and efficiency, reducing parameters by up to 9x compared to traditional MTL fine-tuning while improving overall performance.

Conclusion: FAAR provides an effective parameter-efficient fine-tuning approach for multi-task learning that dynamically adapts ranks and leverages frequency analysis to capture spatial inter-task relationships, achieving superior performance with significantly fewer parameters.
...
<details>
  <summary>Details</summary>
Motivation: Current multimodal personalization methods are limited to static images or offline videos, disconnected from continuous visual input and real-time feedback needed for future AI assistants.

Method: Introduces PEARL-Bench benchmark with 132 videos and 2,173 fine-grained annotations, and proposes PEARL - a plug-and-play, training-free strategy for streaming video understanding.

Result: PEARL achieves state-of-the-art performance across 8 offline and online models, brings consistent PSVU improvements to 3 distinct architectures, proving to be highly effective and robust.

Conclusion: This work advances vision-language model personalization and inspires research into streaming personalized AI assistants, with code publicly available.
...
TL;DR: Benchmark for efficient SfM for novel view synthesis shows that using fewer features speeds up classical methods, and neural network initialization with classical refinement gives best trade-off.


<details>
  <summary>Details</summary>
Motivation: Recent neural SfM methods are fast but inaccurate, while classical SfM is accurate but slow. Need to stimulate research on SfM approaches that are both efficient AND effective for novel view synthesis.

Method: Developed a benchmark for SfM in novel view synthesis using existing datasets. Tested two simple efficiency strategies: 1) using fewer features in classical SfM, 2) using feed-forward neural networks for initial estimates followed by classical refinement.

Result: 1) Fewer features significantly accelerate classical SfM while maintaining high pose accuracy. 2) Neural network initialization with classical refinement achieves the best efficiency-effectiveness trade-off.

Conclusion: Hybrid approach combining neural network efficiency with classical refinement offers optimal balance for SfM in novel view synthesis, providing a benchmark to stimulate research in efficient yet accurate SfM methods.
...
TL;DR: Thermal-only novel view synthesis is challenging due to low dynamic range and photometric instability; proposed lightweight preprocessing and splatting pipeline addresses these issues without dataset-specific tuning.


<details>
  <summary>Details</summary>
Motivation: Thermal cameras work in darkness/adverse conditions but thermal imagery is harder for novel view synthesis than visible-light images due to two main issues: 1) extremely low dynamic range weakens appearance cues and limits optimization gradients, 2) rapid photometric fluctuations and slow radiometric drift destabilize correspondence estimation and create floater artifacts in view synthesis.

Method: Lightweight preprocessing and splatting pipeline that expands usable dynamic range and stabilizes per-frame photometry. The approach doesn't require any dataset-specific tuning.

Result: Achieves state-of-the-art performance across thermal-only novel view synthesis benchmarks.

Conclusion: The proposed pipeline effectively addresses the core challenges of thermal-only novel view synthesis (dynamic range limitations and photometric instability) through preprocessing and splatting techniques, enabling reliable thermal view synthesis without RGB guidance.
...
TL;DR: The paper introduces two methods for solving the inverse problem of finding input images that map to specific neural network classes, revealing vulnerabilities in networks by generating random-like images with perfect classification scores.


<details>
  <summary>Details</summary>
Motivation: Neural networks describe complex mappings that are difficult to understand. The authors want to determine what input images get mapped to specific classes, expecting these images to contain recognizable features associated with their classifications.

Method: Two general methods: 1) Forward pass method using root-finding algorithm and Jacobian with respect to input image, and 2) Backward pass method that iteratively inverts each layer from the top, adding random vectors sampled from the null-space of each linear layer.

Result: The methods produce random-like input images that yield near perfect classification scores in all cases on both transformer architectures and sequential networks, revealing vulnerabilities in the underlying networks.

Conclusion: The proposed methods provide more comprehensive coverage of the input image spaces that solve the inverse mapping problem, demonstrating their effectiveness in revealing network vulnerabilities.
...
TL;DR: CREG is a training-free interpretability framework that projects multi-layer contrastive Grad-times-Act attributions into polar coordinates to reveal directional evidence in vision-language models' spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Current attribution methods (GradCAM, attention rollout) show where models attend but not what directional relations they infer between objects, leaving a gap in understanding how VLMs encode spatial relationships.

Method: CREG projects multi-layer contrastive Grad-times-Act attributions into a reference-centered polar coordinate system, producing directional evidence distributions over compass sectors. Three evaluation metrics are proposed: Direction Alignment Error (DAE), Edge Accuracy (EA), and Causal Occlusion Score (COS).

Result: On Qwen2-VL-7B across VSR and COCO-Pairs, CREG outperforms standard baselines, achieving DAE of 55.5 degrees and EA of 0.553 on COCO-Pairs, improving over attention rollout by 16.1 degrees and 0.120 EA. Causal occlusion experiments (540 samples) show COS ≥ +0.42, supporting faithfulness.

Conclusion: Contrastive, multi-layer attribution in CREG exposes directional evidence more faithfully than standard saliency-based explanations in VLM spatial reasoning, with benefits scaling with model size due to more structured spatial representations.
...
TL;DR: This paper introduces the first unified study of camera-trap species recognition over time, highlighting that temporal shifts at fixed sites pose greater challenges than cross-domain generalization, and provides a benchmark with streaming evaluation protocol.


<details>
  <summary>Details</summary>
Motivation: Current computer vision approaches focus on cross-domain generalization for camera traps, but overlook the real-world challenge of maintaining reliable recognition at fixed sites over time due to dynamic ecosystem changes in both background and animal distributions.

Method: Created a realistic benchmark with 546 camera traps using a streaming protocol that evaluates models over chronologically ordered intervals, simulating real deployment lifecycles where models are updated using past data and evaluated on future intervals.

Result: Four key findings: (1) Biological foundation models underperform at many sites initially; (2) Naive adaptation can degrade below zero-shot performance; (3) Challenges driven by severe class imbalance and pronounced temporal shifts; (4) Integration of model-update and post-processing techniques improves accuracy but gaps remain.

Conclusion: The study provides actionable deployment guidelines for ecological practitioners and establishes new research directions, highlighting critical open questions about predicting zero-shot success and determining when model updates are necessary for camera-trap species recognition over time.
...
<details>
  <summary>Details</summary>
Motivation: Polarization provides rich material information but traditional methods require time-consuming multiple polarization modulations. Need to find optimal measurement angles for efficient material classification.

Method: Proposed end-to-end optimization framework that jointly learns material classifier and determines optimal combinations of rotation angles for polarization elements controlling both incident and reflected light states.

Result: Method achieves high-accuracy material classification even with limited number of measurements, using their Mueller-matrix material dataset.

Conclusion: The framework enables efficient material classification by optimizing polarization measurement angles, reducing time and complexity while maintaining accuracy.
...
TL;DR: This paper addresses CLIP's failure to understand negation in text queries, proposes a better evaluation framework using MLLMs-as-judges, discovers a negation direction in CLIP's embedding space, and shows it can be manipulated via representation engineering without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: CLIP models fail to understand negation in text queries (e.g., "a plain blue shirt with no logos"), and existing evaluation metrics using retrieval-based approaches cannot reliably measure whether negation is actually understood.

Method: 1) Proposes Multimodal LLMs-as-a-judge evaluation framework for negation understanding; 2) Investigates existence of negation direction in CLIP embedding space; 3) Uses representation engineering to manipulate this direction for negation-aware behavior without fine-tuning; 4) Tests generalization on non-common image-text samples.

Result: Found evidence of a negation direction in CLIP embedding space; Showed representation engineering can steer CLIP toward negation-aware behavior without fine-tuning; Demonstrated evaluation framework using MLLMs provides fair assessment of negation understanding.

Conclusion: Negation understanding in CLIP can be improved through representation engineering without fine-tuning, and MLLMs provide a better evaluation framework than traditional retrieval metrics for assessing negation comprehension.
...
TL;DR: SGG (Strong-to-Weak Guidance) is a hybrid guidance method that combines CFG and AG principles to improve diffusion model generalization by addressing gradient misalignment issues during sampling.


<details>
  <summary>Details</summary>
Motivation: Diffusion models suffer from accumulated gradient errors due to misalignment between simulation-free objectives and iterative sampling, leading to unsatisfactory results and poor generalization. Existing guidance methods like CFG and AG have unclear effective regimes, making method selection ambiguous.

Method: Proposes SGG (Strong-to-Weak Guidance), a hybrid method based on the weak-to-strong principle that combines benefits of CFG and AG. Also migrates the W2S principle into training objectives to improve unguided diffusion models.

Result: SGG outperforms existing training-free guidance variants on SD3 and SD3.5 at inference. Training-time experiments on transformer architectures show effective migration and performance gains in both conditional and unconditional settings.

Conclusion: The weak-to-strong principle provides a framework for understanding guidance methods, and SGG effectively combines CFG and AG benefits while being migratable to training objectives for improved generalization.
...
TL;DR: RayMap3R is a training-free streaming framework for dynamic 3D scene reconstruction that identifies moving objects by contrasting RayMap and image predictions, achieving state-of-the-art performance without explicit dynamic reasoning.


<details>
  <summary>Details</summary>
Motivation: Streaming feed-forward 3D reconstruction models can be affected by moving objects, causing artifacts and drift, since they lack explicit dynamic reasoning. The authors aim to address this limitation without requiring training.

Method: The method leverages the observation that RayMap-based predictions exhibit static-scene bias. It constructs a dual-branch inference scheme that identifies dynamic regions by contrasting RayMap and image predictions, suppressing their interference during memory updates. Additional techniques include reset metric alignment and state-aware smoothing to preserve metric consistency and stabilize trajectories.

Result: The method achieves state-of-the-art performance among streaming approaches on dynamic scene reconstruction across multiple benchmarks.

Conclusion: RayMap3R successfully addresses dynamic scene reconstruction challenges in streaming frameworks without requiring training, using internal cues from prediction biases and careful memory management techniques.
...
TL;DR: GaussianPile unifies 3D Gaussian splatting with imaging system-aware focus modeling to compress slice-based volumetric imaging while preserving internal structure for analysis.


<details>
  <summary>Details</summary>
Motivation: Slice-based volumetric imaging needs representations that compress aggressively while preserving internal structure for analysis. Current methods lack efficient compression with diagnostic fidelity.

Method: Three key innovations: (1) slice-aware piling strategy positioning anisotropic 3D Gaussians to model through-slice contributions, (2) differentiable projection operator encoding finite-thickness point spread function, (3) compact encoding and joint optimization pipeline for simultaneous reconstruction and compression.

Result: Reduces storage and reconstruction cost, sustains diagnostic fidelity, enables fast 2D visualization and 3D voxelization. Achieves 11x faster than NeRF-based approaches, consistent 16x compression over voxel grids, high-quality results in as few as 3 minutes.

Conclusion: GaussianPile offers a practical path to deployable compression and exploration of slice-based volumetric datasets, retaining compression and real-time rendering efficiency while preserving high-frequency internal volumetric detail.
...
<details>
  <summary>Details</summary>
Motivation: Existing image editing datasets either rely on costly closed-source models for annotation (limiting scalability) or use fixed synthetic pipelines (limiting quality and generalizability), creating a need for cost-effective, scalable, high-quality dataset construction.

Method: A three-component hierarchical multi-agent framework: 1) source image expansion with world-knowledge infusion, 2) adaptive multi-agent editing instruction-image synthesis, and 3) task-aware data quality verification mechanism.

Result: Created ScaleEdit-12M, the largest open-source image editing dataset spanning 23 task families across diverse domains. Fine-tuning models on this dataset improved performance by up to 10.4% on ImgEdit, 35.1% on GEdit, 150.0% on RISE, and 26.5% on KRIS-Bench.

Conclusion: Open-source agentic pipelines can approach commercial-grade data quality while maintaining cost-effectiveness and scalability, demonstrating the viability of fully open-source solutions for high-quality dataset construction.
...
TL;DR: A continual learning framework for fine-grained fashion image retrieval that handles new attributes incrementally without full retraining, achieving comparable performance to static methods with only 30% training cost.


<details>
  <summary>Details</summary>
Motivation: Current fine-grained fashion image retrieval methods require full retraining when new attributes appear, which is costly and impractical for dynamic scenarios. Pretrained models have accuracy drops without supervision, and no prior work explores class-incremental learning for this task.

Method: Proposes MCL-FIR: multihead continual learning framework with contrastive learning and exponential moving average distillation. Uses multi-head design for evolving classes, reformulates triplet inputs into doublets with InfoNCE for simpler training, and employs EMA distillation for knowledge transfer.

Result: Experiments across four datasets show MCL-FIR significantly outperforms class-incremental learning baselines under similar training cost. Compared to static methods, it delivers comparable performance while using only about 30% of the training cost.

Conclusion: MCL-FIR provides a scalable solution for fine-grained fashion image retrieval in dynamic scenarios, achieving strong balance between efficiency and accuracy without requiring full retraining when new attributes appear.
...
TL;DR: IBCapsNet is a new capsule network architecture that uses Information Bottleneck principle to replace iterative routing with one-pass variational aggregation, achieving better robustness to noise while being faster and more efficient.


<details>
  <summary>Details</summary>
Motivation: Capsule networks are good at modeling hierarchical spatial relationships but have two main problems: high computational cost from iterative dynamic routing and poor robustness to input corruptions/noise.

Method: Replaces iterative routing with a one-pass variational aggregation mechanism based on Information Bottleneck principle. Primary capsules are compressed into a global context representation, then processed by class-specific variational autoencoders (VAEs) to infer latent capsules regularized by KL divergence.

Result: Matches CapsNet accuracy on clean data (99.41% on MNIST, 92.01% on SVHN), significantly outperforms under four types of synthetic noise (+17.10% for clamped additive noise, +14.54% for multiplicative noise). Achieves 2.54x faster training, 3.64x higher inference throughput, and reduces parameters by 4.66%.

Conclusion: IBCapsNet bridges information-theoretic representation learning with capsule networks, offering a principled approach to create robust, efficient, and interpretable deep models while addressing key limitations of traditional capsule networks.
...
TL;DR: MFSR is a distillation framework for Real-ISR that achieves photorealistic results in one step while allowing optional few-step refinement, using MeanFlow as learning target to capture teacher dynamics without explicit rollouts.


<details>
  <summary>Details</summary>
Motivation: Diffusion/flow-based models for Real-ISR have slow inference due to multi-step sampling. One-step distillation reduces cost but degrades quality and removes refinement options. Need efficient, high-quality solution with flexibility.

Method: Uses MeanFlow as learning target to approximate average velocity between arbitrary PF-ODE states, capturing teacher dynamics without rollouts. Improves CFG formulation with teacher CFG distillation to better leverage pretrained generative priors.

Result: Achieves results on par or better than multi-step teachers with much lower computational cost. Demonstrates efficient, flexible, high-quality super-resolution on synthetic and real-world benchmarks.

Conclusion: MFSR provides an effective distillation framework that balances efficiency and quality, enabling single-step photorealistic results while preserving optional refinement capability for Real-ISR.
...
TL;DR: Satellite-to-street view synthesis for disaster assessment using VLM-guided and damage-sensitive MoE methods, with evaluation showing realism-fidelity trade-off where diffusion models hallucinate details despite high perceptual realism.


<details>
  <summary>Details</summary>
Motivation: Bridge the data gap in disaster response by synthesizing ground-level street views from satellite imagery, since satellites lack ground-level perspective while street-view data is inaccessible during time-sensitive events.

Method: Two generative strategies: 1) Vision-Language Model (VLM)-guided approach, and 2) damage-sensitive Mixture-of-Experts (MoE) method. Compared against Pix2Pix and ControlNet baselines using a Structure-Aware Evaluation Framework with pixel-level assessment, ResNet semantic consistency, and VLM-as-a-Judge for perceptual alignment.

Result: Experiments on 300 disaster scenarios reveal critical realism-fidelity trade-off: diffusion-based approaches (ControlNet) achieve high perceptual realism but hallucinate structural details. ControlNet achieves highest semantic accuracy (0.71), while VLM-enhanced and MoE models excel in textural plausibility but struggle with semantic clarity.
...
TL;DR: CogAlign framework aligns MLLMs with clinical cognition pathways for gastrointestinal endoscopy, using hierarchical clinical dataset training and counterfactual reinforcement learning to eliminate visual bias and improve diagnostic accuracy.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs in medical imaging face two critical limitations for gastrointestinal endoscopy: 1) misalignment between general model reasoning and standardized clinical cognitive pathways, and 2) lack of causal association between visual features and diagnostic outcomes, leading to reliance on spurious correlations.

Method: Two-stage approach: 1) Construct hierarchical clinical cognition dataset and use Supervised Fine-Tuning to internalize expert diagnostic logic (anatomical localization → morphological evaluation → microvascular analysis). 2) Counterfactual-driven reinforcement learning strategy using lesion-masked normal samples and clinical-cognition-centric rewards to enforce causal rectification and eliminate visual bias.

Result: Achieves State-of-the-Art performance across multiple benchmarks, significantly enhancing diagnostic accuracy in complex clinical scenarios. Theoretical analysis shows standard supervised tuning converges to spurious background correlations, which their method successfully addresses.

Conclusion: CogAlign framework successfully aligns MLLMs with clinical cognitive pathways for gastrointestinal endoscopy by combining hierarchical clinical dataset training with counterfactual reinforcement learning, enabling more accurate and clinically-relevant diagnoses while eliminating visual bias. All source code and datasets will be publicly available.
...
TL;DR: EHETM: An efficient turbulence mitigation method using event cameras that leverages polarity-weighted gradients and event-tube constraints to achieve high-quality restoration with minimal frames.


<details>
  <summary>Details</summary>
Motivation: Traditional turbulence mitigation methods face a trade-off between accuracy and efficiency - more frames improve restoration but increase latency and data overhead. Event cameras with microsecond temporal resolution offer an opportunity to break this bottleneck by capturing motion patterns more efficiently.

Method: Two key insights guide the method: (1) turbulence-induced events show polarity alternation correlated with sharp image gradients, providing structural cues; (2) dynamic objects form coherent "event tubes" contrasting with irregular turbulent patterns. Based on these, EHETM uses two complementary modules: polarity-weighted gradients for scene refinement and event-tube constraints for motion decoupling.

Result: EHETM outperforms state-of-the-art methods, especially in scenes with dynamic objects, while reducing data overhead by ~77.3% and system latency by ~89.5%. The authors also constructed two real-world event-frame turbulence datasets covering atmospheric and thermal cases.

Conclusion: Event cameras provide a promising solution for efficient turbulence mitigation by leveraging their unique sensing capabilities. EHETM demonstrates that few frames combined with event data can achieve high-quality restoration while significantly reducing computational and data requirements.
...
TL;DR: This paper systematically studies how different initialization methods (from dense laser scans to sparse SfM) affect 3D Gaussian Splatting performance, finding current densification approaches don't fully leverage dense initializations.


<details>
  <summary>Details</summary>
Motivation: While 3DGS has become popular for photo-realistic 3D reconstruction, it typically starts from sparse SfM point clouds and relies on densification. The authors want to understand the relationship between initialization quality and densification effectiveness, questioning whether current methods properly utilize dense initializations.

Method: The authors create a new benchmark to systematically study combinations of different initialization types (dense laser scans, dense multi-view stereo, dense monocular depth, sparse SfM) with various densification schemes in 3DGS.

Result: Current densification approaches are unable to take full advantage of dense initializations, often failing to significantly improve over sparse SfM-based initialization despite having better starting points.

Conclusion: There's a gap in how 3DGS densification leverages initialization quality, suggesting room for improvement in densification algorithms to better utilize dense initial information. The benchmark will be made publicly available for further research.
...
TL;DR: CFANet uses fuzzy logic for token-level alignment and ground-view images as bridge agents to improve text-aerial person retrieval, with a new AERI-PEDES dataset.


<details>
  <summary>Details</summary>
Motivation: Text-aerial person retrieval is challenging due to degraded visual information in UAV images from varying angles/altitudes, causing semantic misalignment with text descriptions.

Method: Cross-modal Fuzzy Alignment Network with: 1) Fuzzy Token Alignment using fuzzy membership functions to model token-level associations and suppress noisy tokens; 2) Context-Aware Dynamic Alignment using ground-view images as bridge agents; 3) AERI-PEDES dataset construction via chain-of-thought text generation.

Result: Experiments on AERI-PEDES and TBAPR datasets demonstrate superior performance compared to existing methods.

Conclusion: The proposed CFANet effectively addresses text-aerial person retrieval challenges through fuzzy logic alignment and ground-view bridging, with the new dataset enabling better evaluation.
...
TL;DR: Premier is a preference modulation framework for personalized image generation that learns user-specific embeddings and uses a preference adapter to fuse them with text prompts, achieving better alignment with user preferences than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image generation struggles to capture nuanced user preferences. Existing approaches using multimodal LLMs to infer preferences often fail to faithfully reflect them in derived prompts or latent codes, leading to suboptimal personalization.

Method: Premier represents each user's preference as a learnable embedding and introduces a preference adapter that fuses the user embedding with text prompts. It uses a dispersion loss to enhance distinctness of individual preferences and improve alignment with user-specific styles. For new users with scarce data, it represents them as linear combinations of existing preference embeddings.

Result: Premier outperforms prior methods under the same history length, achieving stronger preference alignment and superior performance on text consistency, ViPer proxy metrics, and expert evaluations.

Conclusion: Premier provides an effective framework for personalized image generation that accurately captures and modulates user preferences through learnable embeddings and preference adaptation mechanisms.
...
TL;DR: Weakly supervised multimodal segmentation framework combines 2D borehole images with 1D well-logs using confidence-gated depth-aware cross-attention, outperforming threshold-based and unimodal baselines without requiring expert annotations.


<details>
  <summary>Details</summary>
Motivation: Acoustic borehole images provide high-resolution data but large-scale interpretation is difficult due to lack of dense expert annotations and the multimodal nature of subsurface information (2D images + 1D well-logs).

Method: Weakly supervised multimodal segmentation framework that refines threshold-guided pseudo-labels through learned models, preserving annotation-free workflow while adding denoising, confidence-aware pseudo-supervision, and physically structured fusion of 2D images with depth-aligned 1D logs.

Result: Threshold-guided learned refinement provides most robust improvement over baselines. Confidence-gated depth-aware cross-attention (CG-DCA) consistently outperforms threshold-based, image-only, and earlier multimodal baselines. Performance depends on confidence-aware fusion and structured local depth interaction rather than model complexity alone.

Conclusion: Practical, scalable framework for annotation-free segmentation shows multimodal improvement is maximized when auxiliary logs are incorporated selectively and depth-aware, with confidence-gated depth-aware cross-attention being the strongest approach.
...
TL;DR: VSD-MOT: A multi-object tracking framework using visual semantic distillation to handle low-quality videos by compensating for information loss through CLIP-based knowledge distillation and dynamic quality-aware fusion.


<details>
  <summary>Details</summary>
Motivation: Existing MOT algorithms perform poorly in low-quality videos due to information loss, leading to significant performance degradation in real-world scenarios with deteriorating image quality.

Method: Proposes VSD-MOT framework with: 1) CLIP Image Encoder for global visual semantics, 2) Knowledge distillation via teacher-student framework (CLIP as teacher), 3) Dual-Constraint Semantic Distillation (DCSD) to learn task-specific semantic extraction, 4) Dynamic Semantic Weight Regulation (DSWR) module for adaptive fusion based on frame quality assessment.

Result: Extensive experiments show effectiveness and superiority in low-quality video scenarios while maintaining good performance in conventional scenarios.

Conclusion: The proposed VSD-MOT framework successfully addresses MOT challenges in low-quality videos through visual semantic distillation and dynamic quality-aware adaptation, bridging the performance gap between ideal and real-world conditions.
...
TL;DR: SATTC improves cross-subject EEG-to-image retrieval by addressing subject shift and hubness issues through label-free test-time calibration of similarity matrices.


<details>
  <summary>Details</summary>
Motivation: Cross-subject EEG-to-image retrieval faces challenges from subject shift (differences between individuals' brain signals) and hubness (certain items dominating similarity rankings), which distort similarity geometry and make small-k retrieval unreliable.

Method: SATTC is a label-free calibration head that operates on similarity matrices from frozen EEG and image encoders. It combines: 1) Geometric expert with subject-adaptive whitening and adaptive CSLS, and 2) Structural expert using mutual nearest neighbors, bidirectional top-k ranks, and class popularity, fused via Product-of-Experts rule.

Result: On THINGS-EEG dataset with leave-one-subject-out protocol, SATTC improves Top-1 and Top-5 accuracy, reduces hubness and class imbalance, and produces more reliable small-k shortlists compared to baseline methods. Gains transfer across multiple EEG encoders.

Conclusion: SATTC serves as an encoder-agnostic, label-free test-time calibration layer that effectively addresses cross-subject challenges in neural decoding, improving retrieval reliability without requiring labeled data.
...
TL;DR: SADG is a Mamba-based In-Context Learning framework for multi-task domain generalization in point cloud processing, using structure-aware serialization and hierarchical domain modeling to maintain structural consistency across domains.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer and Mamba architectures for point clouds are designed for single-task/single-domain settings and fail in multi-task domain generalization. Transformers have quadratic attention costs and lack explicit structural ordering, while Mamba's coordinate-driven serialization is sensitive to viewpoint changes and missing regions, causing structural drift.

Method: Proposes SADG with: 1) Structure-Aware Serialization (SAS) using centroid-based topology and geodesic curvature continuity for transformation-invariant sequences; 2) Hierarchical Domain-Aware Modeling (HDM) that stabilizes cross-domain reasoning by consolidating intra-domain structure and fusing inter-domain relations; 3) Lightweight Spectral Graph Alignment (SGA) for test-time feature shifting without parameter updates.
...
Conclusion: SADG effectively addresses structural drift in multi-task domain generalization for point clouds by combining structure-aware serialization, hierarchical domain modeling, and test-time spectral alignment, demonstrating superior performance across various 3D processing tasks.
...
<details>
  <summary>Details</summary>
Motivation: Current text-to-image diffusion models struggle with precise text-image alignment due to limitations of conventional diffusion loss, which only provides implicit supervision for fine-grained correspondence.

Method: CTCal leverages reliable text-image alignment (cross-attention maps) from smaller timesteps with less noise to calibrate representation learning at larger timesteps with more noise, providing explicit supervision. Includes timestep-aware adaptive weighting for integration with diffusion loss.

Result: Extensive experiments on T2I-Compbench++ and GenEval benchmarks demonstrate effectiveness and generalizability. CTCal is model-agnostic and works with both diffusion-based (SD 2.1) and flow-based (SD 3) approaches.

Conclusion: CTCal provides explicit supervision for text-image alignment in diffusion models, addressing a fundamental limitation of conventional diffusion loss and improving alignment quality across different model architectures.
...
TL;DR: AI-based real-time gauze counting system using YOLOv5 to prevent surgical gauzes being left inside patients, improving from 8 FPS to 15 FPS with integrated human/gauze detection.


<details>
  <summary>Details</summary>
Motivation: Prevent "Gossypiboma" (retained surgical gauzes) which causes serious patient complications and legal problems for hospitals. Current manual counting methods are time-consuming and divert nursing resources from patient care.

Method: Developed an AI system using YOLOv5 deep learning model with real-time video surveillance and object recognition. Tracks gauzes on designated "In" and "Out" trays, using an integrated model trained on 11,000 images (vs previous 2,800) that detects both humans and gauzes simultaneously.

Result: Improved accuracy and increased frame rate from 8 FPS to 15 FPS. System now supports manual count adjustments based on doctor feedback, enhancing reliability in actual surgical settings.

Conclusion: The AI-based gauze counting system provides an effective prevention method for Gossypiboma, addressing limitations of manual counting while improving efficiency and reliability in surgical environments through integrated detection and real-time monitoring.
...
TL;DR: DiT-BlockSkip: A memory-efficient fine-tuning framework for Diffusion Transformers that reduces computational complexity through dynamic patch sampling and block skipping for on-device personalization.


<details>
  <summary>Details</summary>
Motivation: Fine-tuning Diffusion Transformers for personalized text-to-image generation requires substantial computational resources and memory, limiting practical deployment on resource-constrained devices like smartphones and IoT devices.

Method: 1) Timestep-aware dynamic patch sampling: Adjusts patch sizes based on diffusion timestep (larger patches for global structure at higher timesteps, smaller for details at lower timesteps), then resizes to fixed lower resolution. 2) Block skipping with precomputed residual features: Selectively fine-tunes essential transformer blocks while precomputing features for skipped blocks. 3) Cross-attention masking strategy to identify vital blocks for personalization.
...
Conclusion: DiT-BlockSkip provides an effective memory-efficient fine-tuning framework that moves toward practical deployment of personalized diffusion transformers on resource-constrained devices.
...
TL;DR: PiLoT is a unified framework for UAV-based ego and target geo-localization that directly registers live video against geo-referenced 3D maps, eliminating dependency on GNSS and active sensors while achieving real-time performance.


<details>
  <summary>Details</summary>
Motivation: Conventional UAV localization methods rely on GNSS and VIO for ego-pose estimation and active sensors like laser rangefinders for target localization, making them vulnerable to GNSS-denied environments and requiring expensive hardware with complex setups.

Method: 1) Dual-Thread Engine decouples map rendering from core localization for low latency and drift-free accuracy; 2) Large-scale synthetic dataset with precise annotations enables training lightweight network for zero-shot sim-to-real generalization; 3) Joint Neural-Guided Stochastic-Gradient Optimizer (JNGO) ensures robust convergence under aggressive motion.

Result: PiLoT outperforms state-of-the-art methods on comprehensive benchmarks and runs at over 25 FPS on NVIDIA Jetson Orin platform, demonstrating robust performance in real-time applications.

Conclusion: PiLoT provides a unified, cost-effective solution for UAV geo-localization that works in GNSS-denied environments, reduces hardware complexity, and achieves real-time performance through innovative architectural design and optimization techniques.
...
TL;DR: MEMO produces crisp, human-like edges using only cross-entropy loss through masked training and progressive confidence-based inference, without specialized losses or architecture changes.


<details>
  <summary>Details</summary>
Motivation: Learning-based edge detectors trained with cross-entropy loss produce thick edges that don't match human-annotated crisp, single-pixel edges. Previous solutions required specialized loss functions or architecture modifications.

Method: MEMO uses masked edge prediction: pre-training on synthetic data, fine-tuning with lightweight modules, learning to predict edges under varying input masking ratios, and using progressive prediction based on confidence gradients during inference.

Result: Achieves visually appealing, post-processing-free, human-like edge maps and outperforms prior methods on crispness-aware evaluations.

Conclusion: Carefully designed training and inference strategy alone (without specialized losses or architecture changes) is sufficient to achieve human-like edge quality, using cross-entropy loss and confidence-based progressive prediction.
...
TL;DR: ME-IQA is a memory-enhanced re-ranking framework that improves reasoning-based vision-language models for image quality assessment by addressing discrete collapse through neighbor retrieval, probabilistic comparison, and memory consolidation.


<details>
  <summary>Details</summary>
Motivation: Reasoning-induced VLMs for IQA suffer from discrete collapse where scalar scores lack sensitivity and collapse to a few values, limiting their effectiveness in assessing image quality.

Method: Three-step framework: (1) Build memory bank and retrieve semantically/perceptually aligned neighbors using reasoning summaries, (2) Reframe VLM as probabilistic comparator to get pairwise preference probabilities and fuse with initial scores using Thurstone's Case V model, (3) Perform gated reflection and consolidate memory for future decisions.

Result: ME-IQA produces denser, distortion-sensitive predictions and mitigates discrete collapse, showing consistent gains across multiple IQA benchmarks over reasoning-induced VLM baselines, non-reasoning IQA methods, and test-time scaling alternatives.

Conclusion: The proposed plug-and-play, test-time memory-enhanced re-ranking framework effectively addresses discrete collapse in reasoning-based VLMs for IQA, improving sensitivity and performance through memory-based neighbor retrieval and probabilistic comparison.
...
TL;DR: Co-VLN is a minimalist framework that enables multiple robots navigating the same environment to share perceptual observations, overcoming partial observability constraints in Vision-Language Navigation without additional exploration cost.


<details>
  <summary>Details</summary>
Motivation: VLN systems are constrained by partial observability since agents can only learn from locations they personally visit. With multiple robots increasingly coexisting in shared environments, the paper investigates whether agents can benefit from each other's observations to overcome this limitation.

Method: Co-VLN is a model-agnostic framework where independently navigating agents exchange structured perceptual memory when they identify common traversed locations. This expands each agent's receptive field without requiring additional exploration.

Result: The framework was validated on the R2R benchmark using two representative paradigms (learning-based DUET and zero-shot MapGPT). Vision-sharing enabled models showed substantial performance improvements across both paradigms.

Conclusion: Peer observation sharing significantly benefits VLN performance, establishing a strong foundation for future research in collaborative embodied navigation by overcoming partial observability constraints through multi-agent cooperation.
...
<details>
  <summary>Details</summary>
Motivation: Existing multi-scale medical vision models use explicit frequency decomposition, but ablation studies show these heuristics provide limited benefit while increasing parameters and computation significantly without improving accuracy.

Method: Proposes Clifford-M, a lightweight backbone that replaces feed-forward expansion and frequency-splitting modules with sparse geometric interaction. Uses Clifford-style rolling product to capture alignment and structural variation with linear complexity, enabling efficient cross-scale fusion and self-refinement in a compact dual-resolution architecture.
...
Conclusion: Competitive and efficient fundus diagnosis can be achieved without explicit frequency engineering by designing core feature interaction to capture multi-scale structure directly through geometric approaches.
...
TL;DR: MLLMs suffer from visual representation degradation during language-driven training, compromising visual fidelity for text generation. Proposed Predictive Regularization (PRe) forces intermediate features to predict initial visual features, improving vision-language performance.


<details>
  <summary>Details</summary>
Motivation: While MLLMs excel at vision-language tasks, the cost of their language-driven training on internal visual foundational competence remains unclear. The paper aims to diagnose and address visual representation degradation in MLLMs.

Method: Conduct diagnostic analysis revealing visual degradation in middle layers of LLM. Propose Predictive Regularization (PRe) to force degraded intermediate features to predict initial visual features, maintaining inherent visual attributes of MLLM's internal representations.

Result: Extensive experiments confirm that mitigating visual degradation effectively boosts vision-language performance. The approach underscores the critical importance of fostering robust internal visual representations within MLLMs.

Conclusion: A robust MLLM requires both strong cross-modal reasoning and core visual competence. Predictive Regularization successfully addresses visual representation degradation, enhancing multimodal understanding capabilities.
...
TL;DR: EDC is an efficient optical-SAR semantic segmentation framework that addresses cloud occlusion through compact global modeling and selective fusion, achieving better accuracy with fewer parameters and faster inference.


<details>
  <summary>Details</summary>
Motivation: Cloud occlusion degrades optical remote sensing imagery, and while SAR provides complementary data, existing methods suffer from computational inefficiency and noise propagation when using dense global attention for cross-modal fusion.

Method: Proposes EDC with tri-stream encoder using Carrier Tokens for compact global context modeling, Discrepancy-Conditioned Hybrid Fusion (DCHF) to selectively suppress unreliable regions, and auxiliary cloud removal branch with teacher-guided distillation.

Result: EDC improves mIoU by 0.56% on M3M-CR and 0.88% on WHU-OPT-SAR datasets while reducing parameters by 46.7% and accelerating inference by 1.98× compared to existing methods.

Conclusion: EDC effectively addresses the efficiency-reliability trade-off in cloud-occluded optical-SAR segmentation through compact modeling and selective fusion, offering practical deployment advantages for large-scale remote sensing applications.
...
TL;DR: PlanaReLoc introduces a plane-centric approach for 6-DoF camera relocalization using planar primitives instead of point correspondences, achieving effective performance without textured maps or per-scene training.


<details>
  <summary>Details</summary>
Motivation: Traditional structure-based relocalizers rely on point correspondences, but planar primitives offer richer structural and semantic information as fundamental projective geometry entities, enabling more reliable cross-modal matching in structured environments.

Method: PlanaReLoc uses a deep matcher to associate planar primitives between query images and 3D planar maps in a learned unified embedding space, then solves and refines the 6-DoF pose under a robust framework.

Result: Comprehensive experiments on ScanNet and 12Scenes datasets across hundreds of scenes show superiority of planar primitives in facilitating reliable cross-modal structural correspondences and achieving effective camera relocalization.

Conclusion: Planar primitives enable lightweight 6-DoF camera relocalization without requiring realistically textured/colored maps, pose priors, or per-scene training, demonstrating the effectiveness of plane-centric paradigms for structured environments.
...
TL;DR: EruDiff improves diffusion models' ability to handle implicit prompts requiring world knowledge by refactoring knowledge structures and correcting biases.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image diffusion models struggle with implicit prompts that require deep world knowledge (scientific, cultural, commonsense), producing counter-factual results due to chaotic knowledge organization.

Method: Proposes EruDiff with two key components: 1) Diffusion Knowledge Distribution Matching (DK-DM) to align knowledge distributions of implicit prompts with explicit anchors, and 2) Negative-Only Reinforcement Learning (NO-RL) for fine-grained correction of biases in explicit prompt rendering.
...
Conclusion: EruDiff successfully addresses the fundamental limitation of diffusion models in processing implicit prompts by refactoring knowledge structures and correcting rendering biases, enabling more accurate synthesis of knowledge-intensive images.
...
TL;DR: MERIT is a unified framework for multi-domain RAW image translation using a single model to handle translations across arbitrary camera domains, addressing sensor-specific noise and spectral differences.


<details>
  <summary>Details</summary>
Motivation: RAW images from different camera sensors exhibit substantial domain shifts due to varying spectral responses, noise characteristics, and tone behaviors, making them difficult to use directly in computer vision tasks. Existing methods require training separate domain-specific translators for each source-target pair, which doesn't scale to real-world scenarios with multiple commercial cameras.

Method: MERIT introduces: 1) A sensor-aware noise modeling loss that explicitly aligns signal-dependent noise statistics of generated images with target domain, 2) A conditional multi-scale large kernel attention module for improved context and sensor-aware feature modeling, 3) MDRAW dataset - the first dataset for multi-domain RAW image translation with paired and unpaired RAW captures from five diverse camera sensors.

Result: Extensive experiments show MERIT outperforms prior models with 5.56 dB improvement in quality and 80% reduction in training iterations, demonstrating superior scalability and performance.

Conclusion: MERIT provides the first unified framework for multi-domain RAW image translation that effectively handles domain shifts across different camera sensors while being scalable and efficient, addressing a critical bottleneck in real-world computer vision applications.
...
TL;DR: Dodgersort reduces annotation costs for pairwise comparison labeling by 11-16% while improving ranking reliability through CLIP-based hierarchical pre-ordering, neural ranking, probabilistic ensembles, and information-theoretic pair selection.


<details>
  <summary>Details</summary>
Motivation: Pairwise comparison labeling provides higher inter-rater reliability than conventional classification but requires quadratic annotation costs, making it impractical for large datasets.

Method: Combines CLIP-based hierarchical pre-ordering, neural ranking head, probabilistic ensembles (Elo, BTL, GP), epistemic-aleatoric uncertainty decomposition, and information-theoretic pair selection to intelligently select comparison pairs.

Result: Achieves 11-16% annotation reduction while improving inter-rater reliability in visual ranking tasks across medical imaging, historical dating, and aesthetics. Extracts 5-20× more ranking information per comparison than baselines.

Conclusion: Dodgersort provides Pareto-optimal accuracy-efficiency trade-offs for pairwise comparison labeling, with neural adaptation and ensemble uncertainty being key to its performance gains.
...
TL;DR: GOLDMARK is a standardized benchmarking framework for computational pathology that provides structured data, trained models, and evaluation metrics to enable reproducible comparison of AI methods for biomarker prediction from histopathology images.


<details>
  <summary>Details</summary>
Motivation: Computational pathology lacks standardized intermediate data formats, provenance tracking, checkpointing conventions, and reproducible evaluation metrics needed for clinical-grade deployment of computational biomarkers derived from whole-slide images.

Method: GOLDMARK provides a curated TCGA cohort with clinically actionable biomarker labels, structured intermediate representations (tile coordinate maps, feature embeddings, quality-control metadata), predefined patient splits, trained slide-level models, and evaluation outputs for benchmarking.

Result: Across 33 tumor-biomarker tasks, mean AUROC was 0.689 (TCGA) and 0.630 (MSKCC). For the eight highest-performing tasks, mean AUROCs were 0.831 and 0.801 respectively, showing stable cross-site performance for established morphologic-genomic associations.

Conclusion: GOLDMARK establishes a shared experimental substrate for computational pathology, enabling reproducible benchmarking and direct comparison of methods across datasets and models to advance clinical-grade computational biomarker development.
...
TL;DR: Glove2Hand translates multi-modal sensing glove videos into photorealistic bare hands while preserving physical interaction dynamics, enabling creation of HandSense dataset for enhanced bare-hand applications.


<details>
  <summary>Details</summary>
Motivation: Conventional hand videos lack essential physical information (contact forces, motion signals) and suffer from frequent occlusions, limiting understanding of hand-object interaction for computer vision, robotics, and AR/VR applications.

Method: Introduces Glove2Hand framework with: 1) Novel 3D Gaussian hand model for temporal rendering consistency, 2) Diffusion-based hand restorer for seamless scene integration handling complex hand-object interactions and non-rigid deformations, 3) Creates HandSense dataset with synchronized tactile and IMU signals.

Result: Created HandSense, the first multi-modal HOI dataset featuring glove-to-hand videos with synchronized tactile and IMU signals. Demonstrates significant enhancement of downstream bare-hand applications including video-based contact estimation and hand tracking under severe occlusion.

Conclusion: Glove2Hand successfully addresses limitations of conventional hand videos by translating glove-based multi-modal sensing into photorealistic bare hands while preserving physical dynamics, enabling improved hand-object interaction understanding and applications.
...
TL;DR: Lightweight ensemble of SwinV2-Tiny, DinoBloom-Small, and ConvNeXT-V2-Tiny models achieves excellent performance for white blood cell classification in leukemia diagnosis, addressing challenges of rare cell types and dataset imbalance.


<details>
  <summary>Details</summary>
Motivation: Automating white blood cell classification for leukemia diagnosis is needed to reduce reliance on time-consuming expert pathologist examinations. Current challenges include variations in staining, scanning, inter-patient heterogeneity, and difficulty classifying rare cell types.

Method: Proposed a lightweight ensemble approach using pretrained SwinV2-Tiny, DinoBloom-Small, and ConvNeXT-V2-Tiny models. Used dataset expansion to address class imbalance, trained 3 instantiations of each architecture in stratified 3-fold cross-validation, and aggregated predictions through logit averaging from all 9 models.

Result: The ensemble approach achieves excellent performance on the challenging white blood cell classification dataset, though the model shows weaknesses in confusing similar-looking myelocytes in granulopoiesis and lymphocytes in lymphopoiesis.

Conclusion: A lightweight ensemble of modern vision transformers and convolutional networks provides an effective solution for automated white blood cell classification in leukemia diagnosis, with potential for clinical application despite some remaining challenges in distinguishing similar cell types.
...
TL;DR: FRoG is an efficient and robust framework for dynamic scene reconstruction using 3D Gaussian Splatting, featuring per-Gaussian embedding, coarse-to-fine temporal embedding, depth-guided sampling, and opacity modulation to overcome limitations of existing deformation field methods.


<details>
  <summary>Details</summary>
Motivation: Current deformation field-based methods for dynamic scene reconstruction with 3D Gaussians face challenges: suboptimal rendering speeds, heavy dependence on initial point clouds, vulnerability to local optima in dim scenes, and difficulty with sparse initializations.

Method: FRoG integrates per-Gaussian embedding with coarse-to-fine temporal embedding for early fusion acceleration. It introduces depth- and error-guided sampling to populate canonical field with new Gaussians at optimal positions. Also modulates opacity variations to mitigate local optima in dim scenes.

Result: Achieves accelerated rendering speeds while maintaining state-of-the-art visual quality. Improves detail reconstruction in both static and dynamic regions, enhances robustness against sparse initializations, and improves color fidelity in dim scenes.

Conclusion: FRoG provides an efficient and robust solution for high-quality dynamic scene reconstruction, overcoming key limitations of existing deformation field-based 3D Gaussian methods through novel embedding strategies, sampling techniques, and optimization improvements.
...
TL;DR: Targeted weight re-initialization strategy to restore neural plasticity in transfer learning, improving test accuracy and convergence speed for CNNs and ViTs with minimal overhead.


<details>
  <summary>Details</summary>
Motivation: Pretrained ImageNet models can suffer from loss of neural plasticity during transfer learning, where saturated weights yield insignificant gradients and fail to adapt effectively to downstream tasks, especially atypical datasets.

Method: Proposes targeted weight re-initialization strategy to restore neural plasticity before fine-tuning, compatible with standard transfer learning pipelines.

Result: Both CNNs and vision transformers benefit from the approach, achieving higher test accuracy with faster convergence on multiple image classification benchmarks with negligible computational overhead.

Conclusion: Targeted weight re-initialization effectively addresses loss of plasticity in transfer learning, improving adaptation to downstream tasks while maintaining compatibility with existing pipelines.
...
TL;DR: TAFG-MAN is a latent diffusion framework for LDCT denoising that uses timestep-adaptive frequency-gated conditioning to balance noise suppression and detail preservation.


<details>
  <summary>Details</summary>
Motivation: Low-dose CT reduces radiation exposure but introduces substantial noise and structural degradation, making it difficult to suppress noise without erasing subtle anatomical details.

Method: Combines perceptually optimized autoencoder, conditional latent diffusion restoration in compact latent space, and lightweight Timestep-Adaptive Frequency-Gated (TAFG) conditioning that decomposes features into low/high frequencies and adaptively releases high-frequency guidance during denoising.

Result: Achieves favorable quality-efficiency trade-off against baselines, improves detail preservation and perceptual quality over base variant without TAFG while maintaining same inference cost.

Conclusion: TAFG-MAN effectively balances noise suppression and detail preservation in LDCT denoising through adaptive frequency-gated conditioning mechanism.
...
TL;DR: SG-FSCFormer is a novel framework for Controllable Video Segmentation and Captioning (SegCaptioning) that allows users to guide video understanding through prompts like bounding boxes, generating correlated masks and captions that align with user intent.


<details>
  <summary>Details</summary>
Motivation: Existing video multimodal interpretation methods focus mainly on global comprehension with limited user interaction. There's a need for more controllable systems where users can provide specific prompts to guide video understanding according to their interests.

Method: Proposes SG-FSCFormer with two key components: 1) Prompt-guided Temporal Graph Former that captures user intent through an adaptive prompt adaptor, and 2) Fine-grained Mask-linguistic Decoder that collaboratively predicts caption-mask pairs using Multi-entity Contrastive loss and ensures fine-grained alignment between masks and caption tokens.
...
Conclusion: The proposed Controllable Video Segmentation and Captioning task and SG-FSCFormer framework successfully bridge the gap between user intent and video understanding, enabling precise, user-guided multimodal interpretation of videos through interactive prompts.
...
TL;DR: GraPHFormer is a multimodal architecture that unifies topological and graph-based analysis of neuronal morphology through CLIP-style contrastive learning, achieving SOTA performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current methods analyze neuronal morphology topology or graph structure in isolation, missing the complementary information these views provide about circuit function, development, and disease.

Method: Multimodal architecture with vision branch processing persistence images (unweighted, persistence-weighted, radius-weighted topological densities) via DINOv2-ViT-S, and TreeLSTM encoder capturing geometric/radial attributes from skeleton graphs, both projecting to shared embedding space trained with symmetric InfoNCE loss augmented by persistence-space transformations.

Result: Achieves state-of-the-art performance on five out of six benchmarks (BIL-6, ACT-4, JML-4, N7, M1-Cell, M1-REG), significantly outperforming topology-only, graph-only, and morphometrics baselines. Demonstrates practical utility in discriminating glial morphologies across cortical regions/species and detecting developmental/degenerative signatures.

Conclusion: GraPHFormer successfully unifies topological and graph-based analysis of neuronal morphology through multimodal contrastive learning, providing a powerful framework for comprehensive morphological analysis with applications in neuroscience research and disease detection.
...
TL;DR: Medical VLMs can achieve perfect consistency by ignoring images and relying on text patterns, creating a false reliability trap. A new taxonomy reveals dangerous samples that appear consistent but aren't image-reliant.


<details>
  <summary>Details</summary>
Motivation: Consistency under paraphrase is increasingly used as a proxy for reliability in medical VLMs, but this proxy is fundamentally flawed because models can achieve perfect consistency by ignoring images and relying solely on text patterns.

Method: Introduced a four-quadrant per-sample safety taxonomy evaluating both consistency (stable predictions across paraphrased prompts) and image reliance (predictions that change when image is removed). Evaluated five medical VLM configurations across two chest X-ray datasets (MIMIC-CXR, PadChest).

Result: LoRA fine-tuning dramatically reduces flip rates but shifts majority of samples into Dangerous quadrant (consistent but not image-reliant). LLaVA-Rad Base achieved 1.5% flip rate while 98.5% of samples were Dangerous. Dangerous samples exhibit high accuracy (up to 99.6%) and low entropy, making them invisible to standard confidence-based screening.

Conclusion: Consistency checks alone are insufficient for reliability assessment. Deployment evaluations must pair consistency checks with text-only baselines to expose the false reliability trap where models ignore images while appearing consistent.
...
TL;DR: SkinCLIP-VL is a resource-efficient framework that adapts foundation models for trustworthy skin cancer diagnosis, addressing computational costs, data scarcity, and black-box nature issues in dermatology VLMs.


<details>
  <summary>Details</summary>
Motivation: The deployment of vision-language models in dermatology faces three main challenges: high computational costs, extreme data scarcity, and the black-box nature of deep learning, which hinder clinical trust and practical application.

Method: Uses frozen CLIP encoder with lightweight, quantized Qwen2.5-VL via LoRA adaptation. Introduces Consistency-aware Focal Alignment (CFA) Loss to align visual regions with clinical semantics under long-tailed distributions, combining focal re-weighting, semantic alignment, and calibration.

Result: Surpasses 13B-parameter baselines by 4.3-6.2% in accuracy on ISIC and Derm7pt benchmarks with 43% fewer parameters. Blinded expert evaluation and out-of-distribution testing confirm visually grounded rationales enhance clinical trust compared to traditional saliency maps.

Conclusion: SkinCLIP-VL provides a resource-efficient solution for trustworthy skin cancer diagnosis that addresses the computational, data scarcity, and interpretability challenges in dermatology AI, with demonstrated superior performance and enhanced clinical trust.
...
TL;DR: LPNSR is a prior-enhanced efficient diffusion framework for image super-resolution that achieves state-of-the-art perceptual quality with only 4-step inference by optimizing intermediate noise and initial upsampling.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based SR methods face a trade-off between efficiency and quality. Residual-shifting diffusion achieves 4-step inference but suffers from performance degradation due to suboptimal random Gaussian noise in intermediate steps (causing error accumulation) and insufficient LR prior guidance, plus initialization bias from naive bicubic upsampling.

Method: 1) Mathematically derive closed-form analytical solution for optimal intermediate noise in residual-shifting diffusion; 2) Design LR-guided multi-input-aware noise predictor to replace random Gaussian noise, embedding LR structural priors while preserving efficient residual-shifting; 3) Use high-quality pre-upsampling network to mitigate initial bias and optimize diffusion starting point; 4) End-to-end optimization with compact 4-step trajectory.

Result: LPNSR achieves state-of-the-art perceptual performance on both synthetic and real-world datasets with only 4-step inference, without relying on large-scale text-to-image priors.

Conclusion: LPNSR successfully addresses the efficiency-quality trade-off in diffusion-based SR by optimizing intermediate noise and initial upsampling, enabling high-quality 4-step inference while maintaining computational efficiency.
...
TL;DR: SpatialFly is a geometry-guided spatial representation framework for UAV visual-language navigation that addresses the 2D-3D representation mismatch without explicit 3D reconstruction, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: UAV visual-language navigation (VLN) in complex 3D environments is challenging due to the structural representation mismatch between 2D visual perception and 3D trajectory decision space, which limits spatial reasoning capabilities.

Method: SpatialFly introduces a geometry-guided 2D representation alignment mechanism with two key modules: 1) geometric prior injection module that injects global structural cues into 2D semantic tokens, and 2) geometry-aware reparameterization module that aligns 2D semantic tokens with 3D geometric tokens through cross-modal attention with gated residual fusion.

Result: SpatialFly consistently outperforms state-of-the-art UAV VLN baselines across both seen and unseen environments, reducing Navigation Error (NE) by 4.03m and improving Success Rate (SR) by 1.27% over the strongest baseline on unseen Full split. It produces trajectories with better path alignment and smoother, more stable motion.

Conclusion: The proposed geometry-guided spatial representation framework effectively bridges the 2D-3D representation gap for UAV VLN without requiring explicit 3D reconstruction, demonstrating superior navigation performance and trajectory quality.
...
TL;DR: The paper proposes an adversarial evaluation framework using LLMs to generate clinically plausible prompt variations to test the robustness of medical vision-language models in ultrasound analysis, revealing significant vulnerabilities that need addressing for safe clinical use.


<details>
  <summary>Details</summary>
Motivation: Ultrasound analysis is operator-dependent, motivating AI assistance. While vision-language models show promise in medical imaging, they have trustworthiness concerns, particularly adversarial robustness. Since Med-VLMs use natural-language instructions, prompt formulation becomes a realistic vulnerability point where small variations can significantly affect outputs.

Method: Proposes a scalable adversarial evaluation framework that uses a large language model (LLM) to generate clinically plausible adversarial prompt variants through "humanized" rewrites and minimal edits that mimic routine clinical communication. Tests this on ultrasound multiple-choice question answering benchmarks to systematically assess SOTA Med-VLMs' vulnerability.

Result: The framework reveals significant vulnerabilities in Med-VLMs to adversarial prompt attacks. The study examines how attacker LLM capacity influences attack success, analyzes the relationship between attack success and model confidence, and identifies consistent failure patterns across models, highlighting realistic robustness gaps.

Conclusion: The research demonstrates critical adversarial robustness issues in medical vision-language models that must be addressed for safe clinical translation. The proposed evaluation framework provides a scalable method to assess these vulnerabilities using clinically plausible prompt variations.
...
TL;DR: Two-stage temporal action localization framework for driver monitoring using VideoMAE feature extraction with Augmented Self-Mask Attention detector and SPPF module, showing trade-off between accuracy and computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Current temporal action localization techniques struggle to balance accuracy with computational efficiency for hazardous driving behavior identification from in-cabin video streams, which is essential for road safety and traffic violation detection.

Method: Two-stage pipeline combining VideoMAE-based feature extraction with Augmented Self-Mask Attention (AMA) detector, enhanced by Spatial Pyramid Pooling-Fast (SPPF) module to capture multi-scale temporal features for driver monitoring scenarios.

Result: ViT-Giant backbone achieves 88.09% Top-1 test accuracy but with high computational cost (1584.06 GFLOPs/segment), while ViT-based variant achieves 82.55% accuracy with significantly lower cost (101.85 GFLOPs/segment). SPPF integration consistently improves performance, with ViT-Giant + SPPF achieving peak mAP of 92.67%.

Conclusion: The framework demonstrates a clear trade-off between model capacity and efficiency, offering both high-accuracy (ViT-Giant) and lightweight (ViT-based) configurations suitable for different driver monitoring applications like transportation safety checkpoints and fleet management systems.
...
TL;DR: SGAD-SLAM introduces pixel-aligned Gaussians with adjustable positions along rays and Gaussian depth distributions for fast tracking, improving rendering quality and system scalability over existing 3DGS SLAM methods.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian Splatting methods for RGBD SLAM use either 3D Gaussians (too flexible, slow convergence) or view-tied 3D Gaussians (too limited in movements, poor rendering quality). There's a need for a representation that balances flexibility with rendering quality while maintaining system scalability.

Method: 1) Uses pixel-aligned Gaussians that can adjust their positions along rays to maximize rendering quality while simplifying Gaussians for scalability. 2) Models depth distribution around each pixel as a Gaussian distribution for fast frame-to-scene alignment in tracking.

Result: The method shows advantages over latest methods in view rendering quality, camera tracking accuracy, runtime efficiency, and storage complexity on widely used benchmarks.

Conclusion: SGAD-SLAM successfully addresses the limitations of existing 3DGS SLAM approaches by introducing adaptive pixel-aligned Gaussians and Gaussian depth distributions, achieving better performance across multiple metrics while maintaining system scalability.
...
TL;DR: LRHPerception is a real-time monocular perception system for autonomous driving that combines end-to-end learning efficiency with detailed local mapping, achieving 29 FPS processing speed.


<details>
  <summary>Details</summary>
Motivation: Camera-based autonomous driving systems often prioritize effectiveness over computational efficiency. There's a need for real-time perception systems that balance both performance and speed for practical deployment.

Method: LRHPerception uses single-view camera video to interpret the environment, combining end-to-end learning efficiency with local mapping detail. It processes monocular images into a five-channel tensor (RGB + road segmentation + depth estimation) with integrated object detection and trajectory prediction.

Result: The system achieves real-time processing at 29 FPS on a single GPU, representing a 555% speedup over the fastest mapping-based approach, with strong performance in object tracking, road segmentation, and depth estimation.

Conclusion: LRHPerception successfully addresses the computational efficiency gap in camera-based autonomous driving perception while maintaining strong performance, offering a practical real-time solution for monocular perception tasks.
...
TL;DR: 2Xplat introduces a two-expert framework for pose-free 3D Gaussian Splatting that separates geometry estimation from appearance modeling, achieving state-of-the-art performance in under 5K training iterations.


<details>
  <summary>Details</summary>
Motivation: Current pose-free 3DGS methods use unified monolithic architectures that entangle geometric reasoning and appearance modeling, which may be suboptimal for high-fidelity 3DGS generation. The authors propose that separating these tasks could lead to better performance.

Method: 2Xplat uses a two-expert design: a dedicated geometry expert first predicts camera poses from uncalibrated multi-view images, then explicitly passes these poses to a powerful appearance expert that synthesizes 3D Gaussian representations.

Result: The proposed approach substantially outperforms prior pose-free feed-forward 3DGS methods and achieves performance comparable to state-of-the-art posed methods, all in fewer than 5,000 training iterations.

Conclusion: The success of 2Xplat challenges the prevailing unified paradigm for 3DGS and suggests modular design principles offer advantages for complex 3D geometric estimation and appearance synthesis tasks.
...
TL;DR: NoOVD: A novel training framework for open-vocabulary object detection that uses self-distillation from frozen VLMs to address misclassification of novel objects as background during training and inference.


<details>
  <summary>Details</summary>
Motivation: Current OVD methods suffer from a significant gap between training and testing phases where RPN and RoI heads misclassify unlabeled novel-category objects as background, causing proposals to be filtered out prematurely and weakening novel-category detection performance.

Method: Proposes NoOVD framework with two key components: 1) K-FPN that leverages pretrained VLM knowledge to guide novel object discovery and enable knowledge distillation without additional data, and 2) R-RPN that adjusts proposal confidence scores during inference to improve novel object recall.
...
Conclusion: The NoOVD framework effectively addresses the training-testing gap in OVD by preventing forced alignment of novel objects with background and improving recall through innovative self-distillation and score adjustment mechanisms.
...
TL;DR: Proposes a Collaborative Teacher Semantic Segmentation Framework for forward-looking sonar images using multi-teacher collaboration and cross-teacher reliability assessment to handle noisy pseudo-labels in limited labeled data scenarios.


<details>
  <summary>Details</summary>
Motivation: Forward-looking sonar images suffer from severe speckle noise, low texture contrast, acoustic shadows, and geometric distortions, making traditional teacher-student frameworks ineffective for semantic segmentation with extremely limited labeled data.

Method: Introduces a multi-teacher collaborative mechanism with one general teacher and multiple sonar-specific teachers, using alternating guidance strategy. Also designs a cross-teacher reliability assessment mechanism to dynamically quantify pseudo-label reliability by evaluating consistency and stability across multiple views and teachers.

Result: Achieves 5.08% improvement in mIoU compared to other state-of-the-art approaches on the FLSMD dataset when only 2% of data is labeled.

Conclusion: The proposed framework effectively addresses challenges in sonar image segmentation by enabling comprehensive feature modeling through multi-teacher collaboration and mitigating noisy pseudo-label impact through reliability assessment, demonstrating superior performance in limited labeled data scenarios.
...
TL;DR: CoVFT framework addresses visual preference conflicts in MLLMs by incorporating multimodal context into visual fine-tuning, achieving SOTA performance with better stability than existing methods.


<details>
  <summary>Details</summary>
Motivation: Despite progress in MLLMs, there's inconsistency about whether to fine-tune or freeze vision encoders. Existing visual fine-tuning methods fail to consistently outperform frozen baselines due to visual preference conflicts caused by context-agnostic vision encoders.

Method: Proposes Context-aware Visual Fine-tuning (CoVFT) framework with Context Vector Extraction (CVE) and Contextual Mixture-of-Experts (CoMoE) modules. CVE extracts multimodal context, while CoMoE decomposes conflicting optimization signals to enable stable, context-sensitive visual updates.

Result: Extensive experiments on 12 multimodal benchmarks show CoVFT achieves state-of-the-art performance with superior stability. Fine-tuning a 7B MLLM with CoVFT surpasses average performance of its 13B counterpart, revealing untapped potential in visual encoder optimization.

Conclusion: CoVFT successfully addresses visual preference conflicts in MLLMs by incorporating multimodal context into visual adaptation, demonstrating that proper visual fine-tuning can unlock substantial performance gains previously hindered by optimization instability.
...
TL;DR: TextCSP: A hierarchical text-guided framework for brain tumor segmentation that uses radiological descriptions to improve segmentation of ambiguous tumor sub-regions (WT, TC, ET) through text-modulated soft cascade architecture.


<details>
  <summary>Details</summary>
Motivation: Brain tumor segmentation is challenging due to ambiguous visual boundaries between the three standard sub-regions (WT, TC, ET). Existing multimodal approaches compress radiological reports into single global embeddings, overlooking distinct clinical characteristics of each sub-region.

Method: Proposes TextCSP with three novel components: 1) Text-modulated soft cascade decoder that predicts WT→TC→ET in coarse-to-fine anatomical hierarchy, 2) Sub-region-aware prompt tuning using learnable soft prompts with LoRA-adapted BioBERT encoder for specialized text representations, 3) Text-semantic channel modulators that convert text representations into channel-wise refinement signals for feature emphasis.
...
Conclusion: TextCSP effectively leverages radiological descriptions through hierarchical text guidance to improve brain tumor segmentation by addressing the distinct clinical characteristics of each tumor sub-region and their anatomical containment relationships.
...
TL;DR: Latent diffusion models suffer from sensitivity to sampling perturbations due to overly compact latent spaces from β-VAE tokenizers. The paper proposes a Variance Expansion loss to create robust latent spaces while maintaining reconstruction fidelity.


<details>
  <summary>Details</summary>
Motivation: Previous research focused on reconstruction accuracy and semantic alignment in latent diffusion models, but overlooked robustness to sampling perturbations. The authors observed that β-VAE-based tokenizers produce overly compact latent manifolds that are highly sensitive to stochastic perturbations during diffusion sampling, leading to visual degradation in generated images.

Method: Proposes a Variance Expansion loss that counteracts variance collapse in latent spaces. Leverages adversarial interplay between reconstruction and variance expansion to achieve adaptive balance - preserving reconstruction accuracy while improving robustness to stochastic sampling perturbations.

Result: Extensive experiments demonstrate the approach consistently enhances generation quality across different latent diffusion architectures. Confirms that robustness in latent space is a key missing ingredient for stable and faithful diffusion sampling.

Conclusion: Robustness to sampling perturbations is a critical factor in latent diffusion models alongside reconstruction accuracy. The proposed Variance Expansion loss effectively addresses this issue by creating latent spaces that are both faithful to reconstruction and robust to stochastic perturbations during sampling.
...
TL;DR: DGRNet improves brain tumor segmentation by combining multi-view uncertainty estimation with text-guided refinement using radiology reports.


<details>
  <summary>Details</summary>
Motivation: Current deep learning approaches lack reliable uncertainty quantification and under-utilize valuable information from radiology reports, both of which are critical for clinical decision-making in ambiguous tumor regions.

Method: DGRNet uses four lightweight view-specific adapters on a shared encoder-decoder to generate diverse predictions and uncertainty estimates in a single forward pass. It identifies high-uncertainty regions via disagreement maps and refines them using clinical reports. A diversity-preserving training strategy prevents view collapse.
...
Conclusion: DGRNet successfully addresses both uncertainty quantification and text-guided refinement limitations, offering improved segmentation accuracy and clinically meaningful uncertainty estimates for brain tumor segmentation.
...
TL;DR: A multitask learning framework that jointly predicts thyroid nodule segmentation masks and TI-RADS risk categories, using clinical guidance and adversarial gradient regularization to handle annotator variability.


<details>
  <summary>Details</summary>
Motivation: Thyroid ultrasound analysis suffers from inconsistent supervision due to radiologist variability in both contouring style and TI-RADS risk grading, which degrades standard learning pipelines.

Method: Proposes a clinically guided multitask framework that jointly predicts nodule masks and TI-RADS categories. Uses TI-RADS-aligned radiomics to guide risk prediction while preserving deep features. Introduces RLAR (representation-level adversarial gradient regularizer) to explicitly manage gradient competition between tasks by penalizing excessive angular alignment between task-specific adversarial directions.
...
Conclusion: The proposed framework addresses clinical workflow needs by jointly handling segmentation and risk assessment with explicit management of annotator variability, offering improved performance over standard approaches.
...
TL;DR: PATrack introduces progressive adaptation with modality-dependent, modality-entangled, and task-level adapters to effectively adapt RGB pre-trained models for multi-modal tracking tasks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-modal trackers using parameter-efficient fine-tuning overlook advanced adaptations for RGB pre-trained models, failing to properly modulate specific modalities, cross-modal interactions, and prediction heads.

Method: Progressive Adaptation for Multi-Modal Tracking (PATrack) with three adapters: 1) modality-dependent adapter enhances intra-modal features via frequency decomposition, 2) modality-entangled adapter enables cross-modal interactions via cross-attention, 3) task-level adapter adapts prediction head to fused information.
...
Conclusion: PATrack's unified framework with progressive adaptation effectively bridges the gap in adapting RGB pre-trained networks to multi-modal data, achieving superior tracking performance across diverse multi-modal tasks.
...
TL;DR: Free Sinewich is a parameter-efficient multi-task learning framework that uses frequency switching to enable near-zero-cost weight modulation for different tasks, achieving state-of-the-art performance with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: Current parameter-efficient fine-tuning (PEFT) methods are mostly limited to single-task adaptation, but multi-task learning (MTL) requires efficient ways to adapt a single model to multiple tasks without excessive parameter overhead.

Method: Introduces Sine-AWB (Sinewich) layers that combine low-rank factors and convolutional priors into a single kernel, modulated elementwise by sinusoidal transformations. A lightweight Clock Net produces bounded frequencies to stabilize modulation during training, enabling frequency-based parameter sharing across tasks.
...
TL;DR: MLLMs show strong single-view spatial reasoning but fail to maintain stable spatial representations under counterfactual viewpoint changes, with systematic degradation in relational consistency.


<details>
  <summary>Details</summary>
Motivation: While multimodal large language models (MLLMs) perform well on single-view spatial reasoning tasks, it's unclear whether they maintain stable spatial state representations when faced with hypothetical viewpoint changes. The researchers want to test if MLLMs can maintain relational consistency under counterfactual camera transformations without actual image re-rendering.

Method: Created a controlled diagnostic benchmark with 100 synthetic scenes and 6,000 relational queries to evaluate relational consistency under hypothetical camera orbit transformations. Measured viewpoint consistency, 360° cycle agreement, and relational stability over sequential transformations. Tested multiple input representations: visual input, textual bounding boxes, and structured scene graphs.

Result: Despite high single-view accuracy, state-of-the-art MLLMs show systematic degradation under counterfactual viewpoint changes, with frequent violations of cycle consistency and rapid decay in relational stability. Increasing representational structure (from visual input to textual bounding boxes to structured scene graphs) improves stability.

Conclusion: Single-view spatial accuracy overestimates the robustness of induced spatial representations in MLLMs. Representation structure plays a critical role in counterfactual spatial reasoning, with more structured representations leading to better stability under viewpoint changes.
...
TL;DR: LiFR-Seg enables dense semantic segmentation at any time using only a single past RGB frame and event data, achieving performance nearly identical to high-frame-rate systems.


<details>
  <summary>Details</summary>
Motivation: Standard cameras' low frame rates create perceptual gaps in dynamic environments, limiting dense semantic segmentation. Event cameras offer high temporal resolution but present challenges in robust feature propagation from sparse, noisy event data.

Method: LiFR-Seg propagates deep semantic features through time using uncertainty-aware warping guided by event-driven motion fields with learned confidence, plus a temporal memory attention module for coherence in dynamic scenes.

Result: Achieves 73.82% mIoU on DSEC dataset, statistically indistinguishable (within 0.09%) from high-frame-rate upper-bound systems that have full access to target frames.

Conclusion: Presents an efficient paradigm for robust, high-frame-rate perception using low-frame-rate hardware by combining single RGB frames with event data streams.
...
TL;DR: ReDiffuse is a rotation-equivariant diffusion model for multi-focus image fusion that preserves geometric structure consistency by embedding rotation equivariance into diffusion networks.


<details>
  <summary>Details</summary>
Motivation: Diffusion models face challenges in multi-focus image fusion because defocus blur can warp symmetric geometric structures (textures, edges), causing artifacts. Rotation equivariance is needed to preserve original orientation and structural consistency of geometric patterns.

Method: Constructs basic diffusion architectures to achieve end-to-end rotation equivariance, with rigorous theoretical analysis of intrinsic equivariance error to validate embedding of equivariance structures.
...
Conclusion: ReDiffuse successfully addresses the structural preservation challenge in diffusion-based MFIF by embedding rotation equivariance, leading to improved fusion quality with better geometric pattern consistency.
...
TL;DR: MCM introduces multi-cluster memory organization for practical test-time adaptation, replacing single-cluster designs to better match the multi-modal nature of real-world test streams.


<details>
  <summary>Details</summary>
Motivation: Existing TTA methods use single-cluster memory designs that are fundamentally mismatched to practical test streams, which analysis shows are inherently multi-modal with optimal mixture components far exceeding one.

Method: Proposes Multi-Cluster Memory (MCM) with three mechanisms: descriptor-based cluster assignment using pixel-level statistical descriptors, Adjacent Cluster Consolidation to bound memory usage by merging similar temporally adjacent clusters, and Uniform Cluster Retrieval to ensure balanced supervision across all modes.

Result: MCM achieves consistent improvements across 12 configurations on CIFAR-10-C, CIFAR-100-C, ImageNet-C, and DomainNet, with gains up to 5.00% on ImageNet-C and 12.13% on DomainNet. Gains scale with distributional complexity, and GMM-based diagnostics confirm MCM maintains near-optimal distributional balance, entropy, and mode coverage.

Conclusion: Memory organization is a key design axis for practical test-time adaptation, and multi-cluster designs like MCM effectively address the structural gap between single-cluster memory and the multi-modal nature of real-world test streams.
...
TL;DR: MS-CustomNet is a diffusion-based framework for multi-subject image generation that enables zero-shot integration of multiple user-provided objects with explicit control over hierarchical arrangements and spatial relationships.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image diffusion models struggle with customizing scenes containing multiple distinct subjects while providing explicit user control over compositional structure and precise spatial relationships between subjects.

Method: MS-CustomNet introduces a novel framework for multi-subject customization that allows zero-shot integration of multiple user-provided objects. It enables users to explicitly define hierarchical arrangements and spatial placements. The approach preserves individual subject identities while learning user-specified inter-subject compositions. The authors also created the MSI dataset from COCO for training on complex multi-subject compositions.

Result: The method achieves a DINO-I score of 0.61 for identity preservation and a YOLO-L score of 0.94 for positional control in multi-subject customization tasks, demonstrating superior capability in generating high-fidelity images with precise, user-directed multi-subject compositions and spatial control.

Conclusion: MS-CustomNet offers enhanced, fine-grained control over multi-subject image generation, addressing the limitations of existing methods by providing explicit user-defined control over compositional structure and spatial relationships between multiple subjects.
...
TL;DR: RLVC: A reinforcement learning framework with visual cues for generative zero-shot learning that improves feature synthesis and achieves state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Current generative ZSL methods produce task-agnostic features and struggle to distinguish semantically similar but visually distinct classes, leading to degraded performance.

Method: Proposes RLVC: an outcome-reward reinforcement learning framework with visual cues. Uses RL to self-evolve the generative model, outcome-based rewards for task-relevant features, class-wise visual cues for alignment with visual prototypes, and a novel cold-start training strategy.

Result: Achieves state-of-the-art results on three prevalent ZSL benchmarks with a 4.7% performance gain compared to previous methods.

Conclusion: RLVC effectively addresses limitations of current generative ZSL by improving feature synthesis through reinforcement learning and visual cues, leading to significant performance improvements.
...
TL;DR: Training-free 3D indoor scene reconstruction from sparse unposed RGB images using point cloud reconstruction, instance-aware 3D representation, and diffusion-based rendering without scene-specific optimization.


<details>
  <summary>Details</summary>
Motivation: Traditional radiance field methods require dense views and per-scene optimization, which is computationally expensive. The authors aim to create an efficient, training-free system for 3D scene reconstruction from sparse, unposed images that supports editing without retraining.

Method: Three key components: 1) Robust point cloud reconstruction with warping-based anomaly removal; 2) Warping-guided 2D-to-3D instance lifting that propagates segmentation masks to create instance-aware 3D representation; 3) Novel rendering that projects point clouds into new views and refines with 3D-aware diffusion model to compensate for missing geometry.

Result: Achieves high-fidelity 3D reconstruction and rendering from sparse unposed images without training or pose preprocessing. Supports object-level scene editing (like instance removal) by modifying only the point cloud, enabling consistent edited views without retraining.

Conclusion: Establishes a new direction for efficient, editable 3D content generation that doesn't rely on scene-specific optimization, demonstrating the potential of training-free approaches for 3D scene understanding and manipulation.
...
TL;DR: GIDE introduces a training-free image editing framework for Diffusion Large Language Models that enables precise editing while preserving background structure through discrete noise inversion and a three-stage pipeline.


<details>
  <summary>Details</summary>
Motivation: Diffusion Large Language Models (DLLMs) have strong multi-modal generation capabilities but struggle with precise, training-free image editing due to discrete tokenization that prevents standard noise inversion techniques, leading to structural degradation during editing.

Method: GIDE features a Discrete Noise Inversion mechanism that captures latent noise patterns in discrete token space for high-fidelity reconstruction. The framework decomposes editing into three stages: grounding, inversion, and refinement, supporting various editing instructions (text, point, box) while preserving unedited background.

Result: GIDE significantly outperforms prior training-free methods on the new GIDE-Bench benchmark (805 compositional editing scenarios), improving Semantic Correctness by 51.83% and Perceptual Quality by 50.39%. It also shows consistent gains over trained baselines on ImgEdit-Bench and achieves photorealistic consistency comparable to leading models.

Conclusion: GIDE successfully bridges the gap in training-free image editing for DLLMs by introducing discrete noise inversion and a structured editing pipeline, enabling precise multi-modal editing while maintaining background integrity, as validated by comprehensive benchmarks.
...
TL;DR: DSCSNet: A deep-unfolded network combining ADMM with learnable parameters for unmixing clustered infrared small targets, using ℓ₁-norm sparsity constraints and dynamic thresholding to balance sparsity guarantees and scene adaptability.


<details>
  <summary>Details</summary>
Motivation: Distant clustered infrared small targets appear as mixed spots due to optical limitations. The CSOU task (recovering individual targets from spots) is highly ill-posed. Existing methods struggle to balance rigorous sparsity guarantees (model-driven) and dynamic scene adaptability (data-driven).

Method: Proposes Dynamic Sparse Compressed Sensing Network (DSCSNet), a deep-unfolded network coupling ADMM with learnable parameters. Key innovations: 1) Embed ℓ₁-norm sparsity constraint in ADMM's auxiliary variable update (replacing traditional ℓ₂-norm), preserving discrete energy peaks; 2) Self-attention-based dynamic thresholding mechanism adaptively adjusts sparsification intensity using iterative information; 3) Joint end-to-end optimization across three ADMM iterative steps.

Result: Extensive experiments on synthetic infrared dataset CSIST-100K show DSCSNet outperforms state-of-the-art methods in key metrics: CSO-mAP and sub-pixel localization error.

Conclusion: DSCSNet retains compressed sensing physical logic while achieving robust sparsity induction and scene adaptability, enhancing unmixing accuracy and generalization in complex infrared scenarios.
...
TL;DR: A prompt-free, boundary-aware instance segmentation framework for microscopy videos that uses signed distance functions (SDFs) to separate touching cells without manual prompting.


<details>
  <summary>Details</summary>
Motivation: Separating touching or overlapping cell instances in microscopy videos remains challenging despite advances in foundation models like SAM, which still require extensive prompting for dense microscopy scenes.

Method: Proposes a prompt-free framework that predicts signed distance functions (SDFs) instead of binary masks, uses learned sigmoid mapping to convert SDFs into probability maps, and employs a unified Modified Hausdorff Distance (MHD) loss combining region- and boundary-based terms.

Result: Demonstrates improved boundary accuracy and instance-level performance on both public and private high-throughput microscopy datasets compared to recent SAM-based and foundation-model approaches.

Conclusion: The boundary-aware instance segmentation framework with SDFs provides effective prompt-free separation of adjacent cell instances in dense microscopy scenes, outperforming existing foundation model approaches.
...
TL;DR: JANUS is a lightweight jailbreak framework that optimizes structured prompt distributions using black-box rewards from T2I systems and safety filters, outperforming existing methods on modern text-to-image models.


<details>
  <summary>Details</summary>
Motivation: Current T2I models remain vulnerable to generating harmful/NSFW content despite safety filters. Existing jailbreak attacks either use proxy-loss optimization instead of true end-to-end objectives, or rely on large-scale, costly RL-trained generators.

Method: JANUS formulates jailbreak as optimizing a structured prompt distribution under black-box, end-to-end rewards from T2I systems and safety filters. It replaces high-capacity generators with a low-dimensional mixing policy over two semantically anchored prompt distributions for efficient exploration while preserving target semantics.

Result: Outperforms state-of-the-art jailbreak methods on modern T2I models, improving ASR-8 from 25.30% to 43.15% on Stable Diffusion 3.5 Large Turbo with consistently higher CLIP and NSFW scores. Succeeds across both open-source and commercial models.

Conclusion: The findings expose structural weaknesses in current T2I safety pipelines and motivate stronger, distribution-aware defenses against jailbreak attacks.
...
TL;DR: Positional Seg-CFT enables localized counterfactual image generation by subdividing anatomical structures into regional segments for finer spatial control.


<details>
  <summary>Details</summary>
Motivation: Existing counterfactual image generation methods are limited to global interventions or require tedious manual mask creation, lacking localized structural changes for realistic disease modeling.
...
Conclusion: Positional Seg-CFT provides a practical solution for generating localized counterfactual images with anatomical coherence, advancing disease modeling capabilities.
...
TL;DR: Proposes a continual learning framework with Grouped Knowledge Preservation and Grouped Sharpness Aware modules to address tail performance degradation in long-tail distributions by promoting shared flat minima.


<details>
  <summary>Details</summary>
Motivation: The paper addresses the challenge of balancing performance trade-offs on long-tail data distributions, identifying "tail performance degradation" as the core problem where models overfit head classes while forgetting tail classes. The authors observe that different classes converge to divergent points in the loss landscape, especially when settling into sharp, non-robust minima rather than shared flat solutions beneficial for all classes.

Method: Proposes a continual learning inspired framework with two key components: 1) Grouped Knowledge Preservation module to memorize group-specific convergence parameters and promote convergence toward shared solutions (avoiding inefficient per-class parameter preservation), and 2) Grouped Sharpness Aware module to explicitly address loss landscape geometry and seek flatter minima. The framework requires neither external training samples nor pre-trained models.

Result: Extensive experiments on four benchmarks demonstrate significant performance gains over state-of-the-art methods. The approach effectively addresses tail performance degradation in long-tail distributions.

Conclusion: The proposed framework successfully mitigates tail performance degradation by promoting convergence to shared flat minima through grouped knowledge preservation and sharpness-aware optimization, offering a practical solution without requiring external data or pre-trained models.
...
TL;DR: SYSU-HiRoads is a large hierarchical road dataset with multi-level annotations, and RoadReasoner is a vision-language-geometry framework for automatic multi-grade road mapping from remote sensing imagery.


<details>
  <summary>Details</summary>
Motivation: There's a need for automated transport infrastructure mapping and road inventory updating, requiring comprehensive datasets and frameworks that can handle road segmentation, topology reconstruction, and hierarchy classification simultaneously.

Method: RoadReasoner uses frequency-sensitive cues and multi-scale context for robust feature representation, and performs hierarchy inference at skeleton-segment level with geometric descriptors and geometry-aware textual prompts queried by vision-language models.

Result: RoadReasoner achieves 72.6% OA, 64.2% F1 score, and 60.6% SegAcc on SYSU-HiRoads and CHN6-CUG datasets, surpassing state-of-the-art road extraction baselines.

Conclusion: The proposed dataset and framework enable accurate, semantically consistent road hierarchy mapping and will support automated transport infrastructure management applications.
...
TL;DR: TPC-268 is the first plant counting benchmark with taxonomy integration, featuring 10K images, 678K point annotations across 268 plant categories and 242 species, spanning canopy to microscopy scales for class-agnostic counting evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing counting methods focus on crowds/traffic but lack fine-grained taxonomy-aware plant counting capabilities. Plants have unique challenges: nonrigid morphologies, appearance variations across growth stages/environments, and hierarchical taxonomic structure.

Method: Created TPC-268 dataset with instance-level point annotations, Linnaean taxonomic labels (kingdom->species), and organ categories. Dataset includes 10,000 images with 678,050 annotations covering 268 countable plant categories across 242 species in Plantae and Fungi, spanning multiple observation scales.

Result: Established a comprehensive benchmark for class-agnostic counting with taxonomy-consistent, scale-aware data splits. Benchmarked state-of-the-art regression- and detection-based CAC approaches on this biologically grounded testbed.

Conclusion: TPC-268 fills the gap in fine-grained plant counting by providing a taxonomy-aware, multi-scale benchmark that captures biodiversity and hierarchical structure, advancing research in biologically grounded class-agnostic counting.
...
TL;DR: QMoP is a flexible framework that adaptively compresses visual tokens in multimodal LLMs using three collaborative branches coordinated by a query-guided router, achieving better performance with reduced computational costs.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs face severe computational and memory bottlenecks due to the large number of visual tokens compared to textual tokens. Existing methods use fixed heuristics for visual token compression, limiting adaptability across diverse scenarios.

Method: Proposes Query Guided Mixture-of-Projector (QMoP) with three branches: pooling-based for global semantics, resampler for high-level semantics, and pruning-based for fine-grained detail. Uses Query Guided Router (QGR) to dynamically select/weight branch outputs based on visual input and textual queries, with Mixture-of-Experts-style fusion. Also introduces VTCBench benchmark for evaluating visual token compression.

Result: Extensive experiments show QMoP outperforms strong baselines while delivering significant savings in memory, computation, and inference time.

Conclusion: QMoP provides an effective adaptive framework for visual token compression in multimodal LLMs, overcoming limitations of fixed heuristics and achieving better performance with reduced computational overhead.
...
TL;DR: DepthTCM: A physics-aware end-to-end framework for depth map compression using multiwavelength depth encoding and Transformer-CNN mixed neural networks, achieving high compression rates with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Depth maps require efficient compression for storage and transmission, but traditional methods may not optimally preserve depth information. The paper aims to develop a physics-aware approach that leverages depth-specific properties for better compression.

Method: DepthTCM first converts high-bit depth maps to 3-channel images using multiwavelength depth encoding inspired by sinusoidal fringe pattern profilometry. It then quantizes this representation to 4 bits per channel and compresses it using a learned codec combining convolutional and Transformer layers.

Result: On Middlebury 2014, DepthTCM achieves 0.307 bpp while preserving 99.38% accuracy (comparable to lossless PNG). It shows practical efficiency with 41.48 ms encoder and 47.45 ms decoder times on ScanNet++ iPhone RGB-D subset. 4-bit quantization reduces bitrate by 66% with minimal quality loss.

Conclusion: DepthTCM demonstrates effective depth map compression by combining physics-aware encoding with modern neural network architectures, achieving high compression ratios while maintaining reconstruction quality comparable to lossless methods.
...
TL;DR: Vision Transformer enhanced with colormap features achieves 98.90% accuracy on BRISC2025 brain tumor MRI dataset, outperforming CNN baselines.


<details>
  <summary>Details</summary>
Motivation: Accurate brain tumor classification from MRI is crucial for early diagnosis and treatment planning. Existing methods may not fully capture important structural and intensity variations in MRI scans.

Method: Proposes a deep learning framework combining Vision Transformers (ViT) with colormap-based feature representation. Uses transformer architecture to capture long-range dependencies while incorporating color mapping to emphasize structural and intensity variations in MRI scans.

Result: Achieves 98.90% classification accuracy on BRISC2025 dataset (4 classes: glioma, meningioma, pituitary tumor, non-tumor). Outperforms ResNet50, ResNet101, and EfficientNetB2 baselines. Shows strong generalization with 99.97% AUC, indicating high discriminative performance across all classes.

Conclusion: Combining Vision Transformers with colormap-based feature enhancement is effective for accurate and robust brain tumor classification. The approach shows strong potential for clinical decision support applications.
...
TL;DR: CornOrb: A publicly available multimodal dataset of 1,454 Orbscan corneal topography images with clinical annotations from Algerian patients, including 889 normal eyes and 565 keratoconus cases, designed for AI research.


<details>
  <summary>Details</summary>
Motivation: To address the lack of large-scale Orbscan-based corneal topography datasets from Africa for AI-driven keratoconus research, and to provide standardized multimodal data for robust detection and analysis.

Method: Retrospective collection of fully anonymized Orbscan images and clinical data from 744 patients (1,454 eyes) in Algeria, pre-processed into standardized PNG (four corneal maps per eye) and CSV formats with structured tabular clinical parameters.

Result: Created a comprehensive dataset with 889 normal eyes and 565 keratoconus cases, including four corneal maps per eye (axial curvature, anterior elevation, posterior elevation, pachymetry) plus demographic and clinical parameters, openly available on Zenodo.

Conclusion: CornOrb represents one of the first large-scale Orbscan-based resources from Africa, providing standardized multimodal data to enable robust AI-driven keratoconus detection and analysis, with potential to advance ophthalmic AI research.
...
TL;DR: FoB reformulates SAM-based few-shot medical image segmentation as a prompt localization task, using background-centric prompts to constrain SAM's over-segmentation and achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Conventional few-shot medical image segmentation approaches face performance bottlenecks, and while SAM has strong category-agnostic segmentation capabilities, it often leads to over-segmentation on medical images due to ambiguous anatomical boundaries.

Method: FoB reformulates SAM-based FSMIS as a prompt localization task, generating background-centric prompts to constrain SAM's over-segmentation. It bridges segmentation and prompt localization by category-agnostic generation of support background prompts and localizing them in query images. FoB models contextual information to capture foreground-background spatial dependencies and uses structural patterns of background prompts as constraints for progressive refinement.

Result: Experiments on three diverse medical image datasets show FoB outperforms other baselines by large margins, achieving state-of-the-art performance on FSMIS and exhibiting strong cross-domain generalization.

Conclusion: FoB successfully addresses SAM's over-segmentation problem in medical images by focusing on background prompts, demonstrating superior performance and generalization capabilities for few-shot medical image segmentation.
...
TL;DR: Unsupervised self-evolution framework for multimodal reasoning that improves performance without human annotations or external reward models by using self-consistency signals and group-level relative policy optimization.


<details>
  <summary>Details</summary>
Motivation: Current multimodal reasoning improvements rely on costly high-quality annotated data or teacher-model distillation, which are difficult to scale. Need for more scalable, unsupervised approaches.

Method: Proposes Group Relative Policy Optimization (GRPO): samples multiple reasoning trajectories, models within-group structure, uses Actor's self-consistency as training prior, introduces bounded Judge-based modulation to reweight trajectories, and converts absolute scores to relative advantages within each group.
...
Conclusion: Offers a scalable path toward self-evolving multimodal models through unsupervised training framework that achieves stable performance improvements without human annotations or external reward models.
...
TL;DR: TIGON combines text and image conditioning for 3D generation, outperforming single-modality methods through cross-modal fusion.


<details>
  <summary>Details</summary>
Motivation: Existing 3D generators use either image conditioning (high visual fidelity but viewpoint bias) or text conditioning (broad semantics but lacks visual detail). Users need more flexible ways to express intent by combining both modalities.

Method: TIGON: minimalist dual-branch baseline with separate image- and text-conditioned backbones and lightweight cross-modal fusion for joint reasoning over visual exemplars and textual specifications.
...
Conclusion: Combining text and image conditioning provides complementary vision-language guidance, offering a promising direction for more flexible and faithful 3D generation.
...
TL;DR: Mv²ID: A multi-view conditioned framework that improves identity consistency in reference-to-video generation while maintaining motion naturalness, using region-masking training and reference decoupled-RoPE.


<details>
  <summary>Details</summary>
Motivation: Single-view reference-to-video methods struggle with identity consistency under large facial-angle variations, while multi-view references exacerbate the "view-dependent copy-paste" problem that reduces facial motion naturalness. Cross-paired data can help but is costly to collect.

Method: Proposes Mv²ID, a multi-view conditioned framework with in-paired supervision. Uses region-masking training strategy to prevent shortcut learning and extract essential identity features by aggregating complementary identity cues across views. Introduces reference decoupled-RoPE mechanism with distinct positional encoding for video and conditioning tokens. Constructs large-scale dataset with diverse facial-angle variations.

Result: Extensive experiments show the method significantly improves identity consistency while maintaining motion naturalness, outperforming existing approaches trained with cross-paired data.

Conclusion: Mv²ID effectively balances identity consistency and motion naturalness in multi-view reference-to-video generation through innovative training strategies and architectural improvements, demonstrating superior performance without requiring costly cross-paired data.
...
TL;DR: F4Splat introduces adaptive Gaussian allocation for feed-forward 3D Gaussian Splatting, using densification scores to reduce redundancy and control Gaussian budgets without retraining.


<details>
  <summary>Details</summary>
Motivation: Current feed-forward 3D Gaussian Splatting methods use rigid allocation pipelines that uniformly distribute Gaussians, creating redundancy across views and lacking control over total Gaussian count while maintaining quality.

Method: F4Splat performs feed-forward predictive densification with a densification-score-guided allocation strategy that adaptively distributes Gaussians based on spatial complexity and multi-view overlap, predicting per-region densification scores to estimate required Gaussian density.

Result: The model achieves superior novel-view synthesis performance compared to prior uncalibrated feed-forward methods while using significantly fewer Gaussians, producing compact yet high-quality 3D representations.

Conclusion: F4Splat's spatially adaptive allocation reduces redundancy in simple regions, minimizes duplicate Gaussians across overlapping views, and allows explicit control over final Gaussian budget without retraining, addressing key limitations of existing methods.
...
TL;DR: FedDP-STECAR is a federated learning framework for video action recognition that combines selective layer tuning with differential privacy to address privacy leakage and communication overhead, achieving 70.2% higher accuracy under strict privacy constraints while reducing communication by 99%.


<details>
  <summary>Details</summary>
Motivation: Federated video action recognition faces two major challenges: 1) model exposure where gradients can leak private motion patterns, and 2) communication overhead from full-model synchronization of high-dimensional video networks.

Method: FedDP-STECAR selectively fine-tunes and perturbs only a small subset of task-relevant layers under Differential Privacy (DP), preserving temporal coherence while reducing information leakage. It transmits only tuned layers during aggregation, drastically cutting communication traffic.

Result: On UCF-101 dataset using MViT-B-16x4 transformer: achieves 70.2% higher accuracy under strict privacy (ε=0.65) in centralized settings, 48% faster training with 73.1% accuracy in federated setups, and reduces communication traffic by over 99% compared to full-model updates.

Conclusion: FedDP-STECAR enables scalable and privacy-preserving video action recognition by effectively balancing privacy protection, communication efficiency, and model performance through selective layer tuning with differential privacy.
...
TL;DR: TTA-CaP: A cache-based test-time adaptation method for video facial expression recognition that uses three coordinated caches for cost-effective personalization of vision-language models without gradient optimization.


<details>
  <summary>Details</summary>
Motivation: FER in videos suffers from inter-subject variations, and while VLMs offer strong transfer, they degrade under distribution shifts. Existing TTA methods require computationally expensive unsupervised parameter optimization, making them impractical for real-world deployment.

Method: TTA-CaP uses three caches: personalized source cache (source prototypes), positive target cache (reliable subject samples), and negative target cache (low-confidence cases). Cache updates are controlled by a tri-gate mechanism based on temporal stability, confidence, and consistency. Predictions are refined through embedding fusion for temporally stable video-level outputs.
...
Conclusion: TTA-CaP provides an effective, gradient-free solution for personalizing VLMs in video FER, addressing computational limitations of existing TTA methods while handling distribution shifts through coordinated cache management.
...
TL;DR: KHMP introduces an adaptive Kalman filter in DCT domain with dynamic noise adjustment and physics constraints to generate smooth, physically plausible human motion predictions while reducing jitter artifacts.


<details>
  <summary>Details</summary>
Motivation: Existing stochastic human motion prediction methods suffer from high-frequency jitter and temporal discontinuities, producing corrupted predictions that lack smoothness and physical plausibility.

Method: KHMP uses an adaptive Kalman filter applied in DCT domain, treating high-frequency DCT coefficients as noisy signals. Noise parameters are dynamically adjusted based on estimated SNR, complemented by training-time physical constraints (temporal smoothness and joint angle limits) encoding biomechanical principles.

Result: Experiments on Human3.6M and HumanEva-I datasets show KHMP achieves state-of-the-art accuracy, effectively mitigating jitter artifacts to produce smooth and physically plausible motions.

Conclusion: KHMP establishes a new paradigm integrating adaptive signal processing with physics-informed learning for high-fidelity human motion prediction, addressing key challenges of jitter and temporal discontinuities in existing methods.
...
TL;DR: EmoTaG: A few-shot emotion-aware 3D talking head synthesis framework that improves motion stability and emotional expressiveness by using structured FLAME parameter space and a Gated Residual Motion Network.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot 3D talking head methods suffer from geometric instability and audio-emotion mismatch under expressive facial motion, highlighting the need for more effective emotion-aware motion modeling.

Method: Uses Pretrain-and-Adapt paradigm with two key innovations: 1) Reformulates motion prediction in structured FLAME parameter space instead of directly deforming 3D Gaussians for better geometric priors, and 2) Proposes Gated Residual Motion Network (GRMN) that captures emotional prosody from audio while supplementing head pose and upper-face cues missing from audio.

Result: Extensive experiments demonstrate state-of-the-art performance in emotional expressiveness, lip synchronization, visual realism, and motion stability.

Conclusion: EmoTaG provides an effective framework for few-shot emotion-aware 3D talking head synthesis that addresses key limitations of existing methods through structured motion modeling and comprehensive emotion-aware motion generation.
...
TL;DR: Proposed Coarse-to-Fine Diffusion Models with Time Step Sequence Redistribution to reduce computational overhead in diffusion models while maintaining near-lossless performance.


<details>
  <summary>Details</summary>
Motivation: Diffusion models have high computational costs due to multi-step denoising, hindering deployment on resource-constrained edge devices. Existing methods overlook input redundancy and require lengthy search times.

Method: 1) Coarse-to-Fine Denoising (C2F): Reduces computation during early stages when generated images are indistinguishable. 2) Time Step Sequence Redistribution (TRD): Efficient sampling trajectory adjustment with search time under 10 minutes.

Result: Achieves near-lossless performance with 80% to 90% reduction in computation on CIFAR10 and LSUN-Church datasets.

Conclusion: The proposed methods effectively reduce computational overhead in diffusion models while maintaining image quality, making them suitable for deployment on resource-constrained devices.
...
TL;DR: Video transformers can recognize respiratory distress from video with 0.81 F1 score using ViViT with LieRE and motion masking.


<details>
  <summary>Details</summary>
Motivation: Visual recognition of respiratory distress is a critical clinical skill that enables early intervention. The study aims to leverage recent advances in video transformers to create AI systems that can detect early signs of respiratory deterioration from video.

Method: Collected videos of healthy volunteers recovering after exercise, using natural recovery to create labeled dataset. Split videos into clips with earlier clips corresponding to more shortness of breath. Used ViViT encoder augmented with Lie Relative Encodings (LieRE) and Motion Guided Masking, combined with embedding-based comparison strategy for temporal ordering challenge.

Result: Achieved F1 score of 0.81 on respiratory distress recognition task, demonstrating that the AI system can effectively detect subtle changes in respiratory mechanics.

Conclusion: Modern video transformers can recognize subtle changes in respiratory mechanics, suggesting potential for AI-assisted clinical monitoring of respiratory distress.
...
TL;DR: FluidGaussian: A 3D reconstruction method that integrates fluid-structure interaction simulations to improve both visual and physical plausibility of reconstructed objects.


<details>
  <summary>Details</summary>
Motivation: Current 3D reconstruction methods focus only on visual fidelity, ignoring physical laws and object functionality, leading to implausible interactions in the physical world. There's a need to make reconstruction aware of real-world interactions beyond visual cues.

Method: Proposes FluidGaussian as a plug-and-play method that couples geometry reconstruction with fluid-structure interactions. Uses simulation-based uncertainty metric from fluid simulations integrated with active learning to prioritize views that improve both visual and physical fidelity.

Result: On NeRF Synthetic, Mip-NeRF 360, and DrivAerNet++ datasets, FluidGaussian achieves up to +8.6% visual PSNR improvement and -62.3% velocity divergence during fluid simulations compared to baseline methods.

Conclusion: FluidGaussian successfully bridges the gap between visual reconstruction and physical plausibility by incorporating fluid-structure interaction simulations, enabling more physically accurate 3D reconstructions that behave plausibly in real-world interactions.
...
TL;DR: Relax Forcing introduces structured temporal memory for autoregressive video diffusion, decomposing history into functional roles (Sink, Tail, History) to improve long-horizon generation by selectively using relevant past information.


<details>
  <summary>Details</summary>
Motivation: Existing self-forcing strategies for autoregressive video diffusion still struggle with minute-scale generation due to progressive temporal degradation. The authors find this limitation stems from how temporal memory is utilized during inference, not insufficient memory capacity.

Method: Relax Forcing decomposes temporal context into three functional roles: Sink (global stability), Tail (short-term continuity), and dynamically selected History (structural motion guidance). Instead of attending to dense generated history, it selectively incorporates only the most relevant past information.

Result: Experiments on VBench-Long show that Relax Forcing improves motion dynamics and overall temporal consistency while reducing attention overhead compared to existing approaches.

Conclusion: Structured temporal memory is essential for scalable long video generation, complementing existing forcing-based training strategies. The work demonstrates that how temporal memory is utilized matters more than simply increasing memory capacity.
...
TL;DR: HamVision is a medical image analysis framework that uses damped harmonic oscillator dynamics as an inductive bias, producing three distinct representations (position, momentum, energy) that emerge from the dynamics and can be used for both segmentation and classification tasks without supervision.


<details>
  <summary>Details</summary>
Motivation: The paper aims to leverage fundamental signal processing principles (damped harmonic oscillator) as structured inductive biases for medical image analysis, creating representations that emerge from dynamics rather than supervision, enabling more interpretable and effective models for both segmentation and classification tasks.

Method: The framework uses a damped harmonic oscillator to decompose images into three representations: position (feature content), momentum (spatial gradients for boundaries/texture), and energy (parameter-free saliency). For segmentation (HamSeg), energy gates skip connections while momentum injects boundary information. For classification (HamCls), the three representations are globally pooled and concatenated into a phase-space feature vector.

Result: HamVision achieves state-of-the-art results on multiple medical imaging benchmarks: segmentation SOTA on ISIC 2018 (89.38%), ISIC 2017 (88.40%), TN3K (87.05%), and ACDC (92.40%) with only 8.57M parameters; classification SOTA on BloodMNIST (98.85%) and PathMNIST (96.65%), with competitive results on other MedMNIST datasets against MedMamba and MedViT.

Conclusion: The damped harmonic oscillator provides an effective structured inductive bias for medical image analysis, producing interpretable representations that emerge from dynamics without supervision. The framework demonstrates strong performance across diverse medical imaging tasks and modalities while maintaining parameter efficiency and interpretability through the oscillator's physical properties.
...
<details>
  <summary>Details</summary>
Motivation: Phase unwrapping is critical but challenging in InSAR processing, especially for earthquake deformation with surface-breaking faults causing abrupt displacement discontinuities. Existing learning-based methods fail with large-scale, spatially heterogeneous real interferograms due to fixed small input sizes.

Method: A diffusion model architecture for phase unwrapping that processes large-scale interferograms and addresses phase discontinuities caused by deformation. The method recovers physically consistent unwrapped phase fields even with fault-related phase jumps.

Result: Experimental results on synthetic and real datasets show the method effectively handles discontinuities from near-surface deformation and scales well to large InSAR images.

Conclusion: The diffusion model-based framework offers a practical alternative to manual unwrapping in challenging scenarios involving complex deformation patterns and large-scale interferograms.
...
TL;DR: OVRCOAT is a modular framework for open-vocabulary panoptic segmentation that addresses mask selection bias and limited regional understanding in VLMs, achieving SOTA results on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current open-vocabulary panoptic segmentation suffers from two key issues: (1) mask selection bias where objectness heads trained on closed vocabularies suppress masks of unseen categories, and (2) limited regional understanding in vision-language models like CLIP that were optimized for global image classification rather than localized segmentation.

Method: OVRCOAT consists of two modular components: (1) CLIP-conditioned objectness adjustment (COAT) that updates background/foreground probabilities to preserve high-quality masks for out-of-vocabulary objects, and (2) open-vocabulary mask-to-text refinement (OVR) that strengthens CLIP's region-level alignment to improve classification of both seen and unseen classes with lower memory cost than prior fine-tuning approaches.

Result: OVRCOAT achieves state-of-the-art results on ADE20K (+5.5% PQ), Mapillary Vistas (+7.1% PQ), and Cityscapes (+3% PQ). The framework consistently improves both objectness estimation and mask recognition while being more memory-efficient than previous fine-tuning methods.

Conclusion: OVRCOAT provides a simple yet effective modular solution to the coupled problems of mask selection bias and limited regional understanding in open-vocabulary panoptic segmentation, delivering substantial performance gains across multiple benchmarks with improved memory efficiency.
...
TL;DR: Two-stage framework for privacy-preserving facial expression recognition in videos without identity labels, using identity suppression and denoising modules, validated through falsification-based testing.


<details>
  <summary>Details</summary>
Motivation: Facial expression recognition uses data that exposes identity, raising privacy concerns. Current methods fail in realistic open-set video settings where identities are unknown and identity labels are unavailable.

Method: Two-stage framework: 1) Identity-suppression network trained using intra- and inter-video knowledge priors from unlabeled real-world videos to anonymize identity while preserving expressive cues; 2) Denoising module to restore expression-related information and recover FER performance. Also introduces falsification-based validation method using recognition priors to evaluate privacy robustness without identity labels.

Result: Experiments on three video datasets show the method effectively protects privacy while maintaining FER accuracy comparable to identity-supervised baselines.

Conclusion: The proposed framework enables privacy-preserving facial expression recognition in challenging open-set video settings without requiring identity labels at any stage, addressing both privacy protection and utility preservation.
...
TL;DR: Beta-KD is an uncertainty-aware knowledge distillation framework that adaptively balances learning from data vs. teacher guidance using Bayesian formulation and Gibbs prior.


<details>
  <summary>Details</summary>
Motivation: Current knowledge distillation lacks adaptive balancing between data supervision and teacher guidance, as some samples may be noisy while others have teacher uncertainty, requiring dynamic adjustment.

Method: Formulates teacher-student learning from Bayesian perspective, interprets teacher supervision as Gibbs prior over student activations, yielding closed-form uncertainty-aware weighting mechanism supporting arbitrary distillation objectives.

Result: Extensive experiments on multimodal VQA benchmarks show Beta-KD consistently improves student Vision-Language Models when distilled from large teacher VLM, outperforming existing knowledge distillation methods.

Conclusion: Beta-KD provides effective uncertainty-aware distillation framework that adaptively modulates student reliance on teacher guidance, achieving superior performance on multimodal tasks.
...
<details>
  <summary>Details</summary>
Motivation: To bridge the gap between traditional structural analysis methods and modern computer vision capabilities, enabling automated interpretation of hand-drawn structural diagrams commonly used in engineering education and practice.

Method: Uses computer vision and statistical learning techniques for detection and visual interpretation of structural elements from drawings. Implements the PhotoBeamSolver program to process and analyze hand-drawn beam models.

Result: Developed a documented program capable of solving idealized beam models from human-made drawings, while analyzing challenges and limitations of computer vision integration in structural analysis.

Conclusion: Computer vision shows promise for structural analysis applications but faces challenges in reliable implementation; the paper explores current state and requirements for practical use in civil engineering, particularly for infrastructure inspection and decision-support systems.
...
TL;DR: PAS3R is a pose-adaptive streaming 3D reconstruction framework that dynamically adjusts state updates based on camera motion and scene structure to solve the stability-adaptation dilemma in online monocular reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing streaming 3D reconstruction approaches suffer from the stability-adaptation dilemma: they must incorporate new viewpoints while preserving accumulated scene structure. Current methods with uniform or attention-based updates fail during abrupt viewpoint transitions, causing trajectory drift and geometric inconsistencies in long sequences.

Method: PAS3R uses a motion-aware update mechanism that leverages inter-frame pose variation and image frequency cues to estimate frame importance. It includes trajectory-consistent training with relative pose constraints and acceleration regularization, plus a lightweight online stabilization module to suppress trajectory jitter without increasing memory.

Result: Extensive experiments show PAS3R significantly improves trajectory accuracy, depth estimation, and point cloud reconstruction quality in long video sequences while maintaining competitive performance on shorter sequences.

Conclusion: PAS3R effectively addresses the stability-adaptation dilemma in online monocular 3D reconstruction through pose-adaptive state updates, providing more robust and accurate long-term reconstruction compared to existing methods.
...
TL;DR: EpiMask is a semi-dense image matching network designed specifically for satellite images that incorporates camera geometry modeling and epipolar constraints, achieving 30% better accuracy than ground-based models.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning image matching networks are optimized for pinhole camera geometry from ground-based datasets, leading to suboptimal performance when applied to satellite images due to different camera geometry (line-by-line scanning).

Method: EpiMask incorporates: (1) patch-wise affine approximations to satellite camera geometry, (2) epipolar distance-based attention mask to restrict cross-attention to geometrically plausible regions, and (3) fine-tuning of a foundational pretrained image encoder for robust feature extraction.
...
Conclusion: The paper presents EpiMask as an effective solution for satellite image matching by explicitly incorporating satellite camera geometry and epipolar constraints, significantly outperforming ground-based models adapted for satellite imagery.
...
TL;DR: ALADIN is an attribute-language distillation network that distills CLIP knowledge to lightweight ReID models using fine-grained attribute-local alignment and scene-aware prompts, improving robustness and interpretability.


<details>
  <summary>Details</summary>
Motivation: Current CLIP-guided ReID pipelines rely on global features and fixed prompts, limiting their ability to capture fine-grained attribute cues and adapt to diverse appearances. There's a need for more adaptive and robust person re-identification methods.

Method: Proposes ALADIN with: 1) Attribute-language distillation from frozen CLIP teacher to lightweight ReID student, 2) Fine-grained attribute-local alignment for adaptive text-visual correspondence, 3) Scene-Aware Prompt Generator for image-specific soft prompts, 4) Cross-modal contrastive and relation distillation to preserve attribute relationships, 5) Uses Multimodal LLMs to generate structured attribute descriptions converted to localized attention maps via CLIP.

Result: Experiments on Market-1501, DukeMTMC-reID, and MSMT17 show improvements over CNN-, Transformer-, and CLIP-based methods, with better generalization and interpretability. The method demonstrates enhanced robustness under occlusions.

Conclusion: ALADIN effectively bridges the gap between vision-language models and ReID tasks by distilling fine-grained attribute knowledge, achieving state-of-the-art performance while maintaining efficiency through lightweight student models at inference.
...
TL;DR: A novel continual unlearning framework for vision-language models that grounds refusal behavior in fine-grained visual/textual concepts to selectively forget specific image-instruction pairs while preserving general utility.


<details>
  <summary>Details</summary>
Motivation: Continual unlearning in vision-language models faces challenges where sequential unlearning updates create spurious associations between vision-language pairs and refusal behaviors, leading to inappropriate refusals and difficulty in precisely identifying refusal targets.

Method: Proposes a framework with: 1) Concept modulator to identify visual-linguistic concept combinations characterizing each forget category, 2) Mixture of refusal experts (refusers) specialized for concept-aligned refusal generation, and 3) Multimodal concept-driven routing scheme that reuses refusers for similar concepts and adapts underutilized ones for novel concepts.

Result: Extensive experiments on vision-language benchmarks demonstrate the framework outperforms existing methods by generating concept-grounded refusal responses and preserving general utility across unlearning sequences.

Conclusion: The proposed concept-grounded continual unlearning framework effectively addresses the challenge of selective forgetting in vision-language models by decomposing deletion targets into fine-grained concepts and using specialized refusal experts, enabling precise refusal behavior while maintaining overall model utility.
...
TL;DR: TrajSeg is a unified MLLM-based framework for video reasoning segmentation that introduces bidirectional text-trajectory alignment to better handle video dynamics and object trajectory perception.


<details>
  <summary>Details</summary>
Motivation: Previous video reasoning segmentation methods rely on unidirectional and implicit text-trajectory alignment, which struggles with trajectory perception when faced with severe video dynamics. There's a need for better alignment between text instructions and object trajectories in videos.

Method: Proposes TrajSeg with: 1) Bidirectional text-trajectory alignment where MLLMs accept both grounding-intended (text-to-trajectory) and captioning-intended (trajectory-to-text) instructions; 2) Frame-level content integration module to adapt trajectory-level tokens to frame-specific information; 3) Unified mask decoder that handles segmentation for all frames in a single structure, making the framework end-to-end trainable.

Result: Extensive experiments on referring and reasoning video segmentation datasets show TrajSeg outperforms all existing video reasoning segmentation methods on all metrics.

Conclusion: TrajSeg provides an effective solution for video reasoning segmentation by leveraging bidirectional text-trajectory alignment and unified architecture, achieving state-of-the-art performance while being end-to-end trainable.
...
TL;DR: StreamingEval is a unified evaluation framework for Video-LLMs that benchmarks streaming video understanding under realistic resource constraints, revealing gaps between current models and practical deployability requirements.


<details>
  <summary>Details</summary>
Motivation: Existing research on streaming video understanding focuses on isolated aspects like accuracy or encoding efficiency, but overlooks practical deployability under realistic resource constraints. There's a need for comprehensive evaluation that considers the trade-off between efficiency, storage, and accuracy for real-world interactive AI applications.

Method: Introduces StreamingEval framework with fixed-capacity memory bank to normalize accessible historical visual context. Jointly evaluates visual encoding efficiency, text decoding latency, and task performance to quantify overall system deployability. Benchmarks both mainstream offline models and recent online video models under standardized protocol.

Result: Extensive experiments across multiple datasets reveal substantial gaps between current Video-LLMs and the requirements of realistic streaming applications. The framework provides systematic evaluation of the trade-off between efficiency, storage, and accuracy.

Conclusion: StreamingEval addresses the gap in practical deployability evaluation for streaming video understanding, providing a systematic basis for future research by revealing current limitations and establishing standardized benchmarking protocols for realistic resource-constrained applications.
...
TL;DR: A parameter-efficient prompt tuning method for few-shot weakly supervised WSI classification that reduces computational costs and avoids information loss from instance filtering.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based methods for few-shot WSI classification have high computational overhead from prompt tuning and lose information by discarding low-alignment instances, requiring more efficient approaches.

Method: Two contributions: 1) Parameter-efficient prompt tuning via scaling/shifting features in text encoder, 2) WSI representation learning with soft hierarchical textual guidance without hard instance filtering.

Result: Consistent improvements up to 10.9%, 7.8%, and 13.8% on breast, lung, and ovarian cancer datasets; reduces trainable parameters by 18.1% (breast/lung) and 5.8% (ovarian); excels at tumor localization.

Conclusion: The proposed method effectively addresses computational efficiency and information loss issues in FSWC, achieving state-of-the-art performance while reducing parameter overhead and enabling better tumor localization.
...
TL;DR: BTP is a zero-shot 3D anomaly detection framework that aligns point cloud and text embeddings using multi-granularity patch features and geometric descriptors, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing 3D anomaly detection methods render point clouds to 2D images, losing geometric details and having limited sensitivity to local anomalies. There's a need for better zero-shot 3D anomaly detection that preserves 3D structure.

Method: BTP aligns 3D point cloud and textual embeddings using multi-granularity patch features with textual representations for localized anomaly detection, incorporates geometric descriptors for structural anomalies, and uses joint representation learning with auxiliary point cloud data.

Result: Extensive experiments on Real3D-AD and Anomaly-ShapeNet show BTP achieves superior performance in zero-shot 3D anomaly detection compared to existing approaches.

Conclusion: BTP demonstrates that pre-trained Point-Language Models can effectively address zero-shot 3D anomaly detection by better preserving geometric information and improving sensitivity to local anomalies.
...
TL;DR: VIGIL is a part-centric structured forensic framework for interpretable deepfake detection that separates evidence generation from manipulation localization using a plan-then-examine pipeline, achieving superior generalizability across diverse forgery scenarios.


<details>
  <summary>Details</summary>
Motivation: Current MLLM-based deepfake detection methods combine evidence generation and manipulation localization into a unified step, which blurs the boundary between faithful observations and hallucinated explanations, leading to unreliable conclusions. There's a need for more interpretable and reliable forensic frameworks inspired by expert forensic practices.

Method: VIGIL employs a plan-then-examine pipeline: 1) Planning phase where the model identifies which facial parts warrant inspection based on global visual cues, 2) Examination phase where each part is analyzed with independently sourced forensic evidence using stage-gated injection. The framework uses a progressive three-stage training paradigm with reinforcement learning employing part-aware rewards to enforce anatomical validity and evidence-conclusion coherence.

Result: Extensive experiments on the OmniFake benchmark (5-level hierarchical evaluation) and cross-dataset evaluations show that VIGIL consistently outperforms both expert detectors and concurrent MLLM-based methods across all generalizability levels, demonstrating superior performance even when trained on only three foundational generators and tested on in-the-wild social-media data.

Conclusion: The part-centric structured forensic framework VIGIL provides a more interpretable and reliable approach to deepfake detection by separating evidence generation from manipulation localization, mimicking expert forensic practices. The proposed method shows strong generalizability across diverse forgery scenarios and establishes a rigorous evaluation benchmark for future research.
...
TL;DR: PEARL is a training-free open-vocabulary semantic segmentation method that uses Procrustes alignment and text-aware Laplacian propagation for efficient, plug-and-play inference without retraining.


<details>
  <summary>Details</summary>
Motivation: Existing training-free OVSS methods often rely on heavy post-processing, handle text and vision modalities in isolation, or introduce complex auxiliary backbones/pipelines that increase latency and compromise simplicity. There's a need for a more efficient approach that better utilizes cross-modal geometry.
...
Conclusion: PEARL demonstrates that a compact two-step inference approach with proper cross-modal alignment and propagation can achieve superior training-free OVSS performance while maintaining simplicity, being fully plug-and-play, and adding minimal latency.
...
TL;DR: PROBE introduces a diagnostic protocol to measure reactivation potential of erased concepts in text-to-video diffusion models, revealing that current erasure methods achieve only output-level suppression rather than true representational removal.


<details>
  <summary>Details</summary>
Motivation: Current concept erasure evaluation for T2V models is limited to checking if target concepts are absent from generated frames, treating output-level suppression as evidence of representational removal. This paper aims to develop a more rigorous diagnostic approach to quantify whether erased concepts can be reactivated.

Method: PROBE optimizes a lightweight pseudo-token embedding through denoising reconstruction with a novel latent alignment constraint that anchors recovery to the spatiotemporal structure of original concepts, while keeping all model parameters frozen. Uses multi-level evaluation framework including classifier detection, semantic similarity, temporal reactivation analysis, and human validation.

Result: Systematic experiments across three T2V architectures, three concept categories, and three erasure strategies reveal that all tested methods leave measurable residual capacity whose robustness correlates with intervention depth. Identifies temporal re-emergence - a video-specific failure mode where suppressed concepts progressively resurface across frames.

Conclusion: Current erasure methods achieve only output-level suppression rather than true representational removal. The findings highlight the need for more robust safety auditing in T2V models, and the protocol is released to support reproducible evaluation.
...
TL;DR: A novel part-to-whole 3D generative model that learns adaptive part-whole hierarchies from single images, autonomously discovering latent structural slots and enabling cross-category shape sharing through geometric prototypes.


<details>
  <summary>Details</summary>
Motivation: Existing single-image 3D generation methods struggle with reliable generalization across diverse categories and structural complexity. Monolithic or fixed-part approaches lead to overfitting, fragmented structures, and limited compositional generalization for novel object layouts.

Method: Proposes a part-to-whole 3D generative world model that: 1) autonomously discovers latent structural slots by inferring soft compositional masks from image tokens, 2) uses adaptive slot-gating to dynamically determine activation probabilities and consolidate redundant slots, 3) aligns slots to a learnable class-agnostic prototype bank for cross-category shape sharing, and 4) employs a lightweight 3D denoiser with unified diffusion objectives.

Result: Experiments show consistent gains in cross-category transfer and part-count extrapolation. Ablations confirm complementary benefits of the prototype bank for shape-prior sharing and slot-gating for structural adaptation.

Conclusion: The approach rethinks single-image 3D generation as learning adaptive part-whole hierarchies in flexible 3D latent space, enabling more reliable generalization across diverse semantic categories and structural complexity without requiring user-specified part counts.
...
TL;DR: PALS improves weakly-supervised video scene graph generation by learning pair affinity to filter non-interactive object pairs from off-the-shelf detectors, achieving SOTA results.


<details>
  <summary>Details</summary>
Motivation: Weakly-supervised VSGG methods use off-the-shelf detectors that detect all visible objects indiscriminately, overwhelming relation models with noisy non-interactive pairs, unlike fully-supervised pipelines that filter out non-interactive objects.

Method: Proposes Pair Affinity Learning and Scoring (PALS) with learnable pair affinity to estimate interaction likelihood, incorporating it into inference-time ranking and contextual reasoning via Pair Affinity Modulation (PAM). Also introduces Relation-Aware Matching (RAM) for cleaner supervision using vision-language grounding.

Result: Extensive experiments on Action Genome show consistent substantial improvements across different baselines and backbones, achieving state-of-the-art WS-VSGG performance.

Conclusion: The proposed approach effectively addresses the fundamental discrepancy between weakly-supervised and fully-supervised pipelines by learning to suppress non-interactive pairs, enabling models to focus on relationally meaningful interactions.
...
TL;DR: Proposes a multimodal prompting framework for unsupervised continual anomaly detection that addresses limitations of visual-only approaches by integrating textual information to better capture normality manifolds in complex scenes.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised continuous anomaly detection (UCAD) methods relying solely on visual information are insufficient for capturing normality manifolds in complex scenes, limiting anomaly detection accuracy improvements.

Method: Introduces Continual Multimodal Prompt Memory Bank (CMPMB) to distill and retain prototypical normal patterns from both visual and textual domains across tasks, and Defect-Semantic-Guided Adaptive Fusion Mechanism (DSG-AFM) with Adaptive Normalization Module and Dynamic Fusion Strategy for enhanced accuracy and robustness.
...
Conclusion: Multimodal prompting framework effectively addresses limitations of visual-only UCAD approaches, enabling better normality representation and improved anomaly detection in continual learning scenarios.
...
TL;DR: A frequency-spatial enhancement framework with noise-resilient knowledge guidance (FSCE) for SAR target recognition that improves accuracy under noisy conditions through frequency-spatial feature enhancement and teacher-student learning.


<details>
  <summary>Details</summary>
Motivation: SAR ATR faces challenges from coherent speckle noise that obscures target features, degrading recognition accuracy and limiting model generalization in marine navigation and disaster monitoring applications.

Method: Proposes FSCE framework with DSAF module for frequency-spatial shallow feature enhancement using spatial multi-scale convolution and frequency-domain wavelet convolution. Uses teacher-student learning with online knowledge distillation to guide focus on target regions and enhance noise robustness.
...
Conclusion: The proposed FSCE framework effectively addresses SAR noise challenges through collaborative optimization of attention transfer and noise-resilient representation learning, improving recognition stability under noisy conditions with both lightweight and high-precision variants.
...
TL;DR: CataractSAM-2 is a domain-adapted version of Meta's Segment Anything Model 2 for real-time semantic segmentation of cataract surgery videos, with an interactive annotation framework to reduce labeling time and strong zero-shot generalization to glaucoma procedures.


<details>
  <summary>Details</summary>
Motivation: To enable precise intraoperative perception for robotic-assisted and computer-guided surgical systems in ophthalmology, while addressing the burden of manual labeling for surgical video datasets.

Method: Domain adaptation of Segment Anything Model 2 for cataract surgery videos, combined with an interactive annotation framework using sparse prompts and video-based mask propagation.

Result: Achieves high accuracy real-time semantic segmentation, significantly reduces annotation time, demonstrates strong zero-shot generalization to glaucoma trabeculectomy procedures, and releases open-source model and toolkit.

Conclusion: CataractSAM-2 serves as a foundation for expanding anterior ophthalmic surgical datasets and advancing real-time AI-driven solutions in medical robotics and surgical video understanding.
...
TL;DR: The paper introduces CPRT, a compositional privacy risk taxonomy for images that moves beyond binary privacy classification to assess how combinations of attributes create privacy risks, with graded severity levels and a scoring function.


<details>
  <summary>Details</summary>
Motivation: Existing visual privacy benchmarks treat privacy as binary (private vs non-private), but privacy is fundamentally compositional - benign attributes in isolation can combine to create severe privacy violations. Current approaches fail to capture these compositional risks.

Method: Introduces Compositional Privacy Risk Taxonomy (CPRT), a regulation-aware framework organizing visual attributes by standalone identifiability and compositional harm potential. Defines four graded severity levels with interpretable scoring function. Creates taxonomy-aligned dataset of 6.7K images with ground-truth compositional risk scores. Evaluates frontier and open-weight VLMs, then introduces deployable 8B supervised fine-tuned model.
...
Conclusion: Privacy assessment requires compositional reasoning beyond binary classification. CPRT provides a framework for graded privacy evaluation, and the 8B SFT model offers deployable solution for practical compositional privacy assessment that matches frontier model performance.
...
TL;DR: A hardness-aware curriculum learning framework for semi-supervised 3D rotation regression from 2D images, using dynamic pseudo-label selection and structured data augmentation to outperform existing methods in low-data regimes.


<details>
  <summary>Details</summary>
Motivation: Existing rotation regression models require large labeled datasets or additional information (point clouds, CAD models). Semi-supervised learning with limited labeled 2D images is valuable but current methods like FisherMatch have rigid pseudo-label filtering that fails to distinguish reliable vs. unreliable samples effectively.

Method: Proposes a hardness-aware curriculum learning framework with: 1) Multi-stage and adaptive curriculum strategies for dynamic pseudo-label selection based on sample difficulty (easy to complex progression), 2) Novel structured data augmentation tailored for rotation estimation that assembles composite images from augmented patches to preserve geometric integrity while introducing feature diversity.

Result: Comprehensive experiments on PASCAL3D+ and ObjectNet3D demonstrate superior performance over existing supervised and semi-supervised baselines, especially in low-data regimes, validating the effectiveness of the curriculum learning framework and structured augmentation approach.

Conclusion: The proposed hardness-aware curriculum learning framework with structured data augmentation effectively addresses limitations of rigid pseudo-label filtering in semi-supervised rotation regression, achieving state-of-the-art performance with limited labeled data.
...
<details>
  <summary>Details</summary>
Motivation: Existing end-to-end approaches for 3D fragment reassembly suffer from cascading failures due to unreliable contact reasoning and inaccurate fragment adjacencies, especially as fragment count increases. The problem is challenging because the target shape is unknown and fragments provide weak semantic cues.

Method: Proposes Structure-Aware Reassembly (SARe) framework: 1) SARe-Gen jointly predicts fracture-surface token probabilities and inter-fragment contact graph using query-point-based conditioning with frozen geometry encoder; 2) SARe-Refine performs inference-time refinement with geometric-consistency checks to verify candidate contacts, select reliable substructures, and resample uncertain regions while keeping verified parts fixed.

Result: Achieves state-of-the-art performance across three settings (synthetic fractures, simulated fractures from scanned real objects, and real physically fractured scans), with more graceful degradation and higher success rates as fragment count increases in challenging large-scale reassembly.

Conclusion: SARe addresses limitations of existing approaches through explicit contact modeling and inference-time refinement, enabling more stable and consistent assemblies in many-fragment regimes by localizing contact regions, inferring candidate adjacencies, and verifying geometric consistency.
...
<details>
  <summary>Details</summary>
Motivation: Existing inversion-based image editing methods use fixed injection strategies (binary temporal schedules, uniform spatial mixing, channel-agnostic perturbation) that ignore heterogeneous injection demands across temporal and channel dimensions, leading to background preservation vs. content generation conflicts.

Method: Two innovations: 1) Progressive Injection Schedule using continuous decay functions (sigmoid, cosine, linear) for smooth transition from source-feature preservation to target-feature generation; 2) Channel-Selective Latent Perturbation that estimates per-channel importance based on distributional gap between inverted and random latents, applying differentiated perturbation strengths.

Result: On PIE-Bench (700 images, 10 editing types): 8.7% reduction in LPIPS, 2.6% improvement in SSIM, 2.3% improvement in PSNR over strong baselines while maintaining competitive CLIP similarity. Fully plug-and-play and compatible with multiple ODE solvers.

Conclusion: AdaEdit effectively resolves the injection dilemma in inversion-based image editing by introducing adaptive mechanisms that account for heterogeneous injection demands, achieving superior performance without training and maintaining compatibility with existing solvers.
...
TL;DR: 4DGS360 is a diffusion-free framework for 360° dynamic object reconstruction from monocular video, using 3D-native initialization to overcome geometric ambiguity in occluded regions.


<details>
  <summary>Details</summary>
Motivation: Existing methods fail to reconstruct consistent 360° geometry because they rely too heavily on 2D-native priors, causing overfitting to visible surfaces in each training view and poor handling of occluded regions.

Method: Proposes 3D-native initialization using AnchorTAP3D, a 3D tracker that leverages confident 2D track points as anchors to produce reinforced 3D point trajectories, suppressing drift and preserving geometry in occluded regions. Combines this initialization with optimization for coherent 360° 4D reconstructions.

Result: Achieves state-of-the-art performance on iPhone360 (new benchmark), iPhone, and DAVIS datasets, both qualitatively and quantitatively. The iPhone360 benchmark enables 360° evaluation with test cameras up to 135° apart from training views.

Conclusion: 4DGS360 successfully addresses the challenge of 360° dynamic object reconstruction through 3D-native initialization, outperforming existing methods and enabling comprehensive evaluation through a new benchmark.
...
TL;DR: A training-free AI-generated image detection method that measures representation sensitivity to structured frequency perturbations, achieving high accuracy with significantly faster inference than existing methods.


<details>
  <summary>Details</summary>
Motivation: Current AI-generated image detectors face limitations: training-based methods lack generalization to unseen images, while training-free approaches struggle to capture subtle discrepancies between real and synthetic images. There's a need for a robust, computationally efficient detection method.

Method: Proposes a training-free detection method that measures representation sensitivity to structured frequency perturbations. The approach uses Fourier transform to generate perturbations, requiring only a single transform per image, making it computationally lightweight.

Result: The method achieves 1-2 orders of magnitude faster inference than most training-free detectors. On the OpenFake benchmark, it improves AUC by nearly 10% compared to state-of-the-art methods while maintaining substantially lower computational cost.

Conclusion: The proposed training-free detection method effectively identifies AI-generated images by measuring sensitivity to frequency perturbations, offering superior performance and computational efficiency compared to existing approaches.
...
TL;DR: PGR-Net is a brain tumor MRI segmentation framework that uses data-driven spatial priors and hierarchical ROI selection to address spatial sparsity and improve segmentation accuracy with minimal parameters.


<details>
  <summary>Details</summary>
Motivation: Brain tumor lesions occupy only a small fraction of MRI volumes, creating severe spatial sparsity. Existing segmentation networks overlook clinically observed spatial priors of tumor occurrence, leading to redundant feature computation over extensive background regions and suboptimal performance.

Method: Proposes PGR-Net with three key components: 1) Data-driven spatial prior set capturing tumor distribution and scale characteristics, 2) Hierarchical Top-K ROI decision mechanism that progressively selects confident lesion candidate regions across encoder layers, 3) WinGS-ROI module using multi-window Gaussian templates with spatial decay function to produce center-enhanced guidance maps. Uses windowed RetNet backbone for enhanced localization reliability.

Result: Outperforms existing approaches on BraTS-2019/2023 and MSD Task01 datasets with only 8.64M parameters. Achieves Dice scores of 89.02%, 91.82%, and 89.67% on Whole Tumor region across different benchmarks.

Conclusion: PGR-Net effectively addresses spatial sparsity in brain tumor segmentation by incorporating explicit ROI-aware mechanisms with data-driven spatial priors, achieving state-of-the-art performance with parameter efficiency. The framework demonstrates the value of integrating clinical spatial knowledge into deep learning segmentation models.
...
TL;DR: TCEI: A test-time adaptation framework for MOT that mimics human decision-making with intuitive and experiential systems to handle distribution shifts while maintaining temporal consistency and identity association.


<details>
  <summary>Details</summary>
Motivation: Existing TTA methods fail in MOT because they focus only on frame-level adaptation and neglect temporal consistency and identity association across frames, leading to performance degradation due to distribution shifts in appearance, motion patterns, and categories between training and testing data.

Method: Proposes TCEI framework with two systems: 1) Intuitive system uses transient memory to recall recently observed objects for rapid predictions, 2) Experiential system leverages accumulated experience from prior test videos to reassess and calibrate intuitive predictions. Uses both confident and uncertain objects as historical priors and reflective cases for adaptation.
...
Conclusion: TCEI effectively addresses MOT's test-time adaptation challenges by mimicking human decision-making processes, maintaining temporal consistency and identity association while adapting to distribution shifts in testing environments.
...
TL;DR: SparseVoxelDet: First fully sparse object detector for event cameras using 3D sparse convolutions, achieving 83.38% mAP while processing only 0.23% of grid voxels, with 858× memory compression.


<details>
  <summary>Details</summary>
Motivation: Event cameras produce sparse, efficient neuromorphic data streams, but most detectors convert them to dense tensors, losing the efficiency benefits. The authors aim to create a detector that maintains sparsity throughout the pipeline.

Method: SparseVoxelDet uses 3D sparse convolutions exclusively on occupied voxel positions. The entire pipeline (backbone, feature pyramid fusion, detection head) operates on sparse representations without ever creating dense feature tensors.

Result: Achieves 83.38% mAP@50 on FRED benchmark, processing only 14,900 active voxels per frame (0.23% of grid). Shows 858× GPU memory compression and 3,670× storage reduction vs dense baseline. Error analysis shows 71% of failures are localization near-misses.

Conclusion: Native sparse processing is viable for event-camera detection, exploiting structural sparsity without neuromorphic hardware. Representation cost scales with scene activity rather than pixel count, making it valuable for higher-resolution event cameras.
...
<details>
  <summary>Details</summary>
Motivation: Applying federated learning to cross-view video understanding faces three key challenges: 1) heterogeneous viewpoints/backgrounds create non-IID distributions and view-specific overfitting, 2) local distribution biases cause misaligned representations that hurt cross-view consistency, and 3) large video architectures have prohibitive communication costs.

Method: FedCVU has three components: 1) VS-Norm preserves normalization parameters to handle view-specific statistics, 2) CV-Align uses lightweight contrastive regularization to improve cross-view representation alignment, and 3) SLA employs selective layer aggregation to reduce communication without sacrificing accuracy.

Result: Extensive experiments on action understanding and person re-identification tasks under cross-view protocols show FedCVU consistently boosts unseen-view accuracy while maintaining strong seen-view performance, outperforming state-of-the-art FL baselines with robustness to domain heterogeneity and communication constraints.

Conclusion: FedCVU effectively addresses the core challenges of federated cross-view video understanding through its three-component design, demonstrating superior performance in handling view heterogeneity, representation alignment, and communication efficiency.
...
TL;DR: OmniFM is a modality- and task-agnostic federated learning framework for medical image analysis that uses frequency-domain insights to handle heterogeneous imaging modalities and diverse tasks without pipeline re-engineering.


<details>
  <summary>Details</summary>
Motivation: Existing federated learning frameworks for medical image analysis are tightly coupled to task-specific backbones and fragile under heterogeneous imaging modalities, hindering real-world deployment where institutions vary in modality distributions and need to support diverse downstream tasks.

Method: OmniFM leverages frequency-domain insights that low-frequency spectral components exhibit cross-modality consistency. It integrates: (1) Global Spectral Knowledge Retrieval to inject global frequency priors, (2) Embedding-wise Cross-Attention Fusion to align representations, (3) Prefix-Suffix Spectral Prompting to jointly condition global and personalized cues, all regularized by a Spectral-Proximal Alignment objective that stabilizes aggregation.
...
Conclusion: OmniFM provides a unified, modality- and task-agnostic FL framework that effectively handles heterogeneous medical imaging modalities and diverse tasks, enabling more practical real-world deployment of federated learning in medical image analysis.
...
TL;DR: A cross-scenario deraining adaptation framework that uses rain-free target images to generate realistic pseudo-data for OOD generalization, achieving 32-59% PSNR gains.


<details>
  <summary>Details</summary>
Motivation: Deep learning deraining models fail in OOD scenarios due to domain discrepancy between synthetic training data and real-world rain dynamics. Need adaptation without paired rainy observations in target domain.

Method: 1) Sup-Gen module extracts structural priors from source domain using SLIC. 2) Resolution-adaptive Fusion aligns source structures with target backgrounds via texture similarity. 3) Pseudo-label re-Synthesize with multi-stage noise generation for realistic rain streaks. Plug-and-play module for any deraining architecture.

Result: Achieves 32% to 59% PSNR gains in OOD domains while significantly accelerating training convergence. Extensive experiments on SOTA models validate effectiveness.

Conclusion: Proposed framework enables effective cross-scenario deraining adaptation using only rain-free target images, overcoming OOD generalization challenges without requiring paired rainy observations.
...
TL;DR: The paper introduces VR-SDR and HumanOmni-Speaker Benchmark to address LLMs' inability to track multi-person conversational dynamics, overcoming visual biases and low-frame-rate limitations through a Visual Delta Encoder that captures fine-grained visemes and speaker trajectories.


<details>
  <summary>Details</summary>
Motivation: Current Omni-modal LLMs struggle with accurately identifying "Who said what and when" in multi-person conversations. They suffer from an "illusion of competence" by exploiting visual biases in benchmarks and using sparse, low-frame-rate sampling that destroys crucial high-frequency dynamics like lip movements.

Method: Proposes Visual-Registered Speaker Diarization and Recognition (VR-SDR) paradigm and HumanOmni-Speaker Benchmark that eliminates visual shortcuts. Introduces HumanOmni-Speaker model with a Visual Delta Encoder that samples raw video at 25 fps and compresses inter-frame motion residuals into just 6 tokens per frame to capture fine-grained visemes and speaker trajectories without token explosion.

Result: HumanOmni-Speaker demonstrates strong multimodal synergy, enabling end-to-end lip-reading and high-precision spatial localization without intrusive cropping. It achieves superior performance across a wide spectrum of speaker-centric tasks by capturing crucial conversational dynamics.

Conclusion: The proposed approach shatters the "illusion of competence" in current models by enabling true end-to-end spatio-temporal identity binding using only natural language queries, overcoming architectural perception gaps and providing robust multi-person conversational understanding.
...
TL;DR: RefracGS is a novel view synthesis framework that handles non-planar refractive surfaces by jointly reconstructing the refractive water surface and underlying scene using neural height fields and 3D Gaussian fields with refraction-aware ray tracing.


<details>
  <summary>Details</summary>
Motivation: Traditional NVS methods like NeRF and 3DGS assume straight-line ray propagation, which fails for scenes with refractive surfaces like water waves, causing severe artifacts due to spatially varying optical distortions.

Method: Decouples refractive boundary from target objects: models refractive surface via neural height field for wave geometry, and underlying scene as 3D Gaussian field. Uses refraction-aware Gaussian ray tracing with Snell's law to compute non-linear ray trajectories and enables end-to-end joint optimization.

Result: Outperforms prior refractive methods in visual quality on synthetic and real-world scenes with complex waves, achieves 15x faster training and real-time rendering at 200 FPS.

Conclusion: RefracGS successfully addresses the challenge of novel view synthesis through refractive surfaces by explicitly modeling refraction physics while maintaining efficiency and high-quality rendering performance.
...
TL;DR: PPGL-Swarm: An agentic diagnostic system for pheochromocytomas/paragangliomas that automates GAPP scoring, incorporates genotype risk factors, and provides traceable reasoning through specialized agents.


<details>
  <summary>Details</summary>
Motivation: Current PPGL diagnosis has limitations: GAPP scoring is labor-intensive with subjective components, misses important metastatic risk factors like SDHB mutations, and existing AI systems lack traceable reasoning and domain-specific knowledge integration.

Method: PPGL-Swarm uses an agentic system with specialized agents for micro-tasks, knowledge enhancement for genotype/lab interpretation, automated GAPP scoring with quantified cellularity/Ki-67, and reinforcement learning for tool selection/task assignment optimization.

Result: The system generates comprehensive diagnostic reports with automated GAPP scoring, genotype risk alerts, multimodal evidence integration, and auditable reasoning trails through decomposed micro-tasks.

Conclusion: PPGL-Swarm addresses current diagnostic limitations by providing automated, comprehensive, and traceable PPGL diagnosis with better incorporation of genotype risk factors and reduced clinician workload.
...
TL;DR: MetaCompress is a learning-based token reduction method for multi-turn VQA that formulates token compression as a learnable mapping, achieving better efficiency-accuracy trade-offs than heuristic approaches.


<details>
  <summary>Details</summary>
Motivation: Existing token reduction methods for LVLMs focus on single-turn VQA and are ineffective for multi-turn scenarios where subsequent questions are unknown and may refer to arbitrary image regions. Current approaches either bias toward initial prompts or rely on suboptimal heuristic metrics.

Method: Formulates token reduction as a learnable compression mapping that unifies pruning and merging into a single learning objective. Introduces a data-efficient training paradigm to learn optimal compression mappings with limited computational costs.
...
Conclusion: MetaCompress overcomes limitations of heuristic token reduction methods for multi-turn VQA through a learning-based approach that effectively balances computational efficiency with visual reasoning accuracy.
...
TL;DR: Pointing (predicting object coordinates before counting) improves LVLM accuracy and generalization in zero-shot counting tasks by providing spatial grounding, though spatial biases exist in point predictions.


<details>
  <summary>Details</summary>
Motivation: While pointing has been shown to increase LVLM accuracy, the underlying mechanisms and reliability of intermediate points as visual explanations remain unclear, especially for cognitive tasks like counting.

Method: Fine-tuned state-of-the-art LVLMs using two approaches: Direct Counting (predict only total count) and Point-then-Count (predict object coordinates first, then count). Evaluated on zero-shot counting from visual scenes.

Result: Point-then-Count achieves higher out-of-distribution generalization than Direct Counting. Predicted points are accurately grounded (over 89% F1), but performance varies across image regions revealing spatial biases. Gains come from spatial information in coordinates.

Conclusion: Pointing helps LVLMs learn counting skills rather than overfitting, with coordinates providing valuable spatial information, though spatial biases in point predictions limit their reliability as visual explanations.
...
TL;DR: DaP-ICoT improves interleaved multimodal reasoning by dynamically integrating visual thoughts when needed and ensuring precise visual representations, achieving SOTA performance with 72.6% fewer tokens.


<details>
  <summary>Details</summary>
Motivation: Current Interleaved-modal Chain-of-Thought (ICoT) methods suffer from two limitations: (1) Static Visual Thought Positioning - inserting visual information at fixed steps leads to inefficient and inflexible reasoning, and (2) Broken Visual Thought Representation - visual tokens are discontinuous and semantically incoherent.

Method: DaP-ICoT introduces two key components: (1) Dynamic Visual Thought Integration that adaptively introduces visual inputs based on reasoning needs, reducing redundancy, and (2) Precise Visual Thought Guidance that ensures visual semantically coherent and contextually aligned representations.

Result: Experiments across multiple benchmarks and models demonstrate that DaP-ICoT achieves state-of-the-art performance. It significantly reduces the number of inserted images, leading to a 72.6% decrease in token consumption, enabling more efficient ICoT reasoning.

Conclusion: DaP-ICoT addresses the limitations of current ICoT methods by providing dynamic and precise visual thought integration, achieving both superior performance and significant efficiency improvements in multimodal reasoning.
...
TL;DR: SHARP enables high-resolution remote sensing image generation by combining a domain-specialized FLUX model (RS-FLUX) with a training-free resolution promotion method that dynamically adjusts positional embeddings during denoising.


<details>
  <summary>Details</summary>
Motivation: Remote sensing image synthesis lags behind general text-to-image generation due to: 1) lack of domain-specialized DiT priors, and 2) prohibitive training costs at large resolutions. Existing training-free resolution promotion methods use static positional scaling throughout denoising, which harms RS imagery where medium- and high-frequency details (vehicles, building contours, road markings) are critical for realism.

Method: Two main components: 1) Fine-tune FLUX on 100,000+ curated RS images to create RS-FLUX (domain-specialized prior), and 2) SHARP - a training-free method that introduces a rational fractional time schedule k_rs(t) into Rotary Position Embedding (RoPE). SHARP applies strong positional promotion early in layout formation and progressively relaxes it during detail recovery, aligning with diffusion's frequency-progressive denoising nature.

Result: SHARP consistently outperforms all training-free baselines across six square and rectangular resolutions on CLIP Score, Aesthetic Score, and HPSv2 metrics. Performance advantages widen at more aggressive extrapolation factors with negligible computational overhead.

Conclusion: The combination of domain-specialized RS-FLUX with SHARP's denoising-aware positional adaptation strategy effectively addresses remote sensing image synthesis challenges, enabling robust multi-scale generation from a single set of hyperparameters while preserving critical fine structures.
...
TL;DR: DEBIR introduces a burst image restoration pipeline that dynamically predicts optimal exposure times for each burst image to enhance restoration quality, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing burst image restoration methods use manually designed exposure settings, but optimal exposure settings significantly impact restoration performance and have been overlooked. The paper aims to address this gap by dynamically predicting exposure times tailored to the shooting environment.

Method: DEBIR pipeline includes: 1) Burst Auto-Exposure Network (BAENet) that estimates optimal exposure time for each burst image based on preview image, motion magnitude, and gain; 2) Burst image restoration network that reconstructs high-quality image from burst images captured with optimal exposure times; 3) Differentiable burst simulator and three-stage training strategy for effective training.

Result: The pipeline achieves state-of-the-art restoration quality in experiments. The approach is validated on a real-world camera system, demonstrating practical applicability.

Conclusion: DEBIR successfully addresses the overlooked problem of optimal exposure settings in burst image restoration by dynamically predicting exposure times, leading to improved restoration quality and practical real-world applicability.
...
TL;DR: RL framework for online tuning of VO frontend parameters using image-conditioned policy to adapt to scene characteristics, improving feature tracking and reducing computational cost.


<details>
  <summary>Details</summary>
Motivation: Current VO systems use fixed hyperparameters for feature detection/tracking despite varying optimal values with scene characteristics (texture, illumination, motion blur), leading to brittle performance in real-world environments.

Method: Image-conditioned reinforcement learning framework with sequential decision-making formulation. Uses lightweight texture-aware CNN encoder and privileged critic during training to map visual input to feature detection/tracking parameters.

Result: 3x longer feature tracks and 3x lower computational cost on TartanAirV2 and TUM RGB-D datasets, despite training entirely in simulation.

Conclusion: First RL framework for online VO frontend parameter tuning that proactively adapts to scene content, significantly improving tracking performance and efficiency compared to fixed-parameter approaches.
...
TL;DR: The paper proposes the Universal Normal Embedding (UNE) hypothesis that generative models and vision encoders share a common Gaussian latent space, with encoder embeddings and diffusion noise being noisy linear projections of this shared source.


<details>
  <summary>Details</summary>
Motivation: Generative models and vision encoders have developed separately with different mathematical foundations, but both exhibit latent space Gaussianity. The authors hypothesize they might share a common underlying latent source.

Method: Introduced NoiseZoo dataset containing per-image latents (DDIM-inverted diffusion noise and encoder representations like CLIP, DINO). Used linear probes to test semantic alignment and performed controllable image editing via linear directions in latent space.

Result: Linear probes in both generative noise and encoder spaces showed strong, aligned attribute predictions. Linear directions enabled faithful controllable edits (smile, gender, age) without architectural changes, with orthogonalization mitigating entanglements.

Conclusion: Results provide empirical support for the UNE hypothesis, revealing a shared Gaussian-like latent geometry that concretely links encoding and generation, suggesting both arise from a common latent source.
...
TL;DR: Benchmarking recurrent YOLOv8s on MTEvent dataset for industrial multi-class object detection shows event-domain pretraining (GEN1) yields best results (0.329 mAP50), while mismatched pretraining (PEDRo) underperforms even scratch training.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer advantages for industrial robotics (high temporal resolution, dynamic range, reduced blur), but most event-based detection research focuses on outdoor driving or limited classes, leaving industrial multi-class recognition underexplored.

Method: Benchmark recurrent ReYOLOv8s on MTEvent dataset for industrial multi-class recognition, using non-recurrent YOLOv8s as baseline to analyze temporal memory effects. Compare scratch training vs. event-domain pretraining (GEN1) vs. mismatched pretraining (PEDRo).

Result: Best scratch recurrent model achieves 0.285 mAP50 (9.6% improvement over non-recurrent baseline). GEN1-initialized fine-tuning yields best overall result (0.329 mAP50) and improves consistently with clip length. PEDRo initialization drops to 0.251, worse than scratch training. Main failure modes: class imbalance and human-object interaction.

Conclusion: Event-domain pretraining (GEN1) is crucial for industrial event-based detection, while mismatched pretraining can be detrimental. This work provides focused benchmarking and analysis of recurrent event-based detection in industrial environments, highlighting persistent challenges.
...
TL;DR: TIC-TALK is a multimodal dataset of 90 stand-up comedy specials (2015-2024) with 5,400+ topic segments aligned across language, gesture, and audience laughter, enabling analysis of performance dynamics beyond just verbal content.


<details>
  <summary>Details</summary>
Motivation: Current humor research focuses too much on verbal content, ignoring the embodied performance aspects and audience feedback that are crucial for live stand-up comedy. There's a need for multimodal resources that capture the full performance experience.

Method: Combines BERTopic for thematic segmentation (60s segments), Whisper-AT for laughter detection (0.8s), YOLOv8-cls for shot classification, and YOLOv8s-pose for skeletal keypoint extraction (1 fps). Computes continuous kinematic signals (arm spread, kinetic energy, trunk lean) from raw 17-joint coordinates without clustering.

Result: Kinetic energy negatively predicts audience laughter rate (r = -0.75), supporting stillness-before-punchline pattern. Personal/bodily content elicits more laughter than geopolitical themes. Shot close-up proportion positively correlates with laughter (r = +0.28), consistent with reactive montage.

Conclusion: TIC-TALK enables multimodal analysis of stand-up comedy performance dynamics, revealing how embodied performance elements (kinetic energy, shot framing) interact with thematic content to elicit audience laughter, moving beyond purely verbal humor analysis.
...
TL;DR: A novel active sampling framework for accelerated MRI that uses a pretrained medical image tokenizer and latent transformer to guide k-space sampling based on token entropy uncertainty measures.


<details>
  <summary>Details</summary>
Motivation: Full MRI data acquisition is slow, limiting clinical throughput and patient comfort. Compressed Sensing MRI aims to accelerate this but requires both optimal sampling trajectories and high-fidelity reconstruction models.

Method: Uses a pretrained medical image tokenizer and latent transformer to represent anatomy through quantized visual tokens. Derives uncertainty measures via token entropy and introduces two strategies: Latent Entropy Selection (LES) projects patch-wise token entropy to k-space to identify informative sampling lines, and Gradient-based Entropy Optimization (GEO) finds regions of maximum uncertainty reduction via k-space gradient of total latent entropy loss.

Result: The framework outperforms state-of-the-art baselines on fastMRI single-coil Knee and Brain datasets at 8× and 16× acceleration in perceptual metrics and feature-based distances.

Conclusion: The proposed active sampling framework effectively leverages latent space uncertainty from medical image tokenizers to guide optimal k-space sampling, achieving superior acceleration performance compared to existing methods.
...
TL;DR: Proposes a cascade-free multitask learning architecture for Chinese Mandarin VSR that jointly integrates phoneme and viseme representations with semantic-guided contrastive loss to improve accuracy while reducing error accumulation and latency.


<details>
  <summary>Details</summary>
Motivation: Chinese Mandarin VSR lags behind non-tonal languages due to tonal nature limitations. Existing cascade architectures with intermediate representations (like pinyin) cause error accumulation and increased inference latency from stage dependencies.

Method: Cascade-free architecture based on multitask learning that jointly integrates multiple intermediate representations (phoneme and viseme). Uses semantic-guided local contrastive loss for temporal feature alignment, enabling on-demand activation during inference.

Result: Superior recognition performance demonstrated on publicly available datasets. The approach provides trade-off between inference efficiency and performance while mitigating error accumulation from projection and re-embedding.

Conclusion: The proposed cascade-free multitask learning approach effectively addresses limitations of existing Chinese VSR systems by eliminating error accumulation and reducing latency while improving recognition accuracy through better contextual information exploitation.
...
TL;DR: CGMD transfers MRI-derived hypertension knowledge to fundus models using clinical similarity graphs, improving HTN prediction without paired multimodal data.


<details>
  <summary>Details</summary>
Motivation: Retinal fundus imaging is cost-effective for HTN screening but yields high-variance predictions due to subtle cues. Brain MRI provides stronger HTN markers but is expensive and rarely paired with fundus images, creating modality-siloed datasets.

Method: CGMD constructs a clinical similarity kNN graph using shared biomarkers to bridge unpaired MRI and fundus cohorts. It trains an MRI teacher, propagates representations over the graph, imputes brain-informed targets for fundus patients, and trains a fundus student with joint HTN supervision, target distillation, and relational distillation.

Result: Experiments on unpaired MRI-fundus-biomarker dataset show CGMD consistently improves fundus-based HTN prediction over standard distillation and non-graph imputation baselines, with ablations confirming the importance of clinically grounded graph connectivity.

Conclusion: CGMD effectively transfers MRI-derived HTN knowledge to fundus models without requiring paired multimodal data, leveraging clinical similarity graphs to bridge modality-siloed datasets and improve hypertension screening.
...
TL;DR: Ctrl-A is an automated data augmentation algorithm that uses control theory to dynamically adjust augmentation strength distributions during training, eliminating manual policy engineering.


<details>
  <summary>Details</summary>
Motivation: Traditional data augmentation requires manual tuning of augmentation strengths and policies for each new task, which is time-consuming and suboptimal. The paper aims to automate this process using control theory principles.

Method: Ctrl-A uses a control-loop architecture with relative operation response curves to dynamically and individually adapt augmentation strength distributions during training. It employs an operation-dependent update procedure that can suppress harmful augmentation styles.

Result: Experiments on CIFAR-10, CIFAR-100, and SVHN-core datasets with WideResNet-28-10 show Ctrl-A is highly competitive with state-of-the-art data augmentation strategies.

Conclusion: Ctrl-A successfully automates data augmentation policy design using control theory, eliminating manual engineering while achieving competitive performance with existing methods.
...
TL;DR: This paper challenges the need for strictly paired training data in infrared-visible image fusion, proposing unpaired and arbitrarily paired training paradigms that achieve comparable performance with 100× less data.


<details>
  <summary>Details</summary>
Motivation: Current infrared-visible image fusion methods require extensive sets of rigidly aligned image pairs, which are costly and labor-intensive to acquire. This rigid pairing restricts cross-modal relationships and limits generalization performance.

Method: The authors propose UnPaired and Arbitrarily Paired Training Paradigms (UPTP and APTP) as alternatives to Strictly Paired Training Paradigm (SPTP). They develop a practical framework with three end-to-end lightweight baselines covering CNN, Transformer, and GAN architectures, along with innovative loss functions to handle limited and unaligned training data.

Result: Comprehensive experiments show that APTP and UPTP can train models on severely limited and content-inconsistent datasets, achieving performance comparable to models trained on datasets 100× larger using SPTP.

Conclusion: The proposed training paradigms fundamentally alleviate data collection costs and difficulties while enhancing model robustness, providing a feasible solution for infrared-visible image fusion studies that reduces dependency on perfectly aligned paired data.
...
TL;DR: SteelDefectX: A vision-language dataset with 7,778 steel defect images across 25 categories, featuring coarse-to-fine textual annotations to improve interpretability and generalization in defect detection.


<details>
  <summary>Details</summary>
Motivation: Current steel defect detection methods rely on basic image classification with label-only datasets, limiting interpretability and generalization. There's a need for richer, more detailed defect representations that can improve model performance and explainability.

Method: Created SteelDefectX dataset with 7,778 images across 25 defect categories, annotated with coarse-to-fine textual descriptions. Coarse-grained level includes class-level information (defect categories, visual attributes, industrial causes), while fine-grained level captures sample-specific attributes (shape, size, depth, position, contrast). Established benchmark with four tasks: vision-only classification, vision-language classification, few/zero-shot recognition, and zero-shot transfer.

Result: Experiments with baseline models show that coarse-to-fine textual annotations significantly improve interpretability, generalization, and transferability. The dataset enables models to learn richer and more detailed defect representations.

Conclusion: SteelDefectX serves as a valuable resource for advancing research on explainable, generalizable steel surface defect detection. The dataset will be publicly available to support further research in this domain.
...
TL;DR: MDSVM-UNet: A two-stage coronary artery segmentation framework combining multidirectional snake convolution with residual visual Mamba for efficient long-range dependency modeling.


<details>
  <summary>Details</summary>
Motivation: Coronary artery segmentation from CTA images is clinically important but challenging due to vessels' multi-branching, slender tubular morphology and severe class imbalance. CNN-based methods struggle with long-range dependencies, while ViT-based approaches have prohibitive computational overhead for clinical deployment.

Method: Two-stage framework: 1) Encoding stage uses MDSConv (multidirectional snake convolution) with deformable convolution learning adaptive offsets along three anatomical planes for multi-view feature fusion. 2) Decoding stage uses RVM-based upsampling decoder with selective state space mechanisms for linear-complexity long-range dependency modeling. Progressive two-stage segmentation strategy: coarse whole-image segmentation followed by fine-grained block-level segmentation.

Result: The paper proposes a novel framework that synergistically integrates MDSConv and RVM to address both geometric modeling challenges (through multi-view fusion) and computational efficiency issues (through linear-complexity state space models).

Conclusion: MDSVM-UNet offers an efficient solution for coronary artery segmentation that captures complex vascular geometry while maintaining computational efficiency suitable for clinical deployment, addressing limitations of both CNN and ViT approaches.
...
TL;DR: A video diffusion model trained on atmospheric reanalysis generates realistic Madden-Julian oscillation sequences conditioned on low-dimensional metrics, enabling deconstruction of physical processes and bridging theory with atmospheric complexity.


<details>
  <summary>Details</summary>
Motivation: While generative deep learning can model the MJO, its relationship to traditional theoretical frameworks remains poorly understood. There's a need to bridge the gap between low-dimensional MJO theory and high-resolution atmospheric complexity.

Method: Proposes a video diffusion model trained on atmospheric reanalysis data to synthesize long MJO sequences conditioned on key low-dimensional metrics. The model can generate idealized MJOs based on intentionally simplified conditionings (perpetual MJO, isolated seasonal/ENSO modulation, etc.).

Result: Generated MJOs capture key features including composites, power spectra, and multiscale structures including convectively coupled waves, despite some bias. The model enables deconstructing underlying processes and identifying physical drivers.

Conclusion: The approach provides a practical framework for bridging low-dimensional MJO theory with high-resolution atmospheric complexity and will help tropical atmosphere prediction.
...
TL;DR: Novel distillation framework for video diffusion models that addresses artifacts from image distillation techniques through adaptive spatial supervision, temporal regularization, and inference-time interpolation.


<details>
  <summary>Details</summary>
Motivation: Video generation is computationally expensive, making distillation crucial for efficient deployment. Current methods directly adapt image distillation techniques, leading to artifacts like oversaturation, temporal inconsistency, and mode collapse in video synthesis.

Method: Three core innovations: (1) adaptive regression loss that dynamically adjusts spatial supervision weights to prevent artifacts from excessive distribution shifts; (2) temporal regularization loss to counteract temporal collapse and promote smooth sampling trajectories; (3) inference-time frame interpolation strategy to reduce sampling overhead while preserving quality.

Result: Extensive experiments on VBench and VBench2 benchmarks show the method achieves stable few-step video synthesis, significantly enhancing perceptual fidelity and motion realism. Consistently outperforms existing distillation baselines across multiple metrics.

Conclusion: Proposed distillation framework effectively addresses video-specific artifacts from image distillation techniques, enabling efficient and high-quality video generation through specialized spatial and temporal regularization mechanisms.
...
TL;DR: Adversarial Camouflage is a privacy protection method that creates optimized patterns to degrade face recognition systems, working across multiple models and showing real-world effectiveness.


<details>
  <summary>Details</summary>
Motivation: The rapid development of facial recognition algorithms has enabled beneficial applications but also raised significant concerns about mass surveillance and threats to individual privacy, creating a need for practical privacy protection solutions.

Method: The approach defines a low-dimensional pattern space parameterized by color, shape, and angle, then optimizes patterns and projects them onto semantically valid facial regions to maximize recognition error across multiple architectures, ensuring cross-model transferability.

Result: The method significantly degrades performance of all tested state-of-the-art face recognition models in simulations and shows promising results in real-world human experiments, while revealing differences in model robustness and evidence of attack transferability across architectures.

Conclusion: Adversarial Camouflage presents an efficient, reproducible solution for protecting user privacy against facial recognition systems, demonstrating effectiveness across multiple models and real-world scenarios.
...
TL;DR: SAGE-GRPO is a new method for stable alignment of video generation models that constrains exploration to stay near the pre-trained model's data manifold, improving rollout quality and reward reliability compared to previous GRPO methods.


<details>
  <summary>Details</summary>
Motivation: Current Group Relative Policy Optimization (GRPO) methods for video generation are unreliable compared to language/image models due to complex solution space and ODE-to-SDE conversion injecting excess noise, which lowers rollout quality and destabilizes post-training alignment.

Method: SAGE-GRPO applies constraints at micro and macro levels: micro-level includes manifold-aware SDE with logarithmic curvature correction and gradient norm equalizer; macro-level uses dual trust region with periodic moving anchor and stepwise constraints to limit long-horizon drift.

Result: Evaluation on HunyuanVideo1.5 using VideoAlign reward model shows consistent gains over previous methods in VQ, MQ, TA, and visual metrics (CLIPScore, PickScore), demonstrating superior reward maximization and overall video quality.

Conclusion: Constraining exploration within the vicinity of the pre-trained model's data manifold is crucial for stable video generation alignment, and SAGE-GRPO's multi-level constraint approach effectively addresses the reliability issues of previous GRPO methods.
...
TL;DR: UPPA is the first universal physical attack method for infrared pedestrian detectors using parameterized Bezier blocks and PSO optimization, materialized as physical cold patches for effective attacks without online computation.


<details>
  <summary>Details</summary>
Motivation: Existing physical attack methods for infrared detectors rely on instance-specific online optimization and rigid pattern design, resulting in high deployment costs and insufficient physical robustness.

Method: Uses geometrically constrained parameterized Bezier blocks to model perturbations, employs Particle Swarm Optimization (PSO) for unified optimization across global data distribution, and materializes digital perturbations into physical cold patches with smooth low-temperature distribution.

Result: Achieves outstanding physical attack success rate without online computational overhead, exhibits strong cross-domain generalization, and demonstrates reliable black-box transferability.

Conclusion: UPPA provides an effective universal physical attack framework for infrared pedestrian detectors that addresses limitations of existing methods through geometric modeling, global optimization, and physical materialization.
...
TL;DR: Integration of modern learning-based stereo matchers into satellite pipelines improves DSM accuracy and geometric detail compared to classical approaches, though vegetation remains challenging.


<details>
  <summary>Details</summary>
Motivation: Learning-based stereo matchers achieve SOTA on benchmarks but aren't integrated into operational satellite pipelines due to viewing geometry and disparity assumption differences.

Method: Integrate StereoAnywhere, MonSter, Foundation Stereo, and satellite fine-tuned MonSter into Satellite Stereo Pipeline, adapting rectification for compatible disparity polarity and range.

Result: Consistent improvements over classical cost-volume approaches in DSM accuracy, with substantially improved geometric detail and sharper structures, though vegetation performance remains limited.

Conclusion: Learning-based stereo matchers can enhance satellite pipelines, but need better evaluation strategies for perceptual/structural fidelity and improved performance on challenging surfaces like vegetation.
...
TL;DR: LoRA² adaptively learns optimal ranks for each layer during diffusion model fine-tuning, achieving better performance-memory tradeoffs than fixed-rank LoRA.


<details>
  <summary>Details</summary>
Motivation: Current LoRA fine-tuning uses fixed ranks for all layers regardless of subject complexity, leading to suboptimal memory-performance tradeoffs. Finding optimal per-layer ranks is combinatorially expensive.

Method: Proposes LoRA² which learns adaptive ranks per layer during fine-tuning by imposing importance ordering on rank positions, encouraging higher ranks only when necessary.

Result: Achieves competitive DINO, CLIP-I, and CLIP-T scores across 29 subjects while requiring less memory and lower overall rank than high-rank LoRA versions.

Conclusion: LoRA² provides an effective adaptive rank selection method for diffusion model personalization, overcoming the limitations of fixed-rank approaches.
...
TL;DR: CLEAR is a mask-free, end-to-end video subtitle removal framework that uses two-stage context-aware adaptive learning with minimal parameter overhead (0.77% of base diffusion model).


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based methods require explicit mask sequences during both training and inference, which limits practical deployment. There's a need for truly end-to-end mask-free solutions.

Method: Two-stage design: Stage I learns disentangled subtitle representations via self-supervised orthogonality constraints on dual encoders. Stage II uses LoRA-based adaptation with generation feedback for dynamic context adjustment.

Result: Outperforms mask-dependent baselines by +6.77dB PSNR and -74.7% VFID on Chinese benchmarks. Demonstrates superior zero-shot generalization across six languages (English, Korean, French, Japanese, Russian, German).

Conclusion: CLEAR achieves robust subtitle removal without ground-truth masks during inference through generation-driven feedback, enabling practical deployment with minimal parameter overhead.
...
TL;DR: SHAPE is a new UDA framework for medical segmentation that focuses on global anatomical plausibility through hierarchical feature modulation and hypergraph-based plausibility estimation.


<details>
  <summary>Details</summary>
Motivation: Existing UDA methods for medical segmentation have two key limitations: 1) semantically unaware feature alignment leading to poor distributional fidelity, and 2) pseudo-label validation that ignores global anatomical constraints, allowing globally implausible structures to form.

Method: SHAPE uses DINOv3 foundation with Hierarchical Feature Modulation (HFM) for high-fidelity class-aware features. It introduces Hypergraph Plausibility Estimation (HPE) to assess global anatomical plausibility beyond standard graphs, plus Structural Anomaly Pruning (SAP) to remove artifacts via cross-view stability.

Result: State-of-the-art performance on cardiac and abdominal cross-modality benchmarks: 90.08% (MRI->CT) and 78.51% (CT->MRI) Dice on cardiac data; 87.48% (MRI->CT) and 86.89% (CT->MRI) Dice on abdominal data.
...
TL;DR: A latent representation framework for efficient hyperspectral image generation that outperforms traditional methods in accuracy and fidelity while maintaining downstream application performance.


<details>
  <summary>Details</summary>
Motivation: Traditional radiative transfer models for synthetic hyperspectral image generation are computationally expensive and often limited to spectrum-level outputs, creating a need for more efficient and comprehensive emulation methods.

Method: A latent representation-based framework that learns generative representations of hyperspectral data, supporting both spectrum-level and spatial-spectral emulation, with training options including direct one-step formulation or two-step VAE pretraining with parameter-to-latent interpolation.

Result: The method outperforms classical regression-based emulators in reconstruction accuracy, spectral fidelity, and robustness to real-world spatial variability on PROSAIL-simulated vegetation data and Sentinel-3 OLCI imagery.

Conclusion: The framework provides practical relevance for remote sensing applications as emulated hyperspectral images preserve performance in downstream biophysical parameter retrieval tasks.
...
TL;DR: GOLD proposes a method for Continual Test-Time Adaptation that identifies a minimal "golden subspace" for efficient adaptation, using AGOP to maintain this subspace online with lightweight updates.


<details>
  <summary>Details</summary>
Motivation: Existing CTTA methods face an efficiency-generalization trade-off: updating more parameters improves adaptation but reduces online inference efficiency. The paper aims to find a minimal subspace for adaptation that maintains performance while being efficient.

Method: Proves existence of a "golden subspace" that coincides with the row space of the pretrained classifier. Uses sample-wise Average Gradient Outer Product (AGOP) as an efficient proxy for estimating classifier weights. Proposes GOLD with a lightweight adapter that projects features onto the golden subspace and learns a compact scaling vector while dynamically updating the subspace via AGOP.

Result: Extensive experiments on classification and segmentation benchmarks, including autonomous-driving scenarios, demonstrate that GOLD attains superior efficiency, stability, and overall performance compared to existing methods.

Conclusion: GOLD successfully addresses the efficiency-generalization trade-off in CTTA by identifying and maintaining a minimal golden subspace for adaptation, achieving strong performance with efficient online updates.
...
TL;DR: SatGeo-NeRF introduces three model-agnostic regularizers to reduce geometric artifacts in NeRF for satellite imagery, improving altitude accuracy by 13.9-11.7% over state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: Current NeRF models for satellite imagery suffer from overfitting-induced geometric artifacts that degrade reconstruction quality, particularly in geometric accuracy for satellite-based applications.

Method: Three model-agnostic regularizers: 1) Gravity-Aligned Planarity Regularization aligns depth-inferred surface normals with gravity axis to promote local planarity and enable cross-ray gradient flow; 2) Granularity Regularization enforces coarse-to-fine geometry learning; 3) Depth-Supervised Regularization stabilizes early training.

Result: On DFC2019 satellite reconstruction benchmark, SatGeo-NeRF improves Mean Altitude Error by 13.9% relative to EO-NeRF and 11.7% relative to EO-GS, demonstrating superior geometric accuracy.

Conclusion: Geometric regularization effectively mitigates overfitting artifacts in satellite NeRF models, with the proposed regularizers significantly improving reconstruction accuracy for satellite imagery applications.
...
TL;DR: Camera-agnostic pruning method for 3D Gaussian splats using attribute-derived neighborhood descriptors and statistical evidence estimation.


<details>
  <summary>Details</summary>
Motivation: Existing pruning strategies for 3D Gaussian splats depend on camera parameters, rendered images, or view-dependent measures, which hinders camera-agnostic exchange settings where splats are shared directly as point-based representations.

Method: Proposes a camera-agnostic, one-shot, post-training pruning method that uses hybrid descriptor framework capturing structural and appearance consistency directly from splat representation. Formulates pruning as statistical evidence estimation problem with Beta evidence model that quantifies per-splat reliability through probabilistic confidence scores.

Result: Experiments on standardized ISO/IEC MPEG CTC test sequences demonstrate substantial pruning while preserving reconstruction quality, establishing a practical and generalizable alternative to camera-dependent pruning strategies.

Conclusion: The proposed method provides an effective camera-agnostic pruning solution for 3D Gaussian splats that works in emerging exchange settings where splats are shared directly as point-based representations.
...
TL;DR: ChronoCon uses contrastive learning with temporal ordering of longitudinal scans to learn disease severity representations without expert labels, improving label efficiency for irreversible diseases like rheumatoid arthritis.


<details>
  <summary>Details</summary>
Motivation: Quantitative disease severity scoring is expensive, time-consuming, and variable between readers. Clinical archives contain abundant longitudinal imaging data but few expert-annotated scores. Existing self-supervised methods ignore chronological structure in patient scans.

Method: ChronoCon is a contrastive learning approach that replaces label-based ranking losses with rankings derived solely from the visitation order of a patient's longitudinal scans. It assumes monotonic progression in irreversible diseases and generalizes Rank-N-Contrast from label distances to temporal ordering.

Result: On rheumatoid arthritis radiographs, ChronoCon substantially improves label efficiency. In low-label settings, it significantly outperforms fully supervised baselines. Fine-tuning on expert scores from only five patients yields an intraclass correlation coefficient of 86% for severity prediction.

Conclusion: Chronological contrastive learning can exploit routinely available imaging metadata to reduce annotation requirements in irreversible disease domains, demonstrating strong potential for clinical applications.
...
TL;DR: GSA is a novel method for aligning two independent 3D Gaussian Splatting models via similarity transformation, even for different objects in the same category, while existing methods only work for identical objects.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS alignment methods are limited to aligning models of the same object and often require true scale as input. There's a need for category-level registration that can align different objects within the same category without prior scale knowledge.

Method: Uses viewpoint-guided spherical map features for robust correspondences with a two-step optimization: 1) iterative feature-guided absolute orientation solver for coarse registration, robust to poor initialization; 2) fine registration enforcing multi-view feature consistency inspired by inverse radiance-field formulations.

Result: Outperforms prior works in same-object alignment (often by large margins) even when competitors are given true scale. Provides first effective solution for category-level 3DGS registration, vastly surpassing existing methods for different objects in same category.

Conclusion: GSA enables effective category-level 3DGS registration, unlocking new applications by successfully aligning different objects within the same category without requiring true scale as input.
...
TL;DR: MultiBind is a new benchmark for evaluating cross-subject attribute misbinding in multi-reference image generation, featuring real multi-person photos with structured annotations and a dimension-wise confusion evaluation protocol.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks and metrics for subject-driven image generation focus on holistic fidelity or per-subject self-similarity, making it difficult to diagnose cross-subject attribute misbinding failures where attributes are preserved, edited, or transferred to the wrong subject in multi-person scenes.

Method: The paper introduces MultiBind benchmark built from real multi-person photographs with slot-ordered subject crops, masks, bounding boxes, canonicalized references, inpainted background, and dense entity-indexed prompts. It also proposes a dimension-wise confusion evaluation protocol that matches generated subjects to ground-truth slots and measures slot-to-slot similarity using specialists for face identity, appearance, pose, and expression.

Result: Experiments on modern multi-reference generators show that MultiBind reveals binding failures that conventional reconstruction metrics miss. The method separates self-degradation from true cross-subject interference and exposes interpretable failure patterns such as drift, swap, dominance, and blending.

Conclusion: MultiBind provides a comprehensive benchmark and evaluation protocol specifically designed to diagnose cross-subject attribute misbinding in multi-reference image generation, addressing a key limitation in current evaluation methods for multi-person scene generation.
...
TL;DR: FeatDistill: A robust AI-generated image detection framework using multi-expert ViT ensemble with feature distillation to address real-world challenges like degradation interference and limited generalization.


<details>
  <summary>Details</summary>
Motivation: Deepfake technology's rapid advancement poses severe information security threats, creating need for robust and generalizable detection methods that work in real-world conditions with various degradations and unseen generators.

Method: Four-backbone ViT ensemble (CLIP and SigLIP variants) with expanded training set and comprehensive degradation modeling. Two-stage training: binary classification followed by dense feature-level self-distillation for representation alignment.

Result: Achieves strong robustness and generalization under diverse "in-the-wild" conditions in NTIRE challenge setting. Framework remains efficient (~10GB peak GPU memory) despite ensemble design.

Conclusion: FeatDistill offers effective and practical solution for real-world deepfake detection by addressing three key bottlenecks: degradation interference, insufficient feature representation, and limited generalization through multi-expert ensemble with feature distillation.
...
TL;DR: GeoFlow introduces a lightweight framework for real-time fine-grained cross-view geolocalization that breaks the accuracy-speed trade-off using probabilistic displacement mapping and iterative refinement sampling.


<details>
  <summary>Details</summary>
Motivation: Current fine-grained cross-view geolocalization methods force a difficult trade-off between accuracy and speed, making high-accuracy models too slow for real-time autonomous navigation in GPS-denied areas.

Method: GeoFlow learns a direct probabilistic mapping to predict displacement (distance and direction) needed to correct location hypotheses. It uses Iterative Refinement Sampling (IRS) - an inference algorithm that refines a population of hypotheses iteratively, allowing them to 'flow' from random starting points to a converged consensus.

Result: GeoFlow achieves state-of-the-art efficiency with real-time speeds of 29 FPS while maintaining competitive localization accuracy on KITTI and VIGOR datasets. It offers flexible inference-time scaling for performance-computation trade-offs without retraining.

Conclusion: GeoFlow breaks the accuracy-speed trade-off in fine-grained cross-view geolocalization, opening a new path for practical real-time geolocalization systems for autonomous navigation in GPS-denied environments.
...
TL;DR: Group3D integrates semantic constraints into multi-view open-vocabulary 3D detection to prevent geometry-only merging errors by using semantic compatibility groups from MLLMs.


<details>
  <summary>Details</summary>
Motivation: Existing approaches decouple geometry-based instance construction from semantic labeling, leading to irreversible association errors like over-merging or fragmentation when geometric evidence is incomplete or view-dependent.

Method: Group3D maintains a scene-adaptive vocabulary from MLLMs, organizes it into semantic compatibility groups encoding plausible cross-view category equivalence, and uses these groups as merge-time constraints to associate 3D fragments only when they satisfy both semantic compatibility and geometric consistency.

Result: Group3D achieves state-of-the-art performance on ScanNet and ARKitScenes for multi-view open-vocabulary 3D detection and shows strong generalization in zero-shot scenarios.

Conclusion: Integrating semantic constraints directly into instance construction via semantic compatibility groups effectively mitigates geometry-driven merging errors while handling multi-view category variability, advancing open-vocabulary 3D detection.
...
TL;DR: A unified spatiotemporal token compression method for Video-LLMs that globally selects and merges tokens using attention weights and semantic similarity, achieving 90.1% baseline performance with only ~2% tokens and ~2.6% FLOPs.


<details>
  <summary>Details</summary>
Motivation: Video-LLMs face high computational costs from large visual tokens. Existing two-stage compression methods with spatiotemporal separability assumptions lead to unbalanced token allocation and loss of essential visual evidence under extremely low retention ratios.

Method: Reformulates token compression as spatiotemporal allocation within a global token retention pool. Uses unified selection mechanism integrating attention weights and semantic similarity to globally select high-contribution, low-redundancy tokens. Unselected tokens are merged via clustering and refilled. Inside LLM, introduces text-aware merging for secondary compression based on query relevance.

Result: Retaining only ~2% of visual tokens preserves 90.1% of baseline performance across multiple benchmarks, reducing FLOPs to ~2.6%. Benefits generalize across diverse backbones, decreasing end-to-end inference latency and memory consumption.

Conclusion: The unified spatiotemporal token compression strategy establishes state-of-the-art in video understanding under ultra-low token retention, serving as a plug-and-play module compatible with existing Video-LLMs without requiring retraining.
...
TL;DR: Introduces BHDD - a Burmese handwritten digit dataset with 87,561 MNIST-format images, featuring baseline models achieving up to 99.83% accuracy.


<details>
  <summary>Details</summary>
Motivation: To create a publicly available dataset for Burmese handwritten digits, addressing the lack of resources for Myanmar script digit recognition research.

Method: Collected 87,561 grayscale images from over 150 contributors, organized into MNIST format (28x28 pixels), with 60,000 training and 27,561 test samples. Analyzed class distribution, pixel statistics, and morphological variations.

Result: Dataset created with balanced training set and natural test distribution. Baseline models achieved: MLP 99.40%, 2-layer CNN 99.75%, improved CNN with batch normalization and augmentation 99.83% test accuracy.

Conclusion: BHDD provides a valuable resource for Burmese digit recognition research, with identified challenges in distinguishing certain digit pairs due to Myanmar script's round shapes. Dataset is publicly available under CC BY-SA 4.0 license.
...
TL;DR: GeoFusion-CAD: A diffusion-based framework for scalable parametric CAD generation using hierarchical tree encoding and C-Mamba blocks to handle long command sequences with complex dependencies.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based CAD generation methods struggle with long command sequences due to quadratic attention costs and limited context windows, failing to handle complex geometric and topological dependencies in parametric CAD modeling.

Method: Proposes GeoFusion-CAD: an end-to-end diffusion framework that encodes CAD programs as hierarchical trees capturing geometry and topology. Uses a lightweight C-Mamba block with selective state transitions to model long-range structural dependencies. Introduces DeepCAD-240 benchmark for long-sequence evaluation (40-240 commands).

Result: GeoFusion-CAD achieves superior performance on both short and long command ranges, maintaining high geometric fidelity and topological consistency where Transformer-based models degrade. Sets new state-of-the-art for long-sequence parametric CAD generation.

Conclusion: The approach establishes a scalable foundation for next-generation CAD modeling systems, demonstrating that diffusion frameworks with hierarchical tree encoding and state-space modeling can effectively handle long, complex CAD programs.
...
TL;DR: daVinci-MagiHuman is an open-source audio-video generative foundation model that jointly produces synchronized video and audio using a single-stream Transformer architecture, excelling in human-centric scenarios with multilingual support and efficient inference.


<details>
  <summary>Details</summary>
Motivation: To create a unified audio-video generation model that avoids complex multi-stream or cross-attention architectures while maintaining strong performance in human-centric scenarios with precise audio-video synchronization and multilingual support.

Method: Uses a single-stream Transformer that processes text, video, and audio within a unified token sequence via self-attention only. Combines this backbone with model distillation, latent-space super-resolution, and a Turbo VAE decoder for efficient inference.
...
Conclusion: daVinci-MagiHuman demonstrates that a single-stream Transformer architecture can effectively generate synchronized audio-video content with strong human-centric performance, multilingual capabilities, and efficient inference, making it a competitive open-source foundation model.
...
TL;DR: LRC-WeatherNet is a multi-sensor fusion framework that combines LiDAR, RADAR, and camera data for real-time weather classification in autonomous vehicles, achieving superior performance in adverse conditions.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles struggle with perception in adverse weather (rain, fog, snow) as individual sensors (LiDAR, RADAR, cameras) have limitations when exposed to environmental obstructions. Each sensor has unique strengths but also distinct weaknesses in poor weather conditions.

Method: Proposes LRC-WeatherNet, a multi-sensor fusion framework using both early fusion (unified Bird's Eye View representation) and mid-level gated fusion of modality-specific feature maps. This adaptive approach handles varying sensor reliability under changing weather conditions.
...
Conclusion: This is the first work to combine all three modalities (LiDAR, RADAR, camera) for robust, real-time weather classification in autonomous driving. The trained models and source code are publicly released.
...
TL;DR: STENet introduces superpixel-based cross-modal interaction for RGB-D SOD, using global and local enhancement modules to reduce computational complexity while improving feature representation.


<details>
  <summary>Details</summary>
Motivation: Current transformer-based RGB-D SOD methods suffer from quadratic attention complexity and limited local detail extraction. The paper aims to overcome these limitations by introducing superpixels into cross-modal interaction.

Method: Proposes Superpixel Token Enhancing Network (STENet) with two-stream encoder-decoder structure. Key components: 1) Updated superpixel generation with expanded neighborhood range, 2) Superpixel Attention Global Enhancing Module for region-level modeling, 3) Superpixel Attention Local Refining Module for local detail enhancement, 4) Fusion of globally/locally enhanced features with cross-scale features.

Result: Experiments on seven RGB-D SOD datasets show competitive performance compared to state-of-the-art methods. Code and results available on GitHub.

Conclusion: STENet effectively addresses computational complexity and local detail limitations in RGB-D SOD through superpixel-based cross-modal interaction, achieving state-of-the-art performance while maintaining efficiency.
...
TL;DR: SegMaFormer: A lightweight hybrid Mamba-Transformer architecture for 3D medical image segmentation that reduces parameters by 75x and FLOPs while maintaining competitive performance.


<details>
  <summary>Details</summary>
Motivation: Transformers enable global context modeling for 3D medical segmentation but have high computational complexity and parameter counts, which is prohibitive for volumetric data and limited annotated medical datasets.

Method: Hybrid hierarchical architecture combining Mamba and Transformer modules: Mamba-based layers in early high-resolution stages for computational efficiency and spatial context, self-attention in later low-resolution stages for feature refinement, with generalized rotary position embeddings.

Result: Achieves competitive performance on Synapse, BraTS, and ACDC benchmarks, matching Dice coefficients of larger models while reducing parameters by up to 75x and substantially decreasing FLOPs.

Conclusion: SegMaFormer establishes an efficient and high-performing solution for 3D medical image segmentation by strategically combining Mamba and Transformer strengths in a lightweight architecture.
...
TL;DR: Proposes a marker-based 6D hand-eye calibration method for robot-mounted OCT probes to enable consistent scanning of large curved tissue surfaces without relying on image registration.


<details>
  <summary>Details</summary>
Motivation: Current OCT scanning methods are limited for curved tissue surfaces. Handheld scanning requires image overlap for registration, while robotic scanning typically restricts motion to translations to avoid complex hand-eye calibration. These approaches have limitations when scanning curved surfaces and can accumulate registration errors.

Method: Develops a marker-based approach for full six-dimensional hand-eye calibration of a robot-mounted OCT probe. The method enables accurate transformation estimation between the robot and OCT coordinate systems without relying on image registration.

Result: The calibration produces highly repeatable transformation estimates. Evaluation on two phantom surfaces demonstrates consistent scanning of large, curved surfaces. The approach avoids error accumulation along scan paths that occurs with registration-based methods and shows improvement over conventional 3D-translational robotic scanning.

Conclusion: The proposed marker-based 6D hand-eye calibration enables effective robotic OCT scanning of curved tissue surfaces, overcoming limitations of existing methods that rely on image registration or restrict motion to translations.
...
TL;DR: ResFlow-Tuner is a flow matching-based image restoration framework that leverages FLUX.1-dev with unified multi-modal fusion and test-time scaling to achieve state-of-the-art restoration performance.


<details>
  <summary>Details</summary>
Motivation: Despite progress in diffusion-based real-world image restoration, efficiently leveraging ultra-large-scale pre-trained T2I models and fully exploiting their potential remains challenging. Current methods struggle to effectively utilize these powerful models for restoration tasks.

Method: Proposes ResFlow-Tuner based on FLUX.1-dev flow matching model with two key innovations: 1) Unified Multi-Modal Fusion (UMMF) that encodes multi-modal conditions into a unified sequence to guide synthesis, and 2) Training-free Test-Time Scaling (TTS) that dynamically steers denoising direction using reward model feedback during inference.

Result: Achieves state-of-the-art performance across multiple standard benchmarks. Demonstrates significant performance gains with controllable computational overhead through the test-time scaling paradigm.

Conclusion: Validates flow matching models' powerful capabilities for low-level vision tasks and proposes a novel, efficient inference-time scaling paradigm suitable for large pre-trained models, offering a new direction for leveraging foundation models in restoration tasks.
...
TL;DR: GTSR: A 3D Gaussian Splatting-based pipeline for efficient reconstruction of translucent objects from multi-view images, using surface and interior Gaussians with Fresnel blending and Disney BSDF for improved detail.


<details>
  <summary>Details</summary>
Motivation: Existing methods for translucent object reconstruction either use computationally expensive differentiable path tracing/neural implicit fields, or 3DGS-based methods that work well for opaque objects but fail for translucent ones due to not considering their optical properties.

Method: Proposes GTSR pipeline using two sets of Gaussians: surface Gaussians for geometry and interior Gaussians for scattering color. Uses Fresnel term to blend both sets for appearance rendering. Incorporates Disney BSDF model with deferred rendering to enhance normal and depth constraints for better detail in non-contour areas.

Result: Outperforms baseline reconstruction methods on NeuralTO Syn dataset while maintaining real-time rendering performance. Successfully adapts to different translucent materials on extended dataset with varying material properties.

Conclusion: GTSR provides an efficient 3DGS-based solution for translucent object reconstruction that handles optical properties, achieves high-quality results, and enables real-time rendering across various translucent materials.
...
TL;DR: DTVI is a dual-stage inference-time defense framework for safe text-to-image generation that uses category-aware sequence-level intervention on full prompt embeddings to capture distributed malicious semantics and attenuates remaining unsafe influences during visual generation.


<details>
  <summary>Details</summary>
Motivation: Existing T2I diffusion models can generate unsafe content, and current inference-time defense methods fail to capture malicious semantics distributed across full token sequences and remain vulnerable to adversarial prompts.

Method: DTVI introduces category-aware sequence-level intervention on full prompt embeddings to better capture distributed malicious semantics, and further attenuates remaining unsafe influences during the visual generation stage.

Result: Achieves average Defense Success Rate of 94.43% across sexual-category benchmarks and 88.56% across seven unsafe categories while maintaining generation quality on benign prompts.
...
TL;DR: UNCHA enhances hyperbolic VLMs by modeling part-to-whole semantic representativeness with hyperbolic uncertainty, improving compositional scene understanding through uncertainty-guided contrastive learning and entailment regularization.


<details>
  <summary>Details</summary>
Motivation: Current hyperbolic VLMs don't account for varying semantic representativeness of parts to the whole scene - some parts are more representative than others. This limitation affects understanding of complex multi-object scenes with hierarchical relationships.

Method: UNCHA introduces hyperbolic uncertainty to model part-to-whole semantic representativeness, assigning lower uncertainty to more representative parts. Uses uncertainty-guided weights in contrastive learning, and calibrates uncertainty with entailment loss regularized by entropy-based term.

Result: Achieves state-of-the-art performance on zero-shot classification, retrieval, and multi-label classification benchmarks. Learns hyperbolic embeddings with more accurate part-whole ordering and better compositional structure understanding.

Conclusion: Modeling semantic representativeness through hyperbolic uncertainty significantly enhances hyperbolic VLMs' ability to capture hierarchical relationships and understand complex multi-object scenes, outperforming existing approaches.
...
TL;DR: FontCrafter is an element-driven artistic font generation framework that uses in-context generation with visual elements as style references, achieving high-fidelity reconstruction of both texture and structure through attention mechanisms and shape control.


<details>
  <summary>Details</summary>
Motivation: Existing artistic font generation approaches suffer from limited style diversity and coarse control. The paper aims to enable more diverse and precise font generation by using visual elements as style references, allowing for better preservation of both structural and textural fidelity.

Method: Proposes FontCrafter framework with: 1) Element categorization (object vs amorphous elements), 2) In-context generation treating element images as visual context using inpainting models, 3) Context-aware Mask Adapter (CMA) for shape control, 4) Training-free attention redirection for region-aware style control, and 5) Edge repainting for natural boundaries. Built on ElementFont dataset.

Result: FontCrafter achieves strong zero-shot generation performance, particularly in preserving structural and textural fidelity of reference elements. It supports flexible controls like style mixture and demonstrates superior performance compared to existing approaches.

Conclusion: The element-driven approach enables high-quality artistic font generation with diverse styles and fine-grained control, addressing limitations of previous methods through innovative in-context generation and attention mechanisms.
...
TL;DR: SpatialBoost enhances pre-trained vision encoders by injecting 3D spatial knowledge through linguistic descriptions using LLMs, improving spatial awareness without retraining from scratch.


<details>
  <summary>Details</summary>
Motivation: Current vision encoders are trained on 2D images and lack 3D spatial understanding of object relationships, limiting their effectiveness in real-world applications requiring spatial reasoning.

Method: Converts dense 3D spatial information from 2D images into linguistic expressions, then uses LLMs with multi-turn Chain-of-Thought reasoning to progressively inject spatial knowledge into vision encoders.

Result: SpatialBoost improves DINOv3 performance from 55.9 to 59.7 mIoU on ADE20K (3.8% gain), achieving state-of-the-art performance across benchmarks requiring 3D perception and general vision abilities.

Conclusion: SpatialBoost provides a scalable framework to enhance spatial awareness of existing vision encoders by leveraging linguistic descriptions of 3D spatial relationships, bridging the gap between 2D training and 3D understanding.
...
TL;DR: BayesMM: A Multimodal Bayesian framework for test-time adaptation of 3D vision-language models that fuses textual priors with streaming visual features using Bayesian model averaging to maintain robustness under domain shifts.


<details>
  <summary>Details</summary>
Motivation: Existing test-time adaptation methods for multimodal 3D models suffer from progressive information loss due to limited cache storage and unstable adaptation from heuristic logit fusion, leading to degraded performance under domain shifts.

Method: BayesMM models textual priors (from semantic prompts) and streaming visual features as Gaussian distributions, fuses them via Bayesian model averaging that automatically adjusts modality contributions based on posterior evidence, enabling continual adaptation without training.
...
Conclusion: BayesMM provides an effective Bayesian framework for test-time adaptation that addresses limitations of cache-based methods by properly modeling multimodal distributions and enabling stable, continual adaptation to evolving test data.
...
TL;DR: P-Flow is a training-free framework that customizes dynamic visual effects in video generation through test-time prompt optimization using vision-language models, without modifying the underlying video generation model.


<details>
  <summary>Details</summary>
Motivation: Current video generation models struggle with customizing dynamic visual effects (like explosions or object crushing) because these require complex temporal reasoning and iterative prompt refinement that is difficult for humans to craft manually.

Method: P-Flow uses vision-language models to perform test-time prompt optimization by analyzing the discrepancy between reference video effects and generated outputs, then iteratively refines prompts to better induce desired dynamic effects in new scenes.
...
Conclusion: The proposed training-free framework successfully addresses the challenge of dynamic visual effect customization through iterative prompt optimization, enabling better control over complex temporal visual phenomena without model retraining.
...
TL;DR: NullSteer: A null-space projected activation defense framework that enhances VLM safety against visual jailbreak attacks without impairing general capabilities.


<details>
  <summary>Details</summary>
Motivation: Vision-language models deployed in open-world scenarios are vulnerable to visual jailbreak attacks that induce harmful content generation, posing serious safety risks. Existing activation steering methods can cause over-refusal and degrade performance on benign inputs, while lacking theoretical interpretability and robustness.

Method: Proposes NullSteer, a null-space projected activation defense framework that constructs refusal directions within model activations through linear transformation. It maintains zero perturbation in benign subspace while dynamically inducing refusal along potentially harmful directions, theoretically achieving safety enhancement without impairing general capabilities.

Result: Extensive experiments show NullSteer significantly reduces harmful outputs under various jailbreak attacks (average ASR reduction over 15% on MiniGPT-4) while maintaining comparable performance to the original model on general benchmarks.

Conclusion: NullSteer effectively balances safety and utility by providing a theoretically grounded defense against visual jailbreak attacks without compromising model performance on benign inputs.
...
TL;DR: FreeArtGS reconstructs articulated objects from free-moving monocular RGB-D videos using part segmentation, joint estimation, and 3DGS optimization, achieving high scalability without axis alignment requirements.


<details>
  <summary>Details</summary>
Motivation: Existing articulated object reconstruction methods require non-trivial axis alignment or suffer from insufficient coverage, limiting scalability for AR/robotics applications. There's a need for a simple setup that can handle free-moving scenarios.

Method: Combines free-moving part segmentation with joint estimation and end-to-end optimization. Uses priors from point-tracking and feature models to identify rigid parts from relative motion, calibrates object-to-camera poses, recovers joint types/axes, and implements 3DGS-based optimization for textures, geometry, and joint angles.

Result: Excels in reconstructing free-moving articulated objects and remains highly competitive in previous reconstruction settings. Validated on two benchmarks and real-world free-moving articulated objects.

Conclusion: FreeArtGS provides a practical and effective solution for realistic asset generation with high scalability, requiring only simple monocular RGB-D video input under free-moving scenarios.
...
TL;DR: StreamingClaw is a unified agent framework for real-time streaming video understanding and embodied intelligence, supporting online reasoning, proactive interaction, multimodal memory, and perception-decision-action closed loops.


<details>
  <summary>Details</summary>
Motivation: Current agents have fragmented capabilities - they either only support offline video understanding, lack long-term multimodal memory, or struggle with real-time reasoning and proactive interaction under streaming inputs. These limitations prevent agents from sustaining perception, making real-time decisions, and executing actions in real-world environments.

Method: StreamingClaw integrates five core capabilities: 1) real-time streaming reasoning, 2) future event reasoning and proactive interaction with evolving objectives, 3) multimodal long-term memory with hierarchical evolution and efficient retrieval across agents, 4) perception-decision-action closed loop with streaming tools and action-centric skills, and 5) compatibility with OpenClaw framework for community resources.

Result: The framework enables online real-time reasoning, multimodal long-term memory, and proactive interaction within a unified system. It translates decisions into executable actions for direct physical world control, supporting practical deployment of embodied interaction.

Conclusion: StreamingClaw addresses key bottlenecks in streaming video understanding for embodied intelligence by providing a comprehensive framework that integrates real-time reasoning, memory, and interaction capabilities needed for real-world deployment.
...
TL;DR: A two-stage framework for text-driven video moment retrieval that uses LLM-guided subtitle matching and text-to-video generation to create temporal priors, then processes them through a multi-modal controlled Mamba network for efficient long-sequence grounding.


<details>
  <summary>Details</summary>
Motivation: Current VMR methods struggle with capturing hidden temporal dynamics in untrimmed videos, leading to imprecise grounding in long sequences. Traditional approaches rely on NLQs or static images, overlook motion sequences, have high computational costs in Transformers, and fail to effectively integrate subtitle contexts with generated temporal priors.

Method: Two-stage framework: 1) LLM-guided subtitle matching identifies relevant textual cues from video subtitles, fuses them with queries to generate auxiliary short videos via text-to-video models (capturing implicit motion as temporal priors). 2) Augmented queries processed through multi-modal controlled Mamba network with text-controlled selection and video-guided gating for efficient fusion of generated priors and long sequences while filtering noise.
...
Conclusion: The proposed framework effectively addresses limitations of existing VMR methods by integrating subtitle contexts with generated temporal priors through an efficient two-stage approach, achieving better performance with lower computational costs for long-sequence video moment retrieval.
...
TL;DR: PRISM-RM uses implicit neural representations with physics regularization for respiratory motion modeling in radiotherapy, providing continuous diffeomorphic motion without fixed reference state and improving extrapolation performance.


<details>
  <summary>Details</summary>
Motivation: Respiratory motion in lung/abdominal radiotherapy creates treatment uncertainties; current motion models need improvement for better spatial dose delivery and motion management.

Method: Physics-regularized implicit neural representations (PRISM-RM) for surrogate-based motion modeling, providing trajectory-aware spatio-temporally continuous diffeomorphic motion without fixed reference breathing state.

Result: Trajectory-aware approach performs on par in interpolation and improves extrapolation compared to initial INR-based approach; both INR approaches match sequential registration in interpolation but underperform in extrapolation.

Conclusion: INRs show strong potential for respiratory motion modeling with their methodological advantages, and performance improvements suggest they could advance the field despite current extrapolation limitations.
...
<details>
  <summary>Details</summary>
Motivation: Reducing token count is crucial for efficient training/inference in latent diffusion models, especially at high resolution. Existing high-compression tokenizers lose meaningful structure when trained only for reconstruction, requiring costly diffusion retraining.

Method: DA-VAE uses explicit latent layout: first C channels from pretrained VAE at base resolution, plus D channels for higher-resolution details. Includes detail-alignment mechanism to retain original structure, with warm-start fine-tuning strategy.
...
Conclusion: DA-VAE provides efficient high-resolution generation by expanding latent dimensionality while preserving pretrained structure, requiring only lightweight adaptation of diffusion backbone rather than costly retraining.
...
TL;DR: OpenEarth-Agent: A tool-creation agent framework for open-environment Earth Observation that adaptively creates tools rather than using predefined ones, outperforming traditional tool-calling agents.


<details>
  <summary>Details</summary>
Motivation: Current EO agents are limited to closed environments with predefined tools, restricting their ability to handle diverse multi-source data and heterogeneous tasks in open environments. The immense diversity of EO data and tasks requires more adaptive approaches.

Method: Proposes OpenEarth-Agent framework with adaptive workflow planning and tool creation capabilities. Uses open-ended integration of multi-stage tools and cross-domain knowledge bases. Also introduces OpenEarth-Bench benchmark with 596 real-world cases across 7 domains to evaluate adaptive planning and tool creation.

Result: OpenEarth-Agent successfully masters full-pipeline EO across multiple domains. With only 6 essential pre-trained models, it achieves performance comparable to tool-calling agents using 104 specialized tools. Created tools show superior robustness to data anomalies compared to human-engineered tools in some cases.

Conclusion: Tool-creation agents are more effective than tool-calling agents for open-environment EO. The adaptive approach enables generalization to unseen data and tasks, demonstrating the potential for autonomous EO systems in diverse real-world scenarios.
...
TL;DR: Bearing-UAV: A vision-based cross-view navigation method that jointly predicts UAV absolute location and heading from neighboring features, addressing limitations of existing map-matching approaches.


<details>
  <summary>Details</summary>
Motivation: Existing cross-view geo-localization methods for UAVs have limitations: they focus on matching to map tiles (creating accuracy-storage trade-offs), ignore UAV heading importance, and insufficiently address cross-view discrepancies and varying overlaps, limiting real-world generalization.

Method: Bearing-UAV leverages global and local structural features and explicitly encodes relative spatial relationships. It's a purely vision-driven approach that jointly predicts UAV absolute location and heading from neighboring features, making it robust to cross-view variations, misalignment, and feature-sparse conditions.

Result: Extensive experiments show Bearing-UAV yields lower localization error than previous matching/retrieval paradigms across diverse terrains. The authors also present Bearing-UAV-90k, a multi-city benchmark for evaluating cross-view localization and navigation.

Conclusion: Bearing-UAV enables accurate, lightweight, and robust UAV navigation in GNSS-denied environments by addressing key limitations of existing approaches through joint location-heading prediction and explicit spatial relationship encoding.
...
TL;DR: ACPO addresses DPO's likelihood collapse in multimodal alignment by applying asymmetric scaling to suppress rejected rewards while preserving chosen distributions, reducing hallucinations and improving performance.


<details>
  <summary>Details</summary>
Motivation: Standard DPO suffers from Likelihood Displacement where both chosen and rejected response probabilities collapse, causing Visual Anchor Collapse in multimodal settings where models abandon visual evidence for language priors, leading to significant hallucinations.

Method: ACPO (Asymmetric Constrained Preference Optimization) applies dynamic, target-oriented scaling to preference optimization with a complexity-aware scaling coefficient applied exclusively to the rejected reward, asymmetrically suppressing gradient flow on rejected terms while preserving chosen distribution as gradient-stable reference.

Result: ACPO effectively reverses chosen-reward degradation of standard DPO, halts Visual Anchor Collapse, outperforms baselines on hallucination benchmarks (HallusionBench, MM-IFEval) and general leaderboards (MMBench, MMStar, OCRBenchV2), and drives concurrent improvements in general capabilities.

Conclusion: ACPO provides a modality-agnostic alignment mechanism that breaks gradient symmetry to mitigate suppression of visual tokens by language priors, offering a superior alternative to standard DPO for multimodal alignment tasks.
...
TL;DR: VFLM is a self-improving layout generation framework that uses visual feedback and reinforcement learning to iteratively refine layouts for better readability and aesthetics.


<details>
  <summary>Details</summary>
Motivation: Existing layout generation methods use code-only paradigms that are blind to rendered visual outcomes, making it difficult to guarantee readability and aesthetics in the final images.

Method: Proposes Visual Feedback Layout Model (VFLM) with adaptive reflective generation using visual feedback iterative refinement. Uses reinforcement learning with a visually grounded reward model incorporating OCR accuracy, rewarding only final outcomes to stimulate iterative capabilities.

Result: VFLM consistently outperforms advanced MLLMs, existing layout models, and code-only baselines across multiple benchmarks, establishing visual feedback as critical for design-oriented MLLMs.

Conclusion: Visual feedback is a critical factor in layout generation, and VFLM's self-improving framework with iterative refinement effectively addresses the limitations of code-only approaches.
...
TL;DR: The paper benchmarks different backbones for self-supervised learning (SSL) as an auxiliary task in L-SSAT framework, finding backbone effectiveness varies by downstream face analysis task with no single generalized solution.


<details>
  <summary>Details</summary>
Motivation: To understand the impact of different backbones on self-supervised learning as an auxiliary task for robust face analysis, specifically investigating how backbone architecture affects performance in the L-SSAT framework across various face analysis tasks.

Method: Used Masked Auto-Encoder (MAE) as SSL auxiliary task alongside primary tasks in L-SSAT framework, benchmarked shallow to deep backbones, conducted comprehensive comparative analysis across multiple model configurations to answer three research questions about backbone roles and effectiveness.

Result: Backbone performance is highly task-dependent: achieved average accuracies of 0.94 on FaceForensics++ (deepfake detection), 0.87 on CelebA (face attribute prediction), and 0.88 on AffectNet (emotion classification). No unified backbone works best across all face analysis paradigms.

Conclusion: There is no single generalized backbone for effective face analysis with L-SSAT; backbone selection should be tailored to specific downstream tasks as performance varies significantly across different face analysis applications.
...
TL;DR: PAM is a unified Pose-Appearance-Motion Engine for controllable hand-object interaction video generation that combines pose, appearance, and motion in one framework, outperforming existing fragmented approaches.


<details>
  <summary>Details</summary>
Motivation: Current HOI generation research is fragmented across three disjoint tracks: pose-only synthesis (no pixels), single-image generation (no dynamics), and video generation requiring full pose sequences and ground-truth first frames. There's a need for a unified framework that brings together pose, appearance, and motion for true sim-to-real deployment.

Method: Introduces PAM (Pose-Appearance-Motion Engine), a unified framework for controllable HOI video generation that combines depth, segmentation, and keypoints as input conditions to generate coherent pose, appearance, and motion together.

Result: Achieves state-of-the-art results: FVD of 29.13 (vs. 38.83 for InterDyn) and MPJPE of 19.37mm (vs. 30.05mm for CosHand) on DexYCB; improves FVD from 68.76 to 46.31 on OAKINK2; ablation shows combining depth, segmentation, and keypoints works best; synthetic data augmentation enables matching 100% real data baseline with only 50% real data plus synthetic videos.

Conclusion: PAM successfully unifies pose, appearance, and motion for HOI video generation, enabling controllable synthesis and effective sim-to-real applications like data augmentation for hand pose estimation tasks.
...
TL;DR: MAMMOTH introduces a multi-head mixture of experts module that provides task-specific patch feature transformations in MIL, outperforming standard linear layers across multiple methods and tasks.


<details>
  <summary>Details</summary>
Motivation: Current MIL frameworks focus on patch feature extraction and aggregation but overlook the critical linear layer that transforms general-purpose features into task-specific features, which may be a performance bottleneck.

Method: MAMMOTH is a parameter-efficient, multi-head mixture of experts module that applies low-rank transformations tailored to each patch's phenotype, compatible with any existing MIL approach.

Result: Across 8 MIL methods and 19 classification tasks, MAMMOTH improved performance in 130 of 152 configurations (+3.8% average improvement). Even simple pooling methods with MAMMOTH outperformed standard linear layer approaches.

Conclusion: Task-specific patch feature transformation has greater impact on MIL performance than aggregation method choice. MAMMOTH provides a parameter-efficient way to enhance any MIL model's performance.
...
TL;DR: Proposes Omni-WorldBench, a benchmark for evaluating interactive response capabilities of 4D world models, addressing limitations in current evaluation methods that neglect temporal dynamics and interaction effects.


<details>
  <summary>Details</summary>
Motivation: Current world model evaluations focus narrowly on visual fidelity/text-video alignment for generative models or static 3D reconstruction metrics, fundamentally neglecting temporal dynamics and interactive response capabilities crucial for 4D world modeling.

Method: Omni-WorldBench includes: 1) Omni-WorldSuite - systematic prompt suite spanning diverse interaction levels and scene types; 2) Omni-Metrics - agent-based evaluation framework measuring causal impact of interaction actions on both final outcomes and intermediate state evolution trajectories.

Result: Extensive evaluation of 18 representative world models reveals critical limitations in interactive response capabilities, providing actionable insights for future research.

Conclusion: Omni-WorldBench addresses the gap in evaluating interactive 4D world modeling and will be publicly released to foster progress in this emerging research direction.
...
TL;DR: SpatialReward is a verifiable reward model that evaluates spatial layouts in text-to-image generation, improving spatial consistency through multi-stage analysis and RL training.


<details>
  <summary>Details</summary>
Motivation: Existing reward models for text-to-image generation focus on semantic alignment and visual quality but neglect fine-grained spatial relationships, leading to images that appear plausible but contain object positioning inaccuracies.

Method: SpatialReward uses a multi-stage pipeline: (1) Prompt Decomposer extracts entities, attributes, and spatial metadata; (2) expert detectors provide visual grounding of object positions; (3) vision-language model applies chain-of-thought reasoning to assess complex spatial relations. They also introduce SpatRelBench benchmark for evaluation.

Result: Experiments on Stable Diffusion and FLUX show that incorporating SpatialReward into RL training consistently improves spatial consistency and overall generation quality, with results better aligned to human judgments.

Conclusion: Verifiable reward models like SpatialReward have considerable potential for enabling more accurate and controllable optimization in text-to-image generation models by addressing spatial relationship evaluation.
...
TL;DR: Benchmark study evaluates four deep learning models (KPConv, RandLA-Net, Superpoint Transformer, Point Transformer V3) on aerial LiDAR data, showing KPConv achieves best overall performance (78.51% mIoU) while Point Transformer V3 excels on underrepresented vehicle class.


<details>
  <summary>Details</summary>
Motivation: Most 3D semantic segmentation models focus on indoor/terrestrial datasets, leaving their performance under real aerial acquisition conditions insufficiently explored. Existing studies differ in dataset design, acquisition conditions, and model selection, creating a gap in understanding model behavior for aerial LiDAR applications.

Method: Experimental benchmark evaluating four state-of-the-art architectures (KPConv, RandLA-Net, Superpoint Transformer, Point Transformer V3) on large-scale aerial LiDAR dataset acquired under operational flight conditions in Navarre, Spain. The dataset covers heterogeneous urban, rural, and industrial landscapes with five semantic classes (ground, vegetation, buildings, vehicles, etc.).

Result: All models achieve high overall accuracy exceeding 93%. KPConv attains highest mean IoU (78.51%) with consistent performance across classes, especially on challenging/underrepresented categories. Point Transformer V3 demonstrates superior performance on underrepresented vehicle class (75.11% IoU). Superpoint Transformer and RandLA-Net trade segmentation robustness for computational efficiency.

Conclusion: The study provides valuable insights into deep learning model performance for aerial LiDAR segmentation, highlighting KPConv's overall superiority and Point Transformer V3's strength on underrepresented classes, while revealing trade-offs between segmentation robustness and computational efficiency in different architectures.
...
TL;DR: PTv2 transformer model achieves high accuracy (mIoU 0.950) for riverine land cover mapping using multispectral LiDAR, with multi-dataset training improving generalization.


<details>
  <summary>Details</summary>
Motivation: Accurate land cover mapping in riverine environments is essential for river management, ecological understanding, and geomorphic change monitoring, requiring advanced methods for processing multispectral LiDAR data.

Method: Uses Point Transformer v2 (PTv2) architecture for semantic segmentation of 3-channel LiDAR point clouds (geometry + spectral features). Trained on Oulanka river data with ablation studies on feature importance and multi-dataset training with additional sparsely annotated river data.

Result: Full-feature configuration achieved mIoU of 0.950, significantly outperforming geometry-only baseline. Intensity and reflectance features were key for accuracy. Multi-dataset training improved generalization performance.

Conclusion: Transformer-based architectures show strong potential for multispectral point cloud analysis in riverine environments, offering new capabilities for sediment transport monitoring and river management applications.
...
TL;DR: EgoGroups: A first-person view dataset for social group detection spanning 65 countries with diverse cultural contexts, weather conditions, and crowd densities, showing VLMs/LLMs can outperform supervised models in zero-shot settings.


<details>
  <summary>Details</summary>
Motivation: Existing social group detection benchmarks are limited by low scene diversity, reliance on third-person camera sources (like surveillance footage), and lack of real-world evaluation of how groups form and evolve in diverse cultural contexts and unconstrained settings.

Method: Created EgoGroups dataset with first-person view footage from 65 countries covering low, medium, and high-crowd settings under four weather/time-of-day conditions. Includes dense human annotations for persons and social groups, plus rich geographic and scene metadata. Evaluated state-of-the-art VLM/LLMs and supervised models on group detection capabilities.

Result: VLMs and LLMs can outperform supervised baselines in zero-shot settings. Crowd density and cultural regions clearly influence model performance.

Conclusion: EgoGroups addresses the gap in social group detection benchmarks by providing diverse real-world data, revealing important insights about model performance across different cultural contexts and environmental conditions.
...
TL;DR: A novel framework that synthesizes large-scale, perfectly aligned frame-flow data pairs for supervised optical flow training without human annotations, using pre-trained depth estimation and next-frame generation models.


<details>
  <summary>Details</summary>
Motivation: Optical flow estimation relies on expensive ground-truth annotations, limiting scalability of supervised approaches. Unsupervised/semi-supervised methods suffer from unreliable supervision signals based on brightness constancy and smoothness assumptions, leading to inaccurate motion estimation in complex real-world scenarios.
...
Conclusion: The framework shows potential as a scalable and annotation-free solution for optical flow learning, achieving strong performance without human annotations.
...
TL;DR: DUO-VSR is a three-stage distillation framework that combines distribution matching and adversarial supervision for efficient one-step video super-resolution, achieving high visual quality with reduced sampling costs.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based VSR achieves high fidelity but suffers from prohibitive sampling costs. Existing acceleration methods like DMD cause training instability and insufficient supervision when applied to VSR.

Method: Three-stage framework: 1) Progressive Guided Distillation Initialization for stability, 2) Dual-Stream Distillation combining DMD and RFS-GAN streams for complementary supervision, 3) Preference-Guided Refinement for perceptual quality alignment.

Result: DUO-VSR achieves superior visual quality and efficiency over previous one-step VSR approaches, demonstrating better performance with reduced computational costs.

Conclusion: The proposed unified distillation framework successfully addresses training instability and supervision insufficiency in accelerating diffusion-based VSR, enabling efficient one-step generation with high visual quality.
...
TL;DR: GLD proposes using geometrically consistent feature spaces from foundation models as latent space for novel view synthesis, outperforming VAE/RAE approaches in quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current novel view synthesis methods use view-independent VAE latent spaces that lack geometric consistency across viewpoints, which is essential for multi-view generation.

Method: Geometric Latent Diffusion (GLD) repurposes geometrically consistent feature spaces from geometric foundation models as the latent space for multi-view diffusion models.

Result: GLD outperforms VAE and RAE on 2D image quality and 3D consistency metrics, accelerates training by 4.4x, and remains competitive with methods using large-scale text-to-image pretraining.

Conclusion: Geometrically consistent feature spaces from foundation models provide an optimal latent space for novel view synthesis, enabling high-fidelity, consistent multi-view generation without requiring generative pretraining.
...
TL;DR: VLMs use two mechanisms for spatial reasoning: language model layers compute content-independent spatial relations, but vision encoder representations encoding object layouts play the dominant role.


<details>
  <summary>Details</summary>
Motivation: To understand where and how vision-language models compute associations between objects and their spatial relations, which is crucial for tasks like image captioning and visual question answering.

Method: Analyzed spatial reasoning mechanisms in VLMs by examining where spatial information is computed, comparing contributions from language model backbone vs vision encoder, and testing enhancement of vision-derived spatial representations.

Result: Found two concurrent mechanisms: 1) language model intermediate layers represent content-independent spatial relations, but this plays secondary role; 2) vision encoder representations encode object layouts and are the dominant spatial information source, distributed globally across visual tokens including background areas. Enhancing these vision-derived spatial representations improves spatial reasoning performance.

Conclusion: Spatial association in VLMs is primarily computed in vision encoders, which encode object layouts globally across image tokens, highlighting their central role in enabling spatial reasoning beyond just the language model backbone.
...
TL;DR: A structured reasoning framework for text-guided spatial layout editing using scene graphs, improving spatial understanding over LLMs/VLMs.


<details>
  <summary>Details</summary>
Motivation: LLMs and VLMs struggle with spatial understanding and layout consistency in fine-grained visual editing tasks, despite their impressive reasoning abilities.

Method: Introduces a Structured Reasoning framework that performs text-conditioned spatial layout editing via scene-graph reasoning. Given an input scene graph and natural-language instruction, the model reasons over the graph to generate an updated scene graph that satisfies text conditions while maintaining spatial coherence.

Result: Achieves 15% improvement in IoU and 25% reduction in center-distance error compared to CoT-SFT and vanilla GRPO baselines. Compared to SOTA zero-shot LLMs, achieves up to 20% higher mIoU with markedly improved spatial precision.

Conclusion: Explicit structured reasoning through scene graphs improves both interpretability and control over spatial relationships in visual editing tasks, addressing limitations of current LLMs/VLMs in spatial understanding.
...
<details>
  <summary>Details</summary>
Motivation: Standard VLA models struggle with complex multi-step tasks requiring logical planning and precise spatial perception. Current CoT-based VLA models have two critical limitations: inability to simultaneously capture low-level visual details and high-level planning due to single-modal CoT, and high inference latency with compounding errors from autoregressive decoding.

Method: Proposes DualCoT-VLA with parallel reasoning mechanism: integrates visual CoT for low-level spatial understanding and linguistic CoT for high-level task planning. Uses two sets of learnable query tokens to shift from autoregressive step-by-step reasoning to single-step forward reasoning, reducing latency.
...
Conclusion: DualCoT-VLA successfully addresses limitations of existing CoT-based VLA models by enabling comprehensive multi-modal reasoning through parallel visual and linguistic CoT while significantly reducing inference latency through single-step forward reasoning.
...
TL;DR: VLM-guided JEPA framework combines dense-frame dynamics modeling with long-horizon semantic guidance via dual-temporal pathways for improved trajectory prediction.


<details>
  <summary>Details</summary>
Motivation: Current latent world models (like V-JEPA2) have limited temporal context and bias toward local extrapolation, while VLMs have strong semantic grounding but aren't ideal as standalone dense predictors due to sparse sampling and language-output bottlenecks.

Method: Proposes a VLM-guided JEPA framework with dual-temporal pathways: dense JEPA branch for fine-grained motion/interaction, and uniformly sampled VLM thinker branch for knowledge-rich guidance. Uses hierarchical pyramid representation extraction to aggregate multi-layer VLM representations into guidance features.
...
Conclusion: Combining dense-frame dynamics modeling with long-horizon semantic guidance via VLM integration improves world modeling capabilities for trajectory prediction tasks.
...
TL;DR: UniMotion is the first unified framework for simultaneous understanding and generation of human motion, natural language, and RGB images within a single architecture, overcoming limitations of existing models that handle only restricted modality subsets and rely on discrete tokenization.


<details>
  <summary>Details</summary>
Motivation: Existing unified models have two main limitations: 1) they only handle restricted modality subsets (e.g., Motion-Text or static Pose-Image), and 2) they predominantly rely on discrete tokenization which introduces quantization errors and disrupts temporal continuity. There's a need for a truly unified framework that treats motion as a first-class continuous modality alongside RGB.

Method: UniMotion uses a Cross-Modal Aligned Motion VAE (CMA-VAE) and symmetric dual-path embedders to create parallel continuous pathways for Motion and RGB within a shared LLM backbone. It introduces Dual-Posterior KL Alignment (DPA) to inject visual-semantic priors into motion representations without requiring images at inference. To address the cold-start problem, it uses Latent Reconstruction Alignment (LRA), a self-supervised pre-training strategy that uses dense motion latents to co-calibrate the embedder, backbone, and flow head.

Result: UniMotion achieves state-of-the-art performance across seven tasks spanning any-to-any understanding, generation, and editing among the three modalities (motion, text, RGB images), with especially strong advantages on cross-modal compositional tasks.

Conclusion: UniMotion successfully demonstrates that motion can be treated as a first-class continuous modality alongside RGB within a unified framework, overcoming previous limitations of discrete tokenization and restricted modality handling, while achieving superior performance on diverse cross-modal tasks.
...
TL;DR: UNITE proposes a unified autoencoder architecture that combines tokenization and latent diffusion in a single training stage, eliminating the need for complex staged training of separate tokenizers and diffusion models.


<details>
  <summary>Details</summary>
Motivation: Current latent diffusion models require complex staged training: first train a tokenizer, then train a diffusion model in the frozen latent space. This two-stage process is inefficient and suboptimal.

Method: UNITE uses a Generative Encoder that serves as both image tokenizer and latent generator via weight sharing. It treats tokenization and generation as the same latent inference problem under different conditioning regimes. A single-stage training procedure jointly optimizes both tasks via two forward passes through the same Generative Encoder.

Result: Achieves near state-of-the-art performance without adversarial losses or pretrained encoders: FID 2.12 and 1.73 for Base and Large models on ImageNet 256×256. Works across image and molecule modalities.

Conclusion: Single-stage joint training of tokenization and generation from scratch is feasible and effective, enabling unified latent space learning that serves both compression and generation tasks.
...
TL;DR: VideoDetective improves long-video QA by using visual-temporal affinity graphs and hypothesis-verification loops to identify relevant segments, achieving up to 7.5% accuracy gains on benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with long video understanding due to limited context windows. Existing methods localize clues based only on queries, ignoring video structure and varying segment relevance, leading to inefficient clue hunting.

Method: Divide video into segments and create visual-temporal affinity graph based on visual similarity and temporal proximity. Use Hypothesis-Verification-Refinement loop to estimate query relevance scores for observed segments, propagate to unseen segments, and generate global relevance distribution to guide critical segment localization.

Result: Consistent substantial gains across mainstream MLLMs on representative benchmarks, with up to 7.5% accuracy improvement on VideoMME-long.

Conclusion: VideoDetective effectively addresses long-video QA challenges by integrating query-to-segment relevance and inter-segment affinity, enabling efficient clue hunting with sparse observation.
...
TL;DR: WorldCache is a training-free feature caching framework for Diffusion Transformers that uses motion-adaptive thresholds and saliency-weighted drift estimation to achieve 2.3× inference speedup while maintaining 99.4% of baseline quality.


<details>
  <summary>Details</summary>
Motivation: Diffusion Transformers (DiTs) are computationally expensive for video generation due to sequential denoising and costly spatio-temporal attention. Existing training-free caching methods rely on Zero-Order Hold assumptions that cause ghosting artifacts, blur, and motion inconsistencies in dynamic scenes.

Method: WorldCache introduces a Perception-Constrained Dynamical Caching framework with four key components: 1) motion-adaptive thresholds for feature reuse decisions, 2) saliency-weighted drift estimation, 3) optimal approximation via blending and warping, and 4) phase-aware threshold scheduling across diffusion steps.
...
Conclusion: WorldCache provides an effective training-free solution for accelerating DiT inference through adaptive, motion-consistent feature reuse, addressing the limitations of static caching approaches while maintaining high visual quality.
