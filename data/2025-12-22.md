...
TL;DR: V-Agent is a multi-agent platform for video search and conversation that uses a fine-tuned vision-language model with video preference data and retrieval vectors to overcome text-based retrieval limitations in multimodal scenarios.
...
Method: Fine-tunes a vision-language model with video preference dataset, enhances with retrieval vectors from image-text retrieval model, uses three collaborative agents (routing, search, chat), embeds video frames and ASR transcriptions into shared multimodal space, and includes re-ranking module.
...
Conclusion: V-Agent represents an effective multi-agent framework for advanced video search and interactive conversations that successfully integrates visual and spoken content understanding through collaborative agent architecture.
...
<details>
  <summary>Details</summary>
Motivation: Indonesian government blocked 59,741 websites in 2020 (14,266 for pornography), but VPNs allow public access, creating need for rapid pornographic content identification system.

Method: Developed deep learning system using CNN and VGG-16 models, comprehensively comparing both approaches for effectiveness in detecting pornographic image content.
...
Conclusion: CNN model is more effective than VGG-16 for quickly and accurately detecting pornographic content in websites, providing a solution to circumvent VPN-based access to blocked sites.
...
Method: AVM uses a frozen Vision Transformer-based encoder to capture consistent visual features, with independently trained modulation paths that account for neural response variations driven by stimulus content and subject identity through modular subnetworks.

Result: AVM outperforms state-of-the-art V1T model by ~2% in predictive correlation across two large-scale mouse V1 datasets, with 9.1% improvement in explained variance (FEVE) under cross-dataset adaptation, demonstrating robust generalization and interpretable condition-wise modulation.

Conclusion: AVM provides a unified framework for adaptive neural modeling across biological and experimental conditions, offering a scalable solution under structural constraints that may inform future approaches to cortical modeling in neuroscience and biologically inspired AI systems.
...
TL;DR: The paper proposes a method linking Finer-CAM explanations to TLS projection segments to identify which structural tree features drive species discrimination in deep learning models, finding models primarily rely on crown features but with species-specific variations.


<details>
  <summary>Details</summary>
Motivation: While new sensors and deep learning achieve high accuracy in tree species classification, their decision processes remain unclear. Existing methods like Finer-CAM can highlight contributing features but are uncommon for similar-looking contrastive species, creating a need to systematically understand which features drive species discrimination.

Method: Proposed a novel method linking Finer-CAM explanations to segments of TLS projections representing structural tree features. Used TLS data from 2,445 trees across seven European species, trained five YOLOv8 models with cross-validation, and analyzed 630 saliency maps to evaluate feature contributions.

Result: Achieved 96% mean accuracy (SD = 0.24%). Models primarily rely on crown features for classification, especially for Silver Birch, European Beech, English oak, and Norway spruce. Stem features contribute more to differentiating European ash, Scots pine, and Douglas fir. Finer branches significantly influence decisions, and model perceptions of species similarity align with human expert judgment.

Conclusion: The study highlights the need for improved understanding of tree species classification model decision processes to reveal dataset/model limitations, biases, and build confidence in predictions. The proposed method successfully identifies structural features driving species discrimination and shows models consider species similarly to human experts.
...
Method: A filmmaker-like pipeline that: 1) uses LLM to generate detailed production script, 2) employs text-to-image model to create consistent character visuals as anchors, 3) uses video generation model to synthesize each scene individually guided by the script and visual anchors.
...
Conclusion: Multi-stage decomposition with visual anchoring is essential for maintaining character consistency in long video story generation, and current models exhibit cultural disparities that need addressing.
...
<details>
  <summary>Details</summary>
Motivation: Current video tokenizers use fixed compression rates, leading to redundancy for simple content or information loss for complex content. The variable information density in videos requires adaptive token allocation for efficient representation.

Method: Developed a principled framework based on Shannon's information theory, proved suboptimality of existing methods, and created an ELBO-based algorithm for theoretical optimality. Implemented a transformer-based adaptive compressor for adaptive tokenization.

Result: State-of-the-art compression performance: saves 20% tokens without performance loss, achieves 2.3x compression rates while outperforming prior heuristic adaptive approaches.

Conclusion: InfoTok enables more compressed yet accurate video tokenization by allocating tokens according to informational richness, providing valuable insights for future video representation research.
...
TL;DR: Endo-SemiS is a semi-supervised segmentation framework for endoscopic video frames that uses four strategies to leverage unlabeled data, achieving superior performance with limited annotations.


<details>
  <summary>Details</summary>
Motivation: Medical endoscopic video segmentation requires extensive annotation which is time-consuming and expensive. There's a need for methods that can work effectively with limited labeled data by leveraging abundant unlabeled endoscopic video frames.

Method: Uses four strategies: (1) Cross-supervision between two networks, (2) Uncertainty-guided pseudo-labels from unlabeled data, (3) Joint pseudolabel supervision aggregating reliable pixels from both networks, and (4) Mutual learning at feature and image levels. Also includes a separate corrective network using spatiotemporal information.
...
Conclusion: Endo-SemiS effectively leverages unlabeled endoscopic video data through multiple complementary strategies, providing reliable segmentation with limited annotation and demonstrating strong performance on clinical applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing benchmarks for video understanding either focus on temporal length OR multimodal richness, but rarely both. They mostly use single-score accuracy metrics that obscure failure modes, lacking interpretable evaluation for complex long-form multimodal reasoning.

Method: Created LongShOTBench via scalable human-validated pipeline with open-ended intent-driven questions, single/multi-turn dialogues, multimodal reasoning tasks, and agentic tool use across video/audio/speech. Also developed LongShOTAgent system with preprocessing, search, and iterative refinement for video analysis.
...
<details>
  <summary>Details</summary>
Motivation: Current MLLMs have limited 3D structure and temporal dynamics reasoning due to weak 4D perception. Existing 4D VQA benchmarks focus on static scenes and lack region-level prompting capabilities.

Method: Three contributions: (1) 4D-RGPT - specialized MLLM for 4D video representations with enhanced temporal perception; (2) P4D - perceptual 4D distillation framework transferring knowledge from frozen expert model; (3) R4D-Bench - depth-aware dynamic scene benchmark with region-level prompting via hybrid automated/human pipeline.

Result: 4D-RGPT achieves notable improvements on both existing 4D VQA benchmarks and the proposed R4D-Bench benchmark.

Conclusion: The proposed approach addresses limitations in 4D perception for MLLMs through specialized architecture, distillation training, and comprehensive benchmarking, demonstrating improved video reasoning capabilities.
...
TL;DR: FORMSpoT is a high-resolution (1.5m) forest monitoring system using SPOT satellite time series to map canopy height and detect annual disturbances across France, significantly outperforming existing coarse-resolution products.


<details>
  <summary>Details</summary>
Motivation: The decline of European forest carbon sink requires better monitoring tools. Existing satellite-based disturbance products are too coarse (typically >100m²) to detect changes at individual tree scale, limiting ability to monitor subtle forest dynamics.

Method: Used annual SPOT-6/7 composites (2014-2024) with hierarchical transformer model (PVTv2) trained on airborne laser scanning data. Developed post-processing pipeline combining co-registration and spatio-temporal total variation denoising for robust change detection across heterogeneous acquisitions.

Result: FORMSpoT-Δ substantially outperforms existing disturbance products. In mountainous forests with small, fragmented disturbances, achieves F1-score of 0.44 - an order of magnitude higher than benchmarks. Validated against ALS revisits across 19 sites and 5,087 National Forest Inventory plots.

Conclusion: FORMSpoT enables tree-level forest monitoring at national scale, providing unique tool for analyzing management practices, detecting early forest decline signals, and quantifying carbon losses from subtle disturbances. Highlights importance of sustaining high-resolution satellite missions and open-data initiatives.
...
TL;DR: InfCam is a depth-free framework for camera-controlled video generation that uses infinite homography warping and data augmentation to achieve high camera-pose fidelity without depth estimation.
...
Method: Two key components: (1) infinite homography warping that encodes 3D camera rotations in 2D latent space, with residual parallax prediction, and (2) data augmentation pipeline for diverse trajectory generation.

Result: InfCam outperforms baselines in camera-pose accuracy and visual fidelity, generalizing well from synthetic to real-world data.

Conclusion: InfCam provides a depth-free solution for high-fidelity camera-controlled video generation with better pose accuracy than existing methods.
...
TL;DR: Proposes Interpretable Utility Similarity (IUS), a novel interpretable measure to assess synthetic medical image quality for DL-based clinical decision support systems, outperforming existing methods with up to 54.6% classification improvement.


<details>
  <summary>Details</summary>
Motivation: Current methods for assessing synthetic medical image quality rely on user studies, inception-based measures, or classification performance, but lack interpretability and direct utility assessment for clinical decision support systems.

Method: Develops Interpretable Utility Similarity (IUS) inspired by generalized neural additive models, providing interpretable assessment of synthetic image utility based on clinically relevant image features for DL-based clinical systems.

Result: IUS achieves up to 54.6% relative improvement in classification performance when selecting high-utility synthetic images, validated across multiple medical imaging modalities (endoscopic, dermoscopic, fundus, X-ray, ultrasound).

Conclusion: IUS provides an interpretable, effective measure for assessing synthetic medical image utility in clinical decision support systems, outperforming existing methods and demonstrating generalizability across diverse imaging modalities.
...
TL;DR: DGH is a data-driven framework that learns hair dynamics and appearance using dynamic 3D Gaussian representations, enabling realistic animatable hair without physics-based simulation.


<details>
  <summary>Details</summary>
Motivation: Photorealistic dynamic hair modeling is challenging due to complex motions, occlusions, and light scattering. Existing methods rely on static capture and physics-based models that don't scale well, requiring manual parameter tuning and heavy computation.

Method: Two key components: (1) coarse-to-fine model for temporally coherent hair motion dynamics across hairstyles, (2) strand-guided optimization module that learns dynamic 3D Gaussian representation for hair appearance with differentiable rendering support.

Result: DGH achieves promising geometry and appearance results, provides scalable data-driven alternative to physics-based simulation, and can be integrated into 3D Gaussian avatar frameworks for realistic animatable hair.

Conclusion: DGH offers a fully data-driven approach that scales with training data, generalizes across hairstyles and motions, and enables realistic animatable hair for high-fidelity avatar representation without physics simulation.
...
TL;DR: Survey paper identifies research gap: no transformer-based methods exist for maritime radar frame prediction despite their success in related domains like AIS trajectory prediction and sonar forecasting.


<details>
  <summary>Details</summary>
Motivation: Maritime autonomous systems need robust predictive capabilities for vessel motion and environmental dynamics. Radar's all-weather reliability makes it crucial for navigation, but transformer architectures haven't been applied to maritime radar frame prediction despite their success in related areas.

Method: Systematic review of predictive modeling approaches relevant to maritime radar, focusing on transformer architectures for spatiotemporal sequence forecasting. Analyzes existing methods by data type, architecture, and prediction horizon.

Result: While transformer-based methods exist for AIS trajectory prediction and sonar frame forecasting, no prior work addresses transformer-based maritime radar frame prediction, revealing a clear research gap.

Conclusion: Identifies a concrete research direction: developing transformer-based methods for maritime radar frame prediction to enhance predictive capabilities for maritime autonomous systems.
...
TL;DR: SDUM is a universal deep learning framework for MRI reconstruction that scales across diverse protocols without task-specific fine-tuning, achieving SOTA results across multiple challenges.


<details>
  <summary>Details</summary>
Motivation: Current deep learning MRI reconstructions are protocol-specific, limiting generalization and deployment across diverse clinical protocols (different anatomical targets, contrasts, sampling patterns, acceleration factors).

Method: Combines Restormer-based reconstructor, learned coil sensitivity map estimator (CSME), sampling-aware weighted data consistency (SWDC), universal conditioning (UC) on cascade index and protocol metadata, and progressive cascade expansion training.

Result: Shows foundation-model-like scaling: PSNR ~ log(parameters) with r=0.986 up to 18 cascades. Achieves SOTA across all four CMRxRecon2025 challenge tracks without fine-tuning, surpassing specialized baselines by up to +1.0 dB. Outperforms CMRxRecon2024 winner PromptMR+ by +0.55 dB and fastMRI brain PC-RNN by +1.8 dB.

Conclusion: SDUM establishes a practical path toward universal, scalable MRI reconstruction that generalizes across diverse clinical protocols without task-specific adaptation.
...
TL;DR: The paper presents a method to transform casual photos of people into professional-looking portraits with chosen poses, good lighting, and standard clothing while preserving identity.


<details>
  <summary>Details</summary>
Motivation: Professional photos have beautiful lighting, interesting poses, and flattering quality that ordinary self-photos lack. There's a need to create professional versions of personal photos while preserving the person's unique identity, but no large paired dataset exists of the same person photographed both casually and professionally.

Method: Two key insights: 1) Transform input photo and person's face to canonical UV space, coupled with reposing methodology to model occlusions and novel view synthesis. Operating in UV space allows leveraging existing unpaired datasets. 2) Personalize output via multi-image fine-tuning.
...
Conclusion: The method successfully creates professional-looking portraits from casual photos by using UV space transformation and personalization techniques, overcoming the lack of paired training data.
...
TL;DR: A training-free framework for document background generation with multi-page editing, preserving text readability through latent masking and automated readability optimization while maintaining thematic continuity across pages.


<details>
  <summary>Details</summary>
Motivation: Current document design workflows often struggle with maintaining visual consistency across multiple pages while preserving text readability when editing backgrounds. There's a need for automated systems that can generate aesthetically pleasing backgrounds that don't compromise text legibility and maintain thematic coherence throughout a document.

Method: 1) Latent masking formulation that softly attenuates diffusion updates in text regions using smooth barrier functions; 2) Automated Readability Optimization (ARO) that places semi-transparent rounded backing shapes behind text with minimal opacity to meet WCAG 2.2 contrast standards; 3) Multi-page consistency through summarization-and-instruction process where each page is distilled into compact representations that recursively guide subsequent generations; 4) Document-as-structured-composition approach treating text, figures, and backgrounds as separate layers.

Result: The framework produces visually coherent, text-preserving, and thematically aligned documents without requiring training. It maintains readability while allowing stylistic adjustments through user prompts, balancing automated consistency with flexible customization.

Conclusion: This training-free framework successfully bridges generative modeling with natural design workflows by enabling automated background generation that preserves text readability, maintains multi-page thematic continuity, and allows user customization while treating documents as structured compositions of separate layers.
...
TL;DR: PhysFire-WM: A physics-informed world model that integrates infrared thermal data and fire masks with physical priors from simulators for accurate fine-grained fire spread prediction.
...
Method: PhysFire-WM encodes structured priors from a physical simulator to rectify physical discrepancies, combined with Cross-task Collaborative Training (CC-Train) that integrates thermal radiation dynamics and spatial boundary delineation through parameter sharing and gradient coordination.
...
<details>
  <summary>Details</summary>
Motivation: Current VLMs like CLIP require expensive annotated image-text pairs for modality alignment and use dual-tower encoders that hinder lightweight deployment. There's a need for more efficient, annotation-free approaches.

Method: LGCLIP uses LLMs to generate class-specific prompts, then employs diffusion models to synthesize reference images as visual prototypes. Real images are compared to these prototypes using only a visual encoder for classification.

Result: Experimental results show LGCLIP achieves strong performance in zero-shot classification tasks while being lightweight and efficient, requiring only class labels without manual annotations.

Conclusion: LGCLIP presents a novel paradigm for zero-shot classification that eliminates dependency on annotated image-text pairs and dual encoders, offering a lightweight and efficient alternative to traditional VLM approaches.
...
Method: ABE-CLIP uses a Semantic Refinement Mechanism to refine token embeddings for object and attribute phrases, mitigating attribute confusion. It also employs Local Token-Patch Alignment to compute similarity scores between refined textual tokens and their most relevant image patches, aggregating localized scores for final image-text similarity.
...
Conclusion: ABE-CLIP effectively addresses CLIP's attribute binding limitations through training-free semantic refinement and local alignment, demonstrating strong performance on compositional image-text matching tasks.
...
TL;DR: This paper investigates discrepancies between objective measurements of urban greenery (like Green View Index) and subjective human perceptions, finding that while discrepancies are globally consistent, location/cultural factors significantly influence perception more than demographics or personality.


<details>
  <summary>Details</summary>
Motivation: Urban greenery assessment is crucial for city planning and well-being, but there's often a gap between objective measurements (actual vegetation) and subjective perceptions (how green people feel a place is). The paper aims to quantify and explain these discrepancies.
...
Result: Discrepancies between objective and subjective greenery assessments are comparable worldwide. Demographics and personality don't significantly affect perception. Location (where people live) is the most influential factor among seven features examined, suggesting cultural, environmental, and experiential factors shape greenery perception.

Conclusion: Urban greenery perception is strongly influenced by location-based cultural and environmental factors rather than individual demographics or personality. This highlights the importance of considering local context and subjective experiences in urban planning alongside objective greenery measurements.
...
TL;DR: A globally optimal solver for relative pose estimation of multi-camera systems using affine correspondences with known vertical direction, achieving higher accuracy than state-of-the-art methods.
...
Conclusion: The globally optimal solver using affine correspondences with known vertical direction provides improved accuracy for relative pose estimation in multi-camera systems, making it valuable for applications like self-driving cars and other mobile devices with visual-inertial systems.
...
Conclusion: ARCD provides an effective, data-efficient solution to mitigate hallucinations in medical VLMs by offering targeted anatomical guidance through multi-level contrastive decoding, making it suitable for complex real-world clinical applications.
...
TL;DR: Fose: A lightweight pansharpening network that fuses one-step diffusion model distillation with end-to-end models for fast, high-quality image fusion.


<details>
  <summary>Details</summary>
Motivation: Current pansharpening methods have limitations: diffusion models are computationally expensive due to multi-step processes, while end-to-end models lack prior knowledge and have simple structures that limit performance.

Method: Four-stage training strategy: 1) One-step distillation of enhanced SOTA diffusion model (compressing from 50 to 1 step), 2) Fusion with E2E model using lightweight ensemble blocks, 3) Comprehensive experiments on three benchmarks.

Result: Significant improvement on three benchmarks, achieving 7.42× speedup compared to baseline diffusion model while delivering much better performance.

Conclusion: Fose successfully combines the accuracy of diffusion models with the efficiency of end-to-end models, creating a lightweight, high-performance pansharpening solution with substantial speed improvements.
...
TL;DR: Reasoning Palette uses a latent variable framework with VAE to enable strategic contextualization in vision-language models, allowing internal sampling of reasoning strategies before token generation for improved exploration and performance.


<details>
  <summary>Details</summary>
Motivation: Current stochastic sampling in large vision-language models produces redundant reasoning paths with limited high-level diversity, which constrains both inference performance and reinforcement learning training efficiency.

Method: Proposes a latent-modulation framework using a VAE to infer stochastic latent variables from question-answer pairs. These latents are decoded into token prefixes prepended to input prompts, modulating the model's internal reasoning trajectory. Includes supervised fine-tuning warm-up and integrates with RL optimization.

Result: Achieves consistent performance gains over standard RL methods across multiple reasoning benchmarks, enabling interpretable and controllable strategic behavior with enhanced exploration efficiency and sustained learning capability.

Conclusion: Reasoning Palette provides an effective framework for structured exploration in vision-language models through latent-based strategic contextualization, improving both inference diversity and RL training efficiency.
...
TL;DR: CheXPO-v2 is a novel alignment framework that addresses hallucinations in medical VLMs by shifting from outcome to process supervision, using Knowledge Graph Consistency Reward with Entity-Relation Matching to penalize incoherent logic at atomic level.


<details>
  <summary>Details</summary>
Motivation: Medical Vision-Language Models are prone to hallucinations that compromise clinical reliability. Existing reinforcement learning methods like GRPO rely on sparse outcome-based rewards, which encourage models to "overthink" - generating verbose, unverifiable reasoning that obscures factual errors and poses safety risks.

Method: Proposes CheXPO-v2 alignment framework with Knowledge Graph Consistency Reward mechanism driven by Entity-Relation Matching. The approach explicitly parses reasoning steps into structured "Disease, Relation, Anatomy" triplets, providing fine-grained supervision that penalizes incoherent logic and hallucinations at atomic level. Integrates this with a hard-example mining strategy.
...
Conclusion: CheXPO-v2 effectively addresses the hallucination problem in medical VLMs by shifting from outcome to process supervision, providing a more reliable and clinically sound alignment framework that produces verifiable reasoning with high data efficiency.
...
TL;DR: DAVE is a specialized vision encoder for VLMs that improves document understanding and web agent tasks by incorporating structural/spatial information through self-supervised pretraining on unlabeled data and supervised autoregressive training with model-merging and ensemble techniques.


<details>
  <summary>Details</summary>
Motivation: Current VLMs have a fundamental weakness: their vision encoders lack robust structural and spatial information needed for document understanding and web agent tasks, which require precise layout understanding and localization capabilities.

Method: Two-stage training: 1) Self-supervised pretraining on unlabeled document/web images, 2) Supervised autoregressive pretraining with limited high-quality data for parsing/localization. Uses model-merging to combine encoders trained with different decoders for broad compatibility, and ensemble training to fuse features from generalist encoders (SigLIP2) with domain-specific representations.
...
Conclusion: DAVE successfully bridges the gap in VLMs by providing a purpose-built vision encoder with enhanced structural/spatial understanding for document and web agent tasks, using efficient training strategies that minimize annotation costs.
...
TL;DR: AOM is a universal Remote Sensing Foundation Model that handles arbitrary band compositions, sensor types, and resolution scales, overcoming limitations of fixed-configuration RSFMs.


<details>
  <summary>Details</summary>
Motivation: Existing RSFMs are pretrained on fixed band configurations and resolutions, making them vulnerable to real-world scenarios involving missing bands, cross-sensor fusion, and unseen spatial scales, limiting their generalization and practical deployment.
...
Conclusion: AOM successfully addresses the limitations of existing RSFMs by providing a universal foundation model that accommodates arbitrary optical sensor configurations, significantly improving generalization and practical deployment capabilities.
...
Method: Proposes an aggregator module that learns global descriptors consistent with both geometrical structure and visual similarity. Uses batch-mining strategy based solely on overlap scores and modified contrastive loss, enabling training without manual place labels.
...
TL;DR: MLLMs suffer from "visual forgetting" in long reasoning tasks. The paper proposes a curriculum framework to disentangle abstract reasoning from visual perception, using SFT for reasoning and RL for learning when to look.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs fail at complex visual reasoning due to "visual forgetting" - they lose visual grounding as reasoning extends. This stems from prematurely entangling abstract reasoning and visual perception skills during training.
...
Conclusion: Disentangling reasoning and perception skills through curriculum learning and RL-based timing policy enables MLLMs to maintain visual grounding in long-chain reasoning tasks, solving the "think longer, see less" problem.
...
TL;DR: VideoDetective enables MLLMs to efficiently process long videos (up to 1 hour) by using question-aware memory tokens to compress and seek critical clues, reducing GPU memory usage while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Long Video QA is challenging due to immense context and information overload in MLLMs, causing prohibitive memory consumption. Existing methods that reduce visual tokens or extend context length may miss useful information or require excessive computation.

Method: Proposes VideoDetective with question-aware memory mechanism: iteratively processes video sub-segments, uses special memory tokens for question-aware compression to seek critical clues, and recurrently aggregates/store memory tokens to update history context for subsequent segments.

Result: Enables MLLMs with 32K context length to process 100K tokens (3600 frames, 1-hour video at 1fps) in 2 minutes using only 37GB GPU memory. Outperforms on multiple long video benchmarks by effectively seeking critical clues from massive information.

Conclusion: VideoDetective provides an efficient solution for LVQA by focusing on question-aware compression and recurrent memory aggregation, enabling practical long video processing with limited computational resources while maintaining strong performance.
...
TL;DR: Mitty is a Diffusion Transformer that enables end-to-end Human2Robot video generation using video In-Context Learning, translating human demonstrations directly into robot-execution videos without intermediate representations.


<details>
  <summary>Details</summary>
Motivation: Existing methods rely on intermediate representations like keypoints or trajectories, which introduce information loss and cumulative errors that harm temporal and visual consistency in robot learning from human demonstrations.

Method: Built on a pretrained video diffusion model, Mitty compresses demonstration videos into condition tokens and fuses them with robot denoising tokens through bidirectional attention during diffusion. An automatic synthesis pipeline produces human-robot pairs from large egocentric datasets to mitigate paired-data scarcity.

Result: Experiments on Human2Robot and EPIC-Kitchens show state-of-the-art results, strong generalization to unseen environments, and new insights for scalable robot learning from human observations.

Conclusion: Mitty enables direct learning from human demonstration videos through end-to-end video generation, addressing limitations of intermediate representations and advancing scalable robot learning from human observations.
...
<details>
  <summary>Details</summary>
Motivation: Robust anatomical segmentation of chest X-rays is challenging due to scarce comprehensive annotations and high variability in real-world acquisition conditions. There's a need for a scalable solution that reduces annotation burden while maintaining robustness across diverse imaging conditions.
...
Result: Achieves strong zero-shot generalization on multiple real-world datasets, accurately delineating 54 anatomical structures across PA, lateral, and oblique views. Supports downstream clinical tasks including cardiothoracic ratio estimation, spine curvature assessment, and disease classification with improved diagnostic performance.

Conclusion: AnyCXR establishes a scalable and reliable foundation for anatomy-aware CXR analysis, offering a practical pathway to reduce annotation burdens while improving robustness across diverse imaging conditions through synthetic supervision.
...
TL;DR: WDFFU-Mamba is a novel breast ultrasound segmentation network that combines wavelet-guided enhancement and dual-attention feature fusion in a U-shaped Mamba architecture, achieving state-of-the-art performance on public BUS datasets.


<details>
  <summary>Details</summary>
Motivation: Breast ultrasound segmentation is crucial for clinical diagnosis and early tumor screening, but faces challenges from speckle noise, imaging artifacts, irregular lesion morphology, and blurred boundaries that hinder accurate segmentation.
...
Conclusion: The combination of wavelet-domain enhancement and attention-based fusion improves both accuracy and robustness of BUS image segmentation while maintaining computational efficiency. The model shows strong generalization across datasets and is promising for real-world clinical applications in breast tumor ultrasound analysis.
...
<details>
  <summary>Details</summary>
Motivation: Current ultrasound AI tools are fragmented into single-task applications, limiting their clinical utility compared to modern versatile ultrasound systems that require multi-organ analysis capabilities.

Method: The Universal UltraSound Image Challenge 2025 (UUSIC25) evaluated algorithms on 11,644 training images (public/private) and tested on an independent multi-center set of 2,479 images, including completely unseen center data to assess generalization. Performance was measured using Dice Similarity Coefficient (DSC) for segmentation and AUC for classification, plus computational efficiency metrics.

Result: Top model (SMART) achieved macro-averaged DSC of 0.854 across 5 segmentation tasks and AUC of 0.766 for binary classification. Models excelled at segmentation (fetal head DSC: 0.942) but showed variability in complex tasks. Notably, breast cancer subtyping performance dropped from AUC 0.571 (internal) to 0.508 (unseen external center), highlighting generalization challenges.

Conclusion: General-purpose AI models can achieve high accuracy and efficiency across multiple ultrasound tasks using single architectures, but performance degradation on unseen data indicates domain generalization remains a critical challenge for clinical deployment.
...
TL;DR: VLMIR is a two-stage framework that uses vision-language models (like CLIP) to extract both visual and linguistic features for better image restoration, combining VLM-based feature extraction with diffusion-based restoration.
...
Method: Two-stage approach: 1) VLM-based feature extraction - extract complementary visual and linguistic representations by aligning embeddings of captions from low/high-quality images using cosine similarity loss with LoRA fine-tuning, and using a degradation predictor to separate degradation and clean content embeddings. 2) Diffusion-based restoration - integrate these embeddings into a diffusion model via cross-attention mechanisms.

Result: Extensive experiments show VLMIR achieves superior performance across both universal and degradation-specific image restoration tasks, demonstrating the importance of integrated visual and linguistic knowledge from VLMs.

Conclusion: The VLMIR framework effectively leverages vision-language priors to enhance image restoration through improved visual perception and semantic understanding, highlighting the critical role of integrated visual and linguistic knowledge in advancing restoration capabilities.
...
TL;DR: HiSIR-Net: A high-resolution self-supervised reconstruction framework for pixel-wise PCBA defect inspection that handles 4K images with low false positives using selective reconstruction gates and optimized patch selection.


<details>
  <summary>Details</summary>
Motivation: Automated defect inspection of PCBA is challenging due to insufficient labeled data, micro-defects with few pixels, and visually-complex high-resolution images. There's a gap in high-resolution PCBA datasets.
...
Result: Method produces clean, high-resolution anomaly maps with low false positive rate. Outperforms existing methods on SIPCBA-500 dataset (500 self-collected images) and public benchmarks while running at practical speed.

Conclusion: HiSIR-Net effectively addresses PCBA defect inspection challenges through self-supervised reconstruction with selective gating and optimized patch selection, demonstrating superior localization performance on high-resolution images.
...
Conclusion: ProCache effectively addresses the limitations of existing feature caching methods by aligning with DiT's temporal dynamics and mitigating error accumulation, enabling significant acceleration of Diffusion Transformers without quality degradation.
...
TL;DR: A generative framework for high-quality PBR textures on 3D meshes using fine-tuned latent diffusion models with material-aware regularization.
...
Method: Fine-tunes pretrained VAE to incorporate new material channels with minimal latent distribution deviation, introduces locality-preserving regularization through latent patch cropping and image region alignment, and learns a material latent space (MatLat).

Result: Improves PBR texture fidelity compared to previous baselines, with each component being critical for achieving state-of-the-art performance.

Conclusion: The proposed framework effectively leverages pretrained latent image generative models while addressing distribution shift and cross-view consistency issues through targeted VAE fine-tuning and locality-preserving regularization.
...
TL;DR: EMAG is a training-free guidance method for diffusion transformers that adaptively modifies attention at inference to create harder negative samples, improving image quality over CFG.


<details>
  <summary>Details</summary>
Motivation: Existing guidance methods like CFG and recent strong/weak model approaches lack reliable control over negative sample granularity/difficulty and have fixed target-layer selection, limiting their ability to surface subtle failure modes for refinement.

Method: EMAG modifies attention at inference time in diffusion transformers using a statistics-based adaptive layer-selection rule, producing harder semantically faithful negative samples (fine-grained degradations) without requiring training.

Result: EMAG improves human preference score (HPS) by +0.46 over CFG and can compose with advanced guidance techniques like APG and CADS for further HPS improvements.
...
TL;DR: DRIM enables deep, reliable multi-turn reasoning in vision-language models through self-reflective reasoning patterns and redundancy-penalized policy optimization.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs struggle to reflect on and correct themselves when following incorrect reasoning trajectories in multimodal Chain-of-Thought, limiting their reliability in complex visual tasks.
...
Result: DRIM achieves superior performance on visual understanding benchmarks by developing self-reflective reasoning patterns that enable reliable multi-turn reasoning.

Conclusion: The proposed DRIM framework successfully addresses the self-correction limitation in VLMs by enabling deep, reliable multimodal reasoning through structured training and redundancy-aware optimization.
...
TL;DR: CodeDance uses executable code as a general solver for visual reasoning, enabling flexible tool orchestration and transparent reasoning with visual artifacts, outperforming both schema-driven and closed models across benchmarks.
...
Method: Introduces CodeDance which defines, composes, and executes code to orchestrate multiple tools, compute intermediate results, and render visual artifacts. Uses a reward mechanism for balanced and adaptive tool-call to balance exploration with efficiency and mitigate tool overuse.

Result: Outperforms schema-driven and text-only baselines, and surpasses advanced closed models like GPT-4o and larger open-source models across reasoning benchmarks (visual search, math, chart QA). Shows emergent behaviors: novel tool invocations, unseen compositions, and cross-task transfer without task-specific fine-tuning.

Conclusion: CodeDance demonstrates a general and scalable mechanism for executable visual reasoning through code-based orchestration, enabling transparent, self-checkable reasoning with emergent capabilities beyond atomic supervision.
...
TL;DR: ADK framework enhances VLMs by using LLM-generated descriptive prompts as auxiliary knowledge, improving classification without computational overhead.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with distribution shifts in downstream tasks, and existing PEFT methods rely on fixed prompts that lack semantic understanding of classes. Image-induced prompts help but add computational overhead.

Method: ADK uses LLM to generate descriptive prompts offline, then deploys them as: 1) Compositional Knowledge (averaged representation), and 2) Instance-Specific Knowledge (lightweight attention to select relevant descriptions). It's parameter-free and plug-and-play.

Result: ADK consistently boosts performance of multiple PEFT baselines, achieving state-of-the-art results across various scenarios.

Conclusion: ADK efficiently enriches text representations for VLMs without compromising efficiency, enhancing category distinction across domains as a parameter-free component.
...
<details>
  <summary>Details</summary>
Motivation: Current remote sensing benchmarks have issues: most use low-resolution imagery, some have flawed reasoning-task designs, and text-only LLMs can perform competitively with multimodal models without seeing images, indicating a mismatch between benchmarks and intended visual understanding evaluation.

Method: Created RSHR-Bench with 5,329 super-high-resolution images (long side ≥4,000px, up to ~300M pixels) from RS corpora and UAV collections. Designed four task families: multiple-choice VQA, open-ended VQA, image captioning, and single-image evaluation. Applied adversarial filtering with strong LLMs followed by human verification to reduce language priors.

Result: Constructed 3,864 VQA tasks, 3,913 image captioning tasks, and 500 human-verified single-image evaluation VQA pairs. Evaluations across open-source, closed-source, and RS-specific VLMs show persistent performance gaps in super-high-resolution scenarios, demonstrating the benchmark's effectiveness in revealing model limitations.

Conclusion: RSHR-Bench provides a faithful assessment framework for remote sensing visual understanding and reasoning, addressing critical flaws in existing benchmarks and enabling proper evaluation of multimodal models' capabilities with super-high-resolution imagery.
...
<details>
  <summary>Details</summary>
Motivation: Text-to-image generation raises privacy, bias, and copyright concerns. Concept erasure techniques promise selective removal of undesired concepts without full retraining, but current evaluations are limited and simplistic.

Method: Introduce EMMA benchmark with 12 metrics across five key dimensions: robustness to indirect descriptions, handling visually similar non-target concepts, gender/ethnicity bias analysis, image quality, and time efficiency. Evaluate five concept erasure methods across five domains (objects, celebrities, art styles, NSFW, copyright).
...
Conclusion: Current concept erasure techniques have significant limitations in robustness and social awareness. EMMA provides comprehensive evaluation framework to guide future improvements in concept erasure methods.
...
TL;DR: A diverse, high-quality dataset of color fundus images with detailed artery-vein segmentation annotations for developing ML algorithms in retinal vascular analysis.


<details>
  <summary>Details</summary>
Motivation: To provide a comprehensive dataset supporting the development and evaluation of machine learning algorithms for vascular analysis in ophthalmology, addressing the need for diverse, high-quality annotations under real-world conditions.
...
TL;DR: DESSERT is a diffusion-based framework for event-driven single-frame synthesis that uses residual training with a pre-trained Stable Diffusion model to generate sharper, more temporally consistent future frames from event camera data.


<details>
  <summary>Details</summary>
Motivation: Traditional video frame prediction struggles with dynamic scenes due to lack of future frame information. Event cameras capture high-temporal-resolution brightness changes but existing event-based approaches using optical flow suffer from holes and blurring when pixel displacement is inaccurate.

Method: Two-stage training: (1) Event-to-Residual Alignment VAE (ER-VAE) aligns event frames between anchor/target frames with corresponding residuals, (2) Diffusion model denoises residual latent conditioned on event data. Uses Diverse-Length Temporal augmentation for robustness.
...
Conclusion: DESSERT successfully addresses limitations of optical flow-based approaches by using diffusion-based residual training with event data, achieving superior frame synthesis quality and temporal consistency through its two-stage architecture and temporal augmentation.
...
<details>
  <summary>Details</summary>
Motivation: Current vision-language models for pathology have limitations: they focus on small regions, provide only static slide-level outputs, rely on non-public data limiting reproducibility, and lack sufficient training data with detailed clinical reports paired with whole-slide images.

Method: Three main contributions: 1) Polysome - standardized tool for synthetic instruction generation, 2) Applied Polysome to HISTAI dataset to create HISTAI-Instruct (24,259 slides, 1.1M instruction-response pairs), 3) Trained ANTONI-α VLM using HISTAI-Instruct for visual-question answering tasks.
...
Conclusion: The work addresses key limitations in pathology VLMs by providing standardized synthetic instruction generation, creating a large public dataset, and developing a high-performing VLM that advances transparent and generalizable vision-language models for pathology applications.
...
TL;DR: SynergyWarpNet: Attention-guided cooperative warping framework for high-fidelity talking head synthesis that combines explicit warping with reference-augmented correction and confidence-guided fusion.


<details>
  <summary>Details</summary>
Motivation: Traditional explicit warping struggles with accurate motion transfer and missing regions, while attention-based methods have high complexity and weak geometric grounding. Need for a balanced approach that maintains both structural accuracy and visual quality.
...
Conclusion: SynergyWarpNet effectively addresses limitations of both explicit and attention-based warping methods by combining their strengths through a cooperative framework, achieving high-fidelity portrait animation.
...
TL;DR: MDDN is a novel network for omnidirectional image super-resolution that addresses latitude-dependent distortion in ERP-projected images through multi-level deformable feature extraction with expanded sampling ranges.


<details>
  <summary>Details</summary>
Motivation: Existing ODISR methods have limited sampling ranges and feature extraction capabilities, which hinders their ability to capture distorted patterns in ERP-projected omnidirectional images where distortion becomes severe toward the poles.

Method: Proposes Multi-level Distortion-aware Deformable Network (MDDN) with three parallel branches: deformable attention (dilation=1) and two dilated deformable convolutions (dilation rates 2 and 3). Features are adaptively fused in multi-level feature fusion module, with low-rank decomposition applied to reduce computational cost.
...
Conclusion: MDDN successfully addresses the challenge of latitude-dependent geometric distortion in ERP images by expanding sampling ranges and receptive fields, providing a superior solution for omnidirectional image super-resolution.
...
<details>
  <summary>Details</summary>
Motivation: Current AI-generated image detectors fail to generalize to unseen generative models because they overfit to source-specific semantic cues rather than learning universal generative artifacts. There's a need for methods that can improve cross-generator performance.

Method: Introduces a simple pixel-level mapping pre-processing step that disrupts the pixel value distribution of images. This breaks the fragile, non-essential semantic patterns that detectors commonly exploit as shortcuts, forcing them to focus on more fundamental and generalizable high-frequency traces inherent to the image generation process.

Result: The approach significantly boosts the cross-generator performance of state-of-the-art detectors on both GAN and diffusion-based generators. Extensive analysis verifies that disrupting semantic cues is key to achieving generalization across different generative models.

Conclusion: Breaking semantic shortcuts through pixel-level mapping pre-processing is an effective strategy for improving the generalization capability of AI-generated image detectors across diverse generative models, addressing a critical limitation in current detection methods.
...
TL;DR: The paper introduces Affective Image Filter (AIF) task that transforms text with visually-abstract emotions into emotionally compelling images, proposing two models (AIF-B and AIF-D) that outperform state-of-the-art methods in content consistency and emotional fidelity.


<details>
  <summary>Details</summary>
Motivation: Social media users express emotions through text with images, but current systems don't effectively translate abstract emotional text into visually concrete, emotionally compelling images. There's a need to bridge the gap between textual emotional expression and visual emotional representation.

Method: 1) Introduces AIF dataset and task formulation; 2) Proposes AIF-B as initial multi-modal transformer architecture; 3) Extends to AIF-D using pre-trained large-scale diffusion models for deeper emotional reflection; 4) Uses quantitative, qualitative, and user study evaluations.

Result: AIF models achieve superior performance in content consistency and emotional fidelity compared to state-of-the-art methods. User studies show AIF models are significantly more effective at evoking specific emotions.

Conclusion: AIF models demonstrate significant value and potential for creating emotionally compelling visual content from textual emotional expressions, with promising applications in social media and emotional content generation.
...
TL;DR: RadImageNet-VQA is a large-scale radiologic VQA dataset with 750K CT/MRI images and 7.5M QA pairs covering abnormality detection, anatomy recognition, and pathology identification across 8 anatomical regions and 97 pathology categories.
...
Method: Built from expert-curated annotations, the dataset includes 750K CT and MRI images paired with 7.5M question-answer samples covering three key tasks (abnormality detection, anatomy recognition, pathology identification) across eight anatomical regions and 97 pathology categories, supporting open-ended, closed-ended, and multiple-choice questions.
...
Conclusion: RadImageNet-VQA provides a comprehensive benchmark for radiologic VQA research, revealing current model limitations and enabling future development of more capable medical vision-language systems. The dataset is publicly available.
...
TL;DR: Researchers found a faster alternative to occlusion-based explanation methods for prostate cancer detection AI, reducing computation time by 10x without quality loss.
...
Method: Developed a framework for comparing explanation methods using identified criteria and metrics. Evaluated various explanation techniques to find a faster replacement for occlusion in a prostate cancer detection system.

Result: Successfully identified an alternative explanation method that reduces computation time by at least a factor of 10 compared to occlusion, with no negative impact on explanation quality.

Conclusion: The speedup enables faster model development and debugging, bringing AI-assisted prostate cancer detection closer to clinical adoption. The evaluation framework can be applied to other medical AI applications needing explainable outputs.
...
TL;DR: AIFloodSense is a comprehensive global aerial flood imagery dataset with 470 high-resolution images from 230 flood events across 64 countries, supporting classification, segmentation, and VQA tasks to advance flood detection AI.


<details>
  <summary>Details</summary>
Motivation: Current flood segmentation datasets are scarce, geographically limited, and lack annotation detail, hindering development of robust, generalized computer vision methods for disaster response and risk assessment.

Method: Created AIFloodSense dataset with 470 high-resolution aerial images from 230 distinct flood events across 64 countries and six continents (2022-2024). Supports three tasks: image classification (environment type, camera angle, continent recognition), semantic segmentation (pixel-level masks for flood, sky, buildings), and visual question answering for disaster assessment.
...
Conclusion: AIFloodSense bridges the gap in flood detection datasets by providing comprehensive, globally diverse, and temporally relevant data to support development of robust AI tools for disaster response and climate resilience.
...
TL;DR: Open-sourced MiMo-VL-Miloco-7B, a vision-language model specialized for smart-home environments that achieves strong performance on both home-scenario understanding and general multimodal reasoning.


<details>
  <summary>Details</summary>
Motivation: To develop a specialized vision-language model for smart-home applications that balances home-scenario understanding with general multimodal reasoning capabilities.

Method: Two-stage training pipeline combining supervised fine-tuning with reinforcement learning (Group Relative Policy Optimization), using chain-of-thought supervision and token-budget-aware reasoning for efficient learning.

Result: Achieves leading F1 scores on gesture recognition and home-scenario understanding, with consistent gains across video benchmarks (Video-MME, Video-MMMU, Charades-STA) and language benchmarks (MMMU-Pro, MMLU-Pro), outperforming both closed-source and open-source baselines.
...
<details>
  <summary>Details</summary>
Motivation: To enable fine-grained editing of real-world driving videos using natural language instructions, allowing users to create diverse traffic scenarios through intuitive text commands rather than complex manual editing tools.

Method: Uses explicit 3D scene decomposition to represent driving videos as scene graphs with static background and dynamic objects. Implements an agentic pipeline where an Orchestrator transforms user instructions into execution graphs that coordinate specialized agents: Object Grounding Agent (text-to-object correspondence), Behavior Editing Agent (generates multi-object trajectories), and Behavior Reviewer Agent (iterative refinement). Edited scene graphs are rendered and refined with video diffusion tools to address artifacts.
...
Conclusion: LangDriveCTRL provides an effective framework for natural-language-controllable driving video editing, enabling intuitive creation of diverse traffic scenarios with high realism and instruction alignment through its agentic pipeline and 3D scene decomposition approach.
...
TL;DR: A novel multimodal maritime dataset called MULTIAQUA is introduced for training robust scene interpretation models that work in challenging visual conditions like darkness and poor weather.


<details>
  <summary>Details</summary>
Motivation: Unmanned surface vehicles face varied visual circumstances during operation, with some conditions (like poor weather, nighttime, low visibility) being difficult to interpret using only RGB cameras. There's a need for multimodal data to enable robust scene interpretation regardless of visibility conditions.

Method: Created MULTIAQUA dataset containing synchronized, calibrated and annotated multimodal data (RGB, thermal, IR, LIDAR, etc.). Proposed training approaches that enable multimodal methods to be trained robustly using only daytime images, allowing models to perform reliably even in near-complete darkness.

Result: The dataset enables development of supervised methods that extract useful information from multiple modalities. Evaluation on difficult nighttime test sets shows the approach allows training robust deep neural networks using only daytime images, significantly simplifying data acquisition, annotation, and training.

Conclusion: The MULTIAQUA dataset provides valuable multimodal maritime data for developing robust scene interpretation systems that can operate effectively in challenging visibility conditions, with training approaches that reduce data collection burdens while maintaining performance in darkness.
...
TL;DR: 3D-RE-GEN is a compositional framework that reconstructs single images into textured 3D mesh scenes suitable for artists' workflows, addressing current limitations in object decomposition, spatial relationships, and background generation.


<details>
  <summary>Details</summary>
Motivation: Current 3D scene generation methods produce visually appealing output but are not artist-ready - they suffer from incorrect object decomposition, inaccurate spatial relationships, and missing backgrounds, hindering workflows for visual effects and game development that require modifiable 3D textured mesh scenes.

Method: A compositional pipeline integrating state-of-the-art models for asset detection, reconstruction, and placement. Uses image editing with generative models for occluded objects with scene-level reasoning. Features a novel 4-DoF differentiable optimization to align objects with estimated ground plane, and generates comprehensive backgrounds for spatial constraints and realistic lighting.
...
Conclusion: 3D-RE-GEN addresses artists' requirements by combining domain-specific models to create artist-ready textured 3D mesh scenes suitable for visual effects and game development workflows.
...
TL;DR: TwinSegNet: A privacy-preserving federated learning framework using hybrid ViT-UNet with personalized digital twins for brain tumor segmentation across multiple institutions without centralized data sharing.
...
Method: Proposes TwinSegNet, a federated learning framework that integrates a hybrid ViT-UNet model (convolutional encoders with Vision Transformer bottlenecks) with personalized digital twins. Each institution fine-tunes the global model on their private data to create institution-specific digital twins while preserving data privacy.

Result: Evaluated on nine heterogeneous MRI datasets (BraTS 2019-2021 and custom collections), TwinSegNet achieves high Dice scores (up to 0.90%) and sensitivity/specificity exceeding 90%, demonstrating robustness across non-IID client distributions. Performs comparably to centralized models like TumorVisNet while preserving privacy.

Conclusion: TwinSegNet enables scalable, personalized brain tumor segmentation for multi-institutional clinical settings while adhering to strict data confidentiality requirements, offering a practical solution to privacy-preserving medical image analysis without sacrificing performance.
...
TL;DR: LumiCtrl: A method for learning illuminant prompts from single object images to enable precise lighting control in text-to-image generation.
...
Method: Three-component approach: (1) physics-based illuminant augmentation along Planckian locus for fine-tuning variants, (2) edge-guided prompt disentanglement using frozen ControlNet to focus on illumination rather than structure, (3) masked reconstruction loss for foreground object focus with contextual background adaptation.

Result: Significantly better illuminant fidelity, aesthetic quality, and scene coherence compared to existing personalization baselines, with strong user preference confirmed in human study.

Conclusion: LumiCtrl enables precise illuminant control in T2I generation through physics-based augmentation and prompt disentanglement, addressing a crucial limitation in current creative image generation systems.
...
TL;DR: MMLANDMARKS is a multimodal geo-spatial dataset with 197k aerial images, 329k ground images, text, and GPS coordinates for 18,557 US landmarks, enabling unified training for cross-view retrieval, geolocalization, and text-to-image/GPS tasks.


<details>
  <summary>Details</summary>
Motivation: Current geo-spatial benchmarks have limited modality coverage, restricting progress as approaches cannot integrate all relevant modalities (images from various viewpoints, text, coordinates) within a unified framework for comprehensive geo-spatial understanding.

Method: Introduces MMLANDMARKS dataset with one-to-one correspondence across four modalities: high-resolution aerial images, ground-view images, textual information, and geographic coordinates for distinct US landmarks. Uses a simple CLIP-inspired baseline model for evaluation.

Result: Demonstrates broad generalization and competitive performance against off-the-shelf foundational models and specialized state-of-the-art models across tasks including cross-view Ground-to-Satellite retrieval, geolocalization, and Text-to-Image/GPS retrieval.

Conclusion: The necessity for multimodal datasets like MMLANDMARKS to achieve broad geo-spatial understanding, as unified multimodal frameworks outperform limited-modality approaches and enable comprehensive geo-spatial analysis.
...
TL;DR: GroundingME is a new benchmark that systematically tests MLLMs' visual grounding capabilities across four challenging dimensions, revealing significant gaps between current models and human-like performance.


<details>
  <summary>Details</summary>
Motivation: Current benchmarks fail to capture real-world complexity in visual grounding, where humans effortlessly handle ambiguous references and recognize when grounding is impossible. There's a need to rigorously assess whether MLLMs truly understand visual grounding or just pattern-match on simplified datasets.

Method: Created GroundingME benchmark with 1,005 challenging examples through careful curation combining automated generation with human verification. The benchmark systematically tests four dimensions: Discriminative (similar objects), Spatial (complex relations), Limited (occlusions/tiny objects), and Rejection (ungroundable queries). Evaluated 25 state-of-the-art MLLMs and explored two improvement strategies: test-time scaling and data-mixture training.

Result: Evaluation revealed profound capability gaps: best model achieved only 45.1% accuracy, most scored 0% on rejection tasks (hallucinating objects instead of acknowledging absence). Test-time scaling improved complex grounding by up to 2.9%, while data-mixture training boosted rejection accuracy from 0% to 27.9%.

Conclusion: GroundingME serves as both a diagnostic tool revealing current limitations in MLLMs' visual grounding capabilities and a roadmap toward achieving human-level performance, highlighting critical safety concerns for deployment.
...
TL;DR: First external validation of prostate cancer AI pathology models in Middle Eastern population shows pathologist-level performance and high cross-scanner consistency, supporting global AI adoption.


<details>
  <summary>Details</summary>
Motivation: AI pathology systems have been primarily validated on European/US cohorts, leaving under-represented populations unassessed despite potentially greater benefits. This study addresses the need for validation in diverse global populations, particularly the Middle East.

Method: Collected 339 prostate biopsy specimens from 185 consecutive patients in Kurdistan, Iraq (2013-2024). Evaluated a task-specific end-to-end AI model and two foundation models against pathologist consensus, assessing concordance and cross-scanner consistency across three scanner models (Hamamatsu, Leica, Grundium).

Result: AI-pathologist grading concordance (κ=0.801) was statistically equivalent to pathologist-pathologist concordance (κ=0.799, p=0.9824). High cross-scanner consistency (κ>0.90) for all AI models and scanner pairs, including low-cost compact scanner.

Conclusion: AI models achieve pathologist-level performance in prostate histopathology. Compact scanners enable cost-effective AI adoption in resource-limited settings. The first openly available Middle Eastern digital pathology dataset supports equitable global AI pathology development.
...
TL;DR: InsertAnywhere is a diffusion-based video object insertion framework that achieves geometrically consistent placement and appearance-faithful synthesis through 4D scene understanding and illumination-aware training.
...
Method: 1) 4D aware mask generation module reconstructs scene geometry and propagates object placement across frames with temporal coherence and occlusion consistency. 2) Extends diffusion-based video generation to jointly synthesize inserted objects and local variations like illumination/shading. 3) Uses ROSE++ synthetic dataset with object-removed videos, object-present videos, and VLM-generated reference images for supervised training.
...
Conclusion: InsertAnywhere successfully addresses key challenges in video object insertion by combining 4D scene understanding with illumination-aware synthesis, demonstrating superior performance through both technical innovation and dataset development.
...
TL;DR: FALCON-SFOD improves source-free object detection by enhancing object-focused feature representations and addressing pseudo-label noise, achieving competitive performance on SFOD benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current SFOD methods using Mean-Teacher self-labeling suffer from domain shift that weakens object-focused representations, causing high-confidence background activations and unreliable pseudo-labels. Prior works focus on refining pseudo-labels but neglect strengthening the feature space itself.
...
Conclusion: FALCON-SFOD effectively addresses the core limitation of current SFOD approaches by strengthening object-focused feature representations and handling pseudo-label noise, leading to improved adaptation under domain shift.
...
TL;DR: PathBench-MIL is an open-source AutoML framework for multiple instance learning in histopathology that automates end-to-end MIL pipelines and provides reproducible benchmarking.


<details>
  <summary>Details</summary>
Motivation: To address the need for standardized, reproducible, and automated MIL workflows in histopathology, where manual pipeline construction and inconsistent benchmarking hinder progress.

Method: Develops an open-source framework that automates end-to-end MIL pipeline construction including preprocessing, feature extraction, and MIL-aggregation. Provides unified configuration system, visualization tools, and modular extensibility for rapid experimentation.

Result: Created PathBench-MIL framework that enables reproducible benchmarking of dozens of MIL models and feature extractors, standardizing experiments across datasets and tasks.

Conclusion: PathBench-MIL provides a comprehensive AutoML solution for histopathology MIL that accelerates research through automation, standardization, and reproducibility, with public availability for community adoption.
...
<details>
  <summary>Details</summary>
Motivation: Current robust MLLMs rely on implicit training that focuses only on visual encoder generalization, leading to limited interpretability and isolated optimization. They struggle with extreme real-world visual degradations that hinder practical robustness.

Method: Proposes Robust-R1 with three components: 1) supervised fine-tuning for degradation-aware reasoning foundations, 2) reward-driven alignment for accurate degradation parameter perception, and 3) dynamic reasoning depth scaling adapted to degradation intensity. Also introduces an 11K dataset with realistic degradations across four visual processing stages, annotated with structured reasoning chains.

Result: State-of-the-art robustness: Outperforms all general and robust baselines on R-Bench real-world degradation benchmark. Maintains superior anti-degradation performance under multi-intensity adversarial degradations on MMMB, MMStar, and RealWorldQA.

Conclusion: Explicit modeling of visual degradations through structured reasoning chains significantly improves MLLM robustness against real-world visual degradations, addressing limitations of existing implicit approaches.
...
TL;DR: FLEG is a feed-forward network that reconstructs language-embedded 3D Gaussians from arbitrary views without requiring 3D annotations, using large-scale video data and contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Previous methods for 3D Gaussian reconstruction suffer from fixed input views and insufficient 3D training data, requiring 3D annotations which are expensive to obtain.

Method: Proposes a 3D-annotation-free training framework using 2D-to-3D lifting from uncalibrated multi-view images, instance-guided contrastive learning to align 2D semantics with 3D representations, and geometry-semantic hierarchical sparsification to reduce computational cost.
...
Conclusion: The method enables effective 3D Gaussian reconstruction with semantic embedding without requiring 3D annotations, leveraging large-scale 2D video data and addressing computational efficiency through sparsification.
...
TL;DR: ClothHMR: A 3D human mesh recovery method that handles diverse clothing by tailoring clothing to fit body silhouettes and using foundational human visual models for better generalization.
...
Conclusion: ClothHMR effectively addresses the challenge of 3D human mesh recovery under diverse clothing conditions through clothing tailoring and foundational model integration, with practical applications demonstrated through a web application for online fashion and shopping.
...
TL;DR: G3Splat introduces geometric priors to address ambiguities in learning 3D Gaussian splats from view-synthesis supervision alone, achieving state-of-the-art performance in geometrically consistent reconstruction, pose estimation, and novel-view synthesis.


<details>
  <summary>Details</summary>
Motivation: Prior work on regressing 3D Gaussians from images relies almost exclusively on view-synthesis supervision, which is insufficient for recovering geometrically meaningful splats. There are ambiguities in learning 3D Gaussian splats under self-supervision for pose-free generalizable splatting that need to be addressed.

Method: G3Splat enforces geometric priors to obtain geometrically consistent 3D scene representations. The approach analyzes and addresses the ambiguities of learning 3D Gaussian splats under self-supervision for pose-free generalizable splatting.
...
Conclusion: Geometric priors are essential for learning geometrically meaningful 3D Gaussian splats from view-synthesis supervision alone. G3Splat successfully addresses the ambiguities in self-supervised learning and enables pose-free generalizable splatting with strong geometric consistency.
...
TL;DR: A unified Attention U-Net model for automatic segmentation of FLAIR hyperintensity across various brain tumor types achieves comparable performance to dataset-specific models and generalizes well across tumor types and acquisition time points.


<details>
  <summary>Details</summary>
Motivation: FLAIR MRI is crucial for brain tumor diagnosis and monitoring, but manual segmentation of FLAIR hyperintensity volumes is time-consuming. An automatic segmentation tool would be clinically valuable for assessing tumor volume and surrounding edema across different tumor types and treatment stages.

Method: Trained a unified FLAIR hyperintensity segmentation model using Attention U-Net architecture on ~5000 FLAIR images from various tumor types, acquisition time points, and centers. Compared performance against dataset-specific models and validated on different tumor types, time points, and against BraTS benchmark.

Result: The unified model achieved Dice scores: 88.65% for pre-op meningiomas, 80.08% for pre-op metastasis, 90.92% for pre-op and 84.60% for post-op gliomas (BraTS), and 84.47% for pre-op and 61.27% for post-op lower grade gliomas. It performed comparably to dataset-specific models while enabling generalization across tumor types and time points.

Conclusion: The unified Attention U-Net model provides robust FLAIR hyperintensity segmentation across diverse brain tumor types and clinical scenarios, achieving clinical-grade performance comparable to specialized models while offering better generalization. Integration into open-source Raidionics software facilitates clinical deployment.
...
TL;DR: RoomEditor++ is a diffusion-based architecture for virtual furniture synthesis that integrates reference objects into indoor scenes while maintaining geometric coherence and visual realism, outperforming SOTA methods.
...
Method: Proposes RoomEditor++ with a parameter-sharing dual diffusion backbone compatible with both U-Net and DiT architectures, unifying feature extraction and inpainting for reference and background images to enforce aligned feature representations.

Result: Extensive experiments show RoomEditor++ outperforms state-of-the-art approaches in quantitative metrics, qualitative assessments, and human preference studies, with strong generalization to unseen indoor scenes without task-specific fine-tuning.

Conclusion: The paper introduces RoomBench++ dataset and RoomEditor++ architecture, providing a comprehensive solution for virtual furniture synthesis that achieves superior performance and generalization capabilities.
...
TL;DR: This paper proposes a novel diffusion-based reconstruction algorithm for video snapshot compressive imaging using one-hot modulation, addressing temporal aliasing issues through a generative video inpainting approach with dual optical path hardware enhancement.


<details>
  <summary>Details</summary>
Motivation: Current video SCI using random binary modulation causes temporal aliasing during compression. One-hot modulation offers perfect temporal decoupling but lacks algorithms to exploit its potential. The authors aim to bridge this gap by developing a specialized reconstruction method.

Method: 1) Transform reconstruction into generative video inpainting using SDE aligned with hardware compression; 2) Combine one-step regression initialization with one-step diffusion refinement to address pure diffusion limitations; 3) Implement dual optical path hardware to mitigate spatial degradation from one-hot modulation.

Result: Experiments on synthetic datasets and real scenes demonstrate the effectiveness of the proposed method, showing successful reconstruction for video SCI with one-hot modulation.

Conclusion: This is the first work integrating diffusion models into video SCI reconstruction, successfully addressing temporal aliasing through one-hot modulation and providing a novel framework that combines hardware and algorithmic innovations.
...
TL;DR: Current medical imaging AI benchmarks have fairness issues: datasets lack real-world clinical diversity and have poor accessibility/reusability, creating disconnect between leaderboard success and clinical relevance.


<details>
  <summary>Details</summary>
Motivation: To assess whether medical imaging AI benchmarks provide sufficiently representative, accessible, and reusable data to support clinically meaningful AI, examining fairness in terms of real-world clinical diversity and FAIR principles compliance.

Method: Conducted large-scale systematic study of 241 biomedical image analysis challenges comprising 458 tasks across 19 imaging modalities, analyzing dataset composition biases and accessibility/legal reusability issues.

Result: Found substantial biases in dataset composition (geographic location, modality, problem type) indicating inadequate reflection of real-world clinical diversity. Challenge datasets frequently had restrictive access conditions, inconsistent licensing, and incomplete documentation, limiting reproducibility and reuse.

Conclusion: Current benchmarking ecosystem has foundational fairness limitations, exposing disconnect between leaderboard success and clinical relevance due to unrepresentative datasets and poor accessibility/reusability.
...
TL;DR: HeadHunt-VAD is a tuning-free video anomaly detection method that identifies robust anomaly-sensitive attention heads within frozen multimodal LLMs, bypassing textual outputs to capture subtle anomalous cues more effectively.


<details>
  <summary>Details</summary>
Motivation: Traditional VAD methods require extensive labeled data and are computationally expensive. Existing MLLM-based tuning-free methods rely on textual outputs, which suffer from information loss, normalcy bias, and prompt sensitivity, making them insufficient for capturing subtle anomalies.

Method: Proposes HeadHunt-VAD that directly identifies robust anomaly-sensitive attention heads within frozen MLLMs using a Robust Head Identification module. This module evaluates all attention heads through multi-criteria analysis of saliency and stability to select a sparse subset of consistently discriminative heads across diverse prompts. Features from these expert heads feed into a lightweight anomaly scorer and temporal locator.
...
Conclusion: HeadHunt-VAD demonstrates that directly hunting robust anomaly-sensitive attention heads within frozen MLLMs, rather than relying on textual outputs, provides a more effective and efficient tuning-free approach for video anomaly detection with interpretable outputs.
...
TL;DR: MGRegBench is a public benchmark dataset for mammogram registration with over 5,000 image pairs and 100 manually annotated pairs, enabling standardized evaluation and comparison of diverse registration methods.


<details>
  <summary>Details</summary>
Motivation: Robust mammography registration is crucial for clinical applications like tracking disease progression, but progress has been limited by lack of public datasets and standardized benchmarks, with existing studies using private data and inconsistent evaluation frameworks.

Method: Created MGRegBench dataset with over 5,000 mammogram image pairs, including 100 with manual anatomical landmarks and segmentation masks. Benchmarked diverse registration methods including classical (ANTs), learning-based (VoxelMorph, TransMorph), implicit neural representation (IDIR), mammography-specific approaches, and state-of-the-art deep learning (MammoRegNet).

Result: MGRegBench is one of the largest public 2D registration datasets with manual annotations. The benchmark enables first like-for-like comparison of diverse methods on mammography modality and provides extensive analysis of deep learning-based registration.

Conclusion: The paper establishes a foundational resource for fair comparisons in mammography registration research by publicly releasing code and data, which should catalyze future research in this clinically important field.
...
TL;DR: A semi-supervised learning method for multi-output CNN segmentation models, applied to aortic dissection diagnosis using 3D CTA images with limited labeled data.


<details>
  <summary>Details</summary>
Motivation: Multi-output CNN models improve medical image segmentation accuracy but require extensive labeled data, which is expensive and time-consuming to obtain, especially for 3D medical images like aortic dissection CTAs.

Method: Proposes a semi-supervised learning approach for multi-output segmentation models using data augmentation through rotations and flipping, without assuming probabilistic model responses, making it universally applicable to separate segmentation architectures.

Result: The method addresses the challenge of limited labeled data for Type B aortic dissection segmentation (true lumen, false lumen, false lumen thrombus) on the ImageTBDA dataset containing 100 3D CTA images.

Conclusion: Semi-supervised learning with data augmentation enables effective training of multi-output segmentation models with limited labeled data, overcoming labeling challenges for medical image analysis tasks like aortic dissection diagnosis.
...
<details>
  <summary>Details</summary>
Motivation: Existing sparse query-based 3D detectors using 2D detection priors still suffer from depth ambiguity in single-frame 2D detections, limiting 3D query generation accuracy. There's a need to enhance depth perception while maintaining computational efficiency.

Method: Proposes StereoMV2D framework that integrates temporal stereo modeling into 2D detection-guided multi-view 3D detection. Uses cross-temporal disparities of objects across adjacent frames to enhance depth perception, computes within 2D RoIs for efficiency, and employs dynamic confidence gating to evaluate reliability of temporal stereo cues based on inter-frame matching matrix and appearance consistency.

Result: Extensive experiments on nuScenes and Argoverse 2 datasets show superior detection performance without significant computational overhead compared to existing methods.

Conclusion: StereoMV2D effectively addresses depth ambiguity in multi-view 3D object detection by leveraging temporal stereo cues, achieving better accuracy while maintaining computational efficiency through efficient 2D RoI processing and adaptive confidence gating.
...
TL;DR: PathFLIP is a novel framework for fine-grained vision-language pretraining in computational pathology that decomposes slide-level captions into region-level subcaptions for precise visual-language grounding, outperforming existing methods with less training data.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models struggle with gigapixel-scale whole slide images and spatial heterogeneity in computational pathology. Current alignment methods fail to capture fine-grained correspondences between text descriptions and visual cues across thousands of patches, compromising downstream task performance.

Method: PathFLIP decomposes slide-level captions into region-level subcaptions and generates text-conditioned region embeddings for precise visual-language grounding. It leverages Large Language Models to follow diverse clinical instructions and adapt to varied diagnostic contexts.

Result: PathFLIP outperforms existing large-scale pathological vision-language models on four representative benchmarks while requiring significantly less training data. It demonstrates versatile capabilities across slide-level classification/retrieval, fine-grained lesion localization, and instruction following.

Conclusion: PathFLIP paves the way for fine-grained, instruction-aware whole slide image interpretation in clinical practice by enabling precise visual-language grounding and adapting to diverse clinical instructions with less training data.
...
TL;DR: GRASP-HO reformulates HOI detection from closed-set classification to open-vocabulary generation using a frozen MLLM with lightweight cognitive steering, achieving SOTA closed-set performance and strong zero-shot generalization.
...
Method: Proposes GRASP-HO framework that extracts hybrid interaction representations and uses a lightweight cognitive steering conduit (CSC) module to inject visual evidence into a frozen MLLM. Introduces hybrid guidance strategy combining language modeling loss and auxiliary classification loss to address supervision mismatch.
...
Conclusion: GRASP-HO successfully reformulates HOI detection as an open-vocabulary generation problem, leveraging frozen MLLMs with lightweight steering to achieve both strong closed-set performance and generalization to unseen interactions.
...
TL;DR: ReCo introduces a new instructional video editing paradigm using in-context generation with joint denoising and regularization techniques to address inaccurate editing regions and token interference in video editing.
...
Method: ReCo width-wise concatenates source and target videos for joint denoising, using two regularization terms: latent regularization (increases editing region discrepancy, reduces non-editing discrepancy) and attention regularization (suppresses attention between editing region tokens and source video counterparts). Also creates ReCo-Data dataset with 500K instruction-video pairs.
...
Conclusion: ReCo successfully addresses video editing challenges through constraint modeling between editing/non-editing regions, achieving accurate instructional video editing with reduced interference.
...
TL;DR: Bitbox is an open-source toolkit that makes AI-based computational behavioral analysis from video accessible to behavioral scientists without requiring engineering expertise.


<details>
  <summary>Details</summary>
Motivation: Current AI-based behavioral measurement tools are developed for engineering audiences, require specialized software stacks, and don't provide measurements at a level useful for hypothesis-driven research, creating barriers for behavioral and clinical researchers.

Method: Bitbox provides a standardized interface for extracting high-level behavioral measurements from video using multiple face, head, and body processors. It's designed with principles of reproducibility, modularity, and interpretability, allowing new measures to be added easily.

Result: Bitbox has been tested and validated on clinical samples and serves both behavioral researchers (by providing accessible behavioral metrics) and computer scientists (by offering a dissemination mechanism for their methods).

Conclusion: Bitbox bridges the translational gap between AI methods and behavioral science, accelerating integration of computational behavioral measurement into research while being designed as a community-driven effort that will evolve through contributions from both method developers and domain scientists.
...
TL;DR: ST-Gaze combines CNN with attention modules to fuse eye/face features as spatial sequences, then propagates through time for video-based gaze estimation, achieving SOTA on EVE dataset.


<details>
  <summary>Details</summary>
Motivation: Video-based gaze estimation needs to capture both spatial (within-frame) and temporal (between-frame) relationships, but current methods are limited by how they handle these feature representations.

Method: Proposes ST-Gaze: CNN backbone with channel attention and self-attention modules to optimally fuse eye and face features. Fused features are treated as spatial sequences to capture intra-frame context, then propagated through time to model inter-frame dynamics.
...
Conclusion: The approach enables more robust video-based gaze estimation using commonly available cameras by effectively modeling both spatial and temporal relationships.
...
TL;DR: Conditioning step size is more critical than diffusion step count for diffusion-based super-resolution, with optimal step sizes between 2.0-3.0.


<details>
  <summary>Details</summary>
Motivation: Diffusion models show promise for inverse problems like super-resolution, but conditioning methods (DPS, MCG) introduce hyperparameters that need careful tuning. The paper aims to identify dominant factors affecting performance when applying conditioning to pretrained diffusion models.

Method: Conducted empirical ablation study on FFHQ super-resolution to analyze the impact of conditioning hyperparameters, specifically comparing conditioning step size versus diffusion step count.

Result: Conditioning step size has significantly greater impact than diffusion step count on reconstruction quality. Step sizes in the range [2.0, 3.0] yield the best overall performance in their experiments.

Conclusion: For diffusion-based super-resolution, careful tuning of conditioning step size is more important than diffusion step count, with optimal values around 2.0-3.0 providing best results.
...
<details>
  <summary>Details</summary>
Motivation: The paper aims to address the challenge of creating animatable 3D head avatars from limited input (single or sparse images) without requiring camera poses or expression labels, while achieving detailed dynamic deformations and handling rare expressions like wrinkles and bared teeth.
...
Conclusion: FlexAvatar presents an effective approach for creating high-fidelity 3D head avatars from limited input, offering flexibility, detailed dynamic deformations, and practical usability for avatar animation applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing datasets lack authentic ADAS vehicle behavior under risk conditions (crashes, near-misses, disengagements), limiting safety-critical research in driving behavior for ADAS-equipped vehicles.
...
Result: Three main contributions: 1) Novel framework for real-time TTC computation using semantic segmentation and depth estimation; 2) GEV distribution modeling of extreme risk across roadway types; 3) VLLM benchmarks showing significant performance improvements through domain adaptation.

Conclusion: SAVeD addresses the critical gap in ADAS safety research by providing authentic risk scenario data, enabling better analysis of perception/decision failures and improving model performance in complex near-miss scenarios.
...
TL;DR: MambaMIL+ is a new multiple instance learning framework for whole-slide image analysis that integrates spatial context modeling while maintaining long-range dependencies without memory decay, achieving SOTA performance across 20 benchmarks.


<details>
  <summary>Details</summary>
Motivation: WSIs present challenges due to gigapixel resolution and lack of fine-grained annotations. While MIL treats WSIs as bags of patches, effectively modeling ultra-long sequences with rich spatial context remains difficult. Mamba offers linear scaling for long sequences but suffers from limited spatial context modeling and memory decay in WSI analysis.

Method: MambaMIL+ introduces three key components: 1) overlapping scanning to restructure patch sequences for spatial continuity, 2) selective stripe position encoder (S2PE) to encode positional information while mitigating fixed scanning order biases, and 3) contextual token selection (CTS) mechanism that uses supervisory knowledge to dynamically enlarge contextual memory for stable long-range modeling.
...
Conclusion: MambaMIL+ effectively addresses the limitations of existing MIL approaches for WSI analysis by integrating spatial context while maintaining long-range dependency modeling without memory forgetting, proving to be an effective and robust framework for large-scale computational pathology.
...
TL;DR: This paper introduces Diff-Gen, a large-scale diffusion-generated fake dataset, and AdaptPrompt, a parameter-efficient CLIP-based framework for generalizable deepfake detection that achieves state-of-the-art performance across diverse synthetic content.


<details>
  <summary>Details</summary>
Motivation: The increasing realism of synthetic media makes reliable deepfake detection challenging, especially due to generalization issues where detectors trained on specific generators fail on unseen models. There's a pressing need for detection methods that can generalize across diverse generative techniques.

Method: The paper introduces two main contributions: 1) Diff-Gen - a large-scale benchmark dataset with 100k diffusion-generated fakes capturing broad spectral artifacts, and 2) AdaptPrompt - a parameter-efficient transfer learning framework that jointly learns task-specific textual prompts and visual adapters while keeping the CLIP backbone frozen. The method also involves pruning the final transformer block of the vision encoder to better retain high-frequency generative artifacts.

Result: The approach achieves state-of-the-art performance across 25 challenging test sets covering GANs, diffusion models, and commercial tools. It demonstrates strong cross-domain generalization, few-shot learning capability (using as few as 320 images), and enables source attribution for identifying generator architectures in closed-set settings.

Conclusion: The proposed framework effectively addresses the generalization challenge in deepfake detection by leveraging large vision-language models and parameter-efficient adaptation, establishing new benchmarks for detecting synthetic content across diverse generative techniques while maintaining computational efficiency.
...
TL;DR: Pix2NPHM: A vision transformer network that directly regresses Neural Parametric Head Model parameters from single images, achieving high-fidelity facial reconstruction with interactive frame rates.


<details>
  <summary>Details</summary>
Motivation: Neural Parametric Head Models (NPHMs) offer better geometric detail than traditional 3DMMs, but fitting them to visual inputs is challenging due to their expressive latent space. Current approaches struggle with accurate facial geometry and expression reconstruction from single images.

Method: Proposes Pix2NPHM, a vision transformer network that directly regresses NPHM parameters from single images. Uses domain-specific ViTs pretrained on geometric prediction tasks. Trained on mixed 3D data (100K+ NPHM registrations for SDF space supervision) and large-scale 2D video datasets (using normal estimates as pseudo ground truth). Includes optional inference-time optimization against estimated surface normals and canonical point maps.

Result: Achieves more recognizable facial geometry and accurate facial expressions than existing approaches. Enables 3D reconstructions at interactive frame rates with unprecedented face reconstruction quality. Can run at scale on in-the-wild data and allows geometric fidelity improvement through inference-time optimization.

Conclusion: Pix2NPHM successfully addresses the challenge of fitting expressive NPHMs to visual inputs, enabling high-fidelity facial reconstruction from single images with both speed and accuracy, suitable for real-world applications.
...
<details>
  <summary>Details</summary>
Motivation: Existing learning-based methods for geodesic distance computation rely on large 3D backbones, causing high memory usage and latency that limit use in interactive or resource-constrained settings.

Method: Constructs compact, category-aware shape descriptors by applying PCA to unsigned distance field (UDF) samples at informative voxels, eliminating the need for high-capacity networks.

Result: Reduces memory usage and inference time by up to 300× compared to existing neural approaches, works with sparse point clouds (as few as 300 points), and enables 1000× speedup for shape matching while maintaining comparable accuracy.

Conclusion: LiteGE provides an efficient, lightweight solution for geodesic distance computation that enables practical applications in interactive and resource-constrained settings while maintaining robustness and accuracy.
...
TL;DR: UrbanDIFF: A denoising diffusion model for spatial reconstruction of cloud-contaminated urban land surface temperature imagery using static urban structure conditioning and pixel-guided refinement.


<details>
  <summary>Details</summary>
Motivation: Satellite LST products are crucial for urban heat island monitoring but frequently suffer from cloud contamination. Existing methods rely on temporal or multi-sensor data that may be unavailable under persistent cloud cover, while spatial approaches degrade with large gaps.

Method: UrbanDIFF uses denoising diffusion for spatial LST reconstruction, conditioned on static urban structure data (built-up surfaces and digital elevation model). Includes supervised pixel-guided refinement during inference to enforce consistency with cloud-free pixels.

Result: Outperforms interpolation baseline across 20-85% cloud coverage, achieving SSIM 0.89, RMSE 1.2K, and R² 0.84 at 85% coverage. Shows slower performance degradation as cloud density increases compared to baseline.

Conclusion: UrbanDIFF demonstrates robust spatial reconstruction of cloud-contaminated urban LST using diffusion models with urban structure conditioning, offering improved performance under high missingness scenarios for continuous SUHI monitoring.
...
TL;DR: Hybrid neural network-enhanced distortion model improves long-range camera calibration for 3D localization up to 5km distances.


<details>
  <summary>Details</summary>
Motivation: Existing stereo-camera 3D localization methods are limited to few hundred meters due to inadequate distortion models for camera lens nonlinearities, especially for long-distance applications like 3D mapping and object localization.

Method: Hybrid approach: first extends conventional distortion models with higher-order terms, then enhances them using neural network-based residual correction to model complex lens distortion functions that conventional models alone cannot capture.

Result: Framework substantially improves long-range localization performance, enabling 3D position estimation of objects up to 5 kilometers. Estimated coordinates are transformed to GIS coordinates and visualized on GIS maps.

Conclusion: Proposed hybrid framework offers robust and effective solution for calibrating CCTV cameras for long-range photogrammetry applications, overcoming limitations of conventional distortion models through neural network enhancement.
...
TL;DR: AniX is a world model that combines static 3D scene generation with controllable characters, allowing users to direct characters through natural language to perform diverse actions while preserving visual fidelity with provided 3D scenes.
...
Method: AniX takes a 3DGS scene and character as input, allowing natural language direction for diverse behaviors. It formulates this as a conditional autoregressive video generation problem, built upon a pre-trained video generator with enhanced training strategy for better motion dynamics while maintaining generalization.

Result: The model synthesizes temporally coherent video clips that preserve visual fidelity with provided scenes and characters, supporting diverse behaviors from basic locomotion to object-centric interactions while freely exploring environments.

Conclusion: AniX successfully bridges the gap between static world generation and controllable-entity models, enabling realistic simulation with user-specified characters performing open-ended actions through natural language control while maintaining scene and character consistency.
...
<details>
  <summary>Details</summary>
Motivation: While 3D Gaussian Splatting (3DGS) has emerged as a high-fidelity scene representation, encoding rich, general-purpose features directly from its primitives remains under-explored. There's a gap in learning comprehensive scene representations from 3DGS data.
...
Result: Chorus shows strong performance on open-vocabulary semantic/instance segmentation, linear/decoder probing, and data-efficient supervision. A point cloud variant using only Gaussians' centers, colors, and normals outperforms point cloud baselines while using 39.9 times fewer training scenes. The method demonstrates strong transfer capabilities.

Conclusion: Chorus successfully addresses the gap in learning general-purpose features from 3DGS primitives through multi-teacher distillation, creating a versatile scene encoder that transfers well to various 3D tasks and outperforms existing approaches with significantly less training data.
...
TL;DR: ReX-MLE is a new benchmark for evaluating autonomous coding agents on complex medical imaging tasks, revealing that current state-of-the-art agents perform poorly (0th percentile) compared to human experts due to domain knowledge and engineering limitations.


<details>
  <summary>Details</summary>
Motivation: Current autonomous coding agents built on LLMs can solve general software and ML tasks but fail on complex, domain-specific scientific problems like medical imaging, which requires specialized knowledge, long training cycles, high-dimensional data handling, and complex preprocessing/validation pipelines not measured in existing benchmarks.

Method: Introduced ReX-MLE benchmark with 20 challenges derived from high-impact medical imaging competitions across diverse modalities and task types. Unlike prior benchmarks, it evaluates full end-to-end workflows requiring agents to independently manage data preprocessing, model training, and submission under realistic compute and time constraints.

Result: Evaluated state-of-the-art agents (AIDE, ML-Master, R&D-Agent) with different LLM backends (GPT-5, Gemini, Claude) and found severe performance gap: most submissions rank in the 0th percentile compared to human experts. Failures stem from domain-knowledge and engineering limitations.

Conclusion: ReX-MLE exposes critical bottlenecks in current autonomous AI systems for domain-specific scientific problems and provides a foundation for developing domain-aware autonomous AI systems capable of handling complex medical imaging workflows.
...
<details>
  <summary>Details</summary>
Motivation: Current T2I diffusion models often fail to capture spatial relations in text prompts due to lack of fine-grained spatial supervision in training data and inability of text embeddings to encode spatial semantics.
...
Result: InfSplign establishes new SOTA on VISOR and T2I-CompBench, achieving substantial gains over existing inference-time baselines and even outperforming fine-tuning-based methods.
...
TL;DR: A simulation-driven deep learning framework for denoising Raman spectra by modeling noise sources and training neural networks to suppress detector noise and fluorescence background in biomedical tissue analysis.
...
Method: Developed a simulation-driven denoising framework combining statistically grounded noise modeling with deep learning. Created biologically realistic Raman spectra by modeling major noise sources, then trained a cascaded deep neural network to jointly suppress stochastic detector noise and fluorescence baseline interference.

Result: The approach was validated using simulated human skin spectra derived from real experimental data. Results demonstrate improved spectral quality through physics-informed learning, enabling faster and more accurate Raman-based tissue analysis.

Conclusion: The study shows the potential of combining physics-based noise modeling with deep learning to overcome fluorescence interference in Raman spectroscopy, advancing its application for biomedical tissue diagnostics.
...
TL;DR: The paper proposes CBAM-VGG16, an interpretable attention-guided CNN for plant leaf disease detection that integrates CBAM modules to enhance feature extraction and localization, achieving up to 98.87% accuracy across five datasets.


<details>
  <summary>Details</summary>
Motivation: Plant diseases threaten global food security, requiring accurate and interpretable detection methods. Current approaches need better transparency and reliability for agricultural diagnostics.

Method: CBAM-VGG16 integrates Convolution Block Attention Module (CBAM) at each convolutional stage of VGG16 architecture. The model is trained on five diverse plant disease datasets and evaluated using attention maps, Grad-CAM, Grad-CAM++, and Layer-wise Relevance Propagation for interpretability.

Result: The approach outperforms recent techniques with high accuracy up to 98.87% and demonstrates robust generalization across different plant disease datasets.

Conclusion: The study advances explainable AI in agricultural diagnostics by providing a transparent and reliable system for smart farming, with code publicly available for reproducibility.
...
Result: Experiments on CIFAR-10, Celeb-A, and LSUN show InSPECT achieves average 39.23% reduction in FID and 45.80% improvement in IS against DDPM for 10K iterations. Demonstrates enhanced visual diversity, faster convergence rate, and smoother diffusion process.
...
TL;DR: VLMs are surprisingly fragile to visual prompting details like marker color/size, which can completely change leaderboard rankings and be exploited to make weaker models outperform stronger ones.


<details>
  <summary>Details</summary>
Motivation: Current VLM evaluation benchmarks using visual prompting are unstable - small changes in visual markers (color, size) significantly affect model performance and rankings, making evaluations unreliable.

Method: Evaluated 9 open/closed-source VLMs on 2 visually prompted tasks, testing sensitivity to visual marker design, dataset size, and low-level inference choices like JPEG compression. Created VPBench with 16 marker variants for more stable evaluation.

Result: Visual prompting details have much larger impact than conventional semantic evaluations. Simple changes (red→blue markers, larger markers) can completely change rankings and make smaller models (InternVL3-8B) outperform larger proprietary ones (Gemini 2.5 Pro). JPEG compression levels also affect rankings.

Conclusion: Current visually prompted benchmarks are unstable and easily manipulated. VPBench provides a more robust evaluation framework with multiple marker variants to mitigate these issues and enable fairer VLM comparisons.
...
TL;DR: KCCs transform any pre-trained ViT model into a self-explainable classifier without retraining by leveraging ViT's keypoint matching capabilities for interpretable decision-making.


<details>
  <summary>Details</summary>
Motivation: Current self-explainable models require complex training and specific architectures, making them impractical for ViT-based foundation models. There's a need for methods to provide transparency and reliability to these powerful models.

Method: Keypoint Counting Classifiers (KCCs) convert any well-trained ViT model into a self-explainable model without retraining. The method builds on ViT's ability to identify matching keypoints between images, creating an interpretable decision process that can be visualized directly in the input.

Result: Extensive evaluation shows KCCs improve human-machine communication compared to recent baselines, making ViT-based models more transparent and interpretable.

Conclusion: KCCs represent an important step toward making ViT-based foundation models more transparent and reliable by providing inherent visualizability without requiring model retraining or complex architectures.
...
TL;DR: RadarGen is a diffusion model that generates realistic automotive radar point clouds from multi-view camera images, using BEV representations and visual scene guidance for physically plausible radar patterns.


<details>
  <summary>Details</summary>
Motivation: The paper aims to address the need for scalable multimodal generative simulation by creating realistic radar data from visual inputs, which could reduce the gap between simulation and real-world perception models.

Method: Adapts image-latent diffusion to radar domain by representing radar measurements in bird's-eye-view form with RCS and Doppler attributes. Uses BEV-aligned depth, semantic, and motion cues from pretrained foundation models to guide generation. Includes lightweight recovery step to reconstruct point clouds from generated maps.

Result: Evaluations on large-scale driving data show that RadarGen captures characteristic radar measurement distributions and reduces the gap to perception models trained on real data.

Conclusion: RadarGen represents a step toward unified generative simulation across sensing modalities, offering a scalable approach compatible with existing visual datasets and simulation frameworks.
...
TL;DR: MAGNet is a unified autoregressive diffusion framework for multi-agent motion generation that handles various interaction tasks through flexible conditioning and supports multiple agents.
...
Method: MAGNet uses an autoregressive diffusion framework with Diffusion Forcing modifications that explicitly model inter-agent coupling during denoising. It supports dyadic prediction, partner inpainting, and full multi-agent generation in a single model, with architecture agnostic to agent count.

Result: The approach performs on par with specialized methods on dyadic benchmarks while naturally extending to polyadic scenarios with 3+ agents. It captures both tightly synchronized activities (dancing, boxing) and loosely structured social interactions.

Conclusion: MAGNet provides a unified framework for multi-agent motion generation that generalizes across various interaction tasks and group sizes, enabling coherent coordination across agents in both structured and unstructured interactions.
...
TL;DR: This paper investigates adversarial robustness of vision-language models LLaVA-1.5-13B and Llama 3.2 Vision-8B-2 against visual attacks using PGD on VQA v2 dataset, finding Llama 3.2 Vision shows smaller accuracy drop despite lower baseline performance.


<details>
  <summary>Details</summary>
Motivation: As deep learning models become more complex and opaque, adversaries can exploit vulnerabilities by adding unseen elements to images to confuse AI recognition systems. This paper aims to understand the adversarial robustness of contemporary vision-language models against visual attacks.

Method: The researchers conducted untargeted Projected Gradient Descent (PGD) attacks against the visual input modality of LLaVA-1.5-13B and Meta's Llama 3.2 Vision-8B-2. They empirically evaluated these attacks on a subset of the Visual Question Answering (VQA) v2 dataset and quantified results using standard VQA accuracy metrics, comparing accuracy degradation between the two models.

Result: Llama 3.2 Vision exhibited a smaller drop in performance under attack compared to LLaVA, particularly at higher perturbation levels, despite having lower baseline accuracy in this experimental setup. Both models showed vulnerability to visual adversarial attacks.

Conclusion: The vision modality represents a viable attack vector for degrading performance of contemporary open-weight vision-language models. Adversarial robustness does not necessarily correlate with standard benchmark performance and may be influenced by underlying architectural and training factors, highlighting the need for robustness evaluation separate from standard accuracy metrics.
...
<details>
  <summary>Details</summary>
Motivation: Current 3D digital twins are static and lack embodied interactivity, limiting them to navigation and view synthesis. The authors aim to bridge this gap by creating interactive digital twins that can simulate how human actions dynamically change 3D scenes.
...
Conclusion: DWM represents a first step toward video diffusion-based interactive digital twins, enabling embodied simulation from egocentric actions and bridging the gap between static 3D reconstruction and interactive digital environments.
...
TL;DR: Re-Depth Anything improves monocular depth estimation by using test-time self-supervision with diffusion models to refine Depth Anything V2 outputs without additional labels.


<details>
  <summary>Details</summary>
Motivation: Current foundation models like Depth Anything V2 struggle with real-world images outside their training distribution, creating a domain gap that needs bridging for better generalization.

Method: A test-time self-supervision framework that fuses DA-V2 with 2D diffusion priors. It performs label-free refinement by re-lighting predicted depth maps, using shape-from-shading cues with Score Distillation Sampling instead of photometric reconstruction. Freezes encoder, updates intermediate embeddings, and fine-tunes decoder only.

Result: Substantial gains in depth accuracy and realism across diverse benchmarks compared to DA-V2 baseline.

Conclusion: The approach demonstrates new avenues for self-supervision by augmenting geometric reasoning with generative models, effectively bridging domain gaps in monocular depth estimation.
...
TL;DR: The paper proposes a framework to adapt discriminative encoder features for generative tasks by introducing semantic-pixel reconstruction to regularize latent spaces, achieving compact representations with excellent reconstruction and generation performance.


<details>
  <summary>Details</summary>
Motivation: Current LDMs use VAE latents optimized for pixel reconstruction but lack semantic understanding. Using discriminative encoder features for generation faces two problems: (1) lack of compact regularization leading to off-manifold latents and inaccurate structures, and (2) weak pixel-level reconstruction hindering fine-grained geometry/texture learning.

Method: Proposes a systematic framework with semantic-pixel reconstruction objective to regularize latent space, compressing both semantic info and fine details into compact representation (96 channels, 16x16 downsampling). Uses this representation to build unified T2I and image editing model.

Result: Achieves SOTA image reconstruction, faster convergence, and substantial performance gains in both T2I and editing tasks compared to various feature spaces. Validates that representation encoders can be effectively adapted into robust generative components.

Conclusion: The proposed framework successfully bridges vision generation and understanding by adapting discriminative encoder features for generative tasks through semantic-pixel regularization, creating compact yet semantically rich latent spaces that enable high-quality generation and editing.
