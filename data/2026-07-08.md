...
TL;DR: CanvasCraft is a large-scale multimodal dataset (140K trajectories, 10K RL tasks) and CanvasAgent is a tool-augmented agent that learns to orchestrate multiple visual tools (synthesis, segmentation, editing, compositing, etc.) through SFT + GRPO training for complex image creation and editing workflows.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal agents are optimized for perception or search, not for manipulation-centered visual creation. Complex image editing requires orchestrating multiple tools (synthesis, segmentation, editing, compositing) through multi-turn interaction, but no large-scale supervised dataset exists for this.

Method: Constructs CanvasCraft (140K executable trajectories + 10K RL task specs). Trains CanvasAgent with SFT on reasoning-action trajectories, then optimizes via GRPO with hybrid reward (outcome + process signals). Agent inspects intermediate results and adapts tool decisions to evolving visual state.

Result: Experiments evaluate final image quality and trajectory behavior, demonstrating effectiveness for complex multi-tool image creation workflows.

Conclusion: CanvasAgent + CanvasCraft effectively bridges the gap between perception-focused multimodal agents and manipulation-centered visual creation, enabling complex multi-tool image creation.
...
TL;DR: This paper presents a task-driven evaluation framework linking synthetic fog generation, image restoration, object detection, and tracking for small UAVs in foggy conditions. It shows fog degrades detection/tracking via missed detections, fog-inclusive training improves robustness most, and restoration benefits are detector-dependent—restoration quality doesn't guarantee proportional perception gains.


<details>
  <summary>Details</summary>
Motivation: Fog severely degrades UAV visibility in sky-dominant, long-range imagery, reducing detection and tracking reliability. Collecting and annotating real foggy UAV scenes is practically difficult, motivating synthetic fog generation and a joint evaluation of restoration and downstream perception.

Method: A unified pipeline integrating depth-aware synthetic fog generation (using monocular depth estimation + atmospheric scattering model), image restoration (comparing classical, CNN-based, and transformer-based methods), object detection (with clean-only and fog-inclusive training), and tracking-by-detection on clean, foggy, and restored videos.

Result: Fog substantially degrades detection and tracking via increased missed detections. Fog-inclusive training gives the most consistent robustness improvement. Test-time restoration helps most when detectors are trained only on clean imagery. Restoration quality does not proportionally translate to perception gains.

Conclusion: Image restoration and downstream perception should be evaluated jointly; restoration metrics alone are insufficient to predict detection/tracking performance in foggy UAV scenarios.
...
TL;DR: This paper proposes a gaze estimation method that uses only one camera and one light source (instead of the standard two light sources) by creating a "virtual light source" symmetrical to the real one, estimating a "virtual glint" based on pupil-glint distance relationships, and using polynomial regression with a new normalization factor.


<details>
  <summary>Details</summary>
Motivation: To reduce hardware requirements for gaze tracking (especially for mobile devices) by minimizing the number of light sources needed while still enabling gaze estimation with free head movement.

Method: Introduces a "virtual light source" placed symmetrically to the real light source across the camera. A "virtual glint" is estimated using the geometric relationship between pupil distances and glint distances in the image. Gaze is then estimated via polynomial regression as if two actual light sources were available. A new normalization factor is proposed for the one-glint regression system.

Result: The method achieves acceptable performance with only one light source, though some degradation is observed compared to the standard two-light-source configuration.

Conclusion: The proposed one-camera, one-light-source approach is practical for scenarios where hardware minimization is critical (e.g., mobile devices), offering a trade-off between component reduction and minor accuracy loss.
...
TL;DR: Ground3D-LMM is a unified model that enables 3D spatial conversation with point-grounded responses and metric numerical outputs (e.g., size, thickness, distance) for object and part-level queries. It introduces a new 3D Grounded Measurement task and a large dataset with ~2.5M QA pairs built on ScanNet/ScanNet++, providing a strong baseline for grounded, metric-aware 3D understanding.


<details>
  <summary>Details</summary>
Motivation: Existing 3D LMMs either respond without explicit 3D grounding or lack support for metric-aware interactive dialogue. The paper aims to unify explicit 3D grounding (point-level) with metric numeric outputs (real-world units) for conversational 3D understanding.

Method: Ground3D-LMM takes a point cloud and optional RGB image as input, and supports 3D spatial conversation with point-grounded responses and metric outputs. A large-scale dataset (~2.5M QA pairs) is built on ScanNet/ScanNet++ with dense object and part annotations across eight tasks, plus a manually verified test set.
...
Conclusion: The proposed unified model and dataset establish a foundation for combining 3D grounding with metric measurements in conversational AI, and are publicly available to facilitate future research.
...
TL;DR: Light-Omni introduces a lightweight multimodal agent framework for continuous video understanding that uses dual contextual states (global and latent) to avoid costly iterative reasoning, achieving significant speedup and memory efficiency gains over prior approaches like M3-Agent.


<details>
  <summary>Details</summary>
Motivation: Existing video agents rely on heavy "detective-style" iterative reasoning for action control and evidence aggregation, which incurs prohibitive cost and latency. The authors argue this stems from a lack of global context and semantic misalignment in retrieval, driving the need for a more efficient approach.

Method: Light-Omni employs dual contextual states: (1) a finite-sized multimodal global state continuously consolidated from episodic memory via hierarchical merging, preserving recent details and summarizing past events; (2) a parametric latent state conditioned on the global context that drives autonomous actions and generates retrieval embeddings in a single forward pass, avoiding iterative reasoning.

Result: Light-Omni outperforms M3-Agent with 2.4% accuracy gain, 12.1× speedup, and 2.6× improvement in GPU memory efficiency across multiple video benchmarks. It also serves as an effective memory system to enhance existing MLLMs.

Conclusion: The coupled design of global and latent contextual states enables semantically aligned retrieval and reflexive responses while avoiding costly iterative reasoning, making Light-Omni both accurate and efficient for continuous video understanding.
...
TL;DR: The paper identifies naturally occurring statistical patterns in image datasets (like ImageNet) that act like backdoor triggers, called "statistical adversaries," which can predictably alter model predictions across different architectures without any malicious insertion.


<details>
  <summary>Details</summary>
Motivation: To investigate naturally occurring statistical signals in vision data that behave like backdoor triggers without being maliciously inserted, suggesting that ordinary datasets contain exploitable adversarial surfaces even without poisoning.

Method: Analyze ImageNet to find patterns strongly linked to labels, then use statistical controls to remove random correlations from candidate signals.

Result: These statistical adversaries are more targeted than generic corruptions and transfer across different model architectures, showing that vulnerabilities are driven by dataset structure and distribution rather than a single model's idiosyncrasies.

Conclusion: Ordinary datasets can contain exploitable adversarial surfaces even in the absence of poisoning; dataset audits should treat spurious structure as a latent attack surface for vision models, not just as bias or interpretability failure.
...
TL;DR: This paper introduces a rendering-aware Bayesian 3D Gaussian Splatting framework that uses a Normal-Inverse-Wishart posterior to track Gaussian geometry, enabling native uncertainty quantification for active view selection and interval calibration. The method achieves significant improvements in PSNR and coverage error over standard ensembles at lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Standard 3DGS relies on point estimates and hand-tuned heuristics, lacking uncertainty quantification and principled complexity control. This is especially limiting under sparse views or fixed acquisition budgets, where models must identify weakly supported geometry and select informative views.

Method: The framework tracks Gaussian geometry with a Normal-Inverse-Wishart posterior over means and covariances using renderer-derived surrogate summaries. An optional Dirichlet-process extension adds probabilistic component-usage signal. The training schedule clarifies the boundary between closed-form and approximate inference.

Result: In active view selection (16-to-32 views), NIW acquisition improves PSNR by +0.453 dB and LPIPS by -0.0146 over a standard ensemble baseline. NIW native intervals reduce 95% coverage error by ~17x vs. shared proxy (0.046 vs. 0.796) and ~10x vs. deep ensemble (0.047 vs. 0.454) at ~1/3 training cost. Overall +0.030 dB PSNR with 1.6% additional training time.

Conclusion: Bayesian 3DGS is a practical probabilistic scene representation for decision-facing tasks like active view selection, providing native uncertainty with minimal overhead.
...
TL;DR: MuCoDi distills knowledge from large computational pathology foundation models into compact edge-oriented encoders (MobileOne, RepViT) using contrastive distillation, achieving near-teacher performance with orders-of-magnitude fewer parameters and enabling up to 605x speedup on edge devices like Raspberry Pi 5.


<details>
  <summary>Details</summary>
Motivation: Computational pathology foundation models (PFMs) are too large and computationally expensive for local deployment in pathology departments, creating a need for compact encoders that retain high-quality representation performance.

Method: MuCoDi uses contrastive distillation (adapted from MoCo v3) to train lightweight MobileOne and RepViT student models by distilling frozen tile embeddings from multiple PFMs (Virchow2, UNI2, H-Optimus-1). Cached teacher embeddings replace momentum-encoder keys. Students are pretrained on 14.3M TCGA tiles from 11.8K WSIs.

Result: MuCoEdge-R2.3 and MuCoEdge-R1.5 achieve 71.0% external AUROC (within 0.8 pp of best teacher Virchow2 at 71.8%). MuCoEdge-R1.0 hits 70.9% AUROC with only 6.4M params and 1.12 GFLOPs. On Raspberry Pi 5, MobileOne students achieve 605x speedup over Virchow2 while retaining 66.5-66.9% AUROC.

Conclusion: PFM-quality pathology representations can be effectively compressed into compact edge-deployable models, enabling practical local deployment in pathology settings without sacrificing significant performance.
...
TL;DR: A training-free pipeline that harnesses pretrained vision-language and generative image models to segment 3D objects into semantic parts and fit superquadric primitives, achieving state-of-the-art Chamfer distance on HumanPrim and Toys4K without any learned parameters.


<details>
  <summary>Details</summary>
Motivation: To adapt pretrained generative image models for 3D shape decomposition into geometric primitives without any fine-tuning or task-specific training, overcoming limitations of previous learning-based methods that require category-specific training and are orientation-dependent.

Method: (1) Render multi-view images of a 3D object; (2) Use a vision-language model to analyze semantic parts; (3) Prompt a generative image model to produce color-coded part segmentation masks; (4) Reproject masks onto 3D geometry; (5) Fit superquadric primitives to each part via parameter optimization. Zero learned parameters.

Result: Achieves lowest Chamfer distance on HumanPrim and Toys4K benchmarks using 5-9 primitives per object. Ground-truth study reveals part segmentation (not primitive fitting) is the current accuracy bottleneck, suggesting performance will improve with future generative models.

Conclusion: Pretrained generative models can be directly harnessed (without training) for 3D part decomposition, offering category-agnostic and orientation-invariant properties that prior methods lack. The approach's ceiling is tied to generative model quality.
...
TL;DR: FaceMesh2HPO uses 3D facial meshes and a hierarchical PointNet pipeline to classify HPO-aligned facial phenotypes, achieving AUROCs of ~0.55–0.89, with better performance at parent nodes but limited accuracy on rare leaf terms.


<details>
  <summary>Details</summary>
Motivation: To enable interpretable, ontology-linked facial phenotype classification from 2D images to support clinical diagnosis of genetic disorders.

Method: Annotations from 124 clinicians across 10 disorders (107 HPO terms) + controls → 3D facial meshes (478 landmarks) from 2D images → hierarchical PointNet pipeline with cascading classification and feature elimination, incorporating 3D meshes, facial outline, and demographic metadata.

Result: Best models achieved AUROCs ~0.55–0.89; higher performance at parent nodes than leaf terms; external validation showed variable generalizability.
...
TL;DR: PKT is a knowledge distillation framework for efficient quality assessment of AI-generated images. It uses a teacher-student architecture where the teacher (local-global processing) trains a student (global-only) via multi-level knowledge transfer, achieving comparable performance with 67.7% less computation.


<details>
  <summary>Details</summary>
Motivation: Existing AIGI quality assessment methods suffer from either high computational cost (complex feature extraction) or poor accuracy (simple scaling-based methods), creating an unmet need for efficient yet accurate assessment.

Method: A dual-model knowledge distillation framework: a teacher model with local-global hybrid processing provides multi-level supervision signals to a student model that relies solely on global processing, enabling efficient knowledge transfer.

Result: On 4 AIGIQA databases, the student model maintains teacher-comparable performance while reducing computational costs by 67.7%, achieving a better balance between efficiency and accuracy than existing methods.

Conclusion: The PKT framework successfully addresses the efficiency-accuracy trade-off in AIGI quality assessment, offering a practical solution that achieves both high performance and real-time inference capability.
...
TL;DR: A multimodal framework combining frozen BiomedCLIP with dual-stream LoRA and cross-contextual fusion, plus a wound-specific OOD detection method with temporal drift penalties, for automated wound monitoring and SAE detection.


<details>
  <summary>Details</summary>
Motivation: Wound monitoring is a critical clinical challenge lacking effective automated solutions. Existing VLMs lack domain-specific grounding for wound imagery and heterogeneous clinical data, and have limited capability to detect out-of-distribution cases (i.e., severe adverse events).

Method: Uses paired clinical notes + wound descriptions encoded via dual-stream LoRA on frozen BiomedCLIP backbone. Introduces cross-contextual LoRA fusion for information exchange between modalities. Proposes wound-specific OOD detection combining semantic matching, visual typicality, caption-text alignment, and caption-visual alignment into a unified SAE score, with covariate consistency and temporal drift penalties.

Result: Promising performance on both wound healing assessment and SAE detection on a longitudinal wound dataset collected through clinical visits.

Conclusion: Semantically enriched, temporally aware vision-language systems show strong potential for clinical wound monitoring and early risk identification.
...
TL;DR: This paper proposes two hierarchical multi-label classification methods (loss-based and logit-based) for thoracic disease classification in chest X-rays. Evaluated on CheXpert, PADCHEST, and NIH datasets, both methods significantly outperform baselines in accuracy, AUC, and F1 scores by leveraging hierarchical relationships among pathologies.


<details>
  <summary>Details</summary>
Motivation: Thoracic disease classification in CXR images is challenged by multiple pathologies with overlapping visual characteristics. Existing flat classification methods ignore hierarchical relationships among diseases, limiting performance and interpretability.

Method: Two hierarchical multi-label classification techniques: (1) Loss-based method integrates hierarchical information directly into the optimization process; (2) Logit-based method adjusts predicted probabilities based on parent class in the disease taxonomy.

Result: Logit-based: +12% accuracy, +13% AUC, +24% F1; Loss-based: +11% accuracy, +10% AUC, +12% F1 over baseline. Validated on three large-scale datasets with statistical analysis.

Conclusion: Hierarchical multi-label classification significantly improves accuracy, robustness, and interpretability for CXR-based diagnosis, advancing computer-aided diagnosis systems.
...
TL;DR: A two-stage diffusion-based super-resolution framework enhances multi-spectral satellite cloud imagery by 4×, outperforming existing methods in capturing fine cloud microstructures critical for weather modification decisions.


<details>
  <summary>Details</summary>
Motivation: High-resolution satellite imagery is needed for observing fine-scale cloud structures important for cloud seeding, but current satellites lack sufficient spatial resolution, and existing super-resolution methods designed for natural images fail to generalize to satellite spectral cloud images.

Method: A two-stage diffusion-based super-resolution framework: Stage 1 uses real-world paired data for robust degradation handling and inter-sensor alignment; Stage 2 employs self-supervised internal downgrading of high-resolution data to refine structural learning and texture synthesis.

Result: Outperforms state-of-the-art transformer and diffusion baselines in reconstruction accuracy and visual quality, better capturing fine cloud microstructures like convective turrets and cloud gaps. Stage 1 excels at coarse structural fidelity, Stage 2 adds detail and realism.

Conclusion: The two-stage method provides a practical path toward improving cloud microphysics analysis and represents a step toward using AI for climate and sustainability applications.
...
TL;DR: VEIL systematically studies how chart encoding choices (like colors, axes, scales) affect what CNN models actually learn when classifying time series as chart images, finding models often rely on visual cues rather than true temporal patterns.


<details>
  <summary>Details</summary>
Motivation: It remains unclear whether CNN-based time-series classification models learn underlying temporal patterns or merely exploit visual artifacts introduced by chart design decisions.

Method: VEIL conducts complementary analyses of similarity, transferability, and attribution across different chart encodings, and tests attention-guided training as a potential mitigation strategy.

Result: Chart encoding choices significantly influence learned representations; attention-guided training helps only when encoding sensitivity is consistently identified across diagnostics, and can be harmful otherwise.

Conclusion: Chart-based TSC should be treated as a representation and measurement problem rather than a simple modeling decision, extending graphical perception research from human readers to vision models.
...
TL;DR: This paper introduces REVIVE, a framework for detecting and recovering camera images in autonomous vehicles that have been vandalized by physical occlusion attacks. It combines detection, segmentation, and inpainting methods (including Stable Diffusion and direct pixel replacement) to restore object-detection performance. Under aligned-reference conditions, it recovers recall from 0.588 to 0.967, and includes a quality gate to ensure recovered frames never underperform unrecovered ones.


<details>
  <summary>Details</summary>
Motivation: Autonomous vehicles face increasing threats from vandalism-induced occlusion attacks (VOAs) on cameras. While detection methods exist, restoring camera-stream utility after physical occlusion remains largely unexplored, creating a gap in ensuring reliable AV perception under attack.

Method: The REVIVE framework has four stages: (1) binary VOA detection, (2) multi-class VOA pattern classification, (3) EfficientNet-based U-Net segmentation, and (4) type-aware recovery using BLIP-guided Stable Diffusion inpainting, direct pixel replacement, or adaptive median filtering, plus a quality gate to filter poor recoveries.

Result: Stable Diffusion achieves SSIM 0.667-0.867 and PSNR 15.4-26.7dB across patterns. Direct pixel replacement restores YOLOv8l recall to 0.967 and F1 to 0.970 under aligned-reference conditions. The quality gate improves recall from 0.304 to 0.608, ensuring the stream is never worse than unrecovered frames (baseline recall 0.588).

Conclusion: REVIVE provides a structured, practical pipeline for recovering from vandalism-induced occlusions in AV camera streams, with a quality-gate mechanism that guarantees safe downstream performance.
...
TL;DR: CGVQ is a Gaussian primitive-based image compression method that uses cluster-guided vector quantization to partition Gaussian parameters into homogeneous groups before quantization, achieving 20% bitrate reduction while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: Existing Gaussian-based image representations suffer from poor rate-distortion efficiency at higher fidelity targets because storing many floating-point parameters per primitive consumes too much bitrate.

Method: The proposed Cluster-Guided Vector Quantization (CGVQ) partitions Gaussian parameters into homogeneous groups before quantization, enabling higher compression efficiency and accurate parameter reconstruction.

Result: CGVQ decreases the bpp by 20% compared to the baseline while maintaining on-par visual quality.

Conclusion: CGVQ effectively improves rate-distortion performance in Gaussian-based image compression by clustering parameters prior to quantization.
...
TL;DR: FASR++ is a diffusion-model-based super-resolution algorithm that leverages multiple low-quality auxiliary images from surveillance footage to enhance face recognition by generating high-quality, identity-preserving super-resolved images without explicit guidance.


<details>
  <summary>Details</summary>
Motivation: Surveillance face images suffer from low resolution, pose variation, poor illumination, and occlusions, making face recognition difficult. Existing super-resolution methods often distort identity. Multiple low-quality frames from surveillance cameras provide additional information that can be exploited.

Method: FASR++ uses a diffusion-model-based super-resolution algorithm that takes a reference low-resolution image along with features extracted from multiple auxiliary low-quality images (e.g., consecutive video frames) to generate a super-resolved output without requiring explicit soft attributes or gradient-based guidance.

Result: Achieves state-of-the-art results on two standard face recognition datasets for verification, face recognition, and image quality metrics (PSNR, SSIM, LPIPS).

Conclusion: FASR++ effectively recovers facial features and produces high-quality images that serve as an effective pre-processing step to significantly improve face recognition performance under challenging surveillance conditions.
...
TL;DR: SaGe introduces Scene Graph Thinking, a paradigm that enhances MLLMs with structured scene-graph representations for fine-grained visual reasoning, using an automated data engine, 120K training samples, and two-stage graph-aligned post-training to improve performance across eight multimodal benchmarks.
...
Method: (1) Automated data engine converting flat image-text data into structured scene graphs (hierarchical entities as nodes, visual relations as edges); (2) Construction of 120K training data via reasoning traces from scene graphs; (3) Two-stage graph-aligned post-training: supervised fine-tuning + reinforcement fine-tuning with node-as-proxy graph rewards.
...
Conclusion: Scene Graph Thinking (SaGe) effectively enhances MLLMs with structured visual reasoning, outperforming existing models on fine-grained perception and reasoning benchmarks.
...
TL;DR: The paper proposes ART, a diagnostic test that reveals whether shortcut associations remain functionally usable in classifiers after unlearning, demonstrating that existing evaluation methods miss this aspect.


<details>
  <summary>Details</summary>
Motivation: Existing evaluations for association unlearning only measure output-level robustness or probe shortcut readability in frozen features, but neither tests whether a retained association remains functionally usable by the original classifier.

Method: ART estimates class-conditional association directions, amplifies residual shortcut components, then evaluates modified features using the original classifier head to detect functional restorability.

Result: On Waterbirds, CelebA, SpuCoDogs, and ISIC timestamp-artifact datasets, output metrics, representation probes, and ART capture distinct aspects of shortcut mitigation.

Conclusion: The findings motivate restoration-aware evaluation for unlearning methods that target learned associations rather than individual classes or concepts.
...
TL;DR: SAMPLe is a plug-in sharpness-aware optimizer that resolves the performance-generalization dilemma in VLM prompt learning by dynamically balancing exploration and exploitation based on loss landscape curvature, consistently improving multiple prompt learning frameworks.


<details>
  <summary>Details</summary>
Motivation: Prompt learning in VLMs suffers from a performance-generalization dilemma: tuned prompts achieve high accuracy on seen distributions but overfit, losing generalizability to unseen data due to the limited set of learnable prompts.

Method: SAMPLe (Sharpness-Aware Minimization Prompt Learning) is a plug-in optimizer that considers loss landscape sharpness, dynamically adapting to the optimization state by satisfying objective function constraints at each step based on local curvature and gradient properties.

Result: Integrated into CoOp, CoCoOp, MaPLe, TCP, and Co-Prompt frameworks, SAMPLe consistently outperforms existing optimizers and reduces overfitting on seen distributions while improving adaptability to unseen data.

Conclusion: SAMPLe is a robust, model-agnostic solution that effectively addresses the performance-generalization trade-off in prompt learning for VLMs.
...
TL;DR: ARMS is a framework for streaming text-to-motion generation that handles solo-to-interaction transitions using a causal diffusion model with dynamics-asymmetric representation, enabling temporally continuous and socially coherent human motion generation.
...
Method: ARMS proposes (1) a dynamics-asymmetric representation that decouples per-person temporal evolution from inter-person alignment via partner-referenced relative-translation, (2) a causal latent space with a causal relational diffusion model that refines motion segment-by-segment, and (3) mode-aware relational gating to activate/mask cross-agent connections for solo/interaction generation.

Result: ARMS improves transition smoothness and social coherence compared to interaction-centric baselines, while achieving competitive results on human-human interaction benchmarks.

Conclusion: The proposed framework effectively unifies solo motion and human-human interaction within a single causal generative process, enabling seamless social coupling transitions in streaming scenarios.
...
TL;DR: This paper proposes an optimized ALF framework for VVC that reduces picture buffer access from 152 to 1 and achieves ~25% time savings with negligible coding performance loss, through parallel GALF/CCALF design, adaptive parameter decision, and one-pass CCALF scheme.


<details>
  <summary>Details</summary>
Motivation: The high coding complexity and excessive picture buffer accesses of ALF in VVC cause increased external memory access and are unfriendly to software/hardware design.

Method: Proposed an optimized ALF framework with: (1) parallel design of GALF and CCALF, (2) adaptive parameter decision for GALF, (3) one-pass CCALF scheme that estimates filtering distortion without actual filter operation.

Result: Reduced picture buffer access from 152 to 1, achieved ~25% time savings in the ALF module with negligible coding performance change under RA configuration.

Conclusion: The proposed optimizations significantly reduce complexity and memory access while maintaining coding efficiency, with some methods already adopted in VVC reference software.
...
TL;DR: Image2Sim is a neural simulation framework that converts posed RGB-D image sequences into interactive 3D environments, generating ~20K scenes and 10M+ navigation training samples, achieving strong real-world transfer results.


<details>
  <summary>Details</summary>
Motivation: Current embodied navigation is limited by the lack of scalable, high-fidelity, physically grounded interactive environments. Real-world scanned datasets are realistic but limited in scale, while synthetic simulators scale easily but suffer from sim-to-real gaps.

Method: Decouples 3D spatial anchoring from photorealistic rendering. Uses a feed-forward feature Gaussian model for single-pass 3D scene construction from posed RGB-D, and a Geometry-Aware One-Step Pixel Flow model to generate high-quality panoramic RGB-D observations from sparse/noisy Gaussian projections.

Result: Creates nearly 20K interactive scenes from videos/images, synthesizes 10M+ navigation training samples. Models trained entirely in these neural environments show strong improvements on major benchmarks and transfer effectively to real-world zero-shot settings.

Conclusion: Scalable neural simulation can serve as a practical training substrate for embodied navigation at scale, bridging the gap between simulated training and real-world deployment.
...
TL;DR: Legato 2 is a novel OMR pipeline that extracts symbolic notation and semantic knowledge from sheet music using system-by-system processing and a vision-LM, outperforming prior state-of-the-art across multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Existing OMR models treat pages as undifferentiated images, limiting scalability to long inputs, and lack the ability to embed textual content (titles, annotations) alongside symbolic transcriptions.

Method: Legato 2 combines system-level segmentation (following horizontal notation lines sequentially) with an autoregressive vision-language model to capture local details and overall score structure.

Result: Consistently outperforms prior state-of-the-art across multiple datasets; symbolic transcriptions improve frontier language models' interpretation of dense musical documents.

Conclusion: Legato 2 establishes new SOTA in both OMR and downstream sheet music understanding, demonstrating the value of sequential system-by-system processing and embedding textual content.
...
TL;DR: This paper introduces LanEvil++, the first benchmark for evaluating autonomous driving robustness under environmental illusions (shadows, reflections, tire marks). It shows these illusions degrade lane perception in both conventional lane detection models and vision-language models, with shadows being most harmful. They propose MIDA (Multimodal Illusion Defense Approach) to improve robustness.


<details>
  <summary>Details</summary>
Motivation: Environmental illusions (shadows, reflections, tire marks) are naturally occurring but overlooked phenomena in real driving that can mislead autonomous driving systems. Existing research largely ignores these phenomena, leaving a critical safety gap.

Method: 1) Created LanEvil++ benchmark with 14 illusion types, 94 3D scenes via CARLA simulator, yielding 90,292 images, 1,596 video clips, and 41,855 VQA pairs. 2) Evaluated conventional lane detection (LD) models and AD vision-language models (ADVLMs). 3) Proposed Multimodal Illusion Defense Approach (MIDA).

Result: LD models: 5.27% accuracy drop, 10.49% F1-score decline. ADVLMs: 2.03% GPT-score drop, 0.75% Language-score drop. Shadows caused up to 7.20% accuracy reduction. Closed-loop simulations showed incorrect driving decisions. MIDA improved robustness by 4.23% (LD) and 3.82% (ADVLMs).

Conclusion: Environmental illusions pose significant safety risks to autonomous driving. Shadows are the most disruptive. The proposed MIDA defense approach effectively enhances robustness. The LanEvil++ benchmark provides a foundation for future research on this critical but overlooked problem.
...
TL;DR: DeSeG is a hierarchical framework that decouples semantic intent from geometric constraints for physically plausible human-scene interaction synthesis, achieving 47% less penetration and 29% better semantic alignment.


<details>
  <summary>Details</summary>
Motivation: Existing generative models for human-scene interaction suffer from "semantic-geometric entanglement" — a shortcut bias where spatial constraints override semantic intent due to strong correlations in training data, leading to physical hallucinations like body-scene penetrations.

Method: DeSeG proposes a two-component hierarchical framework: (1) a Residual Semantic Planner that encodes text and goal voxels into a latent space for semantic control independent of spatial trajectories, and (2) a physics-regularized diffusion executor that incorporates differentiable repulsive potential fields into the diffusion objective for collision-aware motion generation.
...
Conclusion: Explicitly decoupling semantic intent from geometric constraints via a hierarchical framework effectively resolves the entanglement issue and produces more physically plausible human-scene interactions.
...
TL;DR: SegAnswer shifts zoom-in operations from bounding boxes to pixel-level segmentation masks, improving MLLM reasoning by filtering out background noise and aligning better with visual token structures.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs use bounding boxes for region zoom-in during visual reasoning, but bounding boxes include redundant background and interfering objects, reducing precision.

Method: SegAnswer replaces bounding box-based zoom-in with pixel-level segmentation masks to isolate target regions more precisely, aligning with how MLLMs structure visual tokens via positional embeddings.

Result: Consistent improvements across high-resolution perception, general perception, and hallucination benchmarks, with strong performance on segmentation tasks.

Conclusion: Segmentation before answering is a more effective zoom-in mechanism than bounding boxes for MLLMs, offering both better perception and reliable pixel grounding capabilities.
...
TL;DR: TRIG is a geometry perception framework that decouples ego-trajectory from camera-rig geometry for metric-consistent learning in vision-centric autonomous driving, achieving SOTA results across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing visual geometry models encode camera poses as entangled representations, mixing time-varying ego-motion with static camera-rig geometry, which limits the use of vehicle-side geometric priors in rigid multi-camera driving systems.

Method: TRIG factorizes camera poses into ego-trajectory and camera-rig components, uses decoupled pose encoding/supervision, and introduces sparse Temporal-Spatial attention to separate cross-camera interaction from temporal aggregation.

Result: Achieves state-of-the-art performance on five autonomous driving benchmarks for pose estimation, metric depth prediction, and 3D reconstruction.

Conclusion: Decoupling trajectory and rig geometry, combined with efficient sparse attention, yields superior metric geometry learning for multi-camera autonomous driving systems.
...
TL;DR: This study evaluated automated ROP Plus disease detection in 121 Kenyan preterm infants using 11 AI configurations, finding that combining classification and vessel segmentation yields the best performance (sensitivity 0.692, specificity 0.914), with classifiers enabling sensitive case-finding and segmentation reducing over-referral.


<details>
  <summary>Details</summary>
Motivation: ROP is a preventable cause of childhood blindness with rising burden in low- and middle-income countries, but ROP-trained ophthalmologists are scarce. Plus disease diagnosis is subjective and variable, so automated screening could extend specialist reach, yet African evidence remains limited.

Method: Analyzed 121 Kenyan preterm infants (237 eyes, 1,635 fundus images graded as No Plus, Pre-Plus or Plus). Used vessel annotations from two graders for segmentation training. Evaluated 11 configurations for eye-level Plus detection using patient-grouped nested cross-validation, including image classifiers, multiple-instance learning, multi-task segmentation-classification, and segment-then-classify pipelines.

Result: Vessel segmentation achieved pooled Dice 0.533, IoU 0.368, sensitivity 0.623, specificity 0.979. RGB classifiers were sensitive but over-referred; segmentation-coupled models were more specific. The OR-based screen achieved highest sensitivity, AND-based confirmation highest specificity, and probability ensemble gave best balanced performance (sensitivity 0.692, specificity 0.914, balanced accuracy 0.803), outperforming vision classifier alone.
...
TL;DR: The paper proposes a pipeline for synthesizing realistic defocus deblurring datasets using efficient wave-optics PSF computation and camera ISP simulation, creating a large-scale synthetic dataset (CLDefocus) that improves cross-device generalization over existing real and synthetic datasets.


<details>
  <summary>Details</summary>
Motivation: Defocus blur degrades image structures and limits visual perception. Existing deep learning deblurring methods suffer from poor generalization across cameras and lenses due to limited optical diversity and realism in current datasets.

Method: A unified pipeline integrating: (1) efficient wave-optics PSF computation via Debye CZT propagation, (2) depth-aware defocus rendering with occlusion handling, (3) blur synthesis in radiometrically linear space with camera ISP simulation. This generates CLDefocus, a large-scale synthetic defocus dataset with diverse lens characteristics.

Result: Models trained on CLDefocus achieve improved cross-device generalization compared to models trained on existing real and synthetic datasets.

Conclusion: The proposed pipeline enables scalable generation of photorealistic defocus datasets with diverse lens characteristics, and the resulting CLDefocus dataset helps models generalize better across different cameras and lenses.
...
TL;DR: The paper proposes a two-stage framework combining generative intervention (zero-shot segmentation + structure-preserving diffusion for context-shifted variants) with Cross-Variant Self-Supervised Learning (contrastive objective where same-object/different-background variants are positive pairs) followed by ERM warm-up and GroupDRO fine-tuning, achieving state-of-the-art worst-group accuracy on Waterbirds (92.5%), MetaShift (81.7%), and NICO++ (87.4%).


<details>
  <summary>Details</summary>
Motivation: Deep neural networks trained with ERM fail under distribution shifts because they exploit spurious correlations between object labels and background context. Existing generative approaches create counterfactual images but treat them as standard data augmentation, leaving models free to retain background-sensitive representations.

Method: Two-stage framework: (1) generative intervention using zero-shot segmentation to isolate foreground objects + structure-preserving diffusion model to generate context-shifted variants; (2) Cross-Variant Self-Supervised Learning where different-background variants of the same object form positive pairs in contrastive learning, followed by ERM warm-up + GroupDRO fine-tuning with layer-wise learning rates.

Result: Best worst-group performance: 92.5% on Waterbirds, 81.7% on MetaShift, 87.4% on NICO++.

Conclusion: The proposed framework effectively learns background-invariant representations by explicitly aligning object-centric features across context-shifted variants, outperforming prior methods on distribution-shift benchmarks.
...
TL;DR: AVA-VLM introduces a coarse-to-fine reasoning strategy for construction-site monitoring, where the model first analyzes low-resolution global images and selectively requests high-resolution crops only when needed, improving reliability and efficiency.


<details>
  <summary>Details</summary>
Motivation: Current VLMs for construction monitoring use direct QA-style fine-tuning from single global images, which limits operational range, reliability under reduced resolution, and inference efficiency for real-world deployment.

Method: Proposes AVA-VLM with a human-inspired coarse-to-fine strategy: reason over low-resolution global images first, then selectively request high-resolution local crops when detailed inspection is needed. Also introduces a region-aware Chain-of-Thought dataset to teach when to inspect, where to crop, and how to use local evidence.
...
Conclusion: The selective attention mechanism inspired by human inspectors effectively addresses the limitations of direct global-image fine-tuning for in-the-wild construction monitoring.
...
TL;DR: Harrison.Rad 1.5 is a radiology-specific multimodal LLM trained via a three-stage pipeline that meets simulated FRCR passing standards, outperforming other systems on clinical reporting accuracy across multiple modalities and anatomical regions, with built-in explainability features.


<details>
  <summary>Details</summary>
Motivation: Imaging demand is growing faster than the radiology workforce can expand, and reporting backlogs cannot be resolved through training and recruitment alone. The goal is to reduce the time and effort radiologists spend producing reports.

Method: Three-stage training pipeline: (1) domain adaptation of base language model on radiology reports; (2) contrastive vision-encoder training with curriculum-based hard negatives on ~6 million image-report pairs; (3) visual-question-answering fine-tuning on multi-turn conversations. Evaluation uses Findings-Diagnosis scoring framework with RadGraph-XL, ontology-based synonym matching, and polarity-contradiction detection.

Result: HR1.5 is the only system evaluated to meet the simulated FRCR passing standard, achieves highest accuracy on closed-format clinical questions across anatomical regions, internal multi-body-part and mammography reporting, and primary clinically-aligned score for public chest reporting.

Conclusion: HR1.5 demonstrates clinically viable radiology reporting capability with explainability features (Grad-CAM, attention analysis, confidence estimation) supporting responsible future clinical evaluation.
...
TL;DR: This paper proposes few-medoids, a simple coreset selection strategy that picks samples closest to each class's centroid, which consistently outperforms random selection and other baselines in few-shot knowledge distillation across multiple datasets and model architectures.


<details>
  <summary>Details</summary>
Motivation: Coreset selection remains challenging in few-shot KD setups, where typical selection strategies often fail to beat the random selection baseline.

Method: Few-medoids selects the samples closest to the centroid (average image) of each class. The method is described as "embarrassingly simple" and serves as a drop-in replacement for existing baselines like herding or k-center Greedy.

Result: Extensive KD experiments on four datasets with three teacher-student model pairs (covering both CNNs and transformers) show that few-medoids consistently surpasses random selection and other coreset selection strategies.

Conclusion: Few-medoids can be used as a simple yet effective drop-in replacement for commonly-used coreset selection baselines in future research.
...
TL;DR: GaussFusion is a multimodal pre-training framework for 3D Gaussian representations that integrates image and text supervision into masked Gaussian modeling, achieving superior transferability on downstream tasks over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing pre-training methods for Gaussian representations (e.g., masked Gaussian reconstruction) primarily capture local structures but offer limited semantic supervision, lacking multimodal understanding.

Method: GaussFusion integrates image and text supervision into masked Gaussian modeling via cross-modal semantic alignment. It also proposes Gaussian Salience-guided Multi-scale Hole Masking (GSHM) to handle non-uniform Gaussian primitive distribution by creating spatially continuous multi-scale masked regions based on Gaussian salience.

Result: Outperforms Gaussian-MAE on ModelNet40 (+0.61%) and ScanObjectNN PB-T50-RS (+3.85%), demonstrating improved transferability on downstream tasks.

Conclusion: GaussFusion effectively learns both visual and language-level semantic information during pre-training, improving the transferability of Gaussian representations for 3D understanding.
...
TL;DR: Proposes PolicyShiftBench (2,000 instances over 265 images for testing policy-adaptive guardrailing) and PolicyShiftGuard (7B model using RP-SFT + BP-Adapt two-stage training) that achieves SOTA 76.9 Avg. F1 and 72.1 Avg. PSS, outperforming existing VLMs and guardrails under policy shifts.


<details>
  <summary>Details</summary>
Motivation: Existing image guardrails treat safety as an intrinsic image property, but real deployments require policy-adaptive behavior—where the same image may be allowed or restricted depending on the active policy. Models need to generalize to held-out policy definitions.

Method: Two-stage training: (1) Randomized Policy SFT (RP-SFT) for policy-conditioned supervised fine-tuning, (2) Boundary-Pair Policy Adaptation (BP-Adapt) with a pairwise comparison loss that separates blocking policies from passing policies using matched prompt pairs for the same image and risk category.

Result: PolicyShiftGuard (7B) achieves 76.9 Avg. F1 and 72.1 Avg. PSS on PolicyShiftBench, transfers well to UnSafeBench and SafeEditBench, and improves latency-performance trade-off with concise output format.

Conclusion: Policy-adaptive guardrailing is necessary for real deployments; existing models are brittle under policy shifts; matched pass/block boundary pairs (BP-Adapt) are essential for stable policy adaptation.
...
TL;DR: PRPC introduces a progressive reasoning framework with primitive correction for Compositional Zero-Shot Learning, using bidirectional dependency modeling and reinforcement learning to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Prior CZSL methods suffer from either independent attribute-object prediction (missing contextual dependency) or unidirectional conditional modeling (causing error propagation). The authors aim to model bidirectional dependency to improve robustness.

Method: PRPC formulates CZSL as a structured Chain-of-Thought reasoning process with Q&A style, constrains MLLM to follow predefined semantic steps, and uses GRPO-based reinforcement learning post-training with step-level rewards for logical consistency.
...
Conclusion: Bidirectional primitive correction and step-wise reinforcement learning are effective for robust compositional generalization in CZSL.
...
TL;DR: NegROI is a transformer-based interactive 3D segmentation framework that uses click-centric multi-resolution ROI refinement and scene-conditioned negative prompts to improve fine boundary handling and reduce false positives, achieving superior click efficiency and cross-dataset robustness on ScanNet, S3DIS, and KITTI.


<details>
  <summary>Details</summary>
Motivation: Existing interactive 3D segmentation methods suffer from (i) coarse voxel resolution blurring fine boundaries under limited clicks, and (ii) hard false positives from confusing background structures, which are exacerbated by density/scale shifts across datasets (e.g., dense RGB-D vs. sparse LiDAR) where fixed heuristics and purely click-driven decoding generalize poorly.

Method: NegROI proposes: (1) click-centric multi-resolution ROI refinement that refines only a local ROI around the click on a finer grid and fuses back to the coarse mask; (2) uncertainty-driven selective refinement prioritizing ambiguous regions; (3) scene-conditioned negative prompts via cross-attention over scene tokens to model hard background patterns, stabilized by a diversity regularizer; (4) boundary-aware hard negative mining to supervise negative-prompt attention toward boundary-proximal high-confidence false positives.
...
Conclusion: NegROI effectively addresses coarse resolution and false positive issues in interactive 3D segmentation via multi-resolution ROI refinement and scene-conditioned negative prompts, with demonstrated cross-dataset generalization.
...
TL;DR: This paper introduces a decoupled framework that detects single-mask annotation noise in vascular CT datasets by leveraging cross-sectional patch self-consistency along vessel centrelines, providing interpretable evidence of labelling errors without requiring costly multirater fusion or coupling with network training.


<details>
  <summary>Details</summary>
Motivation: Single mask annotation noise is pervasive in vascular CT datasets but under-addressed. Existing solutions either require costly multirater fusion or are coupled with network training, preventing explicit auditing of where and why labels fail.

Method: The method samples cross-sectional patches orthogonal to vessel centrelines, retrieves intensity-equivalent neighbours via scalable vector search, and computes patch-level noise scores from statistical mask disagreement, producing explicit image-mask evidence and scan-level quality maps.

Result: Experiments on coronary CT data show the detected noise improves training robustness. Systematic biases were revealed: transverse and oblique vessels exhibit 5.1 times higher error rates than axis-aligned structures, with correlations to cross-sectional area and intensity.

Conclusion: The decoupled framework enables interpretable, auditable annotation noise detection for vascular CT, supporting dataset quality assessment and quality-weighted training without requiring multirater annotations or integrated model training.
...
TL;DR: MTLA is a training-free, post-hoc scoring method that measures attention strength between prediction tokens and their claimed regions, significantly reducing hallucinations in multimodal large language models across object detection, video, and audio localization tasks.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs produce localized predictions (bounding boxes, temporal windows) but hallucinate regions frequently. Token log-probabilities are uninformative for grounding quality. Existing attention-based detectors are weak because they sum attention over the entire input and read only a single token.

Method: MTLA sums attention weights only within the claimed region (not the entire input) and aggregates across all prediction tokens, recovering a stronger grounding signal. It is training-free and applies to object detection (images), temporal localization (video), and audio.

Result: MTLA improves hallucination AUROC by +7 to +38 over best prior training-free baselines across multiple MLLM families and three modalities. As a confidence re-ranker, it nearly doubles zero-shot COCO detection AP (20.4 → 37.0) for an 8B open-source model.

Conclusion: MTLA provides an effective, training-free method to detect and reduce grounding hallucinations, narrowing the gap to supervised detectors without task-specific training.
...
TL;DR: SpecTrack is a spectral-spatial complexity-aware tracker for multispectral/hyperspectral image tracking that uses a Spectral Adaptive Mixture-of-Experts (SAMoE) module to adaptively allocate capacity to search regions based on tracking difficulty, achieving state-of-the-art accuracy-efficiency trade-offs on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing MSI/HSI trackers process all search regions with fixed-capacity spectral-spatial paths, ignoring that tracking difficulty varies across frames. Clear regions need only lightweight processing, while ambiguous ones require stronger reasoning, motivating adaptive capacity allocation.

Method: SpecTrack features a Spectral Adaptive Mixture-of-Experts (SAMoE) module with capacity-ordered experts of increasing rank/receptive field/depth. Expert selection is guided by a Spectral Prompt Router using semantic context, boundary cues, and channel-variation cues. A Shared Global Expert provides common context to mitigate fragmented routing.

Result: SpecTrack-L384 achieves SOTA AUCs of 65.2%, 51.9%, 72.6% on MUST, MSITrack, HOTC20. SpecTrack-B224 achieves 62.4% AUC at 43.7 FPS on MUST. On GOT-10k, it achieves 79.3% AO, showing RGB-domain generalization.

Conclusion: SpecTrack demonstrates that adaptive capacity allocation based on tracking difficulty improves both accuracy and efficiency in MSI/HSI tracking, and the architecture generalizes well to RGB-domain tracking.
...
TL;DR: SparseCtrl-HOI introduces a sparse temporal control framework for Human-Object Interaction video generation that requires only a few keyframes (instead of dense frame-wise guidance), using TiRoPE for temporal anchoring and MLLM-based Motion Prior Injection for plausible intermediate frame dynamics, reducing annotation costs while improving synthesis quality for live-streaming e-commerce.


<details>
  <summary>Details</summary>
Motivation: Existing HOI video generation methods rely on dense temporal guidance (e.g., frame-wise hand-object poses), which incurs high annotation costs and limits motion diversity. The authors aim to reduce this overhead while maintaining or improving generation quality.

Method: (1) Time-Controlled Rotary Positional Embedding (TiRoPE) to temporally anchor sparse keyframes while preserving spatial integrity; (2) Motion Prior Injection Module using Multimodal Large Language Models (MLLMs) to extract high-level motion priors for hallucinating plausible intermediate transitions; (3) SparseHOI-5K dataset built for this task.
...
Conclusion: SparseCtrl-HOI effectively addresses the annotation cost and diversity limitations of dense-guidance HOI generation methods, achieving high-quality results with only sparse keyframe input.
...
TL;DR: LPID is a method that makes unlearnable examples robust against the common face extraction step (crop+resize) by integrating a differentiable model of this extraction into the perturbation optimization. It confines perturbations to the face region and focuses energy in frequency bands that survive extraction, achieving superior protection (attacker accuracy <10%) even for unseen identities.


<details>
  <summary>Details</summary>
Motivation: Existing unlearnable example methods fail because the perturbation is crafted on the full shared image, but the attacker trains on the extracted (cropped+resized) face region. This mismatch causes protection to collapse under extraction.

Method: LPID builds the extraction operation into the unlearnable-example objective as a differentiable module. It confines the perturbation to the face region and optimizes through this differentiable extraction model, concentrating perturbation energy in the frequency band that the extraction preserves.

Result: LPID achieves the lowest attacker accuracy among all methods in every setting evaluated, holding attacker below 10% under crop+resize extraction on unseen identities. It remains imperceptible at 32.7 dB PSNR and 0.161 LPIPS.

Conclusion: By making robustness a property of the extraction transform rather than any identity, LPID is re-optimized per album and protects even never-before-seen users, offering a practical solution for privacy protection against unauthorized face recognition.
...
TL;DR: OBBSeg proposes Oriented Bounding Box (OBB) supervision for medical image segmentation, achieving performance comparable to full supervision while requiring only OBB annotations, validated across 13 datasets and 5 imaging modalities.


<details>
  <summary>Details</summary>
Motivation: Pixel-level annotation is a major bottleneck in medical image segmentation. Weak supervision methods are under-constrained, and existing box-based supervision (axis-aligned) struggles with elongated or anisotropic lesions.

Method: OBBSeg uses Oriented Bounding Boxes (OBBs) as intermediate supervision, introduces a Mask-to-OBB loss for geometric consistency, and employs two prompt-driven modules (PAFE and DBFE) for semantic guidance.

Result: Outperforms existing weakly supervised methods and achieves performance comparable to fully supervised approaches on 13 datasets across 5 imaging modalities.

Conclusion: OBB-based supervision is an efficient and scalable alternative for medical image segmentation that bridges the gap between full and weak supervision.
...
TL;DR: A pipeline using DeepSeek R1 extracts structured data from 3,965 heterogeneous real-estate questionnaire documents (text, scanned, special-char), successfully processing 2,781 and yielding 2,766 unique property records with a Jaccard consistency of 0.82.


<details>
  <summary>Details</summary>
Motivation: Real-estate listing APIs expose structured metadata but miss rich property information locked inside heterogeneous questionnaire documents (legal status, structural condition, utilities) that no automated system currently processes at scale.

Method: (1) Collect 3,965 questionnaire documents via reverse-engineered REST APIs; (2) classify documents into three categories (text_only, scanned, special_char); (3) extract 35 attributes from eligible documents using DeepSeek R1 prompted for structured JSON output.

Result: 2,781 documents processed successfully → 2,766 unique property records; cosine-similarity Jaccard score = 0.82; K-Means silhouette score = 0.2088.

Conclusion: LLM-driven extraction from heterogeneous property questionnaires is both feasible and reliable at scale, enabling downstream market analysis.
...
TL;DR: KOAL is a framework for predicting prostate cancer Gleason Grade Group from mpMRI by integrating clinical data, LLM-extracted expert knowledge, and hierarchical ordinal constraints, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Current GGG prediction methods overlook non-image information (age, PSA, expert priors) and oversimplify GGG as flat categorical labels, ignoring the hierarchical nature of primary/secondary Gleason patterns.

Method: Three synergistic modules: (1) Clinical-Context Modulation (CCM) uses clinical variables to modulate image features; (2) Knowledge-Guided Prototype Alignment (KGPA) uses LLM to extract expert knowledge from reports for contrastive learning; (3) Hierarchical Ordinal-aware Constraints (HOC) decouples primary/secondary Gleason prediction with a differentiable bio-logic mapping layer.

Result: Outperforms state-of-the-art methods on public PI-CAI and in-house datasets.

Conclusion: Integrating clinical context, LLM-derived expert knowledge, and hierarchical ordinal modeling significantly improves non-invasive GGG prediction.
...
TL;DR: This paper investigates whether the success of deep learning-based Visual SLAM is driven by learned 2D data association, learned uncertainty, or the recurrent architecture, finding that learned data association and uncertainty are the key factors, not the recurrent architecture.


<details>
  <summary>Details</summary>
Motivation: To understand which specific components in deep learning-based V-SLAM systems are fundamentally responsible for their superior performance over classical geometry-based methods.

Method: Controlled empirical study comparing the contributions of learned 2D data association, learned uncertainty, and recurrent architecture components in DL-based V-SLAM systems.

Result: The success of DL-based V-SLAM systems hinges on learned 2D data association and uncertainty, rather than the recurrent architecture.

Conclusion: Learning-based paradigms are essential for designing 2D data association and uncertainty components in V-SLAM systems.
...
TL;DR: MSA-DCNN is a unified deformable attention framework that jointly handles multi-scale morphology and limited annotations via adaptive sampling, saliency refinement, cross-scale fusion, and self-distillation, outperforming ViT/CNN/MICCAI baselines on medical classification benchmarks with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods fail to jointly address multi-scale morphology issues (fixed sampling, static fusion) and limited annotation challenges (data-hungry training, label scarcity). Prior works tackle these problems in isolation without a unified framework.

Method: MSA-DCNN integrates adaptive multi-scale sampling, within-scale saliency refinement, learned cross-scale fusion, and auxiliary self-distillation into a single optimization scheme for scale-consistent deformable attention learning.

Result: Outperforms ViT, CNN, and MICCAI semi-supervised baselines on three public benchmarks plus an external leukemia dataset under distribution shift and label scarcity, measured by accuracy, F1, and AUC, while using fewer parameters.

Conclusion: MSA-DCNN is a practical, data-efficient foundation for medical image classification, with complementary component contributions confirmed by ablation studies.
...
TL;DR: PVCap introduces PseudoCap (data augmentation via instance-level random mixing with pseudo-labeling) and VoxelCapNet (voxel-based caption network) to improve 3D dense captioning, achieving SOTA with +11.41% and +13.99% CIDEr@0.5IoU gains on ScanRefer and Nr3D.


<details>
  <summary>Details</summary>
Motivation: Existing 3D dense captioning methods suffer from: (1) global rigid transformations (e.g., rotation) fail to generate diverse spatial layouts crucial for learning spatial relation descriptions; (2) previous work over-focuses on caption generation but uses simple backbones/detection heads, limiting semantic richness.

Method: PVCap has two components: (1) PseudoCap - mixes instances from the dataset to create pseudo frames with diverse spatial layouts, uses a teacher-student framework to generate pseudo caption labels for data augmentation; (2) VoxelCapNet - a voxel-based caption network that adapts the caption head to voxel features, providing a strong baseline for 3D dense captioning.

Result: Surpasses SOTA by 11.41% on ScanRefer and 13.99% on Nr3D in CIDEr@0.5IoU metric.

Conclusion: PVCap effectively addresses spatial layout diversity limitations and backbone design issues in 3D dense captioning, establishing a new SOTA and a competitive baseline for future work.
...
TL;DR: EcoVision uses a modular AI pipeline (transformer segmentation + ConvNeXt classification + grid-based scoring) on UAV imagery to identify two halophytic grasses (*Spartina maritima* and *Puccinellia maritima*), achieving strong segmentation (mIoU=0.56, accuracy=0.96), excellent species classification (F1=0.99), and dominance estimates within 8% of field surveys for scalable salt marsh monitoring.


<details>
  <summary>Details</summary>
Motivation: To develop a scalable, high-resolution monitoring system for salt marsh vegetation using UAV imagery and AI, enabling automated identification and dominance estimation of two ecologically significant halophytic grasses.

Method: A modular pipeline combining transformer-based semantic segmentation, connected-component vegetation extraction, ConvNeXt-based fine-grained species classification, and 2x2m grid-based dominance scoring, trained on manually annotated UAV and publicly available biodiversity imagery.

Result: Segmentation: mean IoU=0.56, pixel accuracy=0.96; Object-level classification: F1=0.99; Dominance estimates: mean absolute difference <8% compared to quadrat-based field surveys, preserving fine-scale spatial structure.

Conclusion: EcoVision provides a practical, AI-driven foundation for scalable high-resolution salt marsh monitoring, successfully translating pixel-level predictions into ecologically interpretable dominance metrics.
...
TL;DR: RoME uses mixture of low-rank experts with dual-scale gating and threat-guided diversification to improve multi-perturbation adversarial training, outperforming state-of-the-art methods in union robustness and natural accuracy.


<details>
  <summary>Details</summary>
Motivation: Multi-perturbation adversarial training (MAT) suffers from robustness trade-offs between different ℓ_p threats, and naive MoE approaches face two issues: experts redundantly capture shared features while ignoring threat-specific ones, and gating networks learn threat-agnostic routing patterns.

Method: RoME employs (1) low-rank additive updates to a shared backbone as experts to capture threat-common features while focusing on threat-specific information, (2) dual-scale gating exploiting threat-discriminative signals from local and global features, and (3) threat-guided gating diversification for diverse expert utilization across threats.

Result: Outperforms existing state-of-the-art MAT methods in union robustness and natural accuracy, and improves robustness against unseen threats.

Conclusion: RoME effectively addresses the robustness trade-off in multi-perturbation settings by enabling threat-specific pathways via low-rank experts and improved gating mechanisms.
...
TL;DR: WebRetriever is a large-scale benchmark with 800 websites and 1,550 tasks across diverse domains, paired with NavEval (a novel LLM-as-Judge framework using richer interaction context) and three complementary evaluation protocols, revealing that navigation success alone poorly predicts real-world web agent performance.


<details>
  <summary>Details</summary>
Motivation: Existing web agent benchmarks suffer from limited scale, narrow domain diversity, inadequate evaluation methods (LLM-as-Judge fails on fine-grained interaction semantics), and overemphasis on navigation success metrics that neglect real-world deployment needs.

Method: (1) WebRetriever: 800 websites, 1,550 tasks across consumer/professional/enterprise domains. (2) NavEval: LLM-as-Judge framework using rich interaction context beyond visual screenshots. (3) Three evaluation protocols: navigation proficiency, knowledge-assisted interaction, end-to-end task completion with information extraction.

Result: NavEval achieves state-of-the-art alignment with human judgment. Substantial performance disparities across the three evaluation protocols demonstrate navigation success alone is insufficient for predicting real-world effectiveness.

Conclusion: WebRetriever provides fine-grained diagnostic insights into web agent capabilities and establishes a rigorous foundation for advancing web agent research and development, highlighting the need for holistic evaluation beyond navigation metrics.
...
TL;DR: Proposes AEGIS, an inference-time defense against Visual Synonym Attacks (VSA) on text-to-image diffusion models that dynamically traces unsafe semantic emergence through sparse attention heads, achieving superior safety (ASR 0.00-0.09) while preserving utility.


<details>
  <summary>Details</summary>
Motivation: Existing defenses against illicit content generation in diffusion models fail against Visual Synonym Attacks (VSA), where benign prompts produce prohibited imagery via implicit associations. Current approaches either miss VSA threats or over-suppress benign concepts, creating a safety-utility dilemma.

Method: Mechanistic analysis reveals VSA and explicit unsafe prompts converge through sparse semantic-injecting attention heads. AEGIS performs inference-time identification of these vulnerable heads and applies similarity-aware repulsion only at those specific heads, avoiding blanket suppression.

Result: On SD 1.4, AEGIS achieves ASR of 0.00/0.03 for in-domain violence/nudity VSA, ≤0.09 on out-of-domain attacks. It preserves benign fidelity without suppressing hard-negative concepts, and transfers to SD 2.1 and FLUX.1 after re-identification of critical heads.

Conclusion: Dynamic tracing of unsafe semantics at inference time through identified attention heads offers a more effective alternative to static concept suppression, solving the safety-utility dilemma for visual synonym attacks in diffusion models.
...
TL;DR: UltraDiffEdit is a tuning-free framework that extends latent diffusion models to ultra-high resolution image editing (up to 8K) using multi-scale progressive editing, multi-patch encoding, global-local consistency denoising, and patch-based hybrid sampling, achieving superior results on a single RTX 3090 GPU.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based image editing methods are limited to resolutions below 1K due to memory constraints and high cost of high-resolution training data, while modern mobile devices capture photos up to 8K, creating a significant gap between current capabilities and real-world demands. Simply upscaling low-resolution edits yields blurry results lacking fine details.

Method: UltraDiffEdit employs: (1) a multi-scale progressive editing strategy that iteratively blends high-resolution edited content with unedited areas in coarse-to-fine manner; (2) multi-patch encoding to preserve both edited and unedited visual details in latent space; (3) global-local consistency denoising to smoothly integrate edited and unedited latent features; (4) patch-based hybrid sampling capturing local, intermediate, and global features for semantic coherence.

Result: UltraDiffEdit handles image resolutions up to 8K using only a single NVIDIA GeForce RTX 3090 GPU, demonstrating superior editing quality and flexibility over existing methods.

Conclusion: The paper introduces a tuning-free framework that successfully bridges the gap between low-resolution diffusion editing capabilities and real-world ultra-high resolution demands, with publicly available source code.
...
TL;DR: RFHNet is a cascaded hierarchical hashing network for fine-grained food image retrieval, using three components (FRM, MFMF, HSS) to capture subtle local and frequency-sensitive features, achieving 4.44%-17.20% mAP improvements over state-of-the-art methods on six benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing hashing-based retrieval methods perform poorly in fine-grained food scenarios because they fail to capture subtle local semantics and frequency-sensitive visual cues essential for distinguishing similar food items.

Method: RFHNet consists of three components: (1) Fine-grained Relation Modeling (FRM) for capturing subtle visual differences; (2) Multi-Frequency Modulated Fusion (MFMF) for extracting informative multi-frequency features; and (3) Hierarchical Semantic Synergy (HSS) for integrating multi-level representations into discriminative hash codes.

Result: On six food-specific benchmarks, RFHNet consistently outperforms state-of-the-art hashing methods, achieving mAP gains ranging from 4.44% to 17.20% at 12 bits.

Conclusion: RFHNet is effective for large-scale visual food retrieval and smart catering applications, validating that cascaded hierarchical hashing with multi-level representations significantly improves fine-grained food image retrieval.
...
TL;DR: A reflection-robust seam segmentation framework using BiSeNetV2 with transfer learning and hybrid Cross-Entropy-Lovász loss that improves segmentation performance under reflective welding conditions without increasing computational complexity.
...
Method: Enhances a BiSeNetV2 backbone through transfer learning and a hybrid Cross-Entropy-Lovász loss, focusing on learning-stability-oriented optimization rather than increasing architectural complexity.

Result: Achieves 81.76% Joint IoU and 90.73% mIoU, improving Joint IoU by +22.36 percentage points over baseline with identical FLOPs and parameters; recovers 96.33% of severe zero-IoU failures under reflective conditions.

Conclusion: The framework provides a practical and lightweight perception solution for robotic welding applications involving reflective metallic surfaces, particularly effective for lightweight real-time segmentation architectures.
...
TL;DR: A two-stage diffusion framework using a global blueprint for layout-controllable, high-resolution image outpainting with parallel patch synthesis, achieving better quality, layout control, and faster inference.


<details>
  <summary>Details</summary>
Motivation: Existing high-resolution outpainting suffers from (i) no global planning causing error accumulation, (ii) limited spatial control beyond text, and (iii) high latency from sequential patch generation.

Method: Stage 1 generates a low-resolution global blueprint via a layout adapter injecting bounding-box conditions into Stable Diffusion. Stage 2 synthesizes high-resolution local patches in parallel using blueprint-derived guidance and low-frequency preservation initialization, eliminating sequential dependency.

Result: On large-scale artwork datasets, the method achieves improved visual fidelity, stronger semantic consistency, substantially reduced inference time, and explicit layout control compared to prior baselines.

Conclusion: The proposed global blueprint-guided two-stage diffusion framework effectively addresses structural instability, spatial controllability, and latency issues in high-resolution outpainting, enabling efficient parallel synthesis with layout control.
...
TL;DR: MobileWan is the first 5B-parameter video diffusion model deployable on mobile devices, using recurrence distillation and attention head pruning to achieve real-time video generation (5s, 480x832@16FPS, 20s latency) with state-of-the-art mobile VBench score of 83.79.


<details>
  <summary>Details</summary>
Motivation: Existing mobile video diffusion models are limited to 0.4-1.8B parameters, restricting generation quality. The authors challenge the assumption that mobile video generation requires small models, aiming to deploy server-scale 5B models on memory-constrained hardware.

Method: 1) Recurrence distillation framework converting video generation into chunk-wise autoregressive process with constant-memory attention; 2) Causal linear attention enabling RNN-style inference; 3) Learnable attention head pruning via binary per-head gates with noise-biased sparsity objective; 4) Sampling-step distillation and memory-optimized VAE decoding.

Result: Generates 5-second 480x832 videos at 16 FPS in 20 seconds end-to-end latency on a commercial mobile device, achieving VBench score of 83.79, establishing new SOTA in mobile video generation.

Conclusion: High-quality mobile video generation does not require small models; server-scale 5B-parameter models can be efficiently deployed on mobile hardware through careful architectural reformulation (recurrence + structured compression), challenging the prevailing small-model paradigm for on-device video generation.
...
TL;DR: This paper identifies complementary predictive behaviors between detector-based and query-based scene graph generation (SGG) methods, and proposes Dual-SGG, a dual-query approach that consolidates both reasoning mechanisms for improved performance.


<details>
  <summary>Details</summary>
Motivation: The discrepancy in predictive behaviors between detector-based and query-based SGG methods has not been systematically analyzed, despite their distinct underlying reasoning mechanisms.

Method: The authors design a controlled experimental setup to examine prediction discrepancies via detector-conditioned reachability, then propose Dual-SGG with a dual-query design that consolidates both detector-based and query-based reasoning mechanisms.

Result: Extensive experiments on Visual Genome, Open Images v6, and GQA-200 datasets demonstrate the effectiveness of the proposed Dual-SGG method.

Conclusion: Detector-based and query-based SGG methods exhibit complementary predictive behaviors, and leveraging both through a dual-query design improves scene graph generation performance.
...
TL;DR: SCPT enhances fine-grained image recognition by modeling inter-class semantic relationships through Semantic Relation Encoding and Semantic Condensation loss, achieving state-of-the-art performance on 14 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Fine-grained recognition is challenging due to manual annotation costs. VLMs like CLIP struggle with subtle distinctions. Existing prompt tuning methods treat class labels as isolated entities, ignoring rich semantic relationships and hierarchical dependencies between categories.

Method: Proposes Structured-Condensed Prompt Tuning (SCPT) with two components: (1) Semantic Relation Encoding (SRE) to explicitly model inter-class semantic topology and structured label relationships, and (2) Semantic Condensation loss (ScLoss) to suppress redundant supervision and extract discriminative components from the global semantic space.

Result: Extensive experiments on 14 fine-grained benchmarks show SCPT effectively mitigates semantic ambiguity and achieves state-of-the-art performance in both few-shot and base-to-novel generalization settings.

Conclusion: Modeling structured semantic relationships between class labels is crucial for fine-grained recognition, and SCPT provides an effective solution through explicit relation encoding and condensed supervision.
...
TL;DR: MoWorld is a cost-efficient, real-time interactive World Model achieving up to 50 FPS on NPU devices with cinematic visual quality, using a 3D-native data engine, cross-frame pre-training, denoising-step distillation, and mixed-precision parallel inference, reducing inference cost to 30-50% of existing models.


<details>
  <summary>Details</summary>
Motivation: To scale World Models for real-world deployment, both capability and inference efficiency must be optimized. Existing approaches rely on large video corpora and high-end GPUs, limiting practicality. This work aims to enable high-frame-rate, low-cost, real-time interaction on accessible hardware.

Method: 1) Scalable 3D-native data engine for geometrically consistent training data across real and synthetic environments. 2) Curriculum cross-frame pre-training for stable and scalable learning. 3) Denoising-step distillation to reduce diffusion training cost. 4) Mixed-precision parallel inference framework for low-cost real-time deployment on NPU.

Result: Achieves up to 50 FPS real-time interaction with cinematic visual quality on NPU devices without high-end GPUs. Inference cost is only 30%-50% of existing World Models. First real-time interactive World Model built on NPU.

Conclusion: MoWorld provides a practical foundation for large-scale real-world applications of World Models by jointly optimizing capability and cost, demonstrating leading performance and efficient deployability across diverse scenarios.
...
TL;DR: EeveeDark is a low-light video enhancement framework using Binary Neural Networks (BNN) on RAW frames and event streams to achieve efficient, high-quality restoration for resource-constrained settings.


<details>
  <summary>Details</summary>
Motivation: Enhancing videos under extreme low-light conditions is difficult due to the trade-off between restoration quality and computational efficiency in resource-constrained environments.

Method: EeveeDark combines sensor-level RAW data with event streams using a Binary Neural Network (BNN) architecture, featuring modality-specific binary encoders, a lightweight fusion block, and an event-guided skip gating mechanism for spatiotemporal refinement.
...
Conclusion: EeveeDark effectively balances restoration quality and computational efficiency for low-light video enhancement using binary neural networks with event-guided processing.
...
TL;DR: WING introduces a window-prior-based generative network that reformulates CT intensity regression into multiple windowed representations, using a Gated Inception Generator, Fuse-and-Refine Transformer, and joint adversarial training to achieve state-of-the-art MRI-to-CT and CBCT-to-CT synthesis.


<details>
  <summary>Details</summary>
Motivation: Direct CT intensity regression suffers from high dynamic range and long-tailed distributions, which average out sparse but clinically important structures. The authors aim to overcome this limitation by leveraging the inductive prior that CT intensities are structure-deterministic and window-separable.

Method: 1) Reformulate regression target into multiple windowed representations; 2) Gated Inception Generator for multi-window predictions with multi-shape kernel interactions; 3) Fuse-and-Refine Transformer to aggregate windowed outputs and learn residuals; 4) Joint adversarial training for window-conditioned realism.

Result: Achieves state-of-the-art performance on both MRI-to-CT and CBCT-to-CT benchmarks, and supports multi-anatomy synthesis with a single model.

Conclusion: The window-prior reformulation effectively addresses the challenge of high dynamic range in CT synthesis, and the compact WING framework outperforms existing methods while offering multi-anatomy capability.
...
TL;DR: This paper rethinks MRI super-resolution as a physics-aware reconstruction problem rather than a deterministic mapping, using 2D Gaussian Splatting with prior-aware representation, physics-constrained signal modeling, and meta-learning to handle dynamic-resolution inputs and achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing MRI super-resolution methods treat it as a deterministic mapping from fixed low-resolution to high-resolution, ignoring the inherent coupling between spatial resolution and SNR in MRI acquisition, where any low-resolution scan is just one of many possible realizations under different acquisition trade-offs.

Method: The authors adapt 2D Gaussian Splatting to MRI as a coordinate-based, resolution-agnostic rendering problem, with three innovations: (1) prior-aware Gaussian representation combining Anatomical Structure Prior and Imaging System Prior; (2) physics-constrained signal modeling predicting intrinsic tissue parameters (proton density ρ and effective relaxation rate R₂) to synthesize biophysically plausible intensities; (3) meta-learning framework pretraining on simulated data and adapting to real-world conditions to alleviate paired-data scarcity.
...
Conclusion: MRI super-resolution should be formulated as a physics-aware reconstruction problem with dynamic resolution, and the proposed method effectively addresses resolution-heterogeneous inputs while maintaining biophysical plausibility, achieving superior results for clinical applications.
...
TL;DR: VendorBench-100 is a cross-paradigm benchmark evaluating 36 deepfake detection models (commercial APIs, vision LLMs, open-source detectors) on a curated 100-image corpus, revealing that commercial APIs lead in performance but a key finding is the divergence between ROC-AUC and MCC metrics.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detection paradigms (commercial APIs, vision LLMs, open-source detectors) are rarely evaluated under a common protocol, making direct comparison difficult and hindering reliable assessment.

Method: Introduces VendorBench-100, which evaluates 36 models using a single adversarial 100-image corpus, unified output schema, common evaluation framework, and ranks models by MCC with ROC-AUC as a secondary measure. The corpus includes eight edge-case families (face swaps, text-to-video stills, AI photo edits, etc.).

Result: Commercial APIs achieve strongest median performance, followed by vision LLMs and open-source detectors. Individual open-source models remain competitive with best vision LLMs. Crucially, a consistent divergence is found between ROC-AUC (ranking ability) and MCC (operating-point quality).

Conclusion: The central finding is the metric disagreement between ROC-AUC and MCC, demonstrating that strong score discrimination does not guarantee reliable default-threshold decisions. The framework and results are released for reproducible future research.
...
TL;DR: A novel alignment-constrained multi-view fusion method for coronary X-ray angiography report generation, using synthetic data to learn explicit cross-view anatomical correspondence, improving stenosis grading and lesion localization over single-view and conventional fusion approaches.


<details>
  <summary>Details</summary>
Motivation: Multi-view reasoning in coronary angiography is inherently cross-projection but automated reporting remains unexplored. Single-view modeling is incomplete/unstable due to 3D vascular topology causing branch overlap and foreshortening. Multi-view fusion is promising but learning anatomically consistent fusion is hindered because cross-view alignment is unobservable and cannot be explicitly supervised in real data.

Method: (1) Reformulates multi-view stenosis reporting as an alignment-constrained aggregation problem. (2) Introduces a controllable synthetic angiography generation strategy to provide geometry-derived patch-level correspondence supervision. (3) An anatomy-correspondence module learns cross-view correspondence matrices to explicitly align auxiliary features within the main-view coordinate space before fusion, constraining aggregation to anatomically consistent regions.

Result: Experiments on synthetic data and zero-shot transfer to real angiograms show that the alignment-constrained design improves correspondence consistency and structured stenosis reporting compared to single-view modeling and conventional multi-view fusion methods.

Conclusion: Explicit cross-view anatomical alignment, learned via synthetic data supervision, significantly enhances multi-view fusion for automated coronary angiography report generation, enabling better generalization to real clinical data.
...
TL;DR: This paper introduces a flow-matching framework for Incomplete Multi-View Clustering, replacing diffusion-based stochastic denoising with deterministic ODE flows to better align with clustering objectives, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end generative approaches for IMVC rely on diffusion models with stochastic noise-to-data trajectories that are not explicitly designed for clustering, as they initialize from cluster-agnostic noise and use stochastic denoising dynamics poorly aligned with clustering objectives.

Method: The authors propose a flow-matching framework with a linear interpolation path between paired view representations, using deterministic ODE flows (instead of stochastic diffusion). The architecture integrates straight-path flow-matching view completion with cluster-level and entropy-based alignment to enforce cross-view clustering consistency.

Result: Extensive experiments on standard IMVC benchmarks demonstrate state-of-the-art performance.

Conclusion: Deterministic ODE flows are inherently better aligned with clustering objectives than diffusion-based stochastic trajectories, especially regarding transport mechanisms that respect class-conditional data distributions and maintain cluster consistency in finite-step regimes.
...
TL;DR: AlayaWorld is a full-stack open-source framework for building interactive generative game worlds using video world models, enabling real-time open-ended interactions like combat and spell casting without traditional asset production pipelines.


<details>
  <summary>Details</summary>
Motivation: Traditional game world development is labor-intensive, costly, difficult to customize, and expensive to modify after deployment. The authors aim to address these limitations by leveraging video world models for automatic world generation.

Method: The framework uses autoregressive video world models trained on gameplay recordings and real-world videos to synthesize future observations conditioned on current world state and user interactions. It provides a full-stack pipeline from data preparation to deployment with a modular and extensible architecture.

Result: AlayaWorld enables open-ended real-time interaction including navigation, combat, spell casting, and monster summoning. It also provides reproducible pipelines, reference implementations, evaluation tools, and comprehensive documentation.

Conclusion: The framework establishes a practical foundation for future research and real-time applications of generative world models, opening opportunities for interactive applications beyond gaming, including embodied intelligence.
...
TL;DR: This paper systematically compares graph construction techniques for visual tasks using a fixed three-layer GCN architecture, showing how network structure affects performance and highlighting the importance of pre-graph computational stages.


<details>
  <summary>Details</summary>
Motivation: Deep learning models struggle to encode spatial, topological, and semantic information in images. Graph neural networks offer potential solutions but their effective use for visual tasks remains underexplored.

Method: A systematic comparison of current graph construction techniques is conducted within a fixed three-layer GCN architecture through empirical study.

Result: The network structure significantly affects performance, and the computational stages before graph utilization are strongly influenced by the structure itself.

Conclusion: This work provides an important methodological contribution on the pre-graph processing stages and demonstrates that graph construction choices critically impact GCN performance in visual tasks.
...
TL;DR: A token-centric dual-view learning framework for mammography classification that uses fusion tokens inserted at multiple Transformer depths to progressively exchange information between CC and MLO views, achieving SOTA results on VinDr-Mammo and CMMD datasets.


<details>
  <summary>Details</summary>
Motivation: Existing multi-view mammography methods use feature-level aggregation or single-stage cross-attention that entangle view-specific and shared representations and restrict interaction to limited network depths, limiting effective cross-view information exchange.

Method: Proposes a token-centric dual-view framework with a frozen vision transformer backbone, where dedicated fusion tokens enable bidirectional information exchange between CC and MLO views via cross-attention at multiple transformer depths, enabling progressive hierarchical interaction while preserving view-specific structure.

Result: On VinDr-Mammo BI-RADS classification achieves 50.40% F1-score and 0.8090 AUC, with a 0.10 AUC improvement over dual-view fusion baseline in binary setting; consistent improvements over linear probing, prompt-only, and conventional fusion baselines on both datasets.

Conclusion: The token-based multi-depth fusion design effectively enables progressive cross-view interaction and outperforms existing multi-view learning approaches for breast cancer classification.
...
TL;DR: This paper proposes a synthetic-to-real motion prediction framework for autonomous driving, featuring objectness-aware prediction and enhancement to bridge domain gaps, along with a new synthetic 4D LiDAR dataset (Motion4D) for training.


<details>
  <summary>Details</summary>
Motivation: Real-world motion labels are expensive to acquire, so the authors aim to transfer motion knowledge from synthetic data to real data. However, naive motion regression methods are sensitive to domain shift, motivating a more robust approach.

Method: (1) Objectness-aware motion prediction — models joint distribution of motion patterns and objectness priors for domain-invariant feature learning. (2) Objectness-aided motion enhancement — refines motion labels using learned objectness priors to filter noise. Also introduces Motion4D, a synthetic 4D LiDAR dataset.

Result: The proposed approach effectively bridges synthetic-to-real domain gaps and achieves superior performance on real scenes compared to baselines.

Conclusion: Synthetic-to-real translation for motion prediction is feasible with the proposed objectness-aware framework, and the new Motion4D dataset provides a valuable resource for SRMP research.
...
TL;DR: This paper proposes combining pruning and step distillation for diffusion models by introducing a short teacher-alignment repair stage between them, achieving competitive FID scores with significantly fewer parameters and steps (e.g., 20% pruned one-step generator: 98.8M params, 1 eval, FID 3.12 vs baseline 124.7M params, 63 evals, FID 3.53).


<details>
  <summary>Details</summary>
Motivation: Diffusion models suffer high inference cost from two sources: large denoising networks (addressed by pruning) and repeated denoising steps (addressed by step distillation). However, pruning requires long retraining, while step distillation assumes a well-behaved student. The authors ask whether post-pruning retraining can be replaced by step distillation.

Method: The paper (1) prunes an EDM2-XS teacher, (2) introduces a short teacher-alignment repair stage that matches the pruned generator to the teacher on noisy real-image latents, and then (3) applies one-step distillation (SiDA) to the repaired checkpoint.

Result: On ImageNet-512: 20% pruning + one-step distillation achieves FID 3.12 (98.8M params, 1 eval), outperforming the baseline EDM2-XS FID 3.53 (124.7M params, 63 evals). 30% pruning achieves FID 4.26 (88.0M params, 1 eval). Direct replacement of retraining with distillation fails—starting SiDA from pruned checkpoint produces unusable samples.

Conclusion: Post-pruning retraining can be replaced by step distillation, but only with a short teacher-alignment repair stage as a bridge. This yields a highly efficient one-step diffusion generator with fewer parameters and a single network evaluation.
...
TL;DR: RNSIDNet is a new forensic framework for detecting AI-generated synthetic images. It uses a dual-branch architecture combining CLIP-based RGB global semantics with Bayar convolution-based noise artifacts, modulated via FiLM, plus a Hard Sample-aware Contrastive Learning (HSCL) strategy to improve discriminative margins. It achieves SOTA performance across eight benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing forensic networks suffer from poor cross-model generalization and robustness under real-world degradations due to single-domain representations and conventional binary classification optimization.

Method: Dual-branch architecture: (1) CLIP backbone with attention refinement extracts global RGB semantics; (2) Bayar convolutions capture high-frequency noise artifacts. Feature-wise Linear Modulation (FiLM) allows RGB features to dynamically modulate noise features. A Hard Sample-aware Contrastive Learning (HSCL) strategy is used to penalize challenging samples and maximize the discriminative margin between pristine and synthetic domains.

Result: State-of-the-art performance across eight public benchmark datasets, with superior generalization ability, robustness to real-world degradations, and computational efficiency.

Conclusion: The proposed enhanced RGB-Noise representation learning approach significantly improves generalized synthetic image detection, setting a new benchmark for the field.
...
TL;DR: VaseMuseum is a multimodal agent framework for digital museums of ancient Greek pottery that improves citation validity, reduces hallucinations, and produces more neutral answers by combining source-level and response-level reliability control with a training-free selection mechanism.


<details>
  <summary>Details</summary>
Motivation: VLMs in cultural heritage face two challenges: (1) open-ended interpretation requires grounding in specialized knowledge but retrieval introduces weak/unverifiable sources, and (2) VLMs produce confident but unsupported answers when evidence is incomplete or ambiguous.

Method: VaseMuseum includes VaseAgent with: multimodal perception of 2D/3D artifacts, external knowledge retrieval from authoritative sources, source-level control (diverse/verifiable evidence selection), response-level control (claim verification against evidence, neutral answers when uncertain), and a training-free GRPO-style selection mechanism.
...
Conclusion: The proposed framework effectively addresses reliability issues in VLM-based cultural heritage applications through multi-level evidence control without requiring VLM backbone updates.
...
TL;DR: FADRA is a frequency-aware diffusion framework for video face restoration that uses LoRA adapters, LQ Pixel-Alignment Feature Fusion, Repeated Residual Adaptation Head, and Frequency-Aware Loss to balance spatial fidelity and temporal coherence, outperforming state-of-the-art methods.
...
Method: 1) Leverages pre-trained text-to-video diffusion model with lightweight LoRA adapters and LQ Pixel-Alignment Feature Fusion; 2) Introduces Repeated Residual Adaptation Head (RRAH) for step-wise residual refinement guided by degraded observations; 3) Uses Frequency-Aware Loss for explicit supervision across multiple spectral bands.

Result: Outperforms state-of-the-art methods in both quantitative metrics and visual perception, recovering better facial structures and producing more temporally consistent videos.

Conclusion: FADRA effectively balances spatial fidelity and temporal coherence for robust video face restoration through frequency-aware diffusion and iterative residual adaptation.
...
TL;DR: SCENT is a multimodal framework that uses language as a semantic bridge between vision and olfaction, employing VLMs to generate scene descriptors for aligning electronic-nose signals with visual/textual representations, achieving state-of-the-art crossmodal retrieval on the New York Smells dataset.


<details>
  <summary>Details</summary>
Motivation: Current visual datasets capture what scenes look like but not how they smell, and aligning smell signals with images is difficult because many olfactory cues come from contextual environmental factors not directly visible in pixels.

Method: SCENT uses Vision-Language Models (VLMs) to generate scene descriptors (objects, context, ambient smells), trains a smell encoder to map e-nose signals into a shared embedding space with visual/textual representations, and introduces language-guided latent decomposition to separate object-specific odors from contextual environmental contributions.

Result: Outperforms vision-only baselines with state-of-the-art performance on smell-to-image and smell-to-text retrieval on the New York Smells dataset, and produces interpretable representations for disentangling complex smell mixtures.

Conclusion: Contextual semantic information is crucial for grounding olfactory perception in multimodal learning, paving the way for future research connecting vision, language, and smell.
...
TL;DR: This paper introduces two self-supervised approaches for verifying Optically Variable Devices (holograms) in identity documents by modeling their temporal dynamics, achieving state-of-the-art results on public datasets without requiring attack samples for training.


<details>
  <summary>Details</summary>
Motivation: Current OVD verification systems face three key limitations: (1) they treat video frames in isolation, ignoring dynamic nature of holograms, (2) they lack ability to verify specific OVD types, and (3) frame-by-frame video annotation is economically infeasible for supervised training.

Method: Two self-supervised approaches for verifying dynamic behavior of transparent OVDs in open-set scenarios, trained without any attack samples, using sequence modeling and anomaly detection to capture temporal dynamics of holograms in user-captured videos.

Result: Surpasses previous state-of-the-art methods on public datasets while adhering to industrial constraints, demonstrating that modeling temporal dynamics is essential for defeating sophisticated attacks under realistic conditions.

Conclusion: Sequence modeling and anomaly detection are promising approaches for OVD verification, as temporal dynamics capture is critical for robust security against swapping attacks and other sophisticated threats.
...
TL;DR: HoloCount is a hierarchical benchmark for evaluating MLLM counting capabilities across semantic, analytical, and robustness dimensions, revealing that even top models struggle with complex reasoning and adversarial scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing counting benchmarks focus on basic perception in simplified contexts and fail to capture complex failure modes under logical constraints or adversarial conditions, leaving a gap in diagnosing MLLM counting limitations.

Method: HoloCount introduces a three-level hierarchical taxonomy: (1) Semantic Counting (atomic/property-based), (2) Analytical Counting (spatial/set-based reasoning), and (3) Robustness Testing (adverse scenarios, high-density, linguistic biases). Over 20 MLLMs were evaluated.

Result: Top-tier MLLMs show significant performance degradation as tasks shift from perception to analytical reasoning and adverse scenarios, revealing a critical performance gap in quantitative precision.

Conclusion: HoloCount provides a systematic landscape of MLLM counting capabilities and offers a roadmap for developing more grounded and reliable multimodal systems.
...
TL;DR: XRFormer is a transformer architecture with multiscale convolutional tokenization designed for XRF spectra analysis. It outperforms existing models on pigment identification and unmixing tasks while using fewer parameters, and benefits from self-supervised pretraining strategies (MSM and PPP).


<details>
  <summary>Details</summary>
Motivation: Existing learning-based models fail to account for the complex nature of XRF spectra (sharp elemental peaks, broad structures, background variations). The authors aim to design a modality-aware architecture that effectively captures these spectral characteristics for cultural heritage material analysis.

Method: XRFormer uses a multiscale convolutional tokenizer that progressively reduces spectral resolution while increasing embedding dimensionality, injecting locality and multi-resolution inductive biases before global self-attention. Self-supervised pretraining is done via Masked Spectral Modeling (MSM) and a physics-informed Peak Presence Prediction (PPP) objective.

Result: On the Pigments Checker STANDARD v.5 dataset, XRFormer outperforms ViT, SpectralFormer, and 1D-CNN for pigment identification. For unmixing, it achieves robust abundance estimation with only 128 tokens and 1.5M parameters vs. SpectralFormer's 512 tokens and 3.37M parameters. MSM and PPP both improve performance.

Conclusion: Multiscale, modality-aware tokenization is an effective and parameter-efficient foundation for transformer-based XRF modeling under data-limited conditions, demonstrating the value of incorporating domain-specific inductive biases into transformer architectures.
...
TL;DR: PIPBench is a new benchmark for evaluating personalized image generation that aligns text-to-image model outputs with individual user aesthetic preferences based on a few historical images, introducing a data pipeline using psychological and demographic profiling.


<details>
  <summary>Details</summary>
Motivation: Existing text-to-image models like DALL-E 3 excel at following diverse prompts but fail to adapt to individual users' aesthetic preferences. There is a lack of standardized benchmarks for evaluating personalized image generation.

Method: The authors propose PIPBench (Profile-Inclusive Benchmark), a novel data construction pipeline leveraging psychological and demographic profiling dimensions for both real-user data collection and scalable agent-based data generation.

Result: Thorough evaluation of representative methods reveals key limitations in existing personalized image generation approaches.

Conclusion: The findings highlight new challenges and opportunities for advancing personalized text-to-image synthesis, with a public project page available.
...
TL;DR: This paper identifies and analyzes a fundamental mismatch between how VLMs encode spatial information internally and how current image editing pipelines extract those features, revealing that localization signals remain hidden in intermediate VLM representations rather than propagating to the predefined layers used for conditioning.


<details>
  <summary>Details</summary>
Motivation: VLMs show strong localization capabilities on their own, but when used as condition encoders in diffusion-based image editing pipelines, their spatial accuracy degrades significantly. The authors hypothesize this is because the VLM is restricted to a single forward pass rather than its optimized autoregressive generation process.

Method: The authors introduce "Analysis-by-Proxy," a framework that trains a lightweight, interpretable proxy model on the VLM's intermediate representations using an auxiliary localization task to probe where spatial information is encoded within the VLM when used as a condition encoder.

Result: Under single-pass constraints, the localization signal does not reliably propagate to the predefined layers commonly used for conditioning; instead, it remains hidden within intermediate representations at locations that vary depending on the input prompt.

Conclusion: There is a fundamental mismatch between spatial knowledge representation in VLM condition encoders and current extraction strategies in editing pipelines, motivating a more principled design of conditioning architectures.
...
TL;DR: This paper introduces the first systematic study of Audio Descriptions (ADs) in Hindi, including a new dataset (Andha-Dhun) from 8 movies, exploration of generation approaches (direct generation vs. translation), and evaluation showing that naive translation fails while human adaptation works better for accessibility.


<details>
  <summary>Details</summary>
Motivation: With growing adoption of ADs and new mandates from India's CBFC requiring ADs beyond English, there is an urgent need to generate Hindi ADs for Indian BLV audiences, yet no prior work exists for any Indian language.

Method: Collected Andha-Dhun dataset of human-authored Hindi ADs from 8 full-length movies. Explored two AD generation approaches: (1) directly generating from English dense video descriptions, and (2) translating English ADs into Hindi. Evaluated using perplexity (fluency) and LLM-as-a-judge (quality).

Result: Naive machine translation introduces artifacts and narrows diversity compared to original Hindi ADs; translation fails to adapt cultural references; human-translated ADs perform better but still fall short of original Hindi ADs.

Conclusion: Hindi ADs must prioritize accessibility for Indian BLV audiences by adapting content culturally rather than maintaining strict fidelity to the source, highlighting that translation-based approaches are insufficient.
...
TL;DR: This paper introduces a new public dataset (MIDV-DynAttack) and a novel verification method for detecting both static and dynamic hologram forgeries on identity documents, achieving state-of-the-art performance while being trainable without dynamic attack samples.


<details>
  <summary>Details</summary>
Motivation: Existing automated hologram verification methods can detect static frauds (e.g., photocopies) but have not been evaluated on real, dynamic attacks (e.g., handcrafted holograms) due to lack of public datasets, and they struggle to generalize to unseen attack types.

Method: 1) Created MIDV-DynAttack dataset tripling attack samples over MIDV-Holo; 2) Proposed a verification method analyzing both dynamic behavior and appearance of holograms, trainable without dynamic attack samples; 3) Established a benchmark with a clear evaluation protocol.

Result: The novel method achieves new state-of-the-art performance; existing approaches fail to handle dynamic attacks; the benchmark highlights challenges posed by new attack types.

Conclusion: The new dataset and method advance automated hologram verification, particularly for detecting realistic dynamic attacks, while public code/dataset enable reproducible research.
...
TL;DR: EgoPolice is a new dataset of police-civilian interactions from body-worn camera videos, designed as a challenging benchmark for egocentric perception models, with findings showing even top models struggle on high-risk actions like "Weapon Out".


<details>
  <summary>Details</summary>
Motivation: To address the lack of curated, real-world egocentric datasets focused on police-civilian interactions, enabling behavioral research and model development for identifying critical events in large-scale body-worn camera footage.

Method: Curated real body-worn camera videos, selected critical action labels, annotated at second-by-second granularity, and created two tasks (classification and multiple-choice QA). Benchmarked both open-source and closed-source models.

Result: Even the best video models (e.g., Gemini 2.5 Pro) struggle to accurately predict high-risk actions like "Weapon Out".

Conclusion: EgoPolice serves as a challenging benchmark and foundation for developing motion-robust, context-aware models to identify events of interest in police body-worn camera video repositories.
...
TL;DR: This paper presents a novel framework integrating Vision Language Models (VLMs) for daytime assessment and LiDAR-derived retroreflectivity for nighttime assessment to evaluate traffic sign conditions comprehensively, offering a cost-effective alternative to manual inspections.


<details>
  <summary>Details</summary>
Motivation: Traditional traffic sign inspections are subjective, labor-intensive, and expensive (retroreflectometers). Existing studies rarely integrate both daytime and nighttime assessment comprehensively.

Method: Three fine-tuned VLMs (LLaVA, Qwen, InternVL) assess daytime factors (legibility, color, surface/shape integrity, environment). VLM outputs are converted to scores via sentiment analysis & CLIP scoring. Nighttime retroreflectivity is assessed via LiDAR. Both are combined into a Sign Condition Index (SCI).

Result: LLaVA and Qwen outperformed InternVL with cosine similarity scores of 0.67-0.76. Among 462 signs, 68 were flagged for immediate replacement due to inadequate retroreflectivity.

Conclusion: The framework provides a cost-effective, comprehensive alternative to manual inspections for traffic sign condition assessment, integrating both daytime and nighttime performance.
...
TL;DR: PACR-Video is a parameter-efficient framework that extends long multi-shot videos by freezing a text-to-video diffusion transformer and adding lightweight low-rank temporal adapters with learned shot-role prompts, achieving state-of-the-art performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: To enable long-horizon multi-shot video extrapolation that preserves recurring entities, scene structure, visual style, and causal progression without the high cost of full generator fine-tuning.

Method: Freezes a text-to-video diffusion transformer; augments it with low-rank temporal adapters conditioned by learned shot-role prompt tokens; builds a recursive prompt bank storing entity, location, action, and style prompts from previous shots; routes prompts through adapter gates based on predicted narrative dependencies; uses a Shot-Local/Story-Global tuning objective with next-shot reconstruction, cross-shot identity contrast, and sparsity regularization; employs an adapter composition schedule balancing early-shot consistency with later-shot progression.
...
Conclusion: Compact prompt routing and lightweight temporal adaptation provide sufficient controllable capacity for stable long video extrapolation.
...
TL;DR: Floor plan generation models are highly sensitive to domain shift, but pre-training on a novel synthetic dataset with strict physical constraints (though unrealistic) significantly improves cross-domain robustness and enables efficient fine-tuning with minimal data.


<details>
  <summary>Details</summary>
Motivation: Floor plans vary greatly across regions due to cultural, spatial, and construction differences, and acquiring annotated datasets for each new domain is costly. No prior work has studied domain shift robustness for conditioned floor plan generation.

Method: The authors evaluate SOTA models from two generative paradigms across three datasets (RPLAN, MagicPlan, Swiss Dwellings). They introduce a procedural synthetic dataset that enforces physical constraints (non-overlapping rooms, valid door placement, graph consistency) but deliberately lacks architectural realism via irregular arrangements and geometric perturbations.

Result: Models show up to 10x performance degradation under domain shift. Pre-training on synthetic data improves zero-shot cross-domain performance (outperforming in-domain training on MagicPlan) and boosts fine-tuning by up to 40% in low-data regimes.

Conclusion: Synthetic pre-training with strict physical constraints, even without architectural realism, is an effective and data-efficient strategy for domain adaptation in floor plan generation.
...
TL;DR: First adversarial attack for infrared remote-sensing VLMs using thermal-airflow turbulence as perturbation prior, achieving 48.5% zero-shot ASR across five CLIP backbones and up to 38.2% relative accuracy reduction on six VLMs.


<details>
  <summary>Details</summary>
Motivation: VLMs are being deployed on infrared remote sensing imagery in security-critical settings, but their adversarial robustness against physical-world attacks has not been studied.
...
Result: 48.5% zero-shot ASR across five CLIP backbones, up to 38.2% relative accuracy reduction on six VLMs, exceeding four IR-specific physical baselines (27.7-37.0%). Paradoxically makes models more confident by confabulating perturbation as genuine thermal evidence.

Conclusion: Critical vulnerabilities exist in the rapidly expanding IR VLM ecosystem; airflow prior enhances physical plausibility without sacrificing attack success.
...
TL;DR: Point as Skeleton is a generative sensor simulation framework for state-updated autoregressive driving video generation. It uses point-cloud skeletons to decouple foreground/background, employs Reset-and-Roll for closed-loop rollout, and improves autoregressive generation quality on nuScenes and nuPlan benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing E2E-AD evaluation methods struggle to balance closed-loop interactivity (e.g., CARLA) and real-world visual fidelity (e.g., nuScenes). The authors aim to bridge this gap with a simulation framework that supports both high visual fidelity and interactive closed-loop rollout.

Method: 1) Point-cloud skeletons decouple foreground and background, projected into camera-view painted-point and template-depth conditions. 2) Reset-and-Roll adapts rolling diffusion inference to prevent future-conditioned latent state leakage across simulation steps. 3) Autoregressive generator synthesizes visual observations from step-wise updated ego states, actor states, scene maps, and skeleton conditions. 4) A nuPlan-based renderer-level closed-loop interface evaluates generation under ego deviations.

Result: Experiments on nuScenes and nuPlan show that Point as Skeleton improves autoregressive generation quality during closed-loop rollout, demonstrating potential for visually faithful closed-loop driving simulation.

Conclusion: Point as Skeleton offers a promising approach to visually faithful closed-loop driving simulation by addressing error accumulation and latent state leakage in autoregressive video generation, paving the way for better E2E-AD evaluation.
...
TL;DR: CAIRN is a topology-aware 3D-LLM that enhances multi-room scene understanding by aligning attention with scene hierarchy, using GNN-based relational context, learned room tokens, and hierarchical attention masks. It outperforms prior models on the new CAIRN-MR benchmark while remaining competitive on single-room tasks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D-LLMs are limited to simplified single-room scenes and cannot handle real-world multi-room environments with interconnected spaces and diverse objects.

Method: CAIRN enriches object tokens with room-local relational context via GNN, introduces learned room tokens for room-level abstraction, and applies a hierarchical attention mask with geometric bias to route information according to scene topology.

Result: CAIRN outperforms prior 3D-LLMs by a large margin across all CAIRN-MR tasks while remaining competitive on five single-room benchmarks.

Conclusion: Explicitly modeling scene topology—both object-level relations and room-level connectivity—significantly improves multi-room 3D scene understanding without sacrificing single-room performance.
...
TL;DR: A domain adaptation framework using style transfer (AdaIN, CycleGAN) and Swin Transformer V2 improves calcification classification across multi-site mammography datasets, raising AUC from 0.68 to 0.72-0.73 on external datasets.


<details>
  <summary>Details</summary>
Motivation: Deep learning CAD systems struggle with domain shifts across multi-site datasets, degrading performance when applied to unseen domains in breast cancer diagnosis.

Method: Two-component framework: (1) unsupervised domain adaptation via style transfer models (AdaIN and CycleGAN) to generate vendor/technique-specific training samples, and (2) supervised classification using Swin Transformer V2 backbone.

Result: Improved cross-site AUC from 0.68 to 0.72 on EMBED and from 0.68 to 0.73 on Duke Calcification Dataset.

Conclusion: Domain adaptation reduces domain shifts and improves generalization for calcification classification across multi-site mammography datasets.
...
TL;DR: MonoIR-RS is a large-scale infrared remote-sensing vision-language dataset and benchmark that enables IR-aware contrastive learning and VLM instruction tuning, demonstrating significant improvements in IR-language alignment over zero-shot baselines.


<details>
  <summary>Details</summary>
Motivation: Existing remote-sensing vision-language resources focus on visible-band (RGB) semantics, leaving infrared (IR) vision-language understanding underexplored despite IR's unique advantages such as illumination invariance and strong object-background contrast.

Method: The authors construct MonoIR-RS from the same source pool as FusionRS, producing 600K synthesized IR images and 59K IR-aware caption records. They fine-tune five CLIP backbones and six VLM backbones with IR-aware contrastive adaptation and instruction tuning, evaluating against zero-shot baselines.

Result: Synthesized IR imagery is markedly closer to real thermal imagery than grayscale conversion. IR-aware adaptation lifts CLIP mean recall by up to 12.8 points, drives VLM captioning IR-cue coverage to 100%, and reduces residual RGB-color leakage to near zero.

Conclusion: MonoIR-RS provides a controlled, reproducible testbed for aligning infrared remote-sensing evidence with language, isolating the IR modality from RGB-IR dual-modal learning.
...
TL;DR: ReChannel adapts pretrained DiT-based text-to-image models for dense prediction by replacing the generative decoder with a simple token-to-patch linear head (33K parameters), achieving SOTA on matting, depth, and referring segmentation while being faster and more accurate than generative decoding approaches.


<details>
  <summary>Details</summary>
Motivation: Existing methods cast dense prediction as image generation, inheriting unnecessary generative output interfaces. The authors observe that DiT's patch-token lattice structure can directly carry task-native quantities per token, avoiding the need for RGB decoding.

Method: ReChannel keeps the VAE encoder for input, drops the target-side decoder, freezes the DiT with task-specific LoRA adaptation, and uses a shared token-local linear head (33K params) to map each token to its corresponding p x p x K_t pixel-space patch with no spatial mixing.

Result: Sets new SOTA on trimap-free matting, KITTI depth, and referring segmentation; competitive on normals, saliency, and pose. In matched 4B setting, it is 2.48x faster and more accurate than edit-plus-latent-decode counterpart.

Conclusion: Dense perception can benefit from generative pretraining without inheriting its output interface—the minimal token-to-patch mapping is simpler, faster, and often more accurate than generative decoding approaches.
...
TL;DR: ProxyPose introduces a video-to-video translation approach for 6-DoF pose tracking. Using a fine-tuned video diffusion model, it converts an input video (with a single marked pixel in the first frame) into a proxy video of a colored polyhedron undergoing the same motion, enabling classical pose estimation without requiring 3D models, depth maps, or masks. It achieves SOTA accuracy using only synthetic fine-tuning data.


<details>
  <summary>Details</summary>
Motivation: Existing 6-DoF pose tracking methods require additional inputs (3D models, depth maps, object masks, task-specific features) and fail on textureless, transparent, reflective, or deformable surfaces. The authors aim to eliminate these prerequisites and handle challenging surface materials.

Method: Pose tracking is recast as video-to-video translation. A fine-tuned video diffusion model translates an input video (with one marked pixel in the first frame) into a "proxy video" showing a colored polyhedron undergoing the same local rigid-body motion. Since the proxy's geometry and appearance are known, 6-DoF pose recovery reduces to classical pose estimation with off-the-shelf solvers.

Result: ProxyPose achieves state-of-the-art 6-DoF pose tracking accuracy without additional inputs required by competing methods, after fine-tuning the video model only on synthetic data. It extends to face tracking, camera pose estimation, and challenging in-the-wild scenes beyond existing approaches.

Conclusion: The video-to-video translation paradigm effectively offloads the hardest aspects of pose tracking (challenging materials, occlusions, deformations) into the diffusion model's translation step, operating at pixel level with no assumptions about object identity, boundaries, or global rigidity.
...
TL;DR: SenseNova-Vision formulates diverse computer vision tasks (detection, segmentation, depth estimation, etc.) as unified multimodal text-and-image generation within a single model, without task-specific architectures, and matches specialized models across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current computer vision systems rely on task-specific architectures and prediction heads, limiting scalability and integration into general-purpose foundation models. The authors aim to unify heterogeneous visual tasks under a single multimodal generation framework.

Method: All vision tasks are expressed as instruction-response pairs in text and image generation spaces. A large-scale instruction-response corpus (SenseNova-Vision Corpus) is built from diverse vision annotations. Starting from a pretrained unified multimodal model, training is done primarily on this corpus with auxiliary multimodal data, without task-specific heads or architectural changes.

Result: The single unified model matches or competes with leading task-specialized systems across structured visual understanding, dense geometric prediction, segmentation, and multi-view visual geometry tasks.

Conclusion: Unified multimodal generation is a scalable and effective route for integrating broad computer vision capabilities into general-purpose foundation models, eliminating the need for task-specific designs.
...
TL;DR: ELSA3D introduces elastic semantic anchoring with a scale-aware octree tokenizer and Anchor Tokens for sparse, precise cross-modal interaction, achieving SOTA across 3D generation and captioning tasks with ~50% reduction in FLOPs and latency.


<details>
  <summary>Details</summary>
Motivation: Existing unified 3D models treat text-3D interaction implicitly by concatenating tokens into flat sequences via self-attention, which collapses coarse structural and fine geometric details into one undifferentiated representation, lacking explicit cross-modal reasoning at proper scales.

Method: ELSA3D uses a scale-aware octree tokenizer for multi-scale geometry representation, introduces Anchor Tokens as sparse cross-modal units that select semantic cues, route them to relevant 3D scales, retrieve scale-specific evidence, and fuse signals back. A lightweight per-block router makes computation elastic by choosing which text tokens instantiate anchors at which geometric scale.

Result: Achieves state-of-the-art on image-to-3D generation, text-to-3D generation, and 3D captioning, outperforming the strongest unified baseline while roughly halving FLOPs and inference latency compared to the non-elastic version.

Conclusion: Elastic semantic anchoring with scale-aware cross-modal interaction effectively addresses the flat representation problem in unified 3D models, enabling both better performance and greater efficiency.
...
TL;DR: TMF-RSE is a tri-modal deep learning framework that combines appearance, structural, and semantic features with evidential regression for accurate lung disease severity quantification from chest imaging, outperforming transformer baselines on two datasets.
...
Method: Proposes TMF-RSE, a tri-modal framework fusing 2D appearance features, lung segmentation structural features, and VLM-based semantic features via complementary fusion mechanisms with hierarchical interactions, using evidential regression for prediction and uncertainty estimation.

Result: Outperforms transformer baselines on Per-COVID-19 CT (MAE 4.02, PC 0.9629) and RALO (MAE 0.339, PC 0.973) datasets.

Conclusion: Tri-modal fusion with evidential uncertainty provides superior lung disease severity quantification compared to existing transformer-based approaches.
