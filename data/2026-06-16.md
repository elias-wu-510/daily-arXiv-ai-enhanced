...
TL;DR: RAMS is a runtime controller that dynamically switches between YOLOv8 model tiers (NANO/SMALL/MEDIUM) based on device pressure, achieving 5.6x faster inference than fixed-MEDIUM while retaining 74% accuracy, with detection-conditioned policies improving safety for vulnerable road users.


<details>
  <summary>Details</summary>
Motivation: Edge object detection on embedded hardware requires balancing inference latency and detection quality under changing resource pressure, and existing solutions lack lightweight runtime control with safety-aware switching.

Method: RAMS monitors device pressure, calibrates switching thresholds from idle behavior, dynamically selects among three YOLOv8 tiers without model-reload latency, defines five switching policies (including detection-conditioned variants that prevent aggressive downgrades after VRU detections), and introduces the VRU-Weighted Accuracy Score (SWAS) for offline policy comparison.

Result: On Jetson Orin TensorRT under heavy load, safety2 policy achieves 3.41 ms mean latency (5.6x faster than fixed-MEDIUM) while retaining 74% proxy accuracy. Detection-conditioned switching improves SWAS by 25.4% (oracle) and 47.3% (detector-derived) over threshold-only policies. Live KITTI evaluation reports per-tier VRU recall of 24.2%, 41.2%, and 59.0%.

Conclusion: RAMS effectively balances latency and detection quality across diverse hardware (37x latency range), with safety-aware policies improving VRU protection. However, reactive overrides are fundamentally limited by baseline detector recall.
...
TL;DR: This paper evaluates six deep learning models (CNN, Xception, VGG16, MobileNetV2, EfficientNetV2L, and ViT) for ship detection in maritime environments using 6,468 images across various weather conditions. ViT achieved 100% accuracy with the fastest processing, while lightweight models are better for resource-limited devices.


<details>
  <summary>Details</summary>
Motivation: To enhance maritime security by developing AI-based object detection systems capable of detecting ships in real-time under diverse and challenging weather conditions (cloudy, foggy, rainy, sunny).

Method: Used a maritime dataset of 6,468 images, evaluated six deep learning architectures: base CNN, Xception, VGG16, MobileNetV2, EfficientNetV2L (transfer learning), and Vision Transformer (ViT). Models were compared on accuracy, Type I/II errors, model size, and video processing time.

Result: ViT achieved the best performance (100% accuracy, lowest error rates, fastest video processing). Lightweight architectures (e.g., MobileNetV2) are more suitable for resource-constrained devices. Performance varies based on computational constraints and deployment conditions.

Conclusion: AI-driven computer vision systems, particularly Vision Transformers, show strong potential for maritime surveillance, border protection, and autonomous navigation, though model selection should consider deployment hardware limitations.
...
TL;DR: A disagreement-based cross-model routing strategy improves multiple-choice video question answering on the ImplicitQA benchmark by routing hard/disagreed questions from a primary video LLM (Gemini 3.1 Pro) to a second model (Claude Opus 4.8), achieving +1.43 AvgAcc gain on validation and +1.81 on a held-out test set.


<details>
  <summary>Details</summary>
Motivation: Conventional self-consistency (majority voting) degrades performance on ImplicitQA because model errors are correlated on hard questions, so a better inference-time method is needed to improve accuracy without training or labels.

Method: Triple-sample a native-video LLM (Gemini 3.1 Pro Preview) at temperature zero, identify the ~20% of questions where samples disagree, and route only those to a second model (Claude Opus 4.8) that uses uniformly sampled frames with adaptive thinking.

Result: +1.43 AvgAcc gain on 1001-question validation set (with largest gains in Motion & Trajectory +5.49, Inferred Counting +3.45, Vertical Spatial Reasoning +1.82); +1.81 AvgAcc gain on held-out 172-question CVPR 2026 test set.

Conclusion: Cross-model routing leveraging sample-to-sample variance in video pipelines outperforms self-consistency and pushes accuracy beyond any single model's ceiling on the ImplicitQA benchmark.
...
TL;DR: VigilFormer jointly optimizes detection accuracy and inference speed for video anomaly detection via deformable sparse attention, causal temporal modeling, and an adaptive frame-skipping scheduler, achieving SOTA results on three benchmarks at 41.5 FPS.


<details>
  <summary>Details</summary>
Motivation: Existing methods trade off accuracy and throughput: stronger feature extractors incur high computation, while efficient architectures sacrifice detection quality. There is a need for a unified framework that excels at both.

Method: (1) Deformable Spatio-Temporal Encoder (DSTE) for sparse attention across frames; (2) Causal Anomaly Classifier (CAC) with dilated causal convolutions and contrastive MIL; (3) Adaptive Confidence Scheduler (ACS) to skip low-information frames at inference.

Result: AUC: 87.83% (UCF-Crime), 97.21% (ShanghaiTech), 89.74% (CUHK Avenue); throughput: 41.5 FPS on a single GPU.

Conclusion: VigilFormer proves that strong spatio-temporal modeling and real-time inference can be jointly achieved, outperforming recent weakly-supervised methods in both accuracy and speed.
...
TL;DR: ConvNN unifies convolution and self-attention into a single k-nearest neighbor aggregation framework, showing they lie on a continuous spectrum rather than being fundamentally distinct.


<details>
  <summary>Details</summary>
Motivation: To bridge the perceived gap between CNNs and Transformers by demonstrating that convolution (local, spatial) and self-attention (global, feature-similarity) are special cases of a unified neighbor selection and weighted aggregation mechanism.

Method: Introduces ConvNN, a k-nearest neighbor framework where convolution is recovered via spatial coordinate neighbor selection and self-attention via dot-product similarity neighbor selection, with configurable similarity functions, positional encodings, and aggregation kernels.

Result: ConvNN exactly recovers standard/depthwise convolution and self-attention (including sparse variants like KVT-attention), and enables systematic exploration of intermediate aggregation behaviors.

Conclusion: ConvNN serves as a drop-in replacement for both convolution and attention layers, enabling a unified view and flexible trade-offs between local and global aggregation in vision architectures.
...
TL;DR: FairGen is a fairness-aware diffusion framework that generates demographically balanced medical images to mitigate diagnostic bias, achieving significant fairness improvements across dermatology, radiology, and neuroimaging tasks while maintaining diagnostic accuracy.


<details>
  <summary>Details</summary>
Motivation: Demographic imbalances in clinical image data, combined with distinct disease manifestations across groups, cause AI models to perpetuate diagnostic bias and widen healthcare disparities.

Method: FairGen is a fairness-aware diffusion framework that embeds physician-aligned preferences into the generation process to synthesize demographically balanced medical images while preserving pathology-relevant visual features.

Result: FairGen achieved fairness improvements of 95.9% (skin images), 80.0% (chest radiography), and 35.2% (brain MRI), while maintaining competitive diagnostic accuracy relative to models trained on original clinical data.

Conclusion: FairGen's gains extend beyond standard fidelity metrics and are validated through clinician review and external cohorts, proving effectiveness beyond in-distribution datasets.
...
TL;DR: FUSE is a probabilistic framework that combines aleatoric embedding-level uncertainty and epistemic model-level uncertainty in VLMs via Bayesian fusion to produce a calibrated scalar uncertainty measure for predicting output correctness.


<details>
  <summary>Details</summary>
Motivation: Quantifying uncertainty in VLM outputs is crucial for safety-critical applications like robotics, but existing methods fail to capture both data ambiguity (aleatoric) and model uncertainty (epistemic) sources.

Method: FUSE uses a Bayesian fusion mechanism to analytically combine (i) aleatoric uncertainty from vision-language embedding-level ambiguity and (ii) epistemic uncertainty from semantic response diversity of VLMs into a single scalar uncertainty measure.

Result: The method outperforms baselines and achieves state-of-the-art uncertainty calibration for predicting VLM output correctness.

Conclusion: FUSE provides reliable uncertainty quantification for VLMs by jointly modeling complementary uncertainty sources, enabling better downstream correctness prediction.
...
TL;DR: A hierarchical model for football ball action anticipation using Transformers, GRU, and frequency-reweighted Hungarian matching, achieving 17.91% mAP on SoccerNet benchmark.


<details>
  <summary>Details</summary>
Motivation: To predict future ball actions in football broadcast videos, addressing the challenge of anticipating rare action classes in dynamic sports scenarios.

Method: The model uses a local Transformer for clip-level encoding, a GRU for temporal aggregation, and a Transformer decoder with K event slots and three decoupled heads (objectness, class, temporal offset), plus frequency-reweighted Hungarian matching and Gaussian soft targets.

Result: 17.91% mAP on the SoccerNet Ball Action Anticipation test set.

Conclusion: The proposed hierarchical architecture with frequency-aware matching and soft temporal supervision effectively anticipates football ball actions, especially rare classes.
...
TL;DR: BBR-Net uses boundary-aware replay for continual cardiac ultrasound segmentation, showing structural consistency is key to knowledge retention; effectiveness breaks down when source data has noisy or unreliable structure.


<details>
  <summary>Details</summary>
Motivation: Replay-based continual learning methods for medical image segmentation under domain shift tend to preserve appearance information without explicitly modeling anatomical structure; the authors investigate whether structural consistency governs knowledge retention.

Method: Proposes Boundary-Balanced Replay Network (BBR-Net), which selects replay samples using boundary-aware priority and class balance. Evaluated on CAMUS and CardiacNet under forward and reverse task orders, with controlled structural perturbation analysis.

Result: In forward setting, BBR-Net retains source-task performance close to offline joint-training and reduces catastrophic forgetting. Reverse setting shows structure-aware replay fails when initial data is noisy/structurally inconsistent. Perturbation analysis confirms forgetting increases as structural reliability decreases.

Conclusion: Preserving anatomical structure under domain shift is central to continual medical image segmentation; replay mechanisms should account for structural reliability, not just memory capacity.
...
TL;DR: Steady-Forcing is a framework for autoregressive video diffusion that preserves background stability while maintaining natural motion (water, fire, smoke) in long-horizon fixed-camera generation, outperforming seven baselines. The paper also identifies flaws in VBench metrics that fail to penalize texture hardening or flow stagnation.


<details>
  <summary>Details</summary>
Motivation: Autoregressive video diffusion models suffer from a stability-motion trade-off: improving spatial stability suppresses natural fluid motion (stagnation), while allowing motion causes background drift. The authors aim to resolve this specifically for fixed-camera nature video generation.

Method: Steady-Forcing combines: (1) V-Sink (persistent visual anchor for background), (2) EMA-Sink (exponential moving-average motion memory), (3) block-relative temporal encoding, (4) periodic cache purification, and (5) distillation from a Wan2.1-14B teacher with motion-rewarded priors.

Result: Outperforms seven baselines in long-horizon background consistency and imaging quality. A blind user study confirms better perceived stability and motion continuity. VBench aggregate scores are shown to be inadequate—they reward drift-induced optical flow as "Dynamic Degree" while ignoring texture hardening and stagnation.

Conclusion: Steady-Forcing effectively balances stability and motion for fixed-camera nature videos. The work motivates the need for task-specific benchmarks for static-camera nature-flow evaluation, as generic VBench metrics are misaligned with artifact detection in this setting.
...
TL;DR: UtVAA is an ultra-tiny Vision Transformer (sub-205K parameters) that combines Affix Attention (local+linear+coordinate attention) and Dilated Bottleneck blocks to achieve competitive image classification accuracy on mobile/edge devices.


<details>
  <summary>Details</summary>
Motivation: ViTs have strong representation capability but suffer from quadratic self-attention complexity and large parameter counts, making them unsuitable for resource-constrained mobile and edge devices.

Method: UtVAA introduces: (1) Affix Attention block combining depthwise-pointwise local extraction, linear self-attention, coordinate attention, and a ternary fusion strategy; (2) Dilated Bottleneck blocks with dilated depthwise separable convolutions and residual connections; (3) scalable Tiny/Medium/Large variants.

Result: The smallest UtVAA variant achieves 204.67K parameters and 53.95M FLOPs, with competitive accuracy on CIFAR-10, CIFAR-100, PlantVillage-Tomato, and SLIF-Tomato datasets within the sub-million-parameter regime.

Conclusion: Transformer-based vision models can be redesigned into ultra-tiny architectures without significant loss in discriminative performance, making UtVAA suitable for mobile and edge deployment.
...
TL;DR: GridVQA-X is a diagnostic framework revealing that current multimodal explainability methods fail to distinguish between genuine cross-modal reasoning and shallow shortcuts, highlighting a critical gap in faithful explainability.


<details>
  <summary>Details</summary>
Motivation: Current Multimodal Explainable AI (MxAI) evaluation protocols lack ground truth to distinguish true cross-modal reasoning (e.g., spatial composition) from shallow shortcuts (e.g., Bag-of-Words attribute matching), making it unclear whether explanations capture synergistic interactions or hallucinate reasoning.

Method: GridVQA-X uses closed-world synthesis logic to generate mathematically guaranteed explanations and trains two paired models on identical architectures: M_pure (robust spatial-relational reasoning) and M_spur (structurally forced cross-modal shortcuts). A faithful explainer must report distinct reasoning pathways for each model.

Result: Widely used explainability methods fail to distinguish between M_pure (genuine spatial-relational reasoning) and M_spur (cross-modal shortcuts), revealing they cannot capture true cross-modal synergy.

Conclusion: Current MxAI methods misrepresent how multimodal models actually make decisions, exposing a critical gap in faithful cross-modal explainability that needs to be addressed.
...
TL;DR: HorusEye introduces a benchmark (RefCOCO-Degraded) with 15,244 images across Clean, Fog, Smoke, and Thermal conditions to evaluate VLMs on visual grounding, language feedback recovery, health VQA, and hallucination analysis. Key findings: language feedback effectiveness is model-dependent (Gemini +47.3% improvement in thermal; Qwen2-VL -5.1% degradation), a "Thermal Paradox" where cropping helps RGB but fails in thermal, and BLIP-2 hallucinates more under degradation, making it unsuitable for emergency use.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate Vision-Language Models (VLMs) under degraded visual conditions (fog, smoke, thermal) for emergency visual analysis, addressing the gap in benchmarking VLM robustness in real-world adverse conditions.

Method: Five-stage investigation: (1) RefCOCO-Degraded benchmark (15,244 images, 4 conditions); (2) Visual grounding evaluation; (3) Language feedback recovery assessment; (4) Health VQA tasks; (5) Hallucination analysis. Models tested: Gemini, Qwen2-VL, BLIP-2, LLaVA, Kosmos-2.

Result: Gemini achieves +47.3% improvement in thermal conditions via iterative language feedback; Qwen2-VL shows -5.1% degradation under same protocol. "Thermal Paradox" identified where cropping helps RGB but fails in thermal. BLIP-2 hallucinates more under degradation.

Conclusion: Language feedback effectiveness is highly model-dependent for degraded visual conditions. BLIP-2 is unsuitable for emergency deployment due to increased hallucination under degradation. The Thermal Paradox highlights that strategies effective for one modality/condition may fail in others.
...
TL;DR: The paper proposes Style-CCL, a Multi-Stage Curriculum Continual Learning framework for Content-Preserving Style Transfer using Diffusion Transformers. It identifies that one-stage training on mixed styles causes semantic domination over texture styles. Style-CCL trains SC-DiT from easy (semantic) to hard (texture) styles and from clean to synthetic data with Random Memory Rehearsal, achieving SOTA in style similarity, content consistency, and aesthetic quality.


<details>
  <summary>Details</summary>
Motivation: Content-preserving style transfer with Diffusion Transformers suffers from entangled content and style features. One-stage training on mixed style categories causes semantic styles to dominate over texture styles, harming content preservation.

Method: 1) Reverse triplet synthesis pipeline to build a million-scale training set. 2) Dual-branch Style-Content DiT (SC-DiT) with separate ROPE embeddings and causal masking for decoupling. 3) Multi-Stage Curriculum Continual Learning (Style-CCL) training from semantic (easy) to texture (hard) styles, and from clean to synthetic data, with Random Memory Rehearsal to prevent catastrophic forgetting.

Result: State-of-the-art performance in three metrics: style similarity, content consistency, and aesthetic quality.

Conclusion: The proposed curriculum continual learning approach effectively addresses the style domination problem in one-stage training, enabling better content preservation and texture style learning in DiT-based style transfer.
...
TL;DR: MMLongEmbed is the first comprehensive benchmark evaluating Multimodal Embedding Models (MEMs) on long-context tasks. It reveals that current MEMs struggle with deep semantic understanding, relying on superficial matching, and performance degrades with context length, key information position, and modality-specific redundancy.


<details>
  <summary>Details</summary>
Motivation: Existing MEMs with large theoretical context windows fail to effectively understand long multimodal inputs in practice, and there is a lack of systematic benchmarks to evaluate this gap.

Method: Proposes MMLongEmbed, a benchmark with four retrieval tasks across varying context lengths, covering text, document, and video modalities, used to evaluate state-of-the-art MEMs.

Result: Current MEMs rely on superficial feature matching, struggle with deep semantic/structural dependencies, performance degrades with context length and key info placement, and model robustness to redundant context varies across modalities.

Conclusion: Longer context windows alone are insufficient; the community needs better architectural designs for true long-context multimodal understanding. Benchmark and code are publicly released.
...
TL;DR: This paper presents MINT (Membership Inference Test), a framework for detecting whether specific data was used to train machine learning models, achieving up to 90% accuracy on face recognition models and LLMs, and introduces a web platform for auditing models to promote AI transparency and regulatory compliance.


<details>
  <summary>Details</summary>
Motivation: To improve transparency in machine learning training processes and provide tools to foster compliance with emerging AI regulations by experimentally determining whether specific data was used during model training.

Method: The paper establishes a theoretical framework and proposes multiple architectures (MINT, aMINT, gMINT) depending on the amount of information known about audited models. The approach is evaluated on a face recognition model, 4 state-of-the-art LLMs, and multiple large-scale public image and text databases.

Result: Achieves up to 90% accuracy in detecting training data across diverse models and modalities. A comprehensive web platform was built integrating MINT, aMINT, and gMINT for auditing image and text models.

Conclusion: MINT provides a practical tool for AI transparency that can help enforce emerging AI regulations by enabling membership inference audits on various ML models.
...
TL;DR: A high-throughput 3D behavioral phenotyping framework combining deep learning and binocular stereo vision enables real-time, non-contact tracking of juvenile tilapia in high-density aquaculture, quantifying body length, 3D swimming trajectories, velocity, and acceleration for early stress detection and vitality assessment.


<details>
  <summary>Details</summary>
Motivation: Precision aquaculture faces a "phenotyping bottleneck" in tracking high-resolution behavioral traits, as conventional methods cannot quantify instantaneous 3D physical exertion in free-roaming fish.

Method: The framework integrates deep learning object detection with binocular stereo vision for real-time monitoring, automating non-contact body length estimation and reconstructing 3D swimming trajectories from absolute spatial coordinates, eliminating 2D perspective distortions.

Result: The system enables the first estimation of true physical swimming speeds in free-roaming juvenile tilapia, successfully establishes circadian locomotor baselines, and provides an early warning system for physiological stress.

Conclusion: This approach provides an objective metric for fish vitality and serves as an early warning system for physiological stress in precision aquaculture.
...
TL;DR: X-Tokenizer introduces a Semantic Residual Quantization (SRQ) framework for action tokenization that transforms action codes from mere motion reconstruction into semantically meaningful interfaces, achieving superior multimodal grounding (+13.5%) and long-horizon task performance (+8.25%) in VLA models.


<details>
  <summary>Details</summary>
Motivation: Existing action tokenizers focus purely on reconstruction, producing codes that preserve motion geometry but fail to provide strong semantic supervision to VLA backbones. The authors argue action tokenization should serve as a semantic interface bridging multimodal reasoning and executable control.

Method: Proposes X-Tokenizer with an encoder-SRQ-decoder architecture. SRQ uses asymmetric residual quantization: first level trained with Masked Action Modeling (MAM) for coarse motion intent, deeper levels for reconstruction. Pretrained with contrastive alignment to foundation model representations and next-frame vision-language feature prediction on 2.4M trajectories (2.0B frames).

Result: Single frozen X-Tokenizer plugged into mixed discrete-continuous VLA achieves top real-world aggregate results, strong RoboTwin 2.0 simulation performance, outperforming FAST by +13.5% in multimodal grounding and +8.25% in long-horizon tasks.

Conclusion: Action tokenizers should be designed as semantic interfaces for VLA pretraining rather than mere action compression tools, enabling better alignment between vision-language reasoning and robot control.
...
TL;DR: This paper proposes an efficient image captioning model that replaces standard quadratic self-attention in Vision Transformers with a Gaussian Mixture Model-based probabilistic attention mechanism, reducing complexity from O(n²) to O(nK) via soft-clustering, and achieves competitive results on Flickr30K.


<details>
  <summary>Details</summary>
Motivation: Existing transformer-based image captioning approaches suffer from a lack of rich local feature representations and high computational cost due to quadratic self-attention (O(n²)).

Method: The standard self-attention in Vision Transformers is replaced with a probabilistic transformer using Gaussian Mixture Model (GMM) soft-clustering. The Expectation-Maximization (EM) algorithm groups similar image patches into K clusters (K << n), reducing complexity to O(nK). An autoregressive GPT-based decoder generates captions.

Result: Evaluated on the Flickr30K dataset, the model demonstrates competitive and significant improvement over existing works.

Conclusion: The GMM-based clustering attention effectively improves computational efficiency while maintaining or improving captioning quality, offering a viable alternative to standard self-attention.
...
TL;DR: This paper introduces sub-semantic image segmentation (between texture and semantic segmentation) and proposes DETECTURE, a method coupling vision-language models with SAM to partition images into language-describable appearance patterns, along with a new dataset TextureADE.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between texture segmentation (visual cues) and semantic segmentation (objects) by proposing a new "sub-semantic" segmentation category where language describes stable appearance patterns rather than naming whole objects.

Method: Couples a general-purpose vision-language model with SAM's text pathway to ground descriptions into masks. Introduces DETECTURE to resolve three failure modes: language leakage between texture regions, prompt competition in the segmentation backbone, and semantic distortion at the language-to-mask interface. Also creates TextureADE dataset from ADE20K.

Result: DETECTURE achieves the strongest performance on several datasets using different metrics compared to baselines.

Conclusion: Sub-semantic segmentation is a viable new category, and DETECTURE effectively resolves key failure modes in coupling vision-language models with segmentation backbones for this task.
...
TL;DR: Frozen SAM preserves texture-relevant evidence in multiscale features and proposal banks, but fails on texture segmentation due to readout mismatch and commitment issues rather than texture blindness.


<details>
  <summary>Details</summary>
Motivation: To understand why SAM fails on texture-defined segmentation and decompose the failure into representation, proposal, readout, and commitment factors.

Method: Probes frozen SAM's multiscale features with minimal clustering readout and automatic proposal bank with supervised consolidation readout, without any fine-tuning, across multiple texture datasets.

Result: Frozen SAM preserves texture organization in coarse features and contains texture-aligned masks/proposals. Synthetic cases need only selection, while natural scenes require assembly and commitment over fragments.

Conclusion: SAM's texture segmentation failure should be decomposed into representation evidence, proposal-bank support, readout mismatch, and commitment failure—not simply texture blindness.
...
TL;DR: Divide-and-Denoise coordinates multiple pre-trained diffusion models during sampling by creating a fair division of labor, using allocation games to assign each model responsibility over different regions of the noisy sample, outperforming baselines on conditional image generation.


<details>
  <summary>Details</summary>
Motivation: Combining multiple pre-trained diffusion models suffers from one model dominating or models disagreeing with each other. Existing composition methods lack a principled way to coordinate multiple models fairly and efficiently.

Method: At each timestep, (i) update allocation by solving a fair division game that divides the sample into regions maximizing total utility under fairness constraints, and (ii) align models with this allocation by guiding each model to denoise within its assigned region, creating a composite denoising process that evolves with a division process.

Result: Outperforms baselines on conditional image generation across multiple quality metrics including GenEval benchmark. Resolves common failures such as missing objects and mismatched attributes. Utilizes each model's expertise without neglecting any model.

Conclusion: Divide-and-Denoise effectively coordinates multiple diffusion models through fair division, preventing domination or disagreement issues, and achieves superior performance on compositional image generation tasks.
...
TL;DR: VIOLIN introduces a lightweight masked attention mechanism using Space Filling Curves to inject spatial inductive bias into Vision Transformers, improving performance with negligible extra parameters (0.0015%) and computational cost, especially benefiting small models and limited data scenarios.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers lack explicit spatial inductive biases due to permutation equivariance in their attention mechanism, which becomes problematic when model capacity is small or training data is limited.

Method: VIOLIN scans images using multiple Space Filling Curves (SFCs) to construct curve-specific decay masks, then combines and multiplies these masks with the attention matrix, requiring less than 0.0015% extra parameters.

Result: VIOLIN boosts accuracy by up to 8.7% on VTAB-1K fine-tuning, up to 7.2% on CIFAR-100 pixel-level training, and consistently improves DeiT/DINO pretraining on ImageNet-1K. It also combines well with LoRA for further gains.

Conclusion: VIOLIN provides a computationally efficient yet effective method to inject spatial inductive bias into ViTs, particularly benefiting small models and limited data settings.
...
TL;DR: This paper identifies "semantic hallucination" in XAI for Vision-Language Models, formalizes it as Linear Semantic Leakage in high-dimensional spaces, proposes a unified framework (LSA), and introduces OSP—a geometric intervention using orthogonal projection—to eliminate hallucination by removing shared/distractor semantic signals.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models are deployed in safety-critical applications, but their explainability methods suffer from semantic hallucination (e.g., highlighting a dog when asked "cat"). A formal mathematical analysis of this phenomenon is missing, and existing solutions are ad-hoc.

Method: The authors propose Linear Semantic Attribution (LSA), a unified theoretical framework generalizing across discriminative XAI methods. They introduce OSP (Orthogonal Semantic Projection), which leverages the residual property of OMP to orthogonalize the query vector against distractor concepts, thereby disentangling unique signals from shared ones.

Result: Both theoretical proofs and empirical demonstrations show that OSP minimizes hallucination by making the attribution model "blind" to shared/distractor features while preserving fidelity for correct prompts.

Conclusion: Semantic hallucination is a fundamental geometric consequence of Linear Semantic Leakage in high-dimensional spaces, not a model-specific artifact. OSP effectively addresses it via orthogonal projection.
...
TL;DR: This paper presents a text-to-video generative framework for cardiac MRI synthesis that decouples spatial structure from temporal motion using a diffusion model for initial frame generation and a latent flow model for motion, achieving high fidelity (FID=31.68) and text alignment (CLIP=31.04).


<details>
  <summary>Details</summary>
Motivation: Public cardiac MRI datasets are scarce, limiting the development of advanced data-driven models. The authors aim to generate synthetic cardiac sequences to address this data scarcity.

Method: A text-to-video framework that decouples cardiac spatial structure from temporal motion: (1) a fine-tuned diffusion model generates an initial frame from clinical text prompts; (2) a latent flow model conditioned on cardiac phase embedding generates complete cardiac motion, ensuring spatial consistency and temporal control.

Result: Achieved FID of 31.68 for image realism and CLIP score of 31.04 for text-image alignment, demonstrating high temporal coherence, anatomical consistency, and fidelity to input prompts.

Conclusion: The proposed method can produce high-fidelity, on-demand medical data, offering a scalable solution to data scarcity in cardiac MRI.
...
TL;DR: GeoRoPE introduces ground-aware positional calibration for remote-sensing foundation models, addressing scale mismatch across sensors and resolutions via Geo-Coordinate Calibration (GCC) and Geo-Frequency Calibration (GFC) with lightweight adapters.


<details>
  <summary>Details</summary>
Motivation: Remote-sensing foundation models suffer from scale mismatch during downstream adaptation because fixed token-grid offsets represent different ground distances across sensors, and heterogeneous spatial granularity demands different positional sensitivities even under the same GSD.

Method: GeoRoPE has two components: (1) Geo-Coordinate Calibration (GCC) rescales token-grid offsets based on ground distance per token step for cross-GSD consistency. (2) Geo-Frequency Calibration (GFC) adjusts RoPE frequency with a relation-specific factor for scene-dependent spatial granularity. Both are injected via a lightweight adapter into frozen pretrained RSFMs.

Result: Experiments across multiple RSFMs, sensors, resolutions, and downstream tasks show improved cross-resolution robustness and scale-sensitive representation learning.

Conclusion: GeoRoPE provides an effective, parameter-efficient solution for spatial adaptation in remote-sensing foundation models, making positional priors physically consistent across varying GSDs and spatial granularities.
...
TL;DR: Proposes an LLM-based framework for detailed video summarization that combines macro-level understanding with micro-level semantic analysis through transcript indexing, sentence-level analysis, and semantic grouping.


<details>
  <summary>Details</summary>
Motivation: Existing AI video summarization tools provide only coarse overviews from transcripts and lack detailed structural, thematic, and semantic analysis needed for comprehensive understanding of long-form video content.

Method: A two-stage LLM-based framework: (1) micro-level indexing by analyzing full transcript, then individual sentences with contextual continuity (global transcript + adjacent sentences), and (2) grouping sentences by semantic similarity using an LLM as a judge.

Result: Establishes a foundation for video analysis tools that visualize semantic chunking and semantic matching through relevance-based heatmaps.

Conclusion: The framework addresses the gap between coarse overviews and detailed video analysis, with limitations and future expansions discussed.
...
TL;DR: This paper proposes a new framework for minimizing submodular functions on distributive lattices without expanding to boolean lattices, achieving significant runtime improvements over traditional methods.


<details>
  <summary>Details</summary>
Motivation: Current methods for submodular function minimization on distributive lattices require transforming the function to boolean lattices, which exponentially expands the working space and makes optimization inefficient.

Method: The authors propose a generic framework that works directly within the distributive lattice, allowing established submodular minimization algorithms for boolean lattices to be applied without the costly transformation step.

Result: Experiments demonstrate huge improvements in running time compared to traditional methods that rely on boolean lattice transformation.

Conclusion: The proposed framework is an efficient alternative for submodular function minimization on distributive lattices, eliminating the exponential space overhead.
...
TL;DR: MoFore introduces a momentum-guided semantic forecasting framework for self-supervised video representation learning that predicts future latent embeddings from past context clips, combining predictive forecasting with contrastive regularization to learn temporally consistent representations without pixel-level reconstruction.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised video representation methods rely on either pixel-level reconstruction (e.g., MAE, VideoMAE) or task-specific semantic alignment (e.g., CLIP), which may not capture temporally predictive and semantically meaningful features efficiently. The authors aim to learn temporally predictive video representations without reconstruction or label supervision.

Method: MoFore forecasts future latent embeddings from temporally distant context clips using momentum-guided semantic forecasting, with randomized temporal-gap training for robustness. It combines predictive latent forecasting with contrastive regularization to maintain temporal consistency and prevent representation collapse.

Result: Experiments on UCF101 show strong temporal stability and emergent category-level structure in learned embeddings without action labels. Qualitative retrieval reveals motion-aware organization across related activities, demonstrating effective and computationally efficient representation learning.

Conclusion: Long-range latent forecasting is an effective and computationally efficient alternative to reconstruction-based self-supervised video representation learning, producing temporally consistent and semantically meaningful representations without relying on pixel-level objectives.
...
TL;DR: XMedFusion is a modular AI framework for autonomous medical systems that decomposes radiology report generation into coordinated agents (visual perception, knowledge graph construction, retrieval-guided drafting, synthesis) to improve visual grounding and diagnostic accuracy, achieving significant gains over baseline models on chest radiograph datasets.


<details>
  <summary>Details</summary>
Motivation: Existing end-to-end multimodal models for radiology report generation suffer from weak visual grounding, unreliable interpretations, and omission of subtle clinical findings, limiting their deployment in autonomous medical and robotic systems.

Method: XMedFusion is a modular framework with four coordinated agents: (1) a visual perception agent for image-grounded evidence extraction, (2) a knowledge graph construction agent for structuring clinical findings, (3) a retrieval-guided drafting process for consistent report structure, and (4) a synthesis agent that iteratively integrates evidence through reasoning-driven verification.

Result: On a public chest radiograph dataset, XMedFusion outperformed baseline vision-language models with gains of 0.0493–0.3359 in BLEU-1, 0.0863–0.2440 in ROUGE-L, 0.0829–0.1708 in METEOR, and substantial improvements in semantic metrics: Consistency (2.38 to 7.80) and Accuracy (2.34 to 6.93).

Conclusion: The structured multi-agent perception and reasoning approach enhances robustness, transparency, and automation in intelligent medical imaging systems, enabling reliable integration into autonomous healthcare and robotic diagnostic workflows.
...
TL;DR: This paper tackles class imbalance in Pedestrian Attribute Recognition by applying Multi-Label Focal Loss to a merged 109K-image dataset. The calibrated loss (alpha=0.50, gamma=2.0) matches BCE performance (Macro F1=62.32%) with better hard-example mining and zero computational overhead. The paper also identifies the "Sparsity Wall" where global reweighting fails for extremely rare attributes (below 0.1% positives).


<details>
  <summary>Details</summary>
Motivation: Extreme class imbalance in PAR datasets (some attributes <1% positive samples) causes standard BCE optimization to suppress rare traits—a problem called the "majority negative class cheating trap." The authors aim to address this without adding computational overhead.

Method: Systematic ablation of Multi-Label Focal Loss hyperparameters (alpha and gamma) on a ResNet-18 backbone, using a composite 109K-image corpus merged from PETA and PA-100K. Pure loss-function engineering with no extra computation.

Result: Calibrated configuration (alpha=0.50, gamma=2.0) achieves Macro F1-score of 62.32%, matching the BCE baseline, while offering superior hard-example mining and convergence dynamics. Identifies the "Sparsity Wall" at <0.1% positive fraction where global reweighting fails.

Conclusion: Multi-Label Focal Loss with proper calibration can match BCE performance in imbalanced PAR while being computationally free. However, global loss reweighting has a hard limit (Sparsity Wall), necessitating instance-level interventions for extremely rare attributes.
...
TL;DR: FG-EQA is a fine-grained active perception benchmark for outdoor EQA with 40K+ simulated and 1K real trajectories. ScoutVLA, inspired by bees' waggle dance, uses a decoupled dual-expert architecture (vision-language + action) with knowledge insulation to perform evidence-driven viewpoint refinement, achieving 10.48× higher success rate and 7.72× higher QA correctness over SOTA.


<details>
  <summary>Details</summary>
Motivation: Existing outdoor EQA systems stop once the target enters the UAV's field of view, failing to perform fine-grained viewpoint adjustments needed for evidence-seeking questions. This gap motivates the need for active, iterative perception behavior in UAVs.

Method: Proposes ScoutVLA, an evidence-driven Vision-Language-Action model with (1) a decoupled dual-expert architecture: a vision-language expert for semantic intent inference and an action expert using high-DoF flow matching for continuous trajectory generation; (2) a decoupled training strategy with knowledge insulation to protect multimodal reasoning from action gradients.

Result: ScoutVLA achieves 10.48× higher average strict success rate and 7.72× higher average QA correctness over state-of-the-art baselines in simulation; validated qualitatively in real-world field study.

Conclusion: Fine-grained active perception inspired by biological behavior (bee waggle dance) significantly improves EQA performance, and decoupled architecture with knowledge insulation effectively balances continuous control and semantic reasoning.
...
TL;DR: Double-Helix Vision (DH) is a geometry-based visual sampler that compresses images into 1D signals using spiral trajectories, achieving up to 1,433x compression ratio and running perception in 0.52ms on CPU-only hardware.


<details>
  <summary>Details</summary>
Motivation: To create an ultra-efficient, geometry-based visual sampling method that dramatically reduces data throughput while preserving spatial structure, without relying on neural networks or GPU acceleration.

Method: Uses two phase-shifted golden-ratio spiral trajectories (Alpha and Beta, 180° offset) to sample images with foveated density (high center, sparse periphery), then performs spatial mapping, temporal collision detection, and intra-frame disparity estimation on the compressed 1D signal.

Result: Achieves 1,433x compression at 4K (99.93% reduction), 0.52ms perception pipeline at 1080p on CPU, +6.03% accuracy gain over uniform random sampling on CIFAR-10 at K=128, and outputs 2.7KB JSON perception reports.

Conclusion: DH demonstrates that geometry-based spiral sampling can achieve extreme compression ratios and real-time performance on CPU-only hardware, offering a viable alternative to neural network-based perception for resource-constrained robotics applications.
...
TL;DR: JoyAI-VL-Interaction is an open-source 8B vision-first interaction model that continuously watches the world, decides when to speak or stay silent, and can delegate hard problems to a background model. Paired with a deployable system, it outperforms Doubao and Gemini across six real-world scenarios.


<details>
  <summary>Details</summary>
Motivation: Current large models are turn-based (require user queries), but many real-world moments (fires, expressions, fleeting products) don't wait for a prompt. The authors argue for a paradigm where models are continuously present and autonomously decide when to interact.

Method: They build an 8B vision-first VL-interaction model that internally decides each second whether to stay silent, respond, or delegate to a background model. They also release a complete deployable system with pluggable ASR/TTS, memory, UI, and a background brain that connects to any API/agent.

Result: Human raters strongly prefer JoyAI-VL-Interaction over Doubao and Gemini's video-call assistants across six real-world scenarios. Emergent capabilities (e.g., guiding shoppers through app screens, improvising lectures from slides) appeared without explicit training.

Conclusion: This is the first open, vision-driven interaction model released with its training recipe, data, and complete deployable system, advancing a new paradigm of continuously present AI.
...
TL;DR: FactCheck is a multi-agent framework for long-term action anticipation that uses a closed-loop "Observe-Plan-Verify" mechanism with Observer, Planner, and Verifier agents to ensure action feasibility, outperforming SOTA on EPIC-Kitchens-55 and EGTEA Gaze+.


<details>
  <summary>Details</summary>
Motivation: Existing open-loop LTA methods lack explicit mechanisms to verify action feasibility, often hallucinating non-existent objects, violating object affordances, or disregarding object states.

Method: FactCheck decomposes LTA into three specialized roles: (1) Observer - recognizes historical actions and builds dual-form structured memory (History Action Abstract + History Action Graph); (2) Planner - generates draft future actions conditioned on historical actions and abstract; (3) Verifier - validates draft against History Action Graph and refines infeasible actions.

Result: Consistently outperforms state-of-the-art methods on EPIC-Kitchens-55 and EGTEA Gaze+ benchmarks.

Conclusion: Establishes a new paradigm for feasibility-aware long-term action anticipation, effectively closing the loop of action recognition, prediction, and verification.
...
TL;DR: The paper presents YTClickbait21K, a large-scale human-annotated multimodal YouTube clickbait dataset with 21,238 videos from 40 channels across 29 countries, featuring structured metadata and thumbnail images labeled by three annotators each with substantial agreement (k=0.65), serving as a benchmark for multimodal clickbait detection.


<details>
  <summary>Details</summary>
Motivation: Clickbait content on video-sharing platforms undermines information reliability, and progress in automated detection has been limited by the lack of large-scale, high-quality multimodal datasets.

Method: The authors constructed YTClickbait21K by collecting 21,238 videos from 40 YouTube channels across 29 countries covering diverse categories. Each sample includes structured metadata (title, description, engagement stats) and thumbnail images. Three independent annotators labeled each video using a standardized decision framework incorporating textual, visual, and cross-modal consistency cues, with final labels determined by majority voting.

Result: The dataset achieved substantial inter-annotator agreement (Cohen's kappa = 0.65), confirming reliable labeling despite the inherent subjectivity of clickbait detection.

Conclusion: This dataset provides a robust benchmark for developing and evaluating machine learning models, facilitates research in cross-modal semantic understanding, and advances automated content moderation systems.
...
TL;DR: A model-based deep unfolding network for underwater image enhancement that integrates variational modeling with Mamba layers and a novel proximal trajectory loss, achieving improved visual quality and competitive performance against state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Underwater imaging data suffer from poor visibility and color distortion, which limits their applicability in ocean engineering.

Method: Proposes a variational-model-based deep unfolding network with: (1) multiplicative residual for artifact absorption, (2) nonlocal gradient constraint for edge preservation, (3) Mamba layers for capturing self-similarities, and (4) proximal trajectory loss for stage-wise consistency.

Result: Achieves improved visual quality and competitive quantitative results compared to recent state-of-the-art methods.

Conclusion: Integrating variational modeling with deep unfolding and Mamba layers is an effective approach for underwater image enhancement, with theoretical soundness and practical performance gains.
...
TL;DR: BACON is a plug-and-play method that improves multimodal KV cache compression by calibrating observation window attention with last-query attention, using intra-layer coherence and inter-layer persistence to suppress noise, achieving 7.5% average improvement under aggressive budgets.


<details>
  <summary>Details</summary>
Motivation: Existing KV cache compression methods for MLLMs rely on observation window attention, which can dilute sparse visual evidence and discard answer-critical tokens under aggressive compression.

Method: BACON calibrates observation window attention with complementary last-query attention evidence, and suppresses isolated noise via intra-layer coherence and inter-layer persistence mechanisms.

Result: BACON improves multimodal KV compression by 7.5% on average under the most aggressive budget, with gains up to 30.9% across diverse benchmarks, models, budgets, and compression methods.

Conclusion: BACON effectively addresses the limitations of existing compression methods by recovering answer-critical tokens lost in observation window attention, making it a robust plug-and-play solution for multimodal KV cache compression.
...
TL;DR: Encoder-free VLMs route image patches directly into the language token stream, creating a privacy side channel where visual tokens can be inverted to recover readable text and images, unlike encoder-based models. The leakage stems from high spatial sampling fidelity, survives defenses like noise and quantization, and exists in KV caches, making vision encoder removal a key privacy consideration.


<details>
  <summary>Details</summary>
Motivation: To identify and characterize a novel privacy attack surface introduced by encoder-free vision-language models, where the absence of a vision encoder exposes intermediate visual tokens as a side channel that can leak sensitive image content.

Method: Token-access adversaries perform decoding inversion on visual-token streams from two encoder-free VLMs (Gemma4 and Fuyu), compared against encoder-based controls. Within-model ablations isolate the operative factor (spatial sampling fidelity), and attacks are tested against KV cache tensors, document degradation, zero-shot transfer, and value-level defenses.

Result: Encoder-free VLMs leak recognizable image structure and readable access codes; encoder-based models localize regions but recover no exact strings. The key factor is spatial sampling fidelity (character-direction density), not token count. Layer-0 KV caches are also directly invertible. Attacks survive clutter, degradation, zero-shot transfer, and resist additive noise/quantization.

Conclusion: Removing the vision encoder is a first-class privacy decision in VLM deployment. Effective mitigation must reduce spatial sampling fidelity rather than rely on value-level defenses.
...
TL;DR: Six image-to-image AI systems were tested using adversarial perturbations and showed two distinct behavioral clusters: edit-trained models (Flux Kontext, Qwen Edit, Gemini) and T2I-base models adapted at sampling time (SDXL, SD3, gpt-image-1).


<details>
  <summary>Details</summary>
Motivation: To systematically compare and characterize behavioral differences across production image-to-image AI systems under adversarial conditions.

Method: Applied content-adaptive sub-JND adversarial perturbation pipeline; scored outputs using frozen DINOv2 ViT-B/14 token distances; analyzed behavior on a 2D (patch_mean, ssim_clean) plane across 3,588 calls covering COCO, CelebA-HQ, and AI-generated inputs.

Result: Six systems partition into two image-invariant behavioral bands: edit-trained models cluster in a tight band, T2I-base models adapted at sampling time cluster in a drift band.

Conclusion: There is a fundamental behavioral dichotomy between edit-trained and T2I-base image-to-image models under adversarial perturbations.
...
TL;DR: This paper shows that multimodal discrete diffusion models can replace autoregressive models for RL-based post-training in interleaved visual-textual reasoning, achieving 26.9% computation savings via localized editing. It also proposes factorized reward assignment to address cross-modal interference, yielding 11.2% improvement over joint reward assignment and 38.04% over the base model.


<details>
  <summary>Details</summary>
Motivation: Existing RL-based post-training for interleaved visual-textual reasoning relies on autoregressive unified models, which require full image regeneration during visual reasoning—this is computationally expensive. The authors aim to find a more efficient alternative.

Method: (1) Use multimodal discrete diffusion models instead of AR models for efficient visual rollouts via localized editing. (2) Propose factorized reward assignment that assigns rewards independently to text and vision segments to avoid cross-modal interference.

Result: (1) Reduces rollout computation during GRPO by 26.9% compared to AR baselines with minimal performance drop. (2) Factorized reward assignment achieves 11.2% improvement over joint reward assignment and 38.04% improvement over the base model.

Conclusion: Multimodal discrete diffusion models are effective and efficient alternatives to AR models for RL-based post-training in interleaved reasoning, and factorized reward assignment mitigates cross-modal interference, significantly boosting performance.
...
TL;DR: This paper identifies that VLMs lack Agency for implicit reasoning (autonomous visual exploration), introduces the V-IRD benchmark to measure this gap, and shows that strong semantic recognition ≠ active visual exploration.


<details>
  <summary>Details</summary>
Motivation: Current VLMs treat visual reasoning as passive semantic retrieval rather than active, situated reasoning requiring autonomous visual exploration, leaving implicit reasoning capabilities largely unmeasured.

Method: The authors introduce the Visual Implicit Reasoning Diagnosing Benchmark (V-IRD), which requires models to derive answers strictly through autonomous visual analysis rather than relying on explicitly specified targets.

Result: Prominent VLMs struggle to utilize reference objects and attend to visual evidence requiring self-directed inquiry, despite having strong retrieval abilities. Strong semantic recognition does not equate to active visual exploration.

Conclusion: A systemic lack of Agency constrains implicit reasoning in VLMs, revealing a critical gap that current benchmarks fail to measure.
...
TL;DR: HSQ-VLM proposes a quadrant segmentation pipeline for DR diagnosis that uses Landmark-Anchored Cartesian Cross-Attention and Topological Latent Partitioning to generate anatomically precise, explainable reports, achieving 99.6% hemorrhage and 96.4% microaneurysm sensitivity.


<details>
  <summary>Details</summary>
Motivation: Current DR diagnostic AI models are black-box systems lacking explainability about anatomical landmarks and lesion distributions that lead to clinical decisions.

Method: HSQ-VLM uses a quadrant segmentation pipeline with Landmark-Anchored Cartesian Cross-Attention and 4-quadrant Topological Latent Partitioning (TLP) aligned to a fovea-centered coordinate system, enabling Vision-Language Models to generate precise natural language reports.

Result: On 3,500 fundus images: 99.6% hemorrhage detection sensitivity, 96.4% microaneurysm detection sensitivity, and significant reduction in boundary-ambiguity errors over baselines.

Conclusion: HSQ-VLM successfully unifies visual feature extraction with structured clinical reasoning, providing explainable DR diagnosis with high accuracy and anatomical precision.
...
TL;DR: WireframeDETR is a DETR-style model that directly predicts 3D building wireframes from multi-view COLMAP point clouds without intermediate vertex detection, achieving 0.575 HSS on the S23DR 2026 Challenge test set.


<details>
  <summary>Details</summary>
Motivation: To address the Structured Semantic 3D Reconstruction (S23DR) 2026 Challenge by predicting 3D building wireframes from multi-view COLMAP point clouds.

Method: Applies DETR-style set prediction directly to 3D point clouds, producing wireframes as sets of edge coordinate pairs without intermediate vertex detection. Three contributions: (1) contrastive denoising training, (2) multi-scale encoder with learned scalar weights, (3) progressive auxiliary loss weighting.

Result: Public test HSS of 0.575 (F1=0.664, IoU=0.516), best validation HSS of 0.534 on cleaned val split.

Conclusion: WireframeDETR successfully demonstrates that direct set prediction for 3D wireframes from point clouds is viable, with three technical contributions improving training stability and performance.
...
TL;DR: Multi-HMR 2 is a DETR-based framework that jointly predicts scene-consistent cameras and human meshes, achieving metric 3D localization and tracking without relying on fixed camera assumptions, handcrafted post-processing, or video supervision.


<details>
  <summary>Details</summary>
Motivation: Existing HMR methods focus on pelvis-centered recovery but neglect metric 3D localization and detection accuracy in camera coordinates, which are critical for real-world applications like human-robot interaction and social scene understanding.

Method: A DETR-based framework that jointly predicts a scene-consistent camera with human meshes, and distills image-based memory features from SAM2 to enable tracking without video supervision.

Result: Achieves state-of-the-art pelvis-centered performance while substantially improving detection accuracy and metric 3D localization.

Conclusion: Despite its simplicity (no handcrafted components, no video input, no ground-truth cameras), Multi-HMR 2 robustly addresses camera-space perception and tracking for real-world deployment.
...
TL;DR: This paper proposes an ensemble of InceptionV3 and MobileNetV2 with adversarial training for lemon leaf disease detection, achieving 99.27% accuracy on a 9-class (8 diseases + 1 healthy) dataset of 1354 images.


<details>
  <summary>Details</summary>
Motivation: Early detection of plant diseases is critical to prevent yield loss, fruit quality reduction, and increased susceptibility to other stresses.

Method: Two pretrained CNNs (InceptionV3 and MobileNetV2) were ensembled, combined with adversarial training for robustness, and evaluated using Grad-CAM for interpretability on a 1354-image lemon leaf dataset split 70/15/15.

Result: The ensemble model achieved 99.27% accuracy.

Conclusion: The ensemble with adversarial training provides reliable and interpretable plant disease classification, robust to noisy data.
...
TL;DR: This paper provides a theoretical framework for understanding cross-modal (vision-language) contributions in continual learning settings, demonstrating that modality-specific analysis helps explain and improve robustness to task order and inter-task similarities in vision-language models.


<details>
  <summary>Details</summary>
Motivation: While continual learning in VLMs has been studied, the theoretical understanding of how vision and language modalities individually contribute across sequential tasks remains unexplored, motivating the need for a cross-modal theoretical perspective.

Method: The authors present a new theoretical perspective to analyze cross-modal contributions in continual VLMs, empirically evaluating their framework on large vision-language models to capture environment-level modality-specific contributions.

Result: The theoretical findings empirically demonstrate effectiveness in capturing cross-modal contributions, showing robustness to varying task orders and inter-task similarities, along with improved generalization performance.

Conclusion: Understanding modality-specific contributions provides deeper insights into continual VLMs, highlighting that cross-modal analysis can improve robustness and generalization in sequential learning settings.
...
TL;DR: This paper proposes an improved Knowledge Distillation framework using VGG16 as teacher and MobileNetV2 as student, combining KL divergence and Cosine Similarity losses for land-use image classification, achieving 99.04% accuracy with efficient model compression.


<details>
  <summary>Details</summary>
Motivation: To achieve competitive land-use classification accuracy while reducing computational complexity through efficient model compression.

Method: Teacher-student learning (VGG16 → MobileNetV2) integrating hard supervision (ground truth labels) with soft supervision (KL divergence + Cosine Similarity losses).

Result: Achieved 99.04% accuracy on three land-use datasets, outperforming baseline student training and single-loss distillation approaches.

Conclusion: The proposed KD framework effectively balances high accuracy and model compression for land-use classification, surpassing both standard training and simpler distillation methods.
...
TL;DR: FedSaltNet introduces a Federated Learning framework with a lightweight Small U-Net and Foreground-Weighted aggregation strategy for privacy-preserving salt-dome segmentation, achieving 4% IoU improvement over conventional FL methods across four seismic datasets.


<details>
  <summary>Details</summary>
Motivation: Salt-dome delineation is critical for hydrocarbon exploration and drilling safety, but existing automated segmentation methods face severe limitations from data sovereignty concerns, dataset bias, and scarcity of labeled seismic volumes.

Method: FedSaltNet combines a lightweight Small U-Net backbone (chosen for efficiency and regularization) with a novel Foreground-Weighted (FG-WEIGHTED) aggregation strategy to address domain-specific class imbalance in non-IID federated settings.

Result: FG-WEIGHTED achieved 4.0% relative IoU improvement over the best conventional FL method, and the simple U-Net outperformed ResNet-18 U-Net by 166% in average IoU across four diverse seismic datasets (TGS, SEAM, F3, GBS).

Conclusion: FedSaltNet establishes the viability of federated deep learning for collaborative, next-generation subsurface interpretation, demonstrating that architectural simplicity is crucial in data-constrained federated environments.
...
TL;DR: The paper proposes a mask proposal voting framework for minimal path-based segmentation that overcomes initialization sensitivity by generating adaptive domain cuts for diverse mask proposals and using a weighted voting scheme with priors, achieving robust and accurate segmentation in complex scenarios.


<details>
  <summary>Details</summary>
Motivation: Minimal path-based segmentation methods are highly sensitive to model initialization, which limits their practical applicability in cluttered backgrounds, complex intensity variations, and challenging topology appearance scenarios.

Method: 1) An efficient method for constructing adaptive domain cuts that constrain region-based min-cut evolution to generate diverse and reliable mask proposal candidates. 2) A novel mask voting scheme that builds a voting score map incorporating priors to assign different importance to each individual mask, unlike classical path voting methods.

Result: The proposed method consistently outperforms state-of-the-art minimal path-based approaches in both accuracy and robustness across complex scenarios.

Conclusion: The mask proposal voting framework effectively eliminates initialization sensitivity of minimal path models, enabling accurate object boundary delineation under complex conditions.
...
TL;DR: FlexPooling is an adaptive pooling method that learns weighted averages over activations, outperforming standard pooling by 1-3% accuracy on image classification tasks.


<details>
  <summary>Details</summary>
Motivation: Pooling is essential in CNNs but is lossy; standard dense pooling methods (max, average, strided conv) may not preserve the most prominent information needed for discriminability.

Method: Proposes FlexPooling, which generalizes average pooling by learning a weighted average over activations jointly with network training. Also uses Simple Auxiliary Classifiers (SAC) to boost performance.

Result: FlexPooling consistently improves accuracy by 1-3% over baseline networks on multiple popular image classification datasets.

Conclusion: FlexPooling is a simple yet effective adaptive pooling method that outperforms standard pooling approaches, especially when combined with SAC.
...
TL;DR: Neuro-JEPA is a multimodal neuroimaging foundation model pretrained on over 1.5 million scans across T1w, T2w, and FLAIR sequences, using a latent predictive objective and Mixture-of-Experts architecture. It outperforms existing foundation models across 47 tasks from 3 health systems and 12 public datasets, while highlighting that prior models showed inconsistent gains over simple CNN baselines.


<details>
  <summary>Details</summary>
Motivation: Current methods lack the ability to learn unified representations across multiple MRI contrast mechanisms (T1w, T2w, FLAIR) at health-system scale. Existing neuroimaging foundation models also show inconsistent improvements over simple baselines, motivating a more robust multimodal framework.

Method: Neuro-JEPA combines a latent predictive objective with a Mixture-of-Experts (MoE) architecture to encode core structural brain MRI sequences (T1w, T2w, FLAIR). The authors systematically study architectural, masking, objective, and sparsity design choices. The model is pretrained on 1,551,862 scans from 428,647 studies across three sequences.

Result: Evaluated on 25 tasks from three health systems and 22 tasks from 12 public datasets (unimodal, multimodal, cross-domain). Existing neuroimaging foundation models showed inconsistent gains over a simple CNN baseline, whereas Neuro-JEPA achieved stronger and more consistent performance across all settings.

Conclusion: Neuro-JEPA establishes a scalable methodological framework for multimodal neuroimaging representation learning. The results emphasize the need for evaluation protocols that include simple baselines, clinically heterogeneous cohorts, and controlled multimodal comparisons when assessing foundation models.
...
TL;DR: MVEB is a 23-task benchmark for evaluating video embeddings across 33 models, revealing that no single model type dominates all tasks, and that audio's contribution depends on annotation modality.


<details>
  <summary>Details</summary>
Motivation: To create a comprehensive, standardized benchmark for video embeddings that covers diverse tasks and enables systematic comparison of models, while reducing evaluation cost.

Method: Constructed MVEB (23 tasks) from MVEB+ (184 tasks) pool, evaluating 33 models on classification, zero-shot classification, clustering, pair classification, retrieval, and video QA tasks. Analyzed audio contribution via paired video-only vs. audio+video evaluations.

Result: No single model dominates; MLLM-based embeddings excel at classification, clustering, pair classification, and QA; multimodal binding leads on retrieval and zero-shot classification. Audio helps when labels come from both modalities but hurts when from visuals alone (6-point gap).

Conclusion: MVEB provides a unified video embedding benchmark integrated into MTEB ecosystem, revealing task-specific model strengths and modality annotation biases.
...
TL;DR: A deep learning framework using multi-modal attention on pre/post-disaster satellite imagery achieves 94.90% accuracy in classifying building damage into four levels, enabling faster and more reliable disaster assessment.


<details>
  <summary>Details</summary>
Motivation: Traditional disaster damage assessment relies on manual inspections or sparse data, which are slow and error-prone. There is a pressing need for automated, accurate, and timely methods to support emergency response and resource allocation.

Method: The framework uses pre- and post-disaster satellite imagery with a multi-modal attention mechanism (cross-attention module) to fuse bi-temporal features and detect structural changes. A lightweight ConvNeXT-Tiny backbone ensures efficiency, with optimized preprocessing and robust data augmentation techniques.

Result: Experiments on a large-scale disaster dataset achieve 94.90% overall classification accuracy. The model effectively discriminates among four damage categories (no damage, minor, major, destroyed) and remains resilient to incomplete data.

Conclusion: The proposed system significantly improves assessment speed and accuracy over traditional methods, offering a scalable solution for real-time disaster response and advancing automated damage detection through multi-temporal deep learning.
...
TL;DR: ReGenHuman is a full-body video anonymization pipeline that regenerates human regions from identity-free structural cues using video diffusion, achieving realistic, temporally consistent, and anonymous results superior to blurring/redaction baselines.


<details>
  <summary>Details</summary>
Motivation: Human-centric video anonymization is understudied; existing methods sacrifice realism, temporal coherence, or downstream utility by blurring, redacting, or frame-by-frame generation.

Method: Proposes a "regenerate, don't edit" paradigm: composites 2D pose, segmentation, and monocular depth into two conditioning streams (StructAll and StructHuman) to fine-tune a video-to-video diffusion backbone, synthesizing human regions from identity-free structural cues.

Result: ReGenHuman achieves the best tradeoff across privacy, quality, and utility compared to baselines; anonymized videos remain effective for downstream tasks like video question answering.

Conclusion: Regenerating human regions from structural cues instead of editing inputs yields realistic, temporally coherent, and anonymous videos that preserve utility.
...
TL;DR: NEXUS is a neural energy-field framework for contact-rich 3D object dynamics that models both conservative and non-conservative forces through scalar energy and dissipation terms, achieving improved long-horizon accuracy and enabling physically plausible video generation.


<details>
  <summary>Details</summary>
Motivation: Existing trajectory-based methods for physics-grounded video generation model isolated physical effects, making it difficult to compose conservative and non-conservative dynamics in contact-rich 3D scenes.

Method: NEXUS represents objects as structural graphs with dynamic contact graphs, formulates motion via Hamiltonian-inspired scalar energy and dissipation terms (gravity, elastic deformation as additive energy; damping, impact loss as Rayleigh dissipation), and rolls out dynamics with a multi-substep semi-implicit integrator.

Result: NEXUS improves long-horizon trajectory accuracy over learned and physics-structured baselines under varying mechanical properties. NEXUS trajectories improve physical plausibility in contact-rich video generation while maintaining competitive visual quality.

Conclusion: Modeling object dynamics through composable energy and dissipation terms is effective for accurate long-horizon simulation of contact-rich 3D scenes and benefits downstream video generation tasks.
...
TL;DR: A deep learning model for cervical cancer screening trained on colposcopy images, outperforming experts on in-distribution data but showing high performance variability across four countries (AUC 0.54-0.80), with comorbidities negatively impacting accuracy.


<details>
  <summary>Details</summary>
Motivation: To address the global need for cervical cancer screening access in low- to middle-income countries (LMICs), where expert and biopsy services are limited.

Method: Multi-task learning approach combining image-level classification and lesion segmentation on acid stain colposcopy images, trained with extensive data augmentation.

Result: Outperformed medical experts on in-distribution validation (Balanced Accuracy: 0.68 vs 0.64). External validation across four countries showed AUC ranging from 0.54 to 0.80. Performance affected by age, transformation zone, comorbidities, and pathognomonic signs.

Conclusion: First DL-based cervical cancer screening validated on multi-country data. While promising, model robustness and generalizability need improvement due to high cross-country variability and negative impact of comorbidities.
...
TL;DR: The paper proposes a Gaussian Spatial Prior (GSP) module that encodes anatomical spatial relationships to improve detection of visually ambiguous structures (like epigastric vessels) in surgical video, achieving +33.5% AP50 improvement over DAB-DETR and +53.9% over YOLOv6 on inguinal hernia repair datasets.


<details>
  <summary>Details</summary>
Motivation: Small, visually ambiguous anatomical structures (e.g., epigastric vessels) are difficult to detect reliably in surgical video using standard methods, yet their identification is critical for intraoperative safety frameworks like CVMPO.

Method: A Gaussian Spatial Prior (GSP) module that encodes anatomically constrained spatial relationships as parametric biases injected into DAB-DETR's self-attention. The prior is computed offline from training annotations as frozen Gaussian parameters and updated per decoder layer using iteratively refined reference points.

Result: GSP improves dependent class detection by +33.5% (AP50) over DAB-DETR and +53.9% over YOLOv6, and anchor detection by +6.0%. Results are statistically significant (p=0.012) across 5-fold cross-validation.

Conclusion: Encoding anatomical spatial priors via Gaussian parameters in transformer attention significantly boosts detection of visually challenging structures in surgical video, offering a practical solution for intraoperative safety.
...
TL;DR: HVSP-LL is a lifelong learning framework for urban streetscape perception that uses hierarchical semantic anchoring and equity-aware rehearsal to reduce geographic bias, achieving a 38% reduction in inter-city perception gap compared to strongest baselines.


<details>
  <summary>Details</summary>
Motivation: Models trained on well-photographed metropolises systematically misjudge underrepresented urban districts, propagating geographic bias into downstream policy decisions in landscape planning, public health, and place-making.

Method: Proposes HVSP-LL with two components: (1) a stratified visual-semantic pivoting module organizing landscape concepts into three-tier ontology (macro/meso/micro) aligning features to learnable semantic anchors; (2) an equity-aware rehearsal mechanism with worst-region sample-reweighting and structurally-aware exemplar buffer for lifelong adaptation.

Result: Achieves 0.834 Spearman correlation on held-out city sequence, 6.1 point improvement over strongest continual baseline; reduces inter-city perception gap to 0.094 (38% reduction vs continual baseline, 57% vs regularization baseline); eliminates catastrophic forgetting (mean backward transfer from -0.038 to +0.013).

Conclusion: Hierarchical anchoring provides a practical pathway toward geographically equitable streetscape inference at city scale.
...
TL;DR: This paper presents the first systematic study on synthetic-to-real domain adaptation for semantic segmentation in NIR automotive images. It proposes a generative augmentation framework using target style adaptation (TSA) with latent diffusion models and Voronoi-based style diversification (VSD), achieving up to 63.6% (exterior) and 28.4% (interior) domain gap reduction.


<details>
  <summary>Details</summary>
Motivation: NIR imaging enables stable detection under poor lighting, but lacks high-quality real annotated data. Synthetic data is scalable but suffers domain shift when applied to real NIR images. The paper aims to bridge this gap.

Method: 1) Target Style Adaptation (TSA): fine-tunes a latent diffusion model via low-rank adaptation on a small set of real NIR images; 2) Structure-preserving multi-signal conditioning to transform synthetic images into realistic NIR variants; 3) Voronoi-based Style Diversification (VSD) to reduce texture bias while preserving geometry.

Result: Domain gap reduced by up to 63.6% on exterior data and 28.4% on interior data. Multiple architectures validated on NIR vehicle interior and street scenes.

Conclusion: Balancing inductive bias during training with the proposed generative augmentation leads to more robust semantic segmentation and effectively reduces the synthetic-to-real domain gap for NIR automotive images.
...
TL;DR: AVA-VLA introduces latent reasoning with reinforcement learning denoising and early-exit strategy, achieving 6x speedup and 98.3% success rate on LIBERO, outperforming explicit CoT methods.


<details>
  <summary>Details</summary>
Motivation: Explicit CoT reasoning in VLA models suffers from high computational costs and error propagation in multi-step tasks.

Method: 1) Latent reasoning with unobservable variables; 2) RL-based denoising treating latent state generation as sequential decision process; 3) Early-Exit Strategy for adaptive reasoning depth.

Result: 6x inference speedup over explicit CoT; 98.3% average success rate on LIBERO; improved long-horizon stability.

Conclusion: Latent reasoning with RL denoising and early-exit can replace explicit CoT, improving both efficiency and robustness in embodied decision-making.
...
TL;DR: A text-driven fusion framework using hyperbolic manifold learning (TEDFusion) that leverages BLIP-extracted text prompts as topological anchors in hyperbolic space to improve infrared and visible image fusion, outperforming existing methods without needing text at inference.


<details>
  <summary>Details</summary>
Motivation: Existing Euclidean methods impose rigid distance metrics that distort multi-modal interactions and parent-to-child semantic hierarchies in infrared and visible image fusion.

Method: Uses BLIP-extracted text prompts as topological anchors in hyperbolic space (Poincaré ball) for vision-attribute alignment, exploiting hyperbolic geometry's exponential volume growth to encode coarse-to-fine semantics and prevent texture distortion during cross-modal fusion.

Result: Outperforms state-of-the-art approaches on benchmark datasets.

Conclusion: Hyperbolic manifold learning with text-driven priors effectively addresses the limitations of Euclidean distance metrics for multi-modal image fusion, and the method works autonomously at inference without textual input.
...
TL;DR: A physics-driven zero-shot self-supervised learning framework for MRI reconstruction that uses CSM-guided dynamic repository, SPIRiT-based regularization, and Non-Local Self-Similarity Pixel Bank to achieve state-of-the-art performance without fully-sampled external data.


<details>
  <summary>Details</summary>
Motivation: Zero-shot self-supervised MRI reconstruction suffers from supervision scarcity and optimization instability when learning from a single under-sampled scan, leading to overfitting and artifacts.

Method: Three innovations: (1) CSM-Guided Dynamic Repository filters artifacts using coil sensitivity constraints; (2) SPIRiT-based regularization enforces k-space self-consistency via learned correlation kernel and stochastic masking; (3) Non-Local Self-Similarity Pixel Bank mines non-local anatomical similarities from high-fidelity references.

Result: State-of-the-art performance on FastMRI dataset, especially under high acceleration factors, bridging the gap between zero-shot and supervised methods.

Conclusion: The proposed framework effectively addresses overfitting and artifact issues in zero-shot self-supervised MRI reconstruction by synergizing physical consistency with image-domain non-local priors.
...
TL;DR: The paper proposes TCL, a satellite video object detection framework using Temporal Consistency Learning to detect oriented and fine-grained objects, achieving 47.7% mAP on SAT-MTB benchmark—a 4.8% improvement over baseline.


<details>
  <summary>Details</summary>
Motivation: Existing SVOD methods focus on coarse-grained categories with horizontal bounding boxes, failing to extract complete, accurate, and consistent information about oriented and fine-grained objects in satellite videos.

Method: TCL framework with three modules: Temporal and Fine-grained Feature Aggregation (TFA), Structure Encoding (SE), and Temporal Consistency Constraint (TCC), leveraging temporal contexts for consistent representation learning across frames.

Result: State-of-the-art 47.7% mAP on SAT-MTB benchmark (4.8% improvement over baseline). The framework also improves existing image-based detectors when integrated.

Conclusion: Temporal consistency learning effectively improves oriented and fine-grained object detection in satellite videos, and the framework is adaptable to existing detectors.
...
TL;DR: The paper proposes a Multi-view Feature High-order Fusion (MHF) method that extends low-order feature fusion to higher orders for better representation of weak objects in space images/videos. It uses multi-view attentions, high-order perception, and recursive task-contribution gated selection to enhance feature aggregation. MHF is a plug-and-play module that improves various vision models and achieves SOTA on space science and satellite video datasets.


<details>
  <summary>Details</summary>
Motivation: Weak objects (e.g., small or low-contrast objects in space images) lack sufficient appearance information for learning robust representations. Existing low-order feature fusion methods fail to capture rich, complementary multi-view features for such objects.

Method: (1) Multi-view attentions are used to generate multi-view features. (2) A Multi-view Feature High-order Fusion (MHF) method extends low-order fusion to higher orders. (3) High-order multi-view feature perception and a recursive task-contribution gated selection mechanism are introduced to capture relevant information.

Result: MHF achieves state-of-the-art accuracies on two newly constructed space science datasets and one large-scale satellite video dataset for both detection and segmentation tasks. It significantly improves various vision transformers and CNN-based models.

Conclusion: MHF is a flexible, plug-and-play module that effectively represents weak objects through multi-view learning and can serve as a new basic module for visual modeling.
...
TL;DR: EyeMVP is a cross-modal retinal foundation model pretrained on 674k paired CFP-OCT images that enriches color fundus photography representations with OCT-derived structural information via cross-modal masked reconstruction, achieving strong performance across 16 downstream tasks and outperforming existing models on macular diseases.


<details>
  <summary>Details</summary>
Motivation: CFP is widely used for retinal screening but lacks depth-resolved structural information (present in OCT), while OCT is less accessible for population-level screening. The authors aim to bridge this gap by learning OCT-informed representations from CFP alone at inference time.

Method: EyeMVP uses cross-modal masked reconstruction pretraining on 674,893 same-eye same-day paired CFP-OCT triples from 112,642 patients. It employs source-constrained cross-attention with CFP-derived structural masks to handle non-aligned geometry between en-face CFP and cross-sectional OCT.

Result: Outperforms representative retinal foundation models across 16 downstream tasks (classification, segmentation, few-shot, retrieval). Achieves AUROC of 0.948 for macular edema (vs. 0.852 for EyeCLIP) and 0.825 for myopic macular schisis. Exceeds junior and intermediate ophthalmologists on macular edema and shows numerically higher accuracy than all reader groups on myopic macular schisis.

Conclusion: Pixel-level cross-modal reconstruction can enrich CFP representations with OCT-associated supervision, offering a practical path toward stronger CFP-based retinal analysis in screening settings.
...
TL;DR: SAGA uses an MLLM's attribute-aware language feedback (via GRPO) to train a vision encoder for retrieval, replacing uniform class-label supervision with attribute-resolved supervision. It improves Recall@1 by 3-6 points on fine-grained datasets while keeping deployment cost equal to standard metric-learning baselines.


<details>
  <summary>Details</summary>
Motivation: Standard vision encoders use class-label supervision where each pair is treated uniformly (pull together or push apart) regardless of which visual attributes actually differ or match. This scalar-level supervision fails to capture fine-grained attribute differences.

Method: (1) Use GRPO to reward a frozen MLLM for correct predictions based on the vision encoder's tokens, pushing the encoder to expose specific matching/differing attributes. (2) Auxiliary attention-distillation loss anchors encoder embeddings to tokens the MLLM attended to. (3) Standard metric-learning loss shapes embedding geometry for retrieval.

Result: SAGA improves Recall@1 by 3 to 6 points over state-of-the-art baselines on CUB-200-2011, Cars-196, FGVC-Aircraft, and iNaturalist Aves in zero-shot image retrieval.

Conclusion: Attribute-resolved supervision from a frozen MLLM can significantly improve vision encoders for retrieval without increasing inference cost, outperforming class-label-based metric learning approaches.
...
TL;DR: DSFT splits motion into Base (low-frequency pose) and physical (high-frequency velocity) streams using DCT truncation and BPE. MotionVLA uses a Qwen3.5-based autoregressive model predicting physical tokens after base tokens, reducing the Diversity gap by 50% on HumanML3D and improving Motion-Condition Consistency by 3.8% on MBench.


<details>
  <summary>Details</summary>
Motivation: Existing methods use a single shared codebook to tokenize human motion, forcing heterogeneous signals (low-frequency pose semantics and high-frequency physical dynamics) into the same quantization space, causing under-representation of high-frequency velocity components.

Method: DSFT (dual-stream frequency tokenizer) separates motion into Base and physical streams, compressing them independently via DCT truncation and BPE. MotionVLA uses a Qwen3.5-based autoregressive model that arranges Base and physical tokens in a unified sequence, predicting physical tokens after base tokens.

Result: On HumanML3D: reduces Diversity gap to real data by over 50%. On MBench: improves Motion-Condition Consistency by 3.8%. All with a lightweight 2B backbone.

Conclusion: Frequency-aware dual-stream decoupling is an effective formulation for autoregressive motion generation, better handling both pose semantics and physical dynamics.
...
TL;DR: HiRo is a parameter-efficient image classification model that uses shifted-window partitioning and hierarchical multi-directional reservoir computing to achieve strong accuracy (99.46% MNIST, 85.57% CIFAR-10, 59.10% CIFAR-100) with under 1M trainable parameters, outperforming transformer-style baselines in efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing high-performing image classification models rely on fully trainable token-mixers that improve representation learning but increase parameter count, optimization complexity, and computational cost. The authors aim to design a parameter-efficient alternative.

Method: HiRo divides images into patches, uses 2D sinusoidal positional encodings, processes tokens within local windows via four-directional scanning, employs a two-stage slice-and-mix reservoir module (fixed reservoirs + trainable closed-loop readouts), alternates between regular and shifted windows for cross-window interaction, and applies global pooling for classification.

Result: HiRo achieves 99.46% on MNIST, 85.57% on CIFAR-10, and 59.10% on CIFAR-100 with under 1M trainable parameters and significantly lower memory and time than transformer-style baselines.

Conclusion: The combination of shifted-window partitioning and hierarchical multi-directional reservoir computing provides an efficient local-to-cross-window token-mixing framework that balances performance and parameter efficiency for image classification.
...
TL;DR: This paper introduces RefGC-SR², a new task that uses the original high-resolution reference image at post-processing to simultaneously recover lost details, refine generative artifacts, and upscale outputs in reference-guided generation.


<details>
  <summary>Details</summary>
Motivation: Current reference-guided generation pipelines downsample high-resolution reference images to low-resolution before processing, causing loss of fine-grained details, and generative models introduce artifacts (e.g., identity distortion) that existing refinement methods cannot fully address as they operate in the LR domain.

Method: The authors propose a new task (RefGC-SR²) with a real-world triplet data generation pipeline and a diptych-conditioned generator to synthesize paired low-quality anchors. They present a frequency-aware diffusion transformer that selectively injects fine details from the HRRI while removing generative artifacts.

Result: The RefGC-SR² model faithfully refines object identity with respect to the reference and recovers high-resolution details, achieving significantly higher quality and practical usability compared to existing RefGCR and RefSR baselines.

Conclusion: The proposed method successfully addresses both detail loss and artifact issues in reference-guided generation through a unified post-processing formulation that leverages the original high-resolution reference.
...
TL;DR: DLWM proposes a multimodal reasoning framework that maintains diverse latent hypotheses of visual inputs and uses reinforcement learning to adaptively allocate computation across them, improving accuracy by 2-5% and reducing memory by 24%.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM reasoning methods assume a single interpretation of visual input, failing under occlusion, blur, or ambiguity. They also use uniform computation budgets, leading to high memory and rollout costs.

Method: (1) Constructs diverse latent world hypotheses in continuous latent space with an orthogonality-based diversity regularizer to prevent collapse. (2) Formulates reasoning as a resource-constrained sequential decision problem with a RL policy that adaptively expands, terminates, or merges paths.

Result: Outperforms existing methods by 2-5% accuracy on multimodal reasoning benchmarks while reducing memory usage by 24%.

Conclusion: Embracing multiple interpretations and adaptive computation allocation is more effective and efficient than fixed-path reasoning for ambiguous multimodal inputs.
...
TL;DR: GeoStream achieves precise metric-scale camera control in autoregressive streaming video generation by maintaining a self-refreshing 3D cache updated online from its own outputs, using on-policy distillation that aligns training and inference conditioning distributions.


<details>
  <summary>Details</summary>
Motivation: Existing video-based world models either learn camera motion implicitly (leading to inaccurate control) or use explicit geometric conditioning with static 3D caches from initial frames, which fails once the viewpoint moves beyond the original frustum.

Method: GeoStream maintains a self-refreshing 3D cache updated online from model outputs: estimates depth from the most recent frame, unprojects to 3D, reprojects into target view for geometric conditioning. Uses on-policy distillation where conditioning during training is rendered from the student's own generated frames.

Result: Quantitative and qualitative results show substantially improved camera controllability over prior approaches.

Conclusion: The on-policy approach mitigates both standard autoregressive drift and the second-order geometric feedback loop from using generated outputs as cache, aligning train and inference distributions.
...
TL;DR: The paper proposes a Variational Network with Wavelet-based U-Net (W-UNet) for accelerated MRI reconstruction, replacing standard pooling with DWT/IWT modules for lossless downsampling. It achieves state-of-the-art performance on fastMRI knee and M4Raw brain datasets by preserving high-frequency details better than prior methods.


<details>
  <summary>Details</summary>
Motivation: Accelerated MRI reconstruction is highly ill-posed and preserving high-frequency anatomical structures under aggressive undersampling remains challenging despite advances in parallel imaging, compressed sensing, and deep learning.

Method: A Variational Network combined with a Wavelet-based U-Net (W-UNet) that replaces standard pooling with Discrete Wavelet Transform (DWT) and Inverse Wavelet Transform (IWT) modules for lossless downsampling, integrated into refinement and sensitivity map estimation stages for single-coil and multi-coil settings.

Result: State-of-the-art performance on fastMRI knee and M4Raw brain datasets in both single-coil and multi-coil settings, with ablation studies confirming the effectiveness of wavelet-based feature decomposition.

Conclusion: The proposed W-UNet framework with wavelet-based lossless downsampling improves artifact suppression, feature preservation, and reconstruction fidelity in accelerated MRI reconstruction.
...
TL;DR: LSA adapts CLIP's predictions for online zero-shot learning by treating it as a domain adaptation problem with label shift correction, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing online zero-shot CLIP methods adapt representations using test data but neglect the mismatch between CLIP's original training distribution and the test label distribution, causing performance degradation.

Method: LSA formulates online zero-shot classification as a domain adaptation problem, adapting CLIP's predictions (trained on unknown source distribution) to target distribution using only unlabeled test data, with label shift correction.

Result: Extensive experiments across multiple datasets show LSA consistently outperforms state-of-the-art online zero-shot learning methods based on CLIP.

Conclusion: Label shift correction is an effective approach for online zero-shot learning with CLIP, addressing the distribution mismatch between source and target domains.
...
TL;DR: UltraSeg, an ultra-lightweight segmentation model (0.13M-0.5M parameters), runs efficiently on CPUs and mobile devices while matching or exceeding large GPU-dependent models (UNet, TransUNet) across ten ultrasound datasets spanning six anatomical sites.


<details>
  <summary>Details</summary>
Motivation: AI deployment in ultrasound is limited by GPU dependency, creating a paradox where the cost of "intelligence" exceeds the cost of the imaging device itself, restricting access in resource-limited settings.

Method: Adapted UltraSeg architecture (two variants: 130K and 500K parameters) and systematically evaluated on ten public POCUS datasets covering six anatomical sites, benchmarking against UNet (31M params) and TransUNet (105M params).

Result: UltraSeg-130K achieves 89.7 FPS on CPU and 34.8 FPS on mobile; UltraSeg-500K achieves 44.6 FPS on CPU and 16.1 FPS on mobile. UltraSeg-500K matches/exceeds UNet's Dice and approaches TransUNet's performance with superior zero-shot cross-dataset generalization.

Conclusion: Clinical-grade ultrasound segmentation is achievable without GPU dependency, aligning AI costs with ultrasound accessibility for resource-limited settings.
...
TL;DR: VeriLatent is a plug-and-play framework that improves instruction-based image editing by verifying initial noise latents at early diffusion steps (without decoding to images), enabling efficient pruning of poor candidates and adaptive inference budget allocation based on editing difficulty.


<details>
  <summary>Details</summary>
Motivation: Existing inference-time scaling methods for image editing suffer from an efficiency-accuracy trade-off: decoding after limited steps yields noisy images unfit for assessment, while full denoising is computationally expensive. The authors aim to solve this by verifying initial noise candidates directly in latent space.

Method: VeriLatent proposes a latent-space verifier using an "editing activation map" at early diffusion steps to score initial noise candidates without decoding. It also includes an adaptive search strategy that allocates inference budgets based on editing difficulty to reduce NFE.

Result: Experiments across multiple benchmarks and base models show VeriLatent consistently improves both editing performance and inference-time scaling efficiency compared to existing methods.

Conclusion: Early-step latent verification is a viable and efficient alternative to decode-then-verify schemes, enabling better scaling of inference compute for image editing tasks.
...
TL;DR: This study maps urban-scale human perceptions of window views using 12,334 real window view images from Wuhan, China. Through VR-based pairwise comparisons (27,477) across six perceptual dimensions from 304 participants, a hybrid neural network predicts perceptions and reveals spatial patterns. Key findings: floor level and view composition (sky, trees, low-rise vs. high-rise buildings) non-linearly influence perceptions of vividness, preference, monotony, and oppression.


<details>
  <summary>Details</summary>
Motivation: City landscapes viewed through home windows influence quality of life, yet perceptions of actual window views at the urban scale remain understudied. Previous research mostly relied on rendered or simulated window views rather than real-world images.

Method: Collected 12,334 window view images (WVIs) from real estate platforms in Wuhan; used non-immersive VR to collect 27,477 pairwise comparisons across six perceptual dimensions from 304 participants on 499 WVIs; trained a hybrid neural network to predict perceptions for all WVIs and mapped spatial distributions.

Result: Significant spatial autocorrelation with distinct hot/cold spots across the city. Higher floors offer more preferred, extensive views; lower floors provide quiet, vivid views. High ratios of sky, trees, and low-rise buildings enhance preference and vividness; high ratios of high-rise buildings increase monotony and oppression. Effects are non-linear.

Conclusion: This work advances urban-scale understanding of residents' visual experiences and provides evidence-based guidance for human-centric urban planning and real estate to optimise visual landscapes from windows.
...
TL;DR: UCS-Bench is a 170+ hour egocentric video dataset with 8.1K+ questions for spatial intelligence, and DirectMe is a framework that builds structured spatial memory from streaming video to improve multimodal LLM spatial reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing spatial reasoning benchmarks lack focus on egocentric, user-centric continual spatial intelligence — specifically the ability to reason dynamically about object locations relative to a moving user over long time horizons.

Method: UCS-Bench provides 170+ hours of egocentric video with timestamped questions. DirectMe incrementally constructs structured spatial memory from streaming observations, coupling perception, memory updates, and reasoning to track objects relative to user movement.

Result: DirectMe significantly improves spatial reasoning of leading multimodal LLMs and outperforms spatially aware and long-form streaming video models on the benchmark.

Conclusion: The benchmark and solution advance spatial intelligence research for egocentric AI assistants by addressing dynamic spatial reasoning, long-term memory, and real-time user alignment.
...
TL;DR: This study evaluates whether large vision-language models (GPT-4o, Gemini Pro, Gemini Flash, Claude) can predict human visual attention in safety-relevant scenes by comparing model-generated saliency maps to eye-tracking data from ten participants. Results show moderate to good spatial alignment, with Gemini Pro excelling in localization and GPT-4o best matching human attention distribution.


<details>
  <summary>Details</summary>
Motivation: Human visual attention is critical for perceiving risks in environments, but collecting eye-tracking data is expensive and time-consuming. This work explores whether vision-language models can approximate human attentional patterns without requiring such training data.

Method: Eye-tracking data from 10 participants viewing 33 safety-relevant scene images were collected using Pupil Invisible glasses. Gaze coordinates were mapped to population-averaged heatmaps. GPT-4o (via OpenAI Vision API) and other models (Gemini Pro, Gemini Flash, Claude) generated saliency maps. Four metrics (Pearson correlation, NSS, KL divergence, AUC-Judd) evaluated spatial alignment between human and model maps.

Result: GPT-4o achieved Pearson r=0.515±0.117, NSS=0.988±0.323, KL=1.766±0.844, AUC-Judd=0.806±0.076. All models exceeded chance (AUC-Judd>0.5, NSS>0). Gemini Pro best in three metrics; GPT-4o best in KL divergence (distribution match).

Conclusion: Large vision-language models can broadly identify where humans attend in safety-relevant scenes without eye-tracking training data, suggesting potential as scalable tools for approximating human attentional patterns in risk perception research.
...
TL;DR: The paper introduces Spectral Forcing, a parameter-free frequency-domain preprocessing method for pixel-space diffusion models that explicitly separates signal from noise based on the time-dependent cutoff frequency, improving capacity allocation and achieving consistent gains on ImageNet-256 and SenseNova-U1 text-to-image models.


<details>
  <summary>Details</summary>
Motivation: Pixel-space diffusion models face a capacity-allocation problem due to the frequency-dependent nature of signal vs. noise in rectified-flow diffusion. Standard denoisers must internally learn the moving bandwidth boundary where useful signal ends and noise begins, potentially wasting computation on regions where prediction collapses to deterministic baselines.

Method: Spectral Forcing: a parameter-free, time-conditional 2D-DCT low-pass filter applied before the patch embedder. The cutoff frequency expands monotonically with diffusion time t following k*(t) = (1-t)^{-2/α}, matching the theoretical signal-noise boundary. It becomes the identity operator at the data endpoint.

Result: On ImageNet-256 with JiT-700M/32, Spectral Forcing consistently improves both FID and Inception Score across training epochs with coarse patch tokenization. In SenseNova-U1 text-to-image model, it improves DPG-Bench and GenEval, demonstrating transferability beyond class-conditional generation.

Conclusion: Explicitly structuring the input-side spectral prior through Spectral Forcing improves capacity-efficient pixel-space diffusion by "showing the signal and hiding the noise," offering a simple yet effective plug-in operator for diffusion models.
...
TL;DR: SPARK is a reinforcement learning-driven framework that adaptively allocates knowledge distillation effort across spatial regions to improve low-bit quantized image restoration, outperforming existing methods without adding inference cost.


<details>
  <summary>Details</summary>
Motivation: Low-bit quantization introduces rounding noise that degrades high-frequency regions (edges, textures) in image restoration, and existing KD methods apply uniform distillation signals across all spatial locations, overlooking varying reconstruction difficulty across image regions.

Method: SPARK uses a lightweight RL policy network that takes four difficulty features (Laplacian variance, pixel variance, student error, teacher-student gap) to produce a stochastic spatial weight map that modulates KD loss during quantization-aware training.

Result: SPARK consistently outperforms PTQ, QAT, and SOTA KD approaches across multiple student architectures, achieving reconstruction quality closest to the full-precision teacher under significant computational constraints.

Conclusion: SPARK is IR task-agnostic, adds no inference cost, and integrates into any existing QAT pipeline without architectural changes, making it an effective and practical solution for deploying quantized image restoration networks.
...
TL;DR: This paper proposes an automated workflow using Implicit Neural Shape Functions (INSF) for lower-limb alignment assessment that encodes anatomy into a latent space and regresses clinical measurements directly, achieving comparable performance to landmark-based methods while offering greater flexibility for extension to new tasks.


<details>
  <summary>Details</summary>
Motivation: Traditional manual measurement of lower-limb alignment is time-consuming, and existing machine learning approaches rely on locating fixed anatomical landmarks, which limits flexibility and requires re-annotation when clinical definitions change.

Method: The method uses Implicit Neural Shape Functions (INSF) to encode femur and tibia outlines into a compact latent space, then regresses clinical alignment measurements directly from these latent codes, avoiding explicit landmark dependency.

Result: Trained on 566 knee radiographs, tested on 50 internal and 402 external (MRKR) cases. Performance was comparable to state-of-the-art landmark-based methods and manual agreement.

Conclusion: The INSF-based approach offers a flexible shape representation that can be extended to additional measurement tasks without altering the backbone architecture.
...
TL;DR: The paper identifies Gradient Dilution as the root cause of degradation in Incremental Object Detection (IOD) and proposes FAS (Focus, Align, Sustain), a unified framework with prior-injected queries, deterministic anchor distillation, and manifold-support replay, achieving over 5.0 AP improvement.


<details>
  <summary>Details</summary>
Motivation: Adapting Detection Transformers to IOD is challenging due to set-based optimization being destabilized by sequential learning. The authors identify Gradient Dilution—where optimization signals for preserving old knowledge progressively weaken—as the root cause of performance degradation.

Method: FAS framework with three components: (1) prior-injected queries to focus discriminative signals by filtering background interference; (2) deterministic anchor distillation to align query-target assignments and enforce semantic consistency; (3) manifold-support replay to sustain distributional support of old classes.

Result: FAS restores robust optimization dynamics and outperforms state-of-the-art methods, achieving over 5.0 AP improvement in the challenging 40+10x4 incremental setting.

Conclusion: The proposed FAS effectively counteracts Gradient Dilution (driven by Signal Dispersion, Assignment Drift, and Support Attrition) and significantly improves IOD performance.
...
TL;DR: This paper presents a multi-view deep learning framework for classifying congenital heart disease (CHD) using echocardiographic images from five views, integrating attention mechanisms and uncertainty-based decision-making to achieve high diagnostic accuracy.


<details>
  <summary>Details</summary>
Motivation: Traditional diagnostics for CHD often suffer from low accuracy and efficiency due to the complexity of cardiac anatomy. The authors aim to improve early CHD detection using a multi-view deep learning approach.

Method: A multi-view deep learning framework is proposed, trained on a large-scale CHD dataset with five echocardiographic views. It uses advanced feature extraction, attention mechanisms, and an uncertainty-based decision-making component to handle low-quality images.

Result: The method achieves top-tier performance on the authors' dataset, demonstrating robust diagnostic capability for early CHD detection.

Conclusion: The framework shows strong potential for clinical use in early CHD detection, and the dataset and code will be released upon acceptance.
...
TL;DR: MamBOA is a backbone-agnostic temporal framework for fine-grained action recognition that uses interleaved state-space recurrence (S6) to naturally encode motion as intrinsic state dynamics, achieving strong results on Diving48 with minimal computational overhead.


<details>
  <summary>Details</summary>
Motivation: Fine-grained action recognition requires temporal reasoning, but existing methods (3D dense operators and difference-based approaches) face tradeoffs between computational cost and expressiveness/flexibility. The paper aims to overcome these limitations by designing a more principled motion modeling approach.

Method: MamBOA introduces an interleaved scan structure that reformulates selective state-space recurrence (S6) as a motion synthesizer. It interleaves consecutive backbone features into an alternating sequence, driving the recurrence to encode inter-frame transitions as intrinsic state dynamics. A cascade of alignment and decoding operations extracts explicit motion representations, followed by dual-path pooling for adaptive aggregation.

Result: On Diving48, MamBOA achieves 85.02% Top-1 accuracy with an image-pretrained backbone and 86.24% with a video-pretrained backbone, processing the entire video in a single forward pass while adding only ~2.1 GFLOPs per feature pair. It works seamlessly with CNN, Transformer, and Mamba backbones.

Conclusion: Structurally induced state-space dynamics constitute a principled and general foundation for motion modeling in fine-grained action recognition, offering a flexible and efficient alternative to existing temporal reasoning approaches.
...
TL;DR: This paper proposes a deep learning architecture for multi-class plant disease classification using leaf images. ResNet-50 achieved 98.74% accuracy while a hybrid ResNet+ViT model achieved 98.58%. Models were trained on 15,200 images across 38 classes, with interpretability techniques like Grad-CAM applied.


<details>
  <summary>Details</summary>
Motivation: To develop accurate, interpretable, and computationally efficient deep learning models for multi-class plant disease classification to assist cultivation management and precision farming.

Method: ResNet-50, MobileNetV2, EfficientNet-B0, and a hybrid ResNet+ViT architecture were trained on 15,200 leaf images (38 classes) with preprocessing (resizing, normalization, augmentation), fine-tuned with AdamW optimizer and cross-entropy loss, using early stopping. Grad-CAM and saliency maps were used for interpretability.

Result: ResNet-50 achieved the highest accuracy (98.74%), while the hybrid ResNet+ViT achieved 98.58%, demonstrating competitive performance in capturing both local and global features.

Conclusion: Transformer-based hybrid models show promise for highly accurate, interpretable plant disease classification, offering practical value for precision agriculture.
...
TL;DR: The paper proposes a decoupled motion representation learning framework for infrared small target detection, using explicit optical flow for global coherent motion and implicit deformable alignment for local target anomalies, outperforming SOTA on dynamic scene benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing multi-frame methods perform implicit temporal modeling where coherent background dynamics dominate motion correspondence learning, creating an inherent trade-off between detection accuracy and false alarms in dynamic scenes.

Method: (1) An explicit motion branch with pretrained optical flow priors and structure-preserving self-supervised adaptation for global coherent motion modeling; (2) An implicit motion branch using deformable feature alignment for target-sensitive local motion anomalies under coherent motion guidance; (3) A coherent-motion-guided local anomaly reasoning module to suppress false responses from background dynamics.

Result: Consistently outperforms existing state-of-the-art approaches on two challenging infrared small target detection benchmarks, especially in dynamic scenes with complex motions, while maintaining favorable inference efficiency.

Conclusion: Decoupling globally coherent background motion from sparse local target motion is an effective strategy for infrared small target detection in dynamic scenes, significantly reducing false alarms caused by background dynamics.
...
TL;DR: G2IA is a geometry-guided instance-aware framework that improves cross-modal place recognition (image-to-point-cloud) by combining geometry-aware representation alignment in retrieval with fine-grained instance-level verification in refinement.


<details>
  <summary>Details</summary>
Motivation: Cross-modal place recognition (CMPR) faces two coupled challenges: the modality gap between RGB images and LiDAR point clouds, and perceptual aliasing in urban environments. The authors argue that simply using global descriptor matching is insufficient and that both geometry-aware alignment and fine-grained verification are needed.

Method: G2IA consists of two stages: (1) Retrieval stage – integrates visual geometry priors from VGGT and instance features to build place descriptors compatible with LiDAR maps; (2) Refinement stage – re-ranks retrieved candidates by verifying local instance shapes and their relative spatial layouts across modalities.

Result: The method consistently improves image-to-point-cloud place recognition across different localization thresholds and shows strong cross-dataset generalization on public benchmarks.

Conclusion: G2IA effectively addresses the modality gap and perceptual aliasing in CMPR by combining geometry-aware descriptor matching with instance-level geometric verification, outperforming existing approaches.
...
TL;DR: HemExp is a latent diffusion model that generates patient-specific follow-up CT images and segmentations for intracerebral hemorrhage, conditioned on baseline imaging, clinical variables, and expansion indicators, enabling uncertainty-aware prediction of hematoma progression.


<details>
  <summary>Details</summary>
Motivation: Existing methods only provide binary expansion risk or single follow-up volume estimates, lacking uncertainty quantification for acute triage and treatment decisions in neurosurgical care of ICH patients.

Method: A hemorrhage-aware multi-head VAE encodes baseline and follow-up scans into latent space; a conditional diffusion model learns the progression as latent difference. Generation is conditioned on baseline imaging, clinical variables, and expansion indicators. Multiple synthetic follow-ups per patient produce spatial HE probability maps.

Result: Trained on 450 multi-center patients and evaluated on 107 held-out patients. Perturbing clinical inputs shifts predicted volume distributions. Outperforms binary predictors in estimating hematoma volume, intraventricular involvement, and mass effects.

Conclusion: Controllable latent diffusion is a promising direction for uncertainty-aware modeling of early ICH progression, supporting realistic scenario simulation and robust clinical outcome estimation.
...
TL;DR: CoMNeT combines a 3D MedNeXt segmentation model with corrective diffusion (CorrDiff) refinement and ensembling to improve glioma segmentation from multiparametric MRI, achieving superior Dice scores (average 0.7798) on the UTSW-Glioma dataset compared to baseline models.


<details>
  <summary>Details</summary>
Motivation: Brain tumor segmentation from multiparametric MRI is critical for treatment planning and research, but remains difficult due to variation in tumor appearance, MRI protocols, and the small size of clinically important regions like enhancing tumor (ET) and tumor core (TC).

Method: The authors propose CoMNeT, a framework combining MedNeXt (3D convolutional segmentation model with Global Response Normalization) and CorrDiff (corrective diffusion-based postprocessing refinement). It takes four MRI modalities as input, predicts ET, TC, and WT regions, and uses fold-level probability ensembling with five-fold cross-validation.

Result: CoMNeT achieved Dice scores of ET: 0.7543, TC: 0.6806, WT: 0.9049, average: 0.7798, outperforming SegResNet (avg 0.7555) and standalone MedNeXt (avg 0.7697) on the UTSW-Glioma dataset.

Conclusion: Corrective diffusion refinement and fold-level probability ensembling are practical additions that improve existing 3D convolutional models for automated glioma segmentation.
...
TL;DR: CoMET-Bench introduces a benchmark for conditional multi-event temporal grounding in long videos, where queries require counting and localizing events under combined temporal and spatial conditions. CoMET-Agent outperforms GPT-5 by 6.1% F1@0.5 through structured reasoning without training.


<details>
  <summary>Details</summary>
Motivation: Real-world video understanding requires localizing multiple events satisfying both temporal and spatial conditions, but existing benchmarks only handle single-moment grounding, counting without temporal constraints, or treat grounding and counting as separate tasks.

Method: Proposes CoMET-Bench (2789 queries, 600 long videos averaging 33.8 min) with compositional queries (4 temporal + 3 spatial conditions) and negative queries. Introduces Rejection-F1 metric to prevent trivial solutions. CoMET-Agent uses a structured search-and-aggregate framework without training.

Result: Existing MLLMs, agent-based, and grounding-specialized methods fall short. CoMET-Agent improves F1@0.5 by 6.1% over GPT-5 through structural reasoning alone.

Conclusion: Three open directions remain: fine-grained entity tracking, position-uniform retrieval, and causal event pairing. The task of conditional multi-event temporal grounding is far from solved.
...
TL;DR: PPDM is a memory- and speed-efficient diffusion model for 3D medical image translation that uses a reversible pixel puzzle-unpuzzle operator, direct bridge diffusion, and puzzle-gradient loss to achieve high fidelity while drastically reducing GPU memory (up to 10x) and inference time.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models for 3D medical image translation suffer from prohibitive computational cost and GPU memory; current memory-efficient methods often compromise global volumetric consistency or fine anatomical detail.

Method: PPDM introduces (1) a reversible pixel puzzle-unpuzzle operator that trades spatial resolution for channel dimensionality to reduce activation memory, (2) a direct bridge diffusion formulation starting from the conditional input instead of pure noise for efficiency, and (3) a puzzle-gradient loss to enforce spatial coherence and suppress grid-like artifacts.

Result: Matches or outperforms full 3D diffusion models across tasks (low-count PET denoising, joint PET denoising & attenuation correction, cross-modal MRI translation) while reducing training GPU memory by up to an order of magnitude and significantly accelerating inference; outperforms existing memory-efficient approaches.

Conclusion: PPDM provides a practical and scalable solution for high-fidelity 3D diffusion-based medical image translation under limited computational resources.
...
TL;DR: SGFormer++ is a Semantic Graph Transformer for 3D scene graph generation that uses Transformer layers, a Graph Embedding Layer++ for efficient edge-aware global context, a Semantic Injection Layer++ for LLM/VLM-informed visual features, and a Spatial-guided Feature Adapter with Cascaded Binary Prediction Head for incremental learning, achieving SOTA on 3DSSG benchmark (+4.49% Predicate A@1 in incremental setting).


<details>
  <summary>Details</summary>
Motivation: Existing GCN-based 3D scene graph generation methods suffer from over-smoothing and limited receptive fields, hindering their ability to model complex global scene structures. Additionally, practical incremental SGG scenarios where new relationship categories arrive sequentially are underexplored.

Method: SGFormer++ replaces GCN backbones with Transformer layers for global message passing, introduces Graph Embedding Layer++ for linear-complexity edge-aware context integration, Semantic Injection Layer++ for incorporating LLM/VLM linguistic priors without extra parameters, and for incremental SGG, a Spatial-guided Feature Adapter to calibrate predicate features and a Cascaded Binary Prediction Head to mitigate catastrophic forgetting.

Result: Achieves state-of-the-art performance on the 3DSSG benchmark in both standard and incremental settings, with a 4.49% absolute improvement in Predicate A@1 under the incremental setting.

Conclusion: The proposed Transformer-based architecture with tailored components effectively addresses the limitations of GCN-based methods for 3D SGG and generalizes well to incremental learning scenarios, setting a new state-of-the-art.
...
TL;DR: CausalDrive is a controllable, real-time foundation driving world renderer that generates causally-reactive video simulation from only the initial front-view frame, ego trajectory, and a text prompt—enabling interactive closed-loop evaluation, RL post-training, and human-in-the-loop simulation at 12 FPS.


<details>
  <summary>Details</summary>
Motivation: Existing video generative models for autonomous driving either require oracle future trajectories of all agents (making them non-reactive) or lack semantic control and suffer from high latency—preventing their use as interactive simulators for closed-loop policy learning.

Method: CausalDrive operates on only the initial front-view frame, ego trajectory, and text prompt, excluding future NPC layouts to force intrinsic causal interaction prediction. It uses a Context-Forced DMD architecture combining continuous flow-matching with a self-correcting distillation objective to achieve 12 FPS interactive speed.

Result: Achieves 12 FPS interactive speed; demonstrates three downstream applications: (1) generative closed-loop evaluation with reduced collision artifacts, (2) large-scale RL post-training via a Video2Reward module, and (3) real-time human-in-the-loop simulation. Policies trained in CausalDrive's reactive scenarios show superior real-world interaction capabilities.

Conclusion: CausalDrive transforms passive video generation into a playable neural simulator for autonomous driving, enabling causal, reactive, and controllable world modeling at interactive speeds for practical closed-loop policy training and evaluation.
...
TL;DR: Dyna-Pruner is an end-to-end framework for input-dependent co-pruning of both data and model structure in spatio-temporal prediction, reducing FLOPs by up to 70% and achieving 2.5x speedup on edge devices with <1% accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Spatio-temporal prediction models (for radar nowcasting, traffic monitoring) are too computationally expensive for real-time deployment due to dense computation on inputs with inherent redundancy (e.g., calm seas, clear skies).

Method: Dyna-Pruner is an end-to-end framework that performs input-dependent co-pruning of data and model structure using a shared-importance synchronization mechanism. It generates coupled masks that prune both redundant spatial regions and their corresponding computational units (e.g., convolutional filters), creating per-sample sparse sub-networks at inference time.

Result: Tested on WeatherBench, SEVIR, and TaxiBJ benchmarks; integrates seamlessly with CNN, RNN, and Transformer backbones; reduces FLOPs by up to 70%; achieves 2.5x speedup on NVIDIA Jetson AGX Orin; accuracy loss is negligible (<1%).

Conclusion: Dyna-Pruner enables automated, resource-aware architecture optimization for scalable media analysis by dynamically pruning both input data and model structure per sample, making real-time spatio-temporal prediction feasible on resource-constrained edge devices.
...
TL;DR: This paper reframes Facial Affect Analysis (FAA) from a standalone recognition task into a composable, dependable service component for Service-Oriented Software Ecosystems (SoSE), emphasizing deployability concerns beyond benchmark accuracy—such as robustness, fairness, privacy, and runtime guarantees.


<details>
  <summary>Details</summary>
Motivation: To transition FAA from isolated academic recognition tasks into reusable, operational perception capabilities that can be reliably integrated into service-oriented software ecosystems.

Method: Reviews representative FAA advances (static/dynamic expression analysis, AU/micro-expression modeling, CNN/Transformer/graph/hybrid architectures) and interprets them through systems-engineering requirements for SoSE—including service contracts, latency/availability envelopes, lifecycle monitoring, governance-aware integration, and interoperability.

Result: Benchmark gains alone are insufficient for SoSE readiness; robustness under distribution shift, intervention stability, fairness, privacy posture, and runtime guarantees are equally critical for deployability.

Conclusion: A roadmap is provided for treating FAA as an operational service component with explicit interfaces, measurable quality attributes, and accountable lifecycle management.
...
TL;DR: This paper proposes a sustainable, edge-deployable face recognition framework using VQ-VAE with knowledge distillation, achieving comparable accuracy to state-of-the-art models while significantly reducing memory, computation, and network overhead for low-power edge devices.


<details>
  <summary>Details</summary>
Motivation: To address the high computational cost, network traffic, energy consumption, and carbon footprint of conventional cloud-based face recognition systems by developing an efficient edge-deployable alternative.

Method: The framework uses VQ-VAE to generate compact latent representations of facial images, combined with knowledge distillation from pre-trained face embeddings, enabling efficient compression and reconstruction while preserving identity characteristics for edge deployment.

Result: The system achieves accuracy comparable to state-of-the-art face embedding models while significantly reducing memory and computation requirements, making it suitable for low-power edge devices with minimized network overhead.

Conclusion: VQ-VAE-based compression with knowledge distillation offers a sustainable and effective solution for edge-deployable face recognition, balancing accuracy with resource efficiency and reducing environmental impact.
...
TL;DR: This paper reproduces MNet (a hybrid 2D/3D CNN for anisotropic medical image segmentation) within nnU-Net, confirms its performance on PROMISE and LiTS datasets, and introduces two lightweight extensions — Fusion Gating and VMamba state-space module — that improve accuracy and consistency while preserving robustness to anisotropic voxel spacing.


<details>
  <summary>Details</summary>
Motivation: To verify the reproducibility of MNet's reported robustness to anisotropic (variable voxel spacing) medical image segmentation, and explore lightweight architectural improvements for adaptive 2D-3D feature fusion and long-range depth modelling.

Method: The original MNet hybrid 2D/3D CNN was re-implemented within the nnU-Net framework under matched preprocessing and compute constraints. Two lightweight extensions were added: (1) a learned Fusion Gating mechanism for adaptive 2D-3D feature blending, and (2) a VMamba state-space module for efficient long-range depth modelling.

Result: Reproduced MNet achieved DSC of 89.0±0.9% (PROMISE, within 0.8% of published) and 94.3±1.9%/54.6±3.1% (LiTS liver/tumor). Spatial Gating improved DSC by +0.8% with <3% overhead. VMamba reduced PROMISE Dice variation to ±0.7% and achieved best LiTS liver at 95.8% Dice. Both extensions preserved robustness (delta Dice=1.5% across 1-4mm spacing).

Conclusion: The study confirms MNet's reproducibility and shows that adaptive fusion (Fusion Gating) and state-space modelling (VMamba) can enhance segmentation reliability under anisotropic conditions, though further tests are needed for definitive conclusions.
...
TL;DR: The paper identifies that diffusion inversion errors follow a parabolic trend relative to timestep size, and proposes a nonuniform timestep scheduler (global rescaling + local dynamic programming) that reduces inversion error without extra parameters or computation, boosting performance of existing inversion methods.


<details>
  <summary>Details</summary>
Motivation: Diffusion inversion is critical for image reconstruction and editing, but existing methods overlook how timestep selection influences inversion fidelity, leading to accumulated errors.

Method: Proposes a nonuniform timestep scheduler combining global rescaling with local dynamic programming-based rescheduling to strategically allocate computational effort and minimize overall inversion error.

Result: The scheduler consistently boosts performance of existing inversion methods across image reconstruction and editing tasks, achieving superior results with no extra parameters or computational overhead.

Conclusion: Timestep selection critically affects inversion fidelity (parabolic error trend), and the proposed scheduler serves as an off-the-shelf enhancement for existing inversion techniques.
...
TL;DR: A compact MobileNetV3 + U-Net architecture achieves 0.9482 overall score for spacecraft classification, detection, and segmentation, ranking 2nd in the SPARK 2026 Challenge.


<details>
  <summary>Details</summary>
Motivation: Space vision perception is hindered by scarce labeled data and challenging visual conditions (illumination changes, low SNR, high contrast), motivating a single efficient model for multi-task space object analysis.

Method: Integrates MobileNetV3 encoder with U-Net-style decoder for dense prediction; detection derived analytically from union of predicted component masks, eliminating separate bounding-box regression head.

Result: Overall leaderboard score 0.9482; classification 1.0000, detection 0.9788, segmentation 0.8917; ranked 2nd overall in SPARK 2026 Challenge.

Conclusion: Lightweight encoder-decoder architectures can deliver strong multi-task performance suitable for practical onboard space vision systems.
...
TL;DR: The paper proposes converting egocentric videos into Temporal Action Graphs via multi-stage prompting to decouple visual perception from symbolic reasoning, showing VLMs perform better as symbolic reasoners than direct pixel-based observers across 11 models (2B-235B parameters).


<details>
  <summary>Details</summary>
Motivation: General-purpose VLMs struggle with fine-grained action reasoning in egocentric video when operating directly on raw pixels, motivating the need to separate visual perception from symbolic reasoning.

Method: A multi-stage prompting pipeline that first generates dense natural language narratives over short temporal windows as a semantic bottleneck, then formalizes them into structured, open-vocabulary Temporal Action Graphs for symbolic reasoning.

Result: On EGTEA and Epic-Kitchens-100, few-shot graph demonstrations significantly outperform zero-shot frame and graph-based inference. Zero-shot graph reasoning remains competitive with pixel-based inference. Across 11 VLMs (2B-235B parameters), VLMs are more effective as symbolic reasoners than direct visual observers.

Conclusion: Projecting video into the language domain provides a scalable, fine-tuning-free alternative that better leverages VLMs' latent reasoning strengths for egocentric action reasoning.
...
TL;DR: Lesion-DDPM is a 3D conditional diffusion model for lesion-aware FLAIR MRI synthesis that uses multi-level anatomical mask injection and lesion-weighted loss, achieving superior image quality and boosting downstream MS lesion segmentation Dice scores to 0.685 when combined with real data.


<details>
  <summary>Details</summary>
Motivation: Public MS datasets are small and heterogeneous across scanners/protocols, hindering robust ML model development; generative models struggle to preserve small, sparse lesions in synthesized images.

Method: 3D conditional diffusion framework (Lesion-DDPM) with multi-level anatomical mask injection and a lesion-weighted reconstruction loss to emphasize lesion voxels while maintaining global brain structure.

Result: Lesion-DDPM achieved the lowest lesion-region reconstruction error; downstream 3D U-Net trained solely on synthetic data reached Dice 0.616 (vs. 0.569 for best competitor); adding synthetic to real data improved Dice to 0.685.

Conclusion: Lesion-DDPM effectively generates realistic lesion-preserving FLAIR images, outperforming GAN and diffusion baselines, and demonstrably improves downstream segmentation performance, suggesting value as a data augmentation tool for MS imaging.
...
TL;DR: This paper explores using sparse autoencoders to interpret ConvNeXt vision model representations trained on FGVC-Aircraft, finding partially interpretable aircraft features with limitations like polysemanticity.


<details>
  <summary>Details</summary>
Motivation: Vision models achieve strong performance on classification but their internal representations are difficult to interpret, motivating the need for methods to decompose these representations into interpretable features.

Method: Train ConvNeXt on FGVC-Aircraft, extract spatial activations from final feature stage, train sparse autoencoder, analyze features via top-activating patches/activation strength/class selectivity, and evaluate with input-space and feature-space ablations.

Result: Sparse autoencoders reveal partially interpretable, class-relevant visual features related to aircraft recognition, but suffer from polysemanticity and coarse spatial localization.

Conclusion: Sparse autoencoders can help interpret vision model representations, though with notable limitations that need further addressing.
...
TL;DR: ST-DiffEye is a diffusion-based framework that jointly models gaze trajectories and scanpaths by concatenating them as input channels, achieving state-of-the-art performance on visual search and free-viewing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing generative gaze models supervise on only one of two gaze representations (trajectories or scanpaths) in isolation, but the authors hypothesize these modalities describe gaze at complementary scales and are jointly informative during training.

Method: ST-DiffEye is a joint trajectory-scanpath diffusion framework that couples both modalities by concatenating them as an additional raw input channel, requiring no architectural overhead beyond input/output channel expansion. They also introduce a CRPS-based evaluation framework that generalizes existing similarity metrics into proper scoring rules for assessing accuracy and diversity.

Result: State-of-the-art performance on task-driven visual search (both target-present and target-absent) and free-viewing benchmarks. Ablations confirm the benefit of joint modeling and distribution-aware evaluation.

Conclusion: Joint modeling of trajectories and scanpaths improves gaze generation, and distribution-aware evaluation better captures the intrinsic variability of human gaze.
...
TL;DR: SSync proposes Selective Synergistic Learning for video object-centric learning, selectively distilling reliable cues from encoder and decoder with linear complexity instead of exhaustive alignment, plus transitive pseudo-label merging to reduce slot redundancy.


<details>
  <summary>Details</summary>
Motivation: Existing VOCL methods force dense alignment between encoder attention maps and decoder object maps via contrastive learning, which propagates weaknesses (noisy encoder predictions, blurred decoder boundaries) and incurs quadratic computational cost in spatio-temporal patches, limiting scalability.

Method: SSync selectively distills reliable cues: encoder for boundary refinement, decoder for interior denoising, via pseudo-labeling with linear complexity. It also introduces transitive pseudo-label merging to consolidate overlapping slots based on spatio-temporal activation consistency.

Result: SSync improves decomposition quality, serves as a versatile plug-and-play module, and exhibits exceptional robustness to slot configurations.

Conclusion: Selective synergistic learning with linear complexity effectively addresses error propagation and scalability issues in video object-centric learning, outperforming dense alignment strategies.
...
TL;DR: Track2View conditions a video diffusion transformer on explicit 3D point tracks to achieve state-of-the-art novel view synthesis for video, reducing rotation error by 30-65% and translation error by 61-72% over baselines.


<details>
  <summary>Details</summary>
Motivation: Existing methods for re-rendering video from novel viewpoints rely on per-frame pose embeddings, noisy point-cloud renderings, or implicit learned correspondences, none of which provides an explicit, temporally continuous link between source and target pixels.

Method: Track2View uses paired 3D point tracks (sparse trajectories of scene points in both source and target views) as conditioning for a video diffusion transformer. A dual-view track conditioner transfers visual context via parameter-free geometric operations and learned temporal aggregation. A data curation pipeline extracts one-to-one track correspondences using a 3D point tracker on multi-camera view pairs.

Result: On a 400-video benchmark (static and dynamic scenes), Track2View achieves SOTA results, reducing rotation error by 30-65% and translation error by 61-72% relative to leading baselines.

Conclusion: Providing explicit temporally continuous 3D point tracks as conditioning significantly improves novel view synthesis for video, particularly in camera accuracy metrics.
...
TL;DR: EcoBin is a two-stage CNN that classifies household waste disposal pathways while detecting contamination in recyclables, achieving significant improvement over base classifiers on contaminated items (24/25 vs 1/25 correct).


<details>
  <summary>Details</summary>
Motivation: Existing waste classification models achieve high accuracy on clean benchmarks but fail to account for contamination in recyclable waste, which is a real-world problem.

Method: A two-stage CNN: Stage 1 uses EfficientNetV2-S for base waste classification (30 categories → 4 disposal pathways); Stage 2 detects contamination in recyclables using a synthesized dataset created by U2-Net segmentation and compositing realistic contamination textures.

Result: Stage 1: 87.42% test accuracy, 96.13% pathway-adjusted accuracy. Contamination stage: 0.99 ROC-AUC. Full pipeline: 24/25 contaminated items correct vs 1/25 for base classifier alone (p<0.001).

Conclusion: Explicitly modeling contamination significantly improves waste disposal routing decisions, addressing a critical gap in current waste classification systems.
...
TL;DR: RaLMPH is a MIL-based framework that reconciles multiple pathologist labels on WSIs by modeling local reliability via feature-space neighborhoods and expert uncertainty, outperforming existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing MIL pipelines assume single gold labels per slide, ignoring inter-pathologist variability common in clinical practice. Current multi-annotator methods fail to handle localized disagreements in MIL settings.

Method: RaLMPH introduces a reliability field modeling local neighborhood structure and expert entropy, performs sample-wise local annotator ranking, and uses an adaptive gating mechanism to fuse labels based on local reliability.

Result: Outperforms existing approaches on a clinical WSI dataset with six pathologists and controlled simulated benchmarks. Reliability-aware mechanism improves label reconciliation and downstream MIL performance.

Conclusion: Modeling local annotator reliability rather than global reliability is key to handling multi-pathologist variability in WSI analysis under the MIL paradigm.
...
TL;DR: I2EBench2.0 is a comprehensive benchmark for evaluating instruction-based image editing (IIE) models across single-round and multi-round tasks, covering 16 and 7 evaluation dimensions respectively, with human-aligned judgments and research insights from testing eight recent IIE models.


<details>
  <summary>Details</summary>
Motivation: Assessing IIE models is challenging due to complex instructions and diverse edits; there is an urgent need for a robust evaluation framework with precise quality metrics and benchmarks to guide future improvements.

Method: I2EBench2.0 evaluates IIE models across single-round and multi-round scenarios using 16 dimensions (single-round) and 7 dimensions (multi-round), incorporates human judgment via user studies, and provides research-driven insights by analyzing model strengths/weaknesses.

Result: Eight recently developed IIE models were tested using I2EBench2.0, yielding academic insights through detailed comparison and analysis. Code, dataset, and generated images are publicly available.

Conclusion: I2EBench2.0 offers a comprehensive, human-aligned evaluation framework for IIE models, providing critical benchmarks and insights to direct future research in instruction-based image editing.
...
TL;DR: This paper proposes an accumulative fingerprint mapping and reconstruction framework for small-area mobile sensors, converting multiple partial touches into a unified fingerprint state for one-shot matching, using a classical baseline pipeline and a broader framework with structured token learning, pose reasoning, and diffusion-based reconstruction.


<details>
  <summary>Details</summary>
Motivation: Small-area fingerprint sensors on mobile devices create a fundamental mismatch: each touch captures only a tiny, pose-varying local patch, yet reliable biometric matching requires a stable and complete representation. Existing multi-capture multi-match pipelines are inefficient and lack global coverage guarantees.

Method: A classical baseline pipeline: patch-wise structural feature extraction → feature-level registration and fusion → fingerprint map construction → phase-based ridge reconstruction. Plus a broader framework integrating structured token learning, two-stage pose reasoning, and diffusion-based generative reconstruction.

Result: The proposed viewpoint reframes mobile fingerprint recognition from multi-capture multi-match to accumulative map building, state refinement, and one-shot matching, offering efficient, pose-robust, deployment-friendly biometrics. A baseline implementation is publicly released.

Conclusion: Accumulative fingerprint mapping and reconstruction provides a principled route toward solving the small-area sensor mismatch problem, converting sequential partial observations into a unified, progressively refined fingerprint state for one-shot matching.
...
TL;DR: This paper improves zero-shot segmentation by introducing Contextual Similarity Maps that fuse high-resolution attention maps with U-Net features, and a dynamic timestep selection mechanism that leverages an emergent hierarchical semantic progression (part-level to object-level) in diffusion models, consistently outperforming existing baselines.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based zero-shot segmentation methods suffer from the trade-off between spatial resolution and contextual information, and rely on a single static timestep for feature extraction, limiting their effectiveness.

Method: (1) Contextual Similarity Maps: fuses high-resolution attention maps with rich U-Net encoder features for fine-grained and robust per-pixel representations. (2) Dynamic timestep selection: leverages the discovered hierarchical semantic progression (part-level → object-level) in the denoising process to adaptively select the optimal timestep for each pixel.

Result: The method consistently outperforms existing zero-shot segmentation baselines across extensive experiments.

Conclusion: Combining contextual features with dynamic, hierarchical timestep selection effectively improves zero-shot segmentation performance.
...
TL;DR: DenseControl is a pipeline for generating dense crowd images with precise control over instance positions, sizes, background, style, and attributes, introducing IOE map for spatial control and ISE strategy for scale embedding, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: The paper addresses two key challenges in crowd image synthesis: controlling signal embedding and maintaining topological integrity when providing instance scale guidance.

Method: 1) Isolated Object Embedding (IOE) map for spatial location control; 2) Implicit Scale Embedding (ISE) strategy for precise scale information; 3) Position Shortcut mechanism to enhance cross-attention and alleviate projection challenges.

Result: DenseControl achieves state-of-the-art results in dense crowd image synthesis across different control conditions, and demonstrates practical utility in crowd analysis augmentation, transfer learning, and weather generalization.

Conclusion: DenseControl effectively solves signal embedding and topological integrity challenges in dense crowd generation, and its practical applications highlight its utility in real-world scenarios.
...
TL;DR: Fusion-E2Pulse is a multimodal network combining RGB video (structural priors) and event camera streams (fine details) to reconstruct high-fidelity pulse waveforms, achieving SOTA performance with 0.78 bpm HR error, 0.89 waveform correlation, and 16.74 ms systolic phase error.


<details>
  <summary>Details</summary>
Motivation: RGB-based methods smooth out subtle vascular details due to integral imaging, while event cameras suffer from noise and motion artifacts. The goal is to synergize both modalities for precise non-contact pulse wave reconstruction including the dicrotic notch.

Method: A multimodal fusion network (Fusion-E2Pulse) uses filtered RGB signals as structural priors to suppress motion artifacts, and event streams for high-sensitivity recovery of fine-grained morphological details.

Result: Mean absolute error of 0.78 bpm for HR estimation, waveform correlation of 0.89, systolic phase duration error of 16.74 ms; state-of-the-art performance in balancing noise suppression and morphological fidelity.

Conclusion: Fusion-E2Pulse effectively validates that fusing frame-based RGB and event-based sensing yields superior non-contact pulse wave reconstruction with high pathological feature fidelity.
...
TL;DR: A lightweight LoRA-fine-tuned SAM 3 framework using only 7 annotated 3D CT volumes with hard negative mining and temporal filtering achieves high-precision 4DCT segmentation (median Dice 0.968/0.910) while eliminating phase-specific artifacts, retaining >95% full-data accuracy on a single consumer GPU.


<details>
  <summary>Details</summary>
Motivation: Current 4DCT Internal Target Volume contouring processes each respiratory phase in isolation, discarding temporal coherence and leaving contours vulnerable to phase-specific artifacts.

Method: Parameter-efficient fine-tuning of SAM 3 via LoRA using only 7 annotated 3D CT volumes, combined with hard negative mining for boundary discrimination, phase-coherent temporal filtering, and spatial connectivity analysis at inference.

Result: Median Dice scores of 0.968 (pulmonary) and 0.910 (cardiac), 95th-percentile Hausdorff distances of 0.998 mm and 2.931 mm, retains >95% full-data accuracy with only 7 volumes, trainable on a single consumer GPU.

Conclusion: The framework effectively eliminates false positives from zero-shot SAM 3, offering a scalable, data-efficient solution for adaptive radiotherapy contouring.
...
TL;DR: This paper introduces RobustMLLMJudge, a framework to evaluate adversarial robustness of MLLMs as judges, and proposes MGSIA, a novel attack method that bypasses evaluation protocol constraints to inflate scores. Experiments show MGSIA effectively deceives advanced MLLM judges across quality and safety scenarios.


<details>
  <summary>Details</summary>
Motivation: Despite MLLMs being increasingly used as automated judges for image quality and safety assessment, their adversarial robustness remains largely unexplored, threatening fairness and reliability of automated judging.

Method: The authors propose MGSIA (Manifold-Guided Semantic Induction Attack), which combines affirmative semantic induction (maximizing probability of "Yes" responses) with high-score manifold alignment (regularizing adversarial representations toward high-score centers from proxy protocols) to generate transferable score-inflating perturbations.

Result: Different MLLM judges are highly vulnerable to score-inflating adversarial attacks. MGSIA achieves superior effectiveness and generalizability in deceiving advanced MLLM judges across various evaluation scenarios.

Conclusion: The findings highlight the critical need for developing robust MLLM judges, as current models are easily manipulated by adversarial attacks designed to inflate scores.
...
TL;DR: MuDuo is a mutual distillation framework using CT (SAM-Med3D) and PET (SegAnyPET) foundation models to achieve state-of-the-art PET/CT organ segmentation with only 5 labeled cases on the AutoPET dataset.


<details>
  <summary>Details</summary>
Motivation: High annotation cost of PET/CT segmentation motivates the need for effective semi-supervised learning methods that can work with limited labeled data.

Method: Proposes MuDuo, a mutual distillation framework that leverages two modality-specific foundation models (SAM-Med3D for CT, SegAnyPET for PET) as generalist teachers to distill knowledge into a lightweight student network, eliminating manual prompts and maximizing unlabeled data utility.

Result: Achieves state-of-the-art performance on the AutoPET dataset with only 5 labeled cases.

Conclusion: Bridging task-specific student models with generalist foundation models via mutual distillation is an effective SSL strategy for PET/CT segmentation with minimal labeled data.
...
TL;DR: This paper derives a mathematically principled synchronization framework for collaborative diffusion generation using optimal control, achieving task-agnostic test-time coordination of multiple diffusion trajectories without additional training, with consistent improvements shown across three representative tasks.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion synchronization approaches rely heavily on heuristics and task-specific tailoring, limiting their generalizability and performance. A principled, scenario-agnostic solution is needed.

Method: The authors mathematically derive a synchronization framework based on optimal control, optimizing control variables during sampling to guide multiple diffusion trajectories toward coherent solutions while staying close to the underlying diffusion prior. The method operates entirely at test-time without additional training.

Result: Demonstrates consistent improvements over baselines on three representative collaborative generation tasks covering a wide range of modalities and applications.

Conclusion: Establishes a novel foundation for collaborative generation, opening a principled path toward extending pretrained generative models to new collaborative generation settings.
...
TL;DR: NeRD is a neuro-symbolic framework that distills efficient, ontology-grounded reasoning chains for medical image diagnosis, outperforming existing concept-based interpretable methods while enabling expert-in-the-loop intervention.


<details>
  <summary>Details</summary>
Motivation: Existing concept-driven interpretable methods (CBMs and rationale-based generative approaches) impose high clinician burden or drift from diagnostic ontologies, failing to balance interpretability, efficiency, and clinical plausibility.

Method: NeRD (Neuro-Symbolic Rule Distillation) produces ontology-grounded, sufficient yet non-redundant reasoning chains without manual rule crafting, and enables multimodal Chain-of-Thought-based diagnosis with expert-in-the-loop concept-level intervention.

Result: Strong diagnostic performance and interpretability on two skin datasets; blinded expert evaluation confirmed clinical plausibility of NeRD rationales; first expert-in-the-loop study for multimodal CoT diagnosis achieved efficient concept-level intervention.

Conclusion: NeRD effectively addresses limitations of prior concept-driven interpretable methods by combining neuro-symbolic distillation with ontology grounding, achieving both high diagnostic accuracy and clinically plausible, efficient expert intervention.
...
TL;DR: XPASS-Vis is the first dataset designed for cross-domain personalized image aesthetic assessment (PIAA), spanning art, fashion, and landscape domains with 87,836 annotations from 129 users. Under unsupervised domain adaptation, the best model recovers ~60% of the supervised upper bound, showing cross-domain transferability of aesthetic preferences but leaving substantial room for improvement.


<details>
  <summary>Details</summary>
Motivation: Existing PIAA datasets are single-domain or lack sufficient per-annotator samples across domains, making cross-domain generalization of personalized aesthetic preferences unexplored. This paper aims to fill that gap.

Method: The authors introduce XPASS-Vis (6,526 stimuli, 3 domains: art/fashion/landscape, 129 annotators, 87,836 interactions with aesthetic + emotion ratings). They build baselines using unsupervised domain adaptation (UDA) to transfer models from labeled source to unlabeled target domains.

Result: Best UDA method achieves Spearman's ρ = .28, recovering ~60% of the supervised upper bound. This demonstrates partial cross-domain transferability of personalized aesthetic preferences.

Conclusion: Personalized aesthetic preferences are meaningfully transferable across visual domains, but a significant gap remains, motivating future PIAA-specific adaptation strategies. XPASS-Vis provides a foundation for this research direction.
...
TL;DR: Savvy proposes a zero-shot open-world video segmentation system with hierarchical mask discovery and track consolidation, plus OGA - a granularity-agnostic evaluation protocol that uses n:1 matching instead of rigid 1:1 matching, demonstrating that standard evaluation underestimates open-world methods.


<details>
  <summary>Details</summary>
Motivation: Existing video segmentation methods fail in open-world long-horizon scenarios due to inability to discover objects and maintain identity over long videos with ego-motion, and current evaluation protocols with rigid 1:1 matching unfairly penalize semantically valid predictions with different granularity.

Method: Savvy combines hierarchical mask discovery, deferred admission, and track consolidation. OGA evaluation uses Granularity-Agnostic (GA) matching with n:1 mapping, sever points for detecting support discontinuities, and scoring via dominant coherent fragments with metrics like Identity Persistence (IP) and Identity Concentration (IC).

Result: On VIPSeg, standard 1:1 evaluation underestimates open-world methods while GA evaluation recovers suppressed performance. On ScanNet and HM3D, Savvy outperforms strong baselines across STQ, VPQ∞, IP, and IC metrics.

Conclusion: The paper establishes a practical benchmark and strong baseline for open-world long-horizon video segmentation, showing both a better system (Savvy) and a fairer evaluation protocol (OGA) are needed for this underexplored direction.
...
TL;DR: Proposes a transfer learning-based underwater image enhancement (UIE) method that doesn't need paired labels. It decomposes UIE into color correction, haze removal, and noise suppression using physics, then leverages cross-domain priors from other vision tasks as supervision, achieving SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Existing UIE methods rely on noisy pseudo-labels from previous methods since true underwater labels are hard to collect, leading to unsatisfactory performance. The authors aim to bypass this label dependency entirely.

Method: (1) Decomposes UIE into three sub-tasks following underwater physics: global color correction, haze removal, and background noise suppression. (2) Uses transfer learning to leverage prior knowledge from other vision tasks as cross-domain supervision in each step, eliminating the need for paired labels.

Result: Achieves SOTA performance on UIE benchmarks and effectively boosts downstream vision tasks, significantly outperforming existing methods.

Conclusion: Physics-aligned decomposition combined with cross-domain prior fusion via transfer learning provides both theoretical soundness and practical superiority over label-dependent UIE methods.
...
TL;DR: The paper proposes a self-questioning framework that trains a VLM to decompose visual questions into sub-questions using reinforcement learning (GRPO), without human-annotated examples, achieving improved compositional reasoning performance on both synthetic (CLEVR) and real-world (A-OKVQA) datasets.


<details>
  <summary>Details</summary>
Motivation: VLMs struggle with compositional visual reasoning that requires multi-step chaining (e.g., identifying objects, counting, comparing). Human-written step-by-step explanations are expensive and difficult to scale.

Method: A self-questioning framework using Group Relative Policy Optimization (GRPO) trains a VLM to decompose questions into sub-questions and answer them. The model receives no decomposition examples; it discovers the strategy via reward signals that score both sub-question presence and final answer correctness.

Result: On A-OKVQA, self-questioning achieved 52.2% accuracy vs. 51.6% (standard RL) and 46.8% (untrained model). Applied to a 3-billion-parameter model on CLEVR and A-OKVQA datasets.

Conclusion: Teaching VLMs to self-generate intermediate questions is a promising strategy for complex visual reasoning, especially when explicit step-by-step decomposition is beneficial.
...
TL;DR: SpatialAvatar-0 bridges feed-forward prediction and per-subject refinement for 4D head avatars using a shared FLAME-mesh-bound Gaussian representation, achieving state-of-the-art results across multiple benchmarks with up to 60x faster per-subject optimization.


<details>
  <summary>Details</summary>
Motivation: Existing 4D head avatar methods have two separate regimes (feed-forward predictors and per-subject refiners) that cannot share representations end-to-end due to domain bias, hard-coded source counts, and incompatible optimization processes (adaptive densification destroys Gaussian layouts).

Method: Proposes SpatialAvatar-0 with: (1) a shared FLAME-mesh-bound Gaussian representation, (2) a parameter-free K-source mean-pool feed-forward generator, (3) a monocular-temporal to multi-view-spatial two-phase training schedule, and (4) a 10K-iter layout-preserving per-subject refinement loop that freezes FLAME-binding, Gaussian count, and replaces densification with three-component anti-spike regularization.

Result: Surpasses GAGAvatar by +1.5 dB PSNR on VFHQ/HDTF cross-domain zero-shot, and leads every reported metric on SplattingAvatar benchmark, surpassing GeoAvatar by +1.3 dB PSNR at up to 60x shorter per-subject schedule.

Conclusion: SpatialAvatar-0 successfully unifies feed-forward and per-subject refinement regimes in a single framework, demonstrating superior cross-domain generalization and efficiency.
...
TL;DR: This paper introduces MMXray (52,124 X-ray image-caption pairs across 28 contraband classes), CleanDET/AnyContraSyn for realistic occlusion synthesis, and OneFocus, a unified VLM achieving SOTA on four X-ray contraband understanding tasks.


<details>
  <summary>Details</summary>
Motivation: Conventional X-ray contraband detectors fail to adapt to emerging contraband types and lack fundamental visual understanding; VLMs offer strong generalization but are limited by the scarcity of high-quality X-ray image-caption data.

Method: (1) MMXray benchmark: 52,124 image-caption pairs, 28 fine-grained classes. (2) CleanDET dataset + AnyContraSyn controllable synthesis method for realistic occlusion patterns. (3) OnePipe extensible curation pipeline. (4) OneFocus unified VLM supporting VQA, localization, classification, and image understanding.

Result: OneFocus achieves state-of-the-art performance in X-ray contraband understanding and demonstrates robust cross-domain generalization.

Conclusion: The proposed benchmark, synthesis method, and unified VLM establish a strong vision-language baseline for security screening, addressing the critical data scarcity issue for VLM-based X-ray contraband detection.
...
TL;DR: This paper proposes a transformer-based framework for automated sealing zone assessment after EVAR, combining 3D centerline tracking with embedding-based geometric prediction. The method outperforms commercial semi-automatic workflows on both standard and no-contrast CT scans.


<details>
  <summary>Details</summary>
Motivation: Long-term mortality after EVAR is elevated due to post-EVAR rupture from loss of seal in stent graft sealing zones. Current CT workflows require manual centerline editing and expert operators, which is time-consuming and limits scalability.

Method: A transformer framework combining 3D centerline tracking with embedding-based geometric prediction. Two state-of-the-art image-to-graph models are evaluated for aorto-iliac centerline extraction and measurement of stent position, vessel diameters, and seal lengths according to the EVAR4C protocol.

Result: The proposed fully automatic method outperforms the commercial semi-automatic workflow across the full test set and a challenging no-contrast subset.

Conclusion: Automated transformer-based sealing zone assessment can replace manual/semi-automatic workflows, potentially improving post-EVAR surveillance and reducing rupture risk.
...
TL;DR: Introduces a self-supervised monocular video depth estimation method reformulated as multi-view 3D reconstruction, using a 3D consistency optimization framework with photometric, geometric alignment, and temporal gradient constraints to achieve globally coherent 3D structure.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised approaches treat frames independently or use weak temporal regularization, lacking holistic 3D scene perception, leading to geometrically inconsistent predictions and cross-frame drift in endoscopic navigation.

Method: Recasts sequential video depth estimation as unconstrained multi-view 3D reconstruction; employs a 3D consistency optimization framework with three constraints: image-level photometric rendering, explicit world-coordinate geometric alignment, and multi-scale temporal gradient consistency.

Result: Validated in self-supervised training and zero-shot clinical environments; achieves state-of-the-art spatial accuracy, outperforming frame-based, video-based depth estimators, and multi-view 3D reconstruction baselines.

Conclusion: The proposed 3D consistency optimization framework effectively anchors isolated frames to a globally coherent 3D structure, leveraging geometric priors from 3D foundation models for reliable endoscopic depth estimation.
...
TL;DR: OmniTraffic is a controllable generation pipeline and benchmark for spatio-temporal traffic reasoning, featuring 12 reconstructed 3D intersections, 8M VQA samples, and a three-level task hierarchy. Evaluation shows large human-model gaps in topology and spatio-temporal reasoning, with simulation data improving real-world performance.


<details>
  <summary>Details</summary>
Motivation: Existing traffic multimodal benchmarks mainly focus on passive visual recognition or isolated video understanding, lacking support for evaluating structure-aware traffic reasoning (lane topology, multi-view geometry, temporal evolution, signal-phase semantics) under controlled conditions.

Method: Built 12 real-world intersections into editable 3D traffic environments, created a three-level task hierarchy (scene perception, multi-view/temporal reasoning, decision support), generated synchronized multi-view VQA samples using structured traffic metadata, totaling 8M VQA samples plus a 3K human-verified test set.

Result: Eleven MLLMs showed large human-model gaps, especially in topology-grounded and spatio-temporal reasoning. Fine-tuning a lightweight MLLM on OmniTraffic simulated data improved real-world traffic scene performance.

Conclusion: OmniTraffic demonstrates the value of simulation-generated supervision for traffic-specific multimodal reasoning, and provides an extensible pipeline with configurable environments for future research.
...
TL;DR: This paper challenges the common assumption that poor performance on rare emotions is due to class imbalance, showing instead that the issue stems from affect geometry—rare classes are degenerate on Russell's circumplex. The authors introduce a circumplex-cost optimal-transport term that improves error structure but not rare-class accuracy. They conclude that progress requires better representations, not better loss functions.


<details>
  <summary>Details</summary>
Motivation: Standard explanation for poor performance on rare emotions is class imbalance; the authors hypothesize the real cause is the geometric degeneracy of rare classes on the circumplex model of affect.

Method: Multi-task controlled study using a circumplex-cost optimal-transport term that prices expression confusions by valence-arousal distance, tested on Aff-Wild2 and AffectNet benchmarks with uniform-cost and action-unit-augmented controls.

Result: The circumplex term improves error structure (affectively nearer errors on Aff-Wild2) but is matched or exceeded by a simple uniform-cost control. Rare-class failures (anger-fear on Aff-Wild2, anger-contempt on AffectNet) persist across all interventions.

Conclusion: Progress on rare expressions requires representations that can distinguish the classes, not supervision that re-prices confusions; the paper provides controls and metrics for disentangling these two approaches.
...
TL;DR: TIGER coordinates multiple heterogeneous vision foundation models for multi-task dense prediction by using natural-language task instructions to guide token-level expert routing, enhanced by a counterfactual loss for causal alignment, achieving SOTA results on NYUD-v2 and Pascal Context.


<details>
  <summary>Details</summary>
Motivation: Existing vision foundation models exhibit strong but fragmented inductive biases from different pre-training objectives/domains, so a single model cannot capture diverse visual representations needed across multiple dense prediction tasks.

Method: TIGER uses a routing network guided by natural-language task instructions to assign token-level expert weights, adaptively integrating complementary frozen VFM features. A counterfactual loss aligns routing decisions with each expert's causal contribution by measuring prediction changes when experts are excluded.

Result: TIGER consistently outperforms recent multi-task learning baselines on NYUD-v2 and Pascal Context benchmarks while keeping all VFMs frozen.

Conclusion: Combining instruction-guided expert routing with counterfactual causal alignment enables effective coordination of heterogeneous vision foundation models for multi-task dense prediction.
...
TL;DR: Presents two frameworks for automatic glaucoma detection from color fundus images: a deep learning fusion approach (99.3% detection rate) and a lightweight image-processing method (92.31% accuracy), both using an adaptive ellipse-based polar transformation to analyze the RNFL.


<details>
  <summary>Details</summary>
Motivation: To address the global challenge of glaucoma, especially in underserved regions, by developing scalable and cost-effective automatic detection methods using easily available color fundus images.

Method: An adaptive ellipse-based polar transformation algorithm enhances RNFL analysis regardless of optic disc and macula position, then feeds into two frameworks: (1) deep learning feature fusion, and (2) bit-plane slicing image-processing algorithm.

Result: Deep learning framework: 99.3% detection rate (high precision, higher computational cost). Image-processing framework: 92.31% accuracy (rapid inference, minimal resource consumption).

Conclusion: Both frameworks offer scalable, cost-effective solutions for early glaucoma detection, with potential impact in underserved regions by leveraging RNFL-based diagnostic tools.
...
TL;DR: FACR is a framework that improves faithfulness of AU->emotion reasoning by using a polarity-aware causal graph and a counterfactual-consistency training objective, achieving significantly higher interventional agreement with known causal structures across datasets.


<details>
  <summary>Details</summary>
Motivation: Multimodal models can name action units (AUs) behind facial emotions, but their AU->emotion rationales are typically plausible rather than faithful—nothing forces the AUs a model invokes to be the AUs that actually drive its prediction.

Method: FACR casts AU->emotion reasoning as a counterfactual-consistency problem between rationale, label, and a structural AU->emotion causal graph G. It uses an independently induced, polarity-aware G and trains with a counterfactual-faithfulness objective: do-intervention on a causal AU must shift prediction, while intervention on an irrelevant AU must leave it unchanged. A language verbalizer with biased AU emission makes rationales faithful by construction.

Result: On UNBC-PAIN, agreement between invoked AUs and PSPI composition rose from 0.08 to 0.57 under subject-independent evaluation. On cross-dataset emotion transfer (7-class), fidelity to G rose from 0.50 to 0.84.

Conclusion: Faithfulness in AU->emotion reasoning is both trainable and measurable via interventional metrics. The counterfactual-faithfulness objective significantly improves fidelity to causal structures without sacrificing detection performance, and the approach extends to text generation through faithful-by-construction rationale design.
...
TL;DR: This paper introduces a zero-shot adaptation framework for using Segment Anything Model (SAM) in seismic interpretation, combining geological target-aware seismic attribute/colormap selection with a hybrid prompting strategy (sparse user points + dense mask prompts from SAM's internal features). The approach achieves competitive segmentation performance without fine-tuning, eliminating the need for retraining on specific geological targets.


<details>
  <summary>Details</summary>
Motivation: Existing applications of SAM for seismic interpretation rely on fine-tuning for specific geological targets, which requires extensive labeled data, high computational costs, and compromises model generalization. The authors aim to enable zero-shot adaptation of foundation models to seismic data.

Method: The framework has two components: (1) aligning seismic attributes and visualization choices (colormaps) with the geological target of interest, and (2) a hybrid prompting strategy combining sparse user-defined point prompts with dense mask prompts derived from SAM's internal feature activations.

Result: Geologic target-aware selection of seismic attributes and colormaps combined with hybrid prompting enhances feature separability, improves boundary delineation, and yields better segmentation accuracy compared to point-based prompting alone. Competitive segmentation performance is achieved in a fully zero-shot setting.

Conclusion: This work establishes a practical and scalable pathway to leverage foundation models in seismic interpretation, reducing reliance on labeled data while preserving model generality, and eliminating the need to retrain SAM for each geologic feature.
...
TL;DR: This paper extends transcoder-based circuit tracing to multimodal diffusion transformers (specifically FLUX.1[schnell]), enabling interpretable circuit discovery via timestep-conditioned transcoders that replace MLP sublayers, revealing mechanisms for attribute binding, cross-stream semantic propagation, and systematic generation errors, with superior intervention precision over SAE-based steering.


<details>
  <summary>Details</summary>
Motivation: While mechanistic interpretability tools (transcoder-based circuit tracing) exist for large language models, multimodal diffusion transformers for image generation remain opaque—lacking methods to understand semantic propagation across denoising steps and text-image interactions within double-stream MM-DiT architectures.

Method: Train timestep-conditioned transcoders that faithfully approximate MLP sublayer input-output behavior in FLUX.1[schnell], replace MLPs with transcoders, linearize remaining computation, and perform exact feature-to-feature attribution to recover compact circuits.

Result: Transcoders match or slightly outperform sparse autoencoders on the sparsity-faithfulness tradeoff. Circuits reveal attribute binding, cross-stream semantic propagation, and causal explanations for systematic generation errors. Circuit-guided interventions are substantially more precise and effective than standard SAE-based steering.

Conclusion: Transcoder-based circuit analysis is feasible for state-of-the-art diffusion transformers and provides a powerful framework for understanding and controlling multimodal generative models.
...
TL;DR: CPS4 is the first text-guided semi-supervised spine segmentation network using VLM with class-specific consistency constraints, achieving 80.44% Dice using only 5% labeled data.


<details>
  <summary>Details</summary>
Motivation: VLMs have potential to improve pseudo labels in semi-supervised spine segmentation via textual class prompts, but existing methods lack constraints to ensure consistency between spine class prompts and spine unit regions, leading to poor multi-class segmentation.

Method: Two-stage approach: (1) Class-specific consistency constrained VLM pretraining with token- and pixel-level attention loss to align class prompts with spine units; (2) Class prompt driven semi-supervised segmentation that generates binary maps per class and integrates them into a unified multi-class segmentation map to enhance pseudo labels.

Result: Achieves 80.44% Dice on public spine segmentation dataset using only 5% labeled data, outperforming existing semi-supervised learning and VLM methods.

Conclusion: CPS4 demonstrates that text-guided consistency constraints effectively improve spine pseudo label quality in semi-supervised settings, setting a new state-of-the-art for spine segmentation with limited annotations.
...
TL;DR: This paper proposes SACE, the first scale-aware concept erasure framework for visual autoregressive (VAR) models, based on the Semantic Singularity Axiom that target semantics are locked at Scale-0. By confining interventions to the first scale with entropy-regularized erasure and restorative preservation loss, it prevents semantic collapse and achieves surgical concept removal.


<details>
  <summary>Details</summary>
Motivation: Existing erasure techniques designed for diffusion models cause catastrophic semantic collapse and visual artifacts when naively applied to VAR models due to fundamental architectural differences (homogeneous denoising steps vs. autoregressive scale generation).

Method: (1) Proposes Semantic Singularity Axiom (target semantics locked at Scale-0), validated by Incremental Semantic Saliency Analysis (ISSA). (2) Introduces SACE framework: scale-aware intervention only at first scale, with entropy-regularized erasure objective (prevents high-entropy sampling degeneration) and restorative preservation loss (protects entangled benign priors).

Result: Achieves surgical concept erasure across multiple domains with minimal training overhead, avoiding semantic collapse and visual artifacts that plague naive erasure application.

Conclusion: SACE timely resolves critical safety vulnerabilities in emerging VAR architectures, providing an interpretable and efficient solution for concept erasure in autoregressive text-to-image models.
...
TL;DR: VarDeepPCA is a lightweight variational DNN framework that restores/refines degraded OOD segmentation maps by learning anatomical geometry priors from small ID datasets, achieving improved anatomical plausibility and reduced errors across 4 clinical applications without requiring target-domain data.


<details>
  <summary>Details</summary>
Motivation: DNNs fail to generalize to OOD medical images due to scanner/protocol variations, and retraining is impractical because acquiring/annotating new medical data is expensive.

Method: A variational learning framework that leverages a reinterpretation of softmax mapping to implicitly perform exact distribution modeling, enabling sampling-free learning and inference while providing uncertainty estimates.

Result: Validated across 4 clinical applications (myocardium, neuroretinal rim, prostate, fetal head) using 14 datasets; compared against 15 methods, showing significant improvements in anatomical plausibility, clinical utility, and error reduction.

Conclusion: VarDeepPCA effectively restores OOD segmentation maps without needing target-domain data or extensive pre-training, using only small ID datasets for learning valid anatomical geometries.
...
TL;DR: EmoZone-Talker is a 3DGS-based talking head synthesis framework that explicitly disentangles speech-driven and expression-driven facial dynamics using Synergy Zones with Prioritized Attention Bias and a Channel-Independent Temporal AU Encoder, achieving improved expression controllability and temporal coherence.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods for talking head synthesis struggle with fine-grained, interpretable, and editable facial expression control due to intrinsic conflicts between speech-driven facial dynamics and explicit expression signals. Current implicit multimodal fusion approaches cause spatial entanglement and temporal instability.

Method: (1) Synergy Zones with Prioritized Attention Bias (SZ-PAB): explicitly decouples modality contributions using region-wise constraints guided by anatomical priors; (2) Channel-Independent Temporal AU Encoder (CIT-AE): models temporally coherent Action Unit dynamics; both integrated into 3D Gaussian deformation for interpretable expression control.

Result: Improves expression controllability and realism, with notable gains in upper-face accuracy and temporal coherence, while preserving high rendering quality and accurate lip synchronization.

Conclusion: Structured spatial-temporal coordination effectively resolves cross-modal conflicts in audio-driven facial animation, enabling precise and interpretable expression control in 3DGS-based talking head synthesis.
...
TL;DR: Proposes a dual-branch underwater image enhancement framework (detail enhancement + color restoration) that improves visual quality and boosts YOLOv8 object detection performance by 2.1% mAP50 on URPC dataset.


<details>
  <summary>Details</summary>
Motivation: Underwater images suffer from color distortion and blurred details due to wavelength-dependent light absorption/scattering, and existing methods struggle to balance enhancement quality, efficiency, and downstream detection performance.

Method: A dual-branch framework: (1) Detail enhancement branch improves brightness and local contrast to recover texture details in dark regions; (2) Color restoration branch uses adaptive compensation to reduce color distortion and improve color gradation. The outputs of both branches are combined to produce clearer images for detection.

Result: UIQM scores of 2.249 (UIEB) and 2.576 (EUVP); improves YOLOv8 mAP50 by 2.1% on URPC dataset.

Conclusion: The method effectively improves object detection in complex underwater scenes while balancing enhancement quality and processing efficiency.
...
TL;DR: The paper proposes a unified framework combining VLM with SAM-based segmentation to jointly perform pixel-level segmentation and VQA in robotic surgery, using object tokens to improve fine-grained spatial understanding and visual reasoning.


<details>
  <summary>Details</summary>
Motivation: Existing surgical VQA methods rely on coarse visual grounding (e.g., bounding boxes), which fails to capture fine-grained spatial structures of surgical objects, limiting the model's ability to understand complex surgical scenes.

Method: A unified framework integrating a VLM with a SAM-based decoder, where scene elements are represented as object tokens generated by the VLM. These tokens guide answer prediction and are projected to SAM for segmentation masks, jointly optimized through both segmentation and QA objectives.

Result: The method consistently outperforms baseline surgical VQA methods on the private RAMIE dataset and public EndoVis18 dataset.

Conclusion: Incorporating context-aware object tokens into VLMs improves fine-grained surgical scene understanding by providing explicit pixel-level grounding and enhancing visual reasoning.
...
TL;DR: CogCanvas is a 1,952-image benchmark for multi-subject reference-based image generation, evaluating 5 SOTA methods across 2-5 person groups, revealing severe degradation in identity preservation and object binding beyond three subjects.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models struggle with multi-subject image generation that jointly preserves multiple identities, binds per-person objects/items, and respects background scenes. Existing benchmarks evaluate only single axes (e.g., identity or object binding) and none captures the full complexity of multi-identity composition with human-object interaction, background grounding, and spatial plausibility.

Method: CogAnchor: 1,952 curated images (100 celebrities, 115 objects/items, 29 backgrounds) → 1,361 compositional prompts (2-5 persons). Pipeline: DINOv2 deduplication, two-stage aesthetic filtering, automated interaction/position graphs. Three tasks: multi-human-object generation (primary), text-to-image generation, reference retrieval. Six-axis evaluation with BG-Sim (DINOv3 on SAM masks) and Attr-VQA (multimodal LLM verification).

Result: All 5 SOTA methods degrade substantially from 2→5 persons; near-complete failure on object/fashion binding beyond three subjects.

Conclusion: Multi-subject reference-based generation remains a severely unsolved challenge, particularly for larger group sizes and fine-grained attribute binding.
...
TL;DR: Metis is an end-to-end World Action Model framework that decouples video generation from action prediction using a Mixture-of-Transformers architecture with asymmetric attention masking, achieving state-of-the-art performance on navigation benchmarks while significantly reducing inference latency.


<details>
  <summary>Details</summary>
Motivation: Existing WAM approaches suffer from high inference latency due to future observation prediction at test time, and tightly coupled video and action modeling leads to representational mismatch and degraded generalization.

Method: Metis uses a Mixture-of-Transformers architecture with dedicated experts for video generation and action prediction, plus an asymmetric attention mask enabling joint training while allowing the action model to bypass video generation during inference.

Result: State-of-the-art performance on NAVSIM navhard/navtest benchmarks and CityWalker navigation benchmark, with real-robot deployment validation demonstrating practical feasibility.

Conclusion: Decoupling video generation and action prediction with asymmetric attention masking effectively reduces computational costs while maintaining planning performance, validated across diverse tasks and real-world deployment.
...
TL;DR: Deep-VRM is a multimodal LLM that preserves semantic knowledge while injecting artifact-specific visual signals via residual paths, achieving state-of-the-art AI-generated image detection by adaptively fusing low-level forensic cues with high-level semantic reasoning.


<details>
  <summary>Details</summary>
Motivation: As AI-generated images become increasingly realistic, semantic-level inconsistencies alone are insufficient for reliable detection. The authors question whether MLLMs can achieve full-spectrum forensic signal perception—capturing low-level generator artifacts without sacrificing pre-trained semantic knowledge.

Method: The authors propose Deep Visual Residual MLLM (Deep-VRM), which preserves early semantic processing in early-to-middle layers while injecting artifact-specific visual signals as a residual path into an intermediate layer. These signals are fused with semantic token representations and propagated through subsequent trainable layers, enabling joint modeling of semantic reasoning and signal-level forensic cues.

Result: The model learns to adaptively leverage different levels of forensic signals depending on the input, achieving robust and generalizable detection performance. Extensive experiments show state-of-the-art results across most benchmarks.

Conclusion: Layer-wise analysis shows semantic information forms in early-to-middle layers, and direct fine-tuning disrupts these representations. Deep-VRM's residual path design successfully preserves semantics while enabling artifact learning, leading to robust AI-generated image detection.
...
TL;DR: This paper introduces a large-scale, open-access dataset of 948 historical astronomical diagrams with 10,940 annotated polygonal text regions spanning ten centuries and seven linguistic traditions, along with Poly-DETR—a simple extension of DINO-DETR for ordered polygon prediction—that achieves state-of-the-art results on existing benchmarks.


<details>
  <summary>Details</summary>
Motivation: Text detection in historical mathematical diagrams has received little attention despite its importance; existing datasets focus on manuscripts and maps, leaving a gap in studying text in astronomical diagrams.

Method: The authors created a dataset of 948 historical astronomical diagrams (8th–18th centuries, 7 languages) with 10,940 oriented polygonal text regions annotated with reading direction. They also designed Poly-DETR, a simple extension of DINO-DETR, to predict ordered polygon vertices for text detection.

Result: Poly-DETR achieves state-of-the-art performance on MTHv2 and cBAD2019 benchmarks and provides a solid baseline on their new dataset.

Conclusion: The dataset and Poly-DETR address the understudied problem of text detection in historical astronomical diagrams, providing resources for future research.
...
TL;DR: SiGnature is a framework for co-speech gesture generation that achieves both semantic meaning and speaker-specific style preservation through a training-free Joint Motion Integration (JMI) mechanism operating in explicit joint-rotation space, enabling plug-and-play injection of external semantic gestures without retraining or artifacts.


<details>
  <summary>Details</summary>
Motivation: Existing co-speech gesture generation methods achieve good rhythmic synchronization but fail to produce semantically meaningful gestures (e.g., iconic shapes, pointing) while preserving a speaker's unique non-verbal style, because semantic gestures are statistically sparse and hard to learn in standard generative models with entangled latent representations.

Method: SiGnature operates in explicit joint-rotation space and introduces Joint Motion Integration (JMI), a training-free inference mechanism that automatically identifies "active joints" conveying a semantic action from an external motion sequence and injects them into the diffusion process, while the backbone synthesizes remaining body dynamics in accordance with the pre-learned speaker style.

Result: Extensive experiments and perceptual studies show SiGnature offers superior semantic motion control while maintaining smooth, natural co-speech gestures and preserving distinct speaker characteristics, outperforming state-of-the-art baselines without retraining or Frankenstein artifacts.

Conclusion: SiGnature effectively reconciles the long-standing tension between semantic gesture control and style preservation in co-speech gesture generation through its training-free JMI mechanism in explicit joint space, enabling plug-and-play integration of arbitrary semantic motions.
...
TL;DR: A template-free, markerless system for robust 4D hand-object interaction reconstruction from multi-view videos using a feed-forward transformer for initialization and a physics-aware Gaussian optimization for refinement.


<details>
  <summary>Details</summary>
Motivation: Existing 4D hand-object interaction reconstruction relies on pre-scanned object templates and physical markers, and current methods are highly sensitive to initial pose estimates which are difficult under severe occlusion.

Method: (1) Multi-view feed-forward transformer aggregating cross-view geometry and temporal cues for robust initialization; (2) Physics-aware Gaussian optimization with tetrahedral constraints, collision refinement, and appearance decomposition.

Result: Validated on public benchmarks and internal dataset, achieving robust, artifact-free reconstruction without templates or markers.

Conclusion: Provides an efficient, automated foundation for 4D asset generation in embodied AI and spatial computing.
...
TL;DR: OmniOPSD is a framework that uses frontier-model-generated rationales as privileged evidence for a teacher model to provide dense token-level supervision, enabling student MLLMs to learn without direct imitation of frontier models or expensive annotations. It achieves SOTA performance (84.19) on MER-UniBench.


<details>
  <summary>Details</summary>
Motivation: Reinforcement learning for MLLMs faces severe reward sparsity in complex reasoning tasks, especially human-centered scenarios involving heterogeneous multimodal signals and subjective human factors. High-quality CoT annotations are expensive, and using ground-truth labels for SFT encourages shortcut learning with limited transparency.

Method: OmniOPSD uses frontier-generated evidence-aware rationales as privileged information for a local teacher model during training. The student samples rollouts from original multimodal inputs, while the rationale-privileged teacher scores tokens and provides dense token-level supervision. This avoids direct imitation of frontier-model completions.

Result: OmniOPSD achieves state-of-the-art performance with an average score of 84.19 on MER-UniBench. Ablation studies confirm the value of rationale-privileged teacher guidance.

Conclusion: The rationale-privileged on-policy self-distillation approach effectively addresses reward sparsity and shortcut learning issues in multimodal reasoning. Inference requires no labels, rationales, CoT annotations, or closed-source model access, making it practical for real-world deployment.
...
TL;DR: TurboGS accelerates 3D Gaussian Splatting training by focusing optimization on perceptually informative pixels, achieving up to 10x speedup over vanilla 3DGS with comparable or better rendering quality in as little as 100 seconds on a single RTX 5090 GPU.


<details>
  <summary>Details</summary>
Motivation: Consumer-level applications need fast optimization of 3DGS with high-fidelity rendering, but existing acceleration methods waste computation on redundant pixels and lose fine details.

Method: Four components: (1) tile-wise sparse pixel sampling guided by multi-view reconstruction errors; (2) tile-wise structure-aware loss with sparse Normalized Cross-Correlation; (3) error-driven Gaussian density control; (4) hybrid optimizer combining Hessian-informed updates with Adam moment damping.

Result: On standard benchmarks, TurboGS achieves on par or superior rendering quality within 100 seconds on a single RTX 5090 GPU, delivering up to 10x training speedup over vanilla 3DGS.

Conclusion: TurboGS effectively accelerates 3DGS training by focusing computation on challenging regions, preserving fine details, and stabilizing convergence, making it suitable for consumer-level applications.
...
TL;DR: GOOSE-M2F is a task-specific adaptation of Mask2Former that achieves 3rd place on the GOOSE 2D FGSS leaderboard (70.08% Composite mIoU) by addressing long-tailed distribution in unstructured outdoor terrain segmentation through 200 object queries, a Feature Refinement Module (ASPP-lite + CBAM), auxiliary supervision, multi-stage training strategies, and dense sliding-window inference with TTA.


<details>
  <summary>Details</summary>
Motivation: To address the GOOSE benchmark's challenge of 64 fine-grained classes with severely long-tailed distribution (rare classes under 50 pixels/image), which causes representational saturation and poor rare-class performance in standard Mask2Former.

Method: Extends Swin-Large Mask2Former with: (1) 200 Object Queries to prevent representational saturation; (2) Feature Refinement Module (FRM) combining ASPP-lite and CBAM dual-attention; (3) Auxiliary Supervision Head for direct per-pixel gradients on rare classes. Multi-stage training uses Distribution-Balanced loss, Rare-Class Copy-Paste, dynamic IoU-aware re-weighting, and EMA. Inference employs dense sliding-window with 2D Gaussian kernel blending and 4-scale TTA.

Result: Achieves 70.08% Official Composite mIoU (63.55% fine, 76.61% coarse), placing 3rd on the GOOSE 2D FGSS leaderboard. Dense sliding-window inference with TTA adds +10.57% improvement.

Conclusion: GOOSE-M2F demonstrates that targeted architectural modifications (increased queries, feature refinement, auxiliary supervision) combined with careful training strategies effectively mitigate long-tail distribution challenges in fine-grained semantic segmentation for unstructured outdoor environments.
...
TL;DR: ProTrans is a vision-language pretraining framework that models disease progression as directional semantic transitions between paired chest X-rays, using radiology reports to anchor representations and outperforming existing methods on longitudinal analysis tasks.


<details>
  <summary>Details</summary>
Motivation: Existing CXR longitudinal analysis methods struggle to capture subtle progression semantics and overlook the inherently directional nature of disease trajectories.

Method: ProTrans formulates disease progression as directional semantic transitions between paired CXR studies, uses radiology reports to anchor individual CXR representations in disease states, introduces a learnable progression feature map for semantic shifts, incorporates reversed temporal modeling, and imposes bidirectional reconstruction consistency.

Result: Outperforms existing methods consistently on disease progression classification and progression captioning tasks.

Conclusion: ProTrans establishes a unified pretraining framework for longitudinal CXR understanding by explicitly encoding directional disease trajectories.
...
TL;DR: This paper proposes that as data scales up, strong inductive biases become bottlenecks, and introduces TDV (Temporal Difference in Vision) - a self-supervised learning method from video that avoids common biases by using only a causal assumption (past causes future), achieving competitive results on dense spatial tasks.


<details>
  <summary>Details</summary>
Motivation: The paper observes a trend in visual representation learning where methods with weaker inductive biases outperform those with stronger assumptions as data/compute scales. The authors hypothesize that even modern SSL biases (augmentations, masking, cropping) will become bottlenecks at scale, motivating the search for approaches with fewer assumptions.

Method: TDV jointly trains an image encoder and a motion encoder such that the current frame's representation plus the encoded motion equals the next frame's representation. It only assumes a causal relationship (past causes future) and avoids augmentations, masking, or cropping biases.

Result: Experiments confirm that optimal inductive bias strength decreases as data grows. Despite having no strong inductive biases, TDV matches state-of-the-art results on dense spatial tasks.

Conclusion: The paper lays the foundation for representation learning without strong assumptions, suggesting that causal assumptions from video may be sufficient for effective visual learning at scale.
...
TL;DR: Presents VEPHand, an end-to-end pipeline for dynamic 3D hand capture from limited multi-view setups (~20 views), featuring mask-free neural reconstruction and physics-inspired registration for high-fidelity hand modeling.


<details>
  <summary>Details</summary>
Motivation: To overcome the challenge of reconstructing high-fidelity 3D hands from practical multi-view systems with limited viewpoint density, addressing issues like geometric ambiguity, limited view overlap, background clutter, and non-linear skin deformations under self-contact.

Method: Two-stage pipeline: (1) A mask-free neural reconstruction method using scene parameterization and scenario-specific density regularization to extract hand geometry/appearance from unmasked images. (2) A physics-inspired registration framework that aligns reconstructions to a personalized hand model via intrinsic volumetric offsets within a canonical tetrahedral mesh and pose optimization, supported by robust losses.

Result: State-of-the-art reconstruction fidelity in view-efficient, unmasked scenarios with highly accurate registration. Validated on over 12,000 sequences including single hands, two-hand interactions, and hand-object manipulations.

Conclusion: The proposed automated pipeline is scalable, robust, and achieves top-tier performance for dynamic hand capture in practical multi-view setups. A large-scale synthetic dataset derived from this work also supports downstream tasks.
...
TL;DR: CRIS is a self-supervised framework for restoring isotropic resolution from anisotropic volumetric data (MRI and electron microscopy) without requiring paired isotropic ground truth. It works by treating 3D restoration as 2D stripe completion on orthogonal reformats, achieving state-of-the-art results across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Anisotropic volumetric acquisitions (e.g., thick slices in clinical MRI and volume electron microscopy) degrade orthogonal view reformats and downstream analysis. Existing methods typically require paired isotropic ground truth or are modality-specific, limiting their applicability.

Method: CRIS casts 3D isotropic restoration as a 2D stripe completion problem on orthogonal reformats of an isotropic grid. During training, high-resolution in-plane slices are synthetically degraded and periodically masked. At inference, blank slices define the isotropic grid, two orthogonal reformats are restored independently, and predictions are fused via multi-view averaging.

Result: On brain MRI: 32.921 dB PSNR, 0.9631 SSIM, best segmentation consistency (Dice 0.940). On abdominal MRI (reference-free): FID 48.714, KID 0.023. On vEM: outperforms baselines at 4x and 8x anisotropy. Robustness experiment: single model handles gap factors 3-7 across coronal/axial/sagittal degradations, outperforming interpolation (36.36-31.14 dB vs 33.07-27.85 dB PSNR).

Conclusion: CRIS provides a modality-flexible, self-supervised route to isotropic restoration that does not require paired isotropic targets or configuration-specific retraining, with code publicly available.
...
TL;DR: HadBalance integrates Hadwiger Shape Priors (area, perimeter, Euler characteristic) as a global regularizer for near-convex medical image segmentation, paired with Conflict-Aware Objective Balancing (CAOB) to prevent over-regularization on non-convex anatomy, enabling shape-heterogeneous, multi-organ, multi-modal use.


<details>
  <summary>Details</summary>
Motivation: Existing geometric priors for medical segmentation are task-specific and lack a unified foundation; many targets are near-convex but datasets are shape-heterogeneous, causing uniform convex priors to over-regularize non-convex structures and wash out fine details.

Method: Derives Hadwiger Shape Priors (area, perimeter, Euler characteristic) from Hadwiger's theorem as a global regularizer; proposes Conflict-Aware Objective Balancing (CAOB) that removes only gradient components conflicting with segmentation while preserving aligned ones, adaptively regulating prior influence to avoid dominance.

Result: Enables stable use of shape priors across shape-heterogeneous data without erasing genuine concavities or fine structural details, improving segmentation accuracy compared to uniform prior enforcement.

Conclusion: HadBalance is a plug-and-play framework that generalizes across organs and modalities by combining interpretable geometric priors with gradient-aware conflict balancing.
...
TL;DR: This paper investigates how multimodal LLMs struggle with identifying secondary visual constraints triggered by local text edits in images. Models achieve only 46% recall under unguided prompting versus 94% with explicit constraints, showing failures stem from deciding which dependencies to surface. Causal explanations are the most effective guidance, and higher self-discovery recall doesn't guarantee better downstream task performance due to false positives.


<details>
  <summary>Details</summary>
Motivation: To study whether multimodal reasoning models can determine which visual dependencies become relevant under a specific task (not just recognize visible content), using edit-induced constraint discovery in text-in-image editing as a controlled diagnostic setting.

Method: Created 461 diagnostic cases with 19 constraint subtypes across 4 MLLMs. Used oracle-field decomposition to compare different guidance types (causal explanations, region names, type labels). Conducted downstream experiments to evaluate precision-aware constraint elicitation.

Result: Unguided prompting yields only 46% case-level macro recall vs 94% with explicit constraints. Causal explanations achieve 0.782 recall (best partial guidance), above region names (0.610) and type labels (0.646). Higher self-discovery recall does not necessarily improve downstream task performance due to false positives.

Conclusion: A substantial portion of multimodal reasoning failures arise when models must decide which unstated dependencies to surface, not just from recognizing visible content. Precision-aware constraint elicitation is needed, as unverified self-discovery can introduce harmful false positives.
...
TL;DR: SCAM is a new line-level handwritten text recognition dataset for the extinct Sahidic Coptic language, featuring realistic historical document challenges (degradations, low-resource language). Benchmarking shows a significant performance gap versus well-resourced modern scripts.


<details>
  <summary>Details</summary>
Motivation: To address the lack of datasets for handwritten text recognition in low-resource scenarios, particularly for extinct languages like Sahidic Coptic with scarce linguistic resources, uncommon alphabets, and degraded historical manuscripts.

Method: Created SCAM dataset from digitized ancient Sahidic Coptic manuscripts across libraries; benchmarked multiple state-of-the-art HTR approaches from different paradigms on this dataset.

Result: Identified a clear performance gap between current HTR systems on well-resourced modern scripts versus low-resource historical scenarios; highlighted limitations and strengths of existing approaches.

Conclusion: SCAM provides a realistic benchmark for low-resource HTR research; substantial progress is still needed to bridge the performance gap with well-resourced modern scripts.
...
TL;DR: A multi-task RGB-video pipeline (TennisTransformerGPU) that recognizes tennis strokes, predicts shot direction, and grades posture quality, achieving 83.7% stroke accuracy but failing to transfer direction across players.


<details>
  <summary>Details</summary>
Motivation: To automate tennis biomechanics analysis from plain RGB video without manual annotation, adding shot direction and posture quality as new tasks beyond existing stroke recognition.

Method: Uses MediaPipe for 3D pose, a weighted joint velocity score for automatic stroke detection, and a compact transformer (564K params, 4 layers, 4 heads, d=128) with three parallel output heads trained on 1,281 labeled strokes.

Result: 83.7% stroke-type accuracy, 61.9% direction, 62.6% posture. Cross-player: stroke type drops 0.8% to 82.9%, direction collapses to majority class. World coordinates are critical (image-space drops accuracy from 83% to 47%).

Conclusion: Stroke type transfers well across players, but direction prediction does not; world coordinates are essential. The pipeline is reproducible on free T4 GPU.
...
TL;DR: Stringalign is a Python library that provides transparent, reproducible, and unambiguous string comparison tools for evaluating text recognition and transcription models, addressing ambiguities in standard metrics like CER and WER.


<details>
  <summary>Details</summary>
Motivation: Current string comparison metrics (CER, WER) are ambiguous due to inconsistent definitions of characters and words. There is a need for flexible, reproducible evaluation tools that go beyond summary statistics to analyze error types.

Method: Stringalign is a Python library that ensures transparent preprocessing (normalization and tokenization), provides tools for visualizing error rates and error types, and adheres to FAIR principles while remaining lightweight.

Result: Through examples, Stringalign demonstrates that it provides easy-to-use and unambiguous alternatives compared to existing tools that yield opaque and confusing results.

Conclusion: Stringalign facilitates transparent evaluation, gives insights into model improvements, and helps inform model selection for handwriting recognition, OCR, and ASR tasks.
...
TL;DR: This paper summarizes the NTIRE 2026 Image Denoising Challenge for high-noise (σ=50) conditions, where 20 teams out of 116 registrants competed to maximize PSNR using unconstrained neural architectures.


<details>
  <summary>Details</summary>
Motivation: To benchmark and advance the state-of-the-art in image denoising under extreme noise conditions without computational constraints, identifying the best-performing architectures.

Method: The challenge focused on additive white Gaussian noise (AWGN) at σ=50, allowing unlimited parameters and computational overhead, with PSNR as the sole metric.

Result: 20 finalist teams out of 116 registrants submitted solutions, showcasing significant innovations in neural architectures for high-noise image restoration.

Conclusion: The challenge provides a comprehensive snapshot of the current state-of-the-art in unconstrained image restoration under high-noise conditions.
...
TL;DR: Three deep learning models (CNN, ResNet50, ViT) achieved near-perfect prediction agreement (r>0.99) on lung cancer CT classification but had very low LIME explanation correlations (r<0.26), revealing that models use different image regions despite reaching the same predictions. Misclassifications consistently involved attention outside lung parenchyma.


<details>
  <summary>Details</summary>
Motivation: Lung cancer diagnosis is critical, but deep learning models are evaluated almost exclusively on predictive accuracy, leaving their decision-making processes insufficiently examined.

Method: Three models (CNN, ResNet50, ViT) were trained on the IQ-OTH/NCCD lung cancer CT dataset. LIME was applied for interpretability. A dual-correlation framework measured both prediction agreement and explanation agreement across model pairs.

Result: All models achieved strong accuracy (ResNet50: 98.61%, CNN: 97.91%, ViT: 93.75%) with ROC-AUC of 0.99. Prediction correlations >0.99, but explanation correlations <0.26. Misclassifications showed attention outside lung parenchyma.

Conclusion: Prediction agreement is a poor proxy for reasoning consistency; interpretability evaluation must be an independent validation criterion in clinical AI systems.
...
TL;DR: This paper proposes a multi-token Gaussian VAE with cross-attention pooling and anchor-based ICP ground truth refinement for stable scene-scale LiDAR compression and single-step diffusion completion, achieving significant improvements in reconstruction quality (16x reduction in squared Chamfer distance) and 25-143x lower inference latency compared to prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing latent diffusion approaches for sparse LiDAR completion suffer from unstable global representations at outdoor scale, corrupted ground truth data due to odometry drift, and prohibitive multi-step inference latency for real-time deployment.

Method: Multi-token Gaussian VAE with cross-attention pooling for stable scene-scale LiDAR compression + anchor-based ICP ground truth refinement to eliminate drift-induced noise + single-step diffusion completion model.

Result: 16x reduction in squared Chamfer distance on SemanticKITTI seq. 08 (0.396→0.024 m²), surpasses LiDiff by 17-19% and ScoreLiDAR by 10-11%, operates at 25-143x lower inference latency.

Conclusion: Data quality dominates model design; multi-token latent spaces provide a stable first stage for latent diffusion-based scene completion.
...
TL;DR: This paper proposes a sequential pointer-style selection mechanism for visual token pruning in MLLMs, dynamically deciding which tokens to keep and when to stop, achieving strong accuracy-efficiency trade-offs (94.6% accuracy with 88.9% token removal and 1.88x speed-up).


<details>
  <summary>Details</summary>
Motivation: Inference cost in MLLMs is dominated by visual token prefix length; existing pruning methods assume token independence and fixed compression ratios, which is suboptimal.

Method: Reformulates visual token pruning as sequential decision-making with a pointer-style selection mechanism that iteratively selects tokens conditioned on previous choices, plus a learned termination action. Uses a differentiable relaxation via variance-preserving noise interpolation for end-to-end training.

Result: Outperforms fixed-ratio baselines on LLaVA-v1.5-7B and Qwen2.5-VL-7B; under 88.9% token removal, preserves 94.6% original accuracy with 1.88x prefill speed-up.

Conclusion: Sequential, adaptive token pruning with learned termination is more effective than fixed-ratio methods, enabling aggressive compression with minimal accuracy loss.
...
TL;DR: Tool-IQA improves Image Quality Assessment by equipping VLMs with view tools (Magnifier and Gamma Corrector) and a structured assessment pipeline, achieving SOTA results on IQA benchmarks.


<details>
  <summary>Details</summary>
Motivation: Humans assess image quality through dynamic visual inspection (adjusting views, verifying details), but current VLMs use static one-shot scoring, which limits assessment of fine local details and may overlook artifacts due to original intensity distributions.

Method: Proposes Tool-IQA with: (1) two view tools - Magnifier for local details and Gamma Corrector for uncovering hidden artifacts; (2) structured pipeline: initial observation with rubric notes → tool-augmented in-depth inspection → final calibrated score; (3) batch-aware training strategy to reward beneficial tool interactions.

Result: Outperforms existing SOTA models on multiple IQA benchmarks; achieves PLCC of 0.854 on the challenging CLIVE dataset.

Conclusion: Shifting from passive scoring to tool-augmented workflow with effective tool calling and calibrated assessment significantly improves VLM-based IQA performance.
...
TL;DR: VinQA introduces a dataset and evaluation framework (M-GroSE) for long-form document QA where answers interleave text with cited visual elements, comparing Page Encoding and Modality Encoding methods, showing that fine-tuning open Qwen2.5-VL models on this dataset narrows the gap with proprietary frontier models.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal LLMs for document QA produce only text-only responses, failing to leverage tables, charts, and diagrams present in real-world documents. The authors aim to enable answers that explicitly cite and interleave visual elements with supporting text.

Method: The authors create the VinQA dataset and study two encoding methods: (1) Page Encoding (full-page images with bounding boxes) and (2) Modality Encoding (separately encoded text and cropped visual elements). They also propose M-GroSE, a multimodal evaluation framework assessing completeness, answer relevancy, faithfulness, and unanswerability, plus Visual Source F1 for citation accuracy.

Result: Proprietary frontier models achieve the best overall scores, but fine-tuning open Qwen2.5-VL models on VinQA substantially narrows this gap. Modality Encoding is initially more robust for complex documents, but Page Encoding reaches comparable levels after training. Fine-tuned models insert visual elements at semantically appropriate positions with faithful text.

Conclusion: The VinQA dataset and M-GroSE framework effectively enable and evaluate multimodal answers with cited visual elements, demonstrating that open models can be fine-tuned to compete with proprietary ones, and that both encoding methods are viable after training.
...
TL;DR: SceneCraft enables intuitive image editing through interactive scene graphs, eliminating the need for complex text prompts by translating visual graph modifications into precise editing instructions for multiple generative models.


<details>
  <summary>Details</summary>
Motivation: Existing generative AI image editing systems struggle with complex scenes containing multiple interacting objects and require users to craft precise text prompts through trial and error, creating a high cognitive burden and linguistic ambiguity.

Method: SceneCraft represents images as editable scene graphs, allowing users to directly interact with visual graph elements to perform spatial and relational operations. Graph modifications are automatically translated into context-aware editing prompts and dispatched to multiple state-of-the-art generative models.

Result: SceneCraft provides more intuitive control, significantly reduces cognitive burden of manual prompt engineering, and generates outputs that users consistently rate as higher in quality and fidelity across diverse editing scenarios.

Conclusion: Scene graph-based interaction bridges user intent and model execution more effectively than text-prompt approaches, offering a structured and intuitive alternative for complex scene editing.
...
TL;DR: EdgeZSAD is a compact zero-shot anomaly detection system using TinyViT-21M backbone that achieves competitive performance (91.6 AUROC on MVTec-AD) while being deployable on edge devices like Jetson Orin Nano and RB5 Gen2.


<details>
  <summary>Details</summary>
Motivation: Existing ZSAD methods rely on large ViT-L backbones (~300M parameters) that exceed the memory and computational limits of typical embedded hardware used in industrial inspection.

Method: EdgeZSAD uses a TinyViT-21M-512 backbone, asymmetric global-local readout (EdgeGLR), and reproducible source-side training recipe (Real-IAD-DR) with a source-trained, target-unseen protocol.

Result: Achieves 91.6 image AUROC on MVTec-AD and 88.2 on VisA; deployable on Jetson Orin Nano (TensorRT FP16) and RB5 Gen2 (QNN GPU FP16); image-AUROC drift stays below 0.2 points across six device-rescored benchmarks.

Conclusion: EdgeZSAD demonstrates that competitive zero-shot anomaly detection is achievable with compact models suitable for edge deployment, with minimal performance loss when exported to embedded hardware.
...
TL;DR: RSVG-ZeroOV is a training-free, zero-shot open-vocabulary framework for remote sensing visual grounding that uses frozen foundation models (VLMs and DMs) via an Overview-Focus-Evolve paradigm to localize objects in images/videos without task-specific annotations, outperforming zero-shot baselines and competing with supervised methods.


<details>
  <summary>Details</summary>
Motivation: Existing RSVG methods rely on costly task-specific manual annotations that limit coverage of diverse real-world geospatial scenarios, making them struggle to generalize to open-vocabulary queries involving novel objects, fine-grained attributes, complex spatial relationships, and functional semantics.

Method: The framework follows an Overview-Focus-Evolve paradigm: (1) Overview uses a VLM to extract cross-attention maps capturing semantic correlations; (2) Focus leverages a diffusion model's fine-grained priors to compensate for object structure/shape information; (3) Evolve introduces an attention evolution module to suppress irrelevant activations. For video, it adds a query-relevant key-frame selector and temporal propagator for spatio-temporal grounding.

Result: Extensive experiments on six image and video grounding benchmarks show RSVG-ZeroOV consistently outperforms existing zero-shot baselines and achieves competitive/superior performance compared with weakly- and fully-supervised methods.

Conclusion: The proposed training-free framework effectively combines complementary strengths of VLMs and DMs for zero-shot open-vocabulary RSVG, eliminating the need for costly annotations while achieving strong performance across both image and video grounding tasks.
...
TL;DR: This paper proposes a novel Post-training Quantization (PTQ) framework for Visual Autoregressive Models (VAR), addressing attention-value reconstruction errors via shift-and-sum quantization and calibration data distribution mismatch via a resampling strategy, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: PTQ for VAR is underexplored, and existing methods face two challenges: (1) large reconstruction errors in attention-value products at coarse scales where high attention scores occur frequently; (2) discrepancy between codebook entry sampling frequencies and their predicted probabilities due to limited calibration data.

Method: (1) Shift-and-sum quantization: reduces reconstruction errors by aggregating quantized results from symmetrically shifted duplicates of value tokens. (2) Resampling strategy for calibration data: aligns sampling frequencies of codebook entries with their predicted probabilities.

Result: Consistent improvements across VAR architectures on class-conditional image generation, inpainting, outpainting, and class-conditional editing tasks, establishing a new state of the art in PTQ for VAR.

Conclusion: The proposed PTQ framework effectively addresses the unique challenges of quantizing VAR models, enabling efficient deployment without significant performance degradation.
...
TL;DR: A comprehensive review of medical image segmentation covering U-Net, Transformer, and SAM architectures, datasets, evaluation metrics, and challenges, with a unified analytical framework and public GitHub resources.


<details>
  <summary>Details</summary>
Motivation: To provide a systematic, unified review of medical image segmentation methods across multiple architectures (U-Net, Transformer, SAM) rather than focusing on a single model family or specific clinical application, guiding future research and clinical translation.

Method: Organizes U-Net-, Transformer-, and SAM-based methods within a unified analytical framework; reviews public datasets, representative methods, and evaluation metrics; analyzes major challenges from multiple perspectives.

Result: A structured taxonomy and comparative analysis of segmentation methods across three major architectural families, highlighting their effectiveness in improving segmentation accuracy and efficiency.

Conclusion: This review serves as a guide for future research and supports clinical translation of medical image segmentation, with all related resources publicly available on GitHub.
...
TL;DR: LazyMCoT is a training-free dynamic framework that adaptively allocates visual grounding efforts based on sample difficulty, using an Adaptive Routing mechanism and Collaborative Grounding module to improve reasoning accuracy while reducing inference latency in MLLMs.


<details>
  <summary>Details</summary>
Motivation: Existing training-free methods apply image scaling and cropping indiscriminately, causing computational redundancy for simple queries and accuracy degradation for complex ones due to truncated context or added noise.

Method: (1) Adaptive Routing mechanism evaluates predictive uncertainty via first-token statistics to bypass confident cases and recall difficult samples via conformal calibration. (2) Collaborative Grounding module integrates cross-modal attention with an external visual expert through two-stage refinement for precise localized display.

Result: LazyMCoT rivals training-based approaches, improving reasoning accuracy while reducing average inference latency across diverse benchmarks.

Conclusion: Dynamic allocation of visual grounding effort based on sample difficulty is effective for balancing accuracy and efficiency in MLLMs, without requiring additional training.
...
TL;DR: CGS-Retinex is the first low-light image enhancement framework combining continuous Gaussian splatting (explicit) with implicit neural representation, addressing color distortion and structural artifacts by decoupling global illumination and local reflectance within Retinex theory.


<details>
  <summary>Details</summary>
Motivation: Existing low-light enhancement methods suffer from color distortion and structural artifacts when balancing global smooth illumination adjustment and local high-frequency detail recovery.

Method: Proposes CGS-Retinex integrating continuous Gaussian splatting (for global illumination) with implicit neural representation (for reflectance), plus physics-inspired brightness consistency and illumination smoothness constraints.

Result: Significantly suppresses dark-region noise and overexposure while achieving high-fidelity structural and color restoration.

Conclusion: Establishes a novel continuous physical representation paradigm for low-light image enhancement.
...
TL;DR: This paper introduces MUSE, an MLLM-empowered re-ranking method for domain generalizable person Re-ID that improves inference-stage performance by computing a robust μ-distance metric, without modifying the encoder itself.


<details>
  <summary>Details</summary>
Motivation: Existing DG Re-ID methods focus on training better encoders but ignore inference-stage refinements. Traditional re-ranking methods fail on unseen target domains due to unreliable feature distances from non-generalizable encoders.

Method: The authors adapt a Multimodal Large Language Model (MLLM) to Re-ID via supervised fine-tuning with a domain-agnostic prompt and a query-candidate hard mining scheme. The adapted MLLM computes a μ-distance during inference that is robust to domain gaps, improving re-ranking performance.

Result: The approach consistently yields substantial performance improvements across multiple DG Re-ID benchmarks, and is model-agnostic, integrating seamlessly into existing re-ranking frameworks.

Conclusion: Leveraging MLLMs for inference-stage re-ranking is a promising direction for DG Re-ID, addressing domain gaps without modifying the encoder. Code will be released.
...
TL;DR: Dehaze-GaussianImage introduces 2D Gaussian Splatting for zero-shot, unsupervised single image dehazing, modeling hazy images as continuous anisotropic Gaussian fields with a reconstruction-decoupling strategy that embeds the atmospheric scattering model into Gaussian parameters.


<details>
  <summary>Details</summary>
Motivation: Existing dehazing methods suffer from computational redundancy in pixel-level optimization and lack physical interpretability in implicit neural networks, hindering the balance between representation efficiency and reconstruction fidelity.

Method: Proposes a zero-shot framework using 2D Gaussian Splatting (2DGS) to model hazy images as continuous anisotropic Gaussian fields, with a reconstruction-decoupling strategy embedding the atmospheric scattering model into Gaussian parameter space, enabling adaptive splitting, cloning, and pruning for geometric-level decoupling of transmission medium and clear textures, plus explicit structure-preserving constraints.

Result: Achieves state-of-the-art (SOTA) performance in fully unsupervised manner with minimal parameters.

Conclusion: Highlights the potential of explicit Gaussian representation for low-level vision tasks, effectively breaking the traditional pixel-grid processing paradigm.
...
TL;DR: Fi-Gaussian proposes a frequency-aware implicit Gaussian splatting network for single image dehazing, using decoupled frequency components and physics-driven scattering renormalization to recover high-frequency details and achieve state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Single image dehazing suffers from loss of high-frequency details and difficulty in accurate physical scattering modeling.

Method: 1) Frequency-aware implicit Gaussian splatting module that decouples low-frequency structural info and high-frequency texture info, then performs adaptive Gaussian aggregation with complex-valued weights. 2) Physics-driven scattering renormalization mechanism to estimate transmission map and atmospheric light under implicit Gaussian priors.

Result: State-of-the-art quantitative performance and visually superior dehazed results on multiple benchmark datasets.

Conclusion: Implicit Gaussian splatting is effective for low-level vision tasks like dehazing.
...
TL;DR: This paper evaluates approximate unlearning strategies on brain MRI segmentation (3D ResNet-50, MRBrainS18) under GDPR compliance. The Noisy Label strategy achieved the best trade-off: 93% forget reduction with 84% retained accuracy after 50 epochs, while other strategies caused catastrophic forgetting.


<details>
  <summary>Details</summary>
Motivation: New data privacy laws (e.g., GDPR) allow individuals to request erasure of personal data from trained ML models, prompting the need for effective machine unlearning methods to comply with these regulations.

Method: Applied multiple approximate unlearning strategies (based on four mechanics) to a 3D ResNet-50 pre-trained with Med3D on the MRBrainS18 dataset. Evaluated using Dice coefficient and MAE over 20 and 50 training epochs on retain and forget subject sets.

Result: Noisy Label strategy achieved the best trade-off: 93% decrease in forget set performance while maintaining 84% accuracy on the retained set after 50 epochs. Other strategies showed extreme forgetting and catastrophic degradation of retain set performance at higher epochs.

Conclusion: The study provides a strict baseline of performance metrics for subject-specific unlearning, offering practitioners clear criteria for selecting appropriate unlearning strategies under privacy regulations.
...
TL;DR: CoTriSyGen is an agentic framework for multi-shot long-form video generation that uses closed-loop visual-text-memory synergy to iteratively correct inconsistencies, achieving significant improvements in cross-shot consistency and cinematic continuity.


<details>
  <summary>Details</summary>
Motivation: Existing multi-shot video generation suffers from identity drift and compounding inconsistencies across shots, and storyboard-driven pipelines lack mechanisms to incorporate generated visual evidence back into subsequent conditioning.

Method: A closed-loop system with a VLM-based analyzer that reasons over planned intent, persistent entity-centric memory, and generated visuals, performing intra-shot refinement (targeted regeneration for violations) and inter-shot refinement (rewriting subsequent prompts). Memory is a mutable visual state updated by both generator and analyzer.

Result: Experiments on a curated StoryBench benchmark show substantial improvements in cross-shot consistency, prompt adherence, and cinematic continuity over representative methods.

Conclusion: The closed-loop visual-text-memory synergy framework effectively addresses identity drift and long-range coherence in multi-shot video generation.
...
TL;DR: J4D proposes a training framework that learns optimal JPEG compression parameters for DNN vision tasks by using a differentiable soft quantizer and entropy estimation, achieving significantly better rate-accuracy trade-offs than default JPEG (up to 11.60% accuracy gain or 80.05% rate reduction), and demonstrating universal encoding parameters across DNN architectures.


<details>
  <summary>Details</summary>
Motivation: JPEG remains dominant but was designed for humans, while in the AI era, DNNs increasingly consume compressed images; thus there is a need to optimize JPEG specifically for DNN inference performance rather than human perception.

Method: A differentiable soft quantizer based on probabilistic quantization is introduced to make the JPEG codec differentiable, and entropy of coded source is computed analytically as a rate estimate. This enables end-to-end optimization via backpropagation to learn JPEG encoding parameters that minimize compression rate while maximizing DNN accuracy.

Result: J4D consistently outperforms default JPEG and other competitive JPEG codecs across multiple datasets and DNN architectures, achieving up to 11.60% accuracy improvement at the same rate or up to 80.05% rate reduction at the same accuracy. Also shows feasibility of universal encoding parameters across various DNN architectures.

Conclusion: J4D successfully bridges the gap between traditional JPEG compression and modern DNN-based image consumption, demonstrating that learned JPEG parameters optimized for DNNs can dramatically improve efficiency while maintaining or improving inference accuracy.
...
TL;DR: TEASER proposes a training-efficient any-step diffusion framework for Real-ISR that enables both one-step and multi-step restoration within a unified model via self-adversarial distillation and timestep-aware rectification, eliminating the need for auxiliary teachers and supporting seamless any-step sampling.


<details>
  <summary>Details</summary>
Motivation: Diffusion models for Real-ISR are powerful but suffer from slow iterative sampling. Existing one-step distillation methods require auxiliary teacher models that inflate training memory, restrict scalability to large architectures, and lack flexibility to trade off speed for quality.

Method: The key idea is self-adversarial distillation within a single diffusion model (no auxiliary teachers/discriminators), plus a timestep-aware rectification strategy for stable one-step generation across noise levels, and a dual-branch diffusion transformer with decoupled timestep condition.

Result: TEASER enables distillation of 20B-parameter diffusion models on a single GPU, supports seamless any-step sampling, and consistently outperforms state-of-the-art methods across multiple datasets.

Conclusion: TEASER provides a training-efficient, scalable, and flexible any-step diffusion framework for Real-ISR that eliminates the need for auxiliary models while achieving superior performance.
...
TL;DR: This paper proposes Cascaded Sparse Autoencoders (CSAEs) for learning hierarchical visual concepts in MLLMs by training a second-level SAE on the decoder weights of the first-level SAE, achieving improved interpretability and effective concept steering compared to existing SAE baselines.


<details>
  <summary>Details</summary>
Motivation: Existing SAE architectures primarily recover flat feature dictionaries and are less suited for explicit multi-level concept organization, lacking the ability to learn hierarchical visual concepts in MLLMs.

Method: CSAEs train a second-level SAE directly on the decoder weights of the first-level SAE, treating learned low-level feature directions as inputs for higher-level abstraction, avoiding drawbacks from nesting or stacking approaches.

Result: Experiments across Qwen3-VL, Gemma-3, and LLaVA show CSAEs improve hierarchical concept coherence over state-of-the-art SAE baselines, and learned concept groups support effective group-level interventions in MLLM outputs.

Conclusion: CSAEs provide a scalable and effective way to learn "concepts of concepts" for hierarchical visual concept organization in MLLMs, enabling better interpretability and controllable steering.
...
TL;DR: The paper proposes a grounded action-centric evidence augmentation framework that enhances video MLLMs for viewer sentiment prediction by extracting SVO triplets and visual entity crops, improving emotional reasoning through explicit event structure and localized visual evidence.


<details>
  <summary>Details</summary>
Motivation: Standard MLLMs rely on holistic frame representations that leave fine-grained, affect-relevant events implicit, making precise emotional reasoning from video advertisements difficult. The gap between what is shown and what is felt needs bridging.

Method: The method extracts temporally ordered SVO triplets and visible textual cues from action-centric video descriptions, grounds subject/object entities as visual crops, and feeds these structured clues to MLLMs for clue-enhanced emotional reasoning.

Result: Consistent improvements over Qwen2.5-VL and Qwen3-VL baselines on the Pitts dataset. Ablation studies, cross-dataset evaluation on AdsQA, and transfer experiments on TVQA subset confirm effectiveness and generalization.

Conclusion: Explicitly providing structured action-event knowledge (SVO triplets + grounded visual crops) significantly improves video MLLMs' ability to infer viewer sentiment, outperforming holistic frame-based approaches.
...
TL;DR: EgoPhys constructs deformable physical digital twins from egocentric RGB-only video using generalizable priors, outperforming baselines in reconstruction, prediction, and zero-shot generalization, and demonstrates real-world utility on a robot arm for deformable-object planning.


<details>
  <summary>Details</summary>
Motivation: Humans naturally understand object physics, but faithfully predicting complex deformable dynamics (elastic materials, fabrics) from video remains a major challenge in computer vision and robotics.

Method: EgoPhys distills per-object inverse-physics solutions into a compact codebook, enabling prediction of dense spring stiffness fields for unseen objects without per-spring test-time optimization, trained on diverse egocentric interaction data.

Result: Outperforms baselines in reconstruction, future prediction, and zero-shot generalization; successfully deployed on a real xArm6 robot using a digital twin initialized from a single egocentric human play video.

Conclusion: Egocentric RGB observations offer a scalable path toward real-to-sim pipelines for deformable-object manipulation, demonstrated through robot planning experiments.
...
TL;DR: A dynamic multimodal Mixture-of-Experts framework integrating functional and structural MRI improves post-traumatic epilepsy diagnosis and risk stratification.


<details>
  <summary>Details</summary>
Motivation: Early identification of post-traumatic epilepsy (PTE) after traumatic brain injury (TBI) is challenging due to complex structural and functional brain alterations.

Method: A dynamic multimodal Mixture-of-Experts (MoE) framework with time-aware functional-structural encoding and class-conditioned expert routing, including a Modality-Class MoE (MCoE) module for dynamic expert weight dispatch.

Result: Outperforms static fusion baselines across three binary classification tasks; reveals meaningful region-of-interest (ROI) interactions via interpretability analyses.

Conclusion: The dynamic multimodal expert framework captures class-dependent brain interaction patterns and provides an interpretable approach for PTE diagnosis and risk stratification.
...
TL;DR: Lucid is a sparsity-adaptive Flow Matching framework for sparse-view CT reconstruction that learns a generative prior from high-quality CT images and adapts inference to different sampling densities via degradation-matched initialization, sparsity-modulated updates, and projection-domain data consistency.


<details>
  <summary>Details</summary>
Motivation: Sparse-view CT suffers from severe ill-posed reconstruction due to angular undersampling. Existing supervised methods are tied to specific sampling settings, and generative methods can produce anatomically inconsistent hallucinations. A sparsity-adaptive, consistent approach is needed.

Method: Lucid trains a Flow Matching model on high-quality CT images to learn a Gaussian-to-CT distribution transport. During inference, it (1) constructs a degradation-matched initial state via sparsity-weighted fusion of FBP image and noise, (2) performs sparsity-modulated Flow Matching updates, and (3) applies projection-domain data-consistency correction after each update.

Result: Across multiple sparse-view settings, Lucid achieves stable reconstruction performance across different sampling densities, improves image quality and structural fidelity, and reduces hallucination-like structures compared to prior generative methods.

Conclusion: Lucid effectively disentangles training from sampling configuration, adapts to varying sparsity levels at inference time, and provides a robust solution for generative sparse-view CT reconstruction with fewer artifacts and better anatomical fidelity.
...
TL;DR: This paper proposes a novel framework that synthesizes FFA images from CFP by incorporating structural guidance from OCT, using a Spatially Aligned Cross-Modal Fusion (SACMF) module and Token-wise Cross-Modality Alignment (TCMA) for improved vascular and pathological reconstruction. It also introduces the first tri-modally aligned retinal dataset (CFP, FFA, OCT) from 3,676 patient eyes.


<details>
  <summary>Details</summary>
Motivation: FFA acquisition is invasive and not always feasible, while CFP is non-invasive but lacks functional vascular information. Prior CFP-to-FFA synthesis methods rely only on surface texture, failing to reconstruct functional details and subtle pathologies.

Method: (1) A multi-modal retinal dataset with paired CFP, FFA, and OCT from 3,676 eyes (first tri-modally aligned dataset). (2) SACMF module: projects depth-resolved OCT features onto the fundus plane and injects them into the CFP encoder via adaptive layer normalization. (3) TCMA: token-level contrastive learning for explicit alignment of CFP and FFA representations at corresponding spatial positions.

Result: Achieves superior synthesis performance compared to state-of-the-art methods, and synthesized FFA images yield greater improvements in downstream disease diagnosis than existing methods.

Conclusion: The proposed OCT-guided CFP-to-FFA synthesis framework demonstrates significant clinical potential as a non-invasive decision-support tool in routine workflows.
...
TL;DR: This paper proposes S2CO-Anagram, a framework using parallel denoising and structure-semantic co-optimization to generate high-resolution visual anagrams with improved visual quality, semantic fidelity, and faster inference speed compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing visual anagram synthesis using T2I diffusion models suffers from computational inefficiency, suboptimal aesthetic quality, and weak semantic fidelity, motivating the need for a faster, higher-quality generation method.

Method: The method adapts parallel denoising from pixel-based T2I to adversarially distilled latent-based T2I, and introduces a Structure-Semantic Co-Optimization (S2CO) framework with three innovations: null-text structure alignment, semantic enhancement optimization, and attention-guided noise fusion.

Result: S2CO-Anagram generates higher-resolution anagram images with superior visual harmony and semantic faithfulness than SOTA approaches, while achieving substantially faster inference speed.

Conclusion: The proposed approach advances intelligent creation of illusionary digital art by enabling efficient, high-quality visual anagram synthesis with minimal computational cost.
...
TL;DR: SPARC is a learned image compression framework for VLA-driven robots that adaptively allocates bitrate across camera views and spatial regions based on task relevance, outperforming conventional codecs in control performance under limited bandwidth.


<details>
  <summary>Details</summary>
Motivation: Visual communication is a major bottleneck for real-time robotic control with VLA models in bandwidth-constrained or distributed settings, yet existing codecs prioritize generic visual fidelity over downstream control performance.

Method: SPARC uses a lightweight temporal mask selector to adaptively allocate bitrate over latent representations based on task relevance, leveraging temporal context. A tilted rate loss stabilizes training by preventing over-suppression of rare but task-critical visual patterns.

Result: On RoboCasa365, VLABench, and LIBERO benchmarks, SPARC achieves stronger control performance than conventional codecs and recent learned compression methods at the same bitrate. Real-world deployment shows improved bitrate-success tradeoff in remote-control settings.

Conclusion: Task-adaptive spatial bitrate allocation for VLA policies significantly improves compression efficiency without sacrificing control performance, offering a practical solution for bandwidth-limited robotic deployments.
...
TL;DR: UniDDT unifies multimodal understanding and generation by using a Noisy ViT encoder with LLM for shared semantic encoding and a separate diffusion decoder, achieving strong performance on both tasks.


<details>
  <summary>Details</summary>
Motivation: Existing UMMs suffer from conflicts between understanding and generation tasks, incompatible visual spaces, and neglect of the inherent duality between text-image understanding and generation.

Method: UniDDT uses a Noisy ViT encoder + LLM for unified semantic encoding across understanding/generation, a separate diffusion decoder to decouple from text decoding, and dual data structures from image-text pairs to exploit task duality.

Result: GenEval: 0.87, DPG: 86.9, MME: 1699.5, SEEDbench: 76.5 overall.

Conclusion: UniDDT achieves effective unification of multimodal understanding and generation with enhanced semantic consistency and scalability.
...
TL;DR: KeepLoRA++ balances three competing objectives in continual learning for vision-language models (retaining pre-trained knowledge, preserving past task knowledge, maintaining plasticity) via a dual-dimensional knowledge retention mechanism that analyzes inter-layer and intra-layer knowledge distribution, then applies layer-scaled residual gradient adaptation with orthogonal projection to prevent interference.


<details>
  <summary>Details</summary>
Motivation: Continual learning for pre-trained vision-language models faces a trilemma: retaining pre-trained knowledge, preserving task-specific knowledge from sequential tasks, and maintaining plasticity for new tasks. Existing methods fail to simultaneously optimize all three objectives.

Method: KeepLoRA++ analyzes Transformer knowledge distribution from inter-layer (shallow layers encode general knowledge, deep layers encode task-specific adaptations) and intra-layer (principal subspace vs. residual subspace) perspectives. It introduces layer-scaled residual gradient adaptation: new task gradients are projected onto the orthogonal complement of both the pre-trained principal subspace and previous task dominant directions, with shallow layers receiving smaller updates and deep layers larger updates.

Result: Theoretical analysis and empirical evaluations show KeepLoRA++ successfully balances the three competing objectives, consistently outperforming representative baselines across image classification, visual question answering, and video understanding tasks.

Conclusion: The dual-dimensional knowledge retention mechanism (inter-layer + intra-layer) effectively resolves the continual learning trilemma for vision-language models by leveraging the structural property that general knowledge resides in shallow layers/principal subspace while task-specific knowledge resides in deep layers/residual subspace.
...
TL;DR: The paper proposes a self-supervised fusion of signal-based and texture-driven methods for 3D seismic horizon tracking, using signal-derived priors to train a deep learning model via contrastive learning, achieving accurate tracking near faults without labeled data.


<details>
  <summary>Details</summary>
Motivation: Unsupervised 3D seismic horizon tracking faces a key limitation: signal-based propagators provide accurate trace-level alignment but fail near faults, while texture-driven deep models handle discontinuities better but require labeled data and sacrifice trace-level precision.

Method: A self-supervised framework fusing signal-derived local horizon correspondences (from reflector slopes) as domain-specific priors to train a texture-based deep learning model via contrastive learning, using positive pairs from reliable trace-to-trace flows and high-confidence neighborhoods, optionally with fault masks.

Result: On the public F3 dataset and a faulted synthetic dataset, the method achieves lower mean absolute error (MAE) than unsupervised baselines and competitive performance against a semi-supervised method using a single labeled slice.

Conclusion: The network learns voxel-wise embeddings that preserve local signal continuity while enabling horizon propagation beyond discontinuities through similarity search, effectively overcoming the limitation of traditional methods near faults.
...
TL;DR: GraphWorld is an end-to-end autonomous driving framework that enhances long-horizon planning via latent world modeling, using an Ego-Centric Interaction Graph and World-State-Conditioned Planning to reduce collision rates and improve performance in complex driving environments.


<details>
  <summary>Details</summary>
Motivation: Existing E2E-AD methods are confined to short-horizon planning and lack the ability to model long-term temporal dependencies, limiting generalization and security in complex, highly interactive driving scenarios.

Method: 1) Ego-Centric Interaction Graph: adaptively models critical neighboring agents via spatial proximity with cross-node cross-attention. 2) World-State-Conditioned Planning: learns ego-centric latent world representations capturing interaction dynamics and safety semantics to guide long-horizon, safety-aware trajectory planning.

Result: Extensive experiments on Bench2Drive, NAVSIMv1/2, and nuScenes show significant reduction in collision rates and improved long-horizon planning performance.

Conclusion: GraphWorld effectively addresses long-term temporal dependency modeling in E2E-AD, validating its effectiveness for complex and highly interactive driving environments.
...
TL;DR: RealityBridge is a Sim-to-Real framework that restores edited 3DGS driving videos by repairing artifacts, improving visual realism, and ensuring temporal consistency using multimodal controls and autoregressive long-video training.


<details>
  <summary>Details</summary>
Motivation: Long-tail hazardous scenarios are critical for safety-oriented autonomous driving but are hard to collect and reproduce at scale. While editable 3DGS simulation offers a solution, edited 3DGS videos suffer from significant Sim-to-Real gaps (artifacts, degraded assets, inconsistent illumination, temporal flickering), and existing restoration methods are insufficient.

Method: RealityBridge uses multimodal controls (rendered videos, foreground masks, edge maps, semantic masks) with a lightweight GateNet for adaptive condition allocation. It also employs autoregressive long-video training with reward-guided post-training to enhance restoration quality, temporal stability, and hallucination suppression.

Result: Experiments on internal and public driving datasets show RealityBridge outperforms existing methods in artifact removal, illumination harmonization, and long-sequence temporal consistency.

Conclusion: RealityBridge effectively bridges the Sim-to-Real gap for edited 3DGS driving videos, enabling more realistic and temporally consistent simulation for autonomous driving safety research.
...
TL;DR: Using the Krakencoder, this study simulates network removal in 702 healthy brain connectomes, finding the Default Mode Network causes the largest cross-modal perturbations while the Somatomotor network causes the smallest, and that intact predicted connectomes retain far more sex-discriminative information (84.76% accuracy) than perturbation signatures (66.09%).


<details>
  <summary>Details</summary>
Motivation: To understand how structural and functional connectome deficiencies propagate across modalities and how network-level perturbations affect information retention, particularly sex-specific information.

Method: Analyzed structural/functional connectomes from 702 HCP participants using the Krakencoder; removed each of Yeo-7 functional networks individually; quantified perturbations via KL divergence, Frobenius norm, and Wasserstein distance; evaluated sex classification accuracy.

Result: Default Mode Network caused largest perturbations; Somatomotor network caused smallest; sex classification accuracy: 66.09% from perturbation signatures vs 84.76% from intact predicted connectomes.

Conclusion: Full predicted connectomes retain significantly more sex-discriminative information than perturbation-derived signatures, emphasizing the value of complete connectome predictions over ablation-based analyses.
...
TL;DR: VisualClaw is a self-evolving multimodal agent that reduces API costs by up to 98% using hybrid encoding (cascaded frame filtering + hot/cold skill injection) and improves accuracy through skill evolution from failures. It also introduces VisualClawArena, a 200-scenario benchmark for testing agents in tool-using workspaces.


<details>
  <summary>Details</summary>
Motivation: VLMs face three deployment gaps: high latency/cost from dense video frames, static agent scaffolds after deployment, and lack of benchmarks testing visual evidence usage in tool-using workspaces.

Method: Two principles: (1) Hybrid encoding - cascaded gate filters non-informative frames, hot/cold top-k injection compresses skill bank; (2) Skill evolution - retrieved memories condition an evolver (via direct context or guided evidence) to produce skill-bank updates for future questions.

Result: Cuts per-question API cost by -98% vs full-frame upload, -25.9% vs uniform 8-frame baseline. Boosts accuracy +3.85% average, +15.80% peak on EgoSchema with Gemini 3 Flash. On VisualClawArena, improves macro accuracy +2.9% (Codex) and +3.2% (Claude Code), with -9.5% cost reduction.

Conclusion: VisualClaw is a natural fit for edge applications, reducing a 1-hour streaming session from ~3,600 API uploads down to 5-20 calls, with self-evolution enabling personalized assistance.
...
TL;DR: DDTNet proposes a degradation disentanglement and transfer approach to generate domain-adaptive paired training data, improving all-in-one adverse weather restoration models across real-world rain, haze, and snow conditions.


<details>
  <summary>Details</summary>
Motivation: Existing all-in-one restoration models compromise performance for individual degradation types and struggle with domain gaps between training and testing data.

Method: DDTNet uses a Degradation Disentanglement Module (DDM) with Degradation Coupled Attention (DCA) to disentangle degradation patterns from target-domain images and transfer them to source-domain clean images, generating adaptive paired training data for fine-tuning.

Result: DDTNet significantly and consistently improves existing all-in-one models across real-world deraining, desnowing, and dehazing datasets.

Conclusion: Modeling degradation patterns is more feasible than recovering clean content; degradation transfer effectively enhances model adaptability across diverse weather conditions and domains.
...
TL;DR: This paper compares CNN (U-Net, U-Net++, DeepLabV3) and vision transformer (SegFormer) models for multi-class flood segmentation from SAR imagery, finding SegFormer-b2 significantly outperforms U-Net on the ETCI dataset but with narrower advantages on Sen1Floods11, while providing explainability and uncertainty analysis of model decisions.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of distinguishing flooded land from permanent water bodies in SAR-based flood monitoring, and to provide a comprehensive comparison between CNN and vision transformer architectures for this task.

Method: Compared three CNN-based models (U-Net, U-Net++, DeepLabV3 with ResNet-34) and three SegFormer variants (b0, b1, b2) on two benchmark datasets (ETCI NASA and SenFloods11) using scene-based data splits, with qualitative/quantitative explainability and uncertainty estimation.

Result: SegFormer-b2 significantly outperformed U-Net on ETCI dataset (higher flood IoU across all 7 test scenes in Wilcoxon test); after fine-tuning on Sen1Floods11, advantage narrowed to within scene variability. SegFormer produced more coherent Grad-CAM activations; U-Net generated more informative uncertainty estimates along flood boundaries.

Conclusion: Vision transformers (SegFormer) show strong potential for SAR-based flood segmentation, especially on complex datasets, but CNNs remain competitive and offer better uncertainty quantification, suggesting complementary strengths for different deployment scenarios.
...
TL;DR: A dynamic thresholding scheme for sparse attention in Diffusion Transformers that improves sparsity from 61.42% to 82% with minimal accuracy loss, achieving 1.61x computational efficiency gains in video generation.


<details>
  <summary>Details</summary>
Motivation: Existing token selection strategies (Top-p, Top-k) cannot simultaneously maximize sparsity and minimize accuracy degradation for sparse attention in video DiTs.

Method: Proposes a dynamic thresholding scheme that maintains a fixed recall rate for accuracy while dynamically adjusting thresholds for sparsity, deeply integrated with Flash Attention to avoid extra masking overhead.

Result: Sparsity improved from 61.42% to 82% with <5% VBench metric drop; ~15% attention computation savings; 1.61x computational efficiency (1.18x higher than BLASST).

Conclusion: Fixed recall rate is sufficient for accuracy, and dynamic thresholding achieves better sparsity-efficiency trade-off than fixed-threshold methods.
...
TL;DR: HAFMat is a hybrid-prior-guided framework for estimating PBR materials from a single human image, using a Multi-layer Adaptive Feature Fusion Mechanism to combine texture-level and semantic-level guidance cues, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: PBR material estimation from a single human image is highly ill-posed due to the entanglement of illumination, geometry, and reflectance in observed appearance. The authors aim to mitigate this ambiguity.

Method: A hybrid-prior-guided framework (HAFMat) that uses guidance maps encoding appearance, body geometry, structure, and prior material predictions. It employs a Multi-layer Adaptive Feature Fusion Mechanism to adaptively fuse heterogeneous guidance cues (texture-dominant vs. semantic-dominant) with decoder features at different stages.

Result: State-of-the-art performance on both synthetic and real data for material estimation and downstream relighting tasks.

Conclusion: The adaptive fusion of heterogeneous guidance cues (texture-level and semantic-level) enables more accurate and physically plausible material estimation from a single human image.
...
TL;DR: This paper proposes a prototype calibration framework for few-shot multi-rater medical image segmentation that models rater-specific variability using an attention operator, enabling personalized segmentation without modifying the backbone feature extractor.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot medical image segmentation methods assume a single ground-truth annotation, ignoring systematic variability across different expert raters commonly found in clinical datasets.

Method: The method uses an attention-based prototype calibration framework that models rater-specific deviations from a consensus representation in prototype space. A lightweight attention operator refines rater prototypes without altering the backbone feature extractor.

Result: Experiments on multi-rater medical imaging datasets show consistent improvements over baseline prototype approaches.

Conclusion: Structured prototype calibration is effective for modeling annotation variability, offering a lightweight and compatible solution for personalized few-shot segmentation.
...
TL;DR: A differentiable packing framework that jointly optimizes all 6N object poses and container side lengths in a single gradient-based loop, achieving 11-32% smaller containers than baselines in under 4 minutes.


<details>
  <summary>Details</summary>
Motivation: Existing packing methods either fix the container size or optimize only a single container dimension manually, lacking a fully joint optimization approach.

Method: Uses six physics-inspired differentiable loss terms on triangle meshes via AABB proxies, with an adaptive squeezing mechanism and tensor-broadcasting for efficiency, implemented purely in Python/PyTorch.

Result: 3.4-54x speedup over loop-based implementation; 11-32% smaller containers than DBLF and simulated annealing at N=100; runs under 4 minutes per instance on a consumer GPU.

Conclusion: The proposed differentiable framework enables efficient, high-quality joint optimization of packing parameters without manual tuning or external physics engines.
...
TL;DR: CHRONOSIGHT benchmark reveals a severe "chronological blindness" in VLMs: humans achieve 89% accuracy on visual temporal reasoning tasks, while the best open VLM (Qwen2.5-VL-7B) reaches only 40%. Fine-tuning on just 151 examples significantly improves performance, suggesting the core issue is instruction following, not visual perception.


<details>
  <summary>Details</summary>
Motivation: Humans naturally perceive temporal dynamics in visual scenes (e.g., ripeness, construction progress), but whether VLMs share this temporal reasoning ability is unknown and practically important.

Method: The authors introduce CHRONOSIGHT, a benchmark with five temporal reasoning dimensions (ChronoRank, ChronoLocate, ChronoDelta, ChronoReverse, ChronoOdd), comprising 1,000 items across 8 process families spanning minutes to millennia. They evaluate 8 open-source VLMs under two prompting regimes and compare against human baselines.

Result: Humans achieve 0.89 average accuracy; best open model (Qwen2.5-VL-7B) reaches 0.40. Lightweight LoRA fine-tuning on 151 examples boosts ChronoDelta accuracy from near-zero to 0.43, with zero-shot transfer to ChronoOdd (0.37) and ChronoReverse (0.64).

Conclusion: VLMs exhibit "chronological blindness"—a large gap in visual temporal reasoning vs. humans. The bottleneck appears to be instruction following rather than visual perception, given that minimal fine-tuning yields substantial gains.
...
TL;DR: Proposes FlashBack Memory (FB), a plug-in module that improves recurrent precipitation nowcasting models by dynamically retrieving key historical states and fusing them adaptively, reducing false alarms and enhancing long-range spatiotemporal predictions across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing precipitation nowcasting methods suffer from false alarms, missed events, and poor long-range dependency modeling at high spatiotemporal resolution.

Method: FlashBack Memory (FB) module that dynamically retrieves key historical states and integrates them via an adaptive fusion gate, incorporated into five recurrent models (PredRNN, PredRNNpp, MIM, MotionRNN, PredRNN-V2).

Result: FB significantly improves MSE, MAE, SSIM, and CSI on CIKM2017, Shanghai2020, and SEVIR datasets, especially for high-intensity rainfall and long-sequence predictions, with fewer false alarms and missed events.

Conclusion: FB provides a general and efficient memory enhancement mechanism that improves overall performance of recurrent-based precipitation nowcasting models.
...
TL;DR: SelectStream is a selective latent-memory framework for streaming video understanding that uses query-conditioned evidence budgeting, surprise-driven adaptive windowing, priority-preserving consolidation, and graph reasoning to allocate fixed memory selectively, achieving strong online and offline performance.


<details>
  <summary>Details</summary>
Motivation: Existing methods indiscriminately inject historical information, which can dilute current-scene perception. The key challenge is not whether to use memory, but how to allocate it selectively under fixed budgets.

Method: SelectStream keeps current observations visible to a frozen VLM while exposing history only through a compact, query-conditioned evidence budget. It uses three coordinated mechanisms: surprise-driven adaptive windowing (when to write), priority-preserving consolidation (what to preserve), and query-conditioned graph reasoning over a fixed-capacity latent memory graph (how to retrieve).

Result: 82.67% on StreamingBench, 67.03% on OVO-Bench, 74.4% average accuracy on offline video benchmarks, outperforming strong recent-window baselines and prior streaming memory methods.

Conclusion: Selective memory allocation under budget constraints is more effective than indiscriminate history injection for online streaming video understanding.
...
TL;DR: GraphBEV++ is a robust multi-modal fusion framework that addresses feature misalignment in BEV perception under calibration uncertainties. It introduces LocalAlign-v2 (neighborhood-aware depth features via graph matching) and GlobalAlign-v2 (Deformable for LSS-based and Diffusion for query-based BEV) to correct projection-induced misalignment. It achieves SOTA on nuScenes/Waymo, improves long-range detection on Argoverse2, generalizes to 3D occupancy prediction, and outperforms baselines in end-to-end autonomous driving tasks.


<details>
  <summary>Details</summary>
Motivation: Feature misalignment in BEV perception caused by calibration uncertainties between LiDAR and camera sensors is a critical yet overlooked challenge in autonomous driving that degrades multi-modal fusion quality.

Method: GraphBEV++ consists of LocalAlign-v2 (graph matching for neighborhood-aware depth features to correct local misalignment, compatible with both LSS-based and query-based BEV) and GlobalAlign-v2 (Deformable variant learns cross-modal offsets for LSS-based BEV; Diffusion variant injects noise and denoises for query-based BEV to handle implicit misalignment).

Result: Achieves SOTA under misalignment noise on nuScenes/Waymo, improves long-range detection on Argoverse2, generalizes to 3D occupancy prediction, and outperforms five baselines (UniAD, VAD, FusionAD, MomAD, WoTE) in both open-loop and closed-loop evaluations.

Conclusion: Systematic mitigation of projection-induced misalignment via LocalAlign-v2 and GlobalAlign-v2 significantly improves robustness, accuracy, and generalization across multiple BEV paradigms and downstream tasks in autonomous driving.
...
TL;DR: A physics-driven dehazing framework (GP-DUN) for UAV images that models non-uniform haze distribution using a proposed UAV Atmospheric Scattering Model (UASM) with flight altitude, viewing pitch, and extinction parameters, outperforming existing methods on synthetic and real benchmarks.


<details>
  <summary>Details</summary>
Motivation: Two key challenges in UAV dehazing: (1) paired real hazy/clean images are unobtainable and classical atmospheric scattering model cannot model spatially non-uniform haze in UAV imagery; (2) existing methods struggle to remove heavy haze accumulated in upper regions of UAV images.

Method: Proposes UAV Atmospheric Scattering Model (UASM) incorporating flight altitude, viewing pitch, and extinction. Develops GP-DUN with three modules: Latent Geometry Estimator (LGE) for geometry-consistent transmittance, Geometry-aware Gradient Descent Module (GeoGDM) embedding UASM into data-fidelity term, and Pooling-Expert Proximal Mapping Module (PE-PMM) for texture restoration. Also constructs UASM-HazeSet dataset.

Result: GP-DUN consistently outperforms existing methods on both UASM-HazeSet (synthetic) and real UAV haze benchmarks.

Conclusion: The proposed geometry-aware physics-driven approach effectively addresses the non-uniform haze distribution specific to UAV imaging, enabling superior dehazing performance.
...
TL;DR: SP$^3$ is a fast Plug-and-Play image restoration algorithm that uses Spherical Encoders as generative priors, achieving 3-630× speedup over diffusion/flow methods while maintaining comparable perceptual quality.


<details>
  <summary>Details</summary>
Motivation: To accelerate maximum a posteriori image restoration by replacing computationally expensive denoisers with Spherical Encoders as generative priors, avoiding gradient computation during inference.

Method: Uses Half-Quadratic Splitting to alternate between a closed-form data-consistency step and a proximal prior step approximated by projecting onto the Spherical Encoder's structured latent space.

Result: Achieves perceptual quality comparable to state-of-the-art zero-shot diffusion/flow methods while being 3-630× faster, with "anytime" restoration capability.

Conclusion: SP$^3$ offers a computationally efficient alternative to diffusion-based restoration, with stable convergence and sharp outputs from the first iteration.
...
TL;DR: A reference-guided, subject-agnostic framework for identity-preserving face video restoration that uses bimodal perceptual-descriptive identity conditioning in a pretrained text-to-video generator, achieving improved fidelity, temporal consistency, and identity preservation under various degradations.


<details>
  <summary>Details</summary>
Motivation: Existing face video restoration methods either suffer from identity loss (reference-free) or lack generalization to unseen identities (subject-specific). The authors aim to bridge this gap with a subject-agnostic, reference-guided approach.

Method: A subject-agnostic, reference-guided framework that introduces bimodal perceptual-descriptive identity conditioning into a pretrained flow-based text-to-video generator, with a two-stage training strategy to strengthen identity guidance during restoration.

Result: Superior performance on restoration fidelity, temporal consistency, and identity preservation under challenging degradations such as downsampling, blur, noise, and compression artifacts.

Conclusion: The proposed method effectively preserves identity while maintaining generalization to unseen subjects, outperforming existing approaches in face video restoration tasks.
...
TL;DR: This paper proposes an instance-aware knowledge distillation framework for semi-supervised learning, enabling lightweight collision avoidance systems on edge devices. By generating pseudo labels from teacher models and foundation models, a student model is trained to perform dense prediction tasks with significantly reduced computational cost, validated on a large-scale country club dataset.


<details>
  <summary>Details</summary>
Motivation: Deploying collision avoidance systems on edge devices (e.g., country clubs) is limited by low computational resources, unreliable communication, and high annotation costs for domain-specific datasets.

Method: Proposes an instance-aware knowledge distillation framework for semi-supervised learning, generating pseudo labels that combine teacher domain priors with instance-centric knowledge from foundation models to train a lightweight student model.

Result: Student outperforms teacher in instance segmentation, mitigates degradation in monocular depth estimation, reduces FLOPs by 22.68× and parameters by 14.33×, and achieves 6.46 FPS on a low-cost edge device.

Conclusion: The proposed framework effectively enables real-time collision avoidance on resource-constrained edge devices by distilling knowledge from large models into lightweight students with competitive or superior performance.
...
TL;DR: This paper proposes a Beer-Lambert guided representation learning framework for unsupervised anomaly detection in Sub-THz food inspection images, introducing an attenuation decomposition module and a Leave-One-Food-Out evaluation protocol.


<details>
  <summary>Details</summary>
Motivation: Existing unsupervised anomaly detection methods rely on RGB-pretrained visual representations that fail to capture the transmission behavior of Sub-THz images, limiting detection of low-density contaminants in food manufacturing.

Method: The proposed framework uses a Beer-Lambert guided representation learning approach with an attenuation decomposition module that constrains student representations via attenuation reconstruction during training. It also introduces a Leave-One-Food-Out protocol for evaluating generalization on unseen food categories.

Result: Experiments on the Inline-Food-Inspection-THz dataset show improved overall anomaly detection performance compared to the baseline method.

Conclusion: The Beer-Lambert guided representation learning framework effectively addresses the limitations of RGB-pretrained methods for Sub-THz food inspection, improving detection of foreign contaminants while also providing a more rigorous generalization evaluation protocol.
...
TL;DR: ExpertDet introduces expert-informed cues (attribute and hierarchy knowledge) into fine-grained aerial object detection, using Vision-aware Masked Attribute Modeling (VMAM) and Hierarchical Visual Instance Promotion (HierVIP), alongside a new benchmark (PSP) with 106 ship and 30 airplane model classes, achieving superior performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: Existing fine-grained aerial object detection methods inherit coarse-grained paradigms with single-label supervision, failing to distinguish model-level categories (e.g., Boeing 787 vs. Airbus A320) that have subtle structural differences, despite the availability of structured prior knowledge (attributes, hierarchies).

Method: ExpertDet includes: (1) Vision-aware Masked Attribute Modeling (VMAM) — aligns attribute semantics with visual structures by reconstructing masked attributes from visual cues; (2) Hierarchical Visual Instance Promotion (HierVIP) — builds a visual prototype tree from hierarchical relations with taxonomy-aware constraints to preserve cross-level continuity and enhance discrimination; (3) A new PSP benchmark with 106 ship and 30 airplane model classes.

Result: ExpertDet consistently outperforms other fine-grained competitors across all hierarchy levels on the PSP benchmark, which features the most extensive model-specific category collection among existing aerial object detection datasets.

Conclusion: Incorporating structured prior knowledge (attribute and hierarchical information) significantly enhances fine-grained aerial object detection, and the new PSP benchmark provides a valuable resource for future research in model-specific recognition.
...
TL;DR: PermaVid is a framework for consistent video generation after editing, using dual memory banks (RGB and depth) with edit-aware updates to maintain long-term semantic and structural coherence.


<details>
  <summary>Details</summary>
Motivation: Existing memory designs struggle to maintain long-term consistency in video generation after editing operations, as stored contexts become outdated or invalid when scene appearance or layout is modified.

Method: PermaVid proposes a multi-modal context memory with two complementary banks: an RGB context bank for appearance-aware observations (with implicit geometry) and a depth context bank for geometry-only structure (disentangled from semantics). It also introduces an edit-aware memory update and retrieval strategy, along with a memory-guided video generation model for multi-modal feature fusion.

Result: The method maintains strong long-term semantic and structural consistency after edits, significantly outperforming state-of-the-art methods.

Conclusion: Disentangling spatial context into appearance (RGB) and geometry (depth) with edit-aware memory management effectively addresses the consistency problem in edited video generation.
...
TL;DR: This paper proposes a method that uses residual image encoding with gradient reversal optimization to improve conditional diffusion-based image editing by preserving identity while enabling targeted edits without paired training data.


<details>
  <summary>Details</summary>
Motivation: Conditional diffusion image generators can be repurposed for editing through inversion without large-scale paired fine-tuning data, but producing high-quality targeted edits while maintaining identity and consistency remains challenging due to conflicting image features embedded in noise during weakly conditioned inversion.

Method: The method incorporates a residual image encoding as additional conditioning to improve identity preservation and editability. It optimizes this residual encoding to provide strong conditioning for reconstruction, reducing reliance on inversion. A gradient reversal-based optimization strategy disentangles the residual from the edited condition to prevent interference with desired edits.

Result: The method produces high-fidelity results across precise intrinsic-based editing and relighting, with proof-of-concept text-guided manipulation.

Conclusion: The residual encoding with gradient reversal approach enables better identity preservation and editability for conditional diffusion-based image editing without requiring paired training data.
...
TL;DR: A modular framework that decouples action recognition (TSM) from object identification (selection algorithm) to generate precise, grammar-free robot commands, achieving 86.79% action accuracy and significant gains over baselines on Something-Something V2.


<details>
  <summary>Details</summary>
Motivation: Existing video-to-robot command methods fail to identify functionally relevant objects in demonstrations, producing linguistically plausible but operationally ambiguous commands.

Method: Integrates Temporal Shift Modules (TSM) for action classification + Object Selection algorithm (trajectory role classification, blur detection, overlap minimization) + VLMs for object recognition and zero-shot generalization.

Result: 86.79% action accuracy; BLEU-4 of 0.337 (standard) / 0.261 (novel), outperforming task-specific baselines by 80.2%-143.9%; higher gains in METEOR (157.9%) and CIDEr (171.7%) on novel objects.

Conclusion: The modular object-centric design consistently outperforms task-specific methods and competes with large general-purpose VLMs while maintaining interpretability and zero-shot generalization.
...
TL;DR: MVOFormer is a transformer-based monocular visual odometry framework that uses a Flow-Semantic Dual Branch Encoder to combine geometric motion with semantic priors, and an Iterative Multimodal Decoder for coarse-to-fine pose refinement, achieving superior zero-shot generalization across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based MVO methods suffer from either lack of interpretable complementary features or overly complex multi-stage architectures, limiting robustness and cross-domain generalization.

Method: Proposes MVOFormer with (1) Flow-Semantic Dual Branch Encoder that synergizes dense geometric motion cues with object-centric semantic priors to distinguish static structures from dynamic distractors, and (2) Iterative Multimodal Decoder that enables coarse-to-fine pose refinement while dynamically suppressing attention on unreliable regions.

Result: Without target-domain fine-tuning, MVOFormer achieves superior zero-shot generalization and robustness, significantly outperforming prior learning-based frame-to-frame methods on TartanAir, KITTI, TUM-RGBD, and ETH3D-SLAM benchmarks.

Conclusion: The dual-branch encoder design with geometric-semantic fusion effectively improves MVO robustness and zero-shot cross-domain generalization compared to existing approaches.
...
TL;DR: AURA is an energy-based inverse attribution model that accurately recovers which antibiotics were actually active (not just applied) in bacterial microscopy, achieving 95.47% exact-match accuracy, compared to naive assumptions being correct only ~37% of the time.


<details>
  <summary>Details</summary>
Motivation: Clinically, the active antibiotic set matters more than the applied set, but these are sharply decoupled (~37% agreement in real E. coli microscopy). Existing tools (scGen, CPA, IMPA) are forward models that degrade when inverted, and discriminative classifiers overfit to strain/batch artifacts.

Method: AURA reframes the task as constrained, energy-based inverse attribution. It uses the inductive bias that the active set is a subset of the applied set, decomposes residual morphology into antibiotic response atoms, and selects the subset with lowest reconstruction energy. AURA-E adds evidence-aware abstention for near-tie cases.

Result: On cross-replicate transfer in E. coli cytological profiling, AURA achieves 95.47% exact-match accuracy for recovering the active antibiotic combination.

Conclusion: AURA effectively solves the inverse attribution problem for antibiotic activity in microscopy, significantly outperforming naive baselines and existing computational approaches, with potential clinical relevance.
...
TL;DR: This paper investigates the quality of uncertainty predictions from VGGT (Best Paper at CVPR-2025), a feed-forward neural network for multi-view 3D reconstruction, identifying effective confidence thresholds and showing that improving uncertainty quality can enhance reconstruction accuracy.


<details>
  <summary>Details</summary>
Motivation: For photogrammetry applications leveraging VGGT's fast, feed-forward 3D reconstruction, high-quality uncertainty estimates are crucial to foster trust and enable robust quality assurance, yet VGGT's uncertainty predictions had not been thoroughly analyzed.

Method: The paper analyzes VGGT's uncertainty predictions by identifying an effective confidence threshold for filtering raw output and evaluating the potential of enhancing uncertainty quality to improve 3D reconstruction accuracy.

Result: An effective confidence threshold for filtering VGGT's raw output was identified. Enhancing uncertainty quality shows strong potential for improving the accuracy of VGGT's 3D reconstructions.

Conclusion: Improving uncertainty quality in VGGT can lead to more accurate 3D reconstructions, supporting its use in real-time, scalable photogrammetry applications.
...
TL;DR: UniBrain is a unified multimodal model for brain MRI analysis that handles missing data through joint imputation and understanding, using a self-alignment strategy for fine-grained features and a dynamic hidden state mechanism to reduce exposure bias during long-context inference.


<details>
  <summary>Details</summary>
Motivation: Medical MLLMs face two key challenges: scarcity of high-quality training data and frequent missing data in real-world clinical settings. The authors aim to address these issues for brain MRI analysis.

Method: 1) Unified training strategy for joint imaging modality imputation and brain image understanding. 2) Interleaved, description-enriched data flow for autoregressive training. 3) Self-alignment strategy leveraging dense image embeddings for fine-grained anatomical features without detailed captions. 4) Dynamic hidden state mechanism to mitigate exposure bias in long-context multimodal inference.

Result: Extensive experiments on multi-disease brain MRI dataset show UniBrain achieves high performance for brain image imputation, understanding, and disease diagnosis under various degrees of modality incompleteness.

Conclusion: UniBrain effectively addresses missing modality challenges in brain MRI analysis, demonstrating strong performance in imputation, understanding, and diagnosis tasks.
...
TL;DR: This paper proposes Active Reference Acquisition for Few-Shot Font Generation, where the model actively queries the designer for the most informative reference glyphs based on part-coverage analysis, improving generation quality with fewer queries.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot font generation methods assume a fixed reference set, but when references are insufficient to represent the target style, generation quality degrades. In practice, designers can provide additional references, so the model should actively decide which characters to query next.

Method: The model sequentially selects which character to acquire as an additional reference using a reference part-coverage-based acquisition function. Each glyph is represented as a histogram of local structural parts, and the system selects characters that maximize expected part coverage of the reference set, prioritizing those containing uncovered parts.

Result: Experiments on the Google Fonts dataset show that the proposed method achieves higher generation quality than random querying and reference-agnostic baselines.

Conclusion: By progressively expanding the diversity of visual parts in the reference set through active querying, the method improves generation quality with fewer designer queries compared to random selection.
...
TL;DR: BadWorld introduces a label-free adversarial attack framework for autoregressive visual world models (VWMs), using self-supervised velocity attacks and trajectory-adaptive bi-level optimization to generate perturbations that cause catastrophic rollout degradation without needing ground-truth future videos or known user controls.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial attacks fail on VWMs because attackers lack ground-truth future videos and cannot predict subsequent user controls. The paper aims to assess the robustness of VWMs against adversarial perturbations under these realistic constraints.

Method: BadWorld uses (1) a self-supervised velocity attack that disrupts early denoising dynamics without future supervision, and (2) trajectory-adaptive bi-level optimization that mines hard control sequences to forge control-agnostic perturbations.

Result: BadWorld exposes severe structural fragility in VWMs (both continuous and discrete controls). Visually indistinguishable perturbations cause catastrophic failures: incomplete denoising, structural collapse, and control inconsistency in future rollouts.

Conclusion: VWMs are highly vulnerable to adversarial perturbations, posing critical risks for deployment in safety-critical systems. However, the mechanism also offers potential for practical privacy protection.
...
TL;DR: Local-GS is a warp-coherent rendering paradigm that optimizes 3D Gaussian Splatting by aligning Gaussian primitives with SIMT execution boundaries, achieving up to 7.76× speedup without quality loss.


<details>
  <summary>Details</summary>
Motivation: The irregular spatial distribution of 3D Gaussian primitives in 3DGS leads to poor GPU utilization due to warp divergence and redundant computation, limiting rendering performance.

Method: Proposes three warp-coherent stages: (1) hoisting stage for tile-level shared parameter precomputation, (2) culling stage to discard non-contributing warps, (3) blending stage replacing per-pixel branching with uniform instruction streams.

Result: Achieves 7.76× speedup on Deep Blending scenes; provides consistent performance gains across all tested baselines as a plug-and-play optimization without quality degradation.

Conclusion: Organizing Gaussians around SIMT execution boundaries rather than scene geometry is an effective plug-and-play strategy for accelerating 3DGS rendering.
...
TL;DR: PROSE uses a pretrained vision-language model to register two egocentric RGB sequences by constructing object-level 3D scene graphs and matching object instances via prompts, achieving state-of-the-art registration without depth sensors, training, or annotated graphs.


<details>
  <summary>Details</summary>
Motivation: Existing registration methods fail for egocentric, RGB-only indoor captures because classical methods need clean point clouds, and learned scene-graph methods require annotated graphs and brittle matchers under egocentric data.

Method: PROSE lifts RGB sequences to object-level 3D scene graphs using off-the-shelf foundation models, then prompts a VLM to match object instances across sequences using object height priors and same/different verification, finally solving for rigid transform via geometric consensus.

Result: Outperforms geometric and learned scene-graph baselines on Aria Digital Twin and Aria Everyday Activities benchmarks, using both ground-truth and RGB-reconstructed point clouds.

Conclusion: PROSE provides a zero-parameter, training-free registration method that works on challenging egocentric RGB data and produces transferable scene graphs for downstream tasks.
...
TL;DR: This paper proposes a transformation-driven framework for generating reproducible synthetic X-ray (VirtualRTG) projections from heterogeneous anatomical scenes where components can move independently, applied to mandibular motion, keeping projection geometry, acquisition, material interpretation, and presentation explicitly separated for controlled, reproducible analysis.


<details>
  <summary>Details</summary>
Motivation: Current DRR methods are optimized for registration, realism, or speed, but lack a framework where independently movable anatomical objects (volumes and surfaces) can be combined in a shared scene to generate reproducible projections under controlled, separable imaging assumptions — needed for studying anatomy-projection relationships in motion scenarios like jaw movement.

Method: Proposes a modular framework embedding independently transformable volumetric and surface-based objects in a shared scene, then propagating them directly into projection space via explicit transformations, while separating projection geometry, acquisition modeling, material interpretation, and image presentation into distinct components.

Result: Demonstrates generation of directly comparable VirtualRTG projections from multiple mandibular configurations (e.g., motion, therapeutic repositioning) using a shared CT/CBCT-based reference scene, maintaining identical imaging assumptions across configurations.

Conclusion: The framework provides a controllable, reproducible methodological environment for studying anatomy–projection relationships and motion observability — not aiming at absolute physical fidelity but at transformation-aware, comparable imaging workflows for craniofacial analysis.
...
TL;DR: MLLMs struggle with fine-grained visual perception due to "visual context rot." LOCUS is a training framework that uses local crops as cues and IoU-based rewards to teach models internal evidence search, improving localization without changing inference.


<details>
  <summary>Details</summary>
Motivation: MLLMs fail to reliably select and use decisive local details from high-resolution images due to redundant visual context, termed "visual context rot."

Method: LOCUS trains MLLMs with local crops as visual cues and optimizes via IoU-based reward to recover the crop's spatial support in the full image. The cue is used only during training, not at inference.

Result: LOCUS improves fine-grained perception, hallucination, general understanding, and reasoning benchmarks. Attention analyses show stronger focus on task-relevant evidence regions.

Conclusion: Training-time visual cue search enables internalized fine-grained evidence selection, providing an effective route to improve localization-sensitive visual understanding without changing inference.
...
TL;DR: The paper proposes a method for object pose estimation without requiring 3D models by leveraging rotational symmetry of industrial objects. Pose and point cloud are jointly refined via iterative optimization using a rotational symmetry constraint loss, achieving performance comparable to model-based methods.


<details>
  <summary>Details</summary>
Motivation: Confidentiality concerns often restrict access to high-quality 3D models of industrial objects, making traditional point-cloud-based pose estimation challenging. The authors aim to overcome this by using rotational symmetry—an easily accessible property—as prior information.

Method: The method jointly estimates object pose and refines the point cloud through iterative optimization. A rotational symmetry constraint loss is constructed by rotating each 3D point according to the current pose estimate, finding multiple correspondences via nearest-neighbor search exploiting symmetry, and computing the loss to iteratively refine both pose and point cloud.

Result: Evaluated on a custom dataset (4 synthetic categories + 1 real wheel hub), the method achieves robust pose estimation and generalizes across diverse objects, with performance comparable to methods that rely on known 3D models.

Conclusion: Explicit incorporation of rotational symmetry into pose optimization enables effective model-free pose estimation, addressing the practical challenge of missing 3D models in industrial settings.
...
TL;DR: DifferAD-R1 is an MLLM-augmented reinforcement learning framework for industrial anomaly localization. It uses a Difference-Guided dual-image paradigm, Dual-Consistency Localization Reward, and difficulty-aware learning strategies. It outperforms baselines and competes with Qwen3-VL (235B), with a new AD-DualDiff dataset of 13K paired images across 20 categories.


<details>
  <summary>Details</summary>
Motivation: Industrial anomaly localization faces poor cross-scenario generalization in closed-set methods. Existing MLLM approaches suffer from QA-style misalignment with localization demands and ineffective optimization (e.g., GRPO) for subtle defects.

Method: Proposes DifferAD-R1 with: (1) Difference-Guided dual-image paradigm reformulating localization as one-shot difference grounding; (2) Dual-Consistency Localization Reward for hard-to-detect anomalies; (3) Difficulty-aware strategy with adaptive reweighting and group-wise resampling; (4) AD-DualDiff dataset (13K paired images, 20 categories).

Result: Significantly outperforms existing baselines and achieves competitive performance compared to large-scale models like Qwen3-VL (235B parameters).

Conclusion: The proposed framework effectively addresses core limitations in MLLM-based industrial anomaly localization, especially for subtle and cross-scenario defects, and is publicly available.
...
TL;DR: SUP-MCRL integrates three mechanisms (SAVE, UEE, PPA) to improve zero-shot neural visual decoding from EEG, achieving state-of-the-art performance on THINGS-EEG dataset.


<details>
  <summary>Details</summary>
Motivation: Conventional multimodal contrastive representation learning for brain-computer interfaces only optimizes geometric distance alignment, ignoring semantic consistency and subject selectivity, leading to spurious zero-shot alignment and poor generalization to natural visual experiences.

Method: Three collaborative components: (1) SAVE - semantic-entity aware visual encoder with spatial attention; (2) UEE - unified EEG enhancer with multi-scale atrous convolutions and inter-band attention for cross-subject robustness; (3) PPA - prototype-based progressive augmenter with EMA-updated pseudo-feature pool to prevent representation collapse.

Result: Zero-shot on THINGS-EEG: 66.0%/91.9% (Top-1/Top-5) intra-subject and 24.0%/52.9% LOSO accuracy, surpassing SOTA methods.

Conclusion: The proposed SUP-MCRL framework effectively addresses semantic consistency and subject selectivity issues, significantly improving brain-computer interface performance for natural visual decoding.
...
TL;DR: This paper addresses instability in vision token pruning under ultra-low token budgets by introducing a distribution consistency metric and a two-stage pruning framework (ACGR + TATCS) that recovers contextual information and dynamically re-selects representative tokens, achieving 92.1% performance retention on LLaVA-1.5-7B with only 16 visual tokens.


<details>
  <summary>Details</summary>
Motivation: Existing vision token pruning methods become unstable under ultra-low token budgets due to severe feature distribution shifts, and there is no effective way to characterize or mitigate this shift.

Method: Proposes a two-stage framework: (1) Anchor-Context Graph Recovery (ACGR) for transferring contextual information before pruning, and (2) Text-Aware Token Cluster Selection (TATCS) for dynamically re-selecting representative tokens when large distribution shifts are detected. Also introduces a lightweight distribution consistency metric to quantify shifts.

Result: Achieves 92.1% of the upper-bound average performance on LLaVA-1.5-7B with only 16 visual tokens, demonstrating superior and more stable performance under ultra-low token budgets.

Conclusion: The proposed method effectively addresses the instability issue in aggressive token pruning by monitoring and mitigating feature distribution shifts, enabling strong performance retention even with extremely few visual tokens.
...
TL;DR: This paper introduces MVM-IOD, a new dataset of industrial object images captured systematically with a robot arm, and evaluates SOTA 3D reconstruction and pose estimation methods, finding that feed-forward methods perform poorly on such out-of-distribution images but can be improved with simple preprocessing.


<details>
  <summary>Details</summary>
Motivation: Existing datasets do not depict realistic industrial scenarios, yet 3D reconstruction and pose estimation in industrial settings are costly if errors occur and computation time is limited. The complexity of industrial objects further challenges existing methods.

Method: Images of 9 industrial objects with 2 background choices (18 scenes) are captured by moving a camera mounted on a robot arm along a hemisphere. The dataset includes reference camera poses and 3D point clouds. SOTA methods (SfM, MVS, DUSt3R/VGGT, π3, 2D Gaussian Splatting) are evaluated.

Result: Feed-forward methods (e.g., VGGT, π3) produce suboptimal results on the captured out-of-distribution images, but simple preprocessing can shift images closer to the training distribution and improve performance.

Conclusion: Feed-forward methods should be used with caution in certain industrial applications, though preprocessing can mitigate distribution shift issues.
...
TL;DR: A coarse-to-fine VLM-based framework for zero-annotation foreground segmentation in computational pathology that generalizes across diverse stain types, achieving state-of-the-art performance on out-of-distribution stains with 7x lower cross-stain variance than supervised baselines, plus VLM-based annotation review matching human expert consensus and lightweight student model distillation.


<details>
  <summary>Details</summary>
Motivation: Existing foreground segmentation methods in computational pathology rely on hand-tuned heuristics or supervised models that overfit to narrow stain/scanner distributions and fail silently on specialized stains (e.g., Jones silver, EVG). There is a need for a generalizable, zero-annotation approach.

Method: Reformulates foreground segmentation as a visual perception task using general-purpose VLMs as zero-annotation oracles; coarse-to-fine pipeline; introduces Leica-75 benchmark (75 renal WSI, 3 stain families); Auto-context few-shot prompting for hard cases; VLM-based annotation review; distills lightweight student models from VLM pseudo-labels.

Result: 0.858 Dice on Jones, 0.853 on EVG (7x lower cross-stain variance than best supervised baseline); competitive on H&E; Auto-context improves Dice from 0.470 to 0.819 on Stress-32; VLM review matches human expert (kappa=0.989 blur detection, 0.708 grading accuracy vs 0.646 human); distilled student models match teacher performance.

Conclusion: The framework provides a principled, scalable solution to the persistent infrastructure bottleneck of foreground segmentation in digital pathology, leveraging VLMs that generalize where domain-specific models fail, and enabling cost-efficient deployment via student distillation.
...
TL;DR: LVLMs hallucinate; standard conformal filtering requires excessive abstention (>80% at 5% hallucination rate). BCEA introduces a three-way decision (answer, abstain, acquire evidence) that maintains statistical guarantees through post-acquisition recalibration, improving coverage while controlling hallucination rates.


<details>
  <summary>Details</summary>
Motivation: Selective prediction with conformal guarantees avoids hallucination by abstaining on ungrounded claims, but this requires abstaining on >80% of claims to keep hallucination rate below 5%—an unacceptably high abstention rate that wastes cheaply available visual evidence.

Method: BCEA replaces binary answer/abstain with a three-way choice: answer, abstain, or acquire more visual evidence (zoom, crop, apply claim-specific intervention) under a bounded budget. Crucially, the acquisition policy is folded into the score function and recalibrated on post-acquisition scores to restore exchangeability and the finite-sample guarantee.

Result: On POPE and COCO-constructed existence/spatial-relation claims across four open VLMs, BCEA controls hallucination rate at the target level and consistently improves coverage over a guaranteed-abstention baseline. Naive acquisition breaks the guarantee (realized risk overshoots target by up to 17 points).

Conclusion: BCEA demonstrates that selective prediction for LVLMs can be made more practical by replacing pure abstention with targeted evidence acquisition, while preserving distribution-free statistical guarantees through proper recalibration.
...
TL;DR: Sinkhorn-CPD replaces CPD's target-side marginal constraint with dual KL penalties, enabling fully unbalanced optimal transport for robust point cloud registration with automatic annealing via variance.


<details>
  <summary>Details</summary>
Motivation: CPD's target-side marginal constraint forces outlier points to receive unit probability mass, degrading accuracy under heavy outliers and partial overlap. Existing OT methods handle missing mass but require hand-tuned annealing schedules.

Method: Replace CPD's target-side marginal constraint with dual Kullback-Leibler penalties, formulating a fully unbalanced entropic optimal transport problem solved by generalized Sinkhorn iterations while preserving closed-form Procrustes and variance updates.

Result: State-of-the-art accuracy on synthetic, cross-category, and scan-to-CAD benchmarks with strong robustness to outliers and partial overlap.

Conclusion: Sinkhorn-CPD achieves both robustness to outliers/partial overlap and automatic annealing (via sigma^2 as entropic regularization) without manual temperature tuning, outperforming prior methods.
...
TL;DR: MMDiff transforms a frozen diffusion transformer into a multi-modal generative system that jointly produces images and dense perceptual modalities (segmentation, depth, saliency) using lightweight decoder heads, leveraging temporally distributed perceptual information through multi-timestep feature fusion.


<details>
  <summary>Details</summary>
Motivation: To repurpose the rich perceptual representations computed across the denoising trajectory of diffusion transformers—which are typically discarded after image generation—for joint multi-modal output generation without retraining the backbone.

Method: Uses a frozen diffusion transformer backbone with lightweight decoder heads; employs multi-timestep feature fusion with spatially varying aggregation weights; adopts concept-driven attention extraction for interpretable spatial guidance.

Result: Multi-timestep feature fusion improves semantic segmentation by up to 28.7% mIoU over single-timestep extraction; frozen diffusion features are competitive with/complementary to DINOv3; strong performance on segmentation, salient object detection, and depth estimation.

Conclusion: Frozen diffusion transformers can serve as effective multi-modal generative backbones, enabling scalable synthetic data generation while being competitive with state-of-the-art dedicated encoders.
...
TL;DR: A simple Noise Amplification method using bit-plane analysis detects AI-generated videos (especially text-to-video) by amplifying noise signals at pixel, region, and frame levels, outperforming SOTA on GenVidBench and a new HardGVD benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing video forgery detection mainly focuses on GAN-generated videos, while detection of text-to-video model outputs remains largely unexplored. The authors observe that even advanced T2V models fail to generate realistic fine-grained details and temporal detail changes, motivating a detail/noise-focused detection approach.

Method: The Noise Amplification pipeline: (1) extract noise signals based on bit-planes, (2) amplify noise through three complementary strategies — pixel-level intensity enhancement, region-level spatial amplification, and frame-level temporal aggregation, (3) feed amplified noise into a discriminator for fake/real classification.

Result: Extensive experiments on GenVidBench and the newly proposed HardGVD benchmark demonstrate that Noise Amplification significantly outperforms existing state-of-the-art methods in AI-generated video detection.

Conclusion: Bit-plane based noise amplification is a simple yet highly effective approach for detecting text-to-video generated content, and the HardGVD benchmark provides a more challenging testbed for future research in this domain.
...
TL;DR: SKMamba is a structure-aware, knowledge-guided Mamba-based multi-modal framework for automated Zygomaticomaxillary Suture (ZMS) maturation assessment, built on the first public ZMS dataset of 3,790 images (ages 4-24). It uses a decoupled dual-path architecture with an Implicit Edge Extractor and Cross-Modal Semantic Alignment module, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: ZMS maturation staging is clinically critical for orthopedic intervention timing but remains challenging due to subtle high-frequency suture transitions and global semantic ambiguity between adjacent stages. No public ZMS dataset previously existed.

Method: SKMamba uses (1) a decoupled dual-path architecture mimicking orthodontists' hierarchical diagnostic process, (2) an Implicit Edge Extractor (IEE) with structural pre-training to reduce noise and enhance sutural boundaries, and (3) a Cross-Modal Semantic Alignment (CSA) module that incorporates LLM-generated anatomical descriptions while preserving objective morphological evidence priority.

Result: SKMamba achieves state-of-the-art performance on the first public ZMS dataset (3,790 images) compared to existing methods.

Conclusion: SKMamba effectively addresses ZMS maturation assessment challenges by combining structural edge extraction with cross-modal semantic alignment, establishing a new benchmark for this clinical task.
...
TL;DR: Proposes a 3D multimodal deep learning framework for classifying paramagnetic rim lesions (Rim⁺) from QSM and FLAIR MRI, using asymmetric multimodal modeling with self-supervised pretraining and contrastive regularization, achieving improved performance on a cohort of 88 MS patients.


<details>
  <summary>Details</summary>
Motivation: Rim⁺ lesions are biomarkers of chronic active inflammation in MS associated with disability progression, but susceptibility imaging and expert assessment are limited to specialized centers, time-consuming, variable, and suffer from severe class imbalance for automated analysis.

Method: A 3D multimodal deep learning framework treating QSM as the primary susceptibility signal conditioned with FLAIR-derived structural context, using self-supervised multimodal pretraining followed by supervised fine-tuning with contrastive regularization.

Result: Improved performance compared to prior architectures on a clinically acquired cohort of 88 MS patients, supporting effectiveness of asymmetric multimodal modeling for chronic active lesion identification.

Conclusion: Asymmetric multimodal modeling is effective for automated Rim⁺/Rim⁻ classification, offering a more accessible and robust approach to chronic active lesion identification in MS.
...
TL;DR: TV-Edit unifies textual and visual instructions for precise, intent-faithful image editing by constructing a paired dataset, a unified framework, and a benchmark, outperforming both text-only and drag-based methods.


<details>
  <summary>Details</summary>
Motivation: Existing methods are limited: textual instructions lack spatial precision, while visual prompts (drag/point) suffer semantic ambiguity. The goal is to unify both for precise and intent-faithful image manipulation.

Method: (1) Built a paired textual-visual instruction dataset (23K+ samples) from dynamic videos. (2) Proposed TV-Edit framework that contextualizes drag/point instructions with image-text semantics into semantic-aware control representations for pretrained editing backbones. (3) Established TV-Edit-Bench for evaluating semantic faithfulness, spatial alignment, and visual consistency.

Result: TV-Edit achieves more precise spatial control, less instruction ambiguity, and stronger structural consistency than text-only or drag-based baselines. Consistently outperforms state-of-the-art instruction-based and drag-based methods across multiple editing backbones.

Conclusion: Integrating semantic intent (text) and spatial constraints (visual prompts) yields superior, intent-faithful image edits. The dataset and benchmark provide a foundation for future cross-modal instruction research.
...
TL;DR: Gen-VCoT is a framework that uses expert vision models to generate RGB images as visual reasoning intermediates for MLLMs, improving spatial and depth reasoning but underperforming on simple factual queries compared to text-based CoT.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs lack interpretable visual intermediates in their reasoning process, relying on opaque tokens or external tools that miss key properties of visual reasoning.

Method: Gen-VCoT uses a three-stage pipeline: (1) visual grounding via SAM segmentation, (2) geometric reasoning via Marigold depth maps, and (3) semantic reasoning via Qwen2-VL integration, with an adaptive router to select reasoning depth.

Result: Gen-VCoT improves spatial questions by 25% and depth questions by 50%, but may hurt simple factual queries. Text CoT outperforms visual intermediates on CLEVR (91.2% vs 62.5%), showing task-dependent optimal representations.

Conclusion: Gen-VCoT establishes a new paradigm for interpretable multimodal reasoning, though the choice between visual and text intermediates depends on the task.
...
TL;DR: This study proposes an LLM-based evaluation framework (using GPT-5.5, Gemini 3.5 Flash, Claude Sonnet 4.6) to assess Grad-CAM explanations in facial skin disease diagnosis models (EfficientNet-B0, MobileNetV3, ResNet18) under different augmentation strategies, focusing on lesion localization and explanation trustworthiness.


<details>
  <summary>Details</summary>
Motivation: Prior work mainly improved classification performance via data augmentation but rarely examined whether model explanations are grounded in clinically relevant lesion regions.

Method: Applied geometric, color-based, and mixed augmentation to EfficientNet-B0, MobileNetV3, ResNet18; used Grad-CAM for visual explanations; designed an LLM-as-a-Judge framework (GPT-5.5, Gemini 3.5 Flash, Claude Sonnet 4.6) with progressive prompt engineering (evaluation rubrics, clinical knowledge, penalty rules, structured outputs).

Result: Not explicitly stated in the abstract; the paper proposes the framework and methodology but does not report quantitative findings here.

Conclusion: The proposed LLM-based evaluation framework aims to improve consistency and clinical grounding in assessing Grad-CAM explanations for skin disease diagnosis models.
...
TL;DR: WaveDINO is a wavelet-based multi-scale denoising framework using frozen DINOv3 features and terrain info, trained on synthetic deformation + real atmospheric noise, that outperforms weather-model-based InSAR corrections, reducing GNSS misfit by ~3-19% at two volcano sites.


<details>
  <summary>Details</summary>
Motivation: InSAR-based volcanic deformation monitoring is corrupted by atmospheric phase delays and decorrelation; existing weather-model corrections are inconsistent and may introduce residual biases.

Method: WaveDINO combines wavelet-based multi-scale denoising with frozen DINOv3 foundation-model features and terrain conditioning. Training uses physically motivated synthetic magma deformation superimposed on real short-term interferograms to provide ground truth while exposing the network to realistic atmospheric statistics.

Result: WaveDINO outperforms competing models on both synthetic and real data from Laguna del Maule and Campi Flegrei, reducing mean GNSS misfit by ~3% and ~19% respectively, surpassing weather-model-based corrections.

Conclusion: The hybrid training strategy combining synthetic deformation with real atmospheric noise is effective for InSAR denoising, and WaveDINO offers a robust alternative to traditional weather-model-based atmospheric correction methods.
...
TL;DR: MST-CLIPIQA is a multi-scale two-stream VLM framework for AI-generated image quality assessment that decouples semantic and perceptual representations via dual CLIP encoders with different patch granularities and an information bottleneck fusion mechanism, achieving SOTA results with only 0.8M parameters.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based AIGIQA methods suffer from a semantic-distortion dimensional conflict where monolithic representations optimized for semantic discrimination entangle compositional understanding with low-level perceptual sensitivity, making them blind to fine-grained quality degradations.

Method: The framework uses dual CLIP encoders with complementary patch granularities: coarse-grained streams capture global semantics, fine-grained streams preserve textures and artifacts. An information bottleneck-inspired gated fusion mechanism performs adaptive cross-scale distillation, with optional cross-attention for prompt-anchored correspondence evaluation when generation prompts are available.

Result: Achieves new SOTA across five benchmarks with average improvements of 1.11% SRCC on quality and 2.35% SRCC on text-image correspondence prediction, using only 0.8M trainable parameters.

Conclusion: Explicit representational decoupling through multi-scale two-stream design effectively resolves the semantic-distortion conflict in AIGIQA, enabling both high-level semantic understanding and low-level perceptual sensitivity efficiently.
...
TL;DR: The paper proposes a Temporal Pyramid Adapter with self-supervised XLS-R representations for multi-scale spoofed speech detection, achieving state-of-the-art results (3.87% EER on PartialSpoof) but showing degradation under domain/language shifts.


<details>
  <summary>Details</summary>
Motivation: Spoofed speech detection faces challenges from realistic synthesis, voice conversion, and replay attacks, with cross-dataset generalization being a major limitation.

Method: Proposes a Temporal Pyramid Adapter using parallel temporal convolutions with varying receptive fields to capture multi-scale spoofing cues, integrated with self-supervised XLS-R representations and front-end adapters (Mel, Sinc, Temporal Pyramid).

Result: Achieved AUC of 99.24% and EER of 3.87% on PartialSpoof, significantly outperforming LCNN-BLSTM (9.87% EER) and TRACE (8.08% EER). Evaluated on ASVspoof 2017/2021, PartialSpoof, DiffSSD, and HQ-MPSD datasets.

Conclusion: Spoofing artifacts are language-independent, but performance degrades under domain/language shifts, indicating need for better adaptation and calibration strategies.
...
TL;DR: The paper presents an open-source monitoring architecture using Grafana-Prometheus for multi-center medical studies, deployed across 38 German university clinics in the RACOON consortium, enabling privacy-preserving data exploration and study progress tracking.


<details>
  <summary>Details</summary>
Motivation: Multi-center studies lack efficient, transparent, and up-to-date monitoring solutions; current reliance on manual communication and shared tables hinders coordination in large distributed studies.

Method: A lightweight, open-source monitoring framework based on the Grafana-Prometheus stack, collecting aggregated metrics from distributed sites and visualizing them via configurable dashboards, integrated into the Kaapana medical imaging platform.

Result: Successfully deployed in the RACOON consortium across all 38 German university clinics, demonstrating privacy-preserving data exploration and real-time study progress monitoring.

Conclusion: The framework supports transparent coordination and efficient management of large-scale multi-center studies, with publicly available source code.
...
TL;DR: CustomShift proposes a dual-branch architecture grounded in maximum entropy theory to address subject-driven image customization, outperforming SOTA on DreamBooth and Custom101 benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing customization methods (test-time fine-tuning, encoder-based, token competition) suffer from limited efficiency, misalignment between reference features and generation, and irrelevant information interference. The authors aim to overcome these limitations.

Method: Formulates customization as a distribution shift using maximum entropy theory, leading to a Conditional Attention Distribution Shift. Implements this via CustomShift: a dual-branch architecture on Stable Diffusion 3 with a Reference-Alignment Branch (self-attention for layer-wise alignment) and a Cross-Guidance Branch (integrates text and reference cues).

Result: Outperforms state-of-the-art methods on DreamBooth and Custom101 benchmarks, achieving a better trade-off between semantic fidelity and subject consistency.

Conclusion: The proposed theoretical grounding and architectural design effectively address key limitations in subject-driven customization, offering a more efficient and aligned approach to preserving subject identity while following textual instructions.
...
TL;DR: This paper introduces a benchmark suite for Federated Noisy Label Learning (FNLL) in medical image segmentation, combining real-world noisy datasets, diverse client-noise scenarios, and targeted evaluation to enable systematic assessment and informed method selection under realistic federated settings.


<details>
  <summary>Details</summary>
Motivation: Federated learning in medical image segmentation is often hindered by real-world label imperfections (e.g., contour disagreements, missing structures, confused labels). Existing FNLL research relies on synthetic noise and simplified settings, lacking realistic evaluation benchmarks.

Method: The authors build a benchmark suite integrating real-world noisy medical image segmentation datasets from diverse sources, multiple client-noise scenarios relevant to deployment, and noise-targeted evaluation metrics within a comprehensive federated segmentation framework.

Result: The suite provides a realistic and discriminative basis for FNLL evaluation, enabling fair benchmarking, dataset-specific label-noise characterization, and future method development under realistic federated settings.

Conclusion: This benchmark fills the gap between synthetic-noise-based FNLL research and real-world deployment needs, establishing a reusable foundation for advancing FNLL in medical image segmentation.
...
TL;DR: This paper trains a goal-conditioned PPO policy to estimate material parameters from fracture behavior in a non-differentiable simulator, using orange peeling as a test case. The policy operates in a normalizing flow latent space, achieving 0.642 recovery in 10ms, and a warm-start CMA-ES refinement improves recovery to 0.828.


<details>
  <summary>Details</summary>
Motivation: Material parameters for realistic food simulation are difficult to measure directly and vary across heterogeneous regions. The authors aim to solve the inverse problem of estimating these parameters from target fracture behavior descriptions.

Method: Train a neural surrogate on 2,000 forward simulations. Compare CMA-ES (evolutionary optimizer) with PPO (reinforcement learning) across original 9D parameter space and learned 4D latent representations. Train a goal-conditioned PPO policy that learns a general inverse mapping for arbitrary targets.

Result: Goal-conditioned PPO achieves 0.642 recovery in 10ms, outperforming original parameter space by 23%. Warm-start CMA-ES refinement improves recovery to 0.828 with 540 evaluations.

Conclusion: Provides a practical framework for inverse food physics and lays groundwork for vision-driven material identification from video observations of food manipulation.
...
TL;DR: Semantic Flip synthesizes out-of-distribution query-video pairs to train a lightweight rejection module for detecting unanswerable queries in embodied VLMs, achieving high F1 scores on a new spatial refusal benchmark.


<details>
  <summary>Details</summary>
Motivation: Embodied VLMs often produce overconfident answers for unanswerable queries due to insufficient visual memory, which poses risks in real-world deployment (e.g., misleading users or physically guiding them to arbitrary locations).

Method: Semantic Flip independently transforms the query and video memory to create auxiliary OOD pairs lacking visual grounding, then trains a lightweight rejection module on top of a frozen pretrained VLM without retraining the base model.

Result: Outperforms strong prompting baselines across two benchmarks; achieves F1 score of 0.9559 on SpaceReject, a new refusal benchmark for spatial localization with unanswerable queries over long video memory.

Conclusion: Semantic Flip provides an effective and annotation-free way to enable embodied VLMs to refuse unanswerable queries, enhancing reliability for real-world deployment.
...
TL;DR: This paper proposes a top-down camera system with physics-based simulation to enable simultaneous multi-fillet Woody breast detection, overcoming the single-fillet throughput bottleneck of side-view approaches.


<details>
  <summary>Details</summary>
Motivation: Current automated WB detection uses side-view imaging limited to single-fillet analysis, creating throughput bottlenecks on commercial processing lines. The authors aim to develop a scalable multi-fillet detection alternative.

Method: (1) Developed a high-fidelity digital twin of an industrial conveyor system; (2) Synthesized a diverse dataset of 3D fillet meshes; (3) Modeled viscoelastic bending dynamics with physics-based simulation; (4) Extracted a continuous 2D shape deformation score from top-down perspective.

Result: The top-down shape score effectively captures contour changes of bending fillets, providing a robust alternative to side-view systems for simultaneous multi-fillet WB evaluation.

Conclusion: A top-down multi-fillet architecture with physics-based simulation can overcome throughput limitations of side-view systems while maintaining detection robustness for Woody breast myopathy.
...
TL;DR: SurroundNEXO is a multi-camera metric depth framework for autonomous driving that handles low-overlap surround cameras by using ego-centric geometry (Ego-Ray Positional Encoding) instead of dense visual correspondences, achieving significant improvements in depth accuracy, cross-view consistency, and metric reconstruction quality across benchmarks.


<details>
  <summary>Details</summary>
Motivation: Multi-camera depth prediction in autonomous driving suffers from limited visual overlap across outward-facing surround-view cameras, making conventional multi-view geometry (which relies on dense correspondences) ineffective.

Method: 1) Ego-Ray Positional Encoding to assign globally comparable viewing directions; 2) Sparse LiDAR measurements as metric anchors for scale propagation; 3) Progressive feature interaction from view-local to spatio-temporal to global integration.

Result: Single-view error reduced by 33.2%, cross-view consistency improved by 10.5%, metric reconstruction quality enhanced by 25.6% on NuScenes, Waymo, DDAD benchmarks; robust under extremely sparse depth prompts; strong zero-shot generalization.

Conclusion: SurroundNEXO effectively bridges the low-overlap multi-camera gap by grounding cross-view reasoning in ego-centric geometry rather than visual correspondences, setting a new SOTA for surround-view metric depth estimation.
...
TL;DR: This paper introduces a multi-center benchmark for abdominal disease diagnosis and radiology report generation that synthesizes contrast-enhanced findings from non-contrast CT (NCCT), using a large paired NCCT-CECT dataset from two centers. Five deep learning architectures were benchmarked, achieving 69.1% AUC internally and 63.1% externally, showing NCCT retains diagnostic signals. The dataset and code are publicly released.


<details>
  <summary>Details</summary>
Motivation: To reduce the risks of contrast-induced nephropathy, lower acquisition burden, and decrease radiologist workload by enabling contrast-free abdominal imaging workflows.

Method: Curated a large-scale paired NCCT-CECT dataset from two centers; benchmarked five deep learning architectures (chest-specific, abdomen-specific, general-purpose multimodal) under a unified evaluation protocol for multi-organ disease diagnosis and automated report generation.

Result: Average multi-organ AUC of 69.1% on internal cohort and 63.1% on external cohort, demonstrating NCCT retains diagnostic signals for contrast-enhanced findings.

Conclusion: NCCT can effectively synthesize contrast-enhanced findings, enabling safer and more resource-efficient abdominal imaging; public release of dataset/benchmark aims to catalyze future research.
...
TL;DR: DreamX-World 1.0 is a general-purpose interactive text/image-to-video world model that supports controllable long-horizon generation with camera navigation, scene revisiting, and promptable events across photorealistic, game-style, and stylized domains. It introduces E-PRoPE for camera control, uses causal forcing and distillation for autoregressive generation, employs memory-conditioned scene persistence and residual recycling, and achieves up to 16 FPS on eight RTX 5090 GPUs, outperforming prior models with an overall score of 84.76.


<details>
  <summary>Details</summary>
Motivation: To develop a general-purpose world model capable of long-horizon, controllable video generation that supports camera navigation, scene revisiting, and event control across diverse visual domains.

Method: The system uses a data engine combining Unreal Engine rendering, gameplay recordings, and real-world videos. Camera control is achieved via E-PRoPE (a lightweight variant of projective positional encoding). A bidirectional video generator is converted into an autoregressive model using causal forcing, DMD-style distillation, and long-rollout training. Memory-Conditioned Scene Persistence retrieves prior views via camera geometry, and residual recycling handles imperfect memory latents. Event Instruction Tuning adds composable event control, and RL alignment recovers quality after distillation. Efficiency is improved via mixed-precision DiT, residual reuse, pruned VAE decoding, and asynchronous pipeline parallelism.

Result: DreamX-World 1.0 achieves 16 FPS on eight RTX 5090 GPUs, with a camera-control score of 73.75 and an overall score of 84.76 on the 5-second basic evaluation, outperforming HY-WorldPlay 1.5 (80.79) and LingBot-World (80.45).

Conclusion: DreamX-World 1.0 demonstrates state-of-the-art performance in controllable long-horizon video generation with effective camera control, scene persistence, and composable events, while maintaining practical inference speed.
...
TL;DR: ActiveSAM is a training-free, zero-shot framework that accelerates SAM 3 for open-vocabulary semantic segmentation by estimating per-image active class subsets, achieving up to 5.5x speedup and +1.4 mIoU improvement over the state-of-the-art.


<details>
  <summary>Details</summary>
Motivation: Direct application of SAM 3 to OVSS is inefficient because full-resolution decoding runs over the entire dataset vocabulary, while each image contains only a small subset of active classes.

Method: ActiveSAM canonicalizes/expands class prompts, estimates an image-conditioned active set via a low-resolution presence preview, decodes only retained classes at full resolution with bucketed prompt multiplexing, and applies margin-aware background calibration.

Result: Across eight OVSS benchmarks, ActiveSAM achieves +1.4 mIoU over SegEarth-OV3 on average while running up to 5.5x faster on large-vocabulary datasets, and shows strongest robustness under image corruption.

Conclusion: ActiveSAM demonstrates that training-free active-vocabulary selection is an effective and efficient paradigm for open-vocabulary semantic segmentation, especially suitable for real-world noisy-input domains like autonomous driving and embodied AI.
...
TL;DR: FusionRS introduces the first large-scale RGB-infrared-text dataset for dual-modal vision-language learning in remote sensing, enabling improved RGB-IR alignment and infrared-aware captioning through IR-specific textual supervision.


<details>
  <summary>Details</summary>
Motivation: Existing remote sensing vision-language models focus on RGB imagery, leaving infrared data underexplored. Infrared provides complementary cues (thermal intensity, boundaries, illumination invariance), but no large-scale RGB-infrared-text dataset exists for dual-modal learning.

Method: FusionRS translates public RGB remote sensing images into infrared-style counterparts to create aligned RGB-IR pairs. Each pair includes conventional scene captions plus IR-aware captions describing infrared-specific visual properties. CLIP-style models are trained for RGB-IR-text alignment, then fine-tuned for generative dual-modal captioning.

Result: FusionRS improves RGB-IR alignment, infrared-to-text retrieval, and dual-modal captioning over RGB-only and non-IR-aware training baselines. Ablations confirm IR-aware captions are crucial for infrared-language alignment.

Conclusion: IR-aware textual supervision is essential for scalable RGB-infrared remote sensing vision-language representation learning, and FusionRS provides a strong foundation for dual-modal understanding.
...
TL;DR: MeshLoom is a feed-forward network that directly reconstructs vertex deformations across mesh sequences for non-rigid registration, using a topology-aware encoder-decoder design with global motion embedding to achieve fast, state-of-the-art results across diverse motions and categories, also enabling motion interpolation and mesh morphing.


<details>
  <summary>Details</summary>
Motivation: Existing non-rigid registration models are limited by costly per-instance optimization, narrow object categories, pairwise-only inputs, or intermediate outputs only. The authors aim to overcome these constraints with a simple, efficient, feed-forward approach.

Method: The core is a topology-aware encoder-decoder: (1) a topology-aware point representation encodes the anchor mesh's topology into per-vertex features; (2) a multi-modal encoder fuses anchor representation with shape latents and image features into a compact global motion embedding; (3) a lightweight decoder queries this embedding to retrieve per-vertex deformations at target timestamps.

Result: Achieves state-of-the-art results on non-rigid registration across diverse motions and object categories. The network registers multiple meshes within seconds.

Conclusion: The global embedding-then-query paradigm naturally extends to generating deformations at intermediate timestamps, enabling motion interpolation and mesh morphing beyond standard registration.
...
TL;DR: Qwen-RobotWorld is a language-conditioned video world model that predicts future visual trajectories across robotic manipulation, autonomous driving, indoor navigation, and human-to-robot transfer, using a 60-layer double-stream diffusion transformer with Qwen2.5-VL, an 8.6M video-text corpus, and a two-stage progressive curriculum training.


<details>
  <summary>Details</summary>
Motivation: To create a unified language-conditioned world model that can generate physically grounded future visual trajectories across diverse embodied tasks (robotics, driving, navigation, human-to-robot transfer) for synthetic data generation, policy evaluation, and planning.

Method: Three-part design: (a) Double-Stream MMDiT with MLLM Action Encoding - 60-layer double-stream diffusion transformer coupling Qwen2.5-VL semantics with video-VAE latents via joint attention; (b) Embodied World Knowledge (EWK) - 8.6M video-text corpus (200M+ frames) with 20+ embodiments and 500+ action categories; (c) General+Expert Progressive Curriculum - two-stage training first learning general visual priors then injecting embodied specialization.

Result: Ranks 1st overall on EWMBench and DreamGen Bench, outperforms all open-source models on WorldModelBench and PBench; demonstrates robust generalization and multi-view consistency on RoboTwin-IF benchmark.

Conclusion: The unified language-conditioned video world model approach effectively supports synthetic data generation, scalable policy evaluation, and language-guided planning across multiple embodied domains, with strong generalization capabilities.
...
TL;DR: Natural image recognition relies on Fourier phase (not magnitude), and trained classifiers like ViT, GFNet, and PRISM2D preserve this phase-dominant identity code in hidden layers, while ResNet-50 hides it behind ReLU nonlinearities but still depends on phase information.


<details>
  <summary>Details</summary>
Motivation: To investigate whether the classic Fourier phase/magnitude asymmetry observed in human vision (Oppenheim & Lim, 1981) is reproduced inside the hidden layers of trained image classifiers, and to test it causally via phase-magnitude transplant experiments.

Method: Given two images, the authors transplant the phase of one onto the magnitude of the other at a chosen layer and record which image the prediction follows. They test this on PRISM2D, GFNet, ViT-B/16, and ResNet-50, including pre- and post-ReLU interventions and a DC-only control.

Result: In PRISM2D, GFNet, and ViT-B/16, prediction follows the phase/sign donor, and deleting image-specific magnitude barely changes accuracy. ResNet-50 initially breaks this pattern (post-ReLU sign transplant does nothing), but a pre-ReLU intervention reveals a strong latent sign code in late blocks, and a DC-only control shows the readout uses a channel-wise spatial average.

Conclusion: All architectures share a phase/sign identity code but expose it in different bases, determined by rectification and readout geometry. This provides a mechanistic account of the texture-shape gap between CNNs and attention models.
...
TL;DR: BRDFusion combines physically-based rendering with generative priors to produce high-quality, controllable videos from inverse rendering of urban scenes, outperforming baselines in reconstruction quality while enabling novel-view relighting, night simulation, and dynamic object editing.


<details>
  <summary>Details</summary>
Motivation: Physically-based rendering suffers from reconstruction/rendering artifacts, while generative models lack consistency and controllability. The authors aim to combine both approaches to overcome their respective limitations.

Method: A unified framework that uses physical modeling to recover explicit scene properties, generative priors to resolve optimization ambiguity, and a generative model to denoise artifacts during forward rendering.

Result: Outperforms baselines in both real and synthetic scenes, producing high-quality videos with precise control, and supports novel-view relighting, night simulation, and dynamic object insertion/editing.

Conclusion: Combining physical and generative models yields superior video quality and controllability for urban scene rendering, enabling practical downstream applications.
