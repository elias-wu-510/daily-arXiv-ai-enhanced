...
TL;DR: WorldVQA is a benchmark that evaluates MLLMs' atomic visual world knowledge by decoupling knowledge retrieval from reasoning, focusing on what models memorize rather than how they reason.


<details>
  <summary>Details</summary>
Motivation: Current evaluations often conflate visual knowledge retrieval with reasoning, making it difficult to assess what MLLMs actually memorize about the visual world. There's a need for a benchmark that strictly measures visual factuality and encyclopedic knowledge.

Method: WorldVQA decouples knowledge retrieval from reasoning to focus on atomic capabilities. It assesses grounding and naming of visual entities across a stratified taxonomy, covering common head-class objects to long-tail rarities.

Result: The paper introduces WorldVQA as a benchmark, but doesn't present specific experimental results. The benchmark is designed to serve as a rigorous test for visual factuality and establish standards for assessing encyclopedic breadth and hallucination rates.

Conclusion: WorldVQA provides a specialized benchmark for evaluating MLLMs' atomic visual world knowledge, enabling better assessment of visual factuality, encyclopedic breadth, and hallucination rates in current and future models.
...
TL;DR: AdaptMMBench is a new benchmark for evaluating adaptive multimodal reasoning in VLMs, focusing on dynamic difficulty assessment and mode selection rationality across five domains, using MCC metrics to isolate meta-cognition abilities.
...
Method: Proposes AdaptMMBench benchmark across five domains (real-world, OCR, GUI, knowledge, math) encompassing direct perception and complex reasoning tasks. Uses Matthews Correlation Coefficient (MCC) metric to evaluate selection rationality of reasoning modes by dynamically identifying task difficulties based on models' capability boundaries. Enables multi-dimensional process evaluation across key step coverage, tool effectiveness, and computational efficiency.

Result: Evaluation reveals that adaptive mode selection scales with model capacity but notably decouples from final accuracy. Key step coverage aligns with performance, while tool effectiveness remains highly inconsistent across model architectures.

Conclusion: AdaptMMBench provides a comprehensive framework for evaluating adaptive multimodal reasoning that isolates meta-cognition abilities and enables fine-grained process analysis, revealing important insights about the relationship between mode selection, model capacity, and performance.
...
Result: Experiments on synthetic corneal OCT dataset demonstrate robust optical map recovery under noise, improved resolution, and enhanced structural fidelity compared to conventional approaches.
...
Method: Proposes SVD-ViT with three components: SPC module, SSVA, and ID-RSVD. Uses singular value decomposition to extract and aggregate singular vectors capturing object foreground information, suppressing task-irrelevant background noise and artifacts.
...
Conclusion: SVD-ViT successfully addresses ViT's foreground-background discrimination limitation by leveraging SVD to prioritize foreground feature learning, leading to better classification performance.
...
<details>
  <summary>Details</summary>
Motivation: Manual landmarking is time-consuming and variable, while rule-based methods are limited to specific geometries or landmarks. There's a need for automated, generalizable landmark detection that can work across different species for translational research.

Method: Landmark Point Transformer (LmPT) uses point cloud representation of anatomical surfaces with a conditioning mechanism for adaptability to different input types, enabling cross-species learning between homologous bones from different species.

Result: The method demonstrates generalization and effectiveness across species, specifically evaluated on femoral landmarking using both human and newly annotated dog femurs.

Conclusion: LmPT provides an effective solution for automatic anatomical landmark detection that overcomes limitations of traditional methods and enables translational research across species through cross-species learning capabilities.
...
TL;DR: Self-supervised multi-view video anonymization framework for operating rooms that retrieves false negatives using temporal and multi-view context without manual annotation or camera calibration.


<details>
  <summary>Details</summary>
Motivation: Privacy preservation is essential for OR video research, but existing methods require manual annotations for each new clinical site and camera calibration when cameras are repositioned, creating scalability bottlenecks.

Method: Uses off-the-shelf whole-body person detector with low threshold, retrieves false negatives via tracking and self-supervised uncalibrated multi-view association, uses recovered detections as pseudo labels to iteratively fine-tune detector, then applies whole-body pose estimation and fine-tunes pose model with its own high-score predictions.

Result: Achieves over 97% recall on 4D-OR dataset of simulated surgeries and real surgery dataset. Also trains real-time whole-body detector using pseudo labels with comparable performance.

Conclusion: Proposed framework effectively addresses scalability bottlenecks in OR video anonymization by eliminating need for manual annotation and camera calibration while maintaining high detection accuracy.
...
TL;DR: ViThinker enables vision-language models to actively query for task-relevant visual features instead of passively processing pre-computed inputs, improving reasoning through generative mental simulation.


<details>
  <summary>Details</summary>
Motivation: Current Chain-of-Thought reasoning in vision-language models suffers from premature visual-to-text conversion that discards continuous spatial information, and existing methods remain passive by processing pre-computed inputs rather than actively seeking task-relevant details.

Method: ViThinker introduces decision tokens that trigger synthesis of expert-aligned visual features on demand. It uses a two-stage curriculum: first distilling frozen experts into model parameters, then learning task-driven querying via sparsity penalties to discover minimal sufficient perception for each reasoning step.

Result: Evaluations across vision-centric benchmarks show consistent improvements, validating that active query generation outperforms passive approaches in both perceptual grounding and reasoning accuracy.

Conclusion: Active perception through query generation enables vision-language models to perform better reasoning by internalizing vision-expert capabilities and performing generative mental simulation during inference without external tool calls.
...
<details>
  <summary>Details</summary>
Motivation: Existing medical imaging approaches rely on nearest neighbor retrieval that returns redundant evidence and reinforces single hypotheses, failing to reason over subtle visual differences between confusable conditions.

Method: Contrastive document-aware reference selection framework using ROCO embeddings and metadata to balance visual relevance, embedding diversity, and source-level provenance. Counterfactual-Contrastive Inference performs structured pairwise visual comparisons with margin-based decision rules and faithful abstention.
...
Conclusion: The framework enables systematic study of contrastive retrieval in medical image reasoning and demonstrates that optimized evidence selection for discrimination rather than similarity significantly improves decision-making in confusable medical conditions.
...
TL;DR: Current privacy-preserving face recognition systems fail to protect identity information despite good pixel-level reconstruction resistance, as shown by FaceLinkGen attack achieving high matching and regeneration success.


<details>
  <summary>Details</summary>
Motivation: Existing PPFR evaluations focus on pixel-level reconstruction resistance (PSNR/SSIM), but this reconstruction-centric view fails to capture real privacy risks where identity information can still be extracted from protected templates.

Method: The paper presents FaceLinkGen, an identity extraction attack that performs linkage/matching and face regeneration directly from protected templates without recovering original pixels. It tests this attack on three recent PPFR systems.

Result: FaceLinkGen achieves over 98.5% matching accuracy and above 96% regeneration success on three PPFR systems, and still exceeds 92% matching and 94% regeneration in near zero knowledge settings.

Conclusion: There's a structural gap between pixel distortion metrics (PSNR/SSIM) used in PPFR evaluation and real privacy. Visual obfuscation leaves identity information broadly exposed to both external attackers and untrusted service providers.
...
TL;DR: MARBLE is the first purely Mamba-based multi-state MIL framework for WSI analysis that processes multiple magnification levels in parallel using linear-time state-space models, achieving significant performance improvements over existing methods.


<details>
  <summary>Details</summary>
Motivation: WSI analysis is challenging due to gigapixel resolutions and hierarchical magnifications. Existing MIL methods typically operate at a single scale, and transformer-based approaches suffer from quadratic attention costs, creating a need for scalable, efficient multi-scale processing.

Method: MARBLE uses a purely Mamba-based architecture with parallel multi-scale processing and integrates coarse-to-fine reasoning within a linear-time state-space model. It captures cross-scale dependencies efficiently with minimal parameter overhead by coupling parallel multi-scale processing with linear-time sequence modeling.

Result: Experiments on five public datasets show improvements of up to 6.9% in AUC, 20.3% in accuracy, and 2.3% in C-index, demonstrating superior performance compared to existing methods.

Conclusion: MARBLE establishes an efficient and generalizable framework for multi-scale WSI analysis, providing a scalable and modular alternative to attention-based architectures with significant performance gains across multiple metrics.
...
Method: Proposes SRA-Seg framework with similarity-alignment loss using frozen DINOv2 embeddings to align synthetic and real features, soft edge blending for smooth anatomical transitions, pseudo-labeling via EMA teacher model, and soft-segmentation losses for uncertainty handling.
...
Conclusion: SRA-Seg effectively bridges the synthetic-real domain gap for medical image segmentation, enabling effective use of synthetic data with minimal labeled real data while achieving state-of-the-art performance.
...
TL;DR: Nüwa is a two-stage token pruning framework for Vision Language Models that maintains spatial integrity to improve performance on both VQA and visual grounding tasks.


<details>
  <summary>Details</summary>
Motivation: Existing token pruning methods for VLMs work well for VQA but degrade substantially on visual grounding tasks because they lose global spatial reference frames derived from token positional interactions.

Method: Two-stage framework: 1) After vision encoder: separation, alignment, and aggregation operations inspired by swarm intelligence to retain information-rich global spatial anchors; 2) Within LLM: text-guided pruning to retain task-relevant visual tokens.

Result: Achieves SOTA on multiple VQA benchmarks (94% to 95%) and substantial improvements on visual grounding tasks (7% to 47%).

Conclusion: Nüwa enables efficient feature aggregation while maintaining spatial integrity, addressing limitations of existing pruning methods that fail on visual grounding tasks.
...
TL;DR: TRACE is the first vision-language model that jointly performs temporal comparison, change classification, and spatial localization for chest X-rays, achieving over 90% grounding accuracy.


<details>
  <summary>Details</summary>
Motivation: Temporal comparison of chest X-rays is crucial for clinical radiology to detect disease progression, treatment response, and new findings, but no existing method combines temporal change detection with spatial localization.

Method: TRACE (Temporal Radiology with Anatomical Change Explanation) takes prior and current chest X-rays as input and jointly learns temporal comparison, change classification (worsened/improved/stable), and spatial grounding with bounding box coordinates.
...
Conclusion: TRACE establishes a foundation for temporal change detection in radiology and reveals that spatial grounding is essential for meaningful temporal reasoning, enabling both natural language descriptions and spatial localization of changes.
...
TL;DR: Proposes Dynamic High-Frequency Convolution (DHiF) for infrared small target detection by generating dynamic local filter banks that adaptively process high-frequency components to distinguish targets from clutter.
...
Method: Dynamic High-Frequency Convolution (DHiF) translates discriminative modeling into generation of dynamic local filter banks. DHiF is sensitive to HFCs due to dynamic parameters being symmetrically adjusted within zero-centered range based on Fourier transformation properties. It works as a drop-in replacement for standard convolution in any SIRST detection network.

Result: Extensive experiments on real-scene datasets show DHiF exhibits superior detection performance compared to other state-of-the-art convolution operations, with promising improvement across different SIRST detection networks.

Conclusion: DHiF effectively addresses the challenge of distinguishing infrared small targets from other high-frequency components through adaptive dynamic processing, achieving better performance while maintaining computational efficiency as a plug-and-play module.
...
TL;DR: Analytical error expressions for fisheye stereo vision depth and range accuracy, focusing on large-angle performance.


<details>
  <summary>Details</summary>
Motivation: Fisheye stereo vision systems have unique error characteristics at large viewing angles that need mathematical modeling for accurate depth estimation.

Method: Derivation of analytical expressions for depth and range error as functions of object distance, specifically accounting for accuracy degradation at large angles.

Result: Closed-form error models that quantify how depth and range estimation accuracy varies with object distance in fisheye stereo systems.

Conclusion: The derived analytical expressions provide a mathematical foundation for understanding and predicting fisheye stereo vision system accuracy, particularly important for applications requiring wide field-of-view depth sensing.
...
<details>
  <summary>Details</summary>
Motivation: To enable adaptive Mixed Reality content based on users' physical spaces, there's a need to generate 3D scenes that reflect real-world layouts by capturing semantic cues of surroundings. Previous methods struggled with capturing contextual relationships between objects or focused mainly on shape synthesis, making it hard to generate 3D scenes aligned with object arrangements.

Method: Designs a graph network with cross-check feature attention for scene graph prediction, and constructs a graph-variational autoencoder (graph-VAE) with joint shape and layout blocks for 3D scene generation.
...
Conclusion: Enables users to generate consistent 3D spaces from their physical environments via scene graphs, allowing creation of spatial Mixed Reality content.
...
TL;DR: CAFT is a hierarchical vision-language model that aligns global and local semantics across images and long captions without pixel-level supervision, achieving SOTA on long-text retrieval.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models like CLIP struggle with long captions because they treat images and texts as undifferentiated wholes. There's a need for hierarchical semantics that capture both global context and localized details across visual and textual domains, but existing approaches have mismatches between linguistic and visual hierarchies.

Method: CAFT couples a fine-to-coarse visual encoder with a hierarchical text transformer, using a hierarchical alignment loss that matches whole images with whole captions while biasing region-sentence correspondences. This allows coarse semantics to be built from fine-grained evidence rather than from aggregation untethered to part-level grounding.

Result: Trained on 30M image-text pairs, CAFT achieves state-of-the-art performance on six long-text retrieval benchmarks and exhibits strong scaling behavior. The model enables fine-grained, visually grounded image-text representations to emerge without explicit region-level supervision.

Conclusion: Hierarchical cross-domain alignment enables effective fine-grained vision-language understanding for long captions without requiring pixel-level supervision, addressing limitations of existing models that treat images and texts as undifferentiated wholes.
...
TL;DR: SharpTimeGS is a lifespan-aware 4D Gaussian framework for dynamic scene reconstruction that balances static and dynamic regions through learnable lifespan parameters and adaptive densification.


<details>
  <summary>Details</summary>
Motivation: Existing Gaussian-based methods struggle to balance long-term static and short-term dynamic regions in both representation and optimization for novel view synthesis of dynamic scenes.

Method: Introduces learnable lifespan parameters that reformulate temporal visibility from Gaussian decay to flat-top profiles, modulates motion based on lifespan, and uses lifespan-velocity-aware densification to allocate capacity appropriately.
...
Conclusion: SharpTimeGS effectively decouples motion magnitude from temporal duration, improving long-term stability without compromising dynamic fidelity through lifespan-aware modeling.
...
Method: Video-OPD uses on-policy distillation with a frontier teacher providing dense token-level supervision via reverse KL divergence. It includes Teacher-Validated Disagreement Focusing (TVDF) curriculum that prioritizes teacher-reliable and informative trajectories.

Result: Video-OPD consistently outperforms GRPO while achieving substantially faster convergence and lower computational cost across empirical evaluations.

Conclusion: On-policy distillation is an effective alternative to conventional reinforcement learning for Temporal Video Grounding, offering better performance with improved efficiency through dense supervision and intelligent curriculum learning.
...
TL;DR: VOILA is a framework for adaptive fidelity selection in Visual Question Answering that optimizes what visual information to retrieve before model execution, achieving 50-60% cost reduction while maintaining 90-95% of full-resolution accuracy.


<details>
  <summary>Details</summary>
Motivation: Most multimodal vision-language systems operate at fixed fidelity levels despite significant costs from retrieving and processing high-fidelity visual inputs, creating inefficiency in resource-constrained scenarios.
...
Conclusion: Pre-retrieval fidelity selection is vital to optimize multimodal inference under resource constraints, demonstrating that adaptive fidelity selection can dramatically reduce costs while maintaining high accuracy.
...
TL;DR: The paper proposes a method to address information loss in structure and texture feature maps during convolutional downsampling in image inpainting by using statistical normalization and denormalization for reconstruction guidance.


<details>
  <summary>Details</summary>
Motivation: Current image inpainting methods using encoder-decoder CNNs suffer from information loss in both structure and texture feature maps during convolutional downsampling, leading to suboptimal upsampling outputs. The paper aims to investigate whether and how structure and texture feature maps can mutually help alleviate this information loss.

Method: The authors use statistical normalization and denormalization strategy for reconstruction guidance during the convolutional downsampling process. Given structure and texture feature maps, this approach helps preserve information that would otherwise be lost during downsampling.

Result: Extensive experiments show advantages over state-of-the-art methods on images from low-to-high resolutions (256×256 and 512×512). The method is particularly effective when substituting all encoders with their proposed approach.

Conclusion: The proposed statistical normalization and denormalization strategy effectively addresses information loss in structure and texture feature maps during convolutional downsampling, leading to improved image inpainting performance across various resolutions.
...
TL;DR: Automated analysis of NYC congestion pricing impact using 900+ traffic cameras shows systematic changes in vehicle density patterns from Nov 2024 to Jan 2026.


<details>
  <summary>Details</summary>
Motivation: To objectively measure the impact of New York City's congestion pricing program on traffic patterns using data-driven analysis.

Method: Computer vision pipeline processing footage from over 900 traffic cameras throughout Manhattan and NYC, comparing traffic patterns from November 2024 (pre-implementation) through January 2025 (implementation) to January 2026.
...
Conclusion: Automated traffic camera analysis provides objective evidence of how congestion pricing systematically alters urban traffic density patterns.
...
TL;DR: MUSE is a multi-agent framework for generating coherent long-form audio-visual stories from short prompts using iterative plan-execute-verify-revise loops with explicit narrative controls.


<details>
  <summary>Details</summary>
Motivation: Existing approaches for generating long-form audio-visual stories suffer from semantic drift and identity inconsistency due to the intent-execution gap, where high-level narrative intent must be preserved across coherent shot-level multimodal generation over long horizons.
...
Result: MUSE substantially improves long-horizon narrative coherence, cross-modal identity consistency, and cinematic quality compared with representative baselines, as evaluated through MUSEBench, a reference-free evaluation protocol validated by human judgments.

Conclusion: The closed-loop multi-agent approach with explicit narrative controls effectively addresses the intent-execution gap in long-form audio-visual storytelling, providing a robust framework for generating coherent multimodal narratives from short prompts.
...
<details>
  <summary>Details</summary>
Motivation: While VLMs excel at everyday visual tasks, they struggle with the novel visual reasoning required by Bongard problems, which test human-like ability to reason in radically new situations.

Method: Neurosymbolic approach: 1) Use LLMs to generate parameterized programmatic representations of hypothesized solution rules, 2) Perform parameter fitting using Bayesian optimization.

Result: The method is evaluated on two tasks: classifying Bongard problem images given ground truth rules, and solving problems from scratch without rule knowledge.

Conclusion: The neurosymbolic approach combining LLMs and Bayesian optimization shows promise for solving challenging visual reasoning problems like Bongard problems.
...
TL;DR: HP-GAN improves GAN image synthesis by using pretrained networks with self-supervised learning and discriminator consistency, achieving better FID scores across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: While GANs have advanced image synthesis quality and pretrained networks are commonly used for perceptual losses, there's potential to better exploit neural network priors through more sophisticated self-supervised techniques and improved discriminator coordination.

Method: HP-GAN introduces two key strategies: 1) FakeTwins - uses pretrained networks as encoders to compute self-supervised loss applied through generated images to train the generator, and 2) Discriminator consistency - enforces alignment between discriminators evaluating CNN and ViT feature maps to promote coherent learning and training robustness.

Result: Extensive evaluation across 17 datasets (large, small, limited data, various domains) shows HP-GAN consistently outperforms state-of-the-art methods in FID scores, with significant improvements in image diversity and quality.

Conclusion: HP-GAN effectively leverages neural network priors through innovative self-supervised learning and discriminator consistency mechanisms, demonstrating superior performance in GAN-based image synthesis across diverse data scenarios.
...
TL;DR: IVC-Prune: A training-free pruning method for Large Vision-Language Models that reduces visual tokens by ~50% while maintaining ≥99% performance by preserving both implicit visual coordinate tokens and semantically relevant foreground tokens.


<details>
  <summary>Details</summary>
Motivation: LVLMs have prohibitive inference costs when processing high-resolution visual inputs. Existing visual token pruning methods focus on semantic relevance but discard tokens crucial for spatial reasoning, creating a performance gap.

Method: IVC-Prune identifies two types of crucial tokens: 1) Implicit Visual Coordinate (IVC) tokens found by analyzing RoPE's mathematical properties (positions where rotation matrices approximate identity or 90° rotation), and 2) Foreground tokens identified through semantic seed discovery followed by contextual refinement via value-vector similarity.

Result: Extensive evaluations across 4 LVLMs and 20 benchmarks show IVC-Prune reduces visual tokens by approximately 50% while maintaining ≥99% of original performance, with improvements on several benchmarks.

Conclusion: IVC-Prune effectively addresses the inference cost problem in LVLMs by preserving both spatial reasoning (IVC tokens) and semantic relevance (foreground tokens), offering a training-free solution that maintains performance while significantly reducing computational overhead.
...
TL;DR: JRDB-Pose3D is a new 3D human pose dataset captured from a mobile robot platform in crowded indoor/outdoor scenes, featuring SMPL-based pose annotations with consistent body shapes and track IDs for 5-35 people per frame, addressing real-world challenges like occlusions and truncations.


<details>
  <summary>Details</summary>
Motivation: Real-world scenes are crowded with multiple humans, but existing 3D pose datasets focus on single-person or controlled lab environments, limiting their relevance for applications like autonomous driving, robot perception, navigation, and human-robot interaction.

Method: Introduce JRDB-Pose3D dataset captured from a mobile robotic platform in multi-human indoor and outdoor environments, providing SMPL-based 3D pose annotations with consistent body-shape parameters and track IDs for each individual over time.

Result: Dataset contains 5-10 human poses per frame on average, with some scenes featuring up to 35 individuals simultaneously, presenting challenges like frequent occlusions, truncated bodies, and out-of-frame body parts that reflect real-world complexity.

Conclusion: JRDB-Pose3D bridges the gap between controlled lab datasets and real-world applications, providing a holistic dataset with rich annotations (2D pose, social grouping, activities, interactions, semantic masks, demographic info) for comprehensive human-centric understanding tasks.
...
TL;DR: GBO is a novel inference framework for weakly supervised temporal video grounding that optimizes segment boundaries through principled optimization rather than heuristic mappings, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing weakly supervised temporal video grounding methods use Gaussian-based temporal proposals but rely on heuristic mappings from Gaussian parameters to segment boundaries, leading to suboptimal localization performance.
...
Result: GBO significantly improves localization performance, achieving state-of-the-art results across standard benchmarks, and demonstrates efficiency and generalizability across various proposal schemes.

Conclusion: GBO provides a principled optimization-based approach to temporal video grounding that outperforms heuristic methods, offering theoretical foundations and practical advantages for weakly supervised video understanding.
...
<details>
  <summary>Details</summary>
Motivation: Current AI models for musculoskeletal imaging are task-specific, annotation-dependent, and lack generalizability across diseases and anatomical regions. There's a clinical need for a generalizable foundation model, but public datasets are limited in size and diversity.

Method: Developed SKELEX using self-supervised learning on 1.2 million diverse, condition-rich musculoskeletal radiographs. Created an interpretable, region-guided model for bone tumor prediction based on the foundation model's zero-shot localization capability.
...
Conclusion: SKELEX provides a scalable, label-efficient, and generalizable AI framework for musculoskeletal imaging, establishing a foundation for both clinical translation and data-efficient research in musculoskeletal radiology.
...
<details>
  <summary>Details</summary>
Motivation: Current state-of-the-art semantic correspondence methods combine DINOv2 and Stable Diffusion features, which is computationally expensive. DINOv2 produces accurate but sparse correspondences, while SD provides spatial consistency but requires heavy computation.

Method: Replaces SD features with an optimal transport algorithm featuring a Gromov-Wasserstein spatial smoothness prior. This enhances DINOv2's matching capabilities by providing the spatial consistency property that SD offered, without needing SD's heavy feature extraction.
...
Conclusion: Optimal transport with spatial priors can effectively replace computationally expensive foundation model ensembles for semantic correspondence, providing similar spatial consistency benefits at much lower computational cost.
...
<details>
  <summary>Details</summary>
Motivation: Traditional data augmentation methods (like cropping/rotation) have limitations in diversity and realism, while modern generative models (diffusion, NeRFs) offer greater diversity but risk performance degradation if poorly matched to tasks. There's a need for automated methods to find optimal task-specific augmentations.

Method: EvoAug combines generative models (conditional diffusion, few-shot NeRFs) with an efficient evolutionary algorithm to learn stochastic augmentation trees that hierarchically compose augmentations, enabling structured and adaptive transformations.

Result: Strong performance across fine-grained classification and few-shot learning tasks. The pipeline discovers augmentations that align with domain knowledge even in low-data settings.
...
TL;DR: Children and CNNs learn differently in few-shot semi-supervised category learning: children generalize rapidly with minimal labels but show feature biases, while CNNs benefit more from added supervision with performance moderated by alignment and feature structure.


<details>
  <summary>Details</summary>
Motivation: To understand how humans and machines learn from sparse data, comparing children and convolutional neural networks under identical conditions in few-shot semi-supervised category learning.

Method: Species-fair design comparing children and CNNs on few-shot semi-supervised category learning with novel object categories. Varied supervision (1/3/6 labels), target features (size, shape, pattern), and perceptual alignment (high/low). Both received mixtures of labeled and unlabeled exemplars.

Result: Children generalize rapidly from minimal labels but show strong feature-specific biases and sensitivity to alignment. CNNs show different interaction profile: added supervision improves performance, but both alignment and feature structure moderate the impact of additional supervision on learning.
...
TL;DR: A diffusion-based framework for 3D human pose estimation that generates multiple plausible poses from 2D images without requiring paired 2D-3D training data.


<details>
  <summary>Details</summary>
Motivation: Traditional 3D pose estimation methods assume deterministic mapping and produce single poses, ignoring the inherent ambiguity in 2D-to-3D projection. Additionally, existing methods require large paired 2D-3D datasets and suffer from generalization issues.

Method: Uses diffusion models with guidance framework: unconditional diffusion model trained only on 3D data, guided by gradients from 2D keypoint detector heatmaps to generate poses consistent with 2D images.

Result: State-of-the-art performance on Human 3.6M under best-of-m evaluation among methods not requiring paired training data. Competitive performance on MPI-INF-3DHP and 3DPW datasets. Framework also enables novel tasks like pose generation and completion.

Conclusion: The diffusion-based approach effectively handles ambiguity in 3D pose estimation, eliminates need for paired 2D-3D training data, and provides flexible framework for multiple pose-related tasks.
...
TL;DR: FinMTM is a multi-turn multimodal benchmark for evaluating VLMs on financial data, featuring 11k+ bilingual QA pairs across diverse chart types and task formats with specialized evaluation protocols.


<details>
  <summary>Details</summary>
Motivation: Existing financial benchmarks are limited - they're mostly single-turn, use narrow question formats, and don't comprehensively evaluate VLMs in realistic financial application scenarios that require specialized chart understanding and knowledge-intensive reasoning.

Method: Created FinMTM benchmark with: 1) 11,133 bilingual Chinese/English QA pairs grounded in financial visuals (candlestick charts, statistical plots, report figures), 2) Diverse task formats including single/multiple-choice, multi-turn dialogues, and agent-based tasks, 3) Task-specific evaluation protocols with specialized scoring rules for each task type.

Result: Evaluation of 22 VLMs revealed significant limitations in: fine-grained visual perception of financial charts, long-context reasoning across multiple turns, and handling complex agent workflows required for realistic financial applications.

Conclusion: FinMTM addresses critical gaps in financial VLM evaluation by providing a comprehensive multi-turn multimodal benchmark that better reflects real-world financial application requirements, revealing current model limitations and setting a foundation for future improvements.
...
TL;DR: SwiftVLM introduces a bypass pruning paradigm that preserves unselected visual tokens for later re-evaluation, avoiding premature information loss in vision-language models.


<details>
  <summary>Details</summary>
Motivation: Existing visual token pruning methods suffer from significant performance degradation on tasks requiring fine-grained visual details due to early pruning decisions that irreversibly discard important tokens that become relevant later in processing.

Method: SwiftVLM uses a bypass pruning paradigm that preserves unselected visual tokens and forwards them to subsequent pruning stages for re-evaluation. It performs pruning at model-specific layers with strong visual token selection capability and enables independent pruning decisions across layers.
...
Conclusion: The bypass paradigm effectively addresses the limitation of premature pruning in vision-language models, enabling better preservation of fine-grained visual details while maintaining computational efficiency.
...
TL;DR: FSOD-VFM is a few-shot object detection framework using vision foundation models with a graph-based confidence reweighting method to address overfragmentation in bounding box proposals.


<details>
  <summary>Details</summary>
Motivation: Vision foundation models have strong generalization but generate fragmented bounding boxes in few-shot object detection, covering only partial object regions and producing many false-positive proposals instead of complete object detections.

Method: Integrates universal proposal network (UPN) for category-agnostic boxes, SAM2 for mask extraction, and DINOv2 features. Introduces graph-based confidence reweighting where bounding boxes are nodes in a directed graph, using graph diffusion to propagate confidence scores, assigning higher confidence to whole objects and lower to fragmented parts.

Result: Substantially outperforms existing approaches on Pascal-5^i, COCO-20^i, and CD-FSOD datasets without additional training. On CD-FSOD 10-shot setting, achieves 31.6 AP vs. previous training-free methods' 21.4 AP.

Conclusion: FSOD-VFM effectively addresses overfragmentation in foundation model-based few-shot object detection through graph-based confidence reweighting, achieving state-of-the-art performance across multiple datasets and domains.
...
TL;DR: DP-DMD is a role-separated distillation framework that prevents mode collapse in diffusion distillation by dedicating the first step to diversity preservation and subsequent steps to quality refinement.


<details>
  <summary>Details</summary>
Motivation: Existing Distribution Matching Distillation (DMD) methods suffer from mode collapse due to their reverse-KL formulation, and current remedies require perceptual/adversarial regularization which adds computational overhead and training instability.

Method: Proposes Diversity-Preserved DMD (DP-DMD) with role-separated distillation: first step uses target-prediction (v-prediction) to preserve diversity, subsequent steps use standard DMD loss for quality refinement, with DMD gradients blocked at the first step.

Result: DP-DMD preserves sample diversity while maintaining visual quality comparable to state-of-the-art methods in text-to-image experiments, without requiring perceptual backbones, discriminators, auxiliary networks, or additional ground-truth images.

Conclusion: DP-DMD offers a simple yet effective solution to mode collapse in diffusion distillation by explicitly separating diversity preservation and quality refinement roles, achieving competitive performance without complex regularization.
...
TL;DR: This paper introduces ALL U-KAN, the first fully Kolmogorov-Arnold (KA) based deep model that completely replaces traditional FC and Conv layers with KA-based layers, overcoming previous limitations of deep KANs through two key innovations: Share-activation KAN (SaKAN) for easier training and Grad-Free Spline for reduced memory usage.


<details>
  <summary>Details</summary>
Motivation: Deeply stacked KANs (Kolmogorov-Arnold Networks) face severe training difficulties and high memory requirements, limiting existing studies to only few KAN layers and preventing comprehensive exploration of KAN architectures in deep learning.

Method: Two main innovations: (1) Share-activation KAN (SaKAN) reformulates Sprecher's variant of Kolmogorov-Arnold theorem for better optimization with simplified parameterization and denser training samples; (2) Grad-Free Spline eliminates spline gradients that contribute negligibly to training but consume huge GPU memory. These enable ALL U-KAN, where KA and KAonv layers completely replace traditional FC and Conv layers.

Result: ALL U-KAN achieves 10x parameter reduction and 20x memory reduction compared to directly deeply stacked KANs. Extensive evaluations on three medical image segmentation tasks show superior segmentation accuracy compared to both partial KA-based and traditional architectures.

Conclusion: The paper successfully demonstrates that KA-based layers can entirely replace traditional architectures in deep learning, achieving superior learning capacity while overcoming previous limitations of deep KANs, unlocking new explorations into deep KAN architectures.
...
<details>
  <summary>Details</summary>
Motivation: To improve group-activity video retrieval performance without requiring expensive group activity annotations, using human feedback to adapt the feature space.

Method: 1) Self-supervised pre-training of group activity feature space based on activity similarity. 2) Interactive fine-tuning where users label selected videos as positive/negative relative to queries. 3) Contrastive learning updates to move positive videos closer and negative videos farther from queries.

Result: Significant improvement in retrieval performance on two team sports datasets, with ablation studies showing contributions of various human-in-the-loop components.

Conclusion: Human-in-the-loop adaptation effectively improves group activity video retrieval without group activity annotations, demonstrating the value of interactive fine-tuning and data-efficient video selection.
...
TL;DR: BinaryDemoire: A binarized image demoiréing framework that uses frequency-aware gating and structured residual adapters to effectively remove moiré artifacts with extreme 1-bit quantization.


<details>
  <summary>Details</summary>
Motivation: Image demoiréing requires removing structured moiré artifacts that are frequency-dependent and vary across scales/directions. While deep networks achieve good restoration, they're computationally expensive for deployment. Binarization offers extreme compression (1-bit quantization) but performs poorly when naively applied to demoiréing due to the complex frequency characteristics of moiré patterns.

Method: 1) Moiré-aware binary gate (MABG): Extracts lightweight frequency descriptors with activation statistics to predict channel-wise gating coefficients for conditioning binary convolution responses. 2) Shuffle-grouped residual adapter (SGRA): Performs structured sparse shortcut alignment with interleaved mixing to promote information exchange across channel partitions.

Result: Extensive experiments on four benchmarks demonstrate that BinaryDemoire surpasses current binarization methods for image demoiréing.

Conclusion: BinaryDemoire effectively addresses the frequency-dependent nature of moiré artifacts in binarized networks, achieving high-quality demoiréing with extreme compression through frequency-aware gating and structured residual adaptation.
...
<details>
  <summary>Details</summary>
Motivation: One-step diffusion models show promise for real-world video super-resolution but suffer from large model size and high computational cost due to Diffusion Transformers. While quantization can help compress models, existing methods struggle with the high dynamic range of input latent and diverse layer behaviors in diffusion models.

Method: LSGQuant uses three key components: 1) Dynamic Range Adaptive Quantizer (DRAQ) to fit video token activations, 2) Variance-Oriented Layer Training Strategy (VOLTS) based on layer sensitivity analysis, and 3) Quantization-Aware Optimization (QAO) to jointly refine quantized and high-precision branches.
...
Conclusion: LSGQuant effectively addresses quantization challenges for diffusion-based VSR models, enabling practical deployment with minimal performance loss while providing significant computational benefits.
...
TL;DR: TempLoc is a LiDAR relocalization framework that improves robustness by modeling sequential consistency across scans through uncertainty-guided coordinate fusion.


<details>
  <summary>Details</summary>
Motivation: Existing regression-based LiDAR relocalization methods are prone to dynamic or ambiguous scenarios because they either rely on single-frame inference or neglect spatio-temporal consistency across scans, leading to reduced robustness.

Method: Three-module framework: 1) Global Coordinate Estimation module predicts point-wise global coordinates and uncertainties for each scan; 2) Prior Coordinate Generation module estimates inter-frame point correspondences using attention mechanism; 3) Uncertainty-Guided Coordinate Fusion module integrates both predictions end-to-end for temporally consistent 6-DoF pose estimation.
...
Conclusion: TempLoc effectively enhances LiDAR relocalization robustness through sequential consistency modeling, with uncertainty-guided coordinate fusion enabling more accurate and temporally consistent pose estimation in dynamic environments.
...
TL;DR: Hand3R is the first online framework for joint 4D hand-scene reconstruction from monocular video, combining hand and scene models for simultaneous reconstruction without offline optimization.


<details>
  <summary>Details</summary>
Motivation: Existing methods reconstruct isolated hands in local coordinates, ignoring the surrounding 3D environment, which is crucial for understanding physical interaction in Embodied AI.

Method: Hand3R synergizes a pre-trained hand expert with a 4D scene foundation model using scene-aware visual prompting, injecting high-fidelity hand priors into a persistent scene memory for single forward pass reconstruction.

Result: The framework bypasses reliance on offline optimization and delivers competitive performance in both local hand reconstruction and global positioning.
...
TL;DR: VIRAL is a visual in-context learning framework that uses a frozen Diffusion Transformer with role-aware multi-image conditioning and Mixture-of-Experts LoRA to handle diverse visual tasks through visual analogy formulation.


<details>
  <summary>Details</summary>
Motivation: Replicating In-Context Learning (ICL) in computer vision is challenging due to task heterogeneity. Current methods struggle with diverse visual tasks, and existing visual context datasets have gaps that need to be addressed.

Method: Formulates ICL as conditional generation via visual analogy (x_s : x_t :: x_q : y_q). Adapts a frozen Diffusion Transformer using role-aware multi-image conditioning and introduces Mixture-of-Experts LoRA to mitigate gradient interference across diverse tasks. Curates a large-scale dataset spanning perception, restoration, and editing.

Result: VIRAL outperforms existing methods and demonstrates that a unified V-ICL paradigm can handle the majority of visual tasks, including open-domain editing.

Conclusion: The VIRAL framework successfully enables visual in-context learning across diverse tasks by leveraging visual analogies and adapted diffusion models, with potential applications in perception, restoration, and editing tasks.
...
TL;DR: ConsisDrive is an identity-preserving driving world model that addresses identity drift in generated driving videos by enforcing temporal consistency at the instance level through instance-masked attention and loss mechanisms.
...
Method: Two key components: (1) Instance-Masked Attention that uses identity and trajectory masks to restrict token interactions to corresponding instances across space and time, and (2) Instance-Masked Loss that adaptively emphasizes foreground regions with probabilistic instance masking while reducing background noise.

Result: Achieves state-of-the-art driving video generation quality and shows significant improvements in downstream autonomous driving tasks on the nuScenes dataset.
...
TL;DR: FARTrack is a fast autoregressive tracking framework that achieves high speed (up to 343 FPS on GPU) while maintaining competitive performance (70.6% AO on GOT-10k) through task-specific self-distillation and inter-frame sparsification.


<details>
  <summary>Details</summary>
Motivation: High-performance visual trackers are often too slow for resource-constrained devices, creating a trade-off between tracking performance and inference speed that needs to be addressed.

Method: FARTrack uses an autoregressive framework with two key techniques: 1) Task-Specific Self-Distillation for layer-by-layer token compression without manual teacher-student assignments, and 2) Inter-frame Autoregressive Sparsification to sequentially condense multiple templates with a temporally-global optimal strategy.

Result: Achieves 70.6% AO on GOT-10k in real-time, with fastest model reaching 343 FPS on GPU and 121 FPS on CPU, demonstrating outstanding speed while maintaining competitive tracking performance.

Conclusion: FARTrack successfully balances speed and performance through autoregressive tracking with innovative distillation and sparsification techniques, making high-quality tracking practical for resource-constrained devices.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches for stylized character generation either rely on under-specified text-only prompting (causing style drift and geometric inconsistency) or use reference-based adapters that require external images at inference (increasing complexity and limiting flexibility).

Method: Proposes PokeFusion Attention - a decoder-level cross-attention mechanism that fuses textual semantics with learned style embeddings directly inside the diffusion decoder. Decouples text and style conditioning at attention level, keeping the pretrained diffusion backbone frozen. Only trains decoder cross-attention layers with a compact style projection module.
...
TL;DR: Spiral RoPE extends standard axial 2D RoPE to enable multi-directional positional encoding in vision transformers by partitioning embedding channels into groups associated with uniformly distributed directions, overcoming the limitation of axis-aligned positional encoding.


<details>
  <summary>Details</summary>
Motivation: Standard axial 2D RoPE in vision transformers restricts positional encoding to horizontal and vertical directions, which hinders modeling of oblique spatial relationships that naturally exist in natural images. This directional constraint is identified as a fundamental limitation.

Method: Spiral RoPE partitions embedding channels into multiple groups associated with uniformly distributed directions. Each group is rotated according to the projection of the patch position onto its corresponding direction, allowing spatial relationships to be encoded beyond just horizontal and vertical axes.

Result: Across vision tasks including classification, segmentation, and generation, Spiral RoPE consistently improves performance. Qualitative analysis shows more concentrated attention activations on semantically relevant objects and better respect for local object boundaries.

Conclusion: Spiral RoPE demonstrates the importance of multi-directional positional encoding in vision transformers, overcoming the limitations of standard axial 2D RoPE and enabling better modeling of natural spatial relationships in images.
...
TL;DR: EventFlash is an efficient multimodal LLM for event-based vision that uses spatiotemporal token sparsification to reduce computational cost while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Current event-based MLLMs use dense image-like processing that ignores the spatiotemporal sparsity of event streams, leading to high computational costs. There's a need for more efficient processing of event data.

Method: 1) Created EventMind dataset with 500k+ instruction sets for curriculum training; 2) Adaptive temporal window aggregation to compress temporal tokens while keeping key cues; 3) Sparse density-guided attention to select informative spatial regions and suppress empty areas.
...
Conclusion: EventFlash serves as an efficient foundation model for event-based vision by effectively leveraging spatiotemporal sparsity to reduce redundancy and accelerate inference.
...
TL;DR: InstaDrive enhances driving video realism by improving instance-level temporal consistency and spatial geometric fidelity in world models for autonomous driving.


<details>
  <summary>Details</summary>
Motivation: Current world models for generating driving videos struggle with maintaining instance-level temporal consistency (preserving object identity over time) and spatial geometric fidelity (accurate positioning and occlusion handling), which are crucial for realistic simulation and downstream autonomous driving tasks.

Method: Proposes InstaDrive with two key components: (1) Instance Flow Guider that extracts and propagates instance features across frames to enforce temporal consistency, and (2) Spatial Geometric Aligner that improves spatial reasoning, ensures precise instance positioning, and explicitly models occlusion hierarchies.

Result: Achieves state-of-the-art video generation quality on the nuScenes dataset and enhances downstream autonomous driving tasks. Also enables rigorous safety evaluation through procedural simulation of rare but safety-critical driving scenarios using CARLA's autopilot across diverse maps and regions.

Conclusion: InstaDrive successfully addresses key limitations of existing world models by incorporating instance-aware mechanisms, resulting in more realistic driving video generation and improved performance on autonomous driving tasks, while also providing a framework for comprehensive safety evaluation.
...
TL;DR: LaVPR introduces a large-scale VPR benchmark with 650K+ natural language descriptions, enabling multi-modal fusion for robustness and cross-modal retrieval for language-based localization.
...
Result: Language descriptions yield consistent gains in visually degraded conditions, especially for smaller backbones. Compact models with language can rival larger vision-only architectures. Cross-modal retrieval with LoRA substantially outperforms standard contrastive methods.

Conclusion: LaVPR enables new localization systems that are resilient to real-world stochasticity and practical for resource-constrained deployment, bridging vision and language for robust place recognition.
...
TL;DR: Hyperbolic representation learning for medical image analysis improves domain generalization across diverse datasets and imaging modalities.


<details>
  <summary>Details</summary>
Motivation: Deep neural networks struggle with robust generalization in medical imaging due to data scarcity, covariate shifts from different devices/protocols, and heterogeneous populations. Euclidean manifolds fail to capture complex hierarchical structures in clinical data.

Method: Proposes hyperbolic representation learning to model complex data characteristics, with an unsupervised domain-invariant hyperbolic cross-branch consistency constraint for improved generalization.

Result: Achieves statistically significant gains across 11 in-distribution datasets and 3 ViT models. Outperforms state-of-the-art Euclidean methods by +2.1% AUC on three domain generalization benchmarks (Fitzpatrick17k, Camelyon17-WILDS, cross-dataset retinal imaging).

Conclusion: Hyperbolic manifolds effectively capture complex hierarchical structures in medical data, enabling better domain generalization across diverse imaging conditions and datasets.
...
TL;DR: Standard geometric metrics fail to predict compositional binding in vision encoders, while functional sensitivity (measured by Jacobian) reliably tracks this capability.


<details>
  <summary>Details</summary>
Motivation: The paper challenges the common assumption that globally well-distributed embeddings are sufficient for robust representations, arguing that global geometry captures "what" elements are present but not "how" they are composed.

Method: Tested 21 vision encoders by comparing standard geometry-based statistics against functional sensitivity (measured by input-output Jacobian) for predicting compositional binding. Provided analytic account showing how objective designs constrain embedding geometry but leave local input-output mapping unconstrained.

Result: Geometry-based statistics showed near-zero correlation with compositional binding, while functional sensitivity reliably tracked this capability. The disparity arises from how training objectives explicitly constrain embedding geometry but leave functional mappings unconstrained.

Conclusion: Global embedding geometry captures only a partial view of representational competence. Functional sensitivity serves as a critical complementary axis for modeling composite structure in representations.
...
TL;DR: A3-TTA is a test-time adaptation framework for image segmentation that uses anchor-guided supervision to create reliable pseudo-labels, addressing issues of error accumulation and catastrophic forgetting in domain shift scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing pseudo-label-based TTA methods rely on heuristic perturbation ensembles that lack distributional grounding, causing unstable training signals, error accumulation, and catastrophic forgetting during adaptation to target domains.

Method: A3-TTA identifies well-predicted target domain images as anchors using class compact density metric, uses them as stable references for pseudo-label generation, and applies semantic consistency and boundary-aware entropy minimization regularization with self-adaptive exponential moving average to mitigate label noise.

Result: A3-TTA improves average Dice scores by 10.40 to 17.68 percentage points over source models, outperforms state-of-the-art TTA methods across medical and natural image segmentation tasks, and excels in continual TTA with strong anti-forgetting ability.

Conclusion: The proposed anchor-guided supervision framework provides a robust solution for test-time adaptation in image segmentation, effectively addressing domain shift challenges while preventing catastrophic forgetting and maintaining stable adaptation performance.
...
TL;DR: LEVIO is a lightweight visual-inertial odometry system optimized for ultra-low-power hardware, achieving 20 FPS with <100 mW power consumption on RISC-V SoCs while maintaining accuracy comparable to state-of-the-art VIO systems.


<details>
  <summary>Details</summary>
Motivation: Current VIO systems are too computationally demanding for resource-constrained hardware like micro-drones and smart glasses, creating a need for efficient, infrastructure-less motion tracking solutions for mobile robotics and AR applications.

Method: LEVIO combines established VIO components (ORB feature tracking, bundle adjustment) with computational efficiency optimizations including parallelization, low memory usage, and hardware-software co-design specifically for embedded microcontrollers and low-power SoCs.

Result: The system achieves real-time 6-DoF tracking at 20 FPS while consuming less than 100 mW on a parallel-processing ultra-low-power RISC-V SoC, with competitive accuracy on public VIO benchmarks.

Conclusion: LEVIO successfully demonstrates that efficient VIO is feasible on resource-constrained hardware through careful algorithmic design and hardware-software co-optimization, offering an open-source solution for ultra-low-power motion tracking applications.
...
TL;DR: FOCUS is a foundation model-driven framework for end-to-end automation of 3D OCT retinal disease diagnosis, achieving expert-level performance across multiple clinical centers.
...
Method: FOCUS uses EfficientNetV2-S for image quality assessment, a fine-tuned Vision Foundation Model for abnormality detection and multi-disease classification, and a unified adaptive aggregation method to integrate 2D slice predictions into 3D patient-level diagnoses.

Result: Achieved high F1 scores: 99.01% for quality assessment, 97.46% for abnormality detection, and 94.39% for patient-level diagnosis. External validation across four centers showed stable performance (90.22%-95.24%). Matched expert performance in human-machine comparisons.

Conclusion: FOCUS automates the image-to-diagnosis pipeline, representing a critical advance toward unmanned ophthalmology with validated blueprint for autonomous screening to enhance population-scale retinal care accessibility and efficiency.
...
Result: Achieves minimum MAE of 0.0094 mm and R exceeding 99%, outperforming other deep learning models.
...
TL;DR: InvLBA: An invisible clean-label backdoor attack method for generative data augmentation using latent perturbation, achieving high attack success rates while maintaining clean accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing pixel-level clean-label backdoor attacks (like COMBAT) perform poorly on generated images from generative data augmentation. The authors observe low attack success rates when applying these methods to generated images, motivating a move from pixel-level to latent feature-level attacks.

Method: InvLBA (Invisible clean-label Backdoor Attack) operates at the latent feature level rather than pixel level. It uses latent perturbation to create invisible backdoors in generated images for generative data augmentation. The method is theoretically proven to guarantee generalization of both clean accuracy and attack success rates.
...
Conclusion: Latent feature-level attacks are more effective than pixel-level attacks for clean-label backdoor attacks in generative data augmentation scenarios. InvLBA demonstrates superior performance while maintaining stealthiness and robustness against defenses.
...
TL;DR: MedSAM-Agent: A multi-step autonomous decision-making framework for medical image segmentation that uses hybrid prompting and two-stage training with process rewards to improve interaction efficiency and performance.


<details>
  <summary>Details</summary>
Motivation: Current MLLM-based approaches for medical segmentation rely on single-turn, rigid interactions and lack process-level supervision, leading to redundant actions and failure to fully exploit interactive tools' dynamic potential.

Method: 1) Reformulates interactive segmentation as multi-step autonomous decision-making; 2) Introduces hybrid prompting for expert-curated trajectory generation; 3) Two-stage training pipeline with multi-turn end-to-end outcome verification and clinical-fidelity process reward design.
...
Conclusion: MedSAM-Agent bridges the gap in current approaches by enabling dynamic, multi-turn interactions with process-level supervision, leading to more efficient and effective medical image segmentation.
...
TL;DR: PWAVEP is a plug-and-play spectral defense that purifies 3D point clouds by removing adversarial outliers and filtering high-frequency noise using graph wavelet transforms.


<details>
  <summary>Details</summary>
Motivation: Current 3D point cloud defenses are cumbersome, requiring invasive model modifications, expensive training, or auxiliary data. There's a need for a non-invasive, plug-and-play defense against spatially imperceptible adversarial attacks.

Method: PWAVEP computes spectral graph wavelet domain saliency and local sparsity scores for each point, then uses a hierarchical strategy: removes highly salient adversarial outliers and applies spectral filtering to moderately salient points by attenuating high-frequency coefficients via graph wavelet transform.
...
Conclusion: PWAVEP provides an effective plug-and-play, non-invasive defense against adversarial attacks on 3D point clouds by leveraging spectral analysis and hierarchical purification in the graph wavelet domain.
...
TL;DR: CompTok is a framework for learning compositional visual tokenizers using token-conditioned diffusion with InfoGAN-style objectives and token swapping for enhanced compositionality.


<details>
  <summary>Details</summary>
Motivation: To create visual tokenizers that produce tokens with better compositionality properties, enabling more meaningful semantic control and editing of generated images through token manipulation.

Method: Uses token-conditioned diffusion decoder with InfoGAN-style objective (recognition model predicts tokens from decoded images). Trains on both original images and token-swapped combinations between images. Applies adversarial flow regularizer to maintain natural image distribution for unpaired swap generations.

Result: Achieves state-of-the-art performance on image class-conditioned generation. Enables high-level semantic editing through token swapping. Improves proposed compositionality metrics and supports SOTA generators for class-conditioned generation.

Conclusion: CompTok successfully creates compositional visual tokenizers that enable better semantic control and editing while maintaining strong generation performance, with proposed metrics to quantify token space compositionality.
...
TL;DR: Tiled Prompts addresses prompt underspecification in diffusion-based super-resolution by generating tile-specific prompts for each latent tile instead of using a single global caption.


<details>
  <summary>Details</summary>
Motivation: Modern super-resolution pipelines use latent tiling for high resolutions, but a single global caption causes prompt underspecification - missing localized details (prompt sparsity) and providing locally irrelevant guidance (prompt misguidance) that gets amplified by classifier-free guidance.
...
Conclusion: Tiled Prompts effectively resolves prompt underspecification in diffusion-based super-resolution by providing localized text guidance, improving results for both image and video super-resolution tasks.
...
TL;DR: Z3D: A zero-shot 3D visual grounding method that localizes objects in 3D scenes from multi-view images without geometric supervision or object priors, achieving SOTA performance on ScanRefer and Nr3D benchmarks.


<details>
  <summary>Details</summary>
Motivation: To address the limitations of prior zero-shot 3DVG methods that suffer from performance degradation, and to enable 3D object localization from multi-view images alone without requiring geometric supervision or object priors.

Method: Introduces Z3D, a universal grounding pipeline that operates on multi-view images with optional camera poses and depth maps. Uses (i) a state-of-the-art zero-shot 3D instance segmentation method for high-quality 3D bounding box proposals, and (ii) advanced reasoning via prompt-based segmentation leveraging modern VLMs.

Result: Extensive experiments on ScanRefer and Nr3D benchmarks demonstrate state-of-the-art performance among zero-shot methods.

Conclusion: Z3D effectively addresses key bottlenecks in zero-shot 3DVG and achieves superior performance without requiring geometric supervision or object priors, making it a flexible and powerful solution for 3D visual grounding from multi-view images.
...
TL;DR: A discrete diffusion framework for handwritten math expression recognition that reframes the task as iterative symbolic refinement rather than sequential generation, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Autoregressive models for HMER suffer from exposure bias and syntactic inconsistency when dealing with diverse symbols and 2D structural layouts. There's a need for a better approach that can handle structural relationships more effectively.

Method: Proposes a discrete diffusion framework that reformulates HMER as iterative symbolic refinement using multi-step remasking. Includes symbol-aware tokenization and Random-Masking Mutual Learning to enhance syntactic alignment and robustness to handwriting diversity.
...
Conclusion: Discrete diffusion provides a new paradigm for structure-aware visual recognition beyond generative modeling, effectively addressing the limitations of autoregressive approaches in HMER.
...
TL;DR: MRA approach addresses voxel sparsity in camera-based 3D semantic scene completion using multi-resolution alignment with scene and instance-level supervision.


<details>
  <summary>Details</summary>
Motivation: Current camera-based 3D SSC methods suffer from voxel sparsity (many empty voxels in autonomous driving scenes), limiting optimization efficiency and performance. They rely only on voxel label supervision.

Method: Proposes Multi-Resolution Alignment (MRA) with three modules: 1) Multi-resolution View Transformer for scene-level alignment, 2) Cubic Semantic Anisotropy for instance-level semantic significance, and 3) Critical Distribution Alignment for instance-level feature consistency across resolutions.

Result: The approach mitigates voxel sparsity in camera-based 3D semantic scene completion, improving optimization efficiency and model performance through auxiliary supervision from multi-resolution feature alignment.

Conclusion: MRA effectively addresses voxel sparsity in camera-based 3D SSC by exploiting multi-resolution alignment at both scene and instance levels, providing better supervision for autonomous driving perception systems.
...
TL;DR: SLIM-Diff: A compact joint diffusion model for generating FCD lesions in epilepsy MRI using a shared-bottleneck U-Net and tunable Lp loss geometry.


<details>
  <summary>Details</summary>
Motivation: Focal cortical dysplasia (FCD) lesions in epilepsy FLAIR MRI are subtle and scarce, making joint image-mask generative modeling prone to instability and memorization issues.

Method: Proposes SLIM-Diff with: (1) single shared-bottleneck U-Net enforcing tight coupling between anatomy and lesion geometry using 2-channel image+mask representation, (2) loss-geometry tuning via tunable Lp objective, comparing x₀-prediction vs ε-prediction with different Lp geometries.

Result: x₀-prediction is consistently strongest for joint synthesis; fractional sub-quadratic penalties (L₁.₅) improve image fidelity while L₂ better preserves lesion mask morphology.

Conclusion: SLIM-Diff provides an effective compact joint diffusion model for FCD lesion generation with optimized loss geometry, addressing instability and memorization issues in scarce lesion data scenarios.
...
TL;DR: DiM is a unified multi-dimensional watermarking framework that models watermark information as payloads of different dimensionalities (1D, 2D, 3D) and shows that dimensional configuration determines watermarking behavior.


<details>
  <summary>Details</summary>
Motivation: Existing deep watermarking methods share similar encoder-decoder architectures but exhibit substantially different functional behaviors, creating a need for a unified framework that explains these differences at the functional level.

Method: DiM formulates watermarking as a dimension-aware mapping problem, modeling watermark information as payloads of different dimensionalities: 1D binary messages, 2D spatial masks, and 3D spatiotemporal structures. The framework shows that same-dimensional mappings preserve payload structure while cross-dimensional mappings enable spatial/spatiotemporal localization.
...
Conclusion: DiM provides a unified framework that explains functional differences in watermarking methods through dimensional configurations, enabling systematic design of watermarking systems with specific capabilities by controlling dimension mappings.
...
TL;DR: C3PO framework reduces hallucinations in multimodal reasoning models through chain-of-thought compression and contrastive preference optimization.


<details>
  <summary>Details</summary>
Motivation: Multimodal reasoning models suffer from hallucinations, and current solutions are insufficient. The authors identify that reasoning mechanisms exacerbate reliance on language priors while overlooking visual inputs, leading to CoTs with reduced visual cues and redundant text tokens.

Method: Proposes C3PO framework with two main components: 1) Chain-of-Thought Compression - selectively filters redundant thinking tokens for compact, signal-efficient CoT representation; 2) Contrastive Preference Optimization - uses reasoning-enhanced preference tuning with AI feedback and multimodal hallucination-inducing mechanism to create informative negative signals for contrastive correction.
...
Conclusion: C3PO effectively addresses hallucination issues in multimodal reasoning models by compressing reasoning traces and using contrastive preference optimization, providing a robust training-based mitigation framework.
...
TL;DR: SRL introduces synergistic representation learning to break the vicious cycle between encoder's sharp attention and decoder's blurry reconstruction in slot-based object-centric models.


<details>
  <summary>Details</summary>
Motivation: Slot-based object-centric learning models suffer from a fundamental conflict: encoder produces sharp, high-frequency attention maps while decoder outputs spatially consistent but blurry reconstructions. This creates a vicious cycle where noisy encoder features force decoder to average, producing blurrier outputs, and blurry reconstructions lack high-frequency details to supervise encoder features.

Method: Synergistic Representation Learning (SRL) establishes a virtuous cycle where encoder and decoder mutually refine each other. Encoder's sharpness helps deblur semantic boundaries in decoder output, while decoder's spatial consistency helps denoise encoder features. Includes warm-up phase with slot regularization to initially allocate distinct entities per slot.
...
Conclusion: The synergistic approach of SRL successfully breaks the vicious cycle in slot-based object-centric learning, enabling mutual refinement between encoder and decoder components for improved performance.
...
TL;DR: UnHype is a framework that uses hypernetworks to enhance LoRA-based machine unlearning in diffusion models, enabling more context-aware and scalable removal of harmful concepts while maintaining overall generative quality.


<details>
  <summary>Details</summary>
Motivation: Address concerns about misuse of large-scale diffusion models for generating harmful content. Current LoRA-based unlearning methods have limitations: poor adaptability to concept semantics, difficulty balancing removal of related concepts while maintaining generalization, and scalability challenges for multi-concept erasure.

Method: Incorporates hypernetworks into single- and multi-concept LoRA training. The architecture can be plugged into Stable Diffusion and modern flow-based text-to-image models. During inference, the hypernetwork dynamically generates adaptive LoRA weights based on CLIP embeddings, enabling context-aware unlearning.

Result: Demonstrates stable training behavior and effective concept control. Evaluated across challenging tasks including object erasure, celebrity erasure, and explicit content removal, showing effectiveness and versatility.

Conclusion: UnHype addresses limitations of existing LoRA-based unlearning methods by providing more adaptable, scalable, and context-aware concept removal while maintaining overall generative capabilities in diffusion models.
...
TL;DR: Socratic-Geo is an autonomous multi-agent framework that dynamically couples data synthesis with model learning to solve the geometric reasoning bottleneck in MLLMs, achieving SOTA results with minimal seed data.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with geometric reasoning due to extreme scarcity of high-quality image-text pairs. Human annotation is expensive, automated methods lack fidelity, and existing approaches are passive or inefficient.

Method: Multi-agent framework with three agents: Teacher generates parameterized Python scripts with reflective feedback (Reflect for solvability, RePI for visual validity); Solver optimizes reasoning through preference learning; Generator learns image generation from accumulated "image-code-instruction" triplets.

Result: Starting from only 108 seed problems, Socratic-Solver achieves 49.11 on six benchmarks using one-quarter of baseline data (surpassing baselines by 2.43 points). Socratic-Generator achieves 42.4% on GenExam, surpassing Seedream-4.0 (39.8%) and approaching Gemini-2.5-Flash-Image (43.1%).

Conclusion: Socratic-Geo demonstrates that autonomous multi-agent interaction can effectively address geometric reasoning bottlenecks in MLLMs by dynamically coupling data synthesis with model learning, achieving state-of-the-art performance with minimal seed data.
...
TL;DR: ConsistentRFT is a framework that reduces visual hallucinations in reinforcement fine-tuned flow-based models by addressing exploration-exploitation issues through dynamic granularity rollouts and consistent policy gradient optimization.


<details>
  <summary>Details</summary>
Motivation: RFT on flow-based models often causes visual hallucinations (over-optimized details, semantic misalignment), but the underlying causes are not well understood. The paper aims to investigate why these hallucinations occur and develop methods to reduce them.

Method: 1) Unified analysis of RFT methods revealing two core problems: limited exploration during SDE rollouts (over-emphasis on local details) and trajectory imitation distorting the model's vector field. 2) Proposed ConsistentRFT framework with: Dynamic Granularity Rollout (DGR) to balance global semantics vs local details via dynamic noise scheduling, and Consistent Policy Gradient Optimization (CPGO) to preserve model consistency by aligning current policy with stable prior.

Result: ConsistentRFT reduces visual hallucinations by 49% for low-level and 38% for high-level perceptual hallucinations. It outperforms other RFT methods on out-of-domain metrics, showing 5.1% improvement (vs baseline's -0.4% decrease) over FLUX1.dev.

Conclusion: The paper identifies exploration-exploitation issues as root causes of visual hallucinations in RFT, proposes ConsistentRFT as an effective solution, and demonstrates significant improvements in reducing hallucinations while maintaining out-of-domain performance.
...
TL;DR: CAG framework improves multi-subject image generation by providing hierarchical guidance from concepts to appearances, addressing identity inconsistency and compositional control issues.
...
Method: Hierarchical Concept-to-Appearance Guidance (CAG) with two key components: 1) VAE dropout training at conceptual level to encourage reliance on robust semantic signals from Visual Language Model, 2) Correspondence-aware masked attention module in Diffusion Transformer that restricts text tokens to attend only to matched reference regions.
...
Conclusion: The proposed hierarchical guidance framework effectively addresses identity preservation and compositional control in multi-subject image generation through explicit structured supervision from concepts to appearances.
...
TL;DR: CoViP is a framework for contextualized visual personalization that enables VLMs to generate personalized responses by associating new images with users' accumulated visual-textual context, addressing limitations in current VLMs.


<details>
  <summary>Details</summary>
Motivation: Current vision-language models fail to generate personalized responses based on users' specific experiences because they lack the ability to associate visual inputs with users' accumulated visual-textual context. The paper formalizes this as "contextualized visual personalization."

Method: Proposes CoViP framework that treats personalized image captioning as a core task, improves capability through reinforcement-learning-based post-training and caption-augmented generation, and introduces diagnostic evaluations to rule out textual shortcuts.

Result: Extensive experiments show existing VLMs have substantial limitations, while CoViP improves personalized image captioning and yields holistic gains across downstream personalization tasks.

Conclusion: CoViP represents a crucial stage for enabling robust and generalizable contextualized visual personalization, addressing key limitations in current vision-language models.
...
TL;DR: InlierQ is a post-training quantization method for object detection that separates task-irrelevant anomalies from informative inliers using gradient-aware saliency scores and EM algorithm, achieving reduced quantization error with minimal calibration data.
...
Conclusion: InlierQ effectively addresses the challenge of distinguishing anomalies from informative features during quantization, enabling more efficient object detection deployment with minimal calibration requirements.
...
TL;DR: DiSCo framework disentangles table structure from content for better LVLM adaptation, combined with Table-GLS for structured reasoning, achieving strong performance without expensive training or external tools.
...
Method: Two-part framework: 1) DiSCo (Disentangled Structure-Content alignment) separates structural abstraction from semantic grounding during multimodal alignment. 2) Table-GLS (Global-to-Local Structure-guided reasoning) performs table reasoning via structured exploration and evidence-grounded inference.

Result: Extensive experiments across diverse benchmarks show the framework efficiently enhances LVLM's table understanding and reasoning capabilities, with particular strength in generalizing to unseen table structures.

Conclusion: The proposed framework enables effective table reasoning for LVLMs with minimal annotation and no external tools, addressing the challenge of complex table layouts through disentangled structure-content alignment and structured reasoning.
...
TL;DR: The paper introduces a dynamic text conditioning framework for DiT-based text-to-image models that uses lightweight gates to fuse multi-layer LLM hidden states based on diffusion time and network depth, finding depth-wise semantic routing superior to time-wise fusion.


<details>
  <summary>Details</summary>
Motivation: Current DiT models use static text conditioning with only single LLM layers, ignoring the semantic hierarchy across LLM layers and the dynamic nature of denoising processes over diffusion time and network depth.

Method: A unified normalized convex fusion framework with lightweight gates to organize multi-layer LLM hidden states through time-wise, depth-wise, and joint fusion strategies.
...
Conclusion: Depth-wise routing is established as a strong baseline for dynamic text conditioning, highlighting the need for trajectory-aware signals to enable robust time-dependent conditioning in diffusion models.
...
TL;DR: LogiCls: A vision-language framework for Logical Anomaly Classification (LAC) that detects and classifies logical constraint violations in industrial images using decomposed subqueries and chain-of-thought supervision.


<details>
  <summary>Details</summary>
Motivation: Current anomaly detection methods treat anomalies as binary decisions without indicating which logical rules are broken, offering limited value for quality assurance in industrial settings where understanding specific constraint violations is crucial.

Method: Proposes LogiCls framework that decomposes complex logical constraints into verifiable subqueries, uses data-centric instruction synthesis with chain-of-thought supervision, couples precise grounding annotations with diverse image-text augmentations, and employs difficulty-aware resampling to emphasize challenging subqueries and long-tail constraint types.

Result: Extensive experiments demonstrate that LogiCls delivers robust, interpretable, and accurate industrial logical anomaly classification, providing both predicted violation categories and their evidence trails.

Conclusion: The paper introduces Logical Anomaly Classification (LAC) as a unified task for anomaly detection and violation classification, and presents LogiCls as an effective vision-language framework that enables interpretable, fine-grained logical constraint violation analysis in industrial quality assurance.
...
TL;DR: Unified 3D framework combining autoregression for understanding and diffusion for generation, achieving SOTA across benchmarks with minimal performance degradation.


<details>
  <summary>Details</summary>
Motivation: Existing unified 3D frameworks using autoregressive paradigms suffer from performance degradation due to forced signal quantization and high training costs. The authors aim to create a unified framework that enables effective information interaction between 3D understanding and generation while preserving the strengths of individual models.

Method: Combines autoregressive next-token prediction for 3D understanding with continuous diffusion for 3D generation. Uses a lightweight transformer to bridge feature spaces between large language models and 3D diffusion models, enabling cross-modal information exchange while leveraging pretrained models.

Result: Achieves state-of-the-art performance across diverse 3D understanding and generation benchmarks, and excels in 3D editing tasks. Demonstrates that the AR+diffusion combination is effective for unified 3D intelligence.

Conclusion: The unified AR+diffusion framework represents a promising direction for building general-purpose 3D intelligence, enabling effective information interaction between understanding and generation while minimizing performance degradation and training costs.
...
TL;DR: CDGS is a budget-constrained dynamic Gaussian splatting framework that enforces strict Gaussian count limits during training using a differentiable budget controller and adaptive allocation, achieving optimal rendering quality under hardware constraints.


<details>
  <summary>Details</summary>
Motivation: Dynamic Gaussian Splatting faces a deployment dilemma: unconstrained densification causes excessive memory usage incompatible with edge devices, while heuristic pruning fails to achieve optimal rendering quality under preset Gaussian budgets.

Method: Proposes Constrained Dynamic Gaussian Splatting (CDGS) with: 1) Differentiable budget controller guided by multi-modal unified importance score (geometric, motion, perceptual cues), 2) Decoupled optimization of static/dynamic elements with adaptive capacity allocation, 3) Three-phase training strategy for constraint integration, 4) Dual-mode hybrid compression scheme.
...
Conclusion: CDGS successfully resolves the deployment dilemma of dynamic Gaussian splatting by formulating it as a budget-constrained optimization problem, enabling high-quality 4D reconstruction within strict hardware limitations through intelligent capacity regulation and allocation.
...
TL;DR: The paper investigates four inter-image data augmentation strategies (CutMix, CarveMix, ObjectAug, AnatoMix) for multi-organ segmentation with limited data, finding that CutMix, CarveMix, and AnatoMix significantly improve segmentation performance over baseline nnUNet.


<details>
  <summary>Details</summary>
Motivation: Deep learning segmentation models require large annotated datasets, but clinical data is often scarce. Traditional data augmentation uses basic intra-image operations, while inter-image augmentation strategies that create new images from separate individuals are not well explored for multi-organ segmentation.

Method: The paper investigates four inter-image data augmentation strategies: CutMix, CarveMix, ObjectAug, and AnatoMix. These methods are tested on two organ segmentation datasets and compared against state-of-the-art nnUNet without data augmentation.

Result: CutMix, CarveMix, and AnatoMix improved average dice scores by 4.9, 2.0, and 1.9 respectively compared to nnUNet without data augmentation. Performance further improved when combined with traditional data augmentation. CutMix proved to be a robust and simple strategy even when producing intuitively 'wrong' images.

Conclusion: Inter-image data augmentation strategies, particularly CutMix, are effective for improving multi-organ segmentation performance with limited data. CutMix is highlighted as a robust and simple approach that works well even when generating unconventional image combinations.
...
TL;DR: ELIQ is a label-free framework for assessing quality of AI-generated images that automatically constructs training pairs and adapts pre-trained models to handle evolving generative models without human annotations.


<details>
  <summary>Details</summary>
Motivation: Generative text-to-image models are advancing rapidly, making previously collected quality labels unreliable for newer generations. There's a need for scalable, label-free quality assessment that can adapt to continuously evolving AI models.

Method: ELIQ automatically constructs positive and aspect-specific negative pairs covering conventional and AIGC-specific distortions. It adapts a pre-trained multimodal model via instruction tuning and uses lightweight gated fusion with a Quality Query Transformer to predict two-dimensional quality (visual quality and prompt-image alignment).

Result: ELIQ consistently outperforms existing label-free methods across multiple benchmarks, generalizes from AI-generated content to user-generated content scenarios without modification, and enables scalable quality assessment for evolving generative models.

Conclusion: ELIQ provides an effective label-free framework for quality assessment of AI-generated images that can adapt to rapidly evolving generative models, paving the way for scalable quality evaluation without human annotations.
...
TL;DR: SlowFocus mechanism enhances video LLMs by combining local high-frequency sampling with global low-frequency context for fine-grained video understanding.


<details>
  <summary>Details</summary>
Motivation: Current video LLMs struggle to simultaneously maintain high-quality frame-level semantics and comprehensive video-level temporal information, limiting fine-grained video understanding capabilities.

Method: Introduces SlowFocus mechanism that: 1) identifies query-related temporal segments, 2) performs dense sampling on these segments for local high-frequency features, 3) uses multi-frequency mixing attention to aggregate local details with global contexts, and 4) implements training strategies for temporal grounding and reasoning.

Result: Demonstrates superiority across existing public video understanding benchmarks and the proposed FineAction-CGR benchmark for fine-grained temporal understanding.

Conclusion: SlowFocus effectively addresses the trade-off between frame-level detail and video-level temporal coverage, advancing video LLMs toward fine-grained video understanding through enhanced equivalent sampling frequency without compromising visual token quality.
...
TL;DR: DeepTopo-Net is a novel framework for underwater camouflaged object detection that integrates topology-aware modeling with frequency-decoupled perception to address challenges like topological fragmentation of slender creatures and subtle feature extraction of transparent organisms.


<details>
  <summary>Details</summary>
Motivation: Underwater Camouflaged Object Detection (UCOD) faces extreme challenges due to visual similarity between targets and backgrounds across marine depths. Existing methods struggle with topological fragmentation of slender deep-sea creatures and subtle feature extraction of transparent organisms.

Method: Proposes DeepTopo-Net framework with two key components: 1) Water-Conditioned Adaptive Perceptor (WCAP) using Riemannian metric tensors to dynamically deform convolutional sampling fields for physical degradation, 2) Abyssal-Topology Refinement Module (ATRM) to maintain structural connectivity of spindly targets through skeletal priors. Also introduces GBU-UCOD, the first high-resolution (2K) benchmark for marine vertical zonation.

Result: Extensive experiments on MAS3K, RMAS, and the proposed GBU-UCOD datasets demonstrate state-of-the-art performance, particularly in preserving morphological integrity of complex underwater patterns. The method effectively addresses challenges in hadal and abyssal zones.

Conclusion: DeepTopo-Net successfully integrates topology-aware modeling with frequency-decoupled perception to overcome key challenges in UCOD, achieving superior performance in detecting camouflaged objects across varying marine depths while preserving structural integrity of complex underwater organisms.
...
TL;DR: The paper proposes TIPSomaly, a zero-shot anomaly detection method using TIPS vision-language model instead of CLIP, addressing spatial misalignment and fine-grained sensitivity issues through decoupled prompts and local-global feature integration.


<details>
  <summary>Details</summary>
Motivation: Zero-shot anomaly detection using CLIP has limitations: spatial misalignment between image and text features, and weak sensitivity to fine-grained anomalies. Prior work adds complex modules but overlooks backbone choice. TIPS VLM addresses these issues but creates a distributional gap between global and local features.

Method: Uses TIPS VLM as backbone instead of CLIP. Introduces decoupled prompts: fixed prompts for image-level detection and learnable prompts for pixel-level localization. Injects local evidence into global anomaly score to bridge the distribution gap between global and local features.

Result: Improves image-level performance by 1.1-3.9% and pixel-level by 1.5-6.9% across seven industrial datasets. Achieves strong generalization with lean architecture, outperforming CLIP-based methods without CLIP-specific tricks.

Conclusion: TIPS VLM is a better backbone than CLIP for zero-shot anomaly detection. The proposed TIPSomaly pipeline with decoupled prompts and local-global integration effectively addresses spatial alignment and fine-grained sensitivity issues, delivering superior performance with simpler architecture.
...
TL;DR: Refer-Agent: A multi-agent system with reasoning-reflection mechanisms for zero-shot referring video object segmentation, outperforming both SFT-based and existing zero-shot methods without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current RVOS methods rely heavily on supervised fine-tuning of MLLMs, which is data-dependent and not scalable with rapidly evolving MLLMs. Zero-shot approaches are more flexible but underperform due to simplistic workflow designs.

Method: Proposes Refer-Agent, a collaborative multi-agent system with alternating reasoning-reflection mechanisms. Features: 1) Coarse-to-Fine frame selection for diversity and textual relevance, 2) Dynamic Focus Layout for adaptive visual focus adjustment, 3) Chain-of-Reflection mechanism using Questioner-Responder pair to verify intermediate results and generate feedback for refinement.

Result: Extensive experiments on five challenging benchmarks show Refer-Agent significantly outperforms state-of-the-art methods, including both SFT-based models and zero-shot approaches. The system is flexible and enables fast integration of new MLLMs without additional fine-tuning costs.

Conclusion: Refer-Agent provides an effective zero-shot solution for RVOS that overcomes limitations of both SFT-based and existing zero-shot methods through collaborative multi-agent reasoning with reflection mechanisms, achieving superior performance while maintaining flexibility and scalability.
...
TL;DR: EB-JEPA is an open-source library for learning representations and world models using Joint-Embedding Predictive Architectures that predict in representation space rather than pixel space, with modular implementations for images, video, and action-conditioned world models.


<details>
  <summary>Details</summary>
Motivation: To make energy-based self-supervised learning accessible for research and education by providing modular, self-contained implementations that demonstrate how representation learning techniques can transfer from images to video and action-conditioned world models, with practical single-GPU training within hours.

Method: Joint-Embedding Predictive Architectures (JEPAs) that learn to predict in representation space rather than pixel space, avoiding generative modeling pitfalls. The library provides implementations for image-level self-supervised learning on CIFAR-10, temporal modeling on Moving MNIST, and action-conditioned world models for navigation tasks.

Result: Achieved 91% accuracy on CIFAR-10 probing, demonstrated multi-step prediction on Moving MNIST, and achieved 97% planning success rate on Two Rooms navigation task. Comprehensive ablations revealed critical importance of each regularization component for preventing representation collapse.

Conclusion: EB-JEPA successfully demonstrates that JEPA-based representation learning techniques can effectively scale from images to video and action-conditioned world models, making energy-based self-supervised learning practical and accessible for research and education with strong performance across different domains.
...
TL;DR: KTV is a two-stage training-free video understanding framework that first selects keyframes via clustering to reduce redundancy, then prunes less important visual tokens to minimize computational overhead while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing training-free video understanding methods suffer from severe visual redundancy and high computational overhead when processing long videos. Current keyframe selection strategies (especially CLIP-based ones) are prone to biases and may overlook critical frames, leading to suboptimal video comprehension.

Method: KTV uses a two-stage approach: 1) Question-agnostic keyframe selection by clustering frame-level visual features to get a compact, diverse, and representative subset of frames; 2) Key visual token selection that prunes redundant or less informative tokens from each selected keyframe based on token importance and redundancy analysis.

Result: KTV outperforms state-of-the-art training-free baselines on Multiple-Choice VideoQA tasks while using significantly fewer visual tokens (only 504 tokens for a 60-min video with 10,800 frames). Achieves 44.8% accuracy on MLVU-Test benchmark and even exceeds several training-based approaches on certain benchmarks.

Conclusion: KTV provides an efficient and effective training-free video understanding framework that addresses visual redundancy and computational overhead issues through intelligent keyframe selection and token pruning, achieving competitive performance with minimal computational resources.
...
TL;DR: A unified multimodal framework for retinal disease classification using synthesized FFA, MSI, and saliency maps with adaptive feature calibration and cross-modal integration.


<details>
  <summary>Details</summary>
Motivation: Retinal disease diagnosis benefits from multimodal data but faces challenges like data heterogeneity, invasiveness, and registration complexity. Current approaches lack unified frameworks for multimodal synthesis and fusion.

Method: 1) Synthesize multimodal data (FFA, MSI, saliency maps highlighting lesions/optic regions). 2) Train parallel models for modality-specific representations. 3) Adaptively calibrate features within/across modalities for information pruning and flexible integration. 4) Provide visual interpretations in image/feature spaces.

Result: Superior performance on two public datasets: multi-label classification (F1-score: 0.683, AUC: 0.953) and diabetic retinopathy grading (Accuracy: 0.842, Kappa: 0.861), outperforming state-of-the-art methods.

Conclusion: The framework enhances retinal disease screening accuracy/efficiency and provides a scalable approach for medical imaging data augmentation across modalities.
...
<details>
  <summary>Details</summary>
Motivation: Semantic segmentation requires expensive pixel-level annotations. Synthetic data from graphics engines is cheap but suffers from domain gap. Style transfer can bridge this gap, but designing effective transformation pipelines is challenging due to large combinatorial search space.

Method: Use multi-objective genetic algorithms to optimize style transfer pipelines. Employ paired-image metrics on individual samples during evolution for rapid evaluation, instead of distributional metrics requiring many images. Evaluate Pareto front using distributional metrics and segmentation performance on GTA5→Cityscapes/ACDC domain adaptation.

Result: Evolutionary algorithms can propose diverse augmentation pipelines adapted to different objectives. The approach demonstrates effective synthetic-to-real domain adaptation, particularly for adverse conditions in Cityscapes and ACDC datasets.

Conclusion: Formulates style transfer as a sequencing problem suitable for evolutionary optimization. Introduces efficient metrics enabling feasible search in large combinatorial space. Provides practical solution for synthetic-to-real domain adaptation in semantic segmentation.
...
TL;DR: First sparse partial weakly-supervised oriented object detection framework that uses few sparse weakly-labeled data and plenty of unlabeled data for remote sensing applications.


<details>
  <summary>Details</summary>
Motivation: High labeling costs in remote sensing due to dense object distribution and wide category variety; need for methods that work with fewer and weaker annotations.

Method: Three innovations: 1) SOS-Student model for separating unlabeled objects from background and learning from weak annotations, 2) Multi-level Pseudo-label Filtering using model prediction distributions, 3) Sparse partitioning approach for equal category treatment.

Result: Significant performance gain over traditional oriented object detection methods on DOTA and DIOR datasets, offering cost-effective solution.

Conclusion: Proposed SPWOOD framework effectively addresses large-scale labeling challenges in remote sensing with sparse weak supervision and unlabeled data.
...
TL;DR: MM-SCALE is a large-scale multimodal dataset with 5-point scalar ratings and explicit modality grounding for aligning VLMs with human moral preferences through listwise preference optimization.


<details>
  <summary>Details</summary>
Motivation: Current VLMs struggle with morally salient judgments in multimodal contexts, and prior works use binary/pairwise supervision that fails to capture the continuous, pluralistic nature of human moral reasoning.

Method: Created MM-SCALE dataset with image-scenario pairs annotated with moral acceptability scores and grounded reasoning labels using a tailored interface. Enables listwise preference optimization over ranked scenario sets.
...
Conclusion: Moving from discrete to scalar supervision provides richer alignment signals and finer calibration of multimodal moral reasoning, improving VLM performance in morally ambiguous contexts.
...
TL;DR: A novel sequential neural network with spatial-temporal attention mechanism for lane detection that outperforms state-of-the-art methods while being computationally efficient.


<details>
  <summary>Details</summary>
Motivation: Current lane detection methods lack versatility in delivering accurate, robust, and real-time performance, especially in challenging scenarios like occlusion and lighting issues. Vision-based methods often neglect critical image regions and their spatial-temporal salience.
...
Result: Outperforms state-of-the-art methods across three large-scale datasets in various testing scenarios. The model exhibits fewer parameters and reduced MACs compared to baseline sequential models, demonstrating computational efficiency.

Conclusion: The spatial-temporal attention mechanism enables robust, accurate lane detection in challenging conditions while maintaining computational efficiency, making it suitable for real-time applications in automated vehicles and ADAS systems.
...
TL;DR: RIAS introduces a language-guided anomaly detection paradigm that uses text descriptions to generate precise anomaly masks without manual thresholds, enabling detection of diverse anomalies with a single model.


<details>
  <summary>Details</summary>
Motivation: Traditional industrial anomaly detection methods have limitations: unsupervised approaches require manual thresholds and yield rough localizations, supervised methods overfit due to scarce/imbalanced data, and both suffer from "One Anomaly Class, One Model" constraint.

Method: Proposes Referring Industrial Anomaly Segmentation (RIAS) paradigm using language guidance. Introduces MVTec-Ref dataset with diverse referring expressions and 95% small anomalies. Presents DQFormer benchmark with Dual Query Token (Anomaly/Background) and Language-Gated Multi-Level Aggregation for efficient visual-textual integration.

Result: RIAS demonstrates effectiveness in advancing industrial anomaly detection toward open-set capabilities, enabling precise mask generation from text descriptions without manual thresholds and detecting diverse anomalies with a single model.

Conclusion: RIAS represents a paradigm shift in industrial anomaly detection by leveraging language guidance to overcome traditional limitations, moving toward more flexible and efficient open-set anomaly detection systems.
...
TL;DR: RegionReasoner: A reinforcement learning framework for multi-round visual reasoning that enforces grounded reasoning by requiring explicit citation of reference bounding boxes, with global-local consistency rewards to maintain semantic coherence.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language models rely on single-step or text-only reasoning, limiting their ability to iteratively refine understanding across multiple visual contexts. There's a need for systems that can perform multi-round visual reasoning with explicit spatial grounding.

Method: Propose RegionReasoner, a reinforcement learning framework that requires each reasoning trace to explicitly cite corresponding reference bounding boxes. Uses global-local consistency reward that extracts key objects/nouns from global scene captions and region-level captions, aligning them with reasoning traces. Optimized with structured rewards combining grounding fidelity and semantic alignment.

Result: RegionReasoner-7B, together with the new benchmark RegionDial-Bench, considerably improves multi-round reasoning accuracy, spatial grounding precision, and global-local consistency on detection and segmentation tasks.

Conclusion: RegionReasoner establishes a strong baseline for multi-round visual reasoning with explicit spatial grounding, addressing limitations of existing single-step or text-only reasoning approaches in vision-language models.
...
TL;DR: Lightweight two-stage pipeline for autonomous underground infrastructure inspection: RAPID-SCAN segmentation (0.64M params, 0.834 F1) + fine-tuned Phi-3.5 VLM for natural language summaries, deployed on edge devices with quantization for real-time performance.


<details>
  <summary>Details</summary>
Motivation: Automated generation of human-readable summaries from robotic inspection data remains challenging on resource-constrained edge devices, despite robotic platforms being able to detect structural deficiencies. There's a gap between automated defect detection and actionable insights for infrastructure maintenance.

Method: Two-stage pipeline: 1) RAPID-SCAN segmentation model (0.64M parameters) for defect segmentation, 2) Fine-tuned Phi-3.5 Vision-Language Model for generating natural language summaries from segmentation outputs. Uses post-training quantization with hardware-specific optimization for edge deployment.

Result: RAPID-SCAN achieves 0.834 F1-score with only 0.64M parameters. Complete pipeline deployed on mobile robotic platform demonstrates effectiveness in real-world inspection scenarios. Quantization reduces model size and inference latency without compromising summarization quality.

Conclusion: The edge-deployable integrated AI system bridges the gap between automated defect detection and actionable insights, paving the way for more scalable and autonomous inspection solutions for underground infrastructure maintenance.
...
TL;DR: LIVE introduces a cycle-consistency objective for autoregressive video world models to bound error accumulation in long-horizon video generation, eliminating need for teacher distillation.


<details>
  <summary>Details</summary>
Motivation: Autoregressive video world models struggle with long-horizon generation due to error accumulation. Prior methods use teacher models and distribution matching but are computationally expensive and don't prevent error propagation beyond training horizons.

Method: LIVE uses a novel cycle-consistency objective: forward rollout from ground-truth frames followed by reverse generation to reconstruct initial state, with diffusion loss computed on reconstructed terminal state. Also introduces unified view of different approaches and progressive training curriculum.
...
Conclusion: LIVE effectively bounds error accumulation in long-horizon video generation without teacher distillation, enabling stable generation beyond training horizons through cycle-consistency and progressive training.
...
TL;DR: Automated framework converts static anime illustrations into manipulatable 2.5D models using semantic layer decomposition and depth inference, trained on bootstrapped Live2D supervision.
...
Method: Decomposes single images into fully inpainted semantic layers with inferred drawing orders. Uses a scalable engine to bootstrap training data from commercial Live2D models. Combines diffusion-based Body Part Consistency Module for global geometric coherence with pixel-level pseudo-depth inference for intricate stratification resolution.

Result: The approach yields high-fidelity, manipulatable models suitable for professional, real-time animation applications, overcoming the limitations of manual workflows.

Conclusion: The framework successfully automates the transformation of static anime illustrations into dynamic 2.5D models, addressing data scarcity through bootstrapped supervision and resolving complex layer stratification for professional animation use.
...
TL;DR: Zero-shot LVLM pipeline automates bone, projection, and laterality identification in heterogeneous paleoradiology images with 92%/80%/100% accuracy.


<details>
  <summary>Details</summary>
Motivation: Paleoradiology images are highly heterogeneous (disarticulated bones, ad hoc positioning, missing laterality markers) with high variability from factors like age, sex, and equipment, making manual content navigation time-consuming and inefficient for expert analysis.

Method: Zero-shot prompting strategy using state-of-the-art Large Vision Language Model (LVLM). Pipeline converts raw DICOM files to bone-windowed PNGs, submits them to LVLM with carefully engineered prompts, receives structured JSON outputs, and formats results onto spreadsheets for validation.

Result: On 100 expert-reviewed images: 92% main bone accuracy, 80% projection view accuracy, 100% laterality accuracy, with low/medium confidence flags for ambiguous cases. Demonstrates LVLMs can substantially accelerate code word development for large paleoradiology datasets.

Conclusion: LVLMs can efficiently automate content navigation in heterogeneous paleoradiology images, accelerating workflow and enabling efficient triaging for expert anthropological analysis.
...
TL;DR: REPA-G uses aligned self-supervised representations for inference-time conditioning in diffusion models, enabling precise control from texture to semantic guidance without retraining.
...
Method: REPA-G leverages aligned representations from pre-trained feature extractors to guide denoising at inference time. It optimizes a similarity objective (potential) to steer generation toward conditioned representations, enabling control at multiple scales from patch-level texture to global semantics, and supports multi-concept composition.

Result: Quantitative results on ImageNet and COCO demonstrate high-quality, diverse generations. The method provides versatile control without retraining, offering a flexible alternative to text prompts.

Conclusion: REPA-G enables precise inference-time conditioning using aligned representations, theoretically justified as sampling from potential-induced tilted distributions, with practical applications for fine-grained control in diffusion models.
...
TL;DR: RAWDet-7 is a large-scale dataset of RAW images for object detection and description research, addressing limitations of ISP-processed RGB images by preserving sensor-level information.


<details>
  <summary>Details</summary>
Motivation: Most vision models use ISP-processed RGB images optimized for human perception, which discards sensor-level information that could be useful for machine reasoning. RAW images preserve unprocessed scene data with richer cues for object detection and description.

Method: Introduced RAWDet-7 dataset with ~25k training and 7.6k test RAW images collected across diverse cameras, lighting conditions, and environments. Images are densely annotated for 7 object categories following MS-COCO and LVIS conventions, with object-level descriptions from corresponding sRGB images.

Result: Created a benchmark dataset supporting evaluation under simulated 4-bit, 6-bit, and 8-bit quantization to reflect realistic sensor constraints. The dataset enables research on detection performance, description quality, and generalization in low-bit RAW image processing.

Conclusion: RAWDet-7 provides a valuable resource for studying object-level information preservation in RAW image processing and low-bit quantization, addressing gaps in current vision research that relies on ISP-optimized images.
...
TL;DR: FOVI is a foveated vision system inspired by human retina and V1 cortex that reformats variable-resolution sensor data into uniform density, enabling efficient kNN-convolution for high-resolution vision tasks.


<details>
  <summary>Details</summary>
Motivation: Human vision efficiently uses foveation (variable resolution) for active sensing, while computer vision typically uses uniform resolution, making high-resolution image processing computationally expensive.

Method: FOVI reformats retina-like variable-resolution sensor arrays into uniformly dense V1-like manifolds, uses k-nearest-neighborhood receptive fields, and implements kNN-convolution via novel kernel mapping. Two approaches: end-to-end kNN-convolution architecture and foveated adaptation of DINOv3 ViT using LoRA.
...
Conclusion: FOVI provides a pathway for efficient and scalable active sensing for high-resolution vision by mimicking biological foveation mechanisms, with code and pre-trained models available.
...
TL;DR: QVLA is an action-centric quantization framework for Vision-Language-Action models that uses channel-wise bit allocation based on action-space sensitivity, achieving significant compression while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: VLA models have immense computational demands that hinder deployment on resource-constrained robotic platforms. Existing quantization methods from LLMs prioritize data fidelity but ignore how minor action deviations compound into catastrophic task failures in robotics.

Method: QVLA introduces a granular channel-wise bit allocation strategy that measures final action-space sensitivity when quantizing each channel to various bit-widths. This yields per-channel importance metrics guiding global optimization that unifies quantization and pruning (0-bit) into a single framework.

Result: On LIBERO benchmark, QVLA reduces OpenVLA-OFT to 29.2% of original VRAM while maintaining 98.9% of original performance with 1.49x speedup, achieving 22.6% performance improvement over SmoothQuant.
...
TL;DR: NeuralShift: A U-Net-based model that predicts brain shift from preoperative MRI alone for temporal lobe resection surgery, achieving accurate global deformation prediction (DICE 0.97) and local displacement accuracy (TRE 1.12mm).


<details>
  <summary>Details</summary>
Motivation: Brain shift during neurosurgery invalidates preoperative MRI guidance, compromising surgical precision. Intraoperative MRI with brain shift compensation is crucial but challenging to obtain. The paper aims to predict brain shift using only preoperative MRI to enhance neuronavigation accuracy.

Method: Proposes NeuralShift, a U-Net-based deep learning model that predicts brain shift entirely from preoperative MRI for temporal lobe resection patients. The model learns to predict intraoperative brain deformation without requiring intraoperative imaging during inference.

Result: The model achieves excellent global deformation prediction (DICE score of 0.97) and accurate local displacements with Target Registration Errors as low as 1.12 mm on anatomical landmarks. It effectively compensates for large brain shifts during temporal lobe removal neurosurgery.

Conclusion: NeuralShift successfully predicts brain deformation during temporal lobe resection using only preoperative images, offering potential to enhance surgical safety, efficiency, and patient outcomes. The model and contributions will be made publicly available.
...
TL;DR: 3DiMo introduces an implicit, view-agnostic motion representation for video generation that avoids rigid 2D poses or inaccurate 3D models, enabling flexible camera control and novel-view synthesis while maintaining motion fidelity.


<details>
  <summary>Details</summary>
Motivation: Existing motion control methods have limitations: 2D poses bind motion to specific viewpoints (no novel-view synthesis), while explicit 3D models (like SMPL) have inherent inaccuracies that override video generators' intrinsic 3D awareness. Need a better approach that aligns with generators' spatial priors.

Method: Jointly trains a motion encoder with a pretrained video generator to distill driving frames into compact, view-agnostic motion tokens injected via cross-attention. Uses view-rich supervision (single-view, multi-view, moving-camera videos) for 3D awareness. Employs auxiliary geometric supervision with SMPL for early initialization only, then anneals to zero to transition to learning genuine 3D motion from data and generator priors.
...
Conclusion: Implicit, view-agnostic motion representation that aligns with video generators' spatial priors is superior to explicit 3D constraints, enabling better motion control with camera flexibility and novel-view synthesis capabilities.
...
TL;DR: Proposes progressive checkerboard ordering for multiscale autoregressive image generation, enabling parallel sampling while maintaining dependencies, achieving competitive performance with fewer steps.


<details>
  <summary>Details</summary>
Motivation: Address the challenge of efficiently sampling independent locations in parallel while modeling mutual dependencies in autoregressive image generation. Existing approaches use multiscale conditioning or parallel sampling with regular/randomized partitions, but need better balance between parallelization and dependency modeling.

Method: Uses a flexible, fixed ordering based on progressive checkerboards for multiscale autoregressive generation. The ordering draws samples in parallel from evenly spaced regions at each scale, maintaining full balance in all levels of a quadtree subdivision, enabling effective conditioning both between and within scales.

Result: Finds that in the balanced setting, a wide range of scale-up factors lead to similar results when total serial steps are constant. On class-conditional ImageNet, achieves competitive performance compared to recent state-of-the-art autoregressive systems with similar model capacity, using fewer sampling steps.

Conclusion: Progressive checkerboard ordering provides an effective approach for parallel sampling in autoregressive image generation, balancing efficiency and dependency modeling, enabling faster generation with competitive quality.
...
<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs suffer from severe training inefficiency due to massive model sizes and visual token numbers. While visual token pruning helps inference efficiency, applying it during training creates a training-inference mismatch where models perform poorly on non-pruned sequences.

Method: Proposes DualSpeed framework with two modes: Fast-mode uses visual token pruning methods as plugins with mode isolator to reduce tokens; Slow-mode trains on full visual sequences for consistency. Uses self-distillation where slow-mode learns from trained fast-mode to boost training.
...
Conclusion: DualSpeed effectively addresses training-inference mismatch in visual token pruning for MLLMs, enabling efficient training without performance degradation through dual-mode framework and self-distillation.
...
<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based editing models lack smooth intensity control for text-guided edits. While CFG affects prompt adherence, scaling it doesn't produce smooth transitions between input and edited results due to unconditional prediction dominating at low scales.

Method: Introduces Adaptive-Origin Guidance (AdaOr) that replaces the standard unconditional prediction guidance origin with an identity-conditioned adaptive origin. Uses identity instruction for identity manipulation and interpolates between identity prediction and standard unconditional prediction based on edit strength.

Result: Provides smoother and more consistent control compared to current slider-based editing approaches for both image and video editing tasks. Enables fine-grained control at inference without per-edit procedures or specialized datasets.

Conclusion: AdaOr successfully addresses the smooth intensity control problem in diffusion-based editing by adjusting the guidance origin mechanism, offering practical continuous control for text-guided semantic manipulation.
...
TL;DR: EventNeuS: Self-supervised neural model for 3D mesh reconstruction from monocular color event streams, combining SDF and density field learning with event-based supervision.


<details>
  <summary>Details</summary>
Motivation: Event cameras offer advantages over RGB cameras in many scenarios, but existing event-based 3D reconstruction techniques are severely limited in accuracy. Dense 3D mesh reconstruction from events remains scarcely explored.

Method: Combines 3D signed distance function (SDF) and density field learning with event-based supervision. Introduces spherical harmonics encodings for enhanced handling of view-dependent effects. Uses self-supervised neural model for learning 3D representations from monocular color event streams.
...
Conclusion: EventNeuS successfully addresses limitations of existing event-based 3D reconstruction techniques, achieving state-of-the-art performance for dense 3D mesh reconstruction from event streams.
