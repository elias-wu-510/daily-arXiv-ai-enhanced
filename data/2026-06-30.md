...
TL;DR: GeoISF is a large-scale LiDAR-to-image geo-localization pipeline that uses an instance semantic forest (built with WordNet) to bridge the modality gap between point clouds and satellite images, achieving 13.22x better R@10 performance on KITTI compared to prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-to-image cross-view localization methods struggle in large-scale scenarios due to limited semantic alignment and the modality gap between point clouds and satellite images.

Method: GeoISF introduces an instance semantic forest constructed using WordNet, integrating semantic trees from multiple frames to enhance temporal semantic representation and discriminative power. It uses environmental semantic representation as a shared medium to bridge the modality gap.

Result: GeoISF achieves 13.22 times better performance than the parallel LiDAR-to-image method in the R@10 metric on the KITTI dataset.

Conclusion: GeoISF addresses the gap in large-scale LiDAR-to-image cross-view localization, offering a robust solution to computational and accuracy challenges. Code will be released as open-source.
...
TL;DR: A fully unsupervised spatiotemporal feature fusion framework using a memory-augmented autoencoder achieves 96.6% (DaLiAc) and 98.4% (PAMAP2) accuracy for HAR from IMU sensors, outperforming supervised baselines despite shorter temporal windows and realistic inter-class segmentation.


<details>
  <summary>Details</summary>
Motivation: To address key challenges in HAR: reliance on labeled data, multi-sensor fusion complexity, and the limited ability of unsupervised methods to capture spatiotemporal dependencies—especially in real-world scenarios with noisy data, overlapping activities, and missing labels.

Method: A fully unsupervised spatiotemporal feature fusion framework using a memory-augmented autoencoder. It extracts hierarchical static features via a Stacked Autoencoder with intra- and inter-sensor fusion, then uses a sequence-to-sequence LSTM Autoencoder for temporal refinement incorporating historical motion patterns without labels.

Result: 96.6% accuracy on DaLiAc and 98.4% on PAMAP2, surpassing supervised baselines and unsupervised approaches. Feature separability improved by up to 9% despite shorter temporal windows. Realistic inter-class segmentation reduced accuracy by ~7% but better reflects real-world transitions.

Conclusion: The proposed unsupervised framework effectively handles real-world HAR challenges (noise, overlapping activities, missing labels) and outperforms supervised methods, with practical relevance for real-time applications using short temporal windows.
...
TL;DR: This paper proposes using temporal prediction error from a self-supervised Joint Embedding Predictive Architecture (JEPA) trained on unlabeled driving data as a zero-shot proxy for driving scenario complexity, achieving higher scores for complex scenarios and 0.512 Average Precision in anomaly detection without any labels.


<details>
  <summary>Details</summary>
Motivation: Identifying complex and safety-critical driving scenarios in large unlabelled datasets is expensive because existing approaches rely on human annotators, supervised classifiers, or rule sets that require substantial prior knowledge about difficult scenarios.

Method: Train a minimal Joint Embedding Predictive Architecture (JEPA) on structured agent state data from nuPlan mini dataset; use temporal prediction error as a zero-shot complexity score without any labels during training or evaluation.

Result: The model assigns significantly higher scores to unprotected turns, crosswalk interactions, and pedestrian proximity scenarios, and lower scores to lane-following and stationary-traffic scenarios. Anomaly detection achieves Average Precision of 0.512 vs 0.436 chance baseline.

Conclusion: Temporal prediction error in a self-supervised latent world model is a practical proxy for driving scenario complexity.
...
TL;DR: The paper proposes a post-hoc framework for detecting characteristic patterns in images generated by Image Autoregressive Models (IARs) to trace their provenance, without requiring any modifications to the generation process or outputs, making it applicable to unwatermarked content.


<details>
  <summary>Details</summary>
Motivation: As IARs achieve photorealistic generation, robust data provenance is needed to trace generated images to their source model, preventing misinformation, fraud, and harmful content attribution.

Method: A post-hoc detection framework that analyzes characteristic patterns naturally introduced by the IAR generation process, without modifying generative models or outputs, enabling provenance tracing on already-published content.

Result: The framework demonstrates effectiveness across a wide range of IARs, showing high potential for robust data provenance tracing.

Conclusion: The proposed approach enables provenance tracing for IAR-generated images in contexts where watermarking is unavailable, such as already-published content or models without watermark integration.
...
TL;DR: A two-stage pipeline (TAAD + DST transformer) with ensemble fusion improves SoccerNet 2026 ball action spotting from 48.6 to 58.94 Macro-F1.


<details>
  <summary>Details</summary>
Motivation: To improve per-player ball action spotting in soccer broadcast video by combining track-aware detection with sequence transduction and ensemble diversity.

Method: Two-stage pipeline: (1) Track-Aware Action Detector (TAAD) with temporal transformer for per-player action logits; (2) Denoising Sequence Transduction (DST) transformer with two-stage per-player attention (spatial-first ordering). Four model variants are trained and combined via Weighted Event Fusion with agreement filtering and a tackle-class exception.

Result: Validation Macro-F1 improved from 48.6 to 58.94; spatial-first attention ordering improved Macro-F1 by 1.87%.

Conclusion: The proposed pipeline significantly outperforms the baseline, with spatial-first attention ordering and ensemble agreement filtering being key contributors.
...
TL;DR: Topo4Vec is a GeoAI framework that uses Spatial Representation Learning to automate vector data quality assessment by simulating topological errors and encoding geometries into latent space, achieving high accuracy across diverse urban areas.


<details>
  <summary>Details</summary>
Motivation: Traditional rule-based quality assessment algorithms for geospatial vector data struggle with diverse urban morphologies and massive data volumes, and GeoAI applications for native vector data remain underexplored.

Method: Topo4Vec simulates topological errors (e.g., overlapping polygons, overshoots/undershoots) to bypass manual annotation, then uses state-of-the-art Spatial Representation Learning (SRL) to encode vector geometries into a latent space where topological errors are separable from valid data.

Result: Peak accuracy of 0.99 for detecting overlapping building footprints and 0.60 for overshoots/undershoots in street networks across three study areas (Los Angeles, Munich, Singapore).

Conclusion: Topo4Vec demonstrates a scalable and autonomous GeoAI approach for large-scale vector data quality monitoring, offering promising potential for fast-growing geospatial data ecosystems.
...
TL;DR: Proposes a variation of Fidelity-based XAI metrics using uncertainty-provoking, in-distribution perturbations for evaluating CNN explainability, especially in low-class real-world settings, and demonstrates correlation between domain, data, and XAI choices.


<details>
  <summary>Details</summary>
Motivation: To address the lack of consensus on XAI evaluation methods and provide metrics suitable for real-conditions (low-class) applications, ensuring proper measurement of faithfulness.

Method: Proposes fidelity-based XAI metrics with in-distribution, uncertainty-provoking perturbations; compares against human-centric object localization and segmentation metrics on medical and natural imaging datasets.

Result: Highlights intricate correlations between domain, data curation, and XAI solution choices for validating CNN training.

Conclusion: The proposed metric variation is useful for real-conditions evaluation, and domain/data characteristics strongly influence XAI method selection.
...
TL;DR: RADIANT-PET combines a permissive segmentation model with an LLM reasoning layer to improve PET/CT lesion segmentation by suppressing false positives and aligning with clinical interpretation.


<details>
  <summary>Details</summary>
Motivation: Lesion segmentation in PET/CT is challenging because physiologic tracer uptake and artifacts can mimic malignant signal, leading to false positives.

Method: Two-stage framework: (1) high-sensitivity voxel-level segmentation generating candidate uptake regions, (2) LLM adjudication that converts regions into structured text descriptions and classifies each candidate as true lesion vs. false positive, optionally using radiology reports. A local LLM is further optimized via reinforcement learning (Group Relative Policy Optimization).

Result: Outperforms strong image-only baselines on AutoPET and OSU test cohorts, with largest improvements when radiology reports are provided.

Conclusion: LLM-based lesion-level reasoning adds a novel reasoning layer beyond conventional segmentation, effectively suppressing physiologic false positives and aligning predictions with clinical interpretation.
...
TL;DR: First training-free best-of-N sampling for longitudinal chest X-ray report generation, leveraging prior-to-current transitions via directional set distances to score candidate reports against ground-truth transition vectors.


<details>
  <summary>Details</summary>
Motivation: Chest X-ray reports are read in context of prior exams, focusing on changes. Existing generators ignore this longitudinal prior.

Method: Proposes transition-aware best-of-N sampling: split reports into sentence sets, compute directional set distances (mean-shift, novelty residual, directed-Hausdorff anchor, cost-weighted OT) to encode change, score candidates by cosine similarity to cached training transition vectors.

Result: Outperforms random selection across three generators and prompts, with largest gains on Impression section.

Conclusion: Training-free longitudinal awareness improves report generation quality, particularly for clinical impression summarization.
...
TL;DR: This paper presents Jenga Inverse Predictor (JIP-2), a GPU-accelerated deep learning framework that treats the physical anastylosis of collapsed architectural monuments as an inverse prediction task. It uses a physics engine, friction-based force thresholds, and a dual-stream ResNet-18 to predict block removal sequences for reverse reconstruction.


<details>
  <summary>Details</summary>
Motivation: Traditional anastylosis relies on expert judgement and manual block matching, which is labor-intensive and subjective. The authors aim to automate this process by framing it as a combinatorial inverse prediction problem inspired by Jenga.

Method: JIP-2 implements a rigid-body physics engine (OBB/SAT collision detection, PGS contact solver) accelerated with Numba JIT and CuPy CUDA. It applies Ziglar's analytical force thresholds across three friction levels, trains a dual-stream ResNet-18 that injects a friction one-hot vector to jointly predict block removal count, per-position removal probabilities, center-of-mass imbalance, and torque risk, then generates 3-D reverse reconstruction videos.

Result: The framework was evaluated across 450 simulated episodes with three friction levels (mu_s in {0.25, 0.40, 0.60}), predicting block removal sequences and structural stability metrics.

Conclusion: The authors discuss implications for computer-assisted anastylosis at the UNESCO Maya site of Uxmal, Yucatan, suggesting the framework could aid conservation efforts by automating structural reconstruction planning.
...
TL;DR: JASPR is a self-supervised deep learning framework that integrates HE images and spatial transcriptomics data by learning joint representations with spatial context, improving gene expression prediction and showing prognostic value for breast cancer.


<details>
  <summary>Details</summary>
Motivation: Spatial properties of tumors are critical for understanding disease biology and predicting outcomes, but existing methods lack effective integration of HE images and spatial transcriptomics that incorporates spatial context across modalities.

Method: JASPR uses a self-supervised deep learning framework with a cross-modal reconstruction objective incorporating spatial context. It employs shared modules for universal spatial properties and modality-specific experts for unique morphological and genomic features.

Result: Trained on breast cancer datasets, JASPR substantially improves HE-based prediction of 9,248 genes and provides prognostic value for breast cancer outcomes.

Conclusion: JASPR effectively integrates HE and ST data through joint spatial representation learning, enabling better gene prediction and prognostic assessment in breast cancer.
...
TL;DR: RadarTwin is a physics-based simulation framework that uses vision-language models and ray tracing to generate synthetic FMCW radar training data from 3D scene reconstructions, achieving 95.3% recognition accuracy on real objects with minimal real labels.


<details>
  <summary>Details</summary>
Motivation: Millimeter-wave radar perception suffers from data scarcity—models trained on existing datasets fail to generalize to new objects, environments, and sensing trajectories. Real radar data collection is expensive and time-consuming.

Method: Given a 3D reconstruction of a target space (via phone LiDAR, robot sensing, or RGB-to-3D), RadarTwin uses a vision-language model to infer radar-relevant surface materials, then applies physics-based ray tracing to synthesize raw FMCW radar measurements with multi-bounce propagation modeling.

Result: A representation trained on simulation alone recognizes real objects at 2.5× chance with no real radar labels; with few labeled examples, accuracy reaches 95.3% on 12-way recognition. Modeling environmental multipath is essential for matching real measurements.

Conclusion: RadarTwin enables training radar perception for a new space before any real radar data is collected there, bridging the simulation-to-reality gap via physics-grounded synthetic data generation.
...
TL;DR: CLOSER is a training-policy-free method for aerial VLN that performs closed-loop reasoning—action generation, reliability verification, targeted retrieval, and correction—before execution, achieving 32.01% SR and 21.28% SPL on CityNav benchmark.


<details>
  <summary>Details</summary>
Motivation: Existing VLN methods using large models adopt open-loop decision-execution, where errors accumulate without verification, causing critical issues in aerial VLN due to trajectory deviations and target loss.

Method: CLOSER: a closed-loop framework with hierarchical reasoner (generates candidate actions), multidimensional action verifier (assesses reliability), and verification-triggered multimodal retriever (retrieves exemplars from memory only when verification fails) before execution.

Result: On CityNav test-unseen split: 32.01% SR and 21.28% SPL, confirming effectiveness of closed-loop reasoning.

Conclusion: Closed-loop verification and correction before execution significantly improves aerial VLN performance without requiring task-specific training.
...
TL;DR: This paper proposes a semantic-aware generative image transmission framework for resource-constrained visual IoT systems. It uses VQ encoding, entropy/complexity-based token recoverability estimation, and instance segmentation-based semantic importance scoring to selectively transmit tokens over narrowband links. The receiver uses MaskGIT with Halton scheduling to restore masked tokens, achieving better bitrate-quality tradeoffs (e.g., 29.9 dB PSNR at 0.074 bpp, using only 44.6% of the bits of a DeepJSCC reference) and improved task-relevant object preservation.


<details>
  <summary>Details</summary>
Motivation: Resource-constrained visual IoT systems (edge cameras, drones, industrial sensors) need to transmit task-relevant visual evidence over low-rate wireless links. Existing image communication methods compress/transmit complete global representations, leaving limited room to exploit receiver-side generative restoration.

Method: 1) VQ encoder encodes captured image into discrete token grid. 2) Token recoverability is estimated from prediction entropy and local structure complexity. 3) Semantic importance is obtained from instance segmentation and category-aware scoring. 4) Spatial dispersal sampler selects tokens under a bitrate budget. 5) Transmitter sends quantization indices of kept tokens + binary mask map. 6) Edge/cloud receiver recovers masked tokens via MaskGIT with Halton sequence scheduling.

Result: At 0.074 bpp, achieves 29.9 dB PSNR using only 44.6% of the transmitted bits of a 0.167-bpp DeepJSCC/WITT reference. Semantic-aware masking preserves task-relevant objects better than random masking at both 30% and 50% mask ratios on Kodak downstream detection.

Conclusion: The proposed semantic-aware generative transmission framework provides a flexible bitrate-quality tradeoff for narrowband visual IoT links, and semantic-aware masking better preserves task-relevant objects compared to random masking.
...
TL;DR: Meta-learning creates human-like visual representations that better predict human similarity judgments, semantic rule learning, and brain activity, suggesting that the flexibility of human vision arises from the demand to learn new tasks from few examples.


<details>
  <summary>Details</summary>
Motivation: To explain why pretrained neural networks still fail to fully capture human visual representations, proposing that the fixed single-objective training of these networks lacks the open-ended, flexible task support that characterizes human vision.

Method: Training a sequence model across thousands of semantically rich tasks (mapping images to high-level concepts) using meta-learning (learning to learn), without any human supervision data, then comparing meta-learned representations to pretrained base encoders.

Result: Meta-learned representations better predict human similarity judgments, semantic rule learning, and high-level visual cortex activity compared to pretrained encoders. Behavioral gains depend on disentangled, high-level task distributions, while brain alignment is driven primarily by the learning-to-learn pressure itself.

Conclusion: The flexibility of human visual representations reflects the functional demand to learn new semantic relationships on the fly, consistent with a meta-learning account.
...
TL;DR: ViPSy is a framework that constructs preference data for VLMs by deriving visual cues from image variants and conditioning policy rollouts on those cues, achieving state-of-the-art hallucination reduction (35.7% on AMBER, 24.5% on Object HalBench) while improving general visual grounding benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing preference alignment methods for reducing VLMs hallucinations suffer from two limitations: (a) intervention-based methods introduce significant deviation from the policy distribution, and (b) sampling-based methods underuse visual information during preference pair construction.

Method: ViPSy operates in two stages: (1) derives visual cues from recurring object-level content across semantically aligned image variants to rely on visual information rather than language priors; (2) conditions the policy's own rollouts on this cue, allowing visually grounded guidance while staying close to the policy's distribution.

Result: Achieves new SOTA in hallucination mitigation: reduces hallucination rates on AMBER by 35.7% and Object HalBench by 24.5% compared to previous SOTA. Also improves performance on MMStar, MMVP, CV-Bench, semantic segmentation, and ImageNet linear probing.

Conclusion: ViPSy effectively enhances visual faithfulness and general visual capabilities by constructing preference pairs that are both policy-aligned and visually grounded.
...
TL;DR: DCSNet transforms global dense prediction into localized refinement via Detection-guided Hierarchical Cropping (DGHC) and Multiscale Feature Aggregation (MSFA), significantly outperforming SOTA on medical small object segmentation.


<details>
  <summary>Details</summary>
Motivation: Small object segmentation is hindered by class imbalance and boundary complexity; global networks fail on sparse targets or suffer edge degradation.

Method: DCSNet: an end-to-end framework with DGHC (region proposals to extract object-centric features, filtering background) and MSFA (Transformer + pixel-adaptive fusion for multiscale feature aggregation).

Result: Outperforms existing SOTA methods across three medical datasets, with substantial improvements in boundary precision and robustness for micro-lesion segmentation.

Conclusion: DCSNet provides a highly robust solution for clinical micro-lesion segmentation by effectively addressing class imbalance and boundary degradation.
...
TL;DR: The paper proposes Key-Correlated Layer Attention (KCLA), a novel layer attention mechanism that achieves linear computational complexity (down from quadratic) by leveraging the observation that Key representations have high cosine similarity. KCLA maintains dynamic information updates and long-range cross-layer connections with fixed spatial complexity, showing strong performance on image recognition, detection, and segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing layer attention methods suffer from quadratic complexity (O(N²)). Prior solutions like RLA and linear attention have limitations: static information updates and poor long-range cross-layer dependency modeling.

Method: KCLA is derived directly from the foundational definition of layer attention. It exploits the observation that Key representations across layers have high cosine similarity, enabling a reformulation that reduces complexity from quadratic to linear while preserving dynamic updates.

Result: KCLA achieves linear computational complexity with fixed spatial complexity (independent of network depth). It delivers good performance on image recognition, object detection, and medical image segmentation tasks.

Conclusion: KCLA successfully addresses the efficiency-complexity trade-off in layer attention, offering a practical solution for deeper networks with maintained performance across vision tasks.
...
TL;DR: RSGPNet is a training-free geometric prompting framework for remote sensing open-vocabulary semantic segmentation that refines segmentation using object geometric areas and consistency constraints, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing OVSS methods modify CLIP attention but suffer from unstable local segmentation in remote sensing domain. The authors aim to address these limitations with a geometric prompting approach.

Method: RSGPNet has three modules: 1) Text-guided Coarse Mask (TCM) for initial masks, 2) Geometric Re-prompting Module (GRP) converting coarse masks to geometric box prompts for refined masks, 3) Coarse-to-fine Consistency Verification Mechanism (CVM) preventing reinforcement of erroneous regions.

Result: RSGPNet significantly outperforms state-of-the-art methods on RS datasets across both quantitative and qualitative metrics, with excellent interpretability.

Conclusion: The training-free geometric prompting framework effectively improves segmentation accuracy in complex areas like category boundaries for remote sensing OVSS.
...
TL;DR: AEGIS is a semantic-aware and uncertainty-guided adversarial detection framework using a SemantiGAN module and Evidential Deep Learning classifier to detect various adversarial attacks on DNNs in vision sensor pipelines, achieving 92.1% AUROC on Tiny ImageNet.


<details>
  <summary>Details</summary>
Motivation: DNNs in vision sensor networks are vulnerable to adversarial manipulations and imperceptible perturbations that lead to erroneous predictions.

Method: AEGIS uses a SemantiGAN module as a multi-class semantic discriminator to filter visually inconsistent inputs, then computes instability metrics (FlipScore, Prediction Inconsistency, Layerwise Cosine Similarity, Entropy) via stochastic augmentation, aggregated into a 5D vector processed by an Evidential Deep Learning (EDL) classifier with Dirichlet distribution.

Result: On Tiny ImageNet across six attack types, AEGIS achieves AUROC 92.1%, AUPRC 90.2%, and accuracy 90.7%, outperforming conventional softmax-based detectors.

Conclusion: AEGIS effectively detects adversarial attacks and provides superior detection performance, robustness, interpretability, and uncertainty calibration compared to traditional methods.
...
TL;DR: DiffRGD is a plug-and-play distribution-aware guidance framework for diffusion models that preserves the latent Gaussian structure via Riemannian Gradient Descent on a spherical manifold, outperforming prior methods in image restoration and conditional generation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing inference-time guidance methods for diffusion models introduce distributional drift that degrades sample quality because they cannot effectively preserve the original Gaussian distribution of the latent space.

Method: DiffRGD formulates each sampling step as a constrained optimization problem on a spherical manifold induced by the latent Gaussian distribution, solving it via Riemannian Gradient Descent (RGD).

Result: Extensive experiments show DiffRGD outperforms previous methods in most image restoration and conditional generation tasks.

Conclusion: DiffRGD is a plug-and-play method that can be integrated into any pre-trained diffusion model without re-training or fine-tuning, effectively preserving the Gaussian structure while providing guidance.
...
TL;DR: MedDiffuseMix proposes a saliency-guided diffusion mixing framework that uses classifier-derived saliency maps to selectively augment low-importance regions of medical images, improving classification accuracy on four benchmarks over standard and diffusion-based augmentation baselines.


<details>
  <summary>Details</summary>
Motivation: Limited data availability, class imbalance, and domain variability hinder reliable medical image classification. Conventional augmentation distorts diagnostic structures, while generative augmentation introduces label-inconsistent content.

Method: Uses classifier-derived saliency maps to separate high-saliency diagnostic regions from low-saliency background areas, applies diffusion-guided mixing mainly to less important regions, with adaptive mixing, Gaussian boundary blending, and a saliency-preservation constraint to reduce semantic distortion.

Result: Evaluated on RSNA pneumonia chest radiography, Musculoskeletal Radiographs, PatchCamelyon, and Breast Cancer Histopathological Image Classification datasets. Outperforms standard augmentation, Mixup, GenMix, SaliencyMix, and diffusion-based baselines in accuracy, F1-score, and AUC. Ablation studies confirm importance of key components.

Conclusion: Saliency-guided diffusion mixing is an effective augmentation strategy for limited-data medical image classification, better preserving diagnostically salient regions.
...
TL;DR: JuZhou 1.0 is a 0.387B-parameter T2I diffusion model for fully offline on-device execution, achieving GenEval 0.69 (outperforming SDXL, SD3-Medium, IF-XL) and running in ~1.6s (U-Net) / 4.5s (full pipeline) on smartphones.


<details>
  <summary>Details</summary>
Motivation: Existing T2I diffusion models require cloud infrastructure, causing high latency, cost, and privacy concerns for edge deployment.

Method: Four key designs: (1) compact 0.387B backbone (U-Net + distilled decoder); (2) Rectified Flow + DMD2 distillation for 4-step inference; (3) Chinese semantic alignment via 9M curated pairs for direct Chinese prompting; (4) training on domestic Sugon K100 accelerators without NVIDIA GPUs.

Result: 28-step base model achieves GenEval 0.69, surpassing SDXL (0.55), SD3-Medium (0.62), IF-XL (0.61). On Snapdragon 8 Elite Gen 5, 4-step U-Net runs ~1.6s; full Android pipeline takes 4.5s on Xiaomi 17 Pro Max.

Conclusion: JuZhou 1.0 demonstrates practical mobile T2I generation with Chinese-native support, domestic-compute training, and fully offline deployment.
...
TL;DR: CLEAR-MoE converts frozen ViTs into sparse MoE models via a four-phase pipeline (scoring, decomposition, routing, dispatch), retaining ~99.9% accuracy with no backbone weight updates, but introducing 1.3-1.7x FFN slowdown on legacy hardware due to routing overhead.


<details>
  <summary>Details</summary>
Motivation: To efficiently convert dense pretrained Vision Transformers into sparse Mixture-of-Experts models without costly retraining or backbone weight updates, preserving performance while enabling conditional computation.

Method: Four-phase pipeline: (i) scoring FFN layers by sparsity/clusterability/sensitivity; (ii) decomposing selected layers into a shared low-rank SVD basis + per-cluster residual experts via k-means; (iii) training lightweight supervised routers; (iv) dispatching tokens via pluggable CUDA backends.

Result: CLEAR-MoE retains 99.9% accuracy (86.70% vs 86.73%) on Imagenette/DeiT-Small. The shared SVD basis is the key accuracy-preserving factor; routing mechanisms have negligible impact (≤0.06pp variation). Generalizes across five ViT backbones (5.7M-86.6M params) with ≤0.10pp accuracy drop. However, routing overhead makes CLEAR-MoE 1.3-1.7x slower on GTX 960.

Conclusion: Converting frozen ViTs to sparse MoE is feasible with minimal accuracy loss, but current routing overhead is memory-bound and limits speedup. Fused dispatch kernels are identified as a promising optimization direction.
...
TL;DR: This paper presents CounterVHD, a vision-traceable hallucination detection framework for LVLMs in clinical settings. It extracts verifiable entities from LVLM responses, uses a medical-domain-adapted Qwen-VL grounding verifier, and introduces counterfactual entity perturbation with uncertainty estimation to detect hallucinations without needing internal model access.


<details>
  <summary>Details</summary>
Motivation: LVLMs used in clinical image understanding suffer from hallucinations—producing text not supported by the image. Existing detection methods lack interpretability, require internal model access, or are not tailored for medical domains.

Method: The framework extracts visually verifiable entities from LVLM responses, uses a medical-domain-adapted Qwen-VL verifier for grounding, employs counterfactual entity perturbation, and computes entity-level uncertainty scores (combining positive confidence, counterfactual confidence, and grounding overlap) for binary hallucination decision-making.

Result: The method consistently outperforms recent baselines across multiple medical imaging modalities and LVLM backbones, with strong cross-model transferability and interpretable localization evidence.

Conclusion: CounterVHD is an effective, interpretable, and model-agnostic approach for detecting hallucinations in LVLMs for clinical applications, with publicly available code and dataset.
...
TL;DR: MammoFlow synthesizes multiview mammograms (CC & MLO views) by enforcing implicit 3D geometric consistency via an alignment module and EMD-based self-consistency loss on tissue distributions, improving downstream classification by 5%.


<details>
  <summary>Details</summary>
Motivation: High-quality multiview mammography datasets are difficult to acquire for deep learning, yet paired CC-MLO views are crucial for 3D breast anomaly localization.

Method: Integrates an alignment module (2D affine transformation subspace search) into a pretrained flow matching model (MammoFlow), with a pixel-space self-consistency loss using Earth Mover's Distance (EMD) on 1D AP-axis tissue distributions to enforce 3D consistency.

Result: Achieves superior image quality, passes expert radiologist evaluation, and improves downstream classification AUC by 5%.

Conclusion: This is the first work to guide multiview mammogram generation using implicit geometric tissue correspondence, demonstrating that physically consistent synthetic pairs enhance downstream task performance.
...
TL;DR: DCVLM is a benchmark for systematically evaluating data curation strategies (filtering, mixing, formatting, sampling) for Vision-Language Models, using a 6T token corpus across 160 datasets. Key finding: data mixing (especially instruction-heavy mixtures) matters more than filtering for performance.


<details>
  <summary>Details</summary>
Motivation: The community lacks systematic benchmarks for evaluating data curation strategies for VLM training, despite the critical importance of dataset quality.

Method: Collected 160 datasets (6T tokens) across four data types; built a controlled benchmark allowing participants to test curation strategies across 1B-8B models and 6.25B-200B token budgets; evaluated on up to 52 downstream benchmarks.

Result: Data mixing is more important than filtering; instruction-heavy mixtures outperform caption-heavy ones with gains widening at scale. DCVLM-Baseline achieves 63.6% accuracy on 33-task core suite with 200B tokens, outperforming FineVision by +5.4pp.

Conclusion: Data mixing strategies, particularly instruction-heavy compositions, are crucial for high-quality VLM training datasets. The benchmark and artifacts will be publicly released.
...
TL;DR: KM-Speaker is a keypoint-conditioned flow-based generative framework for speech-driven 3D facial animation that achieves both high-fidelity motion and precise control (global style + frame-level temporal) by disentangling audio-driven lip motion from keypoint-driven upper-face dynamics, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing controllable 3D facial animation models either sacrifice realism by relying on large-scale low-quality datasets for global style control, or lack fine-grained temporal precision needed for tasks like dialogue localization (dubbing) where matching expressions is as critical as lip-sync.

Method: KM-Speaker proposes: (1) a keypoint-conditioned flow-based generative framework, (2) a disentanglement strategy separating audio-driven lip motion from keypoint-driven upper-face dynamics, and (3) a global style context preservation mechanism for coherent full-face expressiveness.

Result: KM-Speaker consistently outperforms state-of-the-art methods in lip-sync accuracy, style adherence, and expressive temporal control, achieving high-fidelity motion and flexible controllability in a data-constrained setting.

Conclusion: The proposed KM-Speaker framework advances example-based 3D facial animation by successfully combining high-fidelity motion with both global and frame-level control, demonstrating that precise artistic control and realism can be achieved simultaneously without reliance on large-scale low-quality datasets.
...
TL;DR: This paper presents an LLM-based hybrid agentic framework for automated athlete assessment, combining computer vision (MediaPipe) for kinematic tracking with Vision-Language Models (Llama-4-scout) for qualitative analysis. Key innovations include a "Smart Grid" temporal chunking strategy reducing computational cost by 88%, an "LLM-as-a-Judge" self-correction loop, and a dual-persistence RAG pipeline (ChromaDB) enabling natural language semantic queries. The system aligns with SAI protocols for scalable, objective talent identification.


<details>
  <summary>Details</summary>
Motivation: Traditional athlete assessment methods are either subjective manual observation or basic CV systems limited to repetition counting, lacking the "coaching intelligence" to evaluate qualitative markers like form degradation, spinal articulation, and fatigue. There is a need for scalable, objective, holistic profiling aligned with official sports authority protocols.

Method: A dual-pipeline architecture orchestrated via LangGraph: (1) CV pipeline (MediaPipe) for geometric kinematic tracking, and (2) Vision-Language Model (Llama-4-scout) for semantic reasoning. Introduces a 3×3 "Smart Grid" temporal chunking strategy to reduce multimodal video processing overhead by 88%. Implements an "LLM-as-a-Judge" self-correction loop for data integrity. Uses a dual-persistence RAG pipeline (ChromaDB) for semantic querying.

Result: The Smart Grid chunking strategy reduces computational overhead by over 88% while preserving temporal continuity. The multi-agent framework bridges the gap between raw biometric tracking and actionable coaching insights, enabling complex semantic queries like "Identify athletes with high endurance but poor core rigidity."

Conclusion: The proposed hybrid agentic framework offers a scalable, objective solution for national talent identification, significantly advancing beyond manual observation and basic CV systems. It successfully integrates quantitative tracking with qualitative coaching intelligence, aligned with SAI protocols.
...
TL;DR: SatSplat adapts 2D Gaussian Splatting to satellite imagery with online camera adjustment, handling illumination variations via shadow mapping and color correction, achieving improved geometric accuracy and memory efficiency on benchmark datasets.


<details>
  <summary>Details</summary>
Motivation: High-resolution satellite imagery demands 3D reconstruction that is both fast and geometrically accurate, but existing 3DGS adaptations degrade under diverse illumination across multi-date, high-altitude acquisitions with small intersection angles, limiting practical remote sensing applications.

Method: SatSplat adapts 2D Gaussian Splatting (2DGS) to satellite photogrammetry with: (1) affine camera model approximation, (2) minimal delta parameterization for in-splat camera refinement, (3) geometric shadow mapping, and (4) per-camera color correction during training.

Result: On DFC2019 and IARPA2016 benchmarks, SatSplat achieves strong geometric accuracy, outperforming prior 3DGS baselines. On DFC2019, it reduces mean absolute error by 11.93% and peak video memory by 31% compared to state-of-the-art.

Conclusion: SatSplat enables large-scale digital surface modeling with practical computational efficiency, demonstrating that 2DGS with camera adjustment and illumination handling is effective for satellite-based 3D reconstruction.
...
TL;DR: Animation2Code benchmarks VLMs on reconstructing executable web animation code from videos, revealing they struggle with temporal consistency despite high appearance similarity.


<details>
  <summary>Details</summary>
Motivation: Current VLMs perform well on static visual-to-code tasks, but it is unknown whether they can recover temporal dynamics when motion is present.

Method: Introduces Animation2Code dataset (1,069 web animation videos with HTML/CSS/JS code) and two disentangled metrics: appearance similarity and temporal similarity.

Result: State-of-the-art VLMs achieve high appearance similarity but fail to maintain temporal consistency, even with finetuning and iterative refinement.

Conclusion: Temporal visual reasoning remains a significant challenge for current VLMs, highlighting the need for better temporal understanding capabilities.
...
TL;DR: IMU-HOI is a framework that jointly recovers full-body human pose and 6-DoF object trajectory from sparse IMUs, explicitly modeling human-object interaction via probabilistic hand-object contact inference and a three-stage fusion pipeline, achieving substantial accuracy gains over prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing IMU-based motion capture assumes an isolated human and ignores object contacts and dynamics, while vision-based methods suffer from occlusions and constrained capture volumes, limiting applicability in AR/VR and robotics.

Method: The approach infers probabilistic hand-object contacts from IMU streams, uses these as high-level signals to route between kinematic and inertial reasoning, and employs a three-stage fusion pipeline that refines human pose, root translation, and fuses hand-based forward kinematics with object-IMU integration for drift-resilient trajectories.

Result: Experiments on challenging human-object interaction scenarios show substantial accuracy gains over prior inertial motion capture methods. The framework can plug into existing sparse-IMU mocap backbones with minimal changes.

Conclusion: IMU-HOI effectively extends purely inertial motion capture from isolated humans to full human-object interaction and joint motion estimation, bridging a key gap in the field.
...
TL;DR: Meshtryoshka introduces a mesh-based differentiable rendering framework using nested mesh shells (like a matryoshka doll) extracted from a signed distance function, enabling high-quality novel view synthesis for both object-centric and large-scale unbounded scenes with off-the-shelf triangle rasterizers.


<details>
  <summary>Details</summary>
Motivation: Existing differentiable rendering methods rely on specialized custom representations and renderers, while mesh-based approaches are limited to object-centric scenes. The authors aim to bridge this gap by enabling differentiable rendering with conventional mesh tools that can scale to unbounded, real-world scenes.

Method: The framework uses nested mesh shells extracted from a 3D signed distance function via iso-surface extraction. Vertex opacities are computed as a function of signed distance. Each shell is rasterized independently with an off-the-shelf triangle rasterizer, and final images are produced via alpha compositing. Gradients flow through opacity values into the SDF, indirectly updating vertex positions without requiring differentiable vertex position gradients.

Result: Competitive performance on object-centric scenes with surface-based techniques. Scales to unbounded, real-world 3D scenes with high-quality novel view synthesis approaching state-of-the-art non-mesh methods.

Conclusion: Suggests that differentiable rendering can be solved without specialized renderers, using only conventional computer graphics tools, opening up mesh-based rendering for large-scale applications.
...
TL;DR: SIGNET is a framework for motion-level knowledge transfer in cross-language sign language translation, using attention-based hand-prior aggregation and gated fusion to combine multiple pretrained backbones, achieving SOTA results on four SLT benchmarks and WLASL recognition.


<details>
  <summary>Details</summary>
Motivation: Existing SLT methods are dataset/language-specific, struggle to scale, and overlook cross-lingual relationships that could enable effective transfer learning.

Method: SIGNET integrates multiple pretrained sign language backbones via an attention-based, hand-prior aggregation mechanism that guides a gated fusion network to dynamically select the most relevant experts for motion-level knowledge transfer.

Result: Achieves state-of-the-art translation performance on How2Sign, Phoenix14T, CSL-Daily, and MeineDGS, and surpasses prior methods on WLASL for sign language recognition.

Conclusion: Motion-level visual patterns captured by pretrained models can be reused across languages and datasets, enabling effective cross-lingual transfer without gloss annotations.
...
TL;DR: PDF proposes a physics-grounded disentangled flow matching framework that separately models morphology evolution (structural deformation) and intensity evolution (signal changes) for forecasting longitudinal brain lesion progression, outperforming existing methods on three datasets.


<details>
  <summary>Details</summary>
Motivation: Existing approaches learn direct mappings from baseline to future images without explicitly modeling the physical mechanisms of lesion progression, leading to limited plausibility, generalization, and interpretability.

Method: The method decomposes lesion growth into two processes: morphology evolution (structural deformation) and intensity evolution (signal changes), each learned by a dedicated flow matching network. A PDE-regularized loss based on diffusion-reaction-advection formulation enforces physics-grounded constraints.

Result: State-of-the-art performance on three public longitudinal datasets covering diverse brain diseases.

Conclusion: Disentangled modeling with physics-grounded constraints improves forecasting accuracy, plausibility, and interpretability for longitudinal brain lesion evolution.
...
TL;DR: AEGIR introduces explicit area emitters into Gaussian Splatting for inverse rendering, enabling more physically accurate light transport simulation and improved decomposition of illumination and materials.


<details>
  <summary>Details</summary>
Motivation: Inverse rendering suffers from ambiguity between illumination and materials. Existing relightable Gaussian Splatting methods use point lights or environment maps, ignoring the physical spatial extent of emitters, leading to incorrect light attenuation and unrealistic shadows.

Method: AEGIR explicitly models local area emitters within a relightable Gaussian Splatting framework, using differentiable deferred rendering with multiple importance sampling and targeted regularization to jointly optimize emitters, materials, and geometry.

Result: Explicit area emitters improve illumination reconstruction and enhance downstream tasks such as novel view synthesis, controlled relighting, and virtual object insertion, especially in scenes with complex local lighting.

Conclusion: Modeling physical area emitters is beneficial for more accurate inverse rendering and scene editing, outperforming prior point-light or environment-map based approaches.
...
TL;DR: Introduces Obliviate, a guidance-based concept erasure method for autoregressive image generation models, achieving significant reduction of harmful content while preserving utility.


<details>
  <summary>Details</summary>
Motivation: Concept erasure for autoregressive image generation remains largely unexplored, despite growing relevance and misuse concerns of generative AI models.

Method: Obliviate uses three key design choices: (1) KL-based supervision over visual token distributions, (2) trajectory-level updates over full autoregressive rollouts, and (3) aligned visual prefixes for stable target construction.

Result: Outperforms alternatives across three models (Liquid, Emu3-Gen, Janus-Pro), reducing nudity on RAB benchmark from 91.58 to 3.15 while preserving utility.

Conclusion: Obliviate effectively addresses the gap in concept erasure for autoregressive text-to-image models, showing strong defensive performance against harmful content.
...
TL;DR: FedLAS is a plug-and-play label smoothing algorithm that uses feature norm-based confidence indicators and bidirectional calibration gating to dynamically adjust smoothing per sample, addressing both overconfidence and underconfidence in DNN classifiers, improving calibration without sacrificing accuracy.


<details>
  <summary>Details</summary>
Motivation: Existing label smoothing methods (LS, MbLS) use predefined uniform smoothing rules and only address overconfidence, failing to account for diverse sample characteristics (difficulty/ambiguity) and the evolving model state during training, where samples exhibit varying degrees of under- or overconfidence.

Method: FedLAS introduces two key components: (1) Feature Norm-based Confidence Indicator (NCI) to control the degree of smoothing based on sample features, and (2) Bidirectional Calibration Gating (BCG) to detect and handle both overconfidence and underconfidence. It is a plug-and-play module compatible with LS and MbLS based losses.

Result: On standard and fine-grained high-resolution vision benchmarks, FedLAS consistently improves calibration over modern baselines, reducing Expected Calibration Error (ECE) and Adaptive ECE while maintaining Top-1 accuracy.

Conclusion: FedLAS effectively addresses the limitations of prior label smoothing methods by providing per-sample, bidirectional calibration adjustments, leading to better-calibrated DNN classifiers without compromising accuracy.
...
TL;DR: This paper proposes a semantics-guided framework that uses SAM-based segmentation and object-ID maps to enforce consistent deformation regularization at the object level in dynamic 3D Gaussian Splatting, improving rendering quality and spatial coherence.


<details>
  <summary>Details</summary>
Motivation: Existing deformation field-based dynamic 3DGS methods lack explicit object-level modeling, leading to inconsistent Gaussian deformations within objects and unwanted coupling between different objects.

Method: Extract segmentation masks using SAM, derive semantic features, construct an object-ID map via feature matching, identify top-k contributing Gaussians per object, and impose consistency regularization on deformation parameters (position, scale, rotation).

Result: The proposed approach outperforms baseline methods in rendering quality, achieving higher PSNR and SSIM, and lower LPIPS in dynamic 3DGS rendering.

Conclusion: Incorporating semantic cues to guide deformation behavior at the object level improves deformation consistency and rendering quality in dynamic scenes.
...
TL;DR: BackTranslation2.0 is a linguistically grounded, agentic evaluation metric for text-to-sign translation that uses a pipeline of specialized tools and LLM-based cross-referential modules to assess grammatical correctness, phonological accuracy, motion fluency, and generation fidelity, showing strong correlation with human judgments on a BSL dataset.


<details>
  <summary>Details</summary>
Motivation: Existing evaluation metrics for sign language generation are simplistic, poorly aligned with human judgments, and lack linguistic grounding, creating a need for more comprehensive and principled assessment methods.

Method: An agentic framework with a deterministic pipeline of specialized tools assessing four scoring dimensions, plus LLM-based cross-referential comparison modules for structured reasoning, with final scores computed via deterministic weighted formulas.

Result: Strong correlation with human judgments across all four quality dimensions, validated on a BSL dataset rated by humans using a linguist-and-deaf-expert-designed protocol, outperforming six baseline metrics.

Conclusion: BackTranslation2.0 provides a more comprehensive, interpretable, and linguistically principled evaluation framework for sign language production systems.
...
TL;DR: DTMf-MIL improves metastasis prediction by incorporating spatial geometry (signed distance functions) between tumor cells, fibroblasts, and lymphocytes into Multiple Instance Learning, outperforming methods that ignore spatial layout.


<details>
  <summary>Details</summary>
Motivation: Predicting distant metastasis from primary tumor histology is critical but challenging; existing MIL models discard spatial layout which is essential for defining metastatic potential.

Method: Proposes Distance-aware Tissue Modeling for MIL (DTMf-MIL) that computes signed distance functions (SDF) relative to tissue phenotypes (tumor cells, fibroblasts, lymphocytes) to explicitly capture spatial relationships and reinforce visual features with spatial priors.

Result: DTMf-MIL significantly outperforms state-of-the-art methods that ignore spatial layout on metastasis prediction; spatial awareness consistently improves diagnostic accuracy across diverse clinical tasks on public benchmarks.

Conclusion: Geometric arrangement of the tumor microenvironment at the interface with tumor cells is a key determinant of metastatic risk; explicit spatial modeling improves clinical performance in computational pathology.
...
TL;DR: SATB-VR introduces a few-step video restoration paradigm that uses an auxiliary predictor to skip low-SNR denoising steps, combined with SNR-Aware Trajectory Blending (SATB) and Denoiser-Driven Consistency (DDC) loss to balance efficiency and quality, achieving strong results in ≤5 steps.


<details>
  <summary>Details</summary>
Motivation: Diffusion models for video restoration are effective but slow due to many iterative steps; aggressive single-step distillation harms fine texture recovery. The goal is to achieve an optimal balance between efficiency and quality.

Method: SATB-VR uses an auxiliary predictor to jump-start denoising (bypass early low-SNR steps). The SNR-Aware Trajectory Blending (SATB) dynamically blends predictor output with ground-truth trajectory during forward process to reduce train-inference discrepancy. The Denoiser-Driven Consistency (DDC) loss uses the denoiser as a dynamic evaluator to align features and boost predictor accuracy.

Result: Under flexible few-step inference (≤5 steps), SATB-VR performs favorably against existing approaches on synthetic, real-world, and AIGC benchmarks.

Conclusion: The proposed SATB strategy and DDC loss effectively resolve the train-inference discrepancy in few-step video restoration, enabling both efficiency and high-quality texture recovery.
...
TL;DR: LogiCo is a unified framework that detects both logical and structural anomalies via component-level feature reconstruction, achieving SOTA on four benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing anomaly detection methods focus on structural anomalies but miss logical constraint violations, while logical anomaly approaches perform poorly on subtle structural anomalies due to inadequate granularity.

Method: LogiCo maps pre-trained image features into a discrete component-level feature space and performs collaborative feature reconstruction at both component and patch levels, plus a segmentation-map discriminator for count-related logical anomalies.

Result: LogiCo achieves state-of-the-art performance on both logical and structural anomaly detection across MVTec-LOCO, MVTec-AD, VisA, and Real-IAD benchmarks.

Conclusion: LogiCo unifies logical and structural anomaly detection effectively, demonstrating superiority and practical feasibility.
...
TL;DR: GLMP uses multimodal LLMs to convert histology images into text descriptions, then into numerical embeddings, effectively removing batch effects and improving cross-institutional generalization in pathology.


<details>
  <summary>Details</summary>
Motivation: Pathology foundation models suffer from batch effects (non-biological variations across tissue source institutions), and conventional methods like stain normalization are insufficient to address these complex artifacts.

Method: GLMP generates robust numerical embeddings by first converting histology image patches into intermediate textual representations using pretrained general-purpose multimodal LLMs, then encoding these texts with text encoders.

Result: GLMP effectively prioritizes biologically meaningful signals over TSI-specific artifacts, improving cross-institutional generalization; it is the first pathology model to use text descriptions of histological features as an intermediate representation for numerical embeddings.

Conclusion: Broad-domain, non-specialized MLLMs have untapped potential in computational pathology, offering a new paradigm for building generalizable and robust pathology models.
...
TL;DR: This paper introduces ICCS (a new task combining change captioning with segmentation) and proposes CCRC, a dual-chain framework that uses one chain for change-aware captioning and another for segmentation, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Traditional Image Change Captioning (ICC) lacks spatial grounding, limiting precision. The authors aim to jointly achieve structured change description and pixel-level localization.

Method: CCRC is a dual-chain framework: (1) Chain-of-Change-Captioning (CCC) uses Multi-Head Change-aware Attention to enhance fine-grained change perception and decides if a change is segmentable; (2) Chain-of-Change-Segmenting (CCS) is activated for segmentable changes, using spatial priors from CCC and a Change-aware Token Refiner for mask refinement.

Result: CCRC achieves state-of-the-art performance on both synthetic and real-world change detection benchmarks with pixel-level supervision.

Conclusion: The proposed CCRC framework effectively integrates change captioning and segmentation, addressing the limitations of prior ICC methods by adding spatial grounding.
...
TL;DR: FreqOrtho-SR proposes frequency-guided MoE experts and orthogonal gradient projection to resolve fidelity-perception conflicts in diffusion-based super-resolution, achieving competitive performance with single-step inference.


<details>
  <summary>Details</summary>
Motivation: Diffusion prior-based ISR suffers from two issues: (1) frozen pixel modules still allow semantic branch to expand within pixel subspace, preventing genuine perceptual improvement; (2) single static adapters fail to generalize across diverse real-world corruptions.

Method: Two components: (1) FreqMoE — non-parametric FFT-based degradation feature extractor routes inputs to specialized LoRA experts; (2) OGP — uses SVD on expert weight deltas to extract pixel-fidelity subspace, projects semantic gradients onto its null space for truly orthogonal dual-objective learning.

Result: Achieves competitive overall performance with strong fidelity-perception trade-off across multiple benchmarks using efficient single-step inference.

Conclusion: FreqOrtho-SR effectively addresses both the fidelity-perception conflict and degradation diversity in real-world ISR through frequency-guided expert routing and orthogonal gradient projection.
...
TL;DR: CrashTwin is a physics-grounded evaluation framework that tests whether generative world models obey physical laws in multi-agent collision scenarios, revealing that high visual quality often masks severe physical violations.


<details>
  <summary>Details</summary>
Motivation: Current world model evaluations focus on visual fidelity and semantic alignment but fail to assess whether generated dynamics obey fundamental physical laws, creating a critical blind spot for safety-critical applications like autonomous driving.

Method: CrashTwin couples a diverse dataset (25K synthetic + 12K real-world collision sequences) with a calibration-free 3D reconstruction pipeline, and proposes a diagnostic suite evaluating spatio-temporal consistency, momentum/energy conservation, and world-dynamics integrity.

Result: Benchmarking shows that state-of-the-art world models with high perceptual quality frequently exhibit severe physical violations during complex interactions.

Conclusion: CrashTwin serves as a vital diagnostic tool for developing physically grounded world models capable of reliable real-world simulation.
...
TL;DR: X-Mind internalizes Predictive World Models as Visual Chain-of-Thought in VLA models, using compact BEV sketches and recurrent block diffusion to enable efficient future-state reasoning for autonomous driving on resource-constrained platforms.


<details>
  <summary>Details</summary>
Motivation: Current VLA models lack predictive reasoning capability, relying only on reactive perception-action mapping. Existing Predictive World Model integrations suffer from prohibitive latency or shallow terminal task embedding.

Method: X-Mind framework (1) internalizes PWMs as Visual Chain-of-Thought (Visual CoT) enforcing world rollout before action; (2) uses compact abstract sketches (BEV layout + driving priors) with Deep Compression Autoencoder (96 tokens for 12 frames); (3) proposes recurrent block diffusion for efficient iterative refinement within one forward pass.

Result: Achieves competitive end-to-end driving performance on large-scale real-world data with low latency suitable for resource-constrained vehicle platforms.

Conclusion: X-Mind successfully deploys scalable cognitive reasoning in autonomous driving by making predictive world modeling efficient and deeply integrated into the VLA backbone.
...
TL;DR: SOCS introduces a closed-form control update for diffusion-based image inverse problems, achieving efficient measurement alignment with improved perceptual quality across diverse tasks.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion models for image inverse problems require controllably steering sampling toward measurements. Previous SOC-based approaches are computationally expensive because they optimize over the entire trajectory.

Method: The denoising process is modeled as a dynamical system with closed-form control updates applied at each sampling step, pulling measurement-consistent clean predictions onto the denoising flow. Control strength can be modulated to match the diffusion model's capabilities.

Result: SOCS achieves accurate measurement-aligned reconstructions with improved visual fidelity and stronger quantitative performance across a broad spectrum of image inverse tasks.

Conclusion: SOCS is compatible with various linear SDE backbones and offers an efficient, high-quality solution for diffusion-based image inverse problems.
...
TL;DR: BREIT is a modular framework for 3D MF-EIT stroke reconstruction that includes a neuroimaging-to-EIT pipeline, a forward solver, and a D-bar implementation. The authors propose dFNO-bar, which integrates Fourier Neural Operators into D-bar, achieving higher brain SSIM with comparable CC across noise settings.


<details>
  <summary>Details</summary>
Motivation: Progress in 3D deep-learning reconstruction for MF-EIT stroke imaging is limited by the lack of large-scale datasets with paired ground-truth volumes and by non-standardized pipelines for data generation, simulation, and evaluation.

Method: BREIT provides: (i) a neuroimaging-to-EIT pipeline converting CT/MRI into frequency-dependent GT admittivity volumes; (ii) a Python 3D Complete Electrode Model forward solver; (iii) a 3D D-bar implementation. dFNO-bar integrates Fourier Neural Operators into D-bar by learning a mapping from scattering data to conductivity.

Result: dFNO-bar achieves higher brain SSIM with comparable CC across noise settings when evaluated against D-bar, Deep D-bar, and Gauss-Newton reconstructions on UCLH-matched synthetic data.

Conclusion: BREIT provides a standardized framework for 3D MF-EIT stroke reconstruction, and dFNO-bar demonstrates improved reconstruction quality, with code and data publicly available.
...
TL;DR: A mobile app using MediaPipe hand detection and YOLO-V8 ring detection to allow users to virtually try on jewelry rings by capturing hand images and overlaying rings with proper alignment, scaling, and rotation.


<details>
  <summary>Details</summary>
Motivation: To enable users to virtually try on rings using a mobile device, eliminating the need for physical try-on and enhancing the online shopping experience for jewelry.

Method: Combines MediaPipe hand point detection (for hand landmarks and ROI mask) with YOLO-V8 custom object detection (for ring points and background removal). Angular discrepancy between finger axis and ring axis is computed via vector algebra, and ring size is rescaled based on finger thickness while preserving aspect ratio.

Result: An output image showing the user's hand with the selected ring realistically placed on the finger, displayed on the mobile app's user interface.

Conclusion: The proposed approach successfully enables virtual ring try-on through a mobile app by integrating hand detection, object detection, and geometric alignment techniques.
...
TL;DR: PSP framework uses position and shape priors (PCE + SPM + HPP modules) to overcome domain texture discrepancies in cross-domain few-shot medical image segmentation, outperforming SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing FSMIS methods struggle with cross-domain generalization due to drastic texture discrepancies between imaging modalities; they fail to decouple semantic structure from domain-specific appearance.

Method: Three modules: (1) Position Coordinate Embedding (PCE) for rapid organ localization via spatial coordinates; (2) Shape Prototype Modulation (SPM) for domain-invariant structural prototypes filtering texture noise; (3) Hybrid-Prototype Prediction (HPP) for adaptive support-to-query feature calibration.

Result: Extensive experiments on two public medical imaging datasets show PSP significantly outperforms state-of-the-art methods.

Conclusion: Leveraging position and shape invariance across modalities is an effective strategy for cross-domain FSMIS, successfully decoupling semantic structure from domain-specific appearance.
...
TL;DR: ViPSim bridges the representation gap in Embodied World Models by unifying Visual Space (spatial priors) and Parameter Space (numerical drivers) for consistent long-horizon video generation, enhancing trajectory fidelity and enabling robust evaluation of embodied agents.


<details>
  <summary>Details</summary>
Motivation: EWMs suffer from a representation gap between low-dimensional actions and high-dimensional video synthesis, causing trajectory drift and inconsistent robot-object interactions in long-horizon rollouts.

Method: ViPSim synergistically combines Visual Space (pixel-aligned end-effector poses, camera perspectives, depth-guided geometry, robotic masks) with Parameter Space (raw action sequences, camera matrices) to provide both geometric anchoring and numerical motion guidance.

Result: ViPSim is backbone-agnostic, significantly improves trajectory consistency, shows emergent capabilities in complex deformable object interactions (e.g., cloth folding), and maintains robust performance in out-of-distribution and cross-embodiment settings.

Conclusion: ViPSim provides a high-fidelity foundation for automated evaluation and predictive control of embodied agents by effectively bridging the representation gap in EWMs.
...
TL;DR: CoGS is a compositional Gaussian-splatting framework that decomposes monocular video of human-object interaction into three coordinated branches (articulated human, rigid object, static scene) with a six-stage optimization schedule, achieving superior reconstruction fidelity and perceptual quality.


<details>
  <summary>Details</summary>
Motivation: Existing dynamic radiance-field and Gaussian-splatting methods entangle human, object, and background components (which have different motion models but share pixels), causing motion leakage. Monocular human reconstruction also remains underconstrained in rarely observed regions.

Method: CoGS decomposes video into three coordinated Gaussian-splatting branches: (1) articulated human from canonical prior, (2) rigid object field with estimated trajectory, (3) static scene field with weak planar primitives. A six-stage optimization schedule stabilizes components independently, then fuses them with visibility-aware anchoring, silhouette/motion constraints, and delayed scene regularization.

Result: On HOSNeRF and NeuMan benchmarks, CoGS improves both human-object interaction reconstruction and in-the-wild human-scene rendering, achieving stronger fidelity and perceptual quality in full-frame and human-focused evaluations.

Conclusion: The compositional design keeps each component responsible for its own geometry and motion while allowing photometric evidence to correct the final composite, effectively addressing entanglement and underconstrained reconstruction challenges.
...
TL;DR: RefGlass-GS is an end-to-end UAV-based framework for photorealistic, semantic, and interactive digitization of reflective glass facades, integrating glass panel segmentation, viewpoint planning, optimized Gaussian Splatting with reflection modeling, and object-based data organization, achieving significant improvements over SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Existing digitization of buildings with reflective glass facades suffers from geometric reconstruction distortion, unrealistic view-dependent texture rendering, and difficulties in object-based semantic enhancement.

Method: (1) Individual glass panel segmentation via MAP estimation with structural regularities; (2) UAV viewpoint planning to maximize view-dependent appearance coverage; (3) Optimized Gaussian Splatting with Reflection MLP, deferred shading, and regularization terms for high-frequency near-field reflections; (4) Standardized data organization for object-based models on digital twin platforms.

Result: Glass panel segmentation: +0.1927 mIoU over SOTA. UAV view planning: +13.15 dB PSNR over commercial methods. RefGlass-GS modeling: +5.08 dB PSNR average over SOTA GS approaches for reflective scenes.

Conclusion: The proposed method is effective and superior for real-world reflective glass facade digitization, enabling instance-level panel extraction and high-quality novel view synthesis.
...
TL;DR: Ground4D is a two-stage framework that combines 3D foundation models (VGGT) for geometry initialization with dynamic Gaussian Splatting for geometry-consistency-aware refinement, enabling high-quality 4D scene representation and novel-view synthesis from a single monocular video.


<details>
  <summary>Details</summary>
Motivation: Existing methods either achieve strong rendering performance (Dynamic Gaussian Splatting) but lack multi-view geometric consistency, or recover coherent geometry (3D foundation models) but cannot produce photorealistic rendering. Ground4D aims to bridge this gap by grounding dynamic Gaussian representations with geometry priors from foundation models.

Method: Two-stage framework: (1) Geometry initialization via VGGT (3D foundation model) in a training-free manner to obtain multi-view-consistent geometry and camera poses from monocular video; (2) Geometry-consistency-aware refinement via dynamic Gaussian Splatting with differentiable rendering, maintaining multi-view consistency across observed and synthesized views.

Result: Achieves stronger reconstruction fidelity and rendering performance by integrating foundation-level geometric priors into dynamic Gaussian optimization, supporting rendering at arbitrary timestamps with continuous 4D dynamics.

Conclusion: Geometry-grounded constraints are crucial for robust 4D scene modeling, and integrating foundation-level geometric priors significantly improves both reconstruction fidelity and rendering quality in dynamic novel-view synthesis from monocular video.
...
TL;DR: DLGStream is a language-embedded 3D Gaussian Splatting representation for free-viewpoint video that enables 4D interaction and editing by using dual-opacity dynamic language Gaussians and an interpolation-based deformation field, achieving high-quality open-vocabulary segmentation with very small frame sizes (43 KB).


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS-based free-viewpoint videos lack user interaction and editing capabilities; current language Gaussian representations fail to meet the low frame size and high FPS requirements of FVV.

Method: (1) Dual-opacity dynamic language Gaussian representation with separate opacity attributes for color and language features to avoid joint optimization degradation; (2) Interpolation-based deformation field to reduce temporal redundancy and enable 4D frame interpolation for boosting FPS.

Result: Superior open-vocabulary segmentation and reconstruction quality with an average frame size of only 43 KB.

Conclusion: DLGStream effectively brings language interaction capabilities to free-viewpoint video while maintaining compact frame sizes, enabling 4D environment interaction, scene editing, and spatial intelligence.
...
TL;DR: RRG is a learning framework that improves personalization in MLLMs by generating discriminative concept descriptions through a reinforced reference game, achieving SOTA on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM descriptions for personal concepts include irrelevant state/context info that hinders unique identification; descriptions should be accurate, discriminative, and free of distracting details.

Method: Reinforced Reference Game (RRG) - MLLM plays both speaker and listener in a contrastive game, using reinforcement learning with verifiable contrastive rewards over hard positives (same concept, different views) and hard negatives (visually similar but different concepts).

Result: State-of-the-art across multiple tasks on three personalization benchmarks; generalizes to unseen domains; outperforms concept description methods and personalization-specific RL frameworks.

Conclusion: RRG effectively generates discriminative descriptions for personal concepts, improving MLLM personalization by removing distracting details through a contrastive game-based reinforcement learning approach.
...
TL;DR: EpiSAM is a prompt-guided transformer framework that uses neighbor-aware character detection and segmentation to robustly analyze stone inscriptions, achieving strong results and zero-shot generalization on Southeast Asian epigraphy.


<details>
  <summary>Details</summary>
Motivation: Stone inscriptions are valuable historical and linguistic sources, but automated analysis is difficult due to surface irregularities, erosion, and low contrast, where conventional document analysis fails.

Method: EpiSAM is a prompt-guided transformer framework that employs a neighbor-aware strategy, explicitly predicting adjacent characters alongside the target to resolve boundary ambiguities and improve mask generation.

Result: EpiSAM achieves consistent improvements over existing baselines and exhibits strong zero-shot generalization in challenging epigraphic scenarios.

Conclusion: Character detection with contextual neighbor information is an effective core strategy for robust stone inscription analysis, and the expanded dataset enables further research in Southeast Asian epigraphy.
...
TL;DR: HKVLM decouples visual localization from language reasoning by using a frozen detector for region proposals, a frozen LLM for reasoning, and a lightweight alignment hook to bind them via contrastive retrieval, achieving significant grounding improvements and hallucination reduction with minimal training data.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs suffer from binding failures—attending to correct regions but outputting wrong boxes/labels—because localization passes through the autoregressive language head (coordinate-as-text) or is squeezed through a single class string (two-stage pipelines). The goal is to eliminate this coupling and enable faithful perception-grounded reasoning.

Method: HKVLM uses a frozen language-aligned detector (Grounding DINO) to produce class-agnostic region proposals, a frozen LLM (Qwen2.5-VL) to encode reasoning instructions as referential queries, and a trainable alignment hook that binds queries to regions via contrastive retrieval and bipartite assignment in a shared embedding space, plus a faithfulness veto that blocks naming unsupported objects.

Result: Training only the hook lifts grounding accuracy by 50–90× over untrained cross-space matching; the faithfulness veto raises POPE accuracy from near-chance (0.50) to 0.66–0.76 and reduces hallucination from ~0.99 to 0.23–0.43 with only 200 expressions. Increasing proposals from M=50 to M=300 improves grounding by 19–24% without retraining.

Conclusion: Residual errors are primarily perceptual (SeeErr) rather than binding (SayErr), validating the say-vs-see decomposition. HKVLM is effective in small-data cold-start settings where monolithic VLM tuning struggles, offering a modular, data-efficient approach to vision-language grounding.
...
TL;DR: First comprehensive study on whether test-time scaling methods for LLMs transfer to LVLMs, finding small models benefit most (up to ~30% improvement), LVLMs lose focus with excessive compute, and visual information is used only early in reasoning.


<details>
  <summary>Details</summary>
Motivation: Test-time scaling is well-studied for LLMs but its applicability to LVLMs is largely unexplored. The authors ask whether these methods transfer and aim to provide systematic analysis across models, methods, and benchmarks.

Method: Comprehensive empirical study spanning multiple LVLMs (various sizes), nine test-time scaling methods, and six diverse benchmarks with fine-grained analysis of reasoning chain quality and information sufficiency.

Result: (1) Small, well-performing models benefit most, achieving ~30% improvement and often matching/surpassing large models; (2) LVLMs lose focus when given unnecessary extra compute; (3) Visual information is encoded early, then reasoning becomes text-only with diminishing image token contributions.

Conclusion: Provides practical guidance for deploying test-time scaling in LVLMs in research and industry, highlighting when and which models benefit most.
...
TL;DR: This paper proposes a method to couple digital image correlation (3D deformation) and infrared thermography (2D temperature) data on curved surfaces using a pinhole camera model projection, enabling full-field analysis with temperature gradients and rates via radial basis function interpolation.


<details>
  <summary>Details</summary>
Motivation: Existing methods require sophisticated procedures to align deformation and temperature data in the same Lagrangian frame, especially for non-flat surfaces. The authors aim to provide a simpler, more accessible coupling approach using independently calibrated industrial systems.

Method: The method uses a pinhole camera model to project 2D IR temperature data onto 3D point coordinates obtained from stereocorrelation-based DIC. The projection matrix is calibrated using a single reference object image. Radial basis functions are employed for global spatiotemporal interpolation to compute in-plane temperature gradients and rates on curved surfaces.

Result: Temperature fields are accurately represented at material points on curved surfaces using the proposed projection-based coupling, with the pinhole camera model effectively mapping 2D thermal data to 3D geometry.

Conclusion: The method is particularly suited for curved surfaces, straightforward to integrate into existing experimental protocols, and provides rich full-field datasets including temperature gradients and rates.
...
TL;DR: The paper investigates how different species perceive the color of the sky, finding that while humans perceive it as blue due to cone response functions, the dominant wavelength of sky metamer can vary drastically across bichromatic, trichromatic, and tetrachromatic visual systems.


<details>
  <summary>Details</summary>
Motivation: To determine whether the "color" of the sky can be defined for non-human visual systems, given that human perception of sky blue is a product of our specific cone response functions.

Method: A simple computational method to find monochromatic metamers for arbitrary visual systems, using published spectral sensitivity functions of various species to determine dominant wavelengths of metamers to skylight.

Result: Monochromatic metamers to skylight exist for a wide range of species (bichromats, trichromats, tetrachromats), but the dominant wavelength varies drastically between species and often differs significantly from human-perceived blue.

Conclusion: The color of the sky is not a universal perceptual property; different visual systems experience it differently, and the human perception of blue is a product of our specific cone response functions rather than an intrinsic property of skylight.
...
TL;DR: ExACT is a training-free framework for remote sensing visual grounding that uses a one-shot visual exemplar to calibrate cross-modal priors from frozen MLLMs, then refines them via clustering to guide SAM for precise pixel-level localization.


<details>
  <summary>Details</summary>
Motivation: To bridge the modality gap between abstract linguistic semantics and fine-grained visual cues in remote sensing visual grounding, which causes severe localization drift in cluttered scenes when using training-free MLLMs.

Method: ExACT has two components: (1) Vision Exemplar-based Calibrator (VEC) extracts fine-grained visual correspondences from a given exemplar to rectify cross-modal priors from frozen MLLMs; (2) Structure-Aware Refiner (SAR) uses iterative merge-and-select clustering to consolidate calibrated priors into positive/negative geometric prompts, which guide SAM for pixel-level predictions.

Result: Extensive experiments confirm the superiority of ExACT over existing training-free and weakly-supervised methods.

Conclusion: ExACT effectively addresses the modality gap in RSVG without training, achieving precise pixel-level localization through exemplar-driven calibration and structure-aware refinement.
...
TL;DR: This paper presents a robust ANPR system for Nepali license plates using YOLO-based detection and CNN classification, achieving 93% recognition accuracy on Devanagari script characters.


<details>
  <summary>Details</summary>
Motivation: To develop an automatic number plate recognition system tailored specifically for Nepali license plates written in Devanagari script, addressing the lack of robust solutions for traffic management in Nepal.

Method: A pipelined model integrating YOLO-based models for license plate and character detection, followed by a CNN classifier trained on 34 Devanagari characters, with data augmentation and additional training on embossed plates.

Result: The system achieved a recognition accuracy of up to 93% under real-world conditions.

Conclusion: The proposed system demonstrates strong real-world performance and provides a scalable solution for traffic management in Nepal.
...
TL;DR: SEAR is a self-evolving agentic image restoration framework that uses dual-process theory (Intuitive Executor + Deliberate Planner) with Monte Carlo Tree Search and episodic memory to overcome limitations of greedy search and information amnesia in real-world image restoration.


<details>
  <summary>Details</summary>
Motivation: Existing agentic IR frameworks suffer from excessive greedy search strategies (failing to balance exploration and exploitation) and underutilization of information (episodic amnesia), making them inadequate for complex, coupled real-world degradations.

Method: SEAR formulates restoration as sequential decision-making, inspired by dual-process theory: (1) Deliberate Planner uses Pruning-Aware Monte Carlo Tree Search with hybrid no-reference reward and MLLM-based tournament for long-horizon reasoning; (2) Intuitive Executor leverages self-evolving episodic memory indexed by degradation-aware state fingerprints to distill search trajectories into reusable expertise.

Result: Extensive experiments on synthetic and real-world benchmarks demonstrate strong perceptual and quantitative performance.

Conclusion: SEAR effectively addresses exploration-exploitation trade-offs and episodic amnesia in agentic image restoration, achieving superior results through memory reuse and adaptive expertise.
...
TL;DR: OvESyn is a text-conditioned 3D CT synthesis framework for ovarian cancer that generates synthetic abdominal-pelvic CT scans using clinical metadata and automated segmentations (no radiology reports needed), achieving strong distributional fidelity and demonstrating that generator fine-tuning is the key mechanism for cross-domain adaptation from thoracic to abdominal imaging.


<details>
  <summary>Details</summary>
Motivation: Ovarian cancer is diagnosed at advanced stages, and CT is critical for staging, but annotated imaging data is scarce due to privacy regulations and limited annotations, hindering computational model development.

Method: OvESyn constructs "Findings and Impression" text from CT-derived imaging descriptors and clinical metadata, then uses a latent diffusion model adapted to 493 ovarian cancer patients. It compares two adaptation axes: vision-language encoder alignment and generator fine-tuning, with systematic ablation.

Result: Full OvESyn achieves FID2.5D 29.35, Precision 0.671, Wasserstein-1 0.044. Generator-only variant achieves best Recall (0.645). Without generator fine-tuning, synthesis collapses (Precision/Recall near zero, FID2.5D >140). Encoder alignment improves fine detail and intensity.

Conclusion: Generator domain adaptation is the operative mechanism for crossing the domain gap; OvESyn supports transferability to report-scarce settings and synthetic cohort generation in abdomino-pelvic oncologic imaging.
...
TL;DR: MetaCLIP-CMR uses routinely recorded acquisition metadata (modality, view, vendor, field strength, scanner model) as textual supervision via CLIP to pretrain CMR image encoders, outperforming ImageNet and masked reconstruction baselines on classification and segmentation tasks while requiring <1% of the pretraining data of large-scale CMR models.


<details>
  <summary>Details</summary>
Motivation: Current CMR foundation models rely on image-only pre-training, ignoring the widely available structured acquisition metadata that could serve as weak semantic supervision.

Method: MetaCLIP-CMR converts acquisition metadata (modality, view, vendor, field strength, scanner model) into text descriptions and applies contrastive language-image pre-training (CLIP) to learn CMR representations.

Result: 86.8% modality accuracy, 86.5% cine view accuracy, highest Dice scores on ACDC and M&Ms segmentation under full and 20% fine-tuning, comparable to large-scale CMR models with <1% of their pretraining data.

Conclusion: Metadata-driven multimodal pre-training is a natural, easy-to-use, and effective strategy for CMR representation learning, highlighting the promise of leveraging routinely recorded acquisition information as supervision.
...
TL;DR: SciFlow is a training-based approach using self-supervised learning to generalize motion estimation across synthetic-to-real domains by blending synthetic features with semantic interference from open-world images and enforcing geometric consistency, enabling robust domain generalization without requiring ground truth data.


<details>
  <summary>Details</summary>
Motivation: Motion estimation models suffer from synthetic-to-real domain generalization due to the cost and scarcity of high-quality pixel-wise optical flow annotations in real-world domains.

Method: SciFlow is a network-agnostic, training-based approach that: (1) imposes semantic interference from open-world images onto synthetic images during training to blend in-domain features with cross-domain interference, and (2) utilizes geometric consistency to ensure validity of the self-supervision.

Result: SciFlow significantly enhances model robustness amidst domain variations and enables synthetic-to-real domain generalization without requiring any ground truth in the open world.

Conclusion: Self-supervised learning with semantic interference and geometric consistency is effective for bridging the synthetic-to-real domain gap in motion estimation, offering a practical solution for open-world deployment.
...
TL;DR: This paper shows that frozen LLM knowledge can be effectively used in text-to-image generation via shared attention in a MoT architecture, achieving strong benchmark scores (0.85 GenEval, 86.75 DPG-Bench, 0.66 WISE) and revealing emergent capabilities like cross-lingual generation, color-guided composition, and world-knowledge-guided synthesis without multimodal training data.


<details>
  <summary>Details</summary>
Motivation: To investigate whether the knowledge of a frozen, reasoning-capable LLM can be effectively utilized in T2I generation when trained exclusively on standard text-image pairs, without requiring interleaved multimodal signals or explicit reasoning supervision.

Method: Integrates a frozen LLM with a diffusion-based image generator through shared attention within the Mixture-of-Transformers (MoT) architecture, trained only on standard text-image pairs.

Result: Strong benchmark performance (GenEval 0.85, DPG-Bench 86.75, WISE 0.66 with inference-time reasoning). Emergent behaviors discovered: cross-lingual image generation, color-guided composition, emoji/ASCII scene construction, and world-knowledge-directed generation.

Conclusion: Pretrained LLM knowledge can guide image synthesis under standard T2I training paradigms without multimodal interleaved data or explicit reasoning supervision, opening new avenues for resource-constrained multimodal learning.
...
TL;DR: A Semantic-Aware, Physics-Informed, and Geometry-Grounded framework (W-Crafter) that uses three conditional signals (appearance, dynamics, geometry) to steer video editors for synthesizing diverse and realistic weather effects, improving robustness of autonomous driving tasks.


<details>
  <summary>Details</summary>
Motivation: Existing weather synthesis methods lack diversity in weather appearance and effective control over weather dynamics (temporal evolution, particle motion). Text prompts are underspecified, and general video editors suppress heavy weather phenomena.

Method: Factorizes synthesis into three signals: (1) semantic-aware appearance anchoring, (2) physics-informed dynamic simulation (Gaussian particle field under gravity, wind, turbulence), (3) geometry-grounded video synthesis to align particles with scene geometry.

Result: Produces diverse, physically/visually realistic weather effects. Synthesized data significantly improves robustness of autonomous driving semantic segmentation under adverse weather.

Conclusion: The proposed factorization into distinct, stable guidance signals effectively addresses diversity and control limitations in weather video synthesis, with practical benefits for downstream autonomous driving tasks.
...
TL;DR: This paper proposes a pipeline for spatio-temporal grounding in long videos that uses second-level tracking (instead of frame-level), cross-second smoothing, chain-of-thought trajectory synthesis with ground-truth annotations, and reinforcement learning optimization, achieving efficient and accurate localization.


<details>
  <summary>Details</summary>
Motivation: Direct frame-by-frame inference with VLMs on long videos is computationally expensive and unstable.

Method: (1) Shift from frame-level to second-level tracking with cross-second smoothing for continuity and sequence length reduction. (2) Synthesize CoT-style trajectories using advanced multimodal models for temporal localization and target selection. (3) Replace noisy synthetic coordinates with ground-truth annotations. (4) Optimize policy via RL with a verifier based on t_IoU + mv_IoU.

Result: The method achieves a strong trade-off between efficiency and localization quality across multiple FPS settings.

Conclusion: Second-level tracking with CoT supervision and RL optimization enables efficient and stable spatio-temporal grounding in long videos.
...
TL;DR: SFDNet is a small object detection framework that uses Adaptive Spectrum Disentanglement (ASD) to decompose features into spectral components while discarding background distractors, and Class-Wise Prototype Distillation (CPD) to enforce compact representations, achieving SOTA results on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Small object detection is fundamentally challenged by limited spatial resolution and weak visual cues, and background distractors across different frequency spectra further degrade the performance of existing methods.

Method: 1) Adaptive Spectrum Disentanglement (ASD) module that decomposes backbone features into complementary spectral components and discards background distractors per component; 2) Class-Wise Prototype Distillation (CPD) that establishes class prototypes and enforces compact representations via prototype distillation.

Result: Outperforms existing state-of-the-art methods by a large margin on multiple challenging SOD benchmarks.

Conclusion: The proposed spectrum-aware feature disentanglement and prototype distillation approaches effectively address background distraction and semantic inconsistency in small object detection.
...
TL;DR: FlowWM proposes a stochastic world model that performs flow matching directly in high-dimensional pretrained feature spaces (e.g., DINOv3), addressing the trade-off between VAE-based models (limited perception) and deterministic predictors (blurry futures). It introduces a differentiable one-step projection mechanism for efficient training with temporal consistency and task-driven objectives.


<details>
  <summary>Details</summary>
Motivation: Existing visual world models either compromise perception performance (VAE-based stochastic models with low-dimensional latents) or fail to capture multimodal futures (deterministic predictors collapsing to blurry means). A method is needed that preserves rich pretrained features while maintaining stochasticity for diverse future prediction.

Method: FlowWM performs flow matching directly within high-dimensional pretrained feature spaces (e.g., DINOv3). To handle the challenge of high dimensionality, it introduces a differentiable one-step projection mechanism that enables efficient training with temporal consistency and task-driven objectives.

Result: On synthetic and real-world (FuturePerception) benchmarks, FlowWM improves perception performance, mode coverage, and horizon robustness compared to existing approaches.

Conclusion: The proposed design for stochastic world modeling in high-dimensional feature spaces is validated, showing that flow matching in pretrained feature space can simultaneously achieve good perception and diverse future predictions.
...
TL;DR: A defogging pipeline pretrained on controlled lab fog and fine-tuned with randomized synthetic fog generalizes to out-of-distribution real-world settings (e.g., aircraft window video) with no target-domain training, using a paired Laplacian ratio for quality prediction and a NAFNet backbone achieving 24.33 dB / 0.7912 SSIM.


<details>
  <summary>Details</summary>
Motivation: To address the open transfer limitation reported for real-world binocular defogging, where models trained on controlled data fail to generalize to unseen sensors, scenes, and optical paths.

Method: (1) A single-camera fog imager with a 114mm scattering path produces 5,495 pixel-aligned foggy/clear pairs for paired Laplacian ratio quality prediction and pixel-exact L1 reconstruction. (2) The fog-chamber checkpoint is fine-tuned on Mapillary Vistas with on-the-fly randomized synthetic fog (varying strength, spatial variations, airlights, noise).

Result: NAFNet tops 30 backbones at 24.33 dB/0.7912 SSIM; a compact alternative achieves within 1.29 dB at 3% parameters. On aircraft-window video, NIQE drops from 6.22 to 4.97. On O-HAZE/NH-HAZE, reaches 20.71 dB/0.683 SSIM.

Conclusion: The two-stage training (lab pretraining + synthetic fine-tuning) enables robust zero-shot domain transfer for defogging across diverse real-world conditions without target-domain training.
...
TL;DR: A diffusion-based video relighting framework that addresses temporal discontinuities in long-horizon videos by reframing relighting as temporally conditioned latent domain translation, using masked target-domain self-conditioning and warm-start prompting to improve cross-chunk consistency.


<details>
  <summary>Details</summary>
Motivation: Existing video diffusion models for relighting are trained on short clips and applied to long videos via chunked sliding-window inference, which causes temporal discontinuities at chunk boundaries.

Method: 1) Reframes long-horizon relighting as temporally conditioned latent domain translation. 2) Uses masked target-domain self-conditioning to enforce cross-chunk continuity. 3) Introduces warm-start prompting with a relit prompt anchor from a controllable generative model.

Result: Markedly improved temporal consistency, reduced chunk-boundary artifacts, and suppressed unwanted appearance changes across chunks in in-the-wild long-horizon videos.

Conclusion: The proposed approach effectively solves temporal discontinuity issues in long-video relighting by propagating target-domain latents across boundaries and making this behavior learnable.
...
TL;DR: TrafficAlign is an automated framework that uses LLMs to generate realistic traffic scenarios from real-world driving videos, revealing up to 10.8% more collisions in AV models and reducing collision rates by 36.1% when used for fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Pretrained LLMs often fail to align with real-world traffic distributions when generating traffic scenarios for autonomous driving.

Method: An automated framework that synthesizes traffic scenarios from real-world driving videos, performs data validation, and aligns LLMs with the synthesized scenarios.

Result: Generated scenarios reveal up to 10.8% more collisions across three driving models than SOTA methods; fine-tuning with these scenarios reduces collision rates by 36.1%; scenarios align well with traffic distributions from six geographically diverse regions.

Conclusion: TrafficAlign effectively bridges the gap between LLM-generated scenarios and real-world traffic distributions, improving both stress-testing and robustness of autonomous driving models.
...
TL;DR: BTI-Net introduces bidirectional task interaction modules with adaptive uncertainty gating for joint medical image segmentation and classification, achieving consistent improvements over baselines across ultrasound, dermoscopy, and brain MRI benchmarks.


<details>
  <summary>Details</summary>
Motivation: Encoder-sharing architectures in multi-task medical image analysis limit decoder reconstruction to task-private representations, permanently discarding boundary cues and semantic priors that could be shared between segmentation and classification branches.

Method: BTI-Net uses Task Interaction Modules (TIM) for bidirectional communication at every decoder level, with spatial boundary cues gated into classification and global semantic priors modulating segmentation. Uncertainty Proxy Attention (UPA) adaptively gates TIM outputs per instance using cross-task alignment, scene complexity, and prediction confidence signals without external annotations.

Result: Improvements in segmentation IoU and classification accuracy over encoder-sharing and decoder-interaction baselines across ultrasound, dermoscopy, and brain MRI. Adaptive gating contributes +2.36 IoU over fixed bidirectional interaction; classification accuracy improves by up to +2.26 points. UPA provides reliable single-pass task-failure signals.

Conclusion: Bidirectional cross-task communication with adaptive uncertainty-based gating effectively addresses the synergy loss in encoder-sharing architectures, and UPA serves as a practical uncertainty estimation method without stochastic sampling overhead.
...
TL;DR: End-Net is a 24-layer CNN with 21 inception modules designed for multi-class MRI classification of neurological disorders (Alzheimer's, brain tumors, multiple sclerosis, healthy controls), using WGAN-GP augmentation and undersampling to handle class imbalance, and integrated into a web-based real-time inference system.


<details>
  <summary>Details</summary>
Motivation: Existing deep CNNs for MRI-based neurological disorder classification are mostly optimized for binary tasks and fail to capture multi-class features needed to distinguish subtle anatomical differences across multiple neurological conditions.

Method: End-Net uses 24 convolutional layers including 21 optimized inception modules with parallel 1x1, 3x3, factorized 5x5 convolutions and max pooling for multiscale feature extraction, plus global average pooling, compact FC classifier, and dropout. Class imbalance is addressed via WGAN-GP augmentation (minority) and random undersampling (majority).

Result: End-Net outperforms existing architectures in both accuracy and generalization on the Multi-Class Neurological Disorder dataset (Alzheimer's, brain tumors, multiple sclerosis, healthy controls).

Conclusion: End-Net is effective for multi-class neurological disorder classification and is deployed as an online real-time inference system for accessibility.
...
TL;DR: This study evaluates whether stable land-context priors (DEM and AlphaEarth) improve single-temporal SAR flood segmentation when pre-event imagery is unavailable, finding that both auxiliary priors outperform SAR-only baselines across multiple backbones and test events, with AlphaEarth excelling on harder events and DEM offering more stable performance.


<details>
  <summary>Details</summary>
Motivation: Rapid flood mapping is critical for emergency response, but optical imagery is often unusable during flooding and single-temporal SAR is ambiguous because new inundation, permanent water, and smooth surfaces produce similar backscatter. Pre-event acquisitions are often unavailable.

Method: Compared four backbones (CNN UNet, ImageNet-UNet, TerraMind ViT, DINOv3 ViT) in SAR-only, SAR+DEM, and SAR+AlphaEarth configurations under identical fusion design and event-stratified split, evaluated on Hurricane Florence and Louisiana floods.

Result: Both auxiliary priors improve over SAR-only baselines. AlphaEarth exceeds DEM on the harder Florence event for every backbone and achieves best Florence IoU; DEM is competitive on Louisiana. Seed analysis shows DEM is more stable while AlphaEarth offers higher peak performance and recall.

Conclusion: Single-temporal SAR flood segmentation can be reframed as alignment between radar observations and stable land-surface priors, where learned (AlphaEarth) and physical (DEM) context offer complementary pathways to more reliable rapid flood mapping.
...
TL;DR: This paper proposes CMTFormer, a cross-modal transformer that hierarchically fuses RGB frames and event camera data through shallow alignment, cross-modal enhancement, and learnable deep fusion modules to improve object detection performance.


<details>
  <summary>Details</summary>
Motivation: Previous multi-modal fusion techniques fail to handle the inherent heterogeneity between RGB frames and event streams, leading to noise amplification or redundant feature integration during cross-modal fusion.

Method: The proposed CMTFormer uses a shallow-to-deep interaction scheme: (1) Shallow Alignment Module (SAM) for efficient low-level feature fusion, (2) Cross-modal Enhancement Module (CEM) for mutually reinforced middle-level features using texture/edge info, (3) Learnable Deep Fusion Module (LDFM) with adaptive weights for high-level aggregation, plus a Spatial Prior Module for localization accuracy.

Result: CMTFormer consistently outperforms detection counterparts in both uni-modal and multi-modal settings on DSEC-Detection and PKU-DAVIS-SOD benchmarks.

Conclusion: The hierarchical cross-modal fusion paradigm effectively addresses RGB-event heterogeneity, demonstrating strong multimodal collaboration for event-based object detection.
...
TL;DR: Generative Pretrained Controllers (GPC) use tokenization and next-token prediction with reinforcement learning to create reusable, general-purpose physics-based character controllers from large motion datasets, achieving 99.98% success in reproducing motions with emergent robust behaviors.


<details>
  <summary>Details</summary>
Motivation: Developing controllers that can perform a wide range of natural, life-like tasks is a key challenge for practical physics-based character animation.

Method: End-to-end RL jointly optimizes a motion vocabulary via Finite Scalar Quantization (FSQ) and a control policy mapping discrete codes to physics-based controls. A GPT-style autoregressive transformer models the codebook structure for next-token prediction control. Downstream adaptation techniques are also proposed.

Result: Achieves 99.98% success rate in reproducing a vast corpus of motion clips. Exhibits emergent behaviors like perturbation response and recovery after falling.

Conclusion: The framework greatly simplifies training compared to prior tokenized methods and produces highly robust, general-purpose controllers suitable for diverse downstream applications.
...
TL;DR: SLIDE-IQA introduces a dual-branch Vision Transformer with spatially localized synthetic degradations and a Threshold-Bounded Exclusion Mechanism to improve NR-IQA sensitivity to localized distortions, achieving competitive benchmark performance.


<details>
  <summary>Details</summary>
Motivation: Existing SSL pipelines uniformly apply synthetic distortions across entire images, limiting sensitivity to spatially localized and co-occurring real-world degradations. The paper exposes this representational blind spot in current NR-IQA encoders.

Method: A dual-branch Vision Transformer framework (SLIDE-IQA) that injects spatially bounded degradations into contrastive pretraining, plus a Threshold-Bounded Exclusion Mechanism to resolve structural conflicts between degradation type and spatial scale in the latent space.

Result: SLIDE-IQA's synthetic-only pretraining significantly improves sensitivity to localized distortions and achieves competitive performance on NR-IQA benchmarks against existing SSL NR-IQA models.

Conclusion: Spatially localized degradation embeddings are effective for improving NR-IQA, addressing the blind spot of uniform distortion pipelines.
...
TL;DR: ATM is a zero-shot framework for dense 3D shape correspondence that uses pretrained vision models and parametric shape priors via an articulate-then-match paradigm, requiring no correspondence training data and robustly handling diverse 3D representations with topological noise.


<details>
  <summary>Details</summary>
Motivation: Finding dense correspondences between 3D shapes in real-world environments is fundamentally challenging due to lack of training time/samples, extreme-high resolution data with topological distortions, and the need to handle diverse 3D representations. Existing methods fail to address these issues simultaneously.

Method: ATM uses an articulate-then-match paradigm: (1) leverages pretrained vision foundation models and parametric shape priors to estimate parametric models from multi-view renderings; (2) grounds estimations via multi-view geometric consistency; (3) maps diverse inputs into a shared canonical parametric space for robust coarse correspondences; (4) refines coarse correspondences into precise dense mappings via spectral refinement.

Result: Achieves strong non-isometric benchmark results (2.4-TOPKIDS, 3.8-SMAL geodesic errors), reducing errors by 73% and 37% vs URSSM baseline. Handles in-the-wild raw scans up to 200k vertices with near-constant computation time while maintaining superior accuracy.

Conclusion: ATM provides a robust zero-shot solution for dense 3D correspondence that is immune to connectivity artifacts, requires no training data, and seamlessly handles meshes, point clouds, and 3D Gaussians in real-world environments.
...
TL;DR: AF3AD is a modular framework that synthesizes diverse pseudo-anomalies from normal 3D point clouds, improving unsupervised anomaly detection and localization across multiple methods and benchmarks.


<details>
  <summary>Details</summary>
Motivation: Detecting/localizing defects in 3D point clouds is challenging due to scarce and diverse abnormal samples, with training often limited to normal data only.

Method: AF3AD uses a center-conditioned parametric deformation model in local PCA frames with kernel-controlled spatial falloff, anisotropy, directional gating, and normal/tangential displacement fields to generate diverse pseudo-anomalies from normal point clouds.

Result: Consistent improvements in object- and point-level detection and localization on AnomalyShapeNet and Real3D-AD, with ablations on preset groups and robustness under noise.

Conclusion: AF3AD is an effective, standalone synthesis tool that transfers across different 3D anomaly detection paradigms, facilitating broader adoption.
...
TL;DR: The paper proposes Dynamic Trajectory Initialization (DTI) for Generative Face Video Super-Resolution, reformulating it as input-driven directional restoration using a DiT backbone with enhancement-and-injection conditioning and Discriminative Guide for SNR alignment, achieving SOTA performance with minor fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Existing GFVSR methods exploiting diffusion priors suffer from fixed sampling, expensive inference costs without large-scale auxiliary training, and low fidelity due to excessive pursuit of perceptual metrics. The authors aim to address these limitations.

Method: DTI reformulates GFVSR as input-driven directional restoration, using (1) an enhancement-and-injection conditioning mechanism for pretrained DiT backbone to improve fidelity without sacrificing perceptual quality, and (2) a Discriminative Guide (DG) trained via objective SNR alignment to dynamically set the starting sampling point.

Result: SOTA overall performance across diverse metrics and benchmarks with only minor model adaptation and fine-tuning. LPIPS is identified as the most convincing metric in their case, and the perception-distortion trade-off is demonstrated.

Conclusion: The proposed DTI paradigm effectively balances fidelity and perceptual quality in GFVSR with minimal adaptation cost, and the analysis reveals LPIPS as the most reliable metric for evaluating their method.
...
TL;DR: ZGL is a lightweight language-conditioned 3D human motion predictor that improves pose-only baselines by injecting semantic caption cues via zero-gated cross-attention adapters, achieving better MPJPE on Human3.6M and CMUMocap.


<details>
  <summary>Details</summary>
Motivation: Pose histories provide kinematic evidence but lack high-level semantic guidance for accurate 3D human motion prediction.

Method: ZGL renders observed poses, generates a caption via a vision-language model, encodes it with frozen CLIP-L text tower, projects into conditioning tokens, and injects them into a DCT-based spatial-temporal Transformer via compact cross-attention adapters with zero-initialized learnable gates.

Result: ZGL improves overall MPJPE over representative motion-prediction baselines on Human3.6M, and the caption conditioning transfers to CMUMocap.

Conclusion: Compact caption conditioning provides a practical semantic cue for 3D human motion prediction and generalizes across benchmarks.
...
TL;DR: Again-Pose is a robust 3D human pose estimation framework that uses high-quality anchor frames and explicit motion cue propagation to recover poses from severely degraded video frames where other methods fail.


<details>
  <summary>Details</summary>
Motivation: Current methods rely on implicit temporal attention that fails under severe visual degradation (motion blur, occlusion), causing feature collapse and catastrophic reconstruction errors. A more robust approach is needed for extreme motion scenarios.

Method: (1) Identifying high-quality Anchor Frames based on feature saliency; (2) Dual-path Motion-aware Module for capturing fine-grained inter-frame dynamics; (3) Difference-weighted Fusion Module for adaptively propagating kinematic cues from anchor frames to degraded intermediate frames.

Result: Outperforms state-of-the-art methods on Human3.6M, 3DPW, PoseTrack, and FineDiving datasets, demonstrating superior robustness and stability in recovering plausible poses under severe degradation.

Conclusion: Explicit motion-guided recovery using anchor frames is more effective than implicit temporal smoothing for robust 3D pose estimation under extreme motion conditions.
...
TL;DR: The paper shows that transferability of model competence from synthetic to real medical data can be predicted without real labels, based on whether competence is donor-driven (transfers) or host-driven (does not), validated across multiple models and datasets on lung CT tasks.


<details>
  <summary>Details</summary>
Motivation: Medical imaging lacks real labels to verify if synthetic model competence transfers to real data, so the authors aim to predict transferability in advance, label-free.

Method: Using synthetic digital twins, the authors distinguish donor-driven competence (property of transplanted nodule) vs host-driven competence (property of surrounding anatomy). They test on three lung CT vision-language tasks across five VLMs, four guidance conditions, and seven real datasets. A TrialCouncil router is calibrated only on synthetic CT.

Result: Presence and size orderings transfer with R² >= 0.96, while lobe does not. The split survives leave-source-out calibration, and diagnostic names predict the boundary preemptively. TrialCouncil matches the best fixed model where transfer is predicted.

Conclusion: The key contribution is that transfer itself is predictable, label-free, from synthetic data alone, not the router mechanism.
...
TL;DR: A confidence-feedback-weighted graph matching network is proposed to accurately match online-detected pseudo-damage sites to offline ground-truth sites in high-power laser facilities, achieving 96.36% F1-score.


<details>
  <summary>Details</summary>
Motivation: Online inspection images of final optics contain pseudo-damage sites that closely resemble true damage sites, making authenticity determination difficult. Existing matching models mainly suppress distractors implicitly through loss-function supervision, which is insufficient for this challenging task.

Method: A confidence-feedback-weighted graph matching network that takes only damage-site centroid coordinates as input. It estimates node matchability confidence from matching scores and feeds it back as reliability weights to guide edge-feature aggregation, suppressing distractors. Includes geometric consistency constraints and hard-example mining loss.

Result: On the Complex-Scene dataset, the method achieves a matching F1-score of 96.36% with robust and efficient performance.

Conclusion: The proposed confidence-feedback mechanism effectively suppresses distractor propagation and enhances cross-graph discriminability for damage-site matching in high-power laser facilities.
...
TL;DR: This paper presents a method to equip open-source Multimodal Large Language Models (MLLMs) with accurate 2D part-level point grounding by leveraging attention mechanisms and a lightweight decoder, without modifying original model parameters.


<details>
  <summary>Details</summary>
Motivation: MLLMs excel at object-level grounding but struggle with part-level grounding, which is crucial for fine-grained tasks like robotic manipulation.

Method: The approach uses a Q-Synth Module to synthesize grounding-aware queries within intermediate MLLM layers, extracts target-relevant attention patterns, and passes them through a lightweight Attention-to-Point Decoder to produce a point-centric heatmap. All original MLLM parameters remain frozen.

Result: Consistently improves part-level grounding accuracy across datasets and integrates seamlessly into any open-source MLLM.

Conclusion: The proposed method enables fine-grained part-level point grounding in MLLMs without compromising their pre-trained capabilities, offering a more direct alternative for representation in fine-grained tasks.
...
TL;DR: ScaleErasure is an inference-time concept erasure method for next-scale autoregressive image generation that selectively guides logits across three dimensions (scales, tokens, bit channels) to erase unsafe concepts while preserving general capabilities.


<details>
  <summary>Details</summary>
Motivation: Next-scale AR image generation has severe semantic entanglement at early scales, making concept erasure largely unexplored and challenging in this new paradigm.

Method: ScaleErasure performs two extra forward passes conditioned on unsafe and safe concepts, then selects and guides logits across scales, tokens, and bit channels to steer away from unsafe toward safe concepts.

Result: Outperforms adapted baselines in next-scale AR paradigm, achieving more precise concept erasure while preserving general generative capability.

Conclusion: Minimal intervention across three dimensions enables effective concept erasure under severe semantic entanglement in next-scale AR generation.
...
TL;DR: This paper proposes ASTAD (Asymmetric Style Transfer for Autonomous Driving) and a training-free two-stage framework ASTModel that bridges the synthetic-to-real domain gap using labeled synthetic data paired with unlabeled real-world reference images, achieving better perception utility, structural fidelity, and 3.2× inference speedup.


<details>
  <summary>Details</summary>
Motivation: Existing synthetic-to-real style transfer methods rely on symmetric semantic guidance (requiring labels for both synthetic and real domains), which is impractical because real-world reference images generally lack pixel-level annotations. This asymmetry leads to either high annotation costs or semantic misalignment.

Method: ASTModel is a training-free two-stage framework: (1) coarse semantic prior extraction from unlabeled real-world target images; (2) dynamic prior refinement and class-consistent style injection during the diffusion denoising process, enabling semantically consistent transfer without real-world labels.

Result: ASTModel significantly outperforms existing methods in downstream perception utility and structural fidelity, and achieves a 3.2× inference speedup.

Conclusion: This work aligns synthetic-to-real adaptation with practical constraints (asymmetric label availability) and has potential to accelerate scalable deployment of robust autonomous driving systems.
...
TL;DR: Pointer-CAD v2 presents a Plan-Then-Construct paradigm that decouples parameter reasoning from geometric construction, directly predicting continuous numerical values to eliminate quantization errors, achieving substantial improvements in geometric accuracy for CAD generation.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based CAD generation methods focus on visual similarity but overlook precise geometric parameters and correct metric scale. Small numerical deviations, negligible at shape-level, violate industrial tolerance requirements. Current autoregressive paradigms aggressively quantize numerical parameters, introducing errors.

Method: Proposes a Plan-Then-Construct paradigm with: (1) a structured design plan with explicit metric scale parameters; (2) a dictionary-organized parameter reference via pointer mechanism; (3) direct continuous value prediction bypassing quantization; (4) a new large-scale dataset with plan-level annotation; (5) three hierarchical geometry accuracy metrics (vertex, edge, face levels).

Result: Pointer-CAD v2 consistently outperforms existing baselines and achieves substantial improvements in geometric accuracy, enabling reliable CAD generation for precision-critical engineering applications.

Conclusion: By eliminating quantization errors through continuous value prediction and the Plan-Then-Construct paradigm, Pointer-CAD v2 significantly enhances parametric fidelity, making CAD generation viable for industrial applications with strict tolerance requirements.
...
TL;DR: A mask-free method for lossless multi-object 3D reconstruction from sparse, occluded views using 3D Gaussian Splatting, SAM2 segmentation, and SDS with diffusion priors, enabling physically plausible MPM simulations.


<details>
  <summary>Details</summary>
Motivation: To address object coupling caused by occlusion and limited viewpoints in multi-object 3D reconstruction, eliminating the need for manual masks while producing simulation-ready objects.

Method: Uses 3D Gaussian Splatting as base representation, SAM2-trained segmentation field for coarse instance partitions, joint Score Distillation Sampling (SDS) with diffusion priors for fragmented geometry reconstruction, and geometry-aware priors (intra/inter-object similarity) for regularization.

Result: Produces complete, simulation-ready 3D objects without manual masks, enabling realistic dynamic interactions on both synthetic and real-world datasets.

Conclusion: The proposed mask-free approach effectively solves multi-object decoupling under occlusion and sparse views, achieving physically plausible interactions via MPM simulation.
...
TL;DR: MirrorPPR is a novel framework for exemplar-based structural portrait photo retouching that uses a Retouching Operation Extractor to capture subtle facial/body edits from an exemplar pair, injects these via LoRA into a Diffusion Transformer (DiT), and is trained on a self-augmented 47M-pair dataset (MirrorPPR47M) with progressive curriculum learning, outperforming existing methods in retouching quality and identity preservation.


<details>
  <summary>Details</summary>
Motivation: Text-guided image editing struggles with fine-grained structural portrait retouching (e.g., facial features, body proportions). Existing exemplar-based methods handle only pronounced visual transformations, not the delicate, localized edits required for structural portrait retouching. The paper aims to address this gap by introducing exemplar-based portrait retouching and developing a framework that can accurately extract and transfer subtle structural edits.

Method: 1) A Retouching Operation Extractor captures subtle differences from an exemplar pair (before/after). 2) Extracted representations are injected into a pre-trained Diffusion Transformer (DiT) via a connector and Low-Rank Adaptation (LoRA) modules. 3) A data self-augmentation paradigm generates perfectly aligned cross-identity training pairs to overcome operation misalignment. 4) A large-scale dataset (MirrorPPR47M) is constructed with simulated and professional subsets for progressive curriculum learning.

Result: MirrorPPR significantly outperforms existing baselines in both retouching quality and identity preservation, as demonstrated through extensive experiments.

Conclusion: Exemplar-based structural portrait retouching is effectively addressed by MirrorPPR, which combines a Retouching Operation Extractor, LoRA-injected DiT, self-augmented data, and curriculum learning on a large-scale dataset to achieve state-of-the-art performance.
...
TL;DR: D²R²OSR proposes a degradation-disentangled framework for real-world omnidirectional image super-resolution, explicitly modeling distortions from fisheye capture and equirectangular projection using perspective projection representation and degradation-specific modules, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Existing real-world super-resolution methods fail for omnidirectional images because their degradation models do not account for the complex imaging pipeline involving fisheye capture and Equirectangular Projection (ERP), which introduces severe aliasing and projection-specific distortions.

Method: D²R²OSR introduces (1) a Perspective Projection Representation (PPR) branch operating alongside the ERP branch to capture viewpoint-aware features, and (2) a Degradation-Specific Module (DSM) that jointly models ERP-induced geometric distortions and PPR-specific real-world degradations, guided by projection priors and viewpoint-centric human perception.

Result: Achieves state-of-the-art performance on real-world omnidirectional super-resolution, producing visually compelling high-fidelity results while maintaining computational efficiency for low-resource deployment.

Conclusion: Explicitly modeling both fisheye and ERP-specific degradations through a disentangled representation framework is key to effective real-world ODI super-resolution, enabling high-quality 360° detail rendering.
...
TL;DR: FDM-MFVT is a few-step diffusion model for mask-free virtual try-on that uses an Outfit-aware Noise Optimization Module (6 steps vs typical 30) and an Instruction-driven Try-on Module, plus a new 30K-pair mask-free dataset (MFVT), achieving superior results with fewer inference steps.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion-based IVTON methods require many sampling steps and depend on costly masks and auxiliary networks; lack of large-scale mask-free paired datasets further hinders mask-free IVTON development.

Method: Proposes FDM-MFVT with two modules: (1) Outfit-aware Noise Optimization Module (OANO) initializes alignment space with noise using input image, requiring only 6 steps; (2) Instruction-driven Try-on Module (IDT) uses virtual try-on prompts and efficient adaptation. Also introduces MFVT dataset (30K mask-free pairs).

Result: FDM-MFVT achieves superior quantitative and qualitative results with fewer inference steps (6 vs 30) compared to both mask-based and mask-free baseline methods.

Conclusion: The proposed approach enables efficient, high-fidelity mask-free virtual try-on with significantly reduced sampling steps, advancing the practical applicability of IVTON systems.
...
TL;DR: RAGA is a method for physically plausible shadow casting in 3D Gaussian Splatting scenes that computes shadows entirely in Gaussian space using ray-Gaussian line integrals, avoiding mesh reconstruction, and achieves ~50 FPS with improved realism and temporal stability.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS avatar animation lacks realistic shadow casting, especially in multi-avatar and object-interaction scenarios. Prior methods rely on binary hit tests and mesh reconstruction, which are limiting and computationally expensive.

Method: RAGA computes shadows via exact ray-Gaussian line integrals, integrating the opacity profile along shadow rays and normalizing by the theoretical maximum integral to produce continuous weight values. An avatar proxy representation reduces temporal variance from clothing deformations. Implemented with custom CUDA kernels and NVIDIA OptiX framework.

Result: Achieves ~50 FPS shadow tracing. Evaluated on single-avatar, multi-avatar, and avatar-object interaction scenarios, demonstrating substantially improved shadow realism, temporal stability, and scene coherence over prior methods.

Conclusion: RAGA provides a mesh-free, physically plausible shadow casting solution for 3DGS avatars that works efficiently in interactive settings and generalizes to complex multi-agent and object interaction scenarios.
...
TL;DR: HiReFF enables 2K-resolution 360° human video streaming from uncalibrated sparse-view (4 views at 90° intervals) using feed-forward 3D Gaussian reconstruction with Scale-synchronized Camera Calibration, Gaussian-wise Foreground Masking, and High-resolution Side-tuning for efficient rendering.


<details>
  <summary>Details</summary>
Motivation: Current volumetric video streaming methods require per-scene optimization or calibrated cameras, and feed-forward models are limited to low-resolution (0.5K) single-frame synthesis. There is a need for high-resolution (2K), temporally consistent, and computationally efficient reconstruction from uncalibrated sparse-view inputs for AR/VR applications.

Method: (1) Scale-synchronized Camera Calibration — resolves scale ambiguity for multi-view supervision; (2) Gaussian-wise Foreground Masking — modulates Gaussian parameters to reconstruct clean foregrounds; (3) High-resolution Side-tuning — achieves 2K rendering by augmenting the Gaussian head with supplementary features while keeping backbone at 0.5K.

Result: HiReFF significantly outperforms existing methods in high-resolution streaming volumetric video reconstruction at 2K resolution from uncalibrated sparse-view videos.

Conclusion: The proposed feed-forward framework effectively addresses both temporal consistency and computational efficiency challenges, enabling practical high-resolution human reconstruction for holographic communication and AR/VR without need for per-scene optimization or calibrated cameras.
...
TL;DR: Proposes a two-stage gaze target estimation framework that incorporates object semantics, multi-scale feature fusion, and geometric constraints, achieving state-of-the-art results with only 7.1M parameters.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat gaze target estimation as pixel-level regression, failing to explicitly represent gazed objects as distinct semantic entities, leading to unstable and inconsistent predictions in complex scenes.

Method: A two-stage framework: (1) object-level representations during feature encoding to align image features with discrete semantic entities; (2) multi-scale feature fusion and geometric constraints from head pose/gaze direction for fine-grained localization and object-level discrimination.

Result: Achieves AUC of 0.961 (GazeFollow), 0.948 (VideoAttentionTarget), 0.987 (ChildPlay), 0.977 (GOO-Real) with only 7.1M parameters.

Conclusion: Reformulating gaze estimation as hierarchical reasoning guided by object semantics significantly improves performance and semantic consistency while maintaining a compact model size.
...
TL;DR: This paper presents a W4A4 quantization pipeline for Wan2.2-I2V-A14B, combining MixQ-style mixed precision for outlier activations, SmoothQuant-style channel smoothing, and block-wise HiF4 packing. On VBench I2V metrics, it stays within 2-3.5% of FP16 quality, outperforming a native HiFloat4 baseline that degrades ~5%.


<details>
  <summary>Details</summary>
Motivation: To enable 4-bit weight and 4-bit activation inference (W4A4) on a large video generation model (Wan2.2-I2V-A14B) under HiF4/MXFP4 formats while minimizing quality degradation compared to FP16, addressing the challenge of extreme low-bit quantization for large models.

Method: Combines three techniques: (1) MixQ-style mixed precision for sparse activation outliers, (2) SmoothQuant-style per-channel smoothing to rebalance dynamic ranges before W4A4 rounding, and (3) block-wise HiF4 packing for feed-forward linear layers. A dual-branch GEMM preserves outlier columns in higher precision while bulk channels use strict W4A4.

Result: On official VBench I2V metrics, the pipeline stays within 2-3.5% of FP16 on most quality axes, improves motion smoothness, and outperforms a native HiFloat4 baseline that degrades roughly 5% relative to FP16 across all reported scores.

Conclusion: The proposed quantization pipeline effectively reduces quality degradation from ~5% (native HiFloat4) to 2-3.5% of FP16 for W4A4 quantization of Wan2.2-I2V-A14B, demonstrating the effectiveness of combining mixed-precision outlier handling and channel smoothing for extreme low-bit quantization of video generation models.
...
TL;DR: ST-Merge is a plug-and-play, training-free framework that fuses redundant visual tokens during encoding to reduce inference latency in vision-language and action models, achieving up to 8.3× speedup with minimal accuracy loss.


<details>
  <summary>Details</summary>
Motivation: Video and high-resolution images produce massive visual tokens, causing high inference latency that hinders real-time robot control. The authors aim to break this computational bottleneck.

Method: ST-Merge constructs 3D spatiotemporal coordinates, uses multi-queue parallel matching and weighted aggregation to fuse redundant tokens across frames, and introduces a post-merge positional correction mechanism to preserve spatial precision.

Result: On Qwen2.5-VL (VQA task): 2× speedup with only 1% precision loss. On π_{0.5} VLA policy: 8.3× speedup at 1024×1024 resolution matching baseline accuracy.

Conclusion: ST-Merge effectively reduces token redundancy to accelerate inference while maintaining high precision, making it suitable for real-time robotic control.
...
TL;DR: This paper proposes a language dependency parsing mechanism for vision-language tracking that distills core tracking components (target objects, semantic concepts, background context) and uses Qwen-VL for component-aware adaptive text description updates, achieving superior performance on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing vision-language tracking methods that adaptively update textual descriptions suffer from erroneous target updating, excessive background distraction, and hallucination artifacts due to naive use of sequence models or LLMs.

Method: A language dependency parsing mechanism to distill core tracking components (target, semantics, background), combined with Qwen-VL for component-aware adaptive text description updates.

Result: Consistent and superior tracking performance on TNL2K, LaSOT, TNLLT, and OTB-LANG benchmarks.

Conclusion: The proposed dependency parsing mechanism effectively addresses text update defects in vision-language tracking, and integrating with Qwen-VL yields robust tracking performance.
...
TL;DR: SAFE-DiT is a training-free framework that removes redundant spatial attention masks in Diffusion Transformers to avoid performance taxes (MIDT) caused by mask-induced dispatch slowdowns in FlashAttention, achieving up to 5.09× speedup and significant memory reduction while maintaining visual quality.


<details>
  <summary>Details</summary>
Motivation: High-resolution DiT inference suffers from spatial redundancy, but spatially adaptive implementations encode region computation as attention masks, which inadvertently move scaled dot-product attention (SDPA) away from FlashAttention fast paths, creating a systems bottleneck called Mask-Induced Dispatch Tax (MIDT).

Method: SAFE-DiT separates exact mask elision from approximation-based spatial scheduling. It removes provenance-certified image self-attention masks that induce a row-wise constant shift, preserves semantics-bearing masks like text-padding masks, and realizes spatial adaptation through prompt-conditioned token partitioning, selective state updates with global context, and periodic context refresh. SAFE-Core is the acceleration-only configuration, and SAFE-DiT+SW adds sensitivity-weighted classifier-free guidance.

Result: Redundant masks make long-sequence attention 4.1× to 5.8× slower than mask-free path. On Lumina-Next, SAFE-DiT achieves 2.69× end-to-end acceleration at 1024² resolution and 5.09× at 2560², reduces peak memory at 2560² from 94.1 to 27.9 GB, and enables 3072² generation when dense inference runs out of memory.

Conclusion: Paired metrics, component ablations, and a blinded human study support visual non-inferiority of SAFE-Core to the dense fast-path baseline, while SAFE-DiT+SW provides a separate prompt-alignment operating point without reintroducing spatial self-attention masks. The framework effectively mitigates MIDT while preserving generation quality.
...
TL;DR: L2D2-GS reformulates generalizable 3D reconstruction from dense multi-view images as an iterative optimization and densification process, using self-supervised densification and geometric regularization to achieve state-of-the-art urban scene reconstruction fidelity with fewer primitives.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS methods suffer from either expensive per-scene optimization (high quality but not scalable) or feedforward inference (fast but memory-prohibitive at high resolutions and inconsistent in fusing dense multi-view observations). The goal is to achieve both high-fidelity reconstruction and generalization/scalability.

Method: (1) Reformulates reconstruction as iterative optimization and densification rather than one-shot regression. (2) Self-supervised densification policy using explicit reward signals from global reconstruction gains to guide local primitive densification. (3) Geometric regularization via reparameterization to constrain the optimization manifold and prevent early-stage artifacts and poor local optima.

Result: State-of-the-art reconstruction fidelity on PandaSet and Waymo datasets, strong zero-shot generalization, and uses fewer primitives than competing baselines.

Conclusion: L2D2-GS demonstrates that iterative optimization-based generalizable reconstruction can outperform both per-scene optimization and feedforward methods, offering a scalable and high-fidelity solution for dynamic urban environment reconstruction.
...
TL;DR: SAD-GS introduces dynamic geo-semantic anchoring to learn reliable 3D semantic Gaussian fields, addressing multi-view semantic drift and tracking errors through Semantic Anchor Distillation and a Geo-Semantic Feedback Loop.


<details>
  <summary>Details</summary>
Motivation: Multi-view 2D supervision for 3D semantic Gaussian fields is unreliable due to view-dependent semantic identity drift and tracker mask errors (boundary leakage, identity switches), leading to error accumulation in 3D representation.

Method: SAD-GS has two components: (1) Semantic Anchor Distillation (SAD) distills per-view visual embeddings into consensus text anchors for viewpoint-invariant semantics; (2) Geo-Semantic Feedback Loop (GSFL) uses the evolving 3D field to filter tracker anomalies and refine mask assignments via a conservative three-gate update rule.

Result: Achieves best overall performance on LERF-OVS, 3D-OVS, and Mip-NeRF360 in both open-vocabulary localization and semantic segmentation tasks.

Conclusion: Dynamic geo-semantic anchoring is effective and robust for reliable 3D semantic Gaussian field learning.
...
TL;DR: DR-GS is a unified Gaussian framework that disentangles geometry, illumination, and material for deformable, relightable objects, overcoming baked-in illumination and static snapshot limitations in existing GS methods.


<details>
  <summary>Details</summary>
Motivation: Existing GS methods for deformable objects bake illumination into textures, causing unrealistic appearance under dynamic deformation/lighting changes, and snapshot-based reconstruction prevents post-reconstruction material editing.

Method: DR-GS integrates physically-based inverse rendering, relighting, and deformation-aware manipulation within a unified Gaussian framework, explicitly decoupling geometry, illumination, and material representations.

Result: Achieves leading visual quality in static reconstruction, dynamic deformation, and relighting; reliably preserves reflections and specular highlights on glossy surfaces.

Conclusion: Establishes a fully decoupled geometry-illumination-material pipeline enabling high-quality 3D asset creation and comprehensive post-editing.
...
TL;DR: Event-VLA enhances VLA models with event camera streams to achieve robust robotic manipulation under varying and degraded illumination conditions.


<details>
  <summary>Details</summary>
Motivation: Existing VLA models assume well-lit, stable indoor settings, but real-world manipulation faces degraded RGB observations due to illumination shifts, which poses robustness challenges.

Method: Event-VLA introduces an action-query routing pathway that uses learnable action queries to extract task-relevant semantics from VLA reasoning and selectively aggregates event tokens via gated cross-attention, forming event-aware action representations without disrupting pretrained RGB-language priors.

Result: Event-VLA maintains strong manipulation performance under normal lighting and improves success rates under low-light degradation and near-dark real-world settings.

Conclusion: Event streams serve as illumination-robust, motion-sensitive complements to RGB, and the proposed action-query routing effectively integrates event information while preserving RGB-language pretrained knowledge for robust action prediction.
...
TL;DR: NaLA is a native 3D LLM layout agent that directly encodes 3D assets and scene boundaries into LLMs, using coarse-to-fine pose prediction to generate high-quality 3D scene layouts, outperforming prior methods in quality and efficiency.


<details>
  <summary>Details</summary>
Motivation: Existing LLM-based 3D layout agents convert 3D assets and layouts into text descriptions, causing severe information loss due to modality gap between text and 3D data, leading to implausible layouts.

Method: NaLA directly encodes 3D scene boundaries and 3D assets into LLM to preserve geometry; adopts a coarse-to-fine prediction mechanism (discrete autoregressive pose prediction followed by continuous regression refinement) for accurate positioning and orientation.

Result: NaLA outperforms prior layout agents in both generation quality and inference efficiency; ablation studies verify each component's effectiveness.

Conclusion: Native 3D encoding into LLMs with coarse-to-fine pose prediction significantly improves layout plausibility and efficiency for 3D scene generation.
...
TL;DR: QuADA-GS proposes a feed-forward Gaussian Splatting model for Arbitrary-Scale Super-Resolution that uses a Neural Routing Architecture to dynamically allocate computing resources and Hierarchical Pointer Convolution for efficient spatial processing, achieving SOTA performance with low latency and memory.


<details>
  <summary>Details</summary>
Motivation: Existing ASR methods either rely on fixed-scale models with sub-optimal interpolation or inefficiently process all pixels uniformly despite details being sparse. Adapting Gaussian Splatting to feed-forward ASR is challenging because GS optimization requires high-resolution gradients to drive primitive growth, which are unavailable during inference.

Method: QuADA-GS encodes inputs into latent space, uses a Neural Routing Architecture to evaluate local complexity and distribute a global budget with specific upsampling factors, dynamically densifies features into an irregular topology, and decodes into 2D Gaussian primitives. Hierarchical Pointer Convolution enables O(1) neighbor lookup for spatial communication.

Result: Achieves state-of-the-art Arbitrary-Scale Super-Resolution performance while maintaining low latency and a lean memory footprint.

Conclusion: The proposed adaptive, irregular representation approach is effective for continuous-resolution super-resolution, overcoming the gradient availability limitation in standard GS optimization during inference.
...
TL;DR: FiRe introduces a low-rank gating mechanism that lets each neuron in a periodic INR use an input-dependent frequency, acting as an implicit preconditioner via NTK to speed convergence and improve reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Periodic INRs assign the same global frequency to all neurons, wasting representational capacity when local signal content varies across the domain.

Method: FiRe reparameterizes per-neuron frequencies by adding a separate low-rank gating path that produces bounded, input-dependent frequencies, applicable to any periodic activation function.

Result: On 2D image fitting, FiRe achieves up to +1 dB PSNR gains over parameter-matched baselines at short training budgets, with performance depending on resolution, rank, and training budget.

Conclusion: The gating mechanism acts as an implicit preconditioner via the NTK, yielding better-conditioned initialization that speeds convergence and tracks high-frequency content more closely.
...
TL;DR: This paper investigates whether vision models truly understand objects or just exploit local statistical cues. Using a task where two classes differ only in global semantics (a closed vs. slightly open square) but have zero "syntactic distance" (no reliable local features), experiments show models collapse to random guessing beyond a critical image scale. Larger datasets and models only delay collapse, and Vision Transformers collapse earlier than ResNets, revealing a fundamental capability boundary.


<details>
  <summary>Details</summary>
Motivation: To question whether vision models genuinely recognize objects or merely exploit surface-level statistical cues, inspired by Wittgenstein's philosophy that recognition is bounded by the descriptive system learned.

Method: Formalizes "syntactic distance" to measure class separability via symmetry of operations. Constructs a visual self-referential task with maximum-variance binary noise: positive samples have a closed square, negative samples have one flipped boundary pixel, ensuring zero syntactic distance. Tests ResNets and Vision Transformers on this task.

Result: A phase-transition phenomenon occurs: accuracy drops to random guessing once image scale crosses a critical point. Larger training sets and models delay but don't prevent collapse. ViTs collapse earlier than ResNets, despite global attention.

Conclusion: Current vision architectures have a structural capability boundary on global-concept tasks. General intelligence may require creating new descriptive language rather than reusing existing learned features.
...
TL;DR: Bit-ViP is a bit-plane-based image obfuscation scheme that uses Lorenz chaotic system and differential privacy to produce secure, non-invertible, yet trainable images for visual privacy in recognition tasks, validated on UCF101 and HMDB51 datasets.


<details>
  <summary>Details</summary>
Motivation: Computer vision applications (surveillance, social media) raise security and visual privacy concerns when data is stored on cloud servers. Existing obfuscation schemes are either vulnerable to model inversion attacks or generate non-trainable images with poor accuracy.

Method: A bit-plane-based obfuscation scheme (Bit-ViP) with an end-to-end obfuscation function that incorporates non-invertible noise from Lorenz's chaotic system and differential privacy to prevent image reconstruction.

Result: Validated on UCF101 and HMDB51 datasets; rigorous security analysis shows improvements over existing schemes against reconstruction attacks, pixel frequency, information entropy, and pixel inter-correlation attacks.

Conclusion: Bit-ViP effectively balances visual privacy preservation and usability (trainability) for image-based recognition tasks, outperforming prior obfuscation methods in security.
...
TL;DR: DIVE is the first zero-shot anomaly detection method designed for scenarios with limited auxiliary anomaly priors, using a shallow-and-deep text embedding injection strategy and a disentanglement mechanism to improve alignment, outperforming SOTA baselines by up to 47% across twelve datasets.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot anomaly detection models assume auxiliary data contains rich anomaly diversity, but real-world target domains exhibit far more complex and unpredictable variations. This paper addresses the performance degradation when limited anomaly priors are available.

Method: DIVE uses a shallow-and-deep text embedding injection strategy during visual encoding to learn abstract generic anomaly concepts, plus a disentanglement mechanism to resolve suboptimal alignment between visual embeddings (entangled with object semantics) and object-agnostic textual prompts.

Result: Under limited anomaly patterns in auxiliary data, DIVE outperforms SOTA by up to 16.2% and 28.5% on classification metrics, and 23.4%, 24.1%, and 47.0% on segmentation metrics across twelve datasets. It also maintains competitive performance with sufficient anomaly diversity.

Conclusion: DIVE effectively tackles zero-shot anomaly detection under limited anomaly priors and remains competitive when anomaly diversity is sufficient, demonstrating robustness across diverse settings.
...
TL;DR: EvLIR is a temporal-residual framework that uses ordered event camera data and a ConvGRU module to enhance low-light images, achieving state-of-the-art results on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Low-light image enhancement is ill-posed due to missing structure, saturated noise, and weak contrast. Prior event-based methods treat event voxel channels as unordered/static features, ignoring temporal evolution within event windows, thus weakening the useful temporal evidence from events.

Method: EvLIR preserves ordered temporal bins of event streams and introduces a Temporal Event Residual Module (TERM) using lightweight ConvGRU to encode short-window event dynamics. The temporal state is converted into bounded illumination correction for spatially adaptive Retinex-style illumination estimation and reliability-aware image-event restoration.

Result: On SDE and SDSD indoor/outdoor benchmarks, EvLIR achieves best results on 11/12 dataset-metric pairs, with average scores of 25.63 dB PSNR, 28.30 dB PSNR*, and 0.827 SSIM across four benchmarks.

Conclusion: Explicitly modeling temporal evolution of events (rather than treating them as static stacks) significantly improves low-light image enhancement, as demonstrated by EvLIR's state-of-the-art performance.
...
TL;DR: This paper introduces VG-GUIBench, a benchmark for evaluating whether MLLM-based GUI agents can follow video tutorials to complete tasks, and proposes TASKER, a keyframe extraction algorithm that improves performance on both VideoQA and video-guided agentic tasks.


<details>
  <summary>Details</summary>
Motivation: Existing VideoQA benchmarks mainly test shallow visual perception rather than deeper knowledge or procedural skill learning from video tutorials. There is a gap in evaluating whether MLLMs can generalize video-guided knowledge to downstream long-horizon agentic tasks.

Method: The authors propose VG-GUIBench, a benchmark for video-guided GUI tasks, and TASKER, a task-driven and scene-aware keyframe extraction algorithm that jointly considers task relevance and scene dynamics to identify informative frames.

Result: TASKER outperforms the best baseline by 2.0% on EgoSchema fullset and 1.8% on NExT-QA dataset, showing consistent improvements on both VideoQA and video-guided agentic benchmarks.

Conclusion: Keyframe extraction is critical for both VideoQA and video-guided agentic tasks, and generalized keyframe extraction methods like TASKER have strong potential for improving video understanding capabilities.
...
TL;DR: Miti360 is a comprehensive reforestation monitoring dataset from Kenya (Kieni Forest), featuring aerial/terrestrial imagery, ground truth, and weather data collected over two years, which improves DeepForest model performance by 12% in precision and 69% in recall.


<details>
  <summary>Details</summary>
Motivation: Existing ML training datasets for forest monitoring lack geographic diversity, particularly missing African forestry data, which limits model generalizability and hinders reforestation efforts in underrepresented regions like Sub-Saharan Africa.

Method: Data collected in a 770-ha reforested section of Kieni Forest, Kenya (March 2023–Feb 2025). Aerial: DJI Mavic 2 Pro → Agisoft Metashape stitched → ArcGIS Pro tiled. Terrestrial: smartphones + custom stereo cameras. Includes orthophotos/tiles with tree bounding boxes, single/stereo images, biophysical parameters, species, GPS coordinates, and historical weather data.

Result: Fine-tuning DeepForest on Miti360 improved box precision by 12% and box recall by 69% for tracking tree crowns across three years.

Conclusion: Miti360 addresses the Sub-Saharan Africa data gap, enabling ML training for tree censuses, species matching, growth modeling, and digital twin frameworks, ultimately advancing reforestation monitoring in underrepresented regions.
...
TL;DR: Proposes Platonic Representation Defense, a black-box test-time defense against backdoor attacks on SSL models that uses multiple independently trained encoders' representations to detect and purify poisoned representations without needing labels, attack patterns, or training data.


<details>
  <summary>Details</summary>
Motivation: SSL pretrained models are vulnerable to backdoor attacks, and existing defenses require labels, attack patterns, or training data, failing in fully black-box settings.

Method: Inspired by Platonic Representation Hypothesis (independently trained encoders converge toward compatible projections of the same reality). Defines a conditional energy function over source and reference representations, trained via noise-contrastive estimation for detection and denoising score matching for purification.

Result: Effective on multiple self-supervised encoders and 10+ attacks; achieves substantial gains in both detection and purification tasks.

Conclusion: The method is attack-agnostic, model-agnostic, and modality-agnostic, providing a robust test-time defense with theoretical guarantees (energy gap bounded by mutual information).
...
TL;DR: RBS replaces flat Gaussian splats with "Branes" (Gaussian-Hermite modes) for arbitrary-scale super-resolution, achieving better quality and speed by reducing overlapping primitives and using a quantum-inspired culling strategy.


<details>
  <summary>Details</summary>
Motivation: Standard 2D Gaussian Splatting (GS) uses smooth low-pass primitives, so modeling edges and fine textures requires many overlapping splats, creating rasterization bottlenecks.

Method: RBS introduces Branes—Gaussians augmented with internal Gaussian-Hermite modes where each mode has a distinct color coefficient. Zero-order recovers standard GS; higher orders capture high frequencies. A differentiable rasterizer with a quantum turning point culling strategy skips negligible regions.

Result: RBS improves reconstruction quality over implicit and GS baselines on standard ASR benchmarks, achieving a superior speed-quality trade-off compared to prior GS methods.

Conclusion: RBS demonstrates that richer primitive representations (Branes) can reduce primitive overlap, accelerate rasterization, and improve ASR performance, offering a better speed-quality balance.
...
TL;DR: A self-supervised pretraining framework that learns subsurface scattering light transport from just 8 high-frequency PSP images per view, using tailored augmentations and a dedicated cost function to achieve high-fidelity reconstructions with far fewer images than prior methods.


<details>
  <summary>Details</summary>
Motivation: To enable learning generalizable subsurface scattering representations from minimal input data, reducing the enormous image acquisition burden required by prior methods (orders of magnitude fewer images).

Method: (1) Stereo projector-camera setup capturing only 8 high-frequency PSP images per view; (2) Self-supervised pretraining with a tailored augmentation strategy for PSP-based SSS data; (3) Encoder-decoder architecture trained with a dedicated cost function for dense scattering footprint reconstruction.

Result: The tailored augmentation strategy significantly outperforms standard ImageNet-style augmentations for SSL pretraining. The model generalizes to unseen objects with complex geometry and materials, achieving high-fidelity reconstructions while requiring orders of magnitude fewer images than prior methods.

Conclusion: Self-supervised learning on minimal PSP input, combined with domain-specific augmentations and a specialized cost function, is an effective and data-efficient approach for learning subsurface scattering representations that generalize across diverse objects.
...
TL;DR: MIRROR introduces a geometric regularization (SI-GW) to transfer relational priors from language to visual representations in MLLMs, fixing the blind spot where projection-based alignment loses inter-concept relationships during cross-modal transfer.


<details>
  <summary>Details</summary>
Motivation: MLLMs inherit rich relational knowledge from language backbones but fail to apply these relationships in visual contexts because projection-based alignment only ensures individual visual tokens carry correct semantics, ignoring whether inter-concept relationships survive the language-to-vision transfer.

Method: Proposes MIRROR, using Semi-Inverse Gromov-Wasserstein (SI-GW) regularization—a geometric framework with a closed-form solution that aligns visual representations with language-derived relational priors. Includes layer-, head-, and token-level strategies for stable integration into decoder-only Transformers without extra parameters or inference cost.

Result: MIRROR improves relational consistency in visual understanding while preserving performance on general vision-language tasks, with efficient computation applicable to long token sequences.

Conclusion: The SI-GW framework effectively transfers relational structure from language to vision, addressing a fundamental blind spot in MLLM alignment without adding computational overhead or sacrificing general task performance.
...
TL;DR: CellDETR is a detection-guided framework built on Deformable DETR for scalable cell representation learning from whole-slide images, achieving state-of-the-art cell classification and demonstrating transferability across datasets.


<details>
  <summary>Details</summary>
Motivation: Existing pathology foundation models focus on patch and slide-level representations but neglect cell-level representation learning, limiting interpretability, biological discovery, and clinical translation.

Method: CellDETR employs Deformable DETR with location feature decoupling and box-constrained attention, plus contrastive learning for scalable pretraining from unlabeled WSIs and spatial transcriptomics-derived annotations.

Result: Outperforms SOTA in supervised cell classification on PanNuke; pretraining improves downstream classification; cross-dataset transfer shows biological relevance.

Conclusion: CellDETR offers a scalable route toward general cell-level representation learning for interpretable computational pathology.
...
TL;DR: RAHA proposes a rank-aware hyperbolic alignment method for vision-language dataset distillation that uses hierarchical geometry and explicit capacity control to improve cross-modal retrieval and transfer robustness under fixed budgets.


<details>
  <summary>Details</summary>
Motivation: Existing VLDD methods (e.g., LoRS) enforce full-dimensional alignment in Euclidean space, which is overly restrictive due to rank-deficient image-text correlation. LoRS relaxes alignment via low-rank factorization but fails to explicitly control dominant alignment capacity and structure in the representation space.

Method: RAHA lifts multimodal representations to hyperbolic space and uses asymmetric objectives: (1) enforce geodesic alignment in the shared low-dimensional range, (2) regularize the residual subspace to preserve modality-private diversity and improve transfer robustness.

Result: Competitive cross-modal retrieval performance and improved transfer indicators under fixed data and compute budgets on benchmark datasets.

Conclusion: Hyperbolic geometry with explicit alignment-capacity control is effective for vision-language dataset distillation, outperforming prior Euclidean-based methods.
...
TL;DR: MAVIN is the first framework for multi-shot audio-visual generation with narrative control, using boundary-aware attention, ID-aware propagation, and multi-agent scripting to achieve state-of-the-art performance in coherent film-like content creation.


<details>
  <summary>Details</summary>
Motivation: Existing generative models produce high-fidelity videos but struggle with complex narrative control required for coherent multi-shot audio-visual generation, suffering from temporal misalignment, limited controllability, and incomplete scripting.

Method: 1) Boundary-aware attention with hierarchical captions and token routing for temporal alignment; 2) ID-aware propagation using identity embeddings and masks for consistent appearance/timbre; 3) Multi-agent scripting pipeline for transforming user inputs into hierarchical captions; 4) Construction of MAVINSet dataset.

Result: State-of-the-art performance in multi-shot audio-visual generation, enabling integration into professional filmmaking workflows.

Conclusion: MAVIN opens a new avenue for integrating generative models into professional filmmaking by resolving key challenges in narrative control, temporal alignment, and multi-subject consistency.
...
TL;DR: VCS-SLAM improves semantic 3D Gaussian SLAM by weighting semantic observations based on geometric reliability (visibility, boundary evidence, ray conflict) rather than treating all priors equally, reducing artifacts and improving map quality.


<details>
  <summary>Details</summary>
Motivation: Existing semantic 3D Gaussian SLAM methods treat all 2D semantic priors as equally reliable, ignoring that occlusions, unsupported boundaries, and ambiguous ray geometry introduce persistent artifacts in the global map.

Method: VCS-SLAM evaluates geometric reliability of each semantic observation via visibility consistency, surface-supported boundary evidence, and ray-level conflict uncertainty, then uses a reliability-aware objective to suppress unreliable updates.

Result: On Replica, VCS-SLAM achieves improved semantic consistency, boundary preservation, and reconstruction quality. On ScanNet, it maintains competitive tracking performance under real RGB-D inputs.

Conclusion: Geometry-validated semantic evidence fusion effectively mitigates semantic artifacts in 3D Gaussian SLAM without sacrificing tracking performance.
...
TL;DR: RefineSplat is a framework that uses entropy-aware adaptive masking and density control to remove transient distractors from 3D Gaussian splatting, achieving state-of-the-art novel view synthesis on a new Ambiguous wild dataset.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to distinguish transient elements from static scenes when distractors share color or semantic resemblance with the background. The authors aim to build a robust system for identifying diverse ambiguous distractors in novel view synthesis.

Method: Two key components: (1) entropy-aware adaptive masking that leverages entropy and instance masks to capture ambiguous distractors, and (2) entropy-aware density control for aligning Gaussians using entropy-weighted positional gradients.

Result: RefineSplat achieves state-of-the-art performance on multiple datasets, including the newly introduced Ambiguous wild dataset (18 challenging scenes). It successfully produces distractor-free novel view synthesis.

Conclusion: Entropy-aware techniques are effective for handling color and semantic ambiguity in transient distractors, and the proposed framework outperforms prior approaches in ambiguous scenarios.
...
TL;DR: This paper introduces a patch-based spatiotemporal framework for weakly supervised video anomaly detection that jointly localizes anomalies in both space (where) and time (when), using a Proximity-Aware Top-k selection strategy to generate fine-grained spatial anomaly maps without bounding-box supervision, achieving state-of-the-art results across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing WSVAD methods focus only on temporal localization of anomalies, ignoring their spatial extent within frames. Spatial localization is critical for interpretability and real-world deployment.

Method: A patch-based spatiotemporal framework with grid-level patch features and region-level anomaly scores under MIL paradigm, plus a Proximity-Aware Top-k spatiotemporal selection strategy for fine-grained spatial anomaly maps without bounding-box supervision.

Result: Outperforms existing state-of-the-art methods across multiple benchmarks with substantial gains in spatiotemporal localization accuracy.

Conclusion: The proposed framework effectively bridges the gap between temporal and spatial anomaly localization in WSVAD, and the release of new annotations and models will support future research in this direction.
...
TL;DR: This paper proposes a BEHINT touch-detection framework for reconstructing keystrokes from over-the-shoulder video footage using four modalities (MediaPipe, HSV skin filtering, motion detection, Canny edge analysis). Results show F1-scores below 19% in staged settings and complete failure in real-world footage, with skin filters producing 57-205 false touch points per frame. The system does not achieve reliable keystroke reconstruction outside calibrated staged settings.


<details>
  <summary>Details</summary>
Motivation: To monitor human-computer interaction patterns without direct screen recording access, using video intelligence surveillance (VIDINT) on over-the-shoulder footage as an alternative vector for reconstructing typing sequences.

Method: Four parallel detection modalities: (1) MediaPipe anatomical hand landmarks, (2) HSV skin color filtering, (3) temporal frame differencing for motion detection, (4) shape-guided Canny edge analysis. Relative touch coordinates mapped to reference screen layout for keystroke reconstruction.

Result: Motion-Only F1=18.5%, Edge-Only F1=18.2%, combined multi-modal F1=16.7%, sequence similarity 3.0% on iOS passcode layout. In real-world footage, detector emits median 57 (peak 205) false touch points per frame. System fails outside staged settings.

Conclusion: The proposed keystroke reconstruction system does not achieve reliable performance outside calibrated staged settings. The staged result does not survive contact with uncontrolled footage.
...
TL;DR: AI surveillance anomaly detection achieves high ROC-AUC within the same camera/scene, but cross-dataset performance drops to near chance (0.499 AUC), showing that current benchmarks overstate real-world reliability.


<details>
  <summary>Details</summary>
Motivation: To audit whether standard video anomaly detection benchmarks, which train and test on the same camera and scene, reflect deployable reliability in real-world surveillance settings.

Method: Built unsupervised normality models using frozen off-the-shelf embeddings (CLIP, DINOv2, ResNet-50, EfficientNet-B0) and nearest-neighbour distance; tested same-dataset vs. cross-dataset (4 real datasets, 4 backbones); validated with PaDiM-style Mahalanobis detector.

Result: Same-dataset AUC averages 0.704, cross-dataset AUC drops to 0.499 (chance); DINOv2 showed best same-dataset (0.901 on Ped2) but largest cross-dataset drop; false-alarm rate up to ~31,931 per hour; gap reproduced with alternative detector (0.202 vs 0.208).

Conclusion: Current benchmark numbers are calibrated laboratory results that overstate real-world reliability; cross-scene generalization is unsolved and performance is no better than chance outside the training environment.
...
TL;DR: A feed-forward framework that decomposes 3D scenes into instance-structured token groups from unposed multi-view images, enabling object-centric reconstruction, segmentation, and manipulation without 3D annotations.


<details>
  <summary>Details</summary>
Motivation: 3D scene understanding should be object-centric rather than primitive-based, but existing feed-forward methods output unstructured points/Gaussians, requiring post-hoc object recovery. The paper aims to make object instances a native interface of the representation.

Method: Proposes a two-level factorization: instance tokens (entity-level identity) paired with anchor tokens (local geometry/appearance), decoded into 3D Gaussians. Learned via differentiable rendering with joint reconstruction and segmentation supervision, requiring no 3D annotations.

Result: Surpasses per-scene optimization baselines in class-agnostic instance segmentation while remaining competitive in novel view synthesis. Enables instance-level editing (removal, translation, insertion) and efficient open-vocabulary 3D instance retrieval.

Conclusion: Token groups serve as a native object-centric representation that unifies reconstruction, segmentation, editing, and retrieval in a single feed-forward framework.
...
TL;DR: MotionAtlas is a system for detailed, region-aware motion captioning in videos, including a benchmark (MotionAtlas-Bench with 2,073 MCQs), a scalable data pipeline (159k samples via self-bootstrap refinement), and powerful Video-MLLMs that outperform baselines like Qwen3-VL-4B by 5.2% on general motion benchmarks.


<details>
  <summary>Details</summary>
Motivation: Conventional global motion captioning datasets suffer from visual clutter and motion entanglement; there is a need for region-aware motion captioning that focuses on specific spatiotemporal regions to enable precise, quantifiable evaluation.

Method: (1) MotionAtlas-Bench: 2,073 multiple-choice questions for fine-grained motion understanding. (2) A scalable data pipeline using self-bootstrap refinement to suppress hallucinations, producing 159k high-quality samples. (3) A tailored training data composition strategy applied to diverse Video-MLLMs (Molmo2, Qwen3-VL).

Result: MotionAtlas-4B surpasses Qwen3-VL-4B by an average of 5.2 percentage points across general motion benchmarks, with consistent improvements across diverse baseline models.

Conclusion: Region-aware motion captioning with high-quality data and tailored training strategies significantly improves fine-grained video motion understanding; benchmark, data, and code are publicly released.
...
TL;DR: GarmentZoom enhances full-view garment photos using unaligned close-ups as references, enabling seamless zoom-and-pan without per-instance fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Online garment listings show overview and close-up photos separately, forcing users to switch views and breaking browsing continuity.

Method: A single model trained on diverse garments that synthesizes details from spatially unaligned close-up references across a continuous 3-20× scale range, without requiring alignment or per-instance fine-tuning.

Result: Matches the quality of per-instance fine-tuning methods at a fraction of the training cost.

Conclusion: GarmentZoom enables seamless zoom-and-pan exploration of garment photos by enhancing full views with close-up details in a unified, efficient model.
...
TL;DR: The paper identifies that fine-grained visual descriptions from MLLMs are more error-prone than coarse ones, proposes GranFact benchmark with hierarchy-aware evaluation, and introduces a reliability-prioritized preference optimization method to improve fine-grained generation without sacrificing reliability.


<details>
  <summary>Details</summary>
Motivation: MLLMs are expected to generate fine-grained visual descriptions, but fine-grained generation is inherently more unreliable and error-prone than coarse-grained generation, posing a reliability challenge.

Method: The authors develop GranFact (a granularity-aware benchmark with coarse-to-fine annotations), design a hierarchy-aware evaluation algorithm, and propose a reliability-prioritized preference optimization method based on DPO that penalizes unreliable fine-grained claims while rewarding reliable specificity.

Result: Experiments on GranFact show that the proposed method improves fine-grained generation while preserving reliability.

Conclusion: Models should generate the finest description that remains reliable rather than simply produce more specific outputs; the proposed optimization method effectively balances specificity and reliability.
...
TL;DR: ReMAP-PET is a framework that learns structured metabolic representations from PET scans by supervising a 3D ResNet-50 with regional SUVR profiles, achieving superior performance on SUVR prediction, retrieval, and downstream clinical tasks while enabling PET-to-report generation.


<details>
  <summary>Details</summary>
Motivation: Existing 3D brain foundation models treat PET as generic volumetric data, ignoring the structured regional metabolic information that distinguishes it from structural neuroimaging. This limits PET-specific understanding.

Method: A partially-tuned MedicalNet 3D ResNet-50 is supervised with brain regional SUVR profiles using joint regression and contrastive objectives. The metabolic embedding is then aligned with frozen BioClinicalBERT via contrastive learning for language compatibility.

Result: ReMAP-PET achieves 0.070 SUVR MAE and 77.8% PET SUVR Recall@1 on 1015 paired samples, outperforming five frozen pretrained baselines. Linear probing shows clinically relevant information is retained for diagnostic and cognitive tasks.

Conclusion: Grounding PET encoders in regional metabolic semantics yields representations that are structured, interpretable, and language-compatible, pointing to a new direction for metabolic-aware PET understanding.
...
TL;DR: ScAle is an ultra-lightweight method that improves spatial reasoning in VLMs by learning scalar coefficients to modulate last-token attention and MLP activations in frozen backbones, achieving up to 134.1% accuracy gains with only 1K trainable parameters.


<details>
  <summary>Details</summary>
Motivation: Spatial reasoning remains challenging for VLMs, and existing improvements require fine-tuning with many additional parameters. The authors observed that rescaling activations in selected transformer layers can significantly affect performance without modifying pretrained weights.

Method: ScAle learns a small set of scalar coefficients to modulate last-token attention and MLP activations in a fully frozen backbone, requiring only 1K trainable parameters.

Result: Achieves up to 134.1% relative accuracy gains on spatial reasoning benchmarks using only 1K parameters, recovering substantial PEFT performance while preserving non-spatial VQA accuracy.

Conclusion: Bounded activation reweighting offers a simple, architecture-agnostic, and highly parameter-efficient alternative for adapting pretrained VLMs to spatial reasoning tasks.
...
TL;DR: SonoCLIP is the first million-scale region-controllable fetal ultrasound vision-language foundation model that integrates segmentation masks as visual prompts in CLIP, using a sigmoid-based pairwise contrastive loss for joint global-local representation learning on a newly curated 1.44M-image multimodal dataset, achieving superior zero-shot transfer performance.


<details>
  <summary>Details</summary>
Motivation: Ultrasound imaging faces challenges like speckle noise, acquisition variability, and subtle anatomical boundaries, causing high inter-observer variability. Existing CLIP-based models rely on global image-text alignment, limiting sensitivity to clinically important local structures.

Method: (1) Integrates segmentation masks as mask-channel visual prompts within the vision encoder of CLIP for joint global-local contrastive learning. (2) Introduces a sigmoid-based pairwise contrastive loss for stable large-scale supervision. (3) Curates a 1.44M-image multimodal fetal ultrasound dataset spanning 24 standard planes.

Result: Superior zero-shot transfer performance under both global and mask-guided inference across cross-center evaluations.

Conclusion: SonoCLIP establishes a controllable and clinically oriented foundation model for fetal ultrasound analysis, advancing region-controllable vision-language pretraining in the ultrasound domain.
...
TL;DR: This paper introduces MultiDepth-3k, a benchmark for evaluating how monocular depth models handle layered geometry (e.g., transparent glass with visible background). It shows different models prefer different depth layers, and Laplacian Visual Prompting can shift these preferences. The work argues depth estimation should be treated as an ambiguity-aware problem with multiple valid interpretations rather than a single ground truth.


<details>
  <summary>Details</summary>
Motivation: Monocular depth estimation traditionally reduces a 3D scene to one scalar depth per pixel, ignoring layered geometry (e.g., transparent surfaces). This creates ambiguity in what the "correct" depth is, leading to inconsistent supervision and evaluation.

Method: The authors introduce MultiDepth-3k (MD-3k), a sparse two-layer ordinal benchmark to measure depth-layer preference and multi-layer spatial relationship accuracy (ML-SRA). They evaluate leading depth foundation models under RGB input and apply Laplacian Visual Prompting (LVP), a training-free spectral transformation, to shift layer preferences.

Result: Different depth models exhibit diverse layer preferences on MD-3k, showing the same geometry is resolved differently. LVP substantially changes reported layers for some frozen models. The best RGB/LVP pair (DAv2-L) achieves 75.5% ML-SRA.

Conclusion: Depth foundation models express complementary geometric hypotheses not visible under standard RGB inference. The community should rethink depth supervision/evaluation as ambiguity-aware, treating multiple valid 3D interpretations as structure to be measured and preserved.
...
TL;DR: Geospatial foundation models (Prithvi, SpectralGPT, SatMAE) degrade sharply in agricultural tasks when tested on unseen regions, predicting only common crops and missing rare ones.


<details>
  <summary>Details</summary>
Motivation: The geographic transferability of geospatial foundation models has not been systematically tested, especially in agriculture applications.

Method: A controlled benchmark evaluating three models (Prithvi, SpectralGPT, SatMAE) on multi-temporal crop segmentation and change detection across four U.S. states, with train/validation/test splits assigned to separate regions.

Result: All three models degrade sharply under regional distribution shift, predicting only the most common crops while missing rare ones. Fitting models to a shared input format affects each model differently, complicating direct architectural comparison.

Conclusion: Current geospatial foundation models have key limitations for agriculture, and region-aware evaluation should become a necessary standard.
...
TL;DR: MatMMExtract is an open-source pipeline that decomposes compound scientific figures into sub-panels with structured annotations, producing MatSciFig—a dataset of 391,606 panel-level image-text pairs from materials science literature.


<details>
  <summary>Details</summary>
Motivation: Materials science figures contain rich experimental knowledge, but most are compound figures (multiple sub-panels under one caption), making direct image-text pairing unreliable and inaccessible to AI at scale.

Method: MatMMExtract decomposes compound figures using a fine-tuned YOLO12-m detector (MaterialScope dataset), then generates grounded annotations via LLM (Gemini 3.1 Flash Lite) guided by a materials science taxonomy.

Result: 391,606 panel-level pairs from 14,810 articles; detector achieves mAP_50 of 0.9227; 82% of outputs rated good with 4.8% hallucination rate; dual-encoder retrieval achieves 4.4× improvement in R@1 over CLIP.

Conclusion: The pipeline effectively unlocks visual knowledge from materials science literature at scale, and the released dataset demonstrates immediate utility for vision-language learning tasks.
...
TL;DR: PoseShield is a neural collision constraint operating directly in SMPL pose space that corrects self-collisions in human poses and motion sequences using Eikonal-regularized constrained optimization, achieving 95.8% success rate.


<details>
  <summary>Details</summary>
Motivation: Self-collision (self-penetration) in SMPL-based human pose estimation and motion generation remains a persistent challenge, causing physically implausible results under extreme articulations or stochastic motion synthesis.

Method: Formulates collision correction as a constrained optimization problem directly in SMPL pose space, connects the learned constraint with the Eikonal equation, and enforces Eikonal regularization for non-vanishing gradients near collision boundaries.

Result: Achieves 95.8% success rate on a newly constructed SMPL pose benchmark, outperforming state-of-the-art baselines.

Conclusion: The same learned constraint generalizes to human motion sequences as a generator-agnostic post-hoc collision corrector without retraining, offering a theoretically grounded and numerically stable approach compared to prior mesh-space or heuristic methods.
...
TL;DR: BrainPICM is a self-supervised framework for brain network analysis that uses progressive individualized community-aware masking via unbalanced optimal transport to capture both stable modular structures and subject-specific variations, outperforming existing methods on three fMRI datasets.


<details>
  <summary>Details</summary>
Motivation: Existing SSL methods for brain network analysis rely on generic masking strategies that overlook the heterogeneity of modular community structure across individuals and neurological conditions, failing to capture subject-specific functional organization.

Method: (1) Progressive individualized community-aware masking via unbalanced optimal transport for soft ROI-to-community mapping with confidence scores. (2) Curriculum-style masking that gradually incorporates low-confidence (potentially pathological) regions. (3) Deviation-aware aggregation module measuring mass redistribution relative to a population template.

Result: Outperforms state-of-the-art supervised and SSL methods on three fMRI datasets (ABIDE-I, ADHD-200, ADNI) in diagnostic accuracy, producing more functionally consistent and generalizable representations.

Conclusion: Explicitly injecting modular community structure into masked modeling yields better representations, and the approach enables interpretable quantification of functional reorganization.
...
TL;DR: MF-UAVPose6D is a model-free monocular 6-DoF pose estimation framework for fixed-wing UAVs that uses only a single RGB image and camera intrinsics, employing heatmap-guided center localization, a Perspective-Aware Module, Dynamic Topological Sampling, and decoupled translation-rotation decoding to achieve accurate pose estimation without CAD models or keypoint priors.


<details>
  <summary>Details</summary>
Motivation: Estimating 6-DoF poses of non-cooperative UAVs is essential for airspace situational awareness and counter-UAV operations, but existing methods rely on CAD models or keypoint priors that are unavailable for unknown targets, creating a need for a model-free approach.

Method: The framework uses heatmap-guided center localization for stable target anchoring, a Perspective-Aware Module (PAM) to model observation-ray priors, Dynamic Topological Sampling (DTS) to extract weak structural cues from wings/fuselage/tail, and a decoupled translation-rotation pose decoding mechanism. The FW-UAV6DPose synthetic dataset was also constructed.

Result: The method achieves accurate and efficient monocular 6-DoF pose estimation without CAD models, showing strong robustness in long-range rotation estimation, depth recovery, and joint pose evaluation.

Conclusion: MF-UAVPose6D provides a viable model-free solution for fixed-wing UAV pose estimation from a single RGB image, overcoming the limitations of existing model-based or keypoint-matching approaches for non-cooperative targets.
...
TL;DR: This paper shows that internal activations of OpenVLA (a Vision-Language-Action model) contain linearly decodable signals for predicting near-term task failure under perceptual shifts like occlusion. A logistic probe at layer 16 achieves high AUROC (0.972) and AUPRC (0.352) for failure prediction within 15 steps, outperforming baselines. The result is limited to fixed-policy LIBERO rollouts with no causal mechanism or deployed recovery system.


<details>
  <summary>Details</summary>
Motivation: Vision-Language-Action models are hard to diagnose when visual conditions change; the authors aim to detect internal signals that predict failure without modifying the policy, enabling better monitoring under distribution shift.

Method: The paper freezes OpenVLA, logs feedforward activations during LIBERO manipulation rollouts under occlusion (and other shifts), and fits lightweight logistic probes post-hoc to predict failure within a 15-step horizon. Comparisons include mean difference direction and action disagreement baselines, plus layer-wise decodability analysis.

Result: Under occlusion, success rate drops from 57% to 17%. Layer 16 logistic probe achieves AUROC 0.972 and AUPRC 0.352, outperforming baselines. Layer 16 is strongest, layer 8 is informative, layer 10 is weaker. The monitor remains above random under camera jitter but occlusion is the main stress test.

Conclusion: OpenVLA internal states encode failure-relevant information under controlled perceptual shifts, but the findings are limited—no causal mechanism, no generalization to held-out tasks, and no deployable recovery system is established.
...
TL;DR: UniVAD v2 proposes a two-sided support-conditioned boundary construction framework for unified few-shot visual anomaly detection, using Optimal Transport-based Relational Modeling and adaptive coordination on the normal side, plus a Few-Shot Abnormal Reference module on the abnormal side, achieving 84.5% image-level AUC (1N-shot) and up to 85.7% (1N+1A-shot) across six datasets.


<details>
  <summary>Details</summary>
Motivation: Existing few-shot unified anomaly detection methods rely only on normal-side evidence, struggle to jointly capture local correspondences and global support-query relations, and provide limited abnormal-side evidence for deployment-specific tolerance.

Method: UniVAD v2 extends UniVAD with three modules: (1) Optimal Transport-based Relational Modeling (OTRM) for support-query matching via transport-style allocation; (2) Adaptive Coordination for Retrieval and Relational Modeling (ACRRM) to fuse retrieval and relational evidence; (3) Few-Shot Abnormal Reference (FAR) module to convert optional abnormal references into rejection-side evidence for boundary adjustment.

Result: Improves mean image-level AUC over UniVAD from 83.0% to 84.5% (1N-shot) and 85.7% (1N+1A-shot). On MVTec-AD-SS, achieves 96.2% image-level AUC and 96.9% pixel-level AUC.

Conclusion: Abnormal references enable controllable boundary customization without retraining, and the proposed two-sided approach shows strong cross-domain generalization across industrial, logical, and medical anomaly detection.
...
TL;DR: An automatic system for pulmonary disease recognition in chest X-rays using geometric normalization of lung regions, combining landmark detection, geometric warping, and a classifier, achieving high accuracy and demonstrating that anatomical alignment provides artifact-resistant representation.


<details>
  <summary>Details</summary>
Motivation: To improve pulmonary disease classification from chest X-rays by addressing the issue that raw images may achieve high accuracy partly due to acquisition artifacts rather than true pathological features.

Method: Three-module pipeline: (1) ResNet-18 landmark detector with coordinate attention (ensemble of 4 models + TTA) predicting 15 lung-contour landmarks; (2) Geometric normalizer using Generalized Procrustes Analysis, Delaunay triangulation, and piecewise affine warping; (3) ResNet-18 classifier with transfer learning and SAHS contrast enhancement for 3-class classification (COVID-19, Viral Pneumonia, Normal).

Result: Landmark detector achieves 3.61 pixel mean localization error. Normalized-image classifier achieves 98.60±0.26% accuracy and 98.00% F1-Macro on COVID-19 Radiography Database. Normalized images outperform artifact-masked/cropped unaligned images (98.60% vs 96.24% on COVID-19 DB; 94.67% vs 94.17% on mixed adult-pediatric dataset).

Conclusion: Geometric normalization of lung regions provides a more controlled and artifact-resistant representation for pulmonary disease recognition, reducing the influence of acquisition artifacts while maintaining or improving diagnostic performance.
...
TL;DR: This paper introduces AerialMetric, a benchmark dataset for monocular metric depth estimation in aerial UAV imagery, consisting of 52K real-world and 16K synthetic image-depth pairs from diverse sources. The authors systematically evaluate existing models, investigate the impact of aerial parameters, and achieve state-of-the-art performance by fine-tuning on their dataset.


<details>
  <summary>Details</summary>
Motivation: Existing monocular metric depth estimation models trained on street-view and indoor datasets perform poorly on aerial UAV imagery due to significant domain gaps. There is a lack of dedicated benchmark datasets for aerial viewpoints.

Method: The authors construct AerialMetric, a dataset with four complementary subsets: real-world photogrammetry data, controlled aerial acquisition, photorealistic synthetic scenes, and in-the-wild Internet imagery. They systematically evaluate SOTA models and fine-tune a representative metric depth model on this dataset.

Result: AerialMetric provides 52K real-world and 16K synthetic image-depth pairs with reliable metric ground truth. Fine-tuning on this dataset achieves state-of-the-art performance across diverse aerial imagery.

Conclusion: AerialMetric facilitates the adaptation and evaluation of monocular metric depth estimation for aerial UAV viewpoints, bridging the domain gap from ground-level to aerial scenes.
...
TL;DR: HTC-SGA Former is a lightweight hybrid Transformer-CNN framework (0.81M parameters) for coronary DSA vessel segmentation, using MS-GLWA for global-local attention, SGFA for weak-vessel enhancement, and BWACL for boundary-aware loss, outperforming 14 SOTA methods.


<details>
  <summary>Details</summary>
Motivation: Coronary DSA vessel segmentation is challenged by thin low-contrast vessels, background interference, and severe class imbalance. Existing methods cannot balance global contextual reasoning with preservation of weak vessels, vessel continuity, and fine boundaries.

Method: A lightweight hybrid Transformer-CNN framework with: (1) CNN encoder for local vessel morphology, (2) Transformer decoder for contextual modeling, (3) Multi-Scale Global-Local Window Attention (MS-GLWA) block, (4) Self-Guided Feature Attention (SGFA) module for weak-vessel enhancement, and (5) Boundary-Weighted Adaptive Compound Loss (BWACL) focusing on thin-vessel boundaries and adaptive balance.

Result: Outperforms 14 SOTA segmentation methods on private right/left coronary DSA datasets with only 0.81M parameters. BWACL improves performance over BCE and Dice losses across four encoder-decoder architectures, showing cross-backbone applicability.

Conclusion: HTC-SGA Former improves thin-vessel recovery, vessel continuity, and boundary localization via complementary global-local modeling, vessel-focused refinement, and adaptive optimization, enabling reliable and efficient coronary vessel analysis for future computer-assisted cardiovascular interventions.
...
TL;DR: LEIQ-Assessor is a multi-task learning model for assessing low-light image enhancement quality, using SigLIP2 Vision Transformer backbone to predict MOS and six perceptual sub-attributes, achieving second place in QoMEX 2026 Grand Challenge.


<details>
  <summary>Details</summary>
Motivation: Low-light image enhancement algorithms often introduce artifacts (noise, color shift, over-exposure), creating a need for reliable IQA metrics to evaluate enhancement quality.

Method: Uses pre-trained SigLIP2 Vision Transformer backbone with multi-task learning to jointly predict MOS and six sub-attributes (lightness, color fidelity, noise, exposure, naturalness, content recovery), optimized via PLCC loss.

Result: Significantly outperforms existing no-reference IQA models and hand-crafted descriptors on MLE benchmark; achieved second place in the QoMEX 2026 Grand Challenge.

Conclusion: Multi-task learning with correlated perceptual attributes improves quality assessment for low-light enhanced images, validated by competitive challenge ranking.
...
TL;DR: The paper proposes MR-IQA, a quality-margin optimization framework that unifies regression and ranking for blind image quality assessment (BIQA) by revealing that both paradigms fundamentally fit pairwise relational distances (quality margins). Experiments show competitive performance across six benchmarks.


<details>
  <summary>Details</summary>
Motivation: Regression and ranking are commonly used together in BIQA but their relationship remains empirically understood and theoretically underexplored. The paper aims to theoretically bridge these two paradigms through the concept of "quality margin."

Method: The authors derive that both regression and ranking optimize quality margins: regression fits margins induced by score endpoints, while ranking fits transformed/sign-level margins via preference probabilities. Based on this, they propose MR-IQA, which uses reinforcement learning to sample quality scores and optimize pairwise margin errors as policy rewards, explicitly modeling quality structure.

Result: Competitive performance on six BIQA benchmarks; strongest average PLCC/SRCC compared to regression- or ranking-based RL methods in controlled comparisons.

Conclusion: Quality margin serves as a theoretical bridge unifying regression and ranking, providing new insights for quality-structure modeling in BIQA and potentially beyond.
...
TL;DR: TopoAgent is an LLM-based agentic framework that automates topological data analysis for medical images by selecting optimal topological descriptors and generating feature vectors without task-specific training.


<details>
  <summary>Details</summary>
Motivation: Existing TDA methods default to a single fixed topological descriptor (e.g., persistence images), ignoring the diversity of topological representations. No LLM-based agentic framework exists that can automatically determine the most suitable topological descriptor for a given medical image dataset.

Method: TopoAgent uses a Perception-Reasoning-Action-Reflection loop with 21 domain-specific tools, dual memory for experience accumulation, and a skill set distilled from systematic evaluation of 15 topological descriptors across 26 datasets with six classifiers.

Result: TopoAgent can analyze input images, reason about optimal topological descriptors, and determine the best descriptor and its configuration, all without task-specific training.

Conclusion: TopoAgent fills a gap by providing an automated, LLM-driven approach to topological descriptor selection in medical image analysis, leveraging diverse topological representations beyond conventional single-descriptor defaults.
...
TL;DR: UrbanCDNet is a Siamese CNN for building change detection in Korean urban scenes, achieving 0.7511 F1 and 0.6014 IoU, significantly outperforming baselines especially on sparse-change and high-photometric-gap subsets.


<details>
  <summary>Details</summary>
Motivation: Urban building change detection from bi-temporal aerial imagery is important for redevelopment monitoring, infrastructure management, and unauthorized-construction screening, but Korean urban scenes remain difficult due to sparse changed regions, strong appearance variation between dates, and the need for footprint-level outputs rather than coarse blobs.

Method: UrbanCDNet is a task-specific Siamese CNN combining appearance-robust multi-cue comparison, alignment-aware middle-scale differencing, lightweight context refinement, scene calibration, and auxiliary boundary supervision.

Result: On a corrected AIHub-based Korean benchmark (3,998 train, 503 val, 499 test pairs), UrbanCDNet achieves 0.7335 precision, 0.7696 recall, 0.7511 F1, 0.6014 IoU. It outperforms Siamese U-Net (0.7108 F1, 0.5514 IoU) and ChangeFormer-MIT-B0 (0.7107 F1, 0.5512 IoU). Gains are concentrated on sparse-change (F1 0.4765→0.6175) and high-photometric-gap (0.6349→0.7285) subsets, with improved boundary F1 (0.3445→0.4447) and object F1 (0.0690→0.2258).

Conclusion: On this Korean benchmark, task-shaped temporal comparison and boundary-aware supervision matter more than generic model scale alone.
...
TL;DR: This paper introduces a method to merge multiple 3D Gaussian Splatting scenes by reformulating registration as a graph registration problem using 3D scene graphs, with a self-supervised test-time optimization to reduce occlusion artifacts like hollows and floaters.


<details>
  <summary>Details</summary>
Motivation: Merging multiple 3DGS scenes is essential for large-scale 3D mapping and long-term map management, but existing solutions have limitations: learning-based methods require large datasets, and image-based optimization methods depend on coarse initialization and expensive refinement.

Method: Constructs a 3D scene graph from a 3DGS and its rendered images, reformulating 3DGS registration as a graph registration problem. The graph captures higher-level semantic and structural context. Additionally, a Self-Supervised Test-Time Optimization refines merged Gaussians to preserve visual consistency and reduce occlusion artifacts.

Result: Demonstrates competitive registration accuracy and merged scene rendering quality on both real and synthetic benchmarks.

Conclusion: The proposed graph-based registration with self-supervised optimization effectively addresses limitations of prior 3DGS merging methods, achieving better unified scene construction.
...
TL;DR: OP3DSG is an open-vocabulary, part-aware 3D scene graph generation framework that unifies objects, interactive parts, spatial/functional relations, and affordances, achieving SOTA performance on the new UniGraph3D benchmark and demonstrating effectiveness in real-world robotics tasks.


<details>
  <summary>Details</summary>
Motivation: Existing 3D scene graph approaches remain object-centric and encode limited relational information, restricting their applicability in real-world scenarios requiring fine-grained understanding.

Method: Integrates object-part knowledge-guided detection with part-aware 3D fusion, and employs a geometry-initialized prior graph with LLM-based refinement to reduce spurious relational predictions.

Result: Achieves state-of-the-art performance on the introduced UniGraph3D benchmark and demonstrates effectiveness as a perception backbone in diverse real-world robotics tasks.

Conclusion: OP3DSG advances 3D scene understanding by enabling fine-grained, part-aware, and relation-rich graph construction, with strong real-world applicability in robotics.
...
TL;DR: UniTriSplat unifies 3D Gaussian Splatting across different camera types (perspective, fisheye, omnidirectional) by reformulating the rendering process on a unit sphere using HEALPix discretization, enabling consistent performance and improved quality regardless of camera model.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS frameworks are camera-specific and suffer from inconsistent solid-angle sampling, leading to degraded performance when used with heterogeneous camera models (perspective, fisheye, omnidirectional).

Method: Reformulates Gaussian splatting on the unit sphere using HEALPix discretization (equal-area spherical grid), derives forward rendering and gradient propagation in the spherical radian domain, and introduces a HEALPix-aware SSIM loss that respects spherical neighborhood structure.

Result: Consistently improves cross-camera generalization while preserving geometric fidelity and rendering quality across diverse camera models, from narrow-FoV images to full 360-degree panoramas.

Conclusion: UniTriSplat provides a unified, camera-agnostic 3DGS framework that achieves uniform optimization behavior and superior perceptual reconstruction across all camera types.
...
TL;DR: CRG is a training-free method that adaptively calibrates negative guidance per diffusion step via closed-form constrained updates, effectively removing unwanted concepts while preserving image quality and avoiding the safety-fidelity trade-off of fixed-weight approaches.


<details>
  <summary>Details</summary>
Motivation: Existing inference-time safety controls (e.g., negative guidance with fixed weights or dynamic reweighting) suffer from a safety-fidelity trade-off — either over-suppressing content causing artifacts/prompt drift, or under-suppressing content failing under attacks.

Method: CRG estimates unwanted-concept presence from the model's noise predictions per diffusion step, then adaptively calibrates negative guidance via a closed-form constrained update that enforces a target presence threshold while minimally perturbing the conditional trajectory.

Result: CRG reduces attack success rates across red-teaming benchmarks while preserving benign fidelity, and extends to suppressing artist style and violence without fine-tuning or external classifiers.

Conclusion: Training-free adaptive calibration of negative guidance at each step offers a robust and generalizable solution for concept removal in diffusion models without sacrificing image quality.
...
TL;DR: OPPO is an evidence-aware alignment method that reduces hallucination in MLLMs by learning preferences over visual evidence strength rather than just response quality, outperforming baselines across benchmarks.


<details>
  <summary>Details</summary>
Motivation: MLLMs hallucinate because their generation preferences are insufficiently calibrated to visual evidence, causing them to rely on linguistic priors instead of faithful visual grounding. The authors observe that strengthening query-relevant visual evidence via the model's own attention improves generation accuracy, indicating the issue is not missing perception but insufficient trust in attended evidence.

Method: OPPO (Oriented Pickup Preference Optimization) is an evidence-aware alignment objective that contrasts the same faithful response under stronger, anchored, and weaker-evidence views, turning naive visual preference into ordered visual-evidence alignment. It is combined with fine-grained span-level and token-level regularization for training stability. A theoretical analysis shows ordered evidence margins induce a positive lower bound on local visual sensitivity.

Result: Extensive evaluations across hallucination and general-purpose benchmarks demonstrate that OPPO consistently outperforms baseline methods.

Conclusion: The paper concludes that explicit alignment over visual evidence strength, rather than just response quality, effectively reduces hallucination in MLLMs, supported by both empirical results and theoretical analysis.
...
TL;DR: ConsistRoll injects cross-view consistency into MLLM RL training by placing original and augmented views in the same GRPO group and assigning joint rewards only when paired outputs are both correct and consistent, improving multimodal reasoning without extra overhead.


<details>
  <summary>Details</summary>
Motivation: Standard RLVR and data augmentation treat transformed views independently with pointwise rewards, lacking cross-view consistency — a key inductive bias that semantically invariant views of the same instance should yield the same answer. This missing bias limits generalization in MLLMs.

Method: ConsistRoll reuses GRPO's group-sampling mechanism: it puts original and semantically invariant transformed views into the same generation group, and assigns a joint reward only when paired completions are both correct and consistent, turning consistency into an online credit-assignment signal.

Result: ConsistRoll achieves robust improvements across math, general-purpose, and hallucination benchmarks in multimodal reasoning, without extra generation overhead or annotations. It introduces a cross-view correction term that penalizes view dependence and alleviates advantage collapse.

Conclusion: Cross-view consistency is a valid inductive bias for MLLMs, and ConsistRoll effectively injects this bias into RLVR training, outperforming standard DA-based approaches by providing a stronger generalization signal.
...
TL;DR: Nemotron-Labs-Diffusion-Image is a masked discrete diffusion model for text-to-image synthesis that introduces a token-editing mechanism for dynamic revision during inference and a Grouped Cross-Entropy (GCE) loss to handle sparse training signals from large vocabularies, achieving strong results on GenEval (0.90), DPG (86.9), and HPSv3 (10.76).


<details>
  <summary>Details</summary>
Motivation: Standard masked discrete diffusion models cannot revise already-unmasked discrete tokens (lacking self-correction like continuous diffusion models), and larger vocabulary sizes cause sparse per-token training signals that hinder optimization.

Method: (1) Token-editing mechanism allowing dynamic revision of unmasked tokens during inference; (2) Grouped Cross-Entropy (GCE) loss that provides positive signals to tokens neighboring the ground truth in embedding space; (3) A custom fused operator for GCE to reduce VRAM usage with large vocabularies.

Result: Achieves 0.90 on GenEval, 86.9 on DPG, and 10.76 on HPSv3, demonstrating substantial improvements in image fidelity and training efficiency.

Conclusion: The proposed innovations effectively overcome the key limitations of masked discrete diffusion models, closing the gap with continuous diffusion models while improving training efficiency.
...
TL;DR: This paper introduces a new cross-view geo-localization benchmark for lunar surface exploration, featuring 10,438 rendered 360° panoramas matched with overhead lunar terrain tiles, and demonstrates that transformer-based localization methods work effectively for planetary surfaces as an alternative to GNSS.


<details>
  <summary>Details</summary>
Motivation: Global position awareness on planetary surfaces is challenging because satellite-based positioning systems are unavailable and onboard odometry drifts over time. Aligning orbital maps with surface observations is difficult due to viewpoint differences, low texture, repetitive terrain, and illumination changes.

Method: The authors built a cross-view geo-localization benchmark using physically rendered 360° surface panoramas and overhead tiles from a high-resolution lunar terrain model. The dataset contains 10,438 ground-to-overhead image pairs with off-center localization support, and they trained a state-of-the-art transformer-based method from scratch.

Result: Learning-based cross-view localization methods (transformer-based) can be successfully applied to planetary surfaces, achieving measurable retrieval accuracy on the lunar benchmark.

Conclusion: Vision-based cross-view geo-localization offers a viable alternative to global navigation satellite systems for planetary surface exploration.
...
TL;DR: HomeDiffusion is a zero-shot object customization method that uses multi-angle reference images and diffusion models to generate objects (e.g., furniture) that harmonize with background scenes while preserving fine details like patterns and curves.


<details>
  <summary>Details</summary>
Motivation: Existing zero-shot object customization methods struggle with detail preservation (patterns, curves) and generating poses that harmonize with background scenes, especially for asymmetrical objects lacking multi-viewpoint information.

Method: HomeDiffusion uses a novel dataset of multi-angle furniture and indoor scene images. It leverages multi-viewpoint images of the reference object, extracts high-fidelity details, and performs cross-attention with noise latents in latent space during diffusion to preserve object details.

Result: Extensive qualitative and quantitative experiments show superior performance over existing zero-shot and few-shot object customization approaches.

Conclusion: HomeDiffusion effectively solves detail divergence and pose harmonization issues in zero-shot object customization, particularly for asymmetrical objects, by leveraging multi-viewpoint information and cross-attention mechanisms.
...
TL;DR: This paper proposes a trajectory-based dataset distillation framework (SGR + TIAT) that suppresses label noise without requiring clean subsets or relabeling, outperforming existing DD methods on various noise types.


<details>
  <summary>Details</summary>
Motivation: Dataset distillation under noisy supervision risks compressing corrupted associations alongside useful signals, degrading robustness. Existing noisy-label methods (sample selection, loss weighting, label correction) rely on clean anchors and tight noise-model coupling, which are misaligned with DD's goal of compact, plug-and-play supervision.

Method: The framework has two components: (1) Selective Guidance Reweighting (SGR) — fuses global forgetting patterns (second-split forgetting) with local neighborhood consistency for progressive reweighting along the teacher trajectory; (2) Teacher-Inspired Auxiliary Targets (TIAT) — injects auxiliary residual guidance distilled from intermediate teacher dynamics to reinforce informative signals while maintaining internal consistency.

Result: Consistent gains over state-of-the-art DD baselines across symmetric, asymmetric, and real-world noise scenarios, with robustness, label-preserving, and computationally lightweight properties.

Conclusion: The SGR+TIAT framework effectively suppresses noise and preserves transferable knowledge without relabeling or clean subsets, offering a broadly applicable solution for robust dataset distillation under noisy supervision.
...
TL;DR: The paper proposes BRACE, a multi-frame bracketed RAW restoration method for removing flicker-banding artifacts from screen-captured images, along with a synthetic-real dataset (Bricker) and a new evaluation metric (SFC).


<details>
  <summary>Details</summary>
Motivation: Flicker-banding artifacts from rolling shutter and display modulation degrade image readability, and existing single-frame methods cannot reliably distinguish these artifacts from real texture.

Method: The authors construct Bricker (synthetic-real bracketed RAW dataset via ray-tracing and automated capture), and propose BRACE using frequency-aware banding prior and multi-scale spatial cross-attention modulator (MSCAM) for cross-exposure fusion.

Result: State-of-the-art performance on both synthetic and real benchmarks; a new Stripe Frequency Consistency (SFC) metric is introduced for evaluation.

Conclusion: Multi-frame bracketed RAW restoration effectively addresses flicker-banding, outperforming existing single-frame methods.
...
TL;DR: CAI is a training-free inference-time method that selectively strengthens visual grounding only for uncertain tokens in deeper layers, reducing object hallucinations in LVLMs while preserving fluency.


<details>
  <summary>Details</summary>
Motivation: Prior training-free remedies uniformly strengthen visual signals, which can amplify irrelevant regions and introduce spurious evidence, harming fluency. CAI aims to be more selective.

Method: CAI uses two-axis selectivity: (1) token-specific visual relevance from early-layer representations to localize aligned regions, and (2) conservative entropy- and depth-gated attention tilt only for uncertainty-spiking tokens in deeper layers where visual grounding degrades.

Result: Achieves state-of-the-art hallucination mitigation across multiple LVLM backbones and benchmarks. CAI is characterized as a KL-minimal attention reweighting with bounded interference under inactive gates or small tilts.

Conclusion: Targeted intervention strengthens visual grounding while preserving linguistic fluency, with consistent improvements even without contrastive decoding (which remains optional).
...
TL;DR: PointArena 2026 solution achieves 77.2% overall accuracy (2nd place) using agent-driven synthesis, a deterministic steerable-data pipeline, and two model-side modules (AttnRes + ABC correction) with category-aware routing to address three key failure modes in visual pointing.


<details>
  <summary>Details</summary>
Motivation: Visual pointing (mapping language to pixel coordinates) is a core embodied AI skill, but suffers from three failure modes that prior approaches do not adequately address.

Method: 1) Agent-driven synthesis building large candidate pools (55K+ processed outputs). 2) Deterministic steerable-data pipeline with masks, templates, path verification creating 10K verified samples. 3) Two model modules: AttnRes (gated cross-block attention for steerability) and ABC correction (encoding perturbed coordinates with visual features). Category-aware routing combines complementary specialists.

Result: 77.2% overall accuracy, ranks 2nd on benchmark. Expert accuracies: 93.9% Affordance, 82.6% Spatial Relation, 78.2% Reasoning, 70.4% Counting, 63.0% Steerability.

Conclusion: The combination of large-scale synthetic data generation, deterministic data pipelines, and complementary model-side modules effectively addresses visual pointing failure modes, achieving competitive benchmark performance.
...
TL;DR: RainODE reformulates precipitation forecasting as a continuous-time dynamical system using Neural ODE with Brownian Bridge stochastic refinement, enabling arbitrary-time inference with sharp predictions.


<details>
  <summary>Details</summary>
Motivation: Temporal resolution in precipitation forecasting is constrained by observational limitations and computational costs of dense discrete modeling, even though both accuracy and temporal resolution are critical.

Method: RainODE models precipitation evolution in latent space using a Neural ODE for continuous-time dynamics, plus a Brownian Bridge stochastic module to refine non-advective intensity variations and restore fine-grained structures.

Result: Consistent improvements across multiple temporal intervals and precipitation regimes on SEVIR and the new RAPID dataset.

Conclusion: Combining deterministic continuous dynamics with stochastic refinement enables arbitrary-time inference while maintaining sharp predictions, overcoming the temporal resolution limitations of discrete models.
...
TL;DR: SUMO is a zero-shot, training-free framework that integrates nonlinear dynamics (via a State Space Model and Selective Unscented Filter) with vision-based segmentation to achieve state-of-the-art performance on Visual Object Tracking and Moving Object Segmentation.


<details>
  <summary>Details</summary>
Motivation: Existing VOT and MOS methods rely heavily on visual cues and fail under complex, nonlinear motion patterns in real-world scenarios.

Method: A nonlinear State Space Model (SSM) inspired by robotics captures complex dynamics; a Selective Unscented Filter (SUF) with joint scoring and dynamic multi-source fusion estimates object states; a memory selection mechanism evaluates frame reliability.

Result: State-of-the-art performance on both VOT and MOS tasks.

Conclusion: Integrating nonlinear dynamics with vision-based segmentation improves tracking and segmentation accuracy and consistency without requiring training.
...
TL;DR: LWDrive is a VLM-based autonomous driving framework that refines coarse trajectory outputs from VLMs into geometrically accurate, future-aware plans using layer-wise world-model guidance and a Foresight Cascade Planner, achieving high scores on NAVSIM benchmarks.


<details>
  <summary>Details</summary>
Motivation: VLMs generate trajectories that encode only coarse driving intentions and lack geometric accuracy, future-awareness, and multi-view grounding for effective E2E autonomous driving planning.

Method: LWDrive uses coarse VLM trajectories as intent-aware plans, expands diverse candidates, and progressively refines them via a Foresight Cascade Planner (FCP). FCP leverages future-frame generation supervision, multi-layer VLM features, historical states, Action-Query representations, and multi-view BEV features for coarse-to-fine refinement with a score head selecting the best trajectory.

Result: LWDrive achieves 92.0 on NAVSIM benchmark and 89.6 on NAVSIM-v2.

Conclusion: LWDrive effectively bridges the gap between VLM semantic reasoning and precise geometric planning, demonstrating state-of-the-art performance on autonomous driving benchmarks.
...
TL;DR: This paper proposes IREU, a novel identity unlearning method for Customized Portrait Generation that selectively perturbs identity-related features to effectively suppress generation of target identities while preserving high fidelity for other identities, with cross-generator generalizability.


<details>
  <summary>Details</summary>
Motivation: Customized Portrait Generation (CPG) technologies pose significant privacy risks by enabling the spread of fake visual information. There is a need for public generators to suppress their generation ability for a particular person when requested, but no prior methods exist for identity unlearning in CPG.

Method: The paper first establishes a simple baseline that updates the image encoder by minimizing/maximizing identity similarity between generated and input images for unlearned/retained identities. Then proposes IREU, which locates identity-related features offline and only perturbs those features, avoiding global feature perturbation that harms fidelity.

Result: IREU achieves better identity unlearning performance for target identities while maintaining high fidelity for retained identities. The unlearned image encoder generalizes across different generators sharing the same encoder without requiring fine-tuning.

Conclusion: IREU effectively solves the identity unlearning problem in CPG by localized feature perturbation, offering practical deployment advantages through cross-generator generalizability.
...
TL;DR: The paper proposes AI Virtual Tissue (AIVT), an AI framework that uses spatial multimodal data to model tissue states and their transitions, enabling unified, spatially resolved, and dynamically manipulatable representations for analyzing health and disease.


<details>
  <summary>Details</summary>
Motivation: Conventional computational modeling fails to capture the complexity of tissues as spatially organized, multiscale biological systems, necessitating a new AI-driven approach.

Method: AIVT is an AI framework grounded in spatial multimodal data that learns unified, spatially resolved, and dynamically manipulatable representations of tissue state.

Result: The framework enables tissue state representation and analysis, molecular and morphological feature prediction, and simulation of spatiotemporal tissue dynamics.

Conclusion: AIVT provides fundamental assumptions, core capabilities, architectural components, and data/algorithm foundations for AI-driven tissue modeling.
...
TL;DR: This paper proposes an LLM-based framework that fuses facial Action Units (AUs) converted into textual descriptions with interviewees' textual responses to improve personality recognition in asynchronous video interviews.


<details>
  <summary>Details</summary>
Motivation: Existing unimodal (text-only) methods suffer information loss by ignoring facial cues, while multimodal methods using full-face images or sparse frames discard fine-grained temporal dynamics critical for accurate personality assessment in AVIs.

Method: AU sequences are converted into interpretable textual descriptions, fused with textual responses via an LLM, and a lightweight regression head transforms embeddings into continuous personality scores without disrupting the semantic space.

Result: On the AVI-6 benchmark, the method shows consistent improvements over most baselines with lower prediction errors and stronger correlations with human-rated scores across multiple traits.

Conclusion: AU-text fusion provides a psychologically grounded and computationally efficient framework for personality recognition in AVIs, with decoupled semantic understanding and regression leading to better training stability and interpretability.
...
TL;DR: StrucTab is a table parsing model that uses intermediate structural supervision and a decomposed reward RL framework (Uni-TabRL) to achieve state-of-the-art performance, validated on public benchmarks and a new large-scale dataset TableVerse-5K.


<details>
  <summary>Details</summary>
Motivation: Existing VLM-based table parsing methods rely on direct end-to-end supervision, bypassing explicit intermediate reasoning. Additionally, RL optimization is hindered by unstable or ambiguous reward designs, limiting performance gains.

Method: (1) Decomposes parsing into human-inspired subtasks (row-column counting, merged-cell analysis) with sequential reasoning. (2) Introduces Uni-TabRL, a unified RL framework with decomposed rewards: validity, structure, and content. (3) Proposes TableVerse-5K benchmark.

Result: State-of-the-art performance on all evaluated public benchmarks and significant improvements on the new TableVerse-5K benchmark.

Conclusion: Explicit structural modeling via subtask decomposition and decomposed reward optimization effectively improves table parsing performance and training stability.
...
TL;DR: Traffic-CBM is a structurally interpretable multimodal framework for encrypted traffic classification that organizes heterogeneous traffic signals into a unified hierarchical concept space instead of opaque latent features, achieving competitive performance while providing clear structural explanations.


<details>
  <summary>Details</summary>
Motivation: Existing encrypted traffic classification methods achieve strong performance but produce opaque decisions by fusing flow statistics, packet sequences, and byte-level representations into black-box latent features, making it unclear which evidence drives predictions.

Method: Traffic-CBM organizes heterogeneous traffic signals into a unified hierarchical concept space with predefined traffic evidence groups: grouped flow statistics map to statistical concepts, temporal encoders learn temporal concepts from disjoint feature subspaces, and byte-level evidence is organized into packet-level and cross-packet concepts, all without requiring manual semantic annotations.

Result: Evaluated on multiple encrypted traffic benchmarks, Traffic-CBM achieves competitive and balanced classification performance while providing a clearer structural interpretation interface than conventional end-to-end fusion models.

Conclusion: The learned concept space is actively used in the prediction process and provides a clearer structural explanation of multimodal traffic evidence, outperforming black-box fusion models in interpretability without sacrificing classification performance.
...
TL;DR: The paper proposes a Decompositional Evidence Grounding framework that forces VLMs to break down queries into atomic sub-questions with evidence bounding boxes, creating interpretable reasoning paths for more reliable and transparent answers.


<details>
  <summary>Details</summary>
Motivation: VLMs are black boxes prone to hallucinations and superficial shortcuts despite high benchmark performance, lacking interpretability and reliable reasoning.

Method: Decompositional Evidence Grounding: decomposes global queries into atomic sub-questions, each requiring explicit sub-answers and localized evidence bounding boxes, constructing structured reasoning paths.

Result: The framework produces answers that are logical consequences of verified visual facts rather than statistical guesses, with explicit grounding in specific visual regions.

Conclusion: By forcing step-by-step visual evidence grounding, the approach enhances both model performance and interpretability, mimicking human-like deductive reasoning.
...
TL;DR: DCGrasp is a distance-aware controllable grasp generation system that uses a novel Distance Profile (signed distances from hand vertices to nearest object points with distance-aware weighting) to generate high-quality, physically plausible 3D hand-object grasps with flexible user control, generalizing well across diverse object and hand shapes.


<details>
  <summary>Details</summary>
Motivation: Existing methods for generating 3D hand-object interactions rarely satisfy the requirements of flexible controllability and strong generalization to diverse object geometries, limiting their practical applicability in robotics, XR, and synthetic data generation.

Method: DCGrasp introduces a Distance Profile (signed distance from each hand vertex to nearest object point with distance-aware weighting) that captures semantically similar hand-object interaction in near-contact regions while remaining invariant to object and hand identity. It first generates a Distance Profile via a Diffusion Transformer along with a candidate hand pose, then refines the pose through optimization to enforce consistency between the optimized pose and generated Distance Profile in near-contact regions.

Result: DCGrasp produces high-quality, physically plausible grasps with flexible user control, demonstrating strong generalization to diverse object and hand shapes and scales.

Conclusion: DCGrasp establishes a robust and versatile pipeline for the synthesis of controllable 3D hand-object interactions, satisfying requirements for flexible controllability and strong generalization.
...
TL;DR: Latent-CURE is a diagnostic framework for breast ultrasound that uses asymmetric weighted chain-of-thought reasoning in latent space to infer BI-RADS morphological descriptors before diagnosis, coupled with a dual-asymmetric optimization strategy to handle extreme class imbalance, achieving robust and transparent performance.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal large models for breast ultrasound diagnosis use opaque end-to-end paradigms that rely on global statistical correlations, making them susceptible to shortcut learning. They often ignore rare but decisive malignant indicators in favor of dominant benign patterns due to extreme epidemiological imbalances.

Method: (1) Asymmetric weighted chain-of-thought (CoT) methodology in latent space: constructs an implicit reasoning trajectory that forces the model to sequentially infer standardized BI-RADS morphological descriptors before arriving at a final diagnosis. (2) Dual-asymmetric optimization strategy: dynamically adjusts margins and weights to safeguard high-specificity malignant descriptors from being overshadowed by common benign priors.

Result: The knowledge-injected approach provides transparent clinical evidence while achieving robust and accurate diagnostic performance in imbalanced medical cohorts.

Conclusion: Latent-CURE effectively addresses shortcut learning and class imbalance issues in automated breast ultrasound diagnosis by enforcing structured clinical reasoning (BI-RADS descriptors) and asymmetric optimization, producing both transparent evidence and strong performance.
...
TL;DR: This paper proposes a CVAE-based multi-goal prediction method that uses RGB scene images and human pose to generate diverse potential future goals of human movement, addressing stochasticity in human behavior.


<details>
  <summary>Details</summary>
Motivation: Human behavior is stochastic with varying goals; predicting diverse future goals enables autonomous systems to proactively plan, but previous works require semantic scene knowledge or only handle object interactions.

Method: Uses Conditional Variational Autoencoder (CVAE) to model multi-goal prediction by leveraging current RGB scene and human pose, sampling in latent space to generate diverse potential movement goals.

Result: The approach generates multiple movement goals via latent space sampling and shows generalization across GTA-IM and PROX datasets.

Conclusion: The method effectively captures stochasticity in human goal prediction without requiring semantic scene knowledge, and generalizes well across different scenarios.
...
TL;DR: The paper introduces Saliency-LiDAR (SALL), an explainability-guided method that creates universal saliency maps to identify spatially critical regions for LiDAR-based 3D detectors. Using these maps, the Explainability-aware Frustum Attack (EFA) selectively perturbs only the most influential frustums, achieving over 15 percentage points recall reduction with 25-50% fewer perturbed frustums than baselines, revealing that 3D detectors rely on small, exploitable spatial regions.


<details>
  <summary>Details</summary>
Motivation: To understand how LiDAR-based 3D object detectors rely on spatial evidence in complex scenes and whether these reliance patterns can be exploited to induce failures more efficiently than existing attack methods.

Method: The authors propose Saliency-LiDAR (SALL), which aggregates Integrated Gradient attributions across scenes to produce universal saliency maps for LiDAR-based 3D detectors. Based on these maps, they design the Explainability-aware Frustum Attack (EFA) that selectively perturbs only the most influential frustums rather than uniformly attacking entire object regions.

Result: On KITTI and nuScenes datasets, with detectors like PointPillars and SECOND, EFA reduces detection recall by more than 15 percentage points while requiring 25-50% fewer perturbed frustums compared to state-of-the-art non-saliency-aware baselines.

Conclusion: Modern 3D detectors concentrate discriminative evidence in a small subset of spatial regions, revealing a structural robustness vulnerability in current LiDAR perception systems that can be exploited efficiently through saliency-guided attacks.
...
TL;DR: MV-SDI improves 3D generation from 2D diffusion priors by aggregating gradients from multiple views per optimization step (via antithetic antipodal sampling), reducing variance and improving 3D consistency without retraining or multi-view data.


<details>
  <summary>Details</summary>
Motivation: Score distillation suffers from high-variance, view-inconsistent gradient estimates because each step uses only a single randomly chosen view. Prior multi-view solutions retrain the diffusion prior, making gains inseparable from prior quality.

Method: MV-SDI aggregates gradients from K views per optimization step via gradient accumulation (keeping peak memory unchanged), draws views as antithetic antipodal pairs for balanced angular coverage, and keeps the 2D prior frozen.

Result: At fixed 10K UNet budget: K=2 improves CLIP R-Precision from 74.8% to 83.8%, CLIP score 0.297→0.312, with 0.0% divergence rate; K=4 gives fourfold step reduction at R-Precision 86.9%.

Conclusion: MV-SDI is a drop-in, retraining-free enhancement for gradient-based score distillation pipelines that improves quality and consistency by reducing gradient variance through multi-view aggregation.
...
TL;DR: Flow Splatting extends 3D Gaussian Splatting to 4D by introducing a velocity field and splatting-based optical flow rendering to efficiently reconstruct dynamic 3D scenes from monocular videos, achieving better quality, faster training, and higher rendering speed than prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing 4D Gaussian methods suffer from long training times, low rendering speeds, or high memory consumption when reconstructing dynamic scenes from monocular videos, as they fail to fully exploit dense dynamic information.

Method: Flow Splatting constructs a velocity field from time-varying 4D Gaussians (with dynamic means and covariances), then extends conventional splatting to render optical flow from this velocity field while accounting for camera motion, enabling direct supervision of dynamics learning.

Result: The method achieves better image quality with less training time and higher rendering speed compared to state-of-the-art methods on various benchmarks.

Conclusion: By effectively modeling velocity fields and rendering optical flow via splatting, the proposed approach offers a more efficient and effective solution for dynamic 3D scene reconstruction from monocular videos.
...
TL;DR: Rigel is a new automatic evaluation metric for image/video captioning that uses a self-distilled scoring head from a frozen LLM and a new Vid-Lepus dataset, outperforming SOTA metrics by over 10 points on ActivityNet-Fact.


<details>
  <summary>Details</summary>
Motivation: Existing automatic evaluation metrics for image/video captioning show limited alignment with human judgments. LLM-as-a-Judge methods improve alignment but suffer from a mismatch between large-vocabulary language modeling and evaluation over a small label set.

Method: Rigel uses an evaluation-specific scoring head distilled from a frozen LLM, capturing judgment signals in a task-aligned space without relying on large-vocabulary token sets. The LLM backbone is then refined with human judgment data. The Vid-Lepus dataset (3,338 video clips, 33,380 reference captions, 5,637 candidate captions) was constructed for training.

Result: Rigel outperforms state-of-the-art metrics across multiple benchmarks, achieving over 10-point improvements on ActivityNet-Fact in the reference-free setting.

Conclusion: The self-distilled score adaptation approach effectively bridges the gap between LLM language modeling and evaluation tasks, yielding superior caption evaluation metrics.
...
TL;DR: SICAGE is a modular framework for culture-aware co-speech gesture generation that learns speaker-independent cultural representations from audio and text using domain generalization, achieving improved motion realism, diversity, beat synchronization, semantic relevance, and cultural consistency.


<details>
  <summary>Details</summary>
Motivation: Existing co-speech gesture generation methods neglect cultural differences and are evaluated without speaker-disjoint splits, causing apparent "cultural" behavior to be confounded with speaker-specific gesturing style.

Method: SICAGE learns culture-discriminative, speaker-independent representations via adversarial learning or Fishr regularization (domain generalization), then conditions a multimodal gesture generator (ALaDiT, a real-time diffusion-based model) on these embeddings. Evaluated on the TED4C-L dataset (106h, 764 speakers, 4 cultural groups).

Result: SICAGE improves motion realism, diversity, beat synchronization, semantic relevance, and cultural consistency compared to baselines.

Conclusion: Decoupling cultural representations from speaker identity is effective for producing culturally appropriate gestures, and the proposed framework generalizes well across unseen speakers.
...
TL;DR: GeoEdit is a training-free pipeline for 3D-constrained object editing in single images, using Lift-Manipulate-Render-Denoise with variance-homogeneous injection to preserve both object identity and background quality without self-attention leakage.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based editors struggle with 3D physical constraints when manipulating objects in single photographs — they produce perspective violations, ghosting, and background blur due to self-attention leakage from forcing structural proxies into latent space.

Method: GeoEdit uses a Lift-Manipulate-Render-Denoise pipeline: (1) decouple scene and object in 3D, (2) align via point correspondence, (3) render a geometry-aligned proxy with structural depth map, (4) Dual-Branch Denoising with a video diffusion backbone for object identity and variance-homogeneous injection of 3D constraints into foreground within a narrow denoising window, while background denoises freely.

Result: GeoEdit demonstrates consistent improvements in geometric accuracy, identity fidelity, and background quality compared to existing methods, validated on the introduced GeoEditBench benchmark with pose-aware evaluation metrics.

Conclusion: GeoEdit effectively solves the asymmetric problem of rigid object manipulation with free background synthesis by matching injected signal variance to native latent statistics, preventing self-attention disturbance.
...
TL;DR: SkelEM is a self-supervised framework for axial super-resolution in volume microscopy that decouples low-frequency topology and high-frequency detail enhancement using a frozen topological network and a diffusion refiner with cycle-consistent mechanism, achieving high-fidelity restoration in ≤5 steps with state-of-the-art segmentation and zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Volume microscopy suffers from severe anisotropic resolution due to physical axial sectioning, and existing self-supervised ASR methods face a trilemma between overly smoothed regression textures, structural hallucinations of diffusion models, and prohibitive inference latency.

Method: SkelEM decouples ASR at the training-signal level using a frozen topological network and a diffusion refiner optimized by disjoint objectives. It employs a unified cycle-consistent mechanism on input sparse slices to extract real-domain residual prior and bidirectionally align the diffusion refiner, truncating reverse diffusion with this prior for ≤5 step restoration.

Result: SkelEM achieves the most favorable fidelity-perception balance among self-supervised methods, state-of-the-art membrane segmentation performance, robust zero-shot generalization across modalities, and introduces the BRAVE-ASR benchmark for cross-instrument evaluation.

Conclusion: SkelEM effectively resolves the trilemma in self-supervised axial super-resolution by decoupling topology and detail refinement, enabling fast, high-fidelity restoration with strong generalization across instruments and modalities.
...
TL;DR: Shell-supervised Gaussian Splatting uses lightweight facade structural shells as geometric supervision to improve 3D reconstruction of close-range urban facades, achieving better surface geometry while maintaining rendering quality.


<details>
  <summary>Details</summary>
Motivation: Close-range urban facades with glass, reflections, repeated windows, and weak texture are difficult for video-to-3D reconstruction, producing visually plausible renderings but unstable surface geometry.

Method: Introduces shell-supervised Gaussian Splatting: aligns an external facade structural shell to the video reconstruction, renders per-view depth, camera-space normal, and valid-mask maps, and applies mask-gated losses during Gaussian optimization to regularize only visible shell-supported facade regions.

Result: Improved facade orientation and visible-surface point-cloud consistency over photo-only, monocular-cue, and surface-oriented Gaussian baselines, while maintaining comparable held-out rendering quality.

Conclusion: Lightweight geometric supervision from structural shells effectively stabilizes facade geometry without compromising visual appearance, which is beneficial for embodied AI tasks requiring collision reasoning, navigation, and interaction.
...
TL;DR: Flux-GS is a real-time 3D Gaussian Splatting method optimized for mobile platforms. It reduces overhead via a Monte Carlo Specular Energy Aggregator (compressing lighting into lower-order SH bands), an Attribute-Conditioned SH Enhancement module (restoring high-frequency details without extra inference cost), and Multi-view Alpha-based Densification/Pruning (reducing redundant Gaussians while ensuring multi-view consistency). It achieves competitive quality with far fewer parameters.


<details>
  <summary>Details</summary>
Motivation: Existing 3D Gaussian Splatting methods achieve high-quality novel view synthesis but suffer from high inference and storage overhead due to high-order Spherical Harmonics (SH), making them unsuitable for mobile platforms.

Method: (1) Monte Carlo Specular Energy Aggregator: samples third-order radiance residuals and aggregates specular energy into compact latent space. (2) Attribute-Conditioned SH Enhancement: predicts Gaussian-aware offsets to enhance first-order SH representation. (3) Multi-view Alpha-based Densification and Pruning: leverages multi-view guidance for consistent structure and removal of redundant Gaussians.

Result: Substantial parameter reduction while maintaining competitive visual quality; demonstrated in extensive experiments as a robust, scalable solution for real-time mobile rendering.

Conclusion: Flux-GS effectively balances rendering quality and computational/storage efficiency, offering a practical deployment solution for 3D Gaussian Splatting on resource-constrained mobile platforms.
...
TL;DR: CIPE-Dance (300K clips, 400 hrs) is the largest dance video dataset; OmniDance integrates music into text-conditioned video generation via depth-aware specialization, easy-to-hard curriculum learning, and modality-specialized CFG, achieving SOTA on TI2V, MI2V, and MTI2V tasks.


<details>
  <summary>Details</summary>
Motivation: Existing methods lack large-scale high-quality dance video datasets and principled frameworks for integrating music as a conditioning signal into Video Generation Foundation Models.

Method: (1) CIPE-Dance: large-scale dataset built via progressive expert pipeline with choreography-informed text annotations. (2) OmniDance: framework-level recipe with depth-aware specialization, anchored easy-to-hard curriculum learning, and modality-specialized time-dependent CFG strategy.

Result: OmniDance achieves state-of-the-art performance across text-to-video, music-to-video, and music-text-to-video generation tasks on CIPE-Dance.

Conclusion: The paper addresses dataset scarcity and missing music-conditioning frameworks for dance video generation, demonstrating robust multimodal integration capability.
...
TL;DR: DICE is a plug-and-play framework that ensembles multiple frozen pathology foundation models (PFMs) with deep mutual learning to quantify uncertainty, improve classification, calibration, and enable unsupervised patch-level abnormality localization in whole-slide image analysis.


<details>
  <summary>Details</summary>
Motivation: Clinical adoption of PFMs is limited because they lack reliable confidence estimates and no single PFM is universally best across tasks, undermining trust in medical settings.

Method: DICE ensembles K frozen PFMs, models their disagreement as a proxy for uncertainty, aligns members via deep mutual learning (theoretically shown to upper-bound model uncertainty), and uses ensemble consensus for unsupervised patch-level abnormality localization.

Result: DICE provides reliable uncertainty estimates that flag failure-prone cases under in-distribution and out-of-distribution settings, matching or outperforming SOTA baselines in classification, calibration, and localization on three WSI benchmarks.

Conclusion: DICE takes a crucial step toward translating PFMs into uncertainty-aware decision-support systems for clinical use.
...
TL;DR: IBRSteG is a generalizable 3DGS steganography framework using a GAS network to embed secret 3D scenes into cover scenes via feed-forward Gaussian attribute injection, achieving undetectable hiding without per-scene optimization.


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS steganography approaches require per-scene finetuning/optimization and lack generalization across different scenes; the authors aim to design a generalizable feed-forward framework that works across unseen 3DGS scenes.

Method: The GAS (Gaussian Attributes Steganographer) network learns a scene-independent embedding function by injecting secret 3D Gaussian attributes into a cover scene, converting 3D Gaussians into structured attributes compatible with 2D learning paradigms for direct steganographic scene reconstruction.

Result: IBRSteG achieves effective concealment of various scenes with high visual quality, superior capacity, and security, as demonstrated on established datasets.

Conclusion: The proposed generalizable framework overcomes the rigidity of prior per-scene methods and enables scalable, undetectable 3D scene steganography with strong performance.
...
TL;DR: Musebench is a new benchmark (4,016 questions across cinema, visual arts, stage performance, and game design) evaluating MLLMs on reasoning about creative intent in audiovisual arts. Even the best model achieved only 48.29% accuracy vs. human expert 87.18%, revealing a major gap in current models' artistic understanding.


<details>
  <summary>Details</summary>
Motivation: Existing MLLM benchmarks focus on perceptual recognition (what is depicted) but overlook reasoning about creative intent (why it is expressed through particular choices). True artistic understanding requires analyzing the deliberate combinations of visual, auditory, and narrative elements.

Method: 4,016 questions distilled from 10K+ video essays across four art domains (cinema, static visual arts, stage performing arts, game arts). Questions combine single-select and variable-option multi-select formats. Generated via a four-phase iterative pipeline: shortcut filtering, adversarial distractors, and expert validation.

Result: Zero-shot evaluation of 28 state-of-the-art MLLMs: best model achieved only 48.29% accuracy, significantly below human expert performance of 87.18%.

Conclusion: Current MLLMs have a substantial gap in creative domain expertise for artistic understanding, highlighting the need for benchmarks like Musebench that evaluate reasoning about creative intent rather than just perceptual recognition.
...
TL;DR: IDNet is a multimodal framework for ischemic heart disease screening from color fundus photos, using a Cross-Modal Distillation Aggregator (CDA) to fuse left-eye, right-eye, and clinical features, achieving state-of-the-art results on a new UK Biobank benchmark.


<details>
  <summary>Details</summary>
Motivation: CFP offers low-cost IHD screening, but current studies lack public benchmarks and effective fusion of retinal images with sparse clinical variables.

Method: IDNet uses a Cross-Modal Distillation Aggregator (CDA) with learnable queries to sequentially integrate left-eye, right-eye, and clinical features, balancing high-dimensional visual data with low-dimensional tabular inputs.

Result: On a 50,410-image UK Biobank benchmark, IDNet outperforms image-only, clinical-only, and multimodal baselines; CDA consistently improves multiple visual encoders as a plug-in fusion module.

Conclusion: IDNet with CDA effectively addresses multimodal imbalance for IHD screening from CFP, and the reproducible benchmark supports future research.
...
TL;DR: CogSENet is a novel blind image deblurring framework inspired by the eagle's visual system, using semantic-aware token regrouping (SDSSM), frequency decomposition (BFFB), and CLIP-guided continuous blur field estimation to achieve state-of-the-art deblurring with fewer parameters, and generalizes to dehazing, deraining, and denoising.


<details>
  <summary>Details</summary>
Motivation: Current blind image deblurring methods fail to handle real-world spatially varying degradations and lack semantic awareness to distinguish valid textures from artifacts.

Method: CogSENet (1) Semantic-Driven State Space Module (SDSSM) for token regrouping and long-range dependency modeling; (2) BiFreqFusionBlock (BFFB) for wavelet-based frequency decomposition; (3) Continuous Blur Field (CBF) estimation fused with CLIP semantic priors for adaptive restoration.

Result: Outperforms state-of-the-art deblurring methods in visual quality and structural fidelity with fewer parameters; also performs favorably on dehazing, deraining, and denoising tasks.

Conclusion: Bio-inspired design (eagle's visual system) combined with semantic-aware and frequency-decomposed modeling provides a parameter-efficient, generalizable solution for blind image restoration.
...
TL;DR: EnsembleGaze is an unsupervised ensemble learning system for clustering free-viewing gaze data, revealing robust stimulus groupings (ambient vs. focal viewing) and context-dependent user groupings through consensus clustering, subspace clustering, and spectral biclustering methods.


<details>
  <summary>Details</summary>
Motivation: Free-viewing gaze data is rich but understudied in terms of human-information interaction (HII) patterns. Existing exploratory analyses focus on fixations and AOIs but lack systematic unsupervised characterization of both user behavior and stimulus types.

Method: The system uses statistical descriptors of fixation distributions for feature engineering, applies consensus clustering (voting across methods) to build a co-association matrix, and proposes two high-dimensional clustering strategies: consensus subspace clustering and spectral biclustering for joint user-image characterization.

Result: Image stimuli groupings are highly consistent across methods, revealing a robust ambient-vs-focal viewing distinction. User groupings are image-context-dependent, recoverable only by biclustering and two-step conditional approaches. Dataset-specific patterns emerged across public datasets.

Conclusion: EnsembleGaze provides a replicable unsupervised method for analyzing fixation behavior in scene perception, highlighting that stimulus-based clustering is stable while user clustering depends on image context—a structure best captured by biclustering approaches.
...
TL;DR: NeuWorld introduces Neural Implicit Scene (NIS) as a fixed-length renderable state for interactive video generation, decoupling scene evolution from frame rendering, achieving long-horizon consistency and efficiency without pretrained backbones.


<details>
  <summary>Details</summary>
Motivation: Existing interactive video generation entangles state transitions with high-frequency observation synthesis, leading to inconsistency and inefficiency in long-horizon generation.

Method: Proposes scene-centric paradigm using Neural Implicit Scene (NIS) as compact state. NeuWorld uses transformer VAE to learn NIS from sparse posed frames, and diffusion transformer to evolve NIS conditioned on future trajectories and geometry-aware history. VAE encoder unifies camera, reference-image, and history conditioning.

Result: Achieves strong long-horizon consistency with favorable inference efficiency, trained from scratch on public posed-view data without pretrained video backbones or 3D reconstructors.

Conclusion: Decoupling scene state transition from rendering via implicit neural scene representation improves long-horizon interactive video generation quality and efficiency.
...
TL;DR: Realsee3D (10K indoor scenes with 299K panoramic views) and Argus network achieve SOTA metric panoramic 3D reconstruction using a learned covisibility module and decomposed bidirectional pixel-to-world mapping with cross-coordinate constraints.


<details>
  <summary>Details</summary>
Motivation: Metric feed-forward 3D reconstruction for panoramic data is under-explored due to the lack of large-scale panoramic RGB-D training data. Also, sparse unordered capture settings suffer from global pose drift caused by poorly chosen coordinate anchors.

Method: Proposes Realsee3D (1K real + 9K synthetic indoor scenes, 299K panoramic viewpoints with metric annotations) and Argus network featuring: (1) a learned covisibility module to select optimal reference view for metric world frame anchoring; (2) decomposed bidirectional pixel-to-world mapping with per-step supervision and cross-coordinate joint constraints.

Result: State-of-the-art metric performance on camera pose estimation, depth estimation, and point cloud reconstruction on the Realsee3D benchmark.

Conclusion: Combining a large-scale hybrid panoramic dataset with a carefully designed feed-forward network (covisibility-guided anchoring + decomposed pixel-to-world mapping) enables high-quality metric panoramic 3D reconstruction.
...
TL;DR: This paper proposes a visibility-oriented evaluation framework for maritime image dehazing, using a simulated dataset (MSVD) and object detection accuracy to link restoration quality with practical visibility distance estimation, overcoming limitations of conventional metrics.


<details>
  <summary>Details</summary>
Motivation: Conventional image quality metrics (PSNR, SSIM, etc.) cannot establish a physically interpretable relationship between dehazing quality and practical visibility thresholds, which is critical for maritime navigational safety.

Method: (1) Constructs MSVD using Unity3D with graded visibility annotations; (2) Develops a dehazing visibility evaluation metric using object detection accuracy as an intermediate indicator to map restoration improvements to visibility gains.

Result: MSVD provides a reliable benchmark for graded visibility evaluation; the proposed metric enables interpretable visible-distance estimation across different imaging conditions.

Conclusion: The proposed visibility-oriented framework supports navigational safety and operational efficiency assessment by linking dehazing performance to measurable visibility distance.
...
TL;DR: ILLUME-X is a unified multimodal model that improves free-form interleaved text-image generation through better data pipelines, progressive training strategies, and a new evaluation method (ILScore), outperforming prior unified models in tasks like style transfer, image decomposition, and storytelling.


<details>
  <summary>Details</summary>
Motivation: To advance multimodal AI to the next stage, models must autonomously generate free-form interleaved text-image sequences, which existing approaches still struggle with due to data inefficiency and training instability.

Method: ILLUME-X introduces three components: (1) an expanded training data pipeline optimized for interleaved text-image generation, (2) a progressive training strategy with self-adaptive objectives for free-length multimodal token sequences, and (3) an objective evaluation method called ILScore for assessing interleaved text-image sequences.

Result: ILLUME-X outperforms previous unified models across multiple interleaved text-image generation tasks, including style transfer, image decomposition, and storytelling.

Conclusion: The proposed paradigm effectively addresses key challenges in multimodal data efficiency and training stability, leading to superior performance in free-form interleaved text-image generation.
...
TL;DR: A self-supervised method that learns a shared canonical frame from in-the-wild object videos using noisy SfM poses and a shared coarse mesh, achieving competitive category-level pose estimation without manual annotations.


<details>
  <summary>Details</summary>
Motivation: Traditional canonical frame establishment requires manual annotation, creating a scaling bottleneck that limits category and instance diversity. The goal is to remove this annotation dependency.

Method: Uses self-supervised training on 160,000 in-the-wild object videos with noisy camera poses from SfM. A shared coarse canonical mesh serves as a geometric bottleneck, and dense correspondences from image pixels to this mesh are learned. Per-sequence alignments are estimated from noisy SfM geometry, allowing a common canonical frame to emerge from multi-view consistency and semantic priors.

Result: Achieves competitive accuracy on category-level pose estimation benchmarks compared to methods that rely on canonical pose supervision.

Conclusion: Shared canonical frames can emerge purely from self-supervised learning on diverse, unlabeled object videos, eliminating the need for manual canonical pose annotation and enabling scalability to more categories and instances.
...
TL;DR: This paper proposes a multi-level grading model for coronary artery stenosis that fuses CCTA and 3D SCPR images using a Curved Feature Reconstruction module and a Clinical Risk-Aware Loss, achieving superior performance on an in-house dataset.


<details>
  <summary>Details</summary>
Motivation: Multi-level grading of coronary artery stenosis is clinically crucial for diagnosing coronary artery disease, but existing approaches using either CCTA or 3D SCPR images alone suffer from inherent limitations—CCTA has tortuous vessel paths making analysis difficult, while 3D SCPR images are prone to abnormal distortions.

Method: 1) Curved Feature Reconstruction (CFR) module that uses vessel curves as priors and a point-by-point correspondence strategy to align and fuse features from both 3D SCPR and CCTA images. 2) Clinical Risk-Aware (CR) Loss that incorporates clinical risk relevance into network training to better align with clinical diagnosis.

Result: The proposed method significantly outperformed other methods on an in-house dataset. Ablation studies confirmed the effectiveness of the proposed CFR module and CR Loss.

Conclusion: Fusing multi-modal CCTA and 3D SCPR features with clinical risk awareness is an effective strategy for improving multi-level coronary artery stenosis grading.
...
TL;DR: InnerZoom introduces a single-forward framework for GUI grounding that bridges cross-layer evidence to retain target-region awareness during coordinate prediction, achieving state-of-the-art results on six benchmarks while reducing latency by 31.8% and computational cost by 29% compared to two-pass ZoomIn methods.


<details>
  <summary>Details</summary>
Motivation: MLLM-based GUI grounding methods that formulate target localization as autoregressive coordinate generation suffer from a key issue: target-region awareness emerges in intermediate decoder layers but is neither retained nor translated into the final coordinate prediction. Existing ZoomIn-style methods fix this with an external crop-and-rerun pass, but this increases latency and computational cost.

Method: InnerZoom is a single-forward framework that transforms target-related cues from the original forward pass into a compact cross-layer evidence state, then preserves, refines, and reinjects this state throughout later decoding layers to guide coordinate prediction — avoiding the need for a second external pass.

Result: InnerZoom-4B achieves SOTA on all six GUI grounding benchmarks (e.g., 64.7 on OSWorld-G, 40.2 on UI-Vision, 73.1 on OSWorld-GR, 87.6 on MMBench-GUI), surpassing previous best by 4.1-2.3 points. It improves the SFT+RL baseline by 5.3 points on average, outperforms two-pass ZoomIn by 1.3 points on average, while reducing end-to-end latency by 31.8% and TFLOPs by 29%.

Conclusion: InnerZoom effectively solves the target-region awareness retention problem in MLLM-based GUI grounding without extra computational cost, achieving better accuracy than two-pass methods while being more efficient.
...
TL;DR: CylindTrack is a depth-aware cylindrical tracking-by-detection framework for panoramic multi-object tracking that addresses issues like periodic image domains, unreliable IoU near seams, and unstable depth cues through three key components: Depth-Temporal Trajectory Modeling, Spherical Spatio-Temporal Consistency Learning, and a Topology-Aware Cylindrical Motion Model.


<details>
  <summary>Details</summary>
Motivation: Panoramic MOT is not a simple extension of perspective MOT because equirectangular videos have a periodic (non-Euclidean) horizontal domain, breaking planar motion assumptions and making IoU-based association unreliable near the 0°/360° seam. Additionally, large-FoV scenes contain more objects with stronger scale variation and frequent interactions, making online association sensitive to unstable frame-wise depth cues.

Method: CylindTrack proposes three components: (1) Depth-Temporal Trajectory Modeling (DTM) that promotes depth from frame-wise cues to temporally filtered trajectory-level states; (2) Spherical Spatio-Temporal Consistency Learning (SSTC) with a Temporal Mixer and Spherical Geometry-aware Attention for temporal coherence and panoramic geometric alignment; (3) Topology-Aware Cylindrical Motion Model (TCMM) that lifts horizontal motion into a continuous angular state space for seam-consistent prediction and association.

Result: CylindTrack improves identity preservation and trajectory continuity in challenging panoramic scenes by jointly modeling trajectory-level depth consistency and panoramic topology.

Conclusion: The proposed CylindTrack framework effectively addresses the unique challenges of panoramic multi-object tracking, outperforming naive extensions of perspective MOT methods. Source code will be released.
...
TL;DR: LETT-NeXt is a lightweight model that uses RECIST markers (diameter measurements) to guide 3D lesion segmentation from CT scans, achieving competitive scores (70.6% challenge score) with fast CPU inference (6.9s per case) and low memory usage (3.6 GB).


<details>
  <summary>Details</summary>
Motivation: Standard RECIST diameter measurements provide only limited 2D descriptions of lesions, motivating the need to extend them to full 3D segmentation for better tumor response assessment.

Method: The model extracts a RECIST-centered crop from CT volumes, encodes RECIST lines and endpoints as two prompt channels concatenated with CT input, uses a compact MedNeXt-v2 encoder-decoder for mask prediction, followed by prompt-aware component selection and adaptive AutoZoom inference.

Result: Public validation: DSC 79.4±10.1, NSD 72.3±16.2. Hidden test: DSC 73.9, NSD 67.3, challenge score 70.6%. CPU inference: 6.9±3.0s per case, peak memory 3.6 GB.

Conclusion: LETT-NeXt effectively bridges 2D RECIST measurements to 3D lesion segmentation with high efficiency, offering a practical solution for pan-cancer segmentation in CT.
...
TL;DR: This paper introduces SciIR, a framework for scientific image generation that uses Peirce's Semiotic Triad to formalize reasoning into three dimensions (Entity Structure, Scientific Process, Scientific Law). It provides SciIR-82k (80K+ image-text pairs from publications), SciIR-Bench for evaluation, and demonstrates a fine-tuned model (Qwen-Image-SciIR) improving benchmark scores from 35% to 43%.


<details>
  <summary>Details</summary>
Motivation: Current T2I models struggle with semantic alignment and logical reasoning for scientific imagery. There is a lack of training data and evaluation benchmarks specifically designed for scientific image generation.

Method: The paper (1) formalizes scientific reasoning via Peirce's Semiotic Triad into Icon (Entity Structure), Index (Scientific Process), and Symbol (Scientific Law); (2) constructs SciIR-82k dataset hierarchically organized by these dimensions with Scientific Reasoning Chain-of-Thought; (3) proposes SciIR-Bench with Atomic Checklists for fine-grained evaluation; (4) fine-tunes Qwen-Image-SciIR model on the dataset.

Result: Fine-tuning on SciIR-82k improved Qwen-Image-SciIR from 35% to 43% on SciIR-Bench. Experiments revealed significant deficiencies in current models' scientific reasoning capabilities.

Conclusion: The SciIR framework provides a solid foundation for advancing scientific image generation, with structured reasoning dimensions and dedicated data/benchmark resources enabling measurable improvements.
...
TL;DR: This paper introduces a hyper-network that predicts non-linear neural functional maps (NFM) via an MLP with skip connections to better align spectral bases, improving unsupervised 3D shape matching in challenging scenarios like partiality, topological noise, and raw point clouds.


<details>
  <summary>Details</summary>
Motivation: Existing functional map methods struggle with challenging scenarios (partiality, topological noise, raw point clouds) because significant intrinsic distortion prevents truncated spectral bases from being accurately aligned via linear transformations.

Method: The authors model the NFM as an MLP with skip-connection to refine standard functional maps and employ a hyper-network to predict its weights conditioned on standard FM. The framework is trained using a novel unsupervised spectral alignment loss.

Result: The approach can be seamlessly integrated into state-of-the-art unsupervised deep functional map pipelines and substantially improves matching accuracy in demanding scenarios.

Conclusion: Non-linear neural functional maps learned via hyper-networks effectively overcome the limitations of linear functional maps in challenging non-rigid shape matching scenarios.
...
TL;DR: T2LDM++ is a Text-to-LiDAR diffusion model that addresses over-smoothing and limited controllability via Self-Conditioned Representation Guidance (SCRG), two new benchmarks (>100K samples), and supports multiple conditional generation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing Text-to-LiDAR generation suffers from scarce Text-LiDAR pairs, causing over-smoothed scenes and limited controllability. Authors aim to alleviate insufficient training priors and construct controllable Text-LiDAR data.

Method: Proposes T2LDM++ with Self-Conditioned Representation Guidance (SCRG), which uses a Guidance Network (GN) to provide reconstruction-based soft supervision to the Denoising Network (DN), enabling geometry-aware representations. Also constructs two high-quality Text-LiDAR benchmarks with >100K samples, introduces a directional position prior to mitigate street distortion, and supports multiple conditions via a control encoder with frozen DN.

Result: More effective and lightweight SCRG that is decoupled at inference (no extra overhead). High-quality benchmarks with >100K samples. Improved scene fidelity via directional position prior. Realistic LiDAR scenes generated with rich geometric details in both unconditional and conditional settings.

Conclusion: T2LDM++ with effective prior modeling and high-quality benchmarks can generate realistic LiDAR scenes with rich geometric details across various settings, addressing key limitations of prior Text-to-LiDAR generation.
...
TL;DR: The paper proposes DECT-DRNet, an iterative dual-domain refinement network for sparse-view dual-energy CT material decomposition that incorporates a Jacobian approximation module using FBP and U-Net, plus a learnable sparse dual-domain regularization term with Fourier convolutional residual blocks for global and local feature capture.


<details>
  <summary>Details</summary>
Motivation: Sparse-view DECT material decomposition is challenging due to its nonlinear and ill-posed nature. Existing deep unrolling methods lack explicit Jacobian operator incorporation and rely on conventional convolutions that insufficiently model global structural information.

Method: DECT-DRNet iteratively refines material decomposition using (1) an FBP-based Jacobian approximation module with U-Net for the backward process, and (2) a learnable sparse dual-domain regularization term using Fourier convolutional residual blocks that combine image-domain geometric features with frequency-domain noise suppression.

Result: DECT-DRNet achieves more accurate multi-material decomposition under sparse-view conditions compared to existing methods.

Conclusion: Explicitly incorporating the Jacobian operator and dual-domain regularizations (spatial + frequency) significantly improves DECT material decomposition in sparse-view settings.
...
TL;DR: Lens is a question-conditioned visual evidence purification framework that uses a lightweight Lens Evidence Token to score and preserve relevant visual tokens while injecting adaptive noise into low-relevance ones, improving MLLM performance on VQA and grounding tasks without changing the model backbone.


<details>
  <summary>Details</summary>
Motivation: MLLMs fail in fine-grained visual reasoning because question-relevant visual cues are diluted by dense and redundant image tokens. Existing multimodal reasoning methods overlook this negative impact of redundant visual tokens.

Method: Lens introduces a lightweight Lens Evidence Token (LET) to score visual tokens by question relevance, then injects adaptive latent noise into low-relevance tokens to softly suppress distractors, using only one temporary learnable control token and a lightweight noise generator.

Result: Improves base MLLM by 2.4-6.4 points on most VQA datasets and by 4.1-6.4 points on grounding tasks, with minimal overhead.

Conclusion: Multimodal reasoning benefits more directly from cleaner question-relevant visual evidence than from simply extending the reasoning trace.
...
TL;DR: HiRes is a hierarchical cascaded pipeline combining YOLOv8n detection, UNet++ segmentation, and geometric decoding to accurately identify resistor values from full-frame images, achieving 85.8% accuracy and outperforming CVResist and MLLMs.


<details>
  <summary>Details</summary>
Motivation: Accurate resistor value identification from unconstrained real-world images is challenging due to lighting, orientation, scale, and background variations, and existing methods like CVResist fail to generalize beyond controlled conditions.

Method: A three-stage cascaded pipeline: (1) object detection via YOLOv8n, (2) semantic segmentation via UNet++ with EfficientNet-B2 backbone, (3) structured geometric decoding using projection along the resistor axis with geometric filtering, gap-preserving band separation, and validation against the E24 resistor series.

Result: Detection mAP50 of 0.9906, segmentation mIoU of 0.8444, end-to-end identification accuracy of 85.8% (95% CI: 78.0-91.9%), outperforming CVResist and state-of-the-art MLLMs with lower cost and higher efficiency.

Conclusion: Integrating learned visual representations (detection & segmentation) with structured geometric reasoning provides a robust, interpretable, and efficient alternative for resistor value identification.
...
TL;DR: DrivenMorph bridges Demons algorithms with attention mechanisms by using local feature differences as physically inspired driving forces, producing explainable and modular medical image registration that outperforms state-of-the-art methods on 3D brain MRI datasets.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning registration methods lack physical explainability and fine-grained deformation control, motivating the need to integrate traditional registration principles (like Demons algorithms) with modern networks.

Method: DrivenMorph incorporates difference modeling as an inductive bias in latent feature space, computes driving forces from local differences, and uses a neural Demons layer to simulate force-displacement interactions for smooth, anatomically consistent deformation.

Result: Superior performance over both learning-based and optimization-based methods on multiple 3D brain MRI datasets; visualizations confirm learned driving forces align with actual deformation patterns.

Conclusion: The approach provides an explainable, efficient, and modular solution for learning-based medical image registration, separating difference modeling from deformation while maintaining physical interpretability.
...
TL;DR: This paper introduces Few-Shot Domain Incremental Learning (FSDIL) to address data scarcity in domain-incremental learning, and proposes Continual Vision-Language Consolidation (CVLC) which uses latent space reservation, dual coalescent projection (DCP), and LLM-generated language prototypes to achieve up to 16% improvement over prior methods.


<details>
  <summary>Details</summary>
Motivation: Existing DIL methods require massive data and suffer overfitting under data scarcity. The authors propose a new problem FSDIL to address extreme data shortages in DIL.

Method: CVLC combines visual prototype calibration with LLM-generated language prototypes (via multiple templates and synonyms), fuses vision-language prototypes, uses Dual Coalescent Projection (DCP) for parameter-efficient fine-tuning, employs latent space reservation in the base domain, and structures shared vs. domain-specific components for generalization.

Result: CVLC outperforms prior arts by up to 16% on benchmark problems.

Conclusion: The proposed CVLC effectively addresses the new FSDIL problem, demonstrating significant improvements and public code availability.
...
TL;DR: SHOVIR is a benchmark that exposes how VLMs can achieve high report quality scores in radiology report generation by exploiting shortcuts rather than genuinely grounding findings in visual evidence, revealing a gap between clinical fluency and spatial grounding.


<details>
  <summary>Details</summary>
Motivation: Current RRG evaluation metrics (lexical overlap, clinical correctness) fail to test whether diagnostic statements are actually grounded in visual evidence, allowing models to score well via learned priors or spurious correlations (vision shortcuts).

Method: SHOVIR extends MIMIC-CXR and PadChest-GR with per-box CheXpert labels, then defines image-level and disease-level occlusion experiments comparing model predictions on clean vs. locally perturbed images to isolate direct shortcuts (finding persists after evidence removed) and contextual shortcuts (detection degrades when co-occurring pathologies occluded).

Result: Eight state-of-the-art VLMs showed varying shortcut behavior across architectures and datasets. High baseline report quality did not correlate with strong spatial grounding, indicating clinically fluent generation can coexist with shallow visual evidence reliance.

Conclusion: Current RRG evaluation has a blind spot in assessing spatial grounding; region-aware assessment protocols are needed to ensure models genuinely ground findings in visual pathology rather than exploiting shortcuts.
...
TL;DR: This paper presents a multicenter breast FNAC dataset (321 patients, 470 WSIs) with C1-C5 patch-level labels, including NDPI slides, GeoJSON annotations, extracted patches, and code, totaling ~950 GB on Zenodo.


<details>
  <summary>Details</summary>
Motivation: To provide a large-scale, multi-stain, multi-center breast FNAC dataset with standardized C1-C5 reporting labels for patch-wise classification, addressing the lack of publicly available cytology datasets.

Method: Prospective collection of 470 WSIs (Papanicolaou & May-Grünwald Giemsa stains) from 321 patients across Indian tertiary medical centers (May 2023–March 2026), scanned at 40X; 446 WSIs annotated with 7,398 patch images with expert-verified C1-C5 labels.

Result: Dataset includes 470 WSIs (446 annotated), 7,398 PNG patches, GeoJSON annotations, metadata, data dictionary, validation summary, and code; total ~950 GB available on Zenodo.

Conclusion: This comprehensive FNAC dataset supports reproducible research in breast cytology patch classification and is openly accessible with tools for reuse.
...
TL;DR: A sparse fusion mechanism for RGB-T detection that first uses lightweight single-modality scanning to identify proposals, then applies cross-modality fusion only on sparse foreground regions, achieving competitive performance with significantly lower computational cost.


<details>
  <summary>Details</summary>
Motivation: Existing RGB-T detectors use heavy dual backbones and exhaustive cross-modality fusion across entire images, which is computationally wasteful since most regions are smooth backgrounds easily handled by lightweight single-modality models.

Method: A two-stage sparse fusion framework: Stage 1 uses lightweight modality-specific detection to produce high-recall RoIs; Stage 2 applies fusion-driven examination and refinement to filter false positives and refine bounding boxes, adaptively allocating resources to potential foregrounds.

Result: Achieves competitive detection performance with substantially fewer parameters and lower computational cost, while maintaining strong scalability to high-resolution images.

Conclusion: Sparse fusion is an effective paradigm for efficient RGB-T detection, avoiding unnecessary computation on background regions while preserving the benefits of cross-modality fusion for challenging foregrounds.
...
TL;DR: The paper shows that high benchmark accuracy in traffic accident VideoQA often comes from textual shortcuts rather than genuine visual understanding, as VLMs perform competitively without video input. The authors introduce diagnostics (Blind Gap, Visual Gain, Shortcut Score) to measure and filter shortcut-prone questions, improving visual grounding in evaluation.


<details>
  <summary>Details</summary>
Motivation: To expose and address the problem that high benchmark accuracy in traffic accident VideoQA does not guarantee genuine visual understanding, as models may rely on textual shortcuts instead of scene-specific visual evidence.

Method: The authors audit four public VideoQA benchmarks using open-weight VLMs, comparing performance with and without video. They introduce Blind Gap (text-only above-chance performance) and Visual Gain (marginal benefit of video). They also propose a Shortcut Score combining text-only confidence and visual necessity for training-free filtering.

Result: Several VLMs perform competitively or even better without video input. On MM-AU, removing video consistently improves accuracy, and adding more frames degrades performance. The proposed metrics reveal large differences in grounding quality across benchmarks.

Conclusion: Visually grounded evaluation, not just high accuracy, is essential in safety-critical VideoQA. The Shortcut Score enables construction of subsets that reduce shortcut bias and improve visual grounding.
...
TL;DR: S4ECA introduces a parameter-efficient dual-encoder adapter framework for Referring Remote Sensing Image Segmentation, achieving SOTA performance by updating only 2.4% of backbone parameters through semantic-driven scale and spatial selection for cross-modal alignment.


<details>
  <summary>Details</summary>
Motivation: Existing RRSIS models rely on full fine-tuning of foundation models, which is computationally intensive and degrades generalization ability. PET frameworks are single-modal and fail to capture cross-modal dependencies or bridge the domain gap between natural and aerial imagery.

Method: A dual-encoder adapter architecture: (1) textual adapter uses learnable queries to distill semantic language proxies from word embeddings; (2) visual adapter refines hierarchical features via multi-scale dense extractor + language-guided scale and spatial selection mechanism for dynamic cross-modal alignment.

Result: Updating only 2.4% of backbone parameters, S4ECA achieves state-of-the-art performance on the RRSIS-D and RefSegRS datasets, demonstrating superior efficiency and precision.

Conclusion: Parameter-efficient tuning with targeted cross-modal alignment can effectively handle the domain gap and multimodal reasoning challenges in RRSIS, outperforming full fine-tuning approaches with far fewer updated parameters.
...
TL;DR: This paper proposes Shell-LCC, a method that uses the data manifold of high-quality video data as a dense, differentiable, and nearly cost-free reward signal to improve text-to-video diffusion models, enhancing realism and reducing motion blur and over-smoothing.


<details>
  <summary>Details</summary>
Motivation: Existing T2V diffusion models rely on reward signals (e.g., reward models, DPO) to align with human aesthetics, but these are computationally expensive, require costly human annotations, and offer limited improvement in fine-grained local details.

Method: The authors model the manifold structure of SFT data using Local Coordinate Coding (LCC) to capture the manifold skeleton, then extend it to Shell-LCC which models the manifold surface as an isotropic shell to avoid mean regression and preserve high-frequency details.

Result: The approach improves realism, enhances high-frequency details, reduces over-smoothing artifacts, and alleviates motion blur in generated videos.

Conclusion: The data manifold itself can serve as an effective, cost-free reward signal for aligning T2V diffusion models, and Shell-LCC improves upon standard LCC by better modeling the true high-density region of the manifold.
...
TL;DR: T2I diffusion models suffer from concept association bias for one-and-only (OAO) objects (e.g., celestial bodies, landmarks). The authors identify that final text embeddings discard concept-level information from intermediate layers, and propose IR-guided diffusion that injects intermediate text encoder hidden states to recover suppressed concepts without extra training, achieving up to 19.1% improvement in VQAScore on the new OAO-AttackBench benchmark.


<details>
  <summary>Details</summary>
Motivation: T2I diffusion models fail to faithfully render explicit textual descriptions for OAO objects due to concept association bias, where strong learned visual priors resist modification through prompting alone.

Method: IR-guided diffusion: injects intermediate hidden states of the text encoder into the conditioning signal during early denoising steps to recover concept-level information discarded in final text embeddings. No additional training, optimization, or external models required.

Result: Up to 19.1 percentage-point improvement in VQAScore on four benchmarks (including OAO-AttackBench) while preserving generation fidelity and human preference.

Conclusion: The proposed method effectively mitigates concept association bias for OAO objects by leveraging intermediate text representations, demonstrating significant improvements without extra computational cost.
...
TL;DR: VisReflect improves fine-grained perception in LVLMs for high-resolution images and long videos by using latent visual reflection instead of explicit bounding boxes, achieving 4.1% and 1.8% gains on image/video benchmarks while reducing inference time by ~44%.


<details>
  <summary>Details</summary>
Motivation: Existing LVLMs struggle with fine-grained perception in long visual contexts due to the visual attention sink phenomenon, where irrelevant tokens dominate attention. Prior zooming-based methods rely on unreliable discrete token predictions and incur high computational costs from multiple forward passes.

Method: VisReflect generates continuous visual reflection in the latent space (instead of discrete tokens) to represent question-relevant visual features, selectively emphasizing salient regions/frames and guiding attention within a single forward pass.

Result: Achieves consistent improvements on high-resolution image benchmarks (BLINK, V*, HRBench-4K/8K) and video benchmarks (MVBench, VideoMME, MLVU): +4.1% on images, +1.8% on videos. Reduces inference time by ~44% compared to zooming-based methods.

Conclusion: Latent visual reflection offers an efficient and effective alternative to explicit spatial/temporal localization for fine-grained perception in LVLMs, avoiding reliance on discrete token predictions and multiple forward passes.
...
TL;DR: ViDiHand leverages pretrained video diffusion model representations to reconstruct 4D hand motion from egocentric video, outperforming prior methods on ARCTIC, HOT3D, and HOI4D without needing detectors, infillers, or test-time optimization.


<details>
  <summary>Details</summary>
Motivation: Existing image-based and video-based methods for 4D hand motion reconstruction from egocentric video suffer from detector failures under occlusion and limited hand-pose annotations, which cannot adequately model motion dynamics, occlusion reasoning, and hand-object interaction.

Method: ViDiHand adapts a pretrained video diffusion model via a hand-overlay rendering objective to specialize its features for hands while preserving world priors, then uses a decoder to recover metric-scale hand pose from the adapted features, operating directly on full frames.

Result: ViDiHand substantially outperforms prior methods on ARCTIC, HOT3D, and HOI4D benchmarks.

Conclusion: Video diffusion models serve as a powerful new foundation for hand motion reconstruction and offer a promising route to scalable in-the-wild data collection for embodied AI.
...
TL;DR: This paper proposes a transformer-based framework using point clouds from RGBD data to automatically assess rehabilitation exercise quality. It features curve-based feature aggregation and axial self-attention to identify important joints, achieving superior performance on three benchmark datasets (Kimore, UI-PRMD, IRDS) with efficient inference and small model size.


<details>
  <summary>Details</summary>
Motivation: Rehabilitation exercises are costly, unavailable, and require expert supervision. Existing automatic assessment approaches fail to extract relevant features, lack practical applicability, require expensive pre-processing, or overlook crucial features.

Method: A transformer-based framework for point clouds that uses curve-based point-cloud feature aggregation to augment information and axial self-attention to recognize important joints and their roles.

Result: The system outperforms existing approaches on three baseline datasets (Kimore, UI-PRMD, IRDS). It is practically relevant due to small size, fast inference, and generalization on specific joints in similar exercises.

Conclusion: The proposed transformer-based framework with curve-based feature aggregation and axial self-attention effectively addresses limitations of prior work, offering a practical, cost-effective solution for automatic rehabilitation exercise assessment.
...
TL;DR: TRACE is a RANO 2.0-aligned concept bottleneck model for interpretable glioblastoma response classification on longitudinal 3D MRI, achieving competitive performance while enabling clinical inspection and correction through structured concept reasoning.


<details>
  <summary>Details</summary>
Motivation: Current deep learning methods for glioblastoma response assessment predict response labels directly from imaging features, limiting clinical inspection, verification, and correction. There is a need for transparent models aligned with established clinical criteria like RANO.

Method: TRACE uses a shared 3D vision encoder on paired baseline/follow-up multimodal MRI, predicts tumor measurements as root concepts, computes RANO-derived concepts via deterministic rules, and incorporates scan interval/new-lesion passthrough concepts, framing response assessment as structured concept reasoning.

Result: On LUMIERE dataset (5-fold CV), TRACE achieves 4-class macro F1 of 0.4769 and binary progression vs. non-progression macro F1 of 0.7085, improving over concept bottleneck baseline and within range of non-interpretable methods.

Conclusion: Structured concept bottlenecks offer a transparent, clinically aligned direction for longitudinal glioblastoma response assessment, though larger protocol-aligned datasets and external validation are needed.
...
TL;DR: A lightweight real-time underwater image enhancement framework using frequency-guided dual-path attention and reparameterizable convolutions with DCT priors, achieving 4.23K parameters and 600+ FPS.


<details>
  <summary>Details</summary>
Motivation: Real-time UIE for mobile/robotic systems needs low latency and compact models. Existing ultra-lightweight CNNs ignore frequency-sensitive underwater degradation, limiting performance.

Method: Proposes MBRConv-DCT (multi-branch reparameterizable convolution with fixed DCT priors) and FGDPA (frequency-guided dual-path attention) to fuse spatial and spectral cues, fully compatible with structural re-parameterization.

Result: 4.23K parameters, 600+ FPS, state-of-the-art quantitative metrics and visual quality, outperforming much larger methods.

Conclusion: Integrating frequency priors via structural re-parameterization enables ultra-lightweight yet high-performance UIE suitable for resource-constrained deployment.
...
TL;DR: BrainJanus is the first unified brain model integrating brain, vision, and language in a single framework, using a Unified Brain Tokenizer and All-in-One autoregressive architecture to enable bidirectional any-to-any generation (encoding and decoding), achieving superior performance and zero-shot generalization.


<details>
  <summary>Details</summary>
Motivation: Existing approaches treat brain encoding and decoding as isolated tasks, rely on unimodal alignment and external priors, and overlook the brain's intrinsic multimodal integration nature.

Method: 1) Unified Brain Tokenizer: quantizes continuous neural dynamics into discrete tokens aligned with visual and linguistic representations in a shared Omni space. 2) All-in-One autoregressive architecture: uses next-token prediction for any-to-any generation (image-to-brain, text-to-brain encoding; brain-to-image, brain-to-text decoding).

Result: Achieves superior performance across diverse benchmarks; exhibits zero-shot generalization and preserves interpretable biological topography.

Conclusion: BrainJanus highlights its potential as a general-purpose brain modeling paradigm, with code publicly available on GitHub.
...
TL;DR: This paper explores compressing facial images to 1024 bytes using various codecs (JPEG, JPEG 2000, JPEG XL, JPEG AI, HEIF, AVIF, WebP) for 2D barcodes on temporary travel documents. JPEG AI performs best for face recognition, especially with ICAO-compliant images. Grayscale helps for ICAO-compliant comparisons, while color is better for mixed-quality comparisons.


<details>
  <summary>Details</summary>
Motivation: Temporary travel documents lack machine readability (no RFID chip), excluding owners from automated biometric verification. Equipping them with 2D barcodes offers a cheaper, faster alternative, but storage is limited to 1024 bytes per facial image, creating the need to optimize compression while preserving face recognition performance.

Method: The study examines preprocessing steps (grayscale conversion, smoothing, resizing) and compression configurations using seven codecs: JPEG, JPEG 2000, JPEG XL, JPEG AI, HEIF, AVIF, and WebP, targeting a 1024-byte file size. Optimization is done for two scenarios: ICAO-compliant reference samples only, and mixed-quality samples.

Result: JPEG AI (with optimized settings) provides the best face recognition performance, especially when comparing high-quality face images. AVIF and WebP also perform well. Losses from strong compression are comparatively small. Grayscale preprocessing helps for ICAO-compliant comparisons, while color is better for mixed-quality comparisons. Smoothing and resizing before compression are beneficial.

Conclusion: JPEG AI is the most effective codec for extreme compression to 1024 bytes, enabling 2D barcode-based biometric verification on temporary travel documents as a viable, low-cost alternative to RFID chips.
...
TL;DR: UniGP unifies controllable image generation and dense prediction (e.g., depth estimation) within a single MMDiT-based framework via simple joint training, achieving performance on par with specialized methods while enabling cross-task benefits.


<details>
  <summary>Details</summary>
Motivation: Existing methods treat diffusion-based controllable generation and dense prediction as separate tasks, overlooking the potential benefits of jointly modeling their heterogeneous distributions.

Method: UniGP builds upon MMDiT with two key components: (1) DUGP, which copies the image branch to model dense distributions beyond RGB (following Occam's razor), and (2) a unified dataset training strategy that integrates heterogeneous datasets for joint training of generation and perception tasks.

Result: The unified model surpasses prior unified approaches and performs on par with specialized methods. Multi-task joint training provides complementary benefits: generative priors enrich perceptual details, while perceptual learning improves structural alignment in generation.

Conclusion: UniGP effectively unifies controllable generation and dense prediction without complex task-specific designs, demonstrating that joint training benefits both tasks.
...
TL;DR: A4D is a black-box, zero-shot adversarial attack detector using CLIP similarity scores, achieving SOTA detection across diverse attacks and classifiers without requiring attack-specific knowledge or classifier access.


<details>
  <summary>Details</summary>
Motivation: Adversarial attacks threaten deep learning reliability, but existing detection methods rely on attack-specific assumptions, adversarial samples, or white-box classifier access. A truly agnostic, black-box detector is needed.

Method: Uses CLIP-based prompt similarity scores as attack indicators, leveraging two observations: (i) CLIP is sensitive to imperceptible non-semantic perturbations; (ii) embedding shifts from attacks are not arbitrary and serve as robust indicators.

Result: Achieves state-of-the-art detection results across multiple attacks, datasets, and classifiers in attack-agnostic and classifier-agnostic settings.

Conclusion: CLIP can be effectively used as a zero-shot, black-box adversarial attack detector without needing any attack-specific training or classifier information.
...
TL;DR: Visual classifiers rely on low-level cues that fail under conflict, and this failure is shaped by early cue precision. Degraded-but-predictive input doesn't replace cue decorrelation. Conflict accuracy drops sharply with high texture predictability, and benefits of low early precision can be overwritten by fine-tuning.


<details>
  <summary>Details</summary>
Motivation: To understand whether early cue precision (reliability of low-level cues during early learning) shapes classifier failure under cue conflict or suppression.

Method: Synthetic shape-texture tasks, sequential digit training, 10-class frozen-representation audit, and CIFAR-10 texture-overlay benchmark; manipulates object-texture match probability.

Result: Conflict accuracy drops from 0.589 to 0.005 (10-class digits) and 0.569 to 0.114 (CIFAR-10) under high texture predictability. Texture choice rises from 0.049 to 0.855. Low early precision helps initially but fine-tuning overwrites this benefit.

Conclusion: Cue decorrelation must be maintained during downstream adaptation, not treated as a one-time inoculation.
...
TL;DR: FFAvatar is a Transformer-based 3D Gaussian framework that builds high-quality, animatable 4D head avatars from one or more reference images, using alternating attention, sparse-to-dense learning, and a motion refinement module for identity-consistent rendering across expressions and viewpoints.


<details>
  <summary>Details</summary>
Motivation: To overcome the limitations of existing feed-forward approaches that require fixed input views, enabling incremental reconstruction and refinement of animatable head avatars from a variable number of reference portrait images.

Method: 1) Alternating attention mechanism to disentangle identity, expression, and viewpoint. 2) Sparse-to-dense learning paradigm with FLAME-anchored primitives and UV-domain densification. 3) Plug-and-play motion refinement module for subject-specific dynamic personalization.

Result: FFAvatar efficiently produces high-fidelity, controllable 4D head avatars with superior flexibility, driving efficiency, and identity-consistent rendering across diverse expressions and viewpoints.

Conclusion: FFAvatar achieves state-of-the-art performance in constructing animatable 4D head avatars from few reference images, with the benefits of incremental reconstruction and fine-grained detail capture.
...
TL;DR: FastPano3D is an end-to-end framework that generates high-quality 3D Gaussian representations from a single panoramic image in seconds, achieving 156x faster inference than prior methods with half the parameters and no test-time optimization.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of rapidly reconstructing detailed 3D indoor scenes from minimal input (single panoramic image), overcoming the trade-offs among rendering quality, inference efficiency, and data dependency in existing methods.

Method: FastPano3D uses a lightweight feature encoder, adaptive Gaussian sampling, and a point-cloud-guided refinement strategy to directly generate renderable 3D Gaussian representations from a single panoramic image, without multi-view supervision or per-scene optimization.

Result: Achieves up to 156x faster inference than Pano2Room with only half the parameters, delivers rendering quality comparable to NeRF and 3DGS methods, and enables high-fidelity reconstruction within seconds.

Conclusion: FastPano3D establishes a new benchmark for rapid, single-view 3D scene inference by effectively handling panoramic image distortions and enabling efficient feed-forward generation without test-time optimization.
...
TL;DR: MARS is a mixture-of-experts framework for incomplete multimodal learning that uses a residual signal (gap between incomplete and complete representations) to guide expert specialization, with a dual-router system (residual router for training, feature router for deployment) and discrepancy-aware noise regularization to bridge the train-test gap.


<details>
  <summary>Details</summary>
Motivation: Real-world prediction systems often face missing modalities at inference time, and prior methods struggle because representations from incomplete inputs inevitably deviate from full-modality ones. The authors aim to explicitly leverage these deviations rather than just being robust to them.

Method: MARS uses a privileged residual signal (contrasting incomplete vs. complete task representations) to guide a residual router for expert specialization in a MoE framework. A feature router learns to imitate this routing using only incomplete inputs. Discrepancy-aware noise regularization adaptively perturbs residual router decisions when imitation is imperfect.

Result: MARS consistently outperforms baselines on multimodal classification (CASIA-SURF, CREMA-D, UPMC Food-101) and segmentation (MCubeS) under missing modality scenarios, while remaining efficient and extensible.

Conclusion: Explicitly leveraging representational deviations caused by missingness via residual-guided specialization is an effective and efficient approach for incomplete multimodal learning.
...
TL;DR: CouCE is a unified causal framework for Deep Metric Learning that simultaneously addresses two types of shortcut learning (background spurious correlations and foreground nuisance perturbations) via Orthogonal Dictionary-Based Backdoor Adjustment and Multi-Scale Randomized Causal Intervention, achieving SOTA on three benchmarks.


<details>
  <summary>Details</summary>
Motivation: Standard DML objectives capture co-occurrence rather than causation, making models vulnerable to two distinct confounders: background spurious correlations (backdoor paths via scene context) and foreground nuisance perturbations (pose, illumination). No prior method addresses both simultaneously.

Method: Two components: (1) ODBA isolates background spurious patterns via a variance-gated dictionary and disentangles them using soft orthogonal regularization; (2) MSRCI enforces causal invariance against foreground nuisances via multi-scale Fourier amplitude randomization and symmetric KL divergence constraint. Integrates with any proxy-based loss.

Result: State-of-the-art performance on CUB-200-2011, Cars-196, and Stanford Online Products; modest training overhead; no architectural changes needed at inference.

Conclusion: CouCE provides a principled, unified, and robust solution for debiased DML by addressing both confounder types causally, consistently outperforming existing methods.
...
TL;DR: MUSE repurposes the fixed sinusoidal timestep embedding in one-step diffusion models as a parameter-free task steering signal for multi-task dense prediction (depth & normal estimation), achieving competitive performance without extra adapters or task tokens.


<details>
  <summary>Details</summary>
Motivation: Existing multi-task diffusion methods rely on parameter-heavy adapters, experts, or learnable task tokens, leading to computational redundancy. The authors seek a more efficient multi-task learning paradigm.

Method: The authors discover that the native sinusoidal timestep embedding in one-step diffusion models can serve as an endogenous task steering signal. They propose MUSE, which uses timestep embedding to encode task identity. The mechanism is interpreted via Manifold Decoupling, where discrete timestep values steer generation toward decoupled task-specific latent manifolds.

Result: Extensive experiments across 10 datasets show MUSE achieves highly competitive performance on monocular depth and normal estimation, and generalizes across U-Net and DiT architectures.

Conclusion: The work offers a concise and efficient path toward generalist vision models by unlocking latent potential in existing diffusion infrastructure without adding parameters.
...
TL;DR: This paper proposes a probabilistic representation framework for brain tumor segmentation that models representations as Gaussian distributions to handle missing MRI modalities, using variance to measure uncertainty from missing evidence, with regularization and set-inclusive ordering constraints.


<details>
  <summary>Details</summary>
Motivation: Multimodal MRI is essential for accurate brain tumor segmentation, but acquiring all modalities at inference is often challenging, causing uncertainty from information loss. Existing methods lack reliability as they encode incomplete evidence into deterministic representations without modeling this uncertainty.

Method: Models representations as Gaussian distributions where mean captures task information and variance measures uncertainty from missing evidence. Regularizes mean from partial configurations toward full-modality counterpart, scales variance by discrepancy between aligned means, and uses a set-inclusive strategy with ordering constraints for hierarchical modality subset relationships.

Result: Superior performance over baselines across diverse missing-modality scenarios on BraTS 2018 and 2020 datasets.

Conclusion: The probabilistic framework effectively handles missing modality uncertainty, providing more reliable brain tumor segmentation when full MRI modalities are unavailable.
...
TL;DR: OmniCoT is a panoramic spatial reasoning benchmark suite for MLLMs that includes evaluation (6.7K), real-world (1K), and training (14.3K with structured CoT) data, plus OmniCoT-R1 model with two-stage training (SFT+GRPO) to enhance global multi-step reasoning across 360°×180° panoramas.


<details>
  <summary>Details</summary>
Motivation: Existing panoramic benchmarks rely on simple local cues or single-/few-step reasoning, failing to exploit the full potential of 360°×180° panoramas for global multi-step spatial reasoning, which is crucial for applications like embodied intelligence.

Method: They constructed OmniCoT suite with three datasets (B:6.7K eval, Real:1K manual, T:14.3K training with stepwise CoT). They also developed OmniCoT-R1 with a two-stage training: SFT anchors reasoning to panoramic evidence (bearings, proximity), and GRPO penalizes geometrically incoherent paths to enforce global 360° spatial consistency.

Result: The paper introduces the dataset and model but does not report quantitative experimental results in the abstract.

Conclusion: OmniCoT aims to recalibrate panoramic spatial reasoning difficulty to align with panoramas' intrinsic capabilities, fostering meaningful progress in this area.
...
TL;DR: This paper introduces SADL, the first benchmark for subject-aware distractor localization in photographs, comprising 1,800 cases across 1,000 photos with 14,617 annotated candidates. Evaluation of 7 VLMs reveals they excel at classifying distractors but over-apply exclusion rules, suppressing true distractors.


<details>
  <summary>Details</summary>
Motivation: Existing saliency models and open-vocabulary detectors lack subject awareness and fail to adapt to user intent. Context-agnostic removal can disrupt semantic coherence (e.g., removing the chair a person sits on). A systematic benchmark for subject-aware distractor localization is needed.

Method: Created SADL benchmark with 1,800 subject-aware cases across 1,000 photographs, including 14,617 annotated candidates and 1,938 hard negatives. Evaluated 7 proprietary/open-weight VLMs on a sequential pipeline of distractor classification followed by exclusion filtering, structured around 5 inclusion factors and 3 contextual exclusion rules.

Result: VLMs are highly capable at identifying distractors but systematically over-apply exclusion rules, which suppresses true distractors at scale.

Conclusion: SADL serves as a foundational diagnostic tool to advance subject-conditioned reasoning in multimodal systems by exposing the critical bottleneck of over-exclusion in VLMs.
...
TL;DR: SA-Homo is a scale-adaptive homography estimation framework that uses a hierarchical alignment strategy (global-to-local) with a Scale-aware Discrepancy Bridging Module (SDBM) and a lightweight Iterative Homography Estimation Refinement Module (IHERM), achieving robust performance even under 8× scale discrepancies. A new HMSA dataset is also contributed.


<details>
  <summary>Details</summary>
Motivation: Homography estimation under scale variation scenarios remains challenging, as existing deep learning methods rely on limited displacement assumptions and local feature consistency that fail under large scale gaps, causing significant performance degradation.

Method: A hierarchical scale alignment strategy: (1) Scale-aware Discrepancy Bridging Module (SDBM) with Multi-scale Linear Attention Cascade (MLAC) for long-range dependencies and global Cross-scale Similarity Matrix Block (CSMB) for robust correlation; (2) lightweight Iterative Homography Estimation Refinement Module (IHERM) for progressive refinement using local correlations. Also introduces the HMSA dataset.

Result: SA-Homo maintains high precision even under 8× scale discrepancies and outperforms state-of-the-art methods in both conventional scale-similar and challenging scale variation scenarios.

Conclusion: The proposed framework effectively addresses scale-variant homography estimation through a global-to-local hierarchical approach, setting a new benchmark in this domain with both the method and the HMSA dataset.
...
TL;DR: This paper proposes using conditioned denoising diffusion models for probabilistic forecasting of visual fields in glaucoma, producing well-calibrated distributions of future VF measurements that outperform deterministic methods in both accuracy and uncertainty representation.


<details>
  <summary>Details</summary>
Motivation: Existing visual field forecasting methods produce single deterministic predictions that fail to capture the inherent uncertainty due to heterogeneous disease progression and measurement variability, motivating the need for probabilistic approaches.

Method: Formulates VF forecasting as a probabilistic prediction problem using conditioned denoising diffusion models to generate distributions of plausible future VFs from longitudinal observations with irregular follow-up intervals.

Result: On two independent VF cohorts, diffusion-based predictions produce well-calibrated distributions for clinically relevant VF measures, and achieve state-of-the-art accuracy when reduced to point estimates.

Conclusion: Distributional modeling offers significant advantages for VF forecasting, supporting a shift toward uncertainty-aware, clinically interpretable risk assessment in glaucoma.
...
TL;DR: OWMDrive is a generative end-to-end driving framework using an Occupancy World Model for multi-step 3D occupancy forecasting to guide diffusion-based planning, improving trajectory reliability and safety in challenging scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing learning-based driving methods make decisions from static scene representations without explicit future forecasting or temporal causal modeling, leading to unstable or overly conservative planning under high-uncertainty conditions like occlusions.

Method: OWMDrive uses an Occupancy World Model to forecast multi-step 3D occupancy, serving as a conditional prior to guide diffusion-based planning. The planner iteratively refines trajectory candidates conditioned on current observations and predicted future states.

Result: OWMDrive significantly improves planning reliability and safety, especially in challenging and partially observable driving scenarios.

Conclusion: Explicitly modeling scene evolution over future horizons captures key spatiotemporal causal dependencies, leading to more foresighted and robust trajectory generation for autonomous driving.
...
TL;DR: KiloGS-SLAM is a monocular 3D Gaussian Splatting SLAM system that scales to kilometer-level outdoor environments via a motion-adaptive hybrid tracking module (handling geometric degeneracies) and a lifecycle-managed Gaussian mapping strategy (reducing memory redundancy), achieving SOTA results on 10,000+ frame sequences.


<details>
  <summary>Details</summary>
Motivation: Existing monocular 3DGS SLAM systems cannot scale to large outdoor environments due to two coupled challenges: fragile long-term pose tracking (drift) and excessive memory overhead during large-scale mapping.

Method: (1) A motion-adaptive hybrid tracking module with a condition-triggered three-tier pipeline that dynamically switches between Essential matrix and PnP models, and can activate a foundation model to rescue from catastrophic drift. (2) A lifecycle-managed Gaussian mapping strategy integrating probabilistic initialization with chunk-based multi-view densification and pruning to reduce primitive redundancy while preserving details.

Result: State-of-the-art tracking accuracy and rendering quality on three challenging outdoor datasets, successfully scaling to sequences of over 10,000 frames on a single GPU.

Conclusion: The proposed KiloGS-SLAM effectively addresses both tracking robustness and memory efficiency bottlenecks, enabling monocular 3DGS SLAM to scale to kilometer-level outdoor environments.
...
TL;DR: This paper introduces CRST, a CLIP-style framework addressing cross-resolution Text-to-Image Person Re-identification by tackling Evidence Reliability Collapse and Ranking Distribution Drift through resolution-conditioned reasoning, text-guided refinement, and cross-resolution ranking distribution alignment, achieving ~5.7% Rank-1 and ~5.3% mAP improvements on standard benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing TIPR methods overlook resolution variance in real-world surveillance, leading to two coupled failure modes: (1) Evidence Reliability Collapse (ERC) — degraded visual tokens become unreliable for grounding fine-grained text, and (2) Ranking Distribution Drift (RDD) — mixed-resolution galleries distort similarity neighborhoods and destabilize retrieval rankings.

Method: CRST (Cross-Resolution Semantic Transfer) is a CLIP-style framework with three modules: (1) Resolution-conditioned reasoning — estimates token reliability to suppress corrupted evidence; (2) Text-guided refinement — injects semantic priors to recover discriminative cues; (3) CR-RDA (Cross-Resolution Ranking Distribution Alignment) — transfers HR neighborhood geometry to stabilize LR ranking under mixed resolutions.

Result: On CUHK-PEDES, ICFG-PEDES, and RSTPReid, CRST improves ultra-low-resolution Rank-1 and mAP by 5.7% and 5.3% on average, while stabilizing mixed-resolution retrieval without sacrificing high-resolution accuracy.

Conclusion: The proposed CRST framework effectively addresses cross-resolution degradation in TIPR by jointly tackling evidence reliability and ranking distribution issues, achieving robust performance across varied resolutions.
...
TL;DR: FR-DETR is a detector-centric framework for object detection under adverse weather that refines features (not images) using frequency-domain and recurrent focus refinement, achieving superior accuracy and efficiency over enhancer-based methods.


<details>
  <summary>Details</summary>
Motivation: Object detection under adverse weather suffers from visual degradations and domain shifts. Existing enhancer-based methods cascade an enhancer with a detector, which introduces redundant feature extraction, high computational cost, and limited accuracy gains when paired with SOTA detectors.

Method: FR-DETR refines features rather than images by: (I) a Frequency Refinement Module that dynamically separates and reweights low-/high-frequency components to improve foreground-background discrimination, and (II) a Recurrent Focus Refinement Module (RFRM) that iteratively refines features using coarse predictions as guidance.

Result: FR-DETR achieves superior detection accuracy under adverse weather while being significantly more computationally efficient than enhancer-based methods.

Conclusion: A detector-centric feature refinement approach (FR-DETR) outperforms cascaded enhancer-detector pipelines in both accuracy and efficiency for adverse-weather object detection.
...
TL;DR: PS-MOT introduces point-supervised multi-object tracking as a cost-efficient alternative to bounding box supervision. PS-Track solves its challenges (spatial ambiguity, identity drift) via a hierarchical pipeline with Temporal-Feedback Prompting (data level), Point-Excited Wavelet Attention (model level), and Uncertainty-Guided Gaussian Learning (loss level), achieving SOTA on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: To reduce annotation cost in multi-object tracking by replacing expensive bounding box supervision with point-level supervision, while addressing the spatial ambiguity and identity drift problems inherent in point-based tracking.

Method: A hierarchical pipeline with three levels: (1) Data: Temporal-Feedback Prompting (TFP) uses negative spatial cues and motion priors to evolve points into temporally consistent pseudo-labels. (2) Model: Point-Excited Wavelet Attention (PEWA) leverages semantic correlations to activate high-frequency components and hallucinate object boundaries. (3) Loss: Uncertainty-Guided Gaussian Learning (UGL) models pseudo-labels as probabilistic distributions to adaptively calibrate supervision intensity.

Result: PS-Track achieves state-of-the-art performance on DanceTrack, EmboTrack, SportsMOT, and JRDB, demonstrating effectiveness across diverse tracking scenarios.

Conclusion: Point-supervised tracking is a feasible and effective alternative to bounding box supervision, and PS-Track provides a practical framework that can generalize across various tracking domains.
...
TL;DR: PGE-SAM enhances SAM's robustness to image degradations (noise, blur, compression) by using user prompts and prior masks to guide feature restoration, introducing multi-scale feature interaction and foreground reconstruction loss, and benchmarks on DM-Seg (degraded medical images). It achieves SOTA robustness with fewer parameters.


<details>
  <summary>Details</summary>
Motivation: SAM degrades substantially under real-world imaging artifacts (noise, blur, compression). Existing methods restore features globally without focusing on segmentation-relevant regions and neglect SAM's iterative refinement, leading to suboptimal interactive segmentation.

Method: (1) Prompt Guidance Generator to spatially guide feature restoration toward regions of interest using user prompts and prior masks. (2) Multi-Scale Features Interaction to recover fine-grained details from low-level encoder features. (3) Foreground Reconstruction Loss for feature-level supervision restricted to the segmentation target.

Result: PGE-SAM achieves SOTA robustness on both medical and natural image domains across multiple degradation levels, maintains generalization to clean images, and adds less than 1/5 the parameters of prior methods. Also introduces DM-Seg benchmark.

Conclusion: Prompt-guided, segmentation-aware feature restoration effectively improves SAM's robustness to various degradations with minimal parameter overhead, outperforming prior global restoration approaches.
...
TL;DR: RBE-Flow is a novel recurrent Bayesian estimation framework for cross-modal image registration that iteratively optimizes flow on learned feature manifolds, with uncertainty-adaptive self-correction and hybrid supervision, achieving state-of-the-art results with significant margins on multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Cross-modal image registration suffers from severe non-linear radiometric discrepancies and geometric distortions. Existing deterministic matching methods lack uncertainty awareness, struggle with highly non-convex optimization, and accumulate errors in ambiguous regions.

Method: RBE-Flow formulates dense cross-modal flow estimation as closed-loop recurrent Bayesian estimation on learned feature manifolds. Key components: (1) Recurrent Manifold Optimization (RMO) block iteratively generates flow observations + uncertainties; (2) Uncertainty-Adaptive Probabilistic Update (UAPU) assimilates observations into prior state via sigma-point projection; (3) Posterior covariance is fed back to adaptively regularize damping. A hybrid supervision scheme with geometry-aware rectified NLL loss prevents variance collapse.

Result: State-of-the-art performance on OSdataset, WHU-OPT-SAR, and RoadScene benchmarks, significantly outperforming existing methods especially under strict sub-pixel criteria.

Conclusion: The closed-loop feedback of calibrated uncertainty enables robust self-correction and adaptive convergence modulation, setting a new paradigm for uncertainty-aware cross-modal registration.
...
TL;DR: This paper identifies and formalizes two failure modes in Post-Hoc Concept Bottleneck Models (concept projections learned from auxiliary data with covariate shifts and noisy surrogate labels from vision-language models), showing that standard accuracy-based evaluation masks these issues, and proposes new metrics to separately measure concept faithfulness.


<details>
  <summary>Details</summary>
Motivation: Standard accuracy-based evaluation for post-hoc CBMs is insufficient—random concept projections can achieve competitive accuracy despite being semantically meaningless. The authors aim to directly analyze learned projections and detect when concepts are unfaithful.

Method: The authors formalize two failure cases: (1) covariate shift between auxiliary and target data (with an upper bound on the error), and (2) systematic label noise from vision-language model surrogate labels. They propose novel metrics that decouple concept faithfulness from predictive accuracy.

Result: Empirical results on real-world and synthetic benchmarks show that the proposed metrics successfully identify unfaithful behaviors that standard accuracy-based evaluation fails to detect.

Conclusion: Accuracy alone is misleading for evaluating post-hoc CBMs; directly measuring concept faithfulness is necessary. The introduced metrics provide a more reliable way to assess whether learned concepts are semantically meaningful.
...
TL;DR: This paper introduces an integrated flood mapping framework with a new Sentinel-1/2 dataset (contiguous US, 10m resolution) and learning strategies (shift-invariant loss for SAR misalignment, CVAE-based despeckling) that achieve strong multispectral performance (AUPRC up to 0.956) and significant SAR improvements over classical methods.


<details>
  <summary>Details</summary>
Motivation: Existing flood mapping methods are constrained by sensor-specific artifacts: clouds/shadows/urban confounders in optical imagery and speckle noise/coregistration uncertainty in SAR. Current benchmarks also lack challenging weather and urban scenarios.

Method: 1) Curated S2+S1 dataset with pixel-accurate 10m water masks via manual labeling; 2) Weakly labeled SAR data from temporally coincident S2 annotations; 3) Shift-invariant loss to handle SAR geolocation uncertainty; 4) CVAE trained on multitemporal SAR composites for speckle suppression; 5) Evaluation with UNet and UNet++ architectures.

Result: Multispectral AUPRC up to 0.956; statistically significant SAR improvements from shift-invariant loss and CVAE despeckling compared to classical filters.

Conclusion: Dataset fidelity, misalignment-robust training, and generative despeckling are critical for operational flood mapping, demonstrating viability of the proposed framework.
...
TL;DR: A scene-adaptive human image animation framework that controls both human motion and camera trajectories in 3D reconstructed environments, using ground-adaptive motion retargeting and viewpoint-adaptive latent fusion to achieve state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Existing human animation works face challenges in synthesizing trajectory-controllable human motion within natural scenes under changing camera views, as they rely on 2D skeleton/pose guidance and struggle with 3D scene adaptation.

Method: (1) Ground-adaptive 3D motion retargeting for automatic adaptation to ground elevations and orientations. (2) Viewpoint-adaptive latent fusion mechanism injecting point-cloud geometric priors via scene-visibility masking for camera-controlled viewpoint changes.

Result: Outperforms state-of-the-art methods on two standard human image animation benchmarks with notable improvements in video generation metrics.

Conclusion: The proposed scene-adaptive framework enables more realistic, trajectory-controllable human motion synthesis in 3D scenes with camera view control, advancing beyond 2D-guided approaches.
...
TL;DR: HASTE is a plug-and-play module that uses locality-sensitive hashing to dynamically merge redundant channels in pre-trained CNNs at inference time, achieving 46.2% FLOPs reduction on ResNet34 (CIFAR-10) with only 1.25% accuracy drop, without any retraining or data access.


<details>
  <summary>Details</summary>
Motivation: Existing dynamic execution methods for CNNs require specialized training or fine-tuning, limiting their applicability to pre-trained models and requiring data access. There is a need for a training-free, plug-and-play solution for compressing large pre-trained CNNs on resource-constrained devices.

Method: HASTE uses locality-sensitive hashing at inference time to identify and merge redundant channels of latent feature maps on a patch-wise basis, simultaneously compressing the depth of both input features and their corresponding filters for cheaper convolutions.

Result: On CIFAR-10 with ResNet34, HASTE achieves 46.2% FLOPs reduction with only 1.25% accuracy drop, without any retraining. Experiments across CIFAR-10 and ImageNet with various architectures validate its effectiveness.

Conclusion: HASTE provides an effective, steerable solution for training-free, runtime compression of pre-trained CNNs, enabling new possibilities for deploying efficient deep learning on resource-constrained devices. The approach is conceptually connected to token merging in Vision Transformers.
...
TL;DR: μFlow is a one-class deepfake detector trained only on real images that uses averaging to amplify generative traces and normalizing flow alignment to generalize across unseen generators, outperforming SOTA detectors.


<details>
  <summary>Details</summary>
Motivation: Existing deepfake detectors rely on supervised training with both real and fake images, limiting generalization across different generator categories (e.g., GANs vs diffusion models). The goal is to improve real-world applicability by detecting unseen deepfakes.

Method: (1) Average multiple real images to amplify consistent generative traces. (2) Model the distribution of features from averaged images using a normalizing flow. (3) Align individual image features with this distribution, producing a likelihood-based criterion to separate real and fake samples.

Result: μFlow significantly outperforms state-of-the-art detectors in a fully out-of-distribution setting where both real and fake datasets are unseen during training.

Conclusion: The one-class approach using averaged-image feature distributions and normalizing flow alignment achieves strong generalization for deepfake detection without needing fake training samples.
...
TL;DR: Orca is a general world foundation model that learns a unified latent space from multimodal world signals via Next-State-Prediction, using both unconscious learning (continuous videos) and conscious learning (language-described events), and demonstrates strong performance across text, image, and embodied action downstream tasks.


<details>
  <summary>Details</summary>
Motivation: To move beyond isolated next-token, next-frame, or next-action prediction and develop a unified world foundation model capable of understanding, predicting, and acting upon the world through a single shared latent space.

Method: Orca uses Next-State-Prediction modeling with two complementary paradigms: (1) unconscious learning from dense continuous video state transitions, and (2) conscious learning from sparse language-described events and VQA supervision. Pre-training uses 125K hours of video and 160M event annotations, with a frozen backbone and lightweight trainable modality-specific decoders.

Result: Orca outperforms similar-sized specialized baselines in three downstream tasks: text generation, image prediction, and embodied action generation, demonstrating scalability and that stronger world latent representations enable stronger downstream performance.

Conclusion: Orca presents a promising approach as a general world foundation model for understanding, predicting, and acting upon the world, though limitations remain that provide insights for future community work.
...
TL;DR: StereoGS is a sparse-view 3D Gaussian Splatting framework that uses stereo priors (instead of monocular) for absolute scale and binocular-consistent reconstruction, combined with a Gradient-Aware Opacity Decay and Consistency-Aware Dense Initialization, achieving state-of-the-art sparse-view novel view synthesis without extra inference cost.


<details>
  <summary>Details</summary>
Motivation: 3DGS overfits under sparse-view settings due to insufficient geometric constraints; existing monocular depth priors suffer from scale ambiguity and cross-view inconsistency, leading to defective geometry.

Method: (1) Stereo Depth Regularization: constructs virtual stereo pairs during optimization and uses a foundation stereo model to enforce absolute scale and binocular-consistent structures. (2) Gradient-Aware Opacity Decay: dynamically penalizes Gaussians based on relative opacity gradient magnitudes to suppress overfitting and remove redundant primitives. (3) Consistency-Aware Dense Initialization: uses zero-shot multi-view depth estimation to anchor primitives to accurate scene surfaces.

Result: State-of-the-art performance on LLFF, DTU, Mip-NeRF360, and Blender datasets under sparse-view settings, with no additional inference overhead.

Conclusion: Stereo priors are more effective than monocular priors for sparse-view 3DGS, and the proposed regularization/initialization strategies successfully mitigate overfitting and geometric defects.
...
TL;DR: EcoVideo is a dynamic framework for DiT video generation that uses entropy-orchestrated inter-frame decoupling to split denoising between cloud (high-entropy keyframes) and edge (motion-aware interpolation/refinement), achieving up to 2.9x speedup in low-bandwidth edge settings.


<details>
  <summary>Details</summary>
Motivation: DiT video generation is latency-heavy due to iterative full-frame denoising; existing cloud-edge methods rely on static step decoupling and fail to leverage inter-frame similarity or adapt to system dynamics.

Method: EcoVideo uses early-stage self-attention entropy as a training-free estimate of frame-wise information density to select keyframes. The cloud large model denoises sparse high-entropy keyframes, while an edge lightweight model reconstructs remaining frames via motion-aware interpolation with refinement. The keyframe budget and edge refinement depth adapt to real-time bandwidth and compute.

Result: Improves quality-efficiency trade-offs and achieves up to 2.9x end-to-end speedup in low-bandwidth, compute-limited edge settings on representative DiT video generators.

Conclusion: EcoVideo demonstrates that entropy-driven dynamic inter-frame decoupling can effectively reduce latency in cloud-edge video generation while maintaining quality, with adaptive mechanisms for varying system conditions.
...
TL;DR: The paper introduces a large-scale building dataset (multi-modal prompts, camera poses) and a single-stage Geometry-Aware Geo-localization framework (GAGeo) using a 3D foundation model, achieving state-of-the-art cross-view object geo-localization with zero-shot ground-to-drone capability.


<details>
  <summary>Details</summary>
Motivation: Existing CVOGL methods rely on 2D appearance matching and are limited by datasets lacking geometric metadata, diverse prompts, and standard FOV imagery, hindering accurate spatial modeling and generalization.

Method: Builds a large-scale dataset (220K+ pairs) with multi-modal prompts and camera poses. Proposes GAGeo, a single-stage framework integrating visual features, referring prompts, and task tokens into the 3D foundation model π³ for joint prediction of boxes, masks, and camera poses, plus contrastive loss with satellite as universal anchor for zero-shot ground-to-drone.

Result: Significantly outperforms state-of-the-art methods, demonstrating exceptional generalization in unseen scenes and novel cross-view setups, including zero-shot ground-to-drone localization.

Conclusion: The proposed dataset and geometry-aware framework effectively address key CVOGL challenges by incorporating 3D priors and multi-modal prompts, setting a new benchmark for cross-view object geo-localization.
...
TL;DR: APRIL-MedSeg is a modular, YAML-driven framework for 2D medical image segmentation that integrates multiple advanced paradigms (semi-supervised learning, domain adaptation, knowledge distillation, etc.) with a registry-based configuration system for flexible, reproducible research and deployment.


<details>
  <summary>Details</summary>
Motivation: To address the lack of a unified, modular, and extensible ecosystem for medical image segmentation that bridges algorithmic innovation with practical deployment and supports reproducible experiment management across diverse learning paradigms.

Method: A YAML-driven modular framework decomposing segmentation networks into reusable components, featuring a registry-based configuration system with inheritance, unified interfaces for datasets/augmentation/deployment/ensembling, and integration of multiple advanced paradigms including semi-supervised learning, domain adaptation, knowledge distillation, weakly supervised learning, text-guided segmentation, and foundation models.

Result: A general-purpose research and development platform providing flexible, reproducible experiment management with seamless switching across models, datasets, and training strategies, available as open-source code under Apache 2.0 license.

Conclusion: APRIL-MedSeg serves as a structured ecosystem for systematically organizing and reproducing advances in medical image segmentation while bridging research and deployment.
...
TL;DR: This paper proposes a novel "learn-from-latent-prompts" paradigm for LMMs facing missing-modality scenarios, using input-agnostic latent prompts as stable anchors for robust cross-modal compensation, achieving SOTA results even at 90% missing rates.


<details>
  <summary>Details</summary>
Motivation: Existing LMMs suffer from severe performance degradation when modalities are missing. Prior prompt-learning methods rely on instance-level features which become unreliable as missing rates increase, limiting model robustness.

Method: Proposes "Learning from Reliable Latent Prompts" — modeling input-agnostic learnable prompts as stable latent anchors that provide robust guidance and cross-modal knowledge compensation, decoupled from corrupted input features.

Result: Achieves state-of-the-art performance across three benchmark datasets under various missing-modality scenarios, including extreme missing rates (e.g., 90%).

Conclusion: The learn-from-latent-prompts paradigm offers a robust and effective solution to the missing-modality problem in large-scale multimodal models.
...
TL;DR: This paper introduces EPIC-Contact, a large egocentric dataset with dense hand-object contact annotations, and HOPformer, a transformer model that jointly predicts bi-manual hand and object pose, achieving significant improvements over prior SOTA in both in-lab and in-the-wild settings.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with severe occlusions and ambiguous contact in egocentric RGB, lack generalization to in-the-wild scenes, and are limited by insufficient supervision data.

Method: (1) EPIC-Contact dataset: 2.3K clips (62.3K frames) with dense, bijective 3D hand-object contact correspondences and posed meshes. (2) HOPformer: an end-to-end transformer with a cross-attention decoder that conditions object features on hand priors for joint bi-manual hand and object pose estimation in a single forward pass.

Result: HOPformer achieves 82.4% success rate on ARCTIC (+6.2 pts over SOTA). On EPIC-Contact, it nearly doubles success rate while reducing contact deviation by 75%.

Conclusion: The proposed dataset and model effectively address occlusions and generalization challenges in egocentric hand-object pose estimation, setting new state-of-the-art results on both in-lab and in-the-wild benchmarks.
...
TL;DR: The paper introduces Goku, a 2M-pair video editing dataset extending from single-task appearance editing to multi-task and structural manipulations, along with Goku-Edit (a dual-branch MLLM-based model) and Goku-Bench (a 1000-case benchmark with 7 novel metrics), achieving 8% improvement in instruction following.


<details>
  <summary>Details</summary>
Motivation: Existing video editing datasets focus only on single-task appearance editing, which cannot meet complex real-world demands for multi-task and structural manipulations (e.g., controlling subject movement).

Method: (1) A large-scale dataset Goku with 2M pairs covering multi-task and structural edits; (2) An efficient data synthesis pipeline decomposing complex edits into controllable sub-problems with progressive filtering; (3) Goku-Edit model using an MLLM text encoder with a decoupled dual-branch design (mask branch for structure, main branch for appearance).

Result: Goku-Edit achieves up to +8% improvement over other open-source models on instruction following, evaluated on Goku-Bench with 1,000 human-verified cases and 7 novel metrics.

Conclusion: Scaling to complex multi-task and structural video editing via a well-designed dataset, pipeline, model architecture, and benchmark yields significant gains in instruction adherence, advancing video editing toward real-world applicability.
...
TL;DR: This paper presents a debate-driven agentic approach for reconstructing articulated 3D objects from text or images, using high-level and low-level agents that debate articulation parameters, then leverage a video generative prior to expose occluded geometry and produce complete 3D articulated models with motion-consistent states.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle to reconstruct articulated 3D objects from sparse observations due to lack of supervised data and reliable priors for recovering articulation, hidden geometry, and internal structure.

Method: Two-round structured debate: (1) high-level agents (using vision-language/video model knowledge) and low-level agents (estimating articulation parameters) engage in global-local disagreement; (2) agents are grounded in freely generated video. A video generative prior then drives each part through its motion to expose occluded interiors.

Result: The approach jointly infers articulation and reconstructs complete 3D articulated objects with high-fidelity geometry, internal structure, and motion-consistent states beyond observed surfaces.

Conclusion: Combining agentic reasoning with a video generative prior enables reliable articulated 3D reconstruction from sparse inputs, including occluded geometry and internal structure.
...
TL;DR: MiRA is a plug-in framework that redistributes attention in Vision Transformers to better capture subtle facial dynamics for facial expression recognition, without adding extra parameters. It offers an exact mode and a more efficient flashLite mode, showing consistent improvements on FER benchmarks.


<details>
  <summary>Details</summary>
Motivation: ViT-based video models struggle with subtle facial expression dynamics because their attention mechanisms prioritize dominant global motions and coarse temporal cues, missing fine-grained facial variations.

Method: MiRA introduces frame-marginal attention redistribution using two modes: (1) exact mode (post-softmax redistribution) computing frame-level confidence and intra-frame concentration statistics from self-attention maps; (2) flashLite mode (pre-softmax approximation) integrated into FlashAttention for efficiency.

Result: Consistent improvements over strong ViT baselines on challenging Facial Expression Recognition benchmarks.

Conclusion: MiRA effectively enhances spatio-temporal selectivity toward subtle facial dynamics in ViT backbones without adding trainable parameters, offering both accuracy and efficiency.
...
TL;DR: GaussDet enhances 3D Gaussian Splatting with open-vocabulary scene understanding by replacing dense CLIP features with discrete 2D object detectors and a View-Aggregated Semantic Label Distribution (VASD), significantly improving referring expression grounding (+16.7% mIoU).


<details>
  <summary>Details</summary>
Motivation: Existing 3DGS language-driven methods suffer from (1) poor instance grouping (requiring predefined instance numbers or producing noisy bottom-up groupings) and (2) reliance on CLIP features that limit understanding to simple noun phrases, preventing complex spatial reasoning and referential grounding.

Method: GaussDet learns instance features for individual Gaussians to decompose scenes into 3D instance groups, then renders these groups and aggregates semantic votes from multi-view 2D detections (with referring expression capabilities) to produce View-Aggregated Semantic Label Distributions (VASD), which act as a regularizer against noisy groupings.

Result: Consistent improvements over existing methods on open-vocabulary segmentation (LeRF-OVS, ScanNet) and referring expression grounding (Ref-LeRF), with a notable 16.7% mIoU improvement in zero-shot referential grounding.

Conclusion: GaussDet provides a zero-shot extension from simple language queries to complex referential grounding without needing dense CLIP features, offering a more robust and capable framework for language-driven 3D scene understanding.
