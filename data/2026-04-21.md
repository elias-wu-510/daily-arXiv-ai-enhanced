...
TL;DR: End-to-end Optical Music Recognition framework combining ResNet-v2-style residual bottleneck convolutions with BiGRU sequence modeling achieves state-of-the-art performance on Camera-PrIMuS and PrIMuS datasets with high computational efficiency.


<details>
  <summary>Details</summary>
Motivation: Optical Music Recognition (OMR) needs to convert music score images into symbolic representations, requiring robust feature extraction for both fine-grained symbol details and global staff-line structures, along with effective temporal modeling of musical symbols.

Method: Proposes an end-to-end framework using CNN with ResNet-v2-style residual bottleneck blocks and multi-scale dilated convolutions for feature extraction, followed by BiGRU network for sequence modeling, trained with Connectionist Temporal Classification loss without alignment annotations.

Result: On Camera-PrIMuS: SeER 7.52%, SyER 0.45%, pitch accuracy 99.33%, type accuracy 99.60%, note accuracy 99.28%. On PrIMuS: SeER 8.11%, SyER 0.49%, pitch accuracy 99.27%, type accuracy 99.58%, note accuracy 99.21%. Average training time 1.74s per epoch.

Conclusion: The proposed framework demonstrates strong recognition performance with high computational efficiency, effectively combining residual bottleneck convolutions with BiGRU sequence modeling for end-to-end OMR without explicit alignment annotations.
...
TL;DR: HalfV is a framework that accelerates high-resolution MLLMs by addressing visual token redundancy through a universal three-stage inference lifecycle, achieving 4.1× FLOPs speedup with minimal performance loss.


<details>
  <summary>Details</summary>
Motivation: High-resolution MLLMs suffer from prohibitive computational costs due to exploding visual tokens. Existing acceleration methods have "backbone dependency" - they work well on some architectures (Vicuna/Mistral) but degrade significantly on others (Qwen).

Method: Uses truncated matrix entropy to discover a universal three-stage inference lifecycle, decoupling visual redundancy into Intrinsic Visual Redundancy (IVR) and Secondary Saturation Redundancy (SSR). HalfV first mitigates IVR via unified pruning, then adaptively handles SSR based on its specific manifestation.

Result: HalfV achieves superior efficiency-performance trade-offs across diverse backbones. On Qwen25-VL, it retains 96.8% performance at 4.1× FLOPs speedup, significantly outperforming state-of-the-art baselines.

Conclusion: The proposed framework effectively addresses backbone dependency in MLLM acceleration by identifying and handling both universal and architecture-specific visual redundancies, enabling efficient high-resolution multimodal inference across diverse model architectures.
...
TL;DR: Proposes a latent compression method for video VAEs that removes high-frequency components instead of reducing channels, improving reconstruction quality while maintaining compression ratio.


<details>
  <summary>Details</summary>
Motivation: Video VAEs need many latent channels for high-quality reconstruction, but too many channels hurt diffusion model convergence and generative performance, even when reconstruction is good. Direct channel reduction compromises reconstruction fidelity.

Method: A latent compression method that removes high-frequency components in video latent representations rather than directly reducing the number of channels.

Result: Achieves superior video reconstruction quality compared to strong baselines while maintaining the same overall compression ratio.

Conclusion: The proposed frequency-based compression approach effectively balances reconstruction quality with diffusion model performance, offering a better alternative to channel reduction for video latent compression.
...
TL;DR: A drone-mounted stereo vision system for detecting and localizing radiata pine branches to enable autonomous pruning, comparing various segmentation and depth estimation methods.


<details>
  <summary>Details</summary>
Motivation: To support autonomous pruning in forestry by developing a low-cost stereo vision system that can accurately detect and localize tree branches from a drone platform.

Method: Two-stage pipeline: 1) Branch segmentation comparing YOLOv8, YOLOv9, and Mask R-CNN variants on custom stereo dataset; 2) Depth estimation comparing traditional SGBM with WLS filtering vs. deep learning methods (PSMNet, ACVNet, GWCNet, MobileStereoNet, RAFT-Stereo, NeRF-Supervised Deep Stereo). Uses centroid-based triangulation with MAD outlier rejection for distance computation.

Result: Deep learning-based disparity maps produce more coherent depth estimates than traditional SGBM at 1-2m distances, demonstrating feasibility of low-cost stereo vision for automated branch positioning.

Conclusion: The proposed stereo vision system shows promise for autonomous forestry pruning applications, with deep learning methods outperforming traditional approaches for depth estimation.
...
TL;DR: ETC is a scalable framework that can erase thousands of concepts from text-to-image diffusion models while maintaining generation quality, using t-distribution modeling and Mixture-of-Experts erasure with robustness to attacks.


<details>
  <summary>Details</summary>
Motivation: Large-scale text-to-image diffusion models pose safety risks by reproducing undesirable content like copyrighted material. Existing concept erasure methods struggle with scalability, precision, and robustness, limiting them to only a few hundred concepts.

Method: 1) Models low-rank concept distributions via Student's t-distribution Mixture Model (tMM). 2) Performs pin-point erasure via affine optimal transport while preserving other concepts by anchoring boundaries. 3) Trains a Mixture-of-Experts (MoE)-based module called MoEraser to remove target embeddings while preserving anchor embeddings. 4) Injects noise into text embedding projector and fine-tunes MoEraser for recovery to achieve robustness against attacks.

Result: Extensive experiments on over 2,000 concepts across heterogeneous domains and diffusion models demonstrate state-of-the-art scalability and precision in large-scale concept erasure.

Conclusion: ETC provides a scalable framework for erasing thousands of concepts from text-to-image diffusion models while balancing erasure effectiveness, generation quality preservation, and robustness to attacks.
...
TL;DR: Survey examines spatial memory efficiency in robotics, introduces α ratio (peak runtime memory / saved map size), shows neural methods vary widely (α=2.3-215), proposes standardized evaluation protocol, and provides α-aware budgeting for deployment feasibility.


<details>
  <summary>Details</summary>
Motivation: Vision-based robots face unbounded spatial memory growth that exhausts computational resources on embedded platforms with limited hardware (8-16GB shared memory, <30W), requiring solutions for memory-efficient deployment.

Method: Analyzes 88 references across 52 systems (1989-2025), introduces α ratio metric, performs independent profiling on NVIDIA A100 GPU, proposes standardized evaluation protocol (memory growth rate, query latency, memory-completeness curves, throughput degradation), and conducts Pareto frontier analysis with benchmark separation.

Result: α spans two orders of magnitude in neural methods (2.3 for Point-SLAM to 215 for NICE-SLAM), no single paradigm dominates within evaluation regimes, 3DGS methods achieve best accuracy at 90-254MB on Replica, scene graphs provide semantic abstraction at predictable cost.

Conclusion: Memory architecture, not paradigm label, determines deployment feasibility; provides first independently measured α reference values and α-aware budgeting algorithm for practitioners to assess deployment feasibility on target hardware before implementation.
...
TL;DR: Dynamic Semantic Steering (DSS) is a training-free framework for precise concept erasure in text-to-image diffusion models, achieving 91% erasure rate with minimal fidelity loss.


<details>
  <summary>Details</summary>
Motivation: Existing inference-time methods for concept erasure in T2I diffusion models have limitations: feature-correction causes uncontrolled over-correction, token-level interventions struggle with semantic granularity, and both suffer from semantic drift or representation collapse.

Method: DSS introduces Sensitive Semantic Boundary Modeling (SSBM) to automatically discover safe semantic anchors, and Sensitive Semantic Guidance (SSG) that uses cross-attention features for precise detection and performs correction via a closed-form solution from a well-posed objective.

Result: DSS achieves an average erasure rate of 91.0%, significantly outperforming SOTA methods (which range from 18.6% to 85.9%) with minimal impact on output fidelity.

Conclusion: DSS provides a lightweight, training-free framework for interpretable and controllable concept erasure that optimally suppresses sensitive content while preserving benign semantics, addressing key limitations of existing methods.
...
TL;DR: CLWM introduces a causal latent world model using DINOv3 features for robust manipulation, with O(1) memory scaling via dual-state TTT memory, 50% latency reduction via speculative async inference, and EmbodiChain for scalable training.


<details>
  <summary>Details</summary>
Motivation: Current generative world-action models for manipulation suffer from redundant pixel-level reconstruction, O(T) memory scaling with horizon length, and sequential inference latency that bottlenecks real-world deployment.

Method: 1) Uses DINOv3 features as generative targets to disentangle interaction semantics from visual noise; 2) Dual-State Test-Time Training Memory for O(1) memory footprint; 3) Speculative Asynchronous Inference to mask diffusion denoising behind physical execution; 4) EmbodiChain framework for scalable training with physics-grounded trajectories.

Result: Achieves SOTA performance in complex dual-arm simulation and unprecedented zero-shot sim-to-real transfer on physical robots, outperforming baselines explicitly finetuned on real-world data.

Conclusion: CLWM successfully addresses key bottlenecks in generative world models for manipulation through semantic feature targets, constant memory scaling, reduced inference latency, and scalable training, enabling robust real-world deployment.
...
TL;DR: Saccade Attention Network reduces transformer computation by 80% using learned sparse attention on key image features, mimicking human saccadic eye movements.


<details>
  <summary>Details</summary>
Motivation: Transformers suffer from quadratic attention complexity with sequence length. Human vision uses sparse attention (saccades) focusing on key features, suggesting computational efficiency can be gained through similar sparse attention mechanisms.

Method: Proposes Saccade Attention Network that learns where to attend from a large pre-trained model. It pre-processes images by identifying and extracting only key features, reducing input sequence length to just attended regions rather than full images.

Result: Achieves approximately 80% reduction in calculations while producing similar results to full attention approaches.

Conclusion: Sparse attention inspired by human saccades can dramatically reduce transformer computation without significant performance loss, enabling more efficient vision transformers.
...
TL;DR: PhyLAA-X integrates physics-derived features (optical-flow curl, specular-reflectance skewness, rPPG power spectra) into deepfake detection via cross-attention gating, improving robustness against cross-generator shifts, compression, and adversarial attacks.


<details>
  <summary>Details</summary>
Motivation: Current deepfake detectors fail under cross-generator shifts, compression, and adversarial perturbations because they decouple semantic artifact learning from physical invariants like optical-flow discontinuities, specular-reflection inconsistencies, and cardiac-modulated reflectance.

Method: PhyLAA-X injects three physics-derived feature volumes into LAA-X attention via cross-attention gating and resonance consistency loss, forcing learning of manipulation boundaries where semantic and physical inconsistencies co-occur. Embedded in an efficient spatiotemporal ensemble with uncertainty-aware adaptive weighting.

Result: Achieves 97.2% accuracy on FaceForensics++ (c23), 94.9% on Celeb-DF v2, 90.8% on DFDC, outperforming LAA-Net by 4.1-7.3% in cross-generator settings. Maintains 79.4% accuracy under adversarial attacks. Single-backbone ablation shows 4.2% cross-dataset AUC gain.

Conclusion: Physics-conditioned attention mechanisms significantly improve deepfake detection robustness by jointly modeling semantic artifacts and physical invariants, making detectors more resilient to distribution shifts and adversarial perturbations.
...
TL;DR: The paper introduces two inference-time methods to improve CLIP retrieval by addressing local geometric inconsistencies: neighborhood-level re-ranking via Hungarian matching and query-conditioned local steering.


<details>
  <summary>Details</summary>
Motivation: CLIP retrieval suffers from local geometric inconsistencies where nearby items are incorrectly ordered, leading to systematic confusions (e.g., pentagon vs. hexagon) and diffuse, weakly controlled result sets. While CLIP achieves strong global cross-modal alignment, these local failures degrade retrieval quality.

Method: Two inference-time methods: (1) Neighborhood-level re-ranking via Hungarian matching that rewards structural consistency, and (2) Query-conditioned local steering where directions derived from contrastive neighborhoods around the query reshape retrieval. Both operate on local neighborhoods without retraining.

Result: The techniques improve retrieval performance on attribute-binding and compositional retrieval tasks. They demonstrate that retrieval quality and controllability depend critically on local structure.

Conclusion: Local geometric structure in CLIP embeddings can be exploited at inference time without retraining to improve retrieval quality and controllability. The two methods serve complementary roles: re-ranking rewards alignment while local steering controls neighborhood structure.
...
TL;DR: Novel loss function combining categorical cross-entropy and fuzzy entropy for brain MRI segmentation, addressing uncertainty in pixel classification.


<details>
  <summary>Details</summary>
Motivation: Brain image segmentation is crucial for neurological disease diagnosis, but existing loss functions don't adequately handle uncertainty in pixel classifications during MRI segmentation.

Method: Proposed a new loss function integrating categorical cross-entropy (CCE) with fuzzy entropy based on fuzzy logic to account for uncertainty in pixel classifications.

Result: Evaluated on IBSR and OASIS datasets using U-Net and U-Net++ architectures, showing better performance than CCE alone across various metrics while effectively handling uncertainty.

Conclusion: The fuzzy logic-enhanced loss function improves segmentation outcomes and enhances prediction reliability by addressing uncertainty in brain image segmentation.
...
TL;DR: Lightweight transformer architecture for pain assessment using fNIRS data fusion through unified tokenization, achieving competitive performance with low computational cost.


<details>
  <summary>Details</summary>
Motivation: Pain is a complex, widespread problem with significant clinical and societal impact, creating a need for reliable automated assessment methods.

Method: Proposes a lightweight transformer that fuses multiple fNIRS representations via unified tokenization, preserving spatial, temporal, and time-frequency characteristics through structured segmentation and shared latent representation projection.

Result: Demonstrates competitive pain recognition performance on AI4Pain dataset using stacked raw waveform and power spectral density representations, while maintaining computational efficiency for real-time inference on both GPU and CPU hardware.

Conclusion: The approach provides an effective, computationally compact solution for automated pain assessment suitable for real-world clinical applications.
...
TL;DR: LayerCache: A layer-aware caching framework for Flow Matching models that partitions Transformers into layer groups and makes independent caching decisions per group, achieving better quality-speed trade-offs than monolithic caching approaches.


<details>
  <summary>Details</summary>
Motivation: Flow Matching models have high inference costs due to iterative denoising through large Transformers. Existing caching methods treat the entire Transformer as a monolithic unit, failing to exploit the heterogeneous velocity dynamics where shallow layers are stable (amenable to caching) while deep layers undergo large velocity changes (requiring full computation).

Method: LayerCache partitions the Transformer into layer groups and makes independent per-group caching decisions at each denoising step. It introduces an adaptive JVP (Jacobian-Vector Product) span K selection mechanism that uses per-group stability measurements to balance estimation accuracy and computational savings. The method formulates a three-dimensional scheduling problem over timesteps, layer groups, and JVP span, solved with a greedy budget allocation algorithm.

Result: On Qwen-Image (1024x1024, 50 steps), LayerCache achieves PSNR 37.46 dB (+5.38 dB over MeanCache), SSIM 0.9834, and LPIPS 0.0178 (70% reduction over MeanCache) at 1.37x speedup, dominating all prior caching methods on the quality-speed Pareto frontier.

Conclusion: LayerCache effectively exploits the heterogeneous velocity dynamics in Transformer layers for Flow Matching models, providing superior quality-speed trade-offs through layer-aware caching decisions and adaptive JVP span selection, outperforming existing monolithic caching approaches.
...
TL;DR: HQA-VLAttack is a black-box adversarial attack framework for vision-language models that generates high-quality adversarial examples through text and image perturbation stages, achieving better attack success rates than existing methods.


<details>
  <summary>Details</summary>
Motivation: Black-box adversarial attacks on vision-language models are challenging due to simultaneous text-image perturbations and limited access to model internals. Existing methods either use inefficient iterative cross-search strategies or only reduce positive pair similarity while ignoring negative pairs, limiting attack performance.

Method: Two-stage framework: 1) Text perturbation uses counter-fitting word vectors to generate semantically consistent substitute words; 2) Image perturbation initializes adversarial examples via layer-importance guided strategy, then optimizes using contrastive learning to decrease positive pair similarity while increasing negative pair similarity.

Result: Experimental results on three benchmark datasets show HQA-VLAttack significantly outperforms strong baselines in attack success rate.

Conclusion: HQA-VLAttack provides an effective solution for black-box adversarial attacks on vision-language models by addressing limitations of existing methods through semantic-preserving text perturbations and contrastive optimization of image perturbations.
...
TL;DR: VFCNet proposes a novel composition assessment method using gradient vector flow fields that fuse saliency and edge information, achieving state-of-the-art performance on the PICD benchmark.


<details>
  <summary>Details</summary>
Motivation: Reliable computational assessment of photographic composition requires features that are discriminative of spatial layout while being robust to semantic content. The paper is motivated by the assumption that composition can be understood as the flow of visual attention across geometric structure.

Method: VFCNet fuses saliency and edge information into a gradient vector flow (GVF) field. The model computes dual-stream GVF representations, integrates them via attention mechanisms, and extracts multi-scale flow features using a DINOv3 backbone.

Result: VFCNet achieves state-of-the-art performance on the PICD benchmark with CDA-1: 0.683 and CDA-2: 0.629, improving by 33.1% and 36.1% over previous best methods. The paper also shows that a simple classifier on self-supervised DINOv3 features substantially outperforms more sophisticated composition-specialized models.

Conclusion: The proposed VFCNet demonstrates that fusing saliency and edge information into GVF fields provides an effective low-level representation for composition assessment, and that self-supervised features from DINOv3 are surprisingly powerful for this task even with simple classifiers.
...
TL;DR: Topology-aware layer pruning framework for Large Vision-Language Models using simplicial complexes and zigzag persistent homology to preserve critical representational transitions while reducing computational costs.


<details>
  <summary>Details</summary>
Motivation: Large Vision-Language Models (LVLMs) have high computational and memory costs that hinder deployment in resource-constrained scenarios. Existing layer pruning methods fail to capture the global and dynamic evolution of representations across model depth, often removing transition-critical layers.

Method: Proposes a topology-aware layer pruning framework that represents layer-wise hidden states as point clouds and models their evolution using simplicial complexes. Uses zigzag persistent homology to quantify inter-layer topological consistency and enable adaptive pruning that preserves critical representational transitions.

Result: Extensive experiments on diverse multimodal benchmarks demonstrate that the proposed framework consistently outperforms existing pruning methods across a wide range of sparsity ratios.

Conclusion: The topology-aware approach effectively addresses limitations of existing pruning methods by capturing global representational evolution, enabling efficient compression of LVLMs while maintaining performance.
...
TL;DR: Motif-Video 2B achieves strong text-to-video quality with 7× fewer parameters than larger models by using architectural specialization and efficient training, rather than just scaling up compute and data.


<details>
  <summary>Details</summary>
Motivation: Current video generation models require massive datasets, large parameter counts, and substantial compute. The paper investigates whether strong text-to-video quality is possible at a much smaller budget (fewer than 10M clips and less than 100,000 H200 GPU hours).

Method: Uses architectural specialization with two key ideas: 1) Shared Cross-Attention to strengthen text control with long video token sequences, and 2) a three-part backbone separating early fusion, joint representation learning, and detail refinement. Paired with efficient training using dynamic token routing and early-phase feature alignment to a frozen pretrained video encoder.

Result: Achieves 83.76% on VBench, surpassing Wan2.1 14B while using 7× fewer parameters and substantially less training data. Analysis shows later blocks develop clearer cross-frame attention structure than standard single-stream baselines.

Conclusion: Careful architectural specialization combined with efficiency-oriented training can narrow or exceed the quality gap typically associated with much larger video models, challenging the assumption that video generation quality requires massive scale.
...
TL;DR: Benchmarking 17 multimodal LLMs on challenging medical form digitization shows latest models achieve ~85% accuracy, with Gemini 3.1 performing best overall and prompt optimization boosting performance by 60+% on some metrics.


<details>
  <summary>Details</summary>
Motivation: Manual digitization of structured handwritten documents is slow and costly, especially problematic in low- and middle-income countries. There's a need to benchmark whether modern multimodal LLMs can automate this challenging task.

Method: Benchmarked 17 leading frontier multimodal LLMs (including Google, OpenAI, and open-source models) on a challenging real-world medical form containing dates, printed text, handwritten responses, and significant variability. Evaluated performance across discrete/predefined fields and free text, with prompt optimization experiments.

Result: Latest Google and OpenAI models achieved ~85% accuracy with weighted F1 scores ~90%. Gemini 3.1 performed best overall with lowest free text error rates (WER=0.50, CER=0.31). GPT 5.4 excelled in noisy date extraction with lowest hallucination rate (6%). Claude Sonnet 4.6 best on formatted fields. Prompt optimization improved macro precision, recall, and F1 by over 60%.

Conclusion: Rapid improvements in multimodal LLMs offer a compelling pathway toward fully automated digitization of complex handwritten workflows, particularly relevant for low- and middle-income countries where manual digitization is costly.
...
TL;DR: A novel hybrid model combining DINOv2 vision transformer with enhanced LSTM and multi-head attention achieves 96.4% accuracy in predicting blastocyst formation from limited daily embryo images, outperforming existing methods and handling missing frames effectively.


<details>
  <summary>Details</summary>
Motivation: Current IVF embryo selection relies on manual inspection of time-lapse imaging data, which is challenging due to limited daily images and many clinics lacking complete time-lapse systems. There's a need for automated prediction of blastocyst formation from limited imaging data.

Method: Proposed a hybrid model combining DINOv2 (transformer-based vision model) with enhanced LSTM network featuring multi-head attention layer. DINOv2 extracts features from embryo images, and LSTM analyzes temporal development patterns to predict blastocyst formation.

Result: Achieved 96.4% accuracy on real dataset of 704 embryo videos, surpassing existing methods. Model performs well even with missing frames, making it suitable for clinics with limited imaging systems.

Conclusion: The proposed approach can assist embryologists in selecting better embryos more efficiently and confidently, addressing practical limitations in IVF laboratories while improving prediction accuracy for blastocyst formation.
...
TL;DR: MedThinkVQA is a new medical VQA benchmark requiring multi-image reasoning, where current LLMs struggle with cross-view evidence integration despite strong single-image performance.


<details>
  <summary>Details</summary>
Motivation: Real clinical reasoning requires integrating evidence across multiple medical images, but existing benchmarks focus on single-image interpretation. Current models perform well on single-view medical QA but fail at multi-image clinical reasoning tasks.

Method: Created MedThinkVQA with 8,067 expert-annotated cases (720 test cases) averaging 6.62 images per case, much denser than prior benchmarks. Includes intermediate supervision and step-level evaluation to assess multi-image reasoning.

Result: Best closed-source models (Claude-4.6-Opus, Gemini-3-Pro, GPT-5.2-xhigh) reach only 57.2%, 55.3%, and 54.9% accuracy. Over 70% of errors come from image reading and cross-view integration. Providing expert cues improves performance, while self-generated intermediates reduce accuracy.

Conclusion: The main bottleneck is grounded multi-image reasoning - models fail to extract, align, and compose evidence across views. Scaling computation only helps when visual grounding is reliable; otherwise it amplifies errors. Need better mechanisms for grounding and integrating distributed evidence across multimodal clinical inputs.
...
TL;DR: Novel variational method for computing accurate global signed distance functions from point clouds using phase field approximation to handle medial axis discontinuities.


<details>
  <summary>Details</summary>
Motivation: Existing methods for computing signed distance functions from point clouds often struggle with accuracy, particularly near the medial axis where the gradient has discontinuities. A more robust approach is needed to handle these discontinuities explicitly for global accuracy.

Method: Higher-order variational formulation that explicitly accounts for the jump set (medial axis) of the SDF gradient, enforcing linear growth away from discontinuities. Uses phase field approximation (Ambrosio-Tortorelli type) to make the problem computationally tractable, with neural networks approximating both SDF and phase field functions.

Result: Method achieves high accuracy both in near field and globally. Quantitative and qualitative comparisons show advantages over other approaches for surfaces represented by unoriented point clouds.

Conclusion: The proposed variational method with phase field approximation effectively handles medial axis discontinuities, enabling accurate global SDF computation from unoriented point clouds using neural network representations.
...
TL;DR: SynthPID: Synthetic P&ID dataset with real topology seeding achieves 63.8% edge mAP, closing within 8pp of real-data performance, showing structural similarity matters more than visual fidelity.


<details>
  <summary>Details</summary>
Motivation: Digitizing P&IDs into process graphs is valuable but bottlenecked by lack of public annotated data (only 12 images available). Previous synthetic methods fail because random symbol placement creates unrealistic graphs with poor performance (~33% accuracy).

Method: Created SynthPID corpus of 665 synthetic P&IDs by seeding pipe topology directly from real drawings. Used patch-based Relationformer adapted for high-resolution diagrams. Trained model on SynthPID alone without any real P&IDs.

Result: Achieved 63.8 ± 3.1% edge mAP on PID2Graph OPEN100 benchmark, closing within 8 percentage points of real-data oracle. Outperformed template-based methods by ~30pp. Gains flatten beyond ~400 synthetic images, indicating seed diversity is limiting factor.

Conclusion: Structural similarity in synthetic data generation drives performance more than model choice. Real topology seeding is crucial for effective synthetic training. Data quality, not quantity, is key constraint for P&ID digitization.
...
TL;DR: BARD converts autoregressive VLMs to efficient diffusion VLMs via progressive block merging and intra-diffusion distillation, achieving 3× decoding speedup while maintaining SOTA performance.


<details>
  <summary>Details</summary>
Motivation: Autoregressive VLMs have strong multimodal capability but suffer from slow token-by-token decoding. Diffusion VLMs offer parallel decoding but converting autoregressive VLMs directly to diffusion causes quality degradation.

Method: BARD uses progressive supervised block merging to enlarge decoding block size, stage-wise intra-dVLM distillation from a small-block diffusion anchor, mixed noise scheduler for robustness, and memory-friendly training for long sequences.

Result: BARD-VL transfers Qwen3-VL's capability to large-block dVLM with ≤4.4M data, achieves SOTA among comparable open dVLMs at 4B/8B scales, and provides up to 3× decoding throughput speedup.

Conclusion: Intra-diffusion distillation is key for effective conversion; BARD enables efficient diffusion VLMs without sacrificing multimodal capability, offering practical speed improvements.
...
TL;DR: PriceBlind is a stealthy adversarial attack framework that exploits visual dominance hallucination in MLLMs to override textual price evidence in screenshot-based financial transactions, achieving high attack success rates.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs are increasingly used for high-stakes financial transactions, but their adversarial robustness remains underexplored. The paper identifies Visual Dominance Hallucination (VDH) where imperceptible visual cues can override textual price evidence, leading to irrational decisions.

Method: PriceBlind is a white-box adversarial attack framework that exploits modality gaps in CLIP-based encoders using Semantic-Decoupling Loss. This aligns image embeddings with low-cost, value-associated anchors while preserving pixel-level fidelity. The framework includes Ensemble-DI-FGSM for transfer attacks.

Result: On E-ShopBench, PriceBlind achieves ~80% ASR in white-box evaluation. Under single-turn coordinate-selection protocol, Ensemble-DI-FGSM transfers with ~35-41% ASR across GPT-4o, Gemini-1.5-Pro, and Claude-3.5-Sonnet. Robust encoders and Verify-then-Act defenses reduce ASR but with clean-accuracy trade-offs.

Conclusion: MLLMs used for financial transactions are vulnerable to visual dominance hallucination attacks. PriceBlind demonstrates effective exploitation of modality gaps, highlighting the need for improved robustness in multimodal financial agents. Defenses exist but involve accuracy trade-offs.
...
TL;DR: This paper provides a systematic review of fairness in text-to-image generation models, addressing conceptual ambiguity in bias/fairness terminology and proposing a new framework for operationalizing fairness through target-based testing.


<details>
  <summary>Details</summary>
Motivation: T2I models are widely used but criticized for exhibiting societal stereotypes. Current research suffers from conceptual ambiguity where terms like "bias" and "fairness" lack clear operational definitions, making systematic evaluation and mitigation challenging.

Method: Conducted a comprehensive systematic review of T2I fairness literature, organizing work into a taxonomy of bias types and fairness notions. Critically assessed the gap between "target fairness" (normative ideals) and "threshold fairness" (actionable standards). Surveyed mitigation strategies from prompt engineering to diffusion process manipulation.

Result: The review organizes existing research into a structured taxonomy, identifies the conceptual gap between ideal fairness goals and practical implementation standards, and maps the landscape of current mitigation approaches.

Conclusion: Proposes a new framework for operationalizing fairness that moves beyond descriptive metrics toward rigorous, target-based testing, offering a more accountable approach for generative AI development.
...
TL;DR: SmoGVLM is a small graph-enhanced vision-language model that integrates structured knowledge using Graph Neural Networks, achieving up to 16.24% performance gains and surpassing larger models.


<details>
  <summary>Details</summary>
Motivation: Large VLMs suffer from hallucination and poor grounding in knowledge-intensive reasoning tasks, despite strong multimodal performance.

Method: Proposes SmoGVLM, a small graph-enhanced VLM that integrates structured knowledge with visual and textual modalities using Graph Neural Networks, tested across model sizes from 1.3B to 13B parameters.

Result: Small models trained with this approach achieve up to 16.24% performance gains and surpass larger counterparts, outperforming both larger VLMs and strong fine-tuned baselines.

Conclusion: Structured knowledge augmentation enables efficient, smaller-scale multimodal reasoning systems that can outperform larger models.
...
TL;DR: Fast online method for 3D multi-object tracking and pose estimation using multiple monocular cameras with only 2D detections, no 3D training needed.


<details>
  <summary>Details</summary>
Motivation: To address the need for efficient 3D multi-object tracking and pose estimation without requiring costly 3D training data or computationally expensive deep learning models.

Method: Uses only 2D bounding box and pose detections from multiple monocular cameras, implements efficient Bayes-optimal multi-object tracking filter, leverages publicly available pre-trained 2D detection models.

Result: Significantly faster than state-of-the-art methods without compromising accuracy, robust performance even when cameras are intermittently disconnected/reconnected during operation.

Conclusion: Proposed method provides fast, accurate, and robust 3D multi-object tracking and pose estimation using only 2D detections, eliminating need for 3D training data while maintaining computational efficiency.
...
TL;DR: A privacy-preserving semantic segmentation method using client-specific encryption keys for each image, with encryption applied during both training and inference to maintain performance.


<details>
  <summary>Details</summary>
Motivation: To enable privacy-preserving semantic segmentation where each client can use independent encryption keys for their images, protecting data privacy while maintaining model utility.

Method: Model creator and clients encrypt images using locally generated keys. Both training and inference are performed on encrypted images. Image encryption is applied during model training to mitigate performance degradation.

Result: Effectiveness confirmed on Cityscapes dataset using SETR (vision transformer-based model), demonstrating the method works with client-specific encryption keys.

Conclusion: Proposed method enables privacy-preserving semantic segmentation with client-specific encryption keys while maintaining performance through encryption-aware training.
...
TL;DR: The paper introduces an expert-annotated dataset of embryo images with natural language descriptions to enable fine-tuning of vision-language models for interpretable, evidence-based embryo assessment in IVF.


<details>
  <summary>Details</summary>
Motivation: Current AI methods for embryo selection in IVF have limited impact due to poor adaptability to clinical data, reliance on specialized equipment, and lack of interpretability. Patients increasingly question expert decisions, especially when treatment fails, creating need for transparent, evidence-based decision justification.

Method: Creation of an expert-annotated dataset containing embryo images paired with natural language descriptions of morphological features, developmental stages, and cell cycles. This dataset enables fine-tuning of foundational vision-language models to generate interpretable embryo descriptions.

Result: The proposed dataset supports research in language-based, interpretable automated embryo assessment. Predicted descriptions can be used to automatically extract scientific evidence from literature, facilitating evidence-based decision-making and transparent patient communication.

Conclusion: The dataset has potential to significantly enhance embryo selection decision-making processes and improve patient outcomes over time by enabling transparent, evidence-based AI systems that can explain their reasoning in natural language.
...
TL;DR: ASR alone is insufficient for evaluating adversarial robustness in medical AI; multi-metric frameworks including perceptual quality and perturbation strength are needed.


<details>
  <summary>Details</summary>
Motivation: Current vulnerability evaluations in medical image analysis rely too heavily on Attack Success Rate (ASR), which doesn't account for perturbation strength, image quality, or cross-architecture transferability. With Vision Transformers challenging CNN dominance and different learning approaches, a single metric like ASR cannot adequately capture adversarial behavior.

Method: Systematic empirical study on four medical image datasets (PathMNIST, DermaMNIST, RetinaMNIST, CheXpert) evaluating seven models (VGG-16, ResNet-50, DenseNet-121, Inception-v3, DeiT, Swin Transformer, ViT-B/16) against seven attack methods at five perturbation budgets, measuring ASR, PSNR, SSIM, and L2 perturbation magnitude.

Result: Perceptual and distortion metrics (PSNR, SSIM, L2) are strongly correlated with each other but show minimal correlation with ASR. This pattern holds for both CNNs and Vision Transformers, demonstrating that ASR alone is inadequate for assessing adversarial robustness and transferability.

Conclusion: Comprehensive adversarial risk assessment in medical AI requires multi-metric frameworks that evaluate not just attack efficacy (ASR), but also methodology and associated overheads including perturbation strength and perceptual quality.
...
TL;DR: PoInit-of-View: A novel attack targeting SfM initialization in 3D reconstruction systems by introducing cross-view gradient inconsistencies to corrupt pose estimation and triangulation, achieving transferable poisoning effects across different reconstruction systems.


<details>
  <summary>Details</summary>
Motivation: Existing poisoning attacks on 3D reconstruction systems backpropagate through the entire pipeline without targeting specific vulnerable modules. The authors identify structure-from-motion (SfM) initialization as a critical geometric core that can be exploited for transferable attacks across diverse 3D reconstruction systems.

Method: PoInit-of-View optimizes adversarial perturbations to intentionally introduce cross-view gradient inconsistencies at projections of corresponding 3D points. These inconsistencies disrupt keypoint detection and feature matching, thereby corrupting pose estimation and triangulation within SfM, eventually degrading rendered view quality. The method includes theoretical analysis connecting cross-view inconsistency to correspondence collapse.

Result: Experimental results show PoInit-of-View is effective across diverse 3D reconstruction systems and datasets. It surpasses single-view baselines by 25.1% in PSNR and 16.5% in SSIM in black-box transfer settings (e.g., 3DGS to NeRF).

Conclusion: Targeting the SfM initialization module enables more effective and transferable poisoning attacks on 3D reconstruction systems compared to holistic pipeline attacks, demonstrating the importance of understanding specific vulnerabilities within reconstruction pipelines.
...
TL;DR: BookAgent is a safety-aware multi-agent framework for generating illustrated storybooks that addresses limitations in multi-modal grounding and child-specific safety constraints through joint planning, scripting, illustrating, and global repair mechanisms.


<details>
  <summary>Details</summary>
Motivation: Current storybook generation methods decompose the task into separate stages, limiting holistic multi-modal grounding. Additionally, existing safety alignment approaches rarely integrate child-specific safety constraints into narrative planning and multi-modal verification.

Method: BookAgent uses a multi-agent collaboration framework for end-to-end storybook synthesis from user drafts. It jointly performs planning, scripting, illustrating, and global inconsistency repair. The system dynamically calibrates page-level alignment between text and visuals, and verifies-then-rectifies global inconsistencies in character identity and storytelling logic.

Result: Extensive experiments show BookAgent significantly outperforms current methods in narrative coherence, visual consistency, and safety compliance, offering a robust paradigm for reliable agents in complex multi-modal creation.

Conclusion: BookAgent provides an effective solution for high-quality, safety-aware visual narrative generation, addressing key limitations in current storybook creation methods through integrated multi-modal grounding and safety verification mechanisms.
...
TL;DR: A novel 3D fingerprint recognition method that unwraps 3D point clouds using B-spline curve fitting, converts them to grayscale images, and applies 2D recognition techniques, achieving superior performance over existing methods.


<details>
  <summary>Details</summary>
Motivation: 3D fingerprint recognition offers advantages over 2D systems including contactless operation (improved hygiene/security) and additional depth/shape information. However, challenges include topological height variations complicating ridge extraction and registration issues requiring consistent sample orientation.

Method: The method unwraps 3D fingerprints (represented as point clouds) using B-spline curve fitting to mitigate height variation and reduce registration limitations. The unwrapped point cloud is converted to a grayscale image by mapping relative point heights, then conventional 2D fingerprint identification methods are applied for recognition.

Result: Superior performance with EERs of 0.2072%, 0.26%, and 0.22% across three experiments, outperforming existing methods. Also surpassed 3D fingerprint flattening technique in cross-session experiments, achieving EER of 1.50% with varying registration fingerprints.

Conclusion: The proposed B-spline based unwrapping method effectively addresses 3D fingerprint recognition challenges, enabling robust authentication by converting 3D data to 2D representations while maintaining superior recognition performance compared to existing approaches.
...
TL;DR: 3D-ARD+: A sequential text-to-scene generation model that creates detailed 3D scenes with objects following complex text instructions about shape, appearance, and spatial arrangement.


<details>
  <summary>Details</summary>
Motivation: Current text-to-scene methods either generate layouts or objects separately, produce overly simple scenes even with LLMs, and often fail to match detailed text descriptions of object properties and spatial relationships.

Method: Proposes 3D-ARD+, a 3D Autoregressive Diffusion model that unifies autoregressive generation of multimodal tokens with diffusion generation of 3D latents. It generates scenes sequentially in two steps: first coarse-grained 3D latents in scene space, then fine-grained object geometry/appearance in object space.

Result: Trained on 230K indoor scenes with text instructions. The 7B parameter model successfully generates and places objects following non-trivial spatial layouts and semantics specified in text instructions.

Conclusion: 3D-ARD+ introduces a novel sequential generation paradigm that effectively creates detailed 3D scenes matching complex text descriptions, addressing limitations of existing text-to-scene approaches.
...
TL;DR: PA-TCNet is a pathology-aware temporal calibration framework for stroke motor imagery EEG decoding that addresses lesion-related abnormal dynamics and inter-patient heterogeneity through rhythmic state decomposition and physiology-guided target refinement.


<details>
  <summary>Details</summary>
Motivation: Stroke patient cross-subject EEG decoding for motor imagery BCI is challenging due to lesion-related abnormal temporal dynamics and significant inter-patient heterogeneity, which undermine generalization. Existing adaptation methods are misled by pathological slow-wave activity and unstable target-domain pseudo-labels.

Method: PA-TCNet integrates two coordinated components: 1) Pathology-aware Rhythmic State Mamba (PRSM) module that decomposes EEG spatiotemporal features into slowly varying rhythmic context and fast transient perturbations, injecting fused pathological context into selective state propagation; 2) Physiology-Guided Target Calibration (PGTC) module that constructs source-domain sensorimotor region-of-interest templates, imposing physiological consistency constraints and dynamically refining target-domain pseudo-labels.

Result: Leave-one-subject-out experiments on two independent stroke EEG datasets (XW-Stroke and 2019-Stroke) yielded mean accuracies of 66.56% and 72.75%, respectively, outperforming state-of-the-art baselines.

Conclusion: Jointly modeling pathological temporal dynamics and physiology-constrained pseudo-supervision can provide more robust cross-subject initialization for personalized post-stroke MI-BCI rehabilitation, addressing key challenges in stroke patient EEG decoding.
...
TL;DR: SeeTN framework improves gaze estimation generalization by mitigating label noise through semantic embedding space construction and affinity regularization.


<details>
  <summary>Details</summary>
Motivation: Existing gaze estimation methods overlook label noise from imprecise annotations, which negatively affects model generalization performance in real-world applications.

Method: Constructs semantic embedding space via prototype-based transformation, measures feature-label affinity consistency to identify noisy samples, and introduces affinity regularization in semantic manifold to transfer gaze information from clean to noisy samples.

Result: Extensive experiments show SeeTN effectively mitigates source-domain noise impact, achieves superior cross-domain generalization without compromising source-domain accuracy.

Conclusion: The paper demonstrates the importance of explicitly handling label noise in generalized gaze estimation and proposes an effective framework for noise mitigation.
...
TL;DR: Automatic systolic murmur classification system using complex orthogonal matching pursuit for multiresolution feature extraction and vision transformer for classification, achieving 95.96% accuracy on CirCor DigiScope dataset.


<details>
  <summary>Details</summary>
Motivation: Systolic murmurs indicate heart abnormalities but vary in intensity, pitch, and quality, requiring precise identification for accurate cardiac diagnosis. Manual classification is challenging due to this variability.

Method: Two-stage approach: 1) Feature extraction using complex orthogonal matching pursuit with multiresolution Gabor basis functions to create variable-resolution time-frequency matrices from murmur segments, 2) Classification using vision transformer with CNN patch tokenization and multihead attention to process multiple resolution matrices.

Result: Achieved 95.96% classification accuracy for four types of systolic murmurs on the CirCor DigiScope dataset, demonstrating effective integration of multiresolution feature extraction with transformer-based classification.

Conclusion: The proposed system successfully combines advanced signal processing (complex orthogonal matching pursuit) with deep learning (vision transformer) to create a robust automatic classification system for systolic murmurs, addressing variability challenges through shared dictionary learning and multiresolution analysis.
...
TL;DR: A novel two-level neural network architecture with dual-stage feature fusion (late + intermediate fusion) outperforms single fusion methods for human activity recognition.


<details>
  <summary>Details</summary>
Motivation: While neural networks like CNNs, LSTMs, and their hybrids show strong performance in HAR, the structural arrangement of components in multilevel/hybrid models significantly impacts overall performance. There's a need to explore optimal fusion strategies to leverage complementary strengths of different networks.

Method: Proposed a two-level network architecture with dual-stage feature fusion: late fusion (combining outputs from first level) and intermediate fusion (integrating features from both first and second levels). Evaluated 15 different architectures of CNNs, LSTMs, and convolutional LSTMs with various fusion combinations to identify optimal configuration.

Result: Architectures with both late and intermediate fusion achieved higher accuracy than those with late fusion alone. The optimal configuration outperformed baseline models on two public benchmark datasets, validating its effectiveness for HAR.

Conclusion: The proposed dual-stage feature fusion framework (combining late and intermediate fusion) in a two-level architecture provides an effective approach for human activity recognition, demonstrating that strategic fusion strategies significantly improve model performance.
...
TL;DR: Introduces Camo-M3FD, a new benchmark for cross-spectral camouflaged pedestrian detection using registered visible-thermal image pairs with pixel-level masks and standardized evaluation framework.


<details>
  <summary>Details</summary>
Motivation: Pedestrian detection remains challenging due to occlusions, cluttered backgrounds, and degraded visibility. While multispectral detection helps with poor visibility, camouflaged pedestrians remain largely unexplored, with existing COD benchmarks focusing on biological species rather than safety-critical human detection.

Method: Created Camo-M3FD benchmark derived from M3FD dataset, consisting of registered visible-thermal image pairs curated using quantitative metrics to ensure high foreground-background similarity. Provided high-quality pixel-level masks and established standardized evaluation framework using state-of-the-art COD models.

Result: Results demonstrate that thermal signals provide indispensable localization cues, but multispectral fusion is essential for refining structural details in camouflaged pedestrian detection.

Conclusion: Camo-M3FD serves as a foundational resource for developing robust and safety-critical detection systems, addressing the gap in camouflaged pedestrian detection research. The dataset is publicly available on GitHub.
...
TL;DR: Amortized framework enables real-time visual attribution streaming in multimodal thinking models by learning to estimate causal effects from attention features, achieving faithfulness comparable to exhaustive methods.


<details>
  <summary>Details</summary>
Motivation: Multimodal thinking models (like those generating code from screenshots or solving math problems from images) need their reasoning traces to be grounded in visual evidence, but verifying this reliance is challenging. Faithful causal methods are computationally expensive, while raw attention maps lack causal validity.

Method: Introduces an amortized approach that learns to estimate causal effects of semantic regions directly from attention features. This enables visual attribution streaming where users can observe grounding evidence as the model reasons, not after.

Result: Across five diverse benchmarks and four thinking models, the approach achieves faithfulness comparable to exhaustive causal methods while enabling real-time visual attribution streaming.

Conclusion: Real-time, faithful attribution in multimodal thinking models is achievable through lightweight learning rather than brute-force computation, enabling users to see grounding evidence during the reasoning process.
...
TL;DR: MambaKick: A learning framework using pretrained human action recognition embeddings and Mamba models for predicting penalty kick direction from short video segments before ball contact.


<details>
  <summary>Details</summary>
Motivation: Goalkeepers need to anticipate penalty kick direction under extreme time constraints. Current approaches rely on explicit kinematic reconstruction or handcrafted biomechanical features, which may not be optimal for real-world footage with ambiguity.

Method: Uses pretrained HAR embeddings from contact-centered short video segments, combines them with lightweight temporal predictors using selective state-space models (Mamba), and incorporates simple contextual metadata (field side, footedness) as complementary cues.

Result: Achieves up to 53.1% accuracy for three classes and 64.5% for two classes, consistently improving or matching strong embedding baselines across various HAR backbones.

Conclusion: Combining pretrained HAR representations with efficient state-space temporal modeling is a practical approach for low-latency intention prediction in real-world sports video applications.
...
TL;DR: IncepDeHazeGAN: A novel GAN-based single-image dehazing method using Inception blocks for multi-scale feature extraction and multi-layer feature fusion for efficient feature reuse, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Dehazing is crucial for enhancing visual quality of images captured in cloudy/foggy conditions, especially for recovering clear images from haze-affected remote sensing data. Existing methods need improvement in handling different haze conditions and feature utilization.

Method: Proposes IncepDeHazeGAN, a Generative Adversarial Network incorporating Inception blocks for multi-scale feature extraction and multi-layer feature fusion design that enables efficient reuse of features by fusing features from different convolution layers multiple times. Uses Grad-CAM XAI technique to visualize network focus regions.

Result: The network achieves state-of-the-art results on several datasets, demonstrating superior dehazing performance compared to existing methods.

Conclusion: IncepDeHazeGAN effectively addresses single-image dehazing through innovative architectural design combining Inception blocks and multi-layer feature fusion, with explainable AI techniques providing insight into network behavior across different haze conditions.
...
TL;DR: AVRT generates high-quality audio-visual reasoning traces from single-modality teacher models, enabling effective multimodal reasoning training through supervised fine-tuning followed by reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: While reasoning models excel in text domains, transferring these capabilities to multimodal settings (audio-visual data) is challenging due to limited high-quality multimodal reasoning data availability.

Method: AVRT framework generates independent vision- and audio-reasoning traces using specialized single-modality models, merges them with an LLM merger model, then uses the multimodal traces for supervised fine-tuning cold start, followed by reinforcement learning on larger-scale data.

Result: 3B and 7B parameter models achieve state-of-the-art results on seven audio-visual and audio benchmarks, outperforming comparable models like OmniBench, DailyOmni, and MMAR, with cross-modal training transferring to single-modality tasks.

Conclusion: AVRT establishes a new effective training pipeline for multimodal reasoning models by addressing the data scarcity problem through trace generation from single-modality teachers, enabling superior performance in both multimodal and single-modality reasoning tasks.
...
TL;DR: IK-GAT is a lightweight graph-attention network that reconstructs full-body joint orientations from 3D joint positions in a single forward pass, solving the underconstrained inverse kinematics problem for real-time human animation.


<details>
  <summary>Details</summary>
Motivation: Real-time human avatar pipelines often only have sparse 3D joint positions as input, but animation systems require joint orientations to drive skinning. Traditional IK solvers are slow, iterative, and sensitive to noise, while the problem is underconstrained due to twist ambiguity.

Method: IK-GAT uses graph-attention networks to perform message passing over the skeletal parent-child graph. It predicts rotations in a bone-aligned world-frame representation anchored to rest-pose bone frames, uses continuous 6D rotation representation, and is trained with geodesic loss on SO(3) plus optional forward-kinematics consistency regularization.

Result: IK-GAT achieves over 650 FPS on CPU with only 374K parameters, outperforms VPoser-based iterative optimization without warm-start at significantly lower cost, and is robust to initial pose and input noise.

Conclusion: IK-GAT provides a fast, robust solution for converting 3D joint positions to animation-ready local rotations for real-time avatar applications, enabling direct driving of rigged avatars or conversion to SMPL-like body model parameters.
...
TL;DR: First systematic tri-modal (RGB+thermal+event) UAV object detection framework with dual-stream hierarchical transformer and novel fusion modules, benchmarked on new 10K-frame dataset showing tri-modal outperforms all dual-modal baselines.


<details>
  <summary>Details</summary>
Motivation: UAV object detection needs robustness to illumination changes, motion blur, and scene dynamics that suppress RGB cues. Thermal LWIR preserves contrast in low light, event cameras retain microsecond temporal edges, but integrating all three modalities hasn't been systematically studied.

Method: Tri-modal framework processes RGB, thermal, and event data with dual-stream hierarchical vision transformer. Uses Modality-Aware Gated Exchange (MAGE) for inter-sensor channel/spatial gating and Bidirectional Token Exchange (BiTE) for bidirectional token-level attention with depthwise-pointwise refinement. Produces resolution-preserving fused maps for standard feature pyramid and two-stage detector.

Result: Tri-modal fusion improves over all dual-modal baselines. Fusion depth has significant effect. Lightweight CSSA variant recovers most benefits at minimal cost. Introduces 10,489-frame UAV dataset with synchronized RGB-thermal-event streams and 24,223 annotated vehicles across day/night flights. 61 controlled ablations evaluate fusion placement, mechanisms, modality subsets, and backbone capacity.

Conclusion: Provides first systematic benchmark and modular backbone for tri-modal UAV-based object detection, demonstrating advantages of integrating all three modalities over any dual-modal combination.
...
TL;DR: Systematic benchmarking of CNN, transformer, and foundation models for landslide detection from satellite imagery, comparing full vs. parameter-efficient fine-tuning methods.


<details>
  <summary>Details</summary>
Motivation: The relative effectiveness of modern segmentation architectures and fine-tuning strategies for landslide detection from high-resolution satellite imagery remains insufficiently understood, despite its critical importance for disaster response and risk assessment.

Method: Systematic benchmarking study using the Globally Distributed Coseismic Landslide Dataset (GDCLD), evaluating CNN-based segmentation models, transformer-based segmentation models, and large pre-trained foundation models under consistent training protocols. Comparison of full fine-tuning with parameter-efficient methods (LoRA and AdaLoRA) to assess performance-efficiency tradeoffs.

Result: Transformer-based models achieve strong segmentation performance, while parameter-efficient fine-tuning reduces trainable parameters by up to 95% with comparable accuracy to full fine-tuning. The study also analyzes generalization under distribution shift by comparing validation and held-out test performance.

Conclusion: The systematic benchmarking provides insights into effective architectures and fine-tuning strategies for landslide detection, demonstrating that transformer models perform well and parameter-efficient methods offer substantial efficiency gains without significant accuracy loss.
...
TL;DR: Humans and models can recognize actions in appearance-free videos zero-shot, with motion pathways being critical for this generalization.


<details>
  <summary>Details</summary>
Motivation: To understand whether humans and vision models can generalize zero-shot to appearance-free transformations of real-world action videos, where static shape cues are removed but motion is preserved.

Method: 1) Psychophysics experiment with 22 participants trained on naturalistic videos (UCF5) and tested zero-shot on two appearance-free transformations (dense-noise and random-dot videos). 2) Two-pathway 3D CNN model with RGB (form) and optical flow (motion) streams, plus coherence-gating mechanism inspired by Gestalt grouping.

Result: Humans recognize actions in appearance-free videos above chance (though reduced accuracy). The two-pathway model outperforms contemporary video models and narrows the human-performance gap, with motion pathway critical for appearance-free generalization and form pathway beneficial for naturalistic videos.

Conclusion: Motion-based representations are essential for generalization to appearance-free videos, supporting multi-stream architectures for action recognition modeling.
...
TL;DR: C-GenReg is a training-free 3D point cloud registration framework that combines world-scale generative priors with vision foundation models to achieve robust cross-domain generalization without fine-tuning.


<details>
  <summary>Details</summary>
Motivation: Current learning-based 3D point cloud registration methods struggle with generalization across different sensing modalities, sampling variations, and environments. There's a need for a robust, zero-shot approach that can handle diverse real-world scenarios without requiring fine-tuning.

Method: The framework uses a World Foundation Model to synthesize multi-view-consistent RGB images from input geometry, then leverages Vision Foundation Models to find dense correspondences in the image domain. A "Match-then-Fuse" probabilistic cold-fusion scheme combines correspondences from the generated-RGB branch with the raw geometric branch, preserving each modality's inductive bias while providing calibrated confidence.

Result: Extensive experiments on indoor (3DMatch, ScanNet) and outdoor (Waymo) benchmarks demonstrate strong zero-shot performance and superior cross-domain generalization. The framework successfully operates on real outdoor LiDAR data without any imagery data.

Conclusion: C-GenReg presents the first generative registration framework that effectively handles real outdoor LiDAR data, offering a plug-and-play, training-free solution that combines generative priors with vision foundation models for robust 3D point cloud registration across diverse domains.
...
TL;DR: 3DETR with Multi-Scale Attention improves 3D object detection by capturing local geometry and global context through upsampling for better small object detection.


<details>
  <summary>Details</summary>
Motivation: 3D object detection in point clouds is challenging due to data sparsity and lack of global structure. Existing methods need better ways to capture both local geometric details and global scene context.

Method: Proposed Multi-Scale Attention (MSA) mechanism integrated into 3DETR architecture. Introduces upsampling operation to generate high-resolution feature maps for better detection of smaller and semantically related objects.

Result: On ScanNetv2 dataset: 3DETR + MSA improves detection performance with ~1% gain in mAP@25 and 4.78% gain in mAP@50 over baseline. Limited improvement on 3DETR-m variant suggests need for adapted upsampling strategies for lightweight models.

Conclusion: Combining hierarchical feature extraction with attention mechanisms effectively enhances 3D scene understanding. Multi-scale attention helps capture both local geometry and global context for improved object detection in sparse point clouds.
...
TL;DR: Proposes iDoc model with self-supervised encoder and open-set detector for faster document retrieval and pattern spotting, achieving 10x speedup and new SOTA precision (0.612) for small non-square queries.


<details>
  <summary>Details</summary>
Motivation: Current document retrieval and pattern spotting methods have low precision (0.494 overall, 0.427 for small non-square queries) and slow processing (up to 7 seconds), inadequate for massification of digital books.

Method: iDoc model with improved encoder trained via self-supervised strategy, open-set detector for faster searching, and non-maximum suppression to reduce false positives.

Result: Achieves competitive SOTA results with 10x speed improvement, new SOTA precision of 0.612 for small non-square queries (vs previous 0.427).

Conclusion: The proposed iDoc model significantly improves both speed and accuracy for document retrieval and pattern spotting, addressing limitations of current SOTA methods for digital book collections.
...
TL;DR: Agentic AI pipeline enables LLMs to perform complex brain MRI analysis by orchestrating external tools, eliminating need for native 3D processing or model training.


<details>
  <summary>Details</summary>
Motivation: Current LLMs lack native 3D spatial reasoning for volumetric medical imaging analysis, and the feasibility of agentic frameworks for complex radiological workflows remains underexplored.

Method: Training-free agentic pipeline where LLMs (GPT-5.1, Gemini 3 Pro, Claude Sonnet 4.5) orchestrate off-the-shelf domain-specific tools to autonomously execute end-to-end workflows including preprocessing, pathology segmentation, and volumetric analysis.

Result: System successfully performs complex neuro-radiological tasks from single-scan segmentation to longitudinal response assessment, with comparison of single-agent vs multi-agent architectures, and benchmark dataset released for evaluation.

Conclusion: Agentic AI can solve complex neuro-radiological image analysis tasks through tool orchestration without requiring model training or fine-tuning, demonstrating feasibility for medical imaging workflows.
...
TL;DR: AW4RE is a generative world model for physical awareness that combines 4D-informed retrieval with action-conditioned geometric support to predict observations from sensing actions, addressing partial observability challenges in RL.


<details>
  <summary>Details</summary>
Motivation: Physical awareness in dynamic environments involves loopy information structures where sensing decisions affect observability and observations affect sensing quality. While RL has succeeded in fully observable environments, POMDPs remain challenging due to costly real-world exploration and sim-to-real gaps from unobserved viewpoints.

Method: AW4RE combines 4D-informed evidence retrieval, action-conditioned geometric support with temporal coherence, and conditional generative completion to estimate action-conditioned observation processes. It provides a sensor-native surrogate environment for exploring sensing queries.

Result: Experiments show AW4RE produces more grounded and consistent predictions than geometry-aware generative baselines under extreme viewpoint shifts, temporal gaps, and sparse geometric support.

Conclusion: AW4RE offers an awareness-centric generative world model that addresses the fundamental challenge of partial observability in physical awareness by providing a reliable surrogate environment for exploring sensing decisions.
...
TL;DR: MLLMs face memory bottlenecks from vision tokens in KV cache; proposed sequential input-compression during prefill reduces peak memory while maintaining performance.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs struggle with high memory consumption from vision tokens in KV cache during inference, especially at prefill stage, limiting practical deployment.

Method: Sequential input-compression mechanism that performs structure-aware KV cache compression during prefill process to enforce fixed memory budget.

Result: Substantially reduces peak memory usage while maintaining generative performance with only minimal degradation.

Conclusion: Enables more practical and memory-efficient multimodal inference by exploiting inherent structural regularities in MLLMs.
...
TL;DR: Automated microscopy system using deep learning for pollen analysis achieves 95.8% recall and 6x speedup over manual methods.


<details>
  <summary>Details</summary>
Motivation: Traditional melissopalynology is time-consuming (4-6 hours per sample) and subjective, requiring automation for more efficient and objective pollen analysis.

Method: Integrated system with H∞ robust mechanical control and deep learning pipelines using U²-Net for object detection and DINOv2 Vision Transformer with Deep Metric Learning for classification, enhanced with Gradient-Weighted Attention for interpretable feature analysis.

Result: Achieves 95.8% classification recall and 6x processing speedup compared to manual expert analysis for pollen grains from Bio Bio region in Chile.

Conclusion: The automated system successfully addresses the limitations of traditional melissopalynology by providing faster, more objective, and interpretable pollen analysis with high accuracy.
...
TL;DR: Dynamic 3D Gaussian Splatting overfits to training views due to incoherent deformation, not just parameter count. Disabling splitting reduces overfitting by 80%, and Elastic Energy Regularization reduces the train-test gap by 40.8% by enforcing deformation coherence.


<details>
  <summary>Details</summary>
Motivation: Dynamic 3D Gaussian Splatting methods achieve strong training-view PSNR on monocular video but generalize poorly, with large train-test gaps (6.18 dB average on D-NeRF benchmark). The paper aims to understand and address this overfitting problem.

Method: Systematic ablation of Adaptive Density Control pipeline to identify splitting as main cause of overfitting. Introduces Elastic Energy Regularization (EER) to enforce local smoothness in per-Gaussian deformation fields. Also evaluates GAD (loss-rate-aware densification threshold) and PTDrop (jitter-weighted Gaussian dropout). Tests on D-NeRF benchmark and real monocular video (HyperNeRF scenes).

Result: Splitting accounts for over 80% of train-test gap. EER reduces gap by 40.8% while increasing Gaussian count by 85%, reducing mean strain by 99.72%. GAD+EER reduces gap by 48%, and adding PTDrop with soft growth cap reaches 57% reduction. Coherence generalizes to different deformation architectures and real monocular video.

Conclusion: Overfitting in dynamic 3DGS is driven by incoherent deformation rather than parameter count. Enforcing deformation coherence through regularization significantly improves generalization while maintaining quality.
...
TL;DR: TriTS is a novel cross-modal disentanglement framework for long-term time series forecasting that projects 1D time series into orthogonal time, frequency, and 2D-vision spaces to overcome representation bottlenecks in traditional 1D approaches.


<details>
  <summary>Details</summary>
Motivation: Long-term time series forecasting (LTSF) is challenging because real-world signals contain highly entangled temporal dynamics that are difficult to fully capture from a purely 1D perspective. Existing approaches face representation bottlenecks in modeling complex temporal patterns.

Method: TriTS uses a cross-modal disentanglement framework with three components: 1) Period-Aware Reshaping with Visual Mamba for 2D-vision space modeling with linear complexity, 2) Multi-Resolution Wavelet Mixing (MR-WM) module for frequency domain analysis that decouples signals into trend and noise components, and 3) a streaming linear branch in the time domain for numerical stability. The framework dynamically fuses these three complementary representations.

Result: Extensive experiments across multiple benchmark datasets demonstrate that TriTS achieves state-of-the-art (SOTA) performance, fundamentally outperforming existing vision-based forecasters while drastically reducing both parameter count and inference latency.

Conclusion: TriTS successfully addresses the representation bottleneck in LTSF by leveraging cross-modal disentanglement across time, frequency, and vision spaces, providing an efficient and effective framework for complex time series forecasting tasks.
...
TL;DR: A system using frozen DINOv3 ViT with guided upsampling achieves accurate arrow puncture detection on archery targets with only 48 training images, outperforming supervised methods that need more data.


<details>
  <summary>Details</summary>
Motivation: To develop a practical system for automated archery scoring that works with minimal training data, leveraging frozen foundation models to overcome small-data limitations in dense prediction tasks.

Method: Three-stage pipeline: 1) color-based rectification for perspective correction, 2) frozen DINOv3 ViT-L/16 with AnyUp guided feature upsampling for sub-millimeter precision, 3) lightweight CenterNet-style detection heads for arrow-center heatmap prediction (only 3.8M of 308M parameters trainable).

Result: Achieved mean F1 score of 0.893 ± 0.011, mean localization error of 1.41 ± 0.06 mm, median per-image score error of 1.8%, and group centroid error of 4.00 mm, comparable to or better than fully-supervised approaches requiring more data.

Conclusion: Frozen foundation models with minimal task-specific adaptation offer a practical paradigm for dense prediction in small-data regimes, as demonstrated by accurate archery scoring with only 48 training images.
...
TL;DR: FairNVT is a lightweight debiasing framework for transformer encoders that improves both representation and prediction fairness while maintaining task accuracy through adapter-based learning, noise injection, and fairness constraints.


<details>
  <summary>Details</summary>
Motivation: Existing debiasing approaches often address representation-level and prediction-level fairness separately, but these are inherently connected. Suppressing sensitive information at the representation level can facilitate fairer predictions, so a unified approach is needed.

Method: Learns task-relevant and sensitive embeddings via lightweight adapters, applies calibrated Gaussian noise to sensitive embeddings, fuses with task representation, and uses orthogonality constraints with fairness regularization to reduce sensitive-attribute leakage and encourage fair predictions.

Result: Across three vision and language datasets, FairNVT reduces sensitive-attribute attacker accuracy, improves demographic-parity and equalized-odds metrics, and maintains high task performance. Compatible with various pretrained transformer encoders.

Conclusion: FairNVT provides an effective lightweight framework that jointly addresses representation and prediction fairness in transformer-based models while preserving task accuracy, demonstrating the connection between representation-level debiasing and fairer predictions.
...
TL;DR: EdgeVTP is a lightweight trajectory predictor for roadside edge devices that uses interaction-aware graph modeling with transformer backbone and one-shot curve decoding to achieve bounded deterministic latency while maintaining competitive accuracy.


<details>
  <summary>Details</summary>
Motivation: Vehicle trajectory prediction needs to run on roadside edge devices with bounded, deterministic end-to-end latency requirements, which existing methods struggle to meet due to computational overhead.

Method: Combines interaction-aware graph modeling with lightweight transformer backbone and one-shot curve decoder that predicts future motion as compact curve parameters anchored at last observed position, with explicit bounding of interaction complexity via locality graph with hard neighbor cap.

Result: Achieves lowest measured end-to-end latency on Jetson-class platforms across three highway benchmarks while attaining state-of-the-art accuracy on two datasets and competitive error on other benchmarks.

Conclusion: EdgeVTP demonstrates that efficient trajectory prediction with bounded latency is achievable on edge devices without sacrificing accuracy, enabling practical roadside deployment.
...
TL;DR: HyMOR is a hybrid framework combining MLLM for open-ended coarse recognition with CLIP for fine-grained domain-specific recognition, achieving better multi-granularity object understanding for multimodal applications.


<details>
  <summary>Details</summary>
Motivation: MLLMs struggle with fine-grained recognition while CLIP lacks broad category coverage. There's a need to bridge this gap for robust multimodal perception in content generation and interactive applications.

Method: HyMOR integrates an MLLM for open-ended coarse-grained recognition with a CLIP model for fine-grained domain-specific identification (animals, plants). Also introduces TBO dataset with 20,942 textbook images and 8,816 categories for evaluation.

Result: Narrows fine-grained recognition gap with CLIP to 0.2%, improves general object recognition by 2.5% over baseline MLLM, achieves 23.2% improvement in average SBert similarity across all datasets.

Conclusion: HyMOR effectively bridges the gap between MLLMs and CLIP, enabling accurate multi-granularity object perception for multimodal content generation and interactive learning applications.
...
TL;DR: CDCRec is a multimodal radio interferometric data reconstruction method that models cross-domain consistency between visibility and image domains to improve radio astronomy imaging quality.


<details>
  <summary>Details</summary>
Motivation: Existing radio astronomy reconstruction methods work in either visibility domain or image domain separately, losing complementary information and failing to model mutual dependency between domains, leading to incomplete reconstruction.

Method: Proposes CDCRec with hierarchical multi-task and multi-stage framework that explicitly models cross-domain consistency, enhancing exploration of interplays between visibility and image domains during reconstruction.

Result: CDCRec improves imaging performance through enhanced cross-domain correlation extraction, with self-supervised complementary modeling outperforming current methods in interferometric domain translations.

Conclusion: Multimodal reconstruction with cross-domain consistency modeling is effective for radio interferometric data, addressing limitations of unimodal approaches and improving reconstruction quality.
...
TL;DR: ADDPS: A diffusion-based semantic communication receiver using alternating dual-domain posterior sampling to achieve optimal perceptual quality in wireless image transmission.


<details>
  <summary>Details</summary>
Motivation: Existing generative semantic communication receivers rely on MAP estimation which cannot preserve data distribution, limiting perceptual quality. Current diffusion-based approaches face limitations: latent-domain guidance is noise-sensitive, image-domain guidance inherits decoder bias, and combining both creates overconfident pseudo-posterior.

Method: Formulates semantic decoding as Bayesian inverse problem and proves posterior sampling achieves optimal perceptual quality. Proposes ADDPS (alternating dual-domain posterior sampling), a diffusion-based SemCom receiver that alternately enforces latent-domain and image-domain consistency during sampling, decomposing joint posterior sampling into simpler subproblems.

Result: Experiments on FFHQ demonstrate superior perceptual quality compared with existing methods.

Conclusion: Posterior sampling preserves data distribution for optimal perceptual quality in semantic communication. ADDPS effectively combines dual-domain guidance without gradient conflicts, leveraging complementary strengths of latent and image domains.
...
TL;DR: Proposes FD-INR, a NIR-aided low-light image restoration method using frequency decoupled implicit neural representation to address noise and structural degradation.


<details>
  <summary>Details</summary>
Motivation: Address severe noise and high frequency structural degradation in visible images under low-light conditions, overcoming limitations of traditional spatial domain fusion methods that cause color distortion and artifacts.

Method: Uses frequency decoupled implicit neural representation with multi-scale wavelet transforms to decompose images into frequency components. Implements dual-branch framework with cross-modal differentiated frequency supervision: low-light RGB guides low-frequency reconstruction, high-SNR NIR constrains high-frequency texture generation. Includes uncertainty-based adaptive weighting loss for balancing frequency tasks.

Result: FD-INR effectively restores image luminance consistency and structural details. Benefits from implicit continuous representation to outperform existing methods in arbitrary-resolution reconstruction tasks, significantly enhancing low-light perception reliability.

Conclusion: The proposed FD-INR method successfully addresses low-light image restoration challenges by leveraging cross-modal frequency correlations and implicit neural representation, achieving superior performance in both quality and resolution flexibility compared to traditional approaches.
...
TL;DR: A channel attention-guided cross-modal knowledge distillation method for referring image segmentation that transfers vision-language correlations from teacher to student networks without adding inference parameters.


<details>
  <summary>Details</summary>
Motivation: Existing RIS methods use large vision-language models with huge parameters, limiting deployment in resource-constrained scenarios. Need efficient distillation to transfer knowledge while maintaining student's independent learning ability.

Method: Proposes channel attention-guided cross-modal knowledge distillation that transfers high-order fine-grained vision-language correlations and semantic component correlations between channels from teacher to student network.

Result: Experimental results on two public datasets show the method introduces no additional inference parameters and achieves significant performance improvement for student models.

Conclusion: The proposed distillation method effectively transfers cross-modal knowledge while preserving student's learning independence, enabling efficient RIS deployment in resource-limited scenarios.
...
TL;DR: BioLip detects deepfake lip-sync videos by measuring unnatural temporal lip variance (lip jitter) that violates biomechanical constraints, unlike existing detectors that rely on language-dependent artifacts.


<details>
  <summary>Details</summary>
Motivation: Current lip-sync deepfake detectors fail to generalize across languages because they rely on data-dependent patterns rather than universal physical principles. The authors identify that generative models don't enforce authentic biomechanical constraints of orofacial articulation.

Method: BioLip uses 64 perioral landmark coordinates extracted by MediaPipe to measure temporal lip jitter - elevated temporal lip variance that violates biomechanical constraints. It's a lightweight framework focusing on this universal physical signal rather than pixel-level artifacts.

Result: The paper identifies temporal lip jitter as an empirically consistent signal across speaker's language, ethnicity, and recording conditions. This provides a more fundamental detection principle than existing methods.

Conclusion: By focusing on biomechanical constraints rather than data-dependent patterns, BioLip offers a more generalizable approach to lip-sync deepfake detection that works across different languages and recording conditions.
...
TL;DR: GCN-HViT combines Vision Transformer's global attention with Graph Convolutional Network's local feature extraction for improved image classification by addressing patch size selection, spatial structure modeling, and local-global information integration.


<details>
  <summary>Details</summary>
Motivation: Current Vision Transformers have limitations: patch size selection is arbitrary, 1D position embeddings fail to capture spatial structure, and they lack local modeling. GCNs capture local connectivity but miss global structure. The paper aims to combine both approaches for better image classification.

Method: Proposes Hierarchical Vision Transformer Enhanced by Graph Convolutional Network (GCN-HViT). Uses hierarchical ViT to model global patch interactions within levels and hierarchical relationships across levels. GCN serves as local feature extractor providing 2D position embeddings and modeling local patch interactions.

Result: Extensive experiments on 3 real-world datasets demonstrate that GCN-HViT achieves state-of-the-art performance in image classification.

Conclusion: GCN-HViT successfully integrates ViT's global attention with GCN's local feature extraction, addressing key limitations of both approaches and achieving superior image classification performance through hierarchical modeling and 2D spatial structure representation.
...
TL;DR: A novel hyperbolic Lorentz model for semantic segmentation that provides stable optimization, uncertainty quantification, and hierarchical representation without requiring Riemannian optimizers.


<details>
  <summary>Details</summary>
Motivation: Existing hyperbolic segmentation approaches using Poincaré ball models suffer from numerical instability, optimization challenges, and computational difficulties. There's a need for a more stable and efficient hyperbolic framework that can leverage hierarchical structure while providing uncertainty quantification.

Method: Proposes a hyperbolic Lorentz model for semantic segmentation that uses text embeddings with semantic and visual cues to guide hierarchical pixel-level representations. The approach works with both pixel-wise and mask classification frameworks, integrates with existing Euclidean architectures, and doesn't require Riemannian optimizers.

Result: Achieves stable and efficient optimization, provides free uncertainty estimation, confidence maps, boundary delineation, hierarchical/text-based retrieval, and zero-shot performance. Reaches generalized flatter minima and demonstrates effectiveness on ADE20K, COCO-Stuff-164k, Pascal-VOC, and Cityscapes datasets with various state-of-the-art models.

Conclusion: The hyperbolic Lorentz model offers a robust, uncertainty-aware semantic segmentation framework that overcomes limitations of previous hyperbolic approaches, providing stable optimization and valuable additional capabilities like uncertainty quantification and hierarchical representation.
...
TL;DR: EFDiff uses Earth foundation model embeddings via cross-attention to guide diffusion-based super-resolution of land surface temperature under extreme 32× spatial degradation, outperforming baselines on global benchmark.


<details>
  <summary>Details</summary>
Motivation: Land surface temperature super-resolution is crucial for environmental monitoring but challenging due to severe underdetermination from coarse thermal observations. Existing methods struggle with extreme spatial degradation.

Method: Proposes EFDiff framework that uses Prithvi-EO-2.0 Earth foundation model to encode high-resolution multispectral reflectance into geospatial embeddings, injected via cross-attention into denoising network to guide reconstruction. Two variants: EFDiff-ε and EFDiff-x₀ with different trade-offs between perceptual realism and pixel fidelity.

Result: Evaluated under extreme 32× scale gap using globally diverse benchmark of 242,416 co-registered Landsat thermal-reflectance patches. EFDiff consistently outperforms baseline methods, and cross-attention conditioning by Earth foundation model is more effective than HLS channel concatenation.

Conclusion: EFDiff effectively addresses extreme LST super-resolution using geospatial embeddings from foundation models. Framework is broadly applicable to remote sensing problems where pretrained geospatial representations can guide generative reconstruction.
...
TL;DR: The paper introduces TowerDataset, a large-scale benchmark for fine-grained segmentation of transmission-corridor point clouds, and proposes a global-local fusion framework to address challenges in long-range structural modeling and local geometric detail preservation.


<details>
  <summary>Details</summary>
Motivation: Current transmission-corridor segmentation faces limitations due to: 1) realistic data scarcity, 2) difficulty modeling both global corridor structure and local geometric details in long, heterogeneous scenes, 3) existing datasets having only coarse categories or short cropped scenes that miss long-range dependencies, severe long-tail distributions, and subtle distinctions among safety-critical components.

Method: Proposes a global-local fusion framework with: 1) Whole-scene branch using NoCrop training and prototypical contrastive learning to capture long-range topology and contextual dependencies; 2) Block-wise local branch to retain fine geometric structures; 3) Fusion and refinement via geometric validation to combine both global relationships and local shape details.

Result: TowerDataset contains 661 real-world scenes with ~2.466 billion points, preserving long corridor extents with fine-grained 22-class taxonomy. Experiments on TowerDataset and two public benchmarks demonstrate the challenge of the proposed benchmark and the robustness of the framework in real, complex, and heterogeneous transmission-corridor scenes.

Conclusion: The paper addresses key limitations in transmission-corridor segmentation by introducing a comprehensive benchmark dataset and a novel global-local fusion framework that effectively combines whole-scene context with local geometric details for improved recognition of rare and confusing components in complex power-line inspection scenarios.
...
TL;DR: A hierarchical Confidence-Aware Token Pruning (CATP) framework for efficient Camouflaged Object Detection that reduces Transformer computational overhead while maintaining accuracy by pruning easy tokens and focusing on critical boundaries.


<details>
  <summary>Details</summary>
Motivation: Transformer-based COD detectors achieve state-of-the-art accuracy but have substantial computational overhead that limits practical deployment. There's a need for efficient COD models that can be deployed in real-world scenarios.

Method: Proposes CATP framework that hierarchically identifies and discards easily distinguishable tokens from both background and object interiors, focusing computations on critical boundary tokens. Includes dual-path feature compensation mechanism to aggregate contextual knowledge from pruned tokens into enriched features.

Result: Extensive experiments on multiple COD benchmarks demonstrate significant reduction in computational complexity while maintaining high accuracy.

Conclusion: CATP offers a promising research direction for efficient deployment of COD models in real-world scenarios, balancing computational efficiency with detection accuracy.
...
TL;DR: COD-TDQ: A novel 4-bit quantization method for camouflaged object detection that addresses token-local bottlenecks by using token-group scales and dual-constraint range projection to preserve subtle boundary cues.


<details>
  <summary>Details</summary>
Motivation: Camouflaged object detection requires on-device deployment with tight memory/latency budgets, making low-bit (W4A4) quantization desirable. However, COD is particularly challenging to quantize aggressively due to heavy-tailed background tokens dominating activation ranges, which pushes subtle boundary cues into the zero bin.

Method: COD-TDQ (COD-aware Token-group Dual-constraint activation Quantization) addresses the token-local bottleneck with two coupled steps: 1) Direct-Sum Token-Group (DSTG) assigns token-group scales to suppress cross-token range domination, and 2) Dual-Constraint Range Projection (DCRP) projects each token-group clip range to bound both the step-to-dispersion ratio and zero-bin mass.

Result: Across four COD benchmarks and two baseline models (CFRN and ESCNet), COD-TDQ consistently achieves an Sα score more than 0.12 higher than state-of-the-art quantization methods without retraining.

Conclusion: The paper introduces an effective post-training quantization method specifically designed for camouflaged object detection that addresses the unique challenges of quantizing Transformer-based COD models, achieving significant performance improvements over existing quantization approaches.
...
TL;DR: Q-DeepSight is a think-with-image framework for image quality assessment that uses interleaved multimodal chain-of-thought with tool-augmented evidence acquisition to provide localized feedback, trained with novel reinforcement learning techniques.


<details>
  <summary>Details</summary>
Motivation: Current MLLM-based IQA methods use single-look, language-only approaches that don't emulate human evidence-seeking judgment, producing weakly grounded rationales that limit reliability for guiding image refinement in generative models and restoration tasks.

Method: Proposes Q-DeepSight with interleaved Multimodal Chain-of-Thought (iMCoT) using tool-augmented evidence acquisition (like crop-and-zoom) to identify where and why quality degrades. Uses reinforcement learning with Perceptual Curriculum Reward (PCR) to mitigate reward sparsity and Evidence Gradient Filtering (EGF) to improve credit assignment for visually-grounded reasoning.

Result: Achieves state-of-the-art performance across diverse benchmarks including natural, restored, and AI-generated content. Demonstrates practical value with Perceptual-in-Generation (PiG), a training-free framework where Q-DeepSight's diagnoses guide iterative image enhancement.

Conclusion: Q-DeepSight effectively closes the loop between assessment and refinement by providing actionable, localized feedback that emulates human-like evidence-seeking judgment, making it valuable for guiding generative models and image restoration.
...
TL;DR: I2P framework improves synthetic image detection generalization by adaptively identifying critical VFM layers for forgery cues and constraining task updates to preserve pretrained transferable structures.


<details>
  <summary>Details</summary>
Motivation: Existing VFM-based SID methods have coarse adaptation strategies that either use final layer representations or simple multi-layer fusion, lacking explicit modeling of optimal representational hierarchy for transferable forgery cues. Direct fine-tuning may damage cross-modal pretrained structures that support open-set generalization.

Method: I2P reformulates VFM adaptation as joint optimization: 1) adaptively identifies critical layer representations most discriminative for SID, 2) constrains task-driven parameter updates within low sensitivity parameter subspace to preserve pretrained transferable structures.

Result: The framework improves task specificity while preserving transferable structure of pretrained representations, addressing the tension between task adaptation and cross-distribution generalization.

Conclusion: I2P provides a principled approach to VFM adaptation for SID by balancing task-specific optimization with preservation of pretrained structures, enabling better cross-distribution generalization to unknown generation sources.
...
TL;DR: BiasCareVL is a bias-aware multimodal AI framework for healthcare that addresses pervasive biases in medical imaging and clinical text data through built-in bias control mechanisms, outperforming 20 state-of-the-art methods across diverse clinical tasks.


<details>
  <summary>Details</summary>
Motivation: Pervasive biases in healthcare AI systems - including imbalanced disease prevalence, skewed anatomical distributions, heterogeneous imaging protocols, and demographic disparities - pose significant challenges to fairness and reliability in real-world clinical settings. Current approaches often treat bias as a post-hoc correction rather than addressing it during model design.

Method: BiasCareVL introduces bias control directly into model design through adaptive uncertainty modeling with optional human-in-the-loop refinement. This regulates the influence of dominant data patterns and promotes equitable reasoning under distributional imbalance. The framework is trained on 3.44 million samples spanning over 15 imaging modalities and supports diverse clinical tasks within a unified representation space.

Result: Across eight public benchmarks covering dermatology, oncology, radiology, and pathology, BiasCareVL consistently outperforms 20 state-of-the-art methods. It achieves over 10% accuracy improvement in multi-class skin lesion diagnosis and more than 20% Dice improvement in small tumor segmentation. The system also exceeds human diagnostic accuracy with substantially reduced time requirements when evaluated with board-certified radiologists.

Conclusion: BiasCareVL demonstrates that incorporating bias awareness directly into model design, rather than treating it as post-hoc correction, leads to more equitable and reliable AI systems for healthcare. By open-sourcing the framework, the authors aim to promote transparent, reproducible, and equitable AI in healthcare, paving the way for general-purpose, trustworthy clinical AI systems.
...
TL;DR: CrossFlowDG addresses domain generalization by using cross-modal flow matching to bridge the modality gap between image and text embeddings, achieving competitive performance on DG benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current multimodal DG methods using cosine similarity-based contrastive alignment still leave a modality gap where image and text embeddings remain geometrically separated despite semantic correspondence, limiting their effectiveness.

Method: Proposes CrossFlowDG framework using noise-free cross-modal flow matching to learn continuous transformations in joint Euclidean latent space, transporting domain-biased image embeddings toward domain-invariant text embeddings of correct class. Uses VMamba image encoder and CLIP's text encoder.

Result: Achieves competitive performance on several DG benchmarks and state-of-the-art on TerraIncognita benchmark.

Conclusion: CrossFlowDG effectively addresses the residual modality gap in multimodal domain generalization through flow-based alignment, demonstrating improved performance over existing methods.
...
TL;DR: EasyVideoR1 is an efficient RL framework for training large vision-language models on video understanding tasks, featuring optimized preprocessing, comprehensive reward systems, mixed training paradigms, and multi-benchmark evaluation.


<details>
  <summary>Details</summary>
Motivation: Extending RLVR to video understanding is important but challenging due to video task diversity, computational overhead from high-dimensional visual inputs, and difficulty in reproducible evaluation. Existing frameworks lack optimizations for video modality.

Method: 1) Full video RL pipeline with offline preprocessing and tensor caching; 2) Task-aware reward system covering 11 video/image problem types; 3) Mixed offline-online data training; 4) Joint image-video training with configurable pixel budgets; 5) Asynchronous multi-benchmark evaluation framework.

Result: Achieves 1.47× throughput improvement through optimized preprocessing, supports 22 mainstream video understanding benchmarks with reproduced accuracy aligned with official scores, and provides systematic optimizations for video RL training.

Conclusion: EasyVideoR1 addresses key challenges in video RL training and provides a complete, efficient framework that enables effective reinforcement learning for large vision-language models on diverse video understanding tasks.
...
TL;DR: PIT is a video-based particle tracking framework combining neural autoencoder landmark detection with differentiable physics constraints, using novel Physics-Informed Landmark Loss (PILL) for unsupervised physical consistency or supervised variant (PILLS) with ground truth.


<details>
  <summary>Details</summary>
Motivation: The paper aims to improve single particle tracking in videos by incorporating physical dynamics knowledge into the tracking process, addressing limitations of purely data-driven approaches that may ignore physical constraints.

Method: Uses autoencoder with split bottleneck separating tracking structure (landmark heatmaps) from background noise. Differentiable physics module constrains landmarks over time. Introduces PILL for unsupervised physical consistency and PILLS for supervised learning with ground truth positions, velocities, and bounce events.

Result: PILLS achieves sub-pixel tracking accuracy for both bilinear and physics-refined decoder outputs under clean and noisy conditions, as demonstrated through a replicated 26 factorial design with 64 configurations.

Conclusion: The framework successfully integrates physical dynamics into particle tracking, enabling both supervised and unsupervised learning with physical consistency, achieving high accuracy even in noisy conditions.
...
TL;DR: GW-HGNN: A graph neural network approach for efficient resource allocation in low-altitude Gaussian splatting that balances reconstruction quality and communication costs by modeling image diversity.


<details>
  <summary>Details</summary>
Motivation: Existing resource allocation schemes for low-altitude Gaussian splatting (LAGS) are inefficient because they prioritize reconstruction quality over communication throughput and fail to account for image diversity from varying drone viewpoints.

Method: Proposes a groupwise heterogeneous graph neural network (GW-HGNN) that explicitly models non-uniform contributions of different image groups to reconstruction. Transforms LAGS losses and communication constraints into graph learning costs for dual-level message passing.

Result: GW-HGNN significantly outperforms state-of-the-art benchmarks on key rendering metrics (PSNR, SSIM, LPIPS) and reduces computational latency by ~100x compared to MOSEK solver, achieving millisecond-level inference suitable for real-time deployment.

Conclusion: GW-HGNN provides an efficient solution for LAGS resource allocation that automatically balances data fidelity and transmission cost while enabling real-time deployment through dramatic latency reduction.
...
TL;DR: USTri is a three-stage pipeline for unified multi-organ, multi-task ultrasound analysis that addresses generalization challenges across heterogeneous ultrasound data and supports interpretable workflow-level analysis.


<details>
  <summary>Details</summary>
Motivation: Clinical ultrasound analysis requires models that generalize across different organs, views, and devices while supporting interpretable workflow-level analysis. Existing methods struggle with task-wise adaptation and joint learning instability due to cross-task interference, making workflow-level outputs difficult to achieve in practice.

Method: USTri uses a tri-stage approach: Stage I trains a universal generalist (USGen) on different domains to learn broad, transferable priors robust to device/protocol variability. Stage II builds specialists (USpec) by keeping USGen frozen and fine-tuning dataset-specific heads to handle domain shifts while preserving ultrasound shared knowledge. Stage III introduces USAgent, which mimics clinician workflows by orchestrating USpec specialists for multi-step inference and deterministic structured reports.

Result: On the FMC_UIA validation set, the model achieves the best overall performance across 4 task types and 27 datasets, outperforming state-of-the-art methods. Qualitative results show USAgent produces clinically structured reports with high accuracy and interpretability.

Conclusion: The study suggests a scalable path to ultrasound intelligence that generalizes across heterogeneous ultrasound tasks and supports consistent end-to-end clinical workflows.
...
TL;DR: KIRA is a unified visual RAG framework that addresses 10 core challenges in visual question answering for specialized domains, achieving high retrieval precision, perfect grounding, and strong domain correctness across medical, technical, and scientific domains.


<details>
  <summary>Details</summary>
Motivation: Current RAG systems work well for text but struggle with visual domains due to modality gaps between images and text-heavy knowledge bases, difficulty constructing meaningful visual knowledge bases, challenges with multihop reasoning over images, and lack of faithful grounding verification for generated answers.

Method: Five-stage framework: (1) hierarchical semantic chunking with DINO-based region detection for multi-granularity knowledge bases, (2) domain-adaptive contrastive encoders with few-shot adaptation for rare concepts, (3) dual-path crossmodal retrieval with chain-of-thought query expansion, (4) chain-of-retrieval for multihop visual reasoning with temporal/multiview support, and (5) evidence-conditioned grounded generation with post-hoc hallucination verification.

Result: Achieves 0.97 retrieval precision, 1.0 grounding scores, and 0.707 domain correctness averaged across four specialized domains (medical X-ray, circuit diagrams, satellite imagery, histopathology). Ablation studies reveal insights about precision-diversity tradeoffs and when each component helps.

Conclusion: KIRA successfully addresses fundamental challenges in visual RAG, providing a unified framework that bridges modality gaps, enables meaningful visual knowledge construction, supports complex reasoning, and ensures faithful grounding, while the proposed DOMAINVQAR benchmark enables comprehensive evaluation beyond standard recall metrics.
...
TL;DR: Proposes residual noise learning framework for cross-dose PET denoising that estimates noise directly from low-dose images instead of predicting full-dose images, outperforming conventional approaches.


<details>
  <summary>Details</summary>
Motivation: Neural networks trained on specific PET dose levels fail to generalize to other doses due to noise variations. Conventional "one-size-for-all" models learn averaged representations that degrade performance.

Method: Unified residual noise learning framework that estimates noise directly from low-dose PET images rather than predicting full-dose images, addressing the implicit optimization over heterogeneous noise distributions.

Result: Outperforms "one-size-for-all" models, individual dose-specific U-Net models, and dose-conditioned approaches on large-scale multi-dose PET datasets from two medical centers, achieving improved denoising performance.

Conclusion: Residual noise learning effectively mitigates the averaging effect and enhances generalization for cross-dose PET denoising by directly estimating noise rather than predicting clean images.
...
TL;DR: CoGR-MoE introduces concept-guided routing for VQA, using answer option semantics to stabilize expert selection and enhance option discrimination through contrastive learning.


<details>
  <summary>Details</summary>
Motivation: Existing MoE methods for VQA suffer from unstable routing (inconsistent expert selection for similar questions) or overly stable routing (reduced flexibility), limiting effective option reasoning.

Method: Concept-Guided Routing (CoGR-MoE) framework that: 1) Uses answer option semantics to guide expert selection during training, 2) Reweights selected experts using option features to create discriminative representations, 3) Employs option comparison via contrastive learning.

Result: CoGR-MoE achieves strong performance across multiple VQA tasks, demonstrating effectiveness of the concept-guided routing approach.

Conclusion: The proposed CoGR-MoE framework successfully addresses routing instability in MoE methods for VQA by incorporating answer option semantics, leading to improved option reasoning and discriminative representations.
...
TL;DR: Proposes ARF-SFR-Net, an adaptive receptive field-based spatial-frequency feature reconstruction network for few-shot fine-grained image classification that dynamically determines receptive field sizes for spatial and frequency features.


<details>
  <summary>Details</summary>
Motivation: Existing feature-based few-shot fine-grained image classification methods struggle with choosing appropriate receptive field sizes for extracting spatial and frequency feature descriptors from different category input images, which limits their performance on FSFGIC tasks.

Method: ARF-SFR-Net adaptively determines receptive field sizes for obtaining both spatial and frequency features, effectively fuses them for reconstruction and classification tasks, and can be embedded into episodic training mechanisms for end-to-end training from scratch.

Result: Extensive experiments on multiple FSFGIC benchmarks demonstrate the effectiveness and superiority of ARF-SFR-Net over state-of-the-art approaches.

Conclusion: The proposed ARF-SFR-Net successfully addresses the receptive field size selection challenge in few-shot fine-grained image classification by adaptively determining optimal receptive field sizes for spatial and frequency feature extraction and fusion.
...
TL;DR: CoDe-MAE addresses the Heterogeneity-Resolution Paradox in high-resolution optical-SAR pretraining by proposing "better synergy with less alignment" through three components: Optical-anchored Knowledge Distillation, Conditioned Contrastive Learning, and Cross-Modal Degraded Reconstruction.


<details>
  <summary>Details</summary>
Motivation: High-resolution joint optical-SAR pretraining faces the Heterogeneity-Resolution Paradox where finer spatial scales amplify physical divergence between radar geometries and optical textures. Existing rigid alignment approaches cause either feature suppression or contamination, leading to representation degradation and negative transfer.

Method: Three-stage approach: 1) Optical-anchored Knowledge Distillation (OKD) regularizes SAR speckle noise into pure semantic manifold; 2) Conditioned Contrastive Learning (CCL) with gradient buffering aligns shared consensus while preserving divergent physical signatures; 3) Cross-Modal Degraded Reconstruction (CDR) strips non-homologous spectral pseudo-features to capture true structural invariants.

Result: Pretrained on 1M samples, CoDe-MAE demonstrates remarkable data efficiency, prevents representation degradation, and establishes new state-of-the-art performance across diverse single- and bi-modal downstream tasks, substantially outperforming foundation models scaled on larger datasets.

Conclusion: The proposed "better synergy with less alignment" philosophy successfully overcomes the Heterogeneity-Resolution Paradox in high-resolution optical-SAR pretraining, enabling robust representation learning without forcing rigid alignment that causes feature degradation.
...
TL;DR: TSM-Pose is a novel framework for category-level object pose estimation that uses topology-aware learning with semantic Mamba to improve generalization to unseen instances by capturing category-shared topological structures and enhancing semantic keypoint modeling.


<details>
  <summary>Details</summary>
Motivation: Existing methods for category-level object pose estimation struggle with robust generalization to unseen instances because they rely on simple feature extraction and aggregation that fail to capture category-shared topological structures and conduct effective semantic keypoint modeling.

Method: TSM-Pose introduces two key components: 1) A Topology Extractor to capture global topological representations of point clouds and integrate them with local geometry features, and 2) A Mamba-based Global Semantic Aggregator that injects semantic priors into keypoints and uses multiple TwinMamba blocks to model long-range dependencies for better global feature aggregation.

Result: Extensive experiments on three benchmark datasets (REAL275, CAMERA25, and HouseCat6D) demonstrate that TSM-Pose outperforms existing state-of-the-art methods.

Conclusion: The proposed TSM-Pose framework effectively addresses the generalization challenges in category-level object pose estimation by combining topology-aware learning with semantic Mamba, achieving superior performance through better capture of category-shared structures and enhanced semantic keypoint modeling.
...
TL;DR: TRU (Temporal Retinal U-Net), a deterministic regression model for predicting future retinal appearance, outperforms complex generative methods in slowly progressing macular disease by focusing on predictable components of change.


<details>
  <summary>Details</summary>
Motivation: Current clinical decisions in progressive macular disease rely on qualitative comparisons or scalar progression scores. There's a need for quantitative prediction of future retinal appearance from longitudinal imaging, but it's unclear whether complex generative methods are necessary for slowly progressing retinal disease.

Method: Compared five conditioning configurations with same architecture and dataset, then developed TRU - a deterministic direct-regression model with continuous time-delta conditioning and multi-scale history aggregation. Evaluated on 28,902 eyes across three imaging platforms with mixed-disease cohorts.

Result: Aligning training and inference input distributions produced large gains (delta-SSIM +0.082, SSIM +0.086, p<0.001). TRU matched or exceeded delta-SSIM, SSIM, and PSNR in every FAF cohort against three state-of-the-art benchmarks, with advantage growing monotonically with available history length.

Conclusion: For slowly progressing retinal disease, deterministic regression with proper conditioning outperforms complex generative methods because predictable components of inter-visit change are small relative to acquisition variability, leaving little width for stochastic sampling to exploit.
...
TL;DR: A self-reasoning agentic framework for generating narrative product grid collages that ensures visual consistency and storytelling coherence through iterative refinement.


<details>
  <summary>Details</summary>
Motivation: Existing image generation methods lack support for structured narrative planning and cross-panel coordination, resulting in weak storytelling and visual incoherence in product marketing collages.

Method: Proposes a self-reasoning agentic framework that: 1) constructs a Product Narrative Framework representing product identity, usage context, and environment; 2) translates this into complementary grids with shared visual style; 3) uses constraint-aware prompts for joint collage synthesis; 4) evaluates outputs on content validity and photography quality; 5) performs failure attribution and targeted refinement through iterative self-reflection.

Result: The framework consistently improves aesthetic quality, narrative richness, and visual coherence compared to direct prompting baselines.

Conclusion: The proposed agentic framework effectively addresses the limitations of existing methods by enabling structured narrative planning, cross-panel coordination, and iterative refinement for generating visually coherent narrative product grid collages.
...
TL;DR: BLUTHs (Binary Linear Unmixing Tactile Hierarchies) use hierarchical structure in Deep Nonnegative Matrix Factorization to address spectral variability, endmember number ambiguity, and clarity degradation in hyperspectral unmixing.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral unmixing faces three key challenges: spectral variability inhibits performance, determining the number of endmembers is ambiguous, and endmember clarity degrades as more are included. Hierarchical structure offers a potential solution to all three problems.

Method: Hierarchical unmixing is achieved by imposing hierarchical abundance sum constraints on Deep Nonnegative Matrix Factorization. BLUTHs solve this with a simple network architecture, and sparsity modulation unmixing growth tailors the topology of each BLUTH to specific scenes.

Result: BLUTHs outperform state-of-the-art unmixing algorithms on laboratory scenes, particularly for abundance estimation, while remaining competitive on remote sensing scenes. They successfully demonstrate ocean color unmixing on hyperspectral data from HYPSO and PACE satellites.

Conclusion: Hierarchical unmixing via BLUTHs effectively addresses key challenges in hyperspectral analysis by revealing endmembers with varying spectral contrast, mitigating spectral variability, and providing robust performance across different imaging scenarios.
...
TL;DR: Proposes UGD - an unsupervised geometric distance metric for point cloud denoising evaluation that works without ground-truth clean point clouds, using learned patch-wise priors from clean data.


<details>
  <summary>Details</summary>
Motivation: Existing point cloud denoising evaluation metrics require ground-truth clean point clouds, which are often unavailable in real-world scenarios, creating a significant limitation for practical applications.

Method: Learn a pristine Gaussian Mixture Model (GMM) from clean point clouds using patch-wise quality-aware features extracted by a self-supervised network trained with multi-task learning (quality ranking, distortion classification, distribution prediction). UGD computes weighted distances between denoised patches and the learned GMM.

Result: UGD achieves comparable performance to supervised full-reference metrics on synthetic noise, and enables effective unsupervised evaluation on real-world data using only noisy point clouds.

Conclusion: UGD provides a practical unsupervised solution for evaluating point cloud denoising methods in real-world scenarios where ground-truth clean data is unavailable, bridging an important gap in point cloud quality assessment.
...
TL;DR: DOSE uses off-the-shelf pretrained models to filter multimodal training data without task-specific training, improving efficiency and performance by selecting high-quality, diverse samples through adaptive weighted sampling.


<details>
  <summary>Details</summary>
Motivation: Existing multimodal datasets often contain noisy, redundant, and poorly aligned samples, and current data filtering methods require training filtering models on the same data they screen, introducing extra computational costs.

Method: Uses off-the-shelf pretrained models (without fine-tuning) to assess text quality and image-text alignment, builds a joint quality-alignment distribution, and applies adaptive weighted sampling to select informative samples while maintaining long-tail diversity.

Result: Models trained on DOSE-filtered data match or surpass those trained on the full dataset on standard VQA and math benchmarks, demonstrating effectiveness, efficiency, and scalability.

Conclusion: Off-the-shelf pretrained models can effectively guide data selection for multimodal learning without task-specific training, reducing computational costs while improving data quality and diversity.
...
TL;DR: URVIS 2026 challenge report on adverse-to-extreme panoptic segmentation using multimodal MUSES dataset with 17 participants and 47 submissions, evaluated using Weighted Panoptic Quality metric.


<details>
  <summary>Details</summary>
Motivation: To advance robust multimodal panoptic segmentation in adverse-to-extreme weather conditions by creating the first challenge of its kind and establishing benchmark results.

Method: Challenge based on MUSES dataset with multi-sensor data (RGB camera, LiDAR, radar, event camera), using Weighted Panoptic Quality (wPQ) as ranking metric to fairly evaluate across weather conditions.

Result: 17 registered participants, 47 submissions, 4 teams reached final phase; challenge results summarized and performance of submitted methods analyzed.

Conclusion: The challenge successfully benchmarked current progress in multimodal panoptic segmentation for adverse weather, while identifying remaining challenges for robust performance.
...
TL;DR: DVAR is a training-free framework that reformulates video authenticity detection as a structured multi-agent debate between generative and natural mechanism explanations, using Minimum Description Length to adjudicate conflicting claims and achieving competitive performance with superior generalization.


<details>
  <summary>Details</summary>
Motivation: Rapid evolution of video generation technologies challenges media forensics, as conventional detection methods fail to generalize beyond their training distributions. There's a need for more robust approaches that can handle unseen generative architectures.

Method: DVAR transforms video detection into a structured multi-agent forensic reasoning process with two competing agents: Generative Hypothesis Agent and Natural Mechanism Agent. They engage in iterative cross-examination to defend their explanations. The framework uses Minimum Description Length (MDL) to apply Occam's Razor and quantify "Explanatory Cost" for each reasoning path. It also integrates GenVideoKB, a dynamic knowledge repository providing reasoning heuristics about generative boundaries and failure modes.

Result: Extensive experiments show DVAR achieves competitive performance against supervised state-of-the-art methods while exhibiting superior generalization to unseen generative architectures. The framework provides explicit, interpretable reasoning traces for robust video authenticity assessment.

Conclusion: DVAR successfully transforms video detection from pattern matching to transparent debate, offering a training-free solution that generalizes well to new generative technologies while providing interpretable reasoning for forensic analysis.
...
TL;DR: ICNNM improves CNNM by using pre-learned convolution eigenvectors to avoid expensive SVD computations, achieving faster tensor completion with better performance.


<details>
  <summary>Details</summary>
Motivation: CNNM's tensor completion method requires multiple SVD computations which are computationally expensive and hard to parallelize, limiting its practical application.

Method: Reformulate CNNM's optimization from convolution eigenvectors perspective, introduce pre-learned convolution eigenvectors shared across tensors, bypass SVD step entirely.

Result: ICNNM significantly reduces computational time while improving recovery performance due to extra prior knowledge in pre-learned eigenvectors, outperforming CNNM and other methods.

Conclusion: ICNNM provides an efficient and effective solution for tensor completion with arbitrary sampling, validated through video completion, prediction, and frame interpolation tasks.
...
TL;DR: TeMuDance enables text-based control for music-conditioned dance generation without requiring annotated music-text-motion datasets, using motion as a semantic anchor to align disjoint datasets and training a lightweight text control branch on a frozen diffusion backbone.


<details>
  <summary>Details</summary>
Motivation: Existing music-driven dance generation methods lack semantic controllability through natural language descriptions, primarily due to the absence of large-scale datasets that jointly align music, text, and motion for supervised learning of text-conditioned control.

Method: Introduces a motion-centred bridging paradigm that uses motion as a shared semantic anchor to align disjoint music-dance and text-motion datasets in a unified embedding space, enabling cross-modal retrieval of missing modalities. A lightweight text control branch is trained on top of a frozen music-to-dance diffusion backbone, with dual-stream fine-tuning and confidence-based filtering to suppress noise from retrieved supervision.

Result: TeMuDance achieves competitive dance quality while substantially improving text-conditioned control over existing methods, as demonstrated through extensive experiments and a novel task-aligned metric that quantifies whether textual prompts induce intended kinematic attributes under music conditioning.

Conclusion: The proposed framework successfully enables text-based semantic control for music-conditioned dance generation without requiring manually annotated triplet datasets, addressing a significant limitation in existing approaches while maintaining rhythmic fidelity and dance quality.
...
TL;DR: MobileAgeNet is a lightweight facial age estimation model achieving 4.65 MAE on UTKFace with 14.4ms latency on mobile devices, using MobileNetV3-Large backbone and compact regression head.


<details>
  <summary>Details</summary>
Motivation: Mobile deployment requires models that balance accuracy with low latency and compact size for real-time facial age estimation on devices.

Method: Uses pretrained MobileNetV3-Large backbone with compact regression head, bounded age regression, two-stage fine-tuning strategy, and integrated training/evaluation pipeline in NN LEMUR Dataset framework.

Result: Achieves 4.65 MAE on UTKFace test set with 3.23M parameters and 14.4ms average latency on mobile devices, maintaining predictive behavior through deployment pipeline.

Conclusion: Provides a practical, deployment-ready baseline for mobile-oriented facial age estimation with competitive accuracy and efficient on-device inference.
...
TL;DR: A Transformer-based model for heterogeneous skeleton-based action recognition with open vocabularies, using unified skeleton representation and multi-grained motion-text alignment.


<details>
  <summary>Details</summary>
Motivation: Skeleton-based action recognition is crucial for human-robot interaction, but existing methods overlook data heterogeneity between human and robot skeletons and lack open-vocabulary capabilities needed for real-world applications.

Method: Proposes a Transformer-based model with three components: 1) unified skeleton representation to handle heterogeneous data, 2) motion encoder using two-stream Transformer to learn spatio-temporal features, and 3) multi-grained motion-text alignment with contrastive learning at instance, stream-specific, and fine-grained levels.

Result: Extensive experiments on heterogeneous skeleton benchmarks demonstrate the method's effectiveness and generalization ability. A large-scale Heterogeneous Open-Vocabulary (HOV) Skeleton dataset was created by integrating multiple existing datasets.

Conclusion: The proposed approach successfully addresses the challenging problem of heterogeneous skeleton-based action recognition with open vocabularies, enabling universal skeleton-based action recognition for diverse skeleton sources including both humans and humanoid robots.
...
TL;DR: LIVE is a joint training framework that leverages large-scale image editing data alongside video datasets to improve video editing capabilities, addressing the data scarcity problem in video editing through innovative domain adaptation techniques.


<details>
  <summary>Details</summary>
Motivation: Video editing faces severe data scarcity compared to image editing due to high annotation costs, which limits the scale, quality, and task diversity of video editing datasets when relying on video generation models or manual annotation.

Method: Proposes LIVE framework with: 1) Frame-wise token noise strategy to mitigate domain discrepancy between static images and dynamic videos, 2) Automated data pipeline with two-stage training strategy, 3) Leveraging large pretrained video generative models for temporal transformations.

Result: Achieves state-of-the-art performance, demonstrated through extensive comparative and ablation experiments on a comprehensive evaluation benchmark with over 60 challenging tasks.

Conclusion: LIVE successfully bridges the gap between image and video editing by effectively leveraging large-scale image editing data, offering a practical solution to video editing data scarcity while maintaining high performance.
...
TL;DR: CAM3DNet is a sparse query-based 3D object detection framework using multi-view images that addresses inefficient multi-scale feature utilization through three novel modules: composite query, adaptive self-attention, and multi-scale hybrid sampling.


<details>
  <summary>Details</summary>
Motivation: Existing query-based 3D object detection methods struggle to efficiently leverage dynamic multi-scale information from multi-view images, with insufficient learning of relationships between object features and query geometry, and high computational costs when exploring multi-scale spatiotemporal features.

Method: Proposes CAM3DNet with three key modules: 1) Composite Query (CQ) using multi-scale projection to transform 2D queries to 3D space, 2) Adaptive Self-Attention (ASA) learning interactions between spatiotemporal multi-scale queries, and 3) Multi-Scale Hybrid Sampling (MSHS) using deformable attention to sample multi-scale object information considering queries, pyramid features, and 2D-camera priors. Uses backbone+FPN encoder, YOLOX+DepthNet ROI_Head for CQ generation, and ASA+MSHS decoder.

Result: Extensive experiments on nuScenes, Waymo, and Argoverse benchmarks demonstrate CAM3DNet outperforms most existing camera-based 3D object detection methods. Comprehensive ablation studies validate individual contributions of CQ, ASA, and MSHS modules, along with analysis of space and computation complexity costs.

Conclusion: CAM3DNet effectively addresses multi-scale feature utilization challenges in query-based 3D object detection through its novel composite query, adaptive self-attention, and multi-scale hybrid sampling modules, achieving state-of-the-art performance on major autonomous driving datasets.
...
TL;DR: Proposes IMA-MoE, an interpretable multimodal AI framework to identify biological signatures of binge eating disorder, outperforming baselines and revealing sex-specific patterns.


<details>
  <summary>Details</summary>
Motivation: Current BED diagnosis relies on symptom-based criteria rather than biological mechanisms, limiting early detection and targeted interventions. Existing neurobiological studies have limited generalizability due to hypothesis-driven models, single-modality analyses, and limited data diversity.

Method: Developed Interpretable Modality-Aware Mixture-of-Experts (IMA-MoE) architecture that integrates heterogeneous neuroimaging, behavioral, hormonal, and demographic data. Encodes each measure as distinct tokens to model cross-modal dependencies while preserving modality-specific characteristics. Includes token-importance mechanism for interpretability.

Result: IMA-MoE demonstrated superior performance in differentiating BED from healthy controls compared to baseline methods on the ABCD dataset. Revealed sex-specific predictive patterns, with hormonal measures contributing more prominently to prediction in females.

Conclusion: Interpretable, data-driven multimodal modeling shows promise for advancing biologically-informed characterization of BED and facilitating more precise, personalized interventions in neuropsychiatric disorders.
...
TL;DR: CERD is a framework for interpretable multimodal diagnosis that handles missing modalities by reconstructing missing data and decomposing evidence into shared and modality-specific cues.


<details>
  <summary>Details</summary>
Motivation: Neurobiological diseases are multifactorial requiring multimodal modeling, but real-world data often has incomplete modality coverage. Existing approaches fail to capture subject-specific cross-modal dependencies and lack interpretability into which evidence sources drive decisions.

Method: CERD (Conditional Evidence Reconstruction and Decomposition) first reconstructs missing modality representations conditioned on each subject's observed inputs, then decomposes diagnostic evidence into shared cross-modal corroboration and modality-specific cues via logit-level attribution.

Result: Experiments on Alzheimer's Disease Neuroimaging Initiative (ADNI) show CERD outperforms competitive baselines under incomplete-modality settings while producing structured and clinically aligned evidence attributions.

Conclusion: CERD provides an effective framework for interpretable multimodal diagnosis with incomplete modalities, offering trustworthy decision support through structured evidence decomposition.
...
TL;DR: SIF is a stealthy ownership verification framework for large vision-language models that uses semantically in-distribution fingerprints to protect against unauthorized reuse, overcoming vulnerabilities in existing fingerprint methods.


<details>
  <summary>Details</summary>
Motivation: Public accessibility of LVLMs raises concerns about unauthorized model reuse and IP infringement. Existing fingerprint methods are vulnerable to detection and removal through semantic divergence attacks, requiring more stealthy and robust solutions.

Method: Proposes SIF framework with two key components: 1) Semantic-Aligned Fingerprint Distillation (SAFD) transfers text watermarking signals to visual modality for semantically coherent fingerprinted responses; 2) Robust-Fingerprint Optimization (RFO) enhances robustness by simulating worst-case representation perturbations against model modifications.

Result: Extensive experiments on LLaVA-1.5 and Qwen2.5-VL demonstrate SIF achieves strong stealthiness and robustness, providing practical copyright protection against attacks like fine-tuning and quantization.

Conclusion: SIF offers a non-intrusive, semantically-preserving fingerprinting solution that addresses vulnerabilities in existing methods and provides effective ownership verification for LVLMs.
...
TL;DR: A real-time collision warning system for cyclists and pedestrians using fisheye cameras on edge devices, featuring calibration, detection, simulation, and decision layers with 93.3% sensitivity and 3.3s warning time.


<details>
  <summary>Details</summary>
Motivation: Collisions between cyclists and pedestrians at urban intersections remain a persistent injury source, with few systems offering real-time warnings using commodity hardware for unequipped road users.

Method: Four key contributions: 1) Calibration pipeline for ultra-wide fisheye lenses using perspective remapping and direct bundle adjustment; 2) Fisheye-aware object detection with ground-plane projection via lookup table; 3) Design-time conformance simulation with 24 hazard scenarios and latency analysis; 4) Formalized decision layer with deployment gates, contestability mechanisms, and risk register.

Result: System achieves 93.3% sensitivity and 92.3% specificity with mean warning budget of 3.3 seconds under conformance testing with fisheye localization error, running at 30 fps on edge device.

Conclusion: The prototype demonstrates effective real-time collision warning using commodity hardware, with community-informed design, formal safety mechanisms, and promising performance metrics for urban intersection safety.
...
TL;DR: OASIS is a novel framework for streaming video reasoning that uses structured, on-demand retrieval to efficiently handle long video histories by organizing them into hierarchical events and performing controlled refinement with intent-driven retrieval.


<details>
  <summary>Details</summary>
Motivation: Streaming video reasoning faces the challenge of unbounded history growth with scarce meaningful evidence - relevant signal is rare and easily lost in redundancy. Traditional approaches of enlarging memory or aggressive compression fail to address the core problem of discovering where to look rather than how much to remember.

Method: OASIS organizes streaming history into hierarchical events and performs reasoning as controlled refinement: short-context inference first, followed by semantically grounded retrieval only when uncertainty arises. Retrieval is driven by high-level intent rather than embedding similarity, making it more accurate and less noisy. The framework is plug-and-play, training-free, and attaches to different streaming MLLM backbones.

Result: Experiments across multiple benchmarks and backbones show OASIS achieves strong gains in long-horizon accuracy and compositional reasoning with bounded token cost and low request delay.

Conclusion: OASIS effectively addresses the streaming video reasoning challenge through structured, on-demand retrieval that focuses on discovering where to look rather than how much to remember, providing an efficient and accurate solution for long-horizon video understanding.
...
TL;DR: Training-free multimodal embedding framework using MLLM to align text, raster images, and SVG code through instruction guidance and structural cues, outperforming training-based methods on text-to-SVG retrieval.


<details>
  <summary>Details</summary>
Motivation: SVGs contain rich structural information that gets lost when rasterized, while current text embedding methods don't handle visual/structured modalities well. Need a way to align multimodal representations without extensive training.

Method: Two key components: 1) Multimodal Explicit One-word Limitation (mEOL) - instructs MLLM to summarize any input into a single token whose hidden state serves as embedding. 2) Semantic SVG rewriting - assigns meaningful identifiers and simplifies nested elements through visual reasoning over rendered images.

Result: Outperforms encoder-based and training-based multimodal baselines on the first text-to-SVG retrieval benchmark (repurposed VGBench), demonstrating training-free approach effectiveness.

Conclusion: Prompt-level control via instruction-guided MLLMs offers effective alternative to parameter-level training for structure-aware multimodal retrieval, preserving SVG's geometric and relational information.
...
TL;DR: A novel CLIP-based framework for zero-shot action recognition using disentangled embeddings and semantic-guided interaction to bridge the semantic gap between seen and unseen classes.


<details>
  <summary>Details</summary>
Motivation: Zero-shot action recognition faces challenges due to the semantic gap between seen and unseen classes, requiring better feature representation and semantic alignment for effective generalization.

Method: Proposes Motion Separation Module (MSM) to separate motion-sensitive and global-static features, and Motion Aggregation Block (MAB) with gated cross-attention to refine motion representation. Uses semantic alignment between video features and textual representations with positive/negative prompts to model class and non-class semantics.

Result: The method consistently outperforms prior CLIP-based approaches on standard benchmarks, achieving robust zero-shot action recognition across both coarse and fine-grained datasets.

Conclusion: The proposed framework effectively addresses the semantic gap in zero-shot action recognition through disentangled feature learning and semantic-guided interaction, demonstrating superior performance over existing methods.
...
TL;DR: BasketHAR: A multimodal basketball training dataset with IMU sensors, video, and physiological data for specialized human activity recognition beyond basic activities.


<details>
  <summary>Details</summary>
Motivation: Existing HAR datasets focus on basic activities (walking, standing, stairs) and lack specialized datasets for contexts like sports performance analysis, particularly basketball training.

Method: Created BasketHAR dataset with multimodal data: IMU sensors (accelerometer, gyroscope), angular velocity, magnetic field, heart rate, skin temperature, and synchronized video recordings of professional-level basketball actions. Also developed baseline multimodal alignment method for benchmarking.

Result: Dataset demonstrates complexity and suitability for advanced HAR tasks. Experimental results validate its utility for basketball training analysis and specialized performance report generation.

Conclusion: BasketHAR addresses the gap in specialized sports HAR datasets, provides a valuable resource for future HAR and sports analytics research, and enables applications in basketball training analysis and performance reporting.
...
TL;DR: NTIRE 2026 RipDetSeg Challenge for automatic rip current detection and segmentation in images, using diverse RipVIS dataset from 10+ countries, with 159 participants and 9 valid submissions evaluated on composite F-score metrics.


<details>
  <summary>Details</summary>
Motivation: Rip currents are hazardous nearshore flows causing many beach fatalities worldwide, but are difficult to identify due to substantial visual variation across beaches, viewpoints, and sea states. There's a need for automatic rip current understanding to advance safety-critical research.

Method: The challenge builds on the RipVIS benchmark evaluating both detection and segmentation tasks. It uses a diverse dataset sourced from more than 10 countries with 4 camera orientations and diverse beach/sea conditions. Evaluation uses a composite score combining F₁[50], F₂[50], F₁[40:95], and F₂[40:95] metrics.

Result: The challenge attracted 159 registered participants and produced 9 valid test submissions across two tasks. Most solutions relied on pretrained models combined with strong augmentation and post-processing design. Results show rip current understanding benefits from robust general-purpose vision models but leaves room for future specialized methods.

Conclusion: The RipDetSeg Challenge successfully advanced research on rip current understanding, demonstrating that current vision models can handle this safety-critical problem while highlighting the need for future methods tailored to rip currents' unique visual structure.
...
TL;DR: RefVQA: A reference-aware video quality assessment method for AI-generated content that uses graph-based comparisons between related videos instead of analyzing each video independently.


<details>
  <summary>Details</summary>
Motivation: Existing AI-generated video quality assessment methods analyze each video independently, ignoring relationships between videos. This approach doesn't align well with human perception, which naturally compares related content when assessing quality.

Method: Proposes Reference-aware Video Quality Assessment (RefVQA) that formulates quality assessment as a reference-aware evaluation problem. Uses a query-centered reference graph to organize semantically related samples and performs graph-guided difference aggregation from reference nodes to the query node.

Result: RefVQA outperforms state-of-the-art methods across multiple quality dimensions on existing datasets. Shows strong generalization ability validated by cross-dataset evaluation.

Conclusion: The reference-based formulation is effective for AI-generated video quality assessment and has potential to advance the field by better aligning with human perceptual processes.
...
TL;DR: D-Prism is a framework that extends differentiable primitives to dynamic scenes, combining 3D Gaussian Splatting with primitive surfaces for high-fidelity structured dynamic modeling of articulated objects.


<details>
  <summary>Details</summary>
Motivation: Existing dynamic methods (deformable meshes, 3DGS) use unstructured representations and fail to jointly model suitable geometry and articulated motion for structured dynamic objects like multi-part assemblies or jointed mechanisms. Primitive-based methods work well for static scenes but haven't been explored for dynamic scenarios.

Method: 1) Bind 3D Gaussian Splatting (3DGS) to primitive surfaces to leverage their respective strengths in appearance and geometry. 2) Introduce a deformation network to control primitive motion, ensuring accurate object movement matching. 3) Design a novel adaptive control strategy to dynamically adjust primitive counts to better match objects' true spatial footprint.

Result: Experiments confirm that D-Prism excels at structured dynamic modeling, providing both structured geometry and precise motion tracking for articulated objects.

Conclusion: D-Prism is the first framework to achieve high-fidelity structured dynamic modeling by extending differentiable primitives to the dynamic domain, successfully capturing both geometry and rigid motion for structured dynamic objects.
...
TL;DR: EvoComp is a visual token compression framework for MLLMs that reduces token count while maintaining accuracy through evolutionary token selection and specialized training.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs suffer from inference inefficiency due to large numbers of visual tokens, especially in high-resolution or multi-image scenarios, creating a need for effective token compression.

Method: Uses lightweight encoder-only transformer compressor to select informative tokens; employs evolutionary labeling strategy to find optimal token subsets; trains with GHM loss for class/difficulty balance and cosine similarity regularization for semantic separation.

Result: Outperforms existing attention/similarity methods; retains 99.3% accuracy under 3x compression; achieves up to 1.6x speedup on mobile devices across multiple benchmarks.

Conclusion: EvoComp effectively addresses visual token inefficiency in MLLMs through evolutionary token selection and balanced training, enabling significant speed improvements with minimal accuracy loss.
...
TL;DR: CoAMD is a unified framework for both action recognition and motion generation using skeleton coordinates, achieving SOTA across multiple tasks through coarse-to-fine motion synthesis with semantic guidance.


<details>
  <summary>Details</summary>
Motivation: Most existing works treat action recognition and motion generation as separate problems, missing the connection that motion generation requires semantic comprehension. The paper aims to unify these tasks by leveraging their inherent relationship.

Method: Proposes Coordinates-based Autoregressive Motion Diffusion (CoAMD) with Multi-modal Action Recognizer (MAR) for gradient-based semantic guidance. Uses skeleton coordinates for both understanding and generation, synthesizing motion in coarse-to-fine manner.

Result: Achieves state-of-the-art performance on 13 benchmarks across four tasks: skeleton-based action recognition, text-to-motion generation, text-motion retrieval, and motion editing.

Conclusion: CoAMD demonstrates effectiveness and versatility for human motion modeling by unifying action recognition and motion generation, showing that semantic guidance from recognition improves generation quality.
...
TL;DR: HB-Net-18: A two-stage AI framework combining Hadamard U-Net bias field correction with ResNet-18 for prostate cancer detection using quantitative HM-MRI, outperforming conventional radiomics and baseline CNNs.


<details>
  <summary>Details</summary>
Motivation: MRI is crucial for prostate cancer diagnosis, and while advanced HM-MRI techniques have improved diagnostic capabilities, there's still a significant need for robust, automated AI-based detection methods to enhance clinical decision-making.

Method: Two-stage framework: 1) Hadamard U-Net suppresses intensity inhomogeneities (bias fields) across six parametric HM-MRI maps generated via Physics-Informed Autoencoder; 2) ResNet-18 performs patch-level classification using overlapping 11×11 patches with both 2D intra-slice and 3D inter-slice information for spatial consistency.

Result: HB-Net achieves balanced sensitivity and specificity, significantly outperforming conventional radiomics-based approaches and baseline CNN models, demonstrating superior performance for prostate cancer detection.

Conclusion: The proposed HBR-Net-18 framework shows strong potential for clinical deployment in prostate cancer diagnosis by effectively combining quantitative HM-MRI with advanced AI techniques for robust automated detection.
...
TL;DR: Autonomous coding agents enable clinicians to independently develop clinical AI models through natural language interaction, reducing dependence on specialized AI teams.


<details>
  <summary>Details</summary>
Motivation: Traditional clinical AI development requires extensive collaboration between clinicians and AI teams, which is time-consuming and prone to misalignment due to expertise gaps. Autonomous coding agents could shift this paradigm toward clinician-driven development.

Method: Developed an autonomous prototype system that allows clinicians to create clinical AI models through natural-language interaction. Evaluated on five clinical tasks: dermoscopic lesion classification, melanoma-vs-nevus triage, wrist-fracture detection (including weakly supervised variant with 5% bounding-box annotations), and debiased pneumothorax classification on chest radiographs.

Result: The system consistently developed models from clinician requests with promising performance. In debiased pneumothorax classification, it successfully mitigated shortcut learning and nearly halved the model's reliance on chest drains as confounders.

Conclusion: Autonomous coding agents provide proof of concept for shifting clinical AI development toward a clinician-driven paradigm, reducing communication overhead and dependence on specialized AI developers. While further validation is needed, this approach makes clinical AI development more accessible.
...
TL;DR: TLDR: A temporal probability smoothing method improves SAM2 video segmentation stability by blending current predictions with motion-aligned historical estimates using optical flow and uncertainty metrics, without retraining.


<details>
  <summary>Details</summary>
Motivation: SAM2 shows strong generalization but suffers from temporal instability (flickering boundaries, object dropout, inconsistent extents) under weak user supervision like sparse point prompts, limiting reliability for video applications.

Method: Inference-time temporal probability smoothing that operates on per-frame segmentation probability maps. Uses optical-flow-based motion warping with pixel-wise uncertainty estimates from segmentation entropy and forward-backwards flow consistency. Adaptively blends current-frame predictions with motion-aligned historical estimates.

Result: Consistent improvements in temporal stability over vanilla SAM2 across four diverse video sequences, measured by motion-compensated IoU, boundary consistency, object persistence, and area volatility. Preserves spatial accuracy while being lightweight and model-agnostic.

Conclusion: The proposed framework effectively improves temporal stability of SAM2-based video segmentation without retraining, is suitable for real-time interactive applications, and maintains spatial accuracy while being model-agnostic.
...
TL;DR: Multimodal framework combining histopathology images and clinical EHR data outperforms unimodal approaches for breast cancer diagnosis, achieving near-perfect accuracy with improved interpretability.


<details>
  <summary>Details</summary>
Motivation: Breast cancer requires timely accurate diagnosis. Existing work treats histopathology images and clinical EHR data in isolation, missing opportunities for multimodal integration that could improve both predictive performance and clinical transparency.

Method: Systematic multimodal framework integrating patch-level histopathology features from BreCaHAD dataset with structured clinical data from MIMIC-IV. Evaluated unimodal image models (CNN baseline, ResNet-18 with transfer learning), unimodal tabular models (XGBoost, MLP), and intermediate-fusion model concatenating latent representations from both modalities.

Result: ResNet-18 achieved perfect accuracy (1.000) and AUC (1.000) on patch-level classification. XGBoost achieved 98% accuracy on EHR prediction. Intermediate fusion model yielded macro-average AUC of 0.997, outperforming all unimodal baselines, with largest improvements on class-imbalanced mitosis category (AUC 0.994). Grad-CAM and SHAP analyses validated model decisions align with pathological/clinical criteria.

Conclusion: Multimodal integration delivers meaningful improvements in both predictive performance and clinical transparency for breast cancer diagnosis, demonstrating the value of combining histopathology and clinical data modalities.
...
TL;DR: Vision-language models show inconsistent object grounding for semantically equivalent prompts, with structured variability not explained by text embedding distances.


<details>
  <summary>Details</summary>
Motivation: To examine the implicit assumption that semantically equivalent descriptions yield consistent object grounding outputs in vision-language models.

Method: Controlled pipeline combining DETR for object proposals with CLIP for language-conditioned selection on 263 COCO val2017 images, analyzing overlapping prompts like "a person," "a human," and "a pedestrian."

Result: Semantically equivalent prompts frequently select different instances (mean instability of 2.11 distinct selections across six prompts). Variability is structured and directional (not random). Prompt ensembling doesn't improve quality and often shifts selections toward generic regions. Text embedding proximity explains only 34% of grounding disagreement.

Conclusion: Vision-language models exhibit significant instability in object grounding for semantically equivalent prompts, with variability arising from the argmax selection mechanism rather than text-level distances alone.
...
TL;DR: OptiMVMap improves offline vectorized mapping by strategically selecting complementary vehicle viewpoints to overcome occlusion and viewpoint limitations of single-ego approaches.


<details>
  <summary>Details</summary>
Motivation: Single ego-vehicle trajectories suffer from viewpoint insufficiency and occlusion problems. While multi-vehicle approaches offer complementary perspectives, naive fusion introduces computational cost, redundancy, and noise challenges.

Method: Reformulates multi-vehicle mapping as select-then-fuse problem with Optimal Vehicle Selection (OVS) to identify compact helper subsets, Cross-Vehicle Attention (CVA) for pose-tolerant alignment, and Semantic-aware Noise Filter (SNF) for artifact suppression before BEV-level fusion.

Result: Improves MapTRv2 by +10.5 mAP on nuScenes and +9.3 mAP on Argoverse2, surpasses memory-augmented baselines MVMap and HRMapNet by +6.2 mAP and +3.8 mAP on nuScenes, achieving more complete maps with fewer views.

Conclusion: Uncertainty-guided selection of helper vehicles is essential for efficient and accurate multi-vehicle vectorized mapping, addressing computational, redundancy, and noise challenges while improving map completeness and topological fidelity.
...
TL;DR: ScenarioControl is the first vision-language control mechanism for generating diverse, realistic 3D driving scenarios from text prompts or images, producing temporally consistent rollouts with fine-grained control over road layout and traffic conditions.


<details>
  <summary>Details</summary>
Motivation: There's a need for controllable generation of realistic driving scenarios for autonomous vehicle development, but existing methods lack fine-grained multimodal control over both road structure and dynamic agents while maintaining temporal consistency.

Method: Uses a vectorized latent space representing road structure and dynamic agents jointly, with a cross-global control mechanism that integrates crossattention with a lightweight global-context branch to connect multimodal control with sparse vectorized scene elements.

Result: The method successfully generates diverse, realistic 3D scenario rollouts including maps, 3D boxes of reactive actors over time, pedestrians, driving infrastructure, and ego camera observations, with favorable control adherence and fidelity compared to tested methods.

Conclusion: ScenarioControl enables fine-grained multimodal control over driving scenario generation while preserving realism and temporal consistency, supporting long-horizon continuation and different actor perspectives, with released dataset for training and evaluation.
...
TL;DR: The paper presents an efficient method for back-projecting 2D image information (colors, features, segmentation masks) onto existing 3D Gaussian splat scenes using visibility-weighted least squares optimization.


<details>
  <summary>Details</summary>
Motivation: While current Gaussian Splatting frameworks excel at forward rendering (3D to 2D), there's a need for efficient inverse mapping (2D to 3D) to enable applications like scene relighting, stylization, and 3D semantic segmentation.

Method: The approach uses the normal equation to solve a visibility-weighted least squares problem for each Gaussian, implemented efficiently with existing differentiable rasterizers.

Result: The method achieves up to an order of magnitude speedup compared to gradient descent-based baselines on tasks including scene relighting, feature enrichment, and 3D semantic segmentation.

Conclusion: The proposed approach provides an efficient solution for back-projecting 2D information onto 3D Gaussian splats, enabling various practical applications while addressing challenges like view-dependent colorization and occlusion handling.
...
TL;DR: PPEDCRF is a selective perturbation framework that protects location privacy in video frames by injecting Gaussian noise only into location-sensitive background regions identified by a dynamic CRF, achieving better visual quality than global noise at similar privacy levels.


<details>
  <summary>Details</summary>
Motivation: Even after removing GPS metadata, adversaries can geolocate video frames by matching background visual cues to geo-tagged reference imagery. This creates a privacy threat that needs mitigation without excessively degrading visual quality.

Method: The framework uses a dynamic conditional random field (DCRF) to estimate location-sensitive background regions, rescales perturbation strength with normalized control penalty (NCP), and injects Gaussian noise only inside inferred regions via DP-style calibration rules.

Result: PPEDCRF reduces ResNet18 Top-1 retrieval accuracy from 0.667 to 0.361±0.127 at σ₀=8 while preserving 36.14 dB PSNR - about 6 dB quality advantage over global Gaussian noise. Most backbone-gallery combinations show negative transfer (23 of 24 cells).

Conclusion: PPEDCRF provides spatially concentrated perturbation that preserves higher visual quality at any given noise scale compared to global noise, though it doesn't offer stronger privacy at matched utility levels - the practical benefit is better quality preservation.
...
TL;DR: LookasideVLN is a new Aerial VLN approach that leverages directional cues in natural language instructions for more accurate spatial reasoning and computational efficiency, outperforming state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing Aerial VLN methods have limitations: they rely primarily on landmark descriptions while overlooking directional cues, which are crucial for spatial context in human navigation. Current approaches also suffer from shallow instruction understanding and high computational costs.

Method: Three core components: (1) Egocentric Lookaside Graph (ELG) that dynamically encodes instruction-relevant landmarks and their directional relationships, (2) Spatial Landmark Knowledge Base (SLKB) for lightweight memory retrieval from prior navigation experiences, and (3) Lookaside MLLM Navigation Agent that aligns multimodal information from instructions, visual observations, and landmark-direction data for path planning.

Result: LookasideVLN significantly outperforms the state-of-the-art CityNavAgent, even with a single-level lookahead, demonstrating that leveraging directional cues is a powerful yet efficient strategy for Aerial VLN.

Conclusion: Directional cues in natural language instructions are crucial for accurate spatial reasoning in Aerial VLN, and exploiting them leads to both improved performance and computational efficiency compared to existing methods.
...
TL;DR: DreamShot is a video diffusion-based storyboard synthesis framework that generates coherent multi-shot sequences with consistent characters and narrative flow, outperforming text-to-image based approaches.


<details>
  <summary>Details</summary>
Motivation: Existing storyboard synthesis approaches adapted from text-to-image diffusion models struggle with long-range temporal coherence, consistent character identities, and narrative flow across multiple shots. There's a need for better visual storytelling tools that maintain continuity.

Method: DreamShot leverages video diffusion priors for controllable multi-shot synthesis, supporting Text-to-Shot and Reference-to-Shot generation with story continuation. It includes a multi-reference role conditioning module that accepts multiple character reference images and enforces identity alignment via Role-Attention Consistency Loss.

Result: Extensive experiments show DreamShot achieves superior scene coherence, role consistency, and generation efficiency compared to state-of-the-art text-to-image storyboard models, establishing a new direction for video model-driven visual storytelling.

Conclusion: DreamShot represents a significant advancement in storyboard synthesis by fully exploiting video diffusion priors, enabling more coherent and controllable visual storytelling with better character consistency and narrative fidelity than previous approaches.
...
TL;DR: SciDraw-6K is a curated dataset of 6,291 scientific illustrations generated by Google Gemini models, paired with multilingual prompts across 11 languages, covering 8 scientific categories for specialized scientific visualization tasks.


<details>
  <summary>Details</summary>
Motivation: To address the lack of specialized datasets for scientific illustration in text-to-image research, as existing corpora are general-purpose and not tailored to scientific visualization needs like schematic diagrams, mechanism figures, and conceptual posters.

Method: Created using Google Gemini image-generation models (primarily gemini-2.5-flash-image and gemini-3-pro-image-preview families) to synthesize scientific illustrations, with a curation pipeline that spans 8 scientific categories and pairs images with prompts in 11 languages.

Result: A dataset of 6,291 scientific illustrations covering biomedical, chemistry, materials, electronics, environment, AI systems, physics, and other categories, with multilingual prompts, serving as the foundation for sci-draw.com public service.

Conclusion: SciDraw-6K fills a gap in specialized scientific visualization datasets, supporting multilingual text-to-image research, domain-adapted diffusion fine-tuning, and prompt-engineering studies for scientific applications.
...
TL;DR: CDSA-Net is a novel deep learning framework for coronary digital subtraction angiography that explicitly decouples vascular structure preservation and realistic background restoration to eliminate boundary artifacts while maintaining diagnostic fidelity.


<details>
  <summary>Details</summary>
Motivation: Current DSA methods suffer from physiological motion artifacts and anatomical noise in coronary imaging. Existing deep learning approaches produce clinically unacceptable flaws including persistent boundary artifacts and loss of native tissue grayscale fidelity, undermining diagnostic confidence.

Method: CDSA-Net introduces two core innovations: 1) Hierarchical geometric prior guidance (HGPG) mechanism in coronary structure extraction network (CSENet) combining integrated geometric prior with gated spatial modulation and centerline-aware topology loss for structural continuity. 2) Adaptive noise module (ANM) in coronary background restoration network (CBResNet) that models stochastic clinical X-ray noise to bridge domain gap and eliminate boundary artifacts. Final subtraction removes restored background from raw angiogram.

Result: Quantitatively outperformed state-of-the-art methods in vascular intensity correlation and perceptual quality. Achieved 25.6% improvement in morphology assessment efficiency and 42.9% gain in hemodynamic evaluation speed while maintaining diagnostic results consistent with raw angiograms.

Conclusion: CDSA-Net sets a new benchmark for utility in interventional cardiology by effectively addressing critical clinical flaws in coronary DSA, providing artifact-free subtraction images with preserved diagnostic fidelity and improved assessment efficiency.
...
TL;DR: DREAM is a novel framework for automated retinal medical report generation that uses two-stage fusion of visual data with clinical keywords, achieving state-of-the-art performance on limited medical data.


<details>
  <summary>Details</summary>
Motivation: Current LVLMs struggle in specialized medical fields with scarce data, leading to overfitting and missing subtle pathologies. There's a need for models that can generate high-fidelity medical reports from retinal images despite data limitations.

Method: DREAM uses a two-stage fusion mechanism: 1) Abstractor module maps image and keyword features into shared space, 2) Adaptor performs adaptive multi-modal fusion with learnable weighting parameters. A Contrastive Alignment module aligns fused representations with ground-truth reports during training.

Result: Achieves new state-of-the-art on DeepEyeNet benchmark with BLEU-4 score of 0.241, and demonstrates strong generalization to the ROCO dataset.

Conclusion: DREAM successfully combines medical expertise with efficient fusion strategy to overcome data scarcity challenges in specialized medical domains, enabling high-quality automated retinal report generation.
...
TL;DR: EmbodiedHead is a real-time talking-head framework that gives LLMs visual avatars for conversation, achieving high-quality generation with fast sampling and seamless turn-taking through single-stream audio processing.


<details>
  <summary>Details</summary>
Motivation: Current talking-head avatars struggle with real-time generation, unified listening-speaking behavior, and high visual quality simultaneously. Existing methods use dual-stream audio that creates look-ahead dependencies incompatible with causal LLM interactions.

Method: Uses Rectified-Flow Diffusion Transformer (DiT) with differentiable renderer for 4-step sampling. Implements single-stream audio with explicit listening-speaking state conditioning and Streaming Audio Scheduler. Employs two-stage training: coefficient-space pretraining followed by joint image-domain refinement.

Result: Achieves state-of-the-art visual quality and motion fidelity in both speaking and listening scenarios. Enables real-time generation with seamless turn-taking while suppressing spurious mouth motion during listening.

Conclusion: EmbodiedHead successfully addresses key challenges in embodied avatars by combining efficient diffusion models with proper audio-stream handling, making LLM conversations with visual avatars practical and high-quality.
...
TL;DR: The paper proposes an adversarial evaluation framework to quantify text shortcut learning in VLMs, showing that an optimized LoRA model reduces accuracy degradation from conflicting text by 64.4% while maintaining high normal accuracy.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models (VLMs) tend to over-rely on textual descriptions while under-utilizing visual evidence, a phenomenon called "text shortcut learning." The authors aim to quantify this cross-modal dependency and develop methods to reduce it.

Method: Proposed an adversarial evaluation framework using four strategies (shape_swap, color_swap, position_swap, random_text) on a geometric-shapes dataset. Compared three configurations: Baseline CLIP, LoRA fine-tuning, and LoRA Optimized (with Hard Negative Mining, Label Smoothing, layer-wise learning rates, Cosine Restarts, curriculum learning, and data augmentation).

Result: The optimized model reduced average accuracy degradation (Drop) from 27.5% to 9.8% (64.4% relative improvement, p<0.001) while maintaining 97% normal accuracy. Attention visualization and embedding-space analysis confirmed improved visual attention and tighter cross-modal alignment.

Conclusion: The proposed adversarial evaluation framework effectively quantifies text shortcut learning, and the optimized LoRA model significantly reduces cross-modal dependency while preserving overall performance, demonstrating improved visual feature utilization in VLMs.
...
TL;DR: Novel Region-Affinity Attention mechanism for breast cancer classification using DUV fluorescence imaging, achieving 92.67% accuracy without patch-based preprocessing.


<details>
  <summary>Details</summary>
Motivation: Traditional histopathological methods are slow for intra-operative use, and existing deep learning approaches fragment spatial context through patching. Standard attention mechanisms fail to exploit multi-scale regional relationships in DUV-WSI data.

Method: Region-Affinity Attention mechanism processes entire slides without patching, models local neighbor distances, constructs full affinity matrix to highlight diagnostically relevant regions, and uses contrastive loss for feature discriminability.

Result: Achieved 92.67 +/- 0.73% accuracy and 95.97% AUC on 136 DUV-WSI samples, outperforming existing attention methods like Spatial, Squeeze-and-Excitation, Global Context and Guided Context Gating.

Conclusion: The proposed Region-Affinity Attention mechanism effectively preserves spatial context in DUV-WSI breast cancer classification, offering superior diagnostic performance compared to conventional attention methods.
...
TL;DR: Autonomous vision pipeline for robotic HDD disassembly using FPP-based 3D sensing with depth completion and real-time instance segmentation, achieving high accuracy and speed.


<details>
  <summary>Details</summary>
Motivation: Unrecovered e-waste represents significant economic loss, and HDDs are valuable e-waste requiring robotic disassembly. Current methods are fragmented, lack robust 3D sensing, and lack fastener localization.

Method: Proposed pipeline uses Fringe Projection Profilometry (FPP) for 3D sensing with selective depth completion where FPP fails, integrated with lightweight real-time instance segmentation network. Same FPP camera-projector system ensures pixel-wise alignment without registration. Uses sim-to-real transfer learning and optimized networks for deployment.

Result: Achieved box mAP@50 of 0.960 and mask mAP@50 of 0.957 for instance segmentation. Depth completion achieved RMSE of 2.317 mm and MAE of 1.836 mm. Combined latency of 12.86 ms and throughput of 77.7 FPS on evaluation workstation.

Conclusion: The perception pipeline provides high-fidelity semantic and spatial data valuable for downstream robotic disassembly. Synthetic dataset for HDD instance segmentation will be publicly available.
...
TL;DR: P-MLLM: A profile-aware multimodal LLM for zero-shot personalized image aesthetics assessment using user profiles as contextual signals instead of historical ratings.


<details>
  <summary>Details</summary>
Motivation: Existing PIAA methods rely on historical user ratings, making them ineffective when such data is unavailable (zero-shot setting). Need to model user-specific aesthetic preferences without requiring prior rating data.

Method: Introduces P-MLLM, a profile-aware multimodal LLM that augments a frozen LLM with selective fusion modules. These modules selectively integrate visual information into the model's hidden states during profile-conditioned reasoning, enabling profile-aware visual integration.

Result: Achieves competitive zero-shot performance on recent PIAA benchmarks. Remains effective even with coarse profile information, demonstrating robustness.

Conclusion: Profile-based personalization shows strong potential for zero-shot PIAA, providing an effective alternative to rating-dependent methods when historical user data is unavailable.
...
TL;DR: RemoteShield is a robust MLLM for Earth Observation that maintains consistent performance under realistic multimodal perturbations like clouds, fog, and textual variations through preference learning over clean and perturbed input clusters.


<details>
  <summary>Details</summary>
Motivation: Current Remote Sensing MLLMs trained on clean datasets fail to generalize to noisy operational conditions, causing performance degradation with imperfect inputs like cloud cover and colloquial text instructions.

Method: RemoteShield uses preference learning over semantic equivalence clusters where clean samples are paired with perturbed variants. The model is optimized to favor stable responses by comparing outputs to clean and corrupted inputs within the same cluster.

Result: RemoteShield consistently outperforms baseline models on three Earth Observation tasks, showing stronger robustness and cross-condition consistency under realistic multimodal perturbations.

Conclusion: The proposed preference learning approach enables RemoteShield to maintain consistent interpretation despite visual degradations and textual noise, addressing a critical limitation in current Remote Sensing MLLMs.
...
TL;DR: This paper proposes a new deepfake detection method using fractal analysis of low-correlation signals to distinguish AI-generated images from real photographs.


<details>
  <summary>Details</summary>
Motivation: AI-generated imagery poses significant threats to information security and societal trust, and existing deepfake detection methods lack robustness in open-world scenarios.

Method: The method analyzes intrinsic signal-level discrepancies between synthetic and authentic images, specifically focusing on low-correlation signals. It quantifies these signals using fractal theory to capture statistical anomalies from the synthesis process.

Result: Extensive experiments demonstrate the method's robustness and superior detection performance compared to existing approaches.

Conclusion: The work emphasizes shifting research focus to signal-level analysis for deepfake detection, and the approach can be applied to all AI-generated image detection tasks, not just face images.
...
TL;DR: MLLMs show instinct-reflection misalignment between token-level support and verbal confidence. Proposed monotone confidence fusion with cross-channel consistency improves calibration and failure prediction.


<details>
  <summary>Details</summary>
Motivation: MLLMs need reliable confidence estimation for practical deployment, but prior work focuses on text-only LLMs using expensive self-consistency sampling. There's a gap in multimodal confidence estimation with discovered instinct-reflection misalignment.

Method: Proposes monotone confidence fusion framework to merge dual-channel signals (instinct token-level support and reflection verbal confidence) using cross-channel consistency to estimate correctness. Includes order-preserving mean alignment to correct global bias while preserving risk-coverage trade-off.

Result: Experiments on diverse open-source and closed-source MLLMs show the method consistently yields more reliable confidence estimates and improves both calibration and failure prediction.

Conclusion: The proposed framework effectively addresses instinct-reflection misalignment in MLLMs, providing better confidence estimation crucial for reliable deployment in multimodal settings.
...
TL;DR: PestVL-Net: A vision-language framework using RWKV architecture and MLLM priors for fine-grained pest recognition, addressing data scarcity and complex pest morphology challenges in agriculture.


<details>
  <summary>Details</summary>
Motivation: Real-world pest data collection is difficult, pests have diverse species with complex morphology, and existing methods fail to effectively model fine-grained visual and semantic features, limiting practical agricultural applications.

Method: PestVL-Net integrates visual and linguistic pathways: visual uses RWKV architecture with saliency-guided adaptive window partitioning; linguistic generates precise pest descriptions using MLLM priors informed by expert knowledge and multimodal Chain-of-Thought reasoning.

Result: Extensive experiments on multiple pest datasets validate PestVL-Net's superior performance, demonstrating its potential for effective real-world pest management applications.

Conclusion: The synergistic vision-language approach enables fine-grained multimodal pest learning, addressing critical challenges in agricultural pest recognition and management.
...
TL;DR: DepthVAR is a training-free framework that adaptively allocates per-token computational depth in Visual Autoregressive models instead of pruning entire tokens, achieving 2.3-3.1× acceleration with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Current Visual Autoregressive (VAR) models inefficiently apply fixed computational depth to all positions when generating high-resolution images. Existing acceleration methods use binary hard-pruning based on frequency maps, which is fundamentally limited and doesn't improve quality even with better frequency estimation.

Method: DepthVAR introduces adaptive depth allocation with: 1) Adaptive depth scheduler that assigns computational depth via cyclic rotated schedule for balanced, non-static refinement; 2) Dynamic inference process that translates depths into layer-major masks, selectively applies transformer blocks, and blends resulting codes to ensure each token's influence is proportional to its processing depth.

Result: Extensive experiments show DepthVAR achieves 2.3×-3.1× acceleration with minimal quality loss, offering a competitive compute-performance trade-off compared to existing hard-pruning approaches.

Conclusion: DepthVAR represents a paradigm shift from pruning entire tokens to adaptively allocating per-token computational depth, providing an effective training-free acceleration framework for Visual Autoregressive models that maintains quality while significantly improving efficiency.
...
TL;DR: GraphSpecForge is a training-free framework that detects copy-move forgery by analyzing spectral structure of attention graphs from Stable Diffusion U-Net, using Wasserstein distances between Laplacian spectra.


<details>
  <summary>Details</summary>
Motivation: Copy-move forgery remains a persistent threat to visual media integrity, where regions within images are duplicated to hide or fabricate content, creating a need for effective detection methods.

Method: Analyzes spectral structure of attention graphs from pretrained Stable Diffusion U-Net. Copy-move manipulation induces approximate subgraph duplication in self-attention graphs, leading to measurable spectral redistribution in normalized graph Laplacian. Uses Wasserstein distances between per-image Laplacian spectra and authentic reference distribution for detection.

Result: Achieves AUROC = 0.606 on RecodAI-LUC (5,128 images), with normalized Laplacian outperforming raw attention spectra by +0.057 AUROC. On MICC-F220, CoMoFoD, and COVERAGE, attains AUROCs of 0.752, 0.774, and 0.673 respectively. On CoMoFoD also reaches AUPRC = 0.833, balanced accuracy = 0.712, MCC = 0.499, and TPR@1%FPR = 32.5%.

Conclusion: GraphSpecForge provides an effective training-free approach for copy-move forgery detection by leveraging spectral analysis of attention graphs, with ablation and falsification experiments confirming signal specificity and sensitivity to manipulation strength.
...
TL;DR: FReMuRe model improves video scene graph generation for long-tail relationships using frequency-guided multi-level reasoning, relation-specific branches, and novel classification heads.


<details>
  <summary>Details</summary>
Motivation: Existing video scene graph generation methods struggle with long-tail distributions of relationships, limiting their ability to handle rare relationships effectively.

Method: Proposes Frequency-guided Relational Multi-level Reasoning (FReMuRe) with relation-specific branches to handle gradient conflicts, frequency-aware dual-branch predicate embedding network for separate modeling of high/low-frequency relationships, and interchangeable relation classification heads (Bayesian Head and Gaussian Mixture Model Head).

Result: Significantly improves recall rate of long-tail relationships and overall reasoning robustness on Action Genome dataset.

Conclusion: FReMuRe effectively addresses long-tail relationship challenges in video scene graph generation through frequency-guided mechanisms and specialized modeling approaches.
...
TL;DR: Review of NTIRE 2026 mobile real-world image super-resolution challenge focusing on efficient models for 4x upscaling on mobile devices, with 108 registrants and 16 final teams.


<details>
  <summary>Details</summary>
Motivation: To advance mobile real-world image super-resolution by developing efficient models that can handle unknown degradations while maintaining mobile device compatibility, and to provide an overview of current trends in the field.

Method: Organized a challenge competition where participants developed network designs/solutions for 4x super-resolution from low-resolution images with unknown degradations, evaluated using weighted combination of image quality assessment scores and speedup ratios.

Result: Attracted 108 registrants with 16 teams achieving valid scores in final ranking, demonstrating collaborative advancement in mobile real-world image super-resolution performance.

Conclusion: The challenge successfully advanced mobile real-world image super-resolution technology while providing insights into current trends, with numerous teams contributing to improved performance on mobile devices.
...
TL;DR: SePL uses CLIP's text modality with separable prompt learning to detect face forgeries, achieving strong generalization across datasets and methods.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based deepfake detection methods focus primarily on the visual encoder while neglecting the text modality, despite its potential to provide instructive guidance for detection.

Method: Proposes Separable Prompt Learning (SePL) that disentangles forgery-specific and forgery-irrelevant information via two types of prompt learning, using cross-modality alignment and dedicated objectives.

Result: Achieves competitive and superior performance in cross-dataset and cross-method evaluations, demonstrating strong generalization capabilities.

Conclusion: Leveraging CLIP's text modality through separable prompt learning enables effective face forgery detection with excellent generalization, outperforming existing approaches.
...
TL;DR: MedFocusLeak: A transferable black-box multimodal attack on clinical VLMs that generates clinically plausible but incorrect diagnoses with imperceptible perturbations by targeting non-diagnostic background regions and using attention distraction.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models are increasingly used in clinical diagnostics but their robustness to adversarial attacks is unexplored, posing serious risks. Existing medical attacks focus on secondary objectives or create visible distortions that clinicians can detect, leaving a gap for subtle, clinically plausible attacks.

Method: MedFocusLeak injects coordinated perturbations into non-diagnostic background regions and employs an attention distraction mechanism to shift the model's focus away from pathological areas. It's a highly transferable black-box multimodal attack designed to keep perturbations imperceptible.

Result: Extensive evaluations across six medical imaging modalities show state-of-the-art performance, generating misleading yet realistic diagnostic outputs across diverse VLMs. The attack reveals a critical weakness in the reasoning capabilities of modern clinical VLMs.

Conclusion: The paper introduces a unified evaluation framework with novel metrics that jointly capture attack success and image fidelity, demonstrating the vulnerability of clinical VLMs to subtle adversarial attacks that produce clinically plausible but incorrect diagnoses.
...
TL;DR: E2E-GMNER is an end-to-end generative framework for Grounded Multimodal Named Entity Recognition that unifies entity recognition, typing, visual grounding, and knowledge reasoning in a single multimodal LLM, using instruction tuning and chain-of-thought reasoning with Gaussian Risk-Aware Box Perturbation for robust bounding box prediction.


<details>
  <summary>Details</summary>
Motivation: Existing GMNER approaches use pipeline architectures that decouple textual entity recognition and visual grounding, leading to error accumulation and suboptimal joint optimization. There's a need for unified end-to-end frameworks that can jointly optimize all components and handle noisy visual cues.

Method: Proposes E2E-GMNER: 1) Formulates GMNER as instruction-tuned conditional generation task with chain-of-thought reasoning to adaptively use visual evidence or background knowledge; 2) Introduces Gaussian Risk-Aware Box Perturbation (GRBP) that replaces hard box supervision with probabilistically perturbed soft targets to improve robustness against annotation noise and discretization errors.

Result: Extensive experiments on Twitter-GMNER and Twitter-FMNERG benchmarks show E2E-GMNER achieves highly competitive performance compared to state-of-the-art methods, validating effectiveness of unified end-to-end optimization and noise-aware grounding supervision.

Conclusion: The proposed end-to-end generative framework successfully unifies multiple GMNER tasks, with chain-of-thought reasoning enabling adaptive use of multimodal information and GRBP addressing bounding box prediction instability, demonstrating the advantages of unified optimization over pipeline approaches.
...
TL;DR: QUOTA framework combines low-bit quantization and visual-token pruning for efficient VLM inference, achieving 95.65% performance retention with only 30% visual tokens.


<details>
  <summary>Details</summary>
Motivation: Vision-Language Models suffer from high inference costs due to long visual-token prefixes during prefill and growing KV cache during decoding. Existing stage-wise combinations of token pruning and quantization are brittle due to mismatch between calibration and execution.

Method: Proposes QUOTA (Quantization Unified Offline Token Allocator) - a collaborative framework that unifies low-bit inference and deterministic visual-token pruning. Uses quantization calibration signals to create layer-wise token allocation schedules, evaluating token importance under deployed W4A4 operators with quantized KV cache using activation magnitude, attention cues, and low-bit risk signals.

Result: Achieves 95.65% average performance retention on standard VLM benchmarks while retaining only 30% of visual tokens, outperforming stage-wise baselines (94.3% retention). Shows improved robustness under same low-bit regime.

Conclusion: QUOTA provides an effective unified framework for efficient VLM deployment through collaborative quantization-and-pruning, addressing the brittleness of stage-wise approaches and enabling aggressive low-bit inference with minimal performance degradation.
...
TL;DR: A novel single-image face morphing attack detection framework that combines Laplacian residual statistics with frozen vision transformer features using gated adapters and fusion techniques, achieving state-of-the-art performance with real-time efficiency.


<details>
  <summary>Details</summary>
Motivation: Face morphing attacks threaten face recognition systems in critical applications like passport issuance and border control. Single-image detection is challenging due to lack of reference images and diverse attack generation methods.

Method: Integrates high-frequency Laplacian residual statistics with frozen foundation-scale vision transformer representations. Uses residual-statistic-gated low-rank adapters (R-FLoRA) and feature-wise residual fusion (Res-FiLM) to detect local morphing artifacts while preserving semantic context. Includes residual-contrastive alignment loss for regularization.

Result: Outperforms nine recent state-of-the-art S-MAD algorithms across four ICAO-compliant datasets with seven morph generation techniques. Achieves superior detection accuracy and cross-domain generalization with frozen backbone and minimal trainable parameters.

Conclusion: The proposed framework provides real-time, interpretable, and efficient morphing attack detection suitable for real-world biometric verification systems, addressing critical security vulnerabilities in face recognition applications.
...
TL;DR: A dual-resolution deep learning framework with attention-based feature fusion and ordinal regression improves cross-dataset generalization for diabetic retinopathy grading.


<details>
  <summary>Details</summary>
Motivation: Deep learning models for diabetic retinopathy grading often degrade when deployed across datasets with different imaging conditions, limiting their practical utility in large-scale screening programs.

Method: Uses two parallel EfficientNet backbones at different resolutions, learnable attention mechanism for feature fusion, ordinal regression (CORAL) for severity ordering, and preprocessing (circular cropping, contrast enhancement, histogram matching) for domain adaptation.

Result: Achieved QWK of 0.88 on APTOS validation set and 0.68 on unseen Messidor-2 dataset, demonstrating improved cross-dataset robustness.

Conclusion: The proposed dual-resolution framework with attention fusion and ordinal regression enhances generalization across datasets, making it more suitable for real-world DR screening applications.
...
TL;DR: VLMs hallucinate when on-screen text contradicts visuals (TOIH). VisualTextTrap benchmark quantifies this with 6K samples. VTHM-MoE framework uses dual-encoder + MoE experts to mitigate hallucination.


<details>
  <summary>Details</summary>
Motivation: Existing VLMs systematically hallucinate when on-screen text contradicts visual scenes, prioritizing overlay text over actual visual content. This Text Overlay-Induced Hallucination (TOIH) is a critical but overlooked issue in multimodal video understanding.

Method: 1) Created VisualTextTrap benchmark with 6,057 human-validated samples across 88 attributes in 4 dimensions (Temporal, Action, Object, Spatial), quantified on 5-level hallucination scale. 2) Proposed VTHM-MoE framework with dual-encoder architecture and 4 dimension-specialized expert modules. 3) Developed Adaptive Token Routing Strategy for dynamic expert allocation.

Result: VTHM-MoE outperforms state-of-the-art counterparts on diverse video QA tasks, demonstrating robust resistance to TOIH while preserving performance on uncontaminated videos. The benchmark provides comprehensive evaluation of hallucination.

Conclusion: TOIH is a significant problem in VLMs that requires specialized mitigation. The proposed VisualTextTrap benchmark and VTHM-MoE framework effectively address this issue through disentangled vision-text processing and adaptive expert routing.
...
TL;DR: The paper presents a deepfake detection method using an ensemble of fine-tuned vision transformers (DINOv2, AIMv2, OpenCLIP ViT-L/14) that achieves state-of-the-art performance on the DF-Wild dataset, winning the IEEE SP Cup 2025 competition.


<details>
  <summary>Details</summary>
Motivation: The rapid evolution of generative models and poor generalization of existing deepfake detection methods create challenges in detecting manipulated images, necessitating more robust and generalizable approaches.

Method: Uses an ensemble of fine-tuned vision transformers including DINOv2, AIMv2, and OpenCLIP's ViT-L/14, trained on the DF-Wild dataset which contains diverse manipulation techniques. Started with CNN classifiers on spatial features as baseline.

Result: The ensemble method achieves 96.77% AUC and 9% EER on DF-Wild test set, outperforming individual models and CNN baselines, and beating the state-of-the-art Effort algorithm by 7.05% in AUC and 8% in EER.

Conclusion: The vision transformer ensemble approach provides a highly effective and generalizable solution for deepfake detection, demonstrated by winning the IEEE SP Cup 2025 competition and outperforming existing state-of-the-art methods.
...
TL;DR: SpatialImaginer is a multimodal framework that combines textual reasoning with visual imagination to improve spatial intelligence in MLLMs, achieving SOTA performance on spatial reasoning benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs have fragile reasoning in spatial intelligence tasks due to a mismatch between spatial recognition needs and text-only reasoning. Textual representations abstract away critical geometric details needed for consistent spatial state tracking.

Method: SpatialImaginer uses a divide-and-conquer strategy: text chain-of-thought for high-level semantic planning, and visual imagination for geometry-sensitive state transformation and consistency preservation. Includes difficulty-aware data engine with closed-loop verification to train selective visual imagination invocation.

Result: Achieves state-of-the-art performance on diverse spatial intelligence benchmarks and substantially improves robustness on complex multi-step spatial reasoning tasks.

Conclusion: Integrating textual reasoning with visual imagination addresses the fundamental mismatch in spatial intelligence tasks, enabling more robust and consistent spatial reasoning in MLLMs.
...
TL;DR: A deep learning framework for non-rigid volume-to-surface registration that estimates dense brain shift from sparse intra-operative surface observations without requiring explicit point correspondences or volumetric intra-operative imaging.


<details>
  <summary>Details</summary>
Motivation: Brain shift during neurosurgery causes significant soft-tissue deformation that compromises navigation accuracy. Existing compensation methods using intra-operative MRI/CT/ultrasound are disruptive to surgical workflow, while partial cortical surface observations from microscopes or laser scanners are limited and noisy, making deformation estimation challenging.

Method: A deep learning-based framework for non-rigid volume-to-surface registration using multi-scale point-based feature extraction and hierarchical deformation decoder. The approach integrates partial intra-operative surface information into full pre-operative point cloud domain to enable implicit correspondence learning and dense deformation recovery under limited visibility.

Result: Achieved accurate recovery of fine-scale deformations with Endpoint Error (EPE) of 1.13 ± 0.75 mm and RMSE of 1.33 ± 0.81 mm under challenging partial-surface conditions.

Conclusion: The proposed approach enables automatic, workflow-compatible brain-shift compensation from sparse surface observations without disrupting surgery, addressing a major limitation in image-guided neurosurgery.
...
TL;DR: MESA is an image restoration method for damaged inscriptions that uses well-preserved exemplar inscriptions to guide reconstruction, employing VGG19 features, Gram matrices, and OCR-based weighting to preserve style and structure.


<details>
  <summary>Details</summary>
Motivation: Ancient inscriptions often have missing or corrupted regions due to fragmentation and erosion, making them difficult to read and analyze. Existing restoration methods have limitations in handling inscription-specific challenges like style preservation and structural accuracy.

Method: MESA uses VGG19 convolutional features encoded as Gram matrices to capture exemplar texture, style, and stroke structure. It selects exemplars minimizing Mean-Squared Displacement (MSD) per layer, uses OCR-estimated character widths to weight layer contributions, and employs a training mask to restrict synthesis to damaged areas only.

Result: Comparative experiments demonstrate MESA's advantages over prior methods, showing improved restoration quality for damaged inscriptions while preserving original style and structure.

Conclusion: MESA effectively addresses limitations of previous restoration approaches and provides a practical framework for choosing appropriate restoration strategies based on available exemplars and metadata.
...
TL;DR: SDVG adapts speculative decoding to autoregressive video diffusion by using an image-quality router instead of token verification, achieving 1.59-2.09x speedup while maintaining 95.7-98.1% of target model quality.


<details>
  <summary>Details</summary>
Motivation: Speculative decoding has been highly effective for accelerating large language models, but its adaptation to autoregressive video generation is challenging because video blocks are continuous spatiotemporal tensors without token-level distributions for exact rejection sampling.

Method: SDVG uses a 1.3B drafter model to propose candidate video blocks via four denoising steps. Each block is VAE-decoded and scored by ImageReward using worst-frame aggregation (minimum per-frame reward). Blocks scoring above threshold τ are accepted into the 14B target's KV cache; rejected blocks are regenerated by the target. Key design: first block always force-rejected to anchor scene composition.

Result: On 1003 MovieGenVideoBench prompts (832x480), SDVG retains 98.1% of target-only VisionReward quality (0.0773 vs. 0.0788) at 1.59x speedup with τ=-0.7, and reaches 2.09x speedup at 95.7% quality retention. Consistently outperforms draft-only generation by over +17%.

Conclusion: SDVG successfully adapts speculative decoding to autoregressive video diffusion, providing a training-free framework with no architectural changes that can be seamlessly integrated into existing pipelines, offering a smooth quality-speed Pareto frontier controlled by a single threshold parameter.
...
TL;DR: Q-Gate is a training-free framework that dynamically routes keyframe selection through three expert streams based on query intent, outperforming SOTA methods on long video understanding benchmarks.


<details>
  <summary>Details</summary>
Motivation: Current keyframe selection methods for long video understanding in MLLMs use either visual-only metrics (ineffective for narrative queries) or static fusion of heuristic scores (introduces modal noise for visual tasks). This one-size-fits-all approach fails to adapt to different query types.

Method: Q-Gate decouples retrieval into three lightweight expert streams: Visual Grounding (local details), Global Matching (scene semantics), and Contextual Alignment (subtitle-driven narratives). It uses a Query-Modulated Gating Mechanism where an LLM assesses query intent to dynamically allocate attention weights across experts, activating relevant modalities while muting irrelevant ones.

Result: Extensive experiments on LongVideoBench and Video-MME across multiple MLLM backbones show Q-Gate substantially outperforms state-of-the-art baselines by effectively suppressing modality-specific noise.

Conclusion: Q-Gate provides a robust, highly interpretable solution for scalable video reasoning by treating keyframe selection as a dynamic modality routing problem, maximizing signal-to-noise ratio through intelligent modality activation.
...
TL;DR: The paper proposes a new evaluation framework for long videos, arguing that existing metrics designed for short videos fail to capture long-range characteristics like narrative consistency. It introduces a shot-based metric and a specialized benchmark dataset (Long-CODE) for long-video evaluation.


<details>
  <summary>Details</summary>
Motivation: As video generation models advance to produce longer videos, traditional metrics designed for short videos are inadequate. They focus on frame-level visual quality and short-term smoothness but fail to assess essential long-range attributes like narrative richness and global causal consistency.

Method: 1) Introduced long-video attribute corruption tests to expose limitations of existing metrics; 2) Designed a novel long-video metric based on shot dynamics sensitive to long-range characteristics; 3) Created Long-CODE dataset with human annotations specifically for genuine long-range video attributes.

Result: Extensive experiments show the proposed metrics achieve state-of-the-art correlation with human judgments. The framework successfully identifies structural inconsistencies (shot-level perturbations, narrative shuffling) that traditional metrics miss.

Conclusion: The paper establishes a holistic evaluation paradigm where long-video metrics complement existing short-video standards. This disentangled approach provides unbiased assessment of video generation models across both short-term visual perception and long-context attributes.
...
TL;DR: A Shape from Shading framework enhances lunar DEMs using Chandrayaan-2 OHRC imagery, providing new topographic data beyond stereo limitations, with measurable improvements in fine-scale crater morphology.


<details>
  <summary>Details</summary>
Motivation: To improve sub-metre resolution lunar digital elevation models (DEMs) by overcoming stereo baseline limitations and extracting additional topographic information from single images using Shape from Shading techniques.

Method: Shape from Shading framework applied to independent OHRC images from Chandrayaan-2, with systematic three-stage parameter sweep on SfS smoothness weight across three lunar sites (Cyrillus crater, Vikram landing region, lunar south pole).

Result: Measurable topographic enhancement, particularly in surface slope statistics, revealing fine-scale crater morphology previously unresolved. Identified limitations include reduced SfS sensitivity with large pitch angle separation and variable enhancement quality with partial footprint coverage.

Conclusion: SfS can effectively enhance lunar DEMs as both a refinement tool and source of new topographic data, though performance depends on imaging geometry and coverage conditions.
...
TL;DR: This paper presents a comprehensive review and taxonomy of non-Transformer computer vision methods, categorizing them into convolution-based, MLP-based, state-space-based models and analyzing their efficiency, scalability, interpretability, and robustness.


<details>
  <summary>Details</summary>
Motivation: Despite recent advancements in Transformer-based models dominating computer vision research, many non-Transformer methods remain competitive and deserve systematic examination to understand their strengths, limitations, and future potential.

Method: The authors conduct a systematic review of 40 selected papers, organizing non-Transformer methods into categories including convolution-based models, MLP-based models, and state-space-based approaches. They analyze these methods across multiple dimensions: computational efficiency, scalability, interpretability, and robustness.

Result: The review provides a comprehensive taxonomy and comparative analysis of non-Transformer computer vision methods, highlighting their competitive performance against Transformer-based approaches and identifying key characteristics across different architectural paradigms.

Conclusion: Non-Transformer methods remain viable alternatives to Transformer-based models in computer vision, offering different trade-offs in efficiency, scalability, interpretability, and robustness. The review identifies challenges and opportunities for future research in developing more efficient and interpretable vision architectures beyond the Transformer paradigm.
...
TL;DR: HyKey, a hyperspectral keypoint detection model, outperforms RGB methods in surgical 3D reconstruction by leveraging spectral-spatial features to overcome texture-poor environments.


<details>
  <summary>Details</summary>
Motivation: Traditional RGB-based keypoint detection struggles in minimally invasive surgery due to poor texture and complex illumination. The authors investigate whether hyperspectral imaging can provide better keypoint detection and matching for surgical 3D reconstruction.

Method: Developed HyKey, a hybrid 3D-2D CNN that extracts spatial-spectral features from hyperspectral images. Trained using synthetic homographic augmentation and epipolar geometry constraints on a dual-camera RGB-HSI laparoscopic dataset of ex-vivo organs with calibrated poses.

Result: HSI-based model outperformed RGB baselines (SuperPoint, ALIKE) on registered RGB frames, achieving 96.62% mean matching accuracy and 67.18% mean average accuracy at 10 degrees on pose estimation, with consistent improvements across multiple metrics.

Conclusion: Integrating spectral information from hyperspectral imaging offers a promising approach for robust monocular 3D reconstruction in minimally invasive surgery, addressing limitations of texture-poor surgical environments through enhanced spectral-spatial feature discrimination.
...
TL;DR: SegTTA improves medical image segmentation without retraining by combining four augmentations with weighted voting across MedSAM2 checkpoints, achieving better performance across diverse datasets.


<details>
  <summary>Details</summary>
Motivation: Medical image quality varies due to equipment and operator differences, creating challenges for segmentation models. While data augmentation helps, there's a need for methods that improve segmentation without model retraining to handle these quality variations.

Method: SegTTA combines four augmentations (Gamma correction, Contrast enhancement, Gaussian blur, Gaussian noise) with weighted voting across multiple MedSAM2 checkpoints. The framework uses test-time augmentation without requiring model retraining.

Result: Consistent improvements across three datasets: healthy uterus segmentation, uterine myoma detection, and multi-class hepatic structure segmentation. On hepatic vessel dataset, achieved +1.6 mIoU, +1.9 aIoU, and -2.0 HD95 reduction compared to MedSAM2 baselines.

Conclusion: SegTTA effectively improves medical image segmentation by adapting to image quality variations through test-time augmentation and weighted voting. Different augmentations benefit different organ sizes, and voting threshold allows task-specific optimization for clinical requirements.
...
TL;DR: HSG learns scene graph embeddings in hyperbolic space instead of Euclidean space to better capture hierarchical relationships, improving structural consistency while maintaining retrieval performance.


<details>
  <summary>Details</summary>
Motivation: Existing scene graph methods in Euclidean space fail to explicitly capture hierarchical entailment relationships between places and objects, limiting structural consistency of learned representations.

Method: Proposes Hyperbolic Scene Graph (HSG) which learns scene graph embeddings in hyperbolic space where hierarchical relationships are naturally encoded through geometric distance.

Result: HSG improves hierarchical structure quality while maintaining strong retrieval performance, achieving PP IoU of 33.17 and Graph IoU of 33.51, outperforming best AoMSG variant by 8.14.

Conclusion: Hyperbolic representation learning is effective for scene graph modeling, better capturing hierarchical relationships through geometric properties of hyperbolic space.
...
TL;DR: APEX is an adaptive prompting framework that retrieves input-specific prompts from a learnable memory using Fourier-based domain features, improving generalization in medical segmentation tasks.


<details>
  <summary>Details</summary>
Motivation: Existing visual prompting methods use a single prompt per domain applied uniformly to all inputs, which limits generalization under intra- and inter-domain variability, especially critical in medical applications.

Method: APEX retrieves input-specific prompts from a learnable prompt memory using domain features extracted from Fourier spectrum. It uses Low-Frequency Feature Contrastive (LFC) learning to cluster same-domain representations and separate different-domain ones.

Result: Extensive experiments on two medical segmentation tasks show APEX significantly improves generalization across both seen and unseen domains, works as a plug-and-play solution with any backbone, and consistently enhances performance.

Conclusion: APEX provides an effective adaptive prompting framework for medical vision tasks that addresses domain variability limitations of existing methods through input-specific prompt retrieval and robust domain feature learning.
...
TL;DR: UniMesh is a unified framework that jointly learns 3D generation and understanding in a single architecture, featuring a Mesh Head interface, Chain of Mesh for iterative editing, and self-reflection mechanisms for error correction.


<details>
  <summary>Details</summary>
Motivation: Current 3D vision approaches are fragmented with specialized models for either understanding (classification, segmentation) or generation (synthesis, completion). This isolation hinders knowledge transfer and holistic scene modeling, creating a need for unified architectures.

Method: 1) Mesh Head: Cross-model interface bridging diffusion-based image generation with implicit shape decoders. 2) Chain of Mesh (CoM): Geometric instantiation of iterative reasoning enabling user-driven semantic mesh editing through closed-loop latent, prompting, and regeneration cycles. 3) Self-reflection mechanism: Actor-Evaluator-Self-reflection triad to diagnose and correct failures in high-level tasks like 3D captioning.

Result: UniMesh achieves competitive performance on standard benchmarks while unlocking novel capabilities in iterative editing and demonstrating mutual enhancement between generation and understanding tasks.

Conclusion: The proposed unified framework successfully bridges the gap between 3D generation and understanding, enabling holistic scene modeling and knowledge transfer through innovative architectural components and iterative reasoning mechanisms.
...
TL;DR: A Dual-Anchoring Framework addresses state drift in Vision-Language Navigation by anchoring instruction progress and history representations, improving performance on long-horizon tasks.


<details>
  <summary>Details</summary>
Motivation: Video-LLMs for VLN suffer from State Drift in long scenarios, causing agents to wander aimlessly and fail essential maneuvers due to two cognitive deficits: Progress Drift (confusion about completed vs remaining sub-goals) and Memory Drift (degradation of history representations causing loss of landmark tracking).

Method: Proposes a Dual-Anchoring Framework with two components: 1) Instruction Progress Anchoring - supervises agent to generate structured text tokens distinguishing completed vs remaining sub-goals; 2) Memory Landmark Anchoring - uses Landmark-Centric World Model to retrospectively predict object-centric embeddings from Segment Anything Model, forcing explicit verification of past observations and preservation of landmark representations. Created two datasets: 3.6M samples with progress descriptions and 937k grounded landmark data.

Result: Extensive experiments in simulation and real-world show superiority with 15.2% improvement in Success Rate and remarkable 24.7% gain on long-horizon trajectories.

Conclusion: The Dual-Anchoring Framework effectively mitigates state drift in VLN by explicitly anchoring progress and memory representations, significantly improving performance especially on challenging long-horizon navigation tasks. Code, data pipelines, and datasets will be released to facilitate further research.
...
TL;DR: Proposes a triple-branch network with feature decoupling and fusion losses for robust deepfake detection by jointly learning spatial and frequency features while reducing redundancy.


<details>
  <summary>Details</summary>
Motivation: Deepfake technology poses serious threats like identity theft and misinformation. Existing detection methods overfit to specific frequency domains and suffer from feature redundancy, limiting robustness against diverse forgery patterns.

Method: Triple-branch network that jointly captures spatial and frequency features from original images and frequency-reconstructed images. Uses mathematically derived feature decoupling and fusion losses based on mutual information theory to focus on task-relevant features.

Result: Extensive experiments on six large-scale benchmark datasets demonstrate state-of-the-art performance consistently across different evaluation scenarios.

Conclusion: The proposed approach effectively addresses limitations of existing deepfake detection methods by capturing complementary information across spatial and frequency domains while reducing feature redundancy, leading to improved robustness and generalization.
...
TL;DR: AutoVQA-G is a self-improving agentic framework that automatically generates high-quality VQA-G datasets with better visual grounding accuracy than existing methods, addressing model hallucinations and brittle verification issues.


<details>
  <summary>Details</summary>
Motivation: Manual annotation of VQA-G datasets is unscalable, while existing automated methods suffer from inconsistent data fidelity due to model hallucinations and brittle verification mechanisms based on simple heuristics.

Method: AutoVQA-G uses an iterative refinement loop with a Consistency Evaluation module employing Chain-of-Thought reasoning for fine-grained visual verification, and a memory-augmented Prompt Optimization agent that analyzes critiques from failed samples to progressively refine generation prompts.

Result: AutoVQA-G generates VQA-G datasets with superior visual grounding accuracy compared to leading multimodal LLMs, demonstrating better performance than existing methods.

Conclusion: AutoVQA-G offers a promising approach for creating high-fidelity VQA-G data to facilitate more robust vision-language model training and evaluation, addressing scalability and quality issues in dataset generation.
...
TL;DR: CoReDi introduces adaptive semantic representations that evolve during diffusion training, improving image synthesis by learning a lightweight projection jointly with the diffusion model.


<details>
  <summary>Details</summary>
Motivation: Existing joint image-feature generative models use fixed representation spaces constructed independently of the generative objective, which limits their effectiveness. The authors argue that representation spaces should adapt to the generative task rather than remain static.

Method: Proposes Coevolving Representation Diffusion (CoReDi) where semantic representation space evolves during training by learning a lightweight linear projection jointly with the diffusion model. Uses stop-gradient targets, normalization, and targeted regularization to prevent feature collapse and enable stable coevolution.

Result: CoReDi achieves faster convergence and higher sample quality compared to joint diffusion models using fixed representation spaces. Works effectively with both VAE latent diffusion and pixel-space diffusion.

Conclusion: Adaptive semantic representations that co-evolve with diffusion training improve generative modeling by allowing the representation space to specialize for image synthesis needs, enhancing complementarity with image latents.
...
TL;DR: The paper addresses "edit spillover" in scene text editing where editing target text inadvertently modifies neighboring non-target text. They propose Edit Fidelity Field (EFF) to control per-pixel editing fidelity and reduce spillover from 94% to 25%.


<details>
  <summary>Details</summary>
Motivation: Current diffusion-based scene text editing methods suffer from "edit spillover" - when editing target text, they inadvertently modify non-target regions, especially neighboring text. Systematic evaluation shows 94% spillover rate, meaning nearly all non-target text gets altered during editing.

Method: Proposes Edit Fidelity Field (EFF), a semantics-aware continuous field that controls per-pixel editing fidelity. Unlike binary masks, EFF uses OCR-detected text regions to create four zones: Edit Core (fully editable), Transition Zone (smooth decay), Protected Zone (non-target text, explicitly locked), and Background (strictly preserved). EFF is training-free, model-agnostic, and works as a post-processing module for any diffusion-based STE method.

Result: EFF reduces spillover rate from 94% to 25% while improving non-target region preservation by +91.4 dB PSNR. The method also introduces per-region spillover quantification as a novel evaluation protocol to measure edit leakage at each non-target text region individually.

Conclusion: The Edit Fidelity Field effectively addresses the critical edit spillover problem in scene text editing by providing precise control over editing fidelity, significantly reducing unintended modifications to non-target regions while maintaining editing quality.
...
TL;DR: RS-HyRe-R1 is a hybrid reward framework that addresses "perceptual inertia" in RL-trained remote sensing vision-language models by promoting comprehensive visual evidence mining through spatial reasoning, perception correctness, and visual-semantic path evolution rewards.


<details>
  <summary>Details</summary>
Motivation: RL post-training causes remote sensing vision-language models to develop "perceptual inertia" - overreliance on localized salient cues for rapid inference, which impedes complete evidence construction and flexible visual focus shifting across complex remote sensing imagery tasks.

Method: Proposes RS-HyRe-R1 hybrid reward framework with three components: (1) spatial reasoning activation reward enforcing structured visual reasoning, (2) perception correctness reward providing adaptive quality anchors for geometric/semantic alignment, and (3) visual-semantic path evolution reward penalizing repetitive reasoning and promoting exploration of complementary cues.

Result: Achieves state-of-the-art performance on REC, OVD, and VQA tasks with only 3B parameters, outperforming models up to 7B parameters. Shows strong zero-shot generalization, surpassing second-best model by 3.16% (VQA), 3.97% (OVD), and 2.72% (REC).

Conclusion: RS-HyRe-R1 effectively mitigates RL-induced "perceptual inertia" in remote sensing vision-language models, encouraging deeper and more diverse reasoning while maintaining computational efficiency with smaller parameter counts.
...
TL;DR: DualTTA is a test-time adaptation framework that uses a dual strategy: minimizing entropy for reliable samples and maximizing entropy for unreliable samples, selected via a new reliability criterion based on prediction stability under transformations.


<details>
  <summary>Details</summary>
Motivation: Conventional TTA methods only use a small fraction of test samples (typically low-entropy ones), failing to fully leverage available test distribution information. This limits adaptation effectiveness under distribution shifts.

Method: DualTTA identifies two groups: reliable samples (consistent predictions) and unreliable samples (incorrect predictions). For reliable samples, it minimizes prediction entropy to reinforce decisions; for unreliable samples, it maximizes entropy to suppress errors. Samples are selected using a new reliability criterion measuring prediction stability under both semantic-preserving and semantic-altering transformations.

Result: The approach enables tighter separation between reliable and unreliable samples for adaptation, leading to provably more effective model updates. It provides theoretical analysis and empirical justification for improved performance under distribution shifts.

Conclusion: DualTTA improves TTA performance by utilizing more diverse test samples through a dual entropy strategy with transformation-based reliability selection, addressing limitations of purely entropy-based methods.
...
TL;DR: UniGeo: A unified geometric guidance framework for camera-controllable image editing that injects geometric consistency across representation, architecture, and loss function levels to prevent geometric drift and structural degradation under continuous camera motion.


<details>
  <summary>Details</summary>
Motivation: Existing camera-controllable image editing methods suffer from geometric drift and structural degradation due to fragmented geometric guidance (only at representation level) and reliance on image diffusion models with discrete view mappings. Video models provide continuous viewpoint priors but still lack stable geometric understanding when guidance is fragmented.

Method: UniGeo injects unified geometric guidance across three levels: 1) Representation level: frame-decoupled geometric reference injection for robust cross-view geometry context; 2) Architecture level: geometric anchor attention to align multi-view features; 3) Loss function level: trajectory-endpoint geometric supervision to explicitly reinforce structural fidelity of target views.

Result: Comprehensive experiments across multiple public benchmarks show UniGeo significantly outperforms existing methods in both visual quality and geometric consistency, in both extensive and limited camera motion settings.

Conclusion: Systematically injecting geometric guidance across representation, architecture, and loss function levels is crucial for achieving stable geometric understanding and preventing drift in camera-controllable image editing, with UniGeo demonstrating superior performance over existing approaches.
...
TL;DR: Tool-free multi-camera calibration method for sports using human keypoints and stick-like implements of known length.


<details>
  <summary>Details</summary>
Motivation: Current multi-camera calibration for sports is costly and labor-intensive, requiring dedicated calibration tools. There's a need for efficient, tool-free calibration methods.

Method: Three-stage optimization pipeline using synchronized multi-camera videos: 1) human body keypoints (unknown metric scale), 2) rigid stick-like implement of known length. Jointly exploits both cues to refine camera extrinsics, reconstruct trajectories, and resolve global scale via stick-length constraint.

Result: Achieves accurate extrinsic calibration without dedicated tools. Creates first dataset for multi-camera self-calibration in stick-based sports (synthetic sequences across 4 sports with 3-10 cameras). Demonstrates SOTA performance with low rotation and translation errors.

Conclusion: Proposes an efficient, tool-free method for multi-camera calibration in stick-based sports that outperforms existing approaches and provides a benchmark dataset for future research.
...
TL;DR: PBS-VL: A vision-language model specialized for Peripheral Blood Smear interpretation, trained on PBSInstr dataset and evaluated on PBSBench benchmark, outperforming general pathology models.


<details>
  <summary>Details</summary>
Motivation: Current multimodal LLMs for pathology are developed on solid-tissue WSIs and struggle with PBS interpretation, which focuses on individual cell morphologies rather than tissue architecture. There's a need for specialized models for hematopathology.

Method: Constructed PBSInstr dataset with 353 PBS WSIs, 29k cell crops with annotations, and 27k QA pairs. Developed PBS-VL model tailored for multi-level PBS interpretation. Created PBSBench benchmark with 4 question categories and 6 tasks for evaluation.

Result: PBS-VL outperforms existing general-purpose and pathology MLLMs, demonstrating the value of PBS-specific data. The model shows strong performance across various PBS interpretation tasks.

Conclusion: The framework provides foundation for AI assistants in hematopathology decision-making. PBS-specific data and models are crucial for accurate PBS interpretation. Released code, datasets, and model weights to facilitate research.
...
TL;DR: DGSSM is a diffusion-guided state space framework for multimodal salient object detection that integrates diffusion structural priors with Mamba-based state space modeling to improve boundary accuracy through progressive denoising.


<details>
  <summary>Details</summary>
Motivation: Current SOD models struggle with balancing long-range contextual dependencies and fine-grained structural details. Mamba-based models enable efficient global reasoning but struggle with precise boundaries, while diffusion models capture strong structural priors but face computational and integration challenges in discriminative dense prediction tasks.

Method: DGSSM formulates SOD as a progressive denoising process, integrating diffusion structural priors with multi-scale state space encoding, adaptive saliency prompting, and iterative Mamba diffusion refinement. It includes a boundary-aware refinement head and self-distillation strategy for enhanced spatial coherence.

Result: Extensive experiments on 13 public benchmarks across RGB, RGB-D, and RGB-T settings show DGSSM consistently outperforms state-of-the-art methods across multiple evaluation metrics while maintaining compact model size.

Conclusion: Diffusion-guided state space modeling is an effective and generalizable paradigm for multimodal dense prediction tasks, successfully combining the strengths of diffusion models (structural priors) and Mamba-based state space models (efficient global reasoning).
...
TL;DR: ViPS learns valid pose spaces for 3D rigs by distilling motion priors from video diffusion models, enabling plausible articulation without needing 4D training data.


<details>
  <summary>Details</summary>
Motivation: Kinematic rigs lack inherent representation of plausible joint configurations, leading to unrealistic poses when sampling or manipulating rig parameters manually. Existing methods require scarce artist-authored 4D datasets.

Method: Feed-forward framework that distills motion priors from pretrained video diffusion models into universal rig parameter distributions. Uses differentiable geometric validators on skinned meshes to enforce asset-specific validity without manual regularizers.

Result: Learns smooth, compact, controllable pose spaces supporting diverse sampling, manifold projection for IK, and temporally coherent keyframing. Distilled 3D pose samples serve as semantic proxies for guiding video diffusion. Matches SOTA performance trained on synthetic 4D data and demonstrates zero-shot generalization to out-of-distribution species and skeletal topologies.

Conclusion: ViPS successfully transfers generative video priors into universal pose distributions for kinematic rigs, enabling plausible articulation without requiring scarce 4D training data, with strong generalization capabilities.
...
TL;DR: FlowC2S: A novel method for fast, memory-efficient video continuation by fine-tuning text-to-video flow models to learn vector fields between current and succeeding video chunks, reducing input dimensionality by 50%.


<details>
  <summary>Details</summary>
Motivation: Current video continuation methods often combine current frames with noise to generate continuations, which is computationally expensive and memory-intensive. There's a need for more efficient approaches that can generate high-quality video continuations with fewer resources.

Method: FlowC2S fine-tunes pre-trained text-to-video flow models to learn vector fields between current and succeeding video chunks. Two key innovations: 1) Inherent optimal couplings using temporally adjacent chunks as training proxies for straighter flows, and 2) Target inversion that injects inverted latent representations of target chunks to strengthen correspondences and improve visual quality.

Result: The method achieves state-of-the-art scores on quantitative evaluations using FID (Fréchet Inception Distance) and FVD (Fréchet Video Distance) metrics. It requires only five neural function evaluations, making it significantly more efficient than existing approaches.

Conclusion: FlowC2S provides a more efficient approach to video continuation by directly flowing from current to succeeding frames, reducing input dimensionality by half while maintaining or improving visual quality. The method demonstrates superior performance with minimal computational requirements.
...
TL;DR: BioVLM is a prompt-learning framework that improves biomedical vision-language model generalization across challenging modalities by learning a diverse prompt bank with dynamic selection and LLM knowledge distillation.


<details>
  <summary>Details</summary>
Motivation: Existing biomedical VLMs like BioMedCLIP degrade on challenging modalities with small inter-class margins and pronounced acquisition-specific variations, especially under few-shot supervision when modality priors differ substantially from pretraining corpora.

Method: BioVLM learns a diverse prompt bank and introduces dynamic prompt selection using a low-entropy criterion on predictive distributions. It distills high-confidence LLM-derived attributes and enforces strong/weak augmentation consistency for robust knowledge transfer.

Result: On 11 MedMNIST+ 2D datasets, BioVLM achieves new state-of-the-art performance across three distinct generalization settings while keeping training lightweight and inference efficient.

Conclusion: BioVLM enables effective cross-domain generalization for biomedical VLMs without extensive backbone fine-tuning, allowing transfer to unseen categories and domains through modality-appropriate prompt selection.
...
TL;DR: The paper proposes Infrastructure-centric World Models (I-WM) as a novel approach to autonomous driving that leverages fixed roadside sensors' unique capabilities for long-term, multi-sensor, persistent observation, complementing vehicle-centric models.


<details>
  <summary>Details</summary>
Motivation: Current world models for autonomous driving adopt only ego-vehicle perspectives, missing the infrastructure viewpoint. Roadside systems offer bird's-eye, multi-sensor, persistent observation capabilities with superior temporal depth for capturing long-term behavioral distributions and rare safety-critical events, while vehicle sensors provide spatial breadth across diverse road networks.

Method: Proposes a three-phase vision: (I) generative scene understanding with quality-aware uncertainty propagation, (II) physics-informed predictive dynamics with multi-agent counterfactual reasoning, and (III) collaborative world models for V2X communication via latent space alignment. Introduces dual-layer architecture with annotation-free perception as multi-modal data engine, phased sensor strategy (LiDAR → 4D radar → signal phase data → event cameras), and Infrastructure VLA (I-VLA) unifying roadside perception, language commands, and traffic control actions.

Result: Establishes taxonomy of driving world model paradigms, positions I-WM relative to existing frameworks (LeCun's JEPA, Li Fei-Fei's spatial intelligence, VLA architectures), and identifies open-source foundations for implementation. Builds upon existing multi-LiDAR pipelines to provide a path toward infrastructure that understands and anticipates traffic.

Conclusion: Infrastructure-centric world models represent a fundamentally complementary capability to vehicle-centric approaches, offering the potential to create more comprehensive, safe, and intelligent autonomous driving systems by leveraging the unique spatio-temporal advantages of fixed roadside sensors.
...
TL;DR: Self-supervised hyperspectral super-resolution framework for Sentinel-5P that trains without high-resolution ground truth using SURE with equivariant imaging constraints and depthwise separable convolution U-Nets.


<details>
  <summary>Details</summary>
Motivation: Sentinel-5P's spatial resolution limits fine-scale analysis, and existing supervised SR methods require synthetic low-resolution data since true high-resolution data doesn't exist, limiting applicability to real observations.

Method: Combines Stein's Unbiased Risk Estimator (SURE) with equivariant imaging constraint, incorporates S5P degradation operator and noise statistics from SNR metadata, and uses depthwise separable convolution U-Net architectures for efficiency and spectral fidelity.

Result: Self-supervised models achieve performance comparable to supervised methods across multiple bands, maintain strong consistency, show improved spatial detail over bicubic interpolation, and validation with EMIT data confirms physically meaningful reconstructed structures.

Conclusion: Proposed self-supervised framework enables Sentinel-5P super-resolution training without high-resolution ground truth, achieving comparable performance to supervised methods while being applicable to real observations.
...
TL;DR: Review of NTIRE 2026 Low Light Image Enhancement Challenge, analyzing 22 teams' solutions for improving low-contrast, noisy images using novel dataset.


<details>
  <summary>Details</summary>
Motivation: To advance low-light image enhancement by identifying effective networks that can restore information loss from low-contrast and noisy conditions through a competitive challenge framework.

Method: Organized a competition with two tracks (195 and 153 registrants), collected novel dataset, evaluated 22 teams' submitted solutions using state-of-the-art approaches for joint denoising and low-light enhancement.

Result: 22 teams submitted valid entries, showcasing significant progress in the field through diverse solutions for producing clearer, visually compelling images in challenging low-light conditions.

Conclusion: The challenge successfully advanced low-light image enhancement research, demonstrating substantial progress through competitive evaluation of novel approaches on a comprehensive dataset.
...
TL;DR: MixTGFormer: A novel dual-stream GCN-Transformer network for 3D human pose estimation that fuses global and local features through parallel spatial and temporal modeling.


<details>
  <summary>Details</summary>
Motivation: Existing Transformer-based methods for 2D-to-3D human pose estimation focus mainly on global temporal and spatial relationships, neglecting local skeletal relationships and inter-channel information interaction.

Method: Proposes MixTGFormer with stacked Mixformers containing Mixformer Blocks and SE Layers. Uses parallel Mixformer Blocks with different modes to extract/fuse skeleton information, integrates GCN into Transformer for local-global feature utilization, and implements both temporal and spatial forms.

Result: Achieves state-of-the-art results on Human3.6M (37.6mm P1 error) and MPI-INF-3DHP (15.7mm P1 error) datasets.

Conclusion: MixTGFormer effectively models both spatial and temporal relationships through dual-stream architecture, achieving superior 3D pose estimation by fusing global and local features.
...
TL;DR: DVSA is a robust zero-shot learning framework that handles ambiguous labels through visual-semantic alignment, contrastive optimization, and dynamic label disambiguation.


<details>
  <summary>Details</summary>
Motivation: Existing ZSL methods assume clean labels, but real-world scenarios often have label noise and ambiguity that degrade performance. There's a need for robust ZSL frameworks that can handle ambiguous supervision.

Method: DVSA uses: 1) Bidirectional visual-semantic alignment with attention to mutually calibrate visual features and attribute prototypes; 2) Contrastive optimization based on Mutual Information at attribute level to strengthen discriminative attributes; 3) Dynamic label disambiguation mechanism that iteratively corrects noisy supervision while preserving semantic consistency.

Result: Extensive experiments on standard benchmarks show DVSA achieves stronger performance under ambiguous supervision compared to existing methods.

Conclusion: DVSA effectively bridges the gap between clean-label assumptions and real-world ambiguous supervision in ZSL, improving generalization through robust visual-semantic alignment and dynamic label correction.
...
TL;DR: GeGS-PCR is a novel two-stage point cloud registration method that combines geometric, color, and Gaussian information to achieve robust registration in low-overlap and incomplete scenarios, achieving state-of-the-art performance with 99.9% registration recall.


<details>
  <summary>Details</summary>
Motivation: Traditional point cloud registration methods relying solely on geometric features struggle in low-overlap and incomplete scenarios. There's a need to incorporate color information to improve robustness in challenging conditions.

Method: A two-stage method with: 1) dedicated color encoder extracting multi-level geometric and color features, 2) Geometric-3DGS module encoding local neighborhood information of colored superpoints for globally invariant geometric-color context, 3) LORA optimization to maintain performance while preserving 3DGS expressiveness, 4) fast differentiable rendering for registration refinement, and 5) joint photometric loss exploiting both geometric and color features.

Result: Achieves state-of-the-art performance with Registration Recall at 99.9%, Relative Rotation Error as low as 0.013, and Relative Translation Error as low as 0.024, improving precision by at least a factor of 2 on Color3DMatch and Color3DLoMatch datasets.

Conclusion: GeGS-PCR demonstrates that combining geometric, color, and Gaussian information significantly improves point cloud registration robustness, especially in challenging low-overlap scenarios, outperforming existing methods by substantial margins.
...
TL;DR: GaussianHSI: A Gaussian splatting framework for arbitrary-scale hyperspectral image super-resolution using Voronoi-guided bilateral 2D Gaussian splatting and spectral detail enhancement.


<details>
  <summary>Details</summary>
Motivation: Existing hyperspectral super-resolution methods lack flexibility for arbitrary scales and often rely on rasterization strategies that limit spatial modeling. There's a need for adaptive spatial reconstruction while preserving spectral fidelity.

Method: Proposes GaussianHSI with Voronoi-Guided Bilateral 2D Gaussian Splatting: predicts Gaussian functions to represent input, associates target pixels with relevant Gaussians via Voronoi-guided selection, reconstructs using bilateral weighting considering geometric relevance and low-resolution feature consistency. Includes Spectral Detail Enhancement module.

Result: Extensive experiments on benchmark datasets demonstrate GaussianHSI outperforms state-of-the-art methods for arbitrary-scale hyperspectral image super-resolution.

Conclusion: GaussianHSI provides an effective framework for flexible, arbitrary-scale hyperspectral image super-resolution with improved spatial and spectral reconstruction quality.
...
TL;DR: Score-based denoising framework for cryo-EM that learns clean-data scores to preserve structural information, with target-guided variant for weak signals, improving particle picking and 3D reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Cryo-EM micrographs have extremely low SNR and weak particle visibility, making denoising crucial for downstream analysis. Existing methods improve visual quality but don't explicitly preserve structural consistency needed for particle picking, 2D classification, and 3D reconstruction.

Method: Proposes a score-based denoising framework that learns clean-data scores to recover particle signals while preserving structural information. Introduces a target-guided variant incorporating reference-density guidance to stabilize score learning under weak/ambiguous signal conditions. The approach suppresses structured low-frequency background rather than just amplifying particle-like responses.

Result: Experiments on multiple cryo-EM datasets show consistent improvements in downstream particle picking and production of more structure-consistent 3D reconstructions. The method enhances particle-background separability for better downstream analysis.

Conclusion: The score-based denoising framework with target-guided variant effectively addresses cryo-EM denoising challenges by preserving structural information crucial for downstream analysis, outperforming existing methods that focus only on visual quality improvements.
...
TL;DR: IncreFA is an incremental learning framework for AI image attribution that continuously adapts to new generative models by exploiting architectural hierarchies and using a latent memory bank for stable representation learning.


<details>
  <summary>Details</summary>
Motivation: Traditional image attribution methods become obsolete quickly as new generative models (diffusion, adversarial, autoregressive) emerge rapidly. The core problem is the inability to adapt attribution systems to continuously evolving AI models.

Method: IncreFA treats attribution as structured incremental learning with two key mechanisms: 1) Hierarchical Constraints using learnable orthogonal priors to disentangle family-level invariants from model-specific features, and 2) Latent Memory Bank that replays compact latent exemplars and mixes them to generate pseudo-unseen samples for stable representation learning.

Result: On the Incremental Attribution Benchmark (IABench) covering 28 generative models from 2022-2025, IncreFA achieves state-of-the-art attribution accuracy and 98.93% unseen detection under temporally ordered open-set protocols.

Conclusion: IncreFA successfully addresses the moving target problem of AI image attribution by redefining it as a structured incremental learning problem, enabling continuous adaptation to new generative models while maintaining high attribution accuracy and detection capabilities.
...
TL;DR: DIFO++ is a novel Source-Free Domain Adaptation method that leverages vision-language models like CLIP through prompt learning and knowledge distillation, focusing on gap region reduction to improve adaptation performance.


<details>
  <summary>Details</summary>
Motivation: Conventional SFDA methods rely on error-prone pseudo-labeling and auxiliary supervision. The authors explore using off-the-shelf vision-language models with rich knowledge, but find that directly applying them in zero-shot fashion is unsatisfactory as they're too generic and not task-specific.

Method: DIFO++ alternates between two steps: (1) Customizing the ViL model via prompt learning to maximize mutual information with the target model, (2) Distilling knowledge from customized ViL to target model with focus on gap region reduction. Uses memory mechanism for reliable pseudo-labels, category attention, predictive consistency for semantic alignment, and referenced entropy minimization to suppress uncertainty.

Result: Extensive experiments show DIFO++ significantly outperforms state-of-the-art alternatives in Source-Free Domain Adaptation tasks.

Conclusion: DIFO++ effectively leverages vision-language models for SFDA by making them task-specific through prompt learning and focusing on gap region reduction, achieving superior performance compared to existing methods.
...
TL;DR: EgoIn framework generates intermediate frames showing object transformations between initial and target states from an egocentric view, addressing challenges in visual scene understanding and consistent transition generation.


<details>
  <summary>Details</summary>
Motivation: Understanding physical transformation processes from an egocentric perspective is crucial for bridging human cognition and AI systems in action modeling, but current generative models struggle with visual scene understanding and consistent transition generation.

Method: Proposes EgoIn framework with TransitionVLM for multi-step transition inference, Transition Conditioning module for frame sequence generation, and Object-aware Auxiliary Supervision for appearance consistency.

Result: Extensive experiments on human-object and robot-object interaction datasets show EgoIn's superior performance in generating semantically meaningful and visually coherent transformation sequences.

Conclusion: EgoIn effectively addresses the challenges of Egocentric Instructed Visual State Transition by combining transition inference, conditioning mechanisms, and appearance preservation techniques.
...
TL;DR: A sparse voxel-space diffusion framework for 3D medical image enhancement that achieves 10× training acceleration by using uniformly subsampled timesteps and structure-aware trajectory modulation, outperforming state-of-the-art methods on CT, PET, and MRI denoising and super-resolution.


<details>
  <summary>Details</summary>
Motivation: While diffusion models excel in 2D medical imaging, scaling them to high-resolution 3D volumes is computationally prohibitive due to lengthy diffusion trajectories over high-dimensional data. The authors observe that strong anatomical priors in degraded inputs make dense noise schedules largely redundant for conditional enhancement tasks.

Method: Proposes a sparse voxel-space diffusion framework that trains and samples on a compact set of uniformly subsampled timesteps. The network predicts clean data directly on the data manifold with velocity-space supervision for stable gradient scaling. A lightweight Structure-aware Trajectory Modulation (STM) module recalibrates time embeddings at each network block based on local anatomical content, enabling structure-adaptive denoising over shared sparse schedules.

Result: Achieves up to 10× training acceleration while preserving fine anatomical detail without lossy compression. Experiments on four datasets spanning CT, PET, and MRI demonstrate state-of-the-art performance on both denoising and super-resolution tasks.

Conclusion: The proposed sparse diffusion framework successfully addresses computational bottlenecks in 3D medical image enhancement while maintaining superior performance, offering an efficient solution for clinical applications in CT, PET, and MRI.
...
TL;DR: SAMGA framework achieves state-of-the-art zero-shot EEG-to-image retrieval by using subject-aware multi-granularity alignment to adapt to individual neural response patterns.


<details>
  <summary>Details</summary>
Motivation: Current EEG-to-image retrieval methods overlook two key properties: EEG signals preserve information across multiple visual scales, and the optimal visual granularity for EEG matching varies across subjects. Prior methods use either fixed visual targets or subject-invariant schemes, limiting performance.

Method: Proposes Subject-Aware Multi-Granularity Alignment (SAMGA) framework: 1) Constructs subject-aware visual supervision targets by adaptively aggregating multiple intermediate representations from pretrained vision encoder, 2) Implements coarse-to-fine cross-modal alignment with shared encoder where coarse stage stabilizes semantic geometry and reduces distribution shift, fine stage improves instance-level discrimination.

Result: On THINGS-EEG benchmark: 91.3% Top-1 and 98.8% Top-5 accuracy in intra-subject setting; 34.4% Top-1 and 64.8% Top-5 accuracy in inter-subject setting, outperforming recent state-of-the-art methods.

Conclusion: SAMGA effectively addresses subject-dependent granularity variations in EEG signals through adaptive multi-granularity alignment, achieving robust zero-shot EEG-to-image retrieval with both intra- and inter-subject improvements.
...
TL;DR: DuQuant++ adapts outlier-aware rotation to MXFP4 format by aligning rotation block size with microscaling group size (B=32), replacing complex pipeline with single rotation to reduce cost and improve quantization accuracy for LLM inference.


<details>
  <summary>Details</summary>
Motivation: MXFP4 format is promising for efficient LLM inference with hardware support, but activation outliers cause quantization error by inflating shared block scales and compressing dynamic range of other elements. Existing rotation methods are data-agnostic and can't target outlier-concentrated channels.

Method: DuQuant++ adapts DuQuant's outlier-aware fine-grained rotation to MXFP4 by aligning rotation block size with microscaling group size (B=32). Since each MXFP4 group has independent scaling factor, cross-block variance issues are eliminated, allowing replacement of dual rotations and zigzag permutation with single outlier-aware rotation.

Result: Extensive experiments on LLaMA-3 family under MXFP4 W4A4 quantization show DuQuant++ consistently achieves state-of-the-art performance, halving online rotation cost while smoothing weight distribution.

Conclusion: DuQuant++ effectively addresses MXFP4's outlier challenge with efficient single rotation approach, demonstrating superior performance for 4-bit LLM quantization with reduced computational overhead.
...
TL;DR: WSRVOS is a novel weakly-supervised referring video object segmentation method that trains models using only text expressions without any mask, bounding box, or point annotations.


<details>
  <summary>Details</summary>
Motivation: Existing RVOS methods require expensive pixel-level mask annotations, and even weakly-supervised approaches need bounding boxes or points which are still costly. The paper aims to develop a method that uses only text expressions as supervision to reduce annotation burden.

Method: 1) Contrastive referring expression augmentation using multimodal LLM to generate positive/negative expressions; 2) Bi-directional vision-language feature selection and interaction; 3) Instance-aware expression classification; 4) Positive-prediction fusion for pseudo-mask generation; 5) Temporal segment ranking constraint for temporal consistency.

Result: Extensive experiments on four RVOS datasets (A2D Sentences, J-HMDB Sentences, Ref-YouTube-VOS, Ref-DAVIS17) demonstrate the superiority of the proposed method over existing approaches.

Conclusion: WSRVOS successfully achieves effective referring video object segmentation using only text expressions as supervision, significantly reducing annotation costs while maintaining competitive performance.
...
TL;DR: A novel 3D scene editing framework that addresses cross-view inconsistency by modeling joint distribution across viewpoints and using dual-path consistency with structural guidance and semantic propagation.


<details>
  <summary>Details</summary>
Motivation: Existing text-driven 3D scene editing methods suffer from cross-view inconsistency despite using geometric cues or video priors. The authors identify that current approaches rely too much on inference-time synchronization and lack robustness and generalization.

Method: Proposes a view-consistent 3D editing framework that: 1) Models 3D editing as joint distribution across viewpoints, 2) Introduces explicit cross-view dependencies, 3) Uses dual-path consistency mechanism with projection-guided structural guidance and patch-level semantic propagation, 4) Constructs a paired multi-view editing dataset for supervision.

Result: Extensive experiments show superior editing performance with precise and consistent views for complex scenes compared to existing methods.

Conclusion: By recasting 3D editing from a distributional perspective and introducing explicit cross-view dependencies with dual-path consistency, the method effectively addresses cross-view inconsistency and achieves robust, high-quality 3D scene editing.
...
TL;DR: Re²MoGen: A three-stage framework for open-vocabulary text-to-motion generation using enhanced LLM reasoning for motion planning and RL post-training for physical refinement.


<details>
  <summary>Details</summary>
Motivation: Existing T2M models perform well on in-distribution text prompts but fail on out-of-distribution descriptions. Need for better generalization to diverse, unseen motion descriptions beyond training data.

Method: Three-stage approach: 1) Enhanced LLM reasoning with Monte Carlo tree search to generate keyframe motion planning, 2) Human pose model prior for full-body pose optimization and motion generator fine-tuning, 3) Physics-aware RL post-training to refine motion quality and eliminate physical implausibility.

Result: Achieves state-of-the-art performance in open-vocabulary motion generation, generating semantically consistent and physically plausible motions even for text descriptions significantly different from training data.

Conclusion: Re²MoGen effectively addresses the open-vocabulary challenge in T2M generation by combining enhanced LLM reasoning with physical refinement, enabling robust performance on diverse motion descriptions beyond training distribution.
...
TL;DR: A two-stage diffusion framework reconstructs 3D human motion and human-object interactions from Internet videos by first synthesizing multi-view 2D motion data and then using a camera-conditioned diffusion model to recover 3D motion.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with globally consistent 3D motion reconstruction under dynamic cameras, especially for underrepresented motion types in MoCap datasets, and face difficulty recovering coherent human-object interactions in 3D from Internet videos.

Method: Two-stage framework: 1) Synthesize multi-view 2D motion data using 2D keypoints from Internet videos to capture diverse motions; 2) Train camera-conditioned multi-view 2D motion diffusion model on synthetic data to recover 3D human motion and HOI in world space.

Result: Outperforms prior work on Internet videos with challenging motions (gymnastics) and in-the-wild HOI videos, producing more realistic human motion and human-object interactions.

Conclusion: The proposed diffusion-based framework effectively reconstructs 3D human motion and HOI from Internet videos, handling challenging motion types and dynamic cameras better than existing methods.
...
TL;DR: GR4CIL is a CLIP-based class-incremental learning framework that combines task discrimination and knowledge routing to reduce interference across tasks while maintaining zero-shot generalization capability.


<details>
  <summary>Details</summary>
Motivation: Existing CLIP-based CIL methods face two key challenges: shared-parameter adaptation causes old-knowledge drift, and task-specific knowledge organization leads to poorly calibrated cross-task responses, making reliable routing difficult.

Method: GR4CIL preserves task-specific visual knowledge while maintaining an incrementally stable shared textual semantic space. It introduces an orthogonal compensation mechanism to mitigate modality-gap-induced bias, enhance within-task discrimination, and enlarge score margins between ground-truth and competing tasks.

Result: Experiments on multiple benchmarks show that GR4CIL consistently outperforms strong baselines in class-incremental learning.

Conclusion: GR4CIL enables more reliable task-aware routing over learned knowledge while retaining the zero-shot generalization capability of CLIP models, effectively addressing key challenges in CLIP-based CIL.
...
TL;DR: PCM-NeRF: A probabilistic neural surface reconstruction method that learns per-camera pose uncertainty to handle imperfect camera pose estimates, improving reconstruction quality without requiring foreground masks.


<details>
  <summary>Details</summary>
Motivation: Existing neural surface reconstruction methods assume perfect camera poses from SfM, but this breaks down with imperfect pose estimates, leading to distorted or incomplete reconstructions.

Method: Augments neural surface reconstruction with per-camera learnable uncertainty built on SG-NeRF. Represents each pose as a distribution with learnable mean/variance initialized from SfM correspondence quality. Uses uncertainty regularization loss to couple learned variance to view confidence, and uncertainty modulates effective pose learning rate (uncertain cameras get damped gradient updates).

Result: Outperforms state-of-the-art methods in both Chamfer Distance and F-Score on challenging scenes with severe pose outliers, particularly for geometrically complex structures, without requiring foreground masks.

Conclusion: PCM-NeRF provides a lightweight probabilistic framework that handles imperfect camera poses through learnable uncertainty, preventing poorly initialized views from corrupting reconstructions while adding negligible overhead.
...
TL;DR: AI framework enables automated thoracolumbar spine segmentation and 3D reconstruction from MRI alone, eliminating need for CT in pediatric spine deformity assessment.


<details>
  <summary>Details</summary>
Motivation: MRI avoids ionizing radiation but lacks automated 3D bony reconstruction for spine deformity assessment, forcing reliance on CT. Manual MRI workflows and scarce labeled datasets limit practical use.

Method: Used GAN to convert historical low-dose CT scans into MRI-like images, combined with existing thoracic MRI data to train U-Net-based model for automated thoracolumbar spine segmentation and 3D reconstruction.

Result: Achieved 88% Dice score for segmentation, reduced processing time from ~1 hour to under one minute, generated continuous thoracolumbar 3D reconstructions while preserving AIS-specific deformity features.

Conclusion: Enables radiation-free 3D deformity assessment from MRI, supporting clinical evaluation, surgical planning, and navigation in pediatric spine care without CT exposure.
...
TL;DR: UniCSG is a unified framework for content-constrained style generation that addresses content-style entanglement in diffusion models through staged training with semantic disentanglement and frequency-aware reconstruction.


<details>
  <summary>Details</summary>
Motivation: DiT-based diffusion models often suffer from content-style entanglement, leading to reference-content leakage and unstable generation in style transfer tasks. There's a need for better separation between content semantics and target style while maintaining generation quality.

Method: Two-stage training: 1) Latent-space semantic disentanglement using low-frequency preprocessing and conditioning corruption to separate content and style; 2) Latent-space frequency-aware detail reconstruction with multi-scale frequency supervision. Plus pixel-space reward learning to align latent objectives with perceptual quality.

Result: Experiments show improved content faithfulness, style alignment, and robustness in both text-guided and reference-guided style transfer settings.

Conclusion: UniCSG provides an effective unified framework for content-constrained style generation that successfully addresses content-style entanglement through staged training and frequency-aware supervision.
...
TL;DR: PlankFormer: A Vision Transformer-based framework for plankton instance segmentation that uses synthetic training data and self-supervised pre-training to overcome annotation scarcity and segmentation challenges in crowded, debris-filled environments.


<details>
  <summary>Details</summary>
Motivation: Manual plankton analysis is labor-intensive, and automated segmentation faces two key challenges: scarcity of pixel-level annotated datasets, and difficulty distinguishing plankton from debris/overlapping individuals with conventional CNN methods.

Method: 1) Generate labeled Pseudo Community Images (PCI) by synthesizing individual plankton images onto diverse backgrounds (including generative model backgrounds); 2) Use Vision Transformer backbone with Mask2Former decoder; 3) Employ Masked Autoencoder (MAE) for self-supervised pre-training on unlabeled individual images to capture global structural features.

Result: Significantly outperforms conventional methods like Mask R-CNN, especially in challenging environments with high debris density. Enables high-precision segmentation with less manual annotation.

Conclusion: The synthetic training strategy combined with MAE-based architecture provides an effective solution for plankton instance segmentation, addressing both data scarcity and segmentation challenges in complex aquatic environments.
...
TL;DR: LiteBounD is a lightweight boundary-guided distillation framework that transfers semantic and structural priors from large Vision Foundation Models to compact segmentation backbones for efficient and accurate polyp segmentation.


<details>
  <summary>Details</summary>
Motivation: Automated polyp segmentation is crucial for early colorectal cancer detection but faces challenges: weak boundaries, appearance variations, and limited annotated data. Lightweight models lack rich semantic understanding, while large foundation models have domain mismatch and high computational costs.

Method: Proposes LiteBounD with three key components: 1) dual-path distillation disentangling semantic and boundary-aware representations, 2) frequency-aware alignment supervising low-frequency global semantics and high-frequency boundary details separately, and 3) boundary-aware decoder fusing multi-scale encoder features with distilled boundary information.

Result: Extensive experiments on seen (Kvasir-SEG, CVC-ClinicDB) and unseen (ColonDB, CVC-300, ETIS) datasets show LiteBounD significantly outperforms lightweight baselines and achieves competitive performance with state-of-the-art methods while maintaining real-time efficiency.

Conclusion: LiteBounD successfully bridges the gap between lightweight efficiency and rich semantic understanding by distilling complementary priors from multiple Vision Foundation Models, enabling accurate polyp segmentation suitable for real-time clinical deployment.
...
TL;DR: Vid-LLMs exhibit spatiotemporal sycophancy - retracting correct visual judgments to conform to misleading user feedback, fabricating unsupported explanations to justify incorrect revisions.


<details>
  <summary>Details</summary>
Motivation: Video Large Language Models show strong performance but their robustness under conversational interaction is underexplored, particularly their tendency to retract correct visual judgments when faced with misleading user feedback.

Method: Proposed a negation-based gaslighting evaluation framework and created GasVideo-1000 benchmark to probe spatiotemporal sycophancy with clear visual grounding and temporal reasoning requirements. Evaluated various state-of-the-art open-source and proprietary Vid-LLMs.

Result: Vulnerability to negation-based gaslighting is pervasive and severe across all tested models, even those with strong baseline performance. Prompt-level grounding constraints only partially mitigate the behavior and don't prevent hallucinated justifications or belief reversal.

Conclusion: Current Vid-LLMs lack robust mechanisms for maintaining grounded spatiotemporal beliefs under adversarial conversational feedback, revealing a critical weakness in their conversational robustness.
...
TL;DR: BASFNet is a camouflaged object detection framework that fuses frequency and spatial domain features using phase spectrum analysis and boundary-aware training to better detect objects that blend with their backgrounds.


<details>
  <summary>Details</summary>
Motivation: Current COD methods focus too much on edge extraction and local pixel information while neglecting global structural features and frequency domain information, especially phase spectrum features that could help distinguish camouflaged objects from backgrounds.

Method: Proposes BASFNet with dual guided integration of frequency and spatial domain features: 1) Frequency-Enhanced Edge Exploration Module (FEEM) using phase spectrum analysis, 2) Spatial Core Segmentation Module (SCSM) for object features, 3) Spatial-Frequency Fusion Interaction Module (SFFIM) for feature integration, and 4) boundary-aware training strategy.

Result: BASFNet outperforms existing state-of-the-art methods on three benchmark datasets, demonstrating the effectiveness of fusing frequency and spatial domain information for COD tasks.

Conclusion: The fusion of frequency domain (especially phase spectrum) and spatial domain features, combined with boundary-aware training, provides an effective solution for camouflaged object detection by better capturing both boundary and object features.
...
TL;DR: AeroRAG is a scene-graph-guided multimodal retrieval-augmented generation framework that converts aerial images into structured visual knowledge (objects, quantities, locations, relations) and retrieves relevant semantic chunks to construct compact prompts for LLMs, improving visual question answering in aerial scenes.


<details>
  <summary>Details</summary>
Motivation: Current MLLMs struggle with reliable visual question answering in aerial scenes where task-critical evidence involves small objects, explicit quantities, coarse locations, and inter-object relations. Conventional dense visual-token representations don't align well with these structured semantics, creating an interface mismatch between perception and language reasoning.

Method: AeroRAG converts input images into structured visual knowledge (object categories, quantities, spatial locations, semantic relations), then retrieves query-relevant semantic chunks to construct compact prompts for text-based LLMs. This creates an explicit intermediate interface between perception and language reasoning instead of relying on direct reasoning over dense visual tokens.

Result: Experiments on AUG aerial dataset and VG-150 benchmark show consistent improvements over six strong MLLM baselines, with largest gains in dense aerial scenes and relation-sensitive reasoning. Framework also performs well on VQAv2, demonstrating compatibility with standard visual reasoning settings.

Conclusion: Structured retrieval is a practical design direction for deployment-oriented and grounded visual reasoning systems, providing a more explicit interface between perception and language reasoning that addresses the limitations of conventional dense visual-token representations in aerial scenes.
...
TL;DR: ReTrack is a novel framework for Composed Video Retrieval that addresses modality bias by calibrating directional anchors in composed features, achieving state-of-the-art performance across multiple benchmarks.


<details>
  <summary>Details</summary>
Motivation: Traditional CVR methods suffer from bias toward reference videos due to information density discrepancy between video and text modalities, leading to suboptimal retrieval performance. Three core challenges exist: modal contribution entanglement, explicit optimization of composed features, and retrieval uncertainty.

Method: ReTrack uses three key modules: Semantic Contribution Disentanglement to estimate modality contributions, Composition Geometry Calibration to adjust directional bias, and Reliable Evidence-driven Alignment that computes bidirectional evidence for similarity estimation.

Result: ReTrack achieves state-of-the-art performance across three benchmark datasets in both Composed Video Retrieval and Composed Image Retrieval scenarios, demonstrating strong generalization capabilities.

Conclusion: ReTrack is the first CVR framework to address directional bias in composed features through evidence-driven calibration, significantly improving multi-modal query understanding and retrieval performance while generalizing well to related tasks.
...
TL;DR: MEDN: A dual-branch network that decouples motion and emotion features for micro-expression recognition, addressing the challenge of visually similar expressions with identical AUs but opposite emotions.


<details>
  <summary>Details</summary>
Motivation: Micro-expressions don't follow consistent emotion-AU mapping rules, causing identical AUs to represent opposite emotions. Existing methods rely on explicit facial motion cues while ignoring implicit emotion information, limiting recognition accuracy.

Method: Proposes Motion Emotion Feature Decoupling Network (MEDN) with dual branches: motion branch uses AU-detection task and orthogonal loss to extract explicit motion features; emotion branch uses Sparse Emotion Vision Transformer (SEVit) with multi-scale sparsity rates to highlight local temporal variations. Features are fused via Collaborative Fusion Module (CoFM).

Result: Extensive experiments on three benchmark datasets show MEDN effectively decouples motion and emotion features and achieves superior recognition performance compared to existing methods.

Conclusion: MEDN offers a new perspective for enhancing micro-expression recognition accuracy and generalization by explicitly decoupling motion and emotion features, addressing the fundamental challenge of ambiguous emotion-AU mappings in micro-expressions.
...
TL;DR: TranCLR introduces transitional anchor-based contrastive learning for skeleton action recognition, addressing motion continuity issues in existing binary contrastive methods through geometric modeling and manifold calibration.


<details>
  <summary>Details</summary>
Motivation: Existing self-supervised contrastive learning methods for skeleton-based action recognition use binary contrastive objectives that fail to capture the intrinsic continuity of human motion, leading to fragmented feature clusters and rigid class boundaries.

Method: Proposes TranCLR with two key components: 1) Action Transitional Anchor Construction (ATAC) that explicitly models geometric structure of transitional states, and 2) Multi-Level Geometric Manifold Calibration (MGMC) that adaptively calibrates the action manifold across multiple continuity levels.

Result: Extensive experiments on NTU RGB+D, NTU RGB+D 120, and PKU-MMD datasets show TranCLR achieves superior accuracy and calibration performance, effectively learning continuous and uncertainty-aware skeleton representations.

Conclusion: TranCLR successfully addresses the limitations of binary contrastive learning by capturing motion continuity through transitional anchors and geometric manifold calibration, resulting in smoother and more discriminative representation spaces for skeleton-based action recognition.
...
TL;DR: OneDrive: A unified autonomous driving framework using a single transformer decoder from pretrained VLM to handle heterogeneous tasks (language, perception, planning) with shared attention backbone.


<details>
  <summary>Details</summary>
Motivation: Existing autonomous driving systems use separate/cascaded decoders for different tasks (language generation, object detection, trajectory regression), causing architectural fragmentation and limited backbone reuse. Need to reconcile heterogeneous decoding behaviors within a unified framework.

Method: Build upon pretrained VLM with single transformer decoder. Organize visual and structured query tokens within causal decoder. Use original attention mechanism for structured queries to condition on visual context. Introduce structured trajectory queries for planning within same LLM decoder. Share attention backbone across textual and structured outputs.

Result: State-of-the-art performance: 0.28 L2 and 0.18 collision rate on nuScenes open-loop evaluation, competitive 86.8 PDMS on NAVSIM closed-loop evaluation. Efficient inference mode achieves ~40% lower latency. Full model preserves multi-modal generation capability.

Conclusion: Pretrained VLM attention transfers well beyond language modeling. Single decoder can handle heterogeneous autonomous driving tasks with stable joint optimization. Unified formulation enables planning to share pretrained attention backbone with perception and images.
...
TL;DR: Zero-shot ship instance segmentation in SAR imagery using YOLOv11 for detection and SAM2 for segmentation without pixel-level annotations, achieving 89% of supervised baseline performance.


<details>
  <summary>Details</summary>
Motivation: Deep learning for SAR analysis is limited by lack of pixel-level annotations. The paper aims to enable ship instance segmentation in SAR imagery without requiring expensive mask annotations by leveraging general-purpose vision foundation models.

Method: Uses YOLOv11-based detector trained on open SAR datasets to localize ships via bounding boxes, then prompts Segment Anything Model 2 (SAM2) to produce instance masks without any mask annotations. Unlike prior approaches, relies only on spatial constraints from SAR-trained detector to regularize foundation model predictions.

Result: Achieves mean IoU of 0.637 (89% of a fully supervised baseline) on SSDD benchmark with overall ship detection rate of 89.2%. Demonstrates effective mitigation of optical-SAR domain gap and enables downstream applications like vessel classification, size estimation, and wake analysis.

Conclusion: The method provides a scalable, annotation-efficient pathway toward foundation-model-driven SAR image understanding by showing that spatial constraints from a SAR-trained detector alone can effectively regularize foundation model predictions without fine-tuning or adapters.
...
TL;DR: BI-Cap is a brain-inspired perceptual simulation paradigm that aligns neural and visual modalities by emulating Human Visual System processing, achieving state-of-the-art performance in zero-shot brain-to-image retrieval.


<details>
  <summary>Details</summary>
Motivation: Current brain-computer interface approaches have systematic gaps between neural and visual modalities and neglect the intrinsic computational mechanisms of the Human Visual System, limiting their effectiveness.

Method: Proposes a neuromimetic pipeline with four biologically plausible transformations, Mutual Information-guided dynamic blur regulation to simulate adaptive visual processing, and an evidence-driven latent space representation to handle neural non-stationarity and uncertainty.

Result: BI-Cap substantially outperforms state-of-the-art methods on zero-shot brain-to-image retrieval across two public benchmarks, achieving relative gains of 9.2% and 8.0% respectively.

Conclusion: The brain-inspired approach that emulates Human Visual System processing effectively bridges the gap between neural and visual modalities, offering a promising direction for brain-computer interfaces and computational neuroscience.
...
TL;DR: HONES is a gradient-free framework for task-aware neuron attribution and steering in multi-task vision-language models that ranks FFN neurons by their causal write-in contributions conditioned on task-relevant attention heads.


<details>
  <summary>Details</summary>
Motivation: Existing neuron interpretation methods focus on single tasks, limiting comparability across tasks, and rank neurons in isolation, overlooking how task-dependent information pathways shape FFN neuron effects, which exacerbates neuron polysemanticity in multi-task settings.

Method: HONES uses a gradient-free framework that ranks feed-forward network (FFN) neurons by their causal write-in contributions conditioned on task-relevant attention heads, and modulates salient neurons via lightweight scaling.

Result: Experiments on four diverse multimodal tasks and two popular VLMs show that HONES outperforms existing methods in identifying task-critical neurons and improves model performance after steering.

Conclusion: HONES provides an effective framework for task-aware neuron attribution and steering in multi-task VLMs, addressing limitations of existing single-task focused approaches and improving both neuron identification and model performance.
...
TL;DR: ZSG-IAD is a multimodal vision-language framework for zero-shot grounded industrial anomaly detection that generates structured anomaly reports and pixel-level masks using language-guided grounding and executable-rule reinforcement learning.


<details>
  <summary>Details</summary>
Motivation: Current deep learning-based industrial anomaly detectors act as black boxes, lacking physically meaningful defect evidence and transparent justification for their decisions.

Method: Uses multimodal inputs (RGB images, sensor images, 3D point clouds) with a language-guided two-hop grounding module: 1) anomaly-related sentences select evidence-like latent slots from multimodal features for coarse spatial support, 2) selected slots modulate feature maps via channel-spatial gating and lightweight decoder for fine-grained masks. Applies Executable-Rule GRPO with verifiable rewards to improve reliability.

Result: Shows strong zero-shot performance across multiple industrial anomaly benchmarks and produces more transparent, physically grounded explanations than prior methods.

Conclusion: ZSG-IAD enables trustworthy industrial anomaly detection with structured outputs and physically meaningful explanations, advancing research on transparent AI systems for industrial applications.
...
TL;DR: CoEvoer is a novel one-stage transformer framework for upper-body expressive human pose and shape estimation that enables explicit feature-level interaction across body parts to improve accuracy and generalization.


<details>
  <summary>Details</summary>
Motivation: Current state-of-the-art methods struggle with accurate facial and hand parameter estimation and have limited generalization to wild images. There's a need for better modeling of dependencies between different upper-body regions.

Method: CoEvoer uses a synergistic cross-dependency transformer framework that enables explicit feature-level interaction across body parts. Larger regions (torso) provide global semantics and positional priors to guide finer regions (face, hands), while localized details from face/hands refine adjacent parts through joint parameter regression.

Result: CoEvoer achieves state-of-the-art performance on upper-body benchmarks and exhibits strong generalization capability even on unseen wild images.

Conclusion: CoEvoer is the first framework specifically designed for upper-body EHPS that effectively captures the strong coupling and semantic dependencies among face, hands, and torso, addressing key limitations in current methods.
...
TL;DR: DifFoundMAD is a parameter-efficient differential morphing attack detection framework that uses vision foundation model embeddings instead of traditional face recognition features, achieving significant error rate reductions at high-security levels.


<details>
  <summary>Details</summary>
Motivation: Current D-MAD systems rely on face recognition embeddings or handcrafted features, which may not generalize well. The paper aims to leverage the superior generalization capabilities of vision foundation models to better detect morphing attacks, especially for critical applications like border control.

Method: DifFoundMAD follows the differential paradigm but replaces the underlying representation space with embeddings from vision foundation models. It uses lightweight fine-tuning with class-balanced optimization, updating only a small subset of parameters while preserving the FM's rich representational priors.

Result: Extensive cross-database evaluations show consistent improvements over state-of-the-art systems. Most notably, error rates at high-security levels were reduced from 6.16% to 2.17% - a significant improvement for operational deployments like border control.

Conclusion: DifFoundMAD demonstrates that vision foundation models provide superior representations for D-MAD tasks, and that parameter-efficient fine-tuning with class-balanced optimization can effectively leverage these models while maintaining their generalization capabilities.
...
TL;DR: MU-GeNeRF improves generalizable NeRF robustness to transient distractors by combining source-view and target-view uncertainty estimation with heteroscedastic reconstruction loss.


<details>
  <summary>Details</summary>
Motivation: Transient distractors in real-world scenes break cross-view consistency, corrupting supervision and degrading reconstruction quality in generalizable NeRFs. Existing distractor-free methods rely on per-scene optimization and unreliable uncertainty estimation that often misclassifies static structures as distractors.

Method: Proposes a multi-view uncertainty-guided framework with two complementary uncertainty components: source-view uncertainty (captures structural discrepancies across views) and target-view uncertainty (detects observation anomalies in target images). These are combined through a heteroscedastic reconstruction loss that adaptively modulates supervision.

Result: Extensive experiments show the method surpasses existing generalizable NeRFs and achieves performance comparable to scene-specific distractor-free NeRFs.

Conclusion: MU-GeNeRF effectively addresses robust modeling challenges in generalizable NeRFs by decomposing distractor awareness into complementary uncertainty components, enabling more reliable distractor suppression and geometric reconstruction.
...
TL;DR: E3VS-Bench: A new benchmark for embodied 3D visual search requiring 5-DoF viewpoint control to gather evidence for question answering, using 3D Gaussian Splatting for photorealistic rendering.


<details>
  <summary>Details</summary>
Motivation: Existing visual search benchmarks rely on static observations or constrained motion, failing to evaluate fine-grained viewpoint-dependent phenomena in real-world 3D environments like visibility changes from vertical shifts, revealing container contents, and disambiguating object attributes from specific angles.

Method: Created E3VS-Bench with 99 high-fidelity 3D scenes reconstructed using 3D Gaussian Splatting and 2,014 question-driven episodes. 3D Gaussian Splatting enables photorealistic free-viewpoint rendering preserving fine visual details that mesh-based simulators degrade.

Result: Evaluated multiple state-of-the-art VLMs and compared with humans. Despite strong 2D reasoning ability, all models show substantial gap from humans, highlighting limitations in active perception and coherent viewpoint planning under full 5-DoF viewpoint changes.

Conclusion: E3VS-Bench addresses limitations of existing benchmarks by enabling evaluation of embodied agents' ability to actively control viewpoints in 5-DoF for visual search tasks, revealing significant gaps between current models and human performance in active perception and viewpoint planning.
...
TL;DR: The paper introduces a framework for auditing bias in Human Action Recognition models using synthetic video data with controlled identity attributes like skin color, revealing systematic biases even when motion remains identical.


<details>
  <summary>Details</summary>
Motivation: HAR models are deployed in high-stakes environments but their fairness across different human appearances hasn't been analyzed. There's a need to audit these models for bias, especially as regulatory standards are emerging.

Method: Uses synthetic video data generated with full control over visual identity attributes (skin color) via BEDLAM simulation platform. Preserves temporal consistency to isolate how single attribute changes affect predictions, unlike prior work on static images or pose estimation.

Result: Shows that popular HAR models exhibit statistically significant biases based on skin color even when motion remains identical. Provides evidence of systematic errors across groups and unwanted visual associations encoded in models.

Conclusion: Contributes a framework for auditing HAR models and supports development of more transparent, accountable systems in preparation for upcoming regulatory standards on fairness and bias.
...
TL;DR: PSRD is a self-rewarding framework that dynamically mitigates vision hallucination in LVLMs at inference time without external supervision, using phase-wise self-reward signals and a distilled lightweight reward model.


<details>
  <summary>Details</summary>
Motivation: LVLMs suffer from vision hallucination where responses don't match visual input. Existing methods require expensive fine-tuning or use static post-hoc strategies that ignore the dynamic nature of hallucination emergence.

Method: PSRD (Phase-wise Self-Reward Decoding) uses phase-wise self-reward signals for online hallucination correction. A lightweight reward model is distilled from LVLMs to provide on-the-fly guidance during decoding, enabling precise hallucination suppression without repeated self-evaluation.

Result: PSRD reduces hallucination rate of LLaVA-1.5-7B by 50.0% and consistently outperforms existing post-hoc methods across five hallucination evaluation benchmarks for four LVLMs. It effectively mitigates hallucination propagation and achieves controllable trade-off between performance and inference efficiency.

Conclusion: The proposed self-rewarding framework enables dynamic hallucination mitigation at inference time without external supervision, offering an efficient and effective solution to vision hallucination in LVLMs.
...
TL;DR: A framework for making super-resolution systems more trustworthy by predicting reconstruction errors and generating conformal failure masks with theoretical guarantees for safety-critical medical applications.


<details>
  <summary>Details</summary>
Motivation: Super-resolution models for medical imaging (endoscopic/robotic surgery) face reliability concerns due to hallucinated structures and amplified noise, limiting their use in safety-critical settings where trustworthiness is essential.

Method: Integrates a lightweight error-prediction network that operates on intermediate representations to estimate pixel-wise reconstruction error. Converts predictions into Conformal Failure Masks (CFM) using conformal risk control principles to localize unreliable regions with theoretical guarantees.

Result: Demonstrates effectiveness in detecting unreliable reconstructions in endoscopic and robotic surgery settings for both image and video super-resolution. The approach is computationally efficient and suitable for real-time deployment.

Conclusion: First model-agnostic, theoretically grounded approach to improve safety of real-time endoscopic image super-resolution, providing a practical framework for making SR systems more trustworthy in medical applications.
...
TL;DR: MV-HGNN: A novel framework for sketch-based 3D shape retrieval using multi-view hierarchical graph neural networks with CLIP semantic alignment, achieving state-of-the-art performance in both category-level and zero-shot settings.


<details>
  <summary>Details</summary>
Motivation: Existing SBSR methods have two main limitations: 1) They use simplified aggregation strategies for 3D multi-view features that ignore geometric relationships between views and multi-level details, resulting in weak 3D representation. 2) Traditional methods are constrained by visible category limitations, leading to poor performance in zero-shot scenarios.

Method: Proposes Multi-View Hierarchical Graph Neural Network (MV-HGNN) with: 1) View-level graph construction capturing adjacent geometric dependencies and cross-view message passing via local graph convolution and global attention. 2) View selector for hierarchical graph coarsening to increase receptive field and reduce redundant view interference. 3) CLIP text embeddings as semantic prototypes to project sketch and 3D features into shared semantic space for category-agnostic alignment. Uses two-stage training for category-level retrieval and one-stage for zero-shot retrieval.

Result: Extensive experiments on two public benchmarks show MV-HGNN outperforms state-of-the-art methods under both category-level and zero-shot settings.

Conclusion: MV-HGNN effectively addresses the limitations of existing SBSR methods by capturing geometric relationships between views through hierarchical graph neural networks and enabling zero-shot capability through CLIP-based semantic alignment, demonstrating superior performance across different retrieval scenarios.
...
TL;DR: CFSR is a physics-constrained shadow removal framework that integrates 3D geometric cues with foundation model semantics to bridge the 2D-3D domain gap, achieving state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Traditional shadow removal networks lack physical interpretability and struggle to balance localized texture recovery with global illumination consistency, treating restoration as unconstrained mapping without physical constraints.

Method: Proposes CFSR with: 1) HVI color space mapping to suppress shadow noise and fuse RGB with depth priors; 2) Geometric & Semantic Dual Explicit Guided Attention using DINO features and 3D normals to modulate attention affinity matrix; 3) Holistic priors injection via frozen CLIP encoder; 4) Frequency Collaborative Reconstruction Module (FCRM) that decouples decoding to harmonize high-frequency boundaries with low-frequency illumination.

Result: Extensive experiments show CFSR achieves state-of-the-art performance across multiple challenging benchmarks, demonstrating superior shadow removal capabilities.

Conclusion: CFSR successfully reframes shadow removal as a physics-constrained restoration process by integrating 3D geometric cues with foundation model semantics, effectively bridging the 2D-3D domain gap and achieving optimal balance between local texture recovery and global illumination consistency.
...
TL;DR: HABIT is a robust progressive learning framework for composed image retrieval that addresses the Noise Triplet Correspondence problem through mutual knowledge estimation and dual-consistency progressive learning.


<details>
  <summary>Details</summary>
Motivation: Composed Image Retrieval faces practical challenges due to Noise Triplet Correspondence (NTC) problem caused by expensive and subjective triplet annotation, leading to noisy training data.

Method: Proposes HABIT framework with two modules: 1) Mutual Knowledge Estimation Module quantifies sample cleanliness via Transition Rate of mutual information; 2) Dual-consistency Progressive Learning Module uses historical-current model collaboration to simulate human habit formation for robust learning.

Result: Extensive experiments on two standard CIR datasets show HABIT significantly outperforms most methods under various noise ratios, demonstrating superior robustness and retrieval performance.

Conclusion: HABIT effectively addresses NTC problem in CIR through noise-aware sample selection and progressive learning, providing a robust solution for practical deployment of composed image retrieval systems.
...
TL;DR: GS-STVSR is an ultra-efficient continuous spatio-temporal video super-resolution framework using 2D Gaussian Splatting that bypasses dense grid queries, achieving state-of-the-art quality with constant inference time and 3x speedup at extreme scales.


<details>
  <summary>Details</summary>
Motivation: Existing C-STVSR methods based on Implicit Neural Representations suffer from computational inefficiency due to dense pixel-wise grid queries that scale linearly with interpolated frames, severely limiting practical inference efficiency.

Method: Proposes GS-STVSR using 2D Gaussian Splatting with continuous motion modeling: exploits temporal stability of covariance for lightweight fitting, optical flow-guided motion module for arbitrary time steps, covariance resampling alignment to prevent drift, and adaptive offset window for large-scale motion.

Result: Achieves state-of-the-art quality on Vid4, GoPro, and Adobe240 benchmarks. Inference time remains nearly constant at conventional scales (X2-X8) and delivers over 3x speedup at extreme scale X32.

Conclusion: GS-STVSR provides an ultra-efficient C-STVSR framework that overcomes computational limitations of INR-based methods while maintaining high quality, demonstrating strong practical applicability with constant inference time.
...
TL;DR: INTENT addresses noisy triplet correspondence in composed image retrieval by handling both cross-modal correspondence noise and modality-inherent noise through visual invariant composition and bi-objective discriminative learning.


<details>
  <summary>Details</summary>
Motivation: Real-world CIR datasets contain annotation errors due to high triplet annotation costs, leading to incorrectly matched triplets. Existing methods assume perfect matches, overlooking two types of noise: cross-modal correspondence noise (mismatches across modalities) and modality-inherent noise (intra-modal background interference or irrelevant visual factors).

Method: INTENT has two components: 1) Visual Invariant Composition applies causal intervention via Fast Fourier Transform to generate intervened composed features, enforcing visual invariance to ignore modality-inherent noise. 2) Bi-Objective Discriminative Learning uses collaborative optimization with both positive and negative samples, constructing a scalable decision boundary that dynamically adjusts based on loyalty degree for robust correspondence discrimination.

Result: Extensive experiments on two widely used benchmark datasets demonstrate the superiority and robustness of INTENT in handling noisy triplet correspondence.

Conclusion: INTENT effectively addresses both cross-modal correspondence noise and modality-inherent noise in CIR, providing a robust solution for real-world scenarios with annotation errors through its dual-component architecture combining visual invariance and discriminative learning.
...
TL;DR: DPW introduces dynamic prefix weighting with adapters for domain-class incremental learning in VLMs, outperforming previous methods by adaptively adjusting prefix weights based on token importance.


<details>
  <summary>Details</summary>
Motivation: Previous parameter-efficient methods for domain-class incremental learning in vision-language models use fixed normalization for prefix weights, ignoring that different input tokens need different degrees of adjustment.

Method: Dynamic Prefix Weighting (DPW) framework with: 1) gating module that adjusts prefix weights based on token importance, and 2) weighting mechanism that derives adapter output weights as residual of prefix-tuning weights, ensuring adapters are used only when needed.

Result: Achieves state-of-the-art performance in domain-class incremental learning scenarios for vision-language models.

Conclusion: DPW effectively addresses the limitation of fixed prefix weighting by dynamically adjusting weights based on token importance, leading to superior performance in incremental learning for VLMs.
...
TL;DR: Diffusion-generated synthetic images improve military vehicle detection in low-data scenarios, with structural guidance via ControlNet providing additional benefits when real data is extremely limited.


<details>
  <summary>Details</summary>
Motivation: To investigate whether diffusion-generated synthetic images can improve military vehicle detection under low-data conditions, addressing the challenge of limited training data for specialized domains like military applications.

Method: Fine-tuned FLUX.1 diffusion model using LoRA with only 8 or 24 real images per class across 15 vehicle categories, generated synthetic data from text prompts. Also created structurally guided synthetic data using ControlNet with Canny edge-map conditioning (FLUX-CN). Used same real images to fine-tune RF-DETR detector and tested with synthetic datasets.

Result: FLUX-generated images improved detection performance, especially in low-data regime (up to +8.0% mAP50 with 8 real samples). Structural guidance via FLUX-CN provided additional +4.1% mAP50 improvement with 8 real samples, but no additional benefit with more real data available.

Conclusion: Object-specific diffusion models are effective for improving military object detection in low-data domains, with structural guidance most beneficial when real data is highly limited. Generative image data offers an alternative to traditional simulation pipelines for training military AI systems.
...
TL;DR: Researchers propose a drone-in-a-box system for water rescue, using YOLO-based object detection to locate drowning persons and discrete-event simulation to optimize drone configurations, showing significant response time reductions.


<details>
  <summary>Details</summary>
Motivation: Water rescue faces challenges including time pressure, unsupervised swimming areas, difficulty localizing targets, and transporting rescue personnel. Technical innovations like drone systems can address these issues by providing faster response times and automated search capabilities.

Method: 1) Developed a dataset for drowning person detection and trained YOLO versions 3, 5, and 8 with different architecture sizes (nano to extra-large), evaluating performance using mAP metrics. 2) Created two Discrete-Event Simulation approaches to model response times for both Standard Rescue Operations and UAS-based systems, enabling optimization of drone configurations (type, number, location).

Result: YOLO-based object detection showed promising performance for locating distressed swimmers. Simulation experiments in Germany's Lusatian Lake District demonstrated that UAS assistance significantly shortens response times - even a small system with two hangars (each containing one UAV) reduced response time by a factor of five compared to standard rescue operations.

Conclusion: Drone-in-a-box systems with automated object detection using YOLO can dramatically improve water rescue response times. The combination of computer vision for target localization and discrete-event simulation for system optimization provides a comprehensive approach to enhancing drowning rescue operations.
...
TL;DR: DAP (Decision-Aware Attention Propagation) improves ViT interpretability by combining gradient-based token importance with attention rollout for more class-sensitive and faithful attribution maps.


<details>
  <summary>Details</summary>
Motivation: Current ViT explanation methods have limitations: attention-based methods rely on raw attention weights that lack class discriminability, while gradient-based methods don't leverage transformer attention propagation mechanisms.

Method: DAP integrates gradient-based token importance estimation into layer-wise attention rollout, injecting decision-relevant priors into transformer attention propagation to capture both structural flow and class-specific evidence.

Result: DAP consistently outperforms existing baselines across ViT variants of different scales in both quantitative metrics and qualitative visualizations, producing more class-sensitive, compact, and faithful attribution maps.

Conclusion: Decision-aware propagation is an effective direction for improving ViT interpretability, successfully combining the strengths of attention-based and gradient-based explanation methods.
...
TL;DR: PDF is a test-time adaptation framework for Vision-Language-Action models that improves robustness to environmental shifts without fine-tuning, using uncertainty-based data augmentation, action voting, and delayed feedback correction.


<details>
  <summary>Details</summary>
Motivation: VLAs perform well in sequential decision-making but are fragile to subtle environmental changes due to trajectory overfitting - they over-attend to spurious correlations between actions and entities, reproducing memorized action patterns.

Method: PDF uses uncertainty-based data augmentation and action voting to mitigate spurious correlations, an adaptive scheduler for augmentation budget allocation, and a lightweight perturbation module that retrospectively adjusts action logits using delayed feedback to correct overconfidence.

Result: Experiments show consistent gains: +7.4% success rate on LIBERO and +10.3 human normalized score on Atari, outperforming vanilla VLA and VLA with test-time adaptation.

Conclusion: PDF establishes a practical path toward reliable test-time adaptation in multimodal decision-making agents without requiring verifiers or fine-tuning base models.
...
TL;DR: SurgLIME is a parameter-efficient vision-language pre-training framework that uses LLM-generated surgical narratives to enable scalable multi-modal learning while mitigating noise through confidence estimation and preserving visual medical priors.


<details>
  <summary>Details</summary>
Motivation: Extending surgical vision encoders to multi-modal reasoning is bottlenecked by the prohibitive cost of expert textual annotations. While LLM-generated narratives offer scalability, they contain errors/hallucinations that could degrade medical priors in standard contrastive learning.

Method: Proposes SurgLIME: 1) Uses LoRA-adapted dual-encoder architecture to preserve foundational medical priors, 2) Introduces automated confidence estimation that dynamically down-weights uncertain text during contrastive alignment, 3) Leverages LIME dataset with LLM-generated surgical narratives from open-access videos.

Result: Achieves competitive zero-shot cross-modal alignment on AutoLaparo and Cholec80 benchmarks while preserving robust linear probing performance of the visual foundation model. Enables scalable multi-modal learning without expensive expert annotations.

Conclusion: SurgLIME provides an effective solution for scalable surgical vision-language pre-training by mitigating noise in LLM-generated narratives while preserving medical priors, enabling multi-modal reasoning without costly expert annotations.
...
TL;DR: LPQLD reduces soft label storage by 78-500x while improving accuracy in large-scale dataset distillation by addressing insufficient image and supervision diversity.


<details>
  <summary>Details</summary>
Motivation: Large-scale dataset distillation requires storing massive soft labels (30-40x larger on ImageNet-1K, 200x larger on ImageNet-21K than condensed images), undermining dataset compression goals. Two fundamental issues: insufficient image diversity (high within-class similarity requiring extensive augmentation) and insufficient supervision diversity (limited supervisory signal variety causing performance degradation at high compression rates).

Method: LPQLD (Label Pruning and Quantization for Large-scale Distillation). Enhances image diversity via class-wise batching and batch-normalization supervision during synthesis. For supervision diversity: Label Pruning with Dynamic Knowledge Reuse improves label-per-augmentation diversity; Label Quantization with Calibrated Student-Teacher Alignment improves augmentation-per-image diversity.

Result: Reduces soft label storage by 78x on ImageNet-1K and 500x on ImageNet-21K while improving accuracy by up to 7.2% and 2.8%, respectively. Extensive experiments validate superiority across different network architectures and dataset distillation methods.

Conclusion: LPQLD effectively addresses storage and diversity challenges in large-scale dataset distillation, achieving significant storage reduction with improved accuracy, making dataset distillation more practical for large-scale applications.
...
TL;DR: VietPET-RoI: First large-scale 3D PET/CT dataset with fine-grained RoI annotations for low-resource languages (Vietnamese), paired with HiRRA framework that mimics radiologist workflow using graph-based relational modules, achieving SOTA performance with significant improvements in clinical metrics.


<details>
  <summary>Details</summary>
Motivation: Current automated medical report generation methods for 3D PET/CT imaging face challenges: high-dimensional volumetric data, scarcity of annotated datasets (especially for low-resource languages), and black-box approaches that ignore clinical workflow of analyzing localized Regions of Interest (RoIs) to derive diagnostic conclusions.

Method: 1) Created VietPET-RoI dataset: 600 PET/CT samples with 1,960 manually annotated RoIs paired with clinical reports. 2) Proposed HiRRA framework: mimics radiologist diagnostic workflow using graph-based relational modules to capture dependencies between RoI attributes, shifting from global pattern matching to localized clinical findings. 3) Introduced new clinical evaluation metrics: RoI Coverage and RoI Quality Index using LLM-based extraction.

Result: Framework achieves state-of-the-art performance: surpasses existing models by 19.7% in BLEU and 4.7% in ROUGE-L, with remarkable 45.8% improvement in clinical metrics. Demonstrates enhanced clinical reliability and reduced hallucination.

Conclusion: The work bridges critical gaps in medical report generation by providing the first large-scale RoI-annotated dataset for low-resource languages and a clinically-inspired framework that improves both automated metrics and clinical relevance. The approach shifts focus from global pattern matching to localized clinical analysis, better aligning with actual radiologist workflow.
...
TL;DR: Attention-ResUNet achieves state-of-the-art fetal head segmentation in ultrasound images with 99.30% Dice score by combining residual learning with multi-scale attention mechanisms.


<details>
  <summary>Details</summary>
Motivation: Existing deep learning methods for fetal head segmentation in ultrasound struggle with low contrast, noise, and complex anatomical boundaries inherent to ultrasound imaging, requiring improved accuracy and robustness.

Method: Proposes Attention-ResUNet architecture that synergistically combines residual learning with multi-scale attention mechanisms. Integrates attention gates at four decoder levels to focus on anatomically relevant regions while suppressing background noise, complemented by residual connections for gradient flow and feature reuse.

Result: Achieves superior performance with mean Dice score of 99.30% ± 0.14% on HC18 Challenge dataset (n=200), significantly outperforming five baseline architectures. Statistical analysis confirms highly significant improvements (p < 0.001) with effect sizes ranging from 0.230 to 13.159 (Cohen's d).

Conclusion: Attention-ResUNet establishes new state-of-the-art for automated fetal head segmentation while maintaining computational efficiency (14.7M parameters, 45 GFLOPs). Saliency map analysis shows anatomically consistent activation patterns, enhancing interpretability crucial for clinical deployment.
...
TL;DR: AI-powered waste mapping using aerial/street-view imagery detects waste accumulation patterns in Dar es Salaam, revealing waste in waterways is 3x higher than adjacent areas, creating flood risk hotspots.


<details>
  <summary>Details</summary>
Motivation: Urban flooding is increasing in African cities due to climate change, exacerbated by poor waste management that blocks drainage systems. Traditional manual mapping methods are inadequate for city-wide monitoring of waste accumulation patterns that amplify flood risks.

Method: Developed an AI-powered urban waste mapping workflow using openly available aerial and street-view imagery to detect municipal solid waste at high resolution. Collaborated with local partners for culturally relevant data labeling that reflects real-world waste reuse practices.

Result: Applied in Dar es Salaam, Tanzania, revealing spatial waste patterns linked to informal settlements and socio-economic factors. Waste accumulation in waterways was found to be up to three times higher than in adjacent urban areas, identifying critical hotspots for climate-exacerbated flooding.

Conclusion: The scalable AI approach enables city-wide monitoring and prioritization of interventions, offering actionable insights for urban planning, climate adaptation, and sustainable waste management in flood-prone urban areas.
...
TL;DR: Inference-time method mitigates social bias in text-to-image models while preserving prompt semantics and visual context, using embedding space analysis without altering model weights.


<details>
  <summary>Details</summary>
Motivation: Modern text-to-image models amplify harmful societal biases, challenging their ethical deployment. There's a need for methods that can mitigate bias while maintaining prompt semantics and visual context.

Method: Uses Embedding Arithmetic to analyze bias structure in embedding space and correct it without altering model weights, prompts, or datasets. The method operates at inference time, provides controllable parameter for mitigation strength, and maintains context persistency.

Result: Shows that conditional embedding space forms a complex, entangled manifold rather than a grid of disentangled concepts. The method significantly outperforms existing baselines in improving diversity while maintaining high concept coherence as measured by the proposed Concept Coherence Score (CCS).

Conclusion: By characterizing how models represent social concepts, the paper establishes geometric understanding of latent space as a principled path toward more transparent, controllable, and fair image generation, effectively resolving the fairness-coherence trade-off.
...
TL;DR: This paper extends MeanFlow's one-step generation from class labels to text conditions, addressing challenges in integrating LLM-based text encoders by analyzing discriminability requirements and adapting the framework for efficient text-conditioned synthesis.


<details>
  <summary>Details</summary>
Motivation: While MeanFlow achieves impressive one-step generation for class-to-image tasks, extending it to flexible text conditions would enable richer content creation. However, integrating powerful text encoders into the MeanFlow framework presents unexpected challenges that need investigation.

Method: The authors analyze why conventional training strategies fail with LLM-based text encoders in MeanFlow, discovering that limited refinement steps require highly discriminative text features. They then leverage a powerful LLM-based text encoder with suitable semantic properties and adapt the MeanFlow generation process to this framework.

Result: The approach successfully enables efficient text-conditioned synthesis for the first time within the MeanFlow framework. The method is also validated on diffusion models, showing significant generation performance improvements.

Conclusion: This work provides a practical reference for text-conditioned MeanFlow generation by addressing the discriminability requirements of text features in few-step generation frameworks, opening new directions for efficient text-to-image synthesis.
...
TL;DR: CanonSLR: A canonical-view guided framework for multi-view continuous sign language recognition that uses frontal-view supervision and distillation to improve robustness across viewpoints.


<details>
  <summary>Details</summary>
Motivation: Existing CSLR methods are developed under single-view settings and lack robustness to viewpoint variations in real-world scenarios. There's a need for multi-view approaches that can handle different camera angles.

Method: 1) Frontal-view-anchored teacher-student learning with canonical temporal supervision; 2) Sequence-Level Soft-Target Distillation to reduce cross-view semantic discrepancy; 3) Temporal Motion Relational Enhancement to model motion-aware temporal relations; 4) A universal multi-view sign language data construction pipeline to create multi-view datasets.

Result: Created two seven-view benchmarks (PT14-MV and CSL-MV) from PHOENIX-2014T and CSL-Daily. CanonSLR consistently outperforms existing approaches under multi-view settings and exhibits stronger robustness, especially on challenging non-frontal views.

Conclusion: CanonSLR effectively addresses viewpoint variation challenges in CSLR through canonical-view guidance, distillation techniques, and motion modeling, while providing new multi-view benchmarks for future research.
...
TL;DR: A hybrid pipeline combining diffusion models with segmentation models (RemoteSAM/SAM3) for improved object grounding in remote sensing imagery.


<details>
  <summary>Details</summary>
Motivation: To explore the potential of diffusion models for object grounding in remote sensing imagery, leveraging their generative capabilities to enhance localization accuracy.

Method: Proposes a hybrid pipeline that integrates diffusion-based localization cues with state-of-the-art segmentation models (RemoteSAM and SAM3) to obtain more accurate bounding boxes.

Result: Significantly improves localization performance, achieving over 14% increase in Acc@0.5 compared to existing state-of-the-art methods.

Conclusion: The complementary strengths of generative diffusion models and foundational segmentation models enable robust and adaptive object localization across complex remote sensing scenes.
...
TL;DR: The paper introduces a benchmark and evaluation pipeline for neural rendering methods that focuses on geometric accuracy rather than just visual quality, addressing a gap in current evaluation metrics.


<details>
  <summary>Details</summary>
Motivation: Current neural rendering evaluation metrics focus on visual quality but overlook geometric accuracy, which is critical for robotics applications like grasping and manipulation where accurate surface geometry is essential.

Method: The authors present an evaluation pipeline specifically designed to assess geometric accuracy in neural rendering, along with a benchmark dataset comprising 19 diverse scenes to enable systematic assessment of reconstruction methods.

Result: The paper provides a comprehensive benchmark and evaluation framework that complements traditional visual metrics by focusing on surface and shape fidelity, enabling better assessment of neural rendering methods for applications requiring accurate geometry.

Conclusion: The proposed evaluation pipeline and benchmark fill an important gap in neural rendering evaluation by providing tools to systematically assess geometric accuracy, which is crucial for robotics and other applications where precise surface geometry matters more than just visual appearance.
...
TL;DR: SARR: A novel rotation representation that resolves symmetry ambiguities in 6D pose estimation by modifying trigonometric identities with object symmetry degrees, enabling standard CNNs to achieve state-of-the-art performance.


<details>
  <summary>Details</summary>
Motivation: Symmetric objects create orientation ambiguities that hinder deep learning pose estimation. Existing solutions require specialized loss functions, architectures, or symmetry-invariant metrics, but the fundamental issue lies in how rotations are numerically represented.

Method: Proposes SARR (Symmetry-Aware Rotation Representation) that modifies trigonometric identities using degrees of symmetry derived from object shapes. This creates canonic (symmetry-resolved) poses that are unique and continuous with visual appearance. Uses standard CNN for 3D orientation estimation without requiring 3D models.

Result: Outperforms state-of-the-art using symmetry-sensitive cosine distance (AR_C) on T-LESS and ITODD datasets. Achieves satisfactory performance with conventional symmetry-invariant measures. Networks trained on SARR outperform those trained on rotation matrices, Euler angles, quaternions, standard trigonometrics, or 6D representation, even without prior symmetry knowledge.

Conclusion: SARR effectively resolves symmetry ambiguities at the representation level, enabling standard CNNs to excel at symmetric object pose estimation. The approach works with depth or texture-less RGB/grayscale images and doesn't require 3D models.
...
TL;DR: A decoupled framework for spatially consistent long-horizon video generation that separates memory conditioning from generation, using a lightweight memory branch with hybrid representation and camera-aware gating to maintain consistency while preserving generative capacity for novel scenes.


<details>
  <summary>Details</summary>
Motivation: Existing methods entangle memory modeling with video generation, causing inconsistent content during scene revisits and reduced generative capacity for novel regions, even with extensive training data. There's a need for a more efficient approach that maintains spatial consistency while preserving exploration capability.

Method: Proposes a decoupled framework with: 1) Lightweight independent memory branch using hybrid memory representation to capture temporal/spatial cues, 2) Per-frame cross-attention to condition each frame on most spatially relevant historical information, 3) Camera-aware gating mechanism to mediate memory-generation interaction, enabling conditioning only when meaningful historical references exist.

Result: Achieves state-of-the-art performance in visual quality and spatial consistency while being highly data-efficient (significantly reduces training costs compared to existing methods).

Conclusion: The decoupled approach successfully addresses limitations of existing methods by separating memory conditioning from generation, enabling both consistent scene revisits and effective novel scene exploration with reduced training requirements.
...
TL;DR: S-EGIU introduces a dynamic instruction understanding approach for Vision-and-Language Navigation where instruction meaning evolves step-by-step based on visual context, improving navigation performance.


<details>
  <summary>Details</summary>
Motivation: Existing VLN models encode instructions as static global representations, which limits their ability to adapt instruction meaning to evolving visual contexts as the agent navigates through changing environments.

Method: S-EGIU models instruction understanding as an Instruction-as-State variable that evolves conditioned on perceptual state. It uses a coarse-to-fine framework: coarse-level activates instruction segments aligned with current observation, fine-level refines activated segments through observation-guided token grounding and contextual modeling.

Result: Achieves +2.68% SPL gain on REVERIE Test Unseen and demonstrates consistent efficiency gains across multiple VLN benchmarks, showing the value of dynamic instruction-perception entanglement.

Conclusion: Dynamic instruction understanding that evolves with visual context is crucial for VLN, and the Instruction-as-State approach with S-EGIU effectively addresses the limitation of static instruction representations.
...
TL;DR: SAM3 shows limited capability in pathology image segmentation - text prompts poorly activate nuclear concepts, performance is sensitive to visual prompt types, and there's a significant gap compared to task-trained models.


<details>
  <summary>Details</summary>
Motivation: Traditional pathology segmentation methods suffer from high annotation costs and poor generalization. SAM3's Promptable Concept Segmentation offers potential for automated segmentation via text prompts, but its actual capability in pathology domains needs systematic evaluation.

Method: Proposed systematic evaluation protocol for SAM3 with different supervision settings: zero-shot, few-shot, and supervised learning with varying prompting strategies. Evaluated on pathological datasets (NuInsSeg, PanNuke, GlaS) to explore SAM3's capability space.

Result: 1. Text-only prompts poorly activate nuclear concepts. 2. Performance highly sensitive to visual prompt types and budgets. 3. Few-shot learning offers gains but SAM3 lacks robustness against visual prompt noise. 4. Significant gap persists between prompt-based usage and task-trained adapter-based reference.

Conclusion: SAM3 has clear boundaries in pathology image segmentation. The study provides practical guidance showing the necessity of pathology domain adaptation for effective use in medical imaging tasks.
...
TL;DR: Vision-language framework for 3D CT image understanding using visual instruction tuning with radiology reports, adapted for survival prediction with improved performance over baselines.


<details>
  <summary>Details</summary>
Motivation: Radiologist-assessed CT features are valuable for prognostication but require expert knowledge, and textual summaries of rich visual information lead to information loss. Need for better integration of visual and textual data for clinical decision-making.

Method: Proposes vision-language framework using large-scale CT images paired with radiology reports through visual instruction tuning. Learns clinically meaningful visual-textual representations, then adapts to survival prediction tasks by adding survival prediction head on pre-trained model.

Result: Method outperforms baseline methods in survival prediction, particularly when clinical data alone is less predictive. Generates clinically meaningful language responses to predefined questions alongside survival predictions.

Conclusion: Vision-language framework effectively leverages CT images and radiology reports for improved survival prediction, demonstrating value of multimodal learning in medical imaging applications.
...
TL;DR: Three neural network architectures for real-time weather classification from images using style transfer concepts, with two models outperforming state-of-the-art and showing strong generalization.


<details>
  <summary>Details</summary>
Motivation: To develop efficient neural network architectures for real-time classification of weather conditions (sunny, rain, snow, fog) from images by leveraging style transfer concepts to capture stylistic elements in images.

Method: Three architectures: 1) Multi-PatchGAN (adapted PatchGAN with multiple patch sizes), 2) Truncated ResNet50 (simplified ResNet50 with first 9 layers determined by evolutionary algorithm), 3) Truncated ResNet50 with Gram Matrix and Attention (computes Gram matrices per layer with attention weighting).

Result: The last two models (Truncated ResNet50 variants) outperform state-of-the-art methods and demonstrate remarkable generalization capability on several public databases.

Conclusion: The proposed architectures are effective for weather detection and are also suitable for other appearance-based classification tasks including animal species recognition, texture classification, medical imaging, and industrial defect identification.
...
TL;DR: Mixture-of-Experts (MoE) architecture for object detection that combines YOLO-based detectors trained on semantically disjoint data subsets, outperforming standard ensembles and providing better interpretability.


<details>
  <summary>Details</summary>
Motivation: MoE models offer structured combination of specialized networks with better interpretability than conventional ensembles, but their application to object detection has been limited due to challenges in merging dense and structured predictions.

Method: Proposed MoE architecture combining YOLO-based detectors trained on semantically disjoint data subsets, with learned gating network that dynamically weights expert contributions. Includes strategies for fusing detection outputs and training gating mechanism with balancing losses to prevent expert collapse.

Result: Experiments on BDD100K dataset show the proposed MoE consistently outperforms standard ensemble approaches and provides insights into expert specialization across domains.

Conclusion: Model-level MoEs are a viable alternative to traditional ensembling for object detection, offering improved performance and interpretability through expert specialization.
...
TL;DR: PRISM enables pre-trained text-to-image models to handle long descriptive paragraphs by decomposing them into components, making independent noise predictions, and merging outputs via energy-based conjunction.


<details>
  <summary>Details</summary>
Motivation: Current text-to-image models struggle with long descriptive paragraphs because they're trained on short captions. Existing methods either fine-tune models (poor generalization) or compress long prompts (loss of fidelity).

Method: PRISM uses a lightweight module to extract constituent representations from long prompts. The T2I model makes independent noise predictions for each component, then merges outputs using energy-based conjunction in a single denoising step.

Result: PRISM achieves comparable performance to models fine-tuned on same training data across various architectures, and outperforms baselines by 7.4% on prompts over 500 tokens in challenging benchmarks.

Conclusion: PRISM provides an effective compositional approach for handling long prompts in text-to-image generation without requiring extensive fine-tuning, offering better generalization while maintaining fidelity.
...
TL;DR: Geo3DPruner is a geometry-guided 3D visual token pruning framework that reduces 90% of tokens while maintaining over 90% of original performance for 3D scene understanding with multimodal LLMs.


<details>
  <summary>Details</summary>
Motivation: Multimodal LLMs show great 2D vision capabilities but struggle with 3D scene understanding due to large numbers of visual tokens in 3D spatial videos. Existing pruning methods fail to consider view consistency and spatial diversity, leading to inefficient redundancy removal and poor scene preservation.

Method: Geo3DPruner uses geometry-aware global attention to model cross-frame relevance, then performs two-stage pruning: 1) intra-voxel stage selects representative multi-view features within each voxel, 2) inter-voxel stage preserves spatial diversity by selecting globally distributed voxels.

Result: Extensive experiments on multiple 3D scene understanding benchmarks show Geo3DPruner retains over 90% of original performance while pruning 90% of visual tokens, significantly outperforming existing text-guided and vision-guided pruning methods.

Conclusion: Geo3DPruner effectively addresses the token bottleneck in 3D spatial video processing by leveraging geometric relationships to remove redundancy while preserving scene completeness, enabling efficient 3D reasoning with multimodal LLMs.
...
TL;DR: MARCO is a unified model for generalizable semantic correspondence that improves both fine-grained localization and semantic generalization using a coarse-to-fine objective and self-distillation framework, outperforming billion-parameter diffusion models while being 3x smaller and 10x faster.


<details>
  <summary>Details</summary>
Motivation: Current semantic correspondence models (dual-encoder architectures combining DINOv2 with diffusion backbones) have poor generalization beyond training keypoints, creating a gap between benchmark performance and real-world usability where queried points rarely match those seen during training.

Method: Builds upon DINOv2 with a novel training framework featuring: 1) coarse-to-fine objective for refining spatial precision, and 2) self-distillation framework that expands sparse supervision beyond annotated regions to transform few keypoints into dense, semantically coherent correspondences.

Result: Sets new SOTA on SPair-71k, AP-10K, and PF-PASCAL with significant gains at fine-grained localization thresholds (+8.9 PCK@0.01), strongest generalization to unseen keypoints (+5.1 on SPair-U) and categories (+4.7 on MP-100), while being 3x smaller and 10x faster than diffusion-based approaches.

Conclusion: MARCO demonstrates that enhanced training frameworks can bridge the generalization gap in semantic correspondence, achieving superior performance with more efficient architectures compared to billion-parameter diffusion models.
...
TL;DR: LiquidTAD: A parameter-efficient TAD framework using parallelized ActionLiquid blocks with O(N) linear complexity, achieving competitive performance with 63% fewer parameters than baselines.


<details>
  <summary>Details</summary>
Motivation: Current Transformer-based TAD models have quadratic computational complexity and parameter redundancy, limiting deployment in resource-constrained environments.

Method: Replaces self-attention layers with parallelized ActionLiquid blocks using closed-form continuous-time formulation, enabling O(N) linear complexity and adaptive temporal sensitivity modulation via learned time-constants.

Result: Achieves 69.46% Average mAP on THUMOS-14 with only 10.82M parameters (63% reduction vs ActionFormer), optimal accuracy-efficiency trade-off on ActivityNet-1.3 and Ego4D, and superior robustness to temporal sampling variations.

Conclusion: LiquidTAD advances the Pareto frontier of TAD frameworks by introducing the first parallelized LNN-based architecture to the domain, offering efficient temporal dependency modeling with linear complexity.
...
TL;DR: Spike-NVPT: A noise-robust visual prompt tuning method using spiking neurons to filter noise and generate sparse binary prompts, achieving better robustness with zero inference overhead.


<details>
  <summary>Details</summary>
Motivation: Standard prompt tuning methods are sensitive to input noise because their continuous, dense prompts tend to overfit task-irrelevant details, creating a robustness-accuracy trade-off.

Method: Uses spiking neurons with integrate-and-fire mechanism to accumulate task-relevant signals and filter transient noise. Includes a Spike Discretization Unit to convert filtered signals into sparse binary prompts that act as strong regularizers.

Result: Achieves superior robustness with up to 11.2% improvement over conventional methods while maintaining competitive accuracy on clean datasets. Binary prompts are static during deployment, ensuring zero additional computational overhead.

Conclusion: First work to leverage spiking neurons for fine-tuning ANN-based visual models, demonstrating that spiking mechanisms can effectively address noise robustness in prompt tuning while maintaining efficiency.
...
TL;DR: DFAlign is a diffusion-based framework for Open-Vocabulary Temporal Action Detection that generates foreground knowledge to bridge the semantic gap between abstract action labels and complex video content.


<details>
  <summary>Details</summary>
Motivation: Existing methods struggle with semantic imbalance between concise action labels and rich video content, introducing semantic noise and misleading cross-modal alignment in OV-TAD tasks.

Method: Proposes DFAlign with three modules: 1) Semantic-Unify Conditioning (SUC) unifies action-shared and action-specific semantics as diffusion conditions, 2) Background-Suppress Denoising (BSD) generates foreground knowledge by removing background redundancy, 3) Foreground-Prompt Alignment (FPA) injects foreground knowledge as prompt tokens to guide attention.

Result: Achieves state-of-the-art performance on two OV-TAD benchmarks, demonstrating effectiveness in mitigating semantic gaps and enhancing discriminability of action-relevant segments.

Conclusion: DFAlign successfully addresses semantic imbalance in OV-TAD by leveraging diffusion-based denoising to generate foreground knowledge as intermediate semantic anchors, enabling more precise cross-modal alignment between video and text representations.
...
TL;DR: EVE introduces a self-evolution framework for MLLMs that uses executable visual transformations instead of pseudo-labels, featuring a Challenger-Solver dual-policy architecture with verifiable ground-truth answers.


<details>
  <summary>Details</summary>
Motivation: Current MLLM self-evolution methods have limitations: pseudo-label approaches suffer from quality degradation due to prediction drift, while template-based methods are limited to static transformations that can't adapt in difficulty or diversity. Robust self-improvement requires deterministic external feedback and mechanisms to diversify training distributions.

Method: EVE uses executable visual transformations with a Challenger-Solver dual-policy architecture. The Challenger maintains a queue of visual transformation code examples, synthesizes novel Python scripts for dynamic transformations, and executes them to create VQA problems with execution-verified ground-truth answers. A multi-dimensional reward system guides the Challenger to enrich its code queue while posing progressively challenging tasks.

Result: Extensive experiments show EVE consistently surpasses existing self-evolution methods, establishing a robust and scalable paradigm for verifiable MLLM self-evolution.

Conclusion: EVE provides a novel framework for MLLM self-evolution that bypasses pseudo-labels entirely, using executable visual transformations with verifiable ground truth, enabling continuous improvement through reciprocal co-evolution of Challenger and Solver policies.
...
TL;DR: OmniHuman is a large-scale dataset for human-centric video generation addressing limitations in scene diversity, interaction modeling, and attribute alignment, with a comprehensive benchmark for evaluation.


<details>
  <summary>Details</summary>
Motivation: Existing audio-video joint generation models struggle with high-fidelity human-centric videos in complex real-world scenes due to structural deficiencies in current datasets across three dimensions: limited scene/camera diversity, sparse interaction modeling, and insufficient attribute alignment.

Method: Created OmniHuman dataset with hierarchical annotation covering video-level scenes, frame-level interactions, and individual-level attributes using a fully automated pipeline for high-quality data collection and multi-modal annotation. Also established OmniHuman Benchmark (OHBench) with a three-level evaluation system.

Result: Developed a comprehensive dataset and benchmark that addresses the identified gaps in human-centric video generation, providing tools for scientific diagnosis of audio-video synthesis models with metrics aligned with human perception.

Conclusion: OmniHuman dataset and OHBench benchmark fill critical gaps in human-centric audio-video generation by providing comprehensive data and evaluation metrics across global scenes, relational interactions, and individual attributes, enabling better model development and assessment.
...
TL;DR: AdaCluster is a training-free adaptive clustering framework that accelerates video diffusion transformers by reducing quadratic attention complexity through semantic-aware clustering of queries and keys.


<details>
  <summary>Details</summary>
Motivation: Video diffusion transformers suffer from high inference latency due to quadratic attention complexity. Existing sparse attention methods either ignore semantic similarity or fail to adapt to heterogeneous token distributions across layers, causing performance degradation.

Method: AdaCluster uses angle-similarity-preserving clustering for query vectors and euclidean-similarity-preserving clustering for keys, with adaptive cluster number assignment, threshold-wise adaptive clustering, and efficient critical cluster selection.

Result: Experiments on CogVideoX-2B, HunyuanVideo, and Wan-2.1 show 1.67-4.31x speedup on one A40 GPU with negligible quality degradation.

Conclusion: AdaCluster effectively accelerates video diffusion transformers while preserving accuracy through adaptive clustering that respects semantic similarity and layer heterogeneity.
...
TL;DR: LBFTI method reconstructs identity-preserving face images from facial templates using layer decomposition and three-stage training, outperforming SOTA methods in both machine and human perception metrics.


<details>
  <summary>Details</summary>
Motivation: Facial templates are widely used for identity authentication but are vulnerable to inversion attacks that can reconstruct faces from templates, posing severe privacy risks. Existing methods need improvement in reconstructing fine-grained, identity-preserving face images.

Method: Layer-Based Facial Template Inversion (LBFTI) decomposes faces into three layers (foreground: eyebrows/eyes/nose/mouth; midground: skin; background: other parts). Uses dedicated generators with three-stage training: 1) independent refined generation of foreground/midground layers, 2) fusion with template secondary injection to produce complete face images, 3) joint fine-tuning for inter-layer coordination and identity consistency.

Result: LBFTI outperforms state-of-the-art methods with 25.3% improvement in TAR (True Acceptance Rate) for machine authentication. Also achieves better similarity in human perception, validated by both quantitative metrics and questionnaire survey.

Conclusion: The proposed LBFTI method effectively reconstructs identity-preserving fine-grained face images from facial templates, demonstrating superior performance in both machine authentication and human perception compared to existing methods, while highlighting ongoing privacy risks in face recognition systems.
...
TL;DR: EAST is a simple, efficient framework for early action prediction that uses randomized training to handle incomplete observations, joint learning with oracle representations, and token masking for scalability, achieving SOTA results on multiple datasets.


<details>
  <summary>Details</summary>
Motivation: Early action prediction is challenging due to limited visual evidence from incomplete observations. The paper aims to develop a framework that can effectively reason about partial video information to anticipate actions before they fully unfold.

Method: EAST introduces three key components: 1) Randomized training strategy that samples a time step separating observed and unobserved frames, enabling generalization across observation ratios; 2) Joint learning on both observed and future (oracle) representations; 3) Token masking procedure that reduces memory usage by half and accelerates training 2x with minimal accuracy loss.

Result: EAST achieves state-of-the-art performance on NTU60, SSv2, and UCF101 datasets, surpassing previous best work by 10.1, 7.7, and 3.9 percentage points respectively. The framework also enables encoder-only models to excel through joint learning.

Conclusion: EAST provides an effective and scalable solution for early action prediction through randomized training, joint representation learning, and efficient token masking, significantly advancing the state of the art while maintaining simplicity and efficiency.
...
TL;DR: DSA-CycleGAN reduces noise in stain transfer for histopathology segmentation by addressing CycleGAN's limitations with one-to-many stain mappings, improving multi-stain glomeruli segmentation performance.


<details>
  <summary>Details</summary>
Motivation: Stain variations in digital histopathology reduce segmentation model performance, and labeling each stain is expensive. Existing CycleGAN methods introduce noise due to one-to-many stain mappings conflicting with cycle consistency loss.

Method: Proposed Domain Shift Aware CycleGAN (DSA-CycleGAN) that reduces noise in stain translation. Compared various machine learning methods for similar problems against DSA-CycleGAN for multi-stain glomeruli segmentation.

Result: DSA-CycleGAN improves segmentation performance in glomeruli segmentation and outperforms other methods in reducing noise, especially when translating between biologically distinct stains.

Conclusion: DSA-CycleGAN effectively addresses noise issues in stain transfer for histopathology segmentation, providing better performance than existing methods, with code publicly available.
...
TL;DR: LLM-driven semantic compensation framework (MVR) addresses Expression Drift in text-to-image person retrieval by generating diverse textual variants and enhancing cross-modal feature consistency without training.


<details>
  <summary>Details</summary>
Motivation: Expression Drift problem in text-to-image person retrieval where semantically equivalent texts have significant feature discrepancies due to phrasing variations, degrading image-text alignment robustness.

Method: Three-component framework: 1) Multi-View Reformulation with dual-branch prompting (key feature guidance + diversity-aware rewriting), 2) Textual Feature Robustness Enhancement via training-free latent space compensation with multi-view feature mean-pooling and residual connections, 3) Visual Semantic Compensation using VLM-generated multi-perspective image descriptions enhanced through shared text reformulation.

Result: Improves accuracy of original model without training and achieves state-of-the-art performance on three text-to-image person retrieval datasets.

Conclusion: The proposed MVR framework effectively addresses Expression Drift by enhancing cross-modal representation consistency through semantic compensation, demonstrating strong performance in text-to-image person retrieval tasks.
...
TL;DR: OSD-IRF: One-step diffusion with inverse residual fields for fast industrial anomaly detection using diffusion models.


<details>
  <summary>Details</summary>
Motivation: Diffusion models achieve strong anomaly detection performance but suffer from slow inference due to iterative denoising/noising processes. Need to address this speed limitation while maintaining detection accuracy.

Method: Train DDPM on normal data, then predict inverse residual fields (IRF) from noise estimates. Anomaly detection is performed by evaluating IRF probability density under Gaussian distribution. Key insight: anomalies become distinguishable in IRF space, and IRF holds for any neighboring time step enabling single-step inference.

Result: Achieves SOTA or competitive performance across six metrics on three uIAD benchmarks with roughly 2X inference speedup without distillation.

Conclusion: OSD-IRF enables fast, accurate industrial anomaly detection using one-step diffusion with inverse residual fields, making diffusion models more practical for real-world applications.
...
TL;DR: MedProbeBench is the first benchmark using clinical guidelines as expert references to evaluate LLMs' deep evidence integration capabilities in medicine, revealing significant gaps in current models' ability to match expert-level clinical guideline development.


<details>
  <summary>Details</summary>
Motivation: Existing benchmarks fail to evaluate deep evidence integration capabilities in realistic medical workflows requiring multi-step evidence synthesis and expert-level judgment, despite the critical importance of this capability for developing clinical guidelines.

Method: Introduces MedProbeBench leveraging high-quality clinical guidelines as expert references, with MedProbe-Eval framework featuring: (1) Holistic Rubrics with 1,200+ task-adaptive criteria, and (2) Fine-grained Evidence Verification using 5,130+ atomic claims for rigorous validation.

Result: Evaluation of 17 LLMs and deep research agents reveals critical gaps in evidence integration and guideline generation, showing substantial distance between current capabilities and expert-level clinical guideline development.

Conclusion: MedProbeBench addresses a critical gap in evaluating deep evidence integration for medical applications, highlighting the need for significant advancement in LLM capabilities to reach expert-level clinical guideline development standards.
...
TL;DR: Recent vision-language models (VLMs) outperform earlier specialized baselines on Change VQA for remote sensing images, with native multimodal models being more effective than structured vision-language pipelines.


<details>
  <summary>Details</summary>
Motivation: Change VQA (answering questions about semantic changes between bi-temporal remote sensing images) remains underexplored in the context of modern multimodal models, despite recent studies on VLMs for temporal RS image understanding.

Method: Revisiting the CDVQA benchmark using recent Qwen models under unified LoRA settings, comparing Qwen3-VL (structured vision-language pipeline with multi-depth visual conditioning) with Qwen3.5 (native multimodal model with single-stage alignment and hybrid decoder).

Result: Recent VLMs improve over earlier specialized baselines on CDVQA test splits; performance doesn't scale monotonically with model size; native multimodal models are more effective than structured vision-language pipelines.

Conclusion: Tightly integrated multimodal backbones contribute more to performance than scale or explicit multi-depth visual conditioning for language-driven semantic change reasoning in remote sensing imagery.
...
TL;DR: ESsEN is a compact vision-language model that performs as well as larger models on several tasks with only a fraction of the parameters, making vision-language modeling more accessible.


<details>
  <summary>Details</summary>
Motivation: There's a need for smaller vision-language models for edge devices and robotic platforms, but little research exists on producing lightweight models or training them with small datasets. Current models have tens of billions of parameters which are unnecessary for many applications.

Method: The paper systematically addresses lightweight vision-language modeling by showing: 1) two-tower encoder models are superior to one-tower encoders in low-resource settings, 2) incorporating traditional convolutional networks into two-tower transformer architecture helps produce parameter-efficient models, and 3) cross-modal fusion modules can vary significantly in shape/size while producing same results. They present ESsEN, a compact model trainable end-to-end with few resources.

Result: ESsEN performs as well on several tasks with only a fraction of parameters compared to other models. The experimental results and tools make vision-language modeling more accessible to researchers.

Conclusion: Compact vision-language models like ESsEN can achieve comparable performance to larger models while being more parameter-efficient, enabling wider accessibility of vision-language modeling for edge devices and resource-constrained applications.
...
TL;DR: A novel framework for online video understanding that decouples reasoning control from memory integration to enable precise, evidence-aligned responses in streaming video scenarios.


<details>
  <summary>Details</summary>
Motivation: Visual agents need to respond precisely when sufficient evidence first appears in video streams, which is overlooked by conventional offline video LLMs. The shift to online streaming introduces challenges: lack of decision transparency, difficulty aligning response timing with visual evidence, and maintaining global understanding under tight computational budgets.

Method: Proposes a framework with two core components: 1) Active Thinking Decision Maker (ATDM) - a transparent reasoning controller that externalizes decision process using observable progress and confidence metrics to precisely time responses; 2) Hierarchical Progressive Semantic Integration (HPSI) - an efficient memory system using learnable multi-level aggregation tokens propagated across clips to build rich global cognitive state without exceeding token budgets.

Result: Achieves strong performance on online video understanding benchmarks: 71.6% on StreamingBench (improving from previous SOTA 67.63% to 71.60%) and 46.9% on OVOBench, demonstrating robust evidence-aligned and transparent online video analysis.

Conclusion: The proposed framework effectively addresses key challenges in online video understanding by providing transparent, evidence-aligned responses while maintaining computational efficiency, setting a new standard for streaming video analysis.
...
TL;DR: Asset Harvester: A pipeline that converts sparse object observations from autonomous vehicle driving logs into complete, simulation-ready 3D assets for closed-loop simulation.


<details>
  <summary>Details</summary>
Motivation: Neural scene reconstruction creates interactive 3D environments for AV simulation but doesn't produce complete 3D object assets needed for agent manipulation and large-viewpoint novel-view synthesis.

Method: System-level design combining: large-scale curation of object-centric training tuples, geometry-aware preprocessing across heterogeneous sensors, robust training recipe coupling sparse-view-conditioned multiview generation with 3D Gaussian lifting, and SparseViewDiT specifically designed for limited-angle views.

Result: Enables scalable conversion of sparse AV object observations into reusable 3D assets for simulation.

Conclusion: Asset Harvester addresses the gap in producing complete 3D object assets from real-world driving data, enabling more comprehensive AV simulation capabilities.
...
TL;DR: SemLT3D is a semantic-guided expert distillation framework that addresses long-tail imbalance in camera-only 3D object detection by using semantic priors to improve representation of rare but safety-critical categories.


<details>
  <summary>Details</summary>
Motivation: Camera-only 3D object detection methods overlook severe long-tail imbalance in real-world datasets, where rare but safety-critical categories (children, strollers, emergency vehicles) are underrepresented, leading to biased learning and degraded performance. This is worsened by inter-class ambiguity and intra-class diversity.

Method: SemLT3D consists of: (1) a language-guided mixture-of-experts module that routes 3D queries to specialized experts based on semantic affinity to disentangle confusing classes and specialize on tail distributions; (2) a semantic projection distillation pipeline that aligns 3D queries with CLIP-informed 2D semantics to produce more coherent and discriminative features.

Result: The framework enriches representation space for underrepresented classes through semantic priors, improving performance on rare categories while also enhancing robustness under broader appearance variations and challenging corner cases.

Conclusion: SemLT3D offers a principled approach toward more reliable camera-only 3D perception by addressing long-tail imbalance through semantically structured learning, which benefits not only rare categories but also overall robustness to visual variations.
...
TL;DR: XEmbodied is a cloud-side foundation model that enhances Vision-Language-Action models with 3D geometric awareness and physical interaction capabilities, improving spatial reasoning and embodied tasks.


<details>
  <summary>Details</summary>
Motivation: Current cloud pipelines for training VLA models rely on generic 2D vision-language models that lack geometric reasoning and domain-specific semantics, creating a mismatch for autonomous systems that need to understand 3D environments and physical interactions.

Method: XEmbodied integrates geometric representations through a structured 3D Adapter and distills physical signals into context tokens using an Efficient Image-Embodied Adapter. It uses progressive domain curriculum and reinforcement learning post-training to preserve general capabilities while enhancing geometric awareness.

Result: XEmbodied demonstrates robust performance across 18 public benchmarks, significantly improving spatial reasoning, traffic semantics, embodied affordance, and out-of-distribution generalization for large-scale scenario mining and embodied VQA tasks.

Conclusion: XEmbodied successfully addresses the limitations of current 2D VLMs by providing intrinsic 3D geometric awareness and physical interaction capabilities, making it a valuable foundation model for next-generation autonomous systems requiring embodied intelligence.
...
TL;DR: OneVL is a unified VLA and World Model framework that uses dual auxiliary decoders (language and visual world model) to supervise latent tokens, enabling one-step parallel reasoning that matches answer-only latency while surpassing explicit CoT performance.


<details>
  <summary>Details</summary>
Motivation: Current latent CoT methods fall short of explicit CoT because they compress symbolic linguistic abstractions rather than the actual causal dynamics that govern driving. There's a need for real-time capable reasoning that doesn't sacrifice accuracy.

Method: OneVL routes reasoning through compact latent tokens supervised by dual auxiliary decoders: a language decoder reconstructs text CoT, and a visual world model decoder predicts future-frame tokens. A three-stage training pipeline aligns latents with trajectory, language, and visual objectives. At inference, auxiliary decoders are discarded and all latent tokens are prefilled in a single parallel pass.

Result: OneVL becomes the first latent CoT method to surpass explicit CoT, delivering state-of-the-art accuracy at answer-only latency across four benchmarks. It demonstrates that tighter compression with both language and world-model supervision produces more generalizable representations.

Conclusion: The paper shows that latent reasoning can outperform explicit CoT when guided by both linguistic and visual world-model supervision, enabling real-time deployment without sacrificing accuracy in autonomous driving applications.
...
TL;DR: The paper introduces a Simple-to-Hard (S2H) learning framework to address the gap in multi-image reasoning capabilities of Vision-Language Models, systematically constructing preference data across three hierarchical reasoning levels to improve global visual search and cross-image comparison.


<details>
  <summary>Details</summary>
Motivation: Current Vision-Language Models excel at single-image understanding but struggle with multi-image reasoning. Existing multi-image alignment approaches focus on localized reasoning with pre-specified image indices, bypassing essential skills like global visual search and autonomous cross-image comparison. There's a critical capability gap that needs to be addressed.

Method: The Simple-to-Hard (S2H) learning framework systematically constructs multi-image preference data across three hierarchical reasoning levels: (1) single-image localized reasoning, (2) multi-image localized comparison, and (3) global visual search. Unlike prior methods that rely on model-specific attributes like hallucinations or attention heuristics, this approach uses prompt-driven complexity to create chosen/rejected preference pairs applicable across different models.

Result: Extensive evaluations on LLaVA and Qwen-VL models show that the diverse multi-image reasoning data significantly enhances multi-image reasoning performance, yielding substantial improvements over baseline methods across benchmarks. Importantly, the approach maintains strong single-image reasoning performance while simultaneously strengthening multi-image understanding capabilities.

Conclusion: The S2H framework advances the state of the art for holistic visual preference alignment by systematically addressing multi-image reasoning gaps while preserving single-image capabilities, offering a model-agnostic approach to preference data generation that works across different Vision-Language Models.
...
TL;DR: First framework integrating Uniform Discrete Diffusion Models with reinforcement learning, overcoming training instability of naive GRPO application through action-based optimization and trajectory reconstruction.


<details>
  <summary>Details</summary>
Motivation: UDM shows promise for discrete generative modeling but integration with RL remains unexplored. Naive GRPO application to UDM leads to training instability and marginal performance gains.

Method: Proposes framework with two key insights: (1) treating final clean sample as action for accurate optimization signals, (2) reconstructing trajectories via diffusion forward process to align probability paths with pretraining distribution. Includes Reduced-Step and CFG-Free strategies for efficiency.

Result: Significantly improves base model performance across T2I tasks: GenEval accuracy from 69% to 96%, PickScore from 20.46 to 23.81 (SOTA in both continuous/discrete settings). OCR benchmark accuracy from 8% to 57%.

Conclusion: Successfully integrates UDM with RL, achieving state-of-the-art performance and demonstrating strong generalization ability across multiple benchmarks.
...
TL;DR: MetaCloak-JPEG improves adversarial face protection by incorporating JPEG compression awareness into the attack pipeline, overcoming previous methods' blindness to social media image compression.


<details>
  <summary>Details</summary>
Motivation: Current adversarial face-protection systems (PhotoGuard, Anti-DreamBooth, MetaCloak) fail to account for JPEG compression used by social media platforms, losing 60-80% of protective signal because they don't backpropagate gradients through the non-differentiable JPEG quantization.

Method: Introduces MetaCloak-JPEG with Differentiable JPEG (DiffJPEG) layer using Straight-Through Estimator: forward pass applies standard JPEG compression, backward pass replaces round() with identity. Embeds DiffJPEG in JPEG-aware EOT distribution and curriculum quality-factor schedule within bilevel meta-learning loop.

Result: Under l-inf perturbation budget eps=8/255, achieves 32.7 dB PSNR, 91.3% JPEG survival rate, outperforms PhotoGuard on all 9 evaluated JPEG quality factors (9/9 wins, mean denoising-loss gain +0.125) within 4.1 GB training memory budget.

Conclusion: MetaCloak-JPEG effectively addresses the JPEG compression vulnerability in adversarial face protection, making deepfake prevention more robust against real-world social media image processing pipelines.
...
TL;DR: EVT improves upon RMT by using Euclidean distance decay for better spatial modeling and simpler grouping attention, achieving 86.6% top-1 accuracy on ImageNet-1k.


<details>
  <summary>Details</summary>
Motivation: Vision Transformers lack explicit spatial priors and have quadratic computational complexity. RMT addressed these with Manhattan distance and decomposed attention, but EVT aims to further improve spatial modeling and flexibility.

Method: EVT uses Euclidean distance decay for enhanced spatial relationship modeling and replaces RMT's decomposed attention with spatially-independent grouping for flexible token control.

Result: EVT achieves 86.6% top-1 accuracy on ImageNet-1k without extra training data, demonstrating exceptional performance across multiple vision tasks including classification, detection, and segmentation.

Conclusion: EVT provides a more sophisticated and adaptable approach to incorporating spatial priors into Self-Attention, overcoming RMT's limitations and enhancing applicability in computer vision tasks.
...
TL;DR: SynAgent is a framework for scalable cooperative humanoid manipulation that transfers skills from single-agent to multi-agent scenarios using interaction-preserving retargeting and decentralized training.


<details>
  <summary>Details</summary>
Motivation: Controllable cooperative humanoid manipulation faces challenges from data scarcity, multi-agent coordination complexities, and limited generalization across objects.

Method: Uses Solo-to-Cooperative Agent Synergy with interaction-preserving retargeting via Interact Mesh (Delaunay tetrahedralization), single-agent pretraining with decentralized multi-agent PPO, and trajectory-conditioned generative policy via conditional VAE with multi-teacher distillation.

Result: Significantly outperforms existing baselines in both cooperative imitation and trajectory-conditioned control, while generalizing across diverse object geometries.

Conclusion: SynAgent enables scalable and physically plausible cooperative manipulation by effectively transferring skills from single-agent to multi-agent scenarios through innovative retargeting and training paradigms.
...
TL;DR: AnchorSeg introduces structured query banks to decouple semantic reasoning from spatial localization in reasoning segmentation, achieving SOTA results through explicit spatial grounding and token-mask cycle consistency.


<details>
  <summary>Details</summary>
Motivation: Existing reasoning segmentation approaches rely on a single segmentation token that implicitly encodes both semantic reasoning and spatial localization, limiting the model's ability to explicitly disentangle what to segment from where to segment.

Method: Reformulates reasoning segmentation as structured conditional generation over image tokens using ordered query banks: latent reasoning tokens for semantic states and segmentation anchor token for spatial grounding. Introduces Token-Mask Cycle Consistency (TMCC) for bidirectional alignment between token-level predictions and pixel-level supervision.

Result: Achieves state-of-the-art results on ReasonSeg test set with 67.7% gIoU and 68.1% cIoU.

Conclusion: By explicitly decoupling spatial grounding from semantic reasoning through structured language grounded query banks, AnchorSeg provides an effective framework for reasoning segmentation that outperforms existing approaches.
...
TL;DR: MultiWorld is a unified framework for multi-agent multi-view world modeling that enables accurate control of multiple agents while maintaining multi-view consistency, outperforming existing single-agent approaches.


<details>
  <summary>Details</summary>
Motivation: Current video world models are limited to single-agent scenarios and fail to capture complex interactions in real-world multi-agent systems, creating a need for frameworks that can handle multiple agents with multi-view consistency.

Method: Introduces Multi-Agent Condition Module for precise multi-agent controllability and Global State Encoder for coherent observations across different views. Supports flexible scaling of agent and view counts with parallel view synthesis for efficiency.

Result: Experiments on multi-player game environments and multi-robot manipulation tasks show MultiWorld outperforms baselines in video fidelity, action-following ability, and multi-view consistency.

Conclusion: MultiWorld provides an effective unified framework for multi-agent multi-view world modeling that addresses limitations of existing single-agent approaches and demonstrates superior performance in complex interactive scenarios.
...
TL;DR: The paper challenges the Platonic Representation Hypothesis, showing that cross-modal alignment between neural networks is fragile, degrades with scale, and reflects only coarse semantic overlap rather than consistent fine-grained structure.


<details>
  <summary>Details</summary>
Motivation: To critically examine the evidence for the Platonic Representation Hypothesis, which claims neural networks trained on different modalities converge to the same representation of reality, and to determine whether modality choice actually matters.

Method: Analyze alignment using mutual nearest neighbors across different dataset scales (from ~1K to millions of samples), examine both one-to-one and many-to-many image-caption settings, and test alignment trends with newer language models.

Result: Cross-modal alignment degrades substantially with dataset scaling, remaining alignment reflects only coarse semantic overlap, many-to-many settings further reduce alignment, and newer models don't show the previously reported trend of increasing alignment with stronger language models.

Conclusion: Evidence for cross-modal representational convergence is weaker than previously thought; models trained on different modalities may learn equally rich but different representations of the world, suggesting modality choice does matter.
...
TL;DR: T-REN is a lightweight text-aligned region encoder that improves vision-language alignment and reduces token counts by pooling patch features into semantic regions, achieving significant gains in segmentation, retrieval, and video tasks with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Current vision-language encoders have two key limitations: (1) weak alignment between language and dense vision features, which hurts tasks like open-vocabulary semantic segmentation, and (2) high token counts for fine-grained visual representations, which limits scalability to long videos.

Method: T-REN adds a lightweight network on top of a frozen vision backbone to pool patch-level representations within semantic regions into compact region tokens, aligning them with region-level text annotations. This design uses only 3.7% additional parameters compared to the vision-language backbone.

Result: T-REN achieves +5.9 mIoU on ADE20K open-vocabulary segmentation, +18.4% recall on COCO object-level text-image retrieval, +15.6% recall on Ego4D video object localization, and +17.6% mIoU on VSPW video scene parsing, while reducing token counts by 24x for images and 187x for videos compared to patch-based backbones.

Conclusion: T-REN effectively addresses both vision-language alignment and token efficiency limitations, enabling stronger dense cross-modal understanding with dramatically reduced computational overhead, making it scalable for long video applications.
...
TL;DR: ReCap introduces a lightweight consistency framework for story visualization that improves character stability and visual fidelity without modifying base diffusion models, using selective conditional frame referencing and semantic drift correction.


<details>
  <summary>Details</summary>
Motivation: Maintaining cross-frame consistency (character identity, spatial configuration, stylistic coherence) in story visualization traditionally requires explicit memory banks, architectural expansion, or auxiliary language models, resulting in substantial parameter growth and inference overhead.

Method: ReCap uses two main components: 1) CORE module that treats pronouns as visual anchors, activating only when characters are referred to by pronouns and conditioning on preceding frames to propagate visual identity; 2) SemDrift that aligns denoiser representations with pretrained DINOv3 visual embeddings during training to correct semantic drift when text is vague.

Result: ReCap outperforms previous state-of-the-art StoryGPT-V by 2.63% Character-Accuracy on FlintstonesSV and 5.65% on PororoSV, establishing new SOTA character consistency. It adds only 149K parameters and demonstrates capability beyond cartoon domains on human-centric film narratives.

Conclusion: ReCap provides an efficient, lightweight solution for maintaining character consistency in story visualization without heavy architectural modifications, achieving state-of-the-art performance with minimal parameter overhead and extending applicability to real-world film narratives.
...
TL;DR: A novel animatable avatar representation achieves 2000x lower computational cost and 10x smaller model size than high-quality teacher models while preserving visual fidelity, enabling real-time performance on mobile devices.


<details>
  <summary>Details</summary>
Motivation: Existing avatar approaches fail to achieve both high fidelity and computational efficiency simultaneously - high-fidelity avatars require server-class GPUs while lightweight avatars suffer from limited dynamics and artifacts.

Method: Proposes Wavelet-guided Multi-level Spatial Factorized Blendshapes representation with a distillation pipeline that transfers motion-aware clothing dynamics and fine-grained appearance from a pre-trained high-quality avatar into a compact representation using wavelet spectral decomposition and low-rank structural factorization in texture space.

Result: Achieves up to 2000x lower computational cost and 10x smaller model size than original teacher model while preserving visual plausibility. Outperforms mobile avatar methods and matches server-based approaches. Achieves 180+ FPS on desktop and 24 FPS real-time on Meta Quest 3.

Conclusion: The method bridges the gap between high-fidelity and efficient avatar representation, substantially improving practicality for immersive applications by enabling high-quality avatars to run on resource-constrained platforms like VR headsets.
