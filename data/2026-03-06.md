...
TL;DR: CPath-CLIP fine-tuning improves cancer detection but fails in cross-species transfer due to semantic collapse; Semantic Anchoring using language alignment solves this issue.


<details>
  <summary>Details</summary>
Motivation: To investigate how foundation models behave in cross-cancer and cross-species transfer for computational pathology, and understand why current vision-language models fail in cross-species generalization.

Method: Fine-tuned CPath-CLIP on whole-slide image patches from canine and human histopathology, evaluated performance using AUC, analyzed embedding spaces with cosine similarity and Grad-CAM, and introduced Semantic Anchoring using language alignment.

Result: Few-shot fine-tuning improved same-cancer (64.9% to 72.6% AUC) and cross-cancer performance (56.84% to 66.31% AUC). Cross-species transfer remained below SOTA (84.97% AUC). Semantic Anchoring solved embedding collapse, achieving additional gains of 8.52% (same-cancer) and 5.67% (cross-cancer).

Conclusion: Standard vision-language alignment suffers from semantic collapse in cross-species transfer; language acts as a control mechanism enabling semantic re-interpretation without retraining, with Semantic Anchoring effectively addressing embedding collapse.
...
TL;DR: Multi-modal activity recognition system for older adults in AAL settings combining 3D CNN for visual data, Graph CNN for pose data, and object detection with cross-attention fusion.


<details>
  <summary>Details</summary>
Motivation: To develop robust activity recognition for Ambient Assisted Living systems that monitor older adults' well-being and support independence, addressing challenges like intra-class variability, inter-class similarity, environmental variability, camera perspectives, and scene complexity.

Method: Multi-modal approach integrating: 1) 3D CNN for visual information processing, 2) Graph Convolutional Network for 3D human pose analysis, 3) Object detection module for contextual information fused with 3D CNN features using cross-attention mechanism.

Result: Achieves competitive classification accuracy for daily activities on the Toyota SmartHome dataset (real-world indoor activities), demonstrating potential as essential component for advanced AAL monitoring solutions.
...
TL;DR: InverseNet is the first benchmark for operator mismatch in compressive imaging, showing deep learning methods lose 10-21 dB under mismatch, eliminating their advantage over classical methods.


<details>
  <summary>Details</summary>
Motivation: Existing compressive imaging benchmarks don't quantify operator mismatch, which is the default condition in deployed systems, causing state-of-the-art methods to fail when forward operators deviate from physical reality.

Method: Introduces InverseNet benchmark spanning CASSI, CACTI, and single-pixel cameras with four-scenario protocol (ideal, mismatched, oracle-corrected, blind calibration), evaluating 12 methods across 27 simulated scenes and 9 real hardware captures.

Result: Deep learning methods lose 10-21 dB under mismatch, eliminating advantage over classical baselines; mask-oblivious architectures recover 0% of mismatch losses while operator-conditioned methods recover 41-90%; blind calibration recovers 85-100% of oracle bound.

Conclusion: Operator mismatch is critical for compressive imaging evaluation, current methods are fragile to mismatch, and operator-conditioned architectures with blind calibration can recover most performance losses without ground truth.
...
TL;DR: This paper analyzes different multimodal fusion strategies for Local Climate Zone classification using deep learning, comparing four fusion methods and grouping strategies to improve accuracy, especially for underrepresented classes.


<details>
  <summary>Details</summary>
Motivation: There's a gap in comprehensive analysis of fusion mechanisms in deep learning classifiers for multimodal LCZ classification. The study aims to analyze different fusion strategies and grouping approaches to improve classification accuracy, particularly for underrepresented classes.

Method: Four fusion models are analyzed: (i) baseline hybrid fusion (FM1), (ii) with self- and cross-attention mechanisms (FM2), (iii) with multi-scale Gaussian filtered images (FM3), and (iv) weighted decision-level fusion (FM4). Ablation experiments study pixel-, feature-, and decision-level fusion effects. Grouping strategies include band grouping within data modalities and label merging in ground truth. Analysis is done on the So2Sat LCZ42 dataset with SAR and MSI image pairs.

Result: FM1 consistently outperforms simple fusion methods. FM1 with band grouping and label merging achieves the best overall accuracy of 76.6%. The study highlights how these strategies improve prediction accuracy for underrepresented classes.

Conclusion: The baseline hybrid fusion (FM1) with band grouping and label merging is the most effective approach for multimodal LCZ classification. The study demonstrates the importance of carefully designed fusion strategies and grouping approaches for improving classification performance, especially for underrepresented classes in urban climate analysis.
...
TL;DR: Unified diffusion framework for histopathology image synthesis using nuclei centroids as spatial priors, supporting both local structure completion and global structure synthesis with task-specific LoRA adapters.


<details>
  <summary>Details</summary>
Motivation: Existing generative methods treat restoration and generation as separate tasks despite sharing the same objective of structure-consistent tissue synthesis. Current approaches rely on weak or inconsistent structural priors that limit realistic cellular organization in histopathology images.

Method: Dual-LoRA Controllable Diffusion framework using multi-class nuclei centroids as lightweight spatial priors. Features two task-specific LoRA adapters that specialize a shared backbone diffusion model for local structure completion and global structure synthesis without retraining separate models.

Result: Significant improvements over state-of-the-art GAN and diffusion baselines: LPIPS within masked region improved from 0.1797 (HARP) to 0.1524 for local completion, and FID improved from 225.15 (CoSys) to 76.04 for global synthesis, indicating better structural fidelity and realism.

Conclusion: The unified centroid-guided diffusion framework achieves more faithful structural recovery in masked regions and substantially improved realism and morphology consistency in full synthesis, supporting scalable pan-cancer histopathology modeling with annotation-efficient spatial priors.
...
TL;DR: PVM (Partial Vision Mamba) adapts Mamba SSMs to handle arbitrarily shaped invalid/missing data in vision tasks through partial operations similar to Partial Convolutions.


<details>
  <summary>Details</summary>
Motivation: Real-world vision tasks often deal with inputs containing arbitrarily shaped regions of missing/invalid data. While CNNs have Partial Convolutions to handle this, recent State Space Models (SSMs) like Mamba lack inherent mechanisms for processing such invalid data at inference time.
...
Conclusion: PVM successfully bridges the gap in SSMs' ability to handle arbitrarily shaped invalid data, extending the benefits of partial operations to Mamba-based vision architectures.
...
TL;DR: PinPoint is a comprehensive benchmark for Composed Image Retrieval with 7,635 queries and 329K relevance judgments that addresses limitations of existing benchmarks by including multiple correct answers, hard negatives, instruction paraphrases, multi-image queries, and demographic metadata.


<details>
  <summary>Details</summary>
Motivation: Current CIR benchmarks are limited to single ground-truth answers and lack annotations for evaluating false positive avoidance, robustness, multi-image reasoning, and fairness. There's a need for more comprehensive real-world evaluation frameworks.

Method: Created PinPoint benchmark with 7,635 queries across 23 categories, featuring: multiple correct answers (avg 9.1 per query), explicit hard negatives, six instruction paraphrases per query, multi-image composition support (13.4% of queries), and demographic metadata. Evaluated 20+ methods across 4 paradigms.

Result: Best methods achieve mAP@10 of 28.5% but still retrieve irrelevant results 9% of the time. Models show 25.1% performance variation across paraphrases. Multi-image queries perform 40-70% worse. Proposed training-free MLLM-based reranking method to address these issues.

Conclusion: PinPoint reveals significant limitations in current CIR methods and provides a comprehensive benchmark for future research. The proposed reranking method can improve existing systems, and the complete dataset/code is released to advance the field.
...
Method: Proposes SGR3 Model: a training-free framework using multi-modal LLMs with retrieval-augmented generation. It incorporates semantically aligned scene graphs retrieved via ColPali-style cross-modal framework and uses weighted patch-level similarity selection to improve retrieval robustness.

Result: Achieves competitive performance compared to training-free baselines and on par with GNN-based expert models. Ablation studies show retrieved external information is explicitly integrated into token generation rather than implicitly internalized.

Conclusion: SGR3 Model provides an effective training-free alternative for 3D scene graph generation that bypasses explicit reconstruction requirements and demonstrates explicit integration of retrieved knowledge in the generation process.
...
TL;DR: Spinverse is a permeability-aware reconstruction method that inverts diffusion MRI measurements using a differentiable Bloch-Torrey simulator to recover microstructural boundaries without assuming fixed topology.


<details>
  <summary>Details</summary>
Motivation: Existing diffusion MRI methods either assume impermeable boundaries or estimate voxel-level parameters without recovering explicit interfaces, limiting accurate microstructural boundary reconstruction.

Method: Uses a fully differentiable Bloch-Torrey simulator on a fixed tetrahedral grid with learnable face permeability parameters. Optimizes permeabilities through backpropagation of signal-matching loss, then thresholds to recover interfaces. Employs mesh-based geometric priors and staged multi-sequence optimization to mitigate ill-posedness and avoid local minima.

Result: Successfully reconstructs diverse geometries across synthetic voxel meshes, demonstrating that sequence scheduling and regularization are critical for avoiding outline-only solutions while improving boundary accuracy and structural validity.

Conclusion: Spinverse enables permeability-aware reconstruction of microstructural boundaries from diffusion MRI without fixed topology assumptions, using differentiable simulation and careful optimization strategies to overcome ill-posedness challenges.
...
<details>
  <summary>Details</summary>
Motivation: DL methods for medical image restoration can produce visually appealing but hallucinated outputs, with no easy-to-use techniques or robust metrics for identifying these hallucinations.

Method: sFRC (scanning Fourier Ring Correlation) analyzes small patches across DL outputs and reference images to detect hallucinations, using either expert-annotated features or imaging theory-based hallucination maps to set parameters.

Result: sFRC effectively detects hallucinations in CT super-resolution, CT sparse view, and MRI subsampled restoration; shows agreement with imaging theory-based maps; quantifies hallucination rates across data distributions and subsampling rates; works for both DL and conventional methods.

Conclusion: sFRC provides an effective tool for detecting hallucinations in medical image restoration outputs, applicable to various DL and conventional methods, helping characterize method robustness across different conditions.
...
TL;DR: PulseFocus is a training-free inference method that improves multi-image reasoning in VLMs by structuring chain-of-thought into plan/focus blocks with attention gating to address diffuse attention patterns and positional bias.
...
Method: PulseFocus structures CoT reasoning into interleaved plan/focus blocks with soft attention gating. The model first plans which image to examine, then gates decode-time attention to the referenced image, sharpening focus without additional training.
...
Conclusion: PulseFocus effectively addresses attention diffusion and positional bias in VLMs' multi-image reasoning through structured planning and attention gating, achieving significant performance gains without requiring model retraining.
...
<details>
  <summary>Details</summary>
Motivation: Deep neural networks have high computational/memory requirements that limit deployment on resource-constrained platforms like remote sensing devices and edge systems. Network compression is needed to enable efficient deep learning in remote sensing applications.

Method: Systematic evaluation of three CNN compression strategies (pruning, quantization, knowledge distillation) for hyperspectral land cover classification. Experiments conducted on two benchmark hyperspectral datasets measuring classification accuracy, memory consumption, and inference efficiency.
...
Conclusion: Compression techniques have strong potential for enabling efficient deep learning deployment in remote sensing applications by balancing model efficiency with predictive performance.
...
<details>
  <summary>Details</summary>
Motivation: To systematically evaluate MLLMs' reliability for real-world Video Anomaly Detection (VAD), exploring their potential as a paradigm shift from conventional reconstruction/pose-based methods to language-guided reasoning tasks.
...
Result: MLLMs exhibit pronounced conservative bias in zero-shot settings: high confidence but disproportionately favor 'normal' class, resulting in high precision but recall collapse. Class-specific instructions improve F1-score from 0.09 to 0.64 on ShanghaiTech, but recall remains bottleneck.

Conclusion: Significant performance gap exists for MLLMs in noisy VAD environments; future work needed for recall-oriented prompting and model calibration for open-world surveillance requiring complex video understanding and reasoning.
...
TL;DR: FOZO is a forward-only zeroth-order optimization method for Test-Time Adaptation that eliminates backpropagation, making it suitable for resource-limited devices while achieving competitive performance with gradient-based methods.


<details>
  <summary>Details</summary>
Motivation: Current TTA approaches have limitations: backpropagation-based methods are computationally expensive and modify model weights, making them unsuitable for low-end devices, while traditional backpropagation-free methods have limited adaptation capabilities.
...
Result: FOZO achieves 59.52% Top-1 accuracy on ImageNet-C (5K, level 5), outperforming gradient-based methods and SOTA forward-only FOA (58.13%). It also shows strong generalization on quantized (INT8) models.
...
TL;DR: Real-IISR is an autoregressive framework for real-world infrared image super-resolution that addresses coupled optical/sensing degradations through thermal-structural guidance and adaptive codebooks.
...
Method: Proposes Real-IISR: a unified autoregressive framework with Thermal-Structural Guidance module to encode thermal priors, Condition-Adaptive Codebook to handle non-uniform degradations, and Thermal Order Consistency Loss to maintain physical consistency. Also introduces FLIR-IISR dataset with paired LR-HR infrared images.

Result: Extensive experiments demonstrate promising performance. Provides unified foundation for real-world IISR benchmarking with publicly available dataset and code.

Conclusion: Real-IISR effectively addresses real-world infrared super-resolution challenges through thermal-structural guided autoregression, adaptive codebooks, and physical consistency constraints, establishing a benchmark for the field.
...
TL;DR: GPT-5 shows significant improvements over GPT-4o in medical reasoning tasks, with 25+ percentage-point gains on textual exams and 10-40% improvements in mammography VQA, but still lags behind specialized medical AI systems in perception-critical domains.


<details>
  <summary>Details</summary>
Motivation: To evaluate whether general-purpose foundation models like GPT-5 can support the integrated reasoning required in clinical medicine, where diagnosis demands synthesis of ambiguous patient narratives, lab data, and multimodal imaging.

Method: First controlled cross-sectional evaluation of GPT-5 family (GPT-5, GPT-5 Mini, GPT-5 Nano) vs GPT-4o across clinically grounded tasks including medical education exams, text-based reasoning benchmarks, and visual question-answering in neuroradiology, digital pathology, and mammography using standardized zero-shot chain-of-thought protocol.

Result: GPT-5 showed substantial gains in expert-level textual reasoning (25+ percentage-point improvements on MedXpertQA). For multimodal synthesis, GPT-5 achieved state-of-the-art or competitive VQA performance, outperforming GPT-4o by 10-40% in mammography tasks. However, performance remained moderate in neuroradiology (44% macro-average accuracy) and lagged behind domain-specific models in mammography (52-64% vs specialized systems >80%).

Conclusion: GPT-5 represents meaningful advance toward integrated multimodal clinical reasoning that mirrors clinician cognitive processes, but generalist models are not yet substitutes for purpose-built systems in highly specialized, perception-critical medical tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing manual labeling of micro-expressions is error-prone, especially in cross-cultural scenarios where key frame labeling deviations are prominent. There's a need to address subjective errors in multicultural datasets and reconsider the validity of dataset annotation paradigms.

Method: Proposes GAMDSS architecture with dynamic frame reselection mechanism to identify Onset and Apex frames from complete sequences, then determines Offset frames to construct spatio-temporal representations. Uses a two-branch structure with shared parameters for efficient feature extraction.

Result: Extensive experiments on seven micro-expression datasets show GAMDSS effectively reduces subjective errors in multicultural datasets (SAMM, 4DME). Quantitative analyses confirm offset-frame annotations are more uncertain in multicultural datasets, justifying standardization of annotations.

Conclusion: The method can be integrated into existing models without increasing parameters, offering a new approach to enhance micro-expression recognition performance. Findings support reconsidering validity and generalizability of dataset annotation paradigms.
...
TL;DR: DSA-SRGS: A super-resolution Gaussian splatting framework for dynamic sparse-view DSA reconstruction that enhances vascular detail recovery through multi-fidelity texture learning and radiative sub-pixel densification.


<details>
  <summary>Details</summary>
Motivation: Current 3D vessel reconstruction methods from DSA are limited by input projection resolution, causing blurring and aliasing when upsampling. This prevents recovery of fine vascular details needed for precision diagnosis and treatment.

Method: Proposes DSA-SRGS with: 1) Multi-Fidelity Texture Learning Module integrating priors from fine-tuned DSA-specific SR model, 2) Confidence-Aware Strategy to weight supervision between low-res projections and high-res pseudo-labels, 3) Radiative Sub-Pixel Densification using gradient accumulation from high-res sub-pixel sampling.
...
Conclusion: DSA-SRGS enables high-quality 4D vessel reconstruction with super-resolution capability, recovering fine-grained vascular details for improved precision diagnosis and treatment of cerebrovascular diseases.
...
TL;DR: A margin-aware mesh generation framework called TotalFramework for automated dental crown design, using CrownDeformR for template deformation and CrownSegger for margin segmentation to address resolution, noise, and overextension issues in existing methods.


<details>
  <summary>Details</summary>
Motivation: Current CAD systems for dental crown design require extensive manual adjustments, and existing learning-based methods suffer from inadequate spatial resolution, noisy outputs, and overextension of surface reconstruction, limiting clinical applicability.

Method: Proposed TotalFramework with two components: CrownDeformR deforms an initial template to target crown using anatomical context from multi-scale intraoral scan encoder, and CrownSegger (margin segmentation network) extracts cervical margin as constraint for deformation and boundary for postprocessing to remove overextended areas.
...
Conclusion: The proposed margin-aware mesh generation framework effectively addresses limitations of previous automated dental crown design methods, achieving better geometric accuracy and clinical feasibility by incorporating clinical workflow principles and margin constraints.
...
TL;DR: Proposes a privacy-preserving perception framework using AI Flow and edge-cloud architecture that transforms raw images into abstract feature vectors at the edge for mathematically unreconstructable privacy protection, while enabling behavior recognition and semantic reconstruction in the cloud.
...
Result: Achieves mathematically unreconstructable privacy protection by stripping identity-sensitive information while enabling behavior recognition and illustrative visual reference without exposing raw images, balancing perception and privacy.

Conclusion: Proposes a novel framework that solves the privacy-security paradox by ensuring mathematically provable irreversibility of raw images while maintaining semantic understanding capabilities, overcoming limitations of previous approaches.
...
TL;DR: RMK RetinaNet addresses three key bottlenecks in rotated object detection for remote sensing: non-adaptive receptive fields, inadequate multi-scale fusion, and angle regression discontinuities. It achieves SOTA-comparable performance on major benchmarks.


<details>
  <summary>Details</summary>
Motivation: Rotated object detection in remote sensing faces three major challenges: 1) non-adaptive receptive field utilization, 2) inadequate long-range multi-scale feature fusion, and 3) discontinuities in angle regression that hinder accurate orientation prediction.

Method: Four key components: 1) Multi-Scale Kernel (MSK) Block for adaptive multi-scale feature extraction, 2) Multi-Directional Contextual Anchor Attention (MDCAA) for cross-scale contextual modeling, 3) Bottom-up Path to preserve spatial details lost during downsampling, and 4) Euler Angle Encoding Module (EAEM) for continuous angle regression.
...
Conclusion: RMK RetinaNet effectively addresses the three major bottlenecks in rotated object detection through adaptive feature extraction, enhanced contextual modeling, spatial detail preservation, and stable angle regression, demonstrating strong performance across multiple remote sensing benchmarks.
...
TL;DR: The paper introduces two network adapters: LAW for adaptive loss weighting in diffusion-based medical image synthesis and ORDER for efficient segmentation, both addressing spatial imbalance in medical image analysis.


<details>
  <summary>Details</summary>
Motivation: Medical image analysis faces spatial imbalance where lesions occupy small regions against vast backgrounds. Diffusion models drift from prescribed lesion layouts, and efficient segmenters struggle with spatially uncertain regions. Adaptive spatial weighting can address these issues by learning where to allocate computational resources.

Method: Two network adapters: 1) Learnable Adaptive Weighter (LAW) predicts per-pixel loss modulation from features and masks for diffusion training, stabilized via normalization, clamping, and regularization. 2) Optimal Region Detection with Efficient Resolution (ORDER) applies selective bidirectional skip attention at late decoder stages for efficient segmentation.
...
Conclusion: The proposed adaptive spatial weighting approach effectively addresses spatial imbalance in medical image analysis, improving both controllable synthesis and segmentation performance while maintaining computational efficiency.
...
TL;DR: This paper reviews segmentation and classification techniques for brain gliomas, finding that convolutional neural networks outperform traditional methods for both tasks.


<details>
  <summary>Details</summary>
Motivation: Accurate segmentation and classification of brain gliomas are crucial for treatment planning, monitoring, and personalized patient care, but achieving error-free and reproducible segmentation is challenging.

Method: The paper conducts a review of segmentation and classification techniques for brain gliomas, evaluating both fully automatic and semi-automatic methods, with a focus on post-MRI acquisition approaches.
...
Conclusion: CNN-based approaches represent the most effective methods for brain glioma segmentation and classification, with semi-automatic techniques being preferred by radiologists due to their need for accurate evaluations and supervision.
...
TL;DR: MASQuant is a novel PTQ framework for MLLMs that addresses modality-specific quantization challenges through modality-aware smoothing and cross-modal compensation.


<details>
  <summary>Details</summary>
Motivation: Existing PTQ methods for LLMs don't work well for MLLMs due to Smoothing Misalignment and Cross-Modal Computational Invariance issues when handling different modalities.

Method: Proposes MASQuant with two key components: (1) Modality-Aware Smoothing that learns separate smoothing factors for each modality, and (2) Cross-Modal Compensation using SVD whitening to transform multi-modal activation differences into low-rank forms for unified quantization.
...
Conclusion: MASQuant effectively addresses modality-specific quantization challenges in MLLMs through its novel modality-aware approach, enabling efficient quantization while maintaining performance across different modalities.
...
Method: Proposes Diffusion Contrastive Reconstruction (DCR) that injects contrastive signals derived from each reconstructed image (not original input) into the diffusion process. This unified learning objective balances optimization and avoids gradient conflicts from naive combinations.

Result: Theoretical analysis shows DCR loss jointly optimizes both discriminative and perceptual abilities. Extensive experiments across various benchmarks and multi-modal large language models validate effectiveness.

Conclusion: DCR successfully enhances CLIP's visual representation capacity by integrating contrastive learning into diffusion-based reconstruction, achieving comprehensive improvements in both discriminative and perceptual abilities.
...
TL;DR: Meta-D architecture uses MRI metadata (sequence type, plane orientation) to improve brain tumor analysis, boosting 2D detection by 2.62% F1-score and 3D segmentation by 5.12% Dice score under missing modalities.


<details>
  <summary>Details</summary>
Motivation: To improve medical image deep learning by integrating explicit scanner metadata (MRI sequence, plane orientation) to stabilize feature representations and handle missing modality scenarios.

Method: Meta-D architecture that dynamically modulates convolutional features using metadata for 2D tumor detection, and Transformer Maximizer that uses metadata-based cross-attention to route available modalities for 3D missing-modality segmentation.

Result: 2.62% absolute F1-score improvement in 2D tumor detection; 5.12% Dice score improvement in 3D segmentation under extreme modality scarcity while reducing model parameters by 24.1%.

Conclusion: Explicit metadata integration effectively stabilizes feature extraction, improves performance in both complete and missing modality scenarios, and enables more efficient models for brain tumor analysis.
...
<details>
  <summary>Details</summary>
Motivation: Prior SfP methods underperform RGB-only VFMs due to domain gaps, not polarization modality limitations. Two main gaps: unrealistic synthetic datasets and unmodeled sensor noise.

Method: 1) Render high-quality polarization dataset using 1,954 3D-scanned real objects; 2) Incorporate pretrained DINOv3 priors for generalization; 3) Introduce polarization sensor-aware data augmentation for real-world noise.

Result: Method significantly outperforms both state-of-the-art SfP approaches and RGB-only VFMs with only 40K training scenes. Polarization enables 33x reduction in training data or 8x reduction in model parameters while achieving better performance.

Conclusion: Polarization cues remain valuable for surface normal estimation when domain gaps are properly addressed, offering data and parameter efficiency advantages over RGB-only approaches.
...
TL;DR: CAD framework tackles instance entanglement in instance-dependent partial label learning through class-specific augmentation and weighted penalty loss.


<details>
  <summary>Details</summary>
Motivation: Instance-dependent PLL faces instance entanglement where similar classes share overlapping features and candidate labels, causing class confusion that needs to be addressed.

Method: Class-specific Augmentation based Disentanglement (CAD) framework with intra-class regulation (amplifying class-specific features and aligning same-class augmentations) and inter-class regulation (weighted penalty loss that penalizes ambiguous labels more heavily).

Result: Extensive experiments show CAD effectively mitigates entanglement problem and enhances ID-PLL performance.

Conclusion: CAD successfully addresses instance entanglement in ID-PLL through joint intra- and inter-class regulations, improving class boundary clarity and reducing confusion.
...
TL;DR: SADCA is a novel adversarial attack method for vision-language models that enhances transferability through dynamic contrastive learning and semantic augmentation.


<details>
  <summary>Details</summary>
Motivation: Existing attacks on vision-language models have limited cross-modal disruption and poor transferability due to static cross-modal interactions and focus only on disrupting positive image-text pairs.

Method: Proposes Semantic-Augmented Dynamic Contrastive Attack (SADCA) that: 1) progressively disrupts cross-modal alignment through dynamic interactions between adversarial images and texts, 2) establishes a contrastive learning mechanism with adversarial, positive and negative samples to reinforce semantic inconsistency, and 3) incorporates semantic augmentation module to increase diversity and generalization of adversarial examples.
...
Conclusion: SADCA effectively addresses the limitations of existing attacks on vision-language models by enhancing adversarial transferability through progressive semantic-guided perturbation and dynamic contrastive learning.
...
TL;DR: MPCAttack: A multi-paradigm collaborative attack framework that aggregates visual and textual features to boost adversarial transferability against Multi-Modal Large Language Models.


<details>
  <summary>Details</summary>
Motivation: Existing adversarial attacks against MLLMs rely on single learning paradigms and independent optimization in separate feature spaces, which restricts feature richness, limits search space, and impedes perturbation diversity.

Method: Proposes MPCAttack framework with Multi-Paradigm Collaborative Optimisation (MPCO) strategy that aggregates semantic representations from both visual images and language texts, performs contrastive matching on multi-paradigm features, adaptively balances different paradigm representations, and guides global perturbation optimization.
...
Conclusion: MPCAttack effectively addresses representation bias in adversarial attacks against MLLMs by leveraging multi-paradigm collaborative optimization, demonstrating superior transferability across different model architectures.
...
Conclusion: GloSplat demonstrates that preserving explicit SfM feature tracks as first-class entities during joint pose-appearance optimization significantly improves reconstruction quality and robustness compared to photometric-only methods, offering both efficient and high-quality reconstruction options.
...
TL;DR: Monocular video pipeline recovers 18 biomechanical metrics from broadcast footage for scalable injury-risk screening in baseball pitching, achieving clinical-grade accuracy comparable to expensive multi-camera systems.


<details>
  <summary>Details</summary>
Motivation: Current gold-standard biomechanical measurements require expensive stadium-installed multi-camera systems unavailable outside professional venues, limiting scalable injury prediction for pitchers.

Method: Pipeline built on DreamPose3D with drift-controlled global lifting module for pelvis trajectory recovery, plus kinematics refinement with bone-length constraints, joint-limited inverse kinematics, smoothing, and symmetry constraints to handle motion blur and extreme poses.

Result: 16/18 metrics achieve sub-degree agreement (MAE < 1°) on 13 professional pitchers (156 paired pitches). Injury prediction model achieves AUC 0.811 for Tommy John surgery and 0.825 for significant arm injuries on 7,348 pitchers.

Conclusion: Monocular broadcast video provides a viable, scalable alternative to stadium-scale motion capture for biomechanical analysis and injury-risk screening in baseball pitching.
...
TL;DR: SURE is a semi-dense uncertainty-refined matching framework that jointly predicts image correspondences and their confidence by modeling both aleatoric and epistemic uncertainties, outperforming existing methods in challenging scenarios.


<details>
  <summary>Details</summary>
Motivation: Existing image correspondence methods struggle in challenging scenarios with large viewpoint changes or textureless regions because they rely solely on feature similarity without estimating match reliability, leading to overconfident errors.

Method: Proposes SURE framework with: 1) novel evidential head for trustworthy coordinate regression that models both aleatoric and epistemic uncertainties, 2) lightweight spatial fusion module that enhances local feature precision with minimal overhead.

Result: Outperforms existing state-of-the-art semi-dense matching models on multiple standard benchmarks in both accuracy and efficiency.

Conclusion: SURE provides a reliable solution for image correspondence by jointly predicting matches and their confidence through uncertainty modeling, addressing limitations of conventional similarity-based approaches.
...
TL;DR: PNG framework generates realistic noisy images without camera metadata by learning noise characteristics from input prompts, improving real-world denoising performance.


<details>
  <summary>Details</summary>
Motivation: Real-world denoising is limited by scarce noisy-clean image pairs and existing generative methods' dependency on camera metadata, which restricts usability across devices.

Method: Proposes Prompt-Driven Noise Generation (PNG) framework that learns high-dimensional prompt features capturing real-world noise characteristics to synthesize realistic noisy images without camera metadata.

Result: PNG effectively produces realistic noisy images and successfully applies them to remove real-world noise across various benchmark datasets, enhancing generalizability.

Conclusion: The PNG framework eliminates camera metadata dependency for noise synthesis, significantly improving generalizability and applicability for real-world denoising tasks.
...
<details>
  <summary>Details</summary>
Motivation: To understand how much information about upcoming pitches can be extracted from a pitcher's body kinematics alone, without relying on ball-flight data, and to establish the empirical limits of pose-based pitch prediction.
...
Result: Achieved 80.4% accuracy on 119,561 professional pitches, the largest benchmark to date. Upper-body mechanics contributed 64.9% of predictive signal vs. 35.1% for lower body. Wrist position was most informative joint group (14.8%), trunk lateral tilt most informative biomechanical feature. Grip-defined variants (four-seam vs. two-seam fastball) were not separable from pose alone.

Conclusion: Body kinematics alone can predict pitch types with ~80% accuracy, establishing an empirical ceiling for pose-based prediction. This delineates where kinematic information ends and ball-flight information begins, showing grip-based pitch variations cannot be distinguished from body pose alone.
...
TL;DR: A two-stage framework for CT report generation using structure-specific visual queries and contrastive learning to improve clinical efficiency.


<details>
  <summary>Details</summary>
Motivation: CT report generation is challenging due to large CT data volumes and complex details needed. Existing deep learning methods from X-ray report generation don't work well for CT.

Method: Two-stage approach: 1) Structure-learning with learnable visual queries for anatomical structures, structure-wise image-text contrastive learning, soft pseudo targets for false negatives, and diversity-enhanced negative queue. 2) Report-learning with frozen visual queries to select critical image patches and train text decoder.

Result: Achieves state-of-the-art performance on two public datasets, demonstrating clinical efficiency and effectiveness of all components.

Conclusion: The proposed framework effectively addresses CT report generation challenges through structure-specific learning and contrastive techniques, establishing new benchmarks for clinical automation.
...
TL;DR: DeformTrace is a novel temporal forgery localization method that enhances State Space Models with deformable dynamics and relay mechanisms to better identify manipulated segments in video/audio, achieving SOTA performance with efficiency.
...
Conclusion: DeformTrace successfully addresses TFL challenges by enhancing SSMs with deformable dynamics and relay mechanisms, offering an efficient and effective solution for temporal forgery localization with strong interpretability for security applications.
...
TL;DR: FedMEPD: A federated learning framework with modality-specific encoders and partially personalized decoders to handle both intermodal heterogeneity (clients with different modality subsets) and personalization needs in medical image analysis.


<details>
  <summary>Details</summary>
Motivation: Existing FL methods only address intramodal heterogeneity, but in medical imaging, clients often have incomplete subsets of modalities (intermodal heterogeneity). Additionally, each client needs personalized models tailored to local data characteristics.

Method: FedMEPD uses federated modality-specific encoders for each imaging modality and partially personalized decoders. The server with full-modal data employs a fusion decoder and extracts multimodal anchors that are distributed to clients. Clients with incomplete modalities use cross-attention to calibrate missing-modal representations toward global anchors.

Result: FedMEPD outperforms various state-of-the-art methods for multimodal and personalized FL on BraTS 2018 and 2020 brain tumor segmentation benchmarks, demonstrating effectiveness of its novel designs.

Conclusion: FedMEPD successfully addresses both intermodal heterogeneity and personalization needs in multimodal FL for medical image analysis, providing an effective solution for real-world scenarios where clients have different modality subsets.
...
TL;DR: A simple add-on module for vision transformers that improves segmentation performance while preserving classification accuracy by modulating self-attention with learnable Gaussian kernels and refining patch representations.


<details>
  <summary>Details</summary>
Motivation: Vision transformers excel at classification using global self-attention but lose fine-grained spatial details needed for segmentation tasks. The authors want to enhance segmentation performance without compromising the original classification capabilities or changing the training regime.

Method: Introduces two modifications: 1) Modulates self-attention with a learnable Gaussian kernel that biases attention toward neighboring patches, and 2) Refines patch representations to learn better embeddings at patch positions. These encourage local focus while preserving global information.
...
Conclusion: The proposed simple yet effective add-on successfully enhances vision transformers' segmentation capabilities while maintaining their original image-level recognition strengths, offering a practical solution to the spatial detail limitation in transformer-based segmentation.
...
<details>
  <summary>Details</summary>
Motivation: Existing video diffusion models struggle with high-fidelity frame interpolation due to reliance on generative priors that limit detail preservation from start/end frames. Current motion control methods using dense optical flow are error-prone, and sparse points lack structural context.

Method: Proposes FC-VFI with three key components: 1) Temporal modeling strategy on latent sequences to inherit fidelity cues from start/end frames, 2) Semantic matching lines for structure-aware motion guidance to improve motion consistency, 3) Temporal difference loss to mitigate temporal inconsistencies.

Result: FC-VFI achieves high performance and structural integrity across diverse scenarios, supporting 4x and 8x interpolation to boost frame rates from 30 FPS to 120 and 240 FPS at 2560×1440 resolution while preserving visual fidelity and motion consistency.

Conclusion: FC-VFI effectively addresses limitations of existing video frame interpolation methods by combining latent sequence temporal modeling with semantic structure guidance, achieving faithful and consistent interpolation results with improved detail preservation and motion consistency.
...
<details>
  <summary>Details</summary>
Motivation: Hallucination is a major problem in Large Vision-Language Models (LVLMs). While increasing attention to image tokens reduces hallucinations, it causes repetitive descriptions. The authors discovered that real object tokens attend more to generated text than hallucinated ones, suggesting generated text contains useful visual and contextual information.

Method: Proposes IAT (Attention to Generated Text) which amplifies attention to generated text tokens. Further develops Adaptive IAT (AdaIAT) with layer-wise thresholds to control intervention timing and fine-grained amplification magnitude tailored to each attention head's characteristics, preventing impairment of the model's inherent prediction capabilities.

Result: AdaIAT significantly reduces hallucination rates (CS and CI on LLaVA-1.5 by 35.8% and 37.1% respectively) while avoiding repetitive descriptions. It preserves linguistic performance and prediction capability, achieving an attractive trade-off between hallucination reduction and model performance.

Conclusion: AdaIAT effectively mitigates hallucinations in LVLMs by leveraging generated text attention patterns, offering a balanced solution that reduces hallucinations while maintaining linguistic coherence and model capabilities.
...
TL;DR: This paper develops a system for detecting and tracking people in industrial indoor environments using an overhead crane-mounted LiDAR, addressing domain shift challenges and releasing a new dataset.


<details>
  <summary>Details</summary>
Motivation: The overhead viewpoint of crane-mounted LiDAR creates a strong domain shift from standard vehicle-centric LiDAR benchmarks, and there's limited public training data for this specific industrial scenario.

Method: Curated a site-specific overhead LiDAR dataset with 3D human bounding-box annotations, adapted selected 3D detectors under unified training/evaluation protocol, and integrated lightweight tracking-by-detection using AB3DMOT and SimpleTrack.

Result: Best detectors achieved AP up to 0.84 within 5.0m horizontal radius, increasing to 0.97 at 1.0m. VoxelNeXt and SECOND were most reliable backbones. System demonstrated real-time feasibility with latency measurements.

Conclusion: The work bridges the domain gap between standard driving datasets and overhead sensing for person detection/tracking, with released dataset and implementations to support further research in industrial safety applications.
...
TL;DR: A prototype-based weakly-supervised framework for interpretable prostate cancer grading from histopathology images that mimics pathologist reasoning by comparing suspicious regions with clinically validated examples.


<details>
  <summary>Details</summary>
Motivation: Prostate cancer grading is tedious and subjective, creating heavy workload for pathologists. While deep learning shows promise, limited interpretability hinders adoption in medical applications. Existing interpretability methods provide coarse explanations but don't reveal why highlighted regions matter.

Method: 1) Pre-train network at patch-level to learn robust prototypical features for each grade. 2) Fine-tune with new prototype-aware loss function for weakly-supervised grading. 3) Introduce attention-based dynamic pruning mechanism to handle inter-sample heterogeneity and selectively emphasize relevant prototypes.
...
Conclusion: The proposed prototype-based weakly-supervised framework provides interpretable prostate cancer grading that mirrors pathologist reasoning, making it more trustworthy for clinical adoption compared to traditional black-box deep learning approaches.
...
<details>
  <summary>Details</summary>
Motivation: Standard vision encoders trained on contrastive or classification objectives fail to capture subtle visual variations needed for medical differential diagnosis, which requires comparing multiple images to distinguish disease progression from acquisition differences.
...
Conclusion: Location-aware pretraining tasks enable vision encoders to learn fine-grained visual representations crucial for medical differential diagnosis, outperforming traditional pre-training methods in medical difference VQA tasks.
...
Result: VisionPangu achieves competitive performance while producing more structured and detailed captions compared to larger models. Demonstrates that compact multimodal models can excel at detailed image captioning.
...
TL;DR: A novel camera model for monocular 3DMM regression that captures perspective distortion in close-up facial images by extending orthographic projection with a shrinkage parameter.


<details>
  <summary>Details</summary>
Motivation: Existing 3DMM regression methods use orthographic projection for stability but fail with close-up footage (like head-mounted camera videos) where perspective distortion is significant. Orthographic projection eliminates focal length/distance ambiguity but is unsuitable for close-up facial images.

Method: Extends orthographic projection with a new shrinkage parameter to incorporate pseudo-perspective effects while preserving stability. Presents techniques for fine-tuning existing models and validates using custom head-mounted camera dataset.

Result: Demonstrates effectiveness through quantitative and qualitative comparisons on custom dataset recorded with head-mounted cameras, showing improved performance for close-up facial footage.

Conclusion: The proposed camera model successfully addresses perspective distortion in close-up facial images while maintaining the stability advantages of orthographic projection, making 3DMM regression suitable for head-mounted camera applications.
...
TL;DR: BiEvLight is a hierarchical framework for low-light image enhancement using event cameras that jointly optimizes event denoising and image enhancement through bilevel optimization, achieving state-of-the-art results.


<details>
  <summary>Details</summary>
Motivation: Event cameras have high dynamic range but suffer from dual degradation: background activity noise in events and low SNR in images, causing severe noise coupling during fusion. Precise event denoising is crucial for unlocking event-based fusion potential.

Method: Proposes BiEvLight, a hierarchical task-aware framework that collaboratively optimizes enhancement and denoising. Uses gradient-guided event denoising prior based on image-event gradient correlation, and formulates denoising as a bilevel optimization problem constrained by enhancement objectives.

Result: Significantly outperforms SOTA methods on Real-world noise Dataset SDE with average improvements of 1.30dB in PSNR, 2.03dB in PSNR*, and 0.047 in SSIM.

Conclusion: BiEvLight demonstrates that treating event denoising as a bilevel optimization problem constrained by enhancement objectives, rather than static pre-processing, substantially improves low-light image enhancement quality with event cameras.
...
<details>
  <summary>Details</summary>
Motivation: Existing approaches use Supervised Fine-Tuning with token-level cross-entropy loss as an indirect proxy, leading to misalignment between training objectives and task performance. RLVR has shown promise for LLM reasoning but remains under-explored for 3D scene understanding.

Method: 3D-RFT first activates 3D-aware MLLMs via SFT, then performs reinforcement fine-tuning using Group Relative Policy Optimization (GRPO) with strictly verifiable reward functions. Task-specific rewards are designed directly from metrics like 3D IoU and F1-Score.
...
TL;DR: VideoHV-Agent reformulates long-video QA as hypothesis-verification process with four specialized modules: Thinker generates testable hypotheses, Judge derives discriminative clues, Verifier grounds clues in fine-grained video content, and Answer integrates evidence for final prediction.


<details>
  <summary>Details</summary>
Motivation: Long video understanding faces challenges of visual redundancy, long-range dependencies, and semantic drift in chain-of-thought/retrieval approaches. The paper argues for "thinking-before-finding" - first articulating what must be true for each answer candidate before retrieval.

Method: VideoHV-Agent uses four modules: 1) Thinker rewrites answer candidates into testable hypotheses based on video summaries, 2) Judge derives discriminative clues specifying what evidence must be checked, 3) Verifier grounds and tests clues using localized fine-grained video content, 4) Answer integrates validated evidence for final answer.
...
Conclusion: The hypothesis-verification framework with "thinking-before-finding" principle effectively addresses long-video reasoning challenges by reducing semantic drift, improving logical reasoning, and making the process more interpretable and computationally efficient.
...
TL;DR: Wallaroo is an autoregressive model using next-token prediction to unify multimodal understanding, image generation, and editing with multi-resolution support and bilingual capabilities.


<details>
  <summary>Details</summary>
Motivation: To create a unified model that can handle multiple visual tasks (understanding, generation, editing) through a simple autoregressive approach, supporting both Chinese and English languages.

Method: Uses next-token prediction with decoupled visual encoding pathways and a four-stage training strategy to reshape model capabilities for multimodal tasks.

Result: Achieves competitive or superior performance compared to other unified models on various benchmarks, demonstrating autoregressive models' potential for unifying multimodal understanding and generation.

Conclusion: Wallaroo shows great promise for unifying multimodal tasks through autoregressive modeling, with support for multi-resolution images and bilingual operation.
...
TL;DR: TAPFormer is a transformer-based framework for robust point tracking that performs asynchronous fusion of RGB frames and event streams, addressing temporal misalignment and modality failure issues.


<details>
  <summary>Details</summary>
Motivation: Existing methods for combining RGB frames and event streams typically use synchronous or non-adaptive fusion, leading to temporal misalignment and performance degradation when one modality fails, especially in challenging conditions like blur or low light.

Method: TAPFormer uses two key innovations: 1) Transient Asynchronous Fusion (TAF) that models temporal evolution between discrete frames through continuous event updates, bridging low-rate frames and high-rate events; 2) Cross-modal Locally Weighted Fusion (CLWF) that adaptively adjusts spatial attention based on modality reliability for stable features.

Result: The method achieves 28.2% improvement in average pixel error within threshold on their new real-world frame-event dataset, and consistently achieves best performance on standard point tracking benchmarks.

Conclusion: TAPFormer demonstrates that asynchronous temporal-consistent fusion of frames and events enables robust, high-frequency arbitrary point tracking, outperforming existing methods through better handling of temporal alignment and modality reliability.
...
TL;DR: MultiGO++: A monocular 3D clothed human reconstruction framework that addresses texture, geometry, and supervision limitations through multi-source texture synthesis, region-aware shape extraction, and dual reconstruction U-Net for geometry-texture collaboration.


<details>
  <summary>Details</summary>
Motivation: Existing methods suffer from three key limitations: textural limitations due to unavailability of training data, geometric limitations from inaccurate external priors, and systematic limitations from biased single-modality supervision, all leading to suboptimal 3D human reconstruction from single images.

Method: Three core components: (1) Multi-source texture synthesis strategy constructing 15,000+ 3D textured human scans for better texture estimation; (2) Region-aware shape extraction module with Fourier geometry encoder for effective geometry learning; (3) Dual reconstruction U-Net leveraging geometry-texture collaborative features to generate high-fidelity textured 3D meshes.

Result: Extensive experiments on two benchmarks and in-the-wild cases show superiority over state-of-the-art approaches, demonstrating improved reconstruction quality through effective geometry-texture collaboration.

Conclusion: MultiGO++ addresses key limitations in monocular 3D clothed human reconstruction through systematic geometry-texture collaboration, achieving superior performance by overcoming texture data scarcity, geometric inaccuracies, and modality bias in existing methods.
...
TL;DR: Lens2Zernike is a deep learning framework that blindly recovers physical optical parameters from single blurred images, enabling stable non-blind deconvolution for mobile photography restoration.


<details>
  <summary>Details</summary>
Motivation: Current deep learning deblurring methods lack explicit optical modeling and can hallucinate details, while classical blind deconvolution is unstable. There's a need to bridge this gap by recovering physical optical parameters for more reliable restoration.

Method: A multi-task framework with three supervision domains: 1) direct Zernike coefficient regression, 2) differentiable physics constraints covering wavefront and point spread function derivations, and 3) auxiliary multi-task spatial map predictions. Uses ResNet-18 backbone with physics-consistent strategy.

Result: The full multi-task framework (z+p+m) achieves 35% improvement over coefficient-only baselines. Outperforms two established deep learning methods with significantly lower regression errors. Enables stable non-blind deconvolution with substantial in-domain improvement on IDMxS Mobile Camera Lens Database.

Conclusion: Lens2Zernike successfully bridges the gap between black-box deep learning and unstable classical methods by recovering physical optical parameters, enabling reliable restoration of diffraction-limited details from severely aberrated mobile captures.
...
TL;DR: This paper presents a large-scale study of generative image restoration models, revealing a paradigm shift from detail scarcity to detail quality/semantic control challenges, and introduces a new multi-dimensional evaluation pipeline and IQA model.


<details>
  <summary>Details</summary>
Motivation: To systematically evaluate how far generative image restoration (GIR) has truly advanced compared to previous methods, and to understand the current state and limitations of modern GIR models.
...
Result: Uncovered critical performance disparities between different architectures and identified a key evolution in failure modes - a paradigm shift from detail scarcity (under-generation) to detail quality and semantic control (preventing over-generation).

Conclusion: The study provides systematic insights redefining our understanding of generative image restoration's true state, charts a course for future development, and introduces a new IQA model that better aligns with human perceptual judgments.
...
TL;DR: Tell2Adapt is a novel Source Free Unsupervised Domain Adaptation framework for medical image segmentation that leverages Vision Foundation Models to create a unified, multi-modality, multi-target approach through context-aware prompt regularization and visual plausibility refinement.


<details>
  <summary>Details</summary>
Motivation: Existing SFUDA methods are limited to low-gap, specific domain shifts and cannot generalize to unified multi-modality, multi-target frameworks, creating barriers for real-world clinical deployment across diverse settings.

Method: The framework uses Vision Foundation Models with Context-Aware Prompts Regularization (CAPR) to generate high-quality pseudo-labels, and Visual Plausibility Refinement (VPR) to ground predictions in target images' low-level visual features, adapting lightweight student models to target domains.

Result: Extensive evaluation across 10 domain adaptation directions and 22 anatomical targets (brain, cardiac, polyp, abdominal) demonstrates consistent outperformance of existing approaches, achieving state-of-the-art for unified SFUDA in medical image segmentation.

Conclusion: Tell2Adapt provides a robust, unified SFUDA framework that effectively addresses multi-modality, multi-target domain adaptation challenges in clinical settings by leveraging VFM knowledge while ensuring clinical reliability through visual plausibility refinement.
...
TL;DR: This paper introduces Semap, a new benchmark dataset for historical map segmentation, and proposes a generalizable framework that outperforms existing methods by leveraging data diversity and multiscale integration.


<details>
  <summary>Details</summary>
Motivation: Historical map collections are diverse in style, scale, and geographic focus, but most existing map recognition work focuses on specialist models for homogeneous map series. There's a need for generalizable approaches that can handle the variety of historical map documents.

Method: The paper presents a segmentation framework combining procedural data synthesis with multiscale integration to improve robustness and transferability. It introduces Semap, a new open benchmark dataset with 1,439 manually annotated patches designed to reflect historical map variety.

Result: The framework achieves state-of-the-art performance on both HCMSSD and Semap datasets, showing that segmentation performance remains largely stable across map collections, scales, geographic regions, and publication contexts.

Conclusion: A diversity-driven approach to map recognition is viable and beneficial. By proposing benchmark datasets and methods for generic segmentation of historical maps, this work enables integration of diverse cartographic archives into historical geographic studies.
...
TL;DR: IRTTA improves medical image segmentation by adapting downstream networks to intermediate reconstruction representations during test-time, providing better performance and uncertainty estimates without modifying original models.


<details>
  <summary>Details</summary>
Motivation: Low-cost medical imaging devices rely on reconstruction algorithms, but current methods only use final reconstructed images, ignoring valuable intermediate representations that could improve downstream task performance.

Method: IRTTA adapts normalization-layer parameters of a frozen downstream network via a modulator network that conditions on reconstruction timescale. The modulator is learned during test-time using averaged entropy loss across all timesteps, enabling uncertainty estimation from timestep-wise segmentation variations.

Result: The approach enhances segmentation performance and provides semantically meaningful uncertainty estimates without modifying either the reconstruction process or downstream model.

Conclusion: Exploiting intermediate reconstruction representations through test-time adaptation improves medical image analysis performance and provides valuable uncertainty information for clinical decision-making.
...
<details>
  <summary>Details</summary>
Motivation: Current multi-camera 3D object detection models struggle to generalize to unseen platforms with new camera configurations due to spatial prior discrepancies (different intrinsics, extrinsics, and array layouts) between source and target configurations.
...
Conclusion: CoIn3D provides a generalizable framework for multi-camera 3D object detection that effectively addresses spatial prior discrepancies, enabling strong transferability from source configurations to unseen target ones.
...
Conclusion: CLIP-PZSL effectively handles label ambiguity in zero-shot learning through CLIP-driven feature extraction and progressive label refinement, showing superior performance on multiple datasets.
...
TL;DR: Advanced emergency vehicle blue light detection system using 4 fisheye cameras and enhanced RT-DETR with color attention, achieving 94.7% accuracy and 70m detection range for ADAS integration.


<details>
  <summary>Details</summary>
Motivation: To enhance road safety and Advanced Driver Assistance Systems (ADAS) by developing a reliable system for detecting emergency vehicle blue lights, which is crucial for timely driver awareness and response to approaching emergency vehicles.

Method: Uses ABLDataset with European emergency vehicle images under various conditions. System employs 4 fisheye cameras (180° FOV each) mounted on vehicle sides. Comparative analysis of YOLO (v5, v8, v10), RetinaNet, Faster R-CNN, and RT-DETR. RT-DETR selected as base model and enhanced with color attention block. Calibration enables azimuthal localization, and geometric transformations estimate approach angle relative to vehicle center.

Result: Achieved 94.7% accuracy and 94.1% recall on test set. Field tests show detection up to 70 meters. System designed for multimodal integration with acoustic data. High efficiency demonstrated for ADAS applications.

Conclusion: The developed system offers a promising approach for emergency vehicle detection with high accuracy and practical detection range, suitable for integration into multimodal ADAS systems to enhance road safety through combined visual and acoustic data processing.
...
TL;DR: MI-DETR is a bio-inspired dual-pathway detector for infrared small target detection that explicitly models motion using a retina-inspired cellular automaton and parvocellular-magnocellular interconnection, achieving state-of-the-art performance without extra motion labels.


<details>
  <summary>Details</summary>
Motivation: Infrared small target detection is challenging due to tiny, low-contrast targets in complex backgrounds. Existing multi-frame approaches require additional motion supervision or explicit alignment modules, making them complex and inefficient.
...
Result: Achieves 70.3% mAP@50 and 72.7% F1 on IRDST-H (+26.35 mAP@50 improvement), 98.0% mAP@50 on DAUB-R, and 88.3% mAP@50 on ITSDT-15K, demonstrating superior performance on three ISTD benchmarks.

Conclusion: The biologically inspired motion-appearance integration approach provides a simple yet effective solution for infrared small target detection, eliminating the need for extra motion labels or alignment operations while achieving strong performance.
...
TL;DR: UniM is the first unified benchmark for any-to-any interleaved multimodal learning, covering 7 modalities across 30 domains, with evaluation suite and agentic baseline model UniMA.


<details>
  <summary>Details</summary>
Motivation: Real-world multimodal applications require systems to comprehend arbitrarily combined and interleaved multimodal inputs while generating outputs in any interleaved multimedia form, posing new challenges for MLLMs.

Method: Introduces UniM benchmark with 31K high-quality instances across 30 domains and 7 modalities (text, image, audio, video, document, code, 3D), plus UniM Evaluation Suite with three assessment dimensions, and proposes UniMA agentic baseline model with traceable reasoning.
...
Conclusion: UniM provides the first unified benchmark for any-to-any interleaved multimodal learning, with evaluation suite and baseline model, fostering advancement in multimodal AI capabilities.
...
<details>
  <summary>Details</summary>
Motivation: Existing dynamic 4D scene reconstruction methods are computationally expensive and require additional supervision, making them impractical for real-time applications. Moving objects corrupt camera pose estimation, creating challenges for dynamic scene reconstruction.

Method: MoRe uses a feedforward network built on a static reconstruction backbone with attention-forcing strategy to disentangle dynamic motion from static structure. It employs grouped causal attention to capture temporal dependencies and adapt to varying token lengths across frames. The model is fine-tuned on large-scale, diverse datasets containing both dynamic and static scenes.

Result: Extensive experiments on multiple benchmarks demonstrate that MoRe achieves high-quality dynamic reconstructions with exceptional efficiency, outperforming existing methods in terms of computational performance.

Conclusion: MoRe provides an efficient, feedforward solution for dynamic 4D scene reconstruction from monocular videos that addresses the limitations of existing optimization-based methods, making it practical for real-time applications.
...
TL;DR: A novel STD-4D Diffusion framework transfers spatial priors from 3D models and temporal priors from video models to overcome 4D dataset limitations, achieving superior 4D synthesis quality.


<details>
  <summary>Details</summary>
Motivation: Current 4D synthesis research is severely constrained by the lack of large-scale 4D datasets, preventing models from adequately learning spatial-temporal features needed for high-quality 4D generation.

Method: Proposes STD-4D Diffusion model with disentangled spatial and temporal latents, Orthogonal Spatial-temporal Distributional Transfer (Orster) mechanism for feature injection, and spatial-temporal-aware HexPlane (ST-HexPlane) for 4D construction.

Result: Experiments demonstrate the method significantly outperforms existing approaches, achieving superior spatial-temporal consistency and higher-quality 4D synthesis.

Conclusion: The framework successfully addresses 4D dataset limitations by transferring knowledge from existing 3D and video models, enabling high-quality 4D content generation in the AIGC era.
...
TL;DR: GEM-TFL is a weakly supervised temporal forgery localization method that uses graph-based EM optimization to bridge training-inference gaps and improve localization accuracy with only video-level labels.
...
Conclusion: GEM-TFL effectively addresses key limitations in weakly supervised TFL through its EM-powered optimization and graph-based refinement, providing a practical solution that reduces labeling costs while maintaining localization accuracy.
...
TL;DR: Diff-ES: Evolutionary search-based structured pruning for diffusion models that optimizes stage-wise sparsity schedules without model duplication, achieving real acceleration with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Diffusion models are computationally expensive due to multi-step denoising and large model sizes. Existing pruning methods use heuristic sparsity schedules and require stitching multiple models during inference, leading to suboptimal performance and memory overhead.

Method: Divides diffusion trajectory into stages, uses evolutionary search to automatically discover optimal stage-wise sparsity schedules, and implements memory-efficient weight routing without model duplication. Integrates with existing structured pruning methods (depth/width pruning).

Result: Consistently achieves wall-clock speedups on DiT and SDXL with minimal generation quality degradation, establishing state-of-the-art performance for structured diffusion model pruning.

Conclusion: Diff-ES provides an effective framework for efficient diffusion model pruning that balances acceleration and quality preservation through automated sparsity schedule optimization and memory-efficient implementation.
...
TL;DR: BLINK is a trajectory-based recurrent state-space model that learns latent interaction dynamics from NK-tumor cell sequences to predict cytotoxic outcomes and enable forecasting with interpretable behavioral modes.


<details>
  <summary>Details</summary>
Motivation: Current frame-wise classification approaches are insufficient for understanding NK cell cytotoxicity, which emerges over time from cellular interactions and cannot be reliably inferred from single frames alone.

Method: BLINK is a trajectory-based recurrent state-space model that learns latent interaction dynamics from partially observed NK-tumor interaction sequences and predicts apoptosis increments that accumulate into cytotoxic outcomes.

Result: Experiments show improved cytotoxic outcome detection, enable forecasting of future outcomes, and provide an interpretable latent representation that organizes NK trajectories into coherent behavioral modes and temporally structured interaction phases.

Conclusion: BLINK provides a unified framework for quantitative evaluation and structured modeling of NK cytotoxic behavior at the single-cell level, advancing understanding of cellular interaction dynamics.
...
<details>
  <summary>Details</summary>
Motivation: Existing PAR research is limited by the "one-model-per-dataset" paradigm and struggles with domain discrepancies across modalities, attribute definitions, and environmental scenarios. There's a need for a unified approach that can handle diverse data sources and improve generalization.

Method: Proposes UniPAR with: 1) unified data scheduling strategy, 2) dynamic classification head for multiple datasets, 3) phased fusion encoder that aligns visual features with textual attribute queries through late deep fusion, enabling processing of RGB images, video sequences, and event streams.

Result: Achieves performance comparable to specialized SOTA methods on MSP60K, DukeMTMC, and EventPAR datasets. Multi-dataset joint training significantly enhances cross-domain generalization and recognition robustness in extreme environments (low light, motion blur).

Conclusion: UniPAR successfully addresses limitations of traditional PAR approaches by providing a unified framework that handles heterogeneous modalities and datasets, demonstrating strong performance and improved generalization capabilities across diverse domains.
...
TL;DR: SRasP improves cross-domain few-shot learning by using crop-global style perturbations with multi-objective optimization for better domain adaptation and generalization.


<details>
  <summary>Details</summary>
Motivation: Existing style-based perturbation methods for CD-FSL suffer from gradient instability and convergence to sharp minima, limiting their effectiveness in transferring knowledge from seen to unseen domains.

Method: Proposes Self-Reorientation Adversarial Style Perturbation (SRasP) that identifies incoherent crops using global semantic guidance, reorients and aggregates their style gradients with global gradients, and uses multi-objective optimization to maximize visual discrepancy while maintaining semantic consistency.
...
Conclusion: SRasP effectively addresses gradient instability in style-based perturbations, enabling better generalization to unseen domains through crop-global style perturbation and multi-objective optimization.
...
TL;DR: Proposes an adaptive VLA framework that dynamically routes execution based on task complexity, using vision-only embeddings for efficient complexity detection, achieving 80% F1-score with minimal training data.


<details>
  <summary>Details</summary>
Motivation: Current VLA models increase computational complexity indiscriminately, inefficiently allocating resources for trivial tasks while lacking uncertainty estimation to prevent catastrophic failure on out-of-distribution tasks.

Method: Transforms VLA's vision-language backbone into an active detection tool by projecting latent embeddings into ensemble estimators. Dynamically routes execution: Act (immediate execution), Think (reason about ambiguity), Abstain (halt for anomalies). Uses vision-only embeddings for complexity detection due to semantic invariance of language.

Result: Vision-only configuration achieves 80% F1-Score using only 5% of training data on LIBERO and LIBERO-PRO benchmarks and real robot. Visual embeddings alone are superior for inferring task complexity.

Conclusion: Proposed adaptive framework provides reliable and efficient task complexity detection, enabling dynamic resource allocation and preventing catastrophic failures through uncertainty estimation.
...
TL;DR: SSR-GS is a 3D Gaussian splatting framework that improves glossy surface reconstruction by modeling both direct and indirect specular reflections, using visual geometry priors to handle challenging reflection scenarios.


<details>
  <summary>Details</summary>
Motivation: Current 3D Gaussian splatting methods struggle with accurately reconstructing glossy surfaces under complex illumination, especially with strong specular reflections and multi-surface interreflections, which limits their practical applications in realistic scene reconstruction.

Method: Proposes SSR-GS with: 1) Prefiltered Mip-Cubemap for efficient direct specular reflection modeling, 2) IndiASG module for capturing indirect specular reflections, and 3) Visual Geometry Priors (VGP) that combine reflection-aware visual priors (via reflection score) with geometry priors from VGGT including decayed depth supervision and transformed normal constraints.

Result: Extensive experiments on synthetic and real-world datasets show SSR-GS achieves state-of-the-art performance in glossy surface reconstruction, effectively handling challenging reflection scenarios.

Conclusion: SSR-GS successfully addresses the challenge of glossy surface reconstruction in 3D Gaussian splatting by comprehensive modeling of specular reflections and integration of visual geometry priors, advancing the field of novel view synthesis for reflective surfaces.
...
Method: The study investigates three image preprocessing methods: lung masking, lung cropping, and Contrast Limited Adaptive Histogram Equalization (CLAHE). These aim to suppress racial information cues while preserving diagnostic accuracy. The approaches focus on bounding box-based lung cropping as a particularly effective technique.

Result: Simple bounding box-based lung cropping effectively reduces racial shortcut learning while maintaining diagnostic model performance. This finding bypasses the frequently postulated fairness-accuracy trade-offs in AI healthcare models.

Conclusion: Basic image preprocessing techniques like lung cropping can mitigate racial bias in chest X-ray AI models without sacrificing diagnostic accuracy, offering a practical solution to address healthcare equity concerns in medical AI systems.
...
TL;DR: Proposes a method to calibrate generic camera models using blurry images by simultaneously estimating feature locations and point spread functions, reducing the need for many sharp calibration images.


<details>
  <summary>Details</summary>
Motivation: Generic camera calibration requires many images, making motion blur unavoidable for individual users. Conventional deblurring methods don't address the translational ambiguity in calibration tasks.

Method: Uses geometric constraints and local parametric illumination model to simultaneously estimate feature locations and spatially varying point spread functions while resolving translational ambiguity.

Result: Experimental results validate the effectiveness of the approach for calibrating generic camera models using blurry images.

Conclusion: The proposed method successfully addresses the motion blur problem in generic camera calibration, enabling accurate calibration with fewer images despite blur.
...
TL;DR: Mario is a unified LLM-based framework for reasoning on multimodal graphs that addresses cross-modal consistency and modality preference challenges through graph-conditioned VLM design and modality-adaptive instruction tuning.


<details>
  <summary>Details</summary>
Motivation: Existing methods use pretrained VLMs to encode image-text pairs in isolation, ignoring the relational structure of real-world multimodal data. This motivates reasoning on multimodal graphs where nodes have textual/visual attributes and edges provide structural cues.
...
Conclusion: Mario successfully enables effective LLM-based reasoning over multimodal graphs by addressing cross-modal consistency and modality preference challenges, demonstrating superior performance across various graph reasoning tasks.
...
TL;DR: Logi-PAR is a novel logic-infused framework for patient activity recognition that learns explicit logical rules from visual cues, providing auditable explanations and counterfactual reasoning beyond traditional classification.


<details>
  <summary>Details</summary>
Motivation: Current patient activity recognition models only identify activities through implicit neural patterns, lacking explicit reasoning about why visual cues imply risks. Advancing clinical safety requires methods that can compositionally reason through explicit logic beyond mere classification.

Method: Logi-PAR integrates contextual fact fusion as a multi-view primitive extractor and injects neural-guided differentiable rules. It automatically learns rules from visual cues, optimizing them end-to-end while enabling implicit patterns to be explicitly labelled during training.
...
Conclusion: Logi-PAR is the first framework to recognize patient activity by applying learnable logic rules to symbolic mappings, enabling explicit reasoning, auditable explanations, and counterfactual analysis for improved clinical safety.
...
TL;DR: SCDL framework improves medical image segmentation by addressing class imbalance through learning structured class-conditional feature distributions, achieving SOTA results on Synapse and AMOS datasets.


<details>
  <summary>Details</summary>
Motivation: Medical image segmentation faces challenges with expensive pixel-level annotation and severe class imbalance, where minority structures get overwhelmed by dominant classes in feature representations, hindering discriminative feature learning.

Method: Proposes Semantic Class Distribution Learning (SCDL) framework with Class Distribution Bidirectional Alignment (CDBA) to align embeddings with learnable class proxies, and Semantic Anchor Constraints (SAC) to guide proxies using labeled data.

Result: SCDL significantly improves segmentation performance on Synapse and AMOS datasets across overall and class-level metrics, with particularly strong gains on minority classes, achieving state-of-the-art results.

Conclusion: SCDL effectively mitigates supervision and representation biases in medical image segmentation through structured class distribution learning, providing a plug-and-play solution for class imbalance problems.
...
<details>
  <summary>Details</summary>
Motivation: Near-ground sensors for measuring NSAT are sparse and unevenly distributed, limiting continuous spatial measurements, while satellites capture detailed surface properties but don't directly measure atmospheric conditions near the surface.

Method: SPyCer frames NSAT prediction as pixel-wise vision problem, projecting sensors onto satellite images. It uses supervised learning for sensor pixels with observed NSAT and physics constraints, plus physics-guided regularization for surrounding pixels based on surface energy balance and advection-diffusion-reaction PDEs. Multi-head attention guided by land cover with Gaussian distance weighting captures neighboring pixel influences.

Result: SPyCer produces spatially coherent and physically consistent NSAT estimates, outperforming existing baselines in accuracy, generalization, and alignment with underlying physical processes on real-world datasets.

Conclusion: SPyCer successfully bridges the gap between sparse near-ground sensors and satellite imagery by integrating physical modeling into deep learning, enabling continuous spatial estimation of near-surface air temperature with improved physical consistency.
...
TL;DR: A digital twin-driven robotic system for textile sorting uses multimodal perception, grasp prediction, and Visual Language Models to autonomously classify garments and detect foreign objects in cluttered environments.


<details>
  <summary>Details</summary>
Motivation: There's increasing demand for sustainable textile recycling requiring automation that can handle deformable garments and detect foreign objects in cluttered industrial environments.

Method: A dual-arm robotic cell with RGBD sensing, capacitive tactile feedback, and collision-aware motion planning integrates grasp prediction, multimodal perception, and semantic reasoning. Nine VLMs from five families are benchmarked on 223 inspection scenarios. Digital twin technology with MoveIt enables collision-aware path planning and integrates 3D point clouds.

Result: Qwen model family achieves highest overall accuracy (87.9%) with strong foreign object detection. Lighter models like Gemma3 offer competitive speed-accuracy trade-offs for edge deployment. The system successfully demonstrates autonomous textile sorting in realistic settings.

Conclusion: The system shows feasibility of combining semantic VLM reasoning with conventional grasp detection and digital twin technology for scalable, autonomous textile sorting in industrial recycling applications.
...
TL;DR: CATNet is an adaptive compensation framework that addresses temporal latency and noise interference in multi-agent cooperative perception systems through spatio-temporal synchronization, wavelet-based denoising, and adaptive feature selection.
...
Conclusion: CATNet effectively addresses practical limitations in real-world multi-agent cooperative perception by solving temporal latency and noise interference problems, proving to be a robust and adaptable framework for complex traffic scenarios.
...
TL;DR: Wiki-R1 is a curriculum reinforcement learning framework that improves MLLMs' reasoning for KB-VQA by generating training data at appropriate difficulty levels and selecting informative samples.


<details>
  <summary>Details</summary>
Motivation: KB-VQA requires integrating external knowledge with visual understanding, but noisy retrieval and structured knowledge bases create distribution gaps from pretrained MLLMs, making effective reasoning and domain adaptation difficult.

Method: Proposes Wiki-R1 framework with: 1) controllable curriculum data generation that manipulates retrievers to produce samples at desired difficulty levels, 2) curriculum sampling strategy that selects informative samples likely to yield non-zero advantages during RL updates, and 3) difficulty estimation using observed rewards propagated to unobserved samples.

Result: Achieves new SOTA results on two KB-VQA benchmarks: improves accuracy from 35.5% to 37.1% on Encyclopedic VQA and from 40.1% to 44.1% on InfoSeek.

Conclusion: Wiki-R1 effectively bridges the distribution gap from pretraining to KB-VQA by systematically incentivizing reasoning through curriculum reinforcement learning with controlled data generation and sampling strategies.
...
TL;DR: Intermediate layers in vision transformers often outperform final layers, primarily due to distribution shift between pretraining and downstream data. Optimal probing locations vary: FFN activations work best under strong distribution shift, while normalized attention outputs are optimal under weak shift.


<details>
  <summary>Details</summary>
Motivation: To understand why intermediate layers of foundation models (including vision transformers) often yield more discriminative representations than final layers, and to determine the underlying causes of this phenomenon beyond just autoregressive pretraining.

Method: Conducted comprehensive linear probing experiments across diverse image classification benchmarks, analyzed performance degradation patterns, and performed fine-grained module-level analysis of transformer components (feedforward networks and multi-head self-attention modules).

Result: Distribution shift between pretraining and downstream data is the primary cause of performance degradation in deeper layers. Standard probing of transformer block outputs is suboptimal - FFN activations perform best under significant distribution shift, while normalized attention outputs are optimal under weak shift.

Conclusion: The study provides insights into optimal probing strategies for vision transformers based on distribution shift characteristics, challenging conventional approaches to using intermediate layer representations.
...
TL;DR: Fusion4CA enhances BEV-based 3D object detection by better exploiting RGB information through contrastive alignment, camera auxiliary branch, pretrained weights utilization, and coordinate attention, achieving improved performance with minimal parameter overhead.


<details>
  <summary>Details</summary>
Motivation: Existing LiDAR-RGB fusion methods for 3D object detection in autonomous driving over-rely on LiDAR data and insufficiently explore RGB information, limiting performance and generalization capabilities.

Method: Built on BEVFusion framework with four key components: 1) Contrastive alignment module to calibrate image features with 3D geometry, 2) Camera auxiliary branch to sufficiently mine RGB information during training, 3) Off-the-shelf cognitive adapter to leverage pretrained image weights, 4) Coordinate attention module integrated into fusion stage.

Result: Achieves 69.7% mAP on nuScenes dataset with only 6 training epochs (vs. 20 epochs baseline), with only 3.48% increase in inference parameters, yielding 1.2% improvement over baseline. Validated effectiveness in simulated lunar environment.

Conclusion: Fusion4CA effectively addresses RGB underutilization in LiDAR-RGB fusion for 3D detection, achieving superior performance with minimal computational overhead and demonstrating strong generalization to different environments.
...
TL;DR: SpectralCache is a training-free caching framework that accelerates Diffusion Transformers by addressing three axes of non-uniformity in denoising: temporal, depth, and feature dimensions, achieving 2.46x speedup with minimal quality loss.


<details>
  <summary>Details</summary>
Motivation: Existing caching methods for Diffusion Transformers treat the denoising process as uniform across time, depth, and features, but this overlooks important non-uniformities that limit acceleration potential while maintaining quality.

Method: Proposes SpectralCache with three components: 1) Timestep-Aware Dynamic Scheduling (TADS) for temporal non-uniformity, 2) Cumulative Error Budgets (CEB) for depth non-uniformity, and 3) Frequency-Decomposed Caching (FDC) for feature non-uniformity.
...
Conclusion: SpectralCache provides a unified, training-free, plug-and-play caching framework that effectively addresses three orthogonal axes of non-uniformity in DiT denoising, achieving significant speedups with minimal quality degradation.
...
TL;DR: Dark3R is a framework for structure from motion in extreme low-light conditions that works directly on raw images with SNRs below -4dB, using teacher-student distillation from 3D foundation models without 3D supervision.


<details>
  <summary>Details</summary>
Motivation: Conventional feature- and learning-based methods break down in extreme low-light conditions with signal-to-noise ratios below -4dB, creating a need for robust structure from motion in the dark.

Method: Dark3R adapts large-scale 3D foundation models to extreme low-light conditions through teacher-student distillation, trained solely on noisy-clean raw image pairs (either captured or synthesized using Poisson-Gaussian noise model).

Result: Dark3R achieves state-of-the-art structure from motion in low-SNR regime and state-of-the-art novel view synthesis in the dark using predicted poses with coarse-to-fine radiance field optimization.
...
TL;DR: Proposes ORMOT - Omnidirectional Referring Multi-Object Tracking to address FoV limitations in conventional RMOT by using omnidirectional imagery, introduces ORSet dataset and ORTrack LVLM framework.


<details>
  <summary>Details</summary>
Motivation: Existing Referring Multi-Object Tracking (RMOT) methods are limited by conventional cameras' narrow field of view, causing targets to move out of frame and leading to fragmented tracking and loss of contextual information.

Method: Introduces ORMOT task extension to omnidirectional imagery, creates ORSet dataset with 27 omnidirectional scenes, 848 language descriptions, and 3,401 annotated objects, and proposes ORTrack - a Large Vision-Language Model-driven framework for ORMOT.

Result: Extensive experiments on ORSet dataset demonstrate the effectiveness of the ORTrack framework for Omnidirectional Referring Multi-Object Tracking.

Conclusion: ORMOT addresses FoV limitations in RMOT, ORSet provides rich visual-temporal-language data, and ORTrack shows promising results for tracking objects with language descriptions in omnidirectional imagery.
...
TL;DR: Fusion-CAM is a novel XAI framework that combines gradient-based and region-based CAM methods through adaptive fusion to produce more robust and discriminative visual explanations of CNN decisions.


<details>
  <summary>Details</summary>
Motivation: Current CAM methods have limitations: gradient-based approaches (like Grad-CAM) provide fine details but are noisy and incomplete, while region-based approaches (like Score-CAM) capture broader coverage but suffer from over-smoothing and reduced sensitivity. There's a need to bridge this explanatory gap for better interpretability.

Method: Fusion-CAM unifies both paradigms through a three-step fusion mechanism: 1) denoising gradient-based maps for cleaner activations, 2) combining refined gradient maps with region-based maps using contribution weights to enhance class coverage, and 3) adaptive similarity-based pixel-level fusion that dynamically adjusts fusion strength based on agreement between paradigms.

Result: Extensive experiments on standard benchmarks show that Fusion-CAM consistently outperforms existing CAM variants in both qualitative visualization and quantitative evaluation, providing richer, context-aware, and input-adaptive visual explanations.

Conclusion: Fusion-CAM successfully bridges the gap between gradient-based and region-based CAM methods, offering a robust and flexible tool for interpreting deep neural networks that combines the strengths of both approaches while mitigating their individual weaknesses.
...
<details>
  <summary>Details</summary>
Motivation: Monitoring fish health is crucial for early disease detection, animal welfare, and sustainable aquaculture. Locomotion activities can indicate physiological and pathological conditions.

Method: Uses YOLOv11 detector in a tracking-by-detection framework with multi-frame extensions to improve accuracy. Evaluated on annotated dataset of Sulawesi ricefish in home-aquarium setup.

Result: System reliably measures swimming direction and speed for fish health monitoring. Dataset will be made publicly available.

Conclusion: The proposed computer vision system effectively estimates fish locomotion from videos, enabling automated health monitoring in aquaculture settings.
...
TL;DR: Selective Repulsive Knowledge Distillation enables extreme compression of fetal ultrasound AI models (304M→11.4M parameters) for deployment on point-of-care devices, surpassing teacher performance while achieving real-time inference.


<details>
  <summary>Details</summary>
Motivation: Current fetal ultrasound foundation models are too large (300M+ parameters) for deployment on point-of-care devices in low-resource settings, and standard knowledge distillation fails under extreme capacity gaps (~26x).

Method: Selective Repulsive Knowledge Distillation decomposes contrastive KD into diagonal and off-diagonal components: preserves matched pair alignment while making off-diagonal weights negative to repel student from teacher's inter-class confusions, forcing discovery of architecturally native features.
...
TL;DR: RelaxFlow is a training-free dual-branch framework for text-driven amodal 3D generation that handles semantic ambiguity under occlusion by decoupling control granularities between observed regions and unseen regions.


<details>
  <summary>Details</summary>
Motivation: Image-to-3D generation faces inherent semantic ambiguity under occlusion where partial observation alone is insufficient to determine object category. There's a need for text-driven amodal 3D generation where text prompts steer completion of unseen regions while strictly preserving input observation.

Method: Proposes RelaxFlow, a training-free dual-branch framework with Multi-Prior Consensus Module and Relaxation Mechanism. It decouples control granularity: rigid control for observation versus relaxed structural control for prompts. Theoretically, the relaxation applies a low-pass filter on the generative vector field to suppress high-frequency instance details and isolate geometric structure.

Result: Introduced two diagnostic benchmarks (ExtremeOcc-3D and AmbiSem-3D). Extensive experiments show RelaxFlow successfully steers generation of unseen regions to match prompt intent without compromising visual fidelity.

Conclusion: RelaxFlow effectively addresses semantic ambiguity in occluded 3D generation by separating control granularities, enabling text-driven completion of unseen regions while preserving observed regions, with theoretical justification for the relaxation mechanism.
...
TL;DR: SAIL proposes a weakly-supervised dense video captioning method with semantically-aware masks via cross-modal alignment and LLM-based caption augmentation to improve event localization and description without temporal boundary supervision.


<details>
  <summary>Details</summary>
Motivation: Existing weakly-supervised methods generate simplistic, uniformly distributed masks without considering semantic relationships to events, and rely solely on sparse ground-truth captions, leading to suboptimal performance.

Method: SAIL constructs semantically-aware masks through cross-modal alignment with similarity-aware training, and introduces LLM-based caption augmentation with synthetic captions via inter-mask mechanism for additional alignment signals.
...
Conclusion: SAIL effectively addresses limitations of prior weakly-supervised methods by incorporating semantic awareness and handling sparse annotations through cross-modal alignment and synthetic caption augmentation.
...
Method: Proposes CompACT, a discrete tokenizer that compresses each observation into as few as 8 tokens, drastically reducing computational cost while preserving essential information for planning.

Result: An action-conditioned world model using CompACT achieves competitive planning performance with orders-of-magnitude faster planning compared to conventional approaches.

Conclusion: CompACT offers a practical step toward real-world deployment of world models by enabling efficient, real-time planning through extreme observation compression.
...
Result: Experimental results demonstrate that NaiLIA outperforms standard methods. The authors constructed a benchmark with 10,625 images collected from diverse cultural backgrounds, annotated with long and dense intent descriptions by over 200 annotators.

Conclusion: NaiLIA effectively addresses the challenge of retrieving nail design images based on complex user intent descriptions and color palette queries, providing a robust solution for this specialized multimodal retrieval task.
...
TL;DR: RealWonder is the first real-time system for action-conditioned video generation from a single image using physics simulation as an intermediate bridge between actions and video generation.
...
Method: Uses physics simulation as a bridge: translates continuous actions through physics simulation into visual representations (optical flow and RGB) that video models can process. Integrates 3D reconstruction from single images, physics simulation, and a distilled video generator requiring only 4 diffusion steps.
...
Conclusion: RealWonder opens new opportunities to apply video models in immersive experiences, AR/VR, and robot learning. Code and model weights are publicly available.
...
TL;DR: LSP scheduler accelerates diffusion language model inference by 3.4x through contiguous prefix commitment instead of scattered acceptance, improving KV cache efficiency and reducing token flips.


<details>
  <summary>Details</summary>
Motivation: Current diffusion language models suffer from slow inference due to suboptimal decoding schedulers that use scattered acceptance, which fragments KV cache, destroys memory locality, and forces costly repairs across unstable token boundaries.

Method: LSP scheduler is a training-free, model-agnostic inference paradigm based on monolithic prefix absorption. It evaluates token stability via single forward pass, identifies contiguous left-aligned stable predictions, and snaps boundaries to natural linguistic/structural delimiters before atomic commitment.
...
Conclusion: By restructuring commitment topology from scattered to contiguous prefix-first, LSP bridges the gap between theoretical parallelism of DLMs and practical hardware efficiency through systematic KV cache optimization and algorithmic stability preservation.
...
<details>
  <summary>Details</summary>
Motivation: Current SOT methods face trade-offs: segmentation-based trackers with distractor-aware memory are computationally heavy for edge devices, while lightweight trackers are prone to drift when distractors appear. Need a solution that maintains robustness under occlusion/distractors while achieving real-time performance on resource-constrained hardware.

Method: Proposes EdgeDAM with two key strategies: 1) Dual-Buffer Distractor-Aware Memory (DAM) with Recent-Aware Memory for consistent target hypotheses and Distractor-Resolving Memory to store hard negatives; 2) Confidence-Driven Switching with Held-Box Stabilization that adaptively activates detection/re-identification during occlusion while freezing estimates to suppress distractor contamination.

Result: Achieves 88.2% accuracy on DiDi dataset (distractor-focused benchmark) and maintains 25 FPS on iPhone 15, demonstrating improved robustness under occlusion and fast motion while sustaining real-time performance on mobile devices.

Conclusion: EdgeDAM successfully bridges the gap between robustness and efficiency for edge-based SOT, providing a practical solution for real-time tracking under challenging conditions like occlusion and distractors while maintaining mobile-friendly performance.
...
<details>
  <summary>Details</summary>
Motivation: Hallucinations in VLMs are costly to detect after generation; early prediction could enable timely intervention and improve safety/efficiency.

Method: Probe three internal representation families across 8 VLMs: visual-only features, vision-token representations, and query-token representations before generation.

Result: Strong hallucination detection without decoding (up to 0.93 AUROC); late query-token states most predictive for most models, but optimal layer/modality varies by architecture.

Conclusion: Hallucination risk is detectable pre-generation, enabling early abstention, selective routing, and adaptive decoding for safer, more efficient VLMs.
...
TL;DR: NeRF-based 3D reconstruction from sparse longwave infrared hyperspectral images enables gas plume detection with fewer training images and good performance.


<details>
  <summary>Details</summary>
Motivation: Hyperspectral images (HSI) are valuable for material detection and gas plume analysis, but often only a few images are available. Combining information from multiple images into a cohesive 3D representation could enhance scene analysis by providing better geometry and spectral context.

Method: Built on Mip-NeRF architecture, combining state-of-the-art methods for hyperspectral NeRFs and sparse-view NeRFs with a novel adaptive weighted MSE loss. Uses synthetic multi-view LWIR HSI dataset generated with DIRSIG software.

Result: Method requires ~50% fewer training images than standard Mip-NeRF, achieves average PSNR of 39.8 dB with as few as 30 training images. Gas plume detection using adaptive coherence estimator achieves average AUC of 0.821 compared to ground-truth masks.

Conclusion: NeRFs can successfully create 3D scene reconstructions from LWIR hyperspectral images and enable downstream analysis tasks like gas plume detection with reduced data requirements.
...
TL;DR: MM-Lifelong dataset addresses gap in natural, unscripted video understanding with 181.1 hours across Day/Week/Month scales, revealing memory bottlenecks in current methods and proposing Recursive Multimodal Agent (ReMA) with dynamic memory management.
...
Method: Introduce MM-Lifelong dataset with 181.1 hours structured across Day, Week, and Month scales; propose Recursive Multimodal Agent (ReMA) with dynamic memory management to iteratively update recursive belief states; create dataset splits to isolate temporal and domain biases.

Result: Identified two critical failure modes: end-to-end MLLMs suffer from Working Memory Bottleneck due to context saturation, while agentic baselines experience Global Localization Collapse on sparse month-long timelines; ReMA significantly outperforms existing methods.

Conclusion: MM-Lifelong provides rigorous foundation for future research in supervised learning and out-of-distribution generalization, addressing limitations of current video understanding paradigms through structured temporal scaling and memory-aware architectures.
...
TL;DR: CalibAtt accelerates video diffusion models by skipping negligible attention connections identified through offline calibration, achieving up to 1.58× speedup without training.


<details>
  <summary>Details</summary>
Motivation: Current diffusion models for video generation are slow due to spatiotemporal attention bottlenecks in large transformer backbones. Many attention connections have negligible scores that repeat across queries and inputs, suggesting computation can be skipped.

Method: CalibAtt performs offline calibration to identify stable block-level sparsity and repetition patterns across inputs. It compiles these patterns into optimized attention operations per layer, head, and diffusion timestep. At inference, selected connections are computed densely while unselected ones are skipped efficiently.

Result: Achieves up to 1.58× end-to-end speedup on Wan 2.1 14B, Mochi 1, and few-step distilled models at various resolutions. Outperforms existing training-free methods while maintaining video generation quality and text-video alignment.

Conclusion: CalibAtt effectively accelerates video diffusion models by exploiting stable sparsity patterns in attention, providing significant speed improvements without compromising quality or requiring retraining.
...
TL;DR: FaceCam generates portrait videos with customizable camera trajectories from monocular input, using scale-aware camera representations to avoid distortions without 3D priors.


<details>
  <summary>Details</summary>
Motivation: Existing camera control methods for portrait videos suffer from geometric distortions and visual artifacts due to scale-ambiguous camera representations or 3D reconstruction errors.

Method: Proposes face-tailored scale-aware camera representation, trains video generation model on multi-view studio captures and monocular videos, and uses synthetic camera motion and multi-shot stitching strategies for training.

Result: Superior performance on Ava-256 dataset and diverse in-the-wild videos in camera controllability, visual quality, identity and motion preservation.

Conclusion: FaceCam effectively generates high-quality portrait videos with customizable camera trajectories while avoiding distortions common in existing approaches.
...
TL;DR: A transformer-based multi-view inpainting method for real-time 3D streaming that completes missing textures in rendered images as a post-processing step, achieving best quality-speed trade-off.


<details>
  <summary>Details</summary>
Motivation: Limited camera views in real-time 3D streaming cause missing information and incomplete surfaces in rendered images, while existing hole-filling heuristics lead to inconsistencies and visual artifacts.

Method: Proposes a standalone, application-targeted inpainting method using a multi-view aware transformer-based network with spatio-temporal embeddings for cross-frame consistency, resolution-independent design for different camera setups, and adaptive patch selection for real-time performance.
...
Conclusion: The proposed method effectively addresses texture completion in 3D streaming as a post-processing step, offering a practical solution compatible with any calibrated multi-camera system while maintaining real-time performance.
